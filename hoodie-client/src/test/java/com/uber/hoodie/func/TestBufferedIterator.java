...
public class TestBufferedIterator {
  ...
  public void testRecordReading() throws IOException, ExecutionException, InterruptedException {
    ...
    final List<HoodieRecord> hoodieRecords = hoodieTestDataGenerator
        .generateInserts(commitTime, numRecords);
    final BufferedIterator bufferedIterator = new BufferedIterator(hoodieRecords.iterator(),
        FileUtils.ONE_KB, HoodieTestDataGenerator.avroSchema);
  ...
  public void testMemoryLimitForBuffering() throws IOException, InterruptedException {
    ...
    final BufferedIterator bufferedIterator = new BufferedIterator(hoodieRecords.iterator(),
        memoryLimitInBytes, HoodieTestDataGenerator.avroSchema);
    Future<Boolean> result = recordReader.submit(() -> {
      bufferedIterator.startBuffering();
      return true;
    });
  ...
  public void testException() throws IOException, InterruptedException {
    ...
    BufferedIterator bufferedIterator1 = new BufferedIterator(hoodieRecords.iterator(),
        memoryLimitInBytes, HoodieTestDataGenerator.avroSchema);
    Future<Boolean> result = recordReader.submit(() -> {
    ...
    BufferedIterator bufferedIterator2 = new BufferedIterator(mockHoodieRecordsIterator,
        memoryLimitInBytes, HoodieTestDataGenerator.avroSchema);
