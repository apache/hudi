...
/**
  * Timestamp {@link org.apache.flink.formats.parquet.vector.reader.ColumnReader} that supports INT64
  * 8 bytes, TIMESTAMP_MILLIS is the deprecated ConvertedType counterpart of a TIMESTAMP logical type
...
public class Int64TimestampColumnReader extends AbstractColumnReader<WritableTimestampVector> {
    ...
    public Int64TimestampColumnReader(
            boolean utcTimestamp, ColumnDescriptor descriptor, PageReader pageReader, int precision)
            throws IOException {
    ...
    protected void readBatch(int rowId, int num, WritableTimestampVector column) {
        for (int i = 0; i < num; i++) {
            if (runLenDecoder.readInteger() == maxDefLevel) {
                ...
                column.setTimestamp(
                        rowId + i, int64ToTimestamp(utcTimestamp, buffer.getLong(), chronoUnit));
            } else {
    ...
    protected void readBatchFromDictionaryIds(
        ...
        for (int i = rowId; i < rowId + num; ++i) {
            if (!column.isNullAt(i)) {
                column.setTimestamp(
                        i,
                        decodeInt64ToTimestamp(utcTimestamp, dictionary, dictionaryIds.getInt(i), chronoUnit));
            }
        }
    }
    ...
    private static TimestampData int64ToTimestamp(
            boolean utcTimestamp, long interval, ChronoUnit unit) {
