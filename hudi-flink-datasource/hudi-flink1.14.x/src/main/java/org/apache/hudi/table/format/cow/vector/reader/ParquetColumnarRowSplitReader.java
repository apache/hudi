...
/**
  ...
  * org.apache.flink.formats.parquet.vector.ParquetColumnarRowSplitReader} because it is package
  * scope.
  */
public class ParquetColumnarRowSplitReader implements Closeable {
    ...
    public ParquetColumnarRowSplitReader(
        ...
        int[] requestedIndices =
                IntStream.range(0, types.length).filter(i -> types[i] != null).toArray();
        Type[] readTypes = Arrays.stream(requestedIndices).mapToObj(i -> types[i]).toArray(Type[]::new);

        this.requestedTypes =
                Arrays.stream(requestedIndices).mapToObj(i -> selectedTypes[i]).toArray(LogicalType[]::new);
        ...
        ColumnVector[] columnVectors =
                patchedVector(selectedFieldNames.length, createReadableVectors(), requestedIndices);
        this.columnarBatch = generator.generate(columnVectors);
        this.row = new ColumnarRowData(columnarBatch);
    }

    /**
      * Patches the given vectors with nulls. The vector position that is not requested (or read from
      * file) is patched as null.
      *
    ...
    private WritableColumnVector[] createWritableVectors() {
        ...
        for (int i = 0; i < requestedTypes.length; i++) {
            try {
                columns[i] =
                        createWritableColumnVector(batchSize, requestedTypes[i], types.get(i), descriptors);
    ...
    private ColumnVector[] createReadableVectors() {
        ...
        for (int i = 0; i < writableVectors.length; i++) {
            vectors[i] =
                    requestedTypes[i].getTypeRoot() == LogicalTypeRoot.DECIMAL
    ...
    /**
      * Checks if there is at least one row left in the batch to return. If no more row are available,
      * it reads another batch of rows.
      *
    ...
    private void readNextRowGroup() throws IOException {
        ...
        for (int i = 0; i < types.size(); ++i) {
            columnReaders[i] =
                    createColumnReader(utcTimestamp, requestedTypes[i], types.get(i), columns, pages);
        }
