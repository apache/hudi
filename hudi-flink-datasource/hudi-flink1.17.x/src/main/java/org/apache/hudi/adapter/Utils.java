...
public class Utils {
    public static FactoryUtil.DefaultDynamicTableContext getTableContext(
            ObjectIdentifier tablePath, ResolvedCatalogTable catalogTable, ReadableConfig conf) {
    ...
    public static BinaryExternalSorter getBinaryExternalSorter(
            ...
            Configuration conf) {
        return new BinaryExternalSorter(
                owner,
                memoryManager,
                reservedMemorySize,
                ioManager,
                ...
                (int) conf.get(ExecutionConfigOptions.TABLE_EXEC_SPILL_COMPRESSION_BLOCK_SIZE).getBytes(),
                conf.get(ExecutionConfigOptions.TABLE_EXEC_SORT_ASYNC_MERGE_ENABLED));
    }

    public static InternalSchema applyTableChange(
            InternalSchema oldSchema, List changes, Function<LogicalType, Type> convertFunc) {
    ...
    private static InternalSchema applyTableChange(
            InternalSchema oldSchema, TableChange change, Function<LogicalType, Type> convertFunc) {
        ...
        if (change instanceof TableChange.AddColumn) {
            if (((TableChange.AddColumn) change).getColumn().isPhysical()) {
                ...
                Pair<
                                org.apache.hudi.internal.schema.action.TableChange.ColumnPositionChange
                                        .ColumnPositionType,
                                String>
                        colPositionPair = parseColumnPosition(add.getPosition());
            ...
            Pair<
                            org.apache.hudi.internal.schema.action.TableChange.ColumnPositionChange
                                    .ColumnPositionType,
                            String>
                    colPositionPair = parseColumnPosition(modify.getNewPosition());
            ...
            throw new HoodieNotSupportedException(
                    change.getClass().getSimpleName() + " is not supported.");
        }
    }
    ...
    private static Pair<
                    org.apache.hudi.internal.schema.action.TableChange.ColumnPositionChange
                            .ColumnPositionType,
                    String>
            parseColumnPosition(TableChange.ColumnPosition colPosition) {
        org.apache.hudi.internal.schema.action.TableChange.ColumnPositionChange.ColumnPositionType
                positionType;
        String position = "";
