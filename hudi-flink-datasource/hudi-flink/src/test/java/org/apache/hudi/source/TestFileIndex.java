...
public class TestFileIndex {
  ...
  void testFileListingUsingMetadata(boolean hiveStylePartitioning) throws Exception {
    ...
    List<HoodieFileStatus> fielInfoList = fileIndex.getFilesInPartitions();
    assertThat(fielInfoList.size(), is(4));
    assertTrue(fielInfoList.stream().allMatch(fileInfo ->
        fileInfo.getLocation().toString().endsWith(HoodieFileFormat.PARQUET.getFileExtension())));
  }
  ...
  void testFileListingUsingMetadataNonPartitionedTable() throws Exception {
    ...
    List<HoodieFileStatus> fileInfoList = fileIndex.getFilesInPartitions();
    assertThat(fileInfoList.size(), is(1));
    assertTrue(fileInfoList.get(0).getLocation().toString()
  ...
  void testFileListingWithDataSkipping() throws Exception {
    ...
    List<HoodieFileStatus> files = fileIndex.getFilesInPartitions();
