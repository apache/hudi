...
class TestRowDataToAvroConverters {
    ...
    void testRowDataToAvroStringToRowDataWithLocalTimezone() throws JsonProcessingException {
        ...
        Object rowData =
                jsonToRowDataConverter.convert(
                        new ObjectMapper()
                                .readTree("{\"timestamp_from_local\":\"" + timestampFromLocal + "\"}"));

        RowType rowType =
                (RowType)
                        DataTypes.ROW(DataTypes.FIELD("f_timestamp", DataTypes.TIMESTAMP(3))).getLogicalType();
        ...
        Assertions.assertEquals(
                ...
                formatter.format(
                        LocalDateTime.ofInstant(
                                Instant.ofEpochMilli((Long) avroRecord.get(0)), ZoneId.systemDefault())));
    }
    ...
    void testRowDataToAvroStringToRowDataWithUtcTimezone() throws JsonProcessingException {
        ...
        Object rowData =
                jsonToRowDataConverter.convert(
                        new ObjectMapper()
                                .readTree("{\"timestamp_from_utc_0\":\"" + timestampFromUtc0 + "\"}"));
        ...
        Assertions.assertEquals(
                ...
                formatter.format(
                        LocalDateTime.ofInstant(
                                Instant.ofEpochMilli((Long) avroRecord.get(0)), ZoneId.of("UTC"))));
        Assertions.assertEquals(
                "2021-03-30 08:44:29",
                formatter.format(
                        LocalDateTime.ofInstant(
                                Instant.ofEpochMilli((Long) avroRecord.get(0)), ZoneId.of("UTC+1"))));
        Assertions.assertEquals(
                "2021-03-30 15:44:29",
                formatter.format(
                        LocalDateTime.ofInstant(
                                Instant.ofEpochMilli((Long) avroRecord.get(0)), ZoneId.of("Asia/Shanghai"))));
    }
}
