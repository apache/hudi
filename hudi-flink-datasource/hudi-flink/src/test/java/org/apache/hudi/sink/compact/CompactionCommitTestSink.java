...
public class CompactionCommitTestSink extends CompactionCommitSink {
    ...
    public void invoke(CompactionCommitEvent event, Context context) throws Exception {
        ...
        boolean compactCommitted =
                instants.stream()
                        .anyMatch(i -> i.getTimestamp().equals(event.getInstant()) && i.isCompleted());
        if (compactCommitted && getRuntimeContext().getAttemptNumber() == 0) {
            ...
            HoodieTimelineArchiver archiver =
                    new HoodieTimelineArchiver(
                            this.writeClient.getConfig(), this.writeClient.getHoodieTable());
