...
public class TestOptionsInference {
    ...
    void testSetupClientId() throws Exception {
        ...
        assertThat(
                "Explicit client id has higher priority",
        ...
        for (int i = 0; i < 3; i++) {
            ...
            try (ClientIds clientIds = ClientIds.builder().conf(conf).build()) {
                ...
                assertThat(
                        "The client id should auto inc to " + expectedId,
                        conf.getString(FlinkOptions.WRITE_CLIENT_ID),
                        is(expectedId));
            }
        }
        ...
        Thread.sleep(1000);
        conf = getConf();
        try (ClientIds clientIds =
                ClientIds.builder()
                        .conf(conf)
                        .heartbeatIntervalInMs(10) // max 10 milliseconds tolerable heartbeat timeout
                        .numTolerableHeartbeatMisses(1)
                        .build()) {
            String nextId = clientIds.nextId(conf);
            assertThat("The inactive client id should be reused", nextId, is(""));
        }
    }
