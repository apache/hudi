...
public class TestStreamReadMonitoringFunction {
    ...
    public void testConsumeFromLatestCommit() throws Exception {
        ...
        try (AbstractStreamOperatorTestHarness<MergeOnReadInputSplit> harness =
            ...
            assertTrue(
                    latch.await(WAIT_TIME_MILLIS, TimeUnit.MILLISECONDS), "Should finish splits generation");
            assertThat(
                    "Should produce the expected splits",
                    sourceContext.getPartitionPaths(),
                    is("par1,par2,par3,par4"));

            assertTrue(
                    sourceContext.splits.stream().allMatch(split -> split.getInstantRange().isPresent()),
                    "All the instants should have range limit");
            String latestCommit = TestUtils.getLastCompleteInstant(tempFile.getAbsolutePath());
            assertTrue(
                    sourceContext.splits.stream()
                            .allMatch(split -> split.getLatestCommit().equals(latestCommit)),
    ...
    public void testConsumeFromSpecifiedCommit() throws Exception {
        ...
        try (AbstractStreamOperatorTestHarness<MergeOnReadInputSplit> harness =
            ...
            assertTrue(
                    sourceContext.splits.stream()
                            .allMatch(split -> split.getLatestCommit().equals(specifiedCommit)),
    ...
    public void testConsumeFromEarliestCommit() throws Exception {
        ...
        try (AbstractStreamOperatorTestHarness<MergeOnReadInputSplit> harness =
            ...
            assertThat(
                    ...
                    is("par1,par2,par3,par4"));
            assertTrue(
                    sourceContext.splits.stream().noneMatch(split -> split.getInstantRange().isPresent()),
    ...
    public void testConsumingHollowInstants() throws Exception {
        ...
        try (AbstractStreamOperatorTestHarness<MergeOnReadInputSplit> harness =
            ...
            assertTrue(
                    latch.await(WAIT_TIME_MILLIS, TimeUnit.MILLISECONDS), "Should finish splits generation");
            assertThat(
                    "Should produce the expected splits", sourceContext.getPartitionPaths(), is("par1"));
            assertTrue(
                    sourceContext.splits.stream().noneMatch(split -> split.getInstantRange().isPresent()),
                    "No instants should have range limit");
            assertTrue(
                    sourceContext.splits.stream().anyMatch(split -> split.getLatestCommit().equals(c4)),
            ...
            assertTrue(
                    sourceContext.splits.stream()
                            .allMatch(split -> isPointInstantRange(split.getInstantRange().get(), c2)),
                    "All the splits should have point instant range");
            assertTrue(
                    sourceContext.splits.stream().anyMatch(split -> split.getLatestCommit().equals(c2)),
                    "At least one input split's latest commit time should be equal to the specified instant time.");
            ...
            assertTrue(
                    sourceContext.splits.stream()
                            .allMatch(split -> isPointInstantRange(split.getInstantRange().get(), c3)),
                    "All the splits should have point instant range");
            assertTrue(
                    sourceContext.splits.stream().anyMatch(split -> split.getLatestCommit().equals(c3)),
                    "At least one input split's latest commit time should be equal to the specified instant time.");
    ...
    /**
      ...
      * </ul>
      *
      * This test ensured that the state is saved properly when these 3 methods are called in the order
      * listed above.
      */
    @Test
    ...
    private AbstractStreamOperatorTestHarness<MergeOnReadInputSplit> createHarness(
        ...
        StreamSource<MergeOnReadInputSplit, StreamReadMonitoringFunction> streamSource =
                new StreamSource<>(function);
        return new AbstractStreamOperatorTestHarness<>(streamSource, 1, 1, 0);
    }

    private void runAsync(
            CollectingSourceContext sourceContext, StreamReadMonitoringFunction function) {
        Thread task =
                new Thread(
                        () -> {
                            try {
