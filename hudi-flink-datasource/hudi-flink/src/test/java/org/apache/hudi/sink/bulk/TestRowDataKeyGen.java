...
public class TestRowDataKeyGen {
    ...
    void testSimpleKeyAndPartition() {
        ...
        assertThat(
                keyGen2.getPartitionPath(rowData2),
                is(String.format("partition=%s", DEFAULT_PARTITION_PATH)));
        assertThat(
                keyGen2.getPartitionPath(rowData3),
                is(String.format("partition=%s", DEFAULT_PARTITION_PATH)));
    }
    ...
    void testComplexKeyAndPartition() {
        ...
        assertThrows(HoodieKeyException.class, () -> keyGen1.getRecordKey(rowData2));
        assertThat(
                keyGen1.getPartitionPath(rowData2),
                is(String.format("%s/%s", DEFAULT_PARTITION_PATH, DEFAULT_PARTITION_PATH)));
        // empty record key and partition path
        final RowData rowData3 =
                insertRow(
                        StringData.fromString(""),
        ...
        assertThrows(HoodieKeyException.class, () -> keyGen1.getRecordKey(rowData3));
        assertThat(
                keyGen1.getPartitionPath(rowData3),
                is(String.format("%s/1970-01-01T00:00:00.001", DEFAULT_PARTITION_PATH)));

        // hive style partitioning
        conf.set(FlinkOptions.HIVE_STYLE_PARTITIONING, true);
        final RowDataKeyGen keyGen2 = RowDataKeyGen.instance(conf, TestConfigurations.ROW_TYPE);
        assertThat(
                keyGen2.getPartitionPath(rowData1),
                is(String.format("partition=%s/ts=%s", "par1", "1970-01-01T00:00:00.001")));
        assertThat(
                keyGen2.getPartitionPath(rowData2),
                is(String.format("partition=%s/ts=%s", DEFAULT_PARTITION_PATH, DEFAULT_PARTITION_PATH)));
        assertThat(
                keyGen2.getPartitionPath(rowData3),
                is(String.format("partition=%s/ts=%s", DEFAULT_PARTITION_PATH, "1970-01-01T00:00:00.001")));
    }
    ...
    @ParameterizedTest
    @ValueSource(
            strings = {FlinkOptions.PARTITION_FORMAT_DASHED_DAY, FlinkOptions.PARTITION_FORMAT_DAY})
    void testDateBasedKeyGenerator(String partitionFormat) {
        ...
        Configuration conf =
                TestConfigurations.getDefaultConf("path1", TestConfigurations.ROW_DATA_TYPE_DATE);
        ...
        // null record key and partition path
        final RowData rowData2 =
                insertRow(TestConfigurations.ROW_TYPE_DATE, null, StringData.fromString("Danny"), 23, null);
        assertThrows(HoodieKeyException.class, () -> keyGen1.getRecordKey(rowData2));
    ...
    void testRecordKeyContainsTimestamp() {
        ...
        assertThat(
                keyGen1.getRecordKey(rowData1), is("uuid:id1,ts:" + ts.toLocalDateTime().toString()));
