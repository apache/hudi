...
public class TestStreamReadOperator {
    ...
    static {
        EXPECTED.put(
                "par1",
                "+I[id1, Danny, 23, 1970-01-01T00:00:00.001, par1], +I[id2, Stephen, 33, 1970-01-01T00:00:00.002, par1]");
        EXPECTED.put(
                "par2",
                "+I[id3, Julian, 53, 1970-01-01T00:00:00.003, par2], +I[id4, Fabian, 31, 1970-01-01T00:00:00.004, par2]");
        EXPECTED.put(
                "par3",
                "+I[id5, Sophia, 18, 1970-01-01T00:00:00.005, par3], +I[id6, Emma, 20, 1970-01-01T00:00:00.006, par3]");
        EXPECTED.put(
                "par4",
                "+I[id7, Bob, 44, 1970-01-01T00:00:00.007, par4], +I[id8, Han, 56, 1970-01-01T00:00:00.008, par4]");
    }
    ...
    void testWriteRecords() throws Exception {
        ...
        try (OneInputStreamOperatorTestHarness<MergeOnReadInputSplit, RowData> harness =
                createReader()) {
            harness.setup();
            harness.open();
    ...
    public void testCheckpoint() throws Exception {
        ...
        try (OneInputStreamOperatorTestHarness<MergeOnReadInputSplit, RowData> harness =
            ...
            assertThat(
                    TestData.rowDataToString(harness.extractOutputValues()),
    ...
    private static String getSplitExpected(
            List<MergeOnReadInputSplit> splits, Map<String, String> expected) {
    ...
    private List<MergeOnReadInputSplit> generateSplits(StreamReadMonitoringFunction func)
            throws Exception {
    ...
    private OneInputStreamOperatorTestHarness<MergeOnReadInputSplit, RowData> createReader()
            throws Exception {
        final String basePath = tempFile.getAbsolutePath();
        final HoodieTableMetaClient metaClient =
                StreamerUtil.createMetaClient(
        ...
        final MergeOnReadTableState hoodieTableState =
                new MergeOnReadTableState(
                        rowType,
        ...
        MergeOnReadInputFormat inputFormat =
                MergeOnReadInputFormat.builder()
                        ...
                        .defaultPartName(PARTITION_DEFAULT_NAME.defaultValue())
                        .limit(1000L)
                        .emitDelete(true)
                        .build();
