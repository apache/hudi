...
public class ITTestAppendWriteFunctionWithBufferSort extends TestWriteBase {
  ...
  public void before(@TempDir File tempDir) throws Exception {
    ...
    this.conf.setBoolean(FlinkOptions.WRITE_BUFFER_SORT_ENABLED, true);
  ...
  @Test
  public void testBufferFlushOnSize() throws Exception {
  ...
  public void testSortedResult() throws Exception {
    ...
    List<String> expected = Arrays.asList(
            convert(createRowData("uuid1", "Alice", 25, "1970-01-01 00:00:01.124", "p1")),
            convert(createRowData("uuid1", "Bob", 21, "1970-01-01 00:00:31.124", "p1")),
            convert(createRowData("uuid1", "Bob", 30, "1970-01-01 00:00:01.123", "p1"))
    );
    ...
    List<String> filteredResult =
            result.stream()
                    .map(record -> GenericRowData.of(record.get(5), record.get(6), record.get(7), TimestampData.fromEpochMillis((long) record.get(8)), record.get(9)))
                    .map(record -> convert(record)).collect(Collectors.toList());
  ...
  private static String convert(GenericRowData record) {
    List<String> fields = new ArrayList<>();
    for (int i = 0; i < record.getArity(); i++) {
      fields.add(record.getField(i).toString());
    }
