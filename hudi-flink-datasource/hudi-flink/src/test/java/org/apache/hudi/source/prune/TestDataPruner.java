...
class TestDataPruner {
    ...
    void testConvertColumnStats(DataType dataType, Object minValue, Object maxValue) {
        ...
        Map<String, ColumnStats> stats1 =
                DataPruner.convertColumnStats(
                        getIndexRow(indexRowDataType, minValue, maxValue), getDataFields(rowDataType));
    ...
    static Stream<Arguments> testTypes() {
        return Stream.of(arguments(DataTypes.INT(), 1, 5), arguments(DataTypes.BIGINT(), 3L, 4L));
    }
    ...
    private RowData getIndexRow(DataType indexRowDataType, Object minValue, Object maxValue) {
        return TestData.insertRow(
                (RowType) indexRowDataType.getLogicalType(),
    ...
    private DataType getDataRowDataType(DataType dataType) {
        return DataTypes.ROW(DataTypes.FIELD("field", dataType)).notNull();
    }

    private RowType.RowField[] getDataFields(DataType rowDataType) {
        return new RowType.RowField[] {((RowType) rowDataType.getLogicalType()).getFields().get(0)};
    }
}
