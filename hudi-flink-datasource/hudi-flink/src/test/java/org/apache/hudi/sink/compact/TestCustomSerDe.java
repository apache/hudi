...
/**
  * Tests for custom SerDe of non-primitive avro types when using Avro versions > 1.10.0. The avro
  * version used by hudi-flink module is 1.10.0, these tests are placed here so that avro 1.10.0 is
  * used, allowing the error caused by anonymous classes to be thrown.
  */
public class TestCustomSerDe extends HoodieCommonTestHarness {
    ...
    public void testBitCaskDiskMapPutDecimal(boolean isCompressionEnabled) throws IOException {
        BitCaskDiskMap<String, HoodieRecord> bitCaskDiskMap =
    ...
    private static HoodieRecord createAvroRecordWithDecimalOrderingField() {
        Schema precombineFieldSchema =
                LogicalTypes.decimal(20, 0)
        ...
        // nullifying the record attribute in EventTimeAvroPayload here as it is not required in the
        // test
