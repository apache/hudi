...
public class ITTestDataStreamWrite extends TestLogger {
  ...
  public void testStreamWriteUsingFastMode(String tableType) throws Exception {
    Configuration conf = TestConfigurations.getDefaultConf(tempFile.toURI().toString());
    conf.setString(FlinkOptions.TABLE_TYPE, tableType);
    conf.setString(FlinkOptions.WRITE_FAST_MODE.key(), "true");
  ...
  private void writeAndCheckExpected(
    ...
    if (useFastMode) {
      DataStream<HoodieFlinkInternalRow> rowDataStream = Pipelines.bootstrapRowData(conf, rowType, rowDataInfo, dataStream, false, false);
  ...
  public void testColumnDroppingIsNotAllowed() throws Exception {
    ...
    try {
      writeAndCheckExpected(conf, Option.empty(), "failing job", 1, false, false, Collections.emptyMap());
