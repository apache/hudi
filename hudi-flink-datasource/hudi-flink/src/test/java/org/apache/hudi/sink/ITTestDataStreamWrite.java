...
public class ITTestDataStreamWrite extends TestLogger {
    ...
    static {
        EXPECTED.put(
                "par1",
                Arrays.asList("id1,par1,id1,Danny,23,1000,par1", "id2,par1,id2,Stephen,33,2000,par1"));
        EXPECTED.put(
                "par2",
                Arrays.asList("id3,par2,id3,Julian,53,3000,par2", "id4,par2,id4,Fabian,31,4000,par2"));
        EXPECTED.put(
                "par3",
                Arrays.asList("id5,par3,id5,Sophia,18,5000,par3", "id6,par3,id6,Emma,20,6000,par3"));
        EXPECTED.put(
                "par4", Arrays.asList("id7,par4,id7,Bob,44,7000,par4", "id8,par4,id8,Han,56,8000,par4"));

        EXPECTED_TRANSFORMER.put(
                "par1",
                Arrays.asList("id1,par1,id1,Danny,24,1000,par1", "id2,par1,id2,Stephen,34,2000,par1"));
        EXPECTED_TRANSFORMER.put(
                "par2",
                Arrays.asList("id3,par2,id3,Julian,54,3000,par2", "id4,par2,id4,Fabian,32,4000,par2"));
        EXPECTED_TRANSFORMER.put(
                "par3",
                Arrays.asList("id5,par3,id5,Sophia,19,5000,par3", "id6,par3,id6,Emma,21,6000,par3"));
        EXPECTED_TRANSFORMER.put(
                "par4", Arrays.asList("id7,par4,id7,Bob,45,7000,par4", "id8,par4,id8,Han,57,8000,par4"));

        EXPECTED_CHAINED_TRANSFORMER.put(
                "par1",
                Arrays.asList("id1,par1,id1,Danny,25,1000,par1", "id2,par1,id2,Stephen,35,2000,par1"));
        EXPECTED_CHAINED_TRANSFORMER.put(
                "par2",
                Arrays.asList("id3,par2,id3,Julian,55,3000,par2", "id4,par2,id4,Fabian,33,4000,par2"));
        EXPECTED_CHAINED_TRANSFORMER.put(
                "par3",
                Arrays.asList("id5,par3,id5,Sophia,20,5000,par3", "id6,par3,id6,Emma,22,6000,par3"));
        EXPECTED_CHAINED_TRANSFORMER.put(
                "par4", Arrays.asList("id7,par4,id7,Bob,46,7000,par4", "id8,par4,id8,Han,58,8000,par4"));
    }
                                ...
                                (rowdata) -> {
                                    if (rowdata instanceof GenericRowData) {
                                        ...
                                        throw new RuntimeException(
                                                "Unrecognized row type information: " + rowdata.getClass().getSimpleName());
                                    }
                                });
    ...
    public void testWriteCopyOnWriteWithChainedTransformer() throws Exception {
                        ...
                        ds.map(
                                rowData -> {
                                    if (rowData instanceof GenericRowData) {
                                        ...
                                        throw new RuntimeException(
                                                "Unrecognized row type : " + rowData.getClass().getSimpleName());
                                    }
                                });
        ...
        testWriteToHoodie(
                chainedTransformer, "cow_write_with_chained_transformer", EXPECTED_CHAINED_TRANSFORMER);
    }
    ...
    private void testWriteToHoodie(
            Transformer transformer, String jobName, Map<String, List<String>> expected)
            throws Exception {
        testWriteToHoodie(
    ...
    private void testWriteToHoodie(
            Configuration conf, String jobName, int checkpoints, Map<String, List<String>> expected)
            throws Exception {
    ...
    private void testWriteToHoodie(
        ...
        String sourcePath =
                Objects.requireNonNull(
                                Thread.currentThread().getContextClassLoader().getResource("test_source.data"))
                        .toString();
        ...
        if (isMor) {
            ...
            dataStream =
                    execEnv
                            ...
                            .addSource(
                                    new ContinuousFileSource.BoundedSourceFunction(new Path(sourcePath), checkpoints))
    ...
    public void execute(StreamExecutionEnvironment execEnv, boolean isMor, String jobName)
            throws Exception {
        if (isMor) {
    ...
    public void testHoodiePipelineBuilderSource() throws Exception {
        ...
        HoodiePipeline.Builder builder =
                HoodiePipeline.builder("test_source")
    ...
    public void testHoodiePipelineBuilderSink() throws Exception {
        ...
        options.put(FlinkOptions.PATH.key(), tempFile.toURI().toString());
        options.put(
                FlinkOptions.SOURCE_AVRO_SCHEMA_PATH.key(),
                Objects.requireNonNull(
        ...
        HoodiePipeline.Builder builder =
                HoodiePipeline.builder("test_sink")
                        .column("uuid string not null")
                        .column("name string")
                        .column("age int")
                        .column("`ts` timestamp(3)")
    ...
    public void testHoodiePipelineBuilderSourceWithSchemaSet() throws Exception {
        ...
        TableOptionProperties.createProperties(
                testTable.toURI().toString(), HadoopConfigurations.getHadoopConf(catalogConf), options);
    ...
    public void testHoodiePipelineBuilderSinkWithSchemaSet() throws Exception {
        ...
        // sink to hoodie table use low-level sink api.
        HoodiePipeline.Builder builder =
                HoodiePipeline.builder("test_sink").schema(schema).partition("partition").options(options);
    ...
    public void testColumnDroppingIsNotAllowed() throws Exception {
        ...
        throw new AssertionError(
                String.format("Excepted exception %s is not found", MissingSchemaFieldException.class));
    }
}
