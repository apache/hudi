...
public class TestWriteMergeOnReadWithCompact extends TestWriteCopyOnWrite {
    ...
    public void testNonBlockingConcurrencyControlWithPartialUpdatePayload() throws Exception {
        ...
        TestHarness pipeline1 = preparePipeline(conf).consume(dataset1).assertEmptyDataFiles();
        ...
        List<RowData> dataset2 =
                Collections.singletonList(
                        insertRow(
                                ...
                                StringData.fromString("par1")));
        TestHarness pipeline2 = preparePipeline(conf2).consume(dataset2).assertEmptyDataFiles();

        // step to commit the 1st txn
        pipeline1.checkpoint(1).assertNextEvent().checkpointComplete(1);

        // step to commit the 2nd txn
        pipeline2.checkpoint(1).assertNextEvent().checkpointComplete(1);

        // snapshot result is [(id1,Danny,23,2,par1)] after two writers finish to commit
        Map<String, String> tmpSnapshotResult =
                Collections.singletonMap("par1", "[id1,par1,id1,Danny,23,2,par1]");
        ...
        try (HoodieFlinkWriteClient writeClient =
                FlinkWriteClients.createWriteClient(conf, TestUtils.getMockRuntimeContext())) {
        ...
        List<RowData> dataset3 =
                Collections.singletonList(
                        insertRow(
                                StringData.fromString("id3"),
                                StringData.fromString("Julian"),
                                53,
                                TimestampData.fromEpochMillis(4),
                                StringData.fromString("par1")));
        pipeline1.consume(dataset3).checkpoint(2).assertNextEvent().checkpointComplete(2);

        // snapshot read result is [(id1,Danny,23,2,par1), (id3,Julian,53,4,par1)] after three
        // writers finish to commit
        Map<String, String> finalSnapshotResult =
                Collections.singletonMap(
                        "par1", "[id1,par1,id1,Danny,23,2,par1, id3,par1,id3,Julian,53,4,par1]");
        ...
        Map<String, String> readOptimizedResult =
                Collections.singletonMap("par1", "[id1,par1,id1,Danny,23,2,par1]");
    ...
    public void testNonBlockingConcurrencyControlWithInflightInstant() throws Exception {
        ...
        pipeline2.checkpoint(1).assertNextEvent();
        ...
        // snapshot read result is [(id1,Danny,23,1,par1), (id3,Julian,53,4,par1)] after 1st writer
        // and 3rd writer finish to commit
        // and the data of 2nd writer is not included because it is still in inflight state
        Map<String, String> finalSnapshotResult =
                Collections.singletonMap(
                        "par1", "[id1,par1,id1,Danny,23,1,par1, id3,par1,id3,Julian,53,4,par1]");
        pipeline1.checkWrittenData(finalSnapshotResult, 1);
        ...
        Map<String, String> readOptimizedResult =
                Collections.singletonMap("par1", "[id1,par1,id1,Danny,23,1,par1]");
        TestData.checkWrittenData(tempFile, readOptimizedResult, 1);
        pipeline1.end();
        pipeline2.end();
    }
        ...
        List<RowData> dataset2 =
                Collections.singletonList(
                        insertRow(
                                ...
                                TimestampData.fromEpochMillis(2),
                                StringData.fromString("par1")));
        TestHarness pipeline2 = preparePipeline(conf2).consume(dataset2);
        ...
        List<RowData> dataset1 =
                Collections.singletonList(
                        insertRow(
                                ...
                                TimestampData.fromEpochMillis(1),
                                StringData.fromString("par1")));
        TestHarness pipeline1 = preparePipeline(conf1).consume(dataset1);
        ...
        List<RowData> dataset3 =
                Collections.singletonList(
                        insertRow(
                                ...
                                TimestampData.fromEpochMillis(4),
                                StringData.fromString("par1")));
        pipeline2.consume(dataset3).checkpoint(2).assertNextEvent().checkpointComplete(2);
