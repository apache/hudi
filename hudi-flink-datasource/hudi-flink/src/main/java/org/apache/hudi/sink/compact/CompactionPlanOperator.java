...
public class CompactionPlanOperator extends AbstractStreamOperator<CompactionPlanEvent>
    ...
    public void notifyCheckpointComplete(long checkpointId) {
        try {
            ...
            // or is still undergoing. So we use a configured timeout threshold to control the
            // rollback:
            ...
            // make it fail-safe
            LOG.error(
                    "Error while scheduling compaction plan for checkpoint: " + checkpointId, throwable);
        }
    }
    ...
    private void scheduleCompaction(HoodieFlinkTable<?> table, long checkpointId) throws IOException {
        ...
        if (compactionPlan == null
                || (compactionPlan.getOperations() == null)
            ...
            List<CompactionOperation> operations =
                    compactionPlan.getOperations().stream()
                            ...
                            .collect(toList());
            LOG.info(
                    "Execute compaction plan for instant {} as {} file groups",
                    compactionInstantTime,
                    operations.size());
            WriteMarkersFactory.get(table.getConfig().getMarkersType(), table, compactionInstantTime)
                    .deleteMarkerDir(table.getContext(), table.getConfig().getMarkersDeleteParallelism());
            for (CompactionOperation operation : operations) {
                output.collect(
                        new StreamRecord<>(new CompactionPlanEvent(compactionInstantTime, operation)));
            }
        }
    }
