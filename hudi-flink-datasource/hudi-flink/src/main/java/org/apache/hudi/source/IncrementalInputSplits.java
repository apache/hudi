...
/**
  * Utilities to generate incremental input splits {@link MergeOnReadInputSplit}. The input splits
  * are used for streaming and incremental read.
  *
  * <p>How to generate the input splits:
  *
  * <ol>
  *   <li>first fetch all the commit metadata for the incremental instants;
  *   <li>resolve the incremental commit file paths;
  *   <li>filter the full file paths by required partitions;
  *   <li>use the file paths from #step 3 as the back-up of the filesystem view.
  * </ol>
  */
public class IncrementalInputSplits implements Serializable {
    ...
    public Result inputSplits(HoodieTableMetaClient metaClient, boolean cdcEnabled) {
        ...
        // (would be the latest instant time if the specified end commit is greater than the latest
        // instant time)
        ...
        // Step3: find out the files to read, tries to read the files from the commit metadata
        // first,
        ...
        if (fullTableScan) {
            ...
            List<HoodieCommitMetadata> metadataList =
                    instants.stream()
                            .map(
                                    instant ->
                                            WriteProfiles.getCommitMetadata(tableName, path, instant, commitTimeline))
        ...
        List<MergeOnReadInputSplit> inputSplits =
                getInputSplits(
                        ...
                        readPartitions,
                        endInstant,
                        analyzingResult.getMaxCompletionTime(),
                        instantRange,
                        false);
    ...
    public Result inputSplits(
            HoodieTableMetaClient metaClient, @Nullable String issuedOffset, boolean cdcEnabled) {
        ...
        IncrementalQueryAnalyzer analyzer =
                IncrementalQueryAnalyzer.builder()
                        ...
                        .rangeType(
                                issuedOffset != null
                                        ? InstantRange.RangeType.OPEN_CLOSED
    ...
    private List<MergeOnReadInputSplit> getIncInputSplits(
                        ...
                        .map(
                                instant ->
                                        WriteProfiles.getCommitMetadata(
                                                tableName, path, instant, queryContext.getArchivedTimeline()))
                        .collect(Collectors.toList());
        ...
        return getInputSplits(
                metaClient,
                commitTimeline,
                pathInfoList,
                readPartitions,
                endInstant,
                queryContext.getMaxCompletionTime(),
                instantRange,
                skipCompaction);
    }
    ...
    private List<MergeOnReadInputSplit> getInputSplits(
        ...
        final HoodieTableFileSystemView fsView =
                new HoodieTableFileSystemView(metaClient, commitTimeline, pathInfoList);
        final AtomicInteger cnt = new AtomicInteger(0);
        final String mergeType = this.conf.getString(FlinkOptions.MERGE_TYPE);
        return readPartitions.stream()
                .map(
                        relPartitionPath ->
                                getFileSlices(fsView, relPartitionPath, maxCompletionTime, skipBaseFiles)
                                        .map(
                                                fileSlice -> {
                                                    ...
                                                    // the latest commit is used as the limit of the
                                                    // log reader instant upper threshold,
                                                    // it must be at least the latest instant time
                                                    // of the file slice to avoid data loss.
                                                    String latestCommit =
                                                            HoodieTimeline.minInstant(
                                                                    fileSlice.getLatestInstantTime(), endInstant);
    ...
    private List<MergeOnReadInputSplit> getCdcInputSplits(
            HoodieTableMetaClient metaClient, InstantRange instantRange) {
        HoodieCDCExtractor extractor =
                new HoodieCDCExtractor(
                        metaClient, instantRange, OptionsResolver.readCDCFromChangelog(this.conf));
    ...
    private static Stream<FileSlice> getFileSlices(
            ...
            String endInstant,
            boolean skipBaseFiles) {
        return skipBaseFiles
                ? fsView.getAllLogsMergedFileSliceBeforeOrOn(relPartitionPath, endInstant)
    ...
    /**
      * Returns the partitions to read with given metadata list. The partitions would be filtered by
      * the pushed down required partitions.
      *
    ...
    public static class Result {
        ...
        private Result(
                List<MergeOnReadInputSplit> inputSplits, String endInstant, @Nullable String offset) {
        ...
        public static Result instance(
                List<MergeOnReadInputSplit> inputSplits, String endInstant, String offset) {
