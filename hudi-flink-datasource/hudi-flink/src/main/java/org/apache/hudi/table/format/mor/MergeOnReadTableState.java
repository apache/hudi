...
public class MergeOnReadTableState implements Serializable {
    ...
    public int[] getPkOffsetsInRequired() {
        ...
        return Arrays.stream(pkFields).map(fieldNames::indexOf).mapToInt(i -> i).toArray();
    }
    ...
    public LogicalType[] getPkTypes(int[] pkOffsets) {
        final LogicalType[] requiredTypes =
                requiredRowType.getFields().stream()
                        .map(RowType.RowField::getType)
                        .toArray(LogicalType[]::new);
        return Arrays.stream(pkOffsets)
                .mapToObj(offset -> requiredTypes[offset])
                .toArray(LogicalType[]::new);
    }
}
