...
/**
  ...
  *
  * <p>Use {@code ParquetRecordReader} to read files instead of {@link
  * org.apache.flink.core.fs.FSDataInputStream}, overrides {@link #createInputSplits(int)} and {@link
...
public class MergeOnReadInputFormat extends RichInputFormat<RowData, MergeOnReadInputSplit> {
    ...
    /**
      * Flag saying whether to emit the deletes. In streaming read mode, downstream operators need the
      * DELETE messages to retract the legacy accumulator.
      */
        ...
        if (!(split.getLogPaths().isPresent() && split.getLogPaths().get().size() > 0)) {
            if (split.getInstantRange().isPresent()) {
                ...
                return new BaseFileOnlyFilteringIterator(
                        ...
                        this.requiredPos,
                        getBaseFileIterator(
                                split.getBasePath().get(), getRequiredPosWithCommitTime(this.requiredPos)));
            } else {
                // base file only
            ...
            return new SkipMergeIterator(
                    getBaseFileIterator(split.getBasePath().get()), getLogFileIterator(split));
    ...
    protected ClosableIterator<RowData> getBaseFileIteratorWithMetadata(String path) {
        try {
            return getBaseFileIterator(
                    path, IntStream.range(0, this.tableState.getRowType().getFieldCount()).toArray());
        } catch (IOException e) {
    ...
    private ClosableIterator<RowData> getBaseFileIterator(String path, int[] requiredPos)
            throws IOException {
    ...
    private ClosableIterator<RowData> getLogFileIterator(MergeOnReadInputSplit split) {
        ...
        final AvroToRowDataConverters.AvroToRowDataConverter avroToRowDataConverter =
                AvroToRowDataConverters.createRowConverter(
                        tableState.getRequiredRowType(), conf.getBoolean(FlinkOptions.READ_UTC_TIMEZONE));
        final HoodieMergedLogRecordScanner scanner =
                FormatUtils.logScanner(
                        split, tableSchema, internalSchemaManager.getQuerySchema(), conf, hadoopConf);
        ...
        final StringToRowDataConverter converter =
                pkSemanticLost ? null : new StringToRowDataConverter(pkTypes);
        ...
        return new ClosableIterator<RowData>() {
            ...
            public boolean hasNext() {
                while (logRecordsKeyIterator.hasNext()) {
                    ...
                    final HoodieAvroRecord<?> hoodieRecord =
                            (HoodieAvroRecord) scanner.getRecords().get(curAvroKey);
                    try {
                    ...
                    if (!curAvroRecord.isPresent()) {
                        ...
                        if (emitDelete && !pkSemanticLost) {
                            GenericRowData delete =
                                    new GenericRowData(tableState.getRequiredRowType().getFieldCount());
                        ...
                        final RowKind rowKind =
                                FormatUtils.getRowKindSafely(avroRecord, tableState.getOperationPos());
    ...
    private ClosableIterator<RowData> getUnMergedLogFileIterator(MergeOnReadInputSplit split) {
        ...
        final AvroToRowDataConverters.AvroToRowDataConverter avroToRowDataConverter =
                AvroToRowDataConverters.createRowConverter(
                        tableState.getRequiredRowType(), conf.getBoolean(FlinkOptions.READ_UTC_TIMEZONE));
        final FormatUtils.BoundedMemoryRecords records =
                new FormatUtils.BoundedMemoryRecords(
                        split, tableSchema, internalSchemaManager.getQuerySchema(), hadoopConf, conf);
    ...
    protected static Option<IndexedRecord> getInsertVal(
            HoodieAvroRecord<?> hoodieRecord, Schema tableSchema) {
        try {
            return hoodieRecord.getData().getInsertValue(tableSchema);
        } catch (IOException e) {
            throw new HoodieException(
                    "Get avro insert value error for key: " + hoodieRecord.getRecordKey(), e);
        }
    }
    ...
    protected ClosableIterator<RowData> getFullLogFileIterator(MergeOnReadInputSplit split) {
        ...
        final AvroToRowDataConverters.AvroToRowDataConverter avroToRowDataConverter =
                AvroToRowDataConverters.createRowConverter(
                        tableState.getRowType(), conf.getBoolean(FlinkOptions.READ_UTC_TIMEZONE));
        final HoodieMergedLogRecordScanner scanner =
                FormatUtils.logScanner(
                        split, tableSchema, InternalSchema.getEmptyInternalSchema(), conf, hadoopConf);
        final Iterator<String> logRecordsKeyIterator = scanner.getRecords().keySet().iterator();
    ...
    protected static class MergeIterator implements ClosableIterator<RowData> {
                    ...
                    Option.of(
                            record ->
                                    buildAvroRecordBySchema(
                                            ...
                                            requiredSchema,
                                            requiredPos,
                                            new GenericRecordBuilder(requiredSchema))),
                    emitDelete,
                    operationPos,
                    nested);
        }
        ...
        public MergeIterator(
            ...
            this.rowDataToAvroConverter =
                    RowDataToAvroConverters.createConverter(
                            tableRowType, flinkConf.getBoolean(FlinkOptions.WRITE_UTC_TIMEZONE));
            this.avroToRowDataConverter =
                    AvroToRowDataConverters.createRowConverter(
                            requiredRowType, flinkConf.getBoolean(FlinkOptions.READ_UTC_TIMEZONE));
        ...
        public boolean hasNext() {
            while (!readLogs && this.nested.hasNext()) {
                ...
                if (instantRange != null) {
                    boolean isInRange =
                            instantRange.isInRange(
                                    currentRecord.getString(HOODIE_COMMIT_TIME_COL_POS).toString());
                    if (!isInRange) {
                ...
                if (scanner.getRecords().containsKey(curKey)) {
                    ...
                    if (!mergedAvroRecord.isPresent()) {
                        ...
                        final RowKind rowKind =
                                FormatUtils.getRowKindSafely(mergedAvroRecord.get().getData(), this.operationPos);
                        if (!emitDelete && rowKind == RowKind.DELETE) {
                            // deleted
                            continue;
                        }
                        IndexedRecord avroRecord =
                                avroProjection.isPresent()
        ...
        private Option<HoodieRecord<IndexedRecord>> mergeRowWithLog(RowData curRow, String curKey) {
            ...
            GenericRecord historyAvroRecord =
                    (GenericRecord) rowDataToAvroConverter.convert(tableSchema, curRow);
            HoodieAvroIndexedRecord hoodieAvroIndexedRecord =
                    new HoodieAvroIndexedRecord(historyAvroRecord);
            try {
                return recordMerger
                        .merge(hoodieAvroIndexedRecord, tableSchema, record, tableSchema, payloadProps)
                        .map(Pair::getLeft);
            } catch (IOException e) {
