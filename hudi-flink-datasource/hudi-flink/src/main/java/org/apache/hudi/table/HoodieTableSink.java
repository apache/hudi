  ...
  public SinkRuntimeProvider getSinkRuntimeProvider(Context context) {
    return (DataStreamSinkProviderAdapter) dataStream -> {
      ...
      if (conf.getBoolean(FlinkOptions.WRITE_FAST_MODE)) {
        // optimized serde between Flink operators by conversion of `RowData` into `HoodieFlinkInternalRow`
        final DataStream<HoodieFlinkInternalRow> rowDataStream = Pipelines.bootstrapRowData(conf, rowType, rowDataInfo, dataStream, context.isBounded(), overwrite);
