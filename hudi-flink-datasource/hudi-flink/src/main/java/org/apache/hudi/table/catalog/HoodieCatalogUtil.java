...
public class HoodieCatalogUtil {
    ...
    public static HiveConf createHiveConf(
            @Nullable String hiveConfDir, org.apache.flink.configuration.Configuration flinkConf) {
    ...
    public static List<String> getPartitionKeys(CatalogTable table) {
        ...
        if (table.isPartitioned()) {
            ...
            return Arrays.stream(
                            table.getOptions().get(FlinkOptions.PARTITION_PATH_FIELD.key()).split(","))
    ...
    public static String inferPartitionPath(
            boolean hiveStylePartitioning, CatalogPartitionSpec catalogPartitionSpec) {
        return catalogPartitionSpec.getPartitionSpec().entrySet().stream()
                .map(
                        entry ->
    ...
    /**
      ...
      * partition keys. If the partition value is null, it'll be converted into default partition name.
      *
      * @param partitionSpec The partition spec
      * @param partitionKeys The partition keys
      * @param tablePath The table path
      * @return A list of partition values ordered by partition keys
      * @throws PartitionSpecInvalidException thrown if partitionSpec and partitionKeys have different
      *     sizes, or any key in partitionKeys doesn't exist in partitionSpec.
      */
    @VisibleForTesting
    ...
    /**
      * Modifies an existing hoodie table. Note that the new and old {@link CatalogBaseTable} must be
      * of the same kind. For example, this doesn't allow altering a regular table to partitioned
      * table, or altering a mor table to a cow table, or altering index type and vice versa.
      *
    ...
    protected static void alterTable(
        ...
        if (!tableChanges.isEmpty()) {
            ...
            HoodieFlinkWriteClient<?> writeClient =
                    createWriteClient(tablePath, oldTable, hadoopConf, inferTablePathFunc);
    ...
    private static boolean sameOptions(
            Map<String, String> parameters1,
            Map<String, String> parameters2,
            ConfigOption<String> option) {
        return parameters1
                .getOrDefault(option.key(), String.valueOf(option.defaultValue()))
                .equalsIgnoreCase(
                        parameters2.getOrDefault(option.key(), String.valueOf(option.defaultValue())));
    }

    private static boolean isUpdatePermissible(
            AbstractCatalog catalog,
            ObjectPath tablePath,
        ...
        if (!oldPartitionKeys.equals(newPartitionKeys)) {
            throw new HoodieCatalogException(
                    "Hoodie catalog does not support to alter table partition keys");
        }
