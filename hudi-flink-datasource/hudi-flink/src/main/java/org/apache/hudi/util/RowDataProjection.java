...
public class RowDataProjection implements Serializable {
    ...
    protected RowDataProjection(LogicalType[] types, int[] positions) {
        ValidationUtils.checkArgument(
                types.length == positions.length, "types and positions should have the equal number");
    ...
    public static RowDataProjection instanceV2(RowType rowType, int[] positions) {
        ...
        final LogicalType[] types =
                Arrays.stream(positions).mapToObj(fieldTypes::get).toArray(LogicalType[]::new);
        return new RowDataProjection(types, positions);
    }
