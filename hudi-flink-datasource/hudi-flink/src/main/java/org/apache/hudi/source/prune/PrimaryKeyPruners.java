...
public class PrimaryKeyPruners {
    ...
    public static int getBucketId(List<ResolvedExpression> hashKeyFilters, Configuration conf) {
        ...
        List<String> values =
                hashKeyFilters.stream()
                        .map(
                                filter -> {
                                    Pair<FieldReferenceExpression, ValueLiteralExpression> children =
                                            castChildAs(filter.getChildren());
                                    return Pair.of(
                                            pkFields.indexOf(children.getLeft().getName()),
                                            StringUtils.objToString(
                                                    ExpressionUtils.getKeyFromLiteral(
                                                            children.getRight(), logicalTimestamp)));
                                })
        ...
        return BucketIdentifier.getBucketId(
                values, conf.getInteger(FlinkOptions.BUCKET_INDEX_NUM_BUCKETS));
    }

    private static Pair<FieldReferenceExpression, ValueLiteralExpression> castChildAs(
