...
public class HoodieTableFactory implements DynamicTableSourceFactory, DynamicTableSinkFactory {
        ...
        StoragePath path =
                new StoragePath(
                        conf.getOptional(FlinkOptions.PATH)
                                .orElseThrow(() -> new ValidationException("Option [path] should not be empty.")));
    ...
    public DynamicTableSink createDynamicTableSink(Context context) {
        Configuration conf = FlinkOptions.fromMap(context.getCatalogTable().getOptions());
        checkArgument(
                !StringUtils.isNullOrEmpty(conf.getString(FlinkOptions.PATH)),
                ...
                .ifPresent(
                        tableConfig -> {
                            ...
                            if (tableConfig.contains(HoodieTableConfig.TYPE)
                                ...
                                if (!tableConfig
                                        .getString(HoodieTableConfig.TYPE)
                                        .equals(conf.get(FlinkOptions.TABLE_TYPE))) {
                                    LOG.warn(
                                            String.format(
                                                    "Table type conflict : %s in %s and %s in table options. Fix the table type as to be in line with the hoodie.properties.",
                                                    tableConfig.getString(HoodieTableConfig.TYPE),
                                                    HoodieTableConfig.HOODIE_PROPERTIES_FILE,
                                                    conf.get(FlinkOptions.TABLE_TYPE)));
                                    conf.setString(
                                            FlinkOptions.TABLE_TYPE, tableConfig.getString(HoodieTableConfig.TYPE));
                                }
                            }
                            if (tableConfig.contains(HoodieTableConfig.TYPE)
                                    && !conf.contains(FlinkOptions.TABLE_TYPE)) {
    ...
    private static void setupConfOptions(
            Configuration conf, ObjectIdentifier tablePath, CatalogTable table, ResolvedSchema schema) {
        // table name
    ...
    private static void setupHoodieKeyOptions(Configuration conf, CatalogTable table) {
        ...
        if (partitions.length == 1) {
            ...
            DataType partitionFieldType =
                    table
                            .getSchema()
                            .getFieldDataType(partitionField)
                            .orElseThrow(
                                    () ->
                                            new HoodieValidationException("Field " + partitionField + " does not exist"));
    ...
    public static void setupTimestampKeygenOptions(Configuration conf, DataType fieldType) {
        ...
        if (DataTypeUtils.isTimestampType(fieldType)) {
            ...
            conf.setString(
                    TIMESTAMP_TYPE_FIELD.key(), TimestampBasedAvroKeyGenerator.TimestampType.SCALAR.name());
    ...
    /**
      * Inferences the deserialization Avro schema from the table schema (e.g. the DDL) if both options
      * {@link FlinkOptions#SOURCE_AVRO_SCHEMA_PATH} and {@link FlinkOptions#SOURCE_AVRO_SCHEMA} are
      * not specified.
      *
