...
public class ExpressionPredicates {
    ...
    /**
      ...
      *
      * <p>Two steps to bind the call: 1. map the predicate instance; 2. bind the field reference;
      *
      * <p>Normalize the expression to simplify the subsequent decision logic: always put the literal
    ...
    public static Predicate fromExpression(CallExpression callExpression) {
        ...
        if (BuiltInFunctionDefinitions.IN.equals(functionDefinition)) {
            ...
            List<ValueLiteralExpression> valueLiterals =
                    IntStream.range(1, childExpressions.size())
    ...
    public abstract static class ColumnPredicate implements Predicate {
        ...
        public ColumnPredicate bindValueLiteral(ValueLiteralExpression valueLiteral) {
            ...
            if (literalObject instanceof Serializable) {
                ...
                LOG.warn(
                        "Encountered a non-serializable literal. "
        ...
        public FilterPredicate filter() {
            ...
            for (Serializable literal : literals) {
                FilterPredicate predicate =
                        toParquetPredicate(BuiltInFunctionDefinitions.EQUALS, literalType, columnName, literal);
                if (predicate != null) {
    ...
    private static FilterPredicate toParquetPredicate(
            ...
            LogicalType literalType,
            String columnName,
            Serializable literal) {
        switch (literalType.getTypeRoot()) {
            case BOOLEAN:
                return predicateSupportsEqNotEq(
                        functionDefinition, booleanColumn(columnName), (Boolean) literal);
            case TINYINT:
            case SMALLINT:
            case INTEGER:
            case DATE:
            ...
            case DOUBLE:
                return predicateSupportsLtGt(
                        functionDefinition, doubleColumn(columnName), (Double) literal);
            case BINARY:
            case VARBINARY:
                return predicateSupportsLtGt(
                        functionDefinition, binaryColumn(columnName), fromConstantByteArray((byte[]) literal));
            case CHAR:
            case VARCHAR:
                return predicateSupportsLtGt(
                        functionDefinition, binaryColumn(columnName), fromString((String) literal));
            default:
                return null;
        }
    }
    ...
    private static <
                    T extends Comparable<T>, C extends Operators.Column<T> & Operators.SupportsEqNotEq>
            FilterPredicate predicateSupportsEqNotEq(
    ...
    private static <T extends Comparable<T>, C extends Operators.Column<T> & Operators.SupportsLtGt>
            FilterPredicate predicateSupportsLtGt(
                    FunctionDefinition functionDefinition, C column, T value) {
        if (BuiltInFunctionDefinitions.EQUALS.equals(functionDefinition)) {
            return eq(column, value);
        } else if (BuiltInFunctionDefinitions.NOT_EQUALS.equals(functionDefinition)) {
