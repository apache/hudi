...
public class FlinkCompactionConfig extends Configuration {
    ...
    @Parameter(
            ...
            description =
                    "Target IO per compaction (both read and write) for batching compaction, default 512000M.")
    ...
    @Parameter(
            names = {"--schedule", "-sc"},
            description =
                    "Not recommended. Schedule the compaction plan in this job.\n"
    ...
    @Parameter(
            names = {"--seq"},
            description =
                    "Compaction plan execution sequence, two options are supported:\n"
    ...
    @Parameter(
            names = {"--service"},
            description = "Flink Compaction runs in service mode, disable by default")
    public Boolean serviceMode = false;

    @Parameter(
            names = {"--min-compaction-interval-seconds"},
    ...
    @Parameter(
            names = {"--plan-select-strategy"},
            description =
                    "The strategy define how to select compaction plan to compact.\n"
    ...
    @Parameter(
            names = {"--max-num-plans"},
            description =
                    "Max number of compaction plan would be selected in compaction."
                            + "It's only effective for MultiCompactionPlanSelectStrategy.")
    public Integer maxNumCompactionPlans = 1;

    @Parameter(
            names = {"--target-instants"},
            description =
                    "Specify the compaction plan instants to compact,\n"
    ...
    @Parameter(
            names = {"--spillable_map_path"},
            description = "Default file path prefix for spillable map.")
    ...
    /**
      * Transforms a {@code HoodieFlinkCompaction.config} into {@code Configuration}. The latter is
