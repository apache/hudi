...
/**
  ...
  *
  * <ol>
  *   <li>Monitoring a user-provided hoodie table path.
  *   <li>Deciding which files(or split) should be further read and processed.
  *   <li>Creating the {@link MergeOnReadInputSplit splits} corresponding to those files.
  *   <li>Assigning them to downstream tasks for further processing.
  * </ol>
  *
  * <p>The splits to be read are forwarded to the downstream {@link StreamReadOperator} which can
  * have parallelism greater than one.
  *
  * <p><b>IMPORTANT NOTE: </b> Splits are forwarded downstream for reading in ascending instant
  * commits time order, in each downstream task, the splits are also read in receiving sequence. We
  * do not ensure split consuming sequence among the downstream tasks.
  */
public class StreamReadMonitoringFunction extends RichSourceFunction<MergeOnReadInputSplit>
    ...
    public void initializeState(FunctionInitializationContext context) throws Exception {
        ...
        this.instantState =
                context
                        ...
                        .getListState(
                                new ListStateDescriptor<>("file-monitoring-state", StringSerializer.INSTANCE));

        if (context.isRestored()) {
            LOG.info(
                    "Restoring state for the class {} with table {} and base path {}.",
            ...
            ValidationUtils.checkArgument(
                    retrievedStates.size() <= 2, getClass().getSimpleName() + " retrieved invalid state.");
            ...
            if (retrievedStates.size() == 1 && issuedInstant != null) {
                ...
                if (LOG.isDebugEnabled()) {
                    LOG.debug(
                            "{} retrieved an issued instant of time {} for table {} with path {}.",
                            getClass().getSimpleName(),
                            issuedInstant,
                            conf.get(FlinkOptions.TABLE_NAME),
                            path);
                }
