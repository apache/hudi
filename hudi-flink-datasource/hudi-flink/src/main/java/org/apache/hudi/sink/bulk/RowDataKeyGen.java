...
public class RowDataKeyGen implements Serializable {
    ...
    public static RowDataKeyGen instance(Configuration conf, RowType rowType) {
        ...
        if (TimestampBasedAvroKeyGenerator.class
                .getName()
                .equals(conf.getString(FlinkOptions.KEYGEN_CLASS_NAME))) {
            try {
                keyGeneratorOpt =
                        Option.of(
                                new TimestampBasedAvroKeyGenerator(StreamerUtil.flinkConf2TypedProperties(conf)));
            } catch (IOException e) {
        ...
        return new RowDataKeyGen(
                Option.of(conf.getString(FlinkOptions.RECORD_KEY_FIELD)),
    ...
    public String getRecordKey(RowData rowData) {
        if (this.simpleRecordKey) {
            return getRecordKey(
                    recordKeyFieldGetter.getFieldOrNull(rowData),
    ...
    public String getPartitionPath(RowData rowData) {
        if (this.simplePartitionPath) {
            return getPartitionPath(
                    partitionPathFieldGetter.getFieldOrNull(rowData),
    ...
    private static String getRecordKey(
            Object[] keyValues, String[] keyFields, boolean consistentLogicalTimestampEnabled) {
        ...
        if (keyIsNullEmpty) {
            throw new HoodieKeyException(
                    String.format(
                            "recordKey values: \"%s\" for fields: %s cannot be entirely null or empty.",
    ...
    private static String getRecordPartitionPath(
        ...
        for (int i = 0; i < partFields.length; i++) {
            ...
            if (partValue == null || partValue.isEmpty()) {
                ...
                partitionPath.append(
                        hiveStylePartitioning
                                ? String.format(HIVE_PARTITION_TEMPLATE, partField, partValue)
                                : partValue);
            }
    ...
    public static String getRecordKey(
            Object recordKeyValue, String recordKeyField, boolean consistentLogicalTimestampEnabled) {
        ...
        if (recordKey == null || recordKey.isEmpty()) {
            throw new HoodieKeyException(
                    String.format(
                            "recordKey value: \"%s\" for field: \"%s\" cannot be null or empty.",
    ...
    private static Object toEpochMilli(Object val, TimestampBasedAvroKeyGenerator keyGenerator) {
        ...
        if (val == null) {
            // should match the default partition path when STRING partition path re-format is
            // supported
    ...
    /**
      ...
      */
    private static RowDataProjection getProjection(
            String[] fields, List<String> schemaFields, List<LogicalType> schemaTypes) {
        int[] positions = getFieldPositions(fields, schemaFields);
        LogicalType[] types =
                Arrays.stream(positions).mapToObj(schemaTypes::get).toArray(LogicalType[]::new);
        return RowDataProjection.instance(types, positions);
    }

    /**
      * Returns the field positions of the given fields {@code fields} among all the fields {@code
      * allFields}.
      */
