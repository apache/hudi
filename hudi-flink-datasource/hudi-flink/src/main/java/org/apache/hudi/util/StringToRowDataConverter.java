...
/**
  * A converter that converts a string array into internal row data fields. The converter is designed
  * to be stateful(not pure stateless tool) in order to reuse the specific converters.
  */
@Internal
public class StringToRowDataConverter {
    ...
    public Object[] convert(String[] fields) {
        ValidationUtils.checkArgument(
                converters.length == fields.length, "Field types and values should equal with number");
                ...
                if (precision <= 3) {
                    return field ->
                            TimestampData.fromInstant(
                                    Instant.EPOCH.plus(Long.parseLong(field), ChronoUnit.MILLIS));
                } else if (precision <= 6) {
                    return field ->
                            TimestampData.fromInstant(
                                    Instant.EPOCH.plus(Long.parseLong(field), ChronoUnit.MICROS));
                } else {
            ...
            case DECIMAL:
                ...
                return field ->
                        DecimalData.fromBigDecimal(
                                new BigDecimal(field), decimalType.getPrecision(), decimalType.getScale());
            default:
                throw new UnsupportedOperationException("Unsupported type: " + logicalType);
        }
    }
