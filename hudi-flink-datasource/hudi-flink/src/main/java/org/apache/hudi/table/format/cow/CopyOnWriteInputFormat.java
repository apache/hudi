...
/**
  * An implementation of {@link FileInputFormat} to read {@link RowData} records from Parquet files.
  *
  * <p>Note: Reference Flink release 1.11.2 {@code
  * org.apache.flink.formats.parquet.ParquetFileSystemFormatFactory.ParquetInputFormat} to support
  * TIMESTAMP_MILLIS.
  *
  * <p>Note: Override the {@link #createInputSplits} method from parent to rewrite the logic creating
  * the FileSystem, use {@link HadoopFSUtils#getFs} to get a plugin filesystem.
  *
...
public class CopyOnWriteInputFormat extends FileInputFormat<RowData> {
    ...
    public FileInputSplit[] createInputSplits(int minNumSplits) throws IOException {
        ...
        if (unsplittable) {
            ...
            for (final FileStatus file : files) {
                ...
                FileInputSplit fis =
                        new FileInputSplit(
                                splitNum++, new Path(file.getPath().toUri()), 0, len, hosts.toArray(new String[0]));
                inputSplits.add(fis);
            }
        ...
        for (final FileStatus file : files) {
            ...
            if (len > 0) {
                ...
                final FileInputSplit fis =
                        new FileInputSplit(splitNum++, new Path(file.getPath().toUri()), 0, 0, hosts);
                inputSplits.add(fis);
            }
        }
    ...
    private long addFilesInDir(
            org.apache.hadoop.fs.Path path, List<FileStatus> files, boolean logExcludedFiles)
            throws IOException {
        final org.apache.hadoop.fs.Path hadoopPath = new org.apache.hadoop.fs.Path(path.toUri());
        final FileSystem fs = HadoopFSUtils.getFs(hadoopPath.toString(), this.conf.conf());
    ...
    /**
      * Retrieves the index of the <tt>BlockLocation</tt> that contains the part of the file described
      * by the given offset.
      *
    ...
    private InflaterInputStreamFactory<?> getInflaterInputStreamFactory(
