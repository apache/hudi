...
/**
  * Profiling of write statistics for {@link BucketAssigner}, such as the average record size and
  * small files.
  *
...
public class WriteProfile {
    ...
    /**
      * Total records to write for each bucket based on the config option {@link
    ...
    private long averageBytesPerRecord() {
        ...
        long fileSizeThreshold =
                (long) (config.getRecordSizeEstimationThreshold() * config.getParquetSmallFileLimit());
        HoodieTimeline commitTimeline = metaClient.getCommitsTimeline().filterCompletedInstants();
        if (!commitTimeline.empty()) {
            // Go over the reverse ordered commits to get a more recent estimate of average record
            // size.
            ...
            while (instants.hasNext()) {
                ...
                final HoodieCommitMetadata commitMetadata =
                        this.metadataCache.computeIfAbsent(
                                ...
                                k ->
                                        WriteProfiles.getCommitMetadataSafely(
                                                        config.getTableName(), basePath, instant, commitTimeline)
                                                .orElse(null));
    ...
    /**
      * Remove the overdue metadata from the cache whose instant does not belong to the given instants
      * {@code instants}.
      */
    private void cleanMetadataCache(Stream<HoodieInstant> instants) {
        Set<String> timestampSet =
                instants.map(HoodieInstant::getTimestamp).collect(Collectors.toSet());
    ...
    /**
      ...
      *
      * <p>We do these things: i). reload the timeline; ii). re-construct the record profile; iii)
      * clean the small files cache.
      *
      * <p>Note: This method should be thread safe.
      */
    public synchronized void reload(long checkpointId) {
        ...
        recordProfile();
        cleanMetadataCache(
                this.metaClient.getCommitsTimeline().filterCompletedInstants().getInstantsAsStream());
