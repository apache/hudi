...
/**
  * The function to tag each incoming record with a location of a file based on consistent bucket
  * index.
  */
public class ConsistentBucketAssignFunction extends ProcessFunction<HoodieRecord, HoodieRecord>
    ...
    public void processElement(
            HoodieRecord record, Context context, Collector<HoodieRecord> collector) throws Exception {
        ...
        Preconditions.checkArgument(
                ...
                "Consistent hashing node has no file group, partition: "
                        + partition
                        + ", meta: "
        ...
        record.unseal();
        record.setCurrentLocation(
                new HoodieRecordLocation("U", FSUtils.createNewFileId(node.getFileIdPrefix(), 0)));
        record.seal();
        collector.collect(record);
    }

    private ConsistentBucketIdentifier getBucketIdentifier(String partition) {
        return partitionToIdentifier.computeIfAbsent(
                partition,
                p -> {
                    // NOTE: If the metadata does not exist, there maybe concurrent creation of the
                    // metadata. And we allow multiple subtask
                    // trying to create the same metadata as the initial metadata always has the
                    // same content for the same partition.
                    int retryCount = 0;
                        ...
                        try {
                            metadata =
                                    ConsistentBucketIndexUtils.loadOrCreateMetadata(
                                            this.writeClient.getHoodieTable(), p, bucketNum);
                            break;
                        } catch (Exception e) {
                            if (retryCount >= maxRetries) {
                                throw new HoodieLockException(
                                        "Fail to load or create metadata for partition " + partition, e);
                            }
                            try {
                            ...
                            LOG.info(
                                    "Retrying to load or create metadata for partition {} for {} times",
                                    partition,
                                    retryCount + 1);
                        } finally {
                            retryCount++;
    ...
    public void snapshotState(FunctionSnapshotContext functionSnapshotContext) throws Exception {
        HoodieTimeline timeline =
                writeClient
                        ...
                        .findInstantsAfter(lastRefreshInstant);
        if (!timeline.empty()) {
            for (HoodieInstant instant : timeline.getInstants()) {
                ...
                LOG.info(
                        "Clear up cached hashing metadata because find a new replace commit.\n Instant: {}.\n Effected Partitions: {}.",
                        lastRefreshInstant,
                        affectedPartitions);
