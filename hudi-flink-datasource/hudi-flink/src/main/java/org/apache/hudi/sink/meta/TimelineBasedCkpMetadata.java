...
/**
  * Timeline server based CkpMetadata, will read ckpMessages from timeline-server instead of from
...
public class TimelineBasedCkpMetadata extends CkpMetadata {
    ...
    public TimelineBasedCkpMetadata(
            FileSystem fs, String basePath, String uniqueId, HoodieWriteConfig writeConfig) {
        ...
        try {
            List<InstantStateDTO> instantStateDTOList =
                    httpRequestClient.executeRequestWithRetry(
                            ...
                            RequestMethod.GET);
            ckpMessageStream =
                    instantStateDTOList.stream().map(c -> new CkpMessage(c.getInstant(), c.getState()));
        } catch (Exception e) {
