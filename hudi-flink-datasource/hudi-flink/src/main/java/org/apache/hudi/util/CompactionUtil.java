...
public class CompactionUtil {
    ...
    public static void scheduleCompaction(
            HoodieFlinkWriteClient<?> writeClient, boolean deltaTimeCompaction, boolean committed) {
        if (committed) {
            writeClient.scheduleCompaction(Option.empty());
        } else if (deltaTimeCompaction) {
            // if there are no new commits and the compaction trigger strategy is based on elapsed
            // delta time,
    ...
    /**
      * Sets up the avro schema string into the give configuration {@code conf} through reading from
      * the hoodie table metadata.
      *
      * @param conf The configuration
      */
    public static void setAvroSchema(Configuration conf, HoodieTableMetaClient metaClient)
            throws Exception {
    ...
    /**
      * Sets up the avro schema string into the HoodieWriteConfig {@code HoodieWriteConfig} through
      * reading from the hoodie table metadata.
      *
      * @param writeConfig The HoodieWriteConfig
      */
    public static void setAvroSchema(HoodieWriteConfig writeConfig, HoodieTableMetaClient metaClient)
            throws Exception {
        TableSchemaResolver tableSchemaResolver = new TableSchemaResolver(metaClient);
        Schema tableAvroSchema = tableSchemaResolver.getTableAvroSchema(false);
    ...
    /**
      * Sets up the preCombine field into the given configuration {@code conf} through reading from the
      * hoodie table metadata.
      *
      * <p>This value is non-null as compaction can only be performed on MOR tables. Of which, MOR
      * tables will have non-null precombine fields.
      *
      * @param conf The configuration
      */
    ...
    public static void rollbackCompaction(HoodieFlinkTable<?> table, String instantTime) {
        ...
        if (table
                ...
                .containsInstant(inflightInstant)) {
    ...
    public static void rollbackCompaction(HoodieFlinkTable<?> table) {
        HoodieTimeline inflightCompactionTimeline =
                table
                        .getActiveTimeline()
                        .filterPendingCompactionTimeline()
                        .filter(instant -> instant.getState() == HoodieInstant.State.INFLIGHT);
        inflightCompactionTimeline
                .getInstants()
                .forEach(
                        inflightInstant -> {
                            LOG.info(
                                    "Rollback the inflight compaction instant: " + inflightInstant + " for failover");
                            table.rollbackInflightCompaction(inflightInstant);
    ...
    /**
      ...
      *
      * <p>Makes the strategy not that radical: firstly check whether there exists inflight compaction
      * instants, rolls back the first inflight instant only if it has timed out. That means, if there
      * are multiple timed out instants on the timeline, we only roll back the first one at a time.
      */
