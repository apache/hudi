...
public class FilebasedSchemaProvider extends SchemaProvider {
    ...
    public static class Config {
        private static final ConfigProperty<String> SOURCE_SCHEMA_FILE =
                ConfigProperty.key(SCHEMAPROVIDER_CONFIG_PREFIX + "source.schema.file")
                        .noDefaultValue()
        ...
        private static final ConfigProperty<String> TARGET_SCHEMA_FILE =
                ConfigProperty.key(SCHEMAPROVIDER_CONFIG_PREFIX + "target.schema.file")
                        .noDefaultValue()
    ...
    public FilebasedSchemaProvider(TypedProperties props) {
        ...
        FileSystem fs =
                HadoopFSUtils.getFs(
                        sourceSchemaFile, HadoopConfigurations.getHadoopConf(new Configuration()));
        try {
            ...
            if (containsConfigProperty(props, Config.TARGET_SCHEMA_FILE)) {
                this.targetSchema =
                        new Schema.Parser()
                                .parse(fs.open(new Path(getStringWithAltKeys(props, Config.TARGET_SCHEMA_FILE))));
            }
    ...
    public FilebasedSchemaProvider(Configuration conf) {
        ...
        final FileSystem fs =
                HadoopFSUtils.getFs(sourceSchemaPath, HadoopConfigurations.getHadoopConf(conf));
        try {
