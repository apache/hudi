...
public class ClusteringPlanSourceFunction extends AbstractRichFunction
    ...
    public ClusteringPlanSourceFunction(
            String clusteringInstantTime, HoodieClusteringPlan clusteringPlan, Configuration conf) {
    ...
    public void run(SourceContext<ClusteringPlanEvent> sourceContext) throws Exception {
        boolean isPending =
                StreamerUtil.createMetaClient(conf)
                        .getActiveTimeline()
                        .isPendingClusterInstant(clusteringInstantTime);
        if (isPending) {
