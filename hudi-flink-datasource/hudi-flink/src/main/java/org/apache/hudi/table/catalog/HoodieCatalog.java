...
public class HoodieCatalog extends AbstractCatalog {
    ...
    public HoodieCatalog(String name, Configuration options) {
        ...
        options
                .getOptional(CATALOG_PATH)
                .orElseThrow(() -> new ValidationException("Option [catalog.path] should not be empty."));
        ...
        try {
            if (!fs.exists(catalogPath)) {
                throw new CatalogException(
                        String.format("Catalog %s path %s does not exist.", getName(), catalogPathStr));
            }
        } catch (IOException e) {
            throw new CatalogException(
                    String.format("Checking catalog path %s exists exception.", catalogPathStr), e);
        }

        if (!databaseExists(getDefaultDatabase())) {
            LOG.info(
                    "Creating database {} automatically because it does not exist.", getDefaultDatabase());
    ...
    public CatalogBaseTable getTable(ObjectPath tablePath)
        ...
        if (latestSchema != null) {
            ...
            org.apache.flink.table.api.Schema.Builder builder =
                    org.apache.flink.table.api.Schema.newBuilder().fromRowDataType(tableDataType);
    ...
    public void createTable(
        ...
        final String avroSchema =
                AvroSchemaConverter.convertToSchema(
                                ...
                                AvroSchemaUtils.getAvroRecordQualifiedName(tablePath.getObjectName()))
                        .toString();
        ...
        if (resolvedSchema.getPrimaryKey().isPresent()) {
            options.put(
                    TableOptionProperties.PK_CONSTRAINT_NAME, resolvedSchema.getPrimaryKey().get().getName());
        }
        ...
        try {
            ...
            throw new CatalogException(
                    String.format("Initialize table path %s exception.", tablePathStr), e);
        }
    }
    ...
    public boolean partitionExists(ObjectPath tablePath, CatalogPartitionSpec catalogPartitionSpec)
        ...
        boolean hiveStylePartitioning =
                Boolean.parseBoolean(
                        options.getOrDefault(FlinkOptions.HIVE_STYLE_PARTITIONING.key(), "false"));
    ...
    public void dropPartition(
            ObjectPath tablePath, CatalogPartitionSpec catalogPartitionSpec, boolean ignoreIfNotExists)
            throws PartitionNotExistException, CatalogException {
        if (!tableExists(tablePath)) {
            if (ignoreIfNotExists) {
                return;
        ...
        try (HoodieFlinkWriteClient<?> writeClient =
                HoodieCatalogUtil.createWriteClient(options, tablePathStr, tablePath, hadoopConf)) {
            writeClient
                    .deletePartitions(
                            Collections.singletonList(partitionPathStr), writeClient.createNewInstantTime())
                    .forEach(
                            writeStatus -> {
                                if (writeStatus.hasErrors()) {
                                    throw new HoodieMetadataException(
                                            String.format(
                                                    "Failed to commit metadata table records at file id %s.",
            ...
            throw new CatalogException(
                    String.format(
                            "Dropping partition %s of table %s exception.", partitionPathStr, tablePath),
                    e);
        }
    }
    ...
    public void createFunction(
            ObjectPath functionPath, CatalogFunction catalogFunction, boolean ignoreIfExists)
    ...
    public void alterFunction(
            ObjectPath functionPath, CatalogFunction catalogFunction, boolean ignoreIfNotExists)
            throws FunctionNotExistException, CatalogException {
        ...
        if (path != null && StreamerUtil.tableExists(path, hadoopConf)) {
            try {
                ...
                return new TableSchemaResolver(metaClient)
                        .getTableAvroSchema(false); // change log mode is not supported now
