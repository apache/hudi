...
/**
  * The operator that reads the {@link MergeOnReadInputSplit splits} received from the preceding
  * {@link StreamReadMonitoringFunction}. Contrary to the {@link StreamReadMonitoringFunction} which
  * has a parallelism of 1, this operator can have multiple parallelism.
  *
  * <p>As soon as an input split {@link MergeOnReadInputSplit} is received, it is put into a queue,
  * the {@code MailboxExecutor} read the actual data of the split. This architecture allows the
  * separation of split reading from processing the checkpoint barriers, thus removing any potential
  * back-pressure.
  */
public class StreamReadOperator extends AbstractStreamOperator<RowData>
    ...
    // It's the same thread that runs this operator and checkpoint actions. Use this executor to
    // schedule only
    // splits for subsequent reading, so that a new checkpoint could be triggered without blocking a
    // long time
    ...
    // them to the executor. This state is used to ensure that only one read task is in that splits
    // queue at a time, so that
    // read tasks do not accumulate ahead of checkpoint tasks. When there is a read task in the
    // queue, this is set to RUNNING.
    ...
    public void initializeState(StateInitializationContext context) throws Exception {
        ...
        if (context.isRestored()) {
            ...
            LOG.info(
                    "Restoring state for operator {} (task ID: {}).", getClass().getSimpleName(), subtaskIdx);
    ...
    private void processSplits() throws IOException {
        ...
        // 2. if the input split has remaining records to read, switches to another runnable to
        // handle
        // 3. if the input split reads to the end, close the format and remove the split from the
        // queue #splits
    ...
    /**
      * Consumes at most {@link #MINI_BATCH_SIZE} number of records for the given input split {@code
      * split}.
      *
      * <p>Note: close the input format and remove the input split for the queue {@link #splits} if the
      * split reads to the end.
      *
    ...
    public static OneInputStreamOperatorFactory<MergeOnReadInputSplit, RowData> factory(
            MergeOnReadInputFormat format) {
    ...
    private static class OperatorFactory extends AbstractStreamOperatorFactory<RowData>
        ...
        public <O extends StreamOperator<RowData>> O createStreamOperator(
            ...
            StreamReadOperator operator =
                    new StreamReadOperator(format, processingTimeService, getMailboxExecutor());
            operator.setup(
