...
/**
  ...
  *
  * <p>Each time the driver starts a new instant, it writes a commit message into the metadata, the
  * write tasks then consume the message and unblock the data flushing.
  *
  * <p>Why we use the DFS based message queue instead of sending the {@link
  * org.apache.flink.runtime.operators.coordination.OperatorEvent} ? The writer task thread handles
  * the operator event using the main mailbox executor which has the lowest priority for mails, it is
  * also used to process the inputs. When the writer task blocks and waits for the operator event to
  * ack the valid instant to write, it actually blocks all the subsequent events in the mailbox, the
  * operator event would never be consumed then it causes deadlock.
  *
  * <p>The checkpoint metadata is also more lightweight than the active timeline.
  *
  * <p>NOTE: should be removed in the future if we have good manner to handle the async notifications
  * from driver.
  */
public class CkpMetadata implements Serializable, AutoCloseable {
    ...
    private void clean() {
        if (instantCache.size() > MAX_RETAIN_CKP_NUM) {
            ...
            CkpMessage.getAllFileNames(instant).stream()
                    .map(this::fullPath)
                    .forEach(
                            path -> {
                                try {
    ...
    public void abortInstant(String instant) {
        ...
        try {
            ...
            throw new HoodieException(
                    "Exception while adding checkpoint abort metadata for instant: " + instant);
        }
    }
    ...
    private void load() {
        try {
            ...
            throw new HoodieException(
                    "Exception while scanning the checkpoint meta files under path: " + this.path, e);
        }
    }
    ...
    public boolean isAborted(String instant) {
        ...
        return this.messages.stream()
                .anyMatch(ckpMsg -> instant.equals(ckpMsg.getInstant()) && ckpMsg.isAborted());
    }
    ...
    protected List<CkpMessage> scanCkpMetadata(Path ckpMetaPath) throws IOException {
        return fetchCkpMessages(ckpMetaPath).collect(Collectors.groupingBy(CkpMessage::getInstant))
                .values().stream()
                .map(
                        messages ->
                                messages.stream()
