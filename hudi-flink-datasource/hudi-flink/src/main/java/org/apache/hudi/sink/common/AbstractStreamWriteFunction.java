...
/**
  ...
  */
public abstract class AbstractStreamWriteFunction<I> extends AbstractWriteFunction<I>
    ...
    /**
      * Flag saying whether the write task is waiting for the checkpoint success notification after it
      * finished a checkpoint.
      *
      * <p>The flag is needed because the write task does not block during the waiting time interval,
      * some data buckets still flush out with old instant time. There are two cases that the flush may
      * produce corrupted files if the old instant is committed successfully: 1) the write handle was
      * writing data but interrupted, left a corrupted parquet file; 2) the write handle finished the
      * write but was not closed, left an empty parquet file.
      *
      * <p>To solve, when this flag was set to true, we block the data flushing thus the
      * #processElement method, the flag was reset to false if the task receives the checkpoint success
      * event or the latest inflight instant time changed(the last instant committed successfully).
      */
    ...
    /**
      * Since flink 1.15, the streaming job with bounded source triggers one checkpoint after calling
      * #endInput, use this flag to avoid unnecessary data flush.
      */
    ...
    public void initializeState(FunctionInitializationContext context) throws Exception {
        ...
        this.writeMetadataState =
                context
                        .getOperatorStateStore()
                        .getListState(
                                new ListStateDescriptor<>(
                                        "write-metadata-state", TypeInformation.of(WriteMetadataEvent.class)));
    ...
    public void handleOperatorEvent(OperatorEvent event) {
        ValidationUtils.checkArgument(
                event instanceof CommitAckEvent, "The write function can only handle CommitAckEvent");
