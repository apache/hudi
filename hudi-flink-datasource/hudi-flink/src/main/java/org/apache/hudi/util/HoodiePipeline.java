...
/**
  ...
  *
  * <p>How to use ? Method {@link #builder(String)} returns a pipeline builder. The builder can then
  * define the hudi table columns, primary keys and partitions.
  *
  * <p>An example:
  *
  * <pre>
...
public class HoodiePipeline {
    ...
    public static class Builder {
        ...
        public Builder schema(Schema schema) {
            ...
            if (schema.getPrimaryKey().isPresent()) {
                pk(
                        schema.getPrimaryKey().get().getColumnNames().stream()
        ...
        public DataStreamSink<?> sink(DataStream<RowData> input, boolean bounded) {
            ...
            return HoodiePipeline.sink(
                    input, tableDescriptor.getTableId(), tableDescriptor.getResolvedCatalogTable(), bounded);
        }
        ...
        public TableDescriptor getTableDescriptor() {
            ...
            String sql =
                    getCreateHoodieTableDDL(
                            this.tableName, this.columns, this.options, this.pk, this.partitions);
            ...
            try {
                ...
                catalogTable =
                        (ResolvedCatalogTable)
                                catalog.getTable(new ObjectPath(defaultDatabase, this.tableName));
            } catch (TableNotExistException e) {
                throw new HoodieException("Create table " + this.tableName + " exception", e);
            }
            ObjectIdentifier tableId =
                    ObjectIdentifier.of(currentCatalog, defaultDatabase, this.tableName);
            return new TableDescriptor(tableId, catalogTable);
        }

        public DataStream<RowData> source(StreamExecutionEnvironment execEnv) {
            TableDescriptor tableDescriptor = getTableDescriptor();
            return HoodiePipeline.source(
                    execEnv, tableDescriptor.tableId, tableDescriptor.getResolvedCatalogTable());
        }
    }
    ...
    private static String getCreateHoodieTableDDL(
        ...
        if (!partitionField.isEmpty()) {
            String partitions =
                    partitionField.stream()
                            .map(partitionName -> "`" + partitionName + "`")
                            .collect(Collectors.joining(","));
            builder.append("PARTITIONED BY (").append(partitions).append(")\n");
        }
    ...
    /**
      ...
      */
    private static DataStreamSink<?> sink(
            DataStream<RowData> input,
            ObjectIdentifier tablePath,
            ResolvedCatalogTable catalogTable,
            boolean isBounded) {
        FactoryUtil.DefaultDynamicTableContext context =
                Utils.getTableContext(
                        tablePath, catalogTable, Configuration.fromMap(catalogTable.getOptions()));
    ...
    /**
      ...
      * @param catalogTable The hoodie catalog table
      */
    private static DataStream<RowData> source(
        ...
        FactoryUtil.DefaultDynamicTableContext context =
                Utils.getTableContext(
                        tablePath, catalogTable, Configuration.fromMap(catalogTable.getOptions()));
        HoodieTableFactory hoodieTableFactory = new HoodieTableFactory();
        DataStreamScanProvider dataStreamScanProvider =
                (DataStreamScanProvider)
                        ((ScanTableSource) hoodieTableFactory.createDynamicTableSource(context))
