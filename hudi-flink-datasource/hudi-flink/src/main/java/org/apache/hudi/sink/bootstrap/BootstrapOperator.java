...
/**
  ...
  *
  * <p>Each subtask of the function triggers the index bootstrap when the first element came in, the
  * record cannot be sent until all the index records have been sent.
  *
  * <p>The output records should then shuffle by the recordKey and thus do scalable write.
  */
public class BootstrapOperator<I, O extends HoodieRecord<?>> extends AbstractStreamOperator<O>
    ...
    public void initializeState(StateInitializationContext context) throws Exception {
        ListStateDescriptor<String> instantStateDescriptor =
                new ListStateDescriptor<>("instantStateDescriptor", Types.STRING);
    ...
    protected void preLoadIndexRecords() throws Exception {
        ...
        LOG.info(
                "Start loading records in table {} into the index state, taskId = {}", basePath, taskID);
        ...
        LOG.info(
                "Finish sending index records, taskId = {}.", getRuntimeContext().getIndexOfThisSubtask());
            ...
            try {
                readyTaskNum =
                        aggregateManager.updateGlobalAggregate(
                                BootstrapAggFunction.NAME + conf.getString(FlinkOptions.TABLE_NAME),
                                taskID,
    ...
    protected void loadRecords(String partitionPath) throws Exception {
            ...
            List<FileSlice> fileSlices =
                    this.hoodieTable
                            ...
                            .getLatestMergedFileSlicesBeforeOrOn(
                                    partitionPath, latestCommitTime.get().getTimestamp())
                            .collect(toList());
            ...
            for (FileSlice fileSlice : fileSlices) {
                                    ...
                                    try (ClosableIterator<HoodieKey> iterator =
                                        ...
                                        iterator.forEachRemaining(
                                                hoodieKey -> {
                                                    output.collect(
                                                            new StreamRecord(
                                                                    new IndexRecord(generateHoodieRecord(hoodieKey, fileSlice))));
                                                });
                                    }
                                });
                ...
                try (HoodieMergedLogRecordScanner scanner =
                        FormatUtils.logScanner(
                                logPaths, schema, latestCommitTime.get().getTimestamp(), writeConfig, hadoopConf)) {
                    for (String recordKey : scanner.getRecords().keySet()) {
                        output.collect(
                                new StreamRecord(
                                        new IndexRecord(
                                                generateHoodieRecord(new HoodieKey(recordKey, partitionPath), fileSlice))));
                    }
                } catch (Exception e) {
                    throw new HoodieException(
                            String.format("Error when loading record keys from files: %s", logPaths), e);
                }
            }
        }
        ...
        LOG.info(
                "Task [{}}:{}}] finish loading the index under partition {} and sending them to downstream, time cost: {} milliseconds.",
    ...
    public static HoodieRecord generateHoodieRecord(HoodieKey hoodieKey, FileSlice fileSlice) {
        ...
        hoodieRecord.setCurrentLocation(
                new HoodieRecordGlobalLocation(
                        hoodieKey.getPartitionPath(), fileSlice.getBaseInstantTime(), fileSlice.getFileId()));
        hoodieRecord.seal();
    ...
    protected boolean shouldLoadFile(String fileId, int maxParallelism, int parallelism, int taskID) {
        return KeyGroupRangeAssignment.assignKeyToParallelOperator(fileId, maxParallelism, parallelism)
                == taskID;
    }
