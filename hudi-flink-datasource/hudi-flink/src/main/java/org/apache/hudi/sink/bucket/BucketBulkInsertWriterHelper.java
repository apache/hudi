...
public class BucketBulkInsertWriterHelper extends BulkInsertWriterHelper {
    ...
    public BucketBulkInsertWriterHelper(
            Configuration conf,
    ...
    public void write(RowData tuple) throws IOException {
        try {
            ...
            LOG.error(
                    "Global error thrown while trying to write records in HoodieRowDataCreateHandle",
                    ioException);
            throw ioException;
        }
    }
    ...
    private static String getFileId(
        ...
        return bucketIdToFileId.computeIfAbsent(
                bucketId, k -> BucketIdentifier.newBucketFileIdPrefix(bucketNum, needFixedFileIdSuffix));
    }

    public static RowData rowWithFileId(
            Map<String, String> bucketIdToFileId,
            RowDataKeyGen keyGen,
            RowData record,
            String indexKeys,
            ...
            boolean needFixedFileIdSuffix) {
        final String fileId =
                getFileId(bucketIdToFileId, keyGen, record, indexKeys, numBuckets, needFixedFileIdSuffix);
