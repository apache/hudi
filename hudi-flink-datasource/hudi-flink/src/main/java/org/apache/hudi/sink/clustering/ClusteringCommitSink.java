...
/**
  ...
  *
  * <p>Each time after receiving a clustering commit event {@link ClusteringCommitEvent}, it loads
  * and checks the clustering plan {@link org.apache.hudi.avro.model.HoodieClusteringPlan}, if all
  * the clustering operations {@link org.apache.hudi.common.model.ClusteringOperation} of the plan
  * are finished, tries to commit the clustering action.
  *
  * <p>It also inherits the {@link CleanFunction} cleaning ability. This is needed because the SQL
  * API does not allow multiple sinks in one table sink provider.
  */
public class ClusteringCommitSink extends CleanFunction<ClusteringCommitEvent> {
    ...
    /**
      ...
      *
      * <p>Stores the mapping of instant_time -> file_ids -> event. Use a map to collect the events
      * because the rolling back of intermediate clustering tasks generates corrupt events.
      */
    private transient Map<String, Map<String, ClusteringCommitEvent>> commitBuffer;

    /**
      * Cache to store clustering plan for each instant. Stores the mapping of instant_time ->
      * clusteringPlan.
      */
    ...
    public void invoke(ClusteringCommitEvent event, Context context) throws Exception {
        ...
        commitBuffer.computeIfAbsent(instant, k -> new HashMap<>()).put(event.getFileIds(), event);
        commitIfNecessary(instant, commitBuffer.get(instant).values());
    }

    /**
      * Condition to commit: the commit buffer has equal size with the clustering plan operations and
      * all the clustering commit event {@link ClusteringCommitEvent} has the same clustering instant
      * time.
      *
    ...
    private void commitIfNecessary(String instant, Collection<ClusteringCommitEvent> events) {
        HoodieClusteringPlan clusteringPlan =
                clusteringPlanCache.computeIfAbsent(
                        instant,
                        k -> {
                            try {
    ...
    private void doCommit(
        ...
        if (numErrorRecords > 0 && !this.conf.getBoolean(FlinkOptions.IGNORE_FAILED)) {
            // handle failure case
            LOG.error(
                    "Got {} error records during clustering of instant {},\n"
                            + "option '{}' is configured as false,"
                            + "rolls back the clustering",
                    numErrorRecords,
                    instant,
    ...
    private static Map<String, List<String>> getPartitionToReplacedFileIds(
            HoodieClusteringPlan clusteringPlan, HoodieWriteMetadata<List<WriteStatus>> writeMetadata) {
