...
public class HoodieFlinkCompactor {
    ...
    public static class AsyncCompactionService extends HoodieAsyncTableService {
        ...
        private void compact() throws Exception {
            ...
            compactionInstantTimes.forEach(
                    timestamp -> {
                                    ...
                                    timestamp -> {
                                        try {
                                            ...
                                            throw new HoodieException(
                                                    "Get compaction plan at instant " + timestamp + " error", e);
                                        }
                                    })
            ...
            List<HoodieInstant> instants =
                    compactionInstantTimes.stream()
                            .map(HoodieTimeline::getCompactionRequestedInstant)
                            .collect(Collectors.toList());

            int totalOperations =
                    Math.toIntExact(
                            compactionPlans.stream()
                                    .mapToLong(pair -> pair.getRight().getOperations().size())
                                    .sum());
