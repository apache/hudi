...
/**
  ...
  *
  * <p>This assigner assigns the record one by one. If the record is an update, checks and reuse
  * existing UPDATE bucket or generates a new one; If the record is an insert, checks the record
  * partition for small files first, try to find a small file that has space to append new records
  * and reuse the small file's data bucket, if there is no small file(or no left space for new
  * records), generates an INSERT bucket.
  *
  * <p>Use {partition}_{fileId} as the bucket identifier, so that the bucket is unique within and
  * among partitions.
  */
public class BucketAssigner implements AutoCloseable {
    ...
    /**
      * Reset the states of this assigner, should do once for each checkpoint, all the states are
      * accumulated within one checkpoint interval.
      */
    public void reset() {
        bucketInfoMap.clear();
    }
    ...
    public BucketInfo addInsert(String partitionPath) {
        ...
        if (newFileAssignStates.containsKey(partitionPath)) {
            ...
            if (newFileAssignState.canAssign()) {
                ...
                if (bucketInfoMap.containsKey(key)) {
                    // the newFileAssignStates is cleaned asynchronously when received the
                    ...
                    // should still be assigned to the small buckets of last checkpoint instead of
                    // new one.

                    // the bucketInfoMap is cleaned when checkpoint starts.

                    // A promotion: when the HoodieRecord can record whether it is an UPDATE or
                    // INSERT,
                    // we can always return an UPDATE BucketInfo here, and there is no need to
                    // record the
        ...
        BucketInfo bucketInfo =
                new BucketInfo(BucketType.INSERT, createFileIdOfThisTask(), partitionPath);
        ...
        NewFileAssignState newFileAssignState =
                new NewFileAssignState(bucketInfo.getFileIdPrefix(), writeProfile.getRecordsPerBucket());
        newFileAssignState.assign();
    ...
    private synchronized SmallFileAssign getSmallFileAssign(String partitionPath) {
        ...
        if (smallFiles.size() > 0) {
            ...
            SmallFileAssignState[] states =
                    smallFiles.stream()
                            .map(
                                    smallFile ->
                                            new SmallFileAssignState(
                                                    config.getParquetMaxFileSize(), smallFile, writeProfile.getAvgSize()))
    ...
    private boolean fileIdOfThisTask(String fileId) {
        ...
        return KeyGroupRangeAssignment.assignKeyToParallelOperator(fileId, maxParallelism, numTasks)
                == taskID;
    }
    ...
    /**
      ...
      *
      * <p>The tool is initialized with an array of {@link SmallFileAssignState}s. A pointer points to
      * the current small file we are ready to assign, if the current small file can not be assigned
      * anymore (full assigned), the pointer move to next small file.
      *
      * <pre>
      *       |  ->
      *       V
      ...
      * </pre>
      *
      * <p>If all the small files are full assigned, a flag {@code noSpace} was marked to true, and we
      * can return early for future check.
      */
    ...
    /**
      * Candidate bucket state for small file. It records the total number of records that the bucket
      * can append and the current number of assigned records.
      */
    ...
    /**
      * Candidate bucket state for a new file. It records the total number of records that the bucket
      * can append and the current number of assigned records.
      */
