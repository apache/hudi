...
/**
  * Tool class used to perform supported casts from a {@link LogicalType} to another as defined in
  * Hudi Comprehensive Schema Evolution's scope.
  */
@Internal
public class TypeConverters {
    ...
    public static TypeConverter getInstance(LogicalType fromType, LogicalType toType) {
        ...
        switch (to) {
                        ...
                        return new TypeConverter() {
                            ...
                            public Object convert(Object val) {
                                return new BinaryStringData(
                                        LocalDate.ofEpochDay(((Integer) val).longValue()).toString());
                            }
                        };
                    }
                    break;
            ...
            case ROW:
                {
                    if (from == ROW) {
                        // Assumption: InternalSchemaManager should produce a cast that is of the
                        // same size
                        try {
                            ...
                            // ignoring error messages here as the calling function's checked
                            // exception will ignore it
                            ValidationUtils.checkArgument(
                                    fromType.getChildren().size() == toType.getChildren().size());
    ...
    private static TypeConverter createMapConverter(LogicalType fromType, LogicalType toType) {
        // no schema evolution is allowed on the keyType, hence, we only need to care about the
        // valueType
    ...
    private static TypeConverter createRowConverter(LogicalType fromType, LogicalType toType) {
        ...
        final RowData.FieldGetter[] fieldGetters =
                IntStream.range(0, fromChildren.size())
    ...
    private static boolean isPrimitiveTypeRootEqual(
            LogicalTypeRoot fromType, LogicalTypeRoot toType) {
    ...
    /**
      ...
      * @param decimalType DecimalType containing the output decimal's precision and scale
      *     specifications
