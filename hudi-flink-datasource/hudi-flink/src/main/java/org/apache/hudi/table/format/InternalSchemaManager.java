...
/**
  ...
  * point in time. If field is renamed in queried schema, its old name will be returned, which is
  * relevant at the provided time. If type of field is changed, its old type will be returned, and
  * projection will be created that will convert the old type to the queried one.
  */
public class InternalSchemaManager implements Serializable {
    ...
    public static final InternalSchemaManager DISABLED =
            new InternalSchemaManager(null, InternalSchema.getEmptyInternalSchema(), null, null);
    ...
    public static InternalSchemaManager get(Configuration conf, HoodieTableMetaClient metaClient) {
        ...
        return new InternalSchemaManager(
                conf, internalSchema.get(), validCommits, metaClient.getBasePath().toString());
    }

    public InternalSchemaManager(
            Configuration conf, InternalSchema querySchema, String validCommits, String tablePath) {
        this.conf = conf;
    ...
    /**
      * Attempts to merge the file and query schema to produce a mergeSchema, prioritising the use of
      * fileSchema types. An emptySchema is returned if:
      *
      * <ul>
      *   <li>1. An empty querySchema is provided
      *   <li>2. querySchema is equal to fileSchema
      * </ul>
      *
    ...
    /**
      * This method returns a mapping of columns that have type inconsistencies between the mergeSchema
      * and querySchema. This is done by:
      * <li>1. Finding the columns with type changes
      * <li>2. Get a map storing the index of these columns with type changes; Map of ->
      *     (colIdxInQueryFieldNames, colIdxInQuerySchema)
      * <li>3. For each selectedField with type changes, build a castMap containing the cast/conversion
      *     details; Map of -> (selectedPos, Cast([from] fileType, [to] queryType))
      *
      * @param mergeSchema InternalSchema representation of mergeSchema (prioritise use of
      *     fileSchemaType) that is used for reading base parquet files
      ...
      * @param selectedFields array containing the index of the columns of interest required (indexes
      *     are based on queryFieldNames and queryFieldTypes)
      * @return a castMap containing the information of how to cast a selectedField from the fileType
      *     to queryType.
      * @see CastMap
      */
    CastMap getCastMap(
        ...
        // map storing the indexes of columns with type changes Map of -> (colIdxInQueryFieldNames,
        // colIdxInQuerySchema)
    ...
    /**
      * For columns that have been modified via the column renaming operation, the column name might be
      * inconsistent between querySchema and mergeSchema.
      *
      * <p>As such, this method will identify all columns that have been renamed, and return a string
      * array of column names corresponding to the column names found in the mergeSchema.
      *
      * <p>This is done by:
      * <li>1. Get the rename mapping of -> (colNameFromNewSchema, colNameLastPartFromOldSchema)
      * <li>2. For columns that have been renamed, replace them with the old column name
      *
      * @param mergeSchema InternalSchema representation of mergeSchema (prioritise use of
      *     fileSchemaType) that is used for reading base parquet files
      * @param queryFieldNames array containing the columns of a Hudi Flink table
      * @return String array containing column names corresponding to the column names found in the
      *     mergeSchema
