...
public class MetadataCompactionPlanHandler extends CompactionPlanHandler {
   ...
   */
  @Override
  public void scheduleCompaction(long checkpointId, FlinkCompactionMetrics compactionMetrics, Output<StreamRecord<CompactionPlanEvent>> output) {
    ...
    if (instantAndPlanOpt.isPresent()) {
      collectLogCompaction(instantAndPlanOpt.get().getLeft(), instantAndPlanOpt.get().getRight(), output);
    }
  }
   ...
   * @param output                Output stream for emitting compaction plan events
   */
  public void collectLogCompaction(
