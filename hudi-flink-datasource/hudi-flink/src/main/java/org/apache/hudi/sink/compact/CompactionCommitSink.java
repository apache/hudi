...
/**
  ...
  *
  * <p>Each time after receiving a compaction commit event {@link CompactionCommitEvent}, it loads
  * and checks the compaction plan {@link HoodieCompactionPlan}, if all the compaction operations
  * {@link org.apache.hudi.common.model.CompactionOperation} of the plan are finished, tries to
  * commit the compaction action.
  *
...
public class CompactionCommitSink extends CleanFunction<CompactionCommitEvent> {
    ...
    /**
      ...
      *
      * <p>Stores the mapping of instant_time -> file_id -> event. Use a map to collect the events
      * because the rolling back of intermediate compaction tasks generates corrupt events.
      */
    private transient Map<String, Map<String, CompactionCommitEvent>> commitBuffer;

    /**
      * Cache to store compaction plan for each instant. Stores the mapping of instant_time ->
      * compactionPlan.
      */
    ...
    public void invoke(CompactionCommitEvent event, Context context) throws Exception {
        ...
        commitBuffer.computeIfAbsent(instant, k -> new HashMap<>()).put(event.getFileId(), event);
        commitIfNecessary(instant, commitBuffer.get(instant).values());
    }

    /**
      * Condition to commit: the commit buffer has equal size with the compaction plan operations and
      * all the compact commit event {@link CompactionCommitEvent} has the same compaction instant
      * time.
      *
      * @param instant Compaction commit instant time
      * @param events Commit events ever received for the instant
      */
    private void commitIfNecessary(String instant, Collection<CompactionCommitEvent> events)
            throws IOException {
        HoodieCompactionPlan compactionPlan =
                compactionPlanCache.computeIfAbsent(
                        instant,
                        k -> {
                            try {
    ...
    private void doCommit(String instant, Collection<CompactionCommitEvent> events)
            throws IOException {
        ...
        if (numErrorRecords > 0 && !this.conf.getBoolean(FlinkOptions.IGNORE_FAILED)) {
            // handle failure case
            LOG.error(
                    "Got {} error records during compaction of instant {},\n"
                            + "option '{}' is configured as false,"
                            + "rolls back the compaction",
                    numErrorRecords,
                    instant,
