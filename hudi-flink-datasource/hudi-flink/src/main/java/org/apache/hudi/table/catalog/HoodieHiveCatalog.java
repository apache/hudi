...
public class HoodieHiveCatalog extends AbstractCatalog {
    ...
    public HoodieHiveCatalog(String catalogName, Configuration options) {
        this(
                catalogName,
                options,
                HoodieCatalogUtil.createHiveConf(options.getString(CatalogOptions.HIVE_CONF_DIR), options),
                false);
    }

    public HoodieHiveCatalog(
            String catalogName, Configuration options, HiveConf hiveConf, boolean allowEmbedded) {
        ...
        this.catalogPath =
                options.getString(
                        CatalogOptions.CATALOG_PATH, hiveConf.getVar(HiveConf.ConfVars.METASTOREWAREHOUSE));
    ...
    public void open() throws CatalogException {
        ...
        if (!databaseExists(getDefaultDatabase())) {
            ...
            CatalogDatabase database =
                    new CatalogDatabaseImpl(Collections.emptyMap(), "default database");
            try {
    ...
    public void createDatabase(String databaseName, CatalogDatabase database, boolean ignoreIfExists)
        ...
        checkArgument(!isNullOrWhitespaceOnly(databaseName), "Database name can not null or empty");
    ...
    public void alterDatabase(
            ...
            throws DatabaseNotExistException, CatalogException {
        checkArgument(!isNullOrWhitespaceOnly(databaseName), "Database name cannot be null or empty");
    ...
    private Table isHoodieTable(Table hiveTable) {
        if (!hiveTable.getParameters().getOrDefault(SPARK_SOURCE_PROVIDER, "").equalsIgnoreCase("hudi")
            ...
            throw new HoodieCatalogException(
                    String.format("Table %s is not a hoodie table", hiveTable.getTableName()));
        }
        return hiveTable;
    }
    ...
    public Table getHiveTable(ObjectPath tablePath) throws TableNotExistException {
        try {
            ...
            throw new HoodieCatalogException(
                    String.format("Failed to get table %s from Hive metastore", tablePath.getObjectName()),
                    e);
        }
    }
            ...
            try {
                ...
                if (!parameters.containsKey(FlinkOptions.HIVE_STYLE_PARTITIONING.key())) {
                    ...
                    if (tableConfig.isPresent()
                        ...
                        hiveStyle =
                                Arrays.stream(
                                                HadoopFSUtils.getFs(hoodieTablePath, hiveConf).listStatus(hoodieTablePath))
    ...
    public CatalogBaseTable getTable(ObjectPath tablePath)
        ...
        if (latestTableSchema != null) {
            ...
            List<String> pkColumns =
                    StringUtils.isNullOrEmpty(pkColumnsStr) ? null : StringUtils.split(pkColumnsStr, ",");
            ...
            LOG.warn(
                    "{} does not have any hoodie schema, and use hive table schema to infer the table schema",
                    tablePath);
    ...
    public void createTable(ObjectPath tablePath, CatalogBaseTable table, boolean ignoreIfExists)
        ...
        if (!table
                .getOptions()
                .getOrDefault(CONNECTOR.key(), "")
                .equalsIgnoreCase(HoodieTableFactory.FACTORY_ID)) {
            throw new HoodieCatalogException(
                    String.format(
                            "Unsupported connector identity %s, supported identity is %s",
        ...
        try {
            ...
            Table hiveTable =
                    instantiateHiveTable(tablePath, table, inferTablePath(tablePath, table), isMorTable);
            // create hive table
    ...
    public String inferTablePath(ObjectPath tablePath, CatalogBaseTable table) {
        ...
        if (StringUtils.isNullOrEmpty(location)) {
            try {
                Path dbLocation =
                        new Path(client.getDatabase(tablePath.getDatabaseName()).getLocationUri());
                location = new Path(dbLocation, tablePath.getObjectName()).toString();
            } catch (TException e) {
                throw new HoodieCatalogException(
                        String.format("Failed to infer hoodie table path for table %s", tablePath), e);
            }
        }
        return location;
    }

    private Table instantiateHiveTable(
            ObjectPath tablePath, CatalogBaseTable table, String location, boolean useRealTimeInputFormat)
            throws IOException {
        ...
        if (properties.containsKey(FlinkOptions.INDEX_TYPE.key())
            ...
            properties.put(
                    HoodieIndexConfig.INDEX_TYPE.key(), properties.get(FlinkOptions.INDEX_TYPE.key()));
        }
        ...
        if (table.getUnresolvedSchema().getPrimaryKey().isPresent()
            ...
            String pkColumns =
                    String.join(",", table.getUnresolvedSchema().getPrimaryKey().get().getColumnNames());
            properties.put(
                    PK_CONSTRAINT_NAME,
        ...
        boolean withOperationField =
                Boolean.parseBoolean(
                        table.getOptions().getOrDefault(FlinkOptions.CHANGELOG_ENABLED.key(), "false"));
        ...
        serdeProperties.putAll(
                TableOptionProperties.translateFlinkTableProperties2Spark(
                        catalogTable, hiveConf, properties, partitionKeys, withOperationField));
    ...
    public void renameTable(ObjectPath tablePath, String newTableName, boolean ignoreIfNotExists)
        ...
        checkNotNull(tablePath, "Table path cannot be null");
        checkArgument(!isNullOrWhitespaceOnly(newTableName), "New table name cannot be null or empty");
                ...
                if (tableExists(newPath)) {
                    ...
                    HoodieTableMetaClient metaClient =
                            HoodieTableMetaClient.builder()
                                    ...
                                    .setConf(HadoopFSUtils.getStorageConfWithCopy(hiveConf))
                                    .build();
    ...
    public void dropPartition(
        ...
        try (HoodieFlinkWriteClient<?> writeClient =
                HoodieCatalogUtil.createWriteClient(tablePath, table, hiveConf, this::inferTablePath)) {
            boolean hiveStylePartitioning =
                    Boolean.parseBoolean(table.getOptions().get(FlinkOptions.HIVE_STYLE_PARTITIONING.key()));
            writeClient
                    .deletePartitions(
                            Collections.singletonList(
                                    HoodieCatalogUtil.inferPartitionPath(hiveStylePartitioning, partitionSpec)),
            ...
            client.dropPartition(
                    ...
                    HoodieCatalogUtil.getOrderedPartitionValues(
                            ...
                            getHiveConf(),
                            partitionSpec,
                            ((CatalogTable) table).getPartitionKeys(),
                            tablePath),
                    true);
        } catch (NoSuchObjectException e) {
            if (!ignoreIfNotExists) {
    ...
    private void refreshHMSTable(ObjectPath tablePath, CatalogBaseTable newCatalogTable) {
        try {
            ...
            Table hiveTable =
                    instantiateHiveTable(
                            tablePath, newCatalogTable, inferTablePath(tablePath, newCatalogTable), isMorTable);
            // alter hive table
            client.alter_table(tablePath.getDatabaseName(), tablePath.getObjectName(), hiveTable);
        } catch (Exception e) {
            LOG.error("Failed to alter table {}", tablePath.getObjectName(), e);
            throw new HoodieCatalogException(
                    String.format("Failed to alter table %s", tablePath.getObjectName()), e);
        }
    }
    ...
    private Map<String, String> supplementOptions(ObjectPath tablePath, Map<String, String> options) {
        if (HoodieCatalogUtil.isEmbeddedMetastore(hiveConf)) {
            ...
            newOptions.computeIfAbsent(
                    FlinkOptions.HIVE_SYNC_TABLE.key(), k -> tablePath.getObjectName());
            return newOptions;
        }
    }
