...
public class StreamerUtil {
    ...
    public static Schema getSourceSchema(org.apache.flink.configuration.Configuration conf) {
        if (conf.getOptional(FlinkOptions.SOURCE_AVRO_SCHEMA_PATH).isPresent()) {
            ...
            final String errorMsg =
                    String.format(
                            "Either option '%s' or '%s' " + "should be specified for avro schema deserialization",
        ...
        if (OptionsResolver.isLockRequired(conf)
            ...
            return Option.of(
                    HoodieLockConfig.newBuilder()
                            .fromProperties(
                                    FileSystemBasedLockProvider.getLockConfig(conf.getString(FlinkOptions.PATH)))
    ...
    /**
      * Converts the give {@link Configuration} to {@link TypedProperties}. The default values are also
      * set up.
      *
    ...
    public static HoodieTableMetaClient initTableIfNotExists(
            Configuration conf, org.apache.hadoop.conf.Configuration hadoopConf) throws IOException {
        ...
        if (!tableExists(basePath, hadoopConf)) {
            HoodieTableMetaClient.withPropertyBuilder()
                    ...
                    .setKeyGeneratorClassProp(
                            conf.getOptional(FlinkOptions.KEYGEN_CLASS_NAME)
                                    .orElse(SimpleAvroKeyGenerator.class.getName()))
            ...
            LOG.info(
                    "Table [path={}, name={}] already exists, no need to initialize the table",
                    basePath,
    ...
    public static boolean tableExists(
            String basePath, org.apache.hadoop.conf.Configuration hadoopConf) {
        // Hadoop FileSystem
    ...
    /**
      * Returns whether the hoodie partition exists under given table path {@code tablePath} and
      * partition path {@code partitionPath}.
      *
    ...
    public static boolean partitionExists(
            String tablePath, String partitionPath, org.apache.hadoop.conf.Configuration hadoopConf) {
        // Hadoop FileSystem
        ...
        try {
            ...
            throw new HoodieException(
                    String.format(
                            "Error while checking whether partition exists under table path [%s] and partition path [%s]",
                            tablePath, partitionPath),
                    e);
        }
    }
    ...
    /**
      ...
      *
      * <p>The streaming pipeline process is long-running, so empty table path is allowed, the reader
      * would then check and refresh the meta client.
      *
      * @see org.apache.hudi.source.StreamReadMonitoringFunction
      */
    public static HoodieTableMetaClient metaClientForReader(
            Configuration conf, org.apache.hadoop.conf.Configuration hadoopConf) {
        final String basePath = conf.getString(FlinkOptions.PATH);
    ...
    public static Option<HoodieTableConfig> getTableConfig(
            String basePath, org.apache.hadoop.conf.Configuration hadoopConf) {
        HoodieStorage storage =
                HoodieStorageUtils.getStorage(basePath, HadoopFSUtils.getStorageConf(hadoopConf));
    ...
    public static Option<String> medianInstantTime(String highVal, String lowVal) {
        long high =
                HoodieActiveTimeline.parseDateFromInstantTimeSafely(highVal)
                        .orElseThrow(
                                () ->
                                        new HoodieException(
                                                "Get instant time diff with interval [" + highVal + "] error"))
                        .getTime();
        long low =
                HoodieActiveTimeline.parseDateFromInstantTimeSafely(lowVal)
                        .orElseThrow(
                                () ->
                                        new HoodieException(
                                                "Get instant time diff with interval [" + lowVal + "] error"))
                        .getTime();
        ...
        if (HoodieTimeline.compareTimestamps(lowVal, HoodieTimeline.GREATER_THAN_OR_EQUALS, instantTime)
                || HoodieTimeline.compareTimestamps(
                        highVal, HoodieTimeline.LESSER_THAN_OR_EQUALS, instantTime)) {
            return Option.empty();
        }
        ...
        long newTimestamp =
                HoodieActiveTimeline.parseDateFromInstantTimeSafely(newInstantTime)
                        .orElseThrow(
                                () ->
                                        new HoodieException(
                                                "Get instant time diff with interval ["
                                                        + oldInstantTime
                                                        + ", "
                                                        + newInstantTime
                                                        + "] error"))
    ...
    public static Option<Transformer> createTransformer(List<String> classNames) throws IOException {
        try {
            ...
            return transformers.isEmpty()
                    ? Option.empty()
                    : Option.of(new ChainedTransformer(transformers));
        } catch (Throwable e) {
    ...
    /**
      * Returns whether the give file is in valid hoodie format. For example, filtering out the empty
      * or corrupt files.
      */
    ...
    public static String getLastPendingInstant(
            HoodieTableMetaClient metaClient, boolean reloadTimeline) {
        if (reloadTimeline) {
    ...
    public static Schema getTableAvroSchema(
            HoodieTableMetaClient metaClient, boolean includeMetadataFields) throws Exception {
        TableSchemaResolver schemaUtil = new TableSchemaResolver(metaClient);
        return schemaUtil.getTableAvroSchema(includeMetadataFields);
    }

    public static Schema getLatestTableSchema(
            String path, org.apache.hadoop.conf.Configuration hadoopConf) {
    ...
    /**
      ...
      */
    public static boolean isWriteCommit(
            HoodieTableType tableType, HoodieInstant instant, HoodieTimeline timeline) {
