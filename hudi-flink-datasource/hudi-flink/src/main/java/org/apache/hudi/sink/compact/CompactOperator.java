...
/**
  * Operator to execute the actual compaction task assigned by the compaction plan task. In order to
  * execute scalable, the input should shuffle by the compact event {@link CompactionPlanEvent}.
  */
public class CompactOperator extends TableStreamOperator<CompactionCommitEvent>
    ...
    public void setup(
            ...
            StreamConfig config,
            Output<StreamRecord<CompactionCommitEvent>> output) {
    ...
    public void processElement(StreamRecord<CompactionPlanEvent> record) throws Exception {
        ...
        if (asyncCompaction) {
            ...
            executor.execute(
                    ...
                    "Execute compaction for instant %s from task %d",
                    instantTime,
                    taskID);
        } else {
    ...
    private void doCompaction(
        ...
        List<WriteStatus> writeStatuses =
                compactor.compact(
                        new HoodieFlinkCopyOnWriteTable<>(
                                writeConfig, writeClient.getEngineContext(), metaClient),
                        metaClient,
        ...
        collector.collect(
                new CompactionCommitEvent(
                        instantTime, compactionOperation.getFileId(), writeStatuses, taskID));
    }
