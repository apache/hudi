...
/**
  * DeltaWriteProfile for MERGE_ON_READ table type, this allows auto correction of small parquet
...
public class DeltaWriteProfile extends WriteProfile {
    ...
    protected List<SmallFile> smallFilesProfile(String partitionPath) {
        ...
        HoodieTimeline commitTimeline =
                metaClient.getCommitsTimeline().filterCompletedAndCompactionInstants();
        ...
        if (!commitTimeline.empty()) {
            ...
            // If we can index log files, we can add more inserts to log files for fileIds including
            // those under
            // pending compaction.
            List<FileSlice> allFileSlices =
                    fsView
                            .getLatestMergedFileSlicesBeforeOrOn(partitionPath, latestCommitTime.getTimestamp())
            ...
            for (FileSlice smallFileSlice : allSmallFileSlices) {
                ...
                if (smallFileSlice.getBaseFile().isPresent()) {
                    ...
                    smallFileSlice
                            ...
                            .ifPresent(
                                    logFile -> {
                                        // in case there is something error, and the file slice has
                                        // no log file
                                        sf.location =
                                                new HoodieRecordLocation(logFile.getDeltaCommitTime(), logFile.getFileId());
                                        sf.sizeBytes = getTotalFileSize(smallFileSlice);
                                        smallFileLocations.add(sf);
                                    });
                }
