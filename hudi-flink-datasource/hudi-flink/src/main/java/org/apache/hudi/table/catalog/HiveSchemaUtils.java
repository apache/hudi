...
public class HiveSchemaUtils {
    ...
    public static org.apache.flink.table.api.Schema convertTableSchema(Table hiveTable) {
        ...
        List<String> pkColumns =
                pkColumnStr == null ? new ArrayList<>() : StringUtils.split(pkColumnStr, ",");
        ...
        for (int i = 0; i < allCols.size(); i++) {
            ...
            colTypes[i] = toFlinkType(TypeInfoUtils.getTypeInfoFromTypeString(fs.getType()));
    ...
    public static List<FieldSchema> toHiveFieldSchema(
            TableSchema schema, boolean withOperationField) {
    ...
    private static List<FieldSchema> createHiveColumns(TableSchema schema) {
        ...
        for (int i = 0; i < fieldNames.length; i++) {
            columns.add(
                    new FieldSchema(fieldNames[i], toHiveTypeInfo(fieldTypes[i]).getTypeName(), null));
        }
    ...
    /**
      * Convert Flink DataType to Hive TypeInfo. For types with a precision parameter, e.g. timestamp,
      * the supported precisions in Hive and Flink can be different. Therefore the conversion will fail
      * for those types if the precision is not supported by Hive and checkPrecision is true.
      *
    ...
    public static Pair<List<FieldSchema>, List<FieldSchema>> splitSchemaByPartitionKeys(
            List<FieldSchema> fieldSchemas, List<String> partitionKeys) {
