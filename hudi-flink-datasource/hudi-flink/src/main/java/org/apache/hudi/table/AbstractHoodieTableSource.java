 ...
 */
public abstract class AbstractHoodieTableSource implements
    ScanTableSource,
    SupportsProjectionPushDown,
    SupportsLimitPushDown,
    SupportsFilterPushDown {
  private static final Logger LOG = LoggerFactory.getLogger(AbstractHoodieTableSource.class);

  private static final int NO_LIMIT_CONSTANT = -1;

  private final transient org.apache.hadoop.conf.Configuration hadoopConf;
  protected final transient HoodieTableMetaClient metaClient;
  ...
  protected int[] requiredPos;
  protected long limit;

  public AbstractHoodieTableSource(
      ...
      public DataStream<RowData> produceDataStream(StreamExecutionEnvironment execEnv) {
        ...
        if (conf.getBoolean(FlinkOptions.READ_AS_STREAMING)) {
          StreamReadMonitoringFunction monitoringFunction = new StreamReadMonitoringFunction(
              conf, FilePathUtils.toFlinkPath(path), tableRowType, maxCompactionMemoryInBytes, getPartitionPruner());
  ...
  @Nullable protected abstract PartitionPruner getPartitionPruner();

  private List<MergeOnReadInputSplit> buildFileIndex() {
    PartitionPruner partitionPruner = getPartitionPruner();
    fileIndex.setPartitionPruner(partitionPruner);
    ...
    switch (queryType) {
      ...
      case FlinkOptions.QUERY_TYPE_INCREMENTAL:
        IncrementalInputSplits incrementalInputSplits = IncrementalInputSplits.builder()
            ...
            .partitionPruner(getPartitionPruner()).build();
