...
public class AppendWriteFunctionWithBufferSort<T> extends AppendWriteFunction<T> {
  ...
  private final GeneratedNormalizedKeyComputer keyComputer;
  private final GeneratedRecordComparator recordComparator;
  private transient BinaryInMemorySortBuffer buffer;
  private transient MemorySegmentPool memorySegmentPool;
  private transient SortOperatorGen sortOperatorGen;
  ...
  public AppendWriteFunctionWithBufferSort(Configuration config, RowType rowType) {
    ...
    this.sortKeyList = Arrays.stream(sortKeys.split(",")).map(key -> key.trim()).collect(Collectors.toList());
    this.sortOperatorGen = new SortOperatorGen(rowType, sortKeyList.toArray(new String[0]));
  ...
  public void open(Configuration parameters) throws Exception {
    ...
    this.buffer = BufferUtils.createBuffer(rowType,
            memorySegmentPool,
            this.keyComputer.newInstance(Thread.currentThread().getContextClassLoader()),
            this.recordComparator.newInstance(Thread.currentThread().getContextClassLoader()));
  }
  ...
  public void processElement(T value, Context ctx, Collector<RowData> out) throws Exception {
    ...
    // If write result is false or buffer hit size limit
    if (!result || buffer.size() >= writeBufferSize) {
      sortAndSend();
    }
  }
   ...
   *  2. Binary buffer is full. set the size of buffer as the max size of parquet file,
   *     e.g., PARQUET_MAX_FILE_SIZE, to keep aligned with current behavior considering the file's size / quantity.
   *
   * @throws IOException
   */
