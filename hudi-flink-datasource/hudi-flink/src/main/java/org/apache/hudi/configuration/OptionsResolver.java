...
public class OptionsResolver {
    ...
    public static boolean insertClustering(Configuration conf) {
        return isCowTable(conf)
                && isInsertOperation(conf)
                && conf.getBoolean(FlinkOptions.INSERT_CLUSTER);
    }
    ...
    public static boolean isAppendMode(Configuration conf) {
        ...
        return isInsertOperation(conf)
                && ((isCowTable(conf) && !conf.getBoolean(FlinkOptions.INSERT_CLUSTER))
                        || isMorTable(conf));
    }
    ...
    public static boolean isMorTable(Map<String, String> options) {
        return options
                .getOrDefault(FlinkOptions.TABLE_TYPE.key(), FlinkOptions.TABLE_TYPE.defaultValue())
                .equalsIgnoreCase(FlinkOptions.TABLE_TYPE_MERGE_ON_READ);
    }
    ...
    public static boolean isDefaultHoodieRecordPayloadClazz(Configuration conf) {
        return conf.getString(FlinkOptions.PAYLOAD_CLASS_NAME)
                .contains(DefaultHoodieRecordPayload.class.getSimpleName());
    }

    /**
      * Returns the preCombine field or null if the value is set as {@link
    ...
    public static boolean isDeltaTimeCompaction(Configuration conf) {
        final String strategy =
                conf.getString(FlinkOptions.COMPACTION_TRIGGER_STRATEGY).toLowerCase(Locale.ROOT);
    ...
    public static boolean isBucketIndexType(Configuration conf) {
        return conf.getString(FlinkOptions.INDEX_TYPE)
    ...
    public static boolean emitChangelog(Configuration conf) {
        return conf.getBoolean(FlinkOptions.READ_AS_STREAMING)
                ...
                || conf.getBoolean(FlinkOptions.READ_AS_STREAMING)
    ...
    public static boolean needsScheduleClustering(Configuration conf) {
        ...
        if (OptionsResolver.isConsistentHashingBucketIndexType(conf)) {
            // Write pipelines for table with consistent bucket index would detect whether
            // clustering service occurs,
            // and automatically adjust the partitioner and write function if clustering service
            // happens.
            // So it could handle UPSERT.
            // But it could not handle INSERT case, because insert write would not take index into
    ...
    public static boolean isInsertOverwrite(Configuration conf) {
        return conf.getString(FlinkOptions.OPERATION)
                        .equalsIgnoreCase(WriteOperationType.INSERT_OVERWRITE_TABLE.value())
                || conf.getString(FlinkOptions.OPERATION)
                        .equalsIgnoreCase(WriteOperationType.INSERT_OVERWRITE.value());
    }
    ...
    public static boolean overwriteDynamicPartition(Configuration conf) {
        return conf.getString(FlinkOptions.OPERATION)
                        .equalsIgnoreCase(WriteOperationType.INSERT_OVERWRITE.value())
                || conf.getString(FlinkOptions.WRITE_PARTITION_OVERWRITE_MODE)
                        .equalsIgnoreCase(PartitionOverwriteMode.DYNAMIC.name());
    }
    ...
    public static boolean isSpecificStartCommit(Configuration conf) {
        return conf.getOptional(FlinkOptions.READ_START_COMMIT).isPresent()
                && !conf.get(FlinkOptions.READ_START_COMMIT)
                        .equalsIgnoreCase(FlinkOptions.START_COMMIT_EARLIEST);
    }
    ...
    public static boolean isMultiWriter(Configuration conf) {
        return WriteConcurrencyMode.supportsMultiWriter(
                conf.getString(
    ...
    /**
      ...
      *
      * <p>A Hudi instant contains both the txn start time and completion time, for incremental
      * subscription of the source reader, using completion time to filter the candidate instants can
      * avoid data loss in scenarios like multiple writers.
      */
    ...
    /**
      * Returns whether to read the data changes only from changelog files. When CDC is enabled, i) for
      * COW table, the changelog is generated on each file update; ii) for MOR table, the changelog is
      * generated on compaction.
      *
      * <p>By default, always read from the changelog file, once it is disabled, the reader would infer
      * the changes based on the file slice dependencies.
      */
    ...
    public static String getIndexKeyField(Configuration conf) {
        return conf.getString(
                FlinkOptions.INDEX_KEY_FIELD, conf.getString(FlinkOptions.RECORD_KEY_FIELD));
    }
    ...
    public static boolean allowCommitOnEmptyBatch(Configuration conf) {
        return conf.getBoolean(
                HoodieWriteConfig.ALLOW_EMPTY_COMMIT.key(),
    ...
    public static boolean isLazyFailedWritesCleanPolicy(Configuration conf) {
        ...
        return HoodieCleanConfig.FAILED_WRITES_CLEANER_POLICY
                .defaultValue()
                .equalsIgnoreCase(HoodieFailedWritesCleaningPolicy.LAZY.name());
    }
    ...
    public static boolean isOnlyConsumingNewCommits(Configuration conf) {
        return isMorTable(conf)
                        && conf.getBoolean(FlinkOptions.READ_STREAMING_SKIP_COMPACT) // this is only true for
                // flink.
