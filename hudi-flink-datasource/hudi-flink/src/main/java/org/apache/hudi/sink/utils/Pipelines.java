...
public class Pipelines {
    ...
    private static final ConcurrentHashMap<String, Integer> OPERATOR_COUNTERS =
            new ConcurrentHashMap<>();

    /**
      * Bulk insert the input dataset at once.
      *
      * <p>By default, the input dataset would shuffle by the partition path first then sort by the
      * partition path before passing around to the write function. The whole pipeline looks like the
      * following:
      *
      * <pre>
      ...
      * </pre>
      *
      * <p>The write task switches to new file handle each time it receives a record from the different
      * partition path, the shuffle and sort would reduce small files.
      *
      ...
      */
    public static DataStreamSink<Object> bulkInsert(
            Configuration conf, RowType rowType, DataStream<RowData> dataStream) {
        ...
        if (OptionsResolver.isBucketIndexType(conf)) {
            ...
            BucketIndexPartitioner<HoodieKey> partitioner =
                    new BucketIndexPartitioner<>(numBuckets, indexKeys);
            ...
            dataStream =
                    dataStream
                            .partitionCustom(partitioner, keyGen::getHoodieKey)
                            .map(
                                    record ->
                                    ...
                                    typeInfo)
                            .setParallelism(
                                    conf.getInteger(FlinkOptions.WRITE_TASKS)); // same parallelism as write
            // task to avoid shuffle
            if (conf.getBoolean(FlinkOptions.WRITE_BULK_INSERT_SORT_INPUT)) {
                ...
                dataStream =
                        dataStream
                                .transform("file_sorter", typeInfo, sortOperatorGen.createSortOperator(conf))
                                .setParallelism(
                                        conf.getInteger(FlinkOptions.WRITE_TASKS)); // same parallelism as write
                // task to avoid shuffle
            ...
            return dataStream
                    .transform(
                            opName("bucket_bulk_insert", conf), TypeInformation.of(Object.class), operatorFactory)
        ...
        return dataStream
                .transform(
                        opName("hoodie_bulk_insert_write", conf),
    ...
    /**
      ...
      *
      * <p>The write task switches to new file handle each time it receives a record from the different
      * partition path, so there may be many small files.
      *
      * @param conf The configuration
      * @param rowType The input row type
      * @param dataStream The input data stream
    ...
    public static DataStream<Object> append(
        ...
        return dataStream
                .transform(
                        opName("hoodie_append_write", conf), TypeInformation.of(Object.class), operatorFactory)
                .uid(opUID("hoodie_stream_write", conf))
                .setParallelism(conf.getInteger(FlinkOptions.WRITE_TASKS));
    }

    /**
      * Constructs bootstrap pipeline as streaming. The bootstrap operator loads the existing data
      * index (primary key to file id mapping), then sends the indexing data set to subsequent
      * operator(usually the bucket assign operator).
      */
    ...
    /**
      * Constructs bootstrap pipeline. The bootstrap operator loads the existing data index (primary
      * key to file id mapping), then send the indexing data set to subsequent operator(usually the
    ...
    private static DataStream<HoodieRecord> streamBootstrap(
            Configuration conf, RowType rowType, DataStream<RowData> dataStream, boolean bounded) {
    ...
    /**
      * Constructs bootstrap pipeline for batch execution mode. The indexing data set is loaded before
      * the actual data write in order to support batch UPSERT.
      */
    ...
    public static DataStream<HoodieRecord> rowDataToHoodieRecord(
            Configuration conf, RowType rowType, DataStream<RowData> dataStream) {
        return dataStream
    ...
    /**
      ...
      *
      * <p>The input dataset shuffles by the primary key first then shuffles by the file group ID
      * before passing around to the write function. The whole pipeline looks like the following:
      *
      * <pre>
      ...
      * </pre>
      *
      * <p>The bucket assigner assigns the inputs to suitable file groups, the write task caches and
      * flushes the data set to disk.
      *
      * @param conf The configuration
      * @param dataStream The input data stream
      * @return the stream write data stream pipeline
      */
    public static DataStream<Object> hoodieStreamWrite(
            Configuration conf, DataStream<HoodieRecord> dataStream) {
        if (OptionsResolver.isBucketIndexType(conf)) {
            ...
            switch (bucketIndexEngineType) {
                case SIMPLE:
                    ...
                    BucketIndexPartitioner<HoodieKey> partitioner =
                            new BucketIndexPartitioner<>(bucketNum, indexKeyFields);
                    return dataStream
                            .partitionCustom(partitioner, HoodieRecord::getKey)
                            .transform(
                ...
                case CONSISTENT_HASHING:
                    if (OptionsResolver.isInsertOverwrite(conf)) {
                        ...
                        throw new HoodieException(
                                "Consistent hashing bucket index does not work with insert overwrite using FLINK engine. Use simple bucket index or Spark engine.");
                    }
                    return dataStream
                            .transform(
            ...
            return dataStream
                    // Key-by record key, to avoid multiple subtasks write to a bucket at the same
                    // time
                    ...
                    .keyBy(record -> record.getCurrentLocation().getFileId())
                    .transform(
                            opName("stream_write", conf), TypeInformation.of(Object.class), operatorFactory)
    ...
    /**
      ...
      *
      * <p>The compaction plan operator monitors the new compaction plan on the timeline then
      * distributes the sub-plans to the compaction tasks. The compaction task then handle over the
      * metadata to commit task for compaction transaction commit. The whole pipeline looks like the
      * following:
      *
      * <pre>
      *                                     /=== | task1 | ===\
      ...
      */
    public static DataStreamSink<CompactionCommitEvent> compact(
            Configuration conf, DataStream<Object> dataStream) {
        DataStreamSink<CompactionCommitEvent> compactionCommitEventDataStream =
                dataStream
                        .transform(
                                "compact_plan_generate",
    ...
    /**
      ...
      *
      * <p>The clustering plan operator monitors the new clustering plan on the timeline then
      * distributes the sub-plans to the clustering tasks. The clustering task then handle over the
      * metadata to commit task for clustering transaction commit. The whole pipeline looks like the
      * following:
      *
      * <pre>
      *                                     /=== | task1 | ===\
      ...
      */
    public static DataStreamSink<ClusteringCommitEvent> cluster(
            Configuration conf, RowType rowType, DataStream<Object> dataStream) {
        DataStream<ClusteringCommitEvent> clusteringStream =
                dataStream
                        .transform(
                                "cluster_plan_generate",
                        ...
                        .keyBy(
                                plan ->
                                        ...
                                        // on the same bucket files
                                        plan.getClusteringGroupInfo().getOperations().stream()
        ...
        if (OptionsResolver.sortClusteringEnabled(conf)) {
            ExecNodeUtil.setManagedMemoryWeight(
                    clusteringStream.getTransformation(),
                    conf.getInteger(FlinkOptions.WRITE_SORT_MEMORY) * 1024L * 1024L);
        }
    ...
    public static DataStreamSink<Object> clean(Configuration conf, DataStream<Object> dataStream) {
        DataStreamSink<Object> cleanCommitDataStream =
                dataStream.addSink(new CleanFunction<>(conf)).setParallelism(1).name("clean_commits");
    ...
    public static DataStreamSink<Object> dummySink(DataStream<Object> dataStream) {
        return dataStream.addSink(Pipelines.DummySink.INSTANCE).setParallelism(1).name("dummy");
    }
    ...
    public static String opUID(String operatorN, Configuration conf) {
        Integer operatorCount =
                OPERATOR_COUNTERS.merge(operatorN, 1, (oldValue, value) -> oldValue + value);
        return "uid_"
                + operatorN
                + (operatorCount == 1 ? "" : "_" + (operatorCount - 1))
                + "_"
                + getTablePath(conf);
    }
