...
public class PayloadCreation implements Serializable {
    ...
    private PayloadCreation(
            boolean shouldCombine, Constructor<?> constructor, @Nullable String preCombineField) {
    ...
    public static PayloadCreation instance(Configuration conf) throws Exception {
        ...
        boolean needCombine =
                conf.getBoolean(FlinkOptions.PRE_COMBINE)
    ...
    public HoodieRecordPayload<?> createPayload(GenericRecord record) throws Exception {
        if (shouldCombine) {
            ...
            Comparable<?> orderingVal =
                    (Comparable<?>) HoodieAvroUtils.getNestedFieldVal(record, preCombineField, false, false);
