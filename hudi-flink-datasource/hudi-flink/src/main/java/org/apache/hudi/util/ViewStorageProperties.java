...
public class ViewStorageProperties {
    ...
    public static void createProperties(
            String basePath, FileSystemViewStorageConfig config, Configuration flinkConf)
            throws IOException {
        Path propertyPath =
                getPropertiesFilePath(basePath, flinkConf.getString(FlinkOptions.WRITE_CLIENT_ID));
        ...
        try (OutputStream outputStream = fs.create(propertyPath)) {
            config
                    ...
                    .store(
                            outputStream,
                            "Filesystem view storage properties saved on "
                                    + new Date(System.currentTimeMillis()));
        }
    }

    /** Read the {@link FileSystemViewStorageConfig} with given table base path. */
    public static FileSystemViewStorageConfig loadFromProperties(
            String basePath, Configuration conf) {
        Path propertyPath =
                getPropertiesFilePath(basePath, conf.getString(FlinkOptions.WRITE_CLIENT_ID));
        ...
        try {
            ...
            throw new HoodieIOException(
                    "Could not load filesystem view storage properties from " + propertyPath, e);
        }
    }
