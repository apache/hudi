...
/**
  ...
  *
  * <p>This coordinator starts a new instant when a new checkpoint starts. It commits the instant
  * when all the operator tasks write the buffer successfully for a round of checkpoint.
  *
  * <p>If there is no data for a round of checkpointing, it resets the events buffer and returns
  * early.
  *
...
public class StreamWriteOperatorCoordinator implements OperatorCoordinatorAdapter {
    ...
    /**
      * Event buffer for one round of checkpointing. When all the elements are non-null and have the
      * same write instant, then the instant succeed and we can commit it.
      */
    ...
    /**
      ...
      */
    public StreamWriteOperatorCoordinator(Configuration conf, Context context) {
        this.conf = conf;
                    ...
                    try {
                        ...
                        result.completeExceptionally(
                                new CompletionException(
                                        String.format(
                                                "Failed to checkpoint Instant %s for source %s",
    ...
    public void notifyCheckpointComplete(long checkpointId) {
        executor.execute(
                () -> {
                    ...
                    // the stream write task snapshot and flush the data buffer synchronously in
                    // sequence,
                    // so a successful checkpoint subsumes the old one(follows the checkpoint
                    // subsuming contract)
                    final boolean committed = commitInstant(this.instant, checkpointId);
                    // schedules the compaction or clustering if it is enabled in stream execution
                    // mode
    ...
    public void handleEventFromOperator(int i, OperatorEvent operatorEvent) {
        ValidationUtils.checkState(
                operatorEvent instanceof WriteMetadataEvent,
        ...
        if (event.isEndInput()) {
            ...
            executor.executeSync(
                    () -> handleEndInputEvent(event), "handle end input event for instant %s", this.instant);
        } else {
            executor.execute(
                    () -> {
                    ...
                    "handle write metadata event for instant %s",
                    this.instant);
        }
    }
    ...
    private void syncHiveAsync() {
        if (tableState.syncHive) {
            this.hiveSyncExecutor.execute(
                    this::doSyncHive, "sync hive metadata for instant %s", this.instant);
        }
    }
    ...
    private static CkpMetadata initCkpMetadata(HoodieWriteConfig writeConfig, Configuration conf)
            throws IOException {
    ...
    private void startInstant() {
        ...
        this.metaClient
                .getActiveTimeline()
                .transitionRequestedToInflight(tableState.commitAction, this.instant);
        this.writeClient.setWriteTimer(tableState.commitAction);
        this.ckpMetadata.startInstant(this.instant);
        LOG.info(
                "Create instant [{}] for table [{}] with type [{}]",
                this.instant,
    ...
    /**
      ...
      *
      * <p>Recommits the last inflight instant if the write metadata checkpoint successfully but was
      * not committed due to some rare cases.
      *
      * <p>Starts a new instant, a writer can not flush data buffer until it finds a new inflight
    ...
    private void initInstant(String instant) {
        ...
        if (instant.equals(WriteMetadataEvent.BOOTSTRAP_INSTANT)
            ...
            // Recommit should start heartbeat for lazy failed writes clean policy to avoid aborting
            // for heartbeat expired.
    ...
    private void handleBootstrapEvent(WriteMetadataEvent event) {
        ...
        if (Arrays.stream(eventBuffer).allMatch(evt -> evt != null && evt.isBootstrap())) {
            ...
            if (this.metaClient
                            ...
                            .containsInstant(this.instant)
                    && instant.equals(WriteMetadataEvent.BOOTSTRAP_INSTANT)
                    && this.tableState.operationType == WriteOperationType.INSERT) {
                LOG.warn(
                        "Reuse current pending Instant {} with {} operationType, "
                                + "ignoring empty bootstrap event.",
                        this.instant,
    ...
    private void handleWriteMetaEvent(WriteMetadataEvent event) {
        // the write task does not block after checkpointing(and before it receives a checkpoint
        // success event),
        // if it checkpoints succeed then flushes the data buffer again before this coordinator
        // receives a checkpoint
        ...
        ValidationUtils.checkState(
                ...
                String.format(
                        "Receive an unexpected event for instant %s from task %d",
    ...
    /**
      * The coordinator reuses the instant if there is no data for this round of checkpoint, sends the
      * commit ack events to unblock the flushing.
      */
    private void sendCommitAckEvents(long checkpointId) {
        CompletableFuture<?>[] futures =
                Arrays.stream(this.gateways)
                        ...
                        (resp, error) -> {
                            if (!sendToFinishedTasks(error)) {
                                throw new HoodieException(
                                        "Error while waiting for the commit ack events to finish sending", error);
                            }
                        });
    }
    ...
    private void doCommit(String instant, List<WriteStatus> writeResults) {
        ...
        if (!hasErrors || this.conf.getBoolean(FlinkOptions.IGNORE_FAILED)) {
            ...
            if (hasErrors) {
                LOG.warn(
                        "Some records failed to merge but forcing commit since commitOnErrors set to true. Errors/Total="
                                + totalErrorRecords
                                + "/"
                                + totalRecords);
            }
                    ...
                    .forEach(
                            ws -> {
                                ...
                                if (ws.getErrors().size() > 0) {
                                    LOG.error(
                                            "The first 100 records-level errors for partition path {} and fileID {}:",
                                            ws.getPartitionPath(),
                                            ws.getFileId());
                                    ws.getErrors().entrySet().stream()
                                            .limit(100)
                                            .forEach(
                                                    entry ->
                                                            LOG.error(
            ...
            throw new HoodieException(
                    String.format("Commit instant [%s] failed and rolled back !", instant));
        }
    }
            ...
            this.commitAction =
                    CommitUtils.getCommitActionType(
                            ...
                            HoodieTableType.valueOf(
                                    conf.getString(FlinkOptions.TABLE_TYPE).toUpperCase(Locale.ROOT)));
