...
/**
  * The function to build the write profile incrementally for records within a checkpoint, it then
  * assigns the bucket with ID using the {@link BucketAssigner}.
  *
  * <p>All the records are tagged with HoodieRecordLocation, instead of real instant time, INSERT
  * record uses "I" and UPSERT record uses "U" as instant time. There is no need to keep the "real"
  * instant time for each record, the bucket ID (partition path & fileID) actually decides where the
  * record should write to. The "I" and "U" tags are only used for downstream to decide whether the
  * data bucket is an INSERT or an UPSERT, we should factor the tags out when the underneath writer
...
public class BucketAssignFunction<K, I, O extends HoodieRecord<?>>
        extends KeyedProcessFunction<K, I, O> implements CheckpointedFunction, CheckpointListener {

    /**
      * Index cache(speed-up) state for the underneath file based(BloomFilter) indices. When a record
      * came in, we do these check:
      *
      * <ul>
      *   <li>Try to load all the records in the partition path where the record belongs to
      *   <li>Checks whether the state contains the record key
      *   <li>If it does, tag the record with the location
      *   <li>If it does not, use the {@link BucketAssigner} to generate a new bucket ID
      * </ul>
      */
    ...
    /**
      * If the index is global, update the index for the old partition path if same key record with
      * different partition path came in.
      */
    private final boolean globalIndex;

    public BucketAssignFunction(Configuration conf) {
        this.conf = conf;
        this.isChangingRecords =
    ...
    public void initializeState(FunctionInitializationContext context) {
        ValueStateDescriptor<HoodieRecordGlobalLocation> indexStateDesc =
                new ValueStateDescriptor<>(
                        "indexState", TypeInformation.of(HoodieRecordGlobalLocation.class));
        double ttl = conf.getDouble(FlinkOptions.INDEX_STATE_TTL) * 24 * 60 * 60 * 1000;
        if (ttl > 0) {
            indexStateDesc.enableTimeToLive(
                    StateTtlConfig.newBuilder(Time.milliseconds((long) ttl)).build());
        }
    ...
    private void updateIndexState(String partitionPath, HoodieRecordLocation localLoc)
            throws Exception {
