...
public class ClusteringPlanOperator extends AbstractStreamOperator<ClusteringPlanEvent>
    ...
    public void open() throws Exception {
        ...
        ClusteringUtil.rollbackClustering(
                table, FlinkWriteClients.createWriteClient(conf, getRuntimeContext()));
    }
    ...
    public void notifyCheckpointComplete(long checkpointId) {
        try {
            ...
            // make it fail-safe
            LOG.error(
                    "Error while scheduling clustering plan for checkpoint: " + checkpointId, throwable);
        }
    }
    ...
    private void scheduleClustering(HoodieFlinkTable<?> table, long checkpointId) {
        ...
        if (clusteringPlan == null
            ...
            table
                    .getActiveTimeline()
                    .transitionReplaceRequestedToInflight(clusteringInstant, Option.empty());
            table.getMetaClient().reloadActiveTimeline();
            ...
            for (HoodieClusteringGroup clusteringGroup : clusteringPlan.getInputGroups()) {
                LOG.info(
                        ...
                        clusteringGroup.getSlices().size());
                output.collect(
                        new StreamRecord<>(
                                new ClusteringPlanEvent(
                                        clusteringInstantTime,
