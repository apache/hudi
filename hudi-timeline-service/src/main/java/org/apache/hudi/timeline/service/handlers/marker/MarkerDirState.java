...
/**
  ...
  *
  * <p>The operations inside this class is designed to be thread-safe.
  */
public class MarkerDirState implements Serializable {
    ...
    /**
      * Adds a {@code MarkerCreationCompletableFuture} instance from a marker creation request to the
      * queue.
      *
    ...
    /**
      * @return the next file index to use in a round-robin fashion, or empty if no file is available.
      */
    ...
    public void processMarkerCreationRequests(
        ...
        LOG.debug(
                "timeMs="
                        ...
                        + markerDirPath
                        + " numRequests="
                        + pendingMarkerCreationFutures.size()
                        + " fileIndex="
                        + fileIndex);
    ...
    public boolean deleteAllMarkers() {
        boolean result =
                FSUtils.deleteDir(
                        hoodieEngineContext, storage, new StoragePath(markerDirPath), parallelism);
        allMarkers.clear();
    ...
    /**
      * Syncs all markers maintained in the underlying files under the marker directory in the file
      * system.
      */
    ...
    private void writeMarkerTypeToFile() {
        ...
        try {
            if (!storage.exists(dirPath)
                    || !MarkerUtils.doesMarkerTypeFileExist(storage, markerDirPath)) {
                // There is no existing marker directory, create a new directory and write marker
                // type
            ...
            throw new HoodieIOException(
                    "Failed to write marker type file in " + markerDirPath + ": " + e.getMessage(), e);
        }
    }
    ...
    /**
      ...
      *
      * <p>E.g., if the marker file path is /tmp/table/.hoodie/.temp/000/MARKERS3, the index returned
      * is 3.
      *
    ...
    private int parseMarkerFileIndex(String markerFilePathStr) {
        ...
        try {
            return Integer.parseInt(
                    markerFileName.substring(prefixIndex + MARKERS_FILENAME_PREFIX.length()));
    ...
    private void flushMarkersToFile(int markerFileIndex) {
        ...
        StoragePath markersFilePath =
                new StoragePath(markerDirPath, MARKERS_FILENAME_PREFIX + markerFileIndex);
