...
public class TimelineService {
    ...
    public static class Config implements Serializable {
        ...
        @Parameter(
                names = {"--view-storage", "-st"},
                description = "View Storage Type. Default - SPILLABLE_DISK")
        public FileSystemViewStorageType viewStorageType = FileSystemViewStorageType.SPILLABLE_DISK;

        @Parameter(
                names = {"--max-view-mem-per-table", "-mv"},
                description =
                        "Maximum view memory per table in MB to be used for storing file-groups."
        ...
        @Parameter(
                names = {"--mem-overhead-fraction-pending-compaction", "-cf"},
                description =
                        "Memory Fraction of --max-view-mem-per-table to be allocated for managing pending compaction"
        ...
        @Parameter(
                names = {"--base-store-path", "-sp"},
                description =
                        "Directory where spilled view entries will be stored. Used for SPILLABLE_DISK storage type")
        public String baseStorePathForFileGroups =
                FileSystemViewStorageConfig.SPILLABLE_DIR.defaultValue();

        @Parameter(
                names = {"--rocksdb-path", "-rp"},
        ...
        @Parameter(
                names = {"--threads", "-t"},
                description = "Number of threads to use for serving requests. The default number is 250")
        ...
        @Parameter(
                names = {"--enable-marker-requests", "-em"},
                description = "Enable handling of marker-related requests")
        public boolean enableMarkerRequests = false;

        @Parameter(
                names = {"--enable-instant-state-requests"},
                description = "Enable handling of instant state requests")
        public boolean enableInstantStateRequests = false;

        @Parameter(
                names = {"--marker-batch-threads", "-mbt"},
                description = "Number of threads to use for batch processing marker creation requests")
        public int markerBatchNumThreads = 20;

        @Parameter(
                names = {"--marker-batch-interval-ms", "-mbi"},
                description =
                        "The interval in milliseconds between two batch processing of marker creation requests")
        public long markerBatchIntervalMs = 50;

        @Parameter(
                names = {"--marker-parallelism", "-mdp"},
                description = "Parallelism to use for reading and deleting marker files")
        public int markerParallelism = 100;

        @Parameter(
                names = {"--early-conflict-detection-strategy"},
                description =
        ...
        @Parameter(
                names = {"--early-conflict-detection-check-commit-conflict"},
                description =
        ...
        @Parameter(
                names = {"--early-conflict-detection-enable"},
                description =
        ...
        @Parameter(
                names = {"--async-conflict-detector-initial-delay-ms"},
                description =
        ...
        @Parameter(
                names = {"--async-conflict-detector-period-ms"},
                description =
                        "Used for timeline-server-based markers with "
                                + "`AsyncTimelineServerBasedDetectionStrategy`. "
        ...
        @Parameter(
                names = {"--early-conflict-detection-max-heartbeat-interval-ms"},
                description =
                        "Used for timeline-server-based markers with "
                                + "`AsyncTimelineServerBasedDetectionStrategy`. "
        ...
        @Parameter(
                names = {"--instant-state-force-refresh-request-number"},
                description =
        ...
        public static class Builder {
            ...
            public Builder earlyConflictDetectionMaxAllowableHeartbeatIntervalInMs(
            ...
            public Builder instantStateForceRefreshRequestNumber(
                    int instantStateForceRefreshRequestNumber) {
    ...
    private int startServiceOnPort(int port) throws IOException {
        ...
        for (int attempt = 0; attempt < START_SERVICE_MAX_RETRIES; attempt++) {
            // Returns port to try when trying to bind a service. Handles wrapping and skipping
            // privileged ports.
            ...
            try {
                ...
                if (e.getMessage() != null && e.getMessage().contains("Failed to bind to")) {
                    if (tryPort == 0) {
                        ...
                        LOG.warn(
                                String.format(
                                        "Timeline server could not bind on port %d. " + "Attempting port %d + 1.",
                                        tryPort, tryPort));
                    }
                } else {
                    LOG.warn(
                            String.format(
                                    "Timeline server start failed on port %d. Attempting port %d + 1.",
                                    tryPort, tryPort),
                            e);
                }
            }
    ...
    public int startService() throws IOException {
        int maxThreads =
                timelineServerConf.numThreads > 0 ? timelineServerConf.numThreads : DEFAULT_NUM_THREADS;
        ...
        requestHandler =
                new RequestHandler(app, conf, timelineServerConf, context, storage, fsViewsManager);
        ...
        switch (config.viewStorageType) {
            case MEMORY:
                ...
                return FileSystemViewManager.createViewManager(
                        localEngineContext, inMemConfBuilder.build(), commonConfig);
            case SPILLABLE_DISK:
                {
                    FileSystemViewStorageConfig.Builder spillableConfBuilder =
                            FileSystemViewStorageConfig.newBuilder();
                    spillableConfBuilder
                    ...
                    return FileSystemViewManager.createViewManager(
                            localEngineContext, spillableConfBuilder.build(), commonConfig);
                }
            case EMBEDDED_KV_STORE:
                {
                    ...
                    return FileSystemViewManager.createViewManager(
                            localEngineContext, rocksDBConfBuilder.build(), commonConfig);
                }
            default:
                throw new IllegalArgumentException(
                        "Invalid view manager storage type :" + config.viewStorageType);
        }
    }
