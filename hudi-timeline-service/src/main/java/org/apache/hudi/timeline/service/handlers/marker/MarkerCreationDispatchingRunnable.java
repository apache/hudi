...
public class MarkerCreationDispatchingRunnable implements Runnable {
    ...
    /**
      ...
      *
      * <p>For each marker directory, goes through the following steps: (1) find the next available
      * file index for writing. If no file index is available, skip the processing of this marker
      * directory; (2) fetch the pending marker creation requests for this marker directory. If there
      * is no request, skip this marker directory; (3) put the marker directory, marker dir state, list
      * of requests futures, and the file index to a {@code MarkerDirRequestContext} instance and add
      * the instance to the request context list.
      *
      * <p>If the request context list is not empty, spins up a worker thread, {@code
      * MarkerCreationBatchingRunnable}, and pass all the request context to the thread for batch
      * processing. The thread is responsible for responding to the request futures directly.
      */
    @Override
    public void run() {
        ...
        for (Map.Entry<String, MarkerDirState> entry : markerDirStateMap.entrySet()) {
            ...
            if (!fileIndex.isPresent()) {
                LOG.debug(
                        "All marker files are busy, skip batch processing of create marker requests in "
                                + markerDir);
                continue;
            }
