...
public class InstantStateHandler extends Handler {
  ...
  public List<InstantStateDTO> getAllInstantStates(String instantStatePath) {
    ...
    return cachedInstantStates.computeIfAbsent(instantStatePath, k -> scanInstantState(new StoragePath(k)));
  }
  ...
  public boolean refresh(String instantStatePath) {
    try {
      cachedInstantStates.put(instantStatePath, scanInstantState(new StoragePath(instantStatePath)));
      requestCount.set(0);
    } catch (Exception e) {
   ...
   */
  public List<InstantStateDTO> scanInstantState(StoragePath instantStateLocation) {
    try {
