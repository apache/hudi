...
/**
  ...
  *
  * <p>The instant states are cached in timeline server and will be refreshed after the instant
  * states in file system were changed.
  */
public class InstantStateHandler extends Handler {
    ...
    public List<InstantStateDTO> getAllInstantStates(String instantStatePath) {
        ...
        return cachedInstantStates.computeIfAbsent(
                instantStatePath, k -> scanInstantState(new StoragePath(k)));
    }

    /**
      * Refresh the checkpoint messages cached. Will be called when coordinator start/commit/abort
      * instant.
      *
    ...
    public boolean refresh(String instantStatePath) {
        try {
            cachedInstantStates.put(
                    instantStatePath, scanInstantState(new StoragePath(instantStatePath)));
            requestCount.set(0);
        } catch (Exception e) {
        ...
        try {
            ...
            if (this.storage.exists(instantStatePath)) {
                return this.storage.listDirectEntries(instantStatePath).stream()
                        .map(InstantStateDTO::fromStoragePathInfo)
