...
/**
  * Support to avro-record read parquet-log which written by spark-record. See the examples in
  * TestMORDataSource#testRecordTypeCompatibilityWithParquetLog. Exception throw when schema end with
  * map or list.
  */
public class HoodieAvroReadSupport<T> extends AvroReadSupport<T> {
    ...
    public ReadContext init(
        ...
        if (!legacyMode) {
            requestedSchema =
                    new MessageType(requestedSchema.getName(), convertLegacyMap(requestedSchema.getFields()));
        }
        return new ReadContext(requestedSchema, readContext.getReadSupportMetadata());
    }

    /**
      * Here we want set config with which file has been written. Even though user may have overwritten
      * {@link AvroWriteSupport.WRITE_OLD_LIST_STRUCTURE}, it's only applicable to how to produce new
      * files(here is a read path). Later the config value {@link
      * AvroWriteSupport.WRITE_OLD_LIST_STRUCTURE} will still be used to write new file according to
      * the user preferences.
      */
    private void adjustConfToReadWithFileProduceMode(
            boolean isLegacyModeWrittenFile, Configuration configuration) {
        if (isLegacyModeWrittenFile) {
            configuration.set(
                    AvroWriteSupport.WRITE_OLD_LIST_STRUCTURE,
                    "true",
                    "support reading avro from legacy map/list in parquet file");
        } else {
            configuration.set(
                    AvroWriteSupport.WRITE_OLD_LIST_STRUCTURE,
                    "false",
                    "support reading avro from non-legacy map/list in parquet file");
        }
    }

    /**
      * Check whether write map/list with legacy mode. legacy: list: optional group obj_ids (LIST) {
      * repeated binary array (UTF8); } map: optional group obj_ids (MAP) { repeated group map
      * (MAP_KEY_VALUE) { required binary key (UTF8); required binary value (UTF8); } } non-legacy:
      * optional group obj_ids (LIST) { repeated group list { optional binary element (UTF8); } }
      * optional group obj_maps (MAP) { repeated group key_value { required binary key (UTF8); optional
      * binary value (UTF8); } }
      */
        ...
        for (Type type : oldTypes) {
            if (!type.isPrimitive()) {
                ...
                if (type.getOriginalType() == OriginalType.MAP_KEY_VALUE) {
                    ...
                    newTypes.add(
                            new GroupType(
                                    parent.getRepetition(), parent.getName(), parent.getOriginalType(), types));
                }
            } else {
                newTypes.add(type);
            }
