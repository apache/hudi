...
public class HoodieHadoopStorage extends HoodieStorage {
    ...
    public HoodieHadoopStorage(
        ...
        if (enableRetry) {
            fileSystem =
                    new HoodieRetryWrapperFileSystem(
                            fileSystem,
                            maxRetryIntervalMs,
                            maxRetryNumbers,
        ...
        checkArgument(
                !(fileSystem instanceof HoodieWrapperFileSystem),
    ...
    public OutputStream create(
            ...
            throws IOException {
        return fs.create(
                convertToHadoopPath(path), false, bufferSize, replication, sizeThreshold, null);
    }
    ...
    public List<StoragePathInfo> listDirectEntries(List<StoragePath> pathList) throws IOException {
        return Arrays.stream(
                        fs.listStatus(
                                pathList.stream().map(HadoopFSUtils::convertToHadoopPath).toArray(Path[]::new)))
                .map(HadoopFSUtils::convertToStoragePathInfo)
                .collect(Collectors.toList());
    }
    ...
    public List<StoragePathInfo> listDirectEntries(StoragePath path, StoragePathFilter filter)
            throws IOException {
        return Arrays.stream(
                        fs.listStatus(convertToHadoopPath(path), e -> filter.accept(convertToStoragePath(e))))
                .map(HadoopFSUtils::convertToStoragePathInfo)
                .collect(Collectors.toList());
    }
    ...
    public List<StoragePathInfo> globEntries(StoragePath pathPattern, StoragePathFilter filter)
        ...
        return Arrays.stream(
                        fs.globStatus(
                                convertToHadoopPath(pathPattern),
                                path -> filter.accept(convertToStoragePath(path))))
                .map(HadoopFSUtils::convertToStoragePathInfo)
                .collect(Collectors.toList());
    }
