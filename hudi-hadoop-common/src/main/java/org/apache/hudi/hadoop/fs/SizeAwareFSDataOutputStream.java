...
public class SizeAwareFSDataOutputStream extends FSDataOutputStream {
    ...
    public SizeAwareFSDataOutputStream(
            Path path, FSDataOutputStream out, ConsistencyGuard consistencyGuard, Runnable closeCallback)
            throws IOException {
    ...
    public synchronized void write(byte[] b, int off, int len) throws IOException {
        HoodieWrapperFileSystem.executeFuncWithTimeAndByteMetrics(
                HoodieWrapperFileSystem.MetricName.write.name(),
                path,
                len,
                () -> {
