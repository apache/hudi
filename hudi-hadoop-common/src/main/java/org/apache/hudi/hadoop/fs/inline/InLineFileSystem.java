...
/**
  * Enables reading any inline file at a given offset and length. This {@link FileSystem} is used
  * only in read path and does not support any write apis.
  *
  * <p>- Reading an inlined file at a given offset, length, read it out as if it were an independent
  * file of that length - Inlined path is of the form
  * "inlinefs:///path/to/outer/file/<outer_file_scheme>/?start_offset=<start_offset>&length=<length>
  *
  * <p>TODO: The reader/writer may try to use relative paths based on the inlinepath and it may not
  * work. Need to handle this gracefully eg. the parquet summary metadata reading. TODO: If this
  * shows promise, also support directly writing the inlined file to the underneath file without
  * buffer
  */
public class InLineFileSystem extends FileSystem {
    ...
    public FSDataInputStream open(Path inlinePath, int bufferSize) throws IOException {
        ...
        return new InLineFsDataInputStream(
                HadoopInLineFSUtils.startOffset(inlineStoragePath),
                outerStream,
                HadoopInLineFSUtils.length(inlineStoragePath));
    }
    ...
    public FileStatus getFileStatus(Path inlinePath) throws IOException {
        ...
        FileStatus toReturn =
                new FileStatus(
                        HadoopInLineFSUtils.length(convertToStoragePath(inlinePath)),
                        status.isDirectory(),
