...
public class HadoopFSUtils {
    ...
    public static Configuration prepareHadoopConf(Configuration conf) {
        ...
        for (Map.Entry<String, String> prop : System.getenv().entrySet()) {
            if (prop.getKey().startsWith(HOODIE_ENV_PROPS_PREFIX)) {
                ...
                conf.set(
                        prop.getKey().replace(HOODIE_ENV_PROPS_PREFIX, "").replaceAll("_DOT_", "."),
                        prop.getValue());
            }
        }
        return conf;
    ...
    public static <T> FileSystem getFs(
            Path path, StorageConfiguration<T> storageConf, boolean newCopy) {
        Configuration conf =
                newCopy
    ...
    public static FileSystem getFs(Path path, Configuration conf) {
        ...
        try {
            ...
            throw new HoodieIOException(
                    String.format("Failed to get instance of %s", FileSystem.class.getName()), e);
        }
        return fs;
    }
    ...
    /**
      ...
      */
    public static FSDataInputStream getFSDataInputStream(
            FileSystem fs, StoragePath filePath, int bufferSize, boolean wrapStream) {
        FSDataInputStream fsDataInputStream = null;
        try {
            fsDataInputStream = fs.open(convertToHadoopPath(filePath), bufferSize);
        } catch (IOException e) {
            throw new HoodieIOException(
                    String.format("Exception creating input stream from file: %s", filePath), e);
        }
        ...
        if (isGCSFileSystem(fs)) {
            ...
            return new SchemeAwareFSDataInputStream(
                    getFSDataInputStreamForGCS(fsDataInputStream, filePath, bufferSize), true);
        }
        ...
        if (fsDataInputStream.getWrappedStream() instanceof FSInputStream) {
            return new TimedFSDataInputStream(
                    convertToHadoopPath(filePath),
                    new FSDataInputStream(
                            new BufferedFSInputStream(
                                    (FSInputStream) fsDataInputStream.getWrappedStream(), bufferSize)));
        }
    ...
    /**
      * GCS FileSystem needs some special handling for seek and hence this method assists to fetch the
      * right {@link FSDataInputStream} to be used by wrapping with required input streams.
      *
      ...
      * @return the right {@link FSDataInputStream} as required.
      */
    private static FSDataInputStream getFSDataInputStreamForGCS(
            FSDataInputStream fsDataInputStream, StoragePath filePath, int bufferSize) {
        // in case of GCS FS, there are two flows.
        // a. fsDataInputStream.getWrappedStream() instanceof FSInputStream
        // b. fsDataInputStream.getWrappedStream() not an instanceof FSInputStream, but an instance
        // of FSDataInputStream.
        // (a) is handled in the first if block and (b) is handled in the second if block. If not,
        // we fallback to original fsDataInputStream
        if (fsDataInputStream.getWrappedStream() instanceof FSInputStream) {
            return new TimedFSDataInputStream(
                    convertToHadoopPath(filePath),
                    new FSDataInputStream(
    ...
    /**
      * Chdfs will throw {@code IOException} instead of {@code EOFException}. It will cause error in
      * isBlockCorrupted(). Wrapped by {@code BoundedFsDataInputStream}, to check whether the desired
    ...
    private static StorageConfiguration<Configuration> getStorageConf(
            Configuration conf, boolean copy) {
    ...
    public static Configuration registerFileSystem(StoragePath file, Configuration conf) {
        ...
        returnConf.set(
                "fs." + HoodieWrapperFileSystem.getHoodieScheme(scheme) + ".impl",
    ...
    public static FsPermission toFSPermission(HoodieFSPermission fsPermission) {
        ...
        FsAction userAction =
                fsPermission.getUserAction() != null
                        ? FsAction.valueOf(fsPermission.getUserAction())
                        : null;
        FsAction grpAction =
                fsPermission.getGroupAction() != null
                        ? FsAction.valueOf(fsPermission.getGroupAction())
                        : null;
        FsAction otherAction =
                fsPermission.getOtherAction() != null
                        ? FsAction.valueOf(fsPermission.getOtherAction())
                        : null;
    ...
    public static HoodieFSPermission fromFSPermission(FsPermission fsPermission) {
        ...
        String userAction =
                fsPermission.getUserAction() != null ? fsPermission.getUserAction().name() : null;
        String grpAction =
                fsPermission.getGroupAction() != null ? fsPermission.getGroupAction().name() : null;
        String otherAction =
                fsPermission.getOtherAction() != null ? fsPermission.getOtherAction().name() : null;
        return HoodieFSPermission.newBuilder()
                .setUserAction(userAction)
                .setGroupAction(grpAction)
                .setOtherAction(otherAction)
                .setStickyBit(fsPermission.getStickyBit())
                .build();
    }
    ...
    /**
      * Given a base partition and a partition path, return relative path of partition path to the base
      * path.
      */
    public static String getRelativePartitionPath(Path basePath, Path fullPartitionPath) {
        return FSUtils.getRelativePartitionPath(
                new StoragePath(basePath.toUri()), new StoragePath(fullPartitionPath.toUri()));
    }

    /**
      * Get the first part of the file name in the log file. That will be the fileId. Log file do not
      * have instantTime in the file name.
      */
    ...
    /**
      ...
      * HDFS. If a data node went down, it takes about 10 minutes for the lease to be recovered. But if
    ...
    public static boolean recoverDFSFileLease(final DistributedFileSystem dfs, final Path p)
        ...
        for (int nbAttempt = 0; nbAttempt < MAX_ATTEMPTS_RECOVER_LEASE; nbAttempt++) {
            ...
            // Sleep for 1 second before trying again. Typically it takes about 2-3 seconds to
            // recover
    ...
    public static Path constructAbsolutePathInHadoopPath(
            String basePath, String relativePartitionPath) {
    ...
    public static <T> Map<String, T> parallelizeFilesProcess(
            ...
            FileSystem fs,
            int parallelism,
            FSUtils.SerializableFunction<Pair<String, StorageConfiguration<Configuration>>, T>
                    pairFunction,
        ...
        if (subPaths.size() > 0) {
            ...
            result =
                    hoodieEngineContext.mapToPair(
                            ...
                            subPath ->
                                    new ImmutablePair<>(
                                            subPath, pairFunction.apply(new ImmutablePair<>(subPath, conf))),
                            actualParallelism);
        }
        return result;
    }
    ...
    /**
      ...
      *
      * <p>E.g., given "/tmp/hoodie_table" as the rootPath, and 3 as the expected level, this method
      * gives back the {@link FileStatus} of all files under "/tmp/hoodie_table/[*]/[*]/[*]/" folders.
      *
    ...
    public static Map<String, Boolean> deleteFilesParallelize(
        ...
        return HadoopFSUtils.parallelizeFilesProcess(
                context,
