...
public class TimedFSDataInputStream extends FSDataInputStream {
    ...
    public int read(ByteBuffer buf) throws IOException {
        return HoodieWrapperFileSystem.executeFuncWithTimeAndByteMetrics(
                HoodieWrapperFileSystem.MetricName.read.name(), path, 0, () -> super.read(buf));
    }

    @Override
    public int read(long position, byte[] buffer, int offset, int length) throws IOException {
        return HoodieWrapperFileSystem.executeFuncWithTimeAndByteMetrics(
                HoodieWrapperFileSystem.MetricName.read.name(),
                path,
                length,
                () -> super.read(position, buffer, offset, length));
    }
    ...
    public ByteBuffer read(ByteBufferPool bufferPool, int maxLength, EnumSet<ReadOption> opts)
        ...
        return HoodieWrapperFileSystem.executeFuncWithTimeAndByteMetrics(
                ...
                path,
                maxLength,
                () -> super.read(bufferPool, maxLength, opts));
    }
