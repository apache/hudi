...
public class HadoopConfigUtils {
  ...
  /**
   * @param hoodieConfig       {@link HoodieConfig} object.
   * @param includeGlobalProps whether to include global properties.
   * @return {@link TypedProperties} object containing the configuration.
   */
  public static TypedProperties getPropsFromHoodieConfig(HoodieConfig hoodieConfig,
                                                         boolean includeGlobalProps) {
    if (includeGlobalProps) {
      TypedProperties mergedProps = DFSPropertiesConfiguration.getGlobalProps();
      mergedProps.putAll(hoodieConfig.getProps());
      return mergedProps;
    } else {
      return hoodieConfig.getProps();
    }
  }
}
