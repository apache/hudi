...
public class HoodieHadoopStorage implements HoodieStorage {
  ...
  @Override
  public SeekableDataInputStream openLogFile(HoodieLocation location, int bufferSize) throws IOException {
    return new HadoopSeekableDataInputStream(HadoopFSUtils.getFSDataInputStream(fs, location, bufferSize));
  }
  ...
  @Override
  public List<HoodieFileStatus> globEntries(HoodieLocation location)
      throws IOException {
    return Arrays.stream(fs.globStatus(convertHoodieLocationToPath(location)))
        .map(this::convertToHoodieFileStatus)
        .collect(Collectors.toList());
  }

  @Override
  public List<HoodieFileStatus> globEntries(HoodieLocation location, HoodieLocationFilter filter)
      throws IOException {
    return Arrays.stream(fs.globStatus(convertHoodieLocationToPath(location), path ->
            filter.accept(convertPathToHoodieLocation(path))))
        .map(this::convertToHoodieFileStatus)
        .collect(Collectors.toList());
  }
