    ...
    public HoodieAvroOrcWriter(
        ...
        Configuration conf =
                HadoopFSUtils.registerFileSystem(
                        file, config.getStorageConf().unwrapAs(Configuration.class));
    ...
    public void close() throws IOException {
        ...
        if (orcConfig.useBloomFilter()) {
            ...
            if (bloomFilter
                    ...
                    .name()
                    .contains(HoodieDynamicBoundedBloomFilter.TYPE_CODE_PREFIX)) {
                writer.addUserMetadata(
                        HoodieBloomFilterWriteSupport.HOODIE_BLOOM_FILTER_TYPE_CODE,
                        ByteBuffer.wrap(getUTF8Bytes(bloomFilter.getBloomFilterTypeCode().name())));
            }
        }
        writer.addUserMetadata(
                HoodieOrcConfig.AVRO_SCHEMA_METADATA_KEY,
                ByteBuffer.wrap(getUTF8Bytes(avroSchema.toString())));
