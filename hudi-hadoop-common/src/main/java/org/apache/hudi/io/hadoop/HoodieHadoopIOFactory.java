...
/**
  * Creates readers and writers for AVRO record payloads. Currently uses reflection to support SPARK
  * record payloads but this ability should be removed with [HUDI-7746]
  */
public class HoodieHadoopIOFactory extends HoodieIOFactory {
    ...
    public HoodieFileReaderFactory getReaderFactory(HoodieRecord.HoodieRecordType recordType) {
        switch (recordType) {
            ...
            case SPARK:
                ...
                try {
                    return (HoodieFileReaderFactory)
                            ReflectionUtils.loadClass(
                                    "org.apache.hudi.io.storage.HoodieSparkFileReaderFactory",
                                    new Class<?>[] {HoodieStorage.class},
                                    storage);
                } catch (Exception e) {
    ...
    public HoodieFileWriterFactory getWriterFactory(HoodieRecord.HoodieRecordType recordType) {
        switch (recordType) {
            ...
            case SPARK:
                // TODO: remove this case [HUDI-7746]
                try {
                    return (HoodieFileWriterFactory)
                            ReflectionUtils.loadClass(
                                    "org.apache.hudi.io.storage.HoodieSparkFileWriterFactory",
                                    new Class<?>[] {HoodieStorage.class},
                                    storage);
                } catch (Exception e) {
    ...
    public HoodieStorage getStorage(
        ...
        return new HoodieHadoopStorage(
                path,
                storage.getConf(),
                enableRetry,
                maxRetryIntervalMs,
