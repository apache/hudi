...
/**
  * HoodieHFileWriter writes IndexedRecords into an HFile. The record's key is used as the key and
  * the AVRO encoded record bytes are saved as the value.
  *
  * <p>Limitations (compared to columnar formats like Parquet or ORC): 1. Records should be added in
  * order of keys 2. There are no column stats
  */
public class HoodieAvroHFileWriter implements HoodieAvroFileWriter {
    ...
    public HoodieAvroHFileWriter(
        ...
        this.wrapperFs =
                this.isWrapperFileSystem ? Option.of((HoodieWrapperFileSystem) fs) : Option.empty();
        ...
        writer.appendFileInfo(
                getUTF8Bytes(HoodieAvroHFileReaderImplBase.SCHEMA_KEY), getUTF8Bytes(schema.toString()));
        this.prevRecordKey = "";
    }
    ...
    public void writeAvro(String recordKey, IndexedRecord record) throws IOException {
        ...
        if (prevRecordKey.equals(recordKey)
            ...
            throw new HoodieDuplicateKeyException(
                    "Duplicate recordKey "
                            + recordKey
        ...
        if (keyFieldSchema.isPresent()) {
            ...
            boolean isKeyAvailable =
                    (record.get(keyFieldPos) != null && !(record.get(keyFieldPos).toString().isEmpty()));
            if (isKeyAvailable) {
    ...
    public void close() throws IOException {
        if (hfileConfig.useBloomFilter()) {
            ...
            writer.appendFileInfo(
                    getUTF8Bytes(HoodieAvroHFileReaderImplBase.KEY_MIN_RECORD), getUTF8Bytes(minRecordKey));
            writer.appendFileInfo(
                    getUTF8Bytes(HoodieAvroHFileReaderImplBase.KEY_MAX_RECORD), getUTF8Bytes(maxRecordKey));
            writer.appendFileInfo(
                    getUTF8Bytes(HoodieAvroHFileReaderImplBase.KEY_BLOOM_FILTER_TYPE_CODE),
