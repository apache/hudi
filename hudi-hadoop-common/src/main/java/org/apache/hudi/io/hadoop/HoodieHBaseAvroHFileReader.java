...
/**
  ...
  *
  * <p>{@link HoodieFileReader} implementation allowing to read from {@link HFile}.
  */
public class HoodieHBaseAvroHFileReader extends HoodieAvroHFileReaderImplBase {
    ...
    public HoodieHBaseAvroHFileReader(
            StorageConfiguration<?> storageConf, StoragePath path, Option<Schema> schemaOpt)
            throws IOException {
    ...
    public HoodieHBaseAvroHFileReader(StorageConfiguration<?> storageConf, StoragePath path)
            throws IOException {
    ...
    public ClosableIterator<HoodieRecord<IndexedRecord>> getRecordsByKeysIterator(
        ...
        ClosableIterator<IndexedRecord> iterator =
                new RecordByKeyIterator(reader, scanner, sortedKeys, getSchema(), schema);
    ...
    public String[] readMinMaxRecordKeys() {
        ...
        return new String[] {
            new String(fileInfo.get(getUTF8Bytes(KEY_MIN_RECORD))),
    ...
    public BloomFilter readBloomFilter() {
        try {
            ...
            HFileInfo fileInfo = getSharedHFileReader().getHFileInfo();
            ByteBuff buf =
                    getSharedHFileReader()
                            .getMetaBlock(KEY_BLOOM_FILTER_META_BLOCK, false)
            ...
            buf.get(bytes);
            return BloomFilterFactory.fromString(
                    new String(bytes), new String(fileInfo.get(getUTF8Bytes(KEY_BLOOM_FILTER_TYPE_CODE))));
        } catch (IOException e) {
    ...
    /**
      ...
      *
      * <p>Note: This method is performant when the caller passes in a sorted candidate keys.
      *
    ...
    public Set<Pair<String, Long>> filterRowKeys(Set<String> candidateRowKeys) {
        ...
        synchronized (sharedLock) {
            if (!sharedScanner.isPresent()) {
                // For shared scanner, which is primarily used for point-lookups, we're caching
                // blocks
    ...
    @Override
    public ClosableIterator<IndexedRecord> getIndexedRecordIterator(
            Schema readerSchema, Schema requestedSchema) {
    ...
    @VisibleForTesting
    public ClosableIterator<IndexedRecord> getIndexedRecordsByKeysIterator(
            List<String> keys, Schema readerSchema) throws IOException {
        // We're caching blocks for this scanner to minimize amount of traffic
        // to the underlying storage as we fetched (potentially) sparsely distributed
        // keys
        HFile.Reader reader = getHFileReader();
    ...
    public ClosableIterator<IndexedRecord> getIndexedRecordsByKeyPrefixIterator(
        ...
        HFile.Reader reader = getHFileReader();
        HFileScanner scanner = getHFileScanner(reader, true);
        return new RecordByKeyPrefixIterator(
                reader, scanner, sortedKeyPrefixes, getSchema(), readerSchema);
    }
    ...
    private HFile.Reader getHFileReader() {
        ...
        return HoodieHFileUtils.createHFileReader(
                storage, path, config, storageConf.unwrapAs(Configuration.class));
    }
    ...
    private static Iterator<IndexedRecord> getRecordByKeyPrefixIteratorInternal(
            HFileScanner scanner, String keyPrefix, Schema writerSchema, Schema readerSchema)
            throws IOException {
        ...
        // following:
        //    a) -1, if cell < c[0], no position; but w/ prefix search, if the key to be searched is
        // less than first entry, the cursor will be placed at the beginning.
        //        scanner.getCell() could return the next key which could match the key we are
        // interested in.
        //    b) 0, such that c[i] = cell and scanner is left in position i;
        //    c) and 1, such that c[i] < cell, and scanner is left in position i. If key is
        // somewhere in the middle, and if there is no exact match. So, scanner.next()
        //        is expected to return the next entry which could potentially match the prefix we
        // are looking for. If the key being looked up is > last entry in the file,
        //        this could place the cursor in the end. hence we need to check for scanner.next()
        // to ensure there are entries to read or if we reached EOF.
        //
        ...
        //    - "key03", `reseekTo()` will return 0 (exact match) and place the cursor just before
        // "key03",
        //    `getCell()` will return "key03" entry
        //    - "key1", `reseekTo()` will return 1 (first not lower than) and leave the cursor at
        // key10 (assuming there is no exact match i.e. key1)
        //
        // Do remember that reseek will not do any rewind. So, after searching for key05(cursor is
        // placed at key05), if you search for key01(less than current cursor position),
        ...
        class KeyPrefixIterator implements Iterator<IndexedRecord> {
            ...
            public boolean hasNext() {
                ...
                // Extract the byte value before releasing the lock since we cannot hold on to the
                // returned cell afterwards
    ...
    private static Iterator<IndexedRecord> getRecordByKeyIteratorInternal(
            HFileScanner scanner, String key, Schema writerSchema, Schema readerSchema)
            throws IOException {
        ...
        // For the HFile containing list of cells c[0], c[1], ..., c[N], `seekTo(cell)` would return
        // following:
        //    a) -1, if cell < c[0], no position; if the key to be searched is less than first
        // entry, the cursor will be placed at the beginning.
        //    b) 0, such that c[i] = cell and scanner is left in position i;
        //    c) and 1, such that c[i] < cell, and scanner is left in position i.
        // In summary, with exact match, we are interested in return value of 0. in all other cases,
        // key is not found.
        // Also, do remember we are using reseekTo(), which means, the cursor will not rewind after
        // searching for a key.
        // Let's say the file contains key01, key02, .., key20.
        // After searching for key09, if we search for key05, it may not return the matching entry
        // since just after reseeking to key09, the cursor is at key09 and
    ...
    private static GenericRecord getRecordFromCell(
            Cell cell, Schema writerSchema, Schema readerSchema) throws IOException {
    ...
    private static byte[] copyValueFromCell(Cell c) {
        return Arrays.copyOfRange(
                c.getValueArray(), c.getValueOffset(), c.getValueOffset() + c.getValueLength());
    }
    ...
    private static HFileScanner getHFileScanner(
            HFile.Reader reader, boolean cacheBlocks, boolean doSeek) {
        // NOTE: Only scanners created in Positional Read ("pread") mode could share the same
        // reader,
            ...
            try {
                while (true) {
                    ...
                    if (next != null) {
                        ...
                        recordsIterator =
                                getRecordByKeyPrefixIteratorInternal(
                                        scanner, currentKeyPrefix, writerSchema, readerSchema);
                    } else {
                        return false;
                    }
                }
            ...
            try {
                ...
                while (sortedKeyIterator.hasNext()) {
                    currentRecordIterator =
                            getRecordByKeyIteratorInternal(
                                    scanner, sortedKeyIterator.next(), writerSchema, readerSchema);
    ...
    private static class RecordIterator implements ClosableIterator<IndexedRecord> {
        ...
        RecordIterator(
                HFile.Reader reader, HFileScanner scanner, Schema writerSchema, Schema readerSchema) {
            this.reader = reader;
            this.scanner = scanner;
            this.writerSchema = writerSchema;
            this.readerSchema = readerSchema;
