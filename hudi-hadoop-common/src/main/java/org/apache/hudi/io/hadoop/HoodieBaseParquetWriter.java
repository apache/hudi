...
/**
  ...
  *
  * @param <R> target type of the object being written into Parquet files (for ex, {@code
  *     IndexedRecord}, {@code InternalRow})
  */
public abstract class HoodieBaseParquetWriter<R> implements Closeable {
    ...
    public HoodieBaseParquetWriter(
            StoragePath file, HoodieParquetConfig<? extends WriteSupport<R>> parquetConfig)
            throws IOException {
        Configuration hadoopConf = parquetConfig.getStorageConf().unwrapAs(Configuration.class);
        ParquetWriter.Builder parquetWriterbuilder =
                new ParquetWriter.Builder(HoodieWrapperFileSystem.convertToHoodiePath(file, hadoopConf)) {
                    @Override
    ...
    /**
      ...
      * @param hadoopConf
      */
    protected void handleParquetBloomFilters(
            ParquetWriter.Builder parquetWriterbuilder, Configuration hadoopConf) {
        // inspired from
                        ...
                        try {
                            Method method =
                                    parquetWriterbuilder
                                            .getClass()
                                            .getMethod("withBloomFilterEnabled", String.class, boolean.class);
                            method.invoke(
                                    parquetWriterbuilder, column, Boolean.valueOf(conf.getValue()).booleanValue());
                    ...
                    if (key.startsWith(BLOOM_FILTER_EXPECTED_NDV)) {
                        ...
                        try {
                            Method method =
                                    parquetWriterbuilder
                                            .getClass()
                                            .getMethod("withBloomFilterNDV", String.class, long.class);
                            method.invoke(
                                    parquetWriterbuilder, column, Long.valueOf(conf.getValue()).longValue());
                        } catch (NoSuchMethodException | IllegalAccessException | InvocationTargetException e) {
                            // skip
                        }
                    }
                    ...
                    writtenCount
                            + Math.min(
                                    ...
                                    Math.max(
                                            ...
                                            (maxFileSize / avgRecordSize - writtenCount) / 2),
