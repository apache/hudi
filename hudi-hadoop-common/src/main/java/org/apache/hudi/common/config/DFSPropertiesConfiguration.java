...
/**
  * A simplified versions of Apache commons - PropertiesConfiguration, that supports limited field
  * types and hierarchical configurations within the same folder as the root file.
  *
  * <p>Includes denoted by the same include=filename.properties syntax, with relative path from root
  * file's folder. Lines beginning with '#' are ignored as comments. Final values for properties are
  * resolved by the order in which they are specified in the files, with included files treated as if
  * they are inline.
  *
  * <p>Note: Not reusing commons-configuration since it has too many conflicting runtime deps.
  */
public class DFSPropertiesConfiguration extends PropertiesConfig {
    ...
    public static final StoragePath DEFAULT_PATH =
            new StoragePath(DEFAULT_CONF_FILE_DIR, DEFAULT_PROPERTIES_FILE);

    // props read from hudi-defaults.conf
    private static TypedProperties GLOBAL_PROPS = loadGlobalProps();

    @Nullable private final Configuration hadoopConfig;
    ...
    public DFSPropertiesConfiguration(
            @Nonnull Configuration hadoopConf, @Nonnull StoragePath filePath) {
    ...
    public static TypedProperties loadGlobalProps() {
        ...
        if (configFile != null) {
            try (BufferedReader br =
                    new BufferedReader(
                            new InputStreamReader(configFile.openStream(), StandardCharsets.UTF_8))) {
                conf.addPropsFromStream(br, new StoragePath(configFile.toURI()));
                return conf.getProps();
            } catch (URISyntaxException e) {
                throw new HoodieException(
                        String.format("Provided props file url is invalid %s", configFile), e);
    ...
    public void addPropsFromFile(StoragePath filePath) {
        ...
        try (BufferedReader reader =
                new BufferedReader(new InputStreamReader(storage.open(filePath), StandardCharsets.UTF_8))) {
                    ...
                    .forEach(
                            line -> {
                                ...
                                if (line.startsWith("include=") || line.startsWith("include =")) {
                                    ...
                                    HoodieStorage providedStorage =
                                            HoodieStorageUtils.getStorage(
                                                    split[1], HadoopFSUtils.getStorageConf(hadoopConfig));
                                    // In the case that only filename is provided, assume it's in
                                    // the same directory.
                                    if ((!providedPath.isAbsolute()
                                                    || StringUtils.isNullOrEmpty(providedStorage.getScheme()))
                                            && cfgFilePath != null) {
