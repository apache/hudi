...
public class HBaseHFileBootstrapIndexReader extends BootstrapIndex.IndexReader {
    ...
    /**
      ...
      * "2020/03/18//LATEST_TIMESTAMP/Put/vlen=3692/seqid=0". This API returns only the user key part
      * from it.
      *
    ...
    private static HFile.Reader createReader(
            String hFilePath, Configuration conf, FileSystem fileSystem) {
        return HoodieHFileUtils.createHFileReader(
                fileSystem, new HFilePathForReader(hFilePath), new CacheConfig(conf), conf);
    }
    ...
    private HoodieBootstrapIndexInfo fetchBootstrapIndexInfo() throws IOException {
        return TimelineMetadataUtils.deserializeAvroMetadata(
                partitionIndexReader().getHFileInfo().get(INDEX_INFO_KEY), HoodieBootstrapIndexInfo.class);
    }
    ...
    private <T> List<T> getAllKeys(HFileScanner scanner, Function<String, T> converter) {
        ...
        try {
            ...
            while (available) {
                keys.add(
                        converter.apply(getUserKeyFromCellKey(CellUtil.getCellKeyAsString(scanner.getCell()))));
    ...
    public List<BootstrapFileMapping> getSourceFileMappingForPartition(String partition) {
        try (HFileScanner scanner = partitionIndexReader().getScanner(true, false)) {
            KeyValue keyValue =
                    new KeyValue(
                            getUTF8Bytes(getPartitionKey(partition)),
                            new byte[0],
                            new byte[0],
            ...
            if (scanner.seekTo(keyValue) == 0) {
                ...
                HoodieBootstrapPartitionMetadata metadata =
                        TimelineMetadataUtils.deserializeAvroMetadata(
                                valBytes, HoodieBootstrapPartitionMetadata.class);
                return metadata.getFileIdToBootstrapFile().entrySet().stream()
                        .map(
                                e ->
                                        new BootstrapFileMapping(
                                                bootstrapBasePath,
    ...
    public Map<HoodieFileGroupId, BootstrapFileMapping> getSourceFileMappingForFileIds(
        ...
        try (HFileScanner scanner = fileIdIndexReader().getScanner(true, false)) {
            for (HoodieFileGroupId fileGroupId : fileGroupIds) {
                KeyValue keyValue =
                        new KeyValue(
                                getUTF8Bytes(getFileGroupKey(fileGroupId)),
                                new byte[0],
                                new byte[0],
                                HConstants.LATEST_TIMESTAMP,
                                KeyValue.Type.Put,
                ...
                if (scanner.seekTo(keyValue) == 0) {
                    ...
                    HoodieBootstrapFilePartitionInfo fileInfo =
                            TimelineMetadataUtils.deserializeAvroMetadata(
                                    valBytes, HoodieBootstrapFilePartitionInfo.class);
                    BootstrapFileMapping mapping =
                            new BootstrapFileMapping(
                                    bootstrapBasePath,
                                    fileInfo.getBootstrapPartitionPath(),
                                    fileInfo.getPartitionPath(),
                                    fileInfo.getBootstrapFileStatus(),
    ...
    /**
      * IMPORTANT : HFile Readers use HFile name (instead of path) as cache key. This could be fine as
      * long as file names are UUIDs. For bootstrap, we are using well-known index names. Hence, this
      * hacky workaround to return full path string from Path subclass and pass it to reader. The other
      * option is to disable block cache for Bootstrap which again involves some custom code as there
      * is no API to disable cache.
      */
