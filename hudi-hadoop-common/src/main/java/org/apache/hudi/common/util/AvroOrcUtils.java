...
/**
  ...
  * https://github.com/streamsets/datacollector. Source classes: -
  ...
  *
  * <p>Changes made: 1. Flatten nullable Avro schema type when the value is not null in
  * `addToVector`. 2. Use getLogicalType(), constants from LogicalTypes instead of getJsonProp() to
  * handle Avro logical types.
  */
public class AvroOrcUtils {
    ...
    /**
      ...
      * @param avroSchema Avro schema of the value Object. Only used to check logical types for
    ...
    public static void addToVector(
        ...
        switch (type.getCategory()) {
            ...
            case SHORT:
                ...
                break;
            case INT:
                // the Avro logical type could be AvroTypeUtil.LOGICAL_TYPE_TIME_MILLIS, but we will
                // ignore that fact here
                // since Orc has no way to represent a time in the way Avro defines it; we will
                // simply preserve the int value
                LongColumnVector intColVec = (LongColumnVector) colVector;
                intColVec.vector[vectorPos] = (int) value;
                break;
            case LONG:
                // the Avro logical type could be AvroTypeUtil.LOGICAL_TYPE_TIME_MICROS, but we will
                // ignore that fact here
                // since Orc has no way to represent a time in the way Avro defines it; we will
                // simply preserve the long value
                ...
                if (value instanceof String) {
                    ...
                    throw new IllegalStateException(
                            String.format(
                                    ...
                                    type.getCategory().getName(), value.getClass().getName(), value.toString()));
                }
                ...
                if (!added) {
                    throw new IllegalStateException(
                            String.format(
                                    ...
                                    value == null ? "null" : value.toString(), type.toString()));
                }
    ...
    public static boolean addUnionValue(
            ...
            switch (childType.getCategory()) {
                ...
                default:
                    throw new IllegalArgumentException(
                            "Invalid TypeDescription " + childType.getCategory().toString() + ".");
            }
        ...
        if (matchIndex >= 0) {
            ...
            if (value == null) {
                ...
                addToVector(
                        matchType,
                        unionVector.fields[matchIndex],
                        avroSchema.getTypes().get(matchIndex),
                        value,
                        vectorPos);
            }
            return true;
    ...
    /**
      ...
      * @param avroSchema Avro schema of the object to read. Only used to check logical types for
      *     timestamp unit conversion.
    ...
    public static Object readFromVector(
            TypeDescription type, ColumnVector colVector, Schema avroSchema, int vectorPos) {
        ...
        switch (type.getCategory()) {
            ...
            case CHAR:
                ...
                if (result.length() <= maxLength) {
                    ...
                    throw new HoodieIOException(
                            "CHAR/VARCHAR has length " + result.length() + " greater than Max Length allowed");
                }
            case STRING:
                ...
                if (stringType == null || !stringType.equals(StringType.String)) {
                    ...
                    System.arraycopy(
                            ((BytesColumnVector) colVector).vector[vectorPos],
                            stringOffset,
                            stringBytes,
                            0,
                            stringLength);
            ...
            case DATE:
                ...
                return (int) ((LongColumnVector) colVector).vector[vectorPos];
            case TIMESTAMP:
                // The unit of time in ORC is millis. Convert (time,nanos) to the desired unit per
                // logicalType
            ...
            case DECIMAL:
                ...
                if (baseType.equals(Schema.Type.FIXED)) {
                    ...
                    throw new HoodieIOException(
                            baseType.getName() + "is not a valid type for LogicalTypes.DECIMAL.");
                }
            case LIST:
                ...
                for (int i = 0; i < listLength; i++) {
                    list.add(
                            readFromVector(
                                    childType, listVector.child, avroSchema.getElementType(), listOffset + i));
                }
                return list;
            case MAP:
                ...
                for (int i = 0; i < mapLength; i++) {
                    map.put(
                            readFromVector(type.getChildren().get(0), mapVector.keys, keySchema, i + mapOffset)
                                    .toString(),
                            readFromVector(
                                    type.getChildren().get(1),
                                    mapVector.values,
            ...
            case UNION:
                ...
                return readFromVector(
                        type.getChildren().get(tag), fieldVector, avroSchema.getTypes().get(tag), vectorPos);
            default:
    ...
    public static TypeDescription createOrcSchema(Schema avroSchema) {
        ...
        if (logicalType != null) {
            if (logicalType instanceof LogicalTypes.Decimal) {
                ...
                //
                // A timestamp-millis logical type annotates an Avro long, where the long stores the
                ...
                //
                // A timestamp-micros logical type annotates an Avro long, where the long stores the
        ...
        switch (type) {
            ...
            case MAP:
                return TypeDescription.createMap(
                        ...
                        TypeDescription.createString(), createOrcSchema(avroSchema.getValueType()));
            case UNION:
                ...
                if (nonNullMembers.isEmpty()) {
                    ...
                    // this is how Avro represents "nullable" types; as a union of the NULL type
                    // with another
                    // since ORC already supports nullability of all types, just use the child type
                    // directly
            ...
            default:
                throw new IllegalStateException(
                        String.format("Unrecognized Avro type: %s", type.getName()));
        }
    }
    ...
    public static Schema createAvroSchema(TypeDescription orcSchema) {
        switch (orcSchema.getCategory()) {
            ...
            case SHORT:
                ...
                return Schema.create(Schema.Type.INT);
            case INT:
                // the Avro logical type could be AvroTypeUtil.LOGICAL_TYPE_TIME_MILLIS, but there
                // is no way to distinguish
                return Schema.create(Schema.Type.INT);
            case LONG:
                // the Avro logical type could be AvroTypeUtil.LOGICAL_TYPE_TIME_MICROS, but there
                // is no way to distinguish
            ...
            case UNION:
                return Schema.createUnion(
                        orcSchema.getChildren().stream()
                                .map(AvroOrcUtils::createAvroSchema)
                                .collect(Collectors.toList()));
            default:
                throw new IllegalStateException(
                        String.format("Unrecognized ORC type: %s", orcSchema.getCategory().getName()));
        }
    }
    ...
    /**
      ...
      *
      * <p>All types in ORC is nullable whereas Avro uses a union that contains the NULL type to imply
    ...
    public static Schema createAvroSchemaWithDefaultValue(
            TypeDescription orcSchema, String recordName, String namespace, boolean nullable) {
    ...
    private static Schema createAvroSchemaWithNamespace(
            TypeDescription orcSchema, String recordName, String namespace) {
        switch (orcSchema.getCategory()) {
            case BOOLEAN:
                return Schema.create(Schema.Type.BOOLEAN);
            case BYTE:
            ...
            case LIST:
                return Schema.createArray(
                        createAvroSchemaWithNamespace(orcSchema.getChildren().get(0), recordName, ""));
            case MAP:
                return Schema.createMap(
                        createAvroSchemaWithNamespace(orcSchema.getChildren().get(1), recordName, ""));
            case STRUCT:
                List<Field> childFields = new ArrayList<>();
                for (int i = 0; i < orcSchema.getChildren().size(); i++) {
                    TypeDescription childType = orcSchema.getChildren().get(i);
                    ...
                    childFields.add(
                            new Field(
                                    childName, createAvroSchemaWithNamespace(childType, childName, ""), null, null));
                }
