...
public class OrcUtils extends FileFormatUtils {
    ...
    public ClosableIterator<HoodieKey> getHoodieKeyIterator(
        ...
        try {
            ...
            Reader reader =
                    OrcFile.createReader(convertToHadoopPath(filePath), OrcFile.readerOptions(conf));
            ...
            // column indices for the RECORD_KEY_METADATA_FIELD, PARTITION_PATH_METADATA_FIELD
            // fields
            int keyCol = -1;
            int partitionCol = -1;
    ...
    public List<Pair<HoodieKey, Long>> fetchRecordKeysWithPositions(
        ...
        try (ClosableIterator<HoodieKey> iterator =
                getHoodieKeyIterator(storage, filePath, Option.empty())) {
    ...
    public List<GenericRecord> readAvroRecords(
            HoodieStorage storage, StoragePath filePath, Schema avroSchema) {
    ...
    /**
      * Read the rowKey list matching the given filter, from the given ORC file. If the filter is
    ...
    public Set<Pair<String, Long>> filterRowKeys(
            HoodieStorage storage, StoragePath filePath, Set<String> filter) throws HoodieIOException {
        long rowPosition = 0;
        try (Reader reader =
                OrcFile.createReader(
                        convertToHadoopPath(filePath),
            ...
            try (RecordReader recordReader =
                    reader.rows(
                            new Options(storage.getConf().unwrapAs(Configuration.class)).schema(schema))) {
