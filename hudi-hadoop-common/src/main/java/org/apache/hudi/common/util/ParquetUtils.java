...
public class ParquetUtils extends FileFormatUtils {
    ...
    /**
      * Read the rowKey list matching the given filter, from the given parquet file. If the filter is
      * empty, then this will return all the rowkeys and corresponding positions.
      *
      ...
      */
    @Override
    public Set<Pair<String, Long>> filterRowKeys(
            HoodieStorage storage, StoragePath filePath, Set<String> filter) {
        return filterParquetRowKeys(
                storage, new Path(filePath.toUri()), filter, HoodieAvroUtils.getRecordKeySchema());
    }
        ...
        try {
            ...
            footer =
                    ParquetFileReader.readFooter(
                            storage
                                    .newInstance(parquetFilePath, storage.getConf())
                                    .getConf()
                                    .unwrapAs(Configuration.class),
                            parquetFileHadoopPath);
        } catch (IOException e) {
    ...
    /**
      * Read the rowKey list matching the given filter, from the given parquet file. If the filter is
      * empty, then this will return all the rowkeys.
      *
      * @param storage {@link HoodieStorage} instance.
      * @param filePath The parquet file path.
      * @param filter record keys filter
      ...
      */
    private static Set<Pair<String, Long>> filterParquetRowKeys(
            HoodieStorage storage, Path filePath, Set<String> filter, Schema readSchema) {
        ...
        conf.addResource(
                storage
                        .newInstance(convertToStoragePath(filePath), storage.getConf())
                        .getConf()
                        .unwrapAs(Configuration.class));
    ...
    /**
      ...
      * @param storage {@link HoodieStorage} instance.
      * @param filePath The parquet file path.
      * @return {@link List} of pairs of {@link HoodieKey} and row position fetched from the parquet
      *     file
      */
    @Override
    public List<Pair<HoodieKey, Long>> fetchRecordKeysWithPositions(
            HoodieStorage storage, StoragePath filePath) {
        return fetchRecordKeysWithPositions(storage, filePath, Option.empty());
    }

    @Override
    public ClosableIterator<HoodieKey> getHoodieKeyIterator(
            HoodieStorage storage, StoragePath filePath) {
    ...
    @Override
    public ClosableIterator<HoodieKey> getHoodieKeyIterator(
            HoodieStorage storage, StoragePath filePath, Option<BaseKeyGenerator> keyGeneratorOpt) {
        try {
            Configuration conf = storage.getConf().unwrapCopyAs(Configuration.class);
            conf.addResource(
                    storage.newInstance(filePath, storage.getConf()).getConf().unwrapAs(Configuration.class));
            Schema readSchema =
                    keyGeneratorOpt
                            .map(
                                    keyGenerator -> {
                                        ...
                                        return HoodieAvroUtils.getSchemaForFields(
                                                readAvroSchema(storage, filePath), fields);
                                    })
                            .orElse(HoodieAvroUtils.getRecordKeyPartitionPathSchema());
            AvroReadSupport.setAvroReadSchema(conf, readSchema);
            AvroReadSupport.setRequestedProjection(conf, readSchema);
            ParquetReader<GenericRecord> reader =
                    AvroParquetReader.<GenericRecord>builder(new Path(filePath.toUri()))
                            .withConf(conf)
                            .build();
    ...
    @Override
    public Map<String, String> readFooter(
            HoodieStorage storage, boolean required, StoragePath filePath, String... footerNames) {
        ...
        for (String footerName : footerNames) {
            if (metadata.containsKey(footerName)) {
                ...
                throw new MetadataNotFoundException(
                        "Could not find index in Parquet footer. Looked for key "
                                + footerName
                                + " in "
                                + filePath);
            }
    ...
    public Schema readAvroSchema(HoodieStorage storage, StoragePath filePath) {
        ...
        return new AvroSchemaConverter(storage.getConf().unwrapAs(Configuration.class))
                .convert(parquetSchema);
    }

    @Override
    public List<HoodieColumnRangeMetadata<Comparable>> readColumnStatsFromMetadata(
            HoodieStorage storage, StoragePath filePath, List<String> columnList) {
        ...
        Map<String, List<HoodieColumnRangeMetadata<Comparable>>> columnToStatsListMap =
                hoodieColumnRangeMetadataStream.collect(
        ...
        Stream<HoodieColumnRangeMetadata<Comparable>> stream =
                columnToStatsListMap.values().stream().map(this::getColumnRangeInFile);
    ...
    public List<GenericRecord> readAvroRecords(HoodieStorage storage, StoragePath filePath) {
        ...
        try (ParquetReader reader =
                AvroParquetReader.builder(new Path(filePath.toUri()))
    ...
    @Override
    public List<GenericRecord> readAvroRecords(
            HoodieStorage storage, StoragePath filePath, Schema schema) {
    ...
    public void writeMetaFile(HoodieStorage storage, StoragePath filePath, Properties props)
            throws IOException {
        // Since we are only interested in saving metadata to the footer, the schema, blocksizes and
        // other
    ...
    @Override
    public byte[] serializeRecordsToLogBlock(
        ...
        paramsMap.entrySet().stream()
                .forEach(entry -> config.setValue(entry.getKey(), entry.getValue()));
    ...
    private <T extends Comparable<T>> HoodieColumnRangeMetadata<T> getColumnRangeInFile(
        ...
        return blockRanges.stream().sequential().reduce(HoodieColumnRangeMetadata::merge).get();
    }

    private static Comparable<?> convertToNativeJavaType(
            PrimitiveType primitiveType, Comparable<?> val) {
        if (val == null) {
            return null;
        }
    ...
    private static BigDecimal extractDecimal(Object val, DecimalMetadata decimalMetadata) {
        ...
        //    3. FIXED_LEN_BYTE_ARRAY (precision is limited by the array size. Length n can store <=
        ...
        if (val == null) {
            ...
            throw new UnsupportedOperationException(
                    String.format("Unsupported value type (%s)", val.getClass().getName()));
        }
    }
    ...
    /**
      * An iterator that can apply the given function {@code func} to transform records from the
      * underneath record iterator to hoodie keys.
      */
    private static class HoodieKeyIterator implements ClosableIterator<HoodieKey> {
        ...
        public static HoodieKeyIterator getInstance(
                ClosableIterator<GenericRecord> nestedItr, Option<BaseKeyGenerator> keyGenerator) {
            return new HoodieKeyIterator(nestedItr, keyGenerator);
        }

        private HoodieKeyIterator(
                ClosableIterator<GenericRecord> nestedItr, Option<BaseKeyGenerator> keyGenerator) {
                ...
                this.func =
                        retVal -> {
                            ...
                            String partitionPath =
                                    retVal.get(HoodieRecord.PARTITION_PATH_METADATA_FIELD).toString();
                            return new HoodieKey(recordKey, partitionPath);
                        };
            }
        }
