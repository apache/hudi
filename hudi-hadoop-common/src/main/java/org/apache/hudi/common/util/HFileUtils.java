...
public class HFileUtils extends FileFormatUtils {
    ...
    public List<Pair<HoodieKey, Long>> fetchRecordKeysWithPositions(
        ...
        throw new UnsupportedOperationException(
                "HFileUtils does not support fetchRecordKeysWithPositions");
    }
    ...
    public Schema readAvroSchema(HoodieStorage storage, StoragePath filePath) {
        ...
        try (HoodieFileReader fileReader =
                HoodieIOFactory.getIOFactory(storage)
                        ...
                        .getFileReader(ConfigUtils.DEFAULT_HUDI_CONFIG_FOR_READER, filePath)) {
    ...
    public byte[] serializeRecordsToLogBlock(
        ...
        while (itr.hasNext()) {
            ...
            // Get the existing list of recordBytes for the recordKey, or an empty list if it
            // doesn't exist
        ...
        // Write the records
        sortedRecordsMap.forEach(
                (recordKey, recordBytesList) -> {
    ...
    private static Option<String> getRecordKey(
            HoodieRecord record, Schema readerSchema, String keyFieldName) {
        return Option.ofNullable(record.getRecordKey(readerSchema, keyFieldName));
    }

    private static byte[] serializeRecord(HoodieRecord<?> record, Schema schema, String keyFieldName)
            throws IOException {
