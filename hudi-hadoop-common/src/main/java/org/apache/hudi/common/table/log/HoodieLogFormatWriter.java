...
public class HoodieLogFormatWriter implements HoodieLogFormat.Writer {
            ...
            while (!created) {
                try {
                    ...
                    if (re.getClassName().contentEquals(AlreadyBeingCreatedException.class.getName())) {
                        LOG.warn(
                                "Another task executor writing to the same log file({}), rolling over", logFile);
    ...
    public AppendResult appendBlocks(List<HoodieLogBlock> blocks) throws IOException {
        ...
        long sizeWritten = 0;
        // HUDI-2655. here we wrap originalOutputStream to ensure huge blocks can be correctly
        // written
        FSDataOutputStream outputStream =
                new FSDataOutputStream(
                        originalOutputStream, new FileSystem.Statistics(storage.getScheme()), startPos);
        for (HoodieLogBlock block : blocks) {
            ...
            outputStream.writeLong(
                    getLogBlockLength(content.length, headerBytes.length, footerBytes.length));
            ...
            // 9. Write the total size of the log block (including magic) which is everything
            // written
            // until now (for reverse pointer)
            // Update: this information is now used in determining if a block is corrupt by
            // comparing to the
            //   block size in header. This change assumes that the block size will be the last data
            // written
    ...
    private int getLogBlockLength(int contentLength, int headerLength, int footerLength) {
        return Integer.BYTES
                + // Number of bytes to write version
                Integer.BYTES
                + // Number of bytes to write ordinal
                headerLength
                + // Length of the headers
                Long.BYTES
                + // Number of bytes used to write content length
                contentLength
                + // Length of the content
                footerLength
                + // Length of the footers
    ...
    private void rolloverIfNeeded() throws IOException {
        ...
        if (getCurrentSize() > sizeThreshold) {
            LOG.info(
                    "CurrentSize {} has reached threshold {}. Rolling over to the next version",
                    getCurrentSize(),
                    sizeThreshold);
            rollOver();
        }
    ...
    private void flush() throws IOException {
        ...
        output.flush();
        // NOTE : the following API call makes sure that the data is flushed to disk on DataNodes
        // (akin to POSIX fsync())
