...
public class TestHoodieHBaseHFileReaderWriter extends TestHoodieHFileReaderWriterBase {
    ...
    @Override
    protected HoodieAvroHFileReaderImplBase createHFileReader(HoodieStorage storage, byte[] content)
            throws IOException {
        FileSystem fs = HadoopFSUtils.getFs(getFilePath().toString(), new Configuration());
        return new HoodieHBaseAvroHFileReader(
                storage.getConf(), new StoragePath(DUMMY_BASE_PATH), storage, content, Option.empty());
    }

    @Override
    protected void verifyHFileReader(
            byte[] content,
            String hfileName,
    ...
    public void testReaderGetRecordIteratorByKeysWithBackwardSeek() throws Exception {
        ...
        try (HoodieAvroHFileReaderImplBase hfileReader =
            ...
            // Filter for "key00001, key05, key24, key16, key31, key61". Valid entries should be
            // matched.
            // Even though key16 exists, it's a backward seek not in order. So, will not return the
            // matched entry.
            ...
            Iterator<IndexedRecord> iterator =
                    hfileReader.getIndexedRecordsByKeysIterator(
                            Arrays.asList("key00001", "key05", "key24", "key16", "key31", "key61"), avroSchema);
    ...
    void generateHFileForTesting(
            int blockSizeKB, String compressionCodec, int numEntries, boolean uniqueKeys)
            throws IOException {
        writeHFileForTesting(
                String.format(
                        "/tmp/hudi_1_0_hbase_2_4_9_%sKB_%s_%s.hfile",
    ...
    /**
      * Test HFile reader with duplicates. HFile can have duplicates in case of secondary index for
      * instance.
      */
    @Test
    public void testHFileReaderWriterWithDuplicates() throws Exception {
        Schema avroSchema =
                getSchemaFromResource(TestHoodieOrcReaderWriter.class, "/exampleSchema.avsc");
        ...
        for (int i = 0; i < 50; i++) {
            ...
            String key =
                    i != 0 && i % 10 == 0
                            ? String.format("%s%04d", "key", i - 1)
                            : String.format("%s%04d", "key", i);
        ...
        try (HoodieAvroHFileReaderImplBase hFileReader =
            ...
            assertEquals(
                    recordMap.values().stream().flatMap(List::stream).collect(Collectors.toList()), records);
        }
        ...
        for (int i = 0; i < 2; i++) {
            ...
            List<GenericRecord> expectedRecords =
                    rowsList.stream()
                            .flatMap(row -> recordMap.get(row).stream())
                            .collect(Collectors.toList());

            try (HoodieAvroHFileReaderImplBase hFileReader =
                    (HoodieAvroHFileReaderImplBase) createReader(HoodieTestUtils.getStorage(getFilePath()))) {
                List<GenericRecord> result =
                        HoodieAvroHFileReaderImplBase.readRecords(hFileReader, rowsList).stream()
                                .map(r -> (GenericRecord) r)
                                .collect(Collectors.toList());
