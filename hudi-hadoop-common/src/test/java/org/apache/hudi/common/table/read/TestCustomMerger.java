...
public class TestCustomMerger extends HoodieFileGroupReaderTestHarness {
    ...
    public static void setUp() throws IOException {
        ...
        readerContext =
                new HoodieTestReaderContext(
                        Option.of(new CustomAvroMerger()), Option.of(HoodieRecordTestPayload.class.getName()));
    ...
    /**
      * This merger is designed to save records whose record key is odd. That means, if the record is
      * not a delete record, and its record key is odd, then it will be output. Before the flush stage,
      * we only flush records whose timestamp is multiple of 3; but since the write is insert
      * operation, it will not go through the flush stage. Therefore, in this test, we solely test the
      * merge function.
      */
    public static class CustomAvroMerger implements HoodieRecordMerger {
        ...
        public Option<Pair<HoodieRecord, Schema>> merge(
                ...
                throws IOException {
            if (newer
                            .getOrderingValue(newSchema, props)
                            .compareTo(older.getOrderingValue(oldSchema, props))
                    >= 0) {
                ...
                int id =
                        Integer.parseInt(
                                ((HoodieAvroIndexedRecord) newer)
                                        .getData()
                                        .get(newSchema.getField(ROW_KEY).pos())
                                        .toString());
                if (id % 2 == 1L) {
                ...
                int id =
                        Integer.parseInt(
                                ((HoodieAvroIndexedRecord) older)
                                        .getData()
                                        .get(oldSchema.getField(ROW_KEY).pos())
                                        .toString());
                if (id % 2 == 1L) {
        ...
        public boolean shouldFlush(HoodieRecord record, Schema schema, TypedProperties props) {
            long timestamp =
                    (long) ((HoodieAvroIndexedRecord) record).getData().get(schema.getField(TIMESTAMP).pos());
