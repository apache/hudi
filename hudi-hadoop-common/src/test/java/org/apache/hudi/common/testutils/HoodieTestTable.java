...
public class HoodieTestTable implements AutoCloseable {
    ...
    protected HoodieTestTable(
            String basePath, HoodieStorage storage, HoodieTableMetaClient metaClient) {
    ...
    protected HoodieTestTable(
        ...
        ValidationUtils.checkArgument(
                Objects.equals(storage.getFileSystem(), metaClient.getRawHoodieStorage().getFileSystem()));
    ...
    public static HoodieTestTable of(HoodieTableMetaClient metaClient) {
        testTableState = HoodieTestTableState.of();
        return new HoodieTestTable(
                metaClient.getBasePath().toString(), metaClient.getRawHoodieStorage(), metaClient);
    }
    ...
    public static List<String> makeIncrementalCommitTimes(
            int num, int firstOffsetSeconds, int deltaSecs) {
                ...
                .mapToObj(
                        i ->
                                makeNewCommitTime(
                                        now.plus(
                                                deltaSecs == 0
                                                        ? (firstOffsetSeconds + i)
                                                        : (i == 0 ? (firstOffsetSeconds) : (i * deltaSecs) + i),
                                                SECONDS)))
    ...
    public HoodieTestTable addCommit(String instantTime, Option<HoodieCommitMetadata> metadata)
            throws Exception {
        return addCommit(instantTime, Option.empty(), metadata);
    }

    public HoodieTestTable addCommit(
            String instantTime, Option<String> completionTime, Option<HoodieCommitMetadata> metadata)
            throws Exception {
        createRequestedCommit(basePath, instantTime);
        createInflightCommit(basePath, instantTime);
    ...
    public HoodieTestTable addSavepointCommit(
            String instantTime, HoodieSavepointMetadata savepointMetadata) throws IOException {
        createInflightSavepoint(basePath, instantTime);
        createSavepointCommit(basePath, instantTime, savepointMetadata);
        return this;
    }

    public HoodieCommitMetadata createCommitMetadata(
            String commitTime,
        ...
        Map<String, List<Pair<String, Integer>>> partitionToFilesNameLengthMap =
                getPartitionFiles(partitions, filesPerPartition);
        HoodieTestTableState testTableState =
                getTestTableStateWithPartitionFileInfo(
                        operationType, metaClient.getTableType(), commitTime, partitionToFilesNameLengthMap);
        return createCommitMetadata(operationType, commitTime, testTableState, bootstrap);
    }

    public HoodieCommitMetadata createCommitMetadata(
            WriteOperationType operationType, String commitTime, HoodieTestTableState testTableState) {
        String actionType = getCommitActionType(operationType, metaClient.getTableType());
        return createCommitMetadata(
                operationType, commitTime, Collections.emptyMap(), testTableState, false, actionType);
    }
    ...
    public HoodieCommitMetadata createCommitMetadata(
        ...
        String actionType = getCommitActionType(operationType, metaClient.getTableType());
        return createCommitMetadata(
                operationType, commitTime, Collections.emptyMap(), testTableState, bootstrap, actionType);
    }
    ...
    public HoodieCommitMetadata createCommitMetadata(
        ...
        List<HoodieWriteStat> writeStats =
                generateHoodieWriteStatForPartition(
                        testTableState.getPartitionToBaseFileInfoMap(commitTime), commitTime, bootstrap);
        if (MERGE_ON_READ.equals(metaClient.getTableType()) && UPSERT.equals(operationType)) {
            writeStats.addAll(
                    generateHoodieWriteStatForPartitionLogFiles(
                            testTableState.getPartitionToLogFileInfoMap(commitTime), commitTime, bootstrap));
        }
    ...
    public HoodieTestTable moveInflightCommitToComplete(
            String instantTime, HoodieCommitMetadata metadata) throws IOException {
    ...
    public HoodieTestTable addDeltaCommit(String instantTime, HoodieCommitMetadata metadata)
            throws Exception {
        createRequestedDeltaCommit(basePath, instantTime);
        createInflightDeltaCommit(basePath, instantTime);
        createDeltaCommit(basePath, instantTime, metadata);
    ...
    public HoodieTestTable addRequestedReplace(
            String instantTime, Option<HoodieRequestedReplaceMetadata> requestedReplaceMetadata)
            throws Exception {
        createRequestedReplaceCommit(basePath, instantTime, requestedReplaceMetadata);
        currentInstantTime = instantTime;
        return this;
    ...
    public HoodieTestTable addInflightReplace(
            String instantTime, Option<HoodieCommitMetadata> inflightReplaceMetadata) throws Exception {
        createInflightReplaceCommit(basePath, instantTime, inflightReplaceMetadata);
        currentInstantTime = instantTime;
        return this;
    }

    public HoodieTestTable addInflightClean(String instantTime, HoodieCleanerPlan cleanerPlan)
            throws IOException {
    ...
    public HoodieTestTable addClean(
            ...
            boolean isEmptyForAll,
            boolean isEmptyCompleted)
            throws IOException {
    ...
    public HoodieTestTable addClean(String instantTime) throws IOException {
        HoodieCleanerPlan cleanerPlan =
                new HoodieCleanerPlan(
                        new HoodieActionInstant(EMPTY_STRING, EMPTY_STRING, EMPTY_STRING),
                        EMPTY_STRING,
                        EMPTY_STRING,
                        new HashMap<>(),
        ...
        HoodieCleanStat cleanStats =
                new HoodieCleanStat(
                        HoodieCleaningPolicy.KEEP_LATEST_FILE_VERSIONS,
                        HoodieTestUtils.DEFAULT_PARTITION_PATHS[
        ...
        HoodieCleanMetadata cleanMetadata =
                convertCleanMetadata(
                        instantTime,
                        Option.of(0L),
                        Collections.singletonList(cleanStats),
                        Collections.EMPTY_MAP);
        return HoodieTestTable.of(metaClient).addClean(instantTime, cleanerPlan, cleanMetadata);
    }

    public Pair<HoodieCleanerPlan, HoodieCleanMetadata> getHoodieCleanMetadata(
            String commitTime, HoodieTestTableState testTableState) {
        HoodieCleanerPlan cleanerPlan =
                new HoodieCleanerPlan(
                        new HoodieActionInstant(commitTime, CLEAN_ACTION, EMPTY_STRING),
                        EMPTY_STRING,
                        EMPTY_STRING,
                        new HashMap<>(),
                        CleanPlanV2MigrationHandler.VERSION,
    ...
    public HoodieTestTable addRequestedRollback(String instantTime, HoodieRollbackPlan plan)
            throws IOException {
    ...
    public HoodieTestTable addRollback(
            String instantTime, HoodieRollbackMetadata rollbackMetadata, HoodieRollbackPlan rollbackPlan)
            throws IOException {
    ...
    public HoodieTestTable addRollbackCompleted(
            String instantTime, HoodieRollbackMetadata rollbackMetadata, boolean isEmpty)
            throws IOException {
        createRollbackFile(basePath, instantTime, rollbackMetadata, isEmpty);
        currentInstantTime = instantTime;
        return this;
    ...
    public HoodieTestTable addRestore(String instantTime, HoodieRestoreMetadata restoreMetadata)
            throws IOException {
    ...
    public HoodieRollbackMetadata getRollbackMetadata(
            String instantTimeToDelete,
            Map<String, List<String>> partitionToFilesMeta,
            boolean shouldAddRollbackLogFile)
            throws Exception {
        ...
        for (Map.Entry<String, List<String>> entry : partitionToFilesMeta.entrySet()) {
            HoodieRollbackPartitionMetadata rollbackPartitionMetadata =
            ...
            if (shouldAddRollbackLogFile) {
                ...
                FileCreateUtils.createLogFile(
                        basePath, entry.getKey(), instantTimeToDelete, fileId, 0, (int) rollbackLogFileSize);
                rollbackPartitionMetadata.setRollbackLogFiles(
                        singletonMap(logFileName, rollbackLogFileSize));
            }
        ...
        rollbackMetadata.setInstantsRollback(
                Collections.singletonList(
                        new HoodieInstantInfo(instantTimeToDelete, HoodieTimeline.ROLLBACK_ACTION)));
        return rollbackMetadata;
    }

    /**
      * Return a map of log file name to file size that were expected to be rolled back in that
      * partition.
      */
    private Map<String, Long> getWrittenLogFiles(
            String instant, Map.Entry<String, List<String>> entry) {
    ...
    public HoodieSavepointMetadata getSavepointMetadata(
            String instant, Map<String, List<String>> partitionToFilesMeta) {
        ...
        for (Map.Entry<String, List<String>> entry : partitionToFilesMeta.entrySet()) {
            HoodieSavepointPartitionMetadata savepointPartitionMetadata =
    ...
    public HoodieTestTable addRequestedCompaction(String instantTime) throws IOException {
        ...
        HoodieCompactionPlan compactionPlan =
                CompactionUtils.buildFromFileSlices(
                        fileSlices.stream()
                                .map(fs -> Pair.of(fs.getPartitionPath(), fs))
                                .collect(Collectors.toList()),
                        Option.empty(),
                        Option.empty());
        ...
        metaClient
                ...
                .saveToCompactionRequested(
                        compactionInstant, TimelineMetadataUtils.serializeCompactionPlan(compactionPlan));
        currentInstantTime = instantTime;
        return this;
    }

    public HoodieTestTable addRequestedCompaction(
    ...
    public HoodieTestTable addInflightCompaction(
            String instantTime, HoodieCommitMetadata commitMetadata) throws Exception {
        List<FileSlice> fileSlices = new ArrayList<>();
    ...
    public HoodieTestTable addCompaction(String instantTime, HoodieCommitMetadata commitMetadata)
            throws Exception {
    ...
    public HoodieTestTable addDeletePartitionCommit(
            String instantTime, String partition, List<String> fileIds) throws Exception {
        ...
        Pair<HoodieRequestedReplaceMetadata, HoodieReplaceCommitMetadata> metas =
                generateReplaceCommitMetadata(
                        instantTime, partition, fileIds, Option.empty(), operationType);
        return addReplaceCommit(
                instantTime, Option.of(metas.getLeft()), Option.empty(), metas.getRight());
    }
    ...
    private Pair<HoodieRequestedReplaceMetadata, HoodieReplaceCommitMetadata>
        ...
        requestedReplaceMetadata.setClusteringPlan(
                HoodieClusteringPlan.newBuilder()
                        ...
                        .setStrategy(
                                HoodieClusteringStrategy.newBuilder()
        ...
        replacedFileIds.forEach(
                replacedFileId -> replaceMetadata.addReplaceFileId(partition, replacedFileId));
            ...
            writeStat.setPath(
                    partition
                            ...
                            + FSUtils.makeBaseFileName(
                                    instantTime, "1-0-1", newFileId.get(), BASE_FILE_EXTENSION));
    ...
    public HoodieTestTable moveInflightCompactionToComplete(
            String instantTime, HoodieCommitMetadata metadata) throws IOException {
        createCommit(basePath, instantTime, Option.of(metadata));
        inflightCommits.remove(instantTime);
        currentInstantTime = instantTime;
    ...
    public HoodieTestTable addSavepoint(String instantTime, HoodieSavepointMetadata savepointMetadata)
            throws IOException {
        createInflightSavepoint(basePath, instantTime);
        createSavepointCommit(basePath, instantTime, savepointMetadata);
        return this;
    ...
    public HoodieTestTable withMarkerFile(String partitionPath, String fileId, IOType ioType)
            throws IOException {
        createMarkerFile(basePath, partitionPath, currentInstantTime, fileId, ioType);
        return this;
    }

    public HoodieTestTable withMarkerFiles(String partitionPath, int num, IOType ioType)
            throws IOException {
        String[] fileIds =
                IntStream.range(0, num).mapToObj(i -> UUID.randomUUID().toString()).toArray(String[]::new);
        return withMarkerFiles(partitionPath, fileIds, ioType);
    }

    public HoodieTestTable withMarkerFiles(String partitionPath, String[] fileIds, IOType ioType)
            throws IOException {
    ...
    public HoodieTestTable withLogMarkerFile(String partitionPath, String fileId, IOType ioType)
            throws IOException {
        String logFileName =
    ...
    /**
      ...
      */
    public Map<String, String> getFileIdsWithBaseFilesInPartitions(String... partitions)
            throws Exception {
    ...
    public Pair<HoodieTestTable, List<String>> withBaseFilesInPartitions(
            Map<String, String> partitionAndFileId) throws Exception {
    ...
    public Pair<HoodieTestTable, List<String>> withBaseFilesInPartition(
            String partition, String... fileIds) throws Exception {
        List<String> files = new ArrayList<>();
    ...
    public HoodieTestTable withBaseFilesInPartition(String partition, int... lengths)
            throws Exception {
        for (int l : lengths) {
            String fileId = UUID.randomUUID().toString();
    ...
    public HoodieTestTable withBaseFilesInPartition(
            String partition, List<Pair<String, Integer>> fileInfos) throws Exception {
        for (Pair<String, Integer> fileInfo : fileInfos) {
            FileCreateUtils.createBaseFile(
                    basePath, partition, currentInstantTime, fileInfo.getKey(), fileInfo.getValue());
        }
        return this;
    }
    ...
    public Pair<HoodieTestTable, List<String>> withLogFile(
            String partitionPath, String fileId, int... versions) throws Exception {
        List<String> logFiles = new ArrayList<>();
        for (int version : versions) {
            logFiles.add(
                    FileCreateUtils.createLogFile(
                            basePath, partitionPath, currentInstantTime, fileId, version));
        }
        return Pair.of(this, logFiles);
    }

    public HoodieTestTable withLogFilesInPartition(
            String partition, List<Pair<String, Integer[]>> fileInfos) throws Exception {
        for (Pair<String, Integer[]> fileInfo : fileInfos) {
            FileCreateUtils.createLogFile(
                    basePath,
    ...
    public boolean baseFilesExist(Map<String, String> partitionAndFileId, String instantTime) {
        return partitionAndFileId.entrySet().stream()
                .allMatch(
                        entry -> {
    ...
    public boolean baseFileExists(String partition, String instantTime, String fileId) {
        try {
            return fs.exists(
                    new Path(Paths.get(basePath, partition, baseFileName(instantTime, fileId)).toString()));
        } catch (IOException e) {
            throw new HoodieTestTableException(e);
        }
    }
    ...
    public boolean logFilesExist(
            String partition, String instantTime, String fileId, int... versions) {
    ...
    public boolean logFileExists(String partition, String instantTime, String fileId, int version) {
        try {
            return fs.exists(
                    new Path(
                            Paths.get(basePath, partition, logFileName(instantTime, fileId, version))
                                    .toString()));
        } catch (IOException e) {
            throw new HoodieTestTableException(e);
        }
    ...
    public Path getInflightCommitFilePath(String instantTime) {
        return new Path(
                Paths.get(
                                ...
                                instantTime + HoodieTimeline.INFLIGHT_COMMIT_EXTENSION)
                        .toUri());
    }

    public StoragePath getCommitFilePath(String instantTime) {
        return HoodieTestUtils.getCompleteInstantPath(
                storage,
    ...
    public Path getRequestedCompactionFilePath(String instantTime) {
        return new Path(
                Paths.get(
                                ...
                                instantTime + HoodieTimeline.REQUESTED_COMPACTION_EXTENSION)
                        .toUri());
    }
    ...
    public List<String> getEarliestFilesInPartition(String partition, int count) throws IOException {
        ...
        return fileStatuses.subList(0, count).stream()
                .map(entry -> entry.getPath().getName())
                .collect(Collectors.toList());
    }
    ...
    public static List<FileStatus> listFiles(FileSystem fs, Path path, boolean recursive)
            throws IOException {
    ...
    public List<StoragePathInfo> listAllLogFiles(String fileExtension) throws IOException {
        return listRecursive(storage, new StoragePath(basePath)).stream()
                .filter(
                        fileInfo ->
                                !fileInfo.getPath().toString().contains(HoodieTableMetaClient.METAFOLDER_NAME))
    ...
    public FileStatus[] listAllFilesInPartition(String partitionPath) throws IOException {
        return listRecursive(fs, new Path(Paths.get(basePath, partitionPath).toString())).stream()
                .filter(
                        entry -> {
    ...
    public FileStatus[] listAllFilesInTempFolder() throws IOException {
        return listRecursive(
                        fs, new Path(Paths.get(basePath, HoodieTableMetaClient.TEMPFOLDER_NAME).toString()))
                .toArray(new FileStatus[0]);
    }

    public void deleteFilesInPartition(String partitionPath, List<String> filesToDelete)
            throws IOException {
        FileStatus[] allFiles = listAllFilesInPartition(partitionPath);
        Arrays.stream(allFiles)
                .filter(entry -> filesToDelete.contains(entry.getPath().getName()))
                .forEach(
                        entry -> {
                            try {
    ...
    public HoodieTestTable doRollback(String commitTimeToRollback, String commitTime)
        ...
        if (!commitMetadata.isPresent()) {
            throw new IllegalArgumentException(
                    "Instant to rollback not present in timeline: " + commitTimeToRollback);
        }
    ...
    private HoodieRollbackPlan getHoodieRollbackPlan(
            String commitTime, Map<String, List<String>> partitionFiles) {
        ...
        List<HoodieRollbackRequest> rollbackRequestList =
                partitionFiles.keySet().stream()
                        .map(
                                partition ->
                                        new HoodieRollbackRequest(
                                                partition,
                                                EMPTY_STRING,
                                                EMPTY_STRING,
                                                partitionFiles.get(partition),
                                                Collections.emptyMap()))
                        .collect(Collectors.toList());
        rollbackPlan.setRollbackRequests(rollbackRequestList);
        rollbackPlan.setInstantToRollback(
                new HoodieInstantInfo(commitTime, HoodieTimeline.COMMIT_ACTION));
        return rollbackPlan;
    }

    public HoodieTestTable doRollbackWithExtraFiles(
            String commitTimeToRollback, String commitTime, Map<String, List<String>> extraFiles)
            throws Exception {
        metaClient = HoodieTableMetaClient.reload(metaClient);
        Option<HoodieCommitMetadata> commitMetadata = getMetadataForInstant(commitTimeToRollback);
        if (!commitMetadata.isPresent()) {
    ...
    public HoodieTestTable doRestore(String commitToRestoreTo, String restoreTime) throws Exception {
        ...
        List<HoodieInstant> commitsToRollback =
                metaClient
                        ...
                        .findInstantsAfter(commitToRestoreTo)
        ...
        for (HoodieInstant commitInstantToRollback : commitsToRollback) {
            ...
            if (!commitMetadata.isPresent()) {
                throw new IllegalArgumentException(
                        "Instant to rollback not present in timeline: "
                                + commitInstantToRollback.getTimestamp());
            }
            Map<String, List<String>> partitionFiles = getPartitionFiles(commitMetadata.get());
            rollbackMetadataMap.put(
                    commitInstantToRollback.getTimestamp(),
                    Collections.singletonList(
                            getRollbackMetadata(commitInstantToRollback.getTimestamp(), partitionFiles, false)));
            for (Map.Entry<String, List<String>> entry : partitionFiles.entrySet()) {
                deleteFilesInPartition(entry.getKey(), entry.getValue());
            }
        }
        ...
        HoodieRestoreMetadata restoreMetadata =
                TimelineMetadataUtils.convertRestoreMetadata(
                        restoreTime, 1000L, commitsToRollback, rollbackMetadataMap);
        return addRestore(restoreTime, restoreMetadata);
    }

    public HoodieReplaceCommitMetadata doCluster(
            String commitTime,
            Map<String, List<String>> partitionToReplaceFileIds,
            List<String> partitions,
            int filesPerPartition)
        ...
        HoodieTestTableState testTableState =
                getTestTableStateWithPartitionFileInfo(
                        CLUSTER, metaClient.getTableType(), commitTime, partitions, filesPerPartition);
        ...
        List<HoodieWriteStat> writeStats =
                generateHoodieWriteStatForPartition(
                        testTableState.getPartitionToBaseFileInfoMap(commitTime), commitTime, false);
        for (String partition : testTableState.getPartitionToBaseFileInfoMap(commitTime).keySet()) {
            this.withBaseFilesInPartition(
                    partition, testTableState.getPartitionToBaseFileInfoMap(commitTime).get(partition));
        }
    ...
    public HoodieCleanMetadata doClean(
            String commitTime, Map<String, Integer> partitionFileCountsToDelete) throws IOException {
        ...
        for (Map.Entry<String, Integer> entry : partitionFileCountsToDelete.entrySet()) {
            partitionFilesToDelete.put(
                    entry.getKey(), getEarliestFilesInPartition(entry.getKey(), entry.getValue()));
        }
        ...
        for (Map.Entry<String, List<String>> entry : partitionFilesToDelete.entrySet()) {
            testTableState =
                    testTableState.createTestTableStateForCleaner(
                            commitTime, entry.getKey(), entry.getValue());
            deleteFilesInPartition(entry.getKey(), entry.getValue());
        }
    ...
    public void repeatClean(
            String cleanCommitTime, HoodieCleanerPlan cleanerPlan, HoodieCleanMetadata cleanMetadata)
            throws IOException {
        addClean(cleanCommitTime, cleanerPlan, cleanMetadata);
    }

    public HoodieCleanMetadata doCleanBasedOnCommits(
            String cleanCommitTime, List<String> commitsToClean) throws IOException {
        ...
        for (String commitTime : commitsToClean) {
            ...
            if (commitMetadata.isPresent()) {
                ...
                for (String partition : partitionFiles.keySet()) {
                    partitionFileCountsToDelete.put(
                            partition,
    ...
    public HoodieSavepointMetadata doSavepoint(String commitTime) throws IOException {
        ...
        if (!commitMetadata.isPresent()) {
            throw new IllegalArgumentException(
                    "Instant to rollback not present in timeline: " + commitTime);
        }
        Map<String, List<String>> partitionFiles = getPartitionFiles(commitMetadata.get());
    ...
    public HoodieCommitMetadata doCompaction(String commitTime, List<String> partitions)
            throws Exception {
        return doCompaction(commitTime, partitions, false);
    }

    public HoodieCommitMetadata doCompaction(
            String commitTime, List<String> partitions, boolean inflight) throws Exception {
        this.currentInstantTime = commitTime;
        ...
        HoodieTestTableState testTableState =
                getTestTableStateWithPartitionFileInfo(
                        COMPACT, metaClient.getTableType(), commitTime, partitions, 1);
    ...
    public HoodieCommitMetadata doWriteOperation(
            String commitTime,
            WriteOperationType operationType,
            List<String> partitions,
            int filesPerPartition)
        ...
        return doWriteOperation(
                commitTime, operationType, Collections.emptyList(), partitions, filesPerPartition, false);
    }
    ...
    public HoodieCommitMetadata doWriteOperation(
            ...
            throws Exception {
        return doWriteOperation(
                commitTime, operationType, newPartitionsToAdd, partitions, filesPerPartition, false);
    }
    ...
    /**
      ...
      * @param partitionToFilesNameLengthMap - Map of partition names to its list of files name and
      *     length pair
    ...
    public HoodieCommitMetadata doWriteOperation(
        ...
        HoodieCommitMetadata commitMetadata =
                createCommitMetadata(operationType, commitTime, testTableState, bootstrap);
        ...
        for (Map.Entry<String, List<Pair<String, Integer>>> entry :
            ...
            this.withBaseFilesInPartition(
                    partition, testTableState.getPartitionToBaseFileInfoMap(commitTime).get(partition));
            if (MERGE_ON_READ.equals(metaClient.getTableType()) && UPSERT.equals(operationType)) {
                this.withLogFilesInPartition(
                        partition, testTableState.getPartitionToLogFileInfoMap(commitTime).get(partition));
            }
        }
        return commitMetadata;
    }
    ...
    private Option<HoodieCommitMetadata> getMetadataForInstant(String instantTime) {
        ...
        Option<HoodieInstant> hoodieInstant =
                metaClient
                        ...
                        .getCommitsTimeline()
                        .filterCompletedInstants()
                        .filter(i -> i.getTimestamp().equals(instantTime))
                        .firstInstant();
        try {
    ...
    private Option<HoodieCommitMetadata> getCommitMeta(HoodieInstant hoodieInstant)
            throws IOException {
    ...
    private static Map<String, List<String>> getPartitionFiles(HoodieCommitMetadata commitMetadata) {
        ...
        for (Map.Entry<String, List<HoodieWriteStat>> entry : partitionToWriteStats.entrySet()) {
            ...
            entry
                    .getValue()
                    .forEach(
                            writeStat -> partitionFilesToDelete.get(entry.getKey()).add(writeStat.getFileId()));
        }
    ...
    /**
      ...
      */
    protected static Map<String, List<Pair<String, Integer>>> getPartitionFiles(
        ...
        for (String partition : partitions) {
            Stream<Integer> fileLengths =
                    IntStream.range(0, filesPerPartition).map(i -> 100 + RANDOM.nextInt(500)).boxed();
            List<Pair<String, Integer>> fileNameAndLengthList =
                    fileLengths
                            .map(len -> Pair.of(UUID.randomUUID().toString(), len))
                            .collect(Collectors.toList());
    ...
    /**
      ...
      */
    private static HoodieTestTableState getTestTableStateWithPartitionFileInfo(
            WriteOperationType operationType,
        ...
        Map<String, List<Pair<String, Integer>>> partitionToFilesNameLengthMap =
                getPartitionFiles(partitions, filesPerPartition);
        return getTestTableStateWithPartitionFileInfo(
                operationType, tableType, commitTime, partitionToFilesNameLengthMap);
    }
    ...
    /**
      ...
      * @param tableType - Hudi table type
      * @param commitTime - Write commit time
      * @param partitionToFilesNameLengthMap - Map of partition names to its list of files and their
      *     lengths
    ...
    private static HoodieTestTableState getTestTableStateWithPartitionFileInfo(
            ...
            String commitTime,
            Map<String, List<Pair<String, Integer>>> partitionToFilesNameLengthMap) {
        for (Map.Entry<String, List<Pair<String, Integer>>> partitionEntry :
                partitionToFilesNameLengthMap.entrySet()) {
            ...
            if (MERGE_ON_READ.equals(tableType) && UPSERT.equals(operationType)) {
                List<Pair<Integer, Integer>> fileVersionAndLength =
                        fileNameAndLengthList.stream()
                                .map(nameLengthPair -> Pair.of(0, nameLengthPair.getRight()))
                                .collect(Collectors.toList());
                testTableState =
                        testTableState.createTestTableStateForBaseAndLogFiles(
                                commitTime, partitionName, fileVersionAndLength);
            } else {
                testTableState =
                        testTableState.createTestTableStateForBaseFilesOnly(
                                commitTime, partitionName, fileNameAndLengthList);
            }
        }
        return testTableState;
    }

    public static List<HoodieWriteStat> generateHoodieWriteStatForPartition(
        ...
        for (Map.Entry<String, List<Pair<String, Integer>>> entry : partitionToFileIdMap.entrySet()) {
            ...
            for (Pair<String, Integer> fileIdInfo : entry.getValue()) {
                ...
                writeStat.setPartitionPath(partition);
                writeStat.setPath(
                        StringUtils.isNullOrEmpty(partition) ? fileName : partition + "/" + fileName);
    ...
    /**
      * Returns the write stats for log files in the partition. Since log file has version associated
      * with it, the {@param partitionToFileIdMap} contains list of Pair<String, Integer[]> where the
      * Integer[] array has both file version and file size.
      */
    private static List<HoodieWriteStat> generateHoodieWriteStatForPartitionLogFiles(
            Map<String, List<Pair<String, Integer[]>>> partitionToFileIdMap,
            String commitTime,
            boolean bootstrap) {
        List<HoodieWriteStat> writeStats = new ArrayList<>();
        ...
        for (Map.Entry<String, List<Pair<String, Integer[]>>> entry : partitionToFileIdMap.entrySet()) {
            ...
            for (Pair<String, Integer[]> fileIdInfo : entry.getValue()) {
                ...
                String fileName =
                        bootstrap
                                ? fileIdInfo.getKey()
                                : FileCreateUtils.logFileName(
                                        commitTime, fileIdInfo.getKey(), fileIdInfo.getValue()[0]);
                writeStat.setFileId(fileName);
                writeStat.setPartitionPath(partition);
                writeStat.setPath(
                        StringUtils.isNullOrEmpty(partition) ? fileName : partition + "/" + fileName);
    ...
    static class HoodieTestTableState {
        /** Map<commitTime, Map<partitionPath, List<filesToDelete>>> Used in building CLEAN metadata. */
        Map<String, Map<String, List<String>>> commitsToPartitionToFileIdForCleaner = new HashMap<>();
        /**
          * Map<commitTime, Map<partitionPath, List<Pair<fileName, fileLength>>>> Used to build commit
          * metadata for base files for several write operations.
          */
        Map<String, Map<String, List<Pair<String, Integer>>>> commitsToPartitionToBaseFileInfoStats =
                new HashMap<>();
        /**
          * Map<commitTime, Map<partitionPath, List<Pair<fileName, [fileVersion, fileLength]>>>> Used to
          * build commit metadata for log files for several write operations.
          */
        Map<String, Map<String, List<Pair<String, Integer[]>>>> commitsToPartitionToLogFileInfoStats =
                new HashMap<>();
        ...
        HoodieTestTableState createTestTableStateForCleaner(
                String commitTime, String partitionPath, List<String> filesToClean) {
            ...
            if (!this.commitsToPartitionToFileIdForCleaner.get(commitTime).containsKey(partitionPath)) {
                this.commitsToPartitionToFileIdForCleaner
                        .get(commitTime)
                        .put(partitionPath, new ArrayList<>());
            }
        ...
        HoodieTestTableState createTestTableStateForBaseFileLengthsOnly(
                String commitTime, String partitionPath, List<Integer> lengths) {
        ...
        HoodieTestTableState createTestTableStateForBaseFilesOnly(
                String commitTime,
            ...
            if (!this.commitsToPartitionToBaseFileInfoStats.get(commitTime).containsKey(partitionPath)) {
                this.commitsToPartitionToBaseFileInfoStats
                        .get(commitTime)
                        .put(partitionPath, new ArrayList<>());
            }
            ...
            this.commitsToPartitionToBaseFileInfoStats
                    .get(commitTime)
                    .get(partitionPath)
                    .addAll(fileNameAndLengthList);
            return this;
        }

        HoodieTestTableState createTestTableStateForBaseAndLogFiles(
                String commitTime, String partitionPath, List<Pair<Integer, Integer>> versionsAndLengths) {
            if (!commitsToPartitionToBaseFileInfoStats.containsKey(commitTime)) {
            ...
            if (!this.commitsToPartitionToLogFileInfoStats.get(commitTime).containsKey(partitionPath)) {
                this.commitsToPartitionToLogFileInfoStats
                        .get(commitTime)
                        .put(partitionPath, new ArrayList<>());
            }
            ...
            for (int i = 0; i < versionsAndLengths.size(); i++) {
                ...
                String fileId =
                        FSUtils.getFileId(
                                commitsToPartitionToBaseFileInfoStats
                                        .get(commitTime)
                                        .get(partitionPath)
                                        .get(i)
                                        .getLeft());
                fileInfos.add(
                        Pair.of(
                                fileId, new Integer[] {versionAndLength.getLeft(), versionAndLength.getRight()}));
            }
            this.commitsToPartitionToLogFileInfoStats
                    .get(commitTime)
                    .get(partitionPath)
