...
public class TestOverwriteWithLatestMerger extends HoodieFileGroupReaderTestHarness {
    ...
    public static void setUp() throws IOException {
        ...
        readerContext =
                new HoodieTestReaderContext(
                        Option.of(merger), Option.of(OverwriteWithLatestAvroPayload.class.getName()));
        ...
        operationTypes = Arrays.asList(INSERT, DELETE, UPDATE, DELETE, UPDATE);
    ...
    public void testWithOneLogFile(boolean useRecordPositions)
    ...
    @ParameterizedTest
    @ValueSource(booleans = {true, false})
    public void testWithTwoLogFiles(boolean useRecordPositions)
            throws IOException, InterruptedException {
        shouldWritePositions =
                Arrays.asList(useRecordPositions, useRecordPositions, useRecordPositions);
    ...
    @ParameterizedTest
    @ValueSource(booleans = {true, false})
    public void testWithThreeLogFiles(boolean useRecordPositions)
            throws IOException, InterruptedException {
        shouldWritePositions =
                Arrays.asList(
                        useRecordPositions, useRecordPositions, useRecordPositions, useRecordPositions);
    ...
    public void testPositionMergeFallback(
            boolean log1haspositions,
            boolean log2haspositions,
            boolean log3haspositions,
            boolean log4haspositions)
            throws IOException, InterruptedException {
        shouldWritePositions =
                Arrays.asList(true, log1haspositions, log2haspositions, log3haspositions, log4haspositions);
        // The FileSlice contains a base file and three log files.
