...
public abstract class TestInLineFileSystemHFileInLiningBase {
    ...
    public TestInLineFileSystemHFileInLiningBase() {
        ...
        inMemoryConf.set(
                "fs." + InMemoryFileSystem.SCHEME + ".impl", InMemoryFileSystem.class.getName());
        inlineConf = new Configuration();
        inlineConf.set("fs." + InLineFileSystem.SCHEME + ".impl", InLineFileSystem.class.getName());
    }

    protected abstract void validateHFileReading(
            InLineFileSystem inlineFileSystem,
            Configuration conf,
    ...
    public void teardown() throws IOException {
        if (generatedPath != null) {
            File filePath =
                    new File(generatedPath.toString().substring(generatedPath.toString().indexOf(':') + 1));
            if (filePath.exists()) {
    ...
    public void testSimpleInlineFileSystem() throws IOException {
        ...
        HFileContext meta =
                new HFileContextBuilder()
                        .withBlockSize(MIN_BLOCK_BYTES)
                        .withCellComparator(COMPARATOR)
                        .build();
        HFile.Writer writer =
                HFile.getWriterFactory(inMemoryConf, cacheConf)
        ...
        Path inlinePath =
                new Path(
                        getPhantomFile(new StoragePath(outerPath.toUri()), startOffset, inlineLength).toUri());
    ...
    private byte[] getBytesToInline(Path outerInMemFSPath) throws IOException {
        InMemoryFileSystem inMemoryFileSystem =
                (InMemoryFileSystem) outerInMemFSPath.getFileSystem(inMemoryConf);
