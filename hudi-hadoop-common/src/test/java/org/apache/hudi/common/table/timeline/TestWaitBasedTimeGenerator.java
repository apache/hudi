...
public class TestWaitBasedTimeGenerator {
    ...
    public static class MockInProcessLockProvider extends InProcessLockProvider {
        ...
        private static final ThreadLocal<Boolean> NEED_TO_LOCK_LATER =
        ...
        public MockInProcessLockProvider(
                LockConfiguration lockConfiguration, StorageConfiguration<?> conf) {
    ...
    public void initialize() {
        timeGeneratorConfig =
                HoodieTimeGeneratorConfig.newBuilder()
                        ...
                        .build();
        timeGeneratorConfig.setValue(
                HoodieTimeGeneratorConfig.LOCK_PROVIDER_KEY, MockInProcessLockProvider.class.getName());
        MockInProcessLockProvider.initialize();
    }

    /**
      * Two threads concurrently try to get current time, manually let's t2 is always slower than t1,
      * TimeGenerator needs to ensure monotonically increasing time generated no matter which thread
      * firstly acquired lock.
      *
      * <p>To be more specific, pretend t2 is slower than t1 20ms, the MaxExpectedClockSkewMs is 25ms
      * 1. t1 acquired lock first, 1> t1(before lock: 20ms) acquired lock at 20ms, then t1 holds lock
      * for 25ms and 20 is returned. 2> t2(before lock: 0ms) wait for about 25ms until lock is free,
      * then holds lock for 25ms and 25 is returned. 3> whereas t1's timestamp < t2's timestamp 2. t2
      * acquired lock first, 1> t2(before lock: 20ms) acquired lock at 20ms, then t2 holds lock for
      * 25ms and 20 is returned. 2> t1(before lock: 40ms) wait for about 25ms until lock is free, then
      * holds lock for 25ms and 65 (40 + 25) is returned. 3> whereas t1's timestamp > t2's timestamp So
      * no matter which thread firstly acquires lock, the first acquired thread's timestamp should be
      * earlier.
      */
    @ParameterizedTest
    @ValueSource(booleans = {true, false})
    public void testSlowerThreadLaterAcquiredLock(boolean slowerThreadAcquiredLockLater)
