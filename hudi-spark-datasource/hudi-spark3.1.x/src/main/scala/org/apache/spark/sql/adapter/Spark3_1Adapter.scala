...
import org.apache.spark.sql.execution.datasources.parquet.{ParquetFileFormat, Spark31HoodieParquetReader, Spark31LegacyHoodieParquetFileFormat, SparkHoodieParquetReaderProperties}
...
class Spark3_1Adapter extends BaseSpark3Adapter {
  ...
  override def getSchemaUtils: HoodieSchemaUtils = HoodieSpark31SchemaUtils

  override def getSparkPartitionedFileUtils: HoodieSparkPartitionedFileUtils = HoodieSpark31PartitionedFileUtils
  ...
  override def getPropsForReadingParquet(vectorized: Boolean,
    ...
    Spark31HoodieParquetReader.getPropsForReadingParquet(vectorized, sqlConf, options, hadoopConf)
  }
  ...
  override def readParquetFile(file: PartitionedFile,
    ...
    Spark31HoodieParquetReader.readParquetFile(file, requiredSchema, partitionSchema, filters,
