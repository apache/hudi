...
import org.apache.spark.sql.execution.datasources.parquet.{ParquetFileFormat, Spark30HoodieParquetReader, Spark30LegacyHoodieParquetFileFormat}
...
class Spark3_0Adapter extends BaseSpark3Adapter {
  ...
  override def getParquetReader(file: PartitionedFile, requiredSchema: StructType, partitionSchema: StructType,
    ...
    Spark30HoodieParquetReader.getReader(file, requiredSchema, partitionSchema, filters, new Configuration(sharedConf), extraProps)
  }
  ...
  override def getExtraProps(vectorized: Boolean, sqlConf: SQLConf, options: Map[String, String], hadoopConf: Configuration): Map[String, String] = {
    Spark30HoodieParquetReader.getExtraProps(vectorized, sqlConf, options, hadoopConf)
  }
