...
/**
  * DataSource V2 implementation for managing internal write logic. Only called internally. This
  * class is only compatible with datasource V2 API in Spark 3.
  */
    ...
    public Table getTable(
        ...
        boolean arePartitionRecordsSorted =
                Boolean.parseBoolean(
                        properties.getOrDefault(
                                ...
                                Boolean.toString(
                                        HoodieInternalConfig.DEFAULT_BULKINSERT_ARE_PARTITIONER_RECORDS_SORTED)));
        ...
        HoodieWriteConfig config =
                DataSourceUtils.createHoodieConfig(
                        newProps.get(HoodieWriteConfig.AVRO_SCHEMA_STRING.key()), path, tblName, newProps);
        return new HoodieDataSourceInternalTable(
                instantTime,
                config,
                schema,
                getSparkSession(),
