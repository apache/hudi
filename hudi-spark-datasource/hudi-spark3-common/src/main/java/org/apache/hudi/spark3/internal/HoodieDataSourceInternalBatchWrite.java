...
/**
  * Implementation of {@link BatchWrite} for datasource "hudi.spark3.internal" to be used in
...
public class HoodieDataSourceInternalBatchWrite implements BatchWrite {
    ...
    public HoodieDataSourceInternalBatchWrite(
            String instantTime,
        ...
        this.dataSourceInternalWriterHelper =
                new DataSourceInternalWriterHelper(
                        instantTime, writeConfig, structType, jss, storageConf, extraMetadata);
    }
    ...
    public DataWriterFactory createBatchWriterFactory(PhysicalWriteInfo info) {
        ...
        if (WriteOperationType.BULK_INSERT == dataSourceInternalWriterHelper.getWriteOperationType()) {
            return new HoodieBulkInsertDataInternalWriterFactory(
                    dataSourceInternalWriterHelper.getHoodieTable(),
                    writeConfig,
                    instantTime,
                    structType,
                    populateMetaFields,
    ...
    public void commit(WriterCommitMessage[] messages) {
        List<WriteStatus> writeStatuses =
                Arrays.stream(messages)
                        .map(m -> (HoodieWriterCommitMessage) m)
