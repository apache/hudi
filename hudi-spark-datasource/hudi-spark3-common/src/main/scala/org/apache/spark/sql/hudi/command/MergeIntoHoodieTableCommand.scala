...
import org.apache.hudi.core.table.{HoodieTableConfig, HoodieTableVersion, PartialUpdateMode}
import org.apache.hudi.core.util.ConfigUtils
import org.apache.hudi.core.util.ConfigUtils.getStringWithAltKeys
...
case class MergeIntoHoodieTableCommand(mergeInto: MergeIntoTable) extends HoodieLeafRunnableCommand
  ...
  private def executeUpsert(sourceDF: DataFrame, parameters: Map[String, String]): Unit = {
    ...
    val schema = HoodieSchemaConversionUtils.convertStructTypeToHoodieSchema(hoodieCatalogTable.tableSchema, structName, namespace)
