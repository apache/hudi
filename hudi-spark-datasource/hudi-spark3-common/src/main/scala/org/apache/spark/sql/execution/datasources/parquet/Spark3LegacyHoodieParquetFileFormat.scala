...
/**
 * Base [[ParquetFileFormat]] shared by the Spark 3.3, 3.4 and 3.5 legacy readers.
 *
 * It holds the logic common to all three versions. Every expression that relies on a
 * version-specific Spark API is delegated to a protected hook that each concrete subclass
 * overrides, so this class compiles unchanged against each supported Spark 3.x version.
 *
 * This is an extension of [[ParquetFileFormat]] overriding Spark-specific behavior
 * that's not possible to customize in any other way, with the following changes applied:
 * <ol>
 *   <li>Avoiding appending partition values to the rows read from the data file</li>
 *   <li>Schema on-read</li>
 * </ol>
 */
abstract class Spark3LegacyHoodieParquetFileFormat(shouldAppendPartitionValues: Boolean) extends ParquetFileFormat {

  /**
   * Converts a [[StructType]] to its attributes. Spark 3.3/3.4 expose [[StructType.toAttributes]]
   * while Spark 3.5 moved it to [[org.apache.spark.sql.catalyst.types.DataTypeUtils]].
   */
  protected def toAttributes(structType: StructType): Seq[Attribute]

  /**
   * Extracts the [[Path]] of the file being read. Spark 3.3 keeps the path as a string while
   * Spark 3.4+ wraps it in a `SparkPath`.
   */
  protected def getFilePath(file: PartitionedFile): Path

  /**
   * Whether the vectorized reader is enabled for the given schema.
   */
  protected def isVectorizedReaderEnabled(sparkSession: SparkSession, resultSchema: StructType): Boolean

  /**
   * Whether string-predicate push-down is enabled (renamed between Spark 3.3 and 3.4).
   */
  protected def getPushDownStringPredicate(sqlConf: SQLConf): Boolean

  /**
   * Whether the reader should return columnar batches.
   */
  protected def getReturningBatch(sparkSession: SparkSession, resultSchema: StructType): Boolean

  /**
   * Sets the version-specific timestamp and nanos-as-long related flags on the hadoop conf.
   */
  protected def setParquetTimeConfs(hadoopConf: Configuration, sparkSession: SparkSession): Unit
  ...
  override def buildReaderWithPartitionValues(sparkSession: SparkSession,
    ...
    // Version-specific timestamp and nanos-as-long flags.
    setParquetTimeConfs(hadoopConf, sparkSession)
    ...
    val enableVectorizedReader: Boolean = isVectorizedReaderEnabled(sparkSession, resultSchema)
    ...
    val pushDownStringStartWith = getPushDownStringPredicate(sqlConf)
...
object Spark3LegacyHoodieParquetFileFormat {
