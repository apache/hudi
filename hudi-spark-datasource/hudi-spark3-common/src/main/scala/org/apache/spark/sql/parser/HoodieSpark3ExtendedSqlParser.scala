...
class HoodieSpark3ExtendedSqlParser(session: SparkSession, delegate: ParserInterface)
  extends ParserInterface with Logging {

  private lazy val conf = session.sqlContext.conf
  private lazy val builder = new HoodieSpark3ExtendedSqlAstBuilder(conf, delegate)
...
case object PostProcessor extends HoodieSqlBaseBaseListener {
  ...
  override def exitQuotedIdentifier(ctx: HoodieSqlBaseParser.QuotedIdentifierContext): Unit = {
  ...
  override def exitNonReserved(ctx: HoodieSqlBaseParser.NonReservedContext): Unit = {
