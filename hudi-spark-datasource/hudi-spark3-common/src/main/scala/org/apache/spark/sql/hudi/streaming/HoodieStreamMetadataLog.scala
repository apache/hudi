 ...
 */
class HoodieStreamMetadataLog(
    sparkSession: SparkSession,
    metadataPath: String) extends HDFSMetadataLog[HoodieStreamSourceOffset](sparkSession, metadataPath) {

  private val VERSION = 1

  override def serialize(metadata: HoodieStreamSourceOffset, out: OutputStream): Unit = {
   ...
   * offsetJson -- The json string of HoodieStreamSourceOffset in the rest of the file
   * -----------------------------------------------
   * @param in
   * @return
   */
  override def deserialize(in: InputStream): HoodieStreamSourceOffset = {
    ...
    if (firstLineEnd > 0) {
      ...
      HoodieStreamSourceOffset.fromJson(content.substring(firstLineEnd + 1))
    } else {
