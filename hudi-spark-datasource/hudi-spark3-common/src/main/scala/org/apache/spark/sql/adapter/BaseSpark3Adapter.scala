...
abstract class BaseSpark3Adapter extends SparkAdapter with Logging {
  ...
  override def generateVariantShreddingSchema(dataType: DataType, isTopLevel: Boolean, isObjectField: Boolean): StructType = {
  ...
  override def addVariantWriteShreddingMetadata(schema: StructType): StructType = {
    throw new UnsupportedOperationException("Spark 3.x does not support Variant shredding")
  }
