...
import org.apache.spark.sql.execution.datasources.parquet.{ParquetFileFormat, Spark34HoodieParquetReader, Spark34LegacyHoodieParquetFileFormat}
...
class Spark3_4Adapter extends BaseSpark3Adapter {
  ...
  override def getParquetReader(file: PartitionedFile, requiredSchema: StructType, partitionSchema: StructType,
    ...
    Spark34HoodieParquetReader.getReader(file, requiredSchema, partitionSchema, filters, new Configuration(sharedConf), extraProps)
  }
  ...
  override def getExtraProps(vectorized: Boolean, sqlConf: SQLConf, options: Map[String, String], hadoopConf: Configuration): Map[String, String] = {
    Spark34HoodieParquetReader.getExtraProps(vectorized, sqlConf, options, hadoopConf)
  }
}
