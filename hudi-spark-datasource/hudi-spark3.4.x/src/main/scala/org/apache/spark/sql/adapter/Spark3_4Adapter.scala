...
class Spark3_4Adapter extends BaseSpark3Adapter {
  ...
  override def createLegacyHoodieParquetFileFormat(appendPartitionValues: Boolean, tableAvroSchema: Schema,
    ...
    Some(new Spark34LegacyHoodieParquetFileFormat(appendPartitionValues, tableAvroSchema, hasTimestampMillisFieldInTableSchema, new SerializableConfiguration(conf)))
  }
