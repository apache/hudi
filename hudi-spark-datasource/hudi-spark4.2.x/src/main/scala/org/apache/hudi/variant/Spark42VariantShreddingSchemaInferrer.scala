 ...
 *
 * Loaded reflectively from hudi-common via classpath detection; the Spark 4.2 twin of the
 * spark4.1 module's inferrer (each spark4.x profile builds only its own version module, and the
 * Spark class does not exist before 4.1).
 */
class Spark42VariantShreddingSchemaInferrer extends VariantShreddingSchemaInferrer {
