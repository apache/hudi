...
class HoodieSpark4_2ExtendedSqlParser(session: SparkSession, delegate: ParserInterface)
  extends HoodieExtendedParserInterface with Logging {

  private lazy val conf = session.sessionState.conf
  private lazy val builder = new HoodieSpark4_2ExtendedSqlAstBuilder(conf, delegate)
