 ...
 */
case class HoodieSpark42DataSourceV2ToV1Fallback(sparkSession: SparkSession) extends Rule[LogicalPlan]
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = plan match {
    ...
    case iis@InsertIntoStatement(rv2@DataSourceV2Relation(v2Table: HoodieInternalV2Table, _, _, _, _, _), _, _, _, _, _, _, _, _) =>
 ...
 * [[InsertIntoStatement]] for Spark 4.2; see [[HoodieSpark4ResolveColumnsForInsertInto]]
 * for the shared preprocessing logic and the rationale.
 */
case class HoodieSpark42ResolveColumnsForInsertInto() extends HoodieSpark4ResolveColumnsForInsertInto {
