...
class Spark3HoodieParquetFileFormat extends ParquetFileFormat {
      ...
      (file: PartitionedFile) => {
        ...
        val pushed = if (enableParquetFilterPushDown) {
          ...
          val parquetFilters = new ParquetFilters(
            ...
            isCaseSensitive)
          filters.map(Spark3HoodieParquetFileFormat.rebuildFilterFromParquet(_, fileSchema, querySchemaOpt.get()))
        ...
        var typeChangeInfos: java.util.Map[Integer, Pair[DataType, DataType]] = new util.HashMap()
        if (internalSchemaChangeEnabled) {
          val mergedInternalSchema = InternalSchemaUtils.mergeSchema(fileSchema, querySchemaOpt.get())
          val mergedSchema = SparkSchemaUtils.constructSparkSchemaFromInternalSchema(mergedInternalSchema)
          typeChangeInfos = SparkSchemaUtils.collectTypeChangedCols(querySchemaOpt.get(), mergedInternalSchema)
        ...
        if (enableVectorizedReader) {
          val vectorizedReader = new Spark3HoodieVectorizedParquetRecordReader(
            convertTz.orNull,
...
object Spark3HoodieParquetFileFormat {
