...
/**
  ...
  */
case class ResolveHudiAlterTableCommand (sparkSession: SparkSession) extends Rule[LogicalPlan] {
  import org.apache.spark.sql.connector.catalog.CatalogV2Implicits._
  val conf = sparkSession.sessionState.conf
  def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
    case add @ HoodieAlterTableAddColumnsStatement(asTable(table), cols) =>
      if (isHoodieTable(table) && schemaEvolutionEnable){
        ...
        AlterTableCommand(table, newChanges, ColumnChangeID.ADD)
      } else {
        // throw back to spark
    ...
    case a @ HoodieAlterTableAlterColumnStatement(asTable(table), _, _, _, _, _) =>
      if (isHoodieTable(table) && schemaEvolutionEnable){
        ...
        AlterTableCommand(table, normalizeChanges(typeChange.toSeq ++ nullabilityChange ++ commentChange ++ positionChange, table.schema), ColumnChangeID.UPDATE)
      } else {
        // throw back to spark
    ...
    case rename @ HoodieAlterTableRenameColumnStatement(asTable(table), col, newName) =>
      if (isHoodieTable(table) && schemaEvolutionEnable){
        ...
        AlterTableCommand(table, normalizeChanges(changes, table.schema), ColumnChangeID.UPDATE)
      } else {
        // throw back to spark
    ...
    case drop @ HoodieAlterTableDropColumnsStatement(asTable(table), cols) =>
      if (isHoodieTable(table) && schemaEvolutionEnable) {
        ...
        AlterTableCommand(table, normalizeChanges(changes, table.schema), ColumnChangeID.DELETE)
      } else {
        // throw back to spark
    ...
    case set @ HoodieAlterTableSetPropertiesStatement(asTable(table), props) =>
      if (isHoodieTable(table) && schemaEvolutionEnable) {
        ...
        AlterTableCommand(table, normalizeChanges(changes, table.schema), ColumnChangeID.PROPERTY_CHANGE)
      } else {
        // throw back to spark
        AlterTableSetPropertiesStatement(set.tableName, set.properties)
      }
    case unset @ AlterTableUnsetPropertiesStatement(asTable(table), keys, _) =>
      if (isHoodieTable(table) && schemaEvolutionEnable) {
  ...
  private def schemaEvolutionEnable(): Boolean = sparkSession
  ...
  def failNullType(dt: DataType): Unit = {
    def containsNullType(df: DataType): Boolean = dt match {
    ...
    if (containsNullType(dt)) {
      throw new AnalysisException(s"cannot create tables with ${NullType.simpleString} type.")
    }
  }
