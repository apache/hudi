...
case class AlterTableCommand(table: CatalogTable, changes: Seq[TableChange], changeType: ColumnChangeID) extends RunnableCommand with Logging {
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    try {
      ...
      case NonFatal(e) =>
        log.warn(s"Exception when attempting to uncache table ${table.identifier.quotedString}", e)
    }
  ...
  def applyDeleteAction(sparkSession: SparkSession): Unit = {
    ...
    changes.map(_.asInstanceOf[DeleteColumn]).foreach(c => deleteChange.deleteColumn(c.fieldNames().mkString(".")))
    ...
    changes.foreach { change =>
      change match {
        ...
        case updateName: TableChange.RenameColumn =>
          updateChange.renameColumn(updateName.fieldNames().mkString("."), updateName.newName())
...
object AlterTableCommand {
  ...
  def commitWithSchema(internalSchema: InternalSchema, historySchemaStr: String, table: CatalogTable, sparkSession: SparkSession): Unit = {
    ...
    val fullSparkSchema = AvroConversionUtils.convertAvroSchemaToStructType(schema)
    ...
    sparkSession.sessionState.catalog
      .externalCatalog
      .alterTableDataSchema(table.identifier.database.getOrElse("default"), table.identifier.table, dataSparkSchema)
  }
