...
class Spark3_2Adapter extends BaseSpark3Adapter {
  ...
  override def getParquetReader(file: PartitionedFile, requiredSchema: StructType, partitionSchema: StructType,
    ...
    Spark32HoodieParquetReader.getReader(file, requiredSchema, partitionSchema, filters, new Configuration(sharedConf), extraProps)
  }
  ...
  override def getExtraProps(vectorized: Boolean, sqlConf: SQLConf, options: Map[String, String], hadoopConf: Configuration): Map[String, String] = {
    Spark32HoodieParquetReader.getExtraProps(vectorized, sqlConf, options, hadoopConf)
  }
}
