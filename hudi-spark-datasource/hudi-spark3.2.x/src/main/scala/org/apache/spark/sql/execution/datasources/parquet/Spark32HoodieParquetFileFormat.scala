...
class Spark32HoodieParquetFileFormat(private val shouldAppendPartitionValues: Boolean) extends ParquetFileFormat {
  ...
  protected def buildReaderWithPartitionValues(sparkSession: SparkSession,
                                               ...
                                               shouldAppendOverride: Boolean,
                                               readerType: String = ""): PartitionedFile => Iterator[InternalRow] = {
    hadoopConf.set(ParquetInputFormat.READ_SUPPORT_CLASS, classOf[Spark32ParquetReadSupport].getName)
    if (readerType.equals("skeleton")) {
      hadoopConf.set(
        Spark32ParquetReadSupport.SPARK_SKELETON_ROW_REQUESTED_SCHEMA,
        requiredSchema.json)
    } else if (readerType.equals("base")) {
      hadoopConf.set(
        Spark32ParquetReadSupport.SPARK_BASE_ROW_REQUESTED_SCHEMA,
        requiredSchema.json)
    } else {
      hadoopConf.set(
        Spark32ParquetReadSupport.SPARK_ROW_REQUESTED_SCHEMA,
        requiredSchema.json)
    }
    hadoopConf.set(
    ...
    (file: PartitionedFile) => {
      ...
      val typeChangeInfos: java.util.Map[Integer, Pair[DataType, DataType]] = if (shouldUseInternalSchema) {
        ...
        if (readerType.equals("skeleton")) {
          hadoopAttemptConf.set(Spark32ParquetReadSupport.SPARK_SKELETON_ROW_REQUESTED_SCHEMA, mergedSchema.json)
        } else if (readerType.equals("base")) {
          hadoopAttemptConf.set(Spark32ParquetReadSupport.SPARK_BASE_ROW_REQUESTED_SCHEMA, mergedSchema.json)
        } else {
          hadoopAttemptConf.set(Spark32ParquetReadSupport.SPARK_ROW_REQUESTED_SCHEMA, mergedSchema.json)
        }
        ...
        if (!implicitTypeChangeInfo.isEmpty) {
          ...
          if (readerType.equals("skeleton")) {
            hadoopAttemptConf.set(Spark32ParquetReadSupport.SPARK_SKELETON_ROW_REQUESTED_SCHEMA, sparkRequestSchema.json)
          } else if (readerType.equals("base")) {
            hadoopAttemptConf.set(Spark32ParquetReadSupport.SPARK_BASE_ROW_REQUESTED_SCHEMA, sparkRequestSchema.json)
          } else {
            hadoopAttemptConf.set(Spark32ParquetReadSupport.SPARK_ROW_REQUESTED_SCHEMA, sparkRequestSchema.json)
          }
        }
      ...
      if (enableVectorizedReader) {
        ...
        val readSupport = if (HoodieSparkUtils.gteqSpark3_2_1) {
          ...
          val int96RebaseMode =
            Spark32PlusDataSourceUtils.int96RebaseMode(footerFileMetaData.getKeyValueMetaData.get, int96RebaseModeInRead)
          createSpark32ParquetReadSupport(
            convertTz,
            /* enableVectorizedReader = */ false,
            datetimeRebaseMode,
            int96RebaseMode,
...
object Spark32HoodieParquetFileFormat {
   ...
   * NOTE: This method is specific to Spark 3.2.0
   */
  private def createSpark32ParquetReadSupport(args: Any*): Spark32ParquetReadSupport = {
    // NOTE: ParquetReadSupport ctor args contain Scala enum, therefore we can't look it
    //       up by arg types, and have to instead rely on the number of args based on individual class;
    //       the ctor order is not guaranteed
    val ctor = classOf[Spark32ParquetReadSupport].getConstructors.maxBy(_.getParameterCount)
    ctor.newInstance(args.map(_.asInstanceOf[AnyRef]): _*)
      .asInstanceOf[Spark32ParquetReadSupport]
  }
