...
class Spark32HoodieParquetFileFormat(private val shouldAppendPartitionValues: Boolean) extends ParquetFileFormat {
  ...
  override def buildReaderWithPartitionValues(sparkSession: SparkSession,
    ...
    buildReaderWithPartitionValues(sparkSession, dataSchema, partitionSchema, requiredSchema, filters, options, hadoopConf, shouldAppendPartitionValues)
  }

  protected def buildBaseReaderWithPartitionValues(sparkSession: SparkSession,
                                              dataSchema: StructType,
                                              partitionSchema: StructType,
                                              requiredSchema: StructType,
                                              filters: Seq[Filter],
                                              ...
                                              shouldAppendOverride: Boolean): PartitionedFile => Iterator[InternalRow] = {
    hadoopConf.set(ParquetInputFormat.READ_SUPPORT_CLASS, classOf[Spark32BaseParquetReadSupport].getName)
    hadoopConf.set(
      Spark32BaseParquetReadSupport.SPARK_ROW_REQUESTED_SCHEMA,
      requiredSchema.json)
    hadoopConf.set(
    ...
    val enableVectorizedReader: Boolean = false
    ...
    (file: PartitionedFile) => {
      assert(!shouldAppendOverride || file.partitionValues.numFields == partitionSchema.size)
      ...
      val typeChangeInfos: java.util.Map[Integer, Pair[DataType, DataType]] = if (shouldUseInternalSchema) {
        ...
        hadoopAttemptConf.set(Spark32BaseParquetReadSupport.SPARK_ROW_REQUESTED_SCHEMA, mergedSchema.json)
        ...
        if (!implicitTypeChangeInfo.isEmpty) {
          ...
          hadoopAttemptConf.set(Spark32BaseParquetReadSupport.SPARK_ROW_REQUESTED_SCHEMA, sparkRequestSchema.json)
        }
      ...
      if (enableVectorizedReader) {
        ...
        val readSupport = if (HoodieSparkUtils.gteqSpark3_2_1) {
          ...
          val datetimeRebaseSpec =
            DataSourceUtils.datetimeRebaseSpec(footerFileMetaData.getKeyValueMetaData.get, datetimeRebaseModeInRead)
          new Spark32BaseParquetReadSupport(
            convertTz,
          ...
          val int96RebaseMode =
            Spark32PlusDataSourceUtils.int96RebaseMode(footerFileMetaData.getKeyValueMetaData.get, int96RebaseModeInRead)
          createSpark32BaseParquetReadSupport(
            convertTz,
            /* enableVectorizedReader = */ false,
            datetimeRebaseMode,
            int96RebaseMode)
        ...
        try {
          ...
          // NOTE: We're making appending of the partitioned values to the rows read from the
          //       data file configurable
          if (!shouldAppendOverride || partitionSchema.length == 0) {
  ...
  protected def buildSkeletonReaderWithPartitionValues(sparkSession: SparkSession,
                                               dataSchema: StructType,
                                               partitionSchema: StructType,
                                               requiredSchema: StructType,
                                               filters: Seq[Filter],
                                               ...
                                               hadoopConf: Configuration,
                                               shouldAppendOverride: Boolean): PartitionedFile => Iterator[InternalRow] = {
    hadoopConf.set(ParquetInputFormat.READ_SUPPORT_CLASS, classOf[Spark32SkeletonParquetReadSupport].getName)
    hadoopConf.set(
      Spark32SkeletonParquetReadSupport.SPARK_ROW_REQUESTED_SCHEMA,
      requiredSchema.json)
    hadoopConf.set(
      ParquetWriteSupport.SPARK_ROW_SCHEMA,
      requiredSchema.json)
    ...
    (file: PartitionedFile) => {
      ...
      val typeChangeInfos: java.util.Map[Integer, Pair[DataType, DataType]] = if (shouldUseInternalSchema) {
        ...
        hadoopAttemptConf.set(Spark32SkeletonParquetReadSupport.SPARK_ROW_REQUESTED_SCHEMA, mergedSchema.json)
        ...
        if (!implicitTypeChangeInfo.isEmpty) {
          shouldUseInternalSchema = true
          hadoopAttemptConf.set(Spark32SkeletonParquetReadSupport.SPARK_ROW_REQUESTED_SCHEMA, sparkRequestSchema.json)
        }
        implicitTypeChangeInfo
      }
      ...
      if (enableVectorizedReader) {
        ...
        val readSupport = if (HoodieSparkUtils.gteqSpark3_2_1) {
          ...
          val datetimeRebaseSpec =
            DataSourceUtils.datetimeRebaseSpec(footerFileMetaData.getKeyValueMetaData.get, datetimeRebaseModeInRead)
          new Spark32SkeletonParquetReadSupport(
            convertTz,
            enableVectorizedReader = false,
            datetimeRebaseSpec,
            int96RebaseSpec)
          ...
          val int96RebaseMode =
            Spark32PlusDataSourceUtils.int96RebaseMode(footerFileMetaData.getKeyValueMetaData.get, int96RebaseModeInRead)
          createSpark32SkeletonParquetReadSupport(
            convertTz,
            /* enableVectorizedReader = */ false,
            datetimeRebaseMode,
            int96RebaseMode)
...
object Spark32HoodieParquetFileFormat {
   ...
   * NOTE: This method is specific to Spark 3.2.0
   */
  private def createSpark32BaseParquetReadSupport(args: Any*): Spark32BaseParquetReadSupport = {
    // NOTE: ParquetReadSupport ctor args contain Scala enum, therefore we can't look it
    //       up by arg types, and have to instead rely on the number of args based on individual class;
    //       the ctor order is not guaranteed
    val ctor = classOf[Spark32BaseParquetReadSupport].getConstructors.maxBy(_.getParameterCount)
    ctor.newInstance(args.map(_.asInstanceOf[AnyRef]): _*)
      .asInstanceOf[Spark32BaseParquetReadSupport]
  }

  /**
   * NOTE: This method is specific to Spark 3.2.0
   */
  private def createSpark32SkeletonParquetReadSupport(args: Any*): Spark32SkeletonParquetReadSupport = {
    // NOTE: ParquetReadSupport ctor args contain Scala enum, therefore we can't look it
    //       up by arg types, and have to instead rely on the number of args based on individual class;
    //       the ctor order is not guaranteed
    val ctor = classOf[Spark32SkeletonParquetReadSupport].getConstructors.maxBy(_.getParameterCount)
    ctor.newInstance(args.map(_.asInstanceOf[AnyRef]): _*)
      .asInstanceOf[Spark32SkeletonParquetReadSupport]
  }
