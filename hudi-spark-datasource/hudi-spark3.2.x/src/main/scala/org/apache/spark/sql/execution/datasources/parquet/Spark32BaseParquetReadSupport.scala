 ...
 */
class Spark32BaseParquetReadSupport(
  ...
  override def init(context: InitContext): ReadContext = {
    ...
    catalystRequestedSchema = {
      val schemaString = conf.get(Spark32BaseParquetReadSupport.SPARK_ROW_REQUESTED_SCHEMA)
    ...
    val parquetClippedSchema = Spark32BaseParquetReadSupport.clipParquetSchema(parquetFileSchema,
    ...
    val parquetRequestedSchema = if (schemaPruningEnabled && !enableVectorizedReader) {
      ...
      Spark32BaseParquetReadSupport.intersectParquetGroups(parquetClippedSchema, parquetFileSchema)
  ...
  override def prepareForRead(
    ...
    new ParquetRecordMaterializer(
      parquetRequestedSchema,
      Spark32BaseParquetReadSupport.expandUDT(catalystRequestedSchema),
...
object Spark32BaseParquetReadSupport {
  val SPARK_ROW_REQUESTED_SCHEMA = "org.apache.spark.sql.parquet.row.requested_schema.base"
