...
public class TestGlobalIndexEnableUpdatePartitions extends SparkClientFunctionalTestHarness {
    ...
    public void testPartitionChanges(HoodieTableType tableType, IndexType indexType)
            throws IOException {
        ...
        try (SparkRDDWriteClient client = getHoodieWriteClient(writeConfig)) {
            ...
            assertNoWriteErrors(
                    client.upsert(jsc().parallelize(insertsAtEpoch0, 2), commitTimeAtEpoch0).collect());
            ...
            assertNoWriteErrors(
                    client.upsert(jsc().parallelize(updatesAtEpoch5, 2), commitTimeAtEpoch5).collect());
            ...
            assertNoWriteErrors(
                    client.upsert(jsc().parallelize(updatesAtEpoch6, 2), commitTimeAtEpoch6).collect());
            ...
            assertNoWriteErrors(
                    client.upsert(jsc().parallelize(updatesAtEpoch7, 2), commitTimeAtEpoch7).collect());
            ...
            assertNoWriteErrors(
                    client.upsert(jsc().parallelize(updatesAtEpoch2, 2), commitTimeAtEpoch8).collect());
            readTableAndValidate(metaClient, new int[] {0, 1, 2, 3}, p3, 7);
            ...
            assertNoWriteErrors(
                    client.upsert(jsc().parallelize(updatesAtEpoch9, 2), commitTimeAtEpoch9).collect());
    ...
    @ParameterizedTest
    @MethodSource("getTableTypeAndIndexType")
    public void testUpdatePartitionsThenDelete(HoodieTableType tableType, IndexType indexType)
            throws IOException {
        final Class<?> payloadClass = DefaultHoodieRecordPayload.class;
        HoodieWriteConfig writeConfig = getWriteConfig(payloadClass, indexType);
        HoodieTableMetaClient metaClient = getHoodieMetaClient(tableType, writeConfig.getProps());
        try (SparkRDDWriteClient client = getHoodieWriteClient(writeConfig)) {
            ...
            List<HoodieRecord> deletesAtEpoch7 =
                    getDeletesWithNewPartition(insertsAtEpoch0.subList(0, 2), p2, 7, payloadClass);
            client.startCommitWithTime(commitTimeAtEpoch7);
            assertNoWriteErrors(
                    client.upsert(jsc().parallelize(deletesAtEpoch7, 2), commitTimeAtEpoch7).collect());
            ...
            String commitTimeAtEpoch8 = getCommitTimeAtUTC(8);
            List<HoodieRecord> deletesAtEpoch8 =
                    getDeletesWithEmptyPayloadAndNewPartition(insertsAtEpoch0.subList(2, 3), "unknown_pt");
            client.startCommitWithTime(commitTimeAtEpoch8);
            assertNoWriteErrors(
                    client.upsert(jsc().parallelize(deletesAtEpoch8, 1), commitTimeAtEpoch8).collect());
    ...
    @ParameterizedTest
    @MethodSource("getTableTypeAndIndexType")
    public void testUdpateSubsetOfRecUpdates(HoodieTableType tableType, IndexType indexType)
            throws IOException {
        final Class<?> payloadClass = DefaultHoodieRecordPayload.class;
        HoodieWriteConfig writeConfig = getWriteConfig(payloadClass, indexType);
        HoodieTableMetaClient metaClient = getHoodieMetaClient(tableType, writeConfig.getProps());
        try (SparkRDDWriteClient client = getHoodieWriteClient(writeConfig)) {
            ...
            assertNoWriteErrors(
                    client
                            .upsert(jsc().parallelize(allInserts.subList(0, 2), 2), commitTimeAtEpoch0)
                            .collect());
            ...
            assertNoWriteErrors(
                    client.upsert(jsc().parallelize(updatesAtEpoch10, 2), commitTimeAtEpoch10).collect());
            readTableAndValidate(
                    metaClient,
                    new int[] {0, 1, 2, 3},
                    p1,
            ...
            assertNoWriteErrors(
                    client.upsert(jsc().parallelize(updatesAtEpoch20, 2), commitTimeAtEpoch20).collect());
    ...
    private void readTableAndValidate(
        ...
        Arrays.stream(expectedIds)
                .forEach(entry -> expectedTsMap.put(String.valueOf(entry), expectedTs));
        ...
        return getConfigBuilder(true)
                ...
                .withIndexConfig(
                        HoodieIndexConfig.newBuilder()
                                ...
                                .withRecordIndexUpdatePartitionPath(true)
                                .build())
