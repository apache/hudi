...
public class ColumnStatsIndexHelper {
    ...
    public static Pair<Object, Object> fetchMinMaxValues(
            @Nonnull DataType colType, @Nonnull HoodieColumnRangeMetadata<Comparable> colMetadata) {
        if (colType instanceof IntegerType) {
            ...
            return Pair.of(colMetadata.getMinValue().toString(), colMetadata.getMaxValue().toString());
    ...
    /**
      * NOTE: THIS IS ONLY USED IN TESTING CURRENTLY, SINCE DATA SKIPPING IS NOW RELYING ON METADATA
      * TABLE INDEX
      *
      * <p>Parse min/max statistics from Parquet footers for provided columns and composes column-stats
      * index table in the following format with 3 statistics denominated for each
      * linear/Z-curve/Hilbert-curve-ordered column. For ex, if original table contained column {@code
      * A}:
      *
      * <pre>
      * +---------------------------+------------+------------+-------------+
      ...
      * </pre>
      *
      * <p>NOTE: Currently {@link TimestampType} is not supported, since Parquet writer does not
      * support statistics for it. @VisibleForTestingOnly
      *
      ...
      * @return Spark's {@link Dataset} holding an index table @VisibleForTesting
      */
    @Nonnull
    public static Dataset<Row> buildColumnStatsTableFor(
        ...
        List<String> columnNames =
                orderedColumnSchemas.stream().map(StructField::name).collect(Collectors.toList());
                                                ...
                                                .flatMap(
                                                        path -> {
                                                            HoodieStorage storage =
                                                                    new HoodieHadoopStorage(path, serializableConfiguration.value());
                                                            return utils
                                                                    .readColumnStatsFromMetadata(
                                                                            storage, new StoragePath(path), columnNames)
                                                                    .stream();
                                                        })
                                                .iterator();
                                    })
                        ...
                        .map(
                                fileColumnsMetadata -> {
                                            ...
                                            colSchema -> {
                                                ...
                                                if (colMetadata == null || colType == null) {
                                                    throw new HoodieException(
                                                            String.format(
                                                                    "Cannot collect min/max statistics for column (%s)", colSchema));
                                                }
                                    ...
                                    return Row$.MODULE$.apply(
                                            JavaScalaConverters.<Object>convertJavaListToScalaSeq(indexRow));
                                })
