...
public class TestCustomKeyGenerator extends KeyGeneratorTestUtilities {
    ...
    /**
      ...
      *     true use {@code HoodieWriteConfig.KEYGENERATOR_CLASS_PROP}, false use {@code
      *     HoodieWriteConfig.KEYGENERATOR_TYPE_PROP}
      * @return TypedProperties used to initialize KeyGenerator.
      */
    private TypedProperties getCommonProps(
            boolean getComplexRecordKey, boolean useKeyGeneratorClassName) {
        ...
        if (useKeyGeneratorClassName) {
            properties.put(
                    HoodieWriteConfig.KEYGENERATOR_CLASS_NAME.key(), CustomKeyGenerator.class.getName());
        } else {
    ...
    private TypedProperties getComplexRecordKeyWithSimplePartitionProps(
            boolean useKeyGeneratorClassName) {
    ...
    private TypedProperties getComplexRecordKeyAndPartitionPathProps(
        ...
        TypedProperties properties = getCommonProps(true, useKeyGeneratorClassName);
        properties.put(
                KeyGeneratorOptions.PARTITIONPATH_FIELD_NAME.key(), "timestamp:simple,ts_ms:timestamp");
    ...
    public void testSimpleKeyGenerator(TypedProperties props) throws IOException {
        ...
        Assertions.assertEquals(
                UTF8String.fromString("timestamp=4357686"),
    ...
    public void testTimestampBasedKeyGenerator(TypedProperties props) throws IOException {
        ...
        InternalRow internalRow = KeyGeneratorTestUtilities.getInternalRow(row);
        Assertions.assertEquals(
                UTF8String.fromString("ts_ms=20200321"),
                keyGenerator.getPartitionPath(internalRow, row.schema()));
    }
    ...
    public void testInvalidPartitionKeyType(TypedProperties props) {
        try {
            ...
            Assertions.assertTrue(
                    getNestedConstructorErrorCause(e)
                            .getMessage()
                            .contains(
                                    "No enum constant org.apache.hudi.keygen.CustomAvroKeyGenerator.PartitionKeyType.DUMMY"));
        }
    ...
    public void testNoRecordKeyFieldProp(boolean useKeyGeneratorClassName) {
        ...
        try {
            ...
            if (useKeyGeneratorClassName) {
                // "Property hoodie.datasource.write.recordkey.field not found" exception cause
                // CustomKeyGenerator init fail
                Assertions.assertTrue(
                        e.getMessage().contains("Unable to find field names for record key in cfg"));
            } else {
                Assertions.assertTrue(
                        stackTraceToString(e).contains("Unable to find field names for record key in cfg"));
            }
        }
        ...
        try {
            ...
            if (useKeyGeneratorClassName) {
                ...
                Assertions.assertTrue(
                        stackTraceToString(e).contains("All of the values for ([]) were either null or empty"));
            }
        }
    }
    ...
    public void testPartitionFieldsInImproperFormat(TypedProperties props) {
        try {
            ...
            Assertions.assertTrue(
                    getNestedConstructorErrorCause(e)
                            .getMessage()
                            .contains("Unable to find field names for partition path in proper format"));
        }
    ...
    @Test
    public void testComplexRecordKeyWithSimplePartitionPathWithKeyGeneratorClass()
            throws IOException {
    ...
    @Test
    public void testComplexRecordKeysWithComplexPartitionPathWithKeyGeneratorClass()
            throws IOException {
        testComplexRecordKeysWithComplexPartitionPath(getComplexRecordKeyAndPartitionPathProps(true));
    }

    @Test
    public void testComplexRecordKeysWithComplexPartitionPathWithKeyGeneratorType()
            throws IOException {
    ...
    public void testComplexRecordKeysWithComplexPartitionPath(TypedProperties props)
        ...
        InternalRow internalRow = KeyGeneratorTestUtilities.getInternalRow(row);
        Assertions.assertEquals(
                UTF8String.fromString("timestamp=4357686/ts_ms=20200321"),
                keyGenerator.getPartitionPath(internalRow, row.schema()));
    }

    private static Throwable getNestedConstructorErrorCause(Exception e) {
        // custom key generator will fail in the constructor, and we must unwrap the cause for
