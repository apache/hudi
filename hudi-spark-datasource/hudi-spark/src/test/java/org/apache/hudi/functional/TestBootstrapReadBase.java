...
public abstract class TestBootstrapReadBase extends HoodieSparkClientTestBase {
    ...
    protected static String[] dashPartitionPaths = {
        "2016-03-14", "2016-03-15", "2015-03-16", "2015-03-17"
    };
    ...
    protected static String[] dropColumns = {
        "_hoodie_commit_time",
    ...
    protected Map<String, String> basicOptions() {
        ...
        if (nPartitions == 0) {
            ...
            options.put(
                    DataSourceWriteOptions.PARTITIONPATH_FIELD().key(), String.join(",", partitionCols));
            if (nPartitions == 1) {
                options.put(
                        HoodieWriteConfig.KEYGENERATOR_CLASS_NAME.key(), SimpleKeyGenerator.class.getName());
            } else {
                options.put(
                        HoodieWriteConfig.KEYGENERATOR_CLASS_NAME.key(), ComplexKeyGenerator.class.getName());
            }
        }
    ...
    protected void doUpsert(Map<String, String> options, Dataset<Row> df) {
        ...
        if (bootstrapType.equals("mixed")) {
            ...
            // so to align with the regular hudi table, we need to compact after 4 commits instead
            // of 3
            nCompactCommits = "4";
        }
        df.write()
    ...
    protected void compareTables() {
        ...
        compareDf(
                hudiDf.drop(dropColumns).drop(partitionCols),
                bootstrapDf.drop(dropColumns).drop(partitionCols));
        compareDf(
                hudiDf.select("_row_key", partitionCols), bootstrapDf.select("_row_key", partitionCols));
    }
    ...
    protected void verifyMetaColOnlyRead(Integer iteration) {
        ...
        if (iteration > 0) {
            assertEquals(
                    sparkSession
                            .sql("select * from hudi_iteration_" + (iteration - 1))
                            .intersect(hudiDf)
                            .count(),
                    sparkSession
                            .sql("select * from bootstrap_iteration_" + (iteration - 1))
                            .intersect(bootstrapDf)
                            .count());
        }
    ...
    protected void setupDirs() {
        dataGen =
                new HoodieTestDataGenerator(dashPartitions ? dashPartitionPaths : slashPartitionPaths);
    ...
    protected Dataset<Row> makeDeleteDf(String instantTime, Integer n) {
        List<String> records =
                dataGen.generateUniqueDeleteRecords(instantTime, n).stream()
