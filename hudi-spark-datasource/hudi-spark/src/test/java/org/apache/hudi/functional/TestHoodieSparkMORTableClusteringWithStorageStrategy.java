...
@Tag("functional")
class TestHoodieSparkMORTableClusteringWithStorageStrategy extends SparkClientFunctionalTestHarness {
  ...
  void testClustering(boolean clusteringAsRow, boolean doUpdates, boolean populateMetaFields) throws Exception {
    ...
    HoodieWriteConfig.Builder cfgBuilder = HoodieWriteConfig.newBuilder()
        ...
        .withStorageStrategyClass(HoodieTableConfig.StorageStrategy.OBJECT_STORE.value)
    ...
    try (SparkRDDWriteClient client = getHoodieWriteClient(cfg)) {
      ...
      // TODO: verify there are base files. Existing Test Utils can't list under storage path
      /*
      ...
      assertEquals(dataGen.getPartitionPaths().length * 2, hoodieTable.getFileSystemView().getFileGroupsInPendingClustering().count());
  ...
  void testClusteringWithNoBaseFiles(boolean clusteringAsRow, boolean doUpdates) throws Exception {
    ...
    try (SparkRDDWriteClient client = getHoodieWriteClient(cfg)) {
      ...
      List<HoodieRecord> records = dataGen.generateInserts(newCommitTime, 400);
      Stream<HoodieBaseFile> dataFiles = insertRecordsToMORTable(metaClient, records.subList(0, 200), client, cfg, newCommitTime);
      // TODO: verify there is no base files. Existing Test Utils can't list under storage path
      newCommitTime = "002";
      client.startCommitWithTime(newCommitTime);
      dataFiles = insertRecordsToMORTable(metaClient, records.subList(200, 400), client, cfg, newCommitTime);
      // TODO: verify there is no base files. Existing Test Utils can't list under storage path
      ...
      assertEquals(dataGen.getPartitionPaths().length, hoodieTable.getFileSystemView().getFileGroupsInPendingClustering().count());
  ...
  private void doClusteringAndValidate(SparkRDDWriteClient client,
    ...
    if (cfg.populateMetaFields()) {
      assertEquals(400, HoodieClientTestUtils.countRecordsOptionallySince(jsc(), metaClient, sqlContext(), timeline, Option.of("000")),
          "Must contain 200 records");
    } else {
      assertEquals(400, HoodieClientTestUtils.countRecordsOptionallySince(jsc(), metaClient, sqlContext(), timeline, Option.empty()));
    }
  }
}
