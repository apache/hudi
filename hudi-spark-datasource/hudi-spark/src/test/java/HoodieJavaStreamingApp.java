...
public class HoodieJavaStreamingApp {
    ...
    @Parameter(
            names = {"--table-path", "-p"},
            description = "path for Hoodie sample table")
    private String tablePath = "/tmp/hoodie/streaming/sample-table";

    @Parameter(
            names = {"--streaming-source-path", "-ssp"},
            description = "path for streaming source file folder")
    private String streamingSourcePath = "/tmp/hoodie/streaming/source";

    @Parameter(
            names = {"--streaming-checkpointing-path", "-scp"},
    ...
    @Parameter(
            names = {"--use-multi-partition-keys", "-mp"},
            description = "Use Multiple Partition Keys")
    ...
    public void run() throws Exception {
        ...
        try {
            ...
            Future<Integer> showFuture =
                    executor.submit(
                            () -> {
                                LOG.info("===== Showing Starting =====");
                                int numCommits =
                                        addInputAndValidateIngestion(
                                                spark, fs, srcPath, 0, 100, inputDF1, inputDF2, true);
        ...
        if (tableType.equals(HoodieTableType.MERGE_ON_READ.name())) {
            ...
            ValidationUtils.checkArgument(
                    metaClient.getActiveTimeline().getCommitAndReplaceTimeline().countInstants() == 1);
        } else {
            ValidationUtils.checkArgument(
                    metaClient.getActiveTimeline().getCommitAndReplaceTimeline().countInstants() >= 1);
        }
        ...
        // Deletes Stream
        // Need to restart application to ensure spark does not assume there are multiple streams
        // active.
        spark.close();

        SparkSession newSpark =
                SparkSession.builder()
                        .config(getSparkConfForTest("Hoodie Spark Streaming APP"))
                        .getOrCreate();
        ...
        Dataset<Row> delStreamingInput =
                newSpark.readStream().schema(inputDF1.schema()).json(srcPath2 + "/*");
            ...
            Future<Void> showFuture =
                    executor.submit(
                            () -> {
                                LOG.info("===== Showing Starting =====");
                                addInputAndValidateIngestion(
                                        newSpark, fs, srcPath2, numCommits, 80, inputDF3, null, false);
                                LOG.info("===== Showing Ends =====");
                                return null;
                            });
    ...
    private void waitTillNCommits(
            FileSystem fs, int numCommits, int timeoutSecs, int sleepSecsAfterEachRun)
        ...
        while ((currTime - beginTime) < timeoutMsecs) {
            try {
                ...
                HoodieTableMetaClient metaClient =
                        createMetaClient(new HadoopStorageConfiguration(fs.getConf()), tablePath);
        ...
        throw new IllegalStateException(
                "Timedout waiting for " + numCommits + " commits to appear in " + tablePath);
    }
    ...
    /**
      ...
      * @throws Exception
      */
    public int addInputAndValidateIngestion(
            SparkSession spark,
            FileSystem fs,
            String srcPath,
            int initialCommits,
            ...
            Dataset<Row> inputDF2,
            boolean instantTimeValidation)
            throws Exception {
        // Ensure, we always write only one file. This is very important to ensure a single batch is
        // reliably read
        ...
        if (null != inputDF2) {
            ...
            waitTillNCommits(fs, numExpCommits, 180, 3);
            commitInstantTime2 =
                    HoodieDataSourceHelpers.listCommitsSince(fs, tablePath, commitInstantTime1).stream()
                            .sorted()
                            .findFirst()
                            .get();
        ...
        if (tableType.equals(HoodieTableType.MERGE_ON_READ.name())) {
            ...
            waitTillNCommits(fs, numExpCommits, 180, 3);
            LOG.info(
                    "Compaction commit at instant time :"
                            + HoodieDataSourceHelpers.latestCommit(fs, tablePath));
        }

        /** Read & do some queries */
        Dataset<Row> hoodieROViewDF =
                spark
                        .read()
                        .format("hudi")
        ...
        spark
                .sql(
                        "select fare.amount, begin_lon, begin_lat, timestamp from hoodie_ro where fare.amount > 2.0")
                .show();
        ...
        if (instantTimeValidation) {
            ...
            long numRecordsAtInstant2 =
                    spark
                            .sql("select * from hoodie_ro where _hoodie_commit_time = " + commitInstantTime2)
                            .count();
            ValidationUtils.checkArgument(
                    numRecordsAtInstant2 == expRecords,
                    "Expecting " + expRecords + " records, Got " + numRecordsAtInstant2);
        }

        long numRecords = spark.sql("select * from hoodie_ro").count();
        ValidationUtils.checkArgument(
                numRecords == expRecords, "Expecting " + expRecords + " records, Got " + numRecords);

        if (tableType.equals(HoodieTableType.COPY_ON_WRITE.name())) {
            /**
              * Consume incrementally, only changes in commit 2 above. Currently only supported for
              * COPY_ON_WRITE TABLE
              */
            Dataset<Row> hoodieIncViewDF =
                    spark
                            .read()
                            .format("hudi")
                            .option(
    ...
    /**
      ...
      * @throws Exception
      */
    public void stream(Dataset<Row> streamingInput, String operationType, String checkpointLocation)
            throws Exception {
    ...
    private DataStreamWriter<Row> updateHiveSyncConfig(DataStreamWriter<Row> writer) {
        if (enableHiveSync) {
            ...
            writer =
                    writer
                            .option(META_SYNC_TABLE_NAME.key(), hiveTable)
            ...
            if (useMultiPartitionKeys) {
                writer =
                        writer
                                .option(HiveSyncConfig.META_SYNC_PARTITION_FIELDS.key(), "year,month,day")
                                .option(
                ...
                writer =
                        writer
                                .option(HiveSyncConfig.META_SYNC_PARTITION_FIELDS.key(), "dateStr")
                                .option(
                                        HiveSyncConfig.META_SYNC_PARTITION_EXTRACTOR_CLASS.key(),
