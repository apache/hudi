...
public class HoodieJavaApp {
    ...
    @Parameter(
            names = {"--non-partitioned", "-np"},
            description = "Use non-partitioned Table")
    ...
    public void run() throws Exception {
        SparkSession spark =
                SparkSession.builder().config(getSparkConfForTest("Hoodie Spark APP")).getOrCreate();
        ...
        // Generate some input..
        List<HoodieRecord> recordsSoFar =
                new ArrayList<>(dataGen.generateInserts("001" /* ignore */, 100));
        ...
        DataFrameWriter<Row> writer =
                inputDF1
                        ...
                        .option(DataSourceWriteOptions.ASYNC_CLUSTERING_ENABLE().key(), "true")
                        .option(
                                DataSourceWriteOptions.SPARK_SQL_OPTIMIZED_WRITES().key(),
        ...
        List<String> deletes =
                randomSelectAsHoodieKeys(recordsSoFar, 20).stream()
                        .map(
                                hr ->
                                        "{\"_row_key\":\""
                                                + hr.getRecordKey()
