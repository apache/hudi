...
upgrade path against data no current writer produces by default.

`hudi-v6-table-complex-keygen-bare.zip` is generated by the `6-bare` selector, which runs the same script
against the 0.14.1 bundle (the first release that stored bare keys) instead of 0.14.0:

```bash
./generate-fixtures.sh --version 6-bare --script-name generate-fixture-complex-keygen.scala
```
