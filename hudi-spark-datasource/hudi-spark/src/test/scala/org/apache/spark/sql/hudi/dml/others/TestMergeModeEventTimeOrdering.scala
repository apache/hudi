...
class TestMergeModeEventTimeOrdering extends HoodieSparkSqlTestBase {
  Seq(
    ...
    // Covers both table types: a SQL MERGE INTO delete is unconditional, so COW and MOR agree
    // and the expectations below are shared. Updates still honor event time ordering on both.
    test(s"Test merge operations with EVENT_TIME_ORDERING for $tableType table "
      ...
      withSparkSqlSessionConfigWithCondition(
        ...
        withRecordType()(withTempDir { tmp =>
          ...
          // Merge operation - delete with lower, equal and higher ts. All three take effect: a
          // delete issued by MERGE INTO does not consult the ordering value.
          spark.sql(
            s"""
