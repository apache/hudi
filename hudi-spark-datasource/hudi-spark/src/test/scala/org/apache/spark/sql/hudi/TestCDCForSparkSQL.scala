        ...
        """.stripMargin)
          ...
          spark.sql(
            s"""
               ...
               | using (
               |  select * from (
               |  select 1 as id, 'a1_v3' as name, 11 as price, cast(1300 as long) as ts
               |  union all
               |  select 4 as id, 'a4' as name, 14 as price, cast(1300 as long) as ts
               |  )
               | ) s0
        ...
        """.stripMargin)
          ...
          spark.sql(
            s"""
               ...
               | using (
               |  select * from (
               |  select 1 as id, 'a1_v3' as name, 11 as price, cast(1300 as long) as ts, "2021" as pt
               |  union all
               |  select 4 as id, 'a4' as name, 14 as price, cast(1300 as long) as ts, "2022" as pt
               |  )
               | ) s0
               | on s0.id = $tableName.id
