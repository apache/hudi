    ...
    withTempDir { tmp =>
      Seq("cow", "mor").foreach { tableType =>
       ...
       """.stripMargin)
        ...
        assertResult(s"`default`.`$tableName`")(resolvedLogicalPlan.asInstanceOf[ShowIndexesCommand].table.qualifiedName)
        ...
        assertResult(s"`default`.`$tableName`")(resolvedLogicalPlan.asInstanceOf[CreateIndexCommand].table.qualifiedName)
        ...
        resolvedLogicalPlan = analyzer.execute(logicalPlan)
        assertResult(s"`default`.`$tableName`")(resolvedLogicalPlan.asInstanceOf[DropIndexCommand].table.qualifiedName)
