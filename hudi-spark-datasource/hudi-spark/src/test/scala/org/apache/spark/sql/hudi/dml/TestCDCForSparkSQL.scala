...
class TestCDCForSparkSQL extends HoodieSparkSqlTestBase {
  ...
  test("Test delete all records in filegroup") {
    Seq("cow", "mor").foreach { tableType =>
      Seq("'preCombineField' = '_ts',", "").foreach { preCombineLine =>
        withTempDir { tmp =>
    ...
    Seq("cow", "mor").foreach { tableType =>
      Seq("'preCombineField' = '_ts',", "").foreach { preCombineLine =>
        ...
        """.stripMargin)
            ...
            spark.sql(s"update $tableName set name = 'a2_v2', _ts = 1200 where id = 2")
            ...
            spark.sql(
              s"""
                 ...
                 | using (
                 |  select * from (
                 |  select 1 as id, 'a1_v3' as name, cast(11 as double) as price, cast(1300 as long) as _ts
                 |  union all
                 |  select 4 as id, 'a4' as name, cast(14 as double) as price, cast(1300 as long) as _ts
                 |  )
                 | ) s0
                 | on s0.id = $tableName.id
                 | when matched then update set id = s0.id, name = s0.name, price = s0.price, _ts = s0._ts
        ...
        """.stripMargin)
            ...
            spark.sql(
              s"""
                 ...
                 | using (
                 |  select * from (
                 |  select 1 as id, 'a1_v3' as name, cast(11 as double) as price, cast(1300 as long) as _ts, "2021" as pt
                 |  union all
                 |  select 4 as id, 'a4' as name, cast(14 as double) as price, cast(1300 as long) as _ts, "2022" as pt
                 |  )
                 | ) s0
                 | on s0.id = $tableName.id
                 | when matched then update set id = s0.id, name = s0.name, price = s0.price, _ts = s0._ts, pt = s0.pt
                 | when not matched then insert *
        """.stripMargin)
            val commitTime4 = metaClient.reloadActiveTimeline.lastInstant().get().requestedTime
