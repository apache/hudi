...
class HoodieBackedTableMetadataIndexLookupV9Test extends HoodieBackedTableMetadataIndexLookupTestBase {
  ...
  override protected def testVersionSpecificBehavior(): Unit = {
    ...
    val nullResult = hoodieBackedTableMetadata.readSecondaryIndexWithoutMapping(nullKeys, secondaryIndexName).collectAsList().asScala
  ...
  private def testNullValueInSecondaryIndex(): Unit = {
    ...
    spark.sql(s"insert into $tableName " + "values(4, 'a4', 40, 1000)")
//    spark.sql(s"insert into $tableName " + "values(4, '$', 40, 1000)")
    hoodieBackedTableMetadata.reset()

    // Verify the data was inserted
    checkAnswer(s"select name from $tableName where id = 4")(
      Seq("a4")
//      Seq("$")
    )
    
    // Test that null value can be looked up in secondary index
    val nullValueKeys = HoodieListData.eager(List("a4").asJava).asInstanceOf[HoodieListData[String]]
//    val nullValueKeys = HoodieListData.eager(List("$").asJava).asInstanceOf[HoodieListData[String]]
    val nullValueResult = hoodieBackedTableMetadata.readSecondaryIndexWithoutMapping(nullValueKeys, secondaryIndexName).collectAsList().asScala
    ...
    val mixedKeysWithNull = HoodieListData.eager(List("b1", "$", "b2").asJava)
    val mixedWithNullResult = hoodieBackedTableMetadata.readSecondaryIndexWithoutMapping(mixedKeysWithNull, secondaryIndexName).collectAsList().asScala
    ...
    checkAnswer(s"select id, name, price, ts from $tableName where id = 4")()
    ...
    val nullValueResultAfterDelete = hoodieBackedTableMetadata.readSecondaryIndexWithoutMapping(nullValueKeysAfterDelete, secondaryIndexName).collectAsList().asScala
