  ...
  private def setupSharedTestData(): Unit = {
    ...
    spark.sql(
      s"""
         ...
         |  hoodie.metadata.record.index.max.filegroup.count = '1',
    ...
    spark.sql(s"insert into $tableName" + " values(3, 'b3', 30, 1000)")
    ...
    spark.sql("set hoodie.metadata.record.index.min.filegroup.count = 1")
    spark.sql("set hoodie.metadata.record.index.max.filegroup.count = 1")
...
//    checkAnswer(s"select id, name, price, ts from $tableName")(testData: _*)
  ...
  protected def testReadRecordIndexWithMapping(): Unit = {
    ...
    val allKeys = HoodieListData.eager(List("1", "2", "3").asJava)
    ...
    assert(resultKeys == Set("1", "2", "3"), "Keys should match input keys")
    ...
    val dupKeys = HoodieListData.eager(List("1", "1", "2", "2", "3", "3").asJava)
    ...
    assert(dupResultKeys == Set("1", "2", "3"), "Should deduplicate keys")

    // Case 6: Use parallelized RDD
    val jsc = new JavaSparkContext(spark.sparkContext)
    val context = new HoodieSparkEngineContext(jsc, new SQLContext(spark))
    val rddKeys = HoodieJavaRDD.of(List("1", "2", "3").asJava, context, 2)
...
//    val allSecondaryKeys = HoodieListData.eager(List("a1", "a2", "a3").asJava)
...
//    val mixedKeys = HoodieListData.eager(List("a1", "non_exist_1", "a2", "non_exist_2").asJava)
...
//    val dupKeys = HoodieListData.eager(List("a1", "a1", "a2", "a2", "a3", "a3").asJava)
...
//        HoodieListData.eager(List("a1").asJava), invalidIndexName)
...
//    assert(largeResult.collectAsList().isEmpty, "Large key list should return empty result for non-existing keys")
  ...
  protected def testGetSecondaryIndexRecords(): Unit = {
    ...
    val existingKeys = HoodieListData.eager(List("a1", "a2", "a3").asJava)
...
class HoodieBackedTableMetadataIndexLookupV8Test extends HoodieBackedTableMetadataIndexLookupTestBase {
  ...
  override protected def testVersionSpecificBehavior(): Unit = {
    ...
    val jsc = new JavaSparkContext(spark.sparkContext)
    val context = new HoodieSparkEngineContext(jsc, new SQLContext(spark))
    val rddKeys = HoodieJavaRDD.of(List("a1").asJava, context, 1)
    checkExceptionContain(() => {
...
//    val jsc = new JavaSparkContext(spark.sparkContext)
//    val context = new HoodieSparkEngineContext(jsc, new SQLContext(spark))
//    val rddKeys = HoodieJavaRDD.of(List("a1", "a2", "a3").asJava, context, 2)
...
//    val nullKeys = HoodieListData.eager(List("a1", null, "a2", null, "a3").asJava)
...
//    assert(nullResult.size == 3, s"Should return 3 results for non-null keys in table version ${getTableVersion}")
    ...
    // Test with null value in secondary index
    testNullValueInSecondaryIndex()
  }
...
//    spark.sql(s"insert into $tableName " + "values(4, 'a1', 40, 1000)") // this won't fail the test
    spark.sql(s"insert into $tableName " + "values(4, '$', 40, 1000)") // this will fail the test
...
//    val nullValueKeys = HoodieListData.eager(List("a1").asJava).asInstanceOf[HoodieListData[String]]
