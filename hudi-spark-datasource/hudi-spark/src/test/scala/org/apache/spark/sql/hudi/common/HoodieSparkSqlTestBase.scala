...
import org.apache.hudi.common.table.log.block.{HoodieCorruptBlock, HoodieDeleteBlock}
...
object HoodieSparkSqlTestBase {
  ...
  def validateLogBlockPrecombineNullOrZero(basePath: String,
                                           expectedNumLogFile: Int): Unit = {
    ...
    for (i <- 0 until expectedNumLogFile) {
      ...
      if (!logBlock.isInstanceOf[HoodieCorruptBlock]) {
        var a = 1
      }
//        .asInstanceOf[HoodieDeleteBlock].getRecordsToDelete
//      assertTrue(logBlock.apply(0).getOrderingValue == 0 || logBlock.apply(0).getOrderingValue == null)
    }
  }
