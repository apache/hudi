  ...
  test("Test Call run_rollback_inflight_tableservice Procedure for clustering") {
    withTempDir {tmp => {
     ...
     """.stripMargin)
      ...
      val clusteringInstantTime = clusteringInstant.getRequestTime
      ...
      Assertions.assertTrue(!metaClient.reloadActiveTimeline().getInstants
        .contains(INSTANT_FACTORY.createNewInstant(HoodieInstant.State.INFLIGHT, HoodieTimeline.CLUSTERING_ACTION, clusteringInstantTime)))
      Assertions.assertTrue(metaClient.reloadActiveTimeline().getInstants
        .contains(INSTANT_FACTORY.createNewInstant(HoodieInstant.State.REQUESTED, HoodieTimeline.CLUSTERING_ACTION, clusteringInstantTime)))
    }}
  }
  ...
  test("Test Call run_rollback_inflight_tableservice Procedure for compaction") {
    withTempDir {
      tmp => {
        ...
        val compactionInstantTime = compactionInstant.getRequestTime
        ...
        Assertions.assertTrue(!metaClient.reloadActiveTimeline().getInstants
          .contains(INSTANT_FACTORY.createNewInstant(HoodieInstant.State.INFLIGHT, HoodieTimeline.COMPACTION_ACTION, compactionInstantTime)))
        Assertions.assertTrue(!metaClient.reloadActiveTimeline().getInstants
          .contains(INSTANT_FACTORY.createNewInstant(HoodieInstant.State.REQUESTED, HoodieTimeline.COMPACTION_ACTION, compactionInstantTime)))
      }
    }
  }
