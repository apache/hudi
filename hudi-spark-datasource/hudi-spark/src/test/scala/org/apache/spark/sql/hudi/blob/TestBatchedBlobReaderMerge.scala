 ...
 * a null storage configuration (the merge methods do not touch I/O) and call mergeRanges /
...
class TestBatchedBlobReaderMerge {
  ...
  private def reader(maxGapBytes: Int = 4096) =
    new BatchedBlobReader(storageConf = null, maxGapBytes = maxGapBytes, lookaheadRows = 50)
