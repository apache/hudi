...
class TestAlterTableDropPartition extends HoodieSparkSqlTestBase {
  ...
  private def ensureLastCommitIncludesProperSchema(path: String, expectedSchema: Seq[String] = schemaFields): Unit = {
    ...
    val commitMetadata = metaClient.getActiveTimeline.loadInstantContent(lastInstant,  classOf[HoodieCommitMetadata])
    ...
    withTempDir { tmp =>
      Seq("cow", "mor").foreach { tableType =>
       ...
       """.stripMargin)
        ...
        val commitMetadata =
          metaClient.getActiveTimeline.loadInstantContent(lastInstant.get(), classOf[HoodieCommitMetadata])
