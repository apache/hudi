...
abstract class HoodieBackedTableMetadataIndexLookupTestBase extends HoodieSparkSqlTestBase {
  ...
  private def setupSharedTestData(): Unit = {
    ...
    // Insert new record in the log file
   ...
   */
  protected def testRecordIndex(): Unit = {
    // Case 1: Empty input
   ...
   */
  protected def testSecondaryIndex(): Unit = {
    // Get the secondary index partition name
    val secondaryIndexName = "secondary_index_idx_name"

    // Case 1: Empty input
    val emptyResult = hoodieBackedTableMetadata.readSecondaryIndex(
    ...
    val allResult = hoodieBackedTableMetadata.readSecondaryIndex(allSecondaryKeys, secondaryIndexName).collectAsList().asScala
    ...
    allResult.foreach { location =>
      ...
      // Validate location is not null
      assert(location.getValue != null, s"Location should not be null for table version ${getTableVersion}")

      // Validate location fields
      assert(location.getValue.getPartitionPath != null, s"Partition path should not be null for table version ${getTableVersion}")
      assert(location.getValue.getInstantTime != null, s"Instant time should not be null for table version ${getTableVersion}")
      assert(location.getValue.getFileId != null, s"File ID should not be null for table version ${getTableVersion}")

      // Validate position (should be valid or INVALID_POSITION)
      assert(location.getValue.getPosition >= HoodieRecordLocation.INVALID_POSITION,
    ...
    val nonExistResult = hoodieBackedTableMetadata.readSecondaryIndex(nonExistKeys, secondaryIndexName).collectAsList().asScala
    ...
    val mixedResult = hoodieBackedTableMetadata.readSecondaryIndex(mixedKeys, secondaryIndexName).collectAsList().asScala
    ...
    val dupResult = hoodieBackedTableMetadata.readSecondaryIndex(dupKeys, secondaryIndexName).collectAsList().asScala
    ...
    val priceResult = hoodieBackedTableMetadata.readSecondaryIndex(priceKeys, priceIndexName).collectAsList().asScala
    ...
    checkExceptionContain(() => {
      hoodieBackedTableMetadata.readSecondaryIndex(
        HoodieListData.eager(List("b1").asJava), invalidIndexName)
    })("Secondary index is not initialized in MDT for: non_existent_index")
    ...
    val largeResult = hoodieBackedTableMetadata.readSecondaryIndex(largeKeys, secondaryIndexName)
  ...
  protected def testGetSecondaryIndexRecords(): Unit = {
    ...
    val resultMap = result.collectAsMapWithOverwriteStrategy()
    ...
    val nonExistingMap = nonExistingResult.collectAsMapWithOverwriteStrategy()
...
/**
 * Test class for table version 8 (SECONDARY_INDEX_ONE)
 */
class HoodieBackedTableMetadataIndexLookupV8Test extends HoodieBackedTableMetadataIndexLookupTestBase {

  override protected def getTableVersion: String = "8"

  override protected def getExpectedIndexVersion: String = "SECONDARY_INDEX_ONE"
  ...
  test("Exhaustive test for readSecondaryIndex - Version 8") {
    testSecondaryIndex()
  }
  ...
  override protected def testVersionSpecificBehavior(): Unit = {
    ...
    var rddKeys = HoodieJavaRDD.of(List("b1").asJava, context, 1)
    checkExceptionContain(() => {
      hoodieBackedTableMetadata.readSecondaryIndex(rddKeys, secondaryIndexName)
    })("only support HoodieListData")

    rddKeys = HoodieJavaRDD.of(List("1", "2", "$").asJava, context, 2)
    checkExceptionContain(() => {
      hoodieBackedTableMetadata.readRecordIndex(rddKeys)
    })("only support HoodieListData")
  }
}
...
/**
 * Test class for table version 9 (SECONDARY_INDEX_TWO)
 */
class HoodieBackedTableMetadataIndexLookupV9Test extends HoodieBackedTableMetadataIndexLookupTestBase {

  override protected def getTableVersion: String = "9"

  override protected def getExpectedIndexVersion: String = "SECONDARY_INDEX_TWO"
  ...
  override protected def testVersionSpecificBehavior(): Unit = {
    val secondaryIndexName = "secondary_index_idx_name"
    // TODO [HUDI-9501] For version 2, test that it supports both HoodieListData and RDD
    //    val jsc = new JavaSparkContext(spark.sparkContext)
    //    val context = new HoodieSparkEngineContext(jsc, new SQLContext(spark))
    //    val rddKeys = HoodieJavaRDD.of(List("b1", "b2", "$").asJava, context, 2)
    //    val rddResult = hoodieBackedTableMetadata.readSecondaryIndex(rddKeys, secondaryIndexName)
    //    assert(rddResult.collectAsList().asScala.size == 3, "Version 2 should support RDD input")

    // v2 works with null keys
    val nullKeys = HoodieListData.eager(List("b1", null, "b2", null, "$").asJava)
    val nullResult = hoodieBackedTableMetadata.readSecondaryIndex(nullKeys, secondaryIndexName).collectAsList().asScala
    assert(nullResult.size == 3, s"Should return 3 results for non-null keys in table version ${getTableVersion}")
  }

  /**
   * Test null value handling in secondary index for version 2
   */
  private def testNullValueInSecondaryIndex(): Unit = {
    val secondaryIndexName = "secondary_index_idx_name"

    // Insert a row with null value in the name column
    spark.sql(s"insert into $tableName " + "values(4, null, 40, 1000)")
    hoodieBackedTableMetadata.reset()

    // Test that null value can be looked up in secondary index
    val nullValueKeys = HoodieListData.eager(List(null, null).asJava).asInstanceOf[HoodieListData[String]]
    val nullValueResult = hoodieBackedTableMetadata.readSecondaryIndex(nullValueKeys, secondaryIndexName).collectAsList().asScala
    assert(nullValueResult.size == 1, s"Should return 1 result for null value in table version ${getTableVersion}")

    // Test that null value is properly indexed and can be found
    val mixedKeysWithNull = HoodieListData.eager(List("b1", null, "b2").asJava)
    val mixedWithNullResult = hoodieBackedTableMetadata.readSecondaryIndex(mixedKeysWithNull, secondaryIndexName).collectAsList().asScala
    assert(mixedWithNullResult.size == 3, s"Should return 3 results including null value in table version ${getTableVersion}")

    // Remove the record with null value
    spark.sql(s"delete from $tableName where id = 4")
    hoodieBackedTableMetadata.reset()

    // Test that null value lookup now returns empty result
    val nullValueKeysAfterDelete = HoodieListData.eager(List(null).asJava).asInstanceOf[HoodieListData[String]]
    val nullValueResultAfterDelete = hoodieBackedTableMetadata.readSecondaryIndex(nullValueKeysAfterDelete, secondaryIndexName).collectAsList().asScala
    assert(nullValueResultAfterDelete.isEmpty, s"Should return empty result for deleted null value in table version ${getTableVersion}")
  }
}
