...
class MockSlashPartitionValueExtractor extends PartitionValueExtractor {
   ...
   * @param partitionPath PartitionPath provided will be in the format yyyy/mm/dd/country/stat/city
   * @return is a List of string with values template such as yyyy-mm-dd, country, state, city
   */
  override def extractPartitionValuesInPath(partitionPath: String): util.List[String] = {
    ...
    val date = s"${partitionSplitsSeq(0)}-${partitionSplitsSeq(1)}-${partitionSplitsSeq(2)}"
