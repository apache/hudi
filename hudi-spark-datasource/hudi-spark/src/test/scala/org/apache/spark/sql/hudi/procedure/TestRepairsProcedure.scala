...
class TestRepairsProcedure extends HoodieSparkProcedureTestBase {
  ...
  test("Test Call repair_add_partition_meta Procedure") {
    withTempDir { tmp =>
      ...
      assertResult(metaClient.getHoodieStorage.createDirectory(new StoragePath(partition1))) {
        true
      }
      assertResult(metaClient.getHoodieStorage.createDirectory(new StoragePath(partition2))) {
        true
      }
      assertResult(metaClient.getHoodieStorage.createDirectory(new StoragePath(partition3))) {
        true
      }
  ...
  test("Test Call repair_deduplicate Procedure with insert") {
    withTempDir { tmp =>
      ...
      val fsView = new HoodieTableFileSystemView(metaClient, metaClient.getActiveTimeline.getCommitTimeline.filterCompletedInstants,
        metaClient.getHoodieStorage.listDirectEntries(new StoragePath(duplicatedPartitionPath)))
      ...
      val fileStatus = metaClient.getHoodieStorage.listDirectEntries(new StoragePath(repairedOutputPath))
      files = fileStatus.asScala.map((status: StoragePathInfo) => status.getPath.toString).toArray
  ...
  test("Test Call repair_deduplicate Procedure with update") {
    withTempDir { tmp =>
      ...
      // get fs and check number of latest files
      val fsView = new HoodieTableFileSystemView(metaClient, metaClient.getActiveTimeline.getCommitTimeline.filterCompletedInstants,
        metaClient.getHoodieStorage.listDirectEntries(new StoragePath(duplicatedPartitionPathWithUpdates)))
      val filteredStatuses = fsView.getLatestBaseFiles.iterator().asScala.map(value => value.getPath).toList
  ...
  test("Test Call repair_deduplicate Procedure with upsert") {
    withTempDir { tmp =>
      ...
      // get fs and check number of latest files
      val fsView = new HoodieTableFileSystemView(metaClient, metaClient.getActiveTimeline.getCommitTimeline.filterCompletedInstants,
        metaClient.getHoodieStorage.listDirectEntries(new StoragePath(duplicatedPartitionPathWithUpserts)))
      val filteredStatuses = fsView.getLatestBaseFiles.iterator().asScala.map(value => value.getPath).toList
      // there should be 3 files
      assertResult(3) {
        filteredStatuses.size
  ...
  test("Test Call repair_deduplicate Procedure with real") {
    withTempDir { tmp =>
      ...
      // after deduplicate, there are 200 records
      val fileStatus = metaClient.getHoodieStorage.listDirectEntries(new StoragePath(duplicatedPartitionPath))
      files = fileStatus.asScala.map((status: StoragePathInfo) => status.getPath.toString).filter(p => p.endsWith(".parquet")).toArray
      recordCount = getRecordCount(files)
      assertResult(200){recordCount}
    }
  }
