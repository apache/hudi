...
class TestCOWDataSource extends HoodieSparkClientTestBase with ScalaAssertionSupport {
  ...
  def testInferPartitionBy(): Unit = {
    ...
    val snapshot0 = spark.read.format("hudi").options(readOpts).load(basePath)
    snapshot0.cache()
  ...
  def testInsertOverWriteTableWithInsertDropDupes(): Unit = {
    ...
    val inputDF1 = spark.read.json(spark.sparkContext.parallelize(records1, 2))
    inputDF1.withColumn("batchId", lit("batch1")).write.format("hudi")
      .options(writeOpts)
      .mode(SaveMode.Overwrite)
      .save(basePath)
  ...
  def bulkInsertCompositeKeys(): Unit = {
    ...
    assertEquals(200, spark.read.format("hudi").options(readOpts).load(basePath).count())
  }
  ...
  def testCopyOnWriteDeletes(recordType: HoodieRecordType): Unit = {
    ...
    val snapshotDF2 = spark.read.format("hudi").options(readOpts).load(basePath)
  ...
  class UpdateThread(dataGen: HoodieTestDataGenerator,
    ...
    override def run() {
      ...
      try {
        updateDf.union(insertDf).write.format("hudi")
          .options(commonOpts)
          .option("hoodie.write.concurrency.mode", "optimistic_concurrency_control")
          .option("hoodie.clean.failed.writes.policy", "LAZY")
          .option("hoodie.write.lock.provider", "org.apache.hudi.client.transaction.lock.InProcessLockProvider")
  ...
  def testReadPathsOnCopyOnWriteTable(recordType: HoodieRecordType): Unit = {
    ...
    val hudiReadPathDF = spark.read.format("hudi")
      .options(readOpts)
  ...
  def testOverWriteModeUseReplaceActionOnDisJointPartitions(): Unit = {
    ...
    val allRecords = spark.read.format("hudi").options(readOpts).load(basePath)
  ...
  def testDropInsertDup(recordType: HoodieRecordType): Unit = {
    ...
    inputDF1.write.format("hudi")
      ...
      .mode(SaveMode.Overwrite)
      .save(basePath)
    val hoodieROViewDF1 = spark.read.format("hudi").options(readOpts).load(basePath)
    ...
    val hoodieROViewDF2 = spark.read.format("hudi").options(readOpts).load(basePath)
  ...
  def testComplexDataTypeWriteAndReadConsistency(recordType: HoodieRecordType): Unit = {
    ...
    recordsDF.write.format("hudi")
      .options(writeOpts)
      .mode(SaveMode.Overwrite)
      .save(basePath)

    val recordsReadDF = spark.read.format("hudi").options(readOpts).load(basePath)
  ...
  def testSparkPartitionByWithCustomKeyGeneratorWithGlobbing(recordType: HoodieRecordType): Unit = {
    ...
    writer.partitionBy("current_ts")
      .mode(SaveMode.Overwrite)
      .save(basePath)
    var recordsReadDF = spark.read.format("hudi").options(readOpts).load(basePath)
    ...
    writer.partitionBy("current_ts:TIMESTAMP")
      ...
      .mode(SaveMode.Overwrite)
      .save(basePath)
    recordsReadDF = spark.read.format("hudi").options(readOpts).load(basePath)
  ...
  def testSparkPartitionByWithGlobalDeleteKeyGenerator(recordType: HoodieRecordType) {
    ...
    val recordsReadDF = spark.read.format("hudi")
      .options(readOpts)
      .load(basePath)
  ...
  def testHoodieIsDeletedCOW(recordType: HoodieRecordType): Unit = {
    ...
    val snapshotDF0 = spark.read.format("hudi")
      .options(readOpts)
      .load(basePath)
  ...
  def testHiveStyleDelete(): Unit = {
    ...
    df.write.format("hudi").options(hudiOptions).mode(SaveMode.Overwrite).save(basePath)
