 ...
 */
class TestMORDataSourceStorageStrategy extends HoodieClientTestBase with SparkDatasetMixin {

  var spark: SparkSession = null
  // TODO: remove this log is not used
  private val log = LogManager.getLogger(classOf[TestMORDataSourceStorageStrategy])
  val commonOpts = Map(
  ...
  @Test def testStorageStrategy() {
    ...
    resultDF.show(false);
    assertEquals(resultDF.where("partition = '2022/01/01'").count, 5)
  }

  @Test def testStorageStrategyCompactionWithS3() {
    // generate data
    val records1 = recordsToStrings(dataGen.generateInserts("003", 100)).asScala
    val inputDF1 = spark.read.json(spark.sparkContext.parallelize(records1, 2))

    val r = scala.util.Random
    val num = 94439 //r.nextInt(99999)
//    val tablePath = "/Users/yxchang/tables/prefix/" + num
    val tableName = "table_mor_" + num
    val tablePath = "s3a://emr-apps-users/yxchang/tables/storage/" + tableName
    val storagePath = "s3a://yxchang-storage-ut/"
...
//    inputDF1.show(false)
    ...
    inputDF1.write.format("hudi")
      ...
      .option(HoodieCleanConfig.CLEANER_COMMITS_RETAINED.key, 1)
      .option(HoodieTableConfig.HOODIE_STORAGE_STRATEGY_CLASS_NAME.key, HoodieTableConfig.StorageStrategy.OBJECT_STORE.value)
      .option(HoodieWriteConfig.STORAGE_PATH.key, storagePath)
      .mode(SaveMode.Append)
      .save(tablePath)

//    println("yxchang: inserting second record, should trigger compaction")
...
//    assertEquals(102, hudiSnapshotDF6.count())
  ...
  @ParameterizedTest
  @CsvSource(Array("true,true", "false,true"))
...
//    assertEquals(0, hudiReadPathDF2.count())
  ...
  def testReadLogOnlyMergeOnReadTable(): Unit = {
    ...
    assertEquals(20, spark.read.format("hudi").option(HoodieMetadataConfig.ENABLE.key(), "true").load(basePath).count())
  }
  ...
  def testPrunePartitionForTimestampBasedKeyGenerator(): Unit = {
    val options = commonOpts ++ Map(
      ...
      HoodieTableConfig.HOODIE_STORAGE_STRATEGY_CLASS_NAME.key -> HoodieTableConfig.StorageStrategy.OBJECT_STORE.value,
      HoodieWriteConfig.STORAGE_PATH.key -> storagePath
    )
    ...
    // snapshot query
    val snapshotQueryRes = spark.read.format("hudi").option(HoodieMetadataConfig.ENABLE.key(), "true")
      .load(basePath)
