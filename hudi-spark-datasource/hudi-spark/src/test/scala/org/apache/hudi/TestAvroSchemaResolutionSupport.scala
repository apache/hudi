...
class TestAvroSchemaResolutionSupport extends HoodieClientTestBase {
  ...
  def testArrayOfStructsAddNewColumn(isCow: Boolean): Unit = {
    // This bug will prevent Array[Map/Struct] schema evolution from working properly: https://github.com/apache/spark/commit/32a393395ee43b573ae75afba591b587ca51879b
    // Bug fix is only available Spark >= v3.1.3
    if (HoodieSparkUtils.gteqSpark3_1_3) {
      val tempRecordPath = basePath + "/record_tbl/"
  ...
  def testComplexOperationsOnTable(isCow: Boolean): Unit = {
    if (HoodieSparkUtils.gteqSpark3_1) {
      ...
      // Test a series of changes on Avro table, of which, all actions will be performed on
