...
class RecordLevelIndexTestBase extends HoodieStatsIndexTestBase {
  ...
  protected def validateDataAndRecordIndices(hudiOpts: Map[String, String],
    ...
    val recordIndexMap = HoodieDataUtils.collectAsMapWithOverwriteStrategy(metadata.readRecordIndexWithMapping(
      HoodieListData.`lazy`(JavaConverters.seqAsJavaListConverter(rowArr.map(row => row.getAs("_hoodie_record_key").toString).toList).asJava)))
    ...
    val recordIndexMapForDeletedRows = HoodieDataUtils.collectAsMapWithOverwriteStrategy(metadata.readRecordIndexWithMapping(
      HoodieListData.`lazy`(JavaConverters.seqAsJavaListConverter(deletedRows.map(row => row.getAs("_row_key").toString).toList).asJava)))
