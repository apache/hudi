...
class TestRliLookupMetricsOnDataSource extends RliLookupMetricsTestBase {
  ...
  /** A commit that performed no lookup must carry nothing at all -- not a row of zeros. */
  @Test
  def testACommitWithNoLookupCarriesNoCounters(): Unit = {
    ...
    assertTrue(counters.isEmpty,
      s"a commit that looked nothing up must carry no counters, not zero-valued residue; got $counters")
  }
