...
class PartitionStatsIndexTestBase extends HoodieSparkClientTestBase {
  ...
  protected def getLatestMetaClient(enforce: Boolean): HoodieTableMetaClient = {
    ...
    if (enforce || metaClient.getActiveTimeline.lastInstant().get().getRequestTime.compareTo(lastInstant) < 0) {
  ...
  protected def deleteLastCompletedCommitFromDataAndMetadataTimeline(hudiOpts: Map[String, String]): Unit = {
    ...
    assertTrue(storage.deleteFile(new StoragePath(metadataTableMetaClient.getMetaPath, INSTANT_FILE_NAME_FACTORY.getFileName(metadataTableLastInstant))))
    mergedDfList = mergedDfList.take(mergedDfList.size - 1)
  }
...
object PartitionStatsIndexTestBase {
  ...
  def checkIfOverlapped(a: HoodieInstant, b: HoodieInstant): Boolean = {
    !(a.getCompletionTime.compareTo(b.getRequestTime) < 0 || a.getRequestTime.compareTo(b.getCompletionTime) > 0)
  }
}
