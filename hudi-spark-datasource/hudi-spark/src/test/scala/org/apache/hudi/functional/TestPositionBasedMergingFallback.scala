...
class TestPositionBasedMergingFallback extends HoodieSparkClientTestBase {
  ...
  def testSimpleCaseWithoutFallback(updateWithRecordPositions: String, deleteWithRecordPositions: String, secondUpdateWithPositions: String): Unit = {
    ...
    inserts.write.format("hudi").
      ...
      option(HoodieTableConfig.NAME.key(), "test_table").
      option(TABLE_TYPE.key(), "MERGE_ON_READ").
      option(HoodieStorageConfig.LOGFILE_DATA_BLOCK_FORMAT.key(), "parquet").
      option(HoodieWriteConfig.RECORD_MERGER_IMPLS.key(), "org.apache.hudi.HoodieSparkRecordMerger").
    ...
    assertEquals(finalDf.except(expectedDf).count(), 0)
    assertEquals(expectedDf.except(finalDf).count(), 0)
    ...
    assertEquals(finalFilterDf.except(expectedFilterDf).count(), 0)
    assertEquals(expectedFilterDf.except(finalFilterDf).count(), 0)
  }
