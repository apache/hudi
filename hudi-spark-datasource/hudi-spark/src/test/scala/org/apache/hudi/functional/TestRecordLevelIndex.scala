...
class TestRecordLevelIndex extends RecordLevelIndexTestBase {
  ...
  def testRLIWithDTCleaning(tableType: HoodieTableType): Unit = {
    ...
    doWriteAndValidateDataAndRecordIndex(hudiOpts,
      operation = DataSourceWriteOptions.UPSERT_OPERATION_OPT_VAL,
      saveMode = SaveMode.Append)
    assertTrue(getLatestMetaClient(false).getActiveTimeline.getCleanerTimeline.lastInstant().get().getRequestTime
      .compareTo(lastCleanInstant.get().getRequestTime) > 0)
  ...
  def testRLIWithDTCompaction(): Unit = {
    ...
    doWriteAndValidateDataAndRecordIndex(hudiOpts,
      operation = DataSourceWriteOptions.UPSERT_OPERATION_OPT_VAL,
      saveMode = SaveMode.Append)
    assertTrue(getLatestCompactionInstant().get().getRequestTime.compareTo(lastCompactionInstant.get().getRequestTime) > 0)
    lastCompactionInstant = getLatestCompactionInstant()

    var rollbackedInstant: Option[HoodieInstant] = Option.empty
    while (rollbackedInstant.isEmpty || rollbackedInstant.get.getRequestTime != lastCompactionInstant.get().getRequestTime) {
