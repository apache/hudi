...
class TestMORDataSourceStorage extends SparkClientFunctionalTestHarness {
  ...
  def testAutoDisablingRecordPositionsUnderPendingCompaction(enableNBCC: Boolean): Unit = {
    ...
    for (i <- 1 to 3) {
      ...
      logFileList = validateRecordPositionsInLogFiles(metaClient, !enableNBCC)
    }
    ...
    for (i <- 4 to 6) {
      ...
      if (i == 4) {
        ...
        validateRecordPositionsInLogFiles(
          metaClient, shouldContainRecordPosition = !enableNBCC, logFileList,
          shouldBaseFileInstantTimeMatch = false)
      }
