...
class TestRliLookupMetricsOnSparkSql extends RliLookupMetricsTestBase {
  ...
  def testUpdateWithoutOptimizedWritesPublishesCounters(): Unit = {
    ...
    val lookedUp = assertSumInvariant(counters, RecordIndexMetricNames.CALLER_TAG_LOCATION)
    assertEquals(numSeedRecords.toLong, lookedUp, "UPDATE with no predicate tags every row")
    assertEquals(numSeedRecords.toString, counters(tagKey(RecordIndexMetricNames.KEY_HIT_COUNT)),
    ...
    assertTrue(counters.contains(tagKey(RecordIndexMetricNames.KEY_MISS_COUNT)),
      "a caller that looked keys up reports its full counter set, including the zeros")
    assertEquals("0", counters(tagKey(RecordIndexMetricNames.KEY_MISS_COUNT)), "no new keys are introduced")
  ...
  def testMergeIntoPublishesCounters(): Unit = {
    ...
    assertEquals(numMerged.toString, counters(tagKey(RecordIndexMetricNames.KEY_HIT_COUNT)),
