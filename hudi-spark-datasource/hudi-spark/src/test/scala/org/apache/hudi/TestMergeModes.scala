...
class TestMergeModes extends SparkClientFunctionalTestHarness {
  ...
  def testMergeModes(useFgReader: String,
                     tableType: String,
                     recordType: String,
                     positionUsed: String,
    ...
    val updatesDf = spark.createDataFrame(updateData).toDF(columns: _*)
    ...
    val deletesDf = spark.createDataFrame(deletesData).toDF(columns: _*)
    ...
    // Update records.
    val updatesAgain = Seq(
      (19, "3", "rider-S", "driver-S", 19.10, "u"),
      (9, "5", "rider-T", "driver-T", 27.70, "u"))
    val updatesAgainDf = spark.createDataFrame(updatesAgain).toDF(columns: _*)
    updatesAgainDf.write.format("hudi").
      option(OPERATION.key(), "upsert").
      option(HoodieCompactionConfig.INLINE_COMPACT.key(), "false").
      options(opts).
      mode(SaveMode.Append).
    ...
    TestMergeModes.validate(expectedDf, finalDf)
  }
}

object TestMergeModes {
