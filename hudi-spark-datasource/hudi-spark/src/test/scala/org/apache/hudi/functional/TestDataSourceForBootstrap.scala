...
class TestDataSourceForBootstrap {
  ...
  @Test def testFullBootstrapCOWPartitioned(): Unit = {
    ...
    bootstrapDF.write
      ...
      .option(HoodieBootstrapConfig.BOOTSTRAP_KEYGEN_CLASS_NAME.key, classOf[SimpleKeyGenerator].getName)
      .option(HoodieBootstrapConfig.BOOTSTRAP_MODE_SELECTOR_CLASS.key, classOf[FullRecordBootstrapModeSelector].getName)
      .option(HoodieBootstrapConfig.FULL_BOOTSTRAP_INPUT_PROVIDER_CLASS.key, classOf[SparkParquetBootstrapDataProvider].getName)
      .mode(SaveMode.Overwrite)
      .save(basePath)
