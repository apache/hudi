...
class TestPartitionStatsIndex extends PartitionStatsIndexTestBase {
   ...
   * 4. Do a savepoint and restore, and validate partition_stats is deleted, but column_stats is not.
  ...
  def testPartitionStatsWithRestore(): Unit = {
    ...
    doWriteAndValidateDataAndPartitionStats(
      ...
      saveMode = SaveMode.Append)
    metaClient = HoodieTableMetaClient.reload(metaClient)
    val secondCompletedInstant =
      metaClient.getActiveTimeline.getCommitsTimeline.filterCompletedInstants().getInstants().get(1)
    ...
    assertEquals(6, cleanMetadata.getTotalFilesDeleted)
