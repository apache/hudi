 ...
 */
class TestCOWDataSourceStorageStrategy extends HoodieClientTestBase {
  ...
  @Test def testStorageStrategy(): Unit = {
    // generate data
    ...
    val tableName = "table_" + num
    val tablePath = "/Users/yxchang/tables/prefix/" + tableName
    val storagePath = "/Users/yxchang/tables/repair"

    //    val tablePath = basePath

    println("Data being written: ")
    inputDF1.show(false)
    println("yxchang --> creating a table with MDT disabled: " + tablePath)
    ...
    println("yxchang --> table with MDT disabled created: " + tablePath)

    println("yxchang: --> upserting to this table with MDT enabled")

    val dataGen2 = new HoodieTestDataGenerator(Array("2022/02/01"))
    val records2 = recordsToStrings(dataGen2.generateInserts("100", 5)).toList
    ...
    println("yxchang: --> MDT should be initialed by now, show me the MDT")
  ...
  @Test def readMT(): Unit = {
    ...
    spark.read.format("hudi")
      .option(DataSourceReadOptions.EXTRACT_PARTITION_VALUES_FROM_PARTITION_PATH.key(), "true")
      .option(HoodieIndexConfig.INDEX_TYPE.key(), "BLOOM")
      .load("s3a://emr-apps-users/yxchang/tables/storage-benchmark-hms/hudi_store_sales_cow_benchmark_hms/.hoodie/metadata/")
      .show(false)
  }

  @Disabled
  @Test def testStorageStrategyWithS3() {
    // generate data
    val dataGen1 = new HoodieTestDataGenerator(Array("2022/01/01"))
    val records1 = recordsToStrings(dataGen1.generateInserts("001", 5)).toList
    val inputDF1 = spark.read.json(spark.sparkContext.parallelize(records1, 2))
    ...
    val num = r.nextInt(99999)
    val tableName = "table_" + num
    var tablePath = "/Users/yxchang/tables/prefix/" + tableName
    val s3TablePath = "s3a://emr-apps-users/yxchang/tables/storage/" + tableName
    val s3StoragePath = "s3a://yxchang-storage-ut/"

//    val tablePath = basePath
    val storageStrategy = HoodieTableConfig.StorageStrategy.OBJECT_STORE.value

    var storagePath = if (storageStrategy.equals(HoodieTableConfig.StorageStrategy.DEFAULT.value)) {
      tablePath
    } else {
      "/Users/yxchang/tables/storage"
    }
    ...
    inputDF1.write.format("hudi")
      ...
      .option(HoodieWriteConfig.TBL_NAME.key, tableName)
      .option(DataSourceWriteOptions.OPERATION.key, DataSourceWriteOptions.UPSERT_OPERATION_OPT_VAL)
      .option(HoodieTableConfig.HOODIE_STORAGE_STRATEGY_CLASS_NAME.key, storageStrategy)
      .option(HoodieTableConfig.HOODIE_STORAGE_PATH_KEY, storagePath)
      .mode(SaveMode.Overwrite)
      .save(tablePath)
...
//    val deleteDF = resultDF.filter(col("partition") === "2022/01/01")
//
//    deleteDF.show(false)
...
//    resultDF2.show(false);
  ...
  @Test def readS3MT(): Unit = {
    ...
    spark.read.format("hudi")
      .option(DataSourceReadOptions.EXTRACT_PARTITION_VALUES_FROM_PARTITION_PATH.key(), "true")
      .option(HoodieIndexConfig.INDEX_TYPE.key(), "BLOOM")
      .load("s3a://emr-apps-users/yxchang/tables/storage/table_49260" + "/.hoodie/metadata").show(false)
  }

  @Disabled
  @Test def testStorageStrategyUpdateWithS3() {
    val _spark = spark
    ...
    val df2 = Seq(
      ("112", "2015-01-01", "event_name_900", "2015-01-01T13:51:39.340396Z", "type1"),
      ("113", "2015-01-01", "event_name_546", "2015-01-01T12:14:58.597216Z", "type2"),
      ("114", "2015-01-01", "event_name_345", "2015-01-01T13:51:40.417052Z", "type3"),
      ("115", "2015-01-01", "event_name_234", "2015-01-01T13:51:40.519832Z", "type4"),
      ("116", "2015-01-01", "event_name_123", "2015-01-01T12:15:00.512679Z", "type1"),
      ("117", "2015-01-01", "event_name_678", "2015-01-01T13:51:42.248818Z", "type2"),
      ("118", "2015-01-01", "event_name_890", "2015-01-01T13:51:44.735360Z", "type3"),
      ("119", "2015-01-01", "event_name_944", "2015-01-01T13:51:45.019544Z", "type4"),
      ("120", "2015-01-01", "event_name_456", "2015-01-01T13:51:45.208007Z", "type1"),
      ("121", "2015-01-01", "event_name_567", "2015-01-01T13:51:45.369689Z", "type2"),
      ("122", "2015-01-01", "event_name_789", "2015-01-01T12:15:05.664947Z", "type3"),
      ("123", "2015-01-01", "event_name_322", "2015-01-01T13:51:47.388239Z", "type4")
    ).toDF("event_id", "event_date", "event_name", "event_ts", "event_type")

    val df3 = Seq(
      ("142", "2015-01-01", "event_name_900", "2015-01-01T13:51:39.340396Z", "type1"),
      ("143", "2015-01-01", "event_name_546", "2015-01-01T12:14:58.597216Z", "type2"),
      ("124", "2015-01-01", "event_name_345", "2015-01-01T13:51:40.417052Z", "type3"),
      ("125", "2015-01-01", "event_name_234", "2015-01-01T13:51:40.519832Z", "type4"),
      ("126", "2015-01-01", "event_name_123", "2015-01-01T12:15:00.512679Z", "type1"),
      ("127", "2015-01-01", "event_name_678", "2015-01-01T13:51:42.248818Z", "type2"),
      ("128", "2015-01-01", "event_name_890", "2015-01-01T13:51:44.735360Z", "type3"),
      ("129", "2015-01-01", "event_name_944", "2015-01-01T13:51:45.019544Z", "type4"),
      ("130", "2015-01-01", "event_name_456", "2015-01-01T13:51:45.208007Z", "type1"),
      ("131", "2015-01-01", "event_name_567", "2015-01-01T13:51:45.369689Z", "type2"),
      ("132", "2015-01-01", "event_name_789", "2015-01-01T12:15:05.664947Z", "type3"),
      ("133", "2015-01-01", "event_name_322", "2015-01-01T13:51:47.388239Z", "type4")
    ).toDF("event_id", "event_date", "event_name", "event_ts", "event_type")

    val r = scala.util.Random
    val num = r.nextInt(99999)
    val tableName = "storage_small_hms_" + num
    var tablePath = "s3a://yxchang-emr-dev/tables/hudi/tables/" + tableName
    val s3TablePath = "s3a://yxchang-emr-dev/tables/hudi/tables/" + tableName
    val s3StoragePath = "s3a://yxchang-storage-small-hms/"
    ...
    df1.write.format("hudi")
      ...
      .option(DataSourceWriteOptions.TABLE_TYPE.key(), DataSourceWriteOptions.COW_TABLE_TYPE_OPT_VAL)
      .option(DataSourceWriteOptions.RECORDKEY_FIELD.key(), "event_id")
      .option(DataSourceWriteOptions.PARTITIONPATH_FIELD.key(), "event_type")
      .option(DataSourceWriteOptions.PRECOMBINE_FIELD.key(), "event_ts")
      .option(HoodieTableConfig.HOODIE_STORAGE_STRATEGY_CLASS_NAME.key, storageStrategy)
      .option(HoodieWriteConfig.STORAGE_PATH.key, storagePath)
  ...
  @Disabled
  @Test def testStorageStrategyClean() {
    val _spark = spark
    import _spark.implicits._
    // generate data
    val df1 = Seq(
  ...
  def testCopyOnWriteDeletes(): Unit = {
    ...
    val snapshotDFchang = spark.read.format("org.apache.hudi")
      .option(HoodieMetadataConfig.ENABLE.key, "true")
      .load(basePath)

    snapshotDFchang.show(false)
  ...
  @Test def testOverWriteModeUseReplaceActionOnDisJointPartitions(): Unit = {
    ...
    val allRecords = spark.read.format("org.apache.hudi").option(HoodieMetadataConfig.ENABLE.key(), "true").load(basePath)
    allRecords.registerTempTable("tmpTable")

    allRecords.show(false)
  ...
  @Disabled // TODO: need to use the updated test
  ...
  def testCopyOnWriteWithDroppedPartitionColumns(enableDropPartitionColumns: Boolean) {
    ...
    inputDF1.write.format("org.apache.hudi")
      ...
      .mode(SaveMode.Overwrite)
      .save(basePath)
    val snapshotDF1 = spark.read.format("org.apache.hudi").option(HoodieMetadataConfig.ENABLE.key(), "true").load(basePath)
  ...
  def testWriteSmallPrecisionDecimalTable(): Unit = {
    ...
    inputDF2.write.format("org.apache.hudi")
      ...
      .mode(SaveMode.Append)
      .save(basePath)
    val readResult = spark.read.format("hudi").option(HoodieMetadataConfig.ENABLE.key(), "true").load(basePath)
  ...
  @Test
  def testPartitionColumnsProperHandling(): Unit = {
    val _spark = spark
    import _spark.implicits._
    ...
    val firstDF = spark.read.format("hudi").option(HoodieMetadataConfig.ENABLE.key(), "true").load(path)

    assert(firstDF.count() == 2)

    // data_date is the partition field. Persist to the parquet file using the origin values, and read it.
    // TODO(HUDI-3204) we have to revert this to pre-existing behavior from 0.10
    val expectedValues = Seq("2018/09/23", "2018/09/24")
  ...
  def testSaveAsTableInDifferentModes(): Unit = {
    ...
    metaClient = HoodieTableMetaClient.builder()
      ...
      .setConf(spark.sessionState.newHadoopConf)
      .build()
    assertEquals(spark.read.format("hudi").option(HoodieMetadataConfig.ENABLE.key(), "true").load(basePath).count(), 5)
    ...
    inputDF2.write.format("org.apache.hudi")
      ...
      .mode(SaveMode.Append)
      .saveAsTable("hoodie_test")
    assertEquals(spark.read.format("hudi").option(HoodieMetadataConfig.ENABLE.key(), "true").load(basePath).count(), 11)
    ...
    inputDF5.write.format("org.apache.hudi")
      ...
      .mode(SaveMode.Overwrite)
      .saveAsTable("hoodie_test")
    assertEquals(spark.read.format("hudi").option(HoodieMetadataConfig.ENABLE.key(), "true").load(basePath).count(), 9)
  }
