...
class TestColumnStatsIndex extends ColumnStatIndexTestBase {
  ...
  def simulateFailureForLatestCommit(tableType: HoodieTableType, partitionCol: String) : Unit = {
    ...
    if (tableType == HoodieTableType.MERGE_ON_READ) {
      val dataFiles : Array[FileStatus] = if (StringUtils.isNullOrEmpty(partitionCol)) {
        metaClient.getFs.listStatus(new Path(metaClient.getBasePath))
      } else {
        metaClient.getFs.listStatus(new Path(metaClient.getBasePath, "9"))
      }
      val logFileFileStatus : FileStatus = dataFiles.toStream.filter(fileStatus => fileStatus.getPath.toString.contains(".log")).last
      logFileName = logFileFileStatus.getPath.getName
    } else {
      val dataFiles = if (StringUtils.isNullOrEmpty(partitionCol)) {
        metaClient.getFs.listStatus(new Path(metaClient.getBasePath.toString))
      } else {
        metaClient.getFs.listStatus(new Path(metaClient.getBasePath,  "9"))
      }
      val baseFileFileStatus : FileStatus = dataFiles.toStream.filter(fileStatus => fileStatus.getPath.getName.contains(lastCompletedCommit.getTimestamp)).last
      baseFileName = baseFileFileStatus.getPath.getName
    }

    val latestCompletedFileName = lastCompletedCommit.getFileName
    metaClient.getFs.delete(new Path(metaClient.getBasePath.toString + "/.hoodie/" + latestCompletedFileName))

    // re-create marker for the deleted file.
    if (tableType == HoodieTableType.MERGE_ON_READ) {
      if (StringUtils.isNullOrEmpty(partitionCol)) {
        metaClient.getFs.create(new Path(metaClient.getBasePath.toString + "/.hoodie/.temp/" + lastCompletedCommit.getTimestamp + "/" + logFileName + ".marker.APPEND"))
      } else {
        metaClient.getFs.create(new Path(metaClient.getBasePath.toString + "/.hoodie/.temp/" + lastCompletedCommit.getTimestamp + "/9/" + logFileName + ".marker.APPEND"))
      }
    } else {
      if (StringUtils.isNullOrEmpty(partitionCol)) {
        metaClient.getFs.create(new Path(metaClient.getBasePath.toString + "/.hoodie/.temp/" + lastCompletedCommit.getTimestamp + "/" + baseFileName + ".marker.MERGE"))
      } else {
        metaClient.getFs.create(new Path(metaClient.getBasePath.toString + "/.hoodie/.temp/" + lastCompletedCommit.getTimestamp + "/9/" + baseFileName + ".marker.MERGE"))
      }
    }
  }
