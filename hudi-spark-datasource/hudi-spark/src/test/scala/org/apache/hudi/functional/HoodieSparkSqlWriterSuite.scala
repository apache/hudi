...
import org.apache.hudi.common.model.{HoodieRecord, HoodieRecordPayload, WriteOperationType}
...
class HoodieSparkSqlWriterSuite extends FunSuite with Matchers {
  ...
  test("test bulk insert dataset without index") {
    initSparkContext("test_bulk_insert_datasource_non_record_key")
    val path = java.nio.file.Files.createTempDirectory("hoodie_test_path")
    try {
      ...
      val fooTableModifier = Map("path" -> path.toAbsolutePath.toString,
        ...
        HoodieWriteConfig.AVRO_SCHEMA_STRING.key() -> schema.toString,
        ...
        HoodieWriteConfig.KEYGENERATOR_CLASS_NAME.key() -> "org.apache.hudi.keygen.SimpleKeyGenerator",
        HoodieIndexConfig.INDEX_TYPE.key() -> HoodieIndex.IndexType.NON_INDEX.toString
      )
      val fooTableParams = HoodieWriterUtils.parametersWithWriteDefaults(fooTableModifier)
      ...
      for (_ <- 0 to 0) {
        ...
        val actualDf = sqlContext.read.format("org.apache.hudi").load(path.toString + "/*/*/*")

        val updateDf = df.limit(150).select("_row_key")

        HoodieSparkSqlWriter.write(sqlContext, SaveMode.Append, fooTableParams ++ Seq(
          DataSourceWriteOptions.OPERATION.key() -> WriteOperationType.UPDATE_PREPPED.value(),
          HoodieWriteConfig.UPDATE_JOIN_FIELDS -> "_row_key",
          HoodieWriteConfig.UPDATE_NULL_FIELDS -> "_row_key,_hoodie_record_key",
          HoodieWriteConfig.UPDATE_PARTITION_PATHS -> "2015",
          HoodieWriteConfig.AVRO_SCHEMA_STRING.key() -> schema.toString,
          HoodieWriteConfig.UPDATE_PARTITION_PATHS -> "2015/03/16,2015/03/17,2016/03/15"), updateDf)

        val resultDF = sqlContext.read.format("org.apache.hudi").load(path.toString + "/*/*/*")
        assert(resultDF.where("_row_key IS NULL").count() == 150)

        // remove metadata columns so that expected and actual DFs can be compared as is
        val trimmedDf = actualDf.drop(HoodieRecord.HOODIE_META_COLUMNS.get(0)).drop(HoodieRecord.HOODIE_META_COLUMNS.get(1))
          .drop(HoodieRecord.HOODIE_META_COLUMNS.get(2)).drop(HoodieRecord.HOODIE_META_COLUMNS.get(3))
          .drop(HoodieRecord.HOODIE_META_COLUMNS.get(4))
          .join(updateDf, Seq("_row_key"), "outer")

        val trimmedResultDF = resultDF.drop(HoodieRecord.HOODIE_META_COLUMNS.get(0)).drop(HoodieRecord.HOODIE_META_COLUMNS.get(1))
          .drop(HoodieRecord.HOODIE_META_COLUMNS.get(2)).drop(HoodieRecord.HOODIE_META_COLUMNS.get(3))
          .drop(HoodieRecord.HOODIE_META_COLUMNS.get(4))
          .where("_row_key IS NOT NULL")

        assert(trimmedResultDF.except(trimmedDf).count() == 0)
      }
    } finally {
      spark.stop()
      FileUtils.deleteDirectory(path.toFile)
