...
//  @ValueSource(booleans = Array(true, false))
  ...
  def testPayloadClassUpdate(): Unit = {
    ...
    assert(e.getMessage.contains(DataSourceWriteOptions.PAYLOAD_CLASS_NAME.key))
  }
  ...
  def testMergeModeUpdate(): Unit = {
    ...
    assert(e.getMessage.contains("Config conflict(key"))
    assert(e.getMessage.contains(DataSourceWriteOptions.RECORD_MERGE_MODE.key))
  }
  ...
  def testMergeStrategyIdUpdate(): Unit = {
    ...
    assert(e.getMessage.contains("Config conflict(key"))
    assert(e.getMessage.contains(DataSourceWriteOptions.RECORD_MERGE_STRATEGY_ID.key))
  }
