...
class TestMORDataSource extends HoodieSparkClientTestBase with SparkDatasetMixin {
  ...
  def testReadPathsForMergeOnReadTable(recordType: HoodieRecordType): Unit = {
    ...
    inputDF1.write.format("org.apache.hudi")
      ...
      .save(basePath)
    assertTrue(HoodieDataSourceHelpers.hasNewCommits(storage, basePath, "000"))
    val baseFilePath = storage.listDirectEntries(new HoodieLocation(basePath, dataGen.getPartitionPaths.head))
      .asScala
    ...
    val logFilePath = storage.listDirectEntries(new HoodieLocation(basePath, dataGen.getPartitionPaths.head))
      .asScala
      .filter(_.getLocation.getName.contains("log"))
      .map(_.getLocation.toString)
      .mkString(",")
