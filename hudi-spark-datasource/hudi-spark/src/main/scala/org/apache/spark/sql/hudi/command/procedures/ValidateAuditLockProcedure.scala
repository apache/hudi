...
class ValidateAuditLockProcedure extends BaseProcedure with ProcedureBuilder {
      ...
      allFilesResult match {
        ...
        case Some(allFiles) =>
          ...
          if (auditFiles.isEmpty) {
            ...
            val parseResults = auditFiles.map(fileStatus => (fileStatus, parseAuditFile(fileStatus, fs)))
   ...
   */
  private def parseAuditFile(fileStatus: FileStatus, fs: org.apache.hadoop.fs.FileSystem): Option[TransactionWindow] = {
