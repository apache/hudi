...
class RepairMigratePartitionMetaProcedure extends BaseProcedure with ProcedureBuilder with Logging {
  ...
  override def call(args: ProcedureArgs): Seq[Row] = {
    ...
    for (partitionPath <- partitionPaths) {
      ...
      val textFormatFile: Option[StoragePath] = HoodiePartitionMetadata.textFormatMetaLocationIfExists(
        metaClient.getHoodieStorage, partition)
      val baseFormatFile: Option[StoragePath] = HoodiePartitionMetadata.baseFormatMetaLocationIfExists(
        metaClient.getHoodieStorage, partition)
    ...
    HoodieTableConfig.update(metaClient.getHoodieStorage, new StoragePath(metaClient.getMetaPath), props)
