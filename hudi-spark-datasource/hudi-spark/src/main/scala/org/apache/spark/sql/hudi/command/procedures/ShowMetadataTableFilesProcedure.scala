...
class ShowMetadataTableFilesProcedure() extends BaseProcedure with ProcedureBuilder with Logging {
  ...
  override def call(args: ProcedureArgs): Seq[Row] = {
    ...
    var partitionPath = new HoodieLocation(basePath)
    ...
    statuses.asScala.sortBy(p => p.getLocation.getName).foreach((f: HoodieFileStatus) => {
      rows.add(Row(f.getLocation.getName))
    })
    if (limit.isDefined) {
