...
import org.apache.hudi.io.storage.{HoodieAvroParquetWriter, HoodieLocation, HoodieParquetConfig, HoodieStorage}
...
object SparkHelpers {
  ...
  def skipKeysAndWriteNewFile(instantTime: String,
                              ...
                              sourceFile: HoodieLocation,
                              destinationFile: HoodieLocation,
...
class SparkHelper(sqlContext: SQLContext, fs: FileSystem) {
  ...
  def fileKeysAgainstBF(conf: Configuration, sqlContext: SQLContext, file: String): Boolean = {
    val bf = BaseFileUtils.getInstance(HoodieFileFormat.PARQUET).readBloomFilterFromMetadata(conf, new HoodieLocation(file))
