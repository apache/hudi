...
class ValidateMetadataTableFilesProcedure() extends BaseProcedure with ProcedureBuilder with Logging {
  ...
  override def call(args: ProcedureArgs): Seq[Row] = {
    ...
    for (partition <- allPartitions) {
      val fileStatusMap = new util.HashMap[String, StoragePathInfo]
      val metadataFileStatusMap = new util.HashMap[String, StoragePathInfo]
      val metadataStatuses = metadataReader.getAllFilesInPartition(new StoragePath(basePath, partition))
      metadataStatuses.asScala.foreach((entry: StoragePathInfo) => metadataFileStatusMap.put(entry.getPath.getName, entry))
      val fsStatuses = fsMetaReader.getAllFilesInPartition(new StoragePath(basePath, partition))
      fsStatuses.asScala.foreach((entry: StoragePathInfo) => fileStatusMap.put(entry.getPath.getName, entry))
