...
import org.apache.hudi.{common, io, AvroConversionUtils, ColumnStatsIndexSupport}
...
class ShowMetadataTableColumnStatsProcedure extends BaseProcedure with ProcedureBuilder with Logging {
  ...
  def buildFileSystemView(table: Option[Any]): HoodieTableFileSystemView = {
    ...
    new HoodieTableFileSystemView(metaClient, filteredTimeline, new java.util.ArrayList[HoodieFileStatus])
  }
