...
import org.apache.hudi.{AvroConversionUtils, DataSourceReadOptions, DataSourceWriteOptions, HoodieSparkSqlWriter, SparkAdapterSupport, SparkExpressionMerger}
...
case class MergeIntoHoodieTableCommand(mergeInto: MergeIntoTable,
   ...
   */
  private lazy val preCombineAttributeAssociatedExpressions: Seq[(Attribute, Expression)] =
  ...
  private def buildMergeIntoConfig(hoodieCatalogTable: HoodieCatalogTable): Map[String, String] = {
    ...
    val overridingOpts = Map(
      ...
      RECORD_MERGE_IMPL_CLASSES.key -> classOf[SparkExpressionMerger].getName,
