        ...
        mergeInto.sourceTable.output.find(attr => resolver(attr.name, preCombineField)) match {
          ...
          case None =>
            updatingActions.flatMap(_.assignments).collectFirst {
              ...
              throw new HoodieAnalysisException(s"Failed to resolve precombine field `${preCombineField}` w/in the source-table output")
            }
  ...
  def validateTargetTableAttrExistsInAssignments(resolver: Resolver,
    ...
    fields.foreach { field =>
      targetTable.output
        ...
        .getOrElse(throw new HoodieAnalysisException(s"Failed to resolve $fieldType `$field` in target table"))

      if (!assignments.exists {
        case Assignment(attr: AttributeReference, _) if resolver(attr.name, field) => true
        case _ => false
      }) {
        throw new HoodieAnalysisException(s"No matching assignment found for target table $fieldType `$field`")
      }
    }
  }
}
