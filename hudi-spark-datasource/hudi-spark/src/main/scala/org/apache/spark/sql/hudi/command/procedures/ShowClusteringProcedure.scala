 ...
 * - `show_archived`: Optional. Whether to include archived clustering operations (default: false)
 * - `filter`: Optional. SQL expression to filter results (default: empty string)
 * - `start_time`: Optional. Start time for clustering (format: yyyyMMddHHmmss, default: empty)
 * - `end_time`: Optional. End time for clustering (format: yyyyMMddHHmmss, default: empty)
 *
 * == Output Schema ==
