...
/**
  * Class to be used in quickstart guide for generating inserts and updates against a corpus. Test
  * data uses a toy Uber trips, data model.
  */
public class QuickstartUtils {

    public static class DataGenerator {
        private static final String DEFAULT_FIRST_PARTITION_PATH =
        ...
        static String TRIP_EXAMPLE_SCHEMA =
        ...
        /**
          ...
          * provided. The riderDriverSuffix string is a random String to simulate updates by changing the
          * rider driver fields for records belonging to the same commit. It is purely used for demo
          * purposes. In real world, the actual updates are assumed to be provided based on the
          * application requirements.
          */
        public static OverwriteWithLatestAvroPayload generateRandomValue(
                HoodieKey key, String riderDriverSuffix) throws IOException {
            // The timestamp generated is limited to range from 7 days before to now, to avoid
            // generating too many
            // partitionPaths when user use timestamp as partitionPath filed.
            GenericRecord rec =
                    generateGenericRecord(
                            key.getRecordKey(),
                            "rider-" + riderDriverSuffix,
                            "driver-" + riderDriverSuffix,
                            generateRangeRandomTimestamp(7));
        ...
        public HoodieRecord generateUpdateRecord(HoodieKey key, String randomString)
                throws IOException {
            return new HoodieAvroRecord(key, generateRandomValue(key, randomString));
        }
                            ...
                            x -> {
                                try {
                                    return generateUpdateRecord(
                                            existingKeys.get(rand.nextInt(numExistingKeys)), randomString);
                                } catch (IOException e) {
                                    throw new HoodieIOException(e.getMessage(), e);
                                }
                            })
        ...
        public List<HoodieRecord> generateUniqueUpdates(Integer n) {
            ...
            List<Integer> keys =
                    IntStream.range(0, numExistingKeys)
                            .boxed()
        ...
        public List<String> generateDeletes(List<Row> rows) {
            // if row.length() == 2, then the record contains "uuid" and "partitionpath" fields,
            // otherwise,
                    ...
                    .map(
                            row ->
                                    row.length() == 2
                                            ...
                                            : convertToString(
                                                    row.getAs("uuid"), row.getAs("partitionpath"), row.getAs("ts")))
    ...
    private static Option<String> convertToString(HoodieRecord record) {
        try {
            String str =
                    ((OverwriteWithLatestAvroPayload) record.getData())
