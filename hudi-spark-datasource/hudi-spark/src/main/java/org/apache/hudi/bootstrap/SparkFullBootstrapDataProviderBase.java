...
public abstract class SparkFullBootstrapDataProviderBase
    ...
    public SparkFullBootstrapDataProviderBase(
            TypedProperties props, HoodieSparkEngineContext context) {
        super(props, context);
        this.sparkSession =
    ...
    public JavaRDD<HoodieRecord> generateInputRecords(
        ...
        String[] filePaths =
                partitionPathsWithFiles.stream()
                        .map(Pair::getValue)
        ...
        Dataset inputDataset =
                sparkSession.read().format(getFormat()).option("basePath", sourceBasePath).load(filePaths);
        ...
        if (recordType == HoodieRecordType.AVRO) {
            RDD<GenericRecord> genericRecords =
                    HoodieSparkUtils.createRdd(inputDataset, structName, namespace, false, Option.empty());
            ...
            return inputDataset
                    ...
                    .map(
                            internalRow -> {
                                String recordKey =
                                        sparkKeyGenerator.getRecordKey(internalRow, structType).toString();
