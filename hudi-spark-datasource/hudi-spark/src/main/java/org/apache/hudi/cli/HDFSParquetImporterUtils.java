...
public class HDFSParquetImporterUtils implements Serializable {
    ...
    public int dataImport(JavaSparkContext jsc) {
        ...
        this.props =
                this.propsFilePath == null || this.propsFilePath.isEmpty()
                        ? buildProperties(this.configs)
                        : readConfig(fs.getConf(), new StoragePath(this.propsFilePath), this.configs)
                                .getProps(true);
    ...
    public int dataImport(JavaSparkContext jsc, FileSystem fs) {
        try {
            ...
            SparkRDDWriteClient<HoodieRecordPayload> client =
                    createHoodieClient(
                            jsc, this.targetPath, schemaStr, this.parallelism, Option.empty(), props);
    ...
    public JavaRDD<HoodieRecord<HoodieRecordPayload>> buildHoodieRecordsForImport(
        ...
        AvroReadSupport.setAvroReadSchema(
                jsc.hadoopConfiguration(), (new Schema.Parser().parse(schemaStr)));
        ParquetInputFormat.setReadSupportClass(job, (AvroReadSupport.class));

        HoodieEngineContext context = new HoodieSparkEngineContext(jsc);
        context.setJobStatus(
                this.getClass().getSimpleName(), "Build records for import: " + this.tableName);
                        ...
                        entry -> {
                            ...
                            if (partitionField instanceof Number) {
                                try {
                                    ...
                                    LOG.warn(
                                            "Unable to parse date from partition field. Assuming partition as ("
                                                    + partitionField
                                                    + ")");
                                }
                            }
    ...
    /**
      ...
      * @param <T> Type
      */
    public <T extends HoodieRecordPayload> JavaRDD<WriteStatus> load(
            SparkRDDWriteClient<T> client, String instantTime, JavaRDD<HoodieRecord<T>> hoodieRecords) {
    ...
    public static DFSPropertiesConfiguration readConfig(
            Configuration hadoopConfig, StoragePath cfgPath, List<String> overriddenProps) {
