...
/**
  * Performs bootstrap from a non-hudi source. import static
  * org.apache.hudi.common.util.ConfigUtils.filterProperties;
  */
public class BootstrapExecutorUtils implements Serializable {
    ...
    public BootstrapExecutorUtils(
        ...
        ValidationUtils.checkArgument(
                properties.containsKey(HoodieTableConfig.BOOTSTRAP_BASE_PATH.key()),
    ...
    public void execute() throws IOException {
        ...
        try (SparkRDDWriteClient bootstrapClient =
                new SparkRDDWriteClient(new HoodieSparkEngineContext(jssc), bootstrapConfig)) {
    ...
    private void syncHive() {
        if (cfg.enableHiveSync) {
            ...
            if (props.getBoolean(HIVE_SYNC_BUCKET_SYNC.key(), HIVE_SYNC_BUCKET_SYNC.defaultValue())) {
                metaProps.put(
                        HIVE_SYNC_BUCKET_SYNC_SPEC.key(),
                        HiveSyncConfig.getBucketSpec(
                                props.getString(BUCKET_INDEX_HASH_FIELD.key()),
    ...
    private void initializeTable() throws IOException {
        ...
        if (fs.exists(basePath)) {
            if (cfg.bootstrapOverwrite) {
                ...
                throw new HoodieException(
                        "target base path already exists at "
                                + cfg.basePath
        ...
        HoodieTableMetaClient.PropertyBuilder builder =
                HoodieTableMetaClient.withPropertyBuilder()
                        ...
                        .setPopulateMetaFields(
                                props.getBoolean(POPULATE_META_FIELDS.key(), POPULATE_META_FIELDS.defaultValue()))
                        .setArchiveLogFolder(
                                props.getString(ARCHIVELOG_FOLDER.key(), ARCHIVELOG_FOLDER.defaultValue()))
                        ...
                        .setCommitTimezone(
                                HoodieTimelineTimeZone.valueOf(
                                        props.getString(
                                                TIMELINE_TIMEZONE.key(), String.valueOf(TIMELINE_TIMEZONE.defaultValue()))))
        ...
        builder.initTable(
                HadoopFSUtils.getStorageConfWithCopy(jssc.hadoopConfiguration()), cfg.basePath);
    }

    private Pair<String, String> genKeyGenClassAndPartitionColumns() {
        String keyGenClass;
        if (StringUtils.nonEmpty(
    ...
    private Map<String, Object> extractConfigsRelatedToTimestampBasedKeyGenerator(
            String keyGenerator, TypedProperties params) {
