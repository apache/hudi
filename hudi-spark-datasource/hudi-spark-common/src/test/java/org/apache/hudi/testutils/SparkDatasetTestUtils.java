...
/**
  * Dataset test utils. Note: This util class can be only used within `hudi-spark<spark_version>`
  * modules because it relies on SparkAdapterSupport to get encoder for different versions of Spark.
  * If used elsewhere this class won't be initialized properly amd could cause ClassNotFoundException
...
public class SparkDatasetTestUtils {
    ...
    public static final StructType STRUCT_TYPE =
            new StructType(
                    new StructField[] {
                        ...
                        new StructField(
                                PARTITION_PATH_FIELD_NAME, DataTypes.StringType, false, Metadata.empty()),
    ...
    public static final StructType ERROR_STRUCT_TYPE =
            new StructType(
                    new StructField[] {
                        new StructField(
                                HoodieRecord.COMMIT_TIME_METADATA_FIELD,
    ...
    /**
      ...
      */
    public static Dataset<Row> getRandomRows(
            SQLContext sqlContext, int count, String partitionPath, boolean isError) {
    ...
    public static Dataset<Row> getRandomRowsWithCommitTime(
            SQLContext sqlContext, int count, String partitionPath, boolean isError, String commitTime) {
        List<Row> records = new ArrayList<>();
        for (long recordNum = 0; recordNum < count; recordNum++) {
    ...
    public static Dataset<Row> getRandomRowsWithKeys(
            SQLContext sqlContext, List<HoodieKey> keys, boolean isError, String commitTime) {
        List<Row> records = new ArrayList<>();
    ...
    public static HoodieWriteConfig.Builder getConfigBuilder(
            String basePath, int timelineServicePort) {
