...
public class DatasetBulkInsertCommitActionExecutor
    ...
    public DatasetBulkInsertCommitActionExecutor(
    ...
    protected Option<HoodieData<WriteStatus>> doExecute(
        ...
        Map<String, String> opts =
                writeConfig.getProps().entrySet().stream()
                        .collect(
                                Collectors.toMap(
                                        e -> String.valueOf(e.getKey()), e -> String.valueOf(e.getValue())));
        Map<String, String> optsOverrides =
        ...
        if (HoodieSparkUtils.isSpark2()) {
            ...
            customOpts.put(
                    HoodieInternalConfig.BULKINSERT_INPUT_DATA_SCHEMA_DDL.key(), records.schema().json());
        } else {
            throw new HoodieException(
                    "Bulk insert using row writer is not supported with current Spark version."
