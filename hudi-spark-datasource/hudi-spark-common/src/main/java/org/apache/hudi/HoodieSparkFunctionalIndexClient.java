...
public class HoodieSparkFunctionalIndexClient extends BaseHoodieFunctionalIndexClient {
    ...
    public void create(
        ...
        checkArgument(
                columns.size() == 1, "Only one column can be indexed for functional or secondary index.");
        ...
        try (SparkRDDWriteClient writeClient =
                HoodieCLIUtils.createHoodieWriteClient(
                        ...
                        mapAsScalaImmutableMap(buildWriteConfig(metaClient, functionalIndexDefinition)),
            ...
            if (indexInstantTime.isPresent()) {
                ...
                throw new HoodieFunctionalIndexException(
                        "Scheduling of index action did not return any instant.");
            }
        }
    }

    private static Option<String> doSchedule(
            SparkRDDWriteClient<HoodieRecordPayload> client,
            HoodieTableMetaClient metaClient,
            String indexName) {
        List<MetadataPartitionType> partitionTypes =
                Collections.singletonList(MetadataPartitionType.FUNCTIONAL_INDEX);
        checkArgument(
    ...
    private static boolean indexExists(HoodieTableMetaClient metaClient, String indexName) {
        return metaClient.getTableConfig().getMetadataPartitions().stream()
                .anyMatch(partition -> partition.equals(indexName));
    }

    private static Map<String, String> buildWriteConfig(
        ...
        if (metaClient.getTableConfig().isMetadataTableAvailable()) {
            ...
            writeConfig.putAll(
                    JavaConverters.mapAsJavaMapConverter(
                                    HoodieCLIUtils.getLockOptions(metaClient.getBasePath().toString()))
                            .asJava());

            // [HUDI-7472] Ensure write-config contains the existing MDT partition to prevent those
        ...
        HoodieIndexingConfig.fromIndexDefinition(indexDefinition)
                .getProps()
