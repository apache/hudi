...
public class LSMDatasetBulkInsertOverwriteTableCommitActionExecutor extends LSMDatasetBulkInsertOverwriteCommitActionExecutor {
  public  LSMDatasetBulkInsertOverwriteTableCommitActionExecutor(HoodieWriteConfig config,
  ...
  protected Map<String, List<String>> getPartitionToReplacedFileIds(HoodieData<WriteStatus> writeStatuses) {
    ...
    if (canTakeFirstPartition(writeConfig, partitionPaths)) {
      String partition = writeStatuses.first().getStat().getPartitionPath();
      return Collections.singletonMap(partition, getAllExistingFileIds(partition));
    } else {
