...
public class DataSourceUtils {
  ...
  public static String getTablePath(HoodieStorage storage,
                                    List<HoodieLocation> userProvidedPaths) throws IOException {
    LOG.info("Getting table path..");
    for (HoodieLocation path : userProvidedPaths) {
      try {
        Option<HoodieLocation> tablePath = TablePathUtils.getTablePath(storage, path);
    ...
    throw new TableNotFoundException(userProvidedPaths.stream()
        .map(HoodieLocation::toString).collect(Collectors.joining(",")));
  }
