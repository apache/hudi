...
import org.apache.spark.sql.execution.datasources.{FileGroupReaderBasedFileFormat, PartitionedFile}
import org.apache.spark.sql.execution.datasources.FileGroupReaderBasedFileFormat.PARQUET_FILE_EXTENSION
...
class FileGroupReaderBasedParquetFileFormat(tablePath: String,
  ...
  extends FileGroupReaderBasedFileFormat(
    tablePath,
    tableSchema,
    tableName,
    queryTimestamp,
  ...
  override protected def setSchemaEvolutionConfigs(conf: StorageConfiguration[Configuration]): Unit = {
  ...
  override protected def buildCDCRecordIterator(hoodiePartitionCDCFileGroupSliceMapping: HoodiePartitionCDCFileGroupMapping,
  ...
  override protected def appendPartitionAndProject(iter: HoodieFileGroupReader.HoodieFileGroupReaderIterator[InternalRow],
  ...
  override def readBaseFile(file: PartitionedFile, parquetFileReader: SparkFileReader, requestedSchema: StructType,
