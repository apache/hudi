...
case class ShowHoodieCreateTableCommand(table: TableIdentifier)
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    // get hive table.
    val hiveCatalogTable = client.getTable(dbName, tableName)
    val stmt = showCreateHoodieTable(hoodieCatalogTable, hiveCatalogTable)
    Seq(Row(stmt))
  }

  private def showCreateHoodieTable(metadata: HoodieCatalogTable, hiveMetadata: CatalogTable): String = {
    val builder = StringBuilder.newBuilder
    builder ++= s"CREATE TABLE IF NOT EXISTS ${table.quotedString}"
    showHoodieTableHeader(metadata, hiveMetadata, builder)
    showHoodieTableNonDataColumns(metadata, builder)
    showHoodieTableProperties(metadata, builder)
    builder.toString()
  }

  private def showHoodieTableHeader(metadata: HoodieCatalogTable, hiveMetadata: CatalogTable, builder: StringBuilder): Unit = {
    val columns = HoodieSqlCommonUtils.removeMetaFields(hiveMetadata.schema).map(_.toDDL)
    if (columns.nonEmpty) {
  ...
  private def showHoodieTableProperties(metadata: HoodieCatalogTable, builder: StringBuilder): Unit = {
    ...
    } ++ metadata.catalogProperties.filter(_._1.startsWith("hoodie.")).filterNot(_._1.equals(ConfigUtils.IS_QUERY_AS_RO_TABLE)).map {
