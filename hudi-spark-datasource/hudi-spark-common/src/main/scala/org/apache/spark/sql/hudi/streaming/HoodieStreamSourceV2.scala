  ...
  override def getBatch(start: Option[Offset], end: Offset): DataFrame = {
    ...
    if (startOffset == endOffset) {
      ...
      if (isCDCQuery) {
        ...
        if (useNewParquetFileFormat) {
          val relation = if (tableType == HoodieTableType.COPY_ON_WRITE) {
            new HoodieCopyOnWriteIncrementalHadoopFsRelationFactory(sqlContext, metaClient, incParams, Option(schema), false, rangeType)
              .build()
          } else {
            new HoodieMergeOnReadIncrementalHadoopFsRelationFactory(sqlContext, metaClient, incParams, Option(schema), false, rangeType)
              .build()
          }
