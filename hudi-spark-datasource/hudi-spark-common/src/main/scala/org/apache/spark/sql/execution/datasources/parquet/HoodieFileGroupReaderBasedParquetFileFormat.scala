 ...
 */
  ...
  override def buildReaderWithPartitionValues(spark: SparkSession,
    ...
    val reader = sparkAdapter.createHoodieParquetFileReader(supportBatchResult, spark.sessionState.conf, options, augmentedHadoopConf)
    val parquetFileReader = spark.sparkContext.broadcast(reader)
