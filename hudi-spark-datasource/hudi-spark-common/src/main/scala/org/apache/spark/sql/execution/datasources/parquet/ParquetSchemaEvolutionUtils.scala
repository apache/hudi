...
import org.apache.spark.sql.execution.datasources.parquet.ParquetSchemaEvolutionUtils.pruneEvolutionSchema
...
class ParquetSchemaEvolutionUtils(sharedConf: Configuration,
                                  ...
                                  evolutionSchemaOpt: util.Option[HoodieSchema]) extends SparkAdapterSupport {
  // Fetch evolution schema
  private lazy val querySchemaOption: util.Option[HoodieSchema] = pruneEvolutionSchema(evolutionSchemaOpt, requiredSchema)
  ...
  private def rebuildFilterFromParquetHelper(oldFilter: Filter, fileSchema: HoodieSchema, querySchema: HoodieSchema): Filter = {
  ...
  def getHadoopConfClone(footerFileMetaData: FileMetaData, enableVectorizedReader: Boolean): Configuration = {
    ...
    typeChangeInfos = if (shouldUseInternalSchema) {
      val mergedSchema = new HoodieSchemaMerger(fileSchema, querySchemaOption.get(), true, true).mergeSchema()
      val mergedStructType = HoodieSchemaConversionUtils.convertHoodieSchemaToStructType(mergedSchema)
