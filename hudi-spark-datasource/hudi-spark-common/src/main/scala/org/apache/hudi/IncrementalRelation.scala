...
class IncrementalRelation(val sqlContext: SQLContext,
  ...
  override def buildScan(): RDD[Row] = {
    if (usedSchema == StructType(Nil)) {
      ...
      for (commit <- commitsToReturn) {
        val metadata: HoodieCommitMetadata = metaClient.getTimelineLayout.getCommitMetadataSerDe.deserialize(commit,
        ...
        if (filteredRegularFullPaths.isEmpty && filteredMetaBootstrapFullPaths.isEmpty) {
          ...
          if (doFullTableScan) {
            ...
            if (regularFileIdToFullPath.nonEmpty) {
              try {
                val commitTimesToReturn = commitsToReturn.map(_.getRequestTime)
  ...
  private def fullTableScanDataFrame(commitsToFilter: List[HoodieInstant]): DataFrame = {
    val commitTimesToFilter = commitsToFilter.map(_.getRequestTime)
