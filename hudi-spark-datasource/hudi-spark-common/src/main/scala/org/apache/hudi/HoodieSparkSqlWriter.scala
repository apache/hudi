    ...
    if (mode == SaveMode.Ignore && tableExists) {
      ...
      if (!tableExists) {
        ...
        val tableMetaClient = HoodieTableMetaClient.withPropertyBuilder()
          ...
          .setCombineEngineClassName(hoodieConfig.getString(COMBINE_ENGINE_CLASS_NAME))
    ...
    if (mode == SaveMode.Ignore && tableExists) {
      ...
      if (!tableExists) {
        ...
        HoodieTableMetaClient.withPropertyBuilder()
          ...
          .setCombineEngineClassName(hoodieConfig.getStringOrDefault(COMBINE_ENGINE_CLASS_NAME))
      ...
      case HoodieRecord.HoodieRecordType.AVRO =>
        ...
        genericRecords.map(gr => {
          ...
          val hoodieRecord = if (shouldCombine) {
            ...
            DataSourceUtils.createHoodieRecord(processedRecord, keyGenerator.getKey(gr), config.getString(PAYLOAD_CLASS_NAME))
          }
          hoodieRecord
        }).toJavaRDD()
