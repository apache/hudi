...
class HoodieSparkSqlWriterInternal {
  ...
  private def mergeParamsAndGetHoodieConfig(optParams: Map[String, String],
    ...
    if (!mergedParams.contains(DataSourceWriteOptions.RECORD_MERGE_MODE.key())
      ...
      mergedParams.put(HoodieTableConfig.RECORD_MERGER_STRATEGY_ID.key(), inferredMergeConfigs.getRight)
    } else {
      ...
      mergedParams.put(HoodieTableConfig.RECORD_MERGER_STRATEGY_ID.key(), mergedParams(HoodieWriteConfig.RECORD_MERGER_STRATEGY_ID.key()))
    }
