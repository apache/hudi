...
case class HoodieFileIndex(spark: SparkSession,
  ...
  private def allBaseFilesAndLogFiles: Seq[StoragePathInfo] = {
    getAllInputFileSlices.values.asScala.flatMap(_.asScala)
      .flatMap(fs => {
        ...
        val logFilesStatus = fs.getLogFiles.map[StoragePathInfo](JFunction.toJavaFunction[HoodieLogFile, StoragePathInfo](lf => lf.getFileInfo))
        ...
        if (shouldEmbedFileSlices) {
          ...
          val allCandidateFiles: Seq[FileStatus] = fileSlices.flatMap(fs => {
            ...
            val logFilesStatus = if (includeLogFiles) {
              fs.getLogFiles.map[StoragePathInfo](JFunction.toJavaFunction[HoodieLogFile, StoragePathInfo](lf => lf.getFileInfo))
            } else {
   ...
   */
  protected def getBaseFileStatus(baseFileOpt: Option[HoodieBaseFile]): Option[StoragePathInfo] = {
