...
import org.apache.hudi.DataSourceWriteOptions.{BOOTSTRAP_OPERATION_OPT_VAL, SPARK_SQL_WRITE_PREPPED_KEY, OPERATION, STREAMING_CHECKPOINT_IDENTIFIER}
...
class DefaultSource extends RelationProvider
  ...
  override def createRelation(sqlContext: SQLContext,
    ...
    val df = if (optParams.getOrDefault(SPARK_SQL_WRITE_PREPPED_KEY, "false").toBoolean || optParams.getOrDefault(SPARK_SQL_MERGE_INTO_PREPPED_KEY, "false").toBoolean) {
