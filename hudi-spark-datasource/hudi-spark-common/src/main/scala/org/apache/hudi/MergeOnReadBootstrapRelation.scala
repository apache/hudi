...
case class MergeOnReadBootstrapSplit(dataFile: PartitionedFile,
 ...
 */
case class MergeOnReadBootstrapRelation(override val sqlContext: SQLContext,
  ...
  override type FileSplit = MergeOnReadBootstrapSplit
  override type Relation = MergeOnReadBootstrapRelation
  ...
  protected override def collectFileSplits(partitionFilters: Seq[Expression], dataFilters: Seq[Expression]): Seq[FileSplit] = {
    ...
    val fileSlices = if (globPaths.isEmpty) {
      fileIndex.listFileSlices(convertedPartitionFilters).values.flatten.toSeq
    } else {
    ...
    fileSlices.map { fileSlice =>
      ...
      if (baseFile.getBootstrapBaseFile.isPresent) {
        ...
        MergeOnReadBootstrapSplit(dataFile, skeletonFile, logFiles)
      } else {
        ...
        MergeOnReadBootstrapSplit(dataFile,Option.empty,logFiles)
      }
    }
  }

  protected override def composeRDD(fileSplits: Seq[MergeOnReadBootstrapSplit],
    ...
    new HoodieMergeOnReadBootstrapRDD(
  ...
  private def createBootstrapFileReaders(tableSchema: HoodieTableSchema,
    ...
    MergeOnReadBootstrapRelation.validate(requiredDataSchema, requiredBootstrapDataFileSchema, requiredSkeletonFileSchema)
  ...
  override def updatePrunedDataSchema(prunedSchema: StructType): MergeOnReadBootstrapRelation =
...
object MergeOnReadBootstrapRelation {
