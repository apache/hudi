  ...
  override def compute(split: Partition, context: TaskContext): Iterator[InternalRow] = {
    ...
    val iter: Iterator[InternalRow] = partition.split match {
      ...
      case _ =>
        ...
        if (!properties.containsKey(HoodieReaderConfig.ENABLE_OPTIMIZED_LOG_BLOCKS_SCAN.key())) {
          properties.put(HoodieReaderConfig.ENABLE_OPTIMIZED_LOG_BLOCKS_SCAN.key(), HoodieReaderConfig.ENABLE_OPTIMIZED_LOG_BLOCKS_SCAN.defaultValue())
        }
