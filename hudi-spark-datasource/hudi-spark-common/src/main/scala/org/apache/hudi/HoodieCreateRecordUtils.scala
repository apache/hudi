...
import org.apache.spark.sql.{Column, DataFrame, Dataset, Encoders, HoodieInternalRowUtils, Row}
...
object HoodieCreateRecordUtils {
  ...
  private def getSimpleRecordKey(dataFrame: DataFrame, props: TypedProperties): Column = {
    props.getString(KeyGeneratorOptions.RECORDKEY_FIELD_NAME.key, null) match {
      case null => throw new HoodieKeyException(s"${KeyGeneratorOptions.RECORDKEY_FIELD_NAME.key} can not be null.")
      case _ => val recordKeyColumn = props.getString(KeyGeneratorOptions.RECORDKEY_FIELD_NAME.key)
        dataFrame(recordKeyColumn)
    }
  }

  private def getPartitionPath(dataFrame: DataFrame, props: TypedProperties): Column = {
    props.getString(KeyGeneratorOptions.PARTITIONPATH_FIELD_NAME.key, null) match {
      case null => throw new HoodieKeyException(s"${KeyGeneratorOptions.PARTITIONPATH_FIELD_NAME.key} can not be null.")
      case _ => val partitionColumn = props.getString(KeyGeneratorOptions.PARTITIONPATH_FIELD_NAME.key)
        dataFrame(partitionColumn)
    }
  }

  //  private def getInstantTime(dataFrame: DataFrame, props: TypedProperties): Column = {
  //    if (Option.ofNullable(props.getString(KeyGeneratorOptions.PARTITIONPATH_FIELD_NAME.key, null)).isPresent) {
  //      val partitionColumn = props.getString(KeyGeneratorOptions.PARTITIONPATH_FIELD_NAME.key)
  //      dataFrame(partitionColumn)
  //    } else {
  //      throw new HoodieKeyException(s"${KeyGeneratorOptions.PARTITIONPATH_FIELD_NAME.key} can not be null.")
  //    }
  //  }
  //
  //  private def getFileName(dataFrame: DataFrame, props: TypedProperties): Column = {
  //    if (Option.ofNullable(props.getString(KeyGeneratorOptions.PARTITIONPATH_FIELD_NAME.key, null)).isPresent) {
  //      val partitionColumn = props.getString(KeyGeneratorOptions.PARTITIONPATH_FIELD_NAME.key)
  //      dataFrame(partitionColumn)
  //    } else {
  ...
  //  }
  //
  private def getRecordLocation(dataFrame: DataFrame, props: TypedProperties): Column = {
    props.getString(KeyGeneratorOptions.PARTITIONPATH_FIELD_NAME.key, null) match {
      case null => throw new HoodieKeyException(s"${KeyGeneratorOptions.PARTITIONPATH_FIELD_NAME.key} can not be null.")
      case _ => val partitionColumn = props.getString(KeyGeneratorOptions.PARTITIONPATH_FIELD_NAME.key)
        dataFrame(partitionColumn)
  ...
  def createHoodieRecordDF[T](args: createHoodieRecordRddArgs) = {
    val df = args.df
    val config = args.config
    val parameters = args.parameters
    val recordName = args.recordName
    ...
    log.warn(s"Creating HoodieRecords (as $recordType)")

    recordType match {
        //      case HoodieRecord.HoodieRecordType.AVRO =>
        //        // avroRecords will contain meta fields when isPrepped is true.
        //        val avroRecords: Dataset[GenericRecord] = HoodieSparkUtils.createDataFrame(df, recordName, recordNameSpace,
        //          Some(writerSchema))
        //
        //        val dataFileSchema = new Schema.Parser().parse(dataFileSchemaStr)
        //        val keyGenProps = new TypedProperties(config.getProps)
        ...
        //
        //        if (autoGenerateRecordKeys) {
        //            keyGenProps.setProperty(KeyGenUtils.RECORD_KEY_GEN_PARTITION_ID_CONFIG, String.valueOf(TaskContext.getPartitionId()))
        //            keyGenProps.setProperty(KeyGenUtils.RECORD_KEY_GEN_INSTANT_TIME_CONFIG, instantTime)
        //        }
        //
        //        avroRecords.map(sourceRow => {
        //          val (hoodieKey: HoodieKey, recordLocation: Option[HoodieRecordLocation]) = HoodieCreateRecordUtils.getHoodieKeyAndMaybeLocationFromAvroRecord(keyGenerator, sourceRow,
        //            preppedSparkSqlWrites || preppedWriteOperation, preppedSparkSqlWrites || preppedWriteOperation || preppedSparkSqlMergeInto)
        //          val avroRecWithoutMeta: GenericRecord = if (preppedSparkSqlWrites || preppedSparkSqlMergeInto || preppedWriteOperation) {
        //            HoodieAvroUtils.rewriteRecord(sourceRow, HoodieAvroUtils.removeMetadataFields(dataFileSchema))
        //          } else {
        //            sourceRow
        //          }
        //
        ...
        //
        //          val hoodieRecord = if (shouldCombine) {
        //            val orderingVal = HoodieAvroUtils.getNestedFieldVal(sourceRow, config.getString(PRECOMBINE_FIELD),
        //              false, consistentLogicalTimestampEnabled).asInstanceOf[Comparable[_]]
        //            DataSourceUtils.createHoodieRecord(processedRecord, orderingVal, hoodieKey,
        //              config.getString(PAYLOAD_CLASS_NAME), recordLocation)
        //          } else {
        ...
        //          }
        //          hoodieRecord
        //        })(Encoders.bean(classOf[HoodieRecord[_]]))
      ...
      case HoodieRecord.HoodieRecordType.SPARK =>
        ...
        val finalStructType = if (preppedSparkSqlWrites || preppedWriteOperation) {
          val fieldsToExclude = HoodieRecord.HOODIE_META_COLUMNS_WITH_OPERATION.toArray()
          new StructType(targetStructType.fields.filterNot(field => fieldsToExclude.contains(field.name)))
        } else {
          targetStructType
        }
        ...
        df.withColumn("partition_id", spark_partition_id())
          .withColumn(HoodieMetadataField.RECORD_KEY_METADATA_FIELD.getFieldName, getSimpleRecordKey(df, keyGenProps))
          .withColumn(HoodieMetadataField.PARTITION_PATH_METADATA_FIELD.getFieldName, getPartitionPath(df, keyGenProps))
          .withColumn(HoodieMetadataField.FILENAME_METADATA_FIELD.getFieldName, getRecordLocation(df, keyGenProps))
          .map(sourceRow => {
            val key = sourceRow.getAs(HoodieMetadataField.RECORD_KEY_METADATA_FIELD.getFieldName).toString
            val partitionPath: String = sourceRow.getAs(
              HoodieMetadataField.PARTITION_PATH_METADATA_FIELD.getFieldName).toString

            val recordKey = new HoodieKey(key, partitionPath)
            val finalStructTypeRowWriter = getCachedUnsafeRowWriter(sourceStructType, finalStructType)
            val targetRow = finalStructTypeRowWriter(encoder.createSerializer().apply(sourceRow))

            val hoodieSparkRecord = new HoodieSparkRecord(recordKey, targetRow, dataFileStructType, false)
            hoodieSparkRecord
          })(Encoders.kryo)
    }
  }
