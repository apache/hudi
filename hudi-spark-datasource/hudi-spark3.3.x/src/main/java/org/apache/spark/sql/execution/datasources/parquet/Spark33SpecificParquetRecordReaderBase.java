...
public abstract class Spark33SpecificParquetRecordReaderBase<T> extends RecordReader<Void, T> {
  ...
  public void initialize(InputSplit inputSplit, TaskAttemptContext taskAttemptContext)
    ...
    ReadSupport<T> readSupport = (ReadSupport<T>) new Spark33ParquetReadSupport(readerType);
    ReadSupport.ReadContext readContext = readSupport.init(new InitContext(
        taskAttemptContext.getConfiguration(), toSetMultiMap(fileMetadata), fileSchema));
    this.requestedSchema = readContext.getRequestedSchema();
    fileReader.setRequestedSchema(requestedSchema);
    String sparkRequestedSchemaString = configuration.get(Spark33ParquetReadSupport.getSchemaConfig(readerType));
