...
/**
 * Spark 3.3 concrete implementation of [[Spark3LegacyHoodieParquetFileFormat]]. It only overrides
 * the version-specific hooks; the shared reader logic lives in the base class.
 */
class Spark33LegacyHoodieParquetFileFormat(appendPartitionValues: Boolean)
  ...
  override protected def isVectorizedReaderEnabled(sparkSession: SparkSession,
    ...
    ParquetUtils.isBatchReadSupportedForSchema(sparkSession.sessionState.conf, resultSchema)
