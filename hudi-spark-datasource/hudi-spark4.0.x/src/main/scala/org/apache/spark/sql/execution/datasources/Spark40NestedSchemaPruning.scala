...
import org.apache.spark.sql.HoodieSpark4CatalystPlanUtils
  ...
  private def prunePhysicalColumns(output: Seq[AttributeReference],
    ...
    if (requestedRootFields.exists { root: RootField => !root.derivedFromAtt }) {
      ...
      if (countLeaves(dataSchema) > countLeaves(prunedDataSchema)) {
        val planUtils = SparkAdapterSupport.sparkAdapter.getCatalystPlanUtils.asInstanceOf[HoodieSpark4CatalystPlanUtils]
