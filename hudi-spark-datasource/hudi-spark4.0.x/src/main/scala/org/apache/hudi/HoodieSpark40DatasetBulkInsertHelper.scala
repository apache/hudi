...
/**
 * This class is forked from [[BaseHoodieDatasetBulkInsertHelper]]
 * Only the dedupe diff logic has been changed to adapt to UTF8String comparison logic change since Spark 4
 */
object HoodieSpark40DatasetBulkInsertHelper
  ...
  override protected def compareRow(a: Comparable[AnyRef], b: Comparable[AnyRef]): Int = {
    if (a.isInstanceOf[UTF8String])
      a.asInstanceOf[UTF8String].binaryCompare(b.asInstanceOf[UTF8String])
    else
      a.compareTo(b.asInstanceOf[AnyRef])
  }
}
