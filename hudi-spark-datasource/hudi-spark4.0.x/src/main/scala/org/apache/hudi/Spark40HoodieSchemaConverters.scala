...
/**
 * Spark 4.0+ implementation that supports TimestampNTZType for local timestamps.
 *
 * Extends the base Spark 3 implementation and adds TimestampNTZType support.
 */
object Spark40HoodieSchemaConverters extends HoodieSchemaConverters {
  ...
  override def toSqlType(hoodieSchema: HoodieSchema): (DataType, Boolean) = {
    ...
    val (dataType, nullable) = BaseSpark4HoodieSchemaConverters.toSqlType(hoodieSchema)
  ...
  override def toHoodieType(catalystType: DataType, nullable: Boolean, recordName: String, nameSpace: String): HoodieSchema = {
    catalystType match {
      ...
      case _ =>
        BaseSpark4HoodieSchemaConverters.toHoodieType(catalystType, nullable, recordName, nameSpace)
    }
  }
}
