 ...
 */
public abstract class OtherSpecificParquetRecordReaderBase<T> extends RecordReader<Void, T> {
  protected Path file;
  ...
  public void initialize(InputSplit inputSplit, TaskAttemptContext taskAttemptContext)
    ...
    ReadSupport<T> readSupport = (ReadSupport<T>) new Spark32ParquetReadSupport(readerType);
    ...
    if (readerType.equals("skeleton")) {
      sparkRequestedSchemaString = configuration.get(Spark32ParquetReadSupport.SPARK_SKELETON_ROW_REQUESTED_SCHEMA());
    } else if (readerType.equals("base")) {
      sparkRequestedSchemaString = configuration.get(Spark32ParquetReadSupport.SPARK_BASE_ROW_REQUESTED_SCHEMA());
    } else {
      sparkRequestedSchemaString = configuration.get(Spark32ParquetReadSupport.SPARK_ROW_REQUESTED_SCHEMA());
    }
