...
public class DefaultSource extends BaseDefaultSource implements DataSourceV2,
  ...
  public Optional<DataSourceWriter> createWriter(String writeUUID, StructType schema, SaveMode mode,
    ...
    String tblName = options.get(HoodieWriteConfig.TABLE_NAME_CFG.key()).get();
    ...
    HoodieWriteConfig config = DataSourceUtils.createHoodieConfig(options.get(HoodieWriteConfig.AVRO_SCHEMA_CFG.key()).get(), path, tblName, options.asMap());
