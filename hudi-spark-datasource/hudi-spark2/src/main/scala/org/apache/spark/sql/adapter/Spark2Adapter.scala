...
import org.apache.spark.sql.execution.datasources.parquet.{ParquetFileFormat, Spark24HoodieParquetReader, Spark24LegacyHoodieParquetFileFormat}
...
class Spark2Adapter extends SparkAdapter {
  ...
  override def getPropsForReadingParquet(vectorized: Boolean,
    ...
    Spark24HoodieParquetReader.getPropsForReadingParquet(vectorized, sqlConf, options, hadoopConf)
  }
  ...
  override def readParquetFile(file: PartitionedFile,
    ...
    Spark24HoodieParquetReader.readParquetFile(file, requiredSchema, partitionSchema, filters,
