...
class Spark2ParquetInputUtil extends SparkParquetInputUtil {

  def getInputDataFrame(sqlContext: SQLContext,
                        baseFileRDD: RDD[HoodieBaseFile],
                        readStructType: StructType): DataFrame = {
    val serializableConf = new SerializableConfiguration(sqlContext.sparkContext.hadoopConfiguration)
    val inputRDD = baseFileRDD.flatMap(baseFile => {
      getInternalRowIterator(baseFile, serializableConf, readStructType)
    })
    sqlContext.sparkSession.internalCreateDataFrame(inputRDD, readStructType, isStreaming = false)
  }

  def getInternalRowIterator(baseFile: HoodieBaseFile,
                             serializableConf: SerializableConfiguration,
                             readStructType: StructType): RecordReaderIterator[InternalRow] = {
    ...
    val readSupport = constructReadSupport(inputSplit, serializableConf.get())
    val reader = new ParquetRecordReader[UnsafeRow](readSupport)
    val iter = new RecordReaderIterator[UnsafeRow](reader)

    reader.initialize(inputSplit, getAttemptContext(serializableConf, readStructType))
  ...
  private def constructReadSupport(inputSplit: InputSplit, hadoopConf: Configuration): ParquetReadSupport = {
    ...
    val convertTz =
      if (timestampConversion && !isCreatedByParquetMr) {
        Some(DateTimeUtils.getTimeZone(hadoopConf.get(SQLConf.SESSION_LOCAL_TIMEZONE.key)))
      } else {
        None
      }

    new ParquetReadSupport(convertTz)
  }
}
