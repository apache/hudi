...
public class SavepointsCommand {
    ...
    public String showSavepoints() {
        ...
        return HoodiePrintHelper.print(
                new String[] {HoodieTableHeaderFields.HEADER_SAVEPOINT_TIME}, rows);
    }
    ...
    public String savepoint(
            @ShellOption(
                            value = {"--commit"},
                            help = "Commit to savepoint")
                    final String commitTime,
            @ShellOption(
                            value = {"--user"},
                            defaultValue = "default",
                            help = "User who is creating the savepoint")
                    final String user,
            @ShellOption(
                            value = {"--comments"},
                            defaultValue = "default",
                            help = "Comments for creating the savepoint")
    ...
    public String rollbackToSavepoint(
            ...
            @ShellOption(value = "--sparkMaster", defaultValue = "", help = "Spark Master") String master,
            @ShellOption(
                            value = {"--lazyFailedWritesCleanPolicy"},
                            help = "True if FailedWriteCleanPolicy is lazy",
                            defaultValue = "false")
                    final String lazyFailedWritesCleanPolicy,
            @ShellOption(value = "--sparkMemory", defaultValue = "4G", help = "Spark executor memory")
                    final String sparkMemory)
            throws Exception {
        HoodieTableMetaClient metaClient = HoodieCLI.getTableMetaClient();
        ...
        SparkLauncher sparkLauncher = SparkUtil.initLauncher(sparkPropertiesPath);
        sparkLauncher.addAppArgs(
                SparkMain.SparkCommand.ROLLBACK_TO_SAVEPOINT.toString(),
                master,
                sparkMemory,
                instantTime,
                HoodieCLI.basePath,
                lazyFailedWritesCleanPolicy);
        Process process = sparkLauncher.launch();
        InputStreamConsumer.captureOutput(process);
        int exitCode = process.waitFor();
        // Refresh the current
    ...
    public String deleteSavepoint(
        ...
        HoodieInstant savePoint =
                new HoodieInstant(false, HoodieTimeline.SAVEPOINT_ACTION, instantTime);
        ...
        SparkLauncher sparkLauncher = SparkUtil.initLauncher(sparkPropertiesPath);
        sparkLauncher.addAppArgs(
                SparkMain.SparkCommand.DELETE_SAVEPOINT.toString(),
                master,
                sparkMemory,
                instantTime,
                HoodieCLI.basePath);
