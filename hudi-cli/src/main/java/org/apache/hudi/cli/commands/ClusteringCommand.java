...
public class ClusteringCommand {
    ...
    /**
      ...
      *
      * <p>Example: > connect --path {path to hudi table} > clustering schedule --sparkMaster local
      * --sparkMemory 2g
      */
    ...
    public String scheduleClustering(
            @ShellOption(
                            value = "--sparkMaster",
                            defaultValue = SparkUtil.DEFAULT_SPARK_MASTER,
                            help = "Spark master")
                    final String master,
            ...
            @ShellOption(
                            value = "--propsFilePath",
                            help =
                                    "path to properties file on localfs or dfs with configurations "
                                            + "for hoodie client for clustering",
                            defaultValue = "")
                    final String propsFilePath,
            @ShellOption(
                            value = "--hoodieConfigs",
                            help =
                                    "Any configuration that can be set in the properties file can "
                                            + "be passed here in the form of an array",
                            defaultValue = "")
        ...
        // First get a clustering instant time and pass it to spark launcher for scheduling
        // clustering
        String clusteringInstantTime = client.createNewInstantTime();

        sparkLauncher.addAppArgs(
                SparkCommand.CLUSTERING_SCHEDULE.toString(),
                master,
                sparkMemory,
                HoodieCLI.basePath,
    ...
    /**
      ...
      *
      * <p>Example: > connect --path {path to hudi table} > clustering schedule --sparkMaster local
      * --sparkMemory 2g > clustering run --sparkMaster local --sparkMemory 2g --clusteringInstant
      * 20211124005208
      */
    ...
    public String runClustering(
            ...
            @ShellOption(
                            value = "--parallelism",
                            help = "Parallelism for hoodie clustering",
                            defaultValue = "1")
                    final String parallelism,
            @ShellOption(value = "--retry", help = "Number of retries", defaultValue = "1")
                    final String retry,
            @ShellOption(
                            value = "--clusteringInstant",
                            help = "Clustering instant time",
                            defaultValue = ShellOption.NULL)
                    final String clusteringInstantTime,
            @ShellOption(
                            value = "--propsFilePath",
                            help =
            ...
            @ShellOption(
                            value = "--hoodieConfigs",
                            help =
                                    "Any configuration that can be set in the properties file can be "
                                            + "passed here in the form of an array",
                            defaultValue = "")
                    final String[] configs)
            throws Exception {
        HoodieTableMetaClient client = HoodieCLI.getTableMetaClient();
        ...
        SparkLauncher sparkLauncher = SparkUtil.initLauncher(sparkPropertiesPath);
        sparkLauncher.addAppArgs(
                SparkCommand.CLUSTERING_RUN.toString(),
                master,
                sparkMemory,
                HoodieCLI.basePath,
                client.getTableConfig().getTableName(),
    ...
    /**
      * Run clustering table service.
      *
      * <p>Example: > connect --path {path to hudi table} > clustering scheduleAndExecute --sparkMaster
      * local --sparkMemory 2g
      */
    @ShellMethod(
            key = "clustering scheduleAndExecute",
    ...
    public String runClustering(
        ...
        SparkLauncher sparkLauncher = SparkUtil.initLauncher(sparkPropertiesPath);
        sparkLauncher.addAppArgs(
                SparkCommand.CLUSTERING_SCHEDULE_AND_EXECUTE.toString(),
                master,
                sparkMemory,
                HoodieCLI.basePath,
                client.getTableConfig().getTableName(),
