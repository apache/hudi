...
public class CommitsCommand {
    ...
    private String printCommitsWithMetadata(
                ...
                for (Map.Entry<String, List<HoodieWriteStat>> partitionWriteStat :
                    ...
                    for (HoodieWriteStat hoodieWriteStat : partitionWriteStat.getValue()) {
                        if (StringUtils.isNullOrEmpty(partition)
                                || partition.equals(hoodieWriteStat.getPartitionPath())) {
                            rows.add(
                                    new Comparable[] {
                                        commit.getAction(),
                                        commit.getTimestamp(),
        ...
        return HoodiePrintHelper.print(
                HoodieTableHeaderFields.getTableHeaderWithExtraMetadata(),
                fieldNameToConverterMap,
                sortByField,
                descending,
                limit,
    ...
    public String showCommits(
            ...
            @ShellOption(
                            value = {"--createView"},
                            help = "view name to store output table",
                            defaultValue = "")
            ...
            @ShellOption(
                    ...
                    final String partition,
            @ShellOption(
                            value = {"--includeArchivedTimeline"},
                            help = "Include archived commits as well",
                            defaultValue = "false")
                    final boolean includeArchivedTimeline)
            throws IOException {

        HoodieDefaultTimeline timeline =
                getTimeline(HoodieCLI.getTableMetaClient(), includeArchivedTimeline);
        if (includeExtraMetadata) {
            return printCommitsWithMetadata(
                    timeline, limit, sortByField, descending, headerOnly, exportTableName, partition);
        } else {
    ...
    public String showArchivedCommits(
            ...
            @ShellOption(
                            value = {"--startTs"},
                            defaultValue = ShellOption.NULL,
                            help = "start time for commits, default: now - 10 days")
                    String startTs,
            @ShellOption(
                            value = {"--endTs"},
                            defaultValue = ShellOption.NULL,
                            help = "end time for commits, default: now - 1 day")
                    String endTs,
            @ShellOption(
                            value = {"--limit"},
                            help = "Limit commits",
        ...
        try {
            ...
            if (includeExtraMetadata) {
                return printCommitsWithMetadata(
                        timelineRange, limit, sortByField, descending, headerOnly, exportTableName, partition);
            } else {
                return printCommits(
                        timelineRange, limit, sortByField, descending, headerOnly, exportTableName);
            }
        } finally {
    ...
    public String showCommitPartitions(
        ...
        HoodieDefaultTimeline defaultTimeline =
                getTimeline(HoodieCLI.getTableMetaClient(), includeArchivedTimeline);
    ...
    public String showCommitFiles(
        ...
        return HoodiePrintHelper.print(
                header, new HashMap<>(), sortByField, descending, limit, headerOnly, rows, exportTableName);
    }
    ...
    public String compareCommits(
            @ShellOption(
                            value = {"--path"},
                            help = "Path of the table to compare to")
                    final String path) {
        ...
        String targetLatestCommit =
                targetTimeline.getInstantsAsStream().iterator().hasNext()
                        ? targetTimeline.lastInstant().get().getTimestamp()
                        : "0";
        String sourceLatestCommit =
                sourceTimeline.getInstantsAsStream().iterator().hasNext()
                        ? sourceTimeline.lastInstant().get().getTimestamp()
                        : "0";

        if (sourceLatestCommit != null
                && HoodieTimeline.compareTimestamps(
                        targetLatestCommit, HoodieTimeline.GREATER_THAN, sourceLatestCommit)) {
            // source is behind the target
            List<String> commitsToCatchup =
                    targetTimeline
                            .findInstantsAfter(sourceLatestCommit, Integer.MAX_VALUE)
                            .getInstantsAsStream()
            ...
            return "Source "
                    ...
                    + " is behind by "
                    + commitsToCatchup.size()
                    + " commits. Commits to catch up - "
                    + commitsToCatchup;
        } else {
            List<String> commitsToCatchup =
                    sourceTimeline
                            .findInstantsAfter(targetLatestCommit, Integer.MAX_VALUE)
                            .getInstantsAsStream()
                            .map(HoodieInstant::getTimestamp)
                            .collect(Collectors.toList());
            return "Source "
    ...
    public String syncCommits(
            @ShellOption(
                            value = {"--path"},
                            help = "Path of the table to sync to")
                    final String path) {
        ...
        return "Load sync state between "
                + HoodieCLI.getTableMetaClient().getTableConfig().getTableName()
                + " and "
    ...
    * */
    ...
    private Option<HoodieCommitMetadata> getHoodieCommitMetadata(
            HoodieTimeline timeline, Option<HoodieInstant> hoodieInstant) throws IOException {
