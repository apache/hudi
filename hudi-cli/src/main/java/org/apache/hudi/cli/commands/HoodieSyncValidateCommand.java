...
public class HoodieSyncValidateCommand {
    ...
    public String validateSync(
            ...
            @ShellOption(
                            value = {"--targetDb"},
                            defaultValue = "dwh_hoodie",
                            help = "target database")
                    final String tgtDb,
            @ShellOption(
                            value = {"--partitionCount"},
                            defaultValue = "5",
                            help = "total number of recent partitions to validate")
                    final int partitionCount,
            @ShellOption(
                            value = {"--hiveServerUrl"},
                            help = "hiveServerURL to connect to")
                    final String hiveServerUrl,
            @ShellOption(
                            value = {"--hiveUser"},
                            defaultValue = "",
                            help = "hive username to connect to")
                    final String hiveUser,
            @ShellOption(
                            value = {"--hivePass"},
                            defaultValue = "",
                            help = "hive password to connect to")
                    final String hivePass)
            throws Exception {
        ...
        if ("complete".equals(mode)) {
            ...
            sourceCount =
                    HiveUtil.countRecords(hiveServerUrl, source, srcDb, partitionCount, hiveUser, hivePass);
            targetCount =
                    HiveUtil.countRecords(hiveServerUrl, target, tgtDb, partitionCount, hiveUser, hivePass);
        }
        ...
        String sourceLatestCommit =
                sourceTimeline.getInstants().iterator().hasNext()
        ...
        if (sourceLatestCommit != null
            ...
            return getString(
                    target, targetTimeline, source, sourceCount, targetCount, sourceLatestCommit);
        } else {
            return getString(
                    source, sourceTimeline, target, targetCount, sourceCount, targetLatestCommit);
        }
    }
    ...
    private String getString(
        ...
        if (commitsToCatchup.isEmpty()) {
            return "Count difference now is (count("
