...
public class HoodieLogFileCommand {
    ...
    public String showLogFileCommits(
            @ShellOption(value = "--logFilePathPattern", help = "Fully qualified path for the log file")
        ...
        List<String> logFilePaths =
                FSUtils.getGlobStatusExcludingMetaFolder(storage, new StoragePath(logFilePathPattern))
                        .stream()
        ...
        Map<
                        ...
                        List<
                                Tuple3<
                                        Tuple2<String, HoodieLogBlockType>,
                                        Tuple2<Map<HeaderMetadataType, String>, Map<HeaderMetadataType, String>>,
                                        Integer>>>
                commitCountAndMetadata = new HashMap<>();
        ...
        for (String logFilePath : logFilePaths) {
            ...
            try (Reader reader =
                    HoodieLogFormat.newReader(storage, new HoodieLogFile(path), writerSchema)) {
                ...
                while (reader.hasNext()) {
                    ...
                    if (n instanceof HoodieCorruptBlock) {
                        ...
                        instantTime = n.getLogBlockHeader().get(HeaderMetadataType.INSTANT_TIME);
                        if (instantTime == null) {
                            // This can happen when reading archived commit files since they were
                            // written without any instant time
                    ...
                    if (commitCountAndMetadata.containsKey(instantTime)) {
                        commitCountAndMetadata
                                ...
                                .add(
                                        new Tuple3<>(
                                                new Tuple2<>(fileName, n.getBlockType()),
    ...
    public String showLogFileRecords(
            ...
            @ShellOption(
                            value = "--mergeRecords",
                            help = "If the records in the log files should be merged",
                            defaultValue = "false")
        ...
        for (int i = logFilePaths.size() - 1; i >= 0; i--) {
            Schema schema =
                    TableSchemaResolver.readSchemaFromLogFile(storage, new StoragePath(logFilePaths.get(i)));
            if (schema != null) {
                readerSchema = schema;
                break;
            }
            ...
            for (String logFile : logFilePaths) {
                ...
                try (HoodieLogFormat.Reader reader =
                        HoodieLogFormat.newReader(
                                storage, new HoodieLogFile(new StoragePath(logFile)), writerSchema)) {
                    // read the avro blocks
                    while (reader.hasNext()) {
                        HoodieLogBlock n = reader.next();
                        if (n instanceof HoodieDataBlock) {
