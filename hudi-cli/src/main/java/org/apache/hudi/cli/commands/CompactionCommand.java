...
public class CompactionCommand {
    ...
    public static final String COMPACTION_SCH_EXE_SUCCESSFUL =
            "Schedule and execute compaction successfully completed";
    ...
    public String compactionShow(
            @ShellOption(value = "--instant", help = "Base path for the target hoodie table")
    ...
    public String compactionsShowArchived(
            ...
            @ShellOption(
                            value = {"--startTs"},
                            defaultValue = ShellOption.NULL,
                            help = "start time for compactions, default: now - 10 days")
                    String startTs,
            @ShellOption(
                            value = {"--endTs"},
                            defaultValue = ShellOption.NULL,
                            help = "end time for compactions, default: now - 1 day")
                    String endTs,
            @ShellOption(
                            value = {"--limit"},
                            help = "Limit compactions",
                            defaultValue = "-1")
                    final Integer limit,
            @ShellOption(
                            value = {"--sortBy"},
    ...
    public String compactionShowArchived(
        ...
        HoodieInstant instant =
                new HoodieInstant(
                        HoodieInstant.State.COMPLETED, HoodieTimeline.COMPACTION_ACTION, compactionInstantTime);
        try {
    ...
    public String scheduleCompact(
        ...
        // First get a compaction instant time and pass it to spark launcher for scheduling
        // compaction
        ...
        sparkLauncher.addAppArgs(
                ...
                sparkMemory,
                HoodieCLI.basePath,
                client.getTableConfig().getTableName(),
    ...
    public String compact(
            ...
            @ShellOption(value = "--retry", defaultValue = "1", help = "Number of retries")
                    final String retry,
            @ShellOption(
                            value = "--compactionInstant",
                            help = "Instant of compaction.request",
                            defaultValue = ShellOption.NULL)
        ...
        if (null == compactionInstantTime) {
            ...
            Option<String> firstPendingInstant =
                    client
                            ...
                            .filter(instant -> instant.getAction().equals(HoodieTimeline.COMPACTION_ACTION))
                            .firstInstant()
        ...
        SparkLauncher sparkLauncher = SparkUtil.initLauncher(sparkPropertiesPath);
        sparkLauncher.addAppArgs(
                SparkCommand.COMPACT_RUN.toString(),
                master,
                sparkMemory,
                HoodieCLI.basePath,
                client.getTableConfig().getTableName(),
    ...
    @ShellMethod(
            key = "compaction scheduleAndExecute",
    ...
    public String compact(
        ...
        SparkLauncher sparkLauncher = SparkUtil.initLauncher(sparkPropertiesPath);
        sparkLauncher.addAppArgs(
                SparkCommand.COMPACT_SCHEDULE_AND_EXECUTE.toString(),
                master,
                sparkMemory,
                HoodieCLI.basePath,
                client.getTableConfig().getTableName(),
    ...
    private static String printAllCompactions(
        ...
        return HoodiePrintHelper.print(
                header, fieldNameToConverterMap, sortByField, descending, limit, headerOnly, rows);
    }

    /**
      * Compaction reading is different for different timelines. Create partial function to override
      * special logic. We can make these read methods part of HoodieDefaultTimeline and override where
      * necessary. But the BiFunction below has 'hacky' exception blocks, so restricting it to CLI.
      */
    ...
    private HoodieCompactionPlan readCompactionPlanForArchivedTimeline(
            HoodieArchivedTimeline archivedTimeline, HoodieInstant instant) {
        try {
    ...
    private HoodieCompactionPlan readCompactionPlanForActiveTimeline(
            HoodieActiveTimeline activeTimeline, HoodieInstant instant) {
        try {
            if (!HoodieTimeline.COMPACTION_ACTION.equals(instant.getAction())) {
                try {
                    // This could be a completed compaction. Assume a compaction request file is
                    // present but skip if fails
                    return TimelineMetadataUtils.deserializeCompactionPlan(
                            activeTimeline
                                    .readCompactionPlanAsBytes(
    ...
    protected static String printCompaction(
            HoodieCompactionPlan compactionPlan,
            String sortByField,
            boolean descending,
            int limit,
    ...
    private <T> T deSerializeOperationResult(StoragePath inputPath, HoodieStorage storage)
            throws Exception {
    ...
    public String validateCompaction(
        ...
        try {
            ...
            SparkLauncher sparkLauncher = SparkUtil.initLauncher(sparkPropertiesPath);
            sparkLauncher.addAppArgs(
                    SparkCommand.COMPACT_VALIDATE.toString(),
                    master,
                    sparkMemory,
                    HoodieCLI.basePath,
                    compactionInstant,
            ...
            boolean valid =
                    res.stream().map(OperationResult::isSuccess).reduce(Boolean::logicalAnd).orElse(true);
            ...
            res.forEach(
                    r -> {
                        Comparable[] row =
                                new Comparable[] {
                                    r.getOperation().getFileId(),
                                    r.getOperation().getBaseInstantTime(),
                                    r.getOperation().getDataFileName().isPresent()
                                            ? r.getOperation().getDataFileName().get()
                                            : "",
                                    r.getOperation().getDeltaFileNames().size(),
                                    r.isSuccess(),
    ...
    public String unscheduleCompaction(
        ...
        try {
            ...
            SparkLauncher sparkLauncher = SparkUtil.initLauncher(sparkPropertiesPath);
            sparkLauncher.addAppArgs(
                    SparkCommand.COMPACT_UNSCHEDULE_PLAN.toString(),
                    master,
                    sparkMemory,
                    HoodieCLI.basePath,
                    compactionInstant,
            ...
            output =
                    getRenamesToBePrinted(
                            res, limit, sortByField, descending, headerOnly, "unschedule pending compaction");
        } finally {
            // Delete tmp file used to serialize result
            if (HoodieCLI.storage.exists(outputPath)) {
                HoodieCLI.storage.deleteFile(outputPath);
    ...
    public String unscheduleCompactFile(
        ...
        try {
            ...
            SparkLauncher sparkLauncher = SparkUtil.initLauncher(sparkPropertiesPath);
            sparkLauncher.addAppArgs(
                    SparkCommand.COMPACT_UNSCHEDULE_FILE.toString(),
                    master,
                    sparkMemory,
                    HoodieCLI.basePath,
                    fileId,
    ...
    @ShellMethod(
            key = "compaction repair",
            value =
                    "Renames the files to make them consistent with the timeline as "
    ...
    public String repairCompaction(
        ...
        try {
            ...
            SparkLauncher sparkLauncher = SparkUtil.initLauncher(sparkPropertiesPath);
            sparkLauncher.addAppArgs(
                    SparkCommand.COMPACT_REPAIR.toString(),
                    master,
                    sparkMemory,
                    HoodieCLI.basePath,
                    compactionInstant,
            ...
            output =
                    getRenamesToBePrinted(
                            res, limit, sortByField, descending, headerOnly, "repair compaction");
        } finally {
            // Delete tmp file used to serialize result
            if (HoodieCLI.storage.exists(outputPath)) {
                HoodieCLI.storage.deleteFile(outputPath);
    ...
    private String getRenamesToBePrinted(
        ...
        if (result.isPresent()) {
            ...
            if (result.get()) {
                ...
                System.out.println(
                        "Some renames failed. table could be in inconsistent-state. Try running compaction repair");
            }
            ...
            res.forEach(
                    r -> {
                        Comparable[] row =
                                new Comparable[] {
                                    r.getOperation().fileId,
                                    r.getOperation().srcPath,
                                    r.getOperation().destPath,
                                    r.isExecuted(),
                                    r.isSuccess(),
                                    r.getException().isPresent() ? r.getException().get().getMessage() : ""
                                };
