...
public class HoodiePrintHelper {
    ...
    /**
      * Serialize Table to printable string and also export a temporary view to easily write sql
      * queries.
      *
      * <p>Ideally, exporting view needs to be outside PrintHelper, but all commands use this. So this
      * is easy way to add support for all commands
      *
      * @param rowHeader Row Header
      * @param fieldNameToConverterMap Field Specific Converters
      * @param sortByField Sorting field
    ...
    public static String print(
        ...
        if (!sortByField.isEmpty() && !rowHeader.containsField(sortByField)) {
            return String.format(
                    "Field[%s] is not in table, given columns[%s]", sortByField, rowHeader.getFieldNames());
        }
        ...
        Table table =
                new Table(
                                ...
                                Option.ofNullable(limit <= 0 ? null : limit))
                        .addAllRows(rows)
                        .flip();
