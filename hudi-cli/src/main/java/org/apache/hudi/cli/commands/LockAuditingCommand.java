...
public class LockAuditingCommand {
  ...
  private static String getAuditConfigPath(String basePath) {
    return basePath + "/.hoodie/.locks/audit_enabled.json";
  }
  
  private static String getAuditFolderPath(String basePath) {
    return basePath + "/.hoodie/.locks/audit/";
  }
  ...
  public String enableLockAudit() {
    ...
    try {
      ...
      // Write the config file using Hadoop FileSystem
      ...
      return String.format("Lock audit enabled successfully.\nAudit config written to: %s\n"
          + "Audit files will be stored at: %s", auditConfigPath, getAuditFolderPath(HoodieCLI.basePath));
  ...
  public String disableLockAudit(
    ...
    try {
      ...
      if (keepAuditFiles) {
        message += String.format("\nExisting audit files preserved at: %s", getAuditFolderPath(HoodieCLI.basePath));
      } else {
  ...
  public String showLockAuditStatus() {
    ...
    try {
      ...
      return String.format("Lock Audit Status: %s\n"
          ...
          status, HoodieCLI.basePath, auditConfigPath, getAuditFolderPath(HoodieCLI.basePath));
  ...
  public String validateAuditLocks() {
    ...
    try {
      ...
      for (FileStatus fileStatus : allFiles) {
        if (fileStatus.isFile() && fileStatus.getPath().getName().endsWith(".jsonl")) {
          auditFiles.add(fileStatus);
        }
      }
      ...
      for (FileStatus fileStatus : auditFiles) {
        Option<TransactionWindow> window = parseAuditFile(fileStatus);
        if (window.isPresent()) {
          windows.add(window.get());
        } else {
          parseErrors.add(String.format("[ERROR] Failed to parse audit file: %s", fileStatus.getPath().getName()));
        }
      }
  ...
  private CleanupResult performAuditCleanup(boolean dryRun, int ageDays) {
    try {
      if (HoodieCLI.fs == null) {
        String message = "FileSystem not initialized.";
      ...
      if (dryRun) {
        ...
        for (FileStatus fileStatus : oldFiles) {
          try {
            HoodieCLI.fs.delete(fileStatus.getPath(), false);
            deletedCount++;
          } catch (Exception e) {
            failedCount++;
            LOG.warn("Failed to delete audit file: " + fileStatus.getPath(), e);
          }
        }
   ...
   */
  private Option<TransactionWindow> parseAuditFile(FileStatus fileStatus) {
    String filename = fileStatus.getPath().getName();

    try {
      // Read and parse JSONL content
      List<AuditRecord> entries = new ArrayList<>();
      try (InputStream inputStream = HoodieCLI.fs.open(fileStatus.getPath());
