...
public class SparkMain {
    ...
    public static void main(String[] args) {
        ...
        JavaSparkContext jsc =
                SparkUtil.initJavaSparkContext(
                        "hoodie-cli-" + commandString, Option.of(args[1]), Option.of(args[2]));
        ...
        try {
            ...
            switch (cmd) {
                ...
                case DEDUPLICATE:
                    cmd.assertEq(args.length);
                    returnCode =
                            deduplicatePartitionPath(
                                    jsc, args[3], args[4], args[5], Boolean.parseBoolean(args[6]), args[7]);
                    break;
                ...
                case COMPACT_SCHEDULE_AND_EXECUTE:
                    returnCode =
                            compact(
                                    ...
                                    HoodieCompactor.SCHEDULE_AND_EXECUTE,
                                    propsFilePath,
                                    configs);
                    break;
                ...
                case CLUSTERING_SCHEDULE:
                    returnCode =
                            cluster(
                                    jsc, args[3], args[4], args[5], 1, args[2], 0, SCHEDULE, propsFilePath, configs);
                    break;
                case CLEAN:
    ...
    protected static void clean(
            JavaSparkContext jsc, String basePath, String propsFilePath, List<String> configs) {
    ...
    protected static int deleteMarker(JavaSparkContext jsc, String instantTime, String basePath) {
        try (SparkRDDWriteClient client = createHoodieClient(jsc, basePath, false)) {
            ...
            LOG.warn(
                    String.format("Failed: Could not clean marker instantTime: \"%s\".", instantTime), e);
            return -1;
        }
    }
    ...
    private static void doCompactValidate(
            JavaSparkContext jsc,
            String basePath,
    ...
    private static void doCompactRepair(
            JavaSparkContext jsc,
            String basePath,
            String compactionInstant,
            String outputPath,
    ...
    private static void doCompactUnschedule(
            JavaSparkContext jsc,
            String basePath,
            String compactionInstant,
            String outputPath,
    ...
    private static void doCompactUnscheduleFile(
            JavaSparkContext jsc,
            String basePath,
            String fileId,
    ...
    private static int compact(
        ...
        cfg.tableName = tableName;
        cfg.compactionInstantTime = compactionInstant;
        // TODO: Make this configurable along with strategy specific config - For now, this is a
        // generic enough strategy
    ...
    private static int deduplicatePartitionPath(
            JavaSparkContext jsc,
    ...
    public static int repairDeprecatedPartition(JavaSparkContext jsc, String basePath) {
        ...
        Dataset<Row> recordsToRewrite =
                getRecordsToRewrite(
                        basePath, PartitionPathEncodeUtils.DEPRECATED_DEFAULT_PARTITION_PATH, sqlContext);
    ...
    public static int renamePartition(
            JavaSparkContext jsc, String basePath, String oldPartition, String newPartition) {
        SQLContext sqlContext = new SQLContext(jsc);
    ...
    private static void rewriteRecordsToNewPartition(
            String basePath,
            String newPartition,
            Dataset<Row> recordsToRewrite,
            HoodieTableMetaClient metaClient,
    ...
    private static Dataset<Row> getRecordsToRewrite(
            String basePath, String oldPartition, SQLContext sqlContext) {
        return sqlContext
                .read()
                .format("hudi")
    ...
    private static Map<String, String> getPropsForRewrite(HoodieTableMetaClient metaClient) {
        ...
        metaClient
                .getTableConfig()
                .getProps()
                .forEach((k, v) -> propsMap.put(k.toString(), v.toString()));
        propsMap.put(HoodieWriteConfig.SKIP_DEFAULT_PARTITION_VALIDATION.key(), "true");
        propsMap.put(
                DataSourceWriteOptions.RECORDKEY_FIELD().key(),
    ...
    private static int doBootstrap(
            JavaSparkContext jsc,
            String tableName,
            String tableType,
            String basePath,
            String sourcePath,
            String recordKeyCols,
        ...
        TypedProperties properties =
                propsFilePath == null
                        ...
                        : readConfig(jsc.hadoopConfiguration(), new Path(propsFilePath), configs)
                                .getProps(true);

        properties.setProperty(HoodieBootstrapConfig.BASE_PATH.key(), sourcePath);

        if (!StringUtils.isNullOrEmpty(keyGenerator)
                && KeyGeneratorType.getNames().contains(keyGenerator.toUpperCase(Locale.ROOT))) {
            properties.setProperty(
                    HoodieWriteConfig.KEYGENERATOR_TYPE.key(), keyGenerator.toUpperCase(Locale.ROOT));
        } else {
        ...
        new BootstrapExecutor(
                        cfg,
                        jsc,
                        HadoopFSUtils.getFs(basePath, jsc.hadoopConfiguration()),
    ...
    private static int rollback(
            JavaSparkContext jsc, String instantTime, String basePath, Boolean rollbackUsingMarkers)
            throws Exception {
    ...
    private static int createSavepoint(
            JavaSparkContext jsc, String commitTime, String user, String comments, String basePath)
            throws Exception {
        try (SparkRDDWriteClient client = createHoodieClient(jsc, basePath, false)) {
    ...
    private static int rollbackToSavepoint(
            JavaSparkContext jsc, String savepointTime, String basePath, boolean lazyCleanPolicy)
            throws Exception {
    ...
    private static int deleteSavepoint(JavaSparkContext jsc, String savepointTime, String basePath)
            throws Exception {
        try (SparkRDDWriteClient client = createHoodieClient(jsc, basePath, false)) {
    ...
    /**
      ...
      * @throws Exception
      */
    protected static int upgradeOrDowngradeTable(
            JavaSparkContext jsc, String basePath, String toVersion) {
        ...
        try {
            ...
            LOG.warn(
                    String.format(
                            "Failed: Could not upgrade/downgrade table at \"%s\" to version \"%s\".",
                            basePath, toVersion),
                    e);
            return -1;
        }
    ...
    private static SparkRDDWriteClient createHoodieClient(
            JavaSparkContext jsc, String basePath, Boolean rollbackUsingMarkers, boolean lazyCleanPolicy)
            throws Exception {
    ...
    private static SparkRDDWriteClient createHoodieClient(
            JavaSparkContext jsc, String basePath, boolean lazyCleanPolicy) throws Exception {
        return createHoodieClient(
                jsc,
                basePath,
                Boolean.parseBoolean(HoodieWriteConfig.ROLLBACK_USING_MARKERS_ENABLE.defaultValue()),
                lazyCleanPolicy);
    ...
    private static HoodieWriteConfig getWriteConfig(
            String basePath, Boolean rollbackUsingMarkers, boolean lazyCleanPolicy) {
    ...
    private static int archive(
            ...
            String basePath) {
        try {
            return ArchiveExecutorUtils.archive(
                    jsc, minCommits, maxCommits, commitsRetained, enableMetadata, basePath);
