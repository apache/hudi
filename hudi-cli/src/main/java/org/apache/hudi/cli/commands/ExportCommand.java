...
/**
  ...
  *
  * <p>"export instants": Export Instants and their metadata from the Timeline to a local directory
  * specified by the parameter --localFolder The instants are exported in the json format.
  */
@ShellComponent
public class ExportCommand {
    ...
    public String exportInstants(
            ...
            @ShellOption(
                            value = {"--actions"},
                            help = "Comma separated list of Instant actions to export",
                            defaultValue = "clean,commit,deltacommit,rollback,savepoint,restore")
                    final String filter,
            @ShellOption(
                            value = {"--desc"},
                            help = "Ordering",
            ...
            @ShellOption(
                            value = {"--localFolder"},
                            help = "Local Folder to export to")
                    String localFolder)
            throws Exception {

        final StoragePath basePath = HoodieCLI.getTableMetaClient().getBasePath();
        final StoragePath archivePath =
                new StoragePath(HoodieCLI.getTableMetaClient().getArchivePath());
        ...
        List<StoragePathInfo> archivedPathInfoList =
                pathInfoList.stream()
        ...
        if (descending) {
            ...
            if (numCopied < numExports) {
                ...
                numCopied +=
                        copyArchivedInstants(
                                archivedPathInfoList, actionSet, numExports - numCopied, localFolder);
            }
        } else {
            numCopied = copyArchivedInstants(archivedPathInfoList, actionSet, numExports, localFolder);
            if (numCopied < numExports) {
                numCopied +=
                        copyNonArchivedInstants(nonArchivedInstants, numExports - numCopied, localFolder);
            }
        }
    ...
    private int copyArchivedInstants(
            List<StoragePathInfo> pathInfoList, Set<String> actionSet, int limit, String localFolder)
            throws Exception {
        int copyCount = 0;
        HoodieStorage storage =
                HoodieStorageUtils.getStorage(HoodieCLI.getTableMetaClient().getBasePath(), HoodieCLI.conf);
                ...
                while (reader.hasNext() && copyCount++ < limit) {
                    ...
                    try (ClosableIterator<HoodieRecord<IndexedRecord>> recordItr =
                        ...
                        while (recordItr.hasNext()) {
                            ...
                            // Archived instants are saved as arvo encoded HoodieArchivedMetaEntry
                            // records. We need to get the
                            // metadata record from the entry and convert it to json.
                            HoodieArchivedMetaEntry archiveEntryRecord =
                                    (HoodieArchivedMetaEntry)
                                            SpecificData.get().deepCopy(HoodieArchivedMetaEntry.SCHEMA$, ir);
                            ...
                            if (metadata == null) {
                                LOG.error(
                                        "Could not load metadata for action "
                                                + action
                                                + " at instant time "
                                                + instantTime);
                                continue;
                            ...
                            final String outPath =
                                    localFolder + StoragePath.SEPARATOR + instantTime + "." + action;
    ...
    private int copyNonArchivedInstants(List<HoodieInstant> instants, int limit, String localFolder)
            throws Exception {
        int copyCount = 0;
