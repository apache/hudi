...
/**
  ...
  * <p>
  *
  * <p>Example: The default spark.master conf is set to yarn. If you are running on a local
  * deployment, we can set the spark master to local using set conf command. > set --conf
  * SPARK_MASTER=local[2]
  *
  * <p>Connect to the table > connect --path {path to hudi table}
  *
  * <p>Run metadata commands > metadata list-partitions
  */
@ShellComponent
public class MetadataCommand {
    ...
    /**
      ...
      *
      * <p>This can be used to store the metadata table away from the dataset directory. - Useful for
      * testing as well as for using via the HUDI CLI so that the actual dataset is not written to. -
      * Useful for testing Metadata Table performance and operations on existing datasets before
      * enabling.
      */
    public static void setMetadataBaseDirectory(String metadataDir) {
        ValidationUtils.checkState(
                metadataBaseDirectory == null,
    ...
    public String set(
            @ShellOption(
                            value = {"--metadataDir"},
                            help = "Directory to read/write metadata table (can be different from dataset)",
                            defaultValue = "")
    ...
    public String create(
        ...
        try (HoodieTableMetadataWriter writer =
                SparkHoodieBackedTableMetadataWriter.create(
                        HoodieCLI.conf, writeConfig, new HoodieSparkEngineContext(jsc))) {
            return String.format(
                    "Created Metadata Table in %s (duration=%.2f secs)",
    ...
    public String delete(
            @ShellOption(
                            value = "--backup",
                            help = "Backup the metadata table before delete",
                            defaultValue = "true",
                            arity = 1)
        ...
        String backupPath =
                HoodieTableMetadataUtil.deleteMetadataTable(
                        dataMetaClient, new HoodieSparkEngineContext(jsc), backup);
        if (backup) {
    ...
    public String init(
        ...
        return String.format(
                action + " Metadata Table in %s (duration=%.2fsec)",
                metadataPath,
                (timer.endTimer()) / 1000.0);
    }
    ...
    public String stats() throws IOException {
        ...
        try (HoodieBackedTableMetadata metadata =
                new HoodieBackedTableMetadata(
                        new HoodieLocalEngineContext(HoodieCLI.conf),
            ...
            TableHeader header =
                    new TableHeader().addTableHeaderField("stat key").addTableHeaderField("stat value");
            return HoodiePrintHelper.print(
                    header, new HashMap<>(), "", false, Integer.MAX_VALUE, false, rows);
        }
    }
    ...
    @ShellMethod(
            key = "metadata list-files",
            value = "Print a list of all files in a partition from the metadata")
    public String listFiles(
            @ShellOption(
                            value = {"--partition"},
                            help = "Name of the partition to list files",
                            defaultValue = "")
                    final String partition)
            throws IOException {
        HoodieTableMetaClient metaClient = HoodieCLI.getTableMetaClient();
        HoodieMetadataConfig config = HoodieMetadataConfig.newBuilder().enable(true).build();
        try (HoodieBackedTableMetadata metaReader =
                new HoodieBackedTableMetadata(
                        new HoodieLocalEngineContext(HoodieCLI.conf),
                        metaClient.getStorage(),
                        config,
    ...
    public String validateFiles(
        ...
        for (String partition : allPartitions) {
            ...
            List<StoragePathInfo> metadataPathInfoList =
                    metadataReader.getAllFilesInPartition(new StoragePath(HoodieCLI.basePath, partition));
            metadataPathInfoList.forEach(
                    entry -> metadataPathInfoMap.put(entry.getPath().getName(), entry));
            ...
            if (metadataPathInfoList.size() != pathInfoList.size()) {
                LOG.error(
                        " FS and metadata files count not matching for "
                                + partition
                                + ". FS files count "
                                + pathInfoList.size()
                                + ", metadata base files count "
            ...
            for (Map.Entry<String, StoragePathInfo> entry : pathInfoMap.entrySet()) {
                if (!metadataPathInfoMap.containsKey(entry.getKey())) {
                    ...
                    if (entry.getValue().getLength() != metadataPathInfoMap.get(entry.getKey()).getLength()) {
                        LOG.error(
                                " FS file size mismatch "
                                        + entry.getKey()
                                        + ", size equality "
    ...
    private void initJavaSparkContext(Option<String> userDefinedMaster) {
        if (jsc == null) {
            jsc =
                    SparkUtil.initJavaSparkContext(SparkUtil.getDefaultConf("HoodieCLI", userDefinedMaster));
        }
    }
}
