...
public class SparkTempViewProvider implements TempViewProvider {
    ...
    public SparkTempViewProvider(String appName) {
        try {
            ...
            // log full stack trace and rethrow. Without this its difficult to debug failures, if
            // any
        ...
        try {
            ...
            if (rows.stream().filter(row -> row.size() != headers.size()).count() > 0) {
                throw new HoodieException(
                        "Invalid row, does not match headers " + headers.size() + " " + rows.size());
            }

            // replace all whitespaces in headers to make it easy to write sql queries
            List<String> headersNoSpaces =
                    headers.stream().map(title -> title.replaceAll("\\s+", "")).collect(Collectors.toList());
            ...
            for (int i = 0; i < headersNoSpaces.size(); i++) {
                ...
                structType =
                        structType.add(
                                DataTypes.createStructField(headersNoSpaces.get(i), headerDataType, true));
            }
            List<Row> records =
                    rows.stream()
                            .map(row -> RowFactory.create(row.toArray(new Comparable[row.size()])))
