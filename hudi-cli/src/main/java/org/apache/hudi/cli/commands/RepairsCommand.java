...
public class RepairsCommand {
  ...
  public String addPartitionMeta(
    ...
    HoodieLocation basePath = new HoodieLocation(client.getBasePath());
    String[][] rows = new String[partitionPaths.size()][];

    int ind = 0;
    for (String partition : partitionPaths) {
      HoodieLocation partitionPath = FSUtils.getPartitionPath(basePath, partition);
  ...
  public String overwriteHoodieProperties(
    ...
    HoodieLocation metaPathDir = new HoodieLocation(client.getBasePath(), METAFOLDER_NAME);
    HoodieTableConfig.create(client.getHoodieStorage(), metaPathDir, newProps);
  ...
  public void removeCorruptedPendingCleanAction() {
    ...
    cleanerTimeline.filterInflightsAndRequested().getInstants().forEach(instant -> {
      try {
        ...
        HoodieActiveTimeline.deleteInstantFile(client.getHoodieStorage(), client.getMetaPath(),
            instant);
  ...
  public String migratePartitionMeta(
    ...
    for (String partitionPath : partitionPaths) {
      HoodieLocation partition =
          FSUtils.getPartitionPath(client.getBasePath(), partitionPath);
      Option<HoodieLocation> textFormatFile =
          HoodiePartitionMetadata.textFormatMetaLocationIfExists(HoodieCLI.storage, partition);
      Option<HoodieLocation> baseFormatFile =
          HoodiePartitionMetadata.baseFormatMetaLocationIfExists(HoodieCLI.storage, partition);
      String latestCommit =
          client.getActiveTimeline().getCommitTimeline().lastInstant().get().getTimestamp();
    ...
    HoodieTableConfig.update(HoodieCLI.storage, new HoodieLocation(client.getMetaPath()), props);
