...
public class TimelineCommand {
    ...
    private static final SimpleDateFormat DATE_FORMAT_SECONDS =
    ...
    public String showActive(
            @ShellOption(
                            value = {"--limit"},
                            help = "Limit #rows to be displayed",
                            defaultValue = "10")
                    Integer limit,
            @ShellOption(
                            value = {"--sortBy"},
            ...
            @ShellOption(
                            value = {"--with-metadata-table"},
                            help = "Show metadata table timeline together with data table",
                            defaultValue = "false")
                    final boolean withMetadataTable,
            @ShellOption(
                            value = {"--show-rollback-info"},
                            help = "Show instant to rollback for rollbacks",
                            defaultValue = "false")
                    final boolean showRollbackInfo,
            @ShellOption(
                            value = {"--show-time-seconds"},
                            help = "Show seconds in instant file modification time",
                            defaultValue = "false")
                    final boolean showTimeSeconds) {
        ...
        try {
            if (withMetadataTable) {
                ...
                return printTimelineInfoWithMetadataTable(
                        ...
                        mtMetaClient.getActiveTimeline(),
    ...
    @ShellMethod(
            key = "metadata timeline show active",
    ...
    @ShellMethod(
            key = "metadata timeline show incomplete",
    ...
    private Map<String, Map<HoodieInstant.State, HoodieInstantWithModTime>>
            getInstantInfoFromTimeline(HoodieStorage storage, StoragePath metaPath) throws IOException {
        ...
        Stream<HoodieInstantWithModTime> instantStream =
                HoodieTableMetaClient.scanFiles(
                                ...
                                path -> {
                                    ...
                                    return HoodieActiveTimeline.VALID_EXTENSIONS_IN_ACTIVE_TIMELINE.contains(
                                            extension);
                                })
                        .stream()
                        .map(HoodieInstantWithModTime::new);
        instantStream.forEach(
                instant -> {
                    instantMap
                            .computeIfAbsent(instant.getTimestamp(), t -> new HashMap<>())
    ...
    private String printTimelineInfoWithMetadataTable(
            HoodieTimeline dtTimeline,
            HoodieTimeline mtTimeline,
        ...
        List<String> instantTimeList =
                instantTimeSet.stream()
                        .sorted(new HoodieInstantTimeComparator())
                        .collect(Collectors.toList());
                ...
                instantTimeList.stream()
                        .map(
                                instantTimestamp -> {
                                    ...
                                    row[0] = instantTimestamp;
                                    String dtRollbackInfoString =
                                            showRollbackInfo
                                                    ? getRollbackInfoString(dtInstant, dtTimeline, dtRollbackInfoMap)
                                                    : "";
                                    row[1] =
                                            (dtInstant.isPresent() ? dtInstant.get().getAction() : "-")
                                    ...
                                    String mtRollbackInfoString =
                                            showRollbackInfo
                                                    ? getRollbackInfoString(mtInstant, mtTimeline, mtRollbackInfoMap)
                                                    : "";
                                    row[6] =
                                            (mtInstant.isPresent() ? mtInstant.get().getAction() : "-")
    ...
    private Option<HoodieInstant> getInstant(HoodieTimeline timeline, String instantTimestamp) {
        return timeline
                .filter(instant -> instant.getTimestamp().equals(instantTimestamp))
                .firstInstant();
    }
    ...
    private String getInstantToRollback(HoodieTimeline timeline, HoodieInstant instant) {
        try {
            if (instant.isInflight()) {
                HoodieInstant instantToUse =
                        new HoodieInstant(
                                HoodieInstant.State.REQUESTED, instant.getAction(), instant.getTimestamp());
                HoodieRollbackPlan metadata =
                        TimelineMetadataUtils.deserializeAvroMetadata(
                                timeline.getInstantDetails(instantToUse).get(), HoodieRollbackPlan.class);
    ...
    private Map<String, List<String>> getRolledBackInstantInfo(HoodieTimeline timeline) {
        ...
        List<HoodieInstant> rollbackInstants =
                timeline
                        .filter(instant -> HoodieTimeline.ROLLBACK_ACTION.equalsIgnoreCase(instant.getAction()))
                        .getInstants();
        rollbackInstants.forEach(
                rollbackInstant -> {
                    try {
                        if (rollbackInstant.isInflight()) {
                            ...
                            rollbackInfoMap
                                    .computeIfAbsent(
                                            metadata.getInstantToRollback().getCommitTime(), k -> new ArrayList<>())
                                    .add(rollbackInstant.getTimestamp());
                        } else {
                            HoodieRollbackMetadata metadata =
                                    TimelineMetadataUtils.deserializeAvroMetadata(
                                            timeline.getInstantDetails(rollbackInstant).get(),
                            ...
                            metadata
                                    ...
                                    .forEach(
                                            instant -> {
                                                rollbackInfoMap
                                                        .computeIfAbsent(instant, k -> new ArrayList<>())
                                                        .add(rollbackInstant.getTimestamp());
                                            });
                        }
                    } catch (IOException e) {
    ...
    private String getRollbackInfoString(
