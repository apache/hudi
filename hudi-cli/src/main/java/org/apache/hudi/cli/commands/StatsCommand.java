...
public class StatsCommand {
    ...
    @ShellMethod(
            key = "stats wa",
            value =
                    "Write Amplification. Ratio of how many records were upserted to how many "
        ...
        for (HoodieInstant instantTime : timeline.getInstants()) {
            ...
            HoodieCommitMetadata commit =
                    HoodieCommitMetadata.fromBytes(
                            activeTimeline.getInstantDetails(instantTime).get(), HoodieCommitMetadata.class);
            if (commit.fetchTotalUpdateRecordsWritten() > 0) {
                waf =
                        df.format(
                                (float) commit.fetchTotalRecordsWritten()
                                        / commit.fetchTotalUpdateRecordsWritten());
            }
            rows.add(
                    new Comparable[] {
                        instantTime.getTimestamp(),
                        commit.fetchTotalUpdateRecordsWritten(),
                        commit.fetchTotalRecordsWritten(),
                        waf
                    });
    ...
    public String fileSizeStats(
            @ShellOption(
                            value = {"--partitionPath"},
                            help = "regex to select files, eg: 2016/08/02",
                            defaultValue = "*/*/*")
                    final String globRegex,
            @ShellOption(
                            value = {"--limit"},
                            help = "Limit commits",
                            defaultValue = "-1")
        ...
        return HoodiePrintHelper.print(
                header, getFieldNameToConverterMap(), sortByField, descending, limit, headerOnly, rows);
    }
