...
public class HiveUtil {
    ...
    public static long countRecords(
            String jdbcUrl, HoodieTableMetaClient source, String dbName, String user, String pass)
            throws SQLException {
        ResultSet rs = null;
        try (Connection conn = HiveUtil.getConnection(jdbcUrl, user, pass);
            ...
            rs =
                    stmt.executeQuery(
                            "select count(`_hoodie_commit_time`) as cnt from "
                                    + dbName
                                    + "."
            ...
            System.out.println(
                    "Total records in " + source.getTableConfig().getTableName() + " is " + count);
            return count;
        } finally {
            if (rs != null) {
                rs.close();
