...
/**
  * Table to be rendered. This class takes care of ordering rows and limiting before renderer renders
  * it.
  */
public class Table implements Iterable<List<String>> {
    ...
    public Table add(List<Comparable> row) {
        if (finishedAdding) {
            throw new IllegalStateException(
                    "Container already marked done for adding. No more entries can be added.");
        }

        if (rowHeader.getFieldNames().size() != row.size()) {
            throw new IllegalArgumentException(
                    "Incorrect number of fields in row. Expected: "
                            + rowHeader.getFieldNames().size()
                            + ", Got: "
                            + row.size()
                            + ", Row: "
                            + row);
    ...
    private List<List<Comparable>> orderRows() {
        return orderingFieldNameOptional
                .map(
                        orderingColumnName -> {
                            return rawRows.stream()
                                    .sorted(
                                            (row1, row2) -> {
                                                ...
                                                return isDescendingOptional
                                                        .map(isDescending -> isDescending ? -1 * cmpRawResult : cmpRawResult)
                                                        .orElse(cmpRawResult);
                                            })
    ...
    private void sortAndLimit() {
        ...
        renderHeaders =
                addRowNo
                        ? new TableHeader()
                                .addTableHeaderField(HoodieTableHeaderFields.HEADER_ROW_NO)
        ...
        renderRows =
                orderedRows.stream()
                        ...
                        .map(
                                row ->
                                        IntStream.range(0, renderHeaders.getNumFields())
                                                .mapToObj(
                                                        idx -> {
