...
public class FileSystemViewCommand {
    ...
    public String showAllFileSlices(
            @ShellOption(
                            value = {"--pathRegex"},
                            help = "regex to select files, eg: par1",
                            defaultValue = "*")
                    String globRegex,
            @ShellOption(
                            value = {"--baseFileOnly"},
                            help = "Only display base files view",
                            defaultValue = "false")
                    boolean baseFileOnly,
            @ShellOption(
                            value = {"--maxInstant"},
                            help = "File-Slices upto this instant are displayed",
                            defaultValue = "")
                    String maxInstant,
            @ShellOption(
                            value = {"--includeMax"},
                            help = "Include Max Instant",
                            defaultValue = "false")
                    boolean includeMaxInstant,
            @ShellOption(
                            value = {"--includeInflight"},
                            help = "Include Inflight Instants",
                            defaultValue = "false")
                    boolean includeInflight,
            @ShellOption(
                            value = {"--excludeCompaction"},
                            help = "Exclude compaction Instants",
                            defaultValue = "false")
                                                ...
                                                fs -> {
                                                    ...
                                                    row[idx++] =
                                                            fs.getBaseFile().isPresent()
                                                                    ? fs.getBaseFile().get().getFileSize()
                                                                    : -1;
                                                    if (!baseFileOnly) {
                                                        row[idx++] = fs.getLogFiles().count();
                                                        row[idx++] =
                                                                fs.getLogFiles().mapToLong(HoodieLogFile::getFileSize).sum();
    ...
    public String showLatestFileSlices(
            ...
            @ShellOption(
                            value = {"--baseFileOnly"},
                            help = "Only display base file view",
                            defaultValue = "false")
                    boolean baseFileOnly,
            @ShellOption(
                            value = {"--maxInstant"},
            ...
            @ShellOption(
                            value = {"--merge"},
                            help = "Merge File Slices due to pending compaction",
                            defaultValue = "true")
                    final boolean merge,
            @ShellOption(
                            value = {"--includeMax"},
        ...
        if (!baseFileOnly) {
            fieldNameToConverterMap.put(
                    HoodieTableHeaderFields.HEADER_TOTAL_DELTA_SIZE, converterFunction);
            fieldNameToConverterMap.put(
    ...
    /**
      ...
      * @throws IOException
      */
    private HoodieTableFileSystemView buildFileSystemView(
            String globRegex,
            String maxInstant,
            boolean basefileOnly,
            boolean includeMaxInstant,
        ...
        HoodieTableMetaClient metaClient =
                HoodieTableMetaClient.builder()
                        ...
                        .setBasePath(client.getBasePath())
