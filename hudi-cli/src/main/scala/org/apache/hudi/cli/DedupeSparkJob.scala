...
class DedupeSparkJob(
   ...
   * @return
   */
  private def planDuplicateFix(): HashMap[String, HashMap[String, Int]] = {
      ...
      dedupeType match {
        case DeDupeType.INSERT_TYPE =>
          ...
          rows
            .slice(1, rows.length)
            .foreach(r => {
              if (recordsMarkedForDeletion.contains(key)) {
                val f = r(2).asInstanceOf[String].split("_")(0)
                if (!fileToDeleteKeyMap.contains(f)) {
                  fileToDeleteKeyMap(f) = HashMap[String, Int]()
                }
                if (!fileToDeleteKeyMap(f).contains(key)) {
                  fileToDeleteKeyMap(f)(key) = 0
                }
                fileToDeleteKeyMap(f)(key) += 1
              }
            })
