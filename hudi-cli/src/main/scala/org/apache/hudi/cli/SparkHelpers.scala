...
object SparkHelpers {
  ...
  def skipKeysAndWriteNewFile(instantTime: String, fs: FileSystem, sourceFile: Path, destinationFile: Path, keysToSkip: Set[String]) {
    ...
    val filter: BloomFilter = BloomFilterFactory.createBloomFilter(HoodieIndexConfig.BLOOM_FILTER_NUM_ENTRIES_CFG.defaultValue.toInt, HoodieIndexConfig.BLOOM_FILTER_FPP_CFG.defaultValue.toDouble,
      HoodieIndexConfig.HOODIE_BLOOM_INDEX_FILTER_DYNAMIC_MAX_ENTRIES_CFG.defaultValue.toInt, HoodieIndexConfig.BLOOM_INDEX_FILTER_TYPE_CFG.defaultValue);
    ...
    val parquetConfig: HoodieAvroParquetConfig = new HoodieAvroParquetConfig(writeSupport, CompressionCodecName.GZIP, HoodieStorageConfig.PARQUET_BLOCK_SIZE_BYTES_CFG.defaultValue.toInt, HoodieStorageConfig.PARQUET_PAGE_SIZE_BYTES_CFG.defaultValue.toInt, HoodieStorageConfig.PARQUET_FILE_MAX_BYTES_CFG.defaultValue.toInt, fs.getConf, HoodieStorageConfig.PARQUET_COMPRESSION_RATIO_CFG.defaultValue.toDouble)
