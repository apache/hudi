...
object SparkHelpers {
  ...
  def skipKeysAndWriteNewFile(
    ...
    keysToSkip: HashMap[String, Int]
  ) {
    val sourceRecords =
      BaseFileUtils.getInstance(HoodieFileFormat.PARQUET).readAvroRecords(fs.getConf, sourceFile)
    val schema: Schema = sourceRecords.get(0).getSchema
    val filter: BloomFilter = BloomFilterFactory.createBloomFilter(
      HoodieIndexConfig.BLOOM_FILTER_NUM_ENTRIES_VALUE.defaultValue.toInt,
    ...
    val writeSupport: HoodieAvroWriteSupport = new HoodieAvroWriteSupport(
      new AvroSchemaConverter(fs.getConf).convert(schema),
      schema,
    ...
    val writer = new HoodieAvroParquetWriter(
      destinationFile,
      parquetConfig,
      instantTime,
    ...
    for (rec <- sourceRecords) {
      ...
      if (!keysToSkip.contains(key) || keysToSkip(key) == 0) {
...
class SparkHelper(sqlContext: SQLContext, fs: FileSystem) {
  ...
  def fileKeysAgainstBF(conf: Configuration, sqlContext: SQLContext, file: String): Boolean = {
    val bf = BaseFileUtils
      .getInstance(HoodieFileFormat.PARQUET)
      .readBloomFilterFromMetadata(conf, new Path(file))
