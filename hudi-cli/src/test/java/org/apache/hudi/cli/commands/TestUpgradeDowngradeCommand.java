...
public class TestUpgradeDowngradeCommand extends CLIFunctionalTestHarness {
    ...
    public void testUpgradeDowngradeCommand(
    ...
    public void testGetHoodieTableVersionName(boolean overrideWithDefault) {
        assertEquals(
                overrideWithDefault ? HoodieTableVersion.current().name() : null,
                UpgradeOrDowngradeCommand.getHoodieTableVersionName(null, overrideWithDefault));
        assertEquals(
                overrideWithDefault ? HoodieTableVersion.current().name() : "",
                UpgradeOrDowngradeCommand.getHoodieTableVersionName("", overrideWithDefault));
        assertEquals(
                "FIVE", UpgradeOrDowngradeCommand.getHoodieTableVersionName("FIVE", overrideWithDefault));
        assertEquals(
                "FIVE", UpgradeOrDowngradeCommand.getHoodieTableVersionName("5", overrideWithDefault));
    }

    private void verifyTableVersion(HoodieTableVersion expectedVersion) throws IOException {
        metaClient = HoodieTableMetaClient.reload(HoodieCLI.getTableMetaClient());
        assertEquals(
                expectedVersion.versionCode(), metaClient.getTableConfig().getTableVersion().versionCode());
        assertTableVersionFromPropertyFile(expectedVersion);
    }

    private void assertTableVersionFromPropertyFile(HoodieTableVersion expectedVersion)
            throws IOException {
        ...
        inputStream.close();
        assertEquals(
                Integer.toString(expectedVersion.versionCode()),
