...
public class HoodieTestCommitMetadataGenerator extends HoodieTestDataGenerator {
    ...
    public static void createCommitFileWithMetadata(
            String basePath, String commitTime, StorageConfiguration<?> configuration) throws Exception {
        createCommitFileWithMetadata(
                basePath, commitTime, configuration, Option.empty(), Option.empty());
    }
    ...
    public static void createCommitFileWithMetadata(
            ...
            Option<Integer> writes,
            Option<Integer> updates)
            throws Exception {
        createCommitFileWithMetadata(
                basePath, commitTime, configuration, writes, updates, Collections.emptyMap());
    }
        ...
        for (String name : commitFileNames) {
            HoodieCommitMetadata commitMetadata =
                    generateCommitMetadata(
                            basePath, commitTime, fileId1, fileId2, writes, updates, extraMetadata, true);
            createFileWithMetadata(
                    basePath, configuration, name, serializeCommitMetadata(commitMetadata).get());
        }
    }
    ...
    static void createFileWithMetadata(
            String basePath, StorageConfiguration<?> configuration, String name, byte[] content)
            throws IOException {
        Path commitFilePath =
                new Path(basePath + "/" + HoodieTableMetaClient.METAFOLDER_NAME + "/" + name);
        try (OutputStream os =
    ...
    public static HoodieCommitMetadata generateCommitMetadata(
            String basePath, String commitTime, Option<Integer> writes, Option<Integer> updates)
            throws Exception {
    ...
    public static HoodieCommitMetadata generateCommitMetadata(
            ...
            throws Exception {
        return generateCommitMetadata(
                basePath, commitTime, fileId1, fileId2, writes, updates, Collections.emptyMap(), true);
    }
    ...
    private static HoodieCommitMetadata generateCommitMetadata(
            Map<String, List<String>> partitionToFilePaths,
            Option<Integer> writes,
            Option<Integer> updates) {
        return generateCommitMetadata(
                partitionToFilePaths, writes, updates, Collections.emptyMap(), true);
    }
