...
public class AzureStorageUtils {
   ...
   * Supports standard HTTPS: https://account.blob.core.windows.net/container/path
   * Supports standard HTTPS: https://account.dfs.core.windows.net/container/path
   ...
   *
   * <p>For standard HTTPS URLs, uses Azure SDK's BlobUrlParts.parse() for robust parsing.
   * For WASB/ABFS Hadoop filesystem URIs, uses custom parsing to handle the "container@account" format.
   *
  ...
  public static AzureStorageUriComponents parseAzureUri(String uriString) {
    try {
      ...
      // For standard HTTPS URLs, use Azure SDK's built-in parser
      if (scheme.equals("https") || scheme.equals("http")) {
        ...
        if (StringUtils.isNullOrEmpty(accountName)) {
          throw new IllegalArgumentException("Could not extract account name from URI: " + uriString);
        }
        if (StringUtils.isNullOrEmpty(containerName)) {
          throw new IllegalArgumentException("Could not extract container name from URI: " + uriString);
        }
        if (StringUtils.isNullOrEmpty(blobPath)) {
          throw new IllegalArgumentException("Could not extract blob path from URI: " + uriString);
        }

        return new AzureStorageUriComponents(containerName, accountName, blobPath);
      }

      // For WASB/ABFS Hadoop URIs, use custom parsing
