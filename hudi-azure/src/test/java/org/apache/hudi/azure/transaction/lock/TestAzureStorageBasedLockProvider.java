...
public class TestAzureStorageBasedLockProvider extends StorageBasedLockProviderTestBase {
  ...
  private MockedConstruction<BlobServiceClientBuilder> mockedBuilderConstruction;
   ...
   * Uses Mockito to intercept BlobServiceClientBuilder construction and inject
  ...
  protected StorageBasedLockProvider createLockProvider() {
    ...
    // Close any existing mock from previous test
    if (mockedBuilderConstruction != null) {
      mockedBuilderConstruction.close();
    }

    // Pre-create the real BlobServiceClient before mocking
    final BlobServiceClient realClient = new BlobServiceClientBuilder()
        .connectionString(connectionString)
        .buildClient();

    // Mock BlobServiceClientBuilder to inject connection string for Azurite
    mockedBuilderConstruction = mockConstruction(BlobServiceClientBuilder.class,
        (mock, context) -> {
          // When buildClient() is called, return our pre-created client
          Mockito.when(mock.buildClient()).thenReturn(realClient);
          // Return the mock itself for method chaining
          Mockito.when(mock.endpoint(Mockito.anyString())).thenReturn(mock);
          Mockito.when(mock.credential(Mockito.<TokenCredential>any())).thenReturn(mock);
          Mockito.when(mock.retryOptions(Mockito.any())).thenReturn(mock);
          Mockito.when(mock.clientOptions(Mockito.any())).thenReturn(mock);
        });
  ...
  void setupLockProvider() {
    ...
    providerProperties.put(BASE_PATH_KEY, basePath);
    lockProvider = createLockProvider();
  }

  @AfterEach
  void tearDownMocks() {
    // Close the mock construction after each test
    if (mockedBuilderConstruction != null) {
      mockedBuilderConstruction.close();
      mockedBuilderConstruction = null;
    }
  }
  ...
  void testAzurePreconditions() {
    ...
    Response<BlockBlobItem> response1 = containerClient.getBlobClient(blobName)
    ...
    Response<BlockBlobItem> response2 = containerClient.getBlobClient(blobName)
