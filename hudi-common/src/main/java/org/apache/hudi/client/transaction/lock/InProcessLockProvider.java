...
/**
  * InProcess level lock. This {@link LockProvider} implementation is to guard table from concurrent
  * operations happening in the local JVM process. A separate lock is maintained per "table
  * basepath".
  *
  * <p>Note: This Lock provider implementation doesn't allow lock reentrancy. Attempting to reacquire
  * the lock from the same thread will throw HoodieLockException. Threads other than the current lock
  * owner, will block on lock() and return false on tryLock().
  */
public class InProcessLockProvider implements LockProvider<ReentrantReadWriteLock>, Serializable {
    ...
    private static final Map<String, ReentrantReadWriteLock> LOCK_INSTANCE_PER_BASEPATH =
    ...
    public InProcessLockProvider(
        ...
        lock =
                LOCK_INSTANCE_PER_BASEPATH.computeIfAbsent(
                        basePath, (ignore) -> new ReentrantReadWriteLock());
        maxWaitTimeMillis =
                typedProperties.getLong(
                        LockConfiguration.LOCK_ACQUIRE_WAIT_TIMEOUT_MS_PROP_KEY,
    ...
    private String getLogMessage(LockState state) {
        return String.format(
                "Base Path %s, Lock Instance %s, Thread %s, In-process lock state %s",
                basePath, getLock().toString(), Thread.currentThread().getName(), state.name());
    }
}
