...
public enum MetadataPartitionType {
                ...
                return metaClient.getIndexMetadata().get().getIndexDefinitions().values().stream()
                        .anyMatch(
                                indexDef ->
                                        indexDef
                                                .getIndexName()
                                                .startsWith(HoodieTableMetadataUtil.PARTITION_NAME_SECONDARY_INDEX_PREFIX));
            }
            return false;
        }
    },
    PARTITION_STATS(HoodieTableMetadataUtil.PARTITION_NAME_PARTITION_STATS, "partition-stats-") {
        ...
        public boolean isMetadataPartitionEnabled(TypedProperties writeConfig) {
            return getBooleanWithAltKeys(writeConfig, ENABLE_METADATA_INDEX_PARTITION_STATS)
                    && nonEmpty(
                            getStringWithAltKeys(writeConfig, COLUMN_STATS_INDEX_FOR_COLUMNS, EMPTY_STRING));
        }
    };
    ...
    /**
      * Returns the list of metadata table partitions which require WriteStatus to track written
      * records.
      *
      * <p>These partitions need the list of written records so that they can update their metadata.
      */
    ...
    /**
      * Returns the list of metadata partition types enabled based on the metadata config and table
      * config.
      */
    public static List<MetadataPartitionType> getEnabledPartitions(
            TypedProperties writeConfig, HoodieTableMetaClient metaClient) {
        return Arrays.stream(values())
                .filter(
                        partitionType ->
    ...
    public static MetadataPartitionType fromPartitionPath(String partitionPath) {
        ...
        throw new IllegalArgumentException(
                "No MetadataPartitionType for partition path: " + partitionPath);
    }
