...
public class HoodieMetadataMetrics implements Serializable {
    ...
    private Map<String, String> getStats(
        ...
        for (String metadataPartition : metadataPartitions) {
            ...
            stats.put(
                    metadataPartition + "." + STAT_TOTAL_BASE_FILE_SIZE,
                    String.valueOf(totalBaseFileSizeInBytes));
            stats.put(
                    metadataPartition + "." + STAT_TOTAL_LOG_FILE_SIZE,
        ...
        if (detailed) {
            stats.put(
                    HoodieMetadataMetrics.STAT_COUNT_PARTITION,
    ...
    protected void incrementMetric(String action, long value) {
        LOG.debug(
                String.format("Updating metadata metrics (%s=%d) in %s", action, value, metricsRegistry));
