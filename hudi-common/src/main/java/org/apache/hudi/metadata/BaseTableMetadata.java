...
public abstract class BaseTableMetadata extends AbstractHoodieTableMetadata {
   ...
   */
  @Override
  public List<HoodieFileInfo> getAllFilesInPartition(HoodieLocation partitionPath) throws IOException {
    ValidationUtils.checkArgument(isMetadataTableInitialized);
    try {
  ...
  @Override
  public Map<String, List<HoodieFileInfo>> getAllFilesInPartitions(Collection<String> partitions) throws IOException {
    ValidationUtils.checkArgument(isMetadataTableInitialized);
    ...
    try {
      List<HoodieLocation> partitionPaths = partitions.stream().map(HoodieLocation::new).collect(Collectors.toList());
   ...
   */
  List<HoodieFileInfo> fetchAllFilesInPartition(HoodieLocation partitionPath) throws IOException {
    ...
    List<HoodieFileInfo> statuses = recordOpt
        .map(record -> {
          HoodieMetadataPayload metadataPayload = record.getData();
  ...
  Map<String, List<HoodieFileInfo>> fetchAllFilesInPartitionPaths(List<HoodieLocation> partitionPaths) throws IOException {
    Map<String, HoodieLocation> partitionIdToPathMap =
        partitionPaths.parallelStream()
            .collect(
                Collectors.toMap(partitionPath -> {
                  String partitionId = FSUtils.getRelativePartitionPathFromLocation(dataBasePath, partitionPath);
    ...
    HoodieStorage fs = FSUtils.getHoodieStorage(partitionPaths.get(0).toString(), getHadoopConf());

    Map<String, List<HoodieFileInfo>> partitionPathToFilesMap = partitionIdRecordPairs.entrySet().stream()
        .map(e -> {
          ...
          List<HoodieFileInfo> files = metadataPayload.getFileStatuses(fs, partitionPath);
