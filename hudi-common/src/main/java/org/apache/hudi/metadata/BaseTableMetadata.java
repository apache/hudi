...
public abstract class BaseTableMetadata extends AbstractHoodieTableMetadata {
    ...
    protected BaseTableMetadata(
        ...
        this.dataMetaClient =
                HoodieTableMetaClient.builder().setStorage(storage).setBasePath(dataBasePath).build();
    ...
    /**
      ...
      *
      * <p>If the Metadata Table is enabled, the listing is retrieved from the stored metadata.
      * Otherwise, the list of partitions is retrieved directly from the underlying {@code FileSystem}.
      *
      * <p>On any errors retrieving the listing from the metadata, defaults to using the file system
      * listings.
      */
    @Override
    ...
    public Option<BloomFilter> getBloomFilter(final String partitionName, final String fileName)
        ...
        if (!dataMetaClient
                .getTableConfig()
                .isMetadataPartitionAvailable(MetadataPartitionType.BLOOM_FILTERS)) {
        ...
        if (bloomFilters.isEmpty()) {
            LOG.error(
                    "Meta index: missing bloom filter for partition: {}, file: {}", partitionName, fileName);
            return Option.empty();
        }
    ...
    @Override
    public Map<Pair<String, String>, BloomFilter> getBloomFilters(
            final List<Pair<String, String>> partitionNameFileNameList) throws HoodieMetadataException {
        if (!dataMetaClient
                .getTableConfig()
                .isMetadataPartitionAvailable(MetadataPartitionType.BLOOM_FILTERS)) {
            LOG.error("Metadata bloom filter index is disabled!");
        ...
        partitionNameFileNameList.forEach(
                    ...
                    final String bloomFilterIndexKey =
                            HoodieMetadataPayload.getBloomFilterIndexKey(
                                    new PartitionIndexID(
                                            HoodieTableMetadataUtil.getBloomFilterIndexPartitionIdentifier(
        ...
        Map<String, HoodieRecord<HoodieMetadataPayload>> hoodieRecords =
                getRecordsByKeys(
                        partitionIDFileIDStringsList, MetadataPartitionType.BLOOM_FILTERS.getPartitionPath());
        metrics.ifPresent(
                m ->
                        m.updateMetrics(
                                HoodieMetadataMetrics.LOOKUP_BLOOM_FILTERS_METADATA_STR, timer.endTimer()));
        metrics.ifPresent(
                m ->
                        m.setMetric(
                                HoodieMetadataMetrics.LOOKUP_BLOOM_FILTERS_FILE_COUNT_STR,
                                partitionIDFileIDStringsList.size()));

        Map<Pair<String, String>, BloomFilter> partitionFileToBloomFilterMap =
                new HashMap<>(hoodieRecords.size());
        for (final Map.Entry<String, HoodieRecord<HoodieMetadataPayload>> entry :
            ...
            if (bloomFilterMetadata.isPresent()) {
                if (!bloomFilterMetadata.get().getIsDeleted()) {
                    ...
                    //        - [[BloomFilterMetadata]] could be re-used, and hence have to stay
                    // immutable
    ...
    @Override
    public Map<Pair<String, String>, HoodieMetadataColumnStats> getColumnStats(
            final List<Pair<String, String>> partitionNameFileNameList, final String columnName)
            throws HoodieMetadataException {
        if (!dataMetaClient
                .getTableConfig()
        ...
        metrics.ifPresent(
                m ->
                        m.updateMetrics(
                                HoodieMetadataMetrics.LOOKUP_COLUMN_STATS_METADATA_STR, timer.endTimer()));
        metrics.ifPresent(
                m ->
                        m.setMetric(
                                HoodieMetadataMetrics.LOOKUP_COLUMN_STATS_FILE_COUNT_STR,
    ...
    /**
      ...
      *
      * <p>If the Metadata Table is not enabled, an exception is thrown to distinguish this from the
      * absence of the key.
      *
    ...
    public Map<String, List<HoodieRecordGlobalLocation>> readRecordIndex(List<String> recordKeys) {
        // If record index is not initialized yet, we cannot return an empty result here unlike the
        // code for reading from other
        // indexes. This is because results from this function are used for upserts and returning an
        // empty result here would lead
        // to existing records being inserted again causing duplicates.
        // The caller is required to check for record index existence in MDT before calling this
        // method.
        ...
        Map<String, List<HoodieRecord<HoodieMetadataPayload>>> result =
                getAllRecordsByKeys(recordKeys, MetadataPartitionType.RECORD_INDEX.getPartitionPath());
        Map<String, List<HoodieRecordGlobalLocation>> recordKeyToLocation =
        ...
        metrics.ifPresent(
                m -> m.updateMetrics(HoodieMetadataMetrics.LOOKUP_RECORD_INDEX_TIME_STR, timer.endTimer()));
        metrics.ifPresent(
                m ->
                        m.setMetric(
                                HoodieMetadataMetrics.LOOKUP_RECORD_INDEX_KEYS_COUNT_STR, recordKeys.size()));
        metrics.ifPresent(
                m ->
                        m.setMetric(
                                HoodieMetadataMetrics.LOOKUP_RECORD_INDEX_KEYS_HITS_COUNT_STR,
    ...
    /**
      ...
      */
    @Override
    public Map<String, List<HoodieRecordGlobalLocation>> readSecondaryIndex(
            List<String> secondaryKeys, String partitionName) {
        ValidationUtils.checkState(
                dataMetaClient
                        .getTableConfig()
                        .isMetadataPartitionAvailable(MetadataPartitionType.RECORD_INDEX),
    ...
    public Map<String, String> getSecondaryKeys(List<String> recordKeys) {
        ...
        ValidationUtils.checkState(
                dataMetaClient
                        .getTableConfig()
                        .isMetadataPartitionAvailable(MetadataPartitionType.SECONDARY_INDEX),
                "Secondary index is not initialized in MDT");
        return getSecondaryKeysForRecordKeys(
                recordKeys, MetadataPartitionType.SECONDARY_INDEX.getPartitionPath());
    }
    ...
    protected List<String> fetchAllPartitionPaths() {
        ...
        Option<HoodieRecord<HoodieMetadataPayload>> recordOpt =
                getRecordByKey(RECORDKEY_PARTITION_LIST, MetadataPartitionType.FILES.getPartitionPath());
        metrics.ifPresent(
                m -> m.updateMetrics(HoodieMetadataMetrics.LOOKUP_PARTITIONS_STR, timer.endTimer()));
                        ...
                        .map(
                                record -> {
                                    ...
                                    if (relativePaths.size() == 1
                                            && relativePaths.get(0).equals(NON_PARTITIONED_NAME)) {
    ...
    List<StoragePathInfo> fetchAllFilesInPartition(StoragePath partitionPath) {
        ...
        String recordKey =
                relativePartitionPath.isEmpty() ? NON_PARTITIONED_NAME : relativePartitionPath;

        HoodieTimer timer = HoodieTimer.start();
        Option<HoodieRecord<HoodieMetadataPayload>> recordOpt =
                getRecordByKey(recordKey, MetadataPartitionType.FILES.getPartitionPath());
        metrics.ifPresent(
                                ...
                                record -> {
                                    ...
                                    try {
                                        ...
                                        throw new HoodieException(
                                                "Failed to extract file-pathInfoList from the payload", e);
                                    }
                                })
                        .orElseGet(Collections::emptyList);

        LOG.debug(
                "Listed file in partition from metadata: partition={}, #files={}",
                relativePartitionPath,
                pathInfoList.size());
        return pathInfoList;
    }

    Map<String, List<StoragePathInfo>> fetchAllFilesInPartitionPaths(
            List<StoragePath> partitionPaths) {
        ...
        Map<String, HoodieRecord<HoodieMetadataPayload>> partitionIdRecordPairs =
                getRecordsByKeys(
                        new ArrayList<>(partitionIdToPathMap.keySet()),
    ...
    private void checkForSpuriousDeletes(
            HoodieMetadataPayload metadataPayload, String partitionName) {
        if (!metadataPayload.getDeletions().isEmpty()) {
            if (metadataConfig.shouldIgnoreSpuriousDeletes()) {
                LOG.warn(
                        "Metadata record for "
                                + partitionName
                                + " encountered some files to be deleted which was not added before. "
                                + "Ignoring the spurious deletes as the `"
                                + HoodieMetadataConfig.IGNORE_SPURIOUS_DELETES.key()
    ...
    protected abstract Option<HoodieRecord<HoodieMetadataPayload>> getRecordByKey(
            String key, String partitionName);

    protected abstract Map<String, HoodieRecord<HoodieMetadataPayload>> getRecordsByKeys(
            List<String> keys, String partitionName);

    protected abstract Map<String, String> getSecondaryKeysForRecordKeys(
            List<String> recordKeys, String partitionName);

    /**
      * Returns a map of (record-key -> list-of-secondary-index-records) for the provided secondary
      * keys.
      */
