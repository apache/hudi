...
public interface HoodieTableMetadata extends Serializable, AutoCloseable {
    ...
    /**
      ...
      * {@link org.apache.hudi.common.table.timeline.HoodieTimeline#INIT_INSTANT_TS}, such that the
      * metadata table can be prepped even before bootstrap is done.
      */
    ...
    static String getDataTableBasePathFromMetadataTable(String metadataTableBasePath) {
        ...
        return metadataTableBasePath.substring(
                0, metadataTableBasePath.lastIndexOf(HoodieTableMetaClient.METADATA_TABLE_FOLDER_PATH) - 1);
    }
    ...
    static String getDatasetBasePath(String metadataTableBasePath) {
        int endPos =
                metadataTableBasePath.lastIndexOf(
                        StoragePath.SEPARATOR + HoodieTableMetaClient.METADATA_TABLE_FOLDER_PATH);
    ...
    static HoodieTableMetadata create(
    ...
    static HoodieTableMetadata create(
        ...
        if (metadataConfig.isEnabled()) {
            HoodieBackedTableMetadata metadata =
                    createHoodieBackedTableMetadata(
                            engineContext, storage, metadataConfig, datasetBasePath, reuse);
            // If the MDT is not initialized then we fallback to FSBackedTableMetadata
            if (metadata.isMetadataTableInitialized()) {
                return metadata;
            }
            LOG.warn(
                    "Falling back to FileSystemBackedTableMetadata as metadata table is not initialized");
        }
        return createFSBackedTableMetadata(engineContext, storage, datasetBasePath);
    }

    static FileSystemBackedTableMetadata createFSBackedTableMetadata(
    ...
    static HoodieBackedTableMetadata createHoodieBackedTableMetadata(
            HoodieEngineContext engineContext,
            HoodieStorage storage,
            HoodieMetadataConfig metadataConfig,
            String datasetBasePath,
    ...
    /**
      * Retrieve the paths of partitions under the provided sub-directories, and try to filter these
      * partitions using the provided {@link Expression}.
      */
    List<String> getPartitionPathWithPathPrefixUsingFilterExpression(
            List<String> relativePathPrefixes, Types.RecordType partitionFields, Expression expression)
            throws IOException;

    /**
      * Fetches all partition paths that are the sub-directories of the list of provided (relative)
      * paths.
      *
      * <p>E.g., Table has partition 4 partitions: year=2022/month=08/day=30,
      * year=2022/month=08/day=31, year=2022/month=07/day=03, year=2022/month=07/day=04 The relative
      * path "year=2022" returns all partitions, while the relative path "year=2022/month=07" returns
      * only two partitions.
      */
    List<String> getPartitionPathWithPathPrefixes(List<String> relativePathPrefixes)
            throws IOException;
    ...
    /** Fetch all files for given partition paths. NOTE: Absolute partition paths are expected here */
    ...
    /**
      ...
      *
      * @param partitionNameFileNameList - List of partition and file name pair for which bloom filters
      *     need to be retrieved
      * @return Map of partition file name pair to its bloom filter
      * @throws HoodieMetadataException
      */
    Map<Pair<String, String>, BloomFilter> getBloomFilters(
    ...
    /**
      ...
      * @throws HoodieMetadataException
      */
    Map<Pair<String, String>, HoodieMetadataColumnStats> getColumnStats(
            final List<Pair<String, String>> partitionNameFileNameList, final String columnName)
            throws HoodieMetadataException;

    /**
      * Returns the location of record keys which are found in the record index. Records that are not
      * found are ignored and won't be part of map object that is returned.
      */
    Map<String, List<HoodieRecordGlobalLocation>> readRecordIndex(List<String> recordKeys);

    /**
      * Returns the location of records which the provided secondary keys maps to. Records that are not
      * found are ignored and won't be part of map object that is returned.
      */
    Map<String, List<HoodieRecordGlobalLocation>> readSecondaryIndex(
            List<String> secondaryKeys, String partitionName);

    /**
      * Fetch records by key prefixes. Key prefix passed is expected to match the same prefix as stored
      * in Metadata table partitions. For eg, in case of col stats partition, actual keys in metadata
      * partition is encoded values of column name, partition name and file name. So, key prefixes
      * passed to this method is expected to be encoded already.
      *
      ...
      * @return {@link HoodieData} of {@link HoodieRecord}s with records matching the passed in key
      *     prefixes.
      */
    HoodieData<HoodieRecord<HoodieMetadataPayload>> getRecordsByKeyPrefixes(
            List<String> keyPrefixes, String partitionName, boolean shouldLoadInMemory);

    /**
      * Fetch records for given keys. A key could have multiple records associated with it. This method
      * returns all the records for given keys.
      *
      * @param keys list of key for which interested records are looked up for.
      * @param partitionName partition name in metadata table where the records are looked up for.
      * @return Map of key to {@link List} of {@link HoodieRecord}s with records matching the passed in
      *     keys.
      */
    Map<String, List<HoodieRecord<HoodieMetadataPayload>>> getAllRecordsByKeys(
