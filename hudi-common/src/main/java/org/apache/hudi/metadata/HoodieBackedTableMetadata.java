...
public class HoodieBackedTableMetadata extends BaseTableMetadata {
  ...
  private Map<String, Pair<HoodieFileReader, HoodieMetadataMergedLogRecordScanner>> bucketReaders = new ConcurrentHashMap<>();
  ...
  private Pair<HoodieFileReader, HoodieMetadataMergedLogRecordScanner> openReadersIfNeeded(String key, String partitionName) throws IOException {
    return bucketReaders.computeIfAbsent(partitionName, k -> {
      try {
  ...
  @Override
  public void close() {
    for (String partitionName : bucketReaders.keySet()) {
      close(partitionName);
    }
    bucketReaders.clear();
  }
  ...
  private synchronized void close(String partitionName) {
    Pair<HoodieFileReader, HoodieMetadataMergedLogRecordScanner> readers = bucketReaders.remove(partitionName);
    if (readers != null) {
      try {
