...
/**
  * MetadataTable records are persisted with the schema defined in HoodieMetadata.avsc. This class
  * represents the payload for the MetadataTable.
  *
  * <p>This single metadata payload is shared by all the partitions under the metadata table. The
  * partition specific records are determined by the field "type" saved within the record. The
  * following types are supported:
  *
  * <p>METADATA_TYPE_PARTITION_LIST (1): -- List of all partitions. There is a single such record --
  * key = @{@link HoodieTableMetadata#RECORDKEY_PARTITION_LIST}
  *
  * <p>METADATA_TYPE_FILE_LIST (2): -- List of all files in a partition. There is one such record for
  * each partition -- key = partition name
  *
  * <p>METADATA_TYPE_COLUMN_STATS (3): -- This is an index for column stats in the table
  *
  * <p>METADATA_TYPE_BLOOM_FILTER (4): -- This is an index for base file bloom filters. This is a map
  * of FileID to its BloomFilter byte[].
  *
  * <p>During compaction on the table, the deletions are merged with additions and hence records are
  * pruned.
  */
public class HoodieMetadataPayload implements HoodieRecordPayload<HoodieMetadataPayload> {
    ...
    /**
      * Type of the record. This can be an enum in the schema but Avro1.8 has a bug - <a
    ...
    /**
      * FileIndex value saved in record index record when the fileId has no index (old format of base
      * filename)
      */
    ...
    /**
      ...
      */
    private static final Lazy<HoodieMetadataColumnStats.Builder> METADATA_COLUMN_STATS_BUILDER_STUB =
            Lazy.lazily(HoodieMetadataColumnStats::newBuilder);

    private static final HoodieMetadataFileInfo DELETE_FILE_METADATA =
            new HoodieMetadataFileInfo(0L, true);
    ...
    public HoodieMetadataPayload(Option<GenericRecord> recordOpt) {
        if (recordOpt.isPresent()) {
            ...
            if (type == METADATA_TYPE_FILE_LIST || type == METADATA_TYPE_PARTITION_LIST) {
                    ...
                    filesystemMetadata
                            ...
                            .forEach(
                                    k -> {
                                        ...
                                        filesystemMetadata.put(
                                                k,
                                                new HoodieMetadataFileInfo(
                                                        (Long) v.get("size"), (Boolean) v.get("isDeleted")));
                                    });
                }
                ...
                // NOTE: Only legitimate reason for {@code BloomFilterMetadata} to not be present is
                // when
                //       it's not been read from the storage (ie it's not been a part of projected
                // schema).
                //       Otherwise, it has to be present or the record would be considered invalid
                if (bloomFilterRecord == null) {
                    checkArgument(
                            record.getSchema().getField(SCHEMA_FIELD_ID_BLOOM_FILTER) == null,
                            String.format(
                                    "Valid %s record expected for type: %s",
                                    SCHEMA_FIELD_ID_BLOOM_FILTER, METADATA_TYPE_BLOOM_FILTER));
                } else {
                    bloomFilterMetadata =
                            new HoodieMetadataBloomFilter(
                ...
                // NOTE: Only legitimate reason for {@code ColumnStatsMetadata} to not be present is
                // when
                //       it's not been read from the storage (ie it's not been a part of projected
                // schema).
                //       Otherwise, it has to be present or the record would be considered invalid
                if (columnStatsRecord == null) {
                    checkArgument(
                            record.getSchema().getField(SCHEMA_FIELD_ID_COLUMN_STATS) == null,
                            String.format(
                                    "Valid %s record expected for type: %s",
                    ...
                    columnStatMetadata =
                            HoodieMetadataColumnStats.newBuilder(METADATA_COLUMN_STATS_BUILDER_STUB.get())
                                    ...
                                    // org.apache.avro.Schema#FIELD_RESERVED to
                                    ...
                                    .setMinValue(
                                            wrapValueIntoAvro(
                                                    unwrapAvroValueWrapper(
                                                            columnStatsRecord.get(COLUMN_STATS_FIELD_MIN_VALUE))))
                                    .setMaxValue(
                                            wrapValueIntoAvro(
                                                    unwrapAvroValueWrapper(
                                                            columnStatsRecord.get(COLUMN_STATS_FIELD_MAX_VALUE))))
                                    ...
                                    .setTotalUncompressedSize(
                                            (Long) columnStatsRecord.get(COLUMN_STATS_FIELD_TOTAL_UNCOMPRESSED_SIZE))
                ...
                recordIndexMetadata =
                        new HoodieRecordIndexInfo(
                                recordIndexRecord.get(RECORD_INDEX_FIELD_PARTITION).toString(),
                                Long.parseLong(
                                        recordIndexRecord.get(RECORD_INDEX_FIELD_FILEID_HIGH_BITS).toString()),
                                Long.parseLong(
                                        recordIndexRecord.get(RECORD_INDEX_FIELD_FILEID_LOW_BITS).toString()),
                                ...
                                Integer.parseInt(
                                        recordIndexRecord.get(RECORD_INDEX_FIELD_FILEID_ENCODING).toString()),
                                recordIndexPosition != null
                                        ? Long.parseLong(recordIndexPosition.toString())
                                        : null);
            } else if (type == METADATA_TYPE_SECONDARY_INDEX) {
                GenericRecord secondaryIndexRecord =
                        getNestedFieldValue(record, SCHEMA_FIELD_ID_SECONDARY_INDEX);
                checkState(
                        secondaryIndexRecord != null,
                        "Valid SecondaryIndexMetadata record expected for type: "
                                + METADATA_TYPE_SECONDARY_INDEX);
                secondaryIndexMetadata =
                        new HoodieSecondaryIndexInfo(
    ...
    private HoodieMetadataPayload(
            String key, int type, Map<String, HoodieMetadataFileInfo> filesystemMetadata) {
    ...
    protected HoodieMetadataPayload(
            String key,
            int type,
    ...
    /**
      ...
      */
    public static HoodieRecord<HoodieMetadataPayload> createPartitionListRecord(
    ...
    /**
      ...
      */
    public static HoodieRecord<HoodieMetadataPayload> createPartitionListRecord(
            List<String> partitions, boolean isDeleted) {
        ...
        HoodieKey key =
                new HoodieKey(RECORDKEY_PARTITION_LIST, MetadataPartitionType.FILES.getPartitionPath());
        HoodieMetadataPayload payload =
                new HoodieMetadataPayload(key.getRecordKey(), METADATA_TYPE_PARTITION_LIST, fileInfo);
    ...
    /**
      ...
      * @param filesAdded Mapping of files to their sizes for files which have been added to this
      *     partition
      * @param filesDeleted List of files which have been deleted from this partition
      */
    public static HoodieRecord<HoodieMetadataPayload> createPartitionFilesRecord(
            String partition, Map<String, Long> filesAdded, List<String> filesDeleted) {
        ...
        HoodieKey key =
                new HoodieKey(partitionIdentifier, MetadataPartitionType.FILES.getPartitionPath());
        HoodieMetadataPayload payload =
                new HoodieMetadataPayload(key.getRecordKey(), METADATA_TYPE_FILE_LIST, fileInfo);
        return new HoodieAvroRecord<>(key, payload);
    }
    ...
    /**
      ...
      */
    public static HoodieRecord<HoodieMetadataPayload> createBloomFilterMetadataRecord(
        ...
        checkArgument(
                !baseFileName.contains(StoragePath.SEPARATOR)
        ...
        HoodieKey key =
                new HoodieKey(bloomFilterIndexKey, MetadataPartitionType.BLOOM_FILTERS.getPartitionPath());

        HoodieMetadataBloomFilter metadataBloomFilter =
                new HoodieMetadataBloomFilter(bloomFilterType, timestamp, bloomFilter, isDeleted);
        HoodieMetadataPayload metadataPayload =
                new HoodieMetadataPayload(key.getRecordKey(), metadataBloomFilter);
    ...
    public HoodieMetadataPayload preCombine(HoodieMetadataPayload previousRecord) {
        if (this.isDeletedRecord) {
            // This happens when a record has been deleted. The previous version of the record
            // should be ignored.
            return this;
        }
        ...
        checkArgument(
                previousRecord.type == type, "Cannot combine " + previousRecord.type + " with " + type);
        checkArgument(
        ...
        switch (type) {
            ...
            case METADATA_TYPE_SECONDARY_INDEX:
                ...
                // record and discards the prevRecord. Based on the 'isDeleted' marker in the
                // payload,
                // the merger running on top takes the right action (discard current or retain
                // current record).
                return this;
            default:
    ...
    private static String getBloomFilterRecordKey(String partitionName, String fileName) {
        return new PartitionIndexID(
                        HoodieTableMetadataUtil.getBloomFilterIndexPartitionIdentifier(partitionName))
    ...
    public Option<IndexedRecord> combineAndGetUpdateValue(
            IndexedRecord oldRecord, Schema schema, Properties properties) throws IOException {
        HoodieMetadataPayload anotherPayload =
                new HoodieMetadataPayload(Option.of((GenericRecord) oldRecord));
        HoodieRecordPayload combinedPayload = preCombine(anotherPayload);
        return combinedPayload.getInsertValue(schema, properties);
    }

    @Override
    public Option<IndexedRecord> combineAndGetUpdateValue(IndexedRecord oldRecord, Schema schema)
            throws IOException {
        return combineAndGetUpdateValue(oldRecord, schema, new Properties());
    }

    @Override
    public Option<IndexedRecord> getInsertValue(Schema schemaIgnored, Properties propertiesIgnored)
            throws IOException {
        if (key == null || this.isDeletedRecord) {
            return Option.empty();
        }

        HoodieMetadataRecord record =
                new HoodieMetadataRecord(
                        key,
                        type,
                        filesystemMetadata,
                        bloomFilterMetadata,
    ...
    private Stream<Map.Entry<String, HoodieMetadataFileInfo>> filterFileInfoEntries(
            boolean isDeleted) {
        if (filesystemMetadata == null) {
            return Stream.empty();
        }

        return filesystemMetadata.entrySet().stream()
                .filter(e -> e.getValue().getIsDeleted() == isDeleted);
    }

    private Map<String, HoodieMetadataFileInfo> combineFileSystemMetadata(
            HoodieMetadataPayload previousRecord) {
        ...
        if (filesystemMetadata != null) {
            ...
            filesystemMetadata.forEach(
                    (key, fileInfo) -> {
                        combinedFileInfo.merge(
                                key,
                                fileInfo,
                                ...
                                (oldFileInfo, newFileInfo) -> {
                                    ...
                                    if (newFileInfo.getIsDeleted()) {
                                        if (oldFileInfo.getIsDeleted()) {
                                            LOG.warn(
                                                    "A file is repeatedly deleted in the files partition of the metadata table: "
                                                            + key);
                                            return newFileInfo;
                                        }
                                        return null;
    ...
    /**
      ...
      */
    public static String getBloomFilterIndexKey(
            PartitionIndexID partitionIndexID, FileIndexID fileIndexID) {
    ...
    /**
      ...
      */
    public static String getColumnStatsIndexKey(
            PartitionIndexID partitionIndexID, FileIndexID fileIndexID, ColumnIndexID columnIndexID) {
        return columnIndexID
                .asBase64EncodedString()
    ...
    /**
      ...
      */
    public static String getColumnStatsIndexKey(
            String partitionName, HoodieColumnRangeMetadata<Comparable> columnRangeMetadata) {

        final PartitionIndexID partitionIndexID =
                new PartitionIndexID(
                        HoodieTableMetadataUtil.getColumnStatsIndexPartitionIdentifier(partitionName));
        final FileIndexID fileIndexID =
                new FileIndexID(new StoragePath(columnRangeMetadata.getFilePath()).getName());
    ...
    public static Stream<HoodieRecord> createColumnStatsRecords(
            ...
            boolean isDeleted) {
        return columnRangeMetadataList.stream()
                .map(
                        columnRangeMetadata -> {
                            HoodieKey key =
                                    new HoodieKey(
                                            getColumnStatsIndexKey(partitionName, columnRangeMetadata),
                                            MetadataPartitionType.COLUMN_STATS.getPartitionPath());

                            HoodieMetadataPayload payload =
                                    new HoodieMetadataPayload(
                                            key.getRecordKey(),
    ...
    public static Stream<HoodieRecord> createPartitionStatsRecords(
        ...
        return columnRangeMetadataList.stream()
                .map(
                        columnRangeMetadata -> {
                            HoodieKey key =
                                    new HoodieKey(
                                            getPartitionStatsIndexKey(partitionPath, columnRangeMetadata.getColumnName()),
    ...
    public static String getPartitionStatsIndexKey(String partitionPath, String columnName) {
        final PartitionIndexID partitionIndexID =
                new PartitionIndexID(
                        HoodieTableMetadataUtil.getColumnStatsIndexPartitionIdentifier(partitionPath));
    ...
    private static HoodieMetadataColumnStats mergeColumnStatsRecords(
            HoodieMetadataColumnStats prevColumnStats, HoodieMetadataColumnStats newColumnStats) {
        ...
        return HoodieMetadataColumnStats.newBuilder(METADATA_COLUMN_STATS_BUILDER_STUB.get())
                ...
                .setTotalUncompressedSize(
                        prevColumnStats.getTotalUncompressedSize() + newColumnStats.getTotalUncompressedSize())
                .setIsDeleted(newColumnStats.getIsDeleted())
                .build();
    }

    /**
      * Create and return a {@code HoodieMetadataPayload} to insert or update an entry for the record
      * index.
      *
      * <p>Each entry maps the key of a single record in HUDI to its location.
      *
      ...
      */
    public static HoodieRecord<HoodieMetadataPayload> createRecordIndexUpdate(
            String recordKey, String partition, String fileId, String instantTime, int fileIdEncoding) {
        ...
        try {
            ...
            throw new HoodieMetadataException(
                    "Failed to create metadata payload for record index. Instant time parsing for "
                            + instantTime
                            + " failed ",
                    e);
        }
        if (fileIdEncoding == 0) {
            // Data file names have a -D suffix to denote the index (D = integer) of the file
            // written
            ...
            try {
                ...
                throw new HoodieMetadataException(
                        String.format(
                                "Invalid UUID or index: fileID=%s, partition=%s, instantTIme=%s",
                                fileId, partition, instantTime),
                        e);
            }
            ...
            HoodieMetadataPayload payload =
                    new HoodieMetadataPayload(
                            recordKey,
                            new HoodieRecordIndexInfo(
                                    partition, -1L, -1L, -1, fileId, instantTimeMillis, 1, null));
            return new HoodieAvroRecord<>(key, payload);
        }
    }

    /**
      * Create and return a {@code HoodieMetadataPayload} to insert or update an entry for the
      * secondary index.
      *
      * <p>Each entry maps the secondary key of a single record in HUDI to its record (or primary) key
      *
      ...
      */
    public static HoodieRecord<HoodieMetadataPayload> createSecondaryIndex(
        ...
        HoodieMetadataPayload payload =
                new HoodieMetadataPayload(secondaryKey, new HoodieSecondaryIndexInfo(recordKey, isDeleted));
        return new HoodieAvroRecord<>(key, payload);
    }
    ...
    /**
      * Create and return a {@code HoodieMetadataPayload} to delete a record in the Metadata Table's
      * record index.
      *
        ...
        switch (type) {
            ...
            case METADATA_TYPE_BLOOM_FILTER:
                ...
                sb.append("bloom size: ")
                        .append(getBloomFilterMetadata().get().getBloomFilter().array().length)
                        .append(", ");
    ...
    private static void validatePayload(
            int type, Map<String, HoodieMetadataFileInfo> filesystemMetadata) {
        if (type == METADATA_TYPE_FILE_LIST) {
            filesystemMetadata.forEach(
                    (fileName, fileInfo) -> {
                        checkState(
                                fileInfo.getIsDeleted() || fileInfo.getSize() > 0,
