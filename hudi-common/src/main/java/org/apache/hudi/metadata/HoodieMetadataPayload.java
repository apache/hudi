  ...
  public FileStatus[] getFileStatuses(FileSystem fs, Path partitionPath) {
    return filterFileInfoEntries(false)
        .map(e -> {
          ...
          Path logicalPath = new Path(partitionPath, fileName);

          try {
            return fs.getFileStatus(logicalPath);
          } catch (IOException ex) {
            throw new HoodieIOException("Failed to extract file-statuses from the payload, path: " + logicalPath, ex);
          }
        })
