...
public class FileSystemBackedTableMetadata extends AbstractHoodieTableMetadata {
    ...
    public FileSystemBackedTableMetadata(
        ...
        StoragePath metaPath = new StoragePath(dataBasePath, HoodieTableMetaClient.METAFOLDER_NAME);
    ...
    public List<String> getPartitionPathWithPathPrefixUsingFilterExpression(
            ...
            throws IOException {
        return relativePathPrefixes.stream()
                .flatMap(
                        relativePathPrefix -> {
                            try {
                                return getPartitionPathWithPathPrefixUsingFilterExpression(
                                        relativePathPrefix, partitionFields, expression)
                                        .stream();
                            } catch (IOException e) {
                                throw new HoodieIOException(
                                        "Error fetching partition paths with relative path: " + relativePathPrefix, e);
                            }
                        })
    ...
    private List<String> getPartitionPathWithPathPrefixUsingFilterExpression(
            String relativePathPrefix, Types.RecordType partitionFields, Expression pushedExpr)
            throws IOException {
        List<StoragePath> pathsToList = new CopyOnWriteArrayList<>();
        pathsToList.add(
                StringUtils.isNullOrEmpty(relativePathPrefix)
                        ? dataBasePath
                        : new StoragePath(dataBasePath, relativePathPrefix));
        ...
        // Not like `HoodieBackedTableMetadata`, since we don't know the exact partition levels
        // here,
        ...
        // TODO better enable urlEncodePartitioningEnabled if hiveStylePartitioningEnabled is
        // enabled?
        ...
        while (!pathsToList.isEmpty()) {
            ...
            // if current dictionary does not contain PartitionMetadata, add it to queue to be
            // processed.
            int fileListingParallelism = Math.min(DEFAULT_LISTING_PARALLELISM, dirToFileListing.size());
            if (!dirToFileListing.isEmpty()) {
                // result below holds a list of pair. first entry in the pair optionally holds the
                // deduced list of partitions.
                                ...
                                fileInfo -> {
                                    ...
                                    if (fileInfo.isDirectory()) {
                                        if (HoodiePartitionMetadata.hasPartitionMetadata(getStorage(), path)) {
                                            return Pair.of(
                                                    Option.of(FSUtils.getRelativePartitionPath(dataBasePath, path)),
                                                    Option.empty());
                                        ...
                                        String partitionName =
                                                FSUtils.getRelativePartitionPath(dataBasePath, path.getParent());
                ...
                partitionPaths.addAll(
                        result.stream()
                                .filter(entry -> entry.getKey().isPresent())
                                .map(entry -> entry.getKey().get())
                                .filter(
                                        relativePartitionPath ->
                                                fullBoundExpr instanceof Predicates.TrueExpression
                                                        || (Boolean)
                                                                fullBoundExpr.eval(
                ...
                // If partitionPaths is nonEmpty, we're already at the last path level, and all
                // paths
                // are filtered already.
                if (needPushDownExpressions && partitionPaths.isEmpty()) {
                    // Here we assume the path level matches the number of partition columns, so
                    // we'll rebuild
                    // new schema based on current path level.
                    // e.g. partition columns are <region, date, hh>, if we're listing the second
                    // level, then
                    // currentSchema would be <region, date>
                    // `PartialBindVisitor` will bind reference if it can be found from
                    // `currentSchema`, otherwise
                    // will change the expression to `alwaysTrue`. Can see `PartialBindVisitor` for
                    // details.
                    Types.RecordType currentSchema =
                            Types.RecordType.get(partitionFields.fields().subList(0, ++currentPartitionLevel));
                    PartialBindVisitor partialBindVisitor =
                            new PartialBindVisitor(currentSchema, caseSensitive);
                ...
                pathsToList.addAll(
                        result.stream()
                                ...
                                .filter(
                                        path ->
                                                partialBoundExpr instanceof Predicates.TrueExpression
                                                        || (Boolean)
                                                                partialBoundExpr.eval(
                                                                        extractPartitionValues(
                                                                                partitionFields,
                                                                                FSUtils.getRelativePartitionPath(dataBasePath, path),
                                                                                urlEncodePartitioningEnabled)))
                                .collect(Collectors.toList()));
            }
        }
    ...
    public Map<String, List<StoragePathInfo>> getAllFilesInPartitions(
            Collection<String> partitionPaths) throws IOException {
        ...
        return partitionToFiles.stream()
