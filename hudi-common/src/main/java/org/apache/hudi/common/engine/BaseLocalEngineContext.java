...
/**
 * Base engine context that runs everything in the current JVM over in-memory lists.
 * Shared by {@link HoodieLocalEngineContext} and the Java client's engine context.
 */
public abstract class BaseLocalEngineContext extends HoodieEngineContext {

  protected BaseLocalEngineContext(StorageConfiguration<?> conf, TaskContextSupplier taskContextSupplier) {
