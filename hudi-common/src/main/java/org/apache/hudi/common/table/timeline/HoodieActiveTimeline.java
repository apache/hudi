...
public class HoodieActiveTimeline extends HoodieDefaultTimeline {
  ...
  public void deleteInstantFileIfExists(HoodieInstant instant) {
    ...
    try {
      if (metaClient.getHoodieStorage().exists(commitFilePath)) {
        boolean result = metaClient.getHoodieStorage().deleteFile(commitFilePath);
        if (result) {
  ...
  protected void deleteInstantFile(HoodieInstant instant) {
    ...
    try {
      boolean result = metaClient.getHoodieStorage().deleteFile(inFlightCommitFilePath);
      if (result) {
        LOG.info("Removed instant " + instant);
      } else {
  ...
  public Option<byte[]> getInstantDetails(HoodieInstant instant) {
    HoodieLocation detailPath = getInstantFileNamePath(getInstantFileName(instant));
  ...
  public Option<byte[]> readCompactionPlanAsBytes(HoodieInstant instant) {
    return readDataFromPath(new HoodieLocation(metaClient.getMetaPath(), getInstantFileName(instant)));
  }
  ...
  protected void transitionStateToComplete(boolean shouldLock, HoodieInstant fromInstant,
    ...
    try {
      if (metaClient.getTimelineLayoutVersion().isNullVersion()) {
        ...
        HoodieLocation fromInstantPath = getInstantFileNamePath(fromInstantFileName);
        ...
        HoodieLocation toInstantPath =
            getInstantFileNamePath(instantWithCompletionTime.getFileName());
        boolean success = metaClient.getHoodieStorage().rename(fromInstantPath, toInstantPath);
        if (!success) {
        ...
        ValidationUtils.checkArgument(
            metaClient.getHoodieStorage().exists(getInstantFileNamePath(fromInstantFileName)));
  ...
  protected void transitionPendingState(HoodieInstant fromInstant, HoodieInstant toInstant, Option<byte[]> data,
    ...
    try {
      ...
      if (metaClient.getTimelineLayoutVersion().isNullVersion()) {
        ...
        HoodieLocation fromInstantPath = getInstantFileNamePath(fromInstantFileName);
        HoodieLocation toInstantPath = getInstantFileNamePath(toInstantFileName);
        ...
        if (allowRedundantTransitions) {
          FileIOUtils.createFileInLocation(storage, getInstantFileNamePath(toInstantFileName), data);
        } else {
  ...
  protected void revertCompleteToInflight(HoodieInstant completed, HoodieInstant inflight) {
    ...
    HoodieLocation commitFilePath = getInstantFileNamePath(getInstantFileName(completed));
    try {
      if (metaClient.getTimelineLayoutVersion().isNullVersion()) {
        ...
        HoodieLocation requestedInstantFilePath = getInstantFileNamePath(new HoodieInstant(State.REQUESTED, inflight.getAction(),
        ...
        boolean success = metaClient.getHoodieStorage().deleteFile(commitFilePath);
  ...
  public void copyInstant(HoodieInstant instant, HoodieLocation dstDir) {
    HoodieLocation srcPath = new HoodieLocation(metaClient.getMetaPath(), getInstantFileName(instant));
    HoodieLocation dstPath = new HoodieLocation(dstDir, getInstantFileName(instant));
    try {
      HoodieStorage srcStorage = HoodieStorageUtils.getHoodieStorage(srcPath, metaClient.getHadoopConf());
      HoodieStorage dstStorage = HoodieStorageUtils.getHoodieStorage(dstPath, metaClient.getHadoopConf());
