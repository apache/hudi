...
/**
  * A container that can potentially hold one or more table's file-system views. There is one view
  * for each table. This is a view built against a timeline containing completed actions. In an
  * embedded timeline-server mode, this typically holds only one table's view. In a stand-alone
  * server mode, this can hold more than one table's views.
  *
  * <p>FileSystemView can be stored "locally" using the following storage mechanisms: a. In Memory b.
  * Spillable Map c. RocksDB.
  *
  * <p>But there can be cases where the file-system view is managed remoted. For example : Embedded
  * Timeline Server). In this case, the clients will configure a remote filesystem view client
  * (RemoteHoodieTableFileSystemView) for the table which can connect to the remote file system view
  * and fetch views. There are 2 modes here : REMOTE_FIRST and REMOTE_ONLY. REMOTE_FIRST : The
  * file-system view implementation on client side will act as a remote proxy. In case, if there is
  * problem (or exceptions) querying remote file-system view, a backup local file-system view(using
  * either one of in-memory, spillable, rocksDB) is used to server file-system view queries.
  * REMOTE_ONLY : In this case, there is no backup local file-system view. If there is problem (or
  * exceptions) querying remote file-system view, then the exceptions are percolated back to client.
  *
  * <p>FileSystemViewManager is designed to encapsulate the file-system view storage from clients
  * using the file-system view. FileSystemViewManager uses a factory to construct specific
  * implementation of file-system view and passes it to clients for querying.
  */
public class FileSystemViewManager {
    private static final Logger LOG = LoggerFactory.getLogger(FileSystemViewManager.class);

    private static final String HOODIE_METASERVER_FILE_SYSTEM_VIEW_CLASS =
            "org.apache.hudi.common.table.view.HoodieMetaserverFileSystemView";
    ...
    private final Function2<
                    HoodieTableMetaClient, FileSystemViewStorageConfig, SyncableFileSystemView>
            viewCreator;
    ...
    private FileSystemViewManager(
            ...
            Function2<HoodieTableMetaClient, FileSystemViewStorageConfig, SyncableFileSystemView>
                    viewCreator) {
    ...
    public SyncableFileSystemView getFileSystemView(String basePath) {
        return globalViewMap.computeIfAbsent(
                basePath,
                (path) -> {
                    HoodieTableMetaClient metaClient =
                            HoodieTableMetaClient.builder().setConf(conf.newInstance()).setBasePath(path).build();
    ...
    /**
      ...
      */
    private static RocksDbBasedFileSystemView createRocksDBBasedFileSystemView(
    ...
    private static HoodieTableFileSystemView createInMemoryFileSystemView(
        ...
        if (metaClient.getTableConfig().isMetadataTableAvailable()) {
            ValidationUtils.checkArgument(
                    ...
                    "Metadata supplier is null. Cannot instantiate metadata file system view");
            return new HoodieMetadataFileSystemView(
                    metaClient, timeline, metadataCreator.apply(metaClient));
        }
        if (metaClient.getMetaserverConfig().isMetaserverEnabled()) {
            return (HoodieTableFileSystemView)
                    ReflectionUtils.loadClass(
                            HOODIE_METASERVER_FILE_SYSTEM_VIEW_CLASS,
                            new Class<?>[] {
                                HoodieTableMetaClient.class, HoodieTimeline.class, HoodieMetaserverConfig.class
                            },
                            metaClient,
                            timeline,
        ...
        return new HoodieTableFileSystemView(
                metaClient, timeline, viewConf.isIncrementalTimelineSyncEnabled());
    }
    ...
    public static HoodieTableFileSystemView createInMemoryFileSystemView(
        ...
        return createInMemoryFileSystemViewWithTimeline(
                engineContext,
                metaClient,
                metadataConfig,
        ...
        if (metaClient.getMetaserverConfig().isMetaserverEnabled()) {
            return (HoodieTableFileSystemView)
                    ReflectionUtils.loadClass(
                            HOODIE_METASERVER_FILE_SYSTEM_VIEW_CLASS,
                            new Class<?>[] {
                                HoodieTableMetaClient.class, HoodieTimeline.class, HoodieMetadataConfig.class
                            },
                            metaClient,
                            timeline,
                            metaClient.getMetaserverConfig());
    ...
    private static RemoteHoodieTableFileSystemView createRemoteFileSystemView(
            FileSystemViewStorageConfig viewConf, HoodieTableMetaClient metaClient) {
        LOG.info(
                "Creating remote view for basePath {}. Server={}:{}, Timeout={}",
    ...
    public static FileSystemViewManager createViewManager(
            ...
            final FileSystemViewStorageConfig config,
            final HoodieCommonConfig commonConfig,
            final SerializableFunctionUnchecked<HoodieTableMetaClient, HoodieTableMetadata>
                    metadataCreator) {
        ...
        switch (config.getStorageType()) {
            case EMBEDDED_KV_STORE:
                ...
                return new FileSystemViewManager(
                        context,
                        config,
            ...
            case REMOTE_ONLY:
                ...
                return new FileSystemViewManager(
                        context,
                        config,
                        (metaClient, viewConfig) -> createRemoteFileSystemView(viewConfig, metaClient));
            case REMOTE_FIRST:
                ...
                return new FileSystemViewManager(
                        ...
                        (metaClient, viewConfig) -> {
                            ...
                            SerializableSupplier<SyncableFileSystemView> secondaryViewSupplier =
                                    new SecondaryViewSupplier(viewConfig, metaClient, commonConfig, metadataCreator);
                            return new PriorityBasedFileSystemView(remoteFileSystemView, secondaryViewSupplier);
                        });
            default:
                throw new IllegalArgumentException(
                        "Unknown file system view type :" + config.getStorageType());
        }
    }
    ...
    private static class SecondaryViewSupplier
        ...
        private final SerializableFunctionUnchecked<HoodieTableMetaClient, HoodieTableMetadata>
                metadataCreator;

        private SecondaryViewSupplier(
        ...
        public SyncableFileSystemView get() {
            switch (viewConfig.getSecondaryStorageType()) {
                ...
                default:
                    throw new IllegalArgumentException(
                            "Secondary Storage type can only be in-memory or spillable. Was :"
