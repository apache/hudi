...
public abstract class HoodieLogBlock {
    ...
    /**
      ...
      * operations there Current log block version is V3.
      */
    ...
    /**
      * Compacted blocks are created using log compaction which basically merges the consecutive blocks
      * together and create huge block with all the changes.
      */
    ...
    /**
      * @return A {@link Roaring64NavigableMap} bitmap containing the record positions in long type if
      *     the {@link HeaderMetadataType#RECORD_POSITIONS} block header exists; otherwise, an empty
    ...
    public Roaring64NavigableMap getRecordPositions() throws IOException {
        ...
        return LogReaderUtils.decodeRecordPositionsHeader(
                logBlockHeader.get(HeaderMetadataType.RECORD_POSITIONS));
    }
    ...
    protected void addRecordPositionsToHeader(Set<Long> positionSet, int numRecords) {
        if (positionSet.size() == numRecords) {
            try {
                logBlockHeader.put(
                        HeaderMetadataType.RECORD_POSITIONS, LogReaderUtils.encodePositions(positionSet));
            } catch (IOException e) {
                LOG.error("Cannot write record positions to the log block header.", e);
            }
        } else {
            LOG.warn(
                    "There are duplicate keys in the records (number of unique positions: %s, "
    ...
    public static final class HoodieLogBlockContentLocation {
        ...
        public HoodieLogBlockContentLocation(
    ...
    /**
      ...
      */
    public static Map<HeaderMetadataType, String> getLogMetadata(SeekableDataInputStream dis)
            throws IOException {
    ...
    /**
      ...
      */
    public static Option<byte[]> tryReadContent(
            SeekableDataInputStream inputStream, Integer contentLength, boolean readLazily)
            throws IOException {
        if (readLazily) {
    ...
    protected void inflate() throws HoodieIOException {
        ...
        try (SeekableDataInputStream inputStream = inputStreamSupplier.get()) {
            ...
            // because the inputstream might close/timeout for large number of log blocks to be
            // merged
            inflate();
        }
    }
