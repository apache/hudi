...
public class HoodieInstantTimeGenerator {
    ...
    private static DateTimeFormatter MILLIS_INSTANT_TIME_FORMATTER =
    ...
    private static DateTimeFormatter MILLIS_GRANULARITY_DATE_FORMATTER =
            DateTimeFormatter.ofPattern(MILLIS_GRANULARITY_DATE_FORMAT);

    // The last Instant timestamp generated
    private static AtomicReference<String> lastInstantTime =
            new AtomicReference<>(String.valueOf(Integer.MIN_VALUE));
    ...
    public static String createNewInstantTime(
        ...
        return lastInstantTime.updateAndGet(
                (oldVal) -> {
                    ...
                    do {
                        ...
                        if (commitTimeZone.equals(HoodieTimelineTimeZone.UTC)) {
                            newCommitTime =
                                    d.toInstant()
                                            .atZone(HoodieTimelineTimeZone.UTC.getZoneId())
                                            .toLocalDateTime()
                                            .format(MILLIS_INSTANT_TIME_FORMATTER);
                        } else {
                            newCommitTime =
                                    MILLIS_INSTANT_TIME_FORMATTER.format(convertDateToTemporalAccessor(d));
                        }
                    } while (HoodieTimeline.compareTimestamps(
                            newCommitTime, HoodieActiveTimeline.LESSER_THAN_OR_EQUALS, oldVal));
    ...
    public static String instantTimePlusMillis(String timestamp, long milliseconds) {
        ...
        try {
            LocalDateTime dt = LocalDateTime.parse(timestampInMillis, MILLIS_INSTANT_TIME_FORMATTER);
            ZoneId zoneId =
                    HoodieTimelineTimeZone.UTC.equals(commitTimeZone)
                            ? ZoneId.of("UTC")
                            : ZoneId.systemDefault();
            return MILLIS_INSTANT_TIME_FORMATTER.format(
                    dt.atZone(zoneId).toInstant().plusMillis(milliseconds).atZone(zoneId).toLocalDateTime());
        } catch (DateTimeParseException e) {
            // To work with tests, that generate arbitrary timestamps, we need to pad the timestamp
            // with 0s.
            if (isValidInstantTime(timestamp)) {
                return String.format(
                        "%0" + MILLIS_INSTANT_TIMESTAMP_FORMAT_LENGTH + "d",
                        Long.parseLong(timestamp) + milliseconds);
            } else {
        ...
        try {
            ...
            ZoneId zoneId =
                    HoodieTimelineTimeZone.UTC.equals(commitTimeZone)
                            ...
                            : ZoneId.systemDefault();
            return MILLIS_INSTANT_TIME_FORMATTER.format(
                    dt.atZone(zoneId).toInstant().minusMillis(milliseconds).atZone(zoneId).toLocalDateTime());
        } catch (DateTimeParseException e) {
            // To work with tests, that generate arbitrary timestamps, we need to pad the timestamp
            // with 0s.
            if (isValidInstantTime(timestamp)) {
    ...
    public static String fixInstantTimeCompatibility(String instantTime) {
        ...
        if (isSecondGranularity(instantTime)) {
            ...
            // compaction and cleaning in metadata has special format. handling it by trimming extra
            // chars and treating it with ms granularity
    ...
    public static String getInstantForDateString(String dateString) {
        try {
            return getInstantFromTemporalAccessor(
                    LocalDateTime.parse(dateString, MILLIS_GRANULARITY_DATE_FORMATTER));
        } catch (Exception e) {
