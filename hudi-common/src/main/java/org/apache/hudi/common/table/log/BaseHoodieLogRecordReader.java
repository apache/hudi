...
/**
  * A new abstract log record reader, returning records in the type of engine-specific record
  * representation.
  *
...
public abstract class BaseHoodieLogRecordReader<T> {
    ...
    protected BaseHoodieLogRecordReader(
        ...
        this.hoodieTableMetaClient =
                HoodieTableMetaClient.builder()
                        .setStorage(storage)
                        .setBasePath(readerContext.getTablePath())
                        .build();
        ...
        if (keyFieldOverride.isPresent()) {
            ...
            //         - Override partition-path value w/ static "partition-name" (in MT all
            // partitions
    ...
    private void scanInternalV1(Option<KeySpec> keySpecOpt) {
        ...
        try {
            ...
            logFormatReaderWrapper =
                    new HoodieLogFormatReverseReader(
                            storage,
                            logFilePaths.stream()
            ...
            while (logFormatReaderWrapper.hasNext()) {
                ...
                switch (logBlock.getBlockType()) {
                    ...
                    case PARQUET_DATA_BLOCK:
                        LOG.info(
                                "Reading a data block from file {} at instant {}", logFile.getPath(), instantTime);
                        // store the current block
                        currentInstantLogBlocks.push(logBlock);
                        validLogBlockInstants.add(logBlock);
                        updateBlockSequenceTracker(
                                logBlock, instantTime, blockSeqNo, attemptNo, blockSequenceMapPerCommit);
                        break;
                    case DELETE_BLOCK:
                    ...
                    case COMMAND_BLOCK:
                        ...
                        // (Time 1, C1, Task T1) -> Failed (Wrote either a corrupt block or a
                        // correct
                        // DataBlock (B1) with commitTime C1
                        // (Time 2, C1, Task T1.2) -> Running (Task T1 was retried and the attempt
                        // number is 2)
                        // (Time 3, C1, Task T1.2) -> Finished (Wrote a correct DataBlock B2)
                        // Now a logFile L1 can have 2 correct Datablocks (B1 and B2) which are the
                        // same.
                        // Say, commit C1 eventually failed and a rollback is triggered.
                        // Rollback will write only 1 rollback block (R1) since it assumes one block
                        // is
                        // written per ingestion batch for a file but in reality we need to rollback
                        // (B1 & B2)
                        // The following code ensures the same rollback block (R1) is used to
                        // rollback
                        // both B1 & B2
                        ...
                        LOG.info(
                                "Reading a command block {} with targetInstantTime {} from file {}",
                        ...
                        switch (commandBlock.getType()) { // there can be different types of command blocks
                            case ROLLBACK_BLOCK:
                                ...
                                // operation itself
                                // and ensures the same rollback block (R1) is used to rollback both
                                // B1 & B2 with
                                // same instant_time.
                                final int instantLogBlockSizeBeforeRollback = currentInstantLogBlocks.size();
                                currentInstantLogBlocks.removeIf(
                                        block -> {
                                            // handle corrupt blocks separately since they may not
                                            // have metadata
                                            if (block.getBlockType() == CORRUPT_BLOCK) {
                                                LOG.info(
                                                        "Rolling back the last corrupted log block read in {}",
                                                        logFile.getPath());
                                                return true;
                                            }
                                ...
                                validLogBlockInstants =
                                        validLogBlockInstants.stream()
                                                .filter(
                                                        block -> {
                                                            // handle corrupt blocks separately
                                                            // since they may not have metadata
                                                            if (block.getBlockType() == CORRUPT_BLOCK) {
                                                                LOG.info(
                                                                        "Rolling back the last corrupted log block read in {}",
                                                                        logFile.getPath());
                                ...
                                if (numBlocksRolledBack == 0) {
                                    LOG.warn(
                                            "TargetInstantTime {} invalid or extra rollback command block in {}",
                                            targetInstantForCommandBlock,
                                            logFile.getPath());
                                }
                                break;
                    ...
                    case CORRUPT_BLOCK:
                        ...
                        // If there is a corrupt block - we will assume that this was the next data
                        // block
                        currentInstantLogBlocks.push(logBlock);
                        validLogBlockInstants.add(logBlock);
                        // we don't need to update the block sequence tracker here, since the block
                        // sequence tracker is meant to remove additional/spurious valid logblocks.
            ...
            if (!currentInstantLogBlocks.isEmpty()) {
                Pair<Boolean, List<HoodieLogBlock>> dedupedLogBlocksInfo =
                        reconcileSpuriousBlocksAndGetValidOnes(
                                validLogBlockInstants, blockSequenceMapPerCommit);
                if (dedupedLogBlocksInfo.getKey()) {
                    // if there are duplicate log blocks that needs to be removed, we re-create the
                    // queue for valid log blocks from dedupedLogBlocks
                    currentInstantLogBlocks = new ArrayDeque<>();
                    dedupedLogBlocksInfo.getValue().forEach(block -> currentInstantLogBlocks.push(block));
                    LOG.info("Merging the final data blocks");
                    processQueuedBlocksForInstant(
                            currentInstantLogBlocks, scannedLogFiles.size(), keySpecOpt);
                } else {
    ...
    /**
      * There could be spurious log blocks due to spark task retries. So, we will use
      * BLOCK_SEQUENCE_NUMBER in the log block header to deduce such spurious log blocks and return a
      * deduped set of log blocks.
      *
      ...
      * @return a Pair of boolean and list of deduped valid block blocks, where boolean of true means,
      *     there have been dups detected.
      */
    private Pair<Boolean, List<HoodieLogBlock>> reconcileSpuriousBlocksAndGetValidOnes(
            List<HoodieLogBlock> allValidLogBlocks,
            Map<String, Map<Long, List<Pair<Integer, HoodieLogBlock>>>> blockSequenceMapPerCommit) {

        boolean dupsFound =
                blockSequenceMapPerCommit.values().stream()
                        .anyMatch(perCommitBlockList -> perCommitBlockList.size() > 1);
        if (dupsFound) {
            // duplicates are found. we need to remove duplicate log blocks.
            for (Map.Entry<String, Map<Long, List<Pair<Integer, HoodieLogBlock>>>> entry :
                ...
                if (perCommitBlockSequences.size() > 1) {
                    ...
                    int counter = 0;
                    for (Map.Entry<Long, List<Pair<Integer, HoodieLogBlock>>> perAttemptEntries :
                            perCommitBlockSequences.entrySet()) {
                    ...
                    // for other sequence (!= maxSequenceIndex), we need to remove the corresponding
                    // logBlocks from allValidLogBlocks
                    for (Map.Entry<Long, List<Pair<Integer, HoodieLogBlock>>> perAttemptEntries :
                            perCommitBlockSequences.entrySet()) {
                        Long attemptNo = perAttemptEntries.getKey();
                        if (maxAttemptNo != attemptNo) {
                            List<HoodieLogBlock> logBlocksToRemove =
                                    perCommitBlockSequences.get(attemptNo).stream()
                                            .map(Pair::getValue)
                                            .collect(Collectors.toList());
                            logBlocksToRemove.forEach(
                                    logBlockToRemove -> allValidLogBlocks.remove(logBlocksToRemove));
                        }
                    }
                }
            }
    ...
    /**
      * Updates map tracking block seq no. Here is the map structure. Map<String, Map<Long,
      * List<Pair<Integer, HoodieLogBlock>>>> blockSequenceMapPerCommit Key: Commit time. Value:
      * Map<Long, List<Pair<Integer, HoodieLogBlock>>>> Value refers to a Map of different attempts for
      * the commit of interest. List contains the block seq number and the resp HoodieLogBlock.
      *
      * <p>For eg, if there were two attempts for a file slice while writing(due to spark task
      * retries), here is how the map might look like key: commit1 value : { 0L = List = { {0, lb1},
      * {1, lb2} }, 1L = List = { {0, lb3}, {1, lb4}, {2, lb5}} } Meaning: for commit1, there was two
      * attempts with Append Handle while writing. In first attempt, lb1 and lb2 was added. And in 2nd
      * attempt lb3, lb4 and lb5 was added. We keep populating this entire map and finally detect
      * spurious log blocks and ignore them. In most cases, we might just see one set of sequence for a
      * given commit.
      *
      ...
      */
    private void updateBlockSequenceTracker(
            ...
            Map<String, Map<Long, List<Pair<Integer, HoodieLogBlock>>>> blockSequenceMapPerCommit) {
        if (blockSeqNo != -1
                && attemptNo != -1) { // update the block sequence tracker for log blocks containing the
            // same.
            ...
            // all of older blocks are considered valid. there should be only one list for older
            // commits where block sequence number is not present.
            blockSequenceMapPerCommit.computeIfAbsent(instantTime, entry -> new HashMap<>());
            Map<Long, List<Pair<Integer, HoodieLogBlock>>> curCommitBlockMap =
                    blockSequenceMapPerCommit.get(instantTime);
    ...
    private void scanInternalV2(Option<KeySpec> keySpecOption, boolean skipProcessingBlocks) {
        ...
        try {
            ...
            /**
              * Scanning log blocks and placing the compacted blocks at the right place require two
              * traversals. First traversal to identify the rollback blocks and valid data and compacted
              * blocks.
              *
              * <p>Scanning blocks is easy to do in single writer mode, where the rollback block is right
              * after the effected data blocks. With multi-writer mode the blocks can be out of sync. An
              * example scenario. B1, B2, B3, B4, R1(B3), B5 In this case, rollback block R1 is
              * invalidating the B3 which is not the previous block. This becomes more complicated if we
              * have compacted blocks, which are data blocks created using log compaction.
              *
              * <p>To solve this, run a single traversal, collect all the valid blocks that are not
              * corrupted along with the block instant times and rollback block's target instant times.
              *
              * <p>As part of second traversal iterate block instant times in reverse order. While
              * iterating in reverse order keep a track of final compacted instant times for each block. In
              * doing so, when a data block is seen include the final compacted block if it is not already
              * added.
              *
              * <p>find the final compacted block which contains the merged contents. For example B1 and B2
              * are merged and created a compacted block called M1 and now M1, B3 and B4 are merged and
              * created another compacted block called M2. So, now M2 is the final block which contains all
              * the changes of B1,B2,B3,B4. So, blockTimeToCompactionBlockTimeMap will look like (B1 ->
              * M2), (B2 -> M2), (B3 -> M2), (B4 -> M2), (M1 -> M2) This map is updated while iterating and
              * is used to place the compacted blocks in the correct position. This way we can have
              * multiple layers of merge blocks and still be able to find the correct positions of merged
              * blocks.
              */

            // Collect targetRollbackInstants, using which we can determine which blocks are
            // invalid.
            Set<String> targetRollbackInstants = new HashSet<>();

            // This holds block instant time to list of blocks. Note here the log blocks can be
            // normal data blocks or compacted log blocks.
            ...
            while (logFormatReaderWrapper.hasNext()) {
                ...
                switch (logBlock.getBlockType()) {
                    ...
                    case COMMAND_BLOCK:
                        ...
                        // Rollback blocks contain information of instants that are failed, collect
                        // them in a set..
                        if (commandBlock
                                .getType()
                                .equals(HoodieCommandBlock.HoodieCommandBlockTypeEnum.ROLLBACK_BLOCK)) {
            ...
            // Key will have details related to instant time and value will be empty if that instant
            // is not compacted.
            ...
            for (int i = orderedInstantsList.size() - 1; i >= 0; i--) {
                ...
                if (instantsBlocks.isEmpty()) {
                    throw new HoodieException(
                            "Data corrupted while writing. Found zero blocks for an instant " + instantTime);
                }
                ...
                if (firstBlock.getLogBlockHeader().containsKey(COMPACTED_BLOCK_TIMES)) {
                    ...
                    Arrays.stream(firstBlock.getLogBlockHeader().get(COMPACTED_BLOCK_TIMES).split(","))
                            .forEach(
                                    originalInstant -> {
                                        String finalInstant =
                                                blockTimeToCompactionBlockTimeMap.getOrDefault(instantTime, instantTime);
                    ...
                    if (compactedFinalInstantTime == null) {
                        // If it is not compacted then add the blocks related to the instant time at
                        // the end of the queue and continue.
                    ...
                    // If the compacted block exists and it is already included in the dequeue then
                    // ignore and continue.
                    if (instantTimesIncluded.contains(compactedFinalInstantTime)) {
                        continue;
                    }
                    // If the compacted block exists and it is not already added then add all the
                    // blocks related to that instant time.
            ...
            if (LOG.isDebugEnabled()) {
                LOG.info(
                        "Final view of the Block time to compactionBlockMap {}",
            ...
            if (!currentInstantLogBlocks.isEmpty() && !skipProcessingBlocks) {
                LOG.info("Merging the final data blocks");
                processQueuedBlocksForInstant(
                        currentInstantLogBlocks, scannedLogFiles.size(), keySpecOption);
            }
            // Done
            progress = 1.0f;
        } catch (IOException e) {
    ...
    private void processQueuedBlocksForInstant(
            Deque<HoodieLogBlock> logBlocks, int numLogFilesSeen, Option<KeySpec> keySpecOpt)
            throws Exception {
        ...
        // At this step the lastBlocks are consumed. We track approximate progress by number of
        // log-files seen
    ...
    private Pair<ClosableIterator<T>, Schema> getRecordsIterator(
        ...
        if (keySpecOpt.isPresent()) {
            ...
            blockRecordsIterator =
                    dataBlock.getEngineRecordIterator(readerContext, keySpec.getKeys(), keySpec.isFullKey());
        } else {
