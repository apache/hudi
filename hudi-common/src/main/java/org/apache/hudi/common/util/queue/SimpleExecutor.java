...
/**
  ...
  * single-writer/single-reader mode allowing it to consume from the input {@link Iterator} directly
  * avoiding the need for any internal materialization (ie queueing).
  *
  * <p>Such executor is aimed primarily at allowing the production-consumption chain to run w/ as
  * little overhead as possible, at the expense of limited parallelism and therefore throughput,
  * which is not an issue for execution environments such as Spark, where it's used primarily in a
  * parallelism constraint environment (on executors)
  */
public class SimpleExecutor<I, O, E> implements HoodieExecutor<E> {
    ...
    public SimpleExecutor(
            Iterator<I> inputItr, HoodieConsumer<O, E> consumer, Function<I, O> transformFunction) {
        this.itr = inputItr;
