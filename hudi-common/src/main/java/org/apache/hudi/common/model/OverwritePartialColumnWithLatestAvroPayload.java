...
public class OverwritePartialColumnWithLatestAvroPayload extends OverwriteWithLatestAvroPayload {
  ...
  public OverwritePartialColumnWithLatestAvroPayload(GenericRecord record, Comparable orderingVal) {
    super(record, orderingVal);
  }

  public OverwritePartialColumnWithLatestAvroPayload(Option<GenericRecord> record) {
  ...
  public Option<IndexedRecord> combineAndGetUpdateValue(IndexedRecord currentValue, Schema schema, Properties properties) throws IOException {
    ...
    List<String> partialUpdateFields = ((TypedProperties) properties).getStringList("hoodie.payload.partial.update.fields",
        ",", Collections.emptyList());
    if (partialUpdateFields.isEmpty()) {
      return recordOption;
    }
    ...
    if (isDeleteRecord(insertRecord)) {
      ...
      fields.forEach(field -> {
        ...
        if (partialUpdateFields.contains(field.name())) {
