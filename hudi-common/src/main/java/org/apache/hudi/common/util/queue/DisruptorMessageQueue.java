...
public class DisruptorMessageQueue<I, O> implements HoodieMessageQueue<I, O> {
    ...
    public DisruptorMessageQueue(
        ...
        CustomizedThreadFactory threadFactory =
                new CustomizedThreadFactory("disruptor", true, preExecuteRunnable);

        this.queue =
                new Disruptor<>(
                        HoodieDisruptorEvent::new,
                        bufferSize,
                        threadFactory,
                        totalProducers > 1 ? ProducerType.MULTI : ProducerType.SINGLE,
                        waitStrategy);
    ...
    public Option<O> readNextRecord() {
        throw new UnsupportedOperationException(
                "Should not call readNextRecord here. And let DisruptorMessageHandler to handle consuming logic");
    }
        ...
        synchronized (this) {
            if (!isShutdown) {
                ...
                if (Thread.currentThread().isInterrupted()) {
                    // if current thread has been interrupted, we still give executor a chance to
                    // proceeding.
