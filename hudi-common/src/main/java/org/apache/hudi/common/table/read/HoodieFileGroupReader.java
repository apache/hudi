...
/**
  ...
  *
  * <p>This should be used by the every engine integration, by plugging in a {@link
  * HoodieReaderContext<T>} implementation.
  *
...
public final class HoodieFileGroupReader<T> implements Closeable {
    ...
    public HoodieFileGroupReader(
        ...
        readerContext.setHasBootstrapBaseFile(
                hoodieBaseFileOption.isPresent()
        ...
        readerContext.setSchemaHandler(
                readerContext.supportsParquetRowIndex()
                        ? new HoodiePositionBasedSchemaHandler<>(
                                readerContext, dataSchema, requestedSchema, internalSchemaOpt, tableConfig)
                        : new HoodieFileGroupReaderSchemaHandler<>(
                                readerContext, dataSchema, requestedSchema, internalSchemaOpt, tableConfig));
    ...
    private ClosableIterator<T> makeBootstrapBaseFileIterator(HoodieBaseFile baseFile)
        ...
        Option<Pair<ClosableIterator<T>, Schema>> dataFileIterator =
                makeBootstrapBaseFileIteratorHelper(
                        requiredFields.getRight(), allFields.getRight(), dataFile);
        Option<Pair<ClosableIterator<T>, Schema>> skeletonFileIterator =
                makeBootstrapBaseFileIteratorHelper(
                        requiredFields.getLeft(), allFields.getLeft(), baseFile);
        if (!dataFileIterator.isPresent() && !skeletonFileIterator.isPresent()) {
            ...
            return readerContext.mergeBootstrapReaders(
                    skeletonFileIterator.get().getLeft(),
                    skeletonFileIterator.get().getRight(),
                    dataFileIterator.get().getLeft(),
                    dataFileIterator.get().getRight());
        }
    }
    ...
    /**
      ...
      */
    private Option<Pair<ClosableIterator<T>, Schema>> makeBootstrapBaseFileIteratorHelper(
            List<Schema.Field> requiredFields, List<Schema.Field> allFields, BaseFile file)
            throws IOException {
    ...
    public static RecordMergeMode getRecordMergeMode(Properties props) {
        String mergeMode =
                getStringWithAltKeys(props, HoodieCommonConfig.RECORD_MERGE_MODE, true).toUpperCase();
