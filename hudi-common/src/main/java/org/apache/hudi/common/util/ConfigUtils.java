...
public class ConfigUtils {
    ...
    @Deprecated public static final String DELTA_STREAMER_CONFIG_PREFIX = "hoodie.deltastreamer.";
    public static final String SCHEMAPROVIDER_CONFIG_PREFIX =
            STREAMER_CONFIG_PREFIX + "schemaprovider.";

    @Deprecated
    public static final String OLD_SCHEMAPROVIDER_CONFIG_PREFIX =
            DELTA_STREAMER_CONFIG_PREFIX + "schemaprovider.";
    /**
      * Config stored in hive serde properties to tell query engine (spark/flink) to read the table as
      * a read-optimized table when this config is true.
      */
    ...
    /**
      * Convert the key-value config to a map. The format of the config is a key-value pair just like
      * "k1=v1\nk2=v2\nk3=v3".
      *
    ...
    /**
      * Convert the key-value config to a map. The format of the config is a key-value pair with
      * defined separator. For example, if the separator is a comma, the input is "k1=v1,k2=v2,k3=v3".
      *
        ...
        for (String keyValue : keyvalues) {
            ...
            if (keyValueArray.length == 1 || keyValueArray.length == 2) {
                ...
                throw new IllegalArgumentException(
                        "Bad key-value config: " + keyValue + ", must be the" + " format 'key = value'");
            }
        }
    ...
    /**
      * Convert map config to key-value string.The format of the config is a key-value pair just like
      * "k1=v1\nk2=v2\nk3=v3".
      *
    ...
    /**
      * Strips the prefix from a config key. The prefix is defined by a {@link ConfigProperty} which
      * can have alternatives. The method strips any matching prefix.
      *
    ...
    /**
      * Whether the properties contain a config. If any of the key or alternative keys of the {@link
      * ConfigProperty} exists in the properties, this method returns {@code true}.
      *
      * @param props Configs in {@link TypedProperties}
      * @param configProperty Config to look up.
      * @return {@code true} if exists; {@code false} otherwise.
      */
    public static boolean containsConfigProperty(
            TypedProperties props, ConfigProperty<?> configProperty) {
    ...
    /**
      ...
      */
    public static boolean containsConfigProperty(
            Map<String, Object> props, ConfigProperty<?> configProperty) {
        if (!props.containsKey(configProperty.key())) {
            for (String alternative : configProperty.getAlternatives()) {
                if (props.containsKey(alternative)) {
                    return true;
    ...
    /**
      * Validates that all {@link ConfigProperty} configs exist in the properties. For each {@link
      * ConfigProperty} config, if any of the key or alternative keys of the {@link ConfigProperty}
      * exists in the properties, the validation of this config passes.
      *
      * @param props Configs in {@link TypedProperties} to validate.
      * @param configPropertyList List of {@link ConfigProperty} configs that must exist.
      */
    public static void checkRequiredConfigProperties(
            TypedProperties props, List<ConfigProperty<?>> configPropertyList) {
        configPropertyList.forEach(
                configProperty -> {
                    if (!containsConfigProperty(props, configProperty)) {
                        throw new HoodieNotSupportedException(
                                "Required property " + configProperty.key() + " is missing");
                    }
                });
    }

    /**
      * Gets the raw value for a {@link ConfigProperty} config from properties. The key and alternative
      * keys are used to fetch the config.
      *
      ...
      */
    public static Option<Object> getRawValueWithAltKeys(
            Properties props, ConfigProperty<?> configProperty) {
        if (props.containsKey(configProperty.key())) {
        ...
        for (String alternative : configProperty.getAlternatives()) {
            if (props.containsKey(alternative)) {
                LOG.warn(
                        String.format(
                                "The configuration key '%s' has been deprecated "
    ...
    /**
      ...
      * alternative keys are used to fetch the config. If the config is not found, an {@link
      ...
      */
    public static String getStringWithAltKeys(
            Properties props, ConfigProperty<String> configProperty) {
        return getStringWithAltKeys(props, configProperty, false);
    }

    /**
      * Gets the String value for a {@link ConfigProperty} config from properties. The key and
      * alternative keys are used to fetch the config. If using default value, the default value of
      * {@link ConfigProperty} config, if exists, is returned if the config is not found in the
      * properties. If not using default value, if the config is not found, an {@link
      * IllegalArgumentException} is thrown.
      *
      * @param props Configs in {@link Properties}.
      * @param configProperty {@link ConfigProperty} config of String type to fetch.
      * @param useDefaultValue Whether to use default value from {@link ConfigProperty}.
      * @return String value if the config exists; otherwise, if the config does not exist and {@code
      *     useDefaultValue} is true, returns default String value if there is default value defined in
      *     the {@link ConfigProperty} config and {@code null} otherwise.
      */
    public static String getStringWithAltKeys(
            Properties props, ConfigProperty<String> configProperty, boolean useDefaultValue) {
        if (useDefaultValue) {
            return getStringWithAltKeys(
                    props,
                    configProperty,
                    configProperty.hasDefaultValue() ? configProperty.defaultValue() : null);
        }
    ...
    /**
      * Gets the String value for a {@link ConfigProperty} config from properties. The key and
      * alternative keys are used to fetch the config. The default value as the input of the method is
      * returned if the config is not found in the properties.
      *
      * @param props Configs in {@link Properties}.
      * @param configProperty {@link ConfigProperty} config of String type to fetch.
      ...
      */
    public static String getStringWithAltKeys(
            Properties props, ConfigProperty<?> configProperty, String defaultValue) {
        Option<Object> rawValue = getRawValueWithAltKeys(props, configProperty);
        return rawValue.map(Object::toString).orElse(defaultValue);
    }

    /**
      * Gets the String value for a {@link ConfigProperty} config from a {@link Map}. The key and
      ...
      *     and there is default value defined in the {@link ConfigProperty} config; {@code null}
      *     otherwise.
      */
    public static String getStringWithAltKeys(
            Map<String, Object> props, ConfigProperty<String> configProperty) {
    ...
    /**
      ...
      */
    public static String getStringWithAltKeys(
            TypedProperties props, String key, String altKey, String defaultValue) {
    ...
    public static boolean getBooleanWithAltKeys(Properties props, ConfigProperty<?> configProperty) {
        ...
        boolean defaultValue =
                configProperty.hasDefaultValue()
    ...
    /**
      ...
      *     and there is default value defined in the {@link ConfigProperty} config; {@code 0}
      *     otherwise.
      */
    ...
    /**
      ...
      * @param props Configs in {@link TypedProperties}.
      * @param configProperty {@link ConfigProperty} config to fetch.
      * @return long value if the config exists; default long value if the config does not exist and
      *     there is default value defined in the {@link ConfigProperty} config; {@code 0} otherwise.
      */
    public static long getLongWithAltKeys(
            TypedProperties props, ConfigProperty<Long> configProperty) {
        Option<Object> rawValue = getRawValueWithAltKeys(props, configProperty);
    ...
    /**
      ...
      */
    public static void removeConfigFromProps(
            TypedProperties props, ConfigProperty<?> configProperty) {
    ...
    /**
      ...
      */
    public static Map<String, Object> filterProperties(
            TypedProperties props, List<ConfigProperty<String>> configPropertyList) {
    ...
    public static Set<String> getAllConfigKeys(List<ConfigProperty<String>> configPropertyList) {
        return configPropertyList.stream()
                .flatMap(
                        configProperty -> {
    ...
    public static TypedProperties fetchConfigs(
        ...
        while (readRetryCount++ < maxReadRetries) {
            ...
            // Failed to read all files so wait before retrying. This can happen in cases of
            // parallel updates to the properties.
            try {
        ...
        // If we are here then after all retries either no properties file was found or only an
        // invalid file was found.
        if (found) {
            throw new IllegalArgumentException(
    ...
    public static void recoverIfNeeded(
            HoodieStorage storage, StoragePath cfgPath, StoragePath backupCfgPath) throws IOException {
        ...
        config.setValue(
                ...
                Boolean.toString(
                        storageConf.getBoolean(
                                USE_NATIVE_HFILE_READER.key(), USE_NATIVE_HFILE_READER.defaultValue())));
        return config;
    }

    public static TypedProperties loadGlobalProperties() {
        return ((PropertiesConfig)
                        ReflectionUtils.loadClass("org.apache.hudi.common.config.DFSPropertiesConfiguration"))
                .getGlobalProperties();
    }
}
