...
/**
  * Iterable to lazily fetch values spilled to disk. This class uses BufferedRandomAccessFile to
  * randomly access the position of the latest value for a key spilled to disk and returns the
  * result.
  */
public class LazyFileIterable<T, R> implements Iterable<R> {
    ...
    public LazyFileIterable(
            String filePath, Map<T, BitCaskDiskMap.ValueMetadata> map, boolean isCompressionEnabled) {
    ...
    public class LazyFileIterator<T, R> implements ClosableIterator<R> {
        ...
        public LazyFileIterator(String filePath, Map<T, BitCaskDiskMap.ValueMetadata> map)
                throws IOException {
            this.filePath = filePath;
            this.readOnlyFileHandle =
                    new BufferedRandomAccessFile(filePath, "r", BitCaskDiskMap.BUFFER_SIZE);
            readOnlyFileHandle.seek(0);

            // sort the map in increasing order of offset of value so disk seek is only in
            // one(forward) direction
            this.metadataIterator =
                    map.entrySet().stream()
                            .sorted(
                                    (Map.Entry<T, BitCaskDiskMap.ValueMetadata> o1,
                                            Map.Entry<T, BitCaskDiskMap.ValueMetadata> o2) ->
