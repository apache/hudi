...
/**
  * HoodieTimeline is a view of meta-data instants in the hoodie table. Instants are specific points
  * in time represented as HoodieInstant.
  *
  * <p>Timelines are immutable once created and operations create new instance of timelines which
  * filter on the instants and this can be chained.
  *
...
public interface HoodieTimeline extends Serializable {
    ...
    String REQUESTED_LOG_COMPACTION_SUFFIX =
            StringUtils.join(LOG_COMPACTION_ACTION, REQUESTED_EXTENSION);
    String REQUESTED_LOG_COMPACTION_EXTENSION =
            StringUtils.join(".", REQUESTED_LOG_COMPACTION_SUFFIX);
    String INFLIGHT_LOG_COMPACTION_EXTENSION =
            StringUtils.join(".", LOG_COMPACTION_ACTION, INFLIGHT_EXTENSION);
    ...
    /**
      * Filter this timeline to just include the in-flights excluding major and minor compaction
      * instants.
      *
      * @return New instance of HoodieTimeline with just in-flights excluding major and minor
      *     compaction instants
      */
    ...
    /**
      ...
      * A RT filesystem view is constructed with this timeline so that file-slice after pending
      * compaction-requested instant-time is also considered valid. A RT file-system view for reading
      * must then merge the file-slices before and after pending compaction instant so that all
      * delta-commits are read.
      *
      * @return New instance of HoodieTimeline with just completed instants
      */
    ...
    /**
      * Timeline to just include completed commits or all rewrites like compaction, logcompaction and
      * replace actions.
      *
      * @return
      */
    ...
    /**
      * Timeline to just include commits (commit/deltacommit), compaction and replace actions that are
      * completed and contiguous. For example, if timeline is [C0.completed, C1.completed,
      * C2.completed, C3.inflight, C4.completed]. Then, a timeline of [C0.completed, C1.completed,
      * C2.completed] will be returned.
      *
      * @return
      */
    ...
    /**
      * Filter this timeline to just include requested and inflight from both major and minor
      * compaction instants.
      *
      * @return
      */
    ...
    /**
      * ` Create a new Timeline with instants after startTs and before or on endTs by state transition
    ...
    /**
      * @return Get the stream of completed instants in reverse order TODO Change code references to
      *     getInstants() that reverse the instants later on to use this method instead.
      */
    ...
    /**
      * First non-savepoint commit in the active data timeline. Examples: 1. An active data timeline
      * C1, C2, C3, C4, C5 returns C1. 2. If archival is allowed beyond savepoint and let's say C1, C2,
      * C4 have been archived while C3, C5 have been savepointed, then for the data timeline C3,
      * C3_Savepoint, C5, C5_Savepoint, C6, C7 returns C6.
      */
    ...
    BiPredicate<String, String> GREATER_THAN_OR_EQUALS =
    ...
    BiPredicate<String, String> LESSER_THAN_OR_EQUALS =
            (commit1, commit2) -> commit1.compareTo(commit2) <= 0;
    BiPredicate<String, String> LESSER_THAN = (commit1, commit2) -> commit1.compareTo(commit2) < 0;

    static boolean compareTimestamps(
            String commit1, BiPredicate<String, String> predicateToApply, String commit2) {
    ...
    /**
      * Returns the inflight instant corresponding to the instant being passed. Takes care of changes
      * in action names between inflight and completed instants (compaction <=> commit) and
      ...
      */
    static HoodieInstant getInflightInstant(
            final HoodieInstant instant, final HoodieTableMetaClient metaClient) {
                ...
                Option<HoodieInstant> logCompactionInstant =
                        Option.fromJavaOptional(
                                rawActiveTimeline
                                        ...
                                        .filter(
                                                hoodieInstant ->
                                                        hoodieInstant.getTimestamp().equals(instant.getTimestamp())
                                                                && LOG_COMPACTION_ACTION.equals(hoodieInstant.getAction()))
                                        .findFirst());
