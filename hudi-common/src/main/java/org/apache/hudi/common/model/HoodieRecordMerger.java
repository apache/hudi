...
/**
  * HoodieMerge defines how to merge two records. It is a stateless component. It can implement the
  * merging logic of HoodieRecord of different engines and avoid the performance consumption caused
  * by the serialization/deserialization of Avro payload.
  */
...
public interface HoodieRecordMerger extends Serializable {
    ...
    /**
      * This method converges combineAndGetUpdateValue and precombine from HoodiePayload. It'd be
      * associative operation: f(a, f(b, c)) = f(f(a, b), c) (which we can translate as having 3
      * versions A, B, C of the single record, both orders of operations applications have to yield the
      * same result) This method takes only full records for merging.
      */
    ...
    /**
      ...
      *
      * <p>In this case, in the newer record, only "ts" and "price" fields are updated. With the
      * default merging strategy, the newer record updates the older record and the merging result is
      *
      ...
      *
      * <p>In this case, in the older record, only "ts" and "price" fields are updated. In the newer
      ...
      */
    default Option<Pair<HoodieRecord, Schema>> partialMerge(
            HoodieRecord older,
            Schema oldSchema,
            HoodieRecord newer,
            Schema newSchema,
    ...
    /**
      ...
      */
    default boolean shouldFlush(HoodieRecord record, Schema schema, TypedProperties props)
            throws IOException {
        return true;
    }

    /**
      * Merges two records with the same key in full outer merge fashion i.e. all fields from both
      * records are included.
      */
    default List<Pair<HoodieRecord, Schema>> fullOuterMerge(
            HoodieRecord older,
            Schema oldSchema,
            HoodieRecord newer,
            Schema newSchema,
    ...
    /** The record type handled by the current merger. SPARK, AVRO, FLINK */
