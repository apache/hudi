...
public interface HoodieLogFormat {
  ...
  class WriterBuilder {
    ...
    private HoodieLocation parentPath;
    ...
    public WriterBuilder withFs(HoodieStorage storage) {
    ...
    public WriterBuilder onParentPath(HoodieLocation parentPath) {
    ...
    public Writer build() throws IOException {
      ...
      if (bufferSize == null) {
        bufferSize = FSUtils.getDefaultBufferSize((FileSystem) storage.getFileSystem());
      }
      if (replication == null) {
        replication = FSUtils.getDefaultReplication((FileSystem) storage.getFileSystem(), new Path(parentPath.toString()));
      }
      if (sizeThreshold == null) {
        sizeThreshold = DEFAULT_SIZE_THRESHOLD;
      }
      return new HoodieLogFormatWriter(storage, logFile, bufferSize, replication, sizeThreshold, rolloverLogWriteToken);
    }
  }
  ...
  static HoodieLogFormat.Reader newReader(HoodieStorage storage, HoodieLogFile logFile, Schema readerSchema)
      throws IOException {
    return new HoodieLogFileReader(storage, logFile, readerSchema, HoodieLogFileReader.DEFAULT_BUFFER_SIZE, false);
  }

  static HoodieLogFormat.Reader newReader(HoodieStorage storage, HoodieLogFile logFile, Schema readerSchema,
      boolean readBlockLazily, boolean reverseReader) throws IOException {
    return new HoodieLogFileReader(storage, logFile, readerSchema, HoodieLogFileReader.DEFAULT_BUFFER_SIZE, readBlockLazily,
        reverseReader);
  }
