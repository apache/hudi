...
/**
  * An abstraction for pairs of key in type K and value in type V to store the reference and do
  * transformation.
  *
  * @param <K> type of key.
...
public interface HoodiePairData<K, V> extends Serializable {
    ...
    /**
      * Reduces original sequence by de-duplicating the pairs w/ the same key, using provided binary
      * operator {@code combiner}. Returns an instance of {@link HoodiePairData} holding the
      * "de-duplicated" pairs, ie only pairs with unique keys.
      *
    ...
    /**
      ...
      *
      * <p>NOTE: That this returns {@link HoodieData} and not {@link HoodiePairData}
      */
    ...
    /**
      ...
      */
    <L, W> HoodiePairData<L, W> mapToPair(SerializablePairFunction<Pair<K, V>, L, W> mapToPairFunc);

    /**
      * Performs a left outer join of this dataset against {@code other}.
      *
      * <p>For each element (k, v) in this, the resulting {@link HoodiePairData} will either contain
      * all pairs {@code (k, (v, Some(w)))} for every {@code w} in the {@code other}, or the pair
      * {@code (k, (v, None))} if no elements in {@code other} have the pair w/ a key {@code k}
      *
