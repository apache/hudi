...
@EnumDescription(
        "Cleaning policy to be used. The cleaner service deletes older file "
...
public enum HoodieCleaningPolicy {
    @EnumFieldDescription(
            "keeps the last N versions of the file slices written; used "
                    + "when \"hoodie.clean.fileversions.retained\" is explicitly set only.")
    KEEP_LATEST_FILE_VERSIONS,

    @EnumFieldDescription(
            "keeps the file slices written by the last N commits; used "
                    + "when \"hoodie.clean.commits.retained\" is explicitly set only.")
    KEEP_LATEST_COMMITS,

    @EnumFieldDescription(
            "keeps the file slices written in the last N hours based on "
