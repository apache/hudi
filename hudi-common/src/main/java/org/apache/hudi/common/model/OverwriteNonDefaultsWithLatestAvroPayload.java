...
/**
  ...
  *   <li>combineAndGetUpdateValue/getInsertValue - overwrite storage for specified fields that
  *       doesn't equal defaultValue.
  * </ol>
  */
public class OverwriteNonDefaultsWithLatestAvroPayload extends OverwriteWithLatestAvroPayload {
    ...
    /**
      * Merges the given records into one. The fields in {@code baseRecord} has higher priority: it is
      * set up into the merged record if it is not null or equals to the default.
      *
      ...
      */
    protected Option<IndexedRecord> mergeRecords(
            Schema schema, GenericRecord baseRecord, GenericRecord mergedRecord) {
    ...
    protected void setField(
        ...
        value =
                field.schema().getType().equals(Schema.Type.STRING) && value != null
                        ? value.toString()
                        : value;
