...
public class HoodieParquetDataBlock extends HoodieDataBlock {
    ...
    public HoodieParquetDataBlock(
        ...
        super(
                ...
                inputStreamSupplier,
                readBlockLazily,
                Option.of(logBlockContentLocation),
                readerSchema,
                header,
                footer,
                keyField,
    ...
    @Override
    protected byte[] serializeRecords(List<HoodieRecord> records, HoodieStorage storage)
            throws IOException {
        Map<String, String> paramsMap = new HashMap<>();
        paramsMap.put(PARQUET_COMPRESSION_CODEC_NAME.key(), compressionCodecName.get());
        paramsMap.put(
                PARQUET_COMPRESSION_RATIO_FRACTION.key(), String.valueOf(expectedCompressionRatio.get()));
        paramsMap.put(PARQUET_DICTIONARY_ENABLED.key(), String.valueOf(useDictionaryEncoding.get()));
        Schema writerSchema =
                new Schema.Parser()
                        .parse(super.getLogBlockHeader().get(HoodieLogBlock.HeaderMetadataType.SCHEMA));

        return HoodieIOFactory.getIOFactory(storage)
                .getFileFormatUtils(PARQUET)
                .serializeRecordsToLogBlock(
                        storage, records, writerSchema, getSchema(), getKeyFieldName(), paramsMap);
    }

    /**
      * NOTE: We're overriding the whole reading sequence to make sure we properly respect the
      * requested Reader's schema and only fetch the columns that have been explicitly requested by the
      * caller (providing projected Reader's schema)
      */
    @Override
    protected <T> ClosableIterator<HoodieRecord<T>> readRecordsFromBlockPayload(HoodieRecordType type)
            throws IOException {
        ...
        StoragePath inlineLogFilePath =
                InLineFSUtils.getInlineFilePath(
        ...
        HoodieStorage inlineStorage =
                getBlockContentLocation().get().getStorage().newInstance(inlineLogFilePath, inlineConf);
        Schema writerSchema =
                new Schema.Parser().parse(this.getLogBlockHeader().get(HeaderMetadataType.SCHEMA));

        ClosableIterator<HoodieRecord<T>> iterator =
                HoodieIOFactory.getIOFactory(inlineStorage)
                        .getReaderFactory(type)
                        .getFileReader(
                                DEFAULT_HUDI_CONFIG_FOR_READER, inlineLogFilePath, PARQUET, Option.empty())
                        .getRecordIterator(writerSchema, readerSchema);
        return iterator;
    }

    @Override
    protected <T> ClosableIterator<T> readRecordsFromBlockPayload(
        ...
        StoragePath inlineLogFilePath =
                InLineFSUtils.getInlineFilePath(
                        ...
                        blockContentLoc.getBlockSize());
        HoodieStorage inlineStorage =
                blockContentLoc.getStorage().newInstance(inlineLogFilePath, inlineConf);
    ...
    protected <T> ClosableIterator<HoodieRecord<T>> deserializeRecords(
            byte[] content, HoodieRecordType type) throws IOException {
        throw new UnsupportedOperationException("Should not be invoked");
    }

    @Override
    protected <T> ClosableIterator<T> deserializeRecords(
            HoodieReaderContext<T> readerContext, byte[] content) throws IOException {
        throw new UnsupportedOperationException("Should not be invoked");
    }
}
