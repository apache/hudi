...
public class ClusteringUtils {
    ...
    public static Stream<Pair<HoodieInstant, HoodieClusteringPlan>> getAllPendingClusteringPlans(
        ...
        return pendingReplaceInstants.stream()
                .map(instant -> getClusteringPlan(metaClient, instant))
                .filter(Option::isPresent)
                .map(Option::get);
    }

    /**
      * Checks if the requested, inflight, or completed instant of replacecommit action is a clustering
      * operation, by checking whether the requested instant contains a clustering plan.
      *
    ...
    /**
      ...
      * @param pendingReplaceInstant can be in any state, because it will always be converted to
      *     requested state
      * @return option of the replace metadata if present, else empty
      * @throws IOException
      */
    private static Option<HoodieRequestedReplaceMetadata> getRequestedReplaceMetadata(
            HoodieTimeline timeline, HoodieInstant pendingReplaceInstant) throws IOException {
        ...
        if (!pendingReplaceInstant.isRequested()) {
            ...
            // This is because replacecommit inflight can have workload profile for
            // 'insert_overwrite'.
        ...
        if (!content.isPresent() || content.get().length == 0) {
            // few operations create requested file without any content. Assume these are not
            // clustering
            return Option.empty();
        }
        ...
        try {
            ...
            if (requestedReplaceMetadata.isPresent()
                    && WriteOperationType.CLUSTER
                            .name()
                            .equals(requestedReplaceMetadata.get().getOperationType())) {
                return Option.of(
                        Pair.of(pendingReplaceInstant, requestedReplaceMetadata.get().getClusteringPlan()));
            }
            return Option.empty();
        } catch (IOException e) {
            throw new HoodieIOException(
                    "Error reading clustering plan " + pendingReplaceInstant.getTimestamp(), e);
        }
    }

    /**
      * Get filegroups to pending clustering instant mapping for all pending clustering plans. This
      * includes all clustering operations in 'requested' and 'inflight' states.
      */
    public static Map<HoodieFileGroupId, HoodieInstant> getAllFileGroupsInPendingClusteringPlans(
            HoodieTableMetaClient metaClient) {
        Stream<Pair<HoodieInstant, HoodieClusteringPlan>> pendingClusteringPlans =
                getAllPendingClusteringPlans(metaClient);
        Stream<Map.Entry<HoodieFileGroupId, HoodieInstant>> resultStream =
                pendingClusteringPlans.flatMap(
                        clusteringPlan ->
                                // get all filegroups in the plan
                                getFileGroupEntriesInClusteringPlan(
                                        clusteringPlan.getLeft(), clusteringPlan.getRight()));
        ...
        try {
            ...
            if (e instanceof IllegalStateException && e.getMessage().contains("Duplicate key")) {
                throw new HoodieException(
                        "Found duplicate file groups pending clustering. If you're running deltastreamer in continuous mode, consider adding delay using --min-sync-interval-seconds. "
                                + "Or consider setting write concurrency mode to OPTIMISTIC_CONCURRENCY_CONTROL.",
                        e);
            }
    ...
    public static Stream<Pair<HoodieFileGroupId, HoodieInstant>>
        ...
        Stream<HoodieFileGroupId> partitionToFileIdLists =
                clusteringPlan.getInputGroups().stream()
                        .flatMap(ClusteringUtils::getFileGroupsFromClusteringGroup);
    ...
    private static Stream<Map.Entry<HoodieFileGroupId, HoodieInstant>>
            getFileGroupEntriesInClusteringPlan(
                    HoodieInstant instant, HoodieClusteringPlan clusteringPlan) {
        return getFileGroupsInPendingClusteringInstant(instant, clusteringPlan)
                .map(entry -> new AbstractMap.SimpleEntry<>(entry.getLeft(), entry.getRight()));
    }

    public static Stream<HoodieFileGroupId> getFileGroupsFromClusteringPlan(
            HoodieClusteringPlan clusteringPlan) {
        return clusteringPlan.getInputGroups().stream()
                .flatMap(ClusteringUtils::getFileGroupsFromClusteringGroup);
    }

    public static Stream<HoodieFileGroupId> getFileGroupsFromClusteringGroup(
            HoodieClusteringGroup group) {
        return group.getSlices().stream()
                .map(slice -> new HoodieFileGroupId(slice.getPartitionPath(), slice.getFileId()));
    }

    /** Create clustering plan from input fileSliceGroups. */
    public static HoodieClusteringPlan createClusteringPlan(
            String strategyClassName,
            Map<String, String> strategyParams,
            List<FileSlice>[] fileSliceGroups,
            Map<String, String> extraMetadata) {
        List<HoodieClusteringGroup> clusteringGroups =
                Arrays.stream(fileSliceGroups)
                        .map(
                                fileSliceGroup -> {
        ...
        HoodieClusteringStrategy strategy =
                HoodieClusteringStrategy.newBuilder()
                        .setStrategyClassName(strategyClassName)
                                ...
                                HoodieSliceInfo.newBuilder()
                                        ...
                                        .setBootstrapFilePath(
                                                slice
                                                        .getBaseFile()
                                                        .map(
                                                                bf -> bf.getBootstrapBaseFile().map(BaseFile::getPath).orElse(null))
                                                        .orElse(null))
                                        .build())
                .collect(Collectors.toList());
    }
    ...
    private static Map<String, Double> buildMetrics(List<FileSlice> fileSlices) {
        ...
        for (FileSlice slice : fileSlices) {
            ...
            totalIORead =
                    FSUtils.getSizeInMB(
                            (slice.getBaseFile().isPresent() ? slice.getBaseFile().get().getFileSize() : 0L)
                                    + totalLogFileSize);
        }
    ...
    public static List<HoodieInstant> getPendingClusteringInstantTimes(
            HoodieTableMetaClient metaClient) {
        return metaClient
                .getActiveTimeline()
    ...
    /**
      * Returns the earliest instant to retain. Make sure the clustering instant won't be archived
      * before cleaned, and the earliest inflight clustering instant has a previous commit.
      *
    ...
    public static Option<HoodieInstant> getEarliestInstantToRetainForClustering(
        ...
        HoodieTimeline replaceTimeline =
                activeTimeline.getTimelineOfActions(
                        CollectionUtils.createSet(HoodieTimeline.REPLACE_COMMIT_ACTION));
        if (!replaceTimeline.empty()) {
            ...
            if (cleanInstantOpt.isPresent()) {
                // The first clustering instant of which timestamp is greater than or equal to the
                // earliest commit to retain of
                // the clean metadata.
                HoodieInstant cleanInstant = cleanInstantOpt.get();
                HoodieActionInstant earliestInstantToRetain =
                        CleanerUtils.getCleanerPlan(
                                        metaClient,
                ...
                if (earliestInstantToRetain != null
                    ...
                    // maintained for active file groups
                    // not for replaced file groups. So, last clean instant can be considered as a
                    // lower bound, since
                    // the cleaner would have removed all the file groups until then. But there is a
                    // catch to this logic,
                    // while cleaner is running if there is a pending replacecommit then those files
                    // are not cleaned.
    ...
    /**
      ...
      */
    public static boolean isCompletedClusteringInstant(
            HoodieInstant instant, HoodieTimeline timeline) {
        ...
        try {
            return TimelineUtils.getCommitMetadata(instant, timeline)
                    .getOperationType()
                    .equals(WriteOperationType.CLUSTER);
        } catch (IOException e) {
    ...
    public static boolean isInsertOverwriteInstant(HoodieInstant instant, HoodieTimeline timeline) {
        ...
        try {
            WriteOperationType opType =
                    TimelineUtils.getCommitMetadata(instant, timeline).getOperationType();
