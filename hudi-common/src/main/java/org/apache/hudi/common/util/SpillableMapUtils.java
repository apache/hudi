...
public class SpillableMapUtils {
    ...
    /**
      * Reads the given file with specific pattern(|crc|timestamp|sizeOfKey|SizeOfValue|key|value|)
      * then returns an instance of {@link FileEntry}.
      */
    private static FileEntry readInternal(RandomAccessFile file, long valuePosition, int valueLength)
            throws IOException {
        ...
        if (valueSize != valueLength) {
            throw new HoodieCorruptedDataException(
                    "unequal size of payload written to external file, data may be corrupted");
        }
    ...
    /**
      ...
      *
      * <p>|crc|timestamp|sizeOfKey|SizeOfValue|key|value|
      */
    public static long spillToDisk(SizeAwareDataOutputStream outputStream, FileEntry fileEntry)
            throws IOException {
        return spill(outputStream, fileEntry);
    }

    private static long spill(SizeAwareDataOutputStream outputStream, FileEntry fileEntry)
            throws IOException {
    ...
    /**
      * Compute a bytes representation of the payload by serializing the contents This is used to
      * estimate the size of the payload (either in memory or when written to disk).
      */
    ...
    public static <R> HoodieRecord<R> convertToHoodieRecordPayload(
            GenericRecord rec, String payloadClazz, String preCombineField, boolean withOperationField) {
        return convertToHoodieRecordPayload(
                rec,
                payloadClazz,
                preCombineField,
        ...
        HoodieOperation operation =
                withOperationField
                        ? HoodieOperation.fromName(
                                getNullableValAsString(record, HoodieRecord.OPERATION_METADATA_FIELD))
                        : null;
        ...
        HoodieRecord<? extends HoodieRecordPayload> hoodieRecord =
                new HoodieAvroRecord<>(
                        ...
                        HoodieRecordUtils.loadPayload(
                                payloadClazz,
                                new Object[] {record, preCombineVal},
                                GenericRecord.class,
                                Comparable.class),
                        operation);
    ...
    /** Utility method to convert bytes to HoodieRecord using schema and payload class. */
    public static <R> R generateEmptyPayload(
            String recKey, String partitionPath, Comparable orderingVal, String payloadClazz) {
        HoodieRecord<? extends HoodieRecordPayload> hoodieRecord =
                new HoodieAvroRecord<>(
                        new HoodieKey(recKey, partitionPath),
                        HoodieRecordUtils.loadPayload(
                                payloadClazz,
                                new Object[] {null, orderingVal},
                                GenericRecord.class,
                                Comparable.class));
