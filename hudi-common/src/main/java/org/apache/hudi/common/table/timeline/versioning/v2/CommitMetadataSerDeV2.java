...
public class CommitMetadataSerDeV2 implements CommitMetadataSerDe {
  ...
  @Override
  public <T> Option<HoodieInstantWriter> serialize(T metadata) {
    if (metadata instanceof org.apache.hudi.common.model.HoodieCommitMetadata) {
      return getInstantWriter(Option.of(MetadataConversionUtils.convertCommitMetadataToAvro((HoodieCommitMetadata) metadata)));
    }
    return getInstantWriter(Option.of((SpecificRecordBase) metadata));
  }
}
