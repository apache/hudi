...
public class SpillableMapBasedFileSystemView extends HoodieTableFileSystemView {
    ...
    public SpillableMapBasedFileSystemView(
    ...
    protected Map<String, List<HoodieFileGroup>> createPartitionToFileGroups() {
        try {
            LOG.info(
                    "Creating Partition To File groups map using external spillable Map. Max Mem="
                            + maxMemoryForFileGroupMap
                            + ", BaseDir="
                            + baseStoreDir);
    ...
    protected Map<HoodieFileGroupId, Pair<String, CompactionOperation>>
        ...
        try {
            LOG.info(
                    "Creating Pending Compaction map using external spillable Map. Max Mem="
                            + maxMemoryForPendingCompaction
                            + ", BaseDir="
                            + baseStoreDir);
            new File(baseStoreDir).mkdirs();
    ...
    protected Map<HoodieFileGroupId, Pair<String, CompactionOperation>>
        ...
        try {
            LOG.info(
                    "Creating Pending Log Compaction map using external spillable Map. Max Mem="
    ...
    protected Map<HoodieFileGroupId, BootstrapBaseFileMapping> createFileIdToBootstrapBaseFileMap(
        ...
        try {
            LOG.info(
                    "Creating bootstrap base File Map using external spillable Map. Max Mem="
                            + maxMemoryForBootstrapBaseFile
                            + ", BaseDir="
                            + baseStoreDir);
            new File(baseStoreDir).mkdirs();
            Map<HoodieFileGroupId, BootstrapBaseFileMapping> pendingMap =
                    new ExternalSpillableMap<>(
                            maxMemoryForBootstrapBaseFile,
                            baseStoreDir,
                            new DefaultSizeEstimator(),
    ...
    protected Map<HoodieFileGroupId, HoodieInstant> createFileIdToReplaceInstantMap(
        ...
        try {
            LOG.info(
                    "Creating file group id to replace instant map using external spillable Map. Max Mem="
                            + maxMemoryForReplaceFileGroups
                            + ", BaseDir="
                            + baseStoreDir);
            new File(baseStoreDir).mkdirs();
            Map<HoodieFileGroupId, HoodieInstant> pendingMap =
                    new ExternalSpillableMap<>(
                            maxMemoryForReplaceFileGroups,
                            baseStoreDir,
                            new DefaultSizeEstimator(),
    ...
    protected Map<HoodieFileGroupId, HoodieInstant> createFileIdToPendingClusteringMap(
        ...
        try {
            LOG.info(
                    "Creating file group id to clustering instant map using external spillable Map. Max Mem="
                            + maxMemoryForClusteringFileGroups
                            + ", BaseDir="
                            + baseStoreDir);
            new File(baseStoreDir).mkdirs();
            Map<HoodieFileGroupId, HoodieInstant> pendingMap =
    ...
    public Stream<HoodieFileGroup> getAllFileGroups() {
        return ((ExternalSpillableMap) partitionToFileGroupsMap)
                .valueStream()
    ...
    protected void removeReplacedFileIdsAtInstants(Set<String> instants) {
        // TODO should we make this more efficient by having reverse mapping of instant to file
        // group id?
        Stream<HoodieFileGroupId> fileIdsToRemove =
                fgIdToReplaceInstants.entrySet().stream()
                        .map(
                                entry -> {
