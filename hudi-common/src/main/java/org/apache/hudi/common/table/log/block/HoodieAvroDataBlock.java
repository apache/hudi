...
/**
  * HoodieAvroDataBlock contains a list of records serialized using Avro. It is used with the Parquet
  * base file format.
  */
public class HoodieAvroDataBlock extends HoodieDataBlock {
    ...
    protected byte[] serializeRecords(List<HoodieRecord> records, HoodieStorage storage)
            throws IOException {
        Schema schema =
                new Schema.Parser().parse(super.getLogBlockHeader().get(HeaderMetadataType.SCHEMA));
    ...
    protected <T> ClosableIterator<HoodieRecord<T>> deserializeRecords(
        ...
        return new CloseableMappingIterator<>(
                iterator, data -> (HoodieRecord<T>) new HoodieAvroIndexedRecord(data));
    }
    ...
    private static class RecordIterator implements ClosableIterator<IndexedRecord> {
        ...
        private RecordIterator(Schema readerSchema, Schema writerSchema, byte[] content)
                throws IOException {
        ...
        public static RecordIterator getInstance(HoodieAvroDataBlock dataBlock, byte[] content)
                throws IOException {
    ...
    /**
      * This constructor is retained to provide backwards compatibility to HoodieArchivedLogs which
    ...
    public HoodieAvroDataBlock(List<HoodieRecord> records, Schema schema) {
        super(
                records,
                false,
                Collections.singletonMap(HeaderMetadataType.SCHEMA, schema.toString()),
                new HashMap<>(),
                HoodieRecord.RECORD_KEY_METADATA_FIELD);
    }

    public static HoodieAvroDataBlock getBlock(byte[] content, Schema readerSchema)
            throws IOException {
        return getBlock(content, readerSchema, InternalSchema.getEmptyInternalSchema());
    }

    /**
      * This method is retained to provide backwards compatibility to HoodieArchivedLogs which were
      * written using HoodieLogFormat V1.
      */
    @Deprecated
    public static HoodieAvroDataBlock getBlock(
            byte[] content, Schema readerSchema, InternalSchema internalSchema) throws IOException {
