...
/**
  ...
  *
  * <p>For each query engine, this class should be extended and plugged into {@code
  * HoodieFileGroupReader} to realize the file group reading.
  *
...
public abstract class HoodieReaderContext<T> {
    ...
    /**
      ...
      * @param requiredSchema Schema containing required fields to read in {@link Schema} for
      *     projection.
    ...
    /**
      * Constructs a new {@link HoodieRecord} based on the record of engine-specific type and metadata
      * for merging.
      *
      ...
      */
    public abstract HoodieRecord<T> constructHoodieRecord(
            Option<T> recordOption, Map<String, Object> metadataMap);
    ...
    public int compareTo(Comparable o1, Comparable o2) {
        throw new IllegalArgumentException(
                "Cannot compare values in different types: "
                        + o1
                        + "("
                        + o1.getClass()
                        + "), "
    ...
    /**
      ...
      */
    public Map<String, Object> updateSchemaAndResetOrderingValInMetadata(
            Map<String, Object> meta, Schema schema) {
    ...
    /**
      * Merge the skeleton file and data file iterators into a single iterator that will produce rows
      * that contain all columns from the skeleton file iterator, followed by all columns in the data
      * file iterator
      *
      * @param skeletonFileIterator iterator over bootstrap skeleton files that contain hudi metadata
      *     columns
      ...
      */
    public abstract ClosableIterator<T> mergeBootstrapReaders(
            ClosableIterator<T> skeletonFileIterator,
            Schema skeletonRequiredSchema,
            ClosableIterator<T> dataFileIterator,
            Schema dataRequiredSchema);

    /**
      * Creates a function that will reorder records of schema "from" to schema of "to" all fields in
      * "to" must be in "from", but not all fields in "from" must be in "to"
      *
      ...
      */
    public abstract UnaryOperator<T> projectRecord(
    ...
    /**
      ...
      */
    public long extractRecordPosition(
            T record, Schema schema, String fieldName, long providedPositionIfNeeded) {
