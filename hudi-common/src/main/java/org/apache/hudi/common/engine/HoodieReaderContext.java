...
public abstract class HoodieReaderContext<T> {
   ...
   */
  public abstract HoodieStorage getHoodieStorage(String path, Configuration conf);

  /**
   * Gets the record iterator based on the type of engine-specific record representation from the
   * file.
   *
   * @param filePath       {@link HoodieLocation} instance of a file.
  ...
  public abstract ClosableIterator<T> getFileRecordIterator(
      HoodieLocation filePath, long start, long length, Schema dataSchema, Schema requiredSchema,
