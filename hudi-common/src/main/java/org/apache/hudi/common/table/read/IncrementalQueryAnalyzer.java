...
/**
  ...
  *
  * <ul>
  *   <li>The archived instants;
  *   <li>The active instants;
  *   <li>The instant filtering predicate, e.g the instant range with a "startTime" and "endTime"
  *   <li>Whether the query starts from the "earliest" available instant;
  *   <li>Whether the query ends to the "latest" available instant;
  *   <li>The max completion time used for fs view file slice version filtering.
  * </ul>
  *
  * <p>
  *
  * <h2>Criteria for different query ranges:</h2>
  *
  * <table>
  *   <tr>
  *     <th>Query Range</th>
  ...
  * </table>
  *
  * <p>A {@code RangeType} is required for analyzing the query so that the query range boundary
  * inclusiveness have clear semantics.
  *
  * <p>IMPORTANT: the reader may optionally choose to fall back to reading the latest snapshot if
  * there are files decoding the commit metadata are already cleaned.
  */
public class IncrementalQueryAnalyzer {
    ...
    public QueryContext analyze() {
        try (CompletionTimeQueryView completionTimeQueryView =
                new CompletionTimeQueryView(this.metaClient)) {
            ...
            List<String> instantTimeList =
                    completionTimeQueryView.getStartTimes(filteredTimeline, startTime, endTime, rangeType);
            ...
            if (!activeInstantTime.isEmpty()) {
                activeInstants =
                        filteredTimeline
                                .getInstantsAsStream()
                                .filter(instant -> instantTimeSet.contains(instant.getTimestamp()))
                                .collect(Collectors.toList());
                if (limit > 0 && limit < activeInstants.size()) {
                    // streaming read speed limit, limits the maximum number of commits allowed to
                    // read for each run
            ...
            List<String> instants =
                    Stream.concat(archivedInstants.stream(), activeInstants.stream())
            ...
            // null => if starting from earliest, if no start time is specified, start from the
            // latest instant like usual streaming read semantics.
            // if startTime is neither, then use the earliest instant as the start instant.
            String startInstant =
                    START_COMMIT_EARLIEST.equalsIgnoreCase(startTime.orElse(null))
                            ? null
    ...
    /**
      * Splits the given instant time list into a pair of archived instant list and active instant
      * list.
      */
    private static Pair<List<String>, List<String>> splitInstantByActiveness(
            List<String> instantTimeList, CompletionTimeQueryView completionTimeQueryView) {
        int firstActiveIdx =
                IntStream.range(0, instantTimeList.size())
                        .filter(i -> !completionTimeQueryView.isArchived(instantTimeList.get(i)))
                        .findFirst()
                        .orElse(-1);
        if (firstActiveIdx == -1) {
            ...
            return Pair.of(
                    instantTimeList.subList(0, firstActiveIdx),
    ...
    private HoodieTimeline getFilteredTimeline(HoodieTableMetaClient metaClient) {
        ...
        return filterInstantsAsPerUserConfigs(
                metaClient,
                timeline,
                this.skipCompaction,
                this.skipClustering,
    ...
    private HoodieTimeline getArchivedReadTimeline(
            HoodieTableMetaClient metaClient, String startInstant) {
    ...
    public static HoodieTimeline filterInstantsAsPerUserConfigs(
            ...
            boolean skipInsertOverwrite) {
        return filterInstantsAsPerUserConfigs(
                metaClient, timeline, skipCompaction, skipClustering, skipInsertOverwrite, false);
    }
    ...
    private static HoodieTimeline filterInstantsAsPerUserConfigs(
        ...
        if (metaClient.getTableType() == HoodieTableType.MERGE_ON_READ && skipCompaction) {
            ...
            timeline =
                    timeline.filter(instant -> !instant.getAction().equals(HoodieTimeline.COMMIT_ACTION));
        }
        if (metaClient.getTableType() == HoodieTableType.MERGE_ON_READ
                && metaClient.getTableConfig().isCDCEnabled()
                && readCdcFromChangelog) {
            // only compaction yields changelog file
            timeline =
                    timeline.filter(
                            instant -> !instant.getAction().equals(HoodieTimeline.DELTA_COMMIT_ACTION));
        }
        if (skipClustering) {
            timeline =
                    timeline.filter(
                            instant -> !ClusteringUtils.isCompletedClusteringInstant(instant, oriTimeline));
        }
        if (skipInsertOverwrite) {
            timeline =
                    timeline.filter(
                            instant -> !ClusteringUtils.isInsertOverwriteInstant(instant, oriTimeline));
        }
        return timeline;
    }
    ...
    public static class Builder {
        ...
        public IncrementalQueryAnalyzer build() {
            return new IncrementalQueryAnalyzer(
                    Objects.requireNonNull(this.metaClient),
                    this.startTime,
                    this.endTime,
                    Objects.requireNonNull(this.rangeType),
                    this.skipCompaction,
                    this.skipClustering,
                    this.skipInsertOverwrite,
                    this.readCdcFromChangelog,
    ...
    public static class QueryContext {
        ...
        public String getMaxCompletionTime() {
            if (this.activeInstants.size() > 0) {
                return this.activeInstants.stream()
                        .map(HoodieInstant::getCompletionTime)
                        .filter(Objects::nonNull)
                        .max(String::compareTo)
                        .get();
            } else {
                // all the query instants are archived, use the latest active instant completion
                // time as
                // the file slice version upper threshold, because very probably these files already
                // got cleaned,
                // use the max completion time of the archived instants could yield empty file
                // slices.
