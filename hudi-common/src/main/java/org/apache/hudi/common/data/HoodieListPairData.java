...
/**
  * In-memory implementation of {@link HoodiePairData} holding internally a {@link Stream} of {@link
  * Pair}s.
  *
...
public class HoodieListPairData<K, V> extends HoodieBaseListData<Pair<K, V>>
    ...
    public HoodiePairData<K, Iterable<V>> groupByKey() {
        ...
        return new HoodieListPairData<>(
                groupedByKey.entrySet().stream().map(e -> Pair.of(e.getKey(), e.getValue())), lazy);
    }

    @Override
    public HoodiePairData<K, V> reduceByKey(
            SerializableBiFunction<V, V, V> combiner, int parallelism) {
        Map<K, java.util.Optional<V>> reducedMap =
                asStream()
                        .collect(
        ...
        return new HoodieListPairData<>(
                reducedMap.entrySet().stream().map(e -> Pair.of(e.getKey(), e.getValue().orElse(null))),
                lazy);
    }
    ...
    public <W> HoodiePairData<K, W> mapValues(SerializableFunction<V, W> func) {
        ...
        return new HoodieListPairData<>(
                asStream().map(p -> Pair.of(p.getKey(), uncheckedMapper.apply(p.getValue()))), lazy);
    }
                        ...
                        .flatMap(
                                p -> {
                                    ...
                                    return StreamSupport.stream(
                                            Spliterators.spliteratorUnknownSize(mappedPairsIterator, Spliterator.ORDERED),
                                            true);
                                }),
                lazy);
    }
    ...
    public <L, W> HoodiePairData<L, W> mapToPair(
        ...
        return new HoodieListPairData<>(
                asStream().map(p -> throwingMapToPairWrapper(mapToPairFunc).apply(p)), lazy);
    }
    ...
    public <W> HoodiePairData<K, Pair<V, Option<W>>> leftOuterJoin(HoodiePairData<K, W> other) {
        ...
        HashMap<K, List<W>> rightStreamMap =
                ((HoodieListPairData<K, W>) other)
                        .asStream()
                        .collect(
                                Collectors.groupingBy(
                                        Pair::getKey,
                                ...
                                pair -> {
                                    ...
                                    if (rightValues == null) {
                                        ...
                                        return rightValues.stream()
                                                .map(rightValue -> Pair.of(key, Pair.of(leftValue, Option.of(rightValue))));
                                    }
                                });
