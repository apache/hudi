...
public class CleanerUtils {
    ...
    public static HoodieCleanMetadata convertCleanMetadata(
        ...
        for (HoodieCleanStat stat : cleanStats) {
            ...
            if ((null != stat.getDeleteBootstrapBasePathPatterns())
                ...
                HoodieCleanPartitionMetadata bootstrapMetadata =
                        new HoodieCleanPartitionMetadata(
                                stat.getPartitionPath(),
                                stat.getPolicy().name(),
                                stat.getDeleteBootstrapBasePathPatterns(),
                                stat.getSuccessDeleteBootstrapBaseFiles(),
    ...
    /**
      ...
      * @throws IOException
      */
    public static HoodieCleanMetadata getCleanerMetadata(
            HoodieTableMetaClient metaClient, HoodieInstant cleanInstant) throws IOException {
    ...
    /**
      ...
      */
    public static HoodieCleanMetadata getCleanerMetadata(
            HoodieTableMetaClient metaClient, byte[] details) throws IOException {
        CleanMetadataMigrator metadataMigrator = new CleanMetadataMigrator(metaClient);
        HoodieCleanMetadata cleanMetadata =
    ...
    public static Option<HoodieInstant> getEarliestCommitToRetain(
        ...
        if (cleaningPolicy == HoodieCleaningPolicy.KEEP_LATEST_COMMITS
            ...
            if (earliestPendingCommits.isPresent()) {
                ...
                earliestCommitToRetain =
                        completedCommitsTimeline
                                .nthInstant(completedCommitsTimeline.countInstants() - commitsRetained)
                                .map(
                                        nthInstant -> {
                                            if (nthInstant.compareTo(earliestPendingCommits.get()) <= 0) {
                                                ...
                                                return completedCommitsTimeline
                                                        .findInstantsBefore(earliestPendingCommits.get().getTimestamp())
                                                        .lastInstant();
                                            }
                                        })
                                .orElse(Option.empty());
                ...
                earliestCommitToRetain =
                        completedCommitsTimeline.nthInstant(
                                completedCommitsTimeline.countInstants()
                                        - commitsRetained); // 15 instants total, 10 commits to
                // retain, this gives 6th instant in the
                // list
            }
            ...
            String earliestTimeToRetain =
                    HoodieActiveTimeline.formatDate(
                            Date.from(latestDateTime.minusHours(hoursRetained).toInstant()));
            earliestCommitToRetain =
    ...
    /**
      ...
      * @throws IOException
      */
    public static HoodieCleanerPlan getCleanerPlan(
            HoodieTableMetaClient metaClient, HoodieInstant cleanInstant) throws IOException {
    ...
    /**
      * Convert list of cleanFileInfo instances to list of avro-generated HoodieCleanFileInfo
      * instances.
      *
      * @param cleanFileInfoList
      * @return
      */
    public static List<HoodieCleanFileInfo> convertToHoodieCleanFileInfoList(
            List<CleanFileInfo> cleanFileInfoList) {
        return cleanFileInfoList.stream()
                .map(CleanFileInfo::toHoodieFileCleanInfo)
                .collect(Collectors.toList());
    }

    /**
      * Execute {@link HoodieFailedWritesCleaningPolicy} to rollback failed writes for different
      * actions.
      *
      ...
      */
    public static void rollbackFailedWrites(
