...
/**
  * This contains all the information that retrieve the change data at a single file group and at a
  * single commit.
  *
  * <p>For `cdcInferCase` = {@link HoodieCDCInferenceCase#BASE_FILE_INSERT}, `cdcFile` is a current
  * version of the base file in the group, and `beforeFileSlice` is None. For `cdcInferCase` = {@link
  * HoodieCDCInferenceCase#BASE_FILE_DELETE}, `cdcFile` is null, `beforeFileSlice` is the previous
  * version of the base file in the group. For `cdcInferCase` = {@link HoodieCDCInferenceCase#AS_IS},
  * `cdcFile` is a log file with cdc blocks. when enable the supplemental logging, both
  * `beforeFileSlice` and `afterFileSlice` are None, otherwise these two are the previous and current
  * version of the base file. For `cdcInferCase` = {@link HoodieCDCInferenceCase#LOG_FILE}, `cdcFile`
  * is a normal log file and `beforeFileSlice` is the previous version of the file slice. For
  * `cdcInferCase` = {@link HoodieCDCInferenceCase#REPLACE_COMMIT}, `cdcFile` is null,
...
public class HoodieCDCFileSplit implements Serializable, Comparable<HoodieCDCFileSplit> {
    ...
    public HoodieCDCFileSplit(
            String instant, HoodieCDCInferenceCase cdcInferCase, Collection<String> cdcFiles) {
    ...
    public HoodieCDCFileSplit(
        ...
        this(
                instant, cdcInferCase, Collections.singletonList(cdcFile), beforeFileSlice, afterFileSlice);
    }
