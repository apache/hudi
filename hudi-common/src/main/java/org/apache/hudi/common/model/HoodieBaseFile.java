...
/**
  * Hoodie base file - Represents metadata about Hudi file in DFS. Supports APIs to get Hudi FileId,
  * Commit Time and bootstrap file (if any).
  */
public class HoodieBaseFile extends BaseFile {
    ...
    public HoodieBaseFile(StoragePathInfo pathInfo, BaseFile bootstrapBaseFile) {
        this(
                pathInfo,
                getFileIdAndCommitTimeFromFileName(pathInfo.getPath().getName()),
                bootstrapBaseFile);
    }
    ...
    public HoodieBaseFile(
            String filePath, String fileId, String commitTime, BaseFile bootstrapBaseFile) {
        super(filePath);
        this.bootstrapBaseFile = Option.ofNullable(bootstrapBaseFile);
        this.fileId = fileId;
    ...
    private HoodieBaseFile(
            StoragePathInfo pathInfo, String[] fileIdAndCommitTime, BaseFile bootstrapBaseFile) {
        this(pathInfo, fileIdAndCommitTime[0], fileIdAndCommitTime[1], bootstrapBaseFile);
    }

    public HoodieBaseFile(
            StoragePathInfo pathInfo, String fileId, String commitTime, BaseFile bootstrapBaseFile) {
    ...
    private static String[] getFileIdAndCommitTimeFromFileName(String fileName) {
        return ExternalFilePathUtil.isExternallyCreatedFile(fileName)
                ? handleExternallyGeneratedFile(fileName)
                : handleHudiGeneratedFile(fileName);
    }
    ...
    private static String[] handleExternallyGeneratedFile(String fileName) {
        String[] values = new String[2];
        // file name has format <originalFileName>_<commitTime>_hudiext and originalFileName is used
        // as fileId
    ...
    /**
      * If the file was created externally, the original file path will have a '_[commitTime]_hudiext'
      * suffix when stored in the metadata table. That suffix needs to be removed from the FileStatus
      * so that the actual file can be found and read.
      *
      * @param pathInfo an input path info that may require updating
      * @param fileId the fileId for the file
      * @return the original file status if it was not externally created, or a new FileStatus with the
      *     original file name if it was externally created
      */
    private static StoragePathInfo maybeHandleExternallyGeneratedFileName(
            StoragePathInfo pathInfo, String fileId) {
        if (pathInfo == null) {
            return null;
        }
    ...
    public String toString() {
        return "HoodieBaseFile{fullPath="
                + getPath()
                + ", fileLen="
                + getFileLen()
                + ", BootstrapBaseFile="
                + bootstrapBaseFile.orElse(null)
                + '}';
    }
}
