...
public class HoodieBaseFile extends BaseFile {
  ...
  public HoodieBaseFile(StoragePathInfo fileStatus) {
    this(fileStatus, null);
  }

  public HoodieBaseFile(StoragePathInfo fileStatus, BaseFile bootstrapBaseFile) {
    this(fileStatus, getFileIdAndCommitTimeFromFileName(fileStatus.getPath().getName()),
        bootstrapBaseFile);
  }
  ...
  private HoodieBaseFile(StoragePathInfo fileStatus, String[] fileIdAndCommitTime,
                         BaseFile bootstrapBaseFile) {
    this(fileStatus, fileIdAndCommitTime[0], fileIdAndCommitTime[1], bootstrapBaseFile);
  }

  public HoodieBaseFile(StoragePathInfo fileStatus, String fileId, String commitTime,
                        BaseFile bootstrapBaseFile) {
   ...
   */
  private static StoragePathInfo maybeHandleExternallyGeneratedFileName(StoragePathInfo fileStatus,
                                                                        String fileId) {
    if (fileStatus == null) {
      return null;
    }
    if (ExternalFilePathUtil.isExternallyCreatedFile(fileStatus.getPath().getName())) {
      ...
      return new StoragePathInfo(
          new StoragePath(parent, fileId), fileStatus.getLength(), fileStatus.isDirectory(),
