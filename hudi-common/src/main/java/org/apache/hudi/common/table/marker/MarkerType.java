...
@EnumDescription(
        "Marker type indicating how markers are stored in the file system, used for "
                + "identifying the files written and cleaning up files not committed which should be deleted.")
public enum MarkerType {
    @EnumFieldDescription(
            "Individual marker file corresponding to each data file is directly created by the writer.")
    DIRECT,

    @EnumFieldDescription(
            "Marker operations are all handled at the timeline service which serves as a proxy. "
