...
/**
  * Maintains mapping from skeleton file id to external bootstrap file. It maintains 2 physical
  * indices. (a) At partition granularity to lookup all indices for each partition. (b) At file-group
  * granularity to lookup bootstrap mapping for an individual file-group.
  *
  * <p>This implementation uses HFile as physical storage of index. FOr the initial run, bootstrap
  * mapping for the entire dataset resides in a single file but care has been taken in naming the
  * index files in the same way as Hudi data files so that we can reuse file-system abstraction on
  * these index files to manage multiple file-groups.
  */
public class HFileBootstrapIndex extends BootstrapIndex {
    ...
    static StoragePath partitionIndexPath(HoodieTableMetaClient metaClient) {
        return new StoragePath(
                metaClient.getBootstrapIndexByPartitionFolderPath(),
                FSUtils.makeBootstrapIndexFileName(
    ...
    static StoragePath fileIdIndexPath(HoodieTableMetaClient metaClient) {
        return new StoragePath(
                metaClient.getBootstrapIndexByFileIdFolderNameFolderPath(),
                FSUtils.makeBootstrapIndexFileName(
                        HoodieTimeline.METADATA_BOOTSTRAP_INSTANT_TS,
                        BOOTSTRAP_INDEX_FILE_ID,
                        HoodieFileFormat.HFILE.getFileExtension()));
    ...
    public BootstrapIndex.IndexWriter createWriter(String bootstrapBasePath) {
        return (IndexWriter)
                ReflectionUtils.loadClass(
                        "org.apache.hudi.common.bootstrap.index.hfile.HBaseHFileBootstrapIndexWriter",
    ...
    public void dropIndex() {
        try {
            StoragePath[] indexPaths =
                    new StoragePath[] {partitionIndexPath(metaClient), fileIdIndexPath(metaClient)};
