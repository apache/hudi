...
public class HoodieDeleteBlock extends HoodieLogBlock {
    ...
    /**
      * These static builders are added to avoid performance issue in Avro 1.10. You can find more
      * details in HoodieAvroUtils, HUDI-3834, and AVRO-3048.
      */
    ...
    // Records to delete, sorted based on the record position if writing record position to the log
    // block header
    private DeleteRecord[] recordsToDelete;

    public HoodieDeleteBlock(
            List<Pair<DeleteRecord, Long>> recordsToDelete,
        ...
        if (shouldWriteRecordPositions && !recordsToDelete.isEmpty()) {
            ...
            if (isPositionValid(recordsToDelete.get(0).getRight())) {
                ...
                LOG.warn(
                        "There are delete records without valid positions. "
    ...
    public HoodieDeleteBlock(
        ...
        // Setting `shouldWriteRecordPositions` to false as this constructor is only used by the
        // reader
        this(
                content, inputStreamSupplier, readBlockLazily, blockContentLocation, header, footer, false);
    }
    ...
    private byte[] serializeV3() throws IOException {
        DatumWriter<HoodieDeleteRecordList> writer =
                new SpecificDatumWriter<>(HoodieDeleteRecordList.class);
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        ...
        List<HoodieDeleteRecord> deleteRecordList =
                Arrays.stream(getRecordsToDelete())
                        .map(
                                record ->
                                        HoodieDeleteRecord.newBuilder(recordBuilder)
        ...
        writer.write(
                HoodieDeleteRecordList.newBuilder(recordListBuilder)
    ...
    private static DeleteRecord[] deserialize(int version, byte[] data) throws IOException {
        if (version == 1) {
            ...
            DatumReader<HoodieDeleteRecordList> reader =
                    new SpecificDatumReader<>(HoodieDeleteRecordList.class);
            BinaryDecoder decoder = DecoderFactory.get().binaryDecoder(data, 0, data.length, null);
            List<HoodieDeleteRecord> deleteRecordList = reader.read(null, decoder).getDeleteRecordList();
