...
public class CompactionUtils {
    ...
    /**
      ...
      */
    public static HoodieCompactionOperation buildFromFileSlice(
        ...
        builder.setDeltaFilePaths(
                fileSlice.getLogFiles().map(lf -> lf.getPath().getName()).collect(Collectors.toList()));
        if (fileSlice.getBaseFile().isPresent()) {
            builder.setDataFilePath(fileSlice.getBaseFile().get().getFileName());
            builder.setBootstrapFilePath(
                    fileSlice.getBaseFile().get().getBootstrapBaseFile().map(BaseFile::getPath).orElse(null));
        }
    ...
    public static HoodieCompactionPlan buildFromFileSlices(
        ...
        builder.setOperations(
                partitionFileSlicePairs.stream()
                        .map(
                                pfPair ->
                                        buildFromFileSlice(pfPair.getKey(), pfPair.getValue(), metricsCaptureFunction))
    ...
    /**
      * Build Avro generated Compaction operation payload from compaction operation POJO for
      * serialization.
      */
    public static HoodieCompactionOperation buildHoodieCompactionOperation(CompactionOperation op) {
        return HoodieCompactionOperation.newBuilder()
                ...
                .setDeltaFilePaths(op.getDeltaFileNames())
    ...
    public static List<Pair<HoodieInstant, HoodieCompactionPlan>> getAllPendingCompactionPlans(
        ...
        Function<String, HoodieInstant> requestedInstantSupplier =
                HoodieTimeline::getCompactionRequestedInstant;
        return getCompactionPlansByTimeline(
                metaClient, getFilteredTimelineByActionType, requestedInstantSupplier);
    }
    ...
    public static List<Pair<HoodieInstant, HoodieCompactionPlan>> getAllPendingLogCompactionPlans(
        ...
        Function<HoodieTableMetaClient, HoodieTimeline> filteredTimelineSupplier =
                (hoodieTableMetaClient) ->
                        hoodieTableMetaClient.getActiveTimeline().filterPendingLogCompactionTimeline();
        // Hoodie requested instant supplier
        Function<String, HoodieInstant> requestedInstantSupplier =
                HoodieTimeline::getLogCompactionRequestedInstant;
        return getCompactionPlansByTimeline(
                metaClient, filteredTimelineSupplier, requestedInstantSupplier);
    }
    ...
    /**
      ...
      * @param filteredTimelineSupplier gives a timeline object, this can be either filtered to return
      *     pending compactions or log compaction instants.
      * @param requestedInstantWrapper function that gives a requested Hoodie instant.
      * @return List of pair of HoodieInstant and it's corresponding compaction plan. Note here the
      *     compaction plan can be related to a compaction instant or log compaction instant.
      */
                ...
                .map(
                        instant ->
                                Pair.of(
                                        instant,
                                        getCompactionPlan(
                                                metaClient, requestedInstantWrapper.apply(instant.getTimestamp()))))
                .collect(Collectors.toList());
    }

    /**
      * This method will serve only Compaction instants because we use same HoodieCompactionPlan for
      * both the operations.
      */
    public static HoodieCompactionPlan getCompactionPlan(
    ...
    /**
      * This method will serve only log compaction instants, because we use same HoodieCompactionPlan
      * for both the operations.
      */
    public static HoodieCompactionPlan getLogCompactionPlan(
            HoodieTableMetaClient metaClient, String logCompactionInstant) {
    ...
    private static HoodieCompactionPlan getCompactionPlan(
            HoodieTableMetaClient metaClient, HoodieInstant requestedInstant) {
        return getCompactionPlan(
                metaClient, metaClient.getActiveTimeline().readCompactionPlanAsBytes(requestedInstant));
    }

    /** Util method to fetch both compaction and log compaction plan from requestedInstant. */
    public static HoodieCompactionPlan getCompactionPlan(
            HoodieTableMetaClient metaClient, Option<byte[]> planContent) {
    ...
    public static Map<HoodieFileGroupId, Pair<String, HoodieCompactionOperation>>
        ...
        return getAllPendingCompactionOperationsInPendingCompactionPlans(
                pendingCompactionPlanWithInstants);
    }

    /**
      * Get all partition + file Ids with pending Log Compaction operations and their target log
    ...
    public static Map<HoodieFileGroupId, Pair<String, HoodieCompactionOperation>>
        ...
        return getAllPendingCompactionOperationsInPendingCompactionPlans(
                pendingLogCompactionPlanWithInstants);
    }
    ...
    /**
      ...
      */
    public static Map<HoodieFileGroupId, Pair<String, HoodieCompactionOperation>>
            getAllPendingCompactionOperationsInPendingCompactionPlans(
                    List<Pair<HoodieInstant, HoodieCompactionPlan>> pendingLogCompactionPlanWithInstants) {

        Map<HoodieFileGroupId, Pair<String, HoodieCompactionOperation>>
                fgIdToPendingCompactionsWithInstantMap = new HashMap<>();
        pendingLogCompactionPlanWithInstants.stream()
                .flatMap(
                        instantPlanPair ->
                                getPendingCompactionOperations(
                                        instantPlanPair.getKey(), instantPlanPair.getValue()))
                .forEach(
                        pair -> {
                            // Defensive check to ensure a single-fileId does not have more than one
                            // pending log compaction with different
                            ...
                            if (fgIdToPendingCompactionsWithInstantMap.containsKey(pair.getKey())) {
                                ...
                                HoodieCompactionOperation anotherOperation =
                                        fgIdToPendingCompactionsWithInstantMap.get(pair.getKey()).getValue();

                                if (!operation.equals(anotherOperation)) {
                                    String msg =
                                            "Hudi File Id ("
                                                    + pair.getKey()
                                                    + ") has more than 1 pending operation. Instants: "
                                                    + pair.getValue()
                                                    + ", "
                                                    + fgIdToPendingCompactionsWithInstantMap.get(pair.getKey());
    ...
    public static Stream<Pair<HoodieFileGroupId, Pair<String, HoodieCompactionOperation>>>
            getPendingCompactionOperations(HoodieInstant instant, HoodieCompactionPlan compactionPlan) {
    ...
    /**
      ...
      * @return
      */
    public static List<HoodieInstant> getPendingCompactionInstantTimes(
            HoodieTableMetaClient metaClient) {
    ...
    /**
      ...
      * compaction commit, the completed compaction commit instant), if the latest completed compaction
      * commit is present; a pair of (timeline containing all the completed delta commits, the first
      * delta commit instant), if there is no completed compaction commit.
      *
    ...
    /**
      * Returns a pair of (timeline containing the delta commits after the latest completed compaction
      * commit, the completed compaction commit instant), if the latest completed compaction commit is
      * present; a pair of (timeline containing all the delta commits, the first delta commit instant),
      * if there is no completed compaction commit.
      *
      * @param activeTimeline Active timeline of a table.
      * @return Pair of timeline containing delta commits and an instant.
      */
    ...
    public static Option<Pair<HoodieTimeline, HoodieInstant>>
        ...
        Option<HoodieInstant> lastCompaction =
                activeTimeline.getCommitTimeline().filterCompletedInstants().lastInstant();
        Option<HoodieInstant> lastRequestCompaction =
                activeTimeline.getAllCommitsTimeline().filterPendingCompactionTimeline().lastInstant();
        ...
        if (lastCompaction.isPresent()) {
            ...
            return Option.of(
                    Pair.of(
                            deltaCommits.findInstantsAfter(latestInstant.getTimestamp(), Integer.MAX_VALUE),
                            lastCompaction.get()));
        } else {
            if (deltaCommits.countInstants() > 0) {
                latestInstant = deltaCommits.firstInstant().get();
                ...
                return Option.of(
                        Pair.of(
                                deltaCommits.findInstantsAfterOrEquals(
    ...
    /**
      * Gets the earliest instant to retain for MOR compaction. If there is no completed compaction,
      * num delta commits >= "hoodie.compact.inline.max.delta.commits" If there is a completed
      * compaction, num delta commits after latest completed compaction >=
      * "hoodie.compact.inline.max.delta.commits"
      *
      * @param activeTimeline Active timeline of a table.
      * @param maxDeltaCommits Maximum number of delta commits that trigger the compaction plan, i.e.,
      *     "hoodie.compact.inline.max.delta.commits".
        ...
        if (deltaCommitsInfoOption.isPresent()) {
            ...
            if (numDeltaCommits < maxDeltaCommits) {
                ...
                List<HoodieInstant> instants =
                        deltaCommitTimeline
                                .getInstantsAsStream()
                                .limit(numDeltaCommits - maxDeltaCommits + 1)
                                .collect(Collectors.toList());
