...
public class CompactionOperation implements Serializable {
    ...
    public CompactionOperation(
        ...
        this.deltaFileNames =
                logFiles.stream().map(s -> s.getPath().getName()).collect(Collectors.toList());
        this.metrics = metrics;
    }
    ...
    public Option<HoodieBaseFile> getBaseFile(String basePath, String partitionPath) {
        ...
        return dataFileName.map(
                df -> {
                    return externalBaseFile
                            .map(ext -> new HoodieBaseFile(new StoragePath(dirPath, df).toString(), ext))
    ...
    /**
      ...
      * @return
      */
    public static CompactionOperation convertFromAvroRecordInstance(
        ...
        op.dataFileCommitTime =
                op.dataFileName.map(p -> FSUtils.getCommitTime(new StoragePath(p).getName()));
        ...
        op.metrics =
                operation.getMetrics() == null ? new HashMap<>() : new HashMap<>(operation.getMetrics());
    ...
    public String toString() {
        return "CompactionOperation{baseInstantTime='"
                + baseInstantTime
                + '\''
                + ", dataFileCommitTime="
                + dataFileCommitTime
