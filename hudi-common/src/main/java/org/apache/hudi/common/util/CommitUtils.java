...
public class CommitUtils {
    ...
    /**
      * Gets the commit action type for given write operation and table type. Use this API when commit
      * action type can differ not only on the basis of table type but also write operation type. For
      * example, INSERT_OVERWRITE/INSERT_OVERWRITE_TABLE operations have REPLACE commit action type.
      */
    public static String getCommitActionType(
            WriteOperationType operation, HoodieTableType tableType) {
    ...
    /**
      * Gets the commit action type for given table type. Note: Use this API only when the commit
      * action type is not dependent on the write operation type. See {@link
      * CommitUtils#getCommitActionType(WriteOperationType, HoodieTableType)} for more details.
      */
    ...
    public static HoodieCommitMetadata buildMetadata(
        ...
        HoodieCommitMetadata commitMetadata =
                buildMetadataFromStats(
                        writeStats, partitionToReplaceFileIds, commitActionType, operationType);

        // add in extra metadata
        if (extraMetadata.isPresent()) {
            extraMetadata.get().forEach(commitMetadata::addMetadata);
        }
        commitMetadata.addMetadata(
    ...
    private static HoodieCommitMetadata buildMetadataFromStats(
            List<HoodieWriteStat> writeStats,
            Map<String, List<String>> partitionToReplaceFileIds,
            String commitActionType,
            WriteOperationType operationType) {
    ...
    public static Option<HoodieCommitMetadata> buildMetadataFromInstant(
            HoodieDefaultTimeline timeline, HoodieInstant instant) {
        try {
            HoodieCommitMetadata commitMetadata =
    ...
    public static Set<Pair<String, String>> getPartitionAndFileIdWithoutSuffixFromSpecificRecord(
            Map<String, List<org.apache.hudi.avro.model.HoodieWriteStat>> partitionToWriteStats) {
        Set<Pair<String, String>> partitionToFileId = new HashSet<>();
        // list all partitions paths
        for (Map.Entry<String, List<org.apache.hudi.avro.model.HoodieWriteStat>> entry :
    ...
    public static Set<Pair<String, String>> getPartitionAndFileIdWithoutSuffix(
    ...
    public static Set<Pair<String, String>> flattenPartitionToReplaceFileIds(
            Map<String, List<String>> partitionToReplaceFileIds) {
        return partitionToReplaceFileIds.entrySet().stream()
                .flatMap(
                        partitionFileIds ->
                                partitionFileIds.getValue().stream()
                                        .map(replaceFileId -> Pair.of(partitionFileIds.getKey(), replaceFileId)))
                .collect(Collectors.toSet());
    }

    /**
      * Process previous commits metadata in the timeline to determine the checkpoint given a
      * checkpoint key. NOTE: This is very similar in intent to
      * DeltaSync#getLatestCommitMetadataWithValidCheckpointInfo except that different deployment
      * models (deltastreamer or spark structured streaming) could have different checkpoint keys.
      *
      ...
      */
    public static Option<String> getValidCheckpointForCurrentWriter(
            HoodieTimeline timeline, String checkpointKey, String keyToLookup) {
