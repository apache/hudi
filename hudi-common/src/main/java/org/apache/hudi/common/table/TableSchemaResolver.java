...
/**
  * Helper class to read schema from data files and log files and to convert it between different
  * formats.
  */
@ThreadSafe
public class TableSchemaResolver {
    ...
    /**
      * Signals whether suite of the meta-fields should have additional field designating operation
      * particular record was added by. Note, that determining whether this meta-field should be
      * appended to the schema requires reading out the actual schema of some data file, since it's
      * ultimately the source of truth whether this field has to be represented in the schema
      */
    private final Lazy<Boolean> hasOperationField;

    /**
      * NOTE: {@link HoodieCommitMetadata} could be of non-trivial size for large tables (in 100s of
      * Mbs) and therefore we'd want to limit amount of throw-away work being performed while fetching
      * commits' metadata
      *
      * <p>Please check out corresponding methods to fetch commonly used instances of {@link
      * HoodieCommitMetadata}: {@link #getLatestCommitMetadataWithValidSchema()}, {@link
      * #getLatestCommitMetadataWithValidSchema()}, {@link #getCachedCommitMetadata(HoodieInstant)}
      */
    ...
    /**
      ...
      */
    public Schema getTableAvroSchema(HoodieInstant instant, boolean includeMetadataFields)
            throws Exception {
    ...
    private Option<Schema> getTableAvroSchemaInternal(
            boolean includeMetadataFields, Option<HoodieInstant> instantOpt) {
                                                        ...
                                                        tableSchema ->
                                                                includeMetadataFields
                                                                        ? HoodieAvroUtils.addMetadataFields(
                                                                                tableSchema, hasOperationField.get())
                                                                        : tableSchema))
                        .or(
                                () -> {
        ...
        if (metaClient.getTableConfig().shouldDropPartitionColumns() && schema.isPresent()) {
            return metaClient
                    ...
                    .map(
                            partitionFields ->
                                    appendPartitionColumns(schema.get(), Option.ofNullable(partitionFields)))
                    .or(() -> schema);
        }
    ...
    private Option<Schema> getTableParquetSchemaFromDataFile() {
        ...
        try {
            switch (metaClient.getTableType()) {
                case COPY_ON_WRITE:
                case MERGE_ON_READ:
                    // For COW table, data could be written in either Parquet or Orc format
                    // currently;
                    // For MOR table, data could be written in either Parquet, Orc, Hfile or
                    // Delta-log format currently;
                    //
                    // Determine the file format based on the file name, and then extract schema
                    // from it.
                    if (instantAndCommitMetadata.isPresent()) {
                        HoodieCommitMetadata commitMetadata = instantAndCommitMetadata.get().getRight();
                        ...
                        LOG.warn(
                                "Could not find any data file written for commit, so could not get schema for table {}",
    ...
    /**
      * Returns table's latest Avro {@link Schema} iff table is non-empty (ie there's at least a single
      * commit)
      *
      * <p>This method differs from {@link #getTableAvroSchema(boolean)} in that it won't fallback to
      * use table's schema used at creation
      */
    ...
    /**
      ...
      */
    public Schema readSchemaFromLastCompaction(Option<HoodieInstant> lastCompactionCommitOpt)
            throws Exception {
        HoodieActiveTimeline activeTimeline = metaClient.getActiveTimeline();

        HoodieInstant lastCompactionCommit =
                lastCompactionCommitOpt.orElseThrow(
                        () ->
                                new Exception(
        ...
        String filePath =
                compactionMetadata.getFileIdAndFullPaths(metaClient.getBasePath()).values().stream()
                        .findAny()
                        .orElseThrow(
                                () ->
                                        new IllegalArgumentException(
                                                "Could not find any data file written for compaction "
                                                        + lastCompactionCommit
                                                        + ", could not get schema for table "
                                                        + metaClient.getBasePath()));
        StoragePath path = new StoragePath(filePath);
        return HoodieIOFactory.getIOFactory(metaClient.getStorage())
                .getFileFormatUtils(path)
                .readAvroSchema(metaClient.getStorage(), path);
    }
    ...
    /**
      ...
      * @return
      */
    public static Schema readSchemaFromLogFile(HoodieStorage storage, StoragePath path)
            throws IOException {
    ...
    public Option<InternalSchema> getTableInternalSchemaFromCommitMetadata() {
        ...
        HoodieTimeline timeline =
                completedInstants.filter(
                        instant -> { // consider only instants that can update/change schema.
                            try {
                                HoodieCommitMetadata commitMetadata =
                                        HoodieCommitMetadata.fromBytes(
                                                completedInstants.getInstantDetails(instant).get(),
                                                HoodieCommitMetadata.class);
    ...
    /**
      * Gets the history schemas as String for a hoodie table from the HoodieCommitMetadata of the
      * instant.
      *
    ...
    public Option<String> getTableHistorySchemaStrFromCommitMetadata() {
        ...
        FileBasedInternalSchemaStorageManager manager =
    ...
    /** NOTE: This method could only be used in tests @VisibleForTesting */
                ...
                .computeIfAbsent(
                        ...
                        (missingInstant) -> {
                            ...
                            try {
                                ...
                                throw new HoodieIOException(
                                        String.format(
                                                "Failed to fetch HoodieCommitMetadata for instant (%s)", missingInstant),
                                        e);
                            }
                        });
    }
            ...
            if (FSUtils.isLogFile(filePath)) {
                ...
                schema =
                        HoodieIOFactory.getIOFactory(metaClient.getStorage())
                                .getFileFormatUtils(filePath)
                                .readAvroSchema(metaClient.getStorage(), filePath);
            }
        }
        return schema;
    }
    ...
    public static Schema appendPartitionColumns(Schema dataSchema, Option<String[]> partitionFields) {
        ...
        boolean hasPartitionColNotInSchema =
                Arrays.stream(partitionFields.get()).anyMatch(pf -> !containsFieldInSchema(dataSchema, pf));
        boolean hasPartitionColInSchema =
                Arrays.stream(partitionFields.get()).anyMatch(pf -> containsFieldInSchema(dataSchema, pf));
        if (hasPartitionColNotInSchema && hasPartitionColInSchema) {
            throw new HoodieSchemaException(
                    "Partition columns could not be partially contained w/in the data schema");
        }

        if (hasPartitionColNotInSchema) {
            // when hasPartitionColNotInSchema is true and hasPartitionColInSchema is false, all
            // partition columns
    ...
    private Supplier<Exception> schemaNotFoundError() {
        return () ->
                new HoodieSchemaNotFoundException(
                        "No schema found for table at " + metaClient.getBasePath());
    }
}
