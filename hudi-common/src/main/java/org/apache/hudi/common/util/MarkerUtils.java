...
public class MarkerUtils {
    ...
    /**
      ...
      */
    public static String stripMarkerFolderPrefix(
            String fullMarkerPath, String basePath, String instantTime) {
        ValidationUtils.checkArgument(
                fullMarkerPath.contains(HoodieTableMetaClient.MARKER_EXTN),
                String.format(
                        "Using DIRECT markers but marker path does not contain extension: %s",
                        HoodieTableMetaClient.MARKER_EXTN));
        String markerRootPath =
                new StoragePath(
                                String.format(
                                        "%s/%s/%s", basePath, HoodieTableMetaClient.TEMPFOLDER_NAME, instantTime))
    ...
    /**
      ...
      */
    public static boolean doesMarkerTypeFileExist(HoodieStorage storage, String markerDir)
            throws IOException {
    ...
    public static Option<MarkerType> readMarkerType(HoodieStorage storage, String markerDir) {
        ...
        try {
            ...
            throw new HoodieIOException(
                    "Cannot read marker type file " + markerTypeFilePath + "; " + e.getMessage(), e);
        } finally {
    ...
    /**
      ...
      * @param markerDir marker directory.
      */
    public static void writeMarkerTypeToFile(
            MarkerType markerType, HoodieStorage storage, String markerDir) {
        StoragePath markerTypeFilePath = new StoragePath(markerDir, MARKER_TYPE_FILENAME);
        ...
        try {
            ...
            throw new HoodieException(
                    "Failed to create marker type file " + markerTypeFilePath + "; " + e.getMessage(), e);
        } finally {
    ...
    public static void deleteMarkerTypeFile(HoodieStorage storage, String markerDir) {
        ...
        try {
            ...
            throw new HoodieIOException(
                    "Cannot delete marker type file " + markerTypeFilePath + "; " + e.getMessage(), e);
        }
    }
    ...
    public static Map<String, Set<String>> readTimelineServerBasedMarkersFromFileSystem(
        ...
        try {
            if (storage.exists(dirPath)) {
                Predicate<StoragePathInfo> prefixFilter =
                        pathInfo -> pathInfo.getPath().getName().startsWith(MARKERS_FILENAME_PREFIX);
                Predicate<StoragePathInfo> markerTypeFilter =
                        pathInfo -> !pathInfo.getPath().getName().equals(MARKER_TYPE_FILENAME);
                return FSUtils.parallelizeSubPathProcess(
                        context,
                        storage,
                        dirPath,
                        parallelism,
                        prefixFilter.and(markerTypeFilter),
    ...
    /**
      ...
      */
    public static Set<String> readMarkersFromFile(
            StoragePath markersFilePath, StorageConfiguration<?> conf) {
    ...
    /**
      ...
      */
    public static Set<String> readMarkersFromFile(
            StoragePath markersFilePath, StorageConfiguration<?> conf, boolean ignoreException) {
        InputStream inputStream = null;
    ...
    /**
      ...
      */
    public static List<StoragePath> getAllMarkerDir(StoragePath tempPath, HoodieStorage storage)
            throws IOException {
    ...
    /**
      ...
      */
    public static boolean hasCommitConflict(
        ...
        Set<String> missingFileIDs =
                currentInstants.stream()
                        .flatMap(
                                instant -> {
                                    try {
    ...
    /**
      * Get Candidate Instant to do conflict checking: 1. Skip current writer related
      * instant(currentInstantTime) 2. Skip all instants after currentInstantTime 3. Skip dead writers
      * related instants based on heart-beat 4. Skip pending compaction instant (For now we don' do
      * early conflict check with compact action) Because we don't want to let pending compaction block
      * common writer.
      *
      * @param instants
      * @return
      */
    public static List<String> getCandidateInstants(
            HoodieActiveTimeline activeTimeline,
                        ...
                        instantPath -> {
                            try {
                                return !isHeartbeatExpired(
                                        markerDirToInstantTime(instantPath),
