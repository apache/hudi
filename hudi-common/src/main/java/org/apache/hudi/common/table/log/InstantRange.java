...
public abstract class InstantRange implements Serializable {
    ...
    public String toString() {
        return "InstantRange{"
                + "startInstant='"
                + (startInstant.isEmpty() ? "-INF" : startInstant.get())
                + '\''
                + ", endInstant='"
                + (endInstant.isEmpty() ? "+INF" : endInstant.get())
                + '\''
                + ", rangeType='"
    ...
    private static class OpenClosedRange extends InstantRange {
        ...
        public boolean isInRange(String instant) {
            boolean validAgainstStart =
                    HoodieTimeline.compareTimestamps(
                            instant, HoodieTimeline.GREATER_THAN, startInstant.get());
            // if there is an end instant, check against it, otherwise assume +INF and its always
            // valid.
            boolean validAgainstEnd =
                    endInstant
                            .map(
                                    e ->
                                            HoodieTimeline.compareTimestamps(
                                                    instant, HoodieTimeline.LESSER_THAN_OR_EQUALS, e))
                            .orElse(true);
    ...
    private static class OpenClosedRangeNullableBoundary extends InstantRange {
        ...
        public OpenClosedRangeNullableBoundary(String startInstant, String endInstant) {
            ...
            ValidationUtils.checkArgument(
                    !startInstant.isEmpty() || !endInstant.isEmpty(),
    ...
    private static class ClosedClosedRange extends InstantRange {
        ...
        public boolean isInRange(String instant) {
            boolean validAgainstStart =
                    HoodieTimeline.compareTimestamps(
                            instant, HoodieTimeline.GREATER_THAN_OR_EQUALS, startInstant.get());
            boolean validAgainstEnd =
                    endInstant
                            .map(
                                    e ->
            ...
            boolean validAgainstStart =
                    startInstant
                            .map(
                                    s ->
                                            HoodieTimeline.compareTimestamps(
                                                    instant, HoodieTimeline.GREATER_THAN_OR_EQUALS, s))
                            .orElse(true);
            boolean validAgainstEnd =
                    endInstant
                            .map(
    ...
    private static class CompositionRange extends InstantRange {
        ...
        public CompositionRange(List<InstantRange> instantRanges) {
            super(null, null);
            this.instantRanges =
                    Objects.requireNonNull(instantRanges, "Instant ranges should not be null");
        }
