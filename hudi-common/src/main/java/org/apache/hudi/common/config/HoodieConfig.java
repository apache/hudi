...
public class HoodieConfig implements Serializable {
    ...
    /**
      * Sets the default value of a config if user does not set it already. The default value can only
      * be set if the config property has a built-in default value or an infer function. When the infer
      * function is present, the infer function is used first to derive the config value based on other
      * configs. If the config value cannot be inferred, the built-in default value is used if present.
      *
    ...
    public <T> String getStringOrThrow(ConfigProperty<T> configProperty, String errorMessage)
