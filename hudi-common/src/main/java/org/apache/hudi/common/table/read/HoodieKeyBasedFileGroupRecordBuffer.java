...
/**
  ...
  * org.apache.hudi.common.table.log.HoodieMergedLogRecordReader} by calling the {@link
  * #processDataBlock} and {@link #processDeleteBlock} methods into a record key based map. The
  * records from the base file is accessed from an iterator object. These records are merged when the
...
public class HoodieKeyBasedFileGroupRecordBuffer<T> extends HoodieBaseFileGroupRecordBuffer<T> {
    ...
    public HoodieKeyBasedFileGroupRecordBuffer(
    ...
    @Override
    public void processNextDataRecord(T record, Map<String, Object> metadata, Serializable recordKey)
            throws IOException {
    ...
    public void processNextDeletedRecord(DeleteRecord deleteRecord, Serializable recordKey) {
        ...
        Option<DeleteRecord> recordOpt =
                doProcessNextDeletedRecord(deleteRecord, existingRecordMetadataPair);
        if (recordOpt.isPresent()) {
            records.put(
                    ...
                    Pair.of(
                            ...
                            readerContext.generateMetadataForRecord(
                                    (String) recordKey,
                                    recordOpt.get().getPartitionPath(),
