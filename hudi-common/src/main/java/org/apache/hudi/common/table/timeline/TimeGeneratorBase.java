    ...
    public TimeGeneratorBase(HoodieTimeGeneratorConfig config, StorageConfiguration<?> storageConf) {
        ...
        lockAcquireWaitTimeInMs =
                lockConfiguration
                        .getConfig()
                        .getInteger(
                                LOCK_ACQUIRE_WAIT_TIMEOUT_MS_PROP_KEY, DEFAULT_LOCK_ACQUIRE_WAIT_TIMEOUT_MS);
        // The maximum time to wait for each time generation to resolve the clock skew issue on
        // distributed hosts.
