...
public class HoodieAvroRecord<T extends HoodieRecordPayload> extends HoodieRecord<T> {
    ...
    public Object[] getColumnValues(
        ...
        return HoodieAvroUtils.getRecordColumnValues(
                this, columns, recordSchema, consistentLogicalTimestampEnabled);
    }
    ...
    public HoodieRecord prependMetaFields(
        ...
        try {
            ...
            return new HoodieAvroRecord<>(
                    getKey(),
                    new RewriteAvroPayload(newAvroRecord),
                    getOperation(),
    ...
    public HoodieRecord rewriteRecordWithNewSchema(
        ...
        try {
            ...
            return new HoodieAvroRecord<>(
                    getKey(),
                    new RewriteAvroPayload(rewriteRecord),
                    getOperation(),
                    this.currentLocation,
                    this.newLocation);
        } catch (IOException e) {
    ...
    public HoodieRecord truncateRecordKey(Schema recordSchema, Properties props, String keyFieldName)
        ...
        GenericRecord avroRecordPayload =
                (GenericRecord) getData().getInsertValue(recordSchema, props).get();
        avroRecordPayload.put(keyFieldName, StringUtils.EMPTY_STRING);
        return new HoodieAvroRecord<>(
                getKey(),
                new RewriteAvroPayload(avroRecordPayload),
                getOperation(),
                this.currentLocation,
                this.newLocation);
    }
    ...
    public boolean shouldIgnore(Schema recordSchema, Properties props) throws IOException {
        ...
        if (recordPayload instanceof BaseAvroPayload
                && ((BaseAvroPayload) recordPayload).canProduceSentinel()) {
