...
@ConfigClassProperty(
        name = "Consistency Guard Configurations",
        groupName = ConfigGroups.Names.WRITE_CLIENT,
        description =
                "The consistency guard related config options, to help talk to eventually consistent object storage."
                        + "(Tip: S3 is NOT eventually consistent anymore!)")
public class ConsistencyGuardConfig extends HoodieConfig {

    public static final ConfigProperty<String> ENABLE =
            ConfigProperty.key("hoodie.consistency.check.enabled")
                    .defaultValue("false")
                    .markAdvanced()
                    .sinceVersion("0.5.0")
                    .deprecatedAfter("0.7.0")
                    .withDocumentation(
                            "Enabled to handle S3 eventual consistency issue. This property is no longer required "
    ...
    public static final ConfigProperty<Long> OPTIMISTIC_CONSISTENCY_GUARD_SLEEP_TIME_MS =
            ConfigProperty.key("hoodie.optimistic.consistency.guard.sleep_time_ms")
                    .defaultValue(500L)
                    .markAdvanced()
                    .sinceVersion("0.6.0")
                    .withDocumentation(
    ...
    // config to enable OptimisticConsistencyGuard in finalizeWrite instead of
    // FailSafeConsistencyGuard
    public static final ConfigProperty<Boolean> OPTIMISTIC_CONSISTENCY_GUARD_ENABLE =
            ConfigProperty.key("_hoodie.optimistic.consistency.guard.enable")
                    .defaultValue(false)
                    .markAdvanced()
                    .sinceVersion("0.6.0")
                    .withDocumentation(
    ...
    public static class Builder {
        ...
        public Builder withOptimisticConsistencyGuardSleepTimeMs(long sleepTimeMs) {
            consistencyGuardConfig.setValue(
                    OPTIMISTIC_CONSISTENCY_GUARD_SLEEP_TIME_MS, String.valueOf(sleepTimeMs));
            return this;
        }

        public Builder withEnableOptimisticConsistencyGuard(boolean enableOptimisticConsistencyGuard) {
            consistencyGuardConfig.setValue(
                    OPTIMISTIC_CONSISTENCY_GUARD_ENABLE, String.valueOf(enableOptimisticConsistencyGuard));
            return this;
        }
    ...
    @Deprecated private static final String CONSISTENCY_CHECK_ENABLED_PROP = ENABLE.key();
    /** @deprecated use {@link #ENABLE} and its methods. */
    @Deprecated private static final String DEFAULT_CONSISTENCY_CHECK_ENABLED = ENABLE.defaultValue();
    ...
    private static final String INITIAL_CONSISTENCY_CHECK_INTERVAL_MS_PROP =
            INITIAL_CHECK_INTERVAL_MS.key();
    /** @deprecated use {@link #INITIAL_CHECK_INTERVAL_MS} and its methods. */
    @Deprecated
    private static long DEFAULT_INITIAL_CONSISTENCY_CHECK_INTERVAL_MS =
            INITIAL_CHECK_INTERVAL_MS.defaultValue();
    ...
    private static long DEFAULT_MAX_CONSISTENCY_CHECK_INTERVAL_MS =
            MAX_CHECK_INTERVAL_MS.defaultValue();
    /** @deprecated use {@link #MAX_CHECKS} and its methods. */
    @Deprecated private static final String MAX_CONSISTENCY_CHECKS_PROP = MAX_CHECKS.key();
    /** @deprecated use {@link #MAX_CHECKS} and its methods. */
    @Deprecated private static int DEFAULT_MAX_CONSISTENCY_CHECKS = MAX_CHECKS.defaultValue();
    /** @deprecated use {@link #OPTIMISTIC_CONSISTENCY_GUARD_SLEEP_TIME_MS} and its methods. */
    @Deprecated
    private static final String OPTIMISTIC_CONSISTENCY_GUARD_SLEEP_TIME_MS_PROP =
            OPTIMISTIC_CONSISTENCY_GUARD_SLEEP_TIME_MS.key();
    /** @deprecated use {@link #OPTIMISTIC_CONSISTENCY_GUARD_SLEEP_TIME_MS} and its methods. */
    @Deprecated
    private static long DEFAULT_OPTIMISTIC_CONSISTENCY_GUARD_SLEEP_TIME_MS_PROP =
            OPTIMISTIC_CONSISTENCY_GUARD_SLEEP_TIME_MS.defaultValue();
    /** @deprecated use {@link #OPTIMISTIC_CONSISTENCY_GUARD_ENABLE} and its methods. */
    @Deprecated
    private static final String ENABLE_OPTIMISTIC_CONSISTENCY_GUARD =
            OPTIMISTIC_CONSISTENCY_GUARD_ENABLE.key();
    /** @deprecated use {@link #OPTIMISTIC_CONSISTENCY_GUARD_ENABLE} and its methods. */
    @Deprecated
    private static boolean DEFAULT_ENABLE_OPTIMISTIC_CONSISTENCY_GUARD =
            OPTIMISTIC_CONSISTENCY_GUARD_ENABLE.defaultValue();
}
