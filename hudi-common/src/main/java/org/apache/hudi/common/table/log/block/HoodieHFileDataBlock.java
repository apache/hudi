...
/**
  * HoodieHFileDataBlock contains a list of records stored inside an HFile format. It is used with
  * the HFile base file format.
  */
public class HoodieHFileDataBlock extends HoodieDataBlock {
        ...
        return HoodieIOFactory.getIOFactory(storage)
                ...
                .serializeRecordsToLogBlock(
                        ...
                        Collections.singletonMap(
                                HFILE_COMPRESSION_ALGORITHM_NAME.key(), compressionCodec.get()));
    }
    ...
    protected <T> ClosableIterator<HoodieRecord<T>> deserializeRecords(
        ...
        HoodieStorage inlineStorage =
                getBlockContentLocation().get().getStorage().newInstance(pathForReader, storageConf);
        // Read the content
    ...
    protected <T> ClosableIterator<HoodieRecord<T>> lookupRecords(
            List<String> sortedKeys, boolean fullKey) throws IOException {
        ...
        HoodieStorage inlineStorage =
                getBlockContentLocation().get().getStorage().newInstance(inlinePath, inlineConf);
        ...
        try (final HoodieAvroHFileReaderImplBase reader =
            ...
            final ClosableIterator<HoodieRecord<IndexedRecord>> recordIterator =
                    fullKey
                            ? reader.getRecordsByKeysIterator(sortedKeys, readerSchema)
