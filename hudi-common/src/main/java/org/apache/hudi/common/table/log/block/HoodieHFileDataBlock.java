...
public class HoodieHFileDataBlock extends HoodieDataBlock {
  ...
  private void readWithInlineFS(List<String> keys) throws IOException {
    ...
    HoodieHFileReader reader = new HoodieHFileReader(inlineConf, inlinePath, cacheConf, inlinePath.getFileSystem(inlineConf), this.keyField);
  ...
  protected void deserializeRecords() throws IOException {
    ...
    // Read the content
    HoodieHFileReader reader = new HoodieHFileReader<>(getContent().get(), this.keyField);
