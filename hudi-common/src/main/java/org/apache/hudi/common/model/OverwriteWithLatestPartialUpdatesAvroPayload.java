 ...
 *
 *  Instead of updating the entire record, this payload class will use the "_hoodie_change_cols" String column to
 *  determine which fields should be upserted for that record. The fields to updated will be stored in "_hoodie_change_cols"
 *  as a comma separated string. This payload sorts records during precombine phase to avoid out-of-order record issues.
 *  NOTE: Out-of-order records between separate batches are dropped as they cannot use the same sorting logic.
 *
...
public class OverwriteWithLatestPartialUpdatesAvroPayload extends OverwriteWithLatestAvroPayload {
  ...
  private static final Logger LOG = (Logger) LogManager.getLogger(OverwriteWithLatestPartialUpdatesAvroPayload.class);
