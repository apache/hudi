...
public class HoodieTableConfig extends HoodieConfig {
  ...
  static boolean validateConfigVersion(ConfigProperty<?> configProperty, HoodieTableVersion tableVersion) {
    ...
    valid = valid ||
        (tableVersion.lesserThan(HoodieTableVersion.EIGHT)
            && (configProperty.key().equals(KEY_GENERATOR_CLASS_NAME.key()) || configProperty.key().equals(KEY_GENERATOR_TYPE.key())));
    if (!valid) {
