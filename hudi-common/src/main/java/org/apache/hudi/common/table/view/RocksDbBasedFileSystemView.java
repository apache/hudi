...
/**
  * A file-system view implementation on top of embedded Rocks DB store. For each table : 3 column
  * Family is added for storing (1) File-Slices and Data Files for View lookups (2) Pending
  * compaction operations (3) Partitions tracked
  *
  * <p>Fine-grained retrieval API to fetch latest file-slice and data-file which are common
  * operations for ingestion/compaction are supported.
  *
  * <p>TODO: vb The current implementation works in embedded server mode where each restarts blows
  * away the view stores. To support view-state preservation across restarts, Hoodie timeline also
  * needs to be stored inorder to detect changes to timeline across restarts.
  */
public class RocksDbBasedFileSystemView extends IncrementalTimelineSyncFileSystemView {
    ...
    public RocksDbBasedFileSystemView(
    ...
    @Override
    protected void resetPendingCompactionOperations(
                    ...
                    operations.forEach(
                            opPair ->
                                    rocksDB.putInBatch(
                                            ...
                                            schemaHelper.getKeyForPendingCompactionLookup(
    ...
    @Override
    protected void resetPendingLogCompactionOperations(
            Stream<Pair<String, CompactionOperation>> operations) {
        rocksDB.writeBatch(
                batch -> {
                    operations.forEach(
                            opPair ->
                                    rocksDB.putInBatch(
                                            batch,
                                            schemaHelper.getColFamilyForPendingLogCompaction(),
                                            schemaHelper.getKeyForPendingLogCompactionLookup(
                                                    opPair.getValue().getFileGroupId()),
                                            opPair));
        ...
        rocksDB.writeBatch(
                batch ->
                        operations.forEach(
                                opInstantPair -> {
                                    ValidationUtils.checkArgument(
                                            !isPendingLogCompactionScheduledForFileId(
                                                    opInstantPair.getValue().getFileGroupId()),
        ...
        rocksDB.writeBatch(
                batch ->
                        operations.forEach(
                                opInstantPair -> {
                                    ValidationUtils.checkArgument(
                                            getPendingLogCompactionOperationWithInstant(
                                                            opInstantPair.getValue().getFileGroupId())
                                                    != null,
    ...
    public Stream<Pair<HoodieFileGroupId, HoodieInstant>> fetchFileGroupsInPendingClustering() {
        return rocksDB
                .<Pair<HoodieFileGroupId, HoodieInstant>>prefixSearch(
                        schemaHelper.getColFamilyForFileGroupsInPendingClustering(), "")
                .map(Pair::getValue);
    }
    ...
    void resetFileGroupsInPendingClustering(Map<HoodieFileGroupId, HoodieInstant> fgIdToInstantMap) {
        LOG.info(
                "Resetting file groups in pending clustering to ROCKSDB based file-system view at "
                        + config.getRocksdbBasePath()
                        + ", Total file-groups="
                        + fgIdToInstantMap.size());

        // Delete all replaced file groups
        rocksDB.prefixDelete(schemaHelper.getColFamilyForFileGroupsInPendingClustering(), "part=");
        // Now add new entries
        addFileGroupsInPendingClustering(
                fgIdToInstantMap.entrySet().stream()
    ...
    void addFileGroupsInPendingClustering(Stream<Pair<HoodieFileGroupId, HoodieInstant>> fileGroups) {
        rocksDB.writeBatch(
                batch ->
                        fileGroups.forEach(
                                fgIdToClusterInstant -> {
                                    ValidationUtils.checkArgument(
                                            !isPendingClusteringScheduledForFileId(fgIdToClusterInstant.getLeft()),
                                    ...
                                    rocksDB.putInBatch(
                                            ...
                                            schemaHelper.getKeyForFileGroupsInPendingClustering(
                                                    fgIdToClusterInstant.getKey()),
                                            fgIdToClusterInstant);
                                }));
    }

    @Override
    void removeFileGroupsInPendingClustering(
            Stream<Pair<HoodieFileGroupId, HoodieInstant>> fileGroups) {
        rocksDB.writeBatch(
                batch ->
                        fileGroups.forEach(
                                fgToPendingClusteringInstant -> {
                                    ValidationUtils.checkArgument(
                                            !isPendingClusteringScheduledForFileId(
    ...
    @Override
    protected Option<Pair<String, CompactionOperation>> getPendingCompactionOperationWithInstant(
    ...
    @Override
    protected Option<Pair<String, CompactionOperation>> getPendingLogCompactionOperationWithInstant(
            HoodieFileGroupId fgId) {
    ...
    protected void storePartitionView(String partitionPath, List<HoodieFileGroup> fileGroups) {
        LOG.info(
                "Resetting and adding new partition ("
                        + partitionPath
                        + ") to ROCKSDB based file-system view at "
                        + config.getRocksdbBasePath()
                        + ", Total file-groups="
                        + fileGroups.size());
        ...
        rocksDB.prefixDelete(
                schemaHelper.getColFamilyForView(),
        ...
        fileGroups.forEach(
                fg ->
                        rocksDB.writeBatch(
                                batch ->
                                        fg.getAllFileSlicesIncludingInflight()
                                                .forEach(
                                                        fs -> {
                                                            rocksDB.putInBatch(
                                                                    batch,
                                                                    schemaHelper.getColFamilyForView(),
                                                                    schemaHelper.getKeyForSliceView(fg, fs),
                                                                    fs);
                                                            fs.getBaseFile()
                                                                    .ifPresent(
                                                                            df ->
                                                                                    rocksDB.putInBatch(
                                                                                            batch,
                                                                                            schemaHelper.getColFamilyForView(),
                                                                                            schemaHelper.getKeyForDataFileView(fg, fs),
                                                                                            df));
                                                        })));
    ...
    protected void applyDeltaFileSlicesToPartitionView(
            String partition, List<HoodieFileGroup> deltaFileGroups, DeltaApplyMode mode) {
        rocksDB.writeBatch(
                batch ->
                        deltaFileGroups.forEach(
                                fg ->
                                        fg.getAllRawFileSlices()
                                                .map(
                                                        fs -> {
                                                            Option<FileSlice> oldSliceOption =
                                                                    fetchLatestFileSliceBeforeOrOn(
                                                                            partition, fs.getFileId(), fs.getBaseInstantTime());
                                                            if (!oldSliceOption.isPresent()
                                                                    || !shouldMergeFileSlice(oldSliceOption.get(), fs)) {
                                                                return fs;
                                                            } else {
                                                                ...
                                                                Map<String, HoodieLogFile> logFiles =
                                                                        oldSlice
                                                                                .getLogFiles()
                                                                                .map(
                                                                                        lf ->
                                                                                                Pair.of(
                                                                                                        FSUtils.getPathWithoutSchemeAndAuthority(
                                                                                                                        lf.getPath())
                                                                                                                .toString(),
                                                                                                        lf))
                                                                    ...
                                                                    case ADD:
                                                                        {
                                                                            ...
                                                                            Map<String, HoodieLogFile> newLogFiles =
                                                                                    new HashMap<>(logFiles);
                                                                            deltaLogFiles.entrySet().stream()
                                                                                    .filter(e -> !logFiles.containsKey(e.getKey()))
                                                                                    .forEach(p -> newLogFiles.put(p.getKey(), p.getValue()));
                                                                            newLogFiles.values().forEach(newFileSlice::addLogFile);
                                                                            LOG.info(
                                                                                    "Adding back new File Slice after add FS="
                                                                                            + newFileSlice);
                                                                            return newFileSlice;
                                                                        }
                                                                    case REMOVE:
                                                                        {
                                                                            ...
                                                                            fs.getBaseFile()
                                                                                    .orElseGet(
                                                                                            () -> {
                                                                                                oldSlice
                                                                                                        .getBaseFile()
                                                                                                        .ifPresent(newFileSlice::setBaseFile);
                                                                                                return null;
                                                                                            });
                                                                            ...
                                                                            if (newFileSlice.getBaseFile().isPresent()
                                                                                    || (newFileSlice.getLogFiles().count() > 0)) {
                                                                                LOG.info(
                                                                                        "Adding back new file-slice after remove FS="
                                                                                                + newFileSlice);
                                                                                return newFileSlice;
                                                                            }
                                                                            return null;
    ...
    Stream<Pair<String, CompactionOperation>> fetchPendingCompactionOperations() {
        return rocksDB
                .<Pair<String, CompactionOperation>>prefixSearch(
                        schemaHelper.getColFamilyForPendingCompaction(), "")
                .map(Pair::getValue);
    }
    ...
    void resetBootstrapBaseFileMapping(Stream<BootstrapBaseFileMapping> bootstrapBaseFileStream) {
        rocksDB.writeBatch(
                batch -> {
                    bootstrapBaseFileStream.forEach(
                            externalBaseFile -> {
                                rocksDB.putInBatch(
                                        batch,
                                        schemaHelper.getColFamilyForBootstrapBaseFile(),
        ...
        rocksDB.writeBatch(
                batch -> {
                    bootstrapBaseFileStream.forEach(
                            externalBaseFile -> {
                                ValidationUtils.checkArgument(
                                        !isBootstrapBaseFilePresentForFileId(externalBaseFile.getFileGroupId()),
                                        "Duplicate FileGroupId found in external data file. FgId :"
    ...
    Stream<BootstrapBaseFileMapping> fetchBootstrapBaseFiles() {
        return rocksDB
                .<BootstrapBaseFileMapping>prefixSearch(schemaHelper.getColFamilyForBootstrapBaseFile(), "")
                .map(Pair::getValue);
    }
    ...
    Stream<HoodieFileGroup> fetchAllStoredFileGroups() {
        return getFileGroups(
                rocksDB
                        .<FileSlice>prefixSearch(
                                schemaHelper.getColFamilyForView(), schemaHelper.getPrefixForSliceView())
                        .map(Pair::getValue));
    }
                                        ...
                                        ((x == null)
                                                ...
                                                : (y == null)
                                                        ...
                                                        : HoodieTimeline.compareTimestamps(
                                                                        ...
                                                                        y.getBaseInstantTime())
                                                                ? x
                                                                : y)));
    }

    public Option<FileSlice> fetchLatestFileSliceBeforeOrOn(
            String partitionPath, String fileId, String instantTime) {
    ...
    protected void resetReplacedFileGroups(
        ...
        LOG.info(
                "Resetting replacedFileGroups to ROCKSDB based file-system view at "
                        + config.getRocksdbBasePath()
                        + ", Total file-groups="
    ...
    @Override
    protected void addReplacedFileGroups(
            final Map<HoodieFileGroupId, HoodieInstant> replacedFileGroups) {
        Map<String, List<Map.Entry<HoodieFileGroupId, HoodieInstant>>> partitionToReplacedFileGroups =
                replacedFileGroups.entrySet().stream()
                        .collect(Collectors.groupingBy(e -> e.getKey().getPartitionPath()));
        partitionToReplacedFileGroups.entrySet().stream()
                .forEach(
                        partitionToReplacedFileGroupsEntry -> {
                            String partitionPath = partitionToReplacedFileGroupsEntry.getKey();
                            List<Map.Entry<HoodieFileGroupId, HoodieInstant>> replacedFileGroupsInPartition =
                                    partitionToReplacedFileGroupsEntry.getValue();

                            // Now add them
                            rocksDB.writeBatch(
                                    batch ->
                                            replacedFileGroupsInPartition.stream()
                                                    .forEach(
                                                            fgToReplacedInstant -> {
                                                                rocksDB.putInBatch(
                                                                        batch,
                                                                        schemaHelper.getColFamilyForReplacedFileGroups(),
                                                                        schemaHelper.getKeyForReplacedFileGroup(
                                                                                fgToReplacedInstant.getKey()),
                                                                        fgToReplacedInstant.getValue());
                                                            }));

                            LOG.info(
                                    "Finished adding replaced file groups to  partition ("
                                            + partitionPath
                                            + ") to ROCKSDB based view at "
                                            + config.getRocksdbBasePath()
                                            + ", Total file-groups="
                                            + partitionToReplacedFileGroupsEntry.getValue().size());
                        });
    }

    @Override
    protected void removeReplacedFileIdsAtInstants(Set<String> instants) {
        // TODO can we make this more efficient by storing reverse mapping (Instant -> FileGroupId)
        // as well?
        Stream<String> keysToDelete =
                rocksDB
                        .<HoodieInstant>prefixSearch(schemaHelper.getColFamilyForReplacedFileGroups(), "")
        ...
        rocksDB.writeBatch(
                batch ->
                        keysToDelete.forEach(
                                key ->
                                        rocksDB.deleteInBatch(
                                                batch, schemaHelper.getColFamilyForReplacedFileGroups(), key)));
    }

    @Override
    protected boolean hasReplacedFilesInPartition(String partitionPath) {
        return rocksDB
                .<HoodieInstant>prefixSearch(
                        schemaHelper.getColFamilyForReplacedFileGroups(),
    ...
    private static boolean shouldMergeFileSlice(FileSlice oldSlice, FileSlice newSlice) {
        // 1. the new file slice has no compaction been scheduled, it should always be merged into
        // the latest file slice;
        ...
        // 3. the new file slice has compaction been scheduled, should not merge if old and new base
        // instant time is different.
    ...
    private static boolean isFileSliceWithoutCompactionBarrier(FileSlice fileSlice) {
        return fileSlice
                .getLogFiles()
                .anyMatch(logFile -> logFile.getDeltaCommitTime().equals(fileSlice.getBaseInstantTime()));
    }
