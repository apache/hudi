...
public abstract class MetricQueryState implements Serializable {
  ...
  protected MetricQueryState(QueryRotation rotation, float[] paddedQuery) {
    ...
    this.rotatedQuery = rotation.apply(paddedQuery);
  ...
  /**
   * Metric-specific finish from the residual inner-product estimate {@code ripW}.
   */
  public abstract double distance(double ripW, float centerRip, float residualNorm,
                                  float vectorNorm, ClusterQuery cq);

  /**
   * Optimistic distance for pass-1 pruning, using the residual-query norm.
   */
  public final double optimisticDistance(double ripW, float err1, float centerRip,
                                         float residualNorm, float vectorNorm,
                                         ClusterQuery cq) {
    double errAbs = (double) err1 * Math.sqrt(cq.wNormSq);
    return distance(ripW + errAbs, centerRip, residualNorm, vectorNorm, cq);
  }
  ...
  public static MetricQueryState create(VectorDistanceMetric metric, QueryRotation rotation,
    ...
    switch (metric) {
      case L2:
        return new L2QueryState(rotation, paddedQuery);
      case DOT_PRODUCT:
        return new DotQueryState(rotation, paddedQuery);
      case COSINE:
        return new CosineQueryState(rotation, paddedQuery, assumeNormalized);
      default:
  ...
  public static final class L2QueryState extends MetricQueryState {
    ...
    @Override
    public double distance(double ripW, float centerRip, float residualNorm,
                           float vectorNorm, ClusterQuery cq) {
