...
/**
  * Create PreCommitFileSystemView by only filtering instants that are of interest. For example, we
  * want to exclude other inflight instants. This is achieved by combining 1) FileSystemView with
  * completed commits 2) Using list of files written/replaced by inflight instant that we are
  * validating
  */
public class HoodieTablePreCommitFileSystemView {
    ...
    public HoodieTablePreCommitFileSystemView(
    ...
    public final Stream<HoodieBaseFile> getLatestBaseFiles(String partitionStr) {
        ...
        List<String> replacedFileIdsForPartition =
                partitionToReplaceFileIds.getOrDefault(partitionStr, Collections.emptyList());
                ...
                filesWritten.stream()
                        ...
                        .collect(
                                Collectors.toMap(
                                        ...
                                        writeStat ->
                                                new HoodieBaseFile(
                                                        new StoragePath(tableMetaClient.getBasePath(), writeStat.getPath())
                                                                .toString(),
                                                        writeStat.getFileId(),
                                                        preCommitInstantTime,
                                                        null)));

        Stream<HoodieBaseFile> committedBaseFiles =
                this.completedCommitsFileSystemView.getLatestBaseFiles(partitionStr);
        Map<String, HoodieBaseFile> allFileIds =
                committedBaseFiles
