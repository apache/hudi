...
/**
  * Use a private buffer for the read/write/seek operations of the RandomAccessFile to reduce the
  * number of disk operations.
  *
  * <p>S C L/E Buffer: |----+---| File: +---------------D-------------------+ S. startPosition - file
  * position of start of the buffer. C. currentPosition - file position of current pointer in the
  * buffer. L. validLastPosition - file position of the last byte in the buffer. (This is same as
  * endPosition of the buffer. Except for the last file block). S-C--L--E (A) Buffer: |-+--+--| File:
  * +-----------------------------------D E. endPosition() - end file position of the current buffer.
  * D. DiskPosition - Position in the file pointed by underlying RandomAccessFile. When reading from
  * the file, diskPosition aligns with startPosition. When writing to the file, diskPosition(D)
  * aligns with validLastPosition(L/E).
  *
  * <p>A. AvailableSpace - Space between validLastPosition(L) and EndPosition(E) is the available
  * space in buffer, when writing/appending records into the buffer/file.
  *
  * <p>Note: Based on BufferedRandomAccessFile implementation in Apache/Cassandra. - adopted from
  * org.apache.cassandra.io Copyright: 2015-2019 The Apache Software Foundation Home page:
  * http://cassandra.apache.org/ License: http://www.apache.org/licenses/LICENSE-2.0
  */
public final class BufferedRandomAccessFile extends RandomAccessFile {
    ...
    /**
      * If the diskPosition differs from the startPosition, flush the data in the buffer and
      * realign/fill the buffer at startPosition.
      *
      * @throws IOException
      */
    ...
    /**
      * If the new seek position is in the buffer, adjust the currentPosition. If the new seek position
      * is outside of the buffer, flush the contents to the file and reload the buffer corresponding to
      * the position.
      *
      * <p>We logically view the file as group blocks, where each block will perfectly fit into the
      * buffer (except for the last block). Given a position to seek, we identify the block to be
      * loaded using BUFFER_BOUNDARY_MASK.
      *
      * <p>When dealing with the last block, we will have extra space between validLastPosition and
      * endPosition of the buffer.
      *
    ...
    /**
      * If space is available at the end of the buffer, start using it. Otherwise, flush the unwritten
      * data into the file and load the buffer corresponding to startPosition.
      *
      * @throws IOException
      */
    ...
    /**
      * Given a byte array, offset in the array and length of bytes to be written, update the
      * buffer/file.
      *
