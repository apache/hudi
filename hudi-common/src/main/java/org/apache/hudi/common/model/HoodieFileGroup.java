...
public class HoodieFileGroup implements Serializable {
    ...
    /**
      * Potentially add a new file-slice by adding base-instant time A file-slice without any data-file
      * and log-files can exist (if a compaction just got requested).
      */
    ...
    public void addBaseFile(HoodieBaseFile dataFile) {
        if (!fileSlices.containsKey(dataFile.getCommitTime())) {
            fileSlices.put(
                    dataFile.getCommitTime(), new FileSlice(fileGroupId, dataFile.getCommitTime()));
        }
    ...
    public String getBaseInstantTime(
            CompletionTimeQueryView completionTimeQueryView, HoodieLogFile logFile) {
        ...
        Option<String> completionTimeOpt =
                completionTimeQueryView.getCompletionTime(
                        fileSlices.firstKey(), logFile.getDeltaCommitTime());
        if (completionTimeOpt.isPresent()) {
            for (String commitTime : fileSlices.keySet()) {
                // find the largest commit time that is smaller than the log delta commit completion
                // time
                if (HoodieTimeline.compareTimestamps(
                        completionTimeOpt.get(), GREATER_THAN_OR_EQUALS, commitTime)) {
                    return commitTime;
                }
            }
    ...
    private boolean isFileSliceCommitted(FileSlice slice) {
        if (!compareTimestamps(
                slice.getBaseInstantTime(), LESSER_THAN_OR_EQUALS, lastInstant.get().getTimestamp())) {
            return false;
        }
    ...
    public Stream<FileSlice> getAllFileSlicesBeforeOn(String maxInstantTime) {
        return fileSlices.values().stream()
                .filter(
                        slice ->
                                compareTimestamps(
                                        slice.getBaseInstantTime(), LESSER_THAN_OR_EQUALS, maxInstantTime));
    }

    /**
      * Gets the latest slice - this can contain either.
      *
      * <p>- just the log files without data file - (or) data file with 0 or more log files
      */
