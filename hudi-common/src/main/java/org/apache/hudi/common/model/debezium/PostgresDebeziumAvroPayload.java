...
public class PostgresDebeziumAvroPayload extends AbstractDebeziumAvroPayload {
    ...
    @Override
    protected boolean shouldPickCurrentRecord(
            IndexedRecord currentRecord, IndexedRecord insertRecord, Schema schema) throws IOException {
        Long insertSourceLSN =
                extractLSN(insertRecord)
                        .orElseThrow(
                                () ->
                                        new HoodieDebeziumAvroPayloadException(
                                                String.format(
                                                        "%s cannot be null in insert record: %s",
                                                        DebeziumConstants.FLATTENED_LSN_COL_NAME, insertRecord)));
        Option<Long> currentSourceLSNOpt = extractLSN(currentRecord);
        // Pick the current value in storage only if its LSN is latest compared to the LSN of the
        // insert value
    ...
    public Option<IndexedRecord> combineAndGetUpdateValue(
        ...
        Option<IndexedRecord> insertOrDeleteRecord =
                super.combineAndGetUpdateValue(currentValue, schema, properties);
    ...
    private void mergeToastedValuesIfPresent(
            IndexedRecord incomingRecord, IndexedRecord currentRecord) {
        ...
        fields.forEach(
                field -> {
                    ...
                    if (((GenericData.Record) incomingRecord).get(field.name()) != null
                        ...
                        ((GenericData.Record) incomingRecord)
                                .put(field.name(), ((GenericData.Record) currentRecord).get(field.name()));
                    }
                });
    }

    /**
      * Returns true if a column is either of type string or a union of one or more strings that
      * contain a debezium toasted value.
      *
    ...
    private boolean containsStringToastedValues(IndexedRecord incomingRecord, Schema.Field field) {
        return ((field.schema().getType() == Schema.Type.STRING
                ...
                && ((CharSequence) ((GenericData.Record) incomingRecord).get(field.name())).length()
                        == DEBEZIUM_TOASTED_VALUE.length()
                && DEBEZIUM_TOASTED_VALUE.equals(
                        ((CharSequence) ((GenericData.Record) incomingRecord).get(field.name())).toString()));
    }

    /**
      * Returns true if a column is either of type bytes or a union of one or more bytes that contain a
      * debezium toasted value.
      *
      * @param incomingRecord The incoming avro record
      * @param field the column of interest
      * @return
    ...
    private boolean containsBytesToastedValues(IndexedRecord incomingRecord, Schema.Field field) {
        return ((field.schema().getType() == Schema.Type.BYTES
                        || (field.schema().getType() == Schema.Type.UNION
                                && field.schema().getTypes().stream()
                                        .anyMatch(s -> s.getType() == Schema.Type.BYTES)))
                // Check length first as an optimization
                && ((ByteBuffer) ((GenericData.Record) incomingRecord).get(field.name())).array().length
                        == DEBEZIUM_TOASTED_VALUE.length()
                && DEBEZIUM_TOASTED_VALUE.equals(
                        fromUTF8Bytes(
                                ((ByteBuffer) ((GenericData.Record) incomingRecord).get(field.name())).array())));
    }
}
