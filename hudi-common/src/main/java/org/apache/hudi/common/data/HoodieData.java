...
/**
  * An interface abstracting a container holding a collection of objects of type {@code T} allowing
  * to perform common transformation on it.
  *
  * <p>This abstraction provides common API implemented by
  *
  * <ol>
  *   <li>In-memory implementation ({@code HoodieListData}, {@code HoodieListPairData}), where all
  *       objects are held in-memory by the executing process
  *   <li>RDD-based implementation ({@code HoodieJavaRDD}, etc), where underlying collection is held
  ...
  *
  * <ul>
  *   <li>All non-terminal* operations are executed lazily (for ex, {@code map}, {@code filter}, etc)
  *   <li>All terminal operations are executed eagerly, executing all previously accumulated
  *       transformations. Note that, collection could not be re-used after invoking terminal
  *       operation on it.
  * </ul>
  *
...
public interface HoodieData<T> extends Serializable {
    ...
    /**
      ...
      *
      * <p>Use this method only when you call {@link #unpersist()} at some later point for the same
      * {@link HoodieData}. Otherwise, use {@link #persist(String, HoodieEngineContext,
      * HoodieDataCacheKey)} instead for auto-unpersist at the end of a client write operation.
      */
    void persist(String level);

    /**
      * Persists the data w/ provided {@code level} (if applicable), and cache the data's ids within
      * the {@code engineContext}.
      */
    ...
    /**
      ...
      *
      * <p>NOTE: This is a terminal operation
      */
    long count();
    ...
    /**
      * Maps every element in the collection's partition (if applicable) by applying provided mapping
      * {@code func} to every collection's partition
      *
      * <p>This is an intermediate operation
      *
      * @param func serializable map function accepting {@link Iterator} of a single partition's
      *     elements and returning a new {@link Iterator} mapping every element of the partition into a
      *     new one
      * @param preservesPartitioning whether to preserve partitioning in the resulting collection
      * @param <O> output object type
      * @return {@link HoodieData<O>} holding mapped elements
      */
    <O> HoodieData<O> mapPartitions(
    ...
    /**
      ...
      * a single collection
      *
      * <p>NOTE: That this operation will convert container from {@link HoodieData} to {@link
      * HoodiePairData}
      *
      * <p>This is an intermediate operation
      */
    <K, V> HoodiePairData<K, V> flatMapToPair(
            SerializableFunction<T, Iterator<? extends Pair<K, V>>> func);

    /**
      * Maps every element in the collection using provided mapping {@code func} into a {@link Pair<K,
      * V>} of elements {@code K} and {@code V}
      *
      * <p>This is an intermediate operation
      *
      * @param func serializable map function
    ...
    /**
      * Returns new instance of {@link HoodieData} collection only containing elements matching
      * provided {@code filterFunc} (ie ones it returns true on)
      *
    ...
    /**
      * Unions {@link HoodieData} with another instance of {@link HoodieData}. Note that, it's only
      * able to union same underlying collection implementations.
      *
      * <p>This is a stateful intermediate operation
      *
      * @param other {@link HoodieData} collection
      * @return {@link HoodieData<T>} holding superset of elements of this and {@code other}
      *     collections
      */
    ...
    /**
      ...
      *
      * <p>At the end of a write operation, we manually unpersist the {@link HoodieData} associated
      * with that writer. Therefore, in multi-writer scenario, we need to use both {@code basePath} and
      * {@code instantTime} to identify {@link HoodieData}s.
      */
