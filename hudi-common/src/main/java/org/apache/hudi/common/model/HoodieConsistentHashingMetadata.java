...
public class HoodieConsistentHashingMetadata implements Serializable {
    ...
    @JsonIgnore protected List<ConsistentHashingNode> childrenNodes = new ArrayList<>();
    /** Used to indicate if the metadata is newly created, rather than read from the storage. */
    @JsonIgnore private boolean firstCreated;
    ...
    public HoodieConsistentHashingMetadata(
            ...
            @JsonProperty("nodes") List<ConsistentHashingNode> nodes) {
    ...
    /**
      * Only used for creating new hashing metadata. Construct default metadata with all bucket's file
      * group uuid initialized
      */
    public HoodieConsistentHashingMetadata(String partitionPath, int numBuckets) {
        this(
                ...
                numBuckets,
                0,
                constructDefaultHashingNodes(partitionPath, numBuckets));
        this.firstCreated = true;
    }

    private static List<ConsistentHashingNode> constructDefaultHashingNodes(
            String partitionPath, int numBuckets) {
    ...
    private static String generateUUID(String partitionPath, long bucketStart, long bucketEnd) {
        ...
        System.arraycopy(
                partitionPathBytes, 0, combinedBytes, longBytes.length, partitionPathBytes.length);
    ...
    /**
      * Get instant time from the hashing metadata filename Pattern of the filename:
      * <instant>.HASHING_METADATA_FILE_SUFFIX
      */
