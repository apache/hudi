...
public class IteratorConverters {
  ...
  public interface IteratorConverter<T, O> {
     ...
     */
    O convert(HoodieReaderContext<T> readerContext, T row, Option<String> orderingFieldName, Supplier<Boolean> isDelete);
     ...
     * @return Iterator element of type O.
     */
    O convert(HoodieReaderContext<T> readerContext, BufferedRecord<T> record);
  }
  ...
  public static class RecordKeyIteratorConverter<T> extends BaseIteratorConverter<T, String> {
    ...
    @Override
    public String convert(HoodieReaderContext<T> readerContext, T row, Option<String> orderingFieldName, Supplier<Boolean> isDelete) {
      return readerContext.getRecordKey(row, readerContext.getSchemaHandler().getRequiredSchema());
    }

    @Override
    public String convert(HoodieReaderContext<T> readerContext, BufferedRecord<T> record) {
  ...
  public static class RowIteratorConverter<T> extends BaseIteratorConverter<T, T> {
    ...
    @Override
    public T convert(HoodieReaderContext<T> readerContext, T row, Option<String> orderingFieldName, Supplier<Boolean> isDelete) {
      T sealedRow = readerContext.seal(row);
      return outputConverter.map(converter -> converter.apply(sealedRow)).orElse(sealedRow);
    }

    @Override
    public T convert(HoodieReaderContext<T> readerContext, BufferedRecord<T> record) {
  ...
  public static class HoodieRecordIteratorConverter<T> extends BaseIteratorConverter<T, HoodieRecord<T>> {
    ...
    @Override
    public HoodieRecord<T> convert(HoodieReaderContext<T> readerContext, T row, Option<String> orderingFieldName, Supplier<Boolean> isDelete) {
      T sealedRow = readerContext.seal(row);
    ...
    @Override
    public HoodieRecord<T> convert(HoodieReaderContext<T> readerContext, BufferedRecord<T> record) {
      // do not need seal for DELETE records as they come from buffered log records from spillable map
