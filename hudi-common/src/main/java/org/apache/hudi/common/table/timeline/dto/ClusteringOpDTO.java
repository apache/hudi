...
public class ClusteringOpDTO {
    ...
    public static ClusteringOpDTO fromClusteringOp(
            HoodieFileGroupId fileGroupId, HoodieInstant instant) {
    ...
    public static Pair<HoodieFileGroupId, HoodieInstant> toClusteringOperation(ClusteringOpDTO dto) {
        return Pair.of(
                new HoodieFileGroupId(dto.partitionPath, dto.fileId),
                new HoodieInstant(
                        HoodieInstant.State.valueOf(dto.instantState), dto.instantAction, dto.instantTime));
    }
}
