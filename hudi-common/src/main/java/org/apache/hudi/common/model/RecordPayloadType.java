...
public enum RecordPayloadType {
    ...
    @EnumFieldDescription(
            "A payload to wrap a existing Hoodie Avro Record. Useful to create a HoodieRecord over existing GenericRecords.")
    ...
    @EnumFieldDescription(
            "The only difference with HOODIE_AVRO_DEFAULT is that this does not track the event time metadata for efficiency")
    ...
    @EnumFieldDescription(
            "Honors ordering field in preCombine and overwrites storage with latest delta record in combineAndGetUpdateValue")
    ...
    @EnumFieldDescription(
            "A record payload to validate the duplicate key for INSERT statement in spark-sql.")
