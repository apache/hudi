...
public interface TableFileSystemView {
    ...
    interface SliceViewWithLatestSlice {
        ...
        /**
          * Stream all the latest file slices in the given partition without caching the file group
          * mappings.
          *
          * <p>This is useful for some table services such as compaction and clustering, these services
          * may search around the files to clean within some ancient data partitions, if there triggers a
          * full table service for enormous number of partitions, the cache could cause a huge memory
          * pressure to the timeline server which induces an OOM exception.
          *
          * <p>The caching of these file groups does not benefit to writers most often because the
          * writers write to recent data partitions usually.
          */
        ...
        /**
          * Stream all latest file slices in given partition with precondition that commitTime(file)
          * before maxCommitTime.
          *
          * @param partitionPath Partition path
          * @param maxCommitTime Max Instant Time
          * @param includeFileSlicesInPendingCompaction include file-slices that are in pending
          *     compaction
          */
        Stream<FileSlice> getLatestFileSlicesBeforeOrOn(
        ...
        /**
          * Stream all "merged" file-slices before on an instant time If a file-group has a pending
          * compaction request, the file-slice before and after compaction request instant is merged and
          * returned.
          *
          * @param partitionPath Partition Path
    ...
    /**
      ...
      *
      * <p>This is useful for some table services such as cleaning, the cleaning service may search
      * around the files to clean within some ancient data partitions, if there triggers a full table
      * cleaning for enormous number of partitions, the cache could cause a huge memory pressure to the
      * timeline server which induces an OOM exception.
      *
    ...
    Stream<HoodieFileGroup> getReplacedFileGroupsBeforeOrOn(
    ...
    Stream<HoodieFileGroup> getReplacedFileGroupsAfterOrOn(
