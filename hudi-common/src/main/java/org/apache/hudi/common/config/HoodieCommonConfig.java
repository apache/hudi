...
public class HoodieCommonConfig extends HoodieConfig {
  ...
  public static final ConfigProperty<String> TIMESTAMP_LOGICAL_TYPE_OVERRIDES = ConfigProperty
      ...
      .withDocumentation("Per-field authority for the timestamp logical type, taking precedence over the "
          ...
          + "An entry also attaches a timestamp logical type (UTC or local) to a column persisted as a bare "
          + "long, including one that 0.x stored without a logical type because its converter did not recognize "
          + "it. A UTC/local zone change is never authorized by this config, whatever the entry says, since no "
  ...
  public static final ConfigProperty<Boolean> HOODIE_FILE_INDEX_USE_SPILLABLE_MAP = ConfigProperty
      ...
      .sinceVersion("1.1.0")
      .markAdvanced()
      .withDocumentation("Property to enable spillable map for caching input file slices in org.apache.hudi.core.read.BaseHoodieTableFileIndex");
  ...
  public static final ConfigProperty<Long> HOODIE_FILE_INDEX_SPILLABLE_MEMORY = ConfigProperty
      ...
      .markAdvanced()
      .sinceVersion("1.1.0")
      .withDocumentation("Amount of memory to be used in bytes for holding cachedAllInputFileSlices in org.apache.hudi.core.read.BaseHoodieTableFileIndex.");
