...
/**
  * Delete record is a combination of HoodieKey and ordering value. The record is used for {@link
  * org.apache.hudi.common.table.log.block.HoodieDeleteBlock} to support per-record deletions. The
  * deletion block is always appended after the data block, we need to keep the ordering val to
  * combine with the data records when merging, or the data loss may occur if there are intermediate
  * deletions for the inputs (a new INSERT comes after a DELETE in one input batch).
  *
...
public class DeleteRecord implements Serializable {
    ...
    public static DeleteRecord create(
    ...
    public String toString() {
        return "DeleteRecord {" + " key=" + hoodieKey + " orderingVal=" + this.orderingVal + '}';
    }
}
