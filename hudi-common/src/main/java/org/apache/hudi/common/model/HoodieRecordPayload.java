...
/**
  * Every Hoodie table has an implementation of the <code>HoodieRecordPayload</code> This abstracts
...
public interface HoodieRecordPayload<T extends HoodieRecordPayload> extends Serializable {
    ...
    /**
      * This method is deprecated. Please use this {@link #preCombine(HoodieRecordPayload, Properties)}
      * method.
      */
    @Deprecated
    ...
    /**
      * When more than one HoodieRecord have the same HoodieKey in the incoming batch, this function
      * combines them before attempting to insert/upsert by taking in a property map. Implementation
      * can leverage the property to decide their business logic to do preCombine.
      *
      * @param oldValue instance of the old {@link HoodieRecordPayload} to be combined with.
      * @param properties Payload related properties. For example pass the ordering field(s) name to
      *     extract from value in storage.
    ...
    /**
      * When more than one HoodieRecord have the same HoodieKey in the incoming batch, this function
      * combines them before attempting to insert/upsert by taking in a schema. Implementation can
      * leverage the schema to decide their business logic to do preCombine.
      *
      * @param oldValue instance of the old {@link HoodieRecordPayload} to be combined with.
      * @param schema Payload related schema. For example use schema to overwrite old instance for
      *     specified fields that doesn't equal to default value.
      * @param properties Payload related properties. For example pass the ordering field(s) name to
      *     extract from value in storage.
      * @return the combined value
      */
    ...
    /**
      * This methods is deprecated. Please refer to {@link #combineAndGetUpdateValue(IndexedRecord,
      * Schema, Properties)} for java docs.
      */
    @Deprecated
    @PublicAPIMethod(maturity = ApiMaturityLevel.DEPRECATED)
    ...
    /**
      ...
      * of current value on storage and whats contained in this object. Implementations can leverage
      * properties if required.
      *
      * <p>eg: 1) You are updating counters, you may want to add counts to currentValue and write back
    ...
    /**
      ...
      */
    @PublicAPIMethod(maturity = ApiMaturityLevel.STABLE)
    default Option<IndexedRecord> getInsertValue(Schema schema, Properties properties)
            throws IOException {
    ...
    /**
      ...
      * compute some aggregate metrics using the metadata in the context of a write success or failure.
      *
    ...
    /**
      * This method can be used to extract the ordering value of the payload for combining/merging, or
      * 0 if no value is specified which means natural order(arrival time is used).
      *
