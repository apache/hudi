...
/**
  * Implements logic to scan log blocks and expose valid and deleted log records to subclass
  * implementation. Subclass is free to either apply merging or expose raw data back to the caller.
  *
  * <p>NOTE: If readBlockLazily is turned on, does not merge, instead keeps reading log blocks and
  * merges everything at once This is an optimization to avoid seek() back and forth to read new
  * block (forward seek()) and lazily read content of seen block (reverse and forward seek()) during
  * merge | | Read Block 1 Metadata | | Read Block 1 Data | | | Read Block 2 Metadata | | Read Block
  * 2 Data | | I/O Pass 1 | ..................... | I/O Pass 2 | ................. | | | Read Block N
  * Metadata | | Read Block N Data |
  *
  * <p>This results in two I/O passes over the log file.
  */
public abstract class AbstractHoodieLogRecordReader {
    ...
    protected AbstractHoodieLogRecordReader(
        ...
        this.hoodieTableMetaClient =
                hoodieTableMetaClientOption.orElseGet(
                        () ->
                                HoodieTableMetaClient.builder().setStorage(storage).setBasePath(basePath).build());
    ...
    /**
      * Iterate over the GenericRecord in the block, read the hoodie key and partition path and call
      * subclass processors to handle it.
      */
    private void processDataBlock(HoodieDataBlock dataBlock, Option<KeySpec> keySpecOpt)
            throws Exception {
        checkState(
                partitionNameOverrideOpt.isPresent() || partitionPathFieldOpt.isPresent(),
        ...
        while (!logBlocks.isEmpty()) {
            ...
            switch (lastBlock.getBlockType()) {
                ...
                case PARQUET_DATA_BLOCK:
                    ...
                    break;
                case DELETE_BLOCK:
                    Arrays.stream(((HoodieDeleteBlock) lastBlock).getRecordsToDelete())
                            .forEach(this::processNextDeletedRecord);
                    break;
                case CORRUPT_BLOCK:
    ...
    private Pair<ClosableIterator<HoodieRecord>, Schema> getRecordsIterator(
        ...
        if (keySpecOpt.isPresent()) {
            ...
            blockRecordsIterator =
                    (ClosableIterator)
                            dataBlock.getRecordIterator(keySpec.getKeys(), keySpec.isFullKey(), recordType);
        } else {
    ...
    /**
      ...
      */
    private Option<Pair<Function<HoodieRecord, HoodieRecord>, Schema>>
        ...
        InternalSchema mergedInternalSchema =
                new InternalSchemaMerger(fileSchema, internalSchema, true, false).mergeSchema();
        Schema mergedAvroSchema =
                AvroInternalSchemaConverter.convert(mergedInternalSchema, readerSchema.getFullName());
