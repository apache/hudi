...
/**
  * Used for enqueueing input records. Queue limit is controlled by {@link #memoryLimit}. Unlike
  * standard bounded queue implementations, this queue bounds the size by memory bytes occupied by
  * its tenants. The standard implementation bounds by the number of entries in the queue.
  *
  * <p>It internally samples every {@link #RECORD_SAMPLING_RATE}th record and adjusts number of
  * records in queue accordingly. This is done to ensure that we don't OOM.
  *
  * <p>This queue supports multiple producer single consumer pattern.
  *
...
public class BoundedInMemoryQueue<I, O> implements HoodieMessageQueue<I, O>, Iterable<O> {
    ...
    /**
      ...
      * determine how many records we should cache and will change (increase/decrease) permits
      * accordingly.
      */
    public final Semaphore rateLimiter = new Semaphore(1);

    /**
      * Sampling rate used to determine avg record size in bytes, Default is {@link
    ...
    /**
      * it holds the root cause of the Throwable in case either queueing records (consuming from
      * inputIterator) fails or thread reading records from queue fails.
      */
    ...
    /**
      * indicates rate limit (number of records to cache). it is updated whenever there is a change in
      * avg record size.
      */
    ...
    /**
      ...
      * @param transformFunction Transformer Function to convert input payload type to stored payload
      *     type
      */
    ...
    /**
      * Samples records with "RECORD_SAMPLING_RATE" frequency and computes average record size in
      * bytes. It is used for determining how many maximum records to queue. Based on change in avg
      * size it ma increase or decrease available permits.
      *
    ...
    /**
      * Reader interface but never exposed to outside world as this is a single consumer queue. Reading
      * is done through a singleton iterator for this queue.
      */
    @Override
