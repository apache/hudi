...
/**
  ...
  *
  * <p>{@link HoodieListData} can have either of the 2 execution semantics:
  *
  * <ol>
  *   <li>Eager: with every operation being executed right away
  *   <li>Lazy: with every operation being "stacked up", with it execution postponed until "terminal"
  *       operation is invoked
  * </ol>
  *
  * NOTE: This is an in-memory counterpart for {@code HoodieJavaRDD}, and it strives to provide
  * similar semantic as RDD container -- all intermediate (non-terminal, not de-referencing the
  * stream like "collect", "groupBy", etc) operations are executed *lazily*. This allows to make sure
  * that compute/memory churn is minimal since only necessary computations will ultimately be
  * performed.
  *
  * <p>Please note, however, that while RDD container allows the same collection to be de-referenced
  * more than once (ie terminal operation invoked more than once), {@link HoodieListData} allows that
  * only when instantiated w/ an eager execution semantic.
  *
...
public class HoodieListData<T> extends HoodieBaseListData<T> implements HoodieData<T> {
    ...
    @Override
    public void persist(
            String level, HoodieEngineContext engineContext, HoodieDataCacheKey cacheKey) {
        // No OP
    }
    ...
    public <O> HoodieData<O> flatMap(SerializableFunction<T, Iterator<O>> func) {
        ...
        Stream<O> mappedStream =
                asStream()
                        .flatMap(
                                e ->
                                        StreamSupport.stream(
                                                Spliterators.spliteratorUnknownSize(mapper.apply(e), Spliterator.ORDERED),
                                                true));
    ...
    public HoodieData<T> filter(SerializableFunction<T, Boolean> filterFunc) {
        return new HoodieListData<>(
                asStream().filter(r -> throwingMapWrapper(filterFunc).apply(r)), lazy);
    }
    ...
    public HoodieData<T> union(HoodieData<T> other) {
        ...
        return new HoodieListData<>(
                Stream.concat(asStream(), ((HoodieListData<T>) other).asStream()), lazy);
    }
