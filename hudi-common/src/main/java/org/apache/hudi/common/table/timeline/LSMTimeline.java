...
/**
  ...
  *
  * <p>After several instants are accumulated as a batch on the active timeline, they would be
  * flushed as a parquet file into the LSM timeline. In general the timeline is comprised with
  * parquet files with LSM style file layout. Each new operation to the timeline yields a new
  * snapshot version. Theoretically, there could be multiple snapshot versions on the timeline.
  *
  * <p>
  *
  * <h2>The LSM Timeline Layout</h2>
  *
  * <pre>
  *   t111, t112 ... t120 ... ->
  *     \              /
  ...
  * <p>
  *
  * <h2>The LSM Tree Compaction</h2>
  *
  * Use the universal compaction strategy, that is: when N(by default 10) number of parquet files
  * exist in the current layer, they are merged and flush as a compacted file in the next layer. We
  * have no limit for the layer number, assumes there are 10 instants for each file in L0, there
  * could be 100 instants per file in L1, so 3000 instants could be represented as 3 parquets in L2,
  ...
  * <p>
  *
  * <h2>The Archiver & Reader Snapshot Isolation</h2>
  *
  * <p>In order to make snapshot isolation of the LSM timeline write/read, we add two kinds of
  * metadata files for the LSM tree version management:
  *
  * <ol>
  *   <li>Manifest file: Each new file in layer 0 or each compaction would generate a new manifest
  *       file, the manifest file records the valid file handles of the latest snapshot;
  *   <li>Version file: A version file is generated right after a new manifest file is formed.
  * </ol>
  *
  * <p>
  *
  * <h2>The Reader Workflow</h2>
  *
  * <ul>
  *   <li>read the latest version;
  *   <li>read the manifest file for valid file handles;
  *   <li>read the data files, probably do a data skipping with the parquet file name max min
  *       timestamp.
  * </ul>
  *
  * <p>
  *
  * <h2>The Legacy Files Cleaning and Read Retention</h2>
  *
  * Only triggers file cleaning after a valid compaction.
  *
  * <p>
  *
  * <h3>Clean Strategy</h3>
  *
  * Keeps only 3 valid snapshot versions for the reader, that means, a file is kept for at lest 3
  * archival trigger interval, for default configuration, it is 30 instants time span, which is far
  * longer that the LSM timeline loading time.
  *
  * <p>
  *
  * <h3>Instants TTL</h3>
  *
  * The timeline reader only reads instants of last limited days. We will by default skip the
  * instants from LSM timeline that are generated long time ago.
  */
public class LSMTimeline {
    ...
    public static boolean isFileInRange(
            HoodieArchivedTimeline.TimeRangeFilter filter, String fileName) {
    ...
    public static List<Integer> allSnapshotVersions(HoodieTableMetaClient metaClient)
            throws IOException {
    ...
    public static HoodieLSMTimelineManifest latestSnapshotManifest(HoodieTableMetaClient metaClient)
            throws IOException {
    ...
    public static HoodieLSMTimelineManifest latestSnapshotManifest(
            HoodieTableMetaClient metaClient, int latestVersion) {
        if (latestVersion < 0) {
        ...
        byte[] content =
                FileIOUtils.readDataFromPath(
                                metaClient.getStorage(), getManifestFilePath(metaClient, latestVersion))
                        .get();
        try {
            return HoodieLSMTimelineManifest.fromJsonString(
                    fromUTF8Bytes(content), HoodieLSMTimelineManifest.class);
        } catch (Exception e) {
    ...
    public static StoragePath getManifestFilePath(
            HoodieTableMetaClient metaClient, int snapshotVersion) {
    ...
    public static List<StoragePathInfo> listAllMetaFiles(HoodieTableMetaClient metaClient)
            throws IOException {
        return metaClient
                .getStorage()
                .globEntries(new StoragePath(metaClient.getArchivePath(), "*.parquet"));
    }
    ...
    public static StoragePathFilter getManifestFilePathFilter() {
        return path ->
                path.getName().startsWith(MANIFEST_FILE_PREFIX)
