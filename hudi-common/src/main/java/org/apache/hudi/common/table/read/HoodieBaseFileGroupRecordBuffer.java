...
public abstract class HoodieBaseFileGroupRecordBuffer<T> implements HoodieFileGroupRecordBuffer<T> {
    ...
    public HoodieBaseFileGroupRecordBuffer(
        ...
        // Custom merge mode should produce the same results for any merger so we won't fail if
        // there is a mismatch
        ...
        long maxMemorySizeInBytes =
                props.getLong(MAX_MEMORY_FOR_MERGE.key(), MAX_MEMORY_FOR_MERGE.defaultValue());
        String spillableMapBasePath =
                props.getString(
                        SPILLABLE_MAP_BASE_PATH.key(), FileIOUtils.getDefaultSpillableMapBasePath());
        ExternalSpillableMap.DiskMapType diskMapType =
                ExternalSpillableMap.DiskMapType.valueOf(
                        props
                                .getString(
                                        SPILLABLE_DISK_MAP_TYPE.key(), SPILLABLE_DISK_MAP_TYPE.defaultValue().name())
        ...
        try {
            // Store merged records for all versions for this log file, set the in-memory footprint
            // to maxInMemoryMapSize
            this.records =
            ...
            throw new HoodieIOException(
                    "IOException when creating ExternalSpillableMap at " + spillableMapBasePath, e);
        }
    }
    ...
    /**
      * This allows hasNext() to be called multiple times without incrementing the iterator by more
      * than 1 record. It does come with the caveat that hasNext() must be called every time before
      * next(). But that is pretty expected behavior and every user should be doing that anyway.
      */
    ...
    /**
      * Compares two {@link Comparable}s. If both are numbers, converts them to {@link Long} for
      * comparison. If one of the {@link Comparable}s is a String, assumes that both are String values
      * for comparison.
      *
    ...
    /**
      ...
      * @throws IOException
      */
    protected Option<Pair<T, Map<String, Object>>> doProcessNextDataRecord(
            T record,
        ...
        if (existingRecordMetadataPair != null) {
            if (enablePartialMerging) {
                ...
                if (combinedRecord.getData() != existingRecordMetadataPair.getLeft().get()) {
                    return Option.of(
                            Pair.of(
                                    ...
                                    readerContext.updateSchemaAndResetOrderingValInMetadata(
                                            metadata, combinedRecordAndSchema.getRight())));
                }
                return Option.empty();
            } else {
                switch (recordMergeMode) {
                    ...
                    case EVENT_TIME_ORDERING:
                        Comparable existingOrderingValue =
                                readerContext.getOrderingValue(
                                        existingRecordMetadataPair.getLeft(),
                                        existingRecordMetadataPair.getRight(),
                                        readerSchema,
                                        props);
                        if (isDeleteRecordWithNaturalOrder(
                                existingRecordMetadataPair.getLeft(), existingOrderingValue)) {
                            return Option.empty();
                        }
                        Comparable incomingOrderingValue =
                                readerContext.getOrderingValue(Option.of(record), metadata, readerSchema, props);
                    ...
                    default:
                        // Merge and store the combined record
                        // Note that the incoming `record` is from an older commit, so it should be
                        // put as
                        // the `older` in the merge API
                        Option<Pair<HoodieRecord, Schema>> combinedRecordAndSchemaOpt =
                                recordMerger.merge(
            ...
            // engine-specific
            //       payload pointing into a shared, mutable (underlying) buffer we get a clean copy
            // of
    ...
    /**
      ...
      * @return
      */
    protected Option<DeleteRecord> doProcessNextDeletedRecord(
            DeleteRecord deleteRecord, Pair<Option<T>, Map<String, Object>> existingRecordMetadataPair) {
        if (existingRecordMetadataPair != null) {
            switch (recordMergeMode) {
                case OVERWRITE_WITH_LATEST:
                    return Option.empty();
                ...
                case CUSTOM:
                default:
                    Comparable existingOrderingVal =
                            readerContext.getOrderingValue(
                                    existingRecordMetadataPair.getLeft(),
                                    existingRecordMetadataPair.getRight(),
                                    readerSchema,
                                    props);
                    if (isDeleteRecordWithNaturalOrder(
                            existingRecordMetadataPair.getLeft(), existingOrderingVal)) {
                        return Option.empty();
                    }
    ...
    /**
      * Create a record iterator for a data block. The records are filtered by a key set specified by
      * {@code keySpecOpt}.
      *
      * @param dataBlock
      * @param keySpecOpt
    ...
    protected Pair<ClosableIterator<T>, Schema> getRecordsIterator(
            HoodieDataBlock dataBlock, Option<KeySpec> keySpecOpt) {
        ...
        return Pair.of(
                new CloseableMappingIterator<>(
                        blockRecordsIterator, schemaTransformerWithEvolvedSchema.getLeft()),
                schemaTransformerWithEvolvedSchema.getRight());
    }

    /**
      * Get final Read Schema for support evolution. step1: find the fileSchema for current dataBlock.
      * step2: determine whether fileSchema is compatible with the final read internalSchema. step3:
      * merge fileSchema and read internalSchema to produce final read schema.
      *
    ...
    protected Option<Pair<Function<T, T>, Schema>> composeEvolvedSchemaTransformer(
        ...
        InternalSchema fileSchema =
                InternalSchemaCache.searchSchemaAndCache(currentInstantTime, hoodieTableMetaClient);
        Pair<InternalSchema, Map<String, String>> mergedInternalSchema =
                new InternalSchemaMerger(fileSchema, internalSchema, true, false, false)
                        .mergeSchemaGetRenamed();
        Schema mergedAvroSchema =
                AvroInternalSchemaConverter.convert(
                        mergedInternalSchema.getLeft(), readerSchema.getFullName());
        assert mergedAvroSchema.equals(readerSchema);
        return Option.of(
                Pair.of(
                        readerContext.projectRecord(
                                dataBlock.getSchema(), mergedAvroSchema, mergedInternalSchema.getRight()),
                        mergedAvroSchema));
    }
        ...
        if (enablePartialMerging) {
            ...
            Option<Pair<HoodieRecord, Schema>> mergedRecord =
                    recordMerger.partialMerge(
                            ...
                            (Schema) newerInfoMap.get(INTERNAL_META_SCHEMA),
                            readerSchema,
                            props);
            ...
            switch (recordMergeMode) {
                case OVERWRITE_WITH_LATEST:
                    return newer;
                case EVENT_TIME_ORDERING:
                    Comparable oldOrderingValue =
                            readerContext.getOrderingValue(older, olderInfoMap, readerSchema, props);
                    if (isDeleteRecordWithNaturalOrder(older, oldOrderingValue)) {
                        return newer;
                    }
                    Comparable newOrderingValue =
                            readerContext.getOrderingValue(newer, newerInfoMap, readerSchema, props);
                                    ...
                                    (T)
                                            mergedRecord
                                                    ...
                                                    .rewriteRecordWithNewSchema(
                                                            mergedRecord.get().getRight(), null, readerSchema)
                                                    .getData());
                        }
                        return Option.ofNullable((T) mergedRecord.get().getLeft().getData());
                    }
    ...
    protected boolean hasNextBaseRecord(
            T baseRecord, Pair<Option<T>, Map<String, Object>> logRecordInfo) throws IOException {
        Map<String, Object> metadata =
        ...
        while (logRecordIterator.hasNext()) {
            ...
            resultRecord =
                    merge(
                            ...
                            nextRecordInfo.getLeft(),
                            nextRecordInfo.getRight());
            if (resultRecord.isPresent()) {
                nextRecord = readerContext.seal(resultRecord.get());
                return true;
            }
    ...
    protected Pair<Function<T, T>, Schema> getSchemaTransformerWithEvolvedSchema(
            HoodieDataBlock dataBlock) {
        ...
        Function<T, T> transformer =
                schemaEvolutionTransformerOpt.map(Pair::getLeft).orElse(Function.identity());

        Schema evolvedSchema =
                schemaEvolutionTransformerOpt.map(Pair::getRight).orElseGet(dataBlock::getSchema);
