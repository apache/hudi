...
/**
  ...
  *
  * <p>This is used when write handles report back write operation's info and stats, instead of
  * passing back the full {@link HoodieRecord}, this lean delegate of it will be passed instead.
  */
public class HoodieRecordDelegate implements Serializable, KryoSerializable {
    ...
    public static HoodieRecordDelegate create(
            String recordKey, String partitionPath, HoodieRecordLocation currentLocation) {
        return new HoodieRecordDelegate(
                new HoodieKey(recordKey, partitionPath), currentLocation, null, false);
    }
    ...
    public static HoodieRecordDelegate create(
        ...
        return new HoodieRecordDelegate(
                new HoodieKey(recordKey, partitionPath), currentLocation, newLocation, false);
    }
    ...
    public static HoodieRecordDelegate create(
            HoodieKey key, HoodieRecordLocation currentLocation, HoodieRecordLocation newLocation) {
    ...
    public static HoodieRecordDelegate fromHoodieRecord(HoodieRecord record) {
        return new HoodieRecordDelegate(
                record.getKey(),
                record.getCurrentLocation(),
                record.getNewLocation(),
                record.getIgnoreIndexUpdate());
    }

    public static HoodieRecordDelegate fromHoodieRecord(
            HoodieRecord record, @Nullable HoodieRecordLocation newLocationOverride) {
        return new HoodieRecordDelegate(
                record.getKey(),
                record.getCurrentLocation(),
                newLocationOverride,
