...
public class BinaryUtil {
    ...
    /**
      * Lexicographically compare two arrays. copy from hbase
      *
    ...
    /**
      * Interleaving array bytes. Interleaving means take one bit from the first matrix element, one
      * bit from the next, etc, then take the second bit from the first matrix element, second bit from
      * the second, all the way to the last bit of the last element. Combine those bits in that order
      * into a single BigInteger,
      *
        ...
        for (int bitStep = 0; bitStep < totalBits; bitStep++) {
            ...
            for (int i = 0; i < candidateSize; i++) {
                ...
                result[tempResBytePos] =
                        updatePos(
                                result[tempResBytePos], tempResBitPos, buffer[i][currentBytePos], currentBitPos);
                resBitPos++;
            }
        }
        return result;
