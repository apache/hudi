...
/**
  * This interface provides the map interface for storing records in disk after they spill over from
  * memory. Used by {@link ExternalSpillableMap}.
  *
...
public abstract class DiskMap<T extends Serializable, R extends Serializable>
    ...
    public DiskMap(String basePath, String prefix) throws IOException {
        this.diskMapPath =
                String.format(
                        "%s/%s-%s-%s", basePath, SUBFOLDER_PREFIX, prefix, UUID.randomUUID().toString());
    ...
    /** Cleanup all resources, files and folders triggered by shutdownhook. */
