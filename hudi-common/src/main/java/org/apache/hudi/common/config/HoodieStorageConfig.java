...
public class HoodieStorageConfig extends HoodieConfig {
  ...
  public static final ConfigProperty<Boolean> PARQUET_VARIANT_SHREDDING_SCHEMA_INFERENCE_ENABLED = ConfigProperty
      ...
      .withDocumentation("Infers the shredding schema of variant columns that have no explicit "
          ...
          + "back by Spark 4.1+: Spark 4.0, Hive and Flink readers fail fast on them, so disable this "
          + "option (or hoodie.parquet.variant.write.shredding.enabled) on tables those engines read, "
          + "and rewrite already shredded files through clustering with it disabled to return to the "
