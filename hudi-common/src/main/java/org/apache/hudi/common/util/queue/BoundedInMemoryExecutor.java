...
/**
  * Executor which orchestrates concurrent producers and consumers communicating through
  * 'BoundedInMemoryQueue'. This class takes as input the size limit, queue producer(s), consumer and
  * transformer and exposes API to orchestrate concurrent execution of these actors communicating
  * through a central bounded queue
  */
public class BoundedInMemoryExecutor<I, O, E> extends BaseHoodieQueueBasedExecutor<I, O, E> {
    ...
    public BoundedInMemoryExecutor(
            final long bufferLimitInBytes,
            final Iterator<I> inputItr,
            HoodieConsumer<O, E> consumer,
            Function<I, O> transformFunction,
            Runnable preExecuteRunnable) {
        this(
                bufferLimitInBytes,
                Collections.singletonList(new IteratorBasedQueueProducer<>(inputItr)),
                Option.of(consumer),
                transformFunction,
    ...
    public BoundedInMemoryExecutor(
        ...
        super(
                producers,
                consumer,
                new BoundedInMemoryQueue<>(bufferLimitInBytes, transformFunction, sizeEstimator),
                preExecuteRunnable);
    }
