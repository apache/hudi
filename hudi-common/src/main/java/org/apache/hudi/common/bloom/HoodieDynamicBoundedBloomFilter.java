...
/**
  * Hoodie's dynamic bloom bounded bloom filter. This is based largely on Hadoop's
  * DynamicBloomFilter, but with a bound on amount of entries to dynamically expand to. Once the
  * entries added reach the bound, false positive ratio may not be guaranteed.
  */
public class HoodieDynamicBoundedBloomFilter implements BloomFilter {
    ...
    HoodieDynamicBoundedBloomFilter(
            int numEntries, double errorRate, int hashType, int maxNoOfEntries) {
        // Bit size
    ...
    /**
      * Generate {@link HoodieDynamicBoundedBloomFilter} from the given {@code serString} serialized
      * string.
      *
      * @param serString the serialized string which represents the {@link
      *     HoodieDynamicBoundedBloomFilter}
      */
