...
public class HoodiePartitionMetadata {
   ...
   *
   * @param fileLocation location of the file to write.
   * @throws IOException
   */
  private void writeMetafile(StoragePath fileLocation) throws IOException {
    if (format.isPresent()) {
      BaseFileUtils.getInstance(format.get()).writeMetaFile(storage, fileLocation, props);
    } else {
      // Backwards compatible properties file format
      try (OutputStream os = storage.create(fileLocation, true)) {
  ...
  private boolean readTextFormatMetaFile() {
    ...
    StoragePath metafilePath = textFormatMetaFileLocation(partitionPath);
  ...
  private boolean readBaseFormatMetaFile() {
    for (StoragePath metafilePath : baseFormatMetaFileLocations(partitionPath)) {
      try {
  ...
  public static Option<StoragePath> getPartitionMetafilePath(HoodieStorage storage, StoragePath partitionPath) {
    ...
    try {
      Option<StoragePath> textFormatPath = textFormatMetaLocationIfExists(storage, partitionPath);
      if (textFormatPath.isPresent()) {
        return textFormatPath;
      } else {
        return baseFormatMetaLocationIfExists(storage, partitionPath);
      }
    } catch (IOException ioe) {
  ...
  public static Option<StoragePath> baseFormatMetaLocationIfExists(HoodieStorage storage, StoragePath partitionPath) throws IOException {
  ...
  public static Option<StoragePath> textFormatMetaLocationIfExists(HoodieStorage storage, StoragePath partitionPath) throws IOException {
    StoragePath path = textFormatMetaFileLocation(partitionPath);
    return Option.ofNullable(storage.exists(path) ? path : null);
  }

  static StoragePath textFormatMetaFileLocation(StoragePath partitionPath) {
    return new StoragePath(partitionPath, HoodiePartitionMetadata.HOODIE_PARTITION_METAFILE_PREFIX);
  }

  static List<StoragePath> baseFormatMetaFileLocations(StoragePath partitionPath) {
