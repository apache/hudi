...
public class HoodiePartitionMetadata {
   ...
   * @throws IOException
   */
  private void writeMetafile(HoodieLocation fileLocation) throws IOException {
    if (format.isPresent()) {
   ...
   */
  public static Option<HoodieLocation> getPartitionMetafilePath(HoodieStorage storage, HoodieLocation partitionPath) {
    ...
    try {
      Option<HoodieLocation> textFormatPath = textFormatMetaLocationIfExists(storage, partitionPath);
