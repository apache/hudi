...
public class HoodiePartitionMetadata {
    ...
    public HoodiePartitionMetadata(
        ...
        props.setProperty(
                PARTITION_DEPTH_KEY, String.valueOf(partitionPath.depth() - basePath.depth()));
    }
    ...
    public void trySave() throws HoodieIOException {
        StoragePath metaPath =
                new StoragePath(partitionPath, HOODIE_PARTITION_METAFILE_PREFIX + getMetafileExtension());
        ...
        RetryHelper<Void, HoodieIOException> retryHelper =
                new RetryHelper(1000, 3, 1000, HoodieIOException.class.getName())
                        .tryWith(
                                () -> {
    ...
    private String getMetafileExtension() {
        // To be backwards compatible, there is no extension to the properties file base partition
        // metafile
    ...
    /**
      ...
      * @throws IOException
      */
    private void writeMetafileInFormat(StoragePath filePath, HoodieFileFormat format)
            throws IOException {
        StoragePath tmpPath =
                new StoragePath(
                        partitionPath,
    ...
    public void readFromFS() throws IOException {
        ...
        if (!readFile) {
            throw new HoodieException(
                    "Unable to read any partition meta file to locate the table timeline.");
        }
    }
    ...
    private boolean readBaseFormatMetaFile() {
        for (StoragePath metafilePath : baseFormatMetaFilePaths(partitionPath)) {
            try {
                FileFormatUtils reader =
                        HoodieIOFactory.getIOFactory(storage).getFileFormatUtils(metafilePath);
                // Data file format
                Map<String, String> metadata =
                        reader.readFooter(storage, true, metafilePath, PARTITION_DEPTH_KEY, COMMIT_TIME_KEY);
                props.clear();
                ...
                LOG.debug(
                        "Unable to read partition metadata "
                                + metafilePath.getName()
                                + " for partition "
                                + partitionPath);
            }
        }
    ...
    public static boolean hasPartitionMetadata(HoodieStorage storage, StoragePath partitionPath) {
        try {
            ...
            throw new HoodieIOException(
                    "Error checking presence of partition meta file for " + partitionPath, ioe);
        }
    }
    ...
    /**
      ...
      */
    public static Option<StoragePath> getPartitionMetafilePath(
            HoodieStorage storage, StoragePath partitionPath) {
        // The partition listing is a costly operation so instead we are searching for existence of
        // the files instead.
        // This is in expected order as properties file based partition metafiles should be the most
        // common.
        try {
            ...
            throw new HoodieException(
                    "Error checking Hoodie partition metadata for " + partitionPath, ioe);
        }
    }

    public static Option<StoragePath> baseFormatMetaPathIfExists(
    ...
    public static Option<StoragePath> textFormatMetaPathIfExists(
            HoodieStorage storage, StoragePath partitionPath) throws IOException {
    ...
    static List<StoragePath> baseFormatMetaFilePaths(StoragePath partitionPath) {
        return Stream.of(
                        HoodieFileFormat.PARQUET.getFileExtension(), HoodieFileFormat.ORC.getFileExtension())
                .map(
                        ext ->
                                new StoragePath(
                                        partitionPath, HoodiePartitionMetadata.HOODIE_PARTITION_METAFILE_PREFIX + ext))
