...
/**
  ...
  *
  * <p>For RocksDB, 3 colFamilies are used for storing file-system view for each table. (a) View (b)
  * Partitions Cached (c) Pending Compactions
  *
  * <p>View : Key : Store both slice and Data file stored. Slice : Key =
  * "type=slice,part=<PartitionPath>,id=<FileId>,instant=<Timestamp>" Value = Serialized FileSlice
  * Data File : Key = "type=df,part=<PartitionPath>,id=<FileId>,instant=<Timestamp>" Value =
  * Serialized DataFile
  *
  * <p>Partitions : Key = "part=<PartitionPath>" Value = Boolean
  *
  * <p>Pending Compactions Key = "part=<PartitionPath>,id=<FileId>" Value = Pair<CompactionTime,
  * CompactionOperation>
  */
public class RocksDBSchemaHelper {
    ...
    public RocksDBSchemaHelper(HoodieTableMetaClient metaClient) {
        ...
        this.colFamilyForPendingClusteringFileGroups =
                "hudi_pending_clustering_fg" + fixedBasePathString;
    }

    public List<String> getAllColumnFamilies() {
        return Arrays.asList(
                getColFamilyForView(),
                getColFamilyForPendingCompaction(),
                getColFamilyForPendingLogCompaction(),
                getColFamilyForBootstrapBaseFile(),
                getColFamilyForStoredPartitions(),
                getColFamilyForReplacedFileGroups(),
    ...
    public String getKeyForSliceView(HoodieFileGroup fileGroup, FileSlice slice) {
        return getKeyForSliceView(
                fileGroup.getPartitionPath(),
