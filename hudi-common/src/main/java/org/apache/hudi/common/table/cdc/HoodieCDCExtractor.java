...
/**
  ...
  *
  * <p>There are some steps: 1. filter out the completed commit instants, and get the related {@link
  * HoodieCommitMetadata} objects. 2. initialize the {@link HoodieTableFileSystemView} by the touched
  * data files. 3. extract the cdc information: generate a {@link HoodieCDCFileSplit} object for each
  * of the instant in (startInstant, endInstant) and each of the file group which is touched in the
  * range of instants.
  */
public class HoodieCDCExtractor {
    ...
    public HoodieCDCExtractor(
            HoodieTableMetaClient metaClient, InstantRange range, boolean consumeChangesFromCompaction) {
    ...
    /**
      * At the granularity of a file group, trace the mapping between each commit/instant and changes
      * to this file group.
      */
    public Map<HoodieFileGroupId, List<HoodieCDCFileSplit>> extractCDCFileSplits() {
        ...
        for (HoodieInstant instant : commits.keySet()) {
            ...
            for (String partition : ptToWriteStats.keySet()) {
                ...
                hoodieWriteStats.forEach(
                        writeStat -> {
                            HoodieFileGroupId fileGroupId =
                                    new HoodieFileGroupId(partition, writeStat.getFileId());
                            ...
                            HoodieCDCFileSplit changeFile =
                                    parseWriteStat(
                                            fileGroupId, instant, writeStat, commitMetadata.getOperationType());
            ...
            if (commitMetadata instanceof HoodieReplaceCommitMetadata) {
                ...
                Map<String, List<String>> ptToReplacedFileId =
    ...
    /**
      ...
      *
      * <p>There is no need to initialize the fs view when supplemental logging mode is:
      * WITH_BEFORE_AFTER.
      */
    ...
    /**
      * Parse the commit metadata between (startInstant, endInstant], and extract the touched
      * partitions and files to build the filesystem view.
      */
    private HoodieTableFileSystemView initFSView() {
        ...
        try {
            ...
            return new HoodieTableFileSystemView(
                    metaClient, metaClient.getCommitsTimeline().filterCompletedInstants(), touchedFiles);
        } catch (Exception e) {
    ...
    /**
      ...
      *
      * <p>There are some conditions: 1) the instant should be completed; 2) the instant should be in
      * (startInstant, endInstant]; 3) the action of the instant is one of 'commit', 'deltacommit',
      * 'replacecommit'; 4) the write type of the commit should have the ability to change the data.
      *
      * <p>And, we need to recognize which is a 'replacecommit', that help to find the list of file
      * group replaced.
      */
    private void initInstantAndCommitMetadata() {
        try {
            Set<String> requiredActions =
                    new HashSet<>(Arrays.asList(COMMIT_ACTION, DELTA_COMMIT_ACTION, REPLACE_COMMIT_ACTION));
    ...
    /**
      * Parse HoodieWriteStat, judge which type the file is, and what strategy should be used to parse
      * CDC data. Then build a [[HoodieCDCFileSplit]] object.
      */
    private HoodieCDCFileSplit parseWriteStat(
        ...
        if (CollectionUtils.isNullOrEmpty(writeStat.getCdcStats())) {
            // no cdc log files can be used directly. we reuse the existing data file to retrieve
            // the change data.
            String path = writeStat.getPath();
            if (FSUtils.isBaseFile(new StoragePath(path))) {
                // this is a base file
                if (WriteOperationType.isDelete(operation)
                        && writeStat.getNumWrites() == 0L
                        && writeStat.getNumDeletes() != 0) {
                    // This is a delete operation wherein all the records in this file group are
                    // deleted
                    // and no records have been written out a new file.
                    // So, we find the previous file that this operation delete from, and treat each
                    // of
                    // records as a deleted one.
                    HoodieBaseFile beforeBaseFile =
                            getOrCreateFsView()
                                    .getBaseFileOn(
                                            fileGroupId.getPartitionPath(),
                                            writeStat.getPrevCommit(),
                                            fileGroupId.getFileId())
                                    .orElseThrow(
                                            () ->
                                                    new HoodieIOException(
                                                            "Can not get the previous version of the base file"));
                    FileSlice beforeFileSlice =
                            new FileSlice(
                                    fileGroupId, writeStat.getPrevCommit(), beforeBaseFile, Collections.emptyList());
                    cdcFileSplit =
                            new HoodieCDCFileSplit(
                                    instantTs,
                                    BASE_FILE_DELETE,
                                    new ArrayList<>(),
                                    Option.of(beforeFileSlice),
                                    Option.empty());
                ...
                cdcFileSplit =
                        new HoodieCDCFileSplit(instantTs, LOG_FILE, path, beforeFileSliceOpt, Option.empty());
            }
        } else {
            // this is a cdc log
            if (supplementalLoggingMode == HoodieCDCSupplementalLoggingMode.DATA_BEFORE_AFTER) {
                ...
                try {
                    ...
                    if (supplementalLoggingMode == HoodieCDCSupplementalLoggingMode.OP_KEY_ONLY) {
                        beforeFileSlice =
                                new FileSlice(
                                        fileGroupId, writeStat.getPrevCommit(), beforeBaseFile, new ArrayList<>());
                    }
                    cdcFileSplit =
                            new HoodieCDCFileSplit(
                                    instantTs,
                                    AS_IS,
                                    writeStat.getCdcStats().keySet(),
    ...
    private Option<FileSlice> getDependentFileSliceForLogFile(
            HoodieFileGroupId fgId, HoodieInstant instant, String currentLogFile) {
            ...
            if (fileSliceOpt.isPresent()) {
                ...
                try {
                    HoodieBaseFile baseFile =
                            fileSlice.getLeft().isEmpty()
                                    ? null
                                    : new HoodieBaseFile(
                                            storage.getPathInfo(new StoragePath(partitionPath, fileSlice.getLeft())));
                    List<StoragePath> logFilePaths =
                            fileSlice.getRight().stream()
