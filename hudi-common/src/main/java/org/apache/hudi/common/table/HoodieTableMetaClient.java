...
/**
  ...
  * start getting the meta-data.
  *
  * <p>All the timelines are computed lazily, once computed the timeline is cached and never
  * refreshed. Use the <code>HoodieTimeline.reload()</code> to refresh timelines.
  *
  * @see HoodieTimeline
  * @since 0.3.0
  */
public class HoodieTableMetaClient implements Serializable {
    ...
    public static final String AUXILIARYFOLDER_NAME =
            METAFOLDER_NAME + StoragePath.SEPARATOR + ".aux";
    ...
    public static final String SAMPLE_WRITES_FOLDER_PATH =
            AUXILIARYFOLDER_NAME + StoragePath.SEPARATOR + ".sample_writes";
    public static final String HEARTBEAT_FOLDER_NAME =
            METAFOLDER_NAME + StoragePath.SEPARATOR + ".heartbeat";
    public static final String METADATA_TABLE_FOLDER_PATH =
            METAFOLDER_NAME + StoragePath.SEPARATOR + "metadata";
    ...
    public static final String BOOTSTRAP_INDEX_BY_PARTITION_FOLDER_PATH =
            BOOTSTRAP_INDEX_ROOT_FOLDER_PATH + StoragePath.SEPARATOR + ".partitions";
    ...
    /** Instantiate HoodieTableMetaClient. Can only be called if table already exists */
    protected HoodieTableMetaClient(
        ...
        this.tableConfig =
                new HoodieTableConfig(this.storage, metaPath, payloadClassName, recordMergerStrategy);
        ...
        if (layoutVersion.isPresent() && tableConfigVersion.isPresent()) {
            // Ensure layout version passed in config is not lower than the one seen in
            // hoodie.properties
            checkArgument(
                    layoutVersion.get().compareTo(tableConfigVersion.get()) >= 0,
                    "Layout Version defined in hoodie properties has higher version ("
                            + tableConfigVersion.get()
                            + ") than the one passed in config ("
                            + layoutVersion.get()
                            + ")");
        }
        this.timelineLayoutVersion =
                layoutVersion.orElseGet(() -> tableConfig.getTimelineLayoutVersion().get());
        this.loadActiveTimelineOnLoad = loadActiveTimelineOnLoad;
        LOG.info(
                "Finished Loading Table of type "
                        + tableType
                        + "(version="
    ...
    /**
      ...
      */
    public void buildIndexDefinition(
        ...
        HoodieIndexDefinition indexDefinition =
                new HoodieIndexDefinition(indexName, indexType, options.get("func"), columnNames, options);
        if (indexMetadataOpt.isPresent()) {
            indexMetadataOpt.get().getIndexDefinitions().put(indexName, indexDefinition);
        } else {
            indexMetadataOpt =
                    Option.of(new HoodieIndexMetadata(Collections.singletonMap(indexName, indexDefinition)));
        }
        try {
            FileIOUtils.createFileInPath(
                    storage,
                    new StoragePath(indexMetaPath),
                    Option.of(getUTF8Bytes(indexMetadataOpt.get().toJson())));
        } catch (IOException e) {
            throw new HoodieIOException(
                    "Could not write functional index metadata at path: " + indexMetaPath, e);
        }
    }

    /**
      * Returns Option of {@link HoodieIndexMetadata} from index definition file if present, else
    ...
    public Option<HoodieIndexMetadata> getIndexMetadata() {
        ...
        if (tableConfig.getIndexDefinitionPath().isPresent()
            ...
            StoragePath indexDefinitionPath = new StoragePath(tableConfig.getIndexDefinitionPath().get());
            try {
                return Option.of(
                        HoodieIndexMetadata.fromJson(
                                new String(FileIOUtils.readDataFromPath(storage, indexDefinitionPath).get())));
            } catch (IOException e) {
                throw new HoodieIOException(
                        "Could not load functional index metadata at path: "
                                + tableConfig.getIndexDefinitionPath().get(),
                        e);
            }
        }
        return Option.empty();
    ...
    public void updateIndexMetadata(
            HoodieIndexMetadata newFunctionalIndexMetadata, String indexMetaPath) {
    ...
    public HoodieStorage getStorage() {
        if (storage == null) {
            storage =
                    getStorage(metaPath, getStorageConf(), consistencyGuardConfig, fileSystemRetryConfig);
        }
        return storage;
    }

    private static HoodieStorage getStorage(
            StoragePath path,
        ...
        return getIOFactory(newStorage)
                .getStorage(
                        path,
    ...
    /**
      * Returns the cached archived timeline if using in-memory cache or a fresh new archived timeline
      * if not using cache, from startTs (inclusive).
      *
      * <p>Instantiating an archived timeline is costly operation if really early startTs is specified.
      *
    ...
    public void validateTableProperties(Properties properties) {
        ...
        // Meta fields can be disabled only when either {@code SimpleKeyGenerator}, {@code
        // ComplexKeyGenerator}, {@code NonpartitionedKeyGenerator} is used
        if (!getTableConfig().populateMetaFields()) {
            ...
            if (!keyGenClass.equals("org.apache.hudi.keygen.SimpleKeyGenerator")
                ...
                throw new HoodieException(
                        "Only simple, non-partitioned or complex key generator are supported when meta-fields are disabled. Used: "
                                + keyGenClass);
            }
        }
    ...
    /**
      * Helper method to initialize a given path as a hoodie table with configs passed in as
      * Properties.
      *
    ...
    public static HoodieTableMetaClient initTableAndGetMetaClient(
        ...
        HoodieTableMetaClient metaClient =
                HoodieTableMetaClient.builder()
                        ...
                        .build();
        LOG.info(
                "Finished initializing Table of type "
    ...
    private static void initTableMetaClient(
            StorageConfiguration<?> storageConf, StoragePath basePath, Properties props)
            throws IOException {
        ...
        String archiveLogPropVal =
                new HoodieConfig(props).getStringOrDefault(HoodieTableConfig.ARCHIVELOG_FOLDER);
        ...
        final StoragePath temporaryFolder =
                new StoragePath(basePath, HoodieTableMetaClient.TEMPFOLDER_NAME);
        if (!storage.exists(temporaryFolder)) {
            storage.createDirectory(temporaryFolder);
        }

        // Always create auxiliary folder which is needed to track compaction workloads (stats and
        // any metadata in future)
        final StoragePath auxiliaryFolder =
                new StoragePath(basePath, HoodieTableMetaClient.AUXILIARYFOLDER_NAME);
    ...
    public static void initializeBootstrapDirsIfNotExists(StoragePath basePath, HoodieStorage storage)
            throws IOException {
    ...
    /**
      ...
      */
    public static List<StoragePathInfo> scanFiles(
            HoodieStorage storage, StoragePath metaPath, StoragePathFilter nameFilter)
            throws IOException {
    ...
    /**
      * Get the commit + pending-compaction timeline visible for this table. A RT filesystem view is
      * constructed with this timeline so that file-slice after pending compaction-requested
      * instant-time is also considered valid. A RT file-system view for reading must then merge the
      * file-slices before and after pending compaction instant so that all delta-commits are read.
      */
    ...
    /**
      ...
      * @param applyLayoutVersionFilters Depending on Timeline layout version, if there are multiple
      *     states for the same action instant, only include the highest state
      * @return List of Hoodie Instants generated
      * @throws IOException in case of failure
      */
    public List<HoodieInstant> scanHoodieInstantsFromFileSystem(
            Set<String> includedExtensions, boolean applyLayoutVersionFilters) throws IOException {
    ...
    /**
      ...
      */
    public List<HoodieInstant> scanHoodieInstantsFromFileSystem(
            StoragePath timelinePath, Set<String> includedExtensions, boolean applyLayoutVersionFilters)
            throws IOException {
    ...
    private static HoodieTableMetaClient newMetaClient(
            HoodieStorage storage,
            String basePath,
            boolean loadActiveTimelineOnLoad,
            ConsistencyGuardConfig consistencyGuardConfig,
        ...
        return metaserverConfig.isMetaserverEnabled()
                ? (HoodieTableMetaClient)
                        ReflectionUtils.loadClass(
                                ...
                                Option.ofNullable(metaserverConfig.getTableName()),
                                metaserverConfig)
    ...
    public static class Builder {
        ...
        public HoodieTableMetaClient build() {
            ...
            if (storage == null) {
                storage =
                        getStorage(
                                new StoragePath(basePath), conf, consistencyGuardConfig, fileSystemRetryConfig);
            }
            return newMetaClient(
                    storage,
                    basePath,
                    loadActiveTimelineOnLoad,
                    consistencyGuardConfig,
    ...
    public static class PropertyBuilder {
        ...
        /**
          * Persist the configs that is written at the first time, and should not be changed. Like
          * KeyGenerator's configs.
          */
        ...
        public PropertyBuilder setMultipleBaseFileFormatsEnabled(
                Boolean multipleBaseFileFormatsEnabled) {
        ...
        public PropertyBuilder fromProperties(Properties properties) {
            ...
            if (hoodieConfig.contains(HoodieTableConfig.RECORD_MERGE_MODE)) {
                setRecordMergeMode(
                        RecordMergeMode.valueOf(
                                hoodieConfig.getString(HoodieTableConfig.RECORD_MERGE_MODE).toUpperCase()));
            }
            if (hoodieConfig.contains(HoodieTableConfig.PAYLOAD_CLASS_NAME)) {
                ...
                setPayloadClassName(
                        RecordPayloadType.valueOf(hoodieConfig.getString(HoodieTableConfig.PAYLOAD_TYPE))
                                .getClassName());
            }
            if (hoodieConfig.contains(HoodieTableConfig.RECORD_MERGER_STRATEGY)) {
                setRecordMergerStrategy(hoodieConfig.getString(HoodieTableConfig.RECORD_MERGER_STRATEGY));
            }
            ...
            if (hoodieConfig.contains(HoodieTableConfig.BOOTSTRAP_INDEX_TYPE)) {
                setPayloadClassName(
                        BootstrapIndexType.valueOf(
                                        hoodieConfig.getString(HoodieTableConfig.BOOTSTRAP_INDEX_TYPE))
                                .getClassName());
            }
            ...
            if (hoodieConfig.contains(HoodieTableConfig.CDC_SUPPLEMENTAL_LOGGING_MODE)) {
                setCDCSupplementalLoggingMode(
                        hoodieConfig.getString(HoodieTableConfig.CDC_SUPPLEMENTAL_LOGGING_MODE));
            }
            ...
            if (hoodieConfig.contains(HoodieTableConfig.KEY_GENERATOR_CLASS_NAME)) {
                ...
                setKeyGeneratorClassProp(
                        KeyGeneratorType.valueOf(hoodieConfig.getString(HoodieTableConfig.KEY_GENERATOR_TYPE))
                                .getClassName());
            }
            ...
            if (hoodieConfig.contains(HoodieTableConfig.URL_ENCODE_PARTITIONING)) {
                setUrlEncodePartitioning(
                        hoodieConfig.getBoolean(HoodieTableConfig.URL_ENCODE_PARTITIONING));
            }
            if (hoodieConfig.contains(HoodieTableConfig.PARTITION_METAFILE_USE_BASE_FORMAT)) {
                setPartitionMetafileUseBaseFormat(
                        hoodieConfig.getBoolean(HoodieTableConfig.PARTITION_METAFILE_USE_BASE_FORMAT));
            }
            if (hoodieConfig.contains(HoodieTableConfig.DROP_PARTITION_COLUMNS)) {
                setShouldDropPartitionColumns(
                        hoodieConfig.getBoolean(HoodieTableConfig.DROP_PARTITION_COLUMNS));
            }
            ...
            if (hoodieConfig.contains(HoodieTableConfig.TABLE_METADATA_PARTITIONS_INFLIGHT)) {
                setInflightMetadataPartitions(
                        hoodieConfig.getString(HoodieTableConfig.TABLE_METADATA_PARTITIONS_INFLIGHT));
            }
            if (hoodieConfig.contains(HoodieTableConfig.SECONDARY_INDEXES_METADATA)) {
                setSecondaryIndexesMetadata(
                        hoodieConfig.getString(HoodieTableConfig.SECONDARY_INDEXES_METADATA));
            }
            if (hoodieConfig.contains(HoodieTableConfig.MULTIPLE_BASE_FILE_FORMATS_ENABLE)) {
                setMultipleBaseFileFormatsEnabled(
                        hoodieConfig.getBoolean(HoodieTableConfig.MULTIPLE_BASE_FILE_FORMATS_ENABLE));
            }
        ...
        public Properties build() {
            ...
            tableConfig.setValue(
                    HoodieTableConfig.VERSION, String.valueOf(HoodieTableVersion.current().versionCode()));

            if (tableType == HoodieTableType.MERGE_ON_READ) {
                if (null != payloadClassName) {
                    tableConfig.setValue(
                            HoodieTableConfig.PAYLOAD_TYPE,
            ...
            if (null != timelineLayoutVersion) {
                tableConfig.setValue(
                        HoodieTableConfig.TIMELINE_LAYOUT_VERSION, String.valueOf(timelineLayoutVersion));
            }
            ...
            if (null != bootstrapIndexClass) {
                tableConfig.setValue(
                        HoodieTableConfig.BOOTSTRAP_INDEX_TYPE,
                        BootstrapIndexType.fromClassName(bootstrapIndexClass).name());
            }

            if (null != bootstrapIndexEnable) {
                tableConfig.setValue(
                        HoodieTableConfig.BOOTSTRAP_INDEX_ENABLE, Boolean.toString(bootstrapIndexEnable));
            }
            ...
            if (null != cdcEnabled) {
                ...
                if (cdcEnabled && null != cdcSupplementalLoggingMode) {
                    tableConfig.setValue(
                            HoodieTableConfig.CDC_SUPPLEMENTAL_LOGGING_MODE, cdcSupplementalLoggingMode);
                }
            }
            ...
            if (null != keyGeneratorClassProp) {
                tableConfig.setValue(
                        HoodieTableConfig.KEY_GENERATOR_TYPE,
            ...
            if (null != urlEncodePartitioning) {
                tableConfig.setValue(
                        HoodieTableConfig.URL_ENCODE_PARTITIONING, Boolean.toString(urlEncodePartitioning));
            }
            ...
            if (null != partitionMetafileUseBaseFormat) {
                tableConfig.setValue(
                        HoodieTableConfig.PARTITION_METAFILE_USE_BASE_FORMAT,
                        partitionMetafileUseBaseFormat.toString());
            }
            if (null != shouldDropPartitionColumns) {
                tableConfig.setValue(
                        HoodieTableConfig.DROP_PARTITION_COLUMNS, Boolean.toString(shouldDropPartitionColumns));
            }
            ...
            if (null != inflightMetadataPartitions) {
                tableConfig.setValue(
                        HoodieTableConfig.TABLE_METADATA_PARTITIONS_INFLIGHT, inflightMetadataPartitions);
            }
            if (null != secondaryIndexesMetadata) {
                tableConfig.setValue(
                        HoodieTableConfig.SECONDARY_INDEXES_METADATA, secondaryIndexesMetadata);
            }
            if (null != multipleBaseFileFormatsEnabled) {
                tableConfig.setValue(
                        HoodieTableConfig.MULTIPLE_BASE_FILE_FORMATS_ENABLE,
            ...
            if (null == recordMergeMode) {
                ...
                if (!recordMergerStrategySet
                    ...
                    if (payloadClassNameSet) {
                        ...
                        LOG.warn(
                                "One of the payload class name or payload type must be set for the MERGE_ON_READ table");
        ...
        private void validateMergeConfigs() {
            ...
            switch (recordMergeMode) {
                case OVERWRITE_WITH_LATEST:
                    checkArgument(
                            (!payloadClassNameSet && !payloadTypeSet)
                                    || (payloadClassNameSet
                    ...
                    checkArgument(
                            ...
                            "Record merger strategy ("
                                    + (recordMergerStrategySet ? recordMergerStrategy : "null")
                ...
                case EVENT_TIME_ORDERING:
                    checkArgument(
                            ...
                            constructMergeConfigErrorMessage());
                    checkArgument(
                            !recordMergerStrategySet || recordMergerStrategy.equals(DEFAULT_MERGER_STRATEGY_UUID),
                            "Record merger strategy ("
                                    + (recordMergerStrategySet ? recordMergerStrategy : "null")
        ...
        public HoodieTableMetaClient initTable(
                StorageConfiguration<?> configuration, StoragePath basePath) throws IOException {
            return HoodieTableMetaClient.initTableAndGetMetaClient(configuration, basePath, build());
        }
    }
}
