...
public class HoodieLogFileReader implements HoodieLogFormat.Reader {
    ...
    public HoodieLogFileReader(
            HoodieStorage storage, HoodieLogFile logFile, Schema readerSchema, int bufferSize)
            throws IOException {
    ...
    public HoodieLogFileReader(
        ...
        this.logFile =
                updatedPath.equals(logFile.getPath())
                        ? logFile
                        : new HoodieLogFile(updatedPath, logFile.getFileSize());
    ...
    private HoodieLogBlock readBlock() throws IOException {
        ...
        int contentLength =
                nextBlockVersion.getVersion() != HoodieLogFormatVersion.DEFAULT_VERSION
                        ? (int) inputStream.readLong()
                        : blockSize;

        // 6. Read the content or skip content based on IO vs Memory trade-off by client
        long contentPosition = inputStream.getPos();
        boolean shouldReadLazily =
                nextBlockVersion.getVersion() != HoodieLogFormatVersion.DEFAULT_VERSION;
        Option<byte[]> content =
                HoodieLogBlock.tryReadContent(inputStream, contentLength, shouldReadLazily);
        ...
        HoodieLogBlock.HoodieLogBlockContentLocation logBlockContentLoc =
                new HoodieLogBlock.HoodieLogBlockContentLocation(
                        storage, logFile, contentPosition, contentLength, blockEndPos);
        ...
        switch (Objects.requireNonNull(blockType)) {
            ...
            case HFILE_DATA_BLOCK:
                checkState(
                        nextBlockVersion.getVersion() != HoodieLogFormatVersion.DEFAULT_VERSION,
                        String.format(
                                "HFile block could not be of version (%d)",
                ...
                return new HoodieHFileDataBlock(
                        ...
                        storage
                                .getConf()
                                .getBoolean(
                                        HoodieReaderConfig.USE_NATIVE_HFILE_READER.key(),
            ...
            case PARQUET_DATA_BLOCK:
                checkState(
                        nextBlockVersion.getVersion() != HoodieLogFormatVersion.DEFAULT_VERSION,
                        String.format(
                                "Parquet block could not be of version (%d)",
                                HoodieLogFormatVersion.DEFAULT_VERSION));
            ...
            case DELETE_BLOCK:
                return new HoodieDeleteBlock(
                        ...
                        () -> getDataInputStream(storage, this.logFile, bufferSize),
                        true,
                        Option.of(logBlockContentLoc),
                        header,
                        footer);

            case COMMAND_BLOCK:
                return new HoodieCommandBlock(
                        content,
                        () -> getDataInputStream(storage, this.logFile, bufferSize),
                        true,
                        Option.of(logBlockContentLoc),
    ...
    private Option<Schema> getTargetReaderSchemaForBlock() {
        ...
        // since when we have done some DDL operation, the readerSchema maybe different from
        // writeSchema, avro reader will throw exception.
        // eg: origin writeSchema is: "a String, b double" then we add a new column now the
        // readerSchema will be: "a string, c int, b double". it's wrong to use readerSchema to read
        // old log file.
        // after we read those record by writeSchema,  we rewrite those record with readerSchema in
    ...
    @Nullable
    private HoodieLogBlockType tryReadBlockType(HoodieLogFormat.LogFormatVersion blockVersion)
            throws IOException {
        ...
        checkArgument(
                type < HoodieLogBlockType.values().length, "Invalid block byte type found " + type);
    ...
    private HoodieLogBlock createCorruptBlock(long blockStartPos) throws IOException {
        ...
        Option<byte[]> corruptedBytes =
                HoodieLogBlock.tryReadContent(inputStream, corruptedBlockSize, true);
        HoodieLogBlock.HoodieLogBlockContentLocation logBlockContentLoc =
                new HoodieLogBlock.HoodieLogBlockContentLocation(
                        storage, logFile, contentPosition, corruptedBlockSize, nextBlockOffset);
        return new HoodieCorruptBlock(
                corruptedBytes,
                () -> getDataInputStream(storage, this.logFile, bufferSize),
                true,
                Option.of(logBlockContentLoc),
    ...
    private boolean isBlockCorrupted(int blocksize) throws IOException {
        ...
        try {
            ...
            // by seeking and checking the length of a long.  We do not seek `currentPos +
            // blocksize`
            ...
            // Block size in the footer includes the magic header, which the header does not
            // include.
            ...
            LOG.info(
                    "Found corrupted block in file {} with block size({}) running past EOF",
                    logFile,
                    blocksize);
            // this is corrupt
            // This seek is required because contract of seek() is different for naked
            // DFSInputStream vs BufferedFSInputStream
        ...
        if (blocksize != blockSizeFromFooter) {
            LOG.info(
                    "Found corrupted block in file {}. Header block size({}) did not match the footer block size({})",
                    logFile,
                    blocksize,
                    blockSizeFromFooter);
            inputStream.seek(currentPos);
        ...
        try {
            ...
            LOG.info(
                    "Found corrupted block in file {}. No magic hash found right after footer block size entry",
                    logFile);
            return true;
        } finally {
            inputStream.seek(currentPos);
    ...
    private long scanForNextAvailableBlockOffset() throws IOException {
        // Make buffer large enough to scan through the file as quick as possible especially if it
        // is on S3/GCS.
    ...
    public HoodieLogBlock prev() throws IOException {
        ...
        try {
            ...
            throw new CorruptedLogFileException(
                    "Found possible corrupted block, cannot read log file in reverse, fallback to forward reading of logfile");
        }
    ...
    /**
      * Fetch the right {@link SeekableDataInputStream} to be used by wrapping with required input
      * streams.
      *
      ...
      */
    public static SeekableDataInputStream getDataInputStream(
            HoodieStorage storage, HoodieLogFile logFile, int bufferSize) {
        try {
