...
public class HoodieTableFileSystemView extends IncrementalTimelineSyncFileSystemView {
    ...
    protected void resetViewState() {
        ...
        // because there is possibility that this in-memory view is a backend view under
        // TimelineServer,
        // and many methods in the RequestHandler is not thread safe, when performRefreshCheck flag
        // in ViewHandler
        // is set as false, the view does not perform refresh check, if #sync is called just before
        // and the members
        // are nullified, the methods that use these members would throw NPE.

        // actually there is no need to nullify the members here for 1st case, the members are
        // assigned with new values
    ...
    protected Map<HoodieFileGroupId, HoodieInstant> createFileIdToReplaceInstantMap(
            final Map<HoodieFileGroupId, HoodieInstant> replacedFileGroups) {
        return new ConcurrentHashMap<>(replacedFileGroups);
    }

    protected Map<HoodieFileGroupId, HoodieInstant> createFileIdToPendingClusteringMap(
            final Map<HoodieFileGroupId, HoodieInstant> fileGroupsInClustering) {
    ...
    @Override
    protected void removePendingCompactionOperations(
            Stream<Pair<String, CompactionOperation>> operations) {
        operations.forEach(
                opInstantPair -> {
                    ValidationUtils.checkArgument(
    ...
    protected void resetPendingLogCompactionOperations(
        ...
        this.fgIdToPendingLogCompaction =
                createFileIdToPendingLogCompactionMap(
                        operations
                                .map(
                                        entry ->
    ...
    protected void addPendingLogCompactionOperations(
        ...
        operations.forEach(
                opInstantPair -> {
                    ValidationUtils.checkArgument(
                            !fgIdToPendingLogCompaction.containsKey(opInstantPair.getValue().getFileGroupId()),
                            "Duplicate FileGroupId found in pending log compaction operations. FgId :"
                                    + opInstantPair.getValue().getFileGroupId());
                    fgIdToPendingLogCompaction.put(
                            opInstantPair.getValue().getFileGroupId(),
                            Pair.of(opInstantPair.getKey(), opInstantPair.getValue()));
                });
    }
    ...
    @Override
    protected void removePendingLogCompactionOperations(
            Stream<Pair<String, CompactionOperation>> operations) {
        operations.forEach(
                opInstantPair -> {
                    ValidationUtils.checkArgument(
    ...
    protected Stream<Pair<HoodieFileGroupId, HoodieInstant>> fetchFileGroupsInPendingClustering() {
        return fgIdToPendingClustering.entrySet().stream()
    ...
    void addFileGroupsInPendingClustering(Stream<Pair<HoodieFileGroupId, HoodieInstant>> fileGroups) {
        fileGroups.forEach(
                fileGroupInstantPair -> {
                    ValidationUtils.checkArgument(
                            fgIdToPendingClustering.containsKey(fileGroupInstantPair.getLeft()),
                            "Trying to add a FileGroupId which is already in pending clustering operation. FgId :"
                                    + fileGroupInstantPair.getLeft()
                                    + ", new instant: "
                                    + fileGroupInstantPair.getRight()
                                    + ", existing instant "
                                    + fgIdToPendingClustering.get(fileGroupInstantPair.getLeft()));

                    fgIdToPendingClustering.put(
                            fileGroupInstantPair.getLeft(), fileGroupInstantPair.getRight());
                });
    }
    ...
    void resetBootstrapBaseFileMapping(Stream<BootstrapBaseFileMapping> bootstrapBaseFileStream) {
        ...
        this.fgIdToBootstrapBaseFile =
                createFileIdToBootstrapBaseFileMap(
                        bootstrapBaseFileStream.collect(
                                Collectors.toMap(BootstrapBaseFileMapping::getFileGroupId, x -> x)));
    }
    ...
    void addBootstrapBaseFileMapping(Stream<BootstrapBaseFileMapping> bootstrapBaseFileStream) {
        bootstrapBaseFileStream.forEach(
                bootstrapBaseFile -> {
                    ValidationUtils.checkArgument(
                            !fgIdToBootstrapBaseFile.containsKey(bootstrapBaseFile.getFileGroupId()),
    ...
    protected void storePartitionView(String partitionPath, List<HoodieFileGroup> fileGroups) {
        LOG.debug(
                "Adding file-groups for partition :"
                        + partitionPath
                        + ", #FileGroups="
                        + fileGroups.size());
    ...
    protected void removeReplacedFileIdsAtInstants(Set<String> instants) {
        fgIdToReplaceInstants
                .entrySet()
                .removeIf(entry -> instants.contains(entry.getValue().getTimestamp()));
    }

    @Override
    protected boolean hasReplacedFilesInPartition(String partitionPath) {
        return fgIdToReplaceInstants.keySet().stream()
                .anyMatch(fg -> fg.getPartitionPath().equals(partitionPath));
    }
