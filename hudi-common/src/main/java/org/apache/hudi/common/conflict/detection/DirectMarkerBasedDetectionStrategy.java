...
public abstract class DirectMarkerBasedDetectionStrategy implements EarlyConflictDetectionStrategy {
    ...
    public DirectMarkerBasedDetectionStrategy(
    ...
    /**
      * We need to do list operation here. In order to reduce the list pressure as much as possible,
      * first we build path prefix in advance: '$base_path/.temp/instant_time/partition_path', and only
      * list these specific partition_paths we need instead of list all the '$base_path/.temp/'
      *
    ...
    public boolean checkMarkerConflict(String basePath, long maxAllowableHeartbeatIntervalInMs)
        ...
        List<String> candidateInstants =
                MarkerUtils.getCandidateInstants(
                        activeTimeline,
        ...
        long res =
                candidateInstants.stream()
                        .flatMap(
                                currentMarkerDirPath -> {
                                    try {
                                        ...
                                        if (!StringUtils.isNullOrEmpty(partitionPath)
                                                && !storage.exists(markerPartitionPath)) {
                                            return Stream.empty();
                                        } else {
                                            return storage.listDirectEntries(markerPartitionPath).stream()
                                                    .parallel()
                                                    .filter((path) -> path.toString().contains(fileId));
                                        }
                                    } catch (IOException e) {
                                        throw new HoodieIOException(
                                                "IOException occurs during checking marker file conflict");
                                    }
                                })
                        .count();
        ...
        if (res != 0L) {
            LOG.warn(
                    "Detected conflict marker files: "
                            + partitionPath
                            + "/"
                            + fileId
                            + " for "
