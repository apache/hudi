...
public class ExternalFilePathUtil {
    // Suffix acts as a marker when appended to a file path that the path was created by an external
    // system and not a Hudi writer.
    private static final String EXTERNAL_FILE_SUFFIX = "_hudiext";

    /**
      * Appends the commit time and external file marker to the file path. Hudi relies on the commit
      * time in the file name for properly generating views of the files in a table.
      *
    ...
    /**
      * Checks if the file name was created by an external system by checking for the external file
      * marker at the end of the file name.
      *
