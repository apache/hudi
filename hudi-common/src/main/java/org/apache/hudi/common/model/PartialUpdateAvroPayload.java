...
/**
  ...
  * <p>Gotchas:
  *
  * <p>In cases where a batch of records is preCombine before combineAndGetUpdateValue with the
  * underlying records to be updated located in parquet files, the end states of records might not be
  * as how one will expect when applying a straightforward partial update.
  *
  * <p>Gotchas-Example:
  *
  * <pre>
...
public class PartialUpdateAvroPayload extends OverwriteNonDefaultsWithLatestAvroPayload {
    ...
    @Override
    public PartialUpdateAvroPayload preCombine(
            OverwriteWithLatestAvroPayload oldValue, Schema schema, Properties properties) {
        ...
        final boolean shouldPickOldRecord =
                oldValue.orderingVal.compareTo(orderingVal) > 0 ? true : false;
        try {
            GenericRecord oldRecord = HoodieAvroUtils.bytesToAvro(oldValue.recordBytes, schema);
            Option<IndexedRecord> mergedRecord =
                    mergeOldRecord(oldRecord, schema, shouldPickOldRecord, true);
            if (mergedRecord.isPresent()) {
                return new PartialUpdateAvroPayload(
                        (GenericRecord) mergedRecord.get(),
    ...
    public Option<IndexedRecord> combineAndGetUpdateValue(
            IndexedRecord currentValue, Schema schema, Properties prop) throws IOException {
        return mergeOldRecord(
                currentValue, schema, isRecordNewer(orderingVal, currentValue, prop), false);
    }
    ...
    /**
      ...
      * @param isPreCombining flag for deleted record combine logic 1 preCombine: if delete record is
      *     newer, return merged record with _hoodie_is_deleted = true 2 combineAndGetUpdateValue: if
      *     delete record is newer, return empty since we don't need to store deleted data to storage
      * @return
      * @throws IOException
      */
    private Option<IndexedRecord> mergeOldRecord(
            IndexedRecord oldRecord, Schema schema, boolean isOldRecordNewer, boolean isPreCombining)
            throws IOException {
    ...
    /**
      ...
      * @throws IOException
      */
    public Option<IndexedRecord> getInsertValue(Schema schema, boolean isPreCombining)
            throws IOException {
    ...
    /**
      ...
      */
    private static boolean isRecordNewer(
            Comparable orderingVal, IndexedRecord record, Properties prop) {
            ...
            Comparable oldOrderingVal =
                    (Comparable)
                            HoodieAvroUtils.getNestedFieldVal(
                                    (GenericRecord) record, orderingField, true, consistentLogicalTimestampEnabled);
