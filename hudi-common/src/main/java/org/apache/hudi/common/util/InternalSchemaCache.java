...
/**
  * An internal cache implementation for managing different version of schemas. This is a Global
  * cache; all threads in one container/executor share the same cache. A map of (tablePath,
  * HistorySchemas) is maintained.
  */
public class InternalSchemaCache {
    ...
    /**
      * Search internalSchema based on versionID. first step: try to get internalSchema from hoodie
      * commit files, we no need to add lock. if we cannot get internalSchema by first step, then we
      * try to get internalSchema from cache.
      *
      * @param versionID schema version_id need to search
      * @param metaClient current hoodie metaClient
      * @return internalSchema
      */
    public static InternalSchema searchSchemaAndCache(
            long versionID, HoodieTableMetaClient metaClient) {
    ...
    private static TreeMap<Long, InternalSchema> getHistoricalSchemas(
    ...
    private static Option<InternalSchema> getSchemaByReadingCommitFile(
            long versionID, HoodieTableMetaClient metaClient) {
        try {
            ...
            List<HoodieInstant> instants =
                    timeline
                            .getInstantsAsStream()
                            .filter(f -> f.getTimestamp().equals(String.valueOf(versionID)))
    ...
    public static Pair<Option<String>, Option<String>>
            getInternalSchemaAndAvroSchemaForClusteringAndCompaction(
        ...
        if (lastInstantBeforeCurrentCompaction.isPresent()) {
            ...
            byte[] data =
                    timelineBeforeCurrentCompaction
                            .getInstantDetails(lastInstantBeforeCurrentCompaction.get())
                            .get();
            HoodieCommitMetadata metadata;
            try {
                ...
                throw new HoodieException(
                        String.format(
                                "cannot read metadata from commit: %s", lastInstantBeforeCurrentCompaction.get()),
                        e);
            }
    ...
    /**
      * Give a schema versionId return its internalSchema. This method will be called by spark tasks,
      * we should minimize time cost. We try our best to not use metaClient， since the initialization
      * of metaClient is time cost step1： try to parser internalSchema from HoodieInstant directly
      * step2： if we cannot parser internalSchema in step1， (eg: current versionId HoodieInstant has
      * been archived) try to find internalSchema in historySchema. step3: if we cannot parser
      * internalSchema in step2 (eg: schema evolution is not enabled when we create hoodie table,
      * however after some inserts we enable schema evolution) try to convert table schema to
      * internalSchema.
      *
      * @param versionId the internalSchema version to be search.
      * @param tablePath table path
      * @param storage {@link HoodieStorage} instance.
      * @param validCommits current validate commits, use to make up the commit file path/verify the
      *     validity of the history schema files
      * @return a internalSchema.
      */
    public static InternalSchema getInternalSchemaByVersionId(
            long versionId, String tablePath, HoodieStorage storage, String validCommits) {
        String avroSchema = "";
        ...
        if (candidateCommitFile != null) {
            try {
                ...
                LOG.warn(
                        "Cannot find internal schema from commit file {}. Falling back to parsing historical internal schema",
                        candidateCommitFile);
            }
        }
        // step2:
        ...
        String latestHistorySchema =
                fileBasedInternalSchemaStorageManager.getHistorySchemaStrByGivenValidCommits(
                        validateCommitList);
        ...
        return fileSchema.isEmptySchema()
                ? StringUtils.isNullOrEmpty(avroSchema)
                        ...
                        : AvroInternalSchemaConverter.convert(
                                HoodieAvroUtils.addMetadataFields(new Schema.Parser().parse(avroSchema)))
                : fileSchema;
    }

    public static InternalSchema getInternalSchemaByVersionId(
            long versionId, HoodieTableMetaClient metaClient) {
        String validCommitLists =
                metaClient
                        .getCommitsAndCompactionTimeline()
                        .filterCompletedInstants()
                        .getInstantsAsStream()
        ...
        return getInternalSchemaByVersionId(
                versionId, metaClient.getBasePath().toString(), metaClient.getStorage(), validCommitLists);
    }
}
