...
public class HoodieRollbackStat implements Serializable {
  ...
  private final Map<HoodieFileStatus, Long> commandBlocksCount;
  ...
  public HoodieRollbackStat(String partitionPath, List<String> successDeleteFiles, List<String> failedDeleteFiles,
                            Map<HoodieFileStatus, Long> commandBlocksCount, Map<String, Long> logFilesFromFailedCommit) {
  ...
  public Map<HoodieFileStatus, Long> getCommandBlocksCount() {
  ...
  public static class Builder {
    ...
    private Map<HoodieFileStatus, Long> commandBlocksCount;
    private Map<String, Long> logFilesFromFailedCommit;
    private String partitionPath;

    public Builder withDeletedFileResults(Map<HoodieFileStatus, Boolean> deletedFiles) {
      // noinspection Convert2MethodRef
      successDeleteFiles = deletedFiles.entrySet().stream().filter(s -> s.getValue())
          .map(s -> s.getKey().getLocation().toString()).collect(Collectors.toList());
    ...
    public Builder withRollbackBlockAppendResults(Map<HoodieFileStatus, Long> commandBlocksCount) {
      this.commandBlocksCount = commandBlocksCount;
      return this;
    }
