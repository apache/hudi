...
public class HoodieMetadataMergedLogRecordScanner extends BaseHoodieMergedLogRecordScanner<String> {
    ...
    private HoodieMetadataMergedLogRecordScanner(
    ...
    public <T> void processNextRecord(HoodieRecord<T> newRecord) throws IOException {
        ...
        if (oldRecord != null) {
            LOG.debug(
                    "Merging new record with existing record in the map. Key: {}", newRecord.getRecordKey());
            recordMerger
                    .fullOuterMerge(oldRecord, readerSchema, newRecord, readerSchema, this.getPayloadProps())
                    .forEach(
                            mergedRecord -> {
    ...
    public static class Builder extends AbstractHoodieLogRecordReader.Builder {
        ...
        public HoodieMetadataMergedLogRecordScanner build() {
            ...
            checkArgument(
                    nonEmpty(partitionName),
                    "Partition name is required for HoodieMetadataMergedLogRecordScanner.");

            return new HoodieMetadataMergedLogRecordScanner(
                    storage,
                    basePath,
                    logFilePaths,
                    readerSchema,
