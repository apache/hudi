...
public class FailSafeConsistencyGuard implements ConsistencyGuard {
   ...
   * @throws IOException -
   */
  protected boolean checkFileVisibility(StoragePath location, FileVisibility visibility)
      throws IOException {
    try {
      StoragePathInfo status = storage.getPathInfo(location);
      switch (visibility) {
        case APPEAR:
          return status != null;
        case DISAPPEAR:
   ...
   * @param location File Path
   */
  private void waitForFileVisibility(StoragePath location, FileVisibility visibility)
      throws TimeoutException {
