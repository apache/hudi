...
/**
  * A consistency checker that fails if it is unable to meet the required condition within a
  * specified timeout.
  */
public class FailSafeConsistencyGuard implements ConsistencyGuard {
    ...
    public FailSafeConsistencyGuard(
            HoodieStorage storage, ConsistencyGuardConfig consistencyGuardConfig) {
    ...
    /**
      ...
      */
    protected boolean checkFilesVisibility(
            int retryNum, StoragePath dir, List<String> files, FileVisibility event) {
        try {
