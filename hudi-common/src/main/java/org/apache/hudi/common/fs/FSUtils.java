...
public class FSUtils {
    ...
    /**
      * A write token uniquely identifies an attempt at one of the IOHandle operations
      * (Merge/Create/Append).
      */
    ...
    public static String makeBaseFileName(
            String instantTime, String writeToken, String fileId, String fileExtension) {
    ...
    /**
      * Gets all partition paths assuming date partitioning (year, month, day) three levels down. TODO:
      * (Lin) Delete this function after we remove the assume.date.partitioning config completely.
      */
    public static List<String> getAllPartitionFoldersThreeLevelsDown(
        ...
        List<StoragePathInfo> folders =
                storage.globEntries(new StoragePath(basePath + "/*/*/*"), filter);
        for (StoragePathInfo pathInfo : folders) {
            StoragePath path = pathInfo.getPath();
            datePartitions.add(
                    String.format(
                            "%s/%s/%s",
                            path.getParent().getParent().getName(), path.getParent().getName(), path.getName()));
        }
        return datePartitions;
    }

    public static String getRelativePartitionPath(
            StoragePath basePath, StoragePath fullPartitionPath) {
    ...
    /**
      * Recursively processes all files in the base-path. If excludeMetaFolder is set, the meta-folder
      * and all its subdirs are skipped
      *
    ...
    public static void processFiles(
            ...
            throws IOException {
        StoragePathFilter pathFilter =
                excludeMetaFolder ? getExcludeMetaPathFilter() : ALLOW_ALL_FILTER;
        List<StoragePathInfo> topLevelInfoList =
                storage.listDirectEntries(new StoragePath(basePathStr));
    ...
    public static List<String> getAllPartitionPaths(
        ...
        try (HoodieTableMetadata tableMetadata =
                HoodieTableMetadata.create(engineContext, storage, metadataConfig, basePathStr)) {
    ...
    public static List<String> getAllPartitionPaths(
        ...
        return getAllPartitionPaths(
                engineContext, storage, basePath.toString(), useFileListingFromMetadata);
    }
    ...
    public static Map<String, List<StoragePathInfo>> getFilesInPartitions(
            HoodieEngineContext engineContext,
            HoodieStorage storage,
            HoodieMetadataConfig metadataConfig,
            String basePathStr,
        ...
        try (HoodieTableMetadata tableMetadata =
            ...
            throw new HoodieException(
                    "Error get files in partitions: " + String.join(",", partitionPaths), ex);
        }
    }
    ...
    public static String makeLogFileName(
        ...
        String suffix =
                (writeToken == null)
                        ...
                        : String.format(
                                "%s_%s%s.%d_%s", fileId, deltaCommitTime, logFileExtension, version, writeToken);
    ...
    public static boolean isLogFile(StoragePath logPath) {
        ...
        return isLogFile(
                InLineFSUtils.SCHEME.equals(scheme)
    ...
    public static List<StoragePathInfo> getAllDataFilesInPartition(
            HoodieStorage storage, StoragePath partitionPath) throws IOException {
    ...
    public static Option<HoodieLogFile> getLatestLogFile(
            HoodieStorage storage,
        ...
        return getLatestLogFile(
                getAllLogFiles(storage, partitionPath, fileId, logFileExtension, deltaCommitTime));
    }

    /** Get all the log files for the passed in file-id in the partition path. */
    public static Stream<HoodieLogFile> getAllLogFiles(
            HoodieStorage storage,
            StoragePath partitionPath,
            final String fileId,
            final String logFileExtension,
            final String deltaCommitTime)
            throws IOException {
        try {
            StoragePathFilter pathFilter =
                    path ->
                            path.getName().startsWith("." + fileId) && path.getName().contains(logFileExtension);
    ...
    public static Option<Pair<Integer, String>> getLatestLogVersion(
        ...
        if (latestLogFile.isPresent()) {
            return Option.of(
                    Pair.of(latestLogFile.get().getLogVersion(), latestLogFile.get().getLogWriteToken()));
        }
        return Option.empty();
    }
    ...
    public static StoragePath constructAbsolutePath(
            StoragePath basePath, String relativePartitionPath) {
        // For non-partitioned table, return only base-path
        return StringUtils.isNullOrEmpty(relativePartitionPath)
                ? basePath
                : new StoragePath(basePath, relativePartitionPath);
    }

    /**
      * Extracts the file name from the relative path based on the table base path. For example:
      * "/2022/07/29/file1.parquet", "/2022/07/29" -> "file1.parquet" "2022/07/29/file2.parquet",
      * "2022/07/29" -> "file2.parquet" "/file3.parquet", "" -> "file3.parquet" "file4.parquet", "" ->
      * "file4.parquet"
      *
      * @param filePathWithPartition the relative file path based on the table base path.
      * @param partition the relative partition path. For partitioned table, `partition` contains the
      *     relative partition path; for non-partitioned table, `partition` is empty
      * @return Extracted file name in String.
      */
    public static String getFileName(String filePathWithPartition, String partition) {
        int offset =
                StringUtils.isNullOrEmpty(partition)
    ...
    /**
      ...
      */
    public static List<StoragePathInfo> getGlobStatusExcludingMetaFolder(
            HoodieStorage storage, StoragePath globPath) throws IOException {
    ...
    public static boolean deleteDir(
        ...
        try {
            if (storage.exists(dirPath)) {
                FSUtils.parallelizeSubPathProcess(
                        hoodieEngineContext,
                        storage,
                        dirPath,
                        parallelism,
        ...
        try {
            ...
            List<String> subPaths =
                    pathInfoList.stream()
                            ...
                            .collect(Collectors.toList());
            result =
                    parallelizeFilesProcess(
                            hoodieEngineContext, storage, parallelism, pairFunction, subPaths);
        } catch (IOException ioe) {
            throw new HoodieIOException(ioe.getMessage(), ioe);
        }
        return result;
    ...
    public static <T> Map<String, T> parallelizeFilesProcess(
        ...
        if (subPaths.size() > 0) {
            ...
            hoodieEngineContext.setJobStatus(
                    FSUtils.class.getSimpleName(), "Parallel listing paths " + String.join(",", subPaths));

            result =
                    hoodieEngineContext.mapToPair(
                            subPaths,
                            subPath ->
                                    new ImmutablePair<>(
                                            subPath, pairFunction.apply(new ImmutablePair<>(subPath, storageConf))),
                            actualParallelism);
        }
        return result;
    }
    ...
    /**
      ...
      */
    public static boolean deleteSubPath(
            String subPathStr, StorageConfiguration<?> conf, boolean recursive) {
        try {
