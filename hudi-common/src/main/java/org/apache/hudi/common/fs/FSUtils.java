...
public class FSUtils {
  ...
  public static List<String> getAllPartitionFoldersThreeLevelsDown(HoodieStorage storage, String basePath) throws IOException {
    ...
    StoragePathFilter filter = getExcludeMetaLocationFilter();
    List<StoragePathInfo> folders = storage.globEntries(new StoragePath(basePath + "/*/*/*"), filter);
    for (StoragePathInfo status : folders) {
      StoragePath path = status.getPath();
      datePartitions.add(
  ...
  public static String getRelativePartitionPathFromLocation(StoragePath basePath, StoragePath fullPartitionPath) {
  ...
  public static StoragePath getLocationWithoutSchemeAndAuthority(StoragePath location) {
    return location.getPathWithoutSchemeAndAuthority();
  }
  ...
  public static void processFiles(HoodieStorage storage, String basePathStr, Function<StoragePathInfo, Boolean> consumer,
    ...
    StoragePathFilter locationFilter = excludeMetaFolder ? getExcludeMetaLocationFilter() : ALLOW_ALL_FILTER;
    List<StoragePathInfo> topLevelStatuses = storage.listDirectEntries(new StoragePath(basePathStr));
    for (StoragePathInfo child : topLevelStatuses) {
      if (child.isFile()) {
        boolean success = consumer.apply(child);
        if (!success) {
          throw new HoodieException("Failed to process file-status=" + child);
        }
      } else if (locationFilter.accept(child.getPath())) {
        List<StoragePathInfo> list = storage.listFiles(child.getPath());
        for (StoragePathInfo status : list) {
  ...
  private static StoragePathFilter getExcludeMetaLocationFilter() {
    // Avoid listing and including any folders under the metafolder
   ...
   */
  public static String getFileExtensionFromLog(StoragePath logLocation) {
    Matcher matcher = LOG_FILE_PATTERN.matcher(logLocation.getName());
    if (!matcher.find()) {
      throw new InvalidHoodiePathException(logLocation.toString(), "LogFile");
    }
   ...
   */
  public static String getDeltaCommitTimeFromLogPath(StoragePath location) {
    Matcher matcher = LOG_FILE_PATTERN.matcher(location.getName());
    if (!matcher.find()) {
      throw new InvalidHoodiePathException(location.toString(), "LogFile");
    }
    return matcher.group(2);
  }

  /**
   * Get TaskPartitionId used in log-location.
   */
  public static Integer getTaskPartitionIdFromLogPath(StoragePath location) {
    Matcher matcher = LOG_FILE_PATTERN.matcher(location.getName());
    if (!matcher.find()) {
      throw new InvalidHoodiePathException(location.toString(), "LogFile");
    }
   ...
   */
  public static String getWriteTokenFromLogPath(StoragePath location) {
    Matcher matcher = LOG_FILE_PATTERN.matcher(location.getName());
    if (!matcher.find()) {
      throw new InvalidHoodiePathException(location.toString(), "LogFile");
    }
  ...
  /**
   * Get StageId used in log-location.
   */
  public static Integer getStageIdFromLogPath(StoragePath location) {
    Matcher matcher = LOG_FILE_PATTERN.matcher(location.getName());
    if (!matcher.find()) {
      throw new InvalidHoodiePathException(location.toString(), "LogFile");
    }
  ...
  /**
   * Get Task Attempt Id used in log-location.
   */
  public static Integer getTaskAttemptIdFromLogPath(StoragePath location) {
    Matcher matcher = LOG_FILE_PATTERN.matcher(location.getName());
    if (!matcher.find()) {
      throw new InvalidHoodiePathException(location.toString(), "LogFile");
    }
   ...
   */
  public static int getFileVersionFromLog(StoragePath logLocation) {
    return getFileVersionFromLog(logLocation.getName());
  }
  ...
  public static Path getPartitionPathInPath(String basePath, String partitionPath) {
  ...
  public static Configuration registerFileSystem(StoragePath location, Configuration conf) {
  ...
  public static HoodieWrapperFileSystem getFs(String path, SerializableConfiguration hadoopConf,
    ...
    return new HoodieWrapperFileSystem(fileSystem,
        consistencyGuardConfig.isConsistencyCheckEnabled()
            ? new FailSafeConsistencyGuard(HoodieStorageUtils.getHoodieStorage(fileSystem), consistencyGuardConfig)
   ...
   * @param storage             {@link HoodieStorage} instance.
   * @param dirLocation         directory path
   * @param parallelism         parallelism to use for sub-paths
   * @return {@code true} if the directory is delete; {@code false} otherwise.
   */
  public static boolean deleteDir(
      HoodieEngineContext hoodieEngineContext, HoodieStorage storage, StoragePath dirLocation, int parallelism) {
    try {
      if (storage.exists(dirLocation)) {
        FSUtils.parallelizeSubPathProcess(hoodieEngineContext, storage, dirLocation, parallelism, e -> true,
            pairOfSubPathAndConf -> deleteSubPath(
                pairOfSubPathAndConf.getKey(), pairOfSubPathAndConf.getValue(), true)
        );
        boolean result = storage.deleteDirectory(dirLocation);
        LOG.info("Removed directory at " + dirLocation);
        return result;
      }
  ...
  public static <T> Map<String, T> parallelizeSubPathProcess(
    ...
    Map<String, T> result = new HashMap<>();
    try {
      List<StoragePathInfo> fileStatuses = storage.listDirectEntries(dirPath);
      List<String> subPaths = fileStatuses.stream()
          .filter(subPathPredicate)
          .map(fileStatus -> fileStatus.getPath().toString())
          .collect(Collectors.toList());
  ...
  public static List<StoragePathInfo> getAllDataFileStatus(HoodieStorage storage, StoragePath path)
      throws IOException {
    List<StoragePathInfo> statuses = new ArrayList<>();
    for (StoragePathInfo status : storage.listDirectEntries(path)) {
      if (!status.getPath().toString().contains(HoodieTableMetaClient.METAFOLDER_NAME)) {
        if (status.isDirectory()) {
          statuses.addAll(getAllDataFileStatus(storage, status.getPath()));
        } else {
          statuses.add(status);
        }
      }
