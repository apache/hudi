...
public abstract class HoodieRecord<T> implements HoodieRecordCompatibilityInterface, KryoSerializable, Serializable {
  ...
  public boolean isDelete(Schema recordSchema, Properties props, DeleteContext deleteContext) {
    if (isDelete == null) {
      try {
        isDelete = checkIsDelete(recordSchema, props, deleteContext);
      } catch (IOException e) {
   ...
   * todo: all subclass
   */
  protected abstract boolean checkIsDelete(Schema recordSchema, Properties props, DeleteContext deleteContext) throws IOException;
