...
public abstract class HoodieRecord<T>
        implements HoodieRecordCompatibilityInterface, KryoSerializable, Serializable {
    ...
    /**
      * A special record returned by {@link HoodieRecordPayload}, which means we should just skip this
      * record. This record is only used for {@link HoodieRecordPayload} currently, so it should not
      * shuffle though network, we can compare the record locally by the equal method. The
      * HoodieRecordPayload#combineAndGetUpdateValue and HoodieRecordPayload#getInsertValue have 3 kind
      * of return: 1、Option.empty This means we should delete this record. 2、IGNORE_RECORD This means
      * we should not process this record,just skip. 3、Other non-empty record This means we should
      * process this record.
      *
      * <p>We can see the usage of IGNORE_RECORD in
    ...
    public static int COMMIT_TIME_METADATA_FIELD_ORD =
            HOODIE_META_COLUMNS_NAME_TO_POS.get(COMMIT_TIME_METADATA_FIELD);
    public static int COMMIT_SEQNO_METADATA_FIELD_ORD =
            HOODIE_META_COLUMNS_NAME_TO_POS.get(COMMIT_SEQNO_METADATA_FIELD);
    ...
    public HoodieRecord(
            HoodieKey key, T data, HoodieOperation operation, Option<Map<String, String>> metaData) {
        this.key = key;
        this.data = data;
    ...
    public abstract String getRecordKey(
            Schema recordSchema, Option<BaseKeyGenerator> keyGeneratorOpt);
    ...
    /**
      ...
      *
      * <p>This is required in the delete path so that Index can track that this record was deleted.
      */
    ...
    /**
      ...
      */
    public abstract HoodieRecord prependMetaFields(
            Schema recordSchema, Schema targetSchema, MetadataValues metadataValues, Properties props);

    /** Support schema evolution. */
    public abstract HoodieRecord rewriteRecordWithNewSchema(
    ...
    public HoodieRecord rewriteRecordWithNewSchema(
            Schema recordSchema, Properties props, Schema newSchema) {
