...
/**
  * Adds the capability to incrementally sync the changes to file-system view as and when new
  * instants gets completed.
  */
public abstract class IncrementalTimelineSyncFileSystemView extends AbstractTableFileSystemView {

    private static final Logger LOG =
            LoggerFactory.getLogger(IncrementalTimelineSyncFileSystemView.class);
        ...
        try {
            if (incrementalTimelineSyncEnabled) {
                TimelineDiffResult diffResult =
                        TimelineDiffHelper.getNewInstantsForIncrementalSync(oldTimeline, newTimeline);
            ...
            LOG.error(
                    "Got exception trying to perform incremental sync. Reverting to complete sync", ioe);
        }
        clear();
    ...
    private void runIncrementalSync(HoodieTimeline timeline, TimelineDiffResult diffResult) {
        ...
        diffResult.getFinishedCompactionInstants().stream()
                .forEach(
                        instant -> {
                            try {
        ...
        diffResult.getFinishedOrRemovedLogCompactionInstants().stream()
                .forEach(
                        instant -> {
                            try {
        ...
        // Add new completed instants found in the latest timeline, this also contains inflight
        // instants.
    ...
    private void removePendingCompactionInstant(HoodieInstant instant) throws IOException {
        ...
        HoodieCompactionPlan plan =
                CompactionUtils.getCompactionPlan(metaClient, instant.getTimestamp());
        removePendingCompactionOperations(
                CompactionUtils.getPendingCompactionOperations(instant, plan)
                        .map(
                                instantPair ->
                                        Pair.of(
                                                instantPair.getValue().getKey(),
                                                CompactionOperation.convertFromAvroRecordInstance(
                                                        instantPair.getValue().getValue()))));
    }

    /**
      * Remove Pending compaction instant. This is called when logcompaction is converted to delta
      * commit, so you no longer need to track them as pending.
      *
    ...
    private void removePendingLogCompactionInstant(HoodieInstant instant) throws IOException {
        ...
        removePendingLogCompactionOperations(
                CompactionUtils.getPendingCompactionOperations(instant, plan)
                        .map(
                                instantPair ->
                                        Pair.of(
    ...
    /**
      ...
      */
    private void addPendingCompactionInstant(HoodieTimeline timeline, HoodieInstant instant)
            throws IOException {
                        ...
                        .map(
                                opPair -> {
                                    ...
                                    HoodieFileGroup fileGroup =
                                            new HoodieFileGroup(opPair.getValue().getFileGroupId(), timeline);
    ...
    private void addPendingLogCompactionInstant(HoodieInstant instant) throws IOException {
        ...
        // Since logcompaction works similar to a deltacommit. Updating the partition view is not
        // required.
    ...
    private void updatePartitionWriteFileGroups(
        ...
        partitionToWriteStats.entrySet().stream()
                .forEach(
                        entry -> {
                                                ...
                                                .map(
                                                        p ->
                                                                new StoragePathInfo(
                                                                        new StoragePath(
                                                                                String.format(
                                                                                        "%s/%s", metaClient.getBasePath(), p.getPath())),
                                                                        p.getFileSizeInBytes(),
                                                                        false,
                                                                        (short) 0,
                                                                        0,
                                                                        0))
    ...
    /**
      ...
      */
    private void addRestoreInstant(HoodieTimeline timeline, HoodieInstant instant)
            throws IOException {
        ...
        Map<String, List<Pair<String, String>>> partitionFiles =
                metadata.getHoodieRestoreMetadata().entrySet().stream()
                        .flatMap(
                                entry -> {
                                    return entry.getValue().stream()
                                            .flatMap(
                                                    e ->
                                                            e.getPartitionMetadata().entrySet().stream()
                                                                    .flatMap(
                                                                            e2 -> {
        ...
        if (metadata.getRestoreInstantInfo() != null) {
            Set<String> rolledbackInstants =
                    metadata.getRestoreInstantInfo().stream()
                            .filter(
                                    instantInfo ->
                                            HoodieTimeline.REPLACE_COMMIT_ACTION.equals(instantInfo.getAction()))
                            .map(instantInfo -> instantInfo.getCommitTime())
    ...
    /**
      ...
      */
    private void addRollbackInstant(HoodieTimeline timeline, HoodieInstant instant)
            throws IOException {
        ...
        metadata.getPartitionMetadata().entrySet().stream()
                .forEach(
                        e -> {
                            removeFileSlicesForPartition(
                                    timeline, instant, e.getKey(), e.getValue().getSuccessDeleteFiles());
                        });
    ...
    /**
      ...
      */
    private void addReplaceInstant(HoodieTimeline timeline, HoodieInstant instant)
            throws IOException {
        ...
        replaceMetadata.getPartitionToReplaceFileIds().entrySet().stream()
                .forEach(
                        entry -> {
                            String partition = entry.getKey();
                            Map<HoodieFileGroupId, HoodieInstant> replacedFileIds =
                                    entry.getValue().stream()
                                            .collect(
                                                    Collectors.toMap(
                                                            replaceStat -> new HoodieFileGroupId(partition, replaceStat),
                                                            replaceStat -> instant));
    ...
    /**
      * Add newly found clean instant. Note that cleaner metadata (.clean.completed) contains only
      * relative paths unlike clean plans (.clean.requested) which contains absolute paths.
      *
    ...
    private void addCleanInstant(HoodieTimeline timeline, HoodieInstant instant) throws IOException {
        ...
        cleanMetadata.getPartitionMetadata().entrySet().stream()
                .forEach(
                        entry -> {
                            ...
                            List<String> fullPathList =
                                    entry.getValue().getSuccessDeleteFiles().stream()
                                            .map(
                                                    fileName ->
                                                            new StoragePath(
    ...
    private void removeFileSlicesForPartition(
            HoodieTimeline timeline, HoodieInstant instant, String partition, List<String> paths) {
        if (isPartitionAvailableInStore(partition)) {
            LOG.info(
                    "Removing file slices for partition (" + partition + ") for instant (" + instant + ")");
            List<StoragePathInfo> pathInfoList =
                    paths.stream()
            ...
            List<HoodieFileGroup> fileGroups =
                    buildFileGroups(
                            partition, pathInfoList, timeline.filterCompletedAndCompactionInstants(), false);
    ...
    /**
      * Apply changes to partition file-system view. Base Implementation overwrites the entire
      * partitions view assuming some sort of map (in-mem/disk-based) is used. For View implementation
      * which supports fine-granular updates (e:g RocksDB), override this method.
      *
    ...
    protected void applyDeltaFileSlicesToPartitionView(
        ...
        /**
          * Note that while finding the new data/log files added/removed, the path stored in metadata
          * will be missing the base-path,scheme and authority. Ensure the matching process takes care of
          * this discrepancy.
          */
        Map<String, HoodieBaseFile> viewDataFiles =
                fileGroups.stream()
                        ...
                        .map(
                                df ->
                                        Pair.of(
                                                FSUtils.getPathWithoutSchemeAndAuthority(new StoragePath(df.getPath()))
                                                        .toString(),
                                                df))
                        .collect(Collectors.toMap(Pair::getKey, Pair::getValue));
        // Note: Delta Log Files and Data Files can be empty when adding/removing pending
        // compactions
        Map<String, HoodieBaseFile> deltaDataFiles =
                deltaFileGroups.stream()
                        .flatMap(HoodieFileGroup::getAllRawFileSlices)
                        .map(FileSlice::getBaseFile)
                        .filter(Option::isPresent)
                        .map(Option::get)
        ...
        Map<String, HoodieLogFile> viewLogFiles =
                fileGroups.stream()
                        ...
                        .map(
                                lf ->
                                        Pair.of(FSUtils.getPathWithoutSchemeAndAuthority(lf.getPath()).toString(), lf))
                        .collect(Collectors.toMap(Pair::getKey, Pair::getValue));
        ...
        List<HoodieFileGroup> fgs =
                buildFileGroups(
                        partition,
                        viewDataFiles.values().stream(),
                        viewLogFiles.values().stream(),
                        timeline,
                        true);
