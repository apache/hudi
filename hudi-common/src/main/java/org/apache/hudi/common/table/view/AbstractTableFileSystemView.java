...
/**
  * Common thread-safe implementation for multiple TableFileSystemView Implementations. Provides
  * uniform handling of:
  *
  * <ul>
  *   <li>Loading file-system views from underlying file-system;
  *   <li>Pending compaction operations and changing file-system views based on that;
  *   <li>Thread-safety in loading and managing file system views for this table;
  *   <li>resetting file-system views.
  * </ul>
  *
  * The actual mechanism of fetching file slices from different view storages is delegated to
  * sub-classes.
  */
public abstract class AbstractTableFileSystemView implements SyncableFileSystemView, Serializable {
    ...
    // This is nothing but the write timeline, which contains both ingestion and compaction(major
    // and minor) writers.
    ...
    protected void init(HoodieTableMetaClient metaClient, HoodieTimeline visibleActiveTimeline) {
        ...
        resetPendingCompactionOperations(
                CompactionUtils.getAllPendingCompactionOperations(metaClient).values().stream()
                        .map(
                                e ->
                                        Pair.of(
                                                e.getKey(),
        ...
        resetPendingLogCompactionOperations(
                CompactionUtils.getAllPendingLogCompactionOperations(metaClient).values().stream()
                        .map(
                                e ->
                                        Pair.of(
                                                e.getKey(),
    ...
    /**
      * Adds the provided statuses into the file system view, and also caches it inside this object. If
      * the file statuses are limited to a single partition, use {@link #addFilesToView(String, List)}
      * instead.
      */
                                ...
                                Collectors.groupingBy(
                                        fileStatus ->
                                                FSUtils.getRelativePartitionPath(
                                                        metaClient.getBasePath(), fileStatus.getPath().getParent())));
        return statusesByPartitionPath.entrySet().stream()
                .map(entry -> addFilesToView(entry.getKey(), entry.getValue()))
                .flatMap(List::stream)
                .collect(Collectors.toList());
    }

    /**
      * Adds the provided statuses into the file system view for a single partition, and also caches it
      * inside this object.
      */
    public List<HoodieFileGroup> addFilesToView(
            String partitionPath, List<StoragePathInfo> statuses) {
        HoodieTimer timer = HoodieTimer.start();
        List<HoodieFileGroup> fileGroups =
                buildFileGroups(partitionPath, statuses, visibleCommitsAndCompactionTimeline, true);
        long fgBuildTimeTakenMs = timer.endTimer();
        timer.startTimer();
        // Group by partition for efficient updates for both InMemory and DiskBased structures.
        fileGroups.stream()
                .collect(Collectors.groupingBy(HoodieFileGroup::getPartitionPath))
                .forEach(
                        (partition, value) -> {
                            if (!isPartitionAvailableInStore(partition)) {
                                if (bootstrapIndex.useIndex()) {
                                    try (BootstrapIndex.IndexReader reader = bootstrapIndex.createReader()) {
                                        ...
                                        addBootstrapBaseFileMapping(
                                                sourceFileMappings.stream()
                                                        .map(
                                                                s ->
                                                                        new BootstrapBaseFileMapping(
                                                                                new HoodieFileGroupId(s.getPartitionPath(), s.getFileId()),
                                                                                s.getBootstrapFileStatus())));
                                    }
                                }
        ...
        LOG.debug(
                "addFilesToView: NumFiles="
                        + statuses.size()
                        + ", NumFileGroups="
                        + fileGroups.size()
                        + ", FileGroupsCreationTime="
                        + fgBuildTimeTakenMs
                        + ", StoreTimeTaken="
                        + storePartitionsTs);
        return fileGroups;
    ...
    protected List<HoodieFileGroup> buildFileGroups(
            String partition,
        ...
        fileIdSet.forEach(
                fileId -> {
                    ...
                    if (addPendingCompactionFileSlice) {
                        ...
                        // the log files completed later than the compaction instant time could be
                        // included
    ...
    private void resetFileGroupsReplaced(HoodieTimeline timeline) {
                                                                        ...
                                                                        .map(
                                                                                e ->
                                                                                        new AbstractMap.SimpleEntry<>(
                                                                                                new HoodieFileGroupId(entry.getKey(), e),
                                                                                                instant)));
                                    } catch (HoodieIOException ex) {

                                        if (ex.getIOException() instanceof FileNotFoundException) {
                                            // Replace instant could be deleted by archive and
                                            // FileNotFoundException could be threw during
                                            // getInstantDetails function
                                            // So that we need to catch the FileNotFoundException
                                            // here and continue
        ...
        // Duplicate key error when insert_overwrite same partition in multi writer, keep the
        // instant with greater timestamp when the file group id conflicts
        ...
        LOG.info(
                "Took "
                        ...
                        + " ms to read  "
                        + replacedTimeline.countInstants()
                        + " instants, "
    ...
    /**
      ...
      *
      * <p>NOTE: The logic MUST BE guarded by the write lock.
      */
    @Override
    public void reset() {
        try {
    ...
    private void ensurePartitionsLoadedCorrectly(List<String> partitionList) {
        ...
        synchronized (addedPartitions) {
                ...
                try {
                    ...
                    pathInfoMap.forEach(
                            (partitionPair, statuses) -> {
                                ...
                                LOG.debug(
                                        "#files found in partition (" + relativePartitionStr + ") =" + statuses.size());
                            });
                } catch (IOException e) {
            ...
            partitionSet.forEach(
                    partition -> addedPartitions.computeIfAbsent(partition, partitionPathStr -> true));
        }
    }
    ...
    protected List<String> getAllPartitionPaths() throws IOException {
        throw new HoodieException(
                "Getting all partition paths with file system listing sequentially "
    ...
    protected Map<Pair<String, StoragePath>, List<StoragePathInfo>> listPartitions(
        ...
        for (Pair<String, StoragePath> partitionPair : partitionPathList) {
            ...
            try {
                pathInfoMap.put(
                        partitionPair, metaClient.getStorage().listDirectEntries(absolutePartitionPath));
            } catch (IOException e) {
    ...
    private List<StoragePathInfo> getAllFilesInPartition(String relativePartitionPath)
        ...
        StoragePath partitionPath =
                FSUtils.constructAbsolutePath(metaClient.getBasePath(), relativePartitionPath);
        long beginLsTs = System.currentTimeMillis();
        List<StoragePathInfo> pathInfoList = listPartition(partitionPath);
        long endLsTs = System.currentTimeMillis();
        LOG.debug(
                "#files found in partition ("
    ...
    protected void ensurePartitionLoadedCorrectly(String partition) {
        ...
        addedPartitions.computeIfAbsent(
                partition,
                (partitionPathStr) -> {
                    long beginTs = System.currentTimeMillis();
                    if (!isPartitionAvailableInStore(partitionPathStr)) {
                        ...
                        try {
                            ...
                            throw new HoodieIOException(
                                    "Failed to list base files in partition " + partitionPathStr, e);
                        }
                    } else {
    ...
    /**
      ...
      */
    private Stream<HoodieBaseFile> convertFileStatusesToBaseFiles(
            List<StoragePathInfo> pathInfoList) {
        Predicate<StoragePathInfo> roFilePredicate =
                pathInfo -> {
    ...
    private Stream<HoodieLogFile> convertFileStatusesToLogFiles(List<StoragePathInfo> pathInfoList) {
        Predicate<StoragePathInfo> rtFilePredicate =
                pathInfo -> {
    ...
    /**
      * With async compaction, it is possible to see partial/complete base-files due to
      * inflight-compactions, Ignore those base-files.
      *
    ...
    protected boolean isBaseFileDueToPendingCompaction(
        ...
        return (compactionWithInstantTime.isPresent())
                && (null != compactionWithInstantTime.get().getKey())
                && baseFile.getCommitTime().equals(compactionWithInstantTime.get().getKey());
    }

    /**
      * With async clustering, it is possible to see partial/complete base-files due to
      * inflight-clustering, Ignore those base-files.
      *
      * @param baseFile base File
      */
    ...
    /**
      ...
      */
    private Stream<FileSlice> filterUncommittedFiles(
            FileSlice fileSlice, boolean includeEmptyFileSlice) {
        ...
        if ((fileSlice.getBaseFile().isPresent() && !committedBaseFile.isPresent())
            ...
            FileSlice transformed =
                    new FileSlice(
                            fileSlice.getPartitionPath(), fileSlice.getBaseInstantTime(), fileSlice.getFileId());
    ...
    protected HoodieFileGroup addBootstrapBaseFileIfPresent(
        ...
        if (hasBootstrapBaseFile) {
            ...
            newFileGroup
                    .getAllFileSlices()
                    .filter(fs -> fs.getBaseInstantTime().equals(METADATA_BOOTSTRAP_INSTANT_TS))
                    .forEach(
                            fs ->
                                    fs.setBaseFile(
    ...
    protected FileSlice addBootstrapBaseFileIfPresent(
            FileSlice fileSlice,
            Function<HoodieFileGroupId, Option<BootstrapBaseFileMapping>> bootstrapBaseFileMappingFunc) {
            ...
            copy.getBaseFile()
                    .ifPresent(
                            dataFile -> {
                                ...
                                bootstrapBaseFileMappingFunc
                                        .apply(copy.getFileGroupId())
                                        .ifPresent(e -> dataFile.setBootstrapBaseFile(e.getBootstrapBaseFile()));
                            });
            return copy;
        }
        return fileSlice;
    ...
    protected HoodieBaseFile addBootstrapBaseFileIfPresent(
            HoodieFileGroupId fileGroupId, HoodieBaseFile baseFile) {
    ...
    protected HoodieBaseFile addBootstrapBaseFileIfPresent(
        ...
        if (baseFile.getCommitTime().equals(METADATA_BOOTSTRAP_INSTANT_TS)) {
            ...
            bootstrapBaseFileMappingFunc
                    .apply(fileGroupId)
                    .ifPresent(e -> copy.setBootstrapBaseFile(e.getBootstrapBaseFile()));
            return copy;
        }
        return baseFile;
    }
                    ...
                    .map(
                            name ->
                                    name.isEmpty()
                                            ...
                                            : new StoragePath(metaClient.getBasePath(), name))
                    .collect(Collectors.toList());
        } finally {
            readLock.unlock();
        }
    ...
    @Override
    public final Stream<HoodieBaseFile> getLatestBaseFilesBeforeOrOn(
            String partitionStr, String maxCommitTime) {
        try {
            readLock.lock();
            String partitionPath = formatPartitionKey(partitionStr);
            ensurePartitionLoadedCorrectly(partitionPath);
    ...
    @Override
    public final Map<String, Stream<HoodieBaseFile>> getAllLatestBaseFilesBeforeOrOn(
            String maxCommitTime) {
        try {
            readLock.lock();
            List<String> formattedPartitionList = ensureAllPartitionsLoadedCorrectly();
            return formattedPartitionList.stream()
                    .collect(
                            Collectors.toMap(
                                    Function.identity(),
                                    partitionPath ->
    ...
    private Stream<HoodieBaseFile> getLatestBaseFilesBeforeOrOnFromCache(
            String partitionPath, String maxCommitTime) {
        return fetchAllStoredFileGroups(partitionPath)
                .filter(
                        fileGroup -> !isFileGroupReplacedBeforeOrOn(fileGroup.getFileGroupId(), maxCommitTime))
                .map(
                        fileGroup ->
    ...
    @Override
    public final Option<HoodieBaseFile> getBaseFileOn(
            String partitionStr, String instantTime, String fileId) {
        try {
            readLock.lock();
            String partitionPath = formatPartitionKey(partitionStr);
            ensurePartitionLoadedCorrectly(partitionPath);
            if (isFileGroupReplacedBeforeOrOn(
                ...
                return fetchHoodieFileGroup(partitionPath, fileId)
                                ...
                                fileGroup ->
                                        fileGroup
                                                ...
                                                .filter(
                                                        baseFile ->
                                                                HoodieTimeline.compareTimestamps(
                                                                        baseFile.getCommitTime(), HoodieTimeline.EQUALS, instantTime))
                                                .filter(
                                                        df ->
                                                                !isBaseFileDueToPendingCompaction(partitionPath, df)
                                                                        && !isBaseFileDueToPendingClustering(df))
                        ...
                        .map(
                                df ->
                                        addBootstrapBaseFileIfPresent(
                                                new HoodieFileGroupId(partitionPath, fileId), df));
            }
        } finally {
            readLock.unlock();
        }
                                                            ...
                                                            .filter(
                                                                    baseFile ->
                                                                            commitsToReturn.contains(baseFile.getCommitTime())
                                                                                    && !isBaseFileDueToPendingCompaction(
                                                                                            fileGroup.getPartitionPath(), baseFile)
    ...
    public final Stream<HoodieBaseFile> getAllBaseFiles(String partitionStr) {
        try {
            ...
            return fetchAllBaseFiles(partitionPath)
                    ...
                    .filter(
                            df ->
                                    visibleCommitsAndCompactionTimeline.containsOrBeforeTimelineStarts(
                                            df.getCommitTime()))
                    .filter(
                            df ->
                                    !isBaseFileDueToPendingCompaction(partitionPath, df)
        ...
        if (isPartitionAvailableInStore(partition)) {
            ...
            try {
                Stream<FileSlice> fileSliceStream =
                        buildFileGroups(
                                        partition,
                                        getAllFilesInPartition(partition),
                                ...
                                .flatMap(slice -> this.filterUncommittedFiles(slice, true));
                if (bootstrapIndex.useIndex()) {
                    final Map<HoodieFileGroupId, BootstrapBaseFileMapping> bootstrapBaseFileMappings =
                            getBootstrapBaseFileMappings(partition);
                    if (!bootstrapBaseFileMappings.isEmpty()) {
                        return fileSliceStream.map(
                                fileSlice ->
                                        addBootstrapBaseFileIfPresent(
                                                fileSlice,
                                                fileGroupId ->
        ...
        try {
            ...
            if (isFileGroupReplaced(partitionPath, fileId)) {
                ...
                return Option.ofNullable(
                        filterUncommittedFiles(fs.get(), true)
                                .map(this::addBootstrapBaseFileIfPresent)
                                .findFirst()
                                .orElse(null));
            }
            ...
            return fetchAllStoredFileGroups(partitionPath)
                    ...
                    .map(
                            fileGroup -> {
                                ...
                                // if the file-group is under compaction, pick the latest before
    ...
    @Override
    public final Stream<FileSlice> getLatestFileSlicesBeforeOrOn(
            String partitionStr, String maxCommitTime, boolean includeFileSlicesInPendingCompaction) {
        try {
            readLock.lock();
            String partitionPath = formatPartitionKey(partitionStr);
            ensurePartitionLoadedCorrectly(partitionPath);
            Stream<Stream<FileSlice>> allFileSliceStream =
                    fetchAllStoredFileGroups(partitionPath)
                            .filter(
                                    slice -> !isFileGroupReplacedBeforeOrOn(slice.getFileGroupId(), maxCommitTime))
            ...
            if (includeFileSlicesInPendingCompaction) {
                ...
                return allFileSliceStream
                        .map(
                                sliceStream ->
                                        getLatestFileSliceFilteringUncommittedFiles(
                                                sliceStream
                                                        .filter(
                                                                slice ->
    ...
    /**
      ...
      *
      * <p>Note: Checks from the latest file slice first to improve the efficiency. There is no need to
      * check every file slice, the uncommitted files only exist in the latest file slice basically.
      */
    private Option<FileSlice> getLatestFileSliceFilteringUncommittedFiles(
            Stream<FileSlice> fileSlices) {
        return Option.fromJavaOptional(
                fileSlices.flatMap(fileSlice -> filterUncommittedFiles(fileSlice, false)).findFirst());
    }

    @Override
    public final Map<String, Stream<FileSlice>> getAllLatestFileSlicesBeforeOrOn(
            String maxCommitTime) {
        try {
            readLock.lock();
            List<String> formattedPartitionList = ensureAllPartitionsLoadedCorrectly();
                                                    ...
                                                    .filter(
                                                            slice ->
                                                                    !isFileGroupReplacedBeforeOrOn(
                                                                            slice.getFileGroupId(), maxCommitTime))
                                                    .map(fg -> fg.getAllFileSlicesBeforeOn(maxCommitTime))
                                                    .map(this::getLatestFileSliceFilteringUncommittedFiles)
                                                    .filter(Option::isPresent)
                                                    .map(Option::get)
    ...
    @Override
    public final Stream<FileSlice> getLatestMergedFileSlicesBeforeOrOn(
                    ...
                    .map(
                            fileGroup -> {
                                ...
                                if (fileSlice.isPresent()) {
                                    fileSlice =
                                            Option.of(
                                                    fetchMergedFileSlice(fileGroup, filterUncommittedLogs(fileSlice.get())));
                                }
                                return fileSlice;
                            })
                    .filter(Option::isPresent)
    ...
    /**
      * Stream all "merged" file-slices before on an instant time for a MERGE_ON_READ table with index
      * that can index log files(which means it writes pure logs first).
      *
      * <p>In streaming read scenario, in order for better reading efficiency, the user can choose to
      * skip the base files that are produced by compaction. That is to say, we allow the users to
      * consumer only from these partitioned log files, these log files keep the record sequence just
      * like the normal message queue.
      *
      ...
      */
    public final Stream<FileSlice> getAllLogsMergedFileSliceBeforeOrOn(
            String partitionStr, String maxInstantTime) {
        try {
            readLock.lock();
            String partition = formatPartitionKey(partitionStr);
    ...
    /**
      * Ensure there is consistency in handling trailing slash in partition-path. Always trim it which
      * is what is done in other places.
      */
    private String formatPartitionKey(String partitionStr) {
        return partitionStr.endsWith("/")
                ? partitionStr.substring(0, partitionStr.length() - 1)
                : partitionStr;
    }
    ...
    private Map<HoodieFileGroupId, BootstrapBaseFileMapping> getBootstrapBaseFileMappings(
            String partition) {
        try (BootstrapIndex.IndexReader reader = bootstrapIndex.createReader()) {
            LOG.info("Bootstrap Index available for partition " + partition);
            List<BootstrapFileMapping> sourceFileMappings =
                    ...
                    .map(
                            s ->
                                    new BootstrapBaseFileMapping(
                                            new HoodieFileGroupId(s.getPartitionPath(), s.getFileId()),
                                            s.getBootstrapFileStatus()))
                    .collect(Collectors.toMap(BootstrapBaseFileMapping::getFileGroupId, s -> s));
        }
    }
    ...
    private Stream<HoodieFileGroup> getAllFileGroupsIncludingReplaced(final String partitionStr) {
        try {
            readLock.lock();
            // Ensure there is consistency in handling trailing slash in partition-path. Always trim
            // it which is what is done
            // in other places.
            String partition = formatPartitionKey(partitionStr);
            ensurePartitionLoadedCorrectly(partition);
    ...
    @Override
    public Stream<HoodieFileGroup> getReplacedFileGroupsBeforeOrOn(
        ...
        if (hasReplacedFilesInPartition(partition)) {
            return getAllFileGroupsIncludingReplaced(partition)
                    .filter(fg -> isFileGroupReplacedBeforeOrOn(fg.getFileGroupId(), maxCommitTime));
        }
        return Stream.empty();
    }

    @Override
    public Stream<HoodieFileGroup> getReplacedFileGroupsBefore(
            String maxCommitTime, String partitionPath) {
        String partition = formatPartitionKey(partitionPath);
        if (hasReplacedFilesInPartition(partition)) {
            return getAllFileGroupsIncludingReplaced(partition)
                    .filter(fg -> isFileGroupReplacedBefore(fg.getFileGroupId(), maxCommitTime));
        }
        return Stream.empty();
    }

    @Override
    public Stream<HoodieFileGroup> getReplacedFileGroupsAfterOrOn(
            String minCommitTime, String partitionPath) {
        String partition = formatPartitionKey(partitionPath);
        if (hasReplacedFilesInPartition(partition)) {
            return getAllFileGroupsIncludingReplaced(partition)
                    .filter(fg -> isFileGroupReplacedAfterOrOn(fg.getFileGroupId(), minCommitTime));
        }
        return Stream.empty();
    }
    ...
    /**
      ...
      */
    abstract void resetPendingCompactionOperations(
            Stream<Pair<String, CompactionOperation>> operations);
    ...
    /**
      ...
      */
    abstract void addPendingCompactionOperations(
            Stream<Pair<String, CompactionOperation>> operations);
    ...
    /**
      ...
      */
    abstract void removePendingCompactionOperations(
            Stream<Pair<String, CompactionOperation>> operations);
    ...
    /**
      ...
      */
    abstract void resetPendingLogCompactionOperations(
            Stream<Pair<String, CompactionOperation>> operations);
    ...
    /**
      ...
      */
    abstract void addPendingLogCompactionOperations(
            Stream<Pair<String, CompactionOperation>> operations);
    ...
    /**
      ...
      */
    abstract void removePendingLogCompactionOperations(
            Stream<Pair<String, CompactionOperation>> operations);

    /**
      * Check if there is an outstanding clustering operation (requested/inflight) scheduled for this
      * file.
      *
      * @param fgId File-Group Id
    ...
    /**
      * Get pending clustering instant time for specified file group. Return None if file group is not
      * in pending clustering operation.
      */
    protected abstract Option<HoodieInstant> getPendingClusteringInstant(
            final HoodieFileGroupId fileGroupId);
    ...
    abstract void resetFileGroupsInPendingClustering(
            Map<HoodieFileGroupId, HoodieInstant> fgIdToInstantMap);

    /** Add metadata for file groups in pending clustering operations to the view. */
    abstract void addFileGroupsInPendingClustering(
            Stream<Pair<HoodieFileGroupId, HoodieInstant>> fileGroups);

    /** Remove metadata for file groups in pending clustering operations from the view. */
    abstract void removeFileGroupsInPendingClustering(
            Stream<Pair<HoodieFileGroupId, HoodieInstant>> fileGroups);
    ...
    /**
      ...
      */
    abstract void resetBootstrapBaseFileMapping(
            Stream<BootstrapBaseFileMapping> bootstrapBaseFileStream);
    ...
    /**
      ...
      */
    abstract void addBootstrapBaseFileMapping(
            Stream<BootstrapBaseFileMapping> bootstrapBaseFileStream);
    ...
    /**
      ...
      */
    abstract void removeBootstrapBaseFileMapping(
            Stream<BootstrapBaseFileMapping> bootstrapBaseFileStream);
    ...
    /**
      ...
      * @param fileGroupId File-Group Id
      */
    protected abstract Option<BootstrapBaseFileMapping> getBootstrapBaseFile(
            HoodieFileGroupId fileGroupId);
    ...
    protected abstract void resetReplacedFileGroups(
            final Map<HoodieFileGroupId, HoodieInstant> replacedFileGroups);

    /** Track instant time for new file groups replaced. */
    protected abstract void addReplacedFileGroups(
            final Map<HoodieFileGroupId, HoodieInstant> replacedFileGroups);
    ...
    Option<HoodieFileGroup> fetchHoodieFileGroup(String partitionPath, String fileId) {
        return Option.fromJavaOptional(
                fetchAllStoredFileGroups(partitionPath)
                        .filter(fileGroup -> fileGroup.getFileGroupId().getFileId().equals(fileId))
                        .findFirst());
    }
    ...
    private static FileSlice mergeCompactionPendingFileSlices(
            FileSlice lastSlice, FileSlice penultimateSlice) {
        FileSlice merged =
                new FileSlice(
                        penultimateSlice.getPartitionPath(),
                        penultimateSlice.getBaseInstantTime(),
    ...
    /**
      * If the file-slice is because of pending compaction instant, this method merges the file-slice
      * with the one before the compaction instant time.
      *
    ...
    /**
      ...
      */
    private Option<FileSlice> fetchAllLogsMergedFileSlice(
            HoodieFileGroup fileGroup, String maxInstantTime) {
    ...
    protected Option<HoodieBaseFile> fetchLatestBaseFile(String partitionPath, String fileId) {
        return Option.fromJavaOptional(
                fetchLatestBaseFiles(partitionPath)
                        .filter(fs -> fs.getFileId().equals(fileId))
                        .findFirst());
    }
    ...
    public Option<FileSlice> fetchLatestFileSlice(String partitionPath, String fileId) {
        return Option.fromJavaOptional(
                fetchLatestFileSlices(partitionPath)
                        .filter(fs -> fs.getFileId().equals(fileId))
                        .findFirst());
    }
    ...
    private boolean isFileGroupReplacedBeforeAny(
            HoodieFileGroupId fileGroupId, List<String> instants) {
        return isFileGroupReplacedBeforeOrOn(
                fileGroupId, instants.stream().max(Comparator.naturalOrder()).get());
    }
    ...
    private boolean isFileGroupReplacedBefore(HoodieFileGroupId fileGroupId, String instant) {
        ...
        return HoodieTimeline.compareTimestamps(
                instant, GREATER_THAN, hoodieInstantOption.get().getTimestamp());
    }
    ...
    private boolean isFileGroupReplacedBeforeOrOn(HoodieFileGroupId fileGroupId, String instant) {
        ...
        return HoodieTimeline.compareTimestamps(
                instant, GREATER_THAN_OR_EQUALS, hoodieInstantOption.get().getTimestamp());
    }
    ...
    private boolean isFileGroupReplacedAfterOrOn(HoodieFileGroupId fileGroupId, String instant) {
        ...
        return HoodieTimeline.compareTimestamps(
                instant, LESSER_THAN_OR_EQUALS, hoodieInstantOption.get().getTimestamp());
    }
    ...
    /**
      * Syncs the file system view from storage to memory. Performs complete reset of file-system view.
      * Subsequent partition view calls will load file slices against the latest timeline.
      *
      * <p>NOTE: The logic MUST BE guarded by the write lock.
      */
    @Override
