...
public abstract class AbstractTableFileSystemView implements SyncableFileSystemView, Serializable {
  ...
  private String getPartitionPathFor(HoodieBaseFile baseFile) {
    ...
    return FSUtils.getRelativePartitionPathFromLocation(metaClient.getBasePathV2(), new StoragePath(baseFile.getPath()).getParent());
  }
    ...
    synchronized (addedPartitions) {
      ...
      if (!partitionSet.isEmpty()) {
        ...
        try {
          ...
          Map<Pair<String, StoragePath>, List<StoragePathInfo>> statusesMap =
  ...
  protected Map<Pair<String, StoragePath>, List<StoragePathInfo>> listPartitions(
    ...
    Map<Pair<String, StoragePath>, List<StoragePathInfo>> fileStatusMap = new HashMap<>();
  ...
  private List<StoragePathInfo> getAllFilesInPartition(String relativePartitionPath)
    ...
    long beginLsTs = System.currentTimeMillis();
    List<StoragePathInfo> statuses = listPartition(partitionPath);
    long endLsTs = System.currentTimeMillis();
    LOG.debug(
   ...
   */
  private Stream<HoodieBaseFile> convertFileStatusesToBaseFiles(List<StoragePathInfo> statuses) {
    Predicate<StoragePathInfo> roFilePredicate = fileInfo -> {
      String pathName = fileInfo.getPath().getName();
   ...
   * @param statuses List of File-Status
   */
  private Stream<HoodieLogFile> convertFileStatusesToLogFiles(List<StoragePathInfo> statuses) {
    Predicate<StoragePathInfo> rtFilePredicate = fileInfo -> {
