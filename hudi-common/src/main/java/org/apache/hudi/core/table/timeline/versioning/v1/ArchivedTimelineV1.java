...
import static org.apache.hudi.core.table.timeline.TimelineUtils.getInputStreamOptionLegacy;
...
public class ArchivedTimelineV1 extends BaseTimelineV1 implements HoodieArchivedTimeline, HoodieInstantReader {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(org.apache.hudi.core.table.timeline.HoodieArchivedTimeline.class);
...
public Option<byte[]> getInstantDetails(HoodieInstant instant) {
  ...
  public void loadCompletedInstantDetailsInMemory() {
    loadInstants(null, null, true,
        record -> {
          ...
          return action == null || org.apache.hudi.core.table.timeline.HoodieInstant.State.COMPLETED.toString().equals(action.toString());
        });
  }
    ...
    List<HoodieInstant> loadedInstants = loadInstants(new ClosedClosedTimeRangeFilter(startTs, endTs), null, true,
        record -> {
          ...
          return record.get(ACTION_TYPE_KEY).toString().equals(HoodieTimeline.COMPACTION_ACTION)
              && (action == null || org.apache.hudi.core.table.timeline.HoodieInstant.State.INFLIGHT.toString().equals(action.toString()));
        });
  ...
  public void loadCompactionDetailsInMemory(int limit) {
    loadAndCacheInstantsWithLimit(limit, true,
        record -> {
          ...
          return record.get(ACTION_TYPE_KEY).toString().equals(HoodieTimeline.COMPACTION_ACTION)
              && (actionState == null || org.apache.hudi.core.table.timeline.HoodieInstant.State.INFLIGHT.toString().equals(actionState.toString()));
        });
  }
  ...
  public void loadCompletedInstantDetailsInMemory(String startTs, String endTs) {
    List<HoodieInstant> loadedInstants = loadInstants(new ClosedClosedTimeRangeFilter(startTs, endTs), null, true,
        record -> {
          Object actionState = record.get(ACTION_STATE);
          return actionState == null || org.apache.hudi.core.table.timeline.HoodieInstant.State.COMPLETED.toString().equals(actionState.toString());
        });
    appendLoadedInstants(loadedInstants);
  }
