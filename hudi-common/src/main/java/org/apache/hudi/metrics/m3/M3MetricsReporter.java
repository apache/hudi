...
public class M3MetricsReporter extends MetricsReporter {
    ...
    private static Map parseOptionalTags(String tagValueString) {
        ...
        if (!tagValueString.isEmpty()) {
            Arrays.stream(tagValueString.split(","))
                    .forEach(
                            (tagValuePair) -> {
                                String[] parsedTagValuePair =
                                        Arrays.stream(tagValuePair.split("="))
                                                .map((tagOrValue) -> tagOrValue.trim())
                                                .filter((tagOrValue) -> !tagOrValue.isEmpty())
                                                .toArray(String[]::new);
        ...
        synchronized (this) {
            try (Scope scope =
                    new RootScopeBuilder()
                            .reporter(
                                    new M3Reporter.Builder(
                                                    new InetSocketAddress(
                                                            metricsConfig.getM3ServerHost(), metricsConfig.getM3ServerPort()))
                                            .includeHost(true)
                                            .commonTags(tags)
                                            .build())
