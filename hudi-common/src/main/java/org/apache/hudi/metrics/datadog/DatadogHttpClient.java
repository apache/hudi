...
/**
  ...
  *
  * <p>Responsible for API endpoint routing, validating API key, and sending requests with metrics
  * payload.
  */
public class DatadogHttpClient implements Closeable {
    ...
    public DatadogHttpClient(
        ...
        this.apiKey = apiKey;
        this.seriesUrl =
                String.format(SERIES_URL_FORMAT, host.orElse(DEFAULT_HOST), apiSite.getDomain());
        this.validateUrl =
                String.format(VALIDATE_URL_FORMAT, host.orElse(DEFAULT_HOST), apiSite.getDomain());
        this.client = client;
        if (!skipValidation) {
            validateApiKey();
        }
    ...
    public DatadogHttpClient(
            ApiSite apiSite, String apiKey, boolean skipValidation, CloseableHttpClient client) {
    ...
    private void validateApiKey() {
        ValidationUtils.checkArgument(!StringUtils.isNullOrEmpty(apiKey), "API key is null or empty.");
