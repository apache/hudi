...
/**
  * Implementation of Prometheus reporter, which connects to the Http server, and get metrics from
  * that server.
  */
public class PrometheusReporter extends MetricsReporter {
    ...
    public PrometheusReporter(HoodieMetricsConfig metricsConfig, MetricRegistry registry) {
        ...
        if (StringUtils.nonEmpty(metricsConfig.getPushGatewayLabels())) {
            LABEL_PATTERN
                    .splitAsStream(metricsConfig.getPushGatewayLabels().trim())
                    .map(s -> s.split(":", 2))
                    .forEach(
                            parts -> {
        ...
        metricExports =
                new DropwizardExports(registry, new LabeledSampleBuilder(labelNames, labelValues));
    ...
    private static synchronized void startHttpServer(int serverPort) {
        ...
        if (!PORT_TO_SERVER.containsKey(serverPort)) {
            try {
                HTTPServer server =
                        new HTTPServer(
                                new InetSocketAddress(serverPort), PORT_TO_COLLECTOR_REGISTRY.get(serverPort));
    ...
    private static class LabeledSampleBuilder implements SampleBuilder {
        ...
        public Collector.MetricFamilySamples.Sample createSample(
            ...
            return defaultMetricSampleBuilder.createSample(
                    dropwizardName, nameSuffix, labelNames, labelValues, value);
        }
    }
}
