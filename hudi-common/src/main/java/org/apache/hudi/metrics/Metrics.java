...
public class Metrics {
    ...
    public Metrics(HoodieMetricsConfig metricConfig, HoodieStorage storage) {
        ...
        Option<MetricsReporter> defaultReporter =
    ...
    public static synchronized Metrics getInstance(
            HoodieMetricsConfig metricConfig, HoodieStorage storage) {
    ...
    private List<MetricsReporter> addAdditionalMetricsExporters(HoodieMetricsConfig metricConfig) {
        ...
        try (HoodieStorage newStorage =
                storage.newInstance(new StoragePath(propPathList.get(0)), storage.getConf())) {
            for (String propPath : propPathList) {
                HoodieMetricsConfig secondarySourceConfig =
                        HoodieMetricsConfig.newBuilder()
                                .fromInputStream(newStorage.open(new StoragePath(propPath)))
                                .withPath(metricConfig.getBasePath())
                                .build();
                ...
                if (reporter.isPresent()) {
                    ...
                    LOG.error(
                            String.format(
                                    "Could not create reporter using properties path %s base path %s",
