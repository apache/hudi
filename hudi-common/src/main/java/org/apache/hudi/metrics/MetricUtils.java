...
public final class MetricUtils {
    ...
    // Example metric:- with_label_metric;group:a,job:0. Here `with_label_metric` is the metric
    // name.
    ...
    private static Pair<String, String> splitToPair(String label) {
        ...
        ValidationUtils.checkArgument(
                StringUtils.nonEmpty(keyValues[0]), String.format("Key is empty for label %s", label));
                        ...
                        Collectors.toMap(
                                ...
                                (v1, v2) -> {
                                    throw new IllegalStateException(
                                            String.format("Multiple values {%s, %s} for same key", v1, v2));
                                }));
    }

    public static List<String> getLabels(String labels) {
        return Arrays.stream(labels.split(LABELS_SEPARATOR))
                .filter(StringUtils::nonEmpty)
