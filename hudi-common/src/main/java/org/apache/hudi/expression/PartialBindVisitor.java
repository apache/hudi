...
/**
  ...
  *
  * <p>e.g. `year=2023 AND day=12`, if year and day both are provided to `recordType`, then the
  * expression won't change, if day is not provided, the expression will be transformed to `year=2023
  * AND True`, which will be optimized to `year=2023`.
  */
public class PartialBindVisitor extends BindVisitor {
    ...
    /**
      * If the attribute cannot find from the schema, directly return null, visitPredicate will handle
      * it.
      */
    @Override
    ...
    /**
      * If an expression is null after accept method, which means it cannot be bounded from schema,
      * we'll directly return {@link Predicates.TrueExpression}.
      */
    @Override
    public Expression visitPredicate(Predicate predicate) {
        ...
        throw new IllegalArgumentException(
                "The expression " + predicate + " cannot be visited as predicate");
    }
}
