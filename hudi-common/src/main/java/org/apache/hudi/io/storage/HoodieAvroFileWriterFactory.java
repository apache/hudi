...
public class HoodieAvroFileWriterFactory extends HoodieFileWriterFactory {
  ...
  protected HoodieFileWriter newParquetFileWriter(
    ...
    HoodieParquetConfig<HoodieAvroWriteSupport> parquetConfig = new HoodieParquetConfig<>(writeSupport,
        ...
        (Configuration) conf.unwrap(), config.getDoubleOrDefault(HoodieStorageConfig.PARQUET_COMPRESSION_RATIO_FRACTION),
  ...
  protected HoodieFileWriter newParquetFileWriter(
    ...
    HoodieParquetConfig<HoodieAvroWriteSupport> parquetConfig = new HoodieParquetConfig<>(writeSupport,
        ...
        (Configuration) conf.unwrap(), config.getDouble(HoodieStorageConfig.PARQUET_COMPRESSION_RATIO_FRACTION),
  ...
  protected HoodieFileWriter newHFileFileWriter(
    ...
    HoodieHFileConfig hfileConfig = new HoodieHFileConfig((Configuration) conf.unwrap(),
  ...
  protected HoodieFileWriter newOrcFileWriter(
      ...
      TaskContextSupplier taskContextSupplier) throws IOException {
    BloomFilter filter = createBloomFilter(config);
    HoodieOrcConfig orcConfig = new HoodieOrcConfig((Configuration) conf.unwrap(),
  ...
  private HoodieAvroWriteSupport getHoodieAvroWriteSupport(StorageConfiguration<?> conf, Schema schema,
    ...
    return (HoodieAvroWriteSupport) ReflectionUtils.loadClass(
        ...
        new AvroSchemaConverter((Configuration) conf.unwrap()).convert(schema), schema, filter, config.getProps());
  }
}
