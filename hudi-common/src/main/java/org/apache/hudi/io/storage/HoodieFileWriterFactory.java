...
public class HoodieFileWriterFactory {
    ...
    public static <T, I, K, O> HoodieFileWriter getFileWriter(
        ...
        return factory.getFileWriterByFormat(
                extension, instantTime, path, config, schema, taskContextSupplier);
    }
    ...
    protected <T, I, K, O> HoodieFileWriter getFileWriterByFormat(
            HoodieFileFormat format, OutputStream outputStream, HoodieConfig config, Schema schema)
            throws IOException {
        switch (format) {
            case PARQUET:
    ...
    public static boolean enableBloomFilter(boolean populateMetaFields, HoodieConfig config) {
        return populateMetaFields
                && (config.getBooleanOrDefault(HoodieStorageConfig.PARQUET_WITH_BLOOM_FILTER_ENABLED)
                        // HoodieIndexConfig is located in the package hudi-client-common, and the
                        // package hudi-client-common depends on the package hudi-common,
