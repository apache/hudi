...
/**
  * Base class to get {@link HoodieFileReaderFactory}, {@link HoodieFileWriterFactory}, and {@link
  * FileFormatUtils}
  */
...
public abstract class HoodieIOFactory {
    ...
    public static HoodieIOFactory getIOFactory(HoodieStorage storage) {
        ...
        try {
            return (HoodieIOFactory)
                    ReflectionUtils.loadClass(ioFactoryClass, new Class<?>[] {HoodieStorage.class}, storage);
        } catch (Exception e) {
    ...
    public abstract HoodieFileReaderFactory getReaderFactory(
    ...
    /**
      ...
      */
    @PublicAPIMethod(maturity = ApiMaturityLevel.EVOLVING)
    public abstract HoodieFileWriterFactory getWriterFactory(
            HoodieRecord.HoodieRecordType recordType);
    ...
    public final FileFormatUtils getFileFormatUtils(StoragePath path) {
        ...
        throw new UnsupportedOperationException(
                "The format for file " + path + " is not supported yet.");
    }
}
