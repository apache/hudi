...
public abstract class HoodieAvroHFileReaderImplBase extends HoodieAvroFileReader
    ...
    /**
      ...
      *
      * <p>Reads all the records with given schema
      */
    public static List<IndexedRecord> readAllRecords(HoodieAvroFileReader reader) throws IOException {
        Schema schema = reader.getSchema();
        return toStream(reader.getIndexedRecordIterator(schema)).collect(Collectors.toList());
    }

    /**
      * NOTE: THIS SHOULD ONLY BE USED FOR TESTING, RECORDS ARE MATERIALIZED EAGERLY
      *
      * <p>Reads all the records with given schema and filtering keys.
      */
    public static List<IndexedRecord> readRecords(
            HoodieAvroHFileReaderImplBase reader, List<String> keys) throws IOException {
    ...
    /**
      ...
      */
    public static List<IndexedRecord> readRecords(
            HoodieAvroHFileReaderImplBase reader, List<String> keys, Schema schema) throws IOException {
        Collections.sort(keys);
        return toStream(reader.getIndexedRecordsByKeysIterator(keys, schema))
                .collect(Collectors.toList());
    }

    public abstract ClosableIterator<IndexedRecord> getIndexedRecordsByKeysIterator(
    ...
    protected static GenericRecord deserialize(
