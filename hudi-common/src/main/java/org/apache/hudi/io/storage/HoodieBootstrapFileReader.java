...
public abstract class HoodieBootstrapFileReader<T> implements HoodieFileReader<T> {
    ...
    public HoodieBootstrapFileReader(
            HoodieFileReader<T> skeletonFileReader,
    ...
    @Override
    public ClosableIterator<HoodieRecord<T>> getRecordIterator(
        ...
        ClosableIterator<HoodieRecord<T>> dataFileIterator =
                dataFileReader.getRecordIterator(
                        HoodieAvroUtils.removeMetadataFields(readerSchema), requestedSchema);
        return new HoodieBootstrapRecordIterator<T>(
                skeletonIterator, dataFileIterator, readerSchema, partitionFields, partitionValues) {
            @Override
    ...
    public ClosableIterator<HoodieRecord<T>> getRecordIterator(Schema schema) throws IOException {
        ...
        return new HoodieBootstrapRecordIterator<T>(
                skeletonIterator, dataFileIterator, schema, partitionFields, partitionValues) {
            @Override
            protected void setPartitionPathField(int position, Object fieldValue, T row) {
                setPartitionField(position, fieldValue, row);
            }
    ...
    public ClosableIterator<String> getRecordKeyIterator() throws IOException {
        ...
        ClosableIterator<HoodieRecord<T>> skeletonIterator =
                skeletonFileReader.getRecordIterator(schema, schema);
