...
/**
  ...
  *
  * <ul>
  *   <li>Adding record keys to the Bloom Filter
  *   <li>Keeping track of min/max record key (w/in single file)
  * </ul>
  *
...
public abstract class HoodieBloomFilterWriteSupport<T extends Comparable<T>> {
    ...
    /**
      * Since Bloom Filter ingests record-keys represented as UTF8 encoded byte string, this method
      * have to be implemented for converting the original record key into one
      */
    protected abstract byte[] getUTF8Bytes(T key);

    /**
      * This method allows to dereference the key object (t/h cloning, for ex) that might be pointing
      * at a shared mutable buffer, to make sure that we're not keeping references to mutable objects
      */
