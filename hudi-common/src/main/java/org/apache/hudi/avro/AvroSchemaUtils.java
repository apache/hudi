...
public class AvroSchemaUtils {
    ...
    public static boolean isSchemaCompatible(
            Schema prevSchema, Schema newSchema, boolean allowProjection) {
        return isSchemaCompatible(prevSchema, newSchema, true, allowProjection);
    }

    /**
      * Establishes whether {@code newSchema} is compatible w/ {@code prevSchema}, as defined by Avro's
      * {@link AvroSchemaCompatibility}. From avro's compatibility standpoint, prevSchema is writer
      * schema and new schema is reader schema. {@code newSchema} is considered compatible to {@code
      * prevSchema}, iff data written using {@code prevSchema} could be read by {@code newSchema}
      *
      ...
      */
    public static boolean isSchemaCompatible(
            Schema prevSchema, Schema newSchema, boolean checkNaming, boolean allowProjection) {
        ...
        // In case schema projection is not allowed, new schema has to have all the same fields as
        // the
        // old schema
        if (!allowProjection) {
    ...
    /**
      * Check that each top level field in the prevSchema can be populated in the newSchema except
      * specified columns
      *
      * @param prevSchema prev schema.
      * @param newSchema new schema
      ...
      */
    private static List<Schema.Field> findMissingFields(
            Schema prevSchema, Schema newSchema, Set<String> exceptCols) {
    ...
    /**
      ...
      *
      * <p>NOTE: PLEASE READ CAREFULLY BEFORE CHANGING This method should not change for compatibility
      * reasons as older versions of Avro might be comparing fully-qualified names rather than just the
      * record names
      */
    ...
    /**
      ...
      * Only difference of this method from {@link #isStrictProjectionOf(Schema, Schema)} is the fact
      * that it allows some legitimate type promotions (like {@code int -> long}, {@code decimal(3, 2)
      * -> decimal(5, 2)}, etc.) that allows projection to have a "wider" atomic type (whereas strict
      * projection requires atomic type to be identical)
      */
    public static boolean isCompatibleProjectionOf(Schema sourceSchema, Schema targetSchema) {
        return isProjectionOfInternal(
                sourceSchema, targetSchema, AvroSchemaUtils::isAtomicSchemasCompatible);
    }
    ...
    /**
      ...
      *
      * <p>Schema B is considered a strict projection of schema A iff
      *
      * <ol>
      *   <li>Schemas A and B are equal, or
      *   <li>Schemas A and B are array schemas and element-type of B is a strict projection of the
      *       element-type of A, or
      *   <li>Schemas A and B are map schemas and value-type of B is a strict projection of the
      *       value-type of A, or
      *   <li>Schemas A and B are union schemas (of the same size) and every element-type of B is a
      *       strict projection of the corresponding element-type of A, or
      *   <li>Schemas A and B are record schemas and every field of the record B has corresponding
      *       counterpart (w/ the same name) in the schema A, such that the schema of the field of the
      *       schema B is also a strict projection of the A field's schema
      * </ol>
      */
    ...
    private static boolean isProjectionOfInternal(
            Schema sourceSchema,
            Schema targetSchema,
        ...
        if (sourceSchema.getType() == targetSchema.getType()) {
            if (sourceSchema.getType() == Schema.Type.RECORD) {
                for (Schema.Field targetField : targetSchema.getFields()) {
                    ...
                    if (sourceField == null
                            || !isProjectionOfInternal(
                                    sourceField.schema(), targetField.schema(), atomicTypeEqualityPredicate)) {
                        return false;
                    }
                }
                return true;
                ...
                return isProjectionOfInternal(
                        sourceSchema.getElementType(),
                        targetSchema.getElementType(),
                        atomicTypeEqualityPredicate);
            } else if (sourceSchema.getType() == Schema.Type.MAP) {
                return isProjectionOfInternal(
                        sourceSchema.getValueType(), targetSchema.getValueType(), atomicTypeEqualityPredicate);
                ...
                for (int i = 0; i < sourceNestedSchemas.size(); ++i) {
                    if (!isProjectionOfInternal(
                            sourceNestedSchemas.get(i),
    ...
    private static Option<Schema.Field> findNestedField(
            Schema schema, String[] fieldParts, int index) {
        ...
        if (index == fieldParts.length - 1) {
            return Option.of(
                    new Schema.Field(
                            foundField.name(), foundField.schema(), foundField.doc(), foundField.defaultVal()));
        }
        ...
        Schema newSchema =
                createNewSchemaFromFieldsWithReference(
                        foundSchema, Collections.singletonList(nestedPart.get()));
        return Option.of(
                new Schema.Field(
                        foundField.name(),
                        isUnion ? createNullableSchema(newSchema) : newSchema,
                        foundField.doc(),
                        foundField.defaultVal()));
    }

    public static Schema appendFieldsToSchemaDedupNested(
            Schema schema, List<Schema.Field> newFields) {
    ...
    public static Schema mergeSchemas(Schema a, Schema b) {
        ...
        for (Schema.Field f : a.getFields()) {
            ...
            fields.add(
                    new Schema.Field(
                            f.name(),
                            foundField == null ? f.schema() : mergeSchemas(f.schema(), foundField.schema()),
                            f.doc(),
                            f.defaultVal()));
        }
    ...
    /**
      ...
      *
      * <p>NOTE: No deduplication is made, this method simply appends fields at the end of the list of
      * the source schema as is
      */
    ...
    private static Schema appendFieldsToSchemaBase(
            Schema schema, List<Schema.Field> newFields, boolean dedupNested) {
            ...
            for (Schema.Field f : newFields) {
                ...
                if (foundField != null) {
                    fields.set(
                            ...
                            new Schema.Field(
                                    foundField.name(),
                                    mergeSchemas(foundField.schema(), f.schema()),
                                    foundField.doc(),
                                    foundField.defaultVal()));
                } else {
                    fields.add(f);
    ...
    /**
      ...
      */
    public static Schema createNewSchemaFromFieldsWithReference(
    ...
    /**
      * Passed in {@code Union} schema and will try to resolve the field with the {@code
      * fieldSchemaFullName} w/in the union returning its corresponding schema
      *
    ...
    public static Schema resolveUnionSchema(Schema schema, String fieldSchemaFullName) {
        ...
        if (nonNullType == null) {
            throw new HoodieAvroSchemaException(
                    String.format(
                            "Unsupported Avro UNION type %s: Only UNION of a null type and a non-null type is supported",
                            schema));
        }
    ...
    /**
      * Returns true in case provided {@link Schema} is nullable (ie accepting null values), returns
      * false otherwise
      */
    public static boolean isNullable(Schema schema) {
        ...
        List<Schema> innerTypes = schema.getTypes();
        return innerTypes.size() > 1
                && innerTypes.stream().anyMatch(it -> it.getType() == Schema.Type.NULL);
    }

    /**
      * Resolves typical Avro's nullable schema definition: {@code Union(Schema.Type.NULL,
      * <NonNullType>)}, decomposing union and returning the target non-null type
      */
    ...
    /**
      * Creates schema following Avro's typical nullable schema definition: {@code
      * Union(Schema.Type.NULL, <NonNullType>)}, wrapping around provided target non-null type
      */
    ...
    /**
      * Checks whether writer schema is compatible with table schema considering {@code
      * AVRO_SCHEMA_VALIDATE_ENABLE} and {@code SCHEMA_ALLOW_AUTO_EVOLUTION_COLUMN_DROP} options. To
      * avoid collision of {@code SCHEMA_ALLOW_AUTO_EVOLUTION_COLUMN_DROP} and {@code
      * DROP_PARTITION_COLUMNS} partition column names should be passed as {@code
      * dropPartitionColNames}. Passed empty set means {@code DROP_PARTITION_COLUMNS} is disabled.
      *
      ...
      * @param dropPartitionColNames partition column names to being excluded from column dropping
      *     check
    ...
    public static void checkSchemaCompatible(
        ...
        if (!allowProjection) {
            List<Schema.Field> missingFields =
                    findMissingFields(tableSchema, writerSchema, dropPartitionColNames);
            if (!missingFields.isEmpty()) {
                throw new MissingSchemaFieldException(
                        missingFields.stream().map(Schema.Field::name).collect(Collectors.toList()),
                        writerSchema,
                        tableSchema);
            }
        }
    ...
    public static void checkValidEvolution(Schema incomingSchema, Schema tableSchema) {
        ...
        // not really needed for `hoodie.write.set.null.for.missing.columns` but good to check
        // anyway
    ...
    /** Find all fields in the latest table schema that are not in the incoming schema. */
    private static void findAnyMissingFields(
    ...
    /**
      ...
      */
    private static void findAnyMissingFieldsRec(
            Schema incomingSchema,
            Schema latestTableSchema,
            Deque<String> visited,
            List<String> missingFields,
            ...
            if (incomingSchema.getType() == Schema.Type.RECORD) {
                ...
                if (incomingNestedSchemas.size() > 2) {
                    throw new InvalidUnionTypeException(
                            createSchemaErrorString(
                                    String.format(
                                            "Union for incoming batch field '%s' should not have more than 2 types but has %d",
                ...
                for (int i = 0; i < incomingNestedSchemas.size(); ++i) {
                    findAnyMissingFieldsRec(
                            incomingNestedSchemas.get(i),
                            latestTableNestedSchemas.get(i),
                            visited,
                            missingFields,
                            fullIncomingSchema,
                            fullTableSchema);
    ...
    public static String createSchemaErrorString(
            String errorMessage, Schema writerSchema, Schema tableSchema) {
