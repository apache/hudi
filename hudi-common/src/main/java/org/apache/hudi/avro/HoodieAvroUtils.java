...
public class HoodieAvroUtils {
    ...
    /**
      ...
      *
      * <p>In Avro 1.10 generated builders rely on {@code SpecificData.getForSchema} invocation that in
      * turn does use reflection to load the code-gen'd class corresponding to the Avro record model.
      * This has serious adverse effects in terms of performance when gets executed on the hot-path
      * (both, in terms of runtime and efficiency).
      *
      * <p>To work this around instead of using default code-gen'd builder invoking {@code
      * SpecificData.getForSchema}, we instead rely on overloaded ctor accepting another instance of
      * the builder: {@code Builder(Builder)}, which bypasses such invocation. Following corresponding
      * builder's stubs are statically initialized to be used exactly for that purpose.
      *
      * <p>You can find more details in HUDI-3834.
      */
    ...
    private static final Lazy<BooleanWrapper.Builder> BOOLEAN_WRAPPER_BUILDER_STUB =
            Lazy.lazily(BooleanWrapper::newBuilder);
    private static final Lazy<TimestampMicrosWrapper.Builder> TIMESTAMP_MICROS_WRAPPER_BUILDER_STUB =
            Lazy.lazily(TimestampMicrosWrapper::newBuilder);
    private static final Lazy<DecimalWrapper.Builder> DECIMAL_WRAPPER_BUILDER_STUB =
            Lazy.lazily(DecimalWrapper::newBuilder);
    ...
    // Export for test
    public static final Conversions.DecimalConversion DECIMAL_CONVERSION =
            new Conversions.DecimalConversion();

    // As per https://avro.apache.org/docs/current/spec.html#names
    private static final Pattern INVALID_AVRO_CHARS_IN_NAMES_PATTERN =
            Pattern.compile("[^A-Za-z0-9_]");
    private static final Pattern INVALID_AVRO_FIRST_CHAR_IN_NAMES_PATTERN =
    ...
    public static Option<byte[]> recordToBytes(HoodieRecord record, Schema schema)
            throws IOException {
        return Option.of(
                HoodieAvroUtils.indexedRecordToBytes(
    ...
    public static <T extends IndexedRecord> byte[] indexedRecordToBytes(T record) {
        GenericDatumWriter<T> writer =
                new GenericDatumWriter<>(record.getSchema(), ConvertingGenericData.INSTANCE);
    ...
    /**
      * Convert a given avro record to a JSON string. If the record contents are invalid, return the
      * record.toString(). Use this method over {@link HoodieAvroUtils#avroToJsonString} when simply
      * trying to print the record contents without any guarantees around their correctness.
      *
      * @param record The GenericRecord to convert
    ...
    private static ByteArrayOutputStream avroToJsonHelper(GenericRecord record, boolean pretty)
            throws IOException {
    ...
    /** Convert serialized bytes back into avro record. */
    public static GenericRecord bytesToAvro(
            byte[] bytes, int offset, int length, Schema writerSchema, Schema readerSchema)
            throws IOException {
        BinaryDecoder decoder =
                DecoderFactory.get().binaryDecoder(bytes, offset, length, BINARY_DECODER.get());
    ...
    public static Schema removeFields(Schema schema, Set<String> fieldsToRemove) {
        List<Schema.Field> filteredFields =
                                ...
                                field -> {
                                    if (Objects.equals(field.name(), fieldName)) {
                                        return new Schema.Field(
                                                field.name(),
                                                AvroSchemaUtils.resolveNullableSchema(field.schema()),
                                                field.doc(),
                                                fieldDefaultValue);
                                    } else {
                                        return new Schema.Field(
    ...
    public static GenericRecord addHoodieKeyToRecord(
            GenericRecord record, String recordKey, String partitionPath, String fileName) {
    ...
    public static GenericRecord addOperationToRecord(
            GenericRecord record, HoodieOperation operation) {
    ...
    public static GenericRecord addCommitMetadataToRecord(
            GenericRecord record, String instantTime, String commitSeqno) {
    ...
    public static GenericRecord stitchRecords(
            GenericRecord left, GenericRecord right, Schema stitchedSchema) {
    ...
    /**
      * Given an Avro record with a given schema, rewrites it into the new schema while setting fields
      * only from the new schema.
      *
      * <p>NOTE: This method is rewriting every record's field that is record itself recursively. It's
      ...
      *
      * <p>NOTE: Here, the assumption is that you cannot go from an evolved schema (schema with (N)
      * fields) to an older schema (schema with (N-1) fields). All fields present in the older record
      * schema MUST be present in the new schema and the default/existing values are carried over.
      *
      * <p>This particular method does the following:
      *
      * <ol>
      *   <li>Create a new empty GenericRecord with the new schema.
      *   <li>For GenericRecord, copy over the data from the old schema to the new schema or set
      *       default values for all fields of this transformed schema
      *   <li>For SpecificRecord, hoodie_metadata_fields have a special treatment (see below)
      * </ol>
      *
      * <p>For SpecificRecord we ignore Hudi Metadata fields, because for code generated avro classes
      * (HoodieMetadataRecord), the avro record is a SpecificBaseRecord type instead of a
      * GenericRecord. SpecificBaseRecord throws null pointer exception for record.get(name) if name is
      * not present in the schema of the record (which happens when converting a SpecificBaseRecord
      * without hoodie_metadata_fields to a new record with it). In this case, we do NOT set the
      * defaults for the hoodie_metadata_fields explicitly, instead, the new record assumes the default
      * defined in the avro schema itself. TODO: See if we can always pass GenericRecord instead of
      * SpecificBaseRecord in some cases.
      */
    ...
    public static GenericRecord rewriteRecordWithMetadata(
            GenericRecord genericRecord, Schema newSchema, String fileName) {
        GenericRecord newRecord = new GenericData.Record(newSchema);
        for (Schema.Field f : newSchema.getFields()) {
    ...
    // TODO Unify the logical of rewriteRecordWithMetadata and rewriteEvolutionRecordWithMetadata,
    // and delete this function.
    public static GenericRecord rewriteEvolutionRecordWithMetadata(
            GenericRecord genericRecord, Schema newSchema, String fileName) {
        GenericRecord newRecord =
                HoodieAvroUtils.rewriteRecordWithNewSchema(genericRecord, newSchema, new HashMap<>());
        // do not preserve FILENAME_METADATA_FIELD
        newRecord.put(HoodieRecord.FILENAME_META_FIELD_ORD, fileName);
        return newRecord;
    }
    ...
    /**
      ...
      *
      * <p>To better understand conversion rules please check {@link #rewriteRecord(GenericRecord,
      * Schema)}
      */
    ...
    /**
      * Given an Avro record and list of columns to remove, this method removes the list of columns
      * from the given avro record using rewriteRecord method.
      *
      * <p>To better understand how it removes please check {@link #rewriteRecord(GenericRecord,
      * Schema)}
      */
    ...
    private static void copyOldValueOrSetDefault(
            GenericRecord oldRecord, GenericRecord newRecord, Schema.Field field) {
        ...
        if (fieldValue != null) {
            ...
            if (!isNullable(field.schema()) && field.defaultVal() == null) {
                throw new SchemaCompatibilityException(
                        "Field " + field.name() + " has no default value and is null in old record");
            }
    ...
    public static Schema generateProjectionSchema(Schema originalSchema, List<String> fieldNames) {
        Map<String, Field> schemaFieldsMap =
                originalSchema.getFields().stream()
                        .map(r -> Pair.of(r.name().toLowerCase(), r))
        ...
        for (String fn : fieldNames) {
            ...
            if (field == null) {
                throw new HoodieException(
                        "Field "
                                + fn
                                + " not found in log schema. Query cannot proceed! "
    ...
    /**
      * Obtain the root-level field name of a full field name, possibly a nested field. For example,
      * given "a.b.c", the output is "a"; given "a", the output is "a".
      *
    ...
    /**
      * Obtain value of the provided key, when set returnNullIfNotFound false, it is consistent with
      * avro after 1.10
      */
    public static Object getFieldVal(GenericRecord record, String key, boolean returnNullIfNotFound) {
        ...
        if (field == null) {
            if (returnNullIfNotFound) {
                ...
                // Since avro 1.10, arvo will throw AvroRuntimeException("Not a valid schema field:
                // " + key)
                // rather than return null like the previous version if record doesn't contain this
                // key.
    ...
    public static String getNestedFieldValAsString(
        ...
        Object obj =
                getNestedFieldVal(
                        record, fieldName, returnNullIfNotFound, consistentLogicalTimestampEnabled);
        ...
        for (int i = 0; i < parts.length; i++) {
            ...
            if (i == parts.length - 1) {
                ...
                if (val == null) {
                    ...
                    return convertValueForSpecificDataTypes(
                            fieldSchema, val, consistentLogicalTimestampEnabled);
                }
            } else {
    ...
    /**
      * Get schema for the given field and record. Field can be nested, denoted by dot notation. e.g:
      * a.b.c
      *
    ...
    /**
      * Get schema for the given field and write schema. Field can be nested, denoted by dot notation.
      * e.g: a.b.c Use this method when record is not available. Otherwise, prefer to use {@link
      * #getNestedFieldSchemaFromRecord(GenericRecord, String)}
      *
    ...
    /**
      * Returns the string value of the given record {@code rec} and field {@code fieldName}. The field
      * and value both could be missing.
      *
      * @param rec The record
      * @param fieldName The field name
      * @return the string form of the field or empty if the schema does not contain the field name or
      *     the value is null
      */
    ...
    /**
      * This method converts values for fields with certain Avro/Parquet data types that require
      * special handling.
      *
      ...
      */
    public static Object convertValueForSpecificDataTypes(
            Schema fieldSchema, Object fieldValue, boolean consistentLogicalTimestampEnabled) {
        if (fieldSchema == null) {
            return fieldValue;
        ...
        return convertValueForAvroLogicalTypes(
                resolveNullableSchema(fieldSchema), fieldValue, consistentLogicalTimestampEnabled);
    }

    /**
      * This method converts values for fields with certain Avro Logical data types that require
      * special handling.
      *
      * <p>Logical Date Type is converted to actual Date value instead of Epoch Integer which is how it
      * is represented/stored in parquet.
      *
      * <p>Decimal Data Type is converted to actual decimal value instead of bytes/fixed which is how
      * it is represented/stored in parquet.
      *
      * @param fieldSchema avro field schema
      * @param fieldValue avro field value
      * @return field value either converted (for certain data types) or as it is.
      */
    private static Object convertValueForAvroLogicalTypes(
            Schema fieldSchema, Object fieldValue, boolean consistentLogicalTimestampEnabled) {
            ...
            if (fieldSchema.getType() == Schema.Type.FIXED) {
                ...
                BigDecimal convertedValue =
                        decimalConversion.fromBytes(
                                byteBuffer, fieldSchema, LogicalTypes.decimal(dc.getPrecision(), dc.getScale()));
                byteBuffer.rewind();
    ...
    /**
      * Sanitizes Name according to Avro rule for names. Removes characters other than the ones
      * mentioned in https://avro.apache.org/docs/current/spec.html#names .
      *
    ...
    /**
      ...
      * @return Column value.
      */
    public static Object[] getRecordColumnValues(
            HoodieRecord record,
            String[] columns,
            Schema schema,
            boolean consistentLogicalTimestampEnabled) {
        try {
            GenericRecord genericRecord =
                    (GenericRecord) (record.toIndexedRecord(schema, new Properties()).get()).getData();
            List<Object> list = new ArrayList<>();
            for (String col : columns) {
                list.add(
                        HoodieAvroUtils.getNestedFieldVal(
                                genericRecord, col, true, consistentLogicalTimestampEnabled));
            }
            return list.toArray();
        } catch (IOException e) {
    ...
    // TODO java-doc
    public static GenericRecord rewriteRecordWithNewSchema(
            IndexedRecord oldRecord, Schema newSchema) {
        return rewriteRecordWithNewSchema(oldRecord, newSchema, Collections.emptyMap());
    }

    /**
      * Given a avro record with a given schema, rewrites it into the new schema while setting fields
      * only from the new schema. support deep rewrite for nested record. This particular method does
      * the following things : a) Create a new empty GenericRecord with the new schema. b) For
      * GenericRecord, copy over the data from the old schema to the new schema or set default values
      * for all fields of this transformed schema
      *
      * @param oldRecord oldRecord to be rewritten
      * @param newSchema newSchema used to rewrite oldRecord
      * @param renameCols a map store all rename cols, (k, v)-> (colNameFromNewSchema,
      *     colNameFromOldSchema)
      * @return newRecord for new Schema
      */
    public static GenericRecord rewriteRecordWithNewSchema(
            IndexedRecord oldRecord, Schema newSchema, Map<String, String> renameCols) {
        Object newRecord =
                rewriteRecordWithNewSchema(
                        oldRecord, oldRecord.getSchema(), newSchema, renameCols, new LinkedList<>(), false);
        return (GenericData.Record) newRecord;
    }

    public static GenericRecord rewriteRecordWithNewSchema(
            IndexedRecord oldRecord, Schema newSchema, Map<String, String> renameCols, boolean validate) {
        Object newRecord =
                rewriteRecordWithNewSchema(
                        oldRecord, oldRecord.getSchema(), newSchema, renameCols, new LinkedList<>(), validate);
        return (GenericData.Record) newRecord;
    }

    /**
      * Given a avro record with a given schema, rewrites it into the new schema while setting fields
      * only from the new schema. support deep rewrite for nested record and adjust rename operation.
      * This particular method does the following things : a) Create a new empty GenericRecord with the
      * new schema. b) For GenericRecord, copy over the data from the old schema to the new schema or
      * set default values for all fields of this transformed schema
      *
      * @param oldRecord oldRecord to be rewritten
      ...
      * @return newRecord for new Schema
      */
    private static Object rewriteRecordWithNewSchema(
            Object oldRecord,
        ...
        Object newRecord =
                rewriteRecordWithNewSchemaInternal(oldRecord, oldSchema, newSchema, renameCols, fieldNames);
    ...
    private static Object rewriteRecordWithNewSchemaInternal(
            Object oldRecord,
            Schema oldSchema,
            Schema newSchema,
            Map<String, String> renameCols,
        ...
        switch (newSchema.getType()) {
                ...
                for (int i = 0; i < fields.size(); i++) {
                    ...
                    if (oldField != null && !renameCols.containsKey(field.name())) {
                        newRecord.put(
                                i,
                                rewriteRecordWithNewSchema(
                                        indexedRecord.get(oldField.pos()),
                                        oldField.schema(),
                                        fields.get(i).schema(),
                                        renameCols,
                                        fieldNames,
                        ...
                        Schema.Field oldFieldRenamed =
                                fieldNameFromOldSchema == null ? null : oldSchema.getField(fieldNameFromOldSchema);
                        if (oldFieldRenamed != null) {
                            // find rename
                            newRecord.put(
                                    i,
                                    rewriteRecordWithNewSchema(
                                            indexedRecord.get(oldFieldRenamed.pos()),
                                            oldFieldRenamed.schema(),
                                            fields.get(i).schema(),
                                            renameCols,
                                            fieldNames,
                                            false));
            ...
            case ENUM:
                if (oldSchema.getType() != Schema.Type.STRING && oldSchema.getType() != Schema.Type.ENUM) {
                    throw new SchemaCompatibilityException(
                            String.format(
                                    "Only ENUM or STRING type can be converted ENUM type. Schema type was %s",
            ...
            case ARRAY:
                if (!(oldRecord instanceof Collection)) {
                    throw new SchemaCompatibilityException(
                            String.format("Cannot rewrite %s as an array", oldRecord.getClass().getName()));
                }
            ...
            case MAP:
                if (!(oldRecord instanceof Map)) {
                    throw new SchemaCompatibilityException(
                            String.format("Cannot rewrite %s as a map", oldRecord.getClass().getName()));
                }
                ...
                case FIXED:
                    if (oldSchema.getFixedSize() != newSchema.getFixedSize()) {
                        ...
                        if (oldSchema.getLogicalType() instanceof Decimal) {
                            ...
                            BigDecimal bd =
                                    new BigDecimal(new BigInteger(bytes), decimal.getScale())
                                            .setScale(((Decimal) newSchema.getLogicalType()).getScale());
                            return DECIMAL_CONVERSION.toFixed(bd, newSchema, newSchema.getLogicalType());
                        } else {
                            throw new HoodieAvroSchemaException(
                                    "Fixed type size change is not currently supported");
                        }
                    }

                    // For [[Fixed]] data type both size and name have to match
                    //
                    // NOTE: That for values wrapped into [[Union]], to make sure that reverse
                    // lookup (by
                    //       full-name) is working we have to make sure that both schema's name and
                    // namespace
                    //       do match
    ...
    private static Object rewritePrimaryTypeWithDiffSchemaType(
            Object oldValue, Schema oldSchema, Schema newSchema) {
        switch (newSchema.getType()) {
            case NULL:
            case BOOLEAN:
                break;
            ...
            case LONG:
                ...
                break;
            case FLOAT:
                if ((oldSchema.getType() == Schema.Type.INT) || (oldSchema.getType() == Schema.Type.LONG)) {
                    return oldSchema.getType() == Schema.Type.INT
                            ? ((Integer) oldValue).floatValue()
                            : ((Long) oldValue).floatValue();
                }
                break;
            case DOUBLE:
                if (oldSchema.getType() == Schema.Type.FLOAT) {
                    // java float cannot convert to double directly, deal with float precision
                    // change
            ...
            case FIXED:
                ...
                if (newSchema.getLogicalType() instanceof LogicalTypes.Decimal) {
                    ...
                    if (oldSchema.getType() == Schema.Type.STRING
                        ...
                        // due to Java, there will be precision problems in direct conversion, we
                        // should use string instead of use double
                        BigDecimal bigDecimal =
                                new java.math.BigDecimal(oldValue.toString())
        ...
        throw new HoodieAvroSchemaException(
                String.format(
                        "cannot support rewrite value for schema type: %s since the old schema type is: %s",
                        newSchema, oldSchema));
    }

    /**
      * Avro does not support type promotion from numbers to string. This function returns true if it
      * will be necessary to rewrite the record to support this promotion. NOTE: this does not
      * determine whether the writerSchema and readerSchema are compatible. It is just trying to find
      * if the reader expects a number to be promoted to string, as quick as possible.
      */
    public static boolean recordNeedsRewriteForExtendedAvroTypePromotion(
        ...
        switch (readerSchema.getType()) {
            case RECORD:
                ...
                for (Schema.Field field : readerSchema.getFields()) {
                    if (writerFields.containsKey(field.name())) {
                        if (recordNeedsRewriteForExtendedAvroTypePromotion(
                                writerFields.get(field.name()).schema(), field.schema())) {
                            return true;
                        }
                    }
                }
            ...
            case ARRAY:
                if (writerSchema.getType().equals(ARRAY)) {
                    return recordNeedsRewriteForExtendedAvroTypePromotion(
                            writerSchema.getElementType(), readerSchema.getElementType());
                }
                return false;
            case MAP:
                if (writerSchema.getType().equals(MAP)) {
                    return recordNeedsRewriteForExtendedAvroTypePromotion(
                            writerSchema.getValueType(), readerSchema.getValueType());
                }
                return false;
            case UNION:
                return recordNeedsRewriteForExtendedAvroTypePromotion(
                        getActualSchemaFromUnion(writerSchema, null),
                        getActualSchemaFromUnion(readerSchema, null));
            case ENUM:
            case STRING:
            case BYTES:
    ...
    /**
      ...
      *
      * <p>NOTE: This method could only be used in tests @VisibleForTesting
      */
    ...
    public static HoodieRecord createHoodieRecordFromAvro(
        ...
        if (populateMetaFields) {
            return SpillableMapUtils.convertToHoodieRecordPayload(
                    (GenericRecord) data, payloadClass, preCombineField, withOperation);
    ...
    /**
      ...
      */
    public static Iterator<GenericRecord> rewriteRecordWithNewSchema(
            Iterator<GenericRecord> oldRecords,
            Schema newSchema,
            Map<String, String> renameCols,
            boolean validate) {
    ...
    public static Iterator<GenericRecord> rewriteRecordWithNewSchema(
            Iterator<GenericRecord> oldRecords, Schema newSchema, Map<String, String> renameCols) {
    ...
    public static GenericRecord rewriteRecordDeep(
            GenericRecord oldRecord, Schema newSchema, boolean validate) {
    ...
    public static Object wrapValueIntoAvro(Comparable<?> value) {
        if (value == null) {
            ...
            LocalDate localDate =
                    value instanceof LocalDate ? (LocalDate) value : ((Date) value).toLocalDate();
            ...
            BigDecimal upcastDecimal =
                    tryUpcastDecimal((BigDecimal) value, (LogicalTypes.Decimal) valueSchema.getLogicalType());
            return DecimalWrapper.newBuilder(DECIMAL_WRAPPER_BUILDER_STUB.get())
                    .setValue(
                            AVRO_DECIMAL_CONVERSION.toBytes(
                                    upcastDecimal, valueSchema, valueSchema.getLogicalType()))
                    .build();
            ...
            return BooleanWrapper.newBuilder(BOOLEAN_WRAPPER_BUILDER_STUB.get())
                    .setValue((Boolean) value)
                    .build();
        } else if (value instanceof Integer) {
            return IntWrapper.newBuilder(INT_WRAPPER_BUILDER_STUB.get())
                    .setValue((Integer) value)
                    .build();
        } else if (value instanceof Long) {
            return LongWrapper.newBuilder(LONG_WRAPPER_BUILDER_STUB.get()).setValue((Long) value).build();
        } else if (value instanceof Float) {
            return FloatWrapper.newBuilder(FLOAT_WRAPPER_BUILDER_STUB.get())
                    .setValue((Float) value)
                    .build();
        } else if (value instanceof Double) {
            return DoubleWrapper.newBuilder(DOUBLE_WRAPPER_BUILDER_STUB.get())
                    .setValue((Double) value)
                    .build();
        } else if (value instanceof ByteBuffer) {
            return BytesWrapper.newBuilder(BYTES_WRAPPER_BUILDER_STUB.get())
            ...
            throw new UnsupportedOperationException(
                    String.format("Unsupported type of the value (%s)", value.getClass()));
        }
    }
    ...
    public static Comparable<?> unwrapAvroValueWrapper(Object avroValueWrapper) {
        if (avroValueWrapper == null) {
            ...
            Schema valueSchema = DecimalWrapper.SCHEMA$.getField("value").schema();
            return AVRO_DECIMAL_CONVERSION.fromBytes(
                    ((DecimalWrapper) avroValueWrapper).getValue(),
                    valueSchema,
