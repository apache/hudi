...
/**
  ...
  *
  * <p>By default, Avro's JSON encoding for union types is to wrap the union value in a JSON object
  * with the type name as the key (e.g., {"string": "value"}). This encoder overrides that behavior
  * to write just the value, resulting in cleaner JSON output.
  *
  * <p>For instance, a union with schema ["null", "string"] would be encoded as just "value" instead
  * of {"string": "value"}.
  *
  * <p>This encoder is particularly useful when the standard Avro JSON format's verbosity for union
  * types is not desired.
  */
public class JsonEncoder extends ParsingEncoder implements Parser.ActionHandler {
    ...
    JsonEncoder(Schema sc, OutputStream out, boolean pretty) throws IOException {
        this(
                sc,
                getJsonGenerator(
                        out, pretty ? EnumSet.of(JsonOptions.Pretty) : EnumSet.noneOf(JsonOptions.class)));
    }
    ...
    private static JsonGenerator getJsonGenerator(OutputStream out, Set<JsonOptions> options)
            throws IOException {
    ...
    /**
      ...
      *
      * <p>Otherwise, this JsonEncoder will flush its current output and then reconfigure its output to
      * use a default UTF8 JsonGenerator that writes to the provided OutputStream.
      *
    ...
    /**
      ...
      *
      * <p>Otherwise, this JsonEncoder will flush its current output and then reconfigure its output to
      * use the provided JsonGenerator.
      *
