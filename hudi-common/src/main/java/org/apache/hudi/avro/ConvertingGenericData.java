...
/**
  * Custom instance of the {@link GenericData} model incorporating conversions from the common Avro
  * logical types like "decimal", "uuid", "date", "time-micros", "timestamp-micros"
  *
  * <p>NOTE: Given that this code has to be interoperable w/ Spark 2 (which relies on Avro 1.8.2)
...
public class ConvertingGenericData extends GenericData {
    ...
    private static final Conversions.UUIDConversion UUID_CONVERSION =
            new Conversions.UUIDConversion();
    private static final TimeConversions.DateConversion DATE_CONVERSION =
            new TimeConversions.DateConversion();
    private static final TimeConversions.TimeMicrosConversion TIME_MICROS_CONVERSION =
            new TimeConversions.TimeMicrosConversion();
    private static final TimeConversions.TimestampMicrosConversion TIMESTAMP_MICROS_CONVERSION =
            new TimeConversions.TimestampMicrosConversion();

    // NOTE: Those are not supported in Avro 1.8.2
    // TODO re-enable upon upgrading to 1.10
    // private static final TimeConversions.TimestampMillisConversion TIMESTAMP_MILLIS_CONVERSION =
    // new TimeConversions.TimestampMillisConversion();
    // private static final TimeConversions.TimeMillisConversion TIME_MILLIS_CONVERSION = new
    // TimeConversions.TimeMillisConversion();
    // private static final TimeConversions.LocalTimestampMillisConversion
    // LOCAL_TIMESTAMP_MILLIS_CONVERSION = new TimeConversions.LocalTimestampMillisConversion();
    // private static final TimeConversions.LocalTimestampMicrosConversion
    // LOCAL_TIMESTAMP_MICROS_CONVERSION = new TimeConversions.LocalTimestampMicrosConversion();
    ...
    public boolean validate(Schema schema, Object datum) {
        switch (schema.getType()) {
            ...
            case STRING:
                return isString(datum) || UUID_CONVERSION.getConvertedType().isInstance(datum);
            case BYTES:
                return isBytes(datum) || DECIMAL_CONVERSION.getConvertedType().isInstance(datum);
            case INT:
                return isInteger(datum) || DATE_CONVERSION.getConvertedType().isInstance(datum);
            case LONG:
                return isLong(datum)
