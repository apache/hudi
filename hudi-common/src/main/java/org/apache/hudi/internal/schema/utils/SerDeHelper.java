...
public class SerDeHelper {
    ...
    /**
      * Convert history internalSchemas to json. this is used when save history schemas into hudi.
      *
    ...
    private static void toJson(InternalSchema internalSchema, JsonGenerator generator)
            throws IOException {
        toJson(
                internalSchema.getRecord(),
                internalSchema.getMaxColumnId(),
                internalSchema.schemaId(),
                generator);
    }

    private static void toJson(
            Types.RecordType record, Integer maxColumnId, Long versionId, JsonGenerator generator)
            throws IOException {
        generator.writeStartObject();
        if (maxColumnId != null) {
    ...
    private static Type parseTypeFromJson(JsonNode jsonNode) {
        ...
        throw new IllegalArgumentException(
                String.format("cannot parse type from jsonNode: %s", jsonNode));
    }
    ...
    /**
      * Convert json string to history internalSchemas. TreeMap is used to hold history
      * internalSchemas.
      *
      * @param json a json string
      * @return a TreeMap
    ...
    public static TreeMap<Long, InternalSchema> parseSchemas(String json) {
        ...
        try {
            ...
            if (!jsonNode.has(SCHEMAS)) {
                throw new IllegalArgumentException(
                        String.format(
                                "cannot parser schemas from current json string, missing key name: %s", SCHEMAS));
            }
    ...
    /**
      * Add the new schema to the historical schemas. use string operations to reduce overhead.
      *
