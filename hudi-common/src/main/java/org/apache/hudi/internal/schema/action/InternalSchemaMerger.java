...
/**
  * Auxiliary class. help to merge file schema and query schema to produce final read schema for
  * avro/parquet file
  */
public class InternalSchemaMerger {
    ...
    // Whether to use column Type from file schema to read files when we find some column type has
    // changed.
    // spark parquetReader need the original column type to read data, otherwise the parquetReader
    // will failed.
    // eg: current column type is StringType, now we changed it to decimalType,
    // we should not pass decimalType to parquetReader, we must pass StringType to it; when we read
    // out the data, we convert data from String to Decimal, everything is ok.
    // for log reader
    // since our reWriteRecordWithNewSchema function support rewrite directly, so we no need this
    // parameter
    // eg: current column type is StringType, now we changed it to decimalType,
    ...
    // deal with rename
    // Whether to use column name from file schema to read files when we find some column name has
    // changed.
    // spark parquetReader need the original column name to read data, otherwise the parquetReader
    // will read nothing.
    // eg: current column name is colOldName, now we rename it to colNewName,
    // we should not pass colNewName to parquetReader, we must pass colOldName to it; when we read
    // out the data.
    // for log reader
    // since our reWriteRecordWithNewSchema function support rewrite directly, so we no need this
    // parameter
    ...
    /**
      * Create final read schema to read avro/parquet file. this is auxiliary function used by
      * mergeSchema.
      */
                ...
                if (fileSchema.findField(fullName) != null) {
                    newFields.add(
                            Types.Field.get(
                                    oldField.fieldId(),
                                    oldField.isOptional(),
                                    oldField.name() + "suffix",
                                    oldField.type(),
                                    oldField.doc()));
                } else {
                    // find add column
                    // now there exist some bugs when we use spark update/merge api, those operation
                    // will change col optional to required.
                    if (ignoreRequiredAttribute) {
                        newFields.add(
                                Types.Field.get(
                                        oldField.fieldId(), true, oldField.name(), newType, oldField.doc()));
                    } else {
                        newFields.add(
                                Types.Field.get(
                                        oldField.fieldId(),
    ...
    private Types.Field dealWithRename(int fieldId, Type newType, Types.Field oldField) {
        ...
        // Current design mechanism guarantees nestedType change is not allowed, so no need to
        // consider.
        if (newType.isNestedType()) {
            return Types.Field.get(
                    oldField.fieldId(), oldField.isOptional(), finalFieldName, newType, oldField.doc());
        } else {
            return Types.Field.get(
                    oldField.fieldId(),
                    oldField.isOptional(),
                    finalFieldName,
                    useColumnTypeFromFileSchema ? typeFromFileSchema : newType,
                    oldField.doc());
        }
    }
    ...
    private Type buildMapType(Types.MapType map, Type newValue) {
        ...
        if (valueFiled.type() == newValue) {
            ...
            return Types.MapType.get(
                    map.keyId(), map.valueId(), map.keyType(), newValue, map.isValueOptional());
        }
    }

    private Type buildPrimitiveType(
            Type.PrimitiveType typeFromQuerySchema, int currentPrimitiveTypeId) {
