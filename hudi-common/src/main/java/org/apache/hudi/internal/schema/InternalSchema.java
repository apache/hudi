...
/** Internal schema for hudi table. used to support schema evolution. */
public class InternalSchema implements Serializable {
    ...
    /**
      * Returns a sub-field by name as a {@link Field}. The result may be a top-level or a nested
      * field.
      *
    ...
    /**
      * Returns the full name of the field and its position in the schema. This differs from its ID in
      * cases where new fields are not appended to the end of schemas. The output is used when
      * reconciling the order of fields while ingesting.
      *
    ...
    public String toString() {
        return String.format(
                "table {\n%s\n}",
                StringUtils.join(record.fields().stream().map(f -> " " + f).toArray(String[]::new), "\n"));
    }
