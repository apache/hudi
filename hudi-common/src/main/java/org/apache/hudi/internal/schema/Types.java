...
public class Types {
    ...
    public static class RecordType extends NestedType {
        ...
        //       InternalSchema and back (Avro unfortunately relies not only on structural
        // equivalence of
        //       schemas but also corresponding Record type's "name" when evaluating their
        // compatibility);
        ...
        public Field fieldByNameCaseInsensitive(String name) {
            if (lowercaseNameToFields == null) {
                lowercaseNameToFields =
                        Arrays.stream(fields)
                                .collect(
                                        Collectors.toMap(field -> field.name.toLowerCase(Locale.ROOT), field -> field));
            }
        ...
        public Field field(int id) {
            if (idToFields == null) {
                idToFields =
                        Arrays.stream(fields).collect(Collectors.toMap(Field::fieldId, field -> field));
            }
        ...
        public String toString() {
            return String.format(
                    "Record<%s>",
                    Arrays.stream(fields).map(f -> f.toString()).collect(Collectors.joining("-")));
        }
        ...
        @Override
        public int hashCode() {
            // NOTE: {@code hashCode} has to match for objects for which {@code equals} returns
            // true,
    ...
    public static class MapType extends NestedType {
        ...
        public static MapType get(
                int keyId, int valueId, Type keyType, Type valueType, boolean isOptional) {
            return new MapType(
