...
public class TableChangesHelper {
    /**
      ...
      */
    public static List<Types.Field> applyAddChange2Fields(
            List<Types.Field> fields,
        ...
        if (pchanges != null && !pchanges.isEmpty()) {
            for (TableChange.ColumnPositionChange pchange : pchanges) {
                Types.Field srcField =
                        result.stream().filter(f -> f.fieldId() == pchange.getSrcId()).findFirst().get();
