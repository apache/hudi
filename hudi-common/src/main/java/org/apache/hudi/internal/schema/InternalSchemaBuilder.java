...
public class InternalSchemaBuilder implements Serializable {
    ...
    /**
      * Build a mapping from id to full field name for a internal Type. if a field y belong to a struct
      * filed x, then the full name of y is x.y
      *
    ...
    /**
      * Build a mapping from full field name to id for a internal Type. if a field y belong to a struct
      * filed x, then the full name of y is x.y
      *
      * @param type hoodie internal type
    ...
    /**
      * Build a mapping which maintain the relation between child field id and it's parent field id. if
      * a child field y(which id is 9) belong to a nest field x(which id is 6), then (9 -> 6) will be
      * added to the result map. if a field has no parent field, nothings will be added.
      *
        ...
        switch (type.typeId()) {
            case RECORD:
                ...
                for (int i = 0; i < oldFields.size(); i++) {
                    ...
                    internalFields.add(
                            Types.Field.get(
                                    currentId++, oldField.isOptional(), oldField.name(), fieldType, oldField.doc()));
                }
