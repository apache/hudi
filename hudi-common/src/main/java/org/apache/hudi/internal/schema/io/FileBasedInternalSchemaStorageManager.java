...
public class FileBasedInternalSchemaStorageManager extends AbstractInternalSchemaStorageManager {
                    ...
                    HoodieTableMetaClient.builder()
                            ...
                            .setTimeGeneratorConfig(
                                    HoodieTimeGeneratorConfig.defaultConfig(
                                            baseSchemaPath.getParent().getParent().toString()))
                            .build();
        }
        return metaClient;
    }
    ...
    public void persistHistorySchemaStr(String instantTime, String historySchemaStr) {
        ...
        HoodieInstant hoodieInstant =
                new HoodieInstant(HoodieInstant.State.REQUESTED, SCHEMA_COMMIT_ACTION, instantTime);
        ...
        timeline.saveAsComplete(
                false,
                new HoodieInstant(
                        HoodieInstant.State.INFLIGHT, hoodieInstant.getAction(), hoodieInstant.getTimestamp()),
        ...
        try {
            if (storage.exists(baseSchemaPath)) {
                List<String> candidateSchemaFiles =
                        storage.listDirectEntries(baseSchemaPath).stream()
                                .filter(f -> f.isFile())
                                .map(file -> file.getPath().getName())
                                .collect(Collectors.toList());
                List<String> residualSchemaFiles =
                        candidateSchemaFiles.stream()
                                .filter(f -> !validateCommits.contains(f.split("\\.")[0]))
                                .collect(Collectors.toList());
                // clean residual files
                residualSchemaFiles.forEach(
                        f -> {
                            try {
    ...
    public String getHistorySchemaStrByGivenValidCommits(List<String> validCommits) {
        List<String> commitList =
                validCommits == null || validCommits.isEmpty() ? getValidInstants() : validCommits;
        try {
            if (storage.exists(baseSchemaPath)) {
                List<String> validaSchemaFiles =
                        storage.listDirectEntries(baseSchemaPath).stream()
                                ...
                                .filter(f -> commitList.contains(HoodieInstant.extractTimestamp(f)))
                                .sorted()
                                .collect(Collectors.toList());
