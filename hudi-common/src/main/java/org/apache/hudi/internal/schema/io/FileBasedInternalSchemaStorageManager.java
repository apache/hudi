...
public class FileBasedInternalSchemaStorageManager extends AbstractInternalSchemaStorageManager {
  ...
  private final StoragePath baseSchemaLocation;
  private final Configuration conf;
  private HoodieTableMetaClient metaClient;

  public FileBasedInternalSchemaStorageManager(Configuration conf, StoragePath baseTableLocation) {
    StoragePath metaLocation = new StoragePath(baseTableLocation, ".hoodie");
    this.baseSchemaLocation = new StoragePath(metaLocation, SCHEMA_NAME);
    this.conf = conf;
  }

  public FileBasedInternalSchemaStorageManager(HoodieTableMetaClient metaClient) {
    StoragePath metaLocation = new StoragePath(metaClient.getBasePath(), ".hoodie");
    this.baseSchemaLocation = new StoragePath(metaLocation, SCHEMA_NAME);
      ...
      if (storage.exists(baseSchemaLocation)) {
        ...
        if (!validaSchemaFiles.isEmpty()) {
          StoragePath latestFilePath =
              new StoragePath(baseSchemaLocation, validaSchemaFiles.get(validaSchemaFiles.size() - 1));
          byte[] content;
