...
/**
  * Auxiliary class. Converts an avro schema into InternalSchema, or convert InternalSchema to an
  * avro schema
  */
public class AvroInternalSchemaConverter {
    ...
    public static InternalSchema pruneAvroSchemaToInternalSchema(
            Schema schema, InternalSchema originSchema) {
    ...
    private static void collectColNamesFromAvroSchema(
            Schema schema, Deque<String> visited, List<String> resultSet) {
        switch (schema.getType()) {
            ...
            case UNION:
                collectColNamesFromAvroSchema(
                        AvroSchemaUtils.resolveNullableSchema(schema), visited, resultSet);
                return;
    ...
    private static void addFullNameIfLeafNode(
            Schema schema, String name, Deque<String> visited, List<String> resultSet) {
        addFullNameIfLeafNode(
                AvroSchemaUtils.resolveNullableSchema(schema).getType(), name, visited, resultSet);
    }

    private static void addFullNameIfLeafNode(
            Schema.Type type, String name, Deque<String> visited, List<String> resultSet) {
        switch (type) {
            case RECORD:
            case ARRAY:
            case MAP:
    ...
    /**
      * Converting from avro -> internal schema -> avro causes null to always be first in unions. if we
      * compare a schema that has not been converted to internal schema at any stage, the difference in
      * ordering can cause issues. To resolve this, we order null to be first for any avro schema that
      * enters into hudi. AvroSchemaUtils.isProjectionOfInternal uses index based comparison for
      * unions. Spark and flink don't support complex unions so this would not be an issue but for the
      * metadata table HoodieMetadata.avsc uses a trick where we have a bunch of different types
      * wrapped in record for col stats.
      *
    ...
    private static Type convertToField(
            Schema schema, Map<String, Integer> existingFieldNameToPositionMapping) {
        return buildTypeFromAvroSchema(schema, existingFieldNameToPositionMapping);
    }

    /** Convert an avro schema into internalSchema. */
    public static InternalSchema convert(
            Schema schema, Map<String, Integer> existingFieldNameToPositionMapping) {
        return new InternalSchema(
                (Types.RecordType) convertToField(schema, existingFieldNameToPositionMapping));
    }
        ...
        if (schema.getType() == UNION) {
            if (!isOptional(schema)) {
                throw new HoodieSchemaException(
                        String.format("Union schemas are not supported: %s", schema));
            }
            return schema;
        } else {
    ...
    /**
      ...
      * @return a hudi type.
      */
    public static Type buildTypeFromAvroSchema(
            Schema schema, Map<String, Integer> existingNameToPositions) {
    ...
    private static void checkNullType(Type fieldType, String fieldName, Deque<String> visited) {
        if (fieldType == null) {
            ...
            checkNullType(
                    ((Types.ArrayType) fieldType).elementType(), InternalSchema.ARRAY_ELEMENT, visited);
            visited.pop();
    ...
    /**
      ...
      *
      * @param schema a avro schema.
      * @param visited track the visit node when do traversal for avro schema; used to check if the
      *     name of avro record schema is correct.
      * @param currentFieldPath the dot-separated path to the current field; empty at the root and
      *     always ends in a '.' otherwise for ease of concatenation.
      * @param nextId an initial id which used to create id for all fields.
      * @return a hudi type match avro schema.
      */
    private static Type visitAvroSchemaToBuildType(
            Schema schema,
        ...
        switch (schema.getType()) {
            case RECORD:
                ...
                if (visited.contains(name)) {
                    throw new HoodieSchemaException(
                            String.format("cannot convert recursive avro record %s", name));
                }
                visited.push(name);
                List<Schema.Field> fields =
                        existingNameToPosition.isEmpty()
                                ? schema.getFields()
                                : schema.getFields().stream()
                                        .sorted(
                                                Comparator.comparing(
                                                        field ->
                                                                existingNameToPosition.getOrDefault(
                                                                        currentFieldPath + field.name(), Integer.MAX_VALUE)))
                ...
                for (int i = 0; i < fields.size(); i++) {
                    ...
                    internalFields.add(
                            Types.Field.get(
                                    nextAssignId,
                                    AvroInternalSchemaConverter.isOptional(field.schema()),
                                    field.name(),
                                    fieldType,
                                    field.doc()));
                    nextAssignId += 1;
                ...
                schema
                        ...
                        .forEach(
                                t -> {
                                    fTypes.add(
                                            visitAvroSchemaToBuildType(
                                                    t, visited, currentFieldPath, nextId, existingNameToPosition));
                                });
            ...
            case ARRAY:
                ...
                Type elementType =
                        visitAvroSchemaToBuildType(
                                elementSchema, visited, elementPath, nextId, existingNameToPosition);
                return Types.ArrayType.get(
                        elementId,
                        AvroInternalSchemaConverter.isOptional(schema.getElementType()),
                        elementType);
            case MAP:
                ...
                Type valueType =
                        visitAvroSchemaToBuildType(
                                schema.getValueType(), visited, valuePath, nextId, existingNameToPosition);
                return Types.MapType.get(
                        keyId,
                        valueId,
                        Types.StringType.get(),
                        valueType,
                        AvroInternalSchemaConverter.isOptional(schema.getValueType()));
            default:
    ...
    /**
      ...
      * @param recordName auto-generated record name used as a fallback, in case {@link
      *     org.apache.hudi.internal.schema.Types.RecordType} doesn't bear original record-name
      * @return an Avro schema match this type
      */
    private static Schema visitInternalSchemaToBuildAvroSchema(
            Type type, Map<Type, Schema> cache, String recordName) {
        switch (type.typeId()) {
            case RECORD:
                ...
                record
                        ...
                        .forEach(
                                f -> {
                                    ...
                                    Schema result =
                                            f.isOptional()
                                                    ? AvroInternalSchemaConverter.nullableSchema(tempSchema)
                                                    : tempSchema;
                                    schemas.add(result);
                                });
                // check visited
            ...
            default:
                Schema primitiveSchema =
                        visitInternalPrimitiveToBuildAvroPrimitiveType((Type.PrimitiveType) type, recordName);
    ...
    /**
      * Converts hudi RecordType to Avro RecordType. this is auxiliary function used by
      * visitInternalSchemaToBuildAvroSchema
      */
    private static Schema visitInternalRecordToBuildAvroRecord(
            Types.RecordType recordType, List<Schema> fieldSchemas, String recordNameFallback) {
        ...
        for (int i = 0; i < fields.size(); i++) {
            ...
            Schema.Field field =
                    new Schema.Field(
                            ...
                            fieldSchemas.get(i),
                            f.doc(),
                            f.isOptional() ? JsonProperties.NULL_VALUE : null);
    ...
    /**
      * Converts hudi ArrayType to Avro ArrayType. this is auxiliary function used by
      * visitInternalSchemaToBuildAvroSchema
      */
    private static Schema visitInternalArrayToBuildAvroArray(
            Types.ArrayType array, Schema elementSchema) {
        Schema result;
    ...
    /**
      * Converts hudi MapType to Avro MapType. this is auxiliary function used by
      * visitInternalSchemaToBuildAvroSchema
      */
    private static Schema visitInternalMapToBuildAvroMap(
            Types.MapType map, Schema keySchema, Schema valueSchema) {
        Schema mapSchema;
        if (keySchema.getType() == Schema.Type.STRING) {
            mapSchema =
                    Schema.createMap(
                            map.isValueOptional()
                                    ? AvroInternalSchemaConverter.nullableSchema(valueSchema)
                                    : valueSchema);
        } else {
    ...
    /**
      * Converts hudi PrimitiveType to Avro PrimitiveType. this is auxiliary function used by
      * visitInternalSchemaToBuildAvroSchema
      */
    private static Schema visitInternalPrimitiveToBuildAvroPrimitiveType(
            Type.PrimitiveType primitive, String recordName) {
            ...
            case DECIMAL:
                {
                    ...
                    Schema fixedSchema =
                            Schema.createFixed(
                                    name, null, null, computeMinBytesForPrecision(decimal.precision()));
    ...
    /**
      * Return the minimum number of bytes needed to store a decimal with a give 'precision'. reference
      * from Spark release 3.1 .
      */
