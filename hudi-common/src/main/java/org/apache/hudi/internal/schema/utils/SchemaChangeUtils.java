...
public class SchemaChangeUtils {
    ...
    /**
      * Whether to allow the column type to be updated. now only support: int =>
      * long/float/double/String/Decimal long => float/double/String/Decimal float =>
      * double/String/Decimal double => String/Decimal Decimal => Decimal/String String => date/decimal
      * date => String TODO: support more type update.
      *
        ...
        switch (src.typeId()) {
            case INT:
                return dsr == Types.LongType.get()
                        || dsr == Types.FloatType.get()
                        || dsr == Types.DoubleType.get()
                        || dsr == Types.StringType.get()
                        || dsr.typeId() == Type.TypeID.DECIMAL;
            case LONG:
                return dsr == Types.FloatType.get()
                        || dsr == Types.DoubleType.get()
                        || dsr == Types.StringType.get()
                        || dsr.typeId() == Type.TypeID.DECIMAL;
            case FLOAT:
            ...
            case DECIMAL:
                ...
                break;
            case STRING:
                return dsr == Types.DateType.get()
                        || dsr.typeId() == Type.TypeID.DECIMAL
                        || dsr == Types.BinaryType.get();
            default:
                return false;
        }
        return false;
    ...
    /**
      ...
      */
    public static InternalSchema applyTableChanges2Schema(
            InternalSchema internalSchema, TableChanges.ColumnAddChange adds) {
        Types.RecordType newType =
                (Types.RecordType) applyTableChange2Type(internalSchema.getRecord(), adds);
        // deal with root level changes
        List<Types.Field> newFields =
                TableChangesHelper.applyAddChange2Fields(
                        newType.fields(),
                        adds.getParentId2AddCols().get(-1),
                        adds.getPositionChangeMap().get(-1));
        return new InternalSchema(Types.RecordType.get(newFields, newType.name()));
    }

    /**
      * Apply all the DDL add operations to Type to produce a new internalSchema. do not call this
      * method directly. expose this method only for UT.
      *
        ...
        switch (type.typeId()) {
            ...
            case MAP:
                ...
                return Types.MapType.get(
                        map.keyId(), map.valueId(), map.keyType(), newValueType, map.isValueOptional());
            default:
                return type;
        }
    }
    ...
    /**
      ...
      */
    public static InternalSchema applyTableChanges2Schema(
            InternalSchema internalSchema, TableChanges.ColumnDeleteChange deletes) {
        return new InternalSchema(
                (Types.RecordType) applyTableChange2Type(internalSchema.getRecord(), deletes));
    }

    /**
      * Apply all the DDL delete operations to Type to produce a new internalSchema. do not call this
      * method directly. expose this method only for UT.
      *
            ...
            case ARRAY:
                ...
                if (newElementType == null) {
                    throw new IllegalArgumentException(
                            String.format("cannot delete element from arrayType: %s", array));
                }
                return Types.ArrayType.get(array.elementId(), array.isElementOptional(), newElementType);
            case MAP:
                Types.MapType map = (Types.MapType) type;
                ...
                if (deletes.getDeletes().contains(keyId)) {
                    throw new IllegalArgumentException(
                            String.format("cannot delete key from mapType: %s", map));
                }
                ...
                if (newValueType == null) {
                    throw new IllegalArgumentException(
                            String.format("cannot delete value from mapType: %s", map));
                }
                return Types.MapType.get(
                        map.keyId(), map.valueId(), map.keyType(), newValueType, map.isValueOptional());
            default:
    ...
    /**
      ...
      */
    public static InternalSchema applyTableChanges2Schema(
            InternalSchema internalSchema, TableChanges.ColumnUpdateChange updates) {
        Types.RecordType newType =
                (Types.RecordType) applyTableChange2Type(internalSchema.getRecord(), updates);
        // deal with root level changes
        List<Types.Field> newFields =
                TableChangesHelper.applyAddChange2Fields(
                        newType.fields(), new ArrayList<>(), updates.getPositionChangeMap().get(-1));
        return new InternalSchema(Types.RecordType.get(newFields, newType.name()));
    }

    /**
      * Apply all the DDL update operations to type to produce a new internalSchema. do not call this
      * method directly. expose this method only for UT.
      *
    ...
    private static Type applyTableChange2Type(Type type, TableChanges.ColumnUpdateChange updates) {
        switch (type.typeId()) {
            ...
            case ARRAY:
                ...
                boolean optional =
                        elementUpdate == null ? array.isElementOptional() : elementUpdate.isOptional();
            ...
            case MAP:
                ...
                boolean valueOptional =
                        valueUpdate == null ? map.isValueOptional() : valueUpdate.isOptional();
                if (valueOptional == map.isValueOptional() && map.valueType() == newValueType) {
                    return map;
                }
                return Types.MapType.get(
                        map.keyId(), map.valueId(), map.keyType(), newValueType, valueOptional);
            default:
                return type;
        }
    }
