...
public class TableChanges {
    ...
    public static class ColumnUpdateChange extends TableChange.BaseColumnChange {
        ...
        public Type applyUpdates(Types.Field oldField, Type type) {
            ...
            //
            ArrayList<ColumnPositionChange> pchanges =
                    positionChangeMap.getOrDefault(oldField.fieldId(), new ArrayList<>());
            if (!pchanges.isEmpty()) {
                // when we build ColumnAddChange，we have already done some check, so it's safe to
                // convert newType to RecordType
                List<Types.Field> newFields =
                        TableChangesHelper.applyAddChange2Fields(
                                ((Types.RecordType) type).fields(), new ArrayList<>(), pchanges);
        ...
        /**
          * Update a column in the schema to a new type. only support update primitive type. Only updates
          * that widen types are allowed.
          *
        ...
        public ColumnUpdateChange updateColumnType(String name, Type newType) {
            ...
            if (newType.isNestedType()) {
                throw new IllegalArgumentException(
                        String.format("only support update primitive type but find nest column: %s", name));
            }
            Types.Field field = internalSchema.findField(name);
            if (field == null) {
                throw new IllegalArgumentException(
                        String.format("cannot update a missing column: %s", name));
            }

            if (!SchemaChangeUtils.isTypeUpdateAllow(field.type(), newType)) {
                throw new IllegalArgumentException(
                        String.format(
                                "cannot update origin type: %s to a incompatibility type: %s",
                                field.type(), newType));
            }
            ...
            if (update == null) {
                ...
                updates.put(
                        field.fieldId(),
                        Types.Field.get(
                                field.fieldId(), update.isOptional(), update.name(), newType, update.doc()));
            }
            return this;
        }
        ...
        public ColumnUpdateChange updateColumnComment(String name, String newDoc) {
            ...
            if (update == null) {
                updates.put(
                        field.fieldId(),
                        Types.Field.get(
                                field.fieldId(), field.isOptional(), field.name(), field.type(), newDoc));
            } else {
                updates.put(
                        field.fieldId(),
                        Types.Field.get(
                                field.fieldId(), update.isOptional(), update.name(), update.type(), newDoc));
            }
            return this;
        }
        ...
        public ColumnUpdateChange renameColumn(String name, String newName) {
            ...
            if (newName == null || newName.isEmpty()) {
                throw new IllegalArgumentException(
                        String.format("cannot rename column: %s to empty", name));
            }
            if (internalSchema.hasColumn(newName, caseSensitive)) {
                throw new IllegalArgumentException(
                        String.format("cannot rename column: %s to a existing name", name));
            }
            // save update info
            Types.Field update = updates.get(field.fieldId());
            if (update == null) {
                ...
                updates.put(
                        field.fieldId(),
                        Types.Field.get(
                                field.fieldId(), update.isOptional(), newName, update.type(), update.doc()));
            }
            return this;
        }

        /**
          * Update nullable for column. only support required type -> optional type
          *
          * @param name name of the column to update
        ...
        public ColumnUpdateChange updateColumnNullability(
                String name, boolean nullable, boolean force) {
            checkColModifyIsLegal(name);
            Types.Field field = internalSchema.findField(name);
            if (field == null) {
                throw new IllegalArgumentException(
            ...
            if (field.isOptional() && !nullable && !force) {
                throw new IllegalArgumentException(
                        "cannot update column Nullability: optional to required");
            }
            // save update info
            Types.Field update = updates.get(field.fieldId());
            if (update == null) {
                updates.put(
                        field.fieldId(),
                        Types.Field.get(field.fieldId(), nullable, field.name(), field.type(), field.doc()));
            } else {
                updates.put(
                        field.fieldId(),
                        Types.Field.get(field.fieldId(), nullable, update.name(), update.type(), update.doc()));
            }
        ...
        protected Integer findIdByFullName(String fullName) {
            ...
            if (field != null) {
                ...
                throw new IllegalArgumentException(
                        String.format("cannot find col id for given column fullName: %s", fullName));
            }
        }
    ...
    public static class ColumnDeleteChange extends TableChange.BaseColumnChange {
        ...
        public BaseColumnChange addPositionChange(String srcId, String dsrId, String orderType) {
            throw new UnsupportedOperationException(
                    "no support add position change for ColumnDeleteChange");
        }
        ...
        public ColumnDeleteChange deleteColumn(String name) {
            ...
            if (field == null) {
                throw new IllegalArgumentException(
                        String.format("cannot delete missing columns: %s", name));
            }
    ...
    public static class ColumnAddChange extends TableChange.BaseColumnChange {
        ...
        public Type applyAdd(Types.Field originalField, Type type) {
            ...
            ArrayList<ColumnPositionChange> pchanges =
                    positionChangeMap.getOrDefault(fieldId, new ArrayList<>());
            ...
            if (!addFields.isEmpty() || !pchanges.isEmpty()) {
                ...
                List<Types.Field> newFields =
                        TableChangesHelper.applyAddChange2Fields(
                                ((Types.RecordType) type).fields(), addFields, pchanges);
                return Types.RecordType.get(newFields);
            }
            return type;
        }
            ...
            if (!parent.isEmpty()) {
                ...
                if (parentField == null) {
                    throw new HoodieSchemaException(
                            String.format("cannot add column: %s which parent: %s is not exist", name, parent));
                }
                ...
                if (newParentField != null) {
                    throw new HoodieSchemaException(
                            String.format("cannot add column: %s which already exist", name));
                }
