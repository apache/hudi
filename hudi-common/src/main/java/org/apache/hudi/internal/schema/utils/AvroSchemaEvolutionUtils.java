...
public class AvroSchemaEvolutionUtils {
    ...
    /**
      * Support reconcile from a new avroSchema. 1) incoming data has missing columns that were already
      * defined in the table –> null values will be injected into missing columns 2) incoming data
      * contains new columns not defined yet in the table -> columns will be added to the table schema
      * (incoming dataframe?) 3) incoming data has missing columns that are already defined in the
      * table and new columns not yet defined in the table -> new columns will be added to the table
      * schema, missing columns will be injected with null values 4) support type change 5) support
      * nested schema change. Notice: the incoming schema should not have delete/rename semantics. for
      * example: incoming schema: int a, int b, int d; oldTableSchema int a, int b, int c, int d we
      * must guarantee the column c is missing semantic, instead of delete semantic.
      *
      * @param incomingSchema implicitly evolution of avro when hoodie write operation
      * @param oldTableSchema old internalSchema
      * @param makeMissingFieldsNullable if true, fields missing from the incoming schema when compared
      *     to the oldTableSchema will become nullable in the result. Otherwise, no updates will be
      *     made to those fields.
      * @return reconcile Schema
      */
    public static InternalSchema reconcileSchema(
            Schema incomingSchema, InternalSchema oldTableSchema, boolean makeMissingFieldsNullable) {
        ...
        InternalSchema inComingInternalSchema =
                convert(incomingSchema, oldTableSchema.getNameToPosition());
        ...
        List<String> diffFromOldSchema =
                colNamesFromOldSchema.stream()
                        .filter(f -> !colNamesFromIncoming.contains(f))
                        .collect(Collectors.toList());
        List<String> diffFromEvolutionColumns =
                colNamesFromIncoming.stream()
                        .filter(f -> !colNamesFromOldSchema.contains(f))
                        .collect(Collectors.toList());
        // check type change.
        List<String> typeChangeColumns =
                colNamesFromIncoming.stream()
                        .filter(
                                f ->
                                        colNamesFromOldSchema.contains(f)
        ...
        if (colNamesFromIncoming.size() == colNamesFromOldSchema.size()
                ...
                && typeChangeColumns.isEmpty()) {
            return oldTableSchema;
        }

        // Remove redundancy from diffFromEvolutionSchema.
        // for example, now we add a struct col in evolvedSchema, the struct col is " user
        // struct<name:string, age:int> "
        // when we do diff operation: user, user.name, user.age will appear in the resultSet which
        // is redundancy, user.name and user.age should be excluded.
        ...
        finalAddAction.entrySet().stream()
                .forEach(
                        f -> {
                                            ...
                                            .filter(
                                                    c ->
                                                            c.lastIndexOf(".") == splitPoint
                                                                    ...
                                                                    && inComingInternalSchema.findIdByName(c)
                                                                            > inComingInternalSchema.findIdByName(name)
                                                                    && oldTableSchema.findIdByName(c) > 0)
                                            .sorted(
                                                    (s1, s2) ->
                                                            oldTableSchema.findIdByName(s1) - oldTableSchema.findIdByName(s2))
                                            .findFirst();
                            addChange.addColumns(
                                    parentName, rawName, inComingInternalSchema.findType(name), null);
        ...
        typeChangeColumns.stream()
                .filter(f -> !inComingInternalSchema.findType(f).isNestedType())
                .forEach(
                        col -> {
    ...
    public static Schema reconcileSchema(
            Schema incomingSchema, Schema oldTableSchema, boolean makeMissingFieldsNullable) {
        return convert(
                reconcileSchema(incomingSchema, convert(oldTableSchema), makeMissingFieldsNullable),
                oldTableSchema.getFullName());
    }

    /**
      * Reconciles nullability and datatype requirements b/w {@code source} and {@code target} schemas,
      * by adjusting these of the {@code source} schema to be in-line with the ones of the {@code
      * target} one. Source is considered to be new incoming schema, while target could refer to prev
      * table schema. For example, if colA in source is non-nullable, but is nullable in target, output
      * schema will have colA as nullable. if "hoodie.datasource.write.new.columns.nullable" is set to
      * true and if colB is not present in source, but is present in target, output schema will have
      * colB as nullable. if colC has different data type in source schema compared to target schema
      * and if its promotable, (say source is int, and target is long and since int can be promoted to
      * long), colC will be long data type in output schema.
      *
      ...
      * @return schema (based off {@code source} one) that has nullability constraints and datatypes
      *     reconciled
      */
    public static Schema reconcileSchemaRequirements(
            Schema sourceSchema, Schema targetSchema, boolean shouldReorderColumns) {
        if (targetSchema.getType() == Schema.Type.NULL || targetSchema.getFields().isEmpty()) {
            return sourceSchema;
        }

        if (sourceSchema == null
                || sourceSchema.getType() == Schema.Type.NULL
                || sourceSchema.getFields().isEmpty()) {
            return targetSchema;
        }

        InternalSchema targetInternalSchema = convert(targetSchema);
        // Use existing fieldIds for consistent field ordering between commits when
        // shouldReorderColumns is true
        InternalSchema sourceInternalSchema =
                convert(
                        sourceSchema,
                        shouldReorderColumns
                                ? targetInternalSchema.getNameToPosition()
        ...
        colNamesSourceSchema.forEach(
                field -> {
                    ...
                    if (colNamesTargetSchema.contains(field)
                            && SchemaChangeUtils.shouldPromoteType(
                                    sourceInternalSchema.findType(field), targetInternalSchema.findType(field))) {
        ...
        if (!typeUpdateColsInSource.isEmpty()) {
            schemaChange =
                    reduce(
                            typeUpdateColsInSource,
                            schemaChange,
                            (change, field) ->
                                    change.updateColumnType(field, targetInternalSchema.findType(field)));
        }

        return convert(
                SchemaChangeUtils.applyTableChanges2Schema(sourceInternalSchema, schemaChange),
