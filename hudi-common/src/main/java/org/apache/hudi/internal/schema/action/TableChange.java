...
/**
  * TableChange subclasses represent requested changes to a table. now only column changes support.
...
public interface TableChange {
    ...
    abstract class BaseColumnChange implements TableChange {
        ...
        /**
          ...
          * @return this
          */
        public BaseColumnChange addPositionChange(
                String srcName, String dsrName, ColumnPositionChange.ColumnPositionType orderType) {
            Integer srcId = findIdByFullName(srcName);
            Option<Integer> dsrIdOpt =
                    dsrName.isEmpty() ? Option.empty() : Option.of(findIdByFullName(dsrName));
            ...
            switch (orderType) {
                ...
                case AFTER:
                    List<String> checkColumns =
                            HoodieRecord.HOODIE_META_COLUMNS.subList(
                                    0, HoodieRecord.HOODIE_META_COLUMNS.size() - 2);
            ...
            int parentId;
            if (srcParentId != null
                    && dsrParentIdOpt.isPresent()
                    && srcParentId.equals(dsrParentIdOpt.get())) {
                Types.Field parentField = internalSchema.findField(srcParentId);
                if (!(parentField.type() instanceof Types.RecordType)) {
                    throw new HoodieSchemaException(
                            String.format(
                                    "only support reorder fields in struct type, but find: %s", parentField.type()));
                }
                parentId = parentField.fieldId();
            } else if (srcParentId == null && !dsrParentIdOpt.isPresent()) {
                parentId = -1;
            } else if (srcParentId != null
                    && !dsrParentIdOpt.isPresent()
                    && orderType.equals(ColumnPositionChange.ColumnPositionType.FIRST)) {
        ...
        /**
          * Abstract method. give a column fullName and return the field id
          *
        ...
        protected void checkColModifyIsLegal(String colNeedToModify) {
            if (HoodieRecord.HOODIE_META_COLUMNS.stream()
                    .anyMatch(f -> f.equalsIgnoreCase(colNeedToModify))) {
                throw new IllegalArgumentException(
                        String.format("cannot modify hudi meta col: %s", colNeedToModify));
            }
        }
    ...
    /**
      * Column position change. now support three change types: FIRST/AFTER/BEFORE FIRST means the
      * specified column should be the first column. AFTER means the specified column should be put
      * after the given column. BEFORE means the specified column should be put before the given
      * column. Note that, the specified column may be a nested field: AFTER/BEFORE means the given
      * columns should in the same struct; FIRST means this field should be the first one within the
      * struct.
      */
    class ColumnPositionChange {
        ...
        static ColumnPositionChange get(int srcId, int dsrId, ColumnPositionType type) {
            switch (type) {
                ...
                default:
                    throw new IllegalArgumentException(
                            String.format("only support first/before/after but found: %s", type));
            }
        }
