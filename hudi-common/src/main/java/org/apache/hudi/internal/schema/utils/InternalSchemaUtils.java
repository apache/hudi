...
/**
  * Util methods to help us do some operations on InternalSchema. eg: column prune, filter rebuild
  * for query engine...
  */
public class InternalSchemaUtils {
                                ...
                                name -> {
                                    ...
                                    if (id == -1) {
                                        throw new IllegalArgumentException(
                                                String.format(
                                                        "cannot prune col: %s which does not exist in hudi table", name));
                                    }
                                    return id;
                                })
                        .collect(Collectors.toList());
        // find top parent field ID. eg: a.b.c, f.g.h, only collect id of a and f ignore all child
        // field.
    ...
    /**
      * Create project internalSchema. support nested project.
      *
      * @param schema a internal schema.
      * @param fieldIds project col field_ids.
      * @return a project internalSchema.
      */
    public static InternalSchema pruneInternalSchemaByID(
            InternalSchema schema, List<Integer> fieldIds, List<Integer> topParentFieldIds) {
        Types.RecordType recordType = (Types.RecordType) pruneType(schema.getRecord(), fieldIds);
        // reorder top parent fields, since the recordType.fields() produced by pruneType maybe out
        // of order.
            ...
            for (int id : topParentFieldIds) {
                ...
                if (f != null) {
                    ...
                    throw new HoodieSchemaException(
                            String.format("cannot find pruned id %s in currentSchema %s", id, schema.toString()));
                }
            }
        }
    ...
    /**
      * Project hudi type by projected cols field_ids this is auxiliary function used by
      * pruneInternalSchema.
      */
    ...
    /**
      ...
      */
    public static String reBuildFilterName(
            String name, InternalSchema fileSchema, InternalSchema querySchema) {
        int nameId = querySchema.findIdByName(name);
        if (nameId == -1) {
            throw new IllegalArgumentException(
                    String.format("cannot find filter col name：%s from querySchema: %s", name, querySchema));
        }
    ...
    /**
      * Collect all type changed cols to build a colPosition -> (newColType, oldColType) map. only
      * collect top level col changed. eg: a is a nest field(record(b int, d long), now a.b is changed
      * from int to long, only a will be collected, a.b will excluded.
      *
                        ...
                        f -> {
                            if (!schema.findType(f).equals(oldSchema.findType(f))) {
                                ...
                                if (fieldNameParts.length == otherFieldNameParts.length
                                    ...
                                    int position =
                                            schema.getRecord().fields().stream()
                                                    .map(s -> s.fieldId())
                                                    .collect(Collectors.toList())
                                                    .indexOf(index);
                                    if (!result.containsKey(position)) {
                                        result.put(
                                                position,
                                                Pair.of(schema.findType(parentName), oldSchema.findType(otherParentName)));
                                    }
                                }
                            }
                        });
