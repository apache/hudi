...
public class HoodieAdaptablePayloadDataGenerator {
    ...
    public static final Schema SCHEMA =
            SchemaTestUtil.getSchemaFromResource(
                    HoodieAdaptablePayloadDataGenerator.class, "/adaptable-payload.avsc");
    ...
    public static List<HoodieRecord> getInserts(
            int n, String partition, long ts, Class<?> payloadClass) throws IOException {
        return getInserts(n, new String[] {partition}, ts, payloadClass);
    }

    public static List<HoodieRecord> getInserts(
            int n, String[] partitions, long ts, Class<?> payloadClass) throws IOException {
    ...
    private static List<GenericRecord> getInserts(
            int n, String[] partitions, long ts, RecordGen recordGen) {
    ...
    public static List<HoodieRecord> getUpdates(
            List<HoodieRecord> baseRecords, long ts, Class<?> payloadClass) throws IOException {
        RecordGen recordGen = new RecordGen(payloadClass);
        ...
        for (HoodieRecord r : baseRecords) {
            ...
            GenericRecord updated =
                    getUpdate(
                            Integer.parseInt(gr.get("id").toString()), gr.get("pt").toString(), ts, recordGen);
    ...
    public static List<HoodieRecord> getUpdates(
            List<HoodieRecord> baseRecords, String newPartition, long ts, Class<?> payloadClass)
            throws IOException {
        RecordGen recordGen = new RecordGen(payloadClass);
        List<HoodieRecord> updates = new ArrayList<>();
        Properties props = new Properties();
        for (HoodieRecord r : baseRecords) {
            GenericRecord gr = (GenericRecord) r.toIndexedRecord(SCHEMA, props).get().getData();
            GenericRecord updated =
                    getUpdate(Integer.parseInt(gr.get("id").toString()), newPartition, ts, recordGen);
            updates.add(getHoodieRecord(updated, recordGen.getPayloadClass()));
        }
        return updates;
    }
    ...
    public static List<HoodieRecord> getDeletes(
            List<HoodieRecord> baseRecords, long ts, Class<?> payloadClass) throws IOException {
        RecordGen recordGen = new RecordGen(payloadClass);
    ...
    public static List<HoodieRecord> getDeletesWithNewPartition(
            List<HoodieRecord> baseRecords, String newPartition, long ts, Class<?> payloadClass)
            throws IOException {
        RecordGen recordGen = new RecordGen(payloadClass);
        List<HoodieRecord> deletes = new ArrayList<>();
        ...
        for (HoodieRecord r : baseRecords) {
            GenericRecord gr = (GenericRecord) r.toIndexedRecord(SCHEMA, props).get().getData();
            GenericRecord deleted =
                    getDelete(Integer.parseInt(gr.get("id").toString()), newPartition, ts, recordGen);
            deletes.add(getHoodieRecord(deleted, recordGen.getPayloadClass()));
        }
        return deletes;
    }
    ...
    public static List<HoodieRecord> getDeletesWithEmptyPayloadAndNewPartition(
            List<HoodieRecord> baseRecords, String newPartition) {
        List<HoodieRecord> deletes = new ArrayList<>();
        for (HoodieRecord r : baseRecords) {
    ...
    private static HoodieRecord getHoodieRecord(GenericRecord r, Class<?> payloadClass)
            throws IOException {
        if (payloadClass == EmptyHoodieRecordPayload.class) {
            return getHoodieRecordForEmptyPayload(
                    new HoodieKey(r.get("id").toString(), r.get("pt").toString()));
        }
        return new HoodieAvroIndexedRecord(r)
                .prependMetaFields(
                        ...
                        new MetadataValues()
                                .setRecordKey(r.get("id").toString())
                                .setPartitionPath(r.get("pt").toString()),
                        new Properties())
    ...
    public static class RecordGen {
        ...
        public static final Set<Class<?>> SUPPORTED_PAYLOAD_CLASSES =
                new HashSet<>(
                        Arrays.asList(
