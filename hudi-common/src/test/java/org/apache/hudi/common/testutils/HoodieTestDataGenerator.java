...
public class HoodieTestDataGenerator implements AutoCloseable {
  ...
  private RawTripTestPayload generateRandomValue(
    ...
    GenericRecord rec = generateGenericRecord(
        ...
        false, isFlattened);
    return new RawTripTestPayload(rec.toString(), key.getRecordKey(), key.getPartitionPath(), extendedSchema.map(Schema::toString).orElse(TRIP_EXAMPLE_SCHEMA));
  }
 ...
 */
  ...
  public Stream<HoodieRecord> generateUniqueDeleteRecordStream(String instantTime, Integer n, String schemaStr) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      used.add(kp);
      try {
        result.add(new HoodieAvroRecord(kp.key, generateRandomValueAsPerSchema(schemaStr, kp.key, instantTime, false, true)));
      } catch (IOException e) {
        throw new HoodieIOException(e.getMessage(), e);
      }
    }
