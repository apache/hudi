 ...
 */
  ...
  public Stream<HoodieRecord> generateInsertsStream(String instantTime, Integer n, boolean isFlattened, String schemaStr, boolean containsAllPartitions,
    ...
    return IntStream.range(0, n).boxed().map(i -> {
      ...
      try {
        return new HoodieAvroIndexedRecord(key, generateRandomValueAsPerSchema(schemaStr, key, instantTime, isFlattened), null,
            Option.of(Collections.singletonMap("InputRecordCount_1506582000", "2")), null);
      } catch (IOException e) {
        throw new HoodieIOException(e.getMessage(), e);
      }
    });
