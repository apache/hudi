...
public class HoodieRecordTestPayload extends OverwriteWithLatestAvroPayload {
    ...
    public Option<IndexedRecord> combineAndGetUpdateValue(
        ...
        if (isDeleted(schema, properties)) {
            ...
            // If orderingField cannot be found, we can not do the compare, then use the natural
            // order.
            ...
            Comparable<?> currentOrderingVal =
                    (Comparable<?>) currentValue.get(currentValue.getSchema().getField(orderingField).pos());
    ...
    protected boolean isDeleteRecord(GenericRecord genericRecord, Properties properties) {
        ...
        ValidationUtils.checkArgument(
                !StringUtils.isNullOrEmpty(properties.getProperty(DELETE_MARKER)),
    ...
    private static Option<Object> updateEventTime(GenericRecord record, Properties properties) {
        ...
        return Option.ofNullable(
                HoodieAvroUtils.getNestedFieldVal(
                        record, eventTimeField, true, consistentLogicalTimestampEnabled));
    }
    ...
    protected boolean needUpdatingPersistedRecord(
        ...
        Object persistedOrderingVal =
                HoodieAvroUtils.getNestedFieldVal(
                        (GenericRecord) currentValue, orderingField, true, consistentLogicalTimestampEnabled);
