...
public final class Transformations {
    ...
    public static Map<String, List<HoodieRecord>> recordsToPartitionRecordsMap(
    ...
    public static <T> List<T> randomSelect(List<T> items, int n) {
        ...
        if (n < 0 || n > s) {
            throw new IllegalArgumentException(
                    String.format("Invalid number of items to select! Valid range for n: [0, %s]", s));
        }
        List<T> selected = new ArrayList<>();
        for (int i = 0, numSelected = 0; i < s && numSelected < n; i += 2, numSelected++) {
            selected.add(items.get(i));
        }
        for (int i = 1, numSelected = selected.size();
                i < s && numSelected < n;
                i += 2, numSelected++) {
            selected.add(items.get(i));
        }
