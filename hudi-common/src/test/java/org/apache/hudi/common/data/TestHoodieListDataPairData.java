...
public class TestHoodieListDataPairData {
    ...
    public void testKeys() {
        assertHoodieDataEquals(
                Arrays.asList(KEY1, KEY1, KEY2, KEY2, KEY3, KEY4), testHoodiePairData.keys());
    }
    ...
    public void testReduceByKey(
            Map<Integer, List<Integer>> expected, Map<Integer, List<Integer>> original) {
    ...
    public void testLeftOuterJoinSingleValuePerKey() {
        ...
        expectedResultMap.put(
                KEY1, Arrays.asList(ImmutablePair.of(STRING_VALUE1, Option.of(INTEGER_VALUE1))));
        expectedResultMap.put(
                KEY2, Arrays.asList(ImmutablePair.of(STRING_VALUE2, Option.of(INTEGER_VALUE2))));
        expectedResultMap.put(KEY3, Arrays.asList(ImmutablePair.of(STRING_VALUE3, Option.empty())));
        expectedResultMap.put(KEY4, Arrays.asList(ImmutablePair.of(STRING_VALUE4, Option.empty())));

        assertEquals(expectedResultMap, toMap(pairData1.leftOuterJoin(pairData2)));
    }
    ...
    public void testLeftOuterJoinMultipleValuesPerKey() {
        ...
        expectedResultMap.put(
                KEY3, Arrays.asList(ImmutablePair.of(STRING_VALUE5, Option.of(INTEGER_VALUE4))));
        expectedResultMap.put(KEY4, Arrays.asList(ImmutablePair.of(STRING_VALUE6, Option.empty())));

        assertEquals(expectedResultMap, toMap(testHoodiePairData.leftOuterJoin(otherPairData)));
    }
    ...
    private static <K, V> Map<K, List<V>> toMap(HoodiePairData<K, V> pairData) {
        return ((List<Pair<K, Iterable<V>>>) pairData.groupByKey().get())
                .stream()
                        .collect(
                                Collectors.toMap(
                                        p -> p.getKey(),
                                        p ->
                                                StreamSupport.stream(p.getValue().spliterator(), false)
                                                        .collect(Collectors.toList())));
    }
