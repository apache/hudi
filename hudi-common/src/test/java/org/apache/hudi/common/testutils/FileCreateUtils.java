...
public class FileCreateUtils extends FileCreateUtilsBase {
  ...
  private static <T> void createMetaFile(HoodieTableMetaClient metaClient, String instantTime, String suffix, Option<T> metadata) throws IOException {
    createMetaFileInTimelinePath(metaClient, instantTime, InProcessTimeGenerator::createNewInstantTime, suffix, metadata.flatMap(m -> metaClient.getCommitMetadataSerDe().serialize(m)));
  }
  ...
  public static void createCommit(HoodieTableMetaClient metaClient, CommitMetadataSerDe commitMetadataSerDe, String instantTime,
    ...
    if (metadata.isPresent()) {
      ...
      createMetaFileInTimelinePath(metaClient, instantTime, completionTimeSupplier, HoodieTimeline.COMMIT_EXTENSION, metaClient.getCommitMetadataSerDe().serialize(commitMetadata));
    } else {
  ...
  public static void createReplaceCommit(HoodieTableMetaClient metaClient, CommitMetadataSerDe commitMetadataSerDe,
    ...
    createMetaFileInTimelinePath(
        ...
        metaClient.getCommitMetadataSerDe().serialize(metadata));
  }
  ...
  public static <T> void createInflightClusterCommit(HoodieTableMetaClient metaClient, CommitMetadataSerDe commitMetadataSerDe,
    ...
    if (inflightReplaceMetadata.isPresent()) {
      createMetaFile(metaClient, instantTime, HoodieTimeline.INFLIGHT_CLUSTERING_COMMIT_EXTENSION,
          inflightReplaceMetadata.flatMap(m -> metaClient.getCommitMetadataSerDe().serialize(m)));
    } else {
      createMetaFile(metaClient, instantTime, HoodieTimeline.INFLIGHT_CLUSTERING_COMMIT_EXTENSION);
    }
  }
