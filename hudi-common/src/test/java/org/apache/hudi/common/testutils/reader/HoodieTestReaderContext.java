...
public class HoodieTestReaderContext extends HoodieReaderContext<IndexedRecord> {
    ...
    public HoodieTestReaderContext(
            Option<HoodieRecordMerger> customMerger, Option<String> payloadClass) {
        ...
        HoodieAvroFileReader reader =
                (HoodieAvroFileReader)
                        HoodieIOFactory.getIOFactory(storage)
                                ...
                                .getFileReader(
                                        new HoodieConfig(), filePath, HoodieFileFormat.PARQUET, Option.empty());
    ...
    public HoodieRecord constructHoodieRecord(
            Option<IndexedRecord> recordOpt, Map<String, Object> metadataMap) {
        String appliedPayloadClass =
                payloadClass.isPresent() ? payloadClass.get() : DefaultHoodieRecordPayload.class.getName();
        if (!recordOpt.isPresent()) {
    ...
    public UnaryOperator<IndexedRecord> projectRecord(
        ...
        if (!renamedColumns.isEmpty()) {
            throw new UnsupportedOperationException(
                    "Schema evolution is not supported for the test reader context");
        }
        Map<String, Integer> fromFields =
                IntStream.range(0, from.getFields().size())
                        .boxed()
                        .collect(Collectors.toMap(i -> from.getFields().get(i).name(), i -> i));
        Map<String, Integer> toFields =
                IntStream.range(0, to.getFields().size())
                        .boxed()
                        .collect(Collectors.toMap(i -> to.getFields().get(i).name(), i -> i));

        // Check if source schema contains all fields from target schema.
        List<Schema.Field> missingFields =
                to.getFields().stream()
                        .filter(f -> !fromFields.containsKey(f.name()))
        ...
        Map<Integer, Integer> fieldMap =
                toFields.entrySet().stream()
                        .filter(e -> fromFields.containsKey(e.getKey()))
                        .collect(Collectors.toMap(e -> fromFields.get(e.getKey()), Map.Entry::getValue));
    ...
    private Object getFieldValueFromIndexedRecord(
            IndexedRecord record, Schema recordSchema, String fieldName) {
