...
/**
 * RFC-109 validation experiment (pure JVM, no Spark / no cluster).
 *
...
public class TestRaBitQResidualHypothesis {
  ...
  public void residualEncodingFixesL2RecallOnLargeNormData() {
    ...
    for (int qi = 0; qi < NUM_QUERIES; qi++) {
      ...
      for (int i = 0; i < N; i++) {
        float symCos = RaBitQDistanceScorer.symmetricCosine(qsFull.getBinaryCode(), fullCodes[i].code, DIM);
        float asymCos = RaBitQDistanceScorer.asymmetricCosine(qsFull.getRotatedQuery(), fullCodes[i].code, DIM);
        distFullSym[i] = RaBitQDistanceScorer.reconstructDistance(
            VectorDistanceMetric.L2, symCos, qsFull.getQueryNorm(), fullCodes[i].scalar);
        distFullAsym[i] = RaBitQDistanceScorer.reconstructDistance(
            VectorDistanceMetric.L2, asymCos, qsFull.getQueryNorm(), fullCodes[i].scalar);
      }
      ...
      for (int i = 0; i < N; i++) {
        ...
        float symCos = RaBitQDistanceScorer.symmetricCosine(qsRes.getBinaryCode(), resCodes[i].code, DIM);
        float asymCos = RaBitQDistanceScorer.asymmetricCosine(qsRes.getRotatedQuery(), resCodes[i].code, DIM);
        distResSym[i] = RaBitQDistanceScorer.reconstructDistance(
            VectorDistanceMetric.L2, symCos, qsRes.getQueryNorm(), resCodes[i].scalar);
        distResAsym[i] = RaBitQDistanceScorer.reconstructDistance(
            VectorDistanceMetric.L2, asymCos, qsRes.getQueryNorm(), resCodes[i].scalar);
      }
