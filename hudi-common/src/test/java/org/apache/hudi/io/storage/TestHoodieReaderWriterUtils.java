...
public class TestHoodieReaderWriterUtils {
    public static void writeHFileForTesting(
            String fileLocation,
            int blockSize,
        ...
        HFileContext context =
                new HFileContextBuilder().withBlockSize(blockSize).withCompression(compressionAlgo).build();
        ...
        try (HFile.Writer writer =
            ...
            for (int i = 0; i < numEntries; i++) {
                ...
                if (!uniqueKeys) {
                    for (int j = 0; j < 20; j++) {
                        writer.append(
                                new KeyValue(keyBytes, null, null, getUTF8Bytes(valueCreator.apply(i) + "_" + j)));
                    }
                }
            }
            writer.appendFileInfo(getUTF8Bytes(CUSTOM_META_KEY), getUTF8Bytes(CUSTOM_META_VALUE));
            writer.appendMetaBlock(
                    HoodieNativeAvroHFileReader.KEY_BLOOM_FILTER_META_BLOCK,
                    new Writable() {
                        @Override
