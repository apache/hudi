...
public class TestHoodieAvroFileReaderFactory {
  ...
  public void testGetFileReader() throws IOException {
    ...
    FileSystem fs = parquetPath.getFileSystem(hadoopConf);
    HoodieFileReader parquetReader = HoodieFileReaderFactory.getReaderFactory(HoodieRecordType.AVRO).getFileReader(hadoopConf, fs, parquetPath, config);
    ...
    final Throwable thrown = assertThrows(UnsupportedOperationException.class, () -> {
      HoodieFileReader logWriter = HoodieFileReaderFactory.getReaderFactory(HoodieRecordType.AVRO).getFileReader(hadoopConf, fs, logPath, config);
    ...
    HoodieFileReader orcReader = HoodieFileReaderFactory.getReaderFactory(HoodieRecordType.AVRO).getFileReader(hadoopConf, fs, orcPath, config);
