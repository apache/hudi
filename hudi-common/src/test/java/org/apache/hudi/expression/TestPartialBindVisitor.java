...
public class TestPartialBindVisitor {
    ...
    public void testPartialBindIfAllExisting() {
        ...
        Predicates.BinaryComparison gt = Predicates.gt(new NameReference("c"), Literal.from(10));
        Predicates.In in =
                Predicates.in(new NameReference("d"), Arrays.asList(Literal.from(10L), Literal.from(13L)));

        Predicates.And expr = Predicates.and(eq, Predicates.or(gt, in));
        Expression binded = expr.accept(partialBindVisitor);

        Assertions.assertTrue(
                (Boolean) binded.eval(new ArrayData(Arrays.asList("Jane", "2023-04-02", 15, 5L, false))));
        Assertions.assertTrue(
                (Boolean) binded.eval(new ArrayData(Arrays.asList("Jane", "2023-04-02", 5, 10L, false))));
        Assertions.assertFalse(
                (Boolean) binded.eval(new ArrayData(Arrays.asList("Lone", "2023-04-02", 15, 5L, false))));
        Assertions.assertFalse(
                (Boolean) binded.eval(new ArrayData(Arrays.asList("Lone", "2023-04-02", 10, 5L, false))));
    }
    ...
    public void testPartialBindIfFieldMissing() {
        ...
        Predicates.BinaryComparison eq = Predicates.eq(new NameReference("a"), Literal.from("Jane"));
        Predicates.BinaryComparison lt = Predicates.lt(new NameReference("m"), Literal.from(10));
        Predicates.BinaryComparison gteq = Predicates.gteq(new NameReference("d"), Literal.from(10L));
