...
GATEWAY_LLM_PROVIDER=anthropic GATEWAY_LLM_MODEL=claude-haiku-4-5-20251001 \
  .venv/bin/hudi-ai-gateway serve
...
GATEWAY_LLM_PROVIDER=ollama GATEWAY_LLM_MODEL=qwen3:8b hudi-ai-gateway serve
```
...
See `hudi-k8s-lakehouse/charts/hudi-ai-gateway` — the product Helm chart —
and `hudi-k8s-lakehouse/local-dev/` for a complete laptop environment
...
| `GATEWAY_TRINO_CATALOG` / `_SCHEMA` / `_USER` | `hudi` / `default` / `hudi-ai-gateway` | query defaults |
...
Adding a tool: write a module under `src/hudi_ai_gateway/tools/` with a
