...
/**
  * This class adds functionality for all query based DDLExecutors. The classes extending it only
  * have to provide runSQL(sql) functions.
  */
        ...
        try {
            this.partitionValueExtractor =
                    (PartitionValueExtractor)
                            Class.forName(config.getStringOrDefault(META_SYNC_PARTITION_EXTRACTOR_CLASS))
                                    .newInstance();
        } catch (Exception e) {
    ...
    private List<String> constructAddPartitions(String tableName, List<String> partitions) {
        ...
        for (int i = 0; i < partitions.size(); i++) {
            ...
            String fullPartitionPath =
                    FSUtils.constructAbsolutePath(config.getString(META_SYNC_BASE_PATH), partitions.get(i))
                            .toString();
            alterSQL
                    .append("  PARTITION (")
        ...
        for (String partition : partitions) {
            ...
            String fullPartitionPath =
                    StorageSchemes.HDFS.getScheme().equals(partitionScheme)
                            ? HadoopFSUtils.getDFSFullPartitionPath(config.getHadoopFileSystem(), partitionPath)
