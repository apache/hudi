...
/**
  * This class offers DDL executor backed by the hive.ql Driver This class preserves the old useJDBC
  * = false way of doing things.
  */
public class HiveQueryDDLExecutor extends QueryBasedDDLExecutor {
    ...
    public HiveQueryDDLExecutor(HiveSyncConfig config, IMetaStoreClient metaStoreClient) {
        ...
        try {
            this.sessionState =
                    new SessionState(
                            config.getHiveConf(), UserGroupInformation.getCurrentUser().getShortUserName());
    ...
    // TODO Duplicating it here from HMSDLExecutor as HiveQueryQL has no way of doing it on its own
    // currently. Need to refactor it
    @Override
        ...
        try {
            for (String dropPartition : partitionsToDrop) {
                if (HivePartitionUtil.partitionExists(
                        metaStoreClient, tableName, dropPartition, partitionValueExtractor, config)) {
                    String partitionClause =
                            HivePartitionUtil.getPartitionClauseForDrop(
                                    dropPartition, partitionValueExtractor, config);
            ...
            throw new HoodieHiveSyncException(
                    tableId(databaseName, tableName) + " drop partition failed", e);
        }
    }
