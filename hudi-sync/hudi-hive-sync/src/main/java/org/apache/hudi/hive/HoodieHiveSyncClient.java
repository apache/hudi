...
/**
  * This class implements logic to sync a Hudi table with either the Hive server or the Hive
  * Metastore.
  */
public class HoodieHiveSyncClient extends HoodieSyncClient {
    ...
    public HoodieHiveSyncClient(HiveSyncConfig config) {
        ...
        // Support JDBC, HiveQL and metastore based implementations for backwards compatibility.
        // Future users should
                ...
                switch (syncMode) {
                    ...
                    default:
                        throw new HoodieHiveSyncException(
                                "Invalid sync mode given " + config.getString(HIVE_SYNC_MODE));
                }
            } else {
                ddlExecutor =
                        config.getBoolean(HIVE_USE_JDBC)
                                ? new JDBCExecutor(config)
                                : new HiveQueryDDLExecutor(config, this.client);
            }
        } catch (Exception e) {
    ...
    public boolean updateTableProperties(String tableName, Map<String, String> tableProperties) {
        ...
        try {
            ...
            throw new HoodieHiveSyncException(
                    "Failed to update table properties for table: " + tableName, e);
        }
    }

    @Override
    public boolean updateSerdeProperties(
            String tableName, Map<String, String> serdeProperties, boolean useRealtimeFormat) {
        ...
        try {
            ...
            if (!shouldUpdate) {
                LOG.debug(
                        "Table "
                                + tableName
                                + " serdeProperties and formatClass already up to date, skip update.");
                return false;
            }
            ...
            storageDescriptor.setSerdeInfo(
                    new SerDeInfo(serdeInfoName, getSerDeClassName(baseFileFormat), serdeProperties));
            client.alter_table(databaseName, tableName, table);
            return true;
        } catch (Exception e) {
            throw new HoodieHiveSyncException(
                    "Failed to update table serde info for table: " + tableName, e);
        }
    }
    ...
    public List<Partition> getAllPartitions(String tableName) {
        try {
            return client.listPartitions(databaseName, tableName, (short) -1).stream()
                    .map(p -> new Partition(p.getValues(), p.getSd().getLocation()))
                    .collect(Collectors.toList());
        } catch (TException e) {
            throw new HoodieHiveSyncException(
                    "Failed to get all partitions for table " + tableId(databaseName, tableName), e);
        }
    }
    ...
    public List<Partition> getPartitionsFromList(String tableName, List<String> partitions) {
        ...
        try {
            ...
            List<FieldSchema> partitionFields =
                    this.getMetastoreFieldSchemas(tableName).stream()
                            .filter(f -> partitionKeys.contains(f.getName()))
                            .collect(Collectors.toList());
            filter = this.generatePushDownFilter(partitions, partitionFields);

            return client.listPartitionsByFilter(databaseName, tableName, filter, (short) -1).stream()
                    .map(p -> new Partition(p.getValues(), p.getSd().getLocation()))
                    .collect(Collectors.toList());
        } catch (TException e) {
            throw new HoodieHiveSyncException(
                    "Failed to get partitions for table "
    ...
    @Override
    public String generatePushDownFilter(
            List<String> writtenPartitions, List<FieldSchema> partitionFields) {
        return new PartitionFilterGenerator()
                .generatePushDownFilter(writtenPartitions, partitionFields, config);
    }
    ...
    public void createOrReplaceTable(
        ...
        try {
            ...
            throw new HoodieHiveSyncException(
                    "failed to create table " + tableId(databaseName, tableName), ex);
        }
    }
    ...
    public Option<String> getLastCommitTimeSynced(String tableName) {
        ...
        try {
            ...
            return Option.ofNullable(
                    table.getParameters().getOrDefault(HOODIE_LAST_COMMIT_TIME_SYNC, null));
        } catch (Exception e) {
            throw new HoodieHiveSyncException(
                    "Failed to get the last commit time synced from the table " + tableName, e);
        }
    }
    ...
    public Option<String> getLastCommitCompletionTimeSynced(String tableName) {
        ...
        try {
            Table table = client.getTable(databaseName, tableName);
            return Option.ofNullable(
                    table.getParameters().getOrDefault(HOODIE_LAST_COMMIT_COMPLETION_TIME_SYNC, null));
        } catch (Exception e) {
            throw new HoodieHiveSyncException(
                    "Failed to get the last commit completion time synced from the table " + tableName, e);
        }
    }
    ...
    public Option<String> getLastReplicatedTime(String tableName) {
        ...
        try {
            Table table = client.getTable(databaseName, tableName);
            return Option.ofNullable(
                    table.getParameters().getOrDefault(GLOBALLY_CONSISTENT_READ_TIMESTAMP, null));
        } catch (NoSuchObjectException e) {
            LOG.warn("the said table not found in hms " + tableId(databaseName, tableName));
            return Option.empty();
        } catch (Exception e) {
            throw new HoodieHiveSyncException(
                    "Failed to get the last replicated time from the table " + tableName, e);
        }
    }
        ...
        if (lastCommitSynced.isPresent()) {
            try {
                ...
                table.putToParameters(
                        HOODIE_LAST_COMMIT_COMPLETION_TIME_SYNC, lastCommitCompletionSynced.get());
                client.alter_table(databaseName, tableName, table);
            } catch (Exception e) {
                throw new HoodieHiveSyncException(
    ...
    public List<FieldSchema> getMetastoreFieldSchemas(String tableName) {
        try {
            return client.getSchema(databaseName, tableName).stream()
                    .map(f -> new FieldSchema(f.getName(), f.getType(), f.getComment()))
                    .collect(Collectors.toList());
        } catch (Exception e) {
            throw new HoodieHiveSyncException(
                    "Failed to get field schemas from metastore for table : " + tableName, e);
        }
    }
    ...
    public List<FieldSchema> getStorageFieldSchemas() {
        try {
            return new TableSchemaResolver(metaClient)
                    .getTableAvroSchema(false).getFields().stream()
    ...
    @Override
    public boolean updateTableComments(
            String tableName, List<FieldSchema> fromMetastore, List<FieldSchema> fromStorage) {
        Map<String, FieldSchema> metastoreMap =
                fromMetastore.stream()
                        .collect(Collectors.toMap(f -> f.getName().toLowerCase(Locale.ROOT), f -> f));
        Map<String, FieldSchema> storageMap =
                fromStorage.stream()
                        .collect(Collectors.toMap(f -> f.getName().toLowerCase(Locale.ROOT), f -> f));
        Map<String, Pair<String, String>> alterComments = new HashMap<>();
        metastoreMap.forEach(
                (name, metastoreFieldSchema) -> {
                    if (storageMap.containsKey(name)) {
                        ...
                        if (updated) {
                            alterComments.put(
                                    name,
                                    Pair.of(
                                            metastoreFieldSchema.getType(), metastoreFieldSchema.getCommentOrEmpty()));
                        }
                    }
                });
    ...
    public void dropTable(String tableName) {
        try {
            ...
            throw new HoodieHiveSyncException(
                    "Failed to delete the table " + tableId(databaseName, tableName), e);
        }
    }
    ...
    public String getTableLocation(String tableName) {
        try {
            ...
            throw new HoodieHiveSyncException(
                    "Failed to get the basepath of the table " + tableId(databaseName, tableName), e);
        }
    }
}
