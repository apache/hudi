...
public class HiveSyncGlobalCommitTool implements HiveSyncGlobalCommit, AutoCloseable {
    ...
    ReplicationStateSync getReplicatedState(boolean forRemote) {
        ...
        hiveConf.addResource(
                new Path(
                        params.loadedProps.getProperty(
                                forRemote ? REMOTE_HIVE_SITE_URI : LOCAL_HIVE_SITE_URI)));
        // TODO: get clusterId as input parameters
        ReplicationStateSync state =
                new ReplicationStateSync(
                        params.mkGlobalHiveSyncProps(forRemote),
                        hiveConf,
                        forRemote ? "REMOTESYNC" : "LOCALSYNC");
        return state;
    }
    ...
    public boolean commit() {
        ...
        try {
            ...
            LOG.error(
                    String.format("Error while trying to commit replication state %s", e.getMessage()), e);
            return false;
        } finally {
            Thread.currentThread().setName(name);
        }
