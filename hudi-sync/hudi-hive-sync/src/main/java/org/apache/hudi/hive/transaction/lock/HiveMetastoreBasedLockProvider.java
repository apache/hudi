...
/**
  ...
  * This {@link LockProvider} implementation allows to lock table operations using hive metastore
  * APIs. Users need to have a HiveMetastore & Zookeeper cluster deployed to be able to use this
  * lock.
  */
public class HiveMetastoreBasedLockProvider implements LockProvider<LockResponse>, Serializable {
    ...
    public HiveMetastoreBasedLockProvider(
            final LockConfiguration lockConfiguration, final StorageConfiguration<?> conf) {
    ...
    public HiveMetastoreBasedLockProvider(
            final LockConfiguration lockConfiguration, final IMetaStoreClient metaStoreClient) {
        this(lockConfiguration);
    ...
    HiveMetastoreBasedLockProvider(final LockConfiguration lockConfiguration) {
        ...
        this.hiveMetastoreUris =
                this.lockConfiguration.getConfig().getOrDefault(HIVE_METASTORE_URI_PROP_KEY, "").toString();
    }
    ...
    public boolean tryLock(long time, TimeUnit unit) {
        ...
        try {
            ...
            throw new HoodieLockException(
                    generateLogStatement(FAILED_TO_ACQUIRE, generateLogSuffixString()), e);
        }
    ...
    public void unlock() {
        try {
            ...
            throw new HoodieLockException(
                    generateLogStatement(FAILED_TO_RELEASE, generateLogSuffixString()), e);
        }
    }

    public void acquireLock(long time, TimeUnit unit)
        ...
        final LockComponent lockComponent =
                new LockComponent(LockType.EXCLUSIVE, LockLevel.TABLE, this.databaseName);
        lockComponent.setTablename(tableName);
        acquireLockInternal(time, unit, lockComponent);
    }

    // NOTE: HiveMetastoreClient does not implement AutoCloseable. Additionally, we cannot call
    // close() after unlock()
    // because if there are multiple operations started from the same WriteClient (via multiple
    // threads), closing the
    // hive client causes all other threads who may have already initiated the tryLock() to fail
    // since the
        ...
        try {
            ...
            LOG.error(
                    generateLogStatement(
                            org.apache.hudi.common.lock.LockState.FAILED_TO_RELEASE, generateLogSuffixString()));
        }
    }
        ...
        try {
            ...
            future =
                    executor.scheduleAtFixedRate(
                            heartbeat, heartbeatIntervalMs / 2, heartbeatIntervalMs, TimeUnit.MILLISECONDS);
    ...
    private void checkRequiredProps(final LockConfiguration lockConfiguration) {
        ValidationUtils.checkArgument(
                lockConfiguration.getConfig().getString(HIVE_DATABASE_NAME_PROP_KEY) != null);
        ValidationUtils.checkArgument(
                lockConfiguration.getConfig().getString(HIVE_TABLE_NAME_PROP_KEY) != null);
    }
    ...
    private void setHiveLockConfs(HiveConf hiveConf) {
        ...
        hiveConf.set(
                ...
                lockConfiguration.getConfig().getString(LOCK_ACQUIRE_NUM_RETRIES_PROP_KEY));
        hiveConf.set(
                "hive.unlock.numretries",
                lockConfiguration.getConfig().getString(LOCK_ACQUIRE_NUM_RETRIES_PROP_KEY));
        hiveConf.set(
                "hive.lock.sleep.between.retries",
                lockConfiguration.getConfig().getString(LOCK_ACQUIRE_RETRY_WAIT_TIME_IN_MILLIS_PROP_KEY));
        String zkConnectUrl =
                lockConfiguration.getConfig().getOrDefault(ZK_CONNECT_URL_PROP_KEY, "").toString();
        ...
        String zkSessionTimeout =
                lockConfiguration.getConfig().getOrDefault(ZK_SESSION_TIMEOUT_MS_PROP_KEY, "").toString();
    ...
    protected String generateLogStatement(
            org.apache.hudi.common.lock.LockState state, String suffix) {
