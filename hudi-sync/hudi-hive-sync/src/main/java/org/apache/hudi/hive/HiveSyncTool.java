...
/**
  ...
  * HiveSyncTool [args]
  *
  * <p>This utility will get the schema from the latest commit and will sync hive table schema Also
  * this will sync the partitions incrementally (all the partitions modified since the last commit)
  */
...
public class HiveSyncTool extends HoodieSyncTool implements AutoCloseable {
    ...
    public HiveSyncTool(Properties props, Configuration hadoopConf) {
        ...
        final Configuration
                hadoopConfForSync; // the configuration to use for this instance of the sync tool
    ...
    protected void initSyncClient(HiveSyncConfig config) {
        try {
            ...
            if (config.getBoolean(HIVE_IGNORE_EXCEPTIONS)) {
                LOG.error(
                        "Got runtime exception when hive syncing, but continuing as ignoreExceptions config is set ",
                        e);
            } else {
        ...
        if (syncClient != null) {
            switch (syncClient.getTableType()) {
                case COPY_ON_WRITE:
                    ...
                    break;
                case MERGE_ON_READ:
                    this.hiveSyncTableStrategy =
                            config.getStringOrDefault(HIVE_SYNC_TABLE_STRATEGY).toUpperCase();
            ...
            if (syncClient != null) {
                LOG.info(
                        "Syncing target hoodie table with hive table("
                                ...
                                + "). Hive metastore URL from HiveConf:"
                                + config.getHiveConf().get(HiveConf.ConfVars.METASTOREURIS.varname)
                                + "). Hive metastore URL from HiveSyncConfig:"
    ...
    protected void syncHoodieTable(
        ...
        // if table exists and location of the metastore table doesn't match the hoodie base path,
        // recreate the table
        ...
        try {
            if (tableExists) {
                ...
                propertiesChanged =
                        syncProperties(tableName, useRealtimeInputFormat, readAsOptimized, schema);
            } else {
    ...
    private void checkAndCreateDatabase() {
        ...
        if (config.getBoolean(HIVE_AUTO_CREATE_DATABASE)) {
            try {
                ...
                // this is harmless since table creation will fail anyways, creation of DB is needed
    ...
    private boolean validateAndSyncPartitions(String tableName, boolean tableExists) {
        ...
        if (syncIncremental) {
            LOG.info(
                    String.format(
                            "Last commit time synced was found to be %s, last commit completion time is found to be %s",
        ...
        if (!lastCommitTimeSynced.isPresent()
            ...
            List<String> writtenPartitionsSince =
                    syncClient.getWrittenPartitionsSince(
            ...
            Set<String> droppedPartitions =
                    syncClient.getDroppedPartitionsSince(
                            lastCommitTimeSynced, lastCommitCompletionTimeSynced);
    ...
    private void recreateAndSyncHiveTable(
            String tableName, boolean useRealtimeInputFormat, boolean readAsOptimized) {
    ...
    private void createOrReplaceTable(
        ...
        syncClient.createOrReplaceTable(
                tableName,
                schema,
    ...
    private boolean syncProperties(
        ...
        boolean serdePropertiesUpdated =
                syncClient.updateSerdeProperties(tableName, serdeProperties, useRealTimeInputFormat);
    ...
    private boolean syncAllPartitions(String tableName) {
        try {
            if (config.shouldNotSyncPartitionMetadata()
    ...
    private boolean syncPartitions(
            String tableName, List<String> writtenPartitionsSince, Set<String> droppedPartitions) {
        try {
            if (config.shouldNotSyncPartitionMetadata()
                    || writtenPartitionsSince.isEmpty()
                    || config.getSplitStrings(META_SYNC_PARTITION_FIELDS).isEmpty()) {
                return false;
            }
