...
public class PartitionFilterGenerator {
    ...
    private static final String UNSUPPORTED_TYPE_ERROR =
            "The value type: %s doesn't support to "
                    + "be pushed down to HMS, acceptable types: "
                    + String.join(",", SUPPORT_TYPES);
    ...
    /**
      ...
      *
      * <p>ex. partitionSchema(date, hour) [Partition(2022-09-01, 12), Partition(2022-09-02, 13)] =>
      ...
      */
    private Expression buildPartitionExpression(
            List<Partition> partitions, List<FieldSchema> partitionFields) {
                        ...
                        partition -> {
                            ...
                            for (int i = 0; i < partitionFields.size(); i++) {
                                ...
                                BinaryExpression exp =
                                        Predicates.eq(
                                                new NameReference(field.getName()),
    ...
    /**
      * Extract partition values from the {@param partitions}, and binding to corresponding partition
      * fieldSchemas.
      */
    private List<Pair<FieldSchema, String[]>> extractFieldValues(
            List<Partition> partitions, List<FieldSchema> partitionFields) {
    ...
    private class ValueComparator implements Comparator<String> {
        ...
        /**
          * As HMS only accept DATE, INT, STRING, BIGINT to push down partition filters, here we only do
          * the comparison for these types.
          */
        @Override
    ...
    /**
      * This method will extract the min value and the max value of each field, and construct
      * GreatThanOrEqual and LessThanOrEqual to build the expression.
      *
      * <p>This method can reduce the Expression tree level a lot if each field has too many values.
      */
    private Expression buildMinMaxPartitionExpression(
            List<Partition> partitions, List<FieldSchema> partitionFields) {
        return extractFieldValues(partitions, partitionFields).stream()
                .map(
                        fieldWithValues -> {
    ...
    public String generatePushDownFilter(
            List<String> writtenPartitions, List<FieldSchema> partitionFields, HiveSyncConfig config) {
                                ...
                                s -> {
                                    ...
                                    if (values.size() != partitionFields.size()) {
                                        throw new HoodieHiveSyncException(
                                                "Partition fields and values should be same length"
                                                        + ", but got partitionFields: "
                                                        + partitionFields
                                                        + " with values: "
                                                        + values);
                                    }
