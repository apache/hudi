...
/**
  * For all tests, we need to set LockComponent.setOperationType(DataOperationType.NO_TXN). This is
  * needed because of this -> https://github.com/apache/hive/blob/master/standalone-metastore
  * /metastore-server/src/main/java/org/apache/hadoop/hive/metastore/txn/TxnHandler.java#L2892 Unless
  * this is set, we cannot use HiveMetastore server in tests for locking use-cases.
  */
@Tag("functional")
public class TestHiveMetastoreBasedLockProvider extends HiveSyncFunctionalTestHarness {
    ...
    private LockComponent lockComponent =
            new LockComponent(LockType.EXCLUSIVE, LockLevel.TABLE, TEST_DB_NAME);
    ...
    public void init() throws Exception {
        ...
        properties.setProperty(
                ...
                DEFAULT_LOCK_ACQUIRE_RETRY_WAIT_TIME_IN_MILLIS);
        ...
        properties.setProperty(
                ZK_SESSION_TIMEOUT_MS_PROP_KEY, hiveConf().get("hive.zookeeper.session.timeout"));
        properties.setProperty(
                ZK_CONNECTION_TIMEOUT_MS_PROP_KEY, String.valueOf(DEFAULT_ZK_CONNECTION_TIMEOUT_MS));
    ...
    public void testAcquireLock() throws Exception {
        HiveMetastoreBasedLockProvider lockProvider =
                new HiveMetastoreBasedLockProvider(lockConfiguration, storageConf());
        lockComponent.setOperationType(DataOperationType.NO_TXN);
        Assertions.assertTrue(
                lockProvider.acquireLock(
                        lockConfiguration.getConfig().getLong(LOCK_ACQUIRE_WAIT_TIMEOUT_MS_PROP_KEY),
    ...
    public void testReentrantLock() throws Exception {
        ...
        HiveMetastoreBasedLockProvider lockProvider1 =
                new HiveMetastoreBasedLockProvider(lockConfiguration, storageConf());
        HiveMetastoreBasedLockProvider lockProvider2 =
                new HiveMetastoreBasedLockProvider(lockConfiguration, storageConf());
        lockComponent.setOperationType(DataOperationType.NO_TXN);
        Assertions.assertTrue(
                lockProvider1.acquireLock(
                        lockConfiguration.getConfig().getLong(LOCK_ACQUIRE_WAIT_TIMEOUT_MS_PROP_KEY),
                        TimeUnit.MILLISECONDS,
                        lockComponent));
        try {
            boolean acquireStatus =
                    lockProvider2.acquireLock(
                            lockConfiguration.getConfig().getLong(LOCK_ACQUIRE_WAIT_TIMEOUT_MS_PROP_KEY),
                            TimeUnit.MILLISECONDS,
                            lockComponent);
    ...
    public void testWaitingLock() throws Exception {
        ...
        HiveMetastoreBasedLockProvider lockProvider3 =
                new HiveMetastoreBasedLockProvider(lockConfiguration, storageConf());
        boolean acquireStatus =
                lockProvider3.acquireLock(
                        lockConfiguration.getConfig().getLong(LOCK_ACQUIRE_WAIT_TIMEOUT_MS_PROP_KEY),
                        TimeUnit.MILLISECONDS,
                        lockComponent);
