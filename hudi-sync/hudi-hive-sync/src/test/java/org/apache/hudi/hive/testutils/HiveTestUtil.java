...
public class HiveTestUtil {
    ...
    public static void setUp(
            Option<TypedProperties> hiveSyncProperties, boolean shouldClearBasePathAndTables)
            throws Exception {
        ...
        if (hiveSyncProperties.isPresent()) {
            ...
            basePath =
                    Files.createTempDirectory("hivesynctest" + Instant.now().toEpochMilli())
                            .toUri()
                            .toString();
    ...
    public static void commitToTable(
        ...
        HoodieCommitMetadata commitMetadata =
                createPartitions(
                        numberOfPartitions, true, useSchemaFromCommitMetadata, dateTime, instantTime, basePath);
    ...
    public static void removeCommitFromActiveTimeline(String instantTime, String actionType) {
        ...
        try {
            StoragePath completeInstantPath =
                    HoodieTestUtils.getCompleteInstantPath(storage, metaFolderPath, instantTime, actionType);
    ...
    public static void addRollbackInstantToTable(String instantTime, String commitToRollback)
        ...
        createMetaFile(
                basePath, HoodieTimeline.makeRequestedRollbackFileName(instantTime), getUTF8Bytes(""));
        createMetaFile(
                basePath, HoodieTimeline.makeInflightRollbackFileName(instantTime), getUTF8Bytes(""));
        createMetaFile(
                basePath,
                HoodieTimeline.makeRollbackFileName(
    ...
    public static void createMORTable(
            String commitTime,
        ...
        HoodieCommitMetadata commitMetadata =
                createPartitions(
                        numberOfPartitions, true, useSchemaFromCommitMetadata, dateTime, commitTime, basePath);
        ...
        if (createDeltaCommit) {
            ...
            HoodieCommitMetadata deltaMetadata =
                    createLogFiles(
                            commitMetadata.getPartitionToWriteStats(), true, useSchemaFromCommitMetadata);
            createDeltaCommitFile(deltaMetadata, deltaCommitTime);
        }
    }

    public static void addCOWPartitions(
            int numberOfPartitions,
            boolean isParquetSchemaSimple,
            boolean useSchemaFromCommitMetadata,
    ...
    public static void addCOWPartitions(
        ...
        HoodieCommitMetadata commitMetadata =
                createPartitions(
                        partitions, schemaPath, dataPath, useSchemaFromCommitMetadata, instantTime, basePath);
        createdTablesSet.add(DB_NAME + "." + TABLE_NAME);
        createCommitFile(commitMetadata, instantTime, basePath);
    }

    public static void addCOWPartition(
        ...
        HoodieCommitMetadata commitMetadata =
                createPartition(
                        partitionPath, isParquetSchemaSimple, useSchemaFromCommitMetadata, instantTime);
        createdTablesSet.add(DB_NAME + "." + TABLE_NAME);
        createCommitFile(commitMetadata, instantTime, basePath);
    }

    public static void addMORPartitions(
            int numberOfPartitions,
            boolean isParquetSchemaSimple,
    ...
    private static HoodieCommitMetadata createLogFiles(
            Map<String, List<HoodieWriteStat>> partitionWriteStats,
            boolean isLogSchemaSimple,
            boolean useSchemaFromCommitMetadata)
    ...
    private static HoodieCommitMetadata createPartitions(
            int numberOfPartitions,
            boolean isParquetSchemaSimple,
            boolean useSchemaFromCommitMetadata,
            ZonedDateTime startFrom,
        ...
        for (int i = 0; i < numberOfPartitions; i++) {
            ...
            List<HoodieWriteStat> writeStats =
                    createTestData(partPath, isParquetSchemaSimple, instantTime);
    ...
    private static HoodieCommitMetadata createPartitions(
        ...
        for (String partitionPath : partitionsPaths) {
            ...
            FileCreateUtils.createPartitionMetaFile(basePath, partitionPath);
            List<HoodieWriteStat> writeStats =
                    createTestData(partPath, schemaPath, dataPath, instantTime);
            writeStats.forEach(s -> commitMetadata.addWriteStat(partitionPath, s));
        }
        if (useSchemaFromCommitMetadata) {
    ...
    private static HoodieCommitMetadata createPartition(
            String partitionPath,
            boolean isParquetSchemaSimple,
            boolean useSchemaFromCommitMetadata,
            String instantTime)
    ...
    private static List<HoodieWriteStat> createTestData(
            Path partPath, boolean isParquetSchemaSimple, String instantTime)
            throws IOException, URISyntaxException {
        List<HoodieWriteStat> writeStats = new ArrayList<>();
        for (int i = 0; i < 5; i++) {
            // Create 5 files
    ...
    private static List<HoodieWriteStat> createTestData(
            Path partPath, String schemaPath, String dataPath, String instantTime)
            throws IOException, URISyntaxException {
        List<HoodieWriteStat> writeStats = new ArrayList<>();
        for (int i = 0; i < 5; i++) {
            // Create 5 files
    ...
    private static HoodieLogFile generateLogData(
            StoragePath parquetFilePath, boolean isLogSchemaSimple)
        ...
        Writer logWriter =
                HoodieLogFormat.newWriterBuilder()
                        .onParentPath(parquetFilePath.getParent())
                        .withFileExtension(HoodieLogFile.DELTA_EXTENSION)
                        .withFileId(dataFile.getFileId())
                        .withDeltaCommit(dataFile.getCommitTime())
                        .withStorage(storage)
                        .build();
        List<HoodieRecord> records =
                (isLogSchemaSimple
                        ...
                        .stream().map(HoodieAvroIndexedRecord::new).collect(Collectors.toList());
    ...
    private static HoodieLogFile generateLogData(
            StoragePath parquetFilePath, String logSchemaPath, String dataPath)
            throws IOException, InterruptedException, URISyntaxException {
        ...
        Writer logWriter =
                HoodieLogFormat.newWriterBuilder()
                        ...
                        .build();
        List<HoodieRecord> records =
                SchemaTestUtil.generateTestRecords(logSchemaPath, dataPath).stream()
    ...
    private static void addSchemaToCommitMetadata(
            HoodieCommitMetadata commitMetadata, String schema, boolean useSchemaFromCommitMetadata) {
        if (useSchemaFromCommitMetadata) {
    ...
    private static void addSchemaToCommitMetadata(
            HoodieCommitMetadata commitMetadata, String schemaPath) throws IOException {
    ...
    public static void createCommitFile(
            HoodieCommitMetadata commitMetadata, String instantTime, String basePath) throws IOException {
        createMetaFile(
                basePath,
    ...
    public static void createReplaceCommitFile(
            HoodieReplaceCommitMetadata commitMetadata, String instantTime) throws IOException {
        createMetaFile(
                basePath,
                HoodieTimeline.makeReplaceFileName(
                        instantTime + "_" + InProcessTimeGenerator.createNewInstantTime()),
                serializeCommitMetadata(commitMetadata).get());
    }

    public static void createCommitFileWithSchema(
            HoodieCommitMetadata commitMetadata, String instantTime, boolean isSimpleSchema)
            throws IOException {
    ...
    private static void createCompactionCommitFile(
            HoodieCommitMetadata commitMetadata, String instantTime) throws IOException {
        createMetaFile(
                basePath,
                HoodieTimeline.makeCommitFileName(
                        instantTime + "_" + InProcessTimeGenerator.createNewInstantTime()),
    ...
    private static void createDeltaCommitFile(
            HoodieCommitMetadata deltaCommitMetadata, String deltaCommitTime) throws IOException {
        createMetaFile(
                basePath,
                HoodieTimeline.makeDeltaFileName(
                        deltaCommitTime + "_" + InProcessTimeGenerator.createNewInstantTime()),
