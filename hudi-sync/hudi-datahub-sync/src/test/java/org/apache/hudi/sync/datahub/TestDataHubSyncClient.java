...
public class TestDataHubSyncClient {
    ...
    @Mock RestEmitter restEmitterMock;
    ...
    public void testUpdateTableSchemaInvokesRestEmitter() throws IOException {
        Properties props = new Properties();
        props.put(
                META_SYNC_PARTITION_EXTRACTOR_CLASS.key(), DummyPartitionValueExtractor.class.getName());
        props.put(META_SYNC_BASE_PATH.key(), tableBasePath);

        Mockito.when(restEmitterMock.emit(any(MetadataChangeProposalWrapper.class), Mockito.any()))
        ...
        verify(restEmitterMock, times(2)).emit(any(MetadataChangeProposalWrapper.class), Mockito.any());
    }
