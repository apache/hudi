...
public class DataHubSyncClient extends HoodieSyncClient {
    ...
    public void updateLastCommitTimeSynced(String tableName) {
        updateTableProperties(
                ...
                Collections.singletonMap(
                        HOODIE_LAST_COMMIT_TIME_SYNC, getActiveTimeline().lastInstant().get().getTimestamp()));
    }
    ...
    public boolean updateTableProperties(String tableName, Map<String, String> tableProperties) {
        ...
        try (RestEmitter emitter = config.getRestEmitter()) {
            ...
            throw new HoodieDataHubSyncException(
                    "Fail to change properties for Dataset " + datasetUrn + ": " + tableProperties, e);
        }
    }
    ...
    public void updateTableSchema(String tableName, MessageType schema) {
        try (RestEmitter emitter = config.getRestEmitter()) {
            ...
            // When updating an entity, it is necessary to set its soft-delete status to false, or
            // else the update won't get
            // reflected in the UI.
                        ...
                        .map(
                                f ->
                                        new SchemaField()
                                                ...
                                                .setNativeDataType(f.schema().getType().getName()))
