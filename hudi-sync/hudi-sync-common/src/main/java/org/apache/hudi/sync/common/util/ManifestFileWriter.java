...
public class ManifestFileWriter {
    ...
    protected ManifestFileWriter(
            HoodieTableMetaClient metaClient, boolean useFileListingFromMetadata) {
    ...
    public synchronized void writeManifestFile(boolean useAbsolutePath) {
        try {
            List<String> baseFiles =
                    fetchLatestBaseFilesForAllPartitions(useAbsolutePath).collect(Collectors.toList());
    ...
    public Stream<String> fetchLatestBaseFilesForAllPartitions(boolean useAbsolutePath) {
        try {
            ...
            boolean canUseMetadataTable =
                    useFileListingFromMetadata && metaClient.getTableConfig().isMetadataTableAvailable();
    ...
    public StoragePath getManifestFolder(boolean useAbsolutePath) {
        return new StoragePath(
                ...
                useAbsolutePath ? ABSOLUTE_PATH_MANIFEST_FOLDER_NAME : MANIFEST_FOLDER_NAME);
    }

    @VisibleForTesting
    static Stream<String> getLatestBaseFiles(
            boolean canUseMetadataTable,
            HoodieEngineContext engContext,
            HoodieTableMetaClient metaClient,
            boolean useAbsolutePath) {
        List<String> partitions =
                FSUtils.getAllPartitionPaths(
                        engContext, metaClient.getStorage(), metaClient.getBasePath(), canUseMetadataTable);
        ...
        try {
            ...
            if (canUseMetadataTable) {
                // incase of MDT, we can load all partitions at once. If not for MDT, we can rely on
                // fsView.getLatestBaseFiles(partition) for each partition to load from FS.
                fsView.loadAllPartitions();
            }
            HoodieTableFileSystemView finalFsView = fsView;
            // if we do not collect and return stream directly, lazy evaluation happens and we end
            // up closing the fsview in finally block which later
                    ...
                    .flatMap(
                            partition ->
                                    finalFsView
                                            ...
                                            .map(useAbsolutePath ? HoodieBaseFile::getPath : HoodieBaseFile::getFileName))
    ...
    public static class Builder {
        ...
        public ManifestFileWriter build() {
            ValidationUtils.checkArgument(
                    metaClient != null, "MetaClient needs to be set to init ManifestFileGenerator");
