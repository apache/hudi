...
/**
  * Convert the parquet schema to spark schema' json string. This code is refer to
  * org.apache.spark.sql.execution.datasources.parquet.ParquetToSparkSchemaConverter in spark
  * project.
  */
                                    ...
                                    switch (field.getRepetition()) {
                                        ...
                                        default:
                                            throw new UnsupportedOperationException(
                                                    "Unsupport convert " + field + " to spark sql type");
                                    }
                                })
                        .reduce((a, b) -> a + "," + b)
                        .orElse("");
        ...
        switch (typeName) {
            ...
            case INT32:
                ...
                switch (originalType) {
                    ...
                    case DECIMAL:
                        return "decimal("
                                + field.getDecimalMetadata().getPrecision()
                                + ","
                                + field.getDecimalMetadata().getScale()
                                + ")";
                    default:
                        throw new UnsupportedOperationException(
                                "Unsupport convert " + typeName + " to spark sql type");
                }
            case INT64:
                if (originalType == null) {
                    return "long";
        ...
        switch (field.getOriginalType()) {
            ...
            case MAP_KEY_VALUE:
                ...
                return "{\"type\":\"map\", \"keyType\":"
                        + convertFieldType(keyType)
                        + ",\"valueType\":"
                        + convertFieldType(valueType)
                        + ",\"valueContainsNull\":"
                        + valueOptional
                        + "}";
            default:
