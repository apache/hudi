...
public class SyncUtilHelpers {
    ...
    // Locks for each table (base path) to avoid concurrent modification of the same underneath meta
    // storage.
    // Meta store such as Hive may encounter {@code ConcurrentModificationException} for
    // #alter_table.
    private static final ConcurrentHashMap<String, Lock> TABLE_LOCKS = new ConcurrentHashMap<>();

    /**
      * Create an instance of an implementation of {@link HoodieSyncTool} that will sync all the
      * relevant meta information with an external metastore such as Hive etc. to ensure Hoodie tables
      * can be queried or read via external systems.
      *
      ...
      */
    public static void runHoodieMetaSync(
            String syncToolClassName,
        ...
        try {
            try (HoodieSyncTool syncTool =
                    instantiateMetaSyncTool(
                            syncToolClassName, props, hadoopConfig, fs, targetBasePath, baseFileFormat)) {
                syncTool.syncHoodieTable();
            } catch (Throwable e) {
                throw new HoodieMetaSyncException(
                        "Could not sync using the meta sync class " + syncToolClassName, e);
            }
        } finally {
            tableLock.unlock();
        }
    ...
    static HoodieSyncTool instantiateMetaSyncTool(
            String syncToolClassName,
            TypedProperties props,
            Configuration hadoopConfig,
            FileSystem fs,
        ...
        if (properties.containsKey(META_SYNC_TABLE_NAME.key())) {
            ...
            if (!tableName.equals(tableName.toLowerCase())) {
                LOG.warn(
                        "Table name \""
                                + tableName
                                + "\" contains capital letters. Your metastore may automatically convert this to lower case and can cause table not found errors during subsequent syncs.");
            }
        }

        if (ReflectionUtils.hasConstructor(
                syncToolClassName, new Class<?>[] {Properties.class, Configuration.class})) {
            return ((HoodieSyncTool)
                    ReflectionUtils.loadClass(
                            syncToolClassName,
                            new Class<?>[] {Properties.class, Configuration.class},
                            properties,
                            hadoopConfig));
        } else if (ReflectionUtils.hasConstructor(
                syncToolClassName, new Class<?>[] {Properties.class})) {
            return ((HoodieSyncTool)
                    ReflectionUtils.loadClass(
                            syncToolClassName, new Class<?>[] {Properties.class}, properties));
        } else if (ReflectionUtils.hasConstructor(
                syncToolClassName,
            ...
            return ((HoodieSyncTool)
                ...
                syncToolClassName, new Class<?>[] {Properties.class, FileSystem.class})) {
            return ((HoodieSyncTool)
                    ReflectionUtils.loadClass(
                            syncToolClassName,
            ...
            throw new HoodieException(
                    "Could not load meta sync class " + syncToolClassName + ": no valid constructor found.");
        }
    }

    public static HoodieException getHoodieMetaSyncException(
