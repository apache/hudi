...
public class HoodieMetaSyncMetrics {
        ...
        if (metricsConfig.isMetricsOn()) {
            this.storage =
                    HoodieStorageUtils.getStorage(
                            config.getBasePath(), HadoopFSUtils.getStorageConf(config.getHadoopConf()));
            ...
            recreateAndSyncFailureCounterName =
                    getMetricsName(COUNTER_ACTION, META_SYNC_RECREATE_TABLE_FAILURE_METRIC);
        }
    }
    ...
    public void incrementRecreateAndSyncFailureCounter() {
        recreateAndSyncFailureCounter =
                getCounter(recreateAndSyncFailureCounter, recreateAndSyncFailureCounterName);
    ...
    public void updateRecreateAndSyncDurationInMs(long durationInNs) {
        if (metricsConfig.isMetricsOn()) {
            ...
            metrics.registerGauge(
                    getMetricsName(META_SYNC_ACTION, RECREATE_TABLE_DURATION_MS_METRIC), durationInMs);
        }
    }
    ...
    public String getMetricsName(String action, String metric) {
        if (StringUtils.isNullOrEmpty(metricsConfig.getMetricReporterMetricsNamePrefix())) {
            ...
            return String.format(
                    "%s.%s.%s.%s",
                    metricsConfig.getMetricReporterMetricsNamePrefix(), action, metric, syncToolName);
        }
    }
