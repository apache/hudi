...
/**
  * Adb sync tool is mainly used to sync hoodie tables to Alibaba Cloud AnalyticDB(ADB), it can be
  * used as API `AdbSyncTool.syncHoodieTable(AdbSyncConfig)` or as command line `java -cp
  * hoodie-hive.jar AdbSyncTool [args]`
  *
  * <p>This utility will get the schema from the latest commit and will sync ADB table schema,
...
public class AdbSyncTool extends HoodieSyncTool {
    ...
    public AdbSyncTool(Properties props) {
        ...
        switch (syncClient.getTableType()) {
            ...
            case MERGE_ON_READ:
                ...
                this.roTableTableName =
                        config.getBoolean(ADB_SYNC_SKIP_RO_SUFFIX)
                                ? Option.of(tableName)
                                : Option.of(tableName + SUFFIX_READ_OPTIMIZED_TABLE);
                break;
            default:
                throw new HoodieAdbSyncException(
                        "Unknown table type:"
                                + syncClient.getTableType()
                                + ", basePath:"
    ...
    public void syncHoodieTable() {
        try {
            ...
            throw new HoodieAdbSyncException(
                    "Sync hoodie table to ADB failed, tableName:" + tableName, re);
        } finally {
            syncClient.close();
        }
    }

    private void syncHoodieTable(
            String tableName, boolean useRealtimeInputFormat, boolean readAsOptimized) throws Exception {
        LOG.info(
                "Try to sync hoodie table, tableName:{}, path:{}, tableType:{}",
                tableName,
                syncClient.getBasePath(),
                syncClient.getTableType());
    ...
    /**
      * Get the latest schema from the last commit and check if its in sync with the ADB table schema.
    ...
    private void syncSchema(
        ...
        Map<String, String> tableProperties =
                ConfigUtils.toMap(config.getString(ADB_SYNC_TABLE_PROPERTIES));
        Map<String, String> serdeProperties =
                ConfigUtils.toMap(config.getString(ADB_SYNC_SERDE_PROPERTIES));
        if (config.getBoolean(ADB_SYNC_SYNC_AS_SPARK_DATA_SOURCE_TABLE)) {
            Map<String, String> sparkTableProperties =
                    SparkDataSourceTableUtils.getSparkTableProperties(
                            ...
                            config.getInt(ADB_SYNC_SCHEMA_STRING_LENGTH_THRESHOLD),
                            schema);
            Map<String, String> sparkSerdeProperties =
                    SparkDataSourceTableUtils.getSparkSerdeProperties(
                            readAsOptimized, config.getString(META_SYNC_BASE_PATH));
            tableProperties.putAll(sparkTableProperties);
            serdeProperties.putAll(sparkSerdeProperties);
            LOG.info(
                    "Sync as spark datasource table, tableName:{}, tableExists:{}, tableProperties:{}, sederProperties:{}",
                    tableName,
                    tableExists,
                    tableProperties,
                    serdeProperties);
        ...
        if (!tableExists) {
            ...
            String inputFormatClassName =
                    HoodieInputFormatUtils.getInputFormatClassName(
                            HoodieFileFormat.PARQUET, useRealTimeInputFormat);
            ...
            SchemaDifference schemaDiff =
                    HiveSchemaUtil.getSchemaDifference(
                            schema,
                            tableSchema,
                            config.getSplitStrings(META_SYNC_PARTITION_FIELDS),
    ...
    /**
      * Syncs the list of storage partitions passed in (checks if the partition is in adb, if not adds
