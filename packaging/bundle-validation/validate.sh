...
test_spark_hadoop_mr_bundles () {
    ...
    $SPARK_HOME/bin/spark-shell --jars $JARS_DIR/spark.jar \
      ...
      --conf 'spark.sql.catalog.spark_catalog=org.apache.spark.sql.hudi.catalog.HoodieCatalog' < $WORKDIR/spark_hadoop_mr/write.scala
    ...
    $SPARK_HOME/bin/spark-shell --jars $JARS_DIR/spark.jar \
      ...
      --conf 'spark.serializer=org.apache.spark.serializer.KryoSerializer' \
      --conf 'spark.kryo.registrator=org.apache.spark.HoodieSparkKryoRegistrar' \
      --conf 'spark.sql.catalog.spark_catalog=org.apache.spark.sql.hudi.catalog.HoodieCatalog' < $WORKDIR/spark_hadoop_mr/validate.scala
