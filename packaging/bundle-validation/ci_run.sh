...
STAGING_REPO_NUM=$3
MODE=$4 # if MODE is java17 then build on docker and run docker_test_java17.sh

echo "HUDI_VERSION: $HUDI_VERSION JAVA_RUNTIME_VERSION: $JAVA_RUNTIME_VERSION"
echo "SPARK_RUNTIME: $SPARK_RUNTIME SPARK_PROFILE (optional): $SPARK_PROFILE  SCALA_PROFILE (optional): $SCALA_PROFILE"
...
if [[ -z "$MODE" ]] || [[ "$MODE" != "java17" ]]; then
  ...
  docker run -v ${GITHUB_WORKSPACE}:/opt/bundle-validation/docker-test \
    -i hudi-ci-bundle-validation:$IMAGE_TAG bash docker_test_java17.sh $JAVA_RUNTIME_VERSION $SPARK_PROFILE $SCALA_PROFILE
fi
