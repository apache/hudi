...
if [[ ${SPARK_RUNTIME} == 'spark3.3.1' ]]; then
...
elif [[ ${SPARK_RUNTIME} == 'spark3.3.4' && "${FLINK_PROFILE}" == 'flink1.14' ]]; then
  HADOOP_VERSION=2.7.7
  HIVE_VERSION=3.1.3
  DERBY_VERSION=10.14.1.0
  FLINK_VERSION=1.14.6
  ...
  CONFLUENT_VERSION=5.5.12
  KAFKA_CONNECT_HDFS_VERSION=10.1.13
  IMAGE_TAG=flink1146hive313spark334
elif [[ ${SPARK_RUNTIME} == 'spark3.3.4' && "${FLINK_PROFILE}" == 'flink1.15' ]]; then
  HADOOP_VERSION=2.7.7
  HIVE_VERSION=3.1.3
  DERBY_VERSION=10.14.1.0
