...
permalink: /docs/0.5.0-writing_data.html
...
speeding up large Spark jobs via upserts using the [Hudi datasource](#datasource-writer). Such datasets can then be [queried](/docs/0.5.0-querying_data.html) using various query engines.
...
In some cases, you may want to migrate your existing dataset into Hudi beforehand. Please refer to [migration guide](/docs/0.5.0-migration_guide.html). 
...
Here are some ways to efficiently manage the storage of your Hudi datasets.
 ...
 - The [small file handling feature](/docs/0.5.0-configurations.html#compactionSmallFileSize) in Hudi, profiles incoming workload 
   and distributes inserts to existing file groups instead of creating new file groups, which can lead to small files. 
 - Cleaner can be [configured](/docs/0.5.0-configurations.html#retainCommits) to clean up older file slices, more or less aggressively depending on maximum time for queries to run & lookback needed for incremental pull
 - User can also tune the size of the [base/parquet file](/docs/0.5.0-configurations.html#limitFileSize), [log files](/docs/0.5.0-configurations.html#logFileMaxSize) & expected [compression ratio](/docs/0.5.0-configurations.html#parquetCompressionRatio), 
 ...
 - Intelligently tuning the [bulk insert parallelism](/docs/0.5.0-configurations.html#withBulkInsertParallelism), can again in nicely sized initial file groups. It is in fact critical to get this right, since the file groups
   once created cannot be deleted, but simply expanded as explained before.
 - For workloads with heavy updates, the [merge-on-read storage](/docs/0.5.0-concepts.html#merge-on-read-storage) provides a nice mechanism for ingesting quickly into smaller files and then later merging them into larger base files via compaction.
