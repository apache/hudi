...
permalink: /cn/docs/0.5.3-querying_data.html
...
从概念上讲，Hudi物理存储一次数据到DFS上，同时在其上提供三个逻辑视图，如[之前](/cn/docs/0.5.3-concepts.html#views)所述。
数据集同步到Hive Metastore后，它将提供由Hudi的自定义输入格式支持的Hive外部表。一旦提供了适当的Hudi捆绑包，
就可以通过Hive、Spark和Presto之类的常用查询引擎来查询数据集。

具体来说，在写入过程中传递了两个由[table name](/cn/docs/0.5.3-configurations.html#TABLE_NAME_OPT_KEY)命名的Hive表。
...
并与其他表（数据集/维度）结合以[写出增量](/cn/docs/0.5.3-writing_data.html)到目标Hudi数据集。增量视图是通过查询上表之一实现的，并具有特殊配置，
...
请参阅[设置](/cn/docs/0.5.3-configurations.html#spark-datasource)部分，以查看所有数据源选项。
