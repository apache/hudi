...
public class JsonStringToHoodieRecordMapFunction implements MapFunction<String, HoodieRecord> {
  ...
  public JsonStringToHoodieRecordMapFunction(TypedProperties props, String schemaStr) {
    this.props = props;
    this.schemaStr = Option.of(schemaStr);
    init();
  }
  ...
  private void init() {
    ...
    this.payloadClassName = props.getString(HoodieDataSourceConfig.WRITE_PAYLOAD_CLASS,
        OverwriteWithLatestAvroPayload.class.getName());
    this.orderingField = props.getString(HoodieDataSourceConfig.PRECOMBINE_FIELD_PROP, "ts");
    try {
      ...
      throw new HoodieFlinkStreamerException(String.format("KeyGenerator %s initialization failed",
          props.getString(HoodieDataSourceConfig.KEYGENERATOR_CLASS_PROP, SimpleAvroKeyGenerator.class.getName())), e);
    }
  }
}
