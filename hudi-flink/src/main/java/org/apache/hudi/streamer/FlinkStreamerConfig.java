...
public class FlinkStreamerConfig extends Configuration {
  ...
  public String compactionTriggerStrategy = FlinkCompactionOptions.NUM_COMMITS;
  ...
  public static org.apache.flink.configuration.Configuration toFlinkConfig(FlinkStreamerConfig config) {
    ...
    conf.setString(FlinkWriteOptions.TABLE_NAME, config.targetTableName);
    // copy_on_write works same as COPY_ON_WRITE
    conf.setString(FlinkWriteOptions.TABLE_TYPE, config.tableType.toUpperCase());
    conf.setBoolean(FlinkWriteOptions.INSERT_DEDUP, config.insertDedup);
    conf.setString(FlinkWriteOptions.OPERATION, config.operation.value());
    conf.setString(FlinkWriteOptions.PRECOMBINE_FIELD, config.sourceOrderingField);
    conf.setString(FlinkWriteOptions.PAYLOAD_CLASS_NAME, config.payloadClassName);
    conf.setBoolean(FlinkWriteOptions.INSERT_DROP_DUPS, config.filterDupes);
    conf.setInteger(FlinkWriteOptions.RETRY_TIMES, Integer.parseInt(config.instantRetryTimes));
    conf.setLong(FlinkWriteOptions.RETRY_INTERVAL_MS, Long.parseLong(config.instantRetryInterval));
    conf.setBoolean(FlinkWriteOptions.IGNORE_FAILED, config.commitOnErrors);
    conf.setString(FlinkWriteOptions.RECORD_KEY_FIELD, config.recordKeyField);
    conf.setString(FlinkWriteOptions.PARTITION_PATH_FIELD, config.partitionPathField);
    if (!StringUtils.isNullOrEmpty(config.keygenClass)) {
      conf.setString(FlinkWriteOptions.KEYGEN_CLASS_NAME, config.keygenClass);
    } else {
      conf.setString(FlinkWriteOptions.KEYGEN_TYPE, config.keygenType);
    }
    conf.setInteger(FlinkWriteOptions.INDEX_BOOTSTRAP_TASKS, config.indexBootstrapNum);
    conf.setInteger(FlinkWriteOptions.BUCKET_ASSIGN_TASKS, config.bucketAssignNum);
    conf.setInteger(FlinkWriteOptions.WRITE_TASKS, config.writeTaskNum);
    conf.setString(FlinkOptions.PARTITION_DEFAULT_NAME, config.partitionDefaultName);
    conf.setBoolean(FlinkIndexOptions.INDEX_BOOTSTRAP_ENABLED, config.indexBootstrapEnabled);
    conf.setDouble(FlinkIndexOptions.INDEX_STATE_TTL, config.indexStateTtl);
    conf.setBoolean(FlinkIndexOptions.INDEX_GLOBAL_ENABLED, config.indexGlobalEnabled);
    conf.setString(FlinkIndexOptions.INDEX_PARTITION_REGEX, config.indexPartitionRegex);
    conf.setString(FlinkReadOptions.SOURCE_AVRO_SCHEMA_PATH, config.sourceAvroSchemaPath);
    conf.setString(FlinkReadOptions.SOURCE_AVRO_SCHEMA, config.sourceAvroSchema);
    conf.setBoolean(FlinkReadOptions.UTC_TIMEZONE, config.utcTimezone);
    conf.setBoolean(FlinkWriteOptions.URL_ENCODE_PARTITIONING, config.writePartitionUrlEncode);
    conf.setBoolean(FlinkWriteOptions.HIVE_STYLE_PARTITIONING, config.hiveStylePartitioning);
    conf.setDouble(FlinkWriteOptions.WRITE_TASK_MAX_SIZE, config.writeTaskMaxSize);
    conf.setDouble(FlinkWriteOptions.WRITE_BATCH_SIZE, config.writeBatchSize);
    conf.setInteger(FlinkWriteOptions.WRITE_LOG_BLOCK_SIZE, config.writeLogBlockSize);
    conf.setInteger(FlinkWriteOptions.WRITE_LOG_MAX_SIZE, config.writeLogMaxSize);
    conf.setInteger(FlinkWriteOptions.WRITE_MERGE_MAX_MEMORY, config.writeMergeMaxMemory);
    conf.setBoolean(FlinkCompactionOptions.COMPACTION_ASYNC_ENABLED, config.compactionAsyncEnabled);
    ...
    conf.setBoolean(FlinkCompactionOptions.CLEAN_ASYNC_ENABLED, config.cleanAsyncEnabled);
    ...
    conf.setBoolean(FlinkHiveSyncOptions.HIVE_SYNC_ENABLED, config.hiveSyncEnabled);
    conf.setString(FlinkHiveSyncOptions.HIVE_SYNC_DB, config.hiveSyncDb);
    conf.setString(FlinkHiveSyncOptions.HIVE_SYNC_TABLE, config.hiveSyncTable);
    conf.setString(FlinkHiveSyncOptions.HIVE_SYNC_FILE_FORMAT, config.hiveSyncFileFormat);
    conf.setString(FlinkHiveSyncOptions.HIVE_SYNC_MODE, config.hiveSyncMode);
    conf.setString(FlinkHiveSyncOptions.HIVE_SYNC_USERNAME, config.hiveSyncUsername);
    conf.setString(FlinkHiveSyncOptions.HIVE_SYNC_PASSWORD, config.hiveSyncPassword);
    conf.setString(FlinkHiveSyncOptions.HIVE_SYNC_JDBC_URL, config.hiveSyncJdbcUrl);
    conf.setString(FlinkHiveSyncOptions.HIVE_SYNC_METASTORE_URIS, config.hiveSyncMetastoreUri);
    conf.setString(FlinkHiveSyncOptions.HIVE_SYNC_PARTITION_FIELDS, config.hiveSyncPartitionFields);
    conf.setString(FlinkHiveSyncOptions.HIVE_SYNC_PARTITION_EXTRACTOR_CLASS_NAME, config.hiveSyncPartitionExtractorClass);
    conf.setBoolean(FlinkHiveSyncOptions.HIVE_SYNC_ASSUME_DATE_PARTITION, config.hiveSyncAssumeDatePartition);
    conf.setBoolean(FlinkHiveSyncOptions.HIVE_SYNC_USE_JDBC, config.hiveSyncUseJdbc);
    conf.setBoolean(FlinkHiveSyncOptions.HIVE_SYNC_AUTO_CREATE_DB, config.hiveSyncAutoCreateDb);
    conf.setBoolean(FlinkHiveSyncOptions.HIVE_SYNC_IGNORE_EXCEPTIONS, config.hiveSyncIgnoreExceptions);
    conf.setBoolean(FlinkHiveSyncOptions.HIVE_SYNC_SKIP_RO_SUFFIX, config.hiveSyncSkipRoSuffix);
    conf.setBoolean(FlinkHiveSyncOptions.HIVE_SYNC_SUPPORT_TIMESTAMP, config.hiveSyncSupportTimestamp);
    return conf;
  }
}
