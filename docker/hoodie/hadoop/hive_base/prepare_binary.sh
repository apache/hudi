...
HIVE_BINARY_CHECKSUM_FILE_NAME=${HIVE_BINARY_FILE_NAME}.sha256
...
if [[ $(cat ./${HIVE_BINARY_CHECKSUM_FILE_NAME}) == $(shasum -a 256 ./binarycache/${HIVE_BINARY_FILE_NAME} | awk '{print $1}') ]]; then
  echo "The checksum is matched!"
else
  echo "The checksum is not matched, Removing the incompleted file, please download it again."
  rm -f ./binarycache/${HIVE_BINARY_FILE_NAME}
fi
