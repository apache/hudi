...
public class DFSTestSuitePathSelector extends DFSPathSelector {
  ...
  public Pair<Option<String>, String> getNextFilePathsAndMaxModificationTime(
    ...
    try {
      ...
      List<HoodieFileStatus> eligibleFiles = new ArrayList<>();
      List<HoodieFileStatus> fileStatuses = storage.globEntries(
          new HoodieLocation(getStringWithAltKeys(props, DFSPathSelectorConfig.ROOT_INPUT_PATH),
              "*"));
      ...
      Option<String> correctBatchIdDueToRollback = Option.fromJavaOptional(fileStatuses.stream()
          .map(f -> f.getLocation().toString().split("/")[
              f.getLocation().toString().split("/").length - 1])
      ...
      for (HoodieFileStatus fileInfo : fileStatuses) {
        if (!fileInfo.isDirectory() || IGNORE_FILEPREFIX_LIST.stream()
            .anyMatch(pfx -> fileInfo.getLocation().getName().startsWith(pfx))) {
          continue;
        } else if (Integer.parseInt(fileInfo.getLocation().getName()) > lastBatchId
            && Integer.parseInt(fileInfo.getLocation().getName()) <= nextBatchId) {
          eligibleFiles.addAll(storage.listFiles(fileInfo.getLocation()));
        }
      }
      ...
      String pathStr = eligibleFiles.stream().map(f -> f.getLocation().toString())
