...
public class DFSTestSuitePathSelector extends DFSPathSelector {
  ...
  public Pair<Option<String>, String> getNextFilePathsAndMaxModificationTime(
    ...
    try {
      ...
      List<StoragePathInfo> fileStatuses = storage.globEntries(
      ...
      for (StoragePathInfo fileInfo : fileStatuses) {
        if (!fileInfo.isDirectory() || IGNORE_FILEPREFIX_LIST.stream()
            .anyMatch(pfx -> fileInfo.getPath().getName().startsWith(pfx))) {
          continue;
        } else if (Integer.parseInt(fileInfo.getPath().getName()) > lastBatchId
            && Integer.parseInt(fileInfo.getPath().getName()) <= nextBatchId) {
          eligibleFiles.addAll(storage.listFiles(fileInfo.getPath()));
        }
      }
