...
/**
  * Provides I/O APIs on files and directories on storage. The APIs are mainly based on {@code
...
public abstract class HoodieStorage implements Closeable {
    ...
    public abstract HoodieStorage newInstance(StoragePath path, StorageConfiguration<?> storageConf);
    ...
    /**
      ...
      * @param overwrite if a file with this name already exists, then if {@code true}, the file will
      *     be overwritten, and if {@code false} an exception will be thrown.
    ...
    /**
      ...
      */
    @PublicAPIMethod(maturity = ApiMaturityLevel.EVOLVING)
    public abstract OutputStream create(
            StoragePath path,
            boolean overwrite,
            Integer bufferSize,
            Short replication,
    ...
    /**
      ...
      */
    @PublicAPIMethod(maturity = ApiMaturityLevel.EVOLVING)
    public abstract SeekableDataInputStream openSeekable(
            StoragePath path, int bufferSize, boolean wrapStream) throws IOException;
    ...
    /**
      * Lists the path info of the direct files/directories in the given path if the path is a
      * directory.
      *
    ...
    /**
      * Lists the path info of the direct files/directories in the given path and filters the results,
      ...
      */
    @PublicAPIMethod(maturity = ApiMaturityLevel.EVOLVING)
    public abstract List<StoragePathInfo> listDirectEntries(
            StoragePath path, StoragePathFilter filter) throws IOException;

    /**
      * Returns all the files that match the pathPattern and are not checksum files, and filters the
      * results.
      *
      ...
      */
    @PublicAPIMethod(maturity = ApiMaturityLevel.EVOLVING)
    public abstract List<StoragePathInfo> globEntries(
            StoragePath pathPattern, StoragePathFilter filter) throws IOException;
    ...
    /**
      * Creates a new file with overwrite set to false. This ensures files are created only once and
      * never rewritten, also, here we take care if the content is not empty, will first write the
      * content to a temp file if {needCreateTempFile} is true, and then rename it back after the
      * content is written.
      *
      ...
      */
    @PublicAPIMethod(maturity = ApiMaturityLevel.EVOLVING)
    public final void createImmutableFileInPath(StoragePath path, Option<byte[]> content)
    ...
    /**
      * Create an OutputStream at the indicated path. The file is overwritten by default.
      *
      * @param path the file to create.
      * @return the OutputStream to write to.
      * @throws IOException IO error.
    ...
    /**
      ...
      */
    @PublicAPIMethod(maturity = ApiMaturityLevel.EVOLVING)
    public SeekableDataInputStream openSeekable(StoragePath path, boolean wrapStream)
            throws IOException {
        return openSeekable(path, getDefaultBlockSize(path), wrapStream);
    }

    /**
      * Lists the file info of the direct files/directories in the given list of paths, if the paths
      * are directory.
      *
