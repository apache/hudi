...
/**
  * Names a file or directory on storage. Path strings use slash (`/`) as the directory separator.
...
public class StoragePath implements Comparable<StoragePath>, Serializable {
    ...
    public StoragePath getParent() {
        ...
        if (cachedParent == null) {
            ...
            try {
                cachedParent =
                        new StoragePath(
                                new URI(uri.getScheme(), uri.getAuthority(), parentPath, null, uri.getFragment()));
            } catch (URISyntaxException e) {
                throw new IllegalArgumentException(e);
            }
        }
    ...
    public StoragePath getPathWithoutSchemeAndAuthority() {
        try {
            return new StoragePath(new URI(null, null, uri.getPath(), uri.getQuery(), uri.getFragment()));
        } catch (URISyntaxException e) {
            throw new IllegalArgumentException(e);
        }
    }
    ...
    /**
      ...
      *     this URI.
      * @return this path if it contains a scheme and authority, or a new path that includes a path and
      *     authority and is fully qualified.
      */
    @PublicAPIMethod(maturity = ApiMaturityLevel.EVOLVING)
    public StoragePath makeQualified(URI defaultUri) {
        ...
        URI newUri;
        try {
            newUri = new URI(scheme, authority, normalize(pathUri.getPath(), true), null, fragment);
        } catch (URISyntaxException e) {
            throw new IllegalArgumentException(e);
        }
    ...
    /**
      * Normalizes the path by removing the trailing slashes (`/`). When {@code keepSingleSlash} is
      * {@code true}, `/` as the path is not changed; otherwise ({@code false}), `/` becomes empty
