...
public class HFileDataBlock extends HFileBlock {
    ...
    /**
      ...
      *
      * @param cursor {@link HFileCursor} containing the current position relative to the beginning of
      *     the HFile (not the block start offset).
      * @param key key to look up.
      * @param blockStartOffsetInFile the start offset of the block relative to the beginning of the
      *     HFile.
      * @return 0 if the block contains the exact same key as the lookup key, and the cursor points to
      *     the key; or 1 if the lookup key does not exist, and the cursor points to the
    ...
    public int seekTo(HFileCursor cursor, Key key, int blockStartOffsetInFile) {
        ...
        while (relativeOffset < uncompressedContentEndRelativeOffset) {
            ...
            if (comp == 0) {
                ...
                // There is no matched key (otherwise, the method should already stop there and
                // return 0)
                // and the key `relativeOffset` points to is already greater than the lookup key.
                // So set the cursor to the previous offset, pointing the greatest key in the file
                // that is
            ...
            long increment =
                    (long) KEY_OFFSET
                            + (long) kv.getKeyLength()
                            + (long) kv.getValueLength()
    ...
    public boolean next(HFileCursor cursor, int blockStartOffsetInFile) {
        ...
        cursor.increment(
                (long) KEY_OFFSET
                        + (long) keyValue.get().getKeyLength()
