...
public abstract class HFileBlock {
    ...
    protected HFileBlock(
            HFileContext context, HFileBlockType blockType, byte[] byteBuff, int startOffsetInBuff) {
        this.context = context;
        this.blockType = blockType;
        this.onDiskSizeWithoutHeader =
                readInt(byteBuff, startOffsetInBuff + Header.ON_DISK_SIZE_WITHOUT_HEADER_INDEX);
        this.uncompressedSizeWithoutHeader =
                readInt(byteBuff, startOffsetInBuff + Header.UNCOMPRESSED_SIZE_WITHOUT_HEADER_INDEX);
        this.bytesPerChecksum = readInt(byteBuff, startOffsetInBuff + Header.BYTES_PER_CHECKSUM_INDEX);
    ...
    public static HFileBlock parse(HFileContext context, byte[] byteBuff, int startOffsetInBuff)
        ...
        switch (blockType) {
            ...
            default:
                throw new IOException("Parsing of the HFile block type " + blockType + " is not supported");
        }
    }
    ...
    /**
      ...
      *
      * <p>This must be called for an encoded and compressed block before any reads.
      *
    ...
    public void unpack() throws IOException {
        if (!isUnpacked) {
            ...
            if (compression != CompressionCodec.NONE) {
                ...
                try (InputStream byteBuffInputStream =
                        new ByteArrayInputStream(
                                compressedByteBuff,
                                startOffsetInCompressedBuff + HFILEBLOCK_HEADER_SIZE,
    ...
    /**
      ...
      *
      * @return a new byte array based on the size of uncompressed data, holding the same header bytes.
      */
