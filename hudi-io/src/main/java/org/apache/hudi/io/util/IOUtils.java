...
public class IOUtils {
    ...
    /**
      * Reads a variable-length encoded number from input bytes and the decoded size on disk, and
      * returns it.
      *
      * @param bytes input byte array.
      * @param offset offset to start reading.
    ...
    public static int compareTo(
            byte[] bytes1, int offset1, int length1, byte[] bytes2, int offset2, int length2) {
    ...
    /**
      ...
      *
      * <p>More formally, returns the lowest index {@code i} such that the range [i, i + target.length)
      * in {@code array} contains exactly the same elements as {@code target}.
      *
    ...
    public static int readFully(
            InputStream inputStream, byte[] targetByteArray, int offset, int length) throws IOException {
        int totalBytesRead = 0;
        int bytesRead;
        while (totalBytesRead < length) {
            bytesRead =
                    inputStream.read(targetByteArray, offset + totalBytesRead, length - totalBytesRead);
            if (bytesRead < 0) {
                break;
            }
