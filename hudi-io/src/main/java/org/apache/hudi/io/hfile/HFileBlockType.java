...
/**
  * Represents the HFile block type. These types are copied from HBase HFile definition to maintain
  * compatibility. Do not delete or reorder the enums as the ordinal is used as the block type ID.
  */
public enum HFileBlockType {
    ...
    public static HFileBlockType parse(byte[] buf, int offset) throws IOException {
        for (HFileBlockType blockType : values()) {
            if (IOUtils.compareTo(blockType.magic, 0, MAGIC_LENGTH, buf, offset, MAGIC_LENGTH) == 0) {
                return blockType;
            }
        }

        throw new IOException(
                "Invalid HFile block magic: " + IOUtils.bytesToString(buf, offset, MAGIC_LENGTH));
    }

    /**
      * Uses this instead of {@link #ordinal()}. They work exactly the same, except DATA and
    ...
    /**
      * Reads a magic record of the length {@link DataSize#MAGIC_LENGTH} from the given stream and
      * expects it to match this block type.
      *
      * @param in input data.
    ...
    public void readAndCheckMagic(DataInputStream in) throws IOException {
        ...
        if (IOUtils.compareTo(buf, magic) != 0) {
            throw new IOException(
                    "Invalid magic: expected " + new String(magic) + ", got " + new String(buf));
        }
    }
}
