...
public class HFileReaderImpl implements HFileReader {
    ...
    public synchronized void initializeMetadata() throws IOException {
        ...
        this.context = HFileContext.builder().compressionCodec(trailer.getCompressionCodec()).build();
        HFileBlockReader blockReader =
                new HFileBlockReader(
                        context,
                        stream,
        ...
        if (compareCurrent > 0) {
            if (currentDataBlockEntry.get().getNextBlockFirstKey().isPresent()) {
                ...
                if (comparedNextBlockFirstKey >= 0) {
                    ...
                    // all blocks (sorted in the TreeMap of block index entries), using binary
                    // search.
                    ...
                    if (floorEntry == null) {
                        // Key smaller than the start key of the first block which should never
                        // happen here
                        throw new IllegalStateException(
                                "Unexpected state of the HFile reader when looking up the key: "
                                        + key
                                        + " data block index: "
    ...
    /**
      ...
      */
    private static HFileTrailer readTrailer(SeekableDataInputStream stream, long fileSize)
            throws IOException {
    ...
    private boolean isAtFirstKey() {
        if (cursor.isValid() && !dataBlockIndexEntryMap.isEmpty()) {
            return cursor.getOffset()
                    == dataBlockIndexEntryMap.firstKey().getOffset() + HFILEBLOCK_HEADER_SIZE;
        }
        return false;
    }
}
