...
/**
  * Implementation of {@link HoodieDecompressor} for {@link CompressionCodec#NONE} compression codec
  * (no compression) by directly reading the input stream.
  */
public class HoodieNoneDecompressor implements HoodieDecompressor {
    @Override
    public int decompress(InputStream compressedInput, byte[] targetByteArray, int offset, int length)
            throws IOException {
