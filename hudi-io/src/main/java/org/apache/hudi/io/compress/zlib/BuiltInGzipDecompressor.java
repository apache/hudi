...
package org.apache.hudi.io.compress.zlib;
 ...
 * {@code org.apache.hadoop.io.compress.zlib.BuiltInGzipDecompressor}
 */
@DoNotPool
public class BuiltInGzipDecompressor implements Decompressor {
  ...
  private Checksum crc = new CRC32();
  ...
  public synchronized boolean needsInput() {
    if (state == BuiltInGzipDecompressor.GzipStateLabel.DEFLATE_STREAM) {  // most common case
  ...
  private void executeHeaderState() throws IOException {
    ...
    if (state == BuiltInGzipDecompressor.GzipStateLabel.HEADER_BASIC) {
      ...
      if (localBufOff >= 10) {      // should be strictly ==
        ...
        state = BuiltInGzipDecompressor.GzipStateLabel.HEADER_EXTRA_FIELD;
      }
    }
    ...
    if (state == BuiltInGzipDecompressor.GzipStateLabel.HEADER_EXTRA_FIELD) {
      if (hasExtraField) {
        ...
        if (numExtraFieldBytesRemaining == 0) {
          state = BuiltInGzipDecompressor.GzipStateLabel.HEADER_FILENAME;
        }
      } else {
        state = BuiltInGzipDecompressor.GzipStateLabel.HEADER_FILENAME;
      }
    ...
    if (state == BuiltInGzipDecompressor.GzipStateLabel.HEADER_FILENAME) {
      if (hasFilename) {
      ...
      state = BuiltInGzipDecompressor.GzipStateLabel.HEADER_COMMENT;
    }
    ...
    if (state == BuiltInGzipDecompressor.GzipStateLabel.HEADER_COMMENT) {
      if (hasComment) {
      ...
      state = BuiltInGzipDecompressor.GzipStateLabel.HEADER_CRC;
    }
    ...
    if (state == BuiltInGzipDecompressor.GzipStateLabel.HEADER_CRC) {
      if (hasHeaderCRC) {
        ...
        if (localBufOff >= 2) {
          ...
          localBufOff = 0;
          crc.reset();
          state = BuiltInGzipDecompressor.GzipStateLabel.DEFLATE_STREAM;
        }
      } else {
        crc.reset();   // will reuse for CRC-32 of uncompressed data
        state = BuiltInGzipDecompressor.GzipStateLabel.DEFLATE_STREAM;  // switching to Inflater now
      }
    }
  }
  ...
  private void executeTrailerState() throws IOException {
    ...
    // stored in the gzip trailer
    if (state == BuiltInGzipDecompressor.GzipStateLabel.TRAILER_SIZE) {
      assert (localBufOff < 4);  // initially 0, but may need multiple calls
      int n = Math.min(userBufLen, 4 - localBufOff);
