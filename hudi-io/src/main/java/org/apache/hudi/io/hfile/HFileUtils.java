...
public class HFileUtils {
    private static final Map<Integer, CompressionCodec> HFILE_COMPRESSION_CODEC_MAP =
            createCompressionCodecMap();
    ...
    public static int compareKeys(Key key1, Key key2) {
        return IOUtils.compareTo(
                ...
                key1.getContentLength(),
                key2.getBytes(),
                key2.getContentOffset(),
                key2.getContentLength());
    }
    ...
    /**
      * The ID mapping cannot change or else that breaks all existing HFiles out there, even the ones
      * that are not compressed! (They use the NONE algorithm) This is because HFile stores the ID to
      * indicate which compression codec is used.
      *
