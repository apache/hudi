...
public interface HFileReader extends Closeable {
    ...
    /**
      ...
      * found the key or not. Consider the key-value pairs in the file, <code>kv[0] .. kv[n-1]</code> ,
      * where there are n KV pairs in the file.
      *
      * <p>The position only moves forward so the caller has to make sure the keys are sorted before
      * making multiple calls of this method.
      *
      * <p>
      *
      * @param key {@link Key} to seek to.
      * @return -1, if key &lt; kv[0], no position; 0, such that kv[i].key = key and the reader is left
      *     in position i; and 1, such that kv[i].key &lt; key if there is no exact match, and the
      *     reader is left in position i. The reader will position itself between kv[i] and kv[i+1]
      *     where kv[i].key &lt; key &lt;= kv[i+1].key; 2, if there is no KV greater than or equal to
      *     the input key, and the reader positions itself at the end of the file and next() will
    ...
    /**
      ...
      *
      * @return {@code false} if empty file; i.e. a call to next would return false and the current key
    ...
    /**
      ...
      *
      * @return {@code false} if the current position is at the end; otherwise {@code true} if more in
      *     file.
      * @throws IOException upon read errors.
      */
    ...
    /**
      * @return {@code true} if the reader has had one of the seek calls invoked; i.e. {@link
      *     #seekTo()} or {@link #seekTo(Key)}. Otherwise, {@code false}.
      */
    boolean isSeeked();
}
