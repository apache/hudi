...
/**
  ...
  *
  * <ol>
  *   <li>Seeking: enables random access by allowing to seek to an arbitrary position w/in the stream
  *   <li>(Thread-safe) Copying: enables to copy from the underlying buffer not modifying the state
  *       of the stream
  * </ol>
  *
...
public class ByteBufferBackedInputStream extends InputStream {
    ...
    /**
      ...
      *
      * <p>NOTE: Position is relative to the start of the stream (ie its absolute w/in this stream),
      * with following invariant being assumed:
      *
      * <p>0 <= pos <= length (of the stream) This method is NOT thread-safe
      *
    ...
    public void seek(long pos) {
        ...
        if (newPos > buffer.limit() || newPos < offset) {
            throw new IllegalArgumentException(
                    String.format(
                            "Can't seek past the backing buffer (limit %d, offset %d, new %d)",
                            buffer.limit(), offset, newPos));
        }
    ...
    /**
      * Copies at most {@code length} bytes starting from position {@code pos} into the target buffer
      * with provided {@code offset}. Returns number of bytes copied from the backing buffer
      *
      * <p>NOTE: This does not change the current position of the stream and is thread-safe
      *
    ...
    public int copyFrom(long pos, byte[] targetBuffer, int offset, int length) {
        ...
        if (bufferPos > buffer.limit()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Can't read past the backing buffer boundary (offset %d, length %d)",
                            pos, buffer.limit() - bufferOffset));
        } else if (length > targetBuffer.length) {
            throw new IllegalArgumentException(
                    String.format(
                            "Target buffer is too small (length %d, buffer size %d)",
                            length, targetBuffer.length));
        }
        // Determine total number of bytes available to read
