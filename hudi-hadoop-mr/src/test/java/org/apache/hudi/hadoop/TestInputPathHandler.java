...
public class TestInputPathHandler {
    ...
    @TempDir static java.nio.file.Path parentPath;
    ...
    static void initTables() throws IOException {
        ...
        initTableType(
                dfs.getConf(), basePathTable1, RAW_TRIPS_TEST_NAME, HoodieTableType.MERGE_ON_READ);
        incrementalPaths.addAll(generatePartitions(dfs, basePathTable1));

        dfs.mkdirs(new Path(basePathTable2));
        initTableType(
                dfs.getConf(), basePathTable2, MODEL_TRIPS_TEST_NAME, HoodieTableType.MERGE_ON_READ);
        incrementalPaths.addAll(generatePartitions(dfs, basePathTable2));

        dfs.mkdirs(new Path(basePathTable3));
        initTableType(
                dfs.getConf(), basePathTable3, ETL_TRIPS_TEST_NAME, HoodieTableType.COPY_ON_WRITE);
        ...
        initTableType(
                dfs.getConf(), basePathTable5, EMPTY_SNAPSHOT_TEST_NAME, HoodieTableType.COPY_ON_WRITE);
        snapshotPaths.add(new Path(basePathTable5));

        initTableType(
                dfs.getConf(), basePathTable6, EMPTY_INCREMENTAL_TEST_NAME, HoodieTableType.MERGE_ON_READ);
    ...
    static HoodieTableMetaClient initTableType(
            Configuration hadoopConf, String basePath, String tableName, HoodieTableType tableType)
            throws IOException {
        ...
        properties.setProperty(
                HoodieTableConfig.PAYLOAD_CLASS_NAME.key(), HoodieAvroPayload.class.getName());
        properties.setProperty(
                HoodieTableConfig.RECORD_MERGER_STRATEGY.key(),
    ...
    public void testInputPathHandler() throws IOException {
        inputPathHandler =
                new InputPathHandler(dfs.getConf(), inputPaths.toArray(new Path[0]), incrementalTables);
    ...
    public void testInputPathHandlerWithGloballyReplicatedTimeStamp() throws IOException {
        ...
        inputPathHandler =
                new InputPathHandler(
                        dfs.getConf(), inputPaths.toArray(new Path[inputPaths.size()]), incrementalTables);
        List<Path> actualPaths =
                inputPathHandler.getGroupedIncrementalPaths().values().stream()
                        .flatMap(List::stream)
                        .collect(Collectors.toList());
