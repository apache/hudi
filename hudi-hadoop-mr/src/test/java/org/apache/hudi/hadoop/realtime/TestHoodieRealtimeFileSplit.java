...
public class TestHoodieRealtimeFileSplit {
    ...
    public void setUp(@TempDir java.nio.file.Path tempDir) throws Exception {
        ...
        split =
                new HoodieRealtimeFileSplit(
                        baseFileSplit, basePath, deltaLogFiles, maxCommitTime, false, Option.empty());
    }
    ...
    public void testWrite() throws IOException {
        ...
        inorder
                .verify(out, times(1))
                .write(aryEq(Text.encode(fileSplitName).array()), eq(0), eq(fileSplitName.length()));
    ...
    public void testReadFields() throws IOException {
        ...
        // Answer implementation is used to guarantee the response in sequence of the mock method
        // calls
        // since the same method is called many times, we need to return the responses in proper
        // sequence
        when(in.readInt())
                .thenAnswer(
                        new Answer<Integer>() {
                            ...
                            private int[] answers =
                                    new int[] {
                                        basePath.length(),
                                        maxCommitTime.length(),
                                        deltaLogPaths.size(),
                                        deltaLogPaths.get(0).length()
                                    };
