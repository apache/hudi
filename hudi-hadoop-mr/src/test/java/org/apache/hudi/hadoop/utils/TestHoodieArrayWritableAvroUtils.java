...
public class TestHoodieArrayWritableAvroUtils {
    ...
    public void testProjection() {
        ...
        Schema to =
                HoodieAvroUtils.generateProjectionSchema(
                        from, Arrays.asList("trip_type", "current_ts", "weight"));
        ...
        // Reverse projection, the fields are in the original spots, but only the fields we set can
        // be read.
    ...
    private static ArrayWritable convertArrayWritable(GenericRecord record) {
        return (ArrayWritable)
                HoodieRealtimeRecordReaderUtils.avroToArrayWritable(record, record.getSchema(), false);
    }
}
