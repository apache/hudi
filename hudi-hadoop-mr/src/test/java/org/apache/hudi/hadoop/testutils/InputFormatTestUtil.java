...
public class InputFormatTestUtil {
    ...
    public static File prepareTable(
            java.nio.file.Path basePath,
            HoodieFileFormat baseFileFormat,
            int numberOfFiles,
            String commitNumber)
            throws IOException {
        return prepareCustomizedTable(
                basePath, baseFileFormat, numberOfFiles, commitNumber, false, true, false, null);
    }

    public static File prepareCustomizedTable(
            java.nio.file.Path basePath,
            HoodieFileFormat baseFileFormat,
            int numberOfFiles,
            String commitNumber,
            boolean useNonPartitionedKeyGen,
        ...
        java.nio.file.Path partitionPath =
                useNonPartitionedKeyGen ? basePath : basePath.resolve(Paths.get("2016", "05", "01"));
        ...
        if (injectData) {
            ...
            return simulateInserts(
                    partitionPath.toFile(),
                    baseFileFormat.getFileExtension(),
                    "fileId1",
                    numberOfFiles,
                    commitNumber);
        }
    ...
    public static File prepareMultiPartitionTable(
            java.nio.file.Path basePath,
            HoodieFileFormat baseFileFormat,
            int numberOfFiles,
            String commitNumber,
            String finalLevelPartitionName)
            throws IOException {
        HoodieTestUtils.init(
                HoodieTestUtils.getDefaultStorageConf(),
                basePath.toString(),
        ...
        java.nio.file.Path partitionPath =
                basePath.resolve(Paths.get("2016", "05", finalLevelPartitionName));
        setupPartition(basePath, partitionPath);

        return simulateInserts(
                partitionPath.toFile(),
                baseFileFormat.getFileExtension(),
                "fileId1" + finalLevelPartitionName,
                numberOfFiles,
                commitNumber);
    }

    public static File simulateInserts(
            File partitionPath,
                    ...
                    partitionPath
                            ...
                            .resolve(
                                    FSUtils.makeBaseFileName(
                                            commitNumber, TEST_WRITE_TOKEN, fileId + i, baseFileExtension)));
        }
        return partitionPath;
    }

    public static void simulateUpdates(
            File directory,
            String baseFileExtension,
        ...
        for (File file : toUpdateList) {
            ...
            Files.createFile(
                    directory
                            ...
                            .resolve(
                                    FSUtils.makeBaseFileName(
                                            newCommit, TEST_WRITE_TOKEN, fileId, baseFileExtension)));
        }
    }
    ...
    public static void commit(java.nio.file.Path basePath, String commitNumber) throws IOException {
        ...
        Files.createFile(
                basePath.resolve(
                        Paths.get(
                                ".hoodie",
                                commitNumber + "_" + InProcessTimeGenerator.createNewInstantTime() + ".commit")));
    }

    public static void deltaCommit(java.nio.file.Path basePath, String commitNumber)
            throws IOException {
        // create the commit
        Files.createFile(
                basePath.resolve(
    ...
    public static void setupIncremental(
            JobConf jobConf, String startCommit, int numberOfCommitsToPull) {
    ...
    public static void setupIncremental(
            ...
            boolean isIncrementalUseDatabase) {
    ...
    public static void setupIncremental(
        ...
        String modePropertyName =
                String.format(
                        HoodieHiveUtils.HOODIE_CONSUME_MODE_PATTERN,
                        databaseName + "." + HoodieTestUtils.RAW_TRIPS_TEST_NAME);
        jobConf.set(modePropertyName, HoodieHiveUtils.INCREMENTAL_SCAN_MODE);

        String startCommitTimestampName =
                String.format(
                        HoodieHiveUtils.HOODIE_START_COMMIT_PATTERN,
                        databaseName + "." + HoodieTestUtils.RAW_TRIPS_TEST_NAME);
        jobConf.set(startCommitTimestampName, startCommit);

        String maxCommitPulls =
                String.format(
                        HoodieHiveUtils.HOODIE_MAX_COMMIT_PATTERN,
                        databaseName + "." + HoodieTestUtils.RAW_TRIPS_TEST_NAME);
        jobConf.setInt(maxCommitPulls, numberOfCommitsToPull);
    ...
    private static void setupSnapshotScanMode(JobConf jobConf, boolean includePending) {
        ...
        String includePendingCommitsName =
                String.format(
                        HoodieHiveUtils.HOODIE_CONSUME_PENDING_COMMITS, HoodieTestUtils.RAW_TRIPS_TEST_NAME);
    ...
    public static File prepareParquetTable(
            java.nio.file.Path basePath,
            Schema schema,
            int numberOfFiles,
            int numberOfRecords,
    ...
    public static File prepareSimpleParquetTable(
            java.nio.file.Path basePath,
            Schema schema,
            int numberOfFiles,
            int numberOfRecords,
            String commitNumber)
            throws Exception {
        return prepareSimpleParquetTable(
                basePath,
                schema,
                numberOfFiles,
                numberOfRecords,
    ...
    public static File prepareNonPartitionedParquetTable(
            java.nio.file.Path basePath,
            Schema schema,
            int numberOfFiles,
            int numberOfRecords,
            String commitNumber)
            throws IOException {
        return prepareNonPartitionedParquetTable(
                basePath,
                schema,
                numberOfFiles,
                numberOfRecords,
    ...
    public static List<File> prepareMultiPartitionedParquetTable(
            java.nio.file.Path basePath,
            Schema schema,
            int numberPartitions,
            int numberOfRecordsPerPartition,
            String commitNumber,
            HoodieTableType tableType)
            throws IOException {
    ...
    private static void createData(
        ...
        for (int i = 0; i < numberOfFiles; i++) {
            ...
            parquetWriter =
                    new AvroParquetWriter(new Path(partitionPath.resolve(fileId).toString()), schema);
            try {
                for (GenericRecord record :
                        generateAvroRecords(schema, numberOfRecords, commitNumber, fileId)) {
    ...
    private static void createSimpleData(
            Schema schema,
            java.nio.file.Path partitionPath,
            int numberOfFiles,
            int numberOfRecords,
        ...
        for (int i = 0; i < numberOfFiles; i++) {
            String fileId =
                    FSUtils.makeBaseFileName(
                            commitNumber, "1", "fileid" + i, HoodieFileFormat.PARQUET.getFileExtension());
            parquetWriter =
                    new AvroParquetWriter(new Path(partitionPath.resolve(fileId).toString()), schema);
            try {
                ...
                for (IndexedRecord record : records) {
                    GenericRecord p =
                            HoodieAvroUtils.rewriteRecord((GenericRecord) record, hoodieFieldsSchema);
    ...
    private static Iterable<? extends GenericRecord> generateAvroRecords(
            Schema schema, int numberOfRecords, String instantTime, String fileId) throws IOException {
    ...
    public static void simulateParquetUpdates(
            ...
            int numberOfRecordsToUpdate,
            String newCommit)
            throws IOException {
        File fileToUpdate =
                Objects.requireNonNull(directory.listFiles((dir, name) -> name.endsWith("parquet")))[0];
        String fileId = FSUtils.getFileId(fileToUpdate.getName());
        File dataFile =
                new File(
                        directory,
                        FSUtils.makeBaseFileName(
                                newCommit, TEST_WRITE_TOKEN, fileId, HoodieFileFormat.PARQUET.getFileExtension()));
        try (AvroParquetWriter parquetWriter =
                new AvroParquetWriter(new Path(dataFile.getAbsolutePath()), schema)) {
            for (GenericRecord record :
                    generateAvroRecords(schema, totalNumberOfRecords, originalCommit, fileId)) {
    ...
    public static HoodieLogFormat.Writer writeRollback(
            File partitionDir,
        ...
        HoodieLogFormat.Writer writer =
                HoodieLogFormat.newWriterBuilder()
                        .onParentPath(new StoragePath(partitionDir.getPath()))
                        .withFileId(fileId)
    ...
    public static HoodieLogFormat.Writer writeDataBlockToLogFile(
            File partitionDir,
            HoodieStorage storage,
            Schema schema,
            String fileId,
        ...
        return writeDataBlockToLogFile(
                partitionDir,
                storage,
                schema,
                fileId,
        ...
        if (logBlockType == HoodieLogBlock.HoodieLogBlockType.HFILE_DATA_BLOCK) {
            ...
            dataBlock =
                    new HoodieAvroDataBlock(
                            hoodieRecords, false, header, HoodieRecord.RECORD_KEY_METADATA_FIELD);
        }
        writer.appendBlock(dataBlock);
        return writer;
    }

    public static HoodieLogFormat.Writer writeRollbackBlockToLogFile(
            File partitionDir,
            HoodieStorage storage,
            Schema schema,
            String fileId,
    ...
    public static void setProjectFieldsForInputFormat(
            JobConf jobConf, Schema schema, String hiveColumnTypes) {
        List<Schema.Field> fields = schema.getFields();
        String names = fields.stream().map(f -> f.name().toString()).collect(Collectors.joining(","));
        String positions =
                fields.stream().map(f -> String.valueOf(f.pos())).collect(Collectors.joining(","));
    ...
    public static void setPropsForInputFormat(
            JobConf jobConf, Schema schema, String hiveColumnTypes) {
        List<Schema.Field> fields = schema.getFields();
        String names = fields.stream().map(f -> f.name().toString()).collect(Collectors.joining(","));
        String positions =
    ...
    public static void setupPartition(java.nio.file.Path basePath, java.nio.file.Path partitionPath)
            throws IOException {
