...
/**
  ...
  * Hoodie Mode. If paths that does not correspond to a hoodie table then they are passed in as is
  * (as what FileInputFormat.listStatus() would do). The JobConf could have paths from multiple
  * Hoodie/Non-Hoodie tables
  */
...
public class HoodieParquetInputFormat extends HoodieParquetInputFormatBase {
    ...
    /**
      * Spark2 use `parquet.hadoopParquetInputFormat` in `com.twitter:parquet-hadoop-bundle`. So that
      * we need to distinguish the constructions of classes with `parquet.hadoopParquetInputFormat` or
      * `org.apache.parquet.hadoop.ParquetInputFormat`. If we use `org.apache.parquet:parquet-hadoop`,
      * we can use `HudiAvroParquetInputFormat` in Hive or Spark3 to get timestamp with correct type.
      */
                            ...
                            c ->
                                    c.getParameterCount() > 0
                                            && c.getParameterTypes()[0]
                                                    .getName()
                                                    .equals(ParquetInputFormat.class.getName()))) {
    ...
    private static boolean checkIfHudiTable(final InputSplit split, final JobConf job) {
        try {
            ...
            return getTablePath(storage, convertToStoragePath(inputPath))
                    .map(path -> isHoodieTablePath(storage, path))
                    .orElse(false);
        } catch (IOException e) {
            return false;
        }
    ...
    public RecordReader<NullWritable, ArrayWritable> getRecordReader(
        ...
        HoodieRealtimeInputFormatUtils.addProjectionField(
                job, job.get(hive_metastoreConstants.META_TABLE_PARTITION_COLUMNS, "").split("/"));
                            ...
                            (s, j, r) -> {
                                try {
                                    return new ParquetRecordReaderWrapper(
                                            new HoodieTimestampAwareParquetInputFormat(), s, j, r);
    ...
    private RecordReader<NullWritable, ArrayWritable> getRecordReaderInternal(
            InputSplit split, JobConf job, Reporter reporter) throws IOException {
        try {
            if (supportAvroRead && HoodieColumnProjectionUtils.supportTimestamp(job)) {
                return new ParquetRecordReaderWrapper(
                        new HoodieTimestampAwareParquetInputFormat(), split, job, reporter);
            } else {
                return super.getRecordReader(split, job, reporter);
            }
    ...
    private RecordReader<NullWritable, ArrayWritable> createBootstrappingRecordReader(
            InputSplit split, JobConf job, Reporter reporter) throws IOException {
        ...
        List<Pair<Integer, String>> projectedColsWithIndex =
                IntStream.range(0, rawColIds.size())
                        .mapToObj(idx -> Pair.of(rawColIds.get(idx), rawColNames[idx]))
                        .collect(Collectors.toList());

        List<Pair<Integer, String>> hoodieColsProjected =
                projectedColsWithIndex.stream()
                        .filter(
                                idxWithName -> HoodieRecord.HOODIE_META_COLUMNS.contains(idxWithName.getValue()))
                        .collect(Collectors.toList());
        List<Pair<Integer, String>> externalColsProjected =
                projectedColsWithIndex.stream()
                        .filter(
                                idxWithName ->
                                        !HoodieRecord.HOODIE_META_COLUMNS.contains(idxWithName.getValue())
                                                && !HoodieHiveUtils.VIRTUAL_COLUMN_NAMES.contains(idxWithName.getValue()))
                        .collect(Collectors.toList());

        // This always matches hive table description
        List<Pair<String, String>> colNameWithTypes =
                HoodieColumnProjectionUtils.getIOColumnNameAndTypes(job);
        List<Pair<String, String>> colNamesWithTypesForExternal =
                colNameWithTypes.stream()
        ...
        if (hoodieColsProjected.isEmpty()) {
            ...
            // The above config is disabled by default. But when enabled, would cause misalignment
            // between
            // skeleton and bootstrap file. We will disable them specifically when query needs
            ...
            LOG.info(
                    "Generating column stitching reader for "
                            + eSplit.getPath()
                            + " and "
                            + rightSplit.getPath());
            return new BootstrapColumnStichingRecordReader(
                    getRecordReaderInternal(eSplit, jobConfCopy, reporter),
