...
public class RealtimeCompactedRecordReader extends AbstractRealtimeRecordReader
    ...
    public RealtimeCompactedRecordReader(
    ...
    private HoodieMergedLogRecordScanner getMergedLogRecordScanner() throws IOException {
        ...
        return HoodieMergedLogRecordScanner.newBuilder()
                ...
                .withSpillableMapBasePath(
                        jobConf.get(
                                HoodieMemoryConfig.SPILLABLE_MAP_BASE_PATH.key(),
                ...
                .withInternalSchema(
                        schemaEvolutionContext.internalSchemaOption.orElse(
                                InternalSchema.getEmptyInternalSchema()))
                .build();
    }

    private Option<HoodieAvroIndexedRecord> buildGenericRecordwithCustomPayload(HoodieRecord record)
            throws IOException {
        ...
        while (this.parquetReader.next(aVoid, arrayWritable)) {
            if (!deltaRecordMap.isEmpty()) {
                ...
                if (deltaRecordMap.containsKey(key)) {
                    ...
                    // If the record is not present, this is a delete record using an empty payload
                    // so skip this base record
    ...
    private void setUpWritable(
            Option<HoodieAvroIndexedRecord> rec, ArrayWritable arrayWritable, String key) {
        GenericRecord recordToReturn = (GenericRecord) rec.get().getData();
        if (usesCustomPayload) {
            // If using a custom payload, return only the projection fields. The readerSchema is a
            // schema derived from
            // the writerSchema with only the projection fields
            recordToReturn =
                    HoodieAvroUtils.rewriteRecord((GenericRecord) rec.get().getData(), getReaderSchema());
        }
        // we assume, a later safe record in the log, is newer than what we have in the map &
        // replace it. Since we want to return an arrayWritable which is the same length as the
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug(
                    String.format(
                            "key %s, base values: %s, log values: %s",
                            key,
        ...
        try {
            ...
            System.arraycopy(
                    replaceValue, 0, originalValue, 0, Math.min(originalValue.length, replaceValue.length));
            arrayWritable.set(originalValue);
        } catch (RuntimeException re) {
            LOG.error("Got exception when doing array copy", re);
            LOG.error(
                    "Base record :" + HoodieRealtimeRecordReaderUtils.arrayWritableToString(arrayWritable));
    ...
    private Option<HoodieAvroIndexedRecord> mergeRecord(
            HoodieRecord<?> newRecord, ArrayWritable writableFromParquet) throws IOException {
        GenericRecord oldRecord = convertArrayWritableToHoodieRecord(writableFromParquet);
        // presto will not append partition columns to jobConf.get(serdeConstants.LIST_COLUMNS), but
        // hive will do it. This will lead following results
        // eg: current table: col1: int, col2: int, par: string, and column par is partition
        // columns.
        // for hive engine, the hiveSchema will be: col1,col2,par, and the writerSchema will be
        // col1,col2,par
        // for presto engine, the hiveSchema will be: col1,col2, but the writerSchema will be
        // col1,col2,par
        // so to be compatible with hive and presto, we should rewrite oldRecord before we call
        // combineAndGetUpdateValue,
        // once presto on hudi have its own mor reader, we can remove the rewrite logical.
        GenericRecord genericRecord =
                HiveAvroSerializer.rewriteRecordIgnoreResultCheck(oldRecord, getLogScannerReaderSchema());
        HoodieRecord record = new HoodieAvroIndexedRecord(genericRecord);
        Option<Pair<HoodieRecord, Schema>> mergeResult =
                HoodieAvroRecordMerger.INSTANCE.merge(
                        record,
