...
public class HoodieHiveRecord extends HoodieRecord<ArrayWritable> {
    ...
    public HoodieHiveRecord(
            HoodieKey key, ArrayWritable data, Schema schema, ObjectInspectorCache objectInspectorCache) {
        super(key, data);
    ...
    public Comparable<?> getOrderingValue(Schema recordSchema, Properties props) {
        ...
        if (orderingField == null) {
            return 0;
            // throw new IllegalArgumentException("Ordering Field is not set. Precombine must be
            // set. (If you are using a custom record merger it might be something else)");
        }
    ...
    public HoodieRecord<ArrayWritable> copy() {
        if (!copy) {
            this.data =
                    new ArrayWritable(Writable.class, Arrays.copyOf(this.data.get(), this.data.get().length));
            this.copy = true;
        }
        return this;
    }
