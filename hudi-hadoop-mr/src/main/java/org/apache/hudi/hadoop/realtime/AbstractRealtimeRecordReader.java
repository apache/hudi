...
public abstract class AbstractRealtimeRecordReader {
        ...
        try {
            metaClient =
                    HoodieTableMetaClient.builder()
                            ...
                            .setBasePath(split.getBasePath())
                            .build();
            if (metaClient.getTableConfig().getPreCombineField() != null) {
                this.payloadProps.setProperty(
    ...
    private boolean usesCustomPayload(HoodieTableMetaClient metaClient) {
        return !(metaClient
                        .getTableConfig()
                        .getPayloadClass()
                        .contains(HoodieAvroPayload.class.getName())
                || metaClient
                        .getTableConfig()
                        .getPayloadClass()
                        .contains(OverwriteWithLatestAvroPayload.class.getName()));
    }

    private void prepareHiveAvroSerializer() {
        try {
            // hive will append virtual columns at the end of column list. we should remove those
            // columns.
            // eg: current table is col1, col2, col3; jobConf.get(serdeConstants.LIST_COLUMNS):
            // col1, col2, col3 ,BLOCK__OFFSET__INSIDE__FILE ...
            Set<String> writerSchemaColNames =
                    writerSchema.getFields().stream()
                            .map(f -> f.name().toLowerCase(Locale.ROOT))
                            .collect(Collectors.toSet());
            List<String> columnNameList =
                    Arrays.stream(jobConf.get(serdeConstants.LIST_COLUMNS).split(","))
            ...
            for (int i = columnNameListLen; i >= 0; i--) {
                ...
                // virtual columns will only append at the end of column list. it will be ok to
                // break the loop.
            ...
            this.serializer =
                    new HiveAvroSerializer(
                            new ArrayWritableObjectInspector(rowTypeInfo), columnNameList, columnTypeList);
        } catch (Exception e) {
    ...
    /**
      * Gets schema from HoodieTableMetaClient. If not, falls back to the schema from the latest
      * parquet file. Finally, sets the partition column and projection fields into the job conf.
      */
    private void init() throws Exception {
        ...
        List<String> projectionFields =
                HoodieRealtimeRecordReaderUtils.orderFields(
                        jobConf.get(ColumnProjectionUtils.READ_COLUMN_NAMES_CONF_STR, EMPTY_STRING),
                        jobConf.get(ColumnProjectionUtils.READ_COLUMN_IDS_CONF_STR, EMPTY_STRING),
                        partitioningFields);

        Map<String, Field> schemaFieldsMap =
                HoodieRealtimeRecordReaderUtils.getNameToFieldMap(writerSchema);
        hiveSchema =
                constructHiveOrderedSchema(
                        writerSchema,
                        schemaFieldsMap,
        ...
        readerSchema =
                HoodieRealtimeRecordReaderUtils.generateProjectionSchema(
                        writerSchema, schemaFieldsMap, projectionFields);
        LOG.info(
                String.format(
                        "About to read compacted logs %s for base split %s, projecting cols %s",
    ...
    public Schema constructHiveOrderedSchema(
            Schema writerSchema, Map<String, Field> schemaFieldsMap, String hiveColumnString) {
        ...
        for (String columnName : hiveColumns) {
            ...
            if (field != null) {
                hiveSchemaFields.add(
                        new Schema.Field(field.name(), field.schema(), field.doc(), field.defaultVal()));
            } else {
                // Hive has some extra virtual columns like BLOCK__OFFSET__INSIDE__FILE which do not
