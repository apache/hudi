...
public class HiveHoodieReaderContext extends HoodieReaderContext<ArrayWritable> {
    ...
    protected HiveHoodieReaderContext(
        ...
        this.hosts = hosts;
        this.partitionCols =
                getPartitionFieldNames(jobConf).stream()
                        .filter(n -> writerSchema.getField(n) != null)
    ...
    /**
      * If populate meta fields is false, then getRecordKeyFields() should return exactly 1 recordkey
      * field.
      */
    private static String getRecordKeyField(HoodieTableMetaClient metaClient) {
        ...
        ValidationUtils.checkArgument(
                recordKeyFieldsOpt.isPresent(),
                "No record key field set in table config, but populateMetaFields is disabled");
        ValidationUtils.checkArgument(
                recordKeyFieldsOpt.get().length == 1,
                "More than 1 record key set in table config, but populateMetaFields is disabled");
        return recordKeyFieldsOpt.get()[0];
    }

    private void setSchemas(JobConf jobConf, Schema dataSchema, Schema requiredSchema) {
        List<String> dataColumnNameList =
                dataSchema.getFields().stream()
                        .map(f -> f.name().toLowerCase(Locale.ROOT))
                        .collect(Collectors.toList());
        List<TypeInfo> dataColumnTypeList =
                dataColumnNameList.stream()
                        .map(
                                fieldName -> {
                                    TypeInfo type = columnTypeMap.get(fieldName);
                                    if (type == null) {
                                        throw new IllegalArgumentException(
                                                "Field: " + fieldName + ", does not have a defined type");
                                    }
                                    return type;
                                })
                        .collect(Collectors.toList());
    ...
    public ClosableIterator<ArrayWritable> getFileRecordIterator(
        ...
        // move the partition cols to the end, because in some cases it has issues if we don't do
        // that
        Schema modifiedDataSchema =
                HoodieAvroUtils.generateProjectionSchema(
                        ...
                        Stream.concat(
                                        ...
                                        partitionCols.stream().filter(c -> dataSchema.getField(c) != null))
                                .collect(Collectors.toList()));
        setSchemas(jobConfCopy, modifiedDataSchema, requiredSchema);
        InputSplit inputSplit =
                new FileSplit(new Path(filePath.toString()), start, length, hosts.get(filePath.toString()));
        ...
        // record reader puts the required columns in the positions of the data schema and nulls the
        // rest of the columns
    ...
    public ArrayWritable convertAvroRecord(IndexedRecord avroRecord) {
        return (ArrayWritable)
                HoodieRealtimeRecordReaderUtils.avroToArrayWritable(
                        avroRecord, avroRecord.getSchema(), true);
    }
    ...
    public HoodieRecordMerger getRecordMerger(String mergerStrategy) {
        ...
        throw new HoodieException(
                String.format(
                        "The merger strategy UUID is not supported, Default: %s, Passed: %s",
                        mergerStrategy, DEFAULT_MERGER_STRATEGY_UUID));
    }
    ...
    public Object getValue(ArrayWritable record, Schema schema, String fieldName) {
        return StringUtils.isNullOrEmpty(fieldName)
                ? null
                : objectInspectorCache.getValue(record, schema, fieldName);
    }

    @Override
    public HoodieRecord<ArrayWritable> constructHoodieRecord(
            Option<ArrayWritable> recordOption, Map<String, Object> metadataMap) {
    ...
    @Override
    public ClosableIterator<ArrayWritable> mergeBootstrapReaders(
            ClosableIterator<ArrayWritable> skeletonFileIterator,
            ...
            public boolean hasNext() {
                if (dataFileIterator.hasNext() != skeletonFileIterator.hasNext()) {
                    throw new IllegalStateException(
                            "bootstrap data file iterator and skeleton file iterator are out of sync");
                }
    ...
    @Override
    public UnaryOperator<ArrayWritable> projectRecord(
            Schema from, Schema to, Map<String, String> renamedColumns) {
        if (!renamedColumns.isEmpty()) {
            throw new IllegalStateException(
                    "Schema evolution is not supported in the filegroup reader for Hive currently");
        }
    ...
    public long getPos() throws IOException {
        ...
        throw new IllegalStateException(
                "getPos() should not be called before a record reader has been initialized");
    }
    ...
    public float getProgress() throws IOException {
        ...
        throw new IllegalStateException(
                "getProgress() should not be called before a record reader has been initialized");
    }
}
