...
/**
  * To read value from an ArrayWritable, an ObjectInspector is needed. Object inspectors are cached
  * here or created using the column type map.
  */
public class ObjectInspectorCache {
    private final Map<String, TypeInfo> columnTypeMap = new HashMap<>();
    private final Cache<Schema, ArrayWritableObjectInspector> objectInspectorCache =
    ...
    public ObjectInspectorCache(Schema tableSchema, JobConf jobConf) {
        ...
        // columns.
        // eg: current table is col1, col2, col3; jobConf.get(serdeConstants.LIST_COLUMNS): col1,
        // col2, col3 ,BLOCK__OFFSET__INSIDE__FILE ...
        ...
        for (int i = columnNameListLen; i >= 0; i--) {
            ...
            // virtual columns will only append at the end of column list. it will be ok to break
            // the loop.
        ...
        // Use columnNameList.size() instead of columnTypeList because the type list is longer for
        // some reason
                        ...
                        i ->
                                columnTypeMap.put(
                                        ...
                                        TypeInfoUtils.getTypeInfosFromTypeString(
                                                        columnTypeList.get(i).getQualifiedName())
                                                .get(0)));
    ...
    public ArrayWritableObjectInspector getObjectInspector(Schema schema) {
        return objectInspectorCache.get(
                schema,
                s -> {
                    List<String> columnNameList =
    ...
    public Object getValue(ArrayWritable record, Schema schema, String fieldName) {
        try {
            ...
            return objectInspector.getStructFieldData(
                    record, objectInspector.getStructFieldRef(fieldName));
        } catch (Exception e) {
            throw e;
        }
    }
