...
public class HoodieRealtimeInputFormatUtils extends HoodieInputFormatUtils {
    ...
    private static Configuration addProjectionField(
            Configuration conf, String fieldName, int fieldIndex) {
    ...
    public static void addProjectionField(Configuration conf, String[] fieldName) {
        if (fieldName.length > 0) {
            List<String> columnNameList =
                    Arrays.stream(conf.get(serdeConstants.LIST_COLUMNS, "").split(","))
    ...
    public static void addVirtualKeysProjection(
            Configuration configuration, Option<HoodieVirtualKeyInfo> hoodieVirtualKeyInfo) {
        ...
        if (!hoodieVirtualKeyInfo.isPresent()) {
            ...
            addProjectionField(
                    configuration,
                    hoodieVirtualKey.getRecordKeyField(),
                    hoodieVirtualKey.getRecordKeyFieldIndex());
    ...
    public static boolean requiredProjectionFieldsExistInConf(
        ...
        if (!hoodieVirtualKeyInfo.isPresent()) {
            ...
            return readColNames.contains(hoodieVirtualKeyInfo.get().getRecordKeyField())
                    && (hoodieVirtualKeyInfo.get().getPartitionPathField().isPresent()
                            ? readColNames.contains(hoodieVirtualKeyInfo.get().getPartitionPathField().get())
                            : true);
        }
    }

    public static boolean canAddProjectionToJobConf(
            final RealtimeSplit realtimeSplit, final JobConf jobConf) {
        return jobConf.get(HoodieInputFormatUtils.HOODIE_READ_COLUMNS_PROP) == null
                || (!realtimeSplit.getDeltaLogPaths().isEmpty()
                        && !HoodieRealtimeInputFormatUtils.requiredProjectionFieldsExistInConf(
                                jobConf, realtimeSplit.getVirtualKeyInfo()));
    }

    /**
      * Hive will append read columns' ids to old columns' ids during getRecordReader. In some cases,
      * e.g. SELECT COUNT(*), the read columns' id is an empty string and Hive will combine it with
      * Hoodie required projection ids and becomes e.g. ",2,0,3" and will cause an error. Actually this
      * method is a temporary solution because the real bug is from Hive. Hive has fixed this bug after
      * 3.0.0, but the version before that would still face this problem. (HIVE-22438)
      */
