...
/**
  * Utility functions copied from Hive ColumnProjectionUtils.java. Needed to copy as we see
  * NoSuchMethod errors when directly using these APIs with/without Spark. Some of these methods are
  * not available across hive versions.
  */
public class HoodieColumnProjectionUtils {
    ...
    /**
      * the nested column path is the string from the root to the leaf e.g. c:struct_of
      * (a:string,b:string). the column a's path is c.a and b's path is c.b
      */
    ...
    /**
      * Returns an array of column ids(start from zero) which is set in the given parameter
      * <tt>conf</tt>.
      */
    public static List<Integer> getReadColumnIDs(Configuration conf) {
        ...
        for (String element : list) {
            ...
            // NOTE: some code uses this list to correlate with column names, and yet these lists
            // may
            //       contain duplicates, which this call will remove and the other won't. As far as
            // I can
            //       tell, no code will actually use these two methods together; all is good if the
            // code
            //       gets the ID list without relying on this method. Or maybe it just works by
            // magic.
        }
        return result;
    }
    ...
    public static List<Pair<String, String>> getIOColumnNameAndTypes(Configuration conf) {
        ...
        return IntStream.range(0, names.size())
                .mapToObj(idx -> Pair.of(names.get(idx), types.get(idx)))
                .collect(Collectors.toList());
    }

    /**
      * If schema contains timestamp columns, this method is used for compatibility when there is no
      * timestamp fields.
      *
      * <p>We expect to use parquet-avro reader {@link
      * org.apache.hudi.hadoop.avro.HoodieAvroParquetReader} to read timestamp column when read columns
      * contain timestamp type.
      */
    public static boolean supportTimestamp(Configuration conf) {
        ...
        List<String> names = getIOColumns(conf);
        return IntStream.range(0, names.size())
                .filter(i -> readCols.contains(names.get(i)))
