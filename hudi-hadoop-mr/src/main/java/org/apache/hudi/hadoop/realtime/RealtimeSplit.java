...
public interface RealtimeSplit extends InputSplitWithLocationInfo {
    ...
    default List<String> getDeltaLogPaths() {
        return getDeltaLogFiles().stream()
                .map(entry -> entry.getPath().toString())
    ...
    default void writeToOutput(DataOutput out) throws IOException {
        ...
        if (!virtualKeyInfoOpt.isPresent()) {
            ...
            InputSplitUtils.writeString(
                    String.valueOf(virtualKeyInfoOpt.get().getRecordKeyFieldIndex()), out);
            InputSplitUtils.writeBoolean(
                    virtualKeyInfoOpt.get().getPartitionPathField().isPresent(), out);
            if (virtualKeyInfoOpt.get().getPartitionPathField().isPresent()) {
                ...
                InputSplitUtils.writeString(
                        String.valueOf(virtualKeyInfoOpt.get().getPartitionPathFieldIndex()), out);
            }
        }
    }
    ...
    default void readFromInput(DataInput in) throws IOException {
        ...
        if (hoodieVirtualKeyPresent) {
            ...
            Option<String> partitionPathField =
                    isPartitionPathFieldPresent ? Option.of(InputSplitUtils.readString(in)) : Option.empty();
            Option<Integer> partitionPathIndex =
                    isPartitionPathFieldPresent
                            ? Option.of(Integer.parseInt(InputSplitUtils.readString(in)))
                            : Option.empty();
            setVirtualKeyInfo(
                    Option.of(
                            new HoodieVirtualKeyInfo(
                                    recordKeyField, partitionPathField, recordFieldIndex, partitionPathIndex)));
        }
    }
