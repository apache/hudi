...
/**
  ...
  *
  * <p>NOTE: Hive bears optimizations which are based upon validating whether {@link FileInputFormat}
  * implementation inherits from {@link MapredParquetInputFormat}.
  *
  * <p>To make sure that Hudi implementations are leveraging these optimizations to the fullest, this
  * class serves as a base-class for every {@link FileInputFormat} implementations working with
  * Parquet file-format.
  *
  * <p>However, this class serves as a simple delegate to the actual implementation hierarchy: it
  * expects either {@link HoodieCopyOnWriteTableInputFormat} or {@link
  * HoodieMergeOnReadTableInputFormat} to be supplied to which it delegates all of its necessary
  * methods.
  */
public abstract class HoodieParquetInputFormatBase extends MapredParquetInputFormat
    ...
    @Override
    protected final FileSplit makeSplit(Path file, long start, long length, String[] hosts) {
        return inputFormatDelegate.makeSplit(file, start, length, hosts);
    }

    @Override
    protected final FileSplit makeSplit(
