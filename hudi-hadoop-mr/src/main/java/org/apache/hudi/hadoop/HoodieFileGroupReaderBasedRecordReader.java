...
/**
  * {@link HoodieFileGroupReader} based implementation of Hive's {@link RecordReader} for {@link
  * ArrayWritable}.
  */
public class HoodieFileGroupReaderBasedRecordReader
    ...
    public HoodieFileGroupReaderBasedRecordReader(
            HiveReaderCreator readerCreator,
        ...
        this.readerContext =
                new HiveHoodieReaderContext(
                        readerCreator, split, jobConfCopy, reporter, tableSchema, hosts, metaClient);
        this.arrayWritable =
                new ArrayWritable(Writable.class, new Writable[requestedSchema.getFields().size()]);
        ...
        LOG.debug(
                "Creating HoodieFileGroupReaderRecordReader with tableBasePath={}, latestCommitTime={}, fileSplit={}",
                tableBasePath,
                latestCommitTime,
                fileSplit.getPath());
        this.fileGroupReader =
                new HoodieFileGroupReader<>(
                        ...
                        tableBasePath,
                        latestCommitTime,
                        getFileSliceFromSplit(
                                fileSplit, hosts, getFs(tableBasePath, jobConfCopy), tableBasePath),
                        tableSchema,
                        requestedSchema,
                        Option.empty(),
                        metaClient,
                ...
                HoodieAvroUtils.generateProjectionSchema(
                        ...
                        Stream.concat(
                                        tableSchema.getFields().stream()
                                                ...
                                                .filter(n -> !partitionColumns.contains(n)),
    ...
    private static Schema getLatestTableSchema(
            HoodieTableMetaClient metaClient, JobConf jobConf, String latestCommitTime) {
        ...
        try {
            ...
            // these fields
            return HoodieRealtimeRecordReaderUtils.addPartitionFields(
                    schema, getPartitionFieldNames(jobConf));
        } catch (Exception e) {
    ...
    /**
      * Convert FileSplit to FileSlice, but save the locations in 'hosts' because that data is
      * otherwise lost.
      */
    private static FileSlice getFileSliceFromSplit(
            FileSplit split, Map<String, String[]> hosts, FileSystem fs, String tableBasePath)
            throws IOException {
        ...
        if (split instanceof RealtimeSplit) {
            ...
            HoodieFileGroupId fileGroupId =
                    new HoodieFileGroupId(
                            getRelativePartitionPath(
                                    new Path(realtimeSplit.getBasePath()), realtimeSplit.getPath()),
                            fileID);
            if (isLogFile) {
            ...
            HoodieBaseFile hoodieBaseFile =
                    new HoodieBaseFile(
                            convertToStoragePathInfo(fs.getFileStatus(realtimeSplit.getPath())),
                            bootstrapBaseFile);
            return new FileSlice(
                    fileGroupId, commitTime, hoodieBaseFile, realtimeSplit.getDeltaLogFiles());
        }
        // COW
        HoodieFileGroupId fileGroupId =
                new HoodieFileGroupId(
                        getFileId(split.getPath().getName()),
        ...
        return new FileSlice(
                ...
                new HoodieBaseFile(
                        convertToStoragePathInfo(fs.getFileStatus(split.getPath())), bootstrapBaseFile),
                Collections.emptyList());
    }

    private static BaseFile createBootstrapBaseFile(
            FileSplit split, Map<String, String[]> hosts, FileSystem fs) throws IOException {
    ...
    private static Schema createRequestedSchema(Schema tableSchema, JobConf jobConf) {
        ...
        tableSchema
                .getFields()
                .forEach(f -> partitionColumns.remove(f.name().toLowerCase(Locale.ROOT)));
