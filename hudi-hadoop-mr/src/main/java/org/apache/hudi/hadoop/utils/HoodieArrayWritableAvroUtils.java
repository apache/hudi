...
public class HoodieArrayWritableAvroUtils {
    ...
    private static final Cache<String, ObjectInspectorCache> OBJECT_INSPECTOR_TABLE_CACHE =
            Caffeine.newBuilder().maximumSize(1000).build();

    public static ObjectInspectorCache getCacheForTable(
            String table, Schema tableSchema, JobConf jobConf) {
    ...
    private static final Cache<Pair<Schema, Schema>, int[]> PROJECTION_CACHE =
            Caffeine.newBuilder().maximumSize(1000).build();

    public static int[] getProjection(Schema from, Schema to) {
        return PROJECTION_CACHE.get(
                Pair.of(from, to),
                schemas -> {
    ...
    /**
      * Projection will keep the size from the "from" schema because it gets recycled and if the size
      * changes the reader will fail
      */
    ...
    /**
      * After the reading and merging etc is done, we need to put the records into the positions of the
      * original schema
      */
    ...
    public static Object getWritableValue(
            ArrayWritable arrayWritable, ArrayWritableObjectInspector objectInspector, String name) {
        return objectInspector.getStructFieldData(
                arrayWritable, objectInspector.getStructFieldRef(name));
    }
}
