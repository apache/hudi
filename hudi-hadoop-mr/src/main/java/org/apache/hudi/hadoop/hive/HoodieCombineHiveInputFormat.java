...
/**
  ...
  * <p>
  *
  * <p>CombineHiveInputFormat is a parameterized InputFormat which looks at the path name and
  * determine the correct InputFormat for that path name from mapredPlan.pathToPartitionInfo(). It
  * can be used to read files with different input format in the same map-reduce job.
  *
  * <p>NOTE : This class is implemented to work with Hive 2.x +
  */
public class HoodieCombineHiveInputFormat<K extends WritableComparable, V extends Writable>
    ...
    private InputSplit[] getCombineSplits(
            JobConf job, int numSplits, Map<Path, PartitionDesc> pathToPartitionInfo) throws IOException {
        init(job);
        ...
        // combine splits only from same tables and same partitions. Do not combine splits from
        // multiple
        ...
        for (Path path : paths) {
            ...
            if (inputFormatClass.getName().equals(getParquetInputFormatClassName())) {
                ...
                if (job.get(hive_metastoreConstants.META_TABLE_PARTITION_COLUMNS, "").isEmpty()) {
                    ...
                    if (!partitions.isEmpty()) {
                        ...
                        LOG.info(
                                "Setting Partitions in jobConf - Partition Keys for Path : "
                                        + path
                                        + " is :"
                                        + partitionStr);
            ...
            if (!mrwork.isMapperCannotSpanPartns()) {
                ...
                if (f == null) {
                    ...
                    LOG.info(
                            "CombineHiveInputSplit: pool is already created for "
                                    + path
                                    + "; using filter path "
                                    + filterPath);
                ...
                // In the case of tablesample, the input paths are pointing to files rather than
                // directories.
                // We need to get the parent directory as the filtering path so that all files in
                // the same
                // parent directory will be grouped into one pool but not files from different
                // parent
                // directories. This guarantees that a split will combine all files in the same
                // partition
    ...
    public Set<Integer> getNonCombinablePathIndices(JobConf job, Path[] paths, int numThreads)
        ...
        try {
            for (int i = 0; i < numThreads; i++) {
                ...
                futureList.add(
                        executor.submit(new CheckNonCombinablePathCallable(paths, start, length, job)));
            }
    ...
    public InputSplit[] getSplits(JobConf job, int numSplits) throws IOException {
        ...
        int numThreads =
                Math.min(
                        MAX_CHECK_NONCOMBINABLE_THREAD_NUM,
    ...
    private void processPaths(
            JobConf job, CombineFileInputFormatShim combine, List<CombineFileSplit> iss, Path... path)
            throws IOException {
    ...
    /**
      ...
      * @throws IOException
      */
    private static PartitionDesc getPartitionFromPath(
            Map<Path, PartitionDesc> pathToPartitionInfo,
            Path dir,
        ...
        try {
            ...
            try {
                // HiveFileFormatUtils.getPartitionDescFromPathRecursively method only available in
                // Hive 2.x
                method =
                        hiveUtilsClass.getMethod(
                                "getPartitionDescFromPathRecursively", Map.class, Path.class, Map.class);
            } catch (NoSuchMethodException e) {
                // HiveFileFormatUtils.getFromPathRecursively method only available in Hive 3.x
                method =
                        hiveUtilsClass.getMethod("getFromPathRecursively", Map.class, Path.class, Map.class);
            }
    ...
    /**
      ...
      *
      * <p>First, splits are grouped by alias they are for. If one split serves more than one alias or
      * not for any sampled alias, we just directly add it to returned list. Then we find a list of
    ...
    private List<CombineFileSplit> sampleSplits(List<CombineFileSplit> splits) {
        ...
        for (CombineFileSplit split : splits) {
            ...
            for (Path path : split.getPaths()) {
                ...
                List<String> l =
                        HiveFileFormatUtils.doGetAliasesFromPath(
                ...
                if (l.size() != 1
                        || !nameToSamples.containsKey(l.get(0))
                        || (alias != null && !Objects.equals(l.get(0), alias))) {
                    alias = null;
                    break;
                }
                alias = l.get(0);
    ...
    public static class CombineHiveInputSplit extends InputSplitShim {
        ...
        public CombineHiveInputSplit(
                JobConf job, CombineFileSplit inputSplitShim, Map<Path, PartitionDesc> pathToPartitionInfo)
                throws IOException {
            ...
            if (o instanceof CombinePathInputFormat) {
                ...
                return (opList.equals(mObj.opList))
                        ...
                        && (deserializerClassName == null
                                ? (mObj.deserializerClassName == null)
    ...
    public static class HoodieCombineFileInputFormatShim<K, V> extends CombineFileInputFormat<K, V>
        ...
        protected List<FileStatus> listStatus(JobContext job) throws IOException {
            ...
            if (hoodieFilter) {
                ...
                result =
                        new ArrayList<>(Arrays.asList(input.listStatus(new JobConf(job.getConfiguration()))));
            } else {
        ...
        public CombineFileSplit[] getSplits(JobConf job, int numSplits) throws IOException {
            long minSize =
                    job.getLong(org.apache.hadoop.mapreduce.lib.input.FileInputFormat.SPLIT_MINSIZE, 0L);
            long maxSize =
                    job.getLong(org.apache.hadoop.mapreduce.lib.input.FileInputFormat.SPLIT_MAXSIZE, minSize);
            ...
            if (isRealTime) {
                ...
                HoodieCombineRealtimeFileSplit.Builder builder =
                        new HoodieCombineRealtimeFileSplit.Builder();
                int counter = 0;
                ...
                for (int pos = 0; pos < splits.length; ++pos) {
                    ...
                    if (split.getPaths().length > 0) {
                        inputSplitShims.add(
                                new HadoopShimsSecure.InputSplitShim(
                                        job,
                                        split.getPaths(),
                                        split.getStartOffsets(),
                                        split.getLengths(),
                                        split.getLocations()));
                    }
                }
                return (CombineFileSplit[])
                        inputSplitShims.toArray(new HadoopShimsSecure.InputSplitShim[inputSplitShims.size()]);
            }
        }
        ...
        public RecordReader getRecordReader(
                JobConf job, CombineFileSplit split, Reporter reporter, Class<RecordReader<K, V>> rrClass)
                throws IOException {
            isRealTime = Boolean.valueOf(job.get("hudi.hive.realtime", "false"));
            if (isRealTime) {
                List<RecordReader> recordReaders = new LinkedList<>();
                ValidationUtils.checkArgument(
                        split instanceof HoodieCombineRealtimeFileSplit,
                        "Only "
                                + HoodieCombineRealtimeFileSplit.class.getName()
                                + " allowed, found "
                                + split.getClass().getName());
                for (InputSplit inputSplit :
                        ((HoodieCombineRealtimeFileSplit) split).getRealtimeFileSplits()) {
                    if (split.getPaths().length == 0) {
                        continue;
                    }
                    FileInputFormat inputFormat =
                            HoodieInputFormatUtils.getInputFormat(split.getPath(0).toString(), true, job);
