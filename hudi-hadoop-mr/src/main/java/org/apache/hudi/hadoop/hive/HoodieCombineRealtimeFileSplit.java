...
public class HoodieCombineRealtimeFileSplit extends CombineFileSplit {
    ...
    public HoodieCombineRealtimeFileSplit(JobConf jobConf, List<FileSplit> realtimeFileSplits) {
        super(
                jobConf,
                realtimeFileSplits.stream()
                        .map(p -> ((HoodieRealtimeFileSplit) p).getPath())
                        .collect(Collectors.toList())
                        .toArray(new Path[realtimeFileSplits.size()]),
                ArrayUtils.toPrimitive(
                        realtimeFileSplits.stream()
                                .map(p -> ((HoodieRealtimeFileSplit) p).getStart())
                                .collect(Collectors.toList())
                                .toArray(new Long[realtimeFileSplits.size()])),
                ArrayUtils.toPrimitive(
                        realtimeFileSplits.stream()
                                .map(p -> ((HoodieRealtimeFileSplit) p).getLength())
                                .collect(Collectors.toList())
                                .toArray(new Long[realtimeFileSplits.size()])),
                realtimeFileSplits.stream()
                        .map(
                        ...
                        .flatMap(List::stream)
                        .collect(Collectors.toList())
                        .toArray(new String[realtimeFileSplits.size()]));
    ...
    public String toString() {
        return "HoodieCombineRealtimeFileSplit{" + "realtimeFileSplits=" + realtimeFileSplits + '}';
    }
    ...
    public String[] getLocations() throws IOException {
        return realtimeFileSplits.stream()
                .flatMap(
                        x -> {
                            try {
