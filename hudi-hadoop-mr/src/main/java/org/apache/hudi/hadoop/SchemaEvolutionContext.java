...
/**
  * This class is responsible for calculating names and types of fields that are actual at a certain
  * point in time for hive. If field is renamed in queried schema, its old name will be returned,
  * which is relevant at the provided time. If type of field is changed, its old type will be
  * returned, and projection will be created that will convert the old type to the queried one.
  */
public class SchemaEvolutionContext {

    private static final Logger LOG = LoggerFactory.getLogger(SchemaEvolutionContext.class);

    private static final String HIVE_TMP_READ_COLUMN_NAMES_CONF_STR =
            "hive.tmp.io.file.readcolumn.ids";
    ...
    public SchemaEvolutionContext(
            InputSplit split, JobConf job, Option<HoodieTableMetaClient> metaClientOption)
            throws IOException {
        this.split = split;
        this.job = job;
        this.metaClient =
                metaClientOption.isPresent() ? metaClientOption.get() : setUpHoodieTableMetaClient();
        ...
        try {
            ...
            LOG.warn(
                    String.format(
                            "failed to get internal Schema from hudi table：%s", metaClient.getBasePath()),
                    e);
        }
    ...
    private HoodieTableMetaClient setUpHoodieTableMetaClient() throws IOException {
        try {
            ...
            return HoodieTableMetaClient.builder()
                    .setBasePath(tablePath.get().toString())
                    .setConf(HadoopFSUtils.getStorageConfWithCopy(job))
                    .build();
        } catch (Exception e) {
            LOG.warn(
                    String.format("Not a valid hoodie table, table path: %s", ((FileSplit) split).getPath()),
                    e);
            return null;
        }
    }
    ...
    /**
      ...
      * @return
      */
    public void doEvolutionForRealtimeInputFormat(AbstractRealtimeRecordReader realtimeRecordReader)
            throws Exception {
        ...
        if (internalSchemaOption.isPresent()) {
            ...
            InternalSchema prunedInternalSchema =
                    InternalSchemaUtils.pruneInternalSchema(internalSchemaOption.get(), requiredColumns);
            // Add partitioning fields to writer schema for resulting row to contain null values for
            // these fields
            ...
            Schema writerSchema =
                    AvroInternalSchemaConverter.convert(
                            internalSchemaOption.get(), tableAvroSchema.getName());
            writerSchema =
                    HoodieRealtimeRecordReaderUtils.addPartitionFields(writerSchema, partitioningFields);
            Map<String, Schema.Field> schemaFieldsMap =
                    HoodieRealtimeRecordReaderUtils.getNameToFieldMap(writerSchema);
            // we should get HoodieParquetInputFormat#HIVE_TMP_COLUMNS,since
            // serdeConstants#LIST_COLUMNS maybe change by
            // HoodieParquetInputFormat#setColumnNameList
            Schema hiveSchema =
                    realtimeRecordReader.constructHiveOrderedSchema(
                            writerSchema, schemaFieldsMap, job.get(HIVE_TMP_COLUMNS));
            Schema readerSchema =
            ...
            LOG.info(
                    "About to read compacted logs {} for base split {}, projecting cols {}",
                    realtimeSplit.getDeltaLogPaths(),
                    realtimeSplit.getPath(),
                    requiredColumns);
        }
    }
    ...
    public void doEvolutionForParquetFormat() {
        if (internalSchemaOption.isPresent()) {
            ...
            if (!disableSchemaEvolution) {
                ...
                InternalSchema mergedInternalSchema =
                        new InternalSchemaMerger(fileSchema, querySchema, true, true).mergeSchema();
        ...
        for (int i = 0; i < tmpColIdList.size(); i++) {
            ...
            TypeInfo typeInfo =
                    TypeInfoUtils.getTypeInfosFromTypeString(
                                    fullTypeInfos.get(tmpColIdList.get(i)).getQualifiedName())
                            .get(0);
    ...
    private TypeInfo constructHiveSchemaFromType(Type type, TypeInfo typeInfo) {
        switch (type.typeId()) {
            case RECORD:
                ...
                for (int index = 0; index < fields.size(); index++) {
                    ...
                    TypeInfo subTypeInfo =
                            getSchemaSubTypeInfo(
                                    structTypeInfo.getAllStructFieldTypeInfos().get(index), fields.get(index).type());
            ...
            case ARRAY:
                ...
                TypeInfo subTypeInfo =
                        getSchemaSubTypeInfo(listTypeInfo.getListElementTypeInfo(), array.elementType());
            ...
            case MAP:
                ...
                TypeInfo valueType =
                        getSchemaSubTypeInfo(mapTypeInfo.getMapValueTypeInfo(), map.valueType());
            ...
            case TIME:
                throw new UnsupportedOperationException(
                        String.format("cannot convert %s type to hive", type));
            default:
                LOG.error("cannot convert unknown type: {} to Hive", type);
                throw new UnsupportedOperationException(
                        String.format("cannot convert unknown type: %s to Hive", type));
        }
    }

    private TypeInfo getSchemaSubTypeInfo(TypeInfo hoodieTypeInfo, Type hiveType) {
        TypeInfo subTypeInfo =
                TypeInfoUtils.getTypeInfosFromTypeString(hoodieTypeInfo.getQualifiedName()).get(0);
        TypeInfo typeInfo;
    ...
    private void pushDownFilter(JobConf job, InternalSchema querySchema, InternalSchema fileSchema) {
        ...
        if (filterExprSerialized != null) {
            ...
            while (!exprNodes.isEmpty()) {
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    if (expr instanceof ExprNodeColumnDesc) {
                        ...
                        String newColumn =
                                InternalSchemaUtils.reBuildFilterName(oldColumn, fileSchema, querySchema);
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug(
                        "Pushdown initiated with filterText = {}, filterExpr = {}, serializedFilterExpr = {}",
                        filterText,
                        filterExpr,
    ...
    public static List<String> getRequireColumn(JobConf jobConf) {
        ...
        if (StringUtils.isNullOrEmpty(originColumnString)) {
            jobConf.set(
                    HIVE_TMP_READ_COLUMN_NAMES_CONF_STR,
