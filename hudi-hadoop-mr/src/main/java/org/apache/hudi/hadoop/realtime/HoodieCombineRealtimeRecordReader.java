...
public class HoodieCombineRealtimeRecordReader
    ...
    public HoodieCombineRealtimeRecordReader(
            JobConf jobConf, CombineFileSplit split, List<RecordReader> readers) {
        useFileGroupReader = shouldUseFilegroupReader(jobConf);
        try {
            ValidationUtils.checkArgument(
                    ((HoodieCombineRealtimeFileSplit) split).getRealtimeFileSplits().size() == readers.size(),
                    "Num Splits does not match number of unique RecordReaders!");
            for (InputSplit rtSplit : ((HoodieCombineRealtimeFileSplit) split).getRealtimeFileSplits()) {
                if (useFileGroupReader) {
                    ...
                    ValidationUtils.checkArgument(
                            reader instanceof HoodieFileGroupReaderBasedRecordReader,
                            reader.toString() + "not instance of HoodieFileGroupReaderRecordReader ");
                    recordReaders.add(reader);
                } else {
                    LOG.info("Creating new RealtimeRecordReader for split");
                    recordReaders.add(
                            new HoodieRealtimeRecordReader(
                                    (HoodieRealtimeFileSplit) rtSplit, jobConf, readers.remove(0)));
                }
            }
