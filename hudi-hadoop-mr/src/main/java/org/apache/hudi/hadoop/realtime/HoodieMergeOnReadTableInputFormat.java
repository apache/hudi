...
/**
  ...
  *
  * <ul>
  *   <li>Snapshot mode: reading table's state as of particular timestamp (or instant, in Hudi's
  *       terms)
  *   <li>Incremental mode: reading table's state as of particular timestamp (or instant, in Hudi's
  *       terms)
  *   <li>External mode: reading non-Hudi partitions
  * </ul>
  *
  * <p>NOTE: This class is invariant of the underlying file-format of the files being read
  */
public class HoodieMergeOnReadTableInputFormat extends HoodieCopyOnWriteTableInputFormat
        implements Configurable {

    @Override
    public InputSplit[] getSplits(JobConf job, int numSplits) throws IOException {
        List<FileSplit> fileSplits =
                Arrays.stream(super.getSplits(job, numSplits))
                        .map(is -> (FileSplit) is)
                        .collect(Collectors.toList());

        return (containsIncrementalQuerySplits(fileSplits)
                        ? filterIncrementalQueryFileSplits(fileSplits)
                        : fileSplits)
                .toArray(new FileSplit[0]);
    }

    @Override
    protected FileStatus createFileStatusUnchecked(
            FileSlice fileSlice,
        ...
        if (baseFileOpt.isPresent()) {
            return createRealtimeFileStatusUnchecked(
                    baseFileOpt.get(),
                    logFiles,
                    tableBasePath,
            ...
            throw new IllegalStateException(
                    "Invalid state: either base-file or log-file has to be present");
        }
    }
        ...
        for (FileStatus fileStatus : fileStatuses) {
            ...
            RealtimeFileStatus realtimeFileStatus =
                    new RealtimeFileStatus(
                            fileStatus, baseFilePath, new ArrayList<>(), false, Option.empty());
    ...
    protected boolean checkIfValidFileSlice(FileSlice fileSlice) {
        ...
        if (baseFileOpt.isPresent() || latestLogFileOpt.isPresent()) {
            ...
            throw new IllegalStateException(
                    "Invalid state: either base-file or log-file has to be present for "
                            + fileSlice.getFileId());
        }
    }

    /**
      * Keep the logic of mor_incr_view as same as spark datasource. Step1: Get list of commits to be
      * fetched based on start commit and max commits(for snapshot max commits is -1). Step2: Get list
      * of affected files status for these affected file status. Step3: Construct
      * HoodieTableFileSystemView based on those affected file status. a. Filter affected partitions
      * based on inputPaths. b. Get list of fileGroups based on affected partitions by
      * fsView.getAllFileGroups. Step4: Set input paths based on filtered affected partition paths.
      * changes that among original input paths passed to this method. some partitions did not have
      * commits as part of the trimmed down list of commits and hence we need this step. Step5: Find
      * candidate fileStatus, since when we get baseFileStatus from HoodieTableFileSystemView, the
      * BaseFileStatus will missing file size information. We should use candidate fileStatus to update
      * the size information for BaseFileStatus. Step6: For every file group from step3(b) Get 1st
      * available base file from all file slices. then we use candidate file status to update the
      * baseFileStatus, and construct RealTimeFileStatus and add it to result along with log files. If
      * file group just has log files, construct RealTimeFileStatus and add it to result. TODO: unify
      * the incremental view code between hive/spark-sql and spark datasource
      */
    @Override
    protected List<FileStatus> listStatusForIncrementalMode(
        ...
        HoodieTimeline commitsTimelineToReturn =
                HoodieInputFormatUtils.getHoodieTimelineForIncrementalQuery(
                        jobContext, incrementalTableName, timeline.get());
        ...
        List<StoragePathInfo> affectedPathInfoList =
                HoodieInputFormatUtils.listAffectedFilesForCommits(
                        job, tableMetaClient.getBasePath(), metadataList);
        // step3
        ...
        List<String> affectedPartition =
                HoodieTableMetadataUtil.getWritePartitionPaths(metadataList).stream()
                        .filter(
                                k ->
                                        k.isEmpty()
                                                ? inputPaths.contains(basePath)
                                                : inputPaths.contains(new Path(basePath, k)))
                        .collect(Collectors.toList());
        if (affectedPartition.isEmpty()) {
            return result;
        }
        List<HoodieFileGroup> fileGroups =
                affectedPartition.stream()
    ...
    // make split for path.
    // When query the incremental view, the read files may be bootstrap files, we wrap those
    ...
    @Override
    protected FileSplit makeSplit(
            Path file, long start, long length, String[] hosts, String[] inMemoryHosts) {
        if (file instanceof HoodieRealtimePath) {
            return doMakeSplitForRealtimePath(
                    (HoodieRealtimePath) file, start, length, hosts, inMemoryHosts);
        }
        return super.makeSplit(file, start, length, hosts, inMemoryHosts);
    }

    private static List<FileStatus> collectAllIncrementalFiles(
            List<HoodieFileGroup> fileGroups,
                            ...
                            try {
                                ...
                                if (!baseFiles.isEmpty()) {
                                    ...
                                    if (!candidateFileStatus.containsKey(baseFilePath)) {
                                        throw new HoodieException(
                                                "Error obtaining fileStatus for file: " + baseFilePath);
                                    }
                                    List<HoodieLogFile> deltaLogFiles =
                                            f.getLatestFileSlice().get().getLogFiles().collect(Collectors.toList());
                                    // We cannot use baseFileStatus.getPath() here, since
                                    // baseFileStatus.getPath() missing file size information.
                                    // So we use candidateFileStatus.get(baseFileStatus.getPath())
                                    // to get a correct path.
                                    RealtimeFileStatus fileStatus =
                                            new RealtimeFileStatus(
                                                    candidateFileStatus.get(baseFilePath),
                                                    basePath,
                                                    deltaLogFiles,
                                                    true,
                                                    virtualKeyInfoOpt);
                                ...
                                if (f.getLatestFileSlice().isPresent() && baseFiles.isEmpty()) {
                                    List<StoragePathInfo> logPathInfoList =
                                            f.getLatestFileSlice()
                                                    .get()
                                                    .getLogFiles()
                                                    .map(logFile -> logFile.getPathInfo())
                                                    .collect(Collectors.toList());
                                    if (logPathInfoList.size() > 0) {
                                        List<HoodieLogFile> deltaLogFiles =
                                                logPathInfoList.stream()
                                                        .map(l -> new HoodieLogFile(l.getPath(), l.getLength()))
                                                        .collect(Collectors.toList());
                                        RealtimeFileStatus fileStatus =
                                                new RealtimeFileStatus(
                                                        HadoopFSUtils.convertToHadoopFileStatus(logPathInfoList.get(0)),
                                                        basePath,
                                                        deltaLogFiles,
                                                        true,
                                                        virtualKeyInfoOpt);
    ...
    private FileSplit doMakeSplitForRealtimePath(
            HoodieRealtimePath path, long start, long length, String[] hosts, String[] inMemoryHosts) {
        if (path.includeBootstrapFilePath()) {
            FileSplit bf =
                    inMemoryHosts == null
                            ? super.makeSplit(path.getPathWithBootstrapFileStatus(), start, length, hosts)
                            : super.makeSplit(
                                    path.getPathWithBootstrapFileStatus(), start, length, hosts, inMemoryHosts);
    ...
    private static boolean containsIncrementalQuerySplits(List<FileSplit> fileSplits) {
        return fileSplits.stream()
                .anyMatch(HoodieRealtimeInputFormatUtils::doesBelongToIncrementalQuery);
    }

    private static List<FileSplit> filterIncrementalQueryFileSplits(List<FileSplit> fileSplits) {
        return fileSplits.stream()
                .filter(HoodieRealtimeInputFormatUtils::doesBelongToIncrementalQuery)
                .collect(Collectors.toList());
    }

    private static HoodieRealtimeBootstrapBaseFileSplit createRealtimeBootstrapBaseFileSplit(
            BootstrapBaseFileSplit split,
            String basePath,
        ...
        try {
            String[] hosts =
                    split.getLocationInfo() != null
                            ? Arrays.stream(split.getLocationInfo())
                                    .filter(x -> !x.isInMemory())
                                    .toArray(String[]::new)
                            : new String[0];
            String[] inMemoryHosts =
                    split.getLocationInfo() != null
                            ? Arrays.stream(split.getLocationInfo())
                                    .filter(SplitLocationInfo::isInMemory)
                                    .toArray(String[]::new)
                            : new String[0];
            FileSplit baseSplit =
                    new FileSplit(split.getPath(), split.getStart(), split.getLength(), hosts, inMemoryHosts);
            return new HoodieRealtimeBootstrapBaseFileSplit(
                    baseSplit,
                    basePath,
                    logFiles,
                    maxInstantTime,
                    split.getBootstrapFileSplit(),
                    belongsToIncrementalQuery,
                    virtualKeyInfoOpt);
        } catch (IOException e) {
    ...
    private static RealtimeFileStatus createRealtimeFileStatusUnchecked(
        ...
        try {
            RealtimeFileStatus rtFileStatus =
                    new RealtimeFileStatus(
                            baseFileStatus, basePath, sortedLogFiles, false, virtualKeyInfoOpt);
            ...
            throw new HoodieIOException(
                    String.format("Failed to init %s", RealtimeFileStatus.class.getSimpleName()), e);
        }
    }
    ...
    private static RealtimeFileStatus createRealtimeFileStatusUnchecked(
        ...
        try {
            RealtimeFileStatus rtFileStatus =
                    new RealtimeFileStatus(
                            HadoopFSUtils.convertToHadoopFileStatus(latestLogFile.getPathInfo()),
                            basePath,
                            sortedLogFiles,
                            false,
                            virtualKeyInfoOpt);
    ...
    private static Option<HoodieVirtualKeyInfo> getHoodieVirtualKeyInfo(
            HoodieTableMetaClient metaClient) {
