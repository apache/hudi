...
/**
  * Given a path is a part of - Hoodie table = accepts ONLY the latest version of each path -
  * Non-Hoodie table = then always accept
  *
  * <p>We can set this filter, on a query engine's Hadoop Config and if it respects path filters,
  * then you should be able to query both hoodie and non-hoodie tables as you would normally do.
  *
  * <p>hadoopConf.setClass("mapreduce.input.pathFilter.class", org.apache.hudi.hadoop
        ...
        try {
            ...
            if (hoodiePathCache.containsKey(folder.toString())) {
                if (LOG.isDebugEnabled()) {
                    LOG.debug(
                            String.format(
                                    "%s Hoodie path checked against cache, accept => %s \n",
            ...
            if (baseDir != null) {
                ...
                try {
                    ...
                    if (nonEmpty(timestampAsOf)) {
                        ...
                        // Build FileSystemViewManager with specified time, it's necessary to set
                        // this config when you may
                        // access old version files. For example, in spark side, using
                        // "hoodie.datasource.read.paths"
                        // which contains old version files, if not specify this value, these files
                        // will be filtered.
                        fsView =
                                FileSystemViewManager.createInMemoryFileSystemViewWithTimeline(
                                        engineContext,
                                        metaClient,
                                        HoodieInputFormatUtils.buildMetadataConfig(conf),
                                        metaClient
                                                .getActiveTimeline()
                                                .filterCompletedInstants()
                                                .findInstantsBeforeOrEquals(timestampAsOf));
                    } else {
                        fsView =
                                FileSystemViewManager.createInMemoryFileSystemView(
                                        engineContext, metaClient, HoodieInputFormatUtils.buildMetadataConfig(conf));
                    }
                    String partition =
                            HadoopFSUtils.getRelativePartitionPath(
                                    new Path(metaClient.getBasePath().toString()), folder);
                    ...
                    LOG.info(
                            "Based on hoodie metadata from base path: "
                                    + baseDir.toString()
                                    + ", caching "
                                    + latestFiles.size()
                                    + " files under "
                    ...
                    if (LOG.isDebugEnabled()) {
                        LOG.debug(
                                String.format(
                                        "%s checked after cache population, accept => %s \n",
                                        path, hoodiePathCache.get(folder.toString()).contains(path)));
                    }
                    return hoodiePathCache.get(folder.toString()).contains(path);
                    ...
                    if (LOG.isDebugEnabled()) {
                        LOG.debug(
                                String.format(
                                        "(1) Caching non-hoodie path under %s with basePath %s \n",
