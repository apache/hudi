...
public class HoodieVirtualKeyInfo implements Serializable {
    ...
    public String toString() {
        return "HoodieVirtualKeyInfo{"
                ...
                + '\''
                + ", partitionPathField='"
                + (partitionPathField.isPresent() ? partitionPathField.get() : "null")
                + '\''
                + ", recordKeyFieldIndex="
                + recordKeyFieldIndex
                + ", partitionPathFieldIndex="
                + (partitionPathFieldIndex.isPresent() ? partitionPathFieldIndex.get() : "-1")
                + '}';
    }
}
