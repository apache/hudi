...
public class HoodieParquetRealtimeInputFormat extends HoodieParquetInputFormat {
    ...
    // not handling empty list correctly, the ParquetRecordReaderWrapper ends up adding the same
    // column ids multiple
    // times which ultimately breaks the query.
    @Override
    public RecordReader<NullWritable, ArrayWritable> getRecordReader(
            final InputSplit split, final JobConf jobConf, final Reporter reporter) throws IOException {
        // sanity check
        ValidationUtils.checkArgument(
                split instanceof RealtimeSplit,
        ...
        HoodieTableMetaClient metaClient =
                HoodieTableMetaClient.builder()
                        ...
                        .setBasePath(realtimeSplit.getBasePath())
                        .build();
        HoodieTableConfig tableConfig = metaClient.getTableConfig();
        addProjectionToJobConf(realtimeSplit, jobConf, tableConfig);
        LOG.info(
                "Creating record reader with readCols :"
        ...
        if (isLogFile(realtimeSplit.getPath())) {
            return new HoodieRealtimeRecordReader(
                    realtimeSplit, jobConf, new HoodieEmptyRecordReader(realtimeSplit, jobConf));
        }

        return new HoodieRealtimeRecordReader(
                realtimeSplit, jobConf, super.getRecordReader(split, jobConf, reporter));
    }

    void addProjectionToJobConf(
            final RealtimeSplit realtimeSplit, final JobConf jobConf, HoodieTableConfig tableConfig) {
        // Hive on Spark invokes multiple getRecordReaders from different threads in the same spark
        // task (and hence the
        // same JVM) unlike Hive on MR. Due to this, accesses to JobConf, which is shared across all
        // threads, is at the
        // risk of experiencing race conditions. Hence, we synchronize on the JobConf object here.
        // There is negligible
        // latency incurred here due to the synchronization since get record reader is called once
        // per spilt before the
        ...
        if (HoodieRealtimeInputFormatUtils.canAddProjectionToJobConf(realtimeSplit, jobConf)) {
            synchronized (jobConf) {
                ...
                if (HoodieRealtimeInputFormatUtils.canAddProjectionToJobConf(realtimeSplit, jobConf)) {
                    ...
                    // In this case, the projection fields gets removed. Looking at HiveInputFormat
                    // implementation, in some cases
                    // hoodie additional projection columns are reset after calling setConf and only
                    // natural projections
                    // (one found in select queries) are set. things would break because of this.
                    // For e:g _hoodie_record_key would be missing and merge step would throw
                    // exceptions.
                    // TO fix this, hoodie columns are appended late at the time record-reader gets
                    // built instead of construction
                    // time.
                    List<String> fieldsToAdd = new ArrayList<>();
                    if (!realtimeSplit.getDeltaLogPaths().isEmpty()) {
                        HoodieRealtimeInputFormatUtils.addVirtualKeysProjection(
                                jobConf, realtimeSplit.getVirtualKeyInfo());
                    ...
                    HoodieRealtimeInputFormatUtils.addProjectionField(
                            jobConf, fieldsToAdd.toArray(new String[0]));
