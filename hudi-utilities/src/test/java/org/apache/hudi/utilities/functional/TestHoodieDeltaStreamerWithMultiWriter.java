...
public class TestHoodieDeltaStreamerWithMultiWriter extends SparkClientFunctionalTestHarness {
  ...
  private void runJobsInParallel(String tableBasePath, HoodieTableType tableType, int totalRecords,
      ...
      HoodieDeltaStreamer.Config cfgBackfillJob, boolean expectConflict, String str) throws Exception {
    ...
    try {
      regularIngestionJobFuture = service.submit(() -> {
        try {
          deltaStreamerTestRunner(ingestionJob, cfgIngestionJob, conditionForRegularIngestion, str);
        } catch (Throwable ex) {
      ...
      backfillJobFuture = service.submit(() -> {
        try {
          // trigger backfill atleast after 1 requested entry is added to timline from continuous job.
      ...
      if (expectConflict && e.getCause().getMessage().contains(ConcurrentModificationException.class.getName())) {
        ...
        if (backfillFailed.get()) {
          LOG.warn("Calling shutdown on ingestion job since the backfill job has failed for " + str);
