...
public class TestHoodieStreamerUtils extends UtilitiesTestBase {
    ...
    public void testCreateHoodieRecordsWithError(HoodieRecordType recordType) {
        ...
        JavaRDD<GenericRecord> recordRdd =
                jsc.parallelize(Collections.singletonList(1))
                        .map(
                                i -> {
                                    ...
                                    record.put(1, "key" + i);
                                    record.put(2, "path" + i);
                                    // The field is non-null in schema but the value is null, so
                                    // this fails the Hudi record creation
                                    record.put(3, null);
                                    record.put(4, "driver");
                                    return record;
                                });
        ...
        List<ErrorEvent<String>> actualErrorEvents =
                (List<ErrorEvent<String>>) errorEventCaptor.getValue().collect();
        ErrorEvent<String> expectedErrorEvent =
                new ErrorEvent<>(
                        "{\"timestamp\": 1000, \"_row_key\": \"key1\", \"partition_path\": \"path1\", \"rider\": null, \"driver\": \"driver\"}",
