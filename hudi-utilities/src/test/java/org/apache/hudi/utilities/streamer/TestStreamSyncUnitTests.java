...
public class TestStreamSyncUnitTests {
    ...
    void testFetchNextBatchFromSource(
            Boolean useRowWriter,
            Boolean hasTransformer,
            Boolean hasSchemaProvider,
            Boolean isNullTargetSchema,
            Boolean hasErrorTable,
            Boolean shouldTryWriteToErrorTable) {
        ...
        InputBatch<Dataset<Row>> fakeRowInputBatch =
                new InputBatch<>(fakeDataFrame, "chkpt", inputBatchSchemaProvider);
        when(sourceFormatAdapter.fetchNewDataInRowFormat(any(), anyLong()))
                .thenReturn(fakeRowInputBatch);
        // batch is empty because we don't want getBatch().map() to do anything because it calls
        // static method we can't mock
        InputBatch<JavaRDD<GenericRecord>> fakeAvroInputBatch =
                new InputBatch<>(Option.empty(), "chkpt", inputBatchSchemaProvider);
        when(sourceFormatAdapter.fetchNewDataInAvroFormat(any(), anyLong()))
                .thenReturn(fakeAvroInputBatch);

        // transformer
        // return empty because we don't want .map() to do anything because it calls static method
        // we can't mock
    ...
    void testGetCheckpointToResume(
        ...
        StreamSync streamSync =
                new StreamSync(
                        ...
                        true);
        StreamSync spy = spy(streamSync);
        doReturn(Option.of(commitMetadata))
                .when(spy)
                .getLatestCommitMetadataWithValidCheckpointInfo(any());
    ...
    private static Stream<Arguments> getCheckpointToResumeCases() {
        return Stream.of(
                ...
                Arguments.of(
                        generateDeltaStreamerConfig("new-reset-checkpoint", null),
                        generateCommitMetadata("old-reset-checkpoint", null, null),
                        Option.of("new-reset-checkpoint")),
                // Checkpoint not reset/ Ignored, continuing from previous run
                Arguments.of(
                        generateDeltaStreamerConfig("old-reset-checkpoint", null),
                        generateCommitMetadata("old-reset-checkpoint", null, "checkpoint-prev-run"),
                        Option.of("checkpoint-prev-run")),
                // Checkpoint not reset/ Ignored, continuing from previous run (ignore checkpoint
                // has not changed)
                Arguments.of(
                        generateDeltaStreamerConfig("old-reset-checkpoint", "123445"),
                        generateCommitMetadata("old-reset-checkpoint", "123445", "checkpoint-prev-run"),
                        Option.of("checkpoint-prev-run")),
                // Ignore checkpoint set, existing checkpoints will be ignored
                Arguments.of(
                        generateDeltaStreamerConfig("old-reset-checkpoint", "123445"),
                        generateCommitMetadata("old-reset-checkpoint", "123422", "checkpoint-prev-run"),
                        Option.empty()),
                // Ignore checkpoint set, existing checkpoints will be ignored (reset-checkpoint
                // ignored)
                Arguments.of(
                        generateDeltaStreamerConfig("new-reset-checkpoint", "123445"),
                        generateCommitMetadata("old-reset-checkpoint", "123422", "checkpoint-prev-run"),
                        Option.empty()));
    }

    private static HoodieStreamer.Config generateDeltaStreamerConfig(
            String checkpoint, String ignoreCheckpoint) {
        HoodieStreamer.Config cfg = new HoodieStreamer.Config();
    ...
    private static HoodieCommitMetadata generateCommitMetadata(
            String resetCheckpointValue, String ignoreCheckpointValue, String checkpointValue) {
    ...
    static Stream<Arguments> testCasesFetchNextBatchFromSource() {
        ...
        for (Boolean useRowWriter : new Boolean[] {false, true}) {
            for (Boolean hasErrorTable : new Boolean[] {false, true}) {
                ...
                b.add(Arguments.of(useRowWriter, false, false, false, hasErrorTable, errorTableEnabled));
            }
        }
