...
public class TestHoodieDeltaStreamerSchemaEvolutionBase extends HoodieDeltaStreamerTestBase {
    ...
    protected Map<String, String> readOpts =
    ...
    protected HoodieDeltaStreamer.Config getDeltaStreamerConfig(boolean nullForDeletedCols)
            throws IOException {
        String[] transformerClasses =
                useTransformer
                        ? new String[] {TestHoodieDeltaStreamer.TestIdentityTransformer.class.getName()}
                        : new String[0];
        return getDeltaStreamerConfig(transformerClasses, nullForDeletedCols);
    }

    protected HoodieDeltaStreamer.Config getDeltaStreamerConfig(
            String[] transformerClasses, boolean nullForDeletedCols) throws IOException {
        return getDeltaStreamerConfig(transformerClasses, nullForDeletedCols, new TypedProperties());
    }

    protected HoodieDeltaStreamer.Config getDeltaStreamerConfig(
            String[] transformerClasses, boolean nullForDeletedCols, TypedProperties extraProps)
            throws IOException {
        ...
        extraProps.setProperty(
                DataSourceWriteOptions.SET_NULL_FOR_MISSING_COLUMNS().key(),
                Boolean.toString(nullForDeletedCols));

        // we set to 0 so that we create new base files on insert instead of adding inserts to
        // existing filegroups via small file handling
        extraProps.setProperty("hoodie.parquet.small.file.limit", "0");

        // We only want compaction/clustering to kick in after the final commit. This is because
        // after compaction/clustering we have base files again
        // and adding to base files is already covered by the tests. This is important especially
        // for mor, because we want to see how compaction/clustering
        ...
        if (shouldCluster) {
            ...
            extraProps.setProperty(
                    HoodieClusteringConfig.INLINE_CLUSTERING_MAX_COMMITS.key(), Integer.toString(maxCommits));
        ...
        if (withErrorTable) {
            ...
            extraProps.setProperty(
                    HoodieErrorTableConfig.ERROR_ENABLE_VALIDATE_TARGET_SCHEMA.key(), "true");
            extraProps.setProperty(
                    HoodieErrorTableConfig.ERROR_ENABLE_VALIDATE_RECORD_CREATION.key(), "true");
            extraProps.setProperty(HoodieErrorTableConfig.ERROR_TARGET_TABLE.key(), tableName + "ERROR");
            extraProps.setProperty(
                    HoodieErrorTableConfig.ERROR_TABLE_BASE_PATH.key(), basePath + tableName + "ERROR");
            extraProps.setProperty(
                    HoodieErrorTableConfig.ERROR_TABLE_WRITE_CLASS.key(), TestErrorTable.class.getName());
        ...
        if (useKafkaSource) {
            prepareAvroKafkaDFSSource(
                    PROPS_FILENAME_TEST_AVRO_KAFKA, null, topicName, "partition_path", extraProps);
            cfg =
                    TestHoodieDeltaStreamer.TestHelpers.makeConfig(
                            tableBasePath,
                            WriteOperationType.UPSERT,
                            AvroKafkaSource.class.getName(),
                            transformerClassNames,
                            PROPS_FILENAME_TEST_AVRO_KAFKA,
                            false,
                            useSchemaProvider,
                            100000,
                            false,
        ...
        if (useSchemaProvider) {
            TestSchemaProvider.sourceSchema =
                    AvroConversionUtils.convertStructTypeToAvroSchema(
                            df.schema(), HOODIE_RECORD_STRUCT_NAME, HOODIE_RECORD_NAMESPACE);
            if (withErrorTable && isFirst) {
                TestSchemaProvider.setTargetSchema(
                        AvroConversionUtils.convertStructTypeToAvroSchema(
                                TestHoodieSparkUtils.getSchemaColumnNotNullable(df.schema(), "_row_key"),
                                "idk",
                                "idk"));
            }
        }
    ...
    protected void addParquetData(Dataset<Row> df, Boolean isFirst) {
        df.write()
                .format("parquet")
                .mode(isFirst ? SaveMode.Overwrite : SaveMode.Append)
                .save(PARQUET_SOURCE_ROOT);
    }
    ...
    protected void addKafkaData(Dataset<Row> df, Boolean isFirst) {
        ...
        List<GenericRecord> records =
                HoodieSparkUtils.createRdd(
                                df, HOODIE_RECORD_STRUCT_NAME, HOODIE_RECORD_NAMESPACE, false, Option.empty())
                        .toJavaRDD()
                        .collect();
        try (Producer<String, byte[]> producer = new KafkaProducer<>(getProducerProperties())) {
            for (GenericRecord record : records) {
                producer.send(
                        new ProducerRecord<>(topicName, 0, "key", HoodieAvroUtils.avroToBytes(record)));
            }
        }
    }
    ...
    /**
      * see how many files are read from in the latest commit. This verification is for making sure the
      * test scenarios are setup as expected, rather than testing schema evolution functionality
      */
    protected void assertFileNumber(int expected, boolean isCow) {
        if (isCow) {
            assertBaseFileOnlyNumber(expected);
        } else {
            // we can't differentiate between _hoodie_file_name for log files, so we use commit time
            // as the differentiator between them
            assertEquals(
                    expected,
                    sparkSession
                            .read()
                            .format("hudi")
                            .load(tableBasePath)
                            .select("_hoodie_commit_time", "_hoodie_file_name")
                            .distinct()
                            .count());
        }
    }
    ...
    /**
      * Base files might have multiple different commit times in the same file. To ensure this is only
      * used when there are only base files there is a check that every file ends with .parquet, as log
      * files don't in _hoodie_file_name
      */
    protected void assertBaseFileOnlyNumber(int expected) {
        Dataset<Row> df =
                sparkSession.read().format("hudi").load(tableBasePath).select("_hoodie_file_name");
        df.createOrReplaceTempView("assertFileNumberPostCompactCluster");
        assertEquals(
                df.count(),
                sparkSession
                        .sql(
                                "select * from assertFileNumberPostCompactCluster where _hoodie_file_name like '%.parquet'")
                        .count());
    ...
    protected StructType createFareStruct(DataType amountType, Boolean dropCols) {
        if (dropCols) {
            return DataTypes.createStructType(
                    new StructField[] {new StructField("amount", amountType, true, Metadata.empty())});
        }
        return DataTypes.createStructType(
                new StructField[] {
                    new StructField("amount", amountType, true, Metadata.empty()),
    ...
    public static class TestErrorTable extends BaseErrorTableWriter {
        ...
        @Override
        public Option<JavaRDD<HoodieAvroRecord>> getErrorEvents(
                String baseTableInstantTime, Option commitedInstantTime) {
            return Option.empty();
        }
    }
}
