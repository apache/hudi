...
public class TestGenericRddTransform extends SparkClientFunctionalTestHarness {
    ...
    public void testGenericRddTransform() {
        Dataset ds =
                spark()
                        .range(10)
                        .withColumn(
                                "null_check_col", when(expr("id % 2 == 0"), lit("test")).otherwise(lit(null)));
        ...
        Tuple2<RDD<GenericRecord>, RDD<String>> failSafeRdds =
                HoodieSparkUtils.safeCreateRDD(ds, "record", "record", false, Option.of(nonNullSchema));
    ...
    public void testGenericRddConvert() {
        ...
        String schemaStr =
                makeFieldNonNull(HoodieTestDataGenerator.AVRO_SCHEMA, fieldToNull, "").toString();
        ...
        failSafeRdds =
                HoodieSparkUtils.safeRewriteRDD(rdd, HoodieTestDataGenerator.AVRO_SCHEMA.toString());
