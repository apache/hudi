...
public class TestPostgresDebeziumSource extends TestAbstractDebeziumSource {
    ...
    protected void validateMetaFields(Dataset<Row> records) {
        ...
        assertTrue(
                records.select(DebeziumConstants.FLATTENED_TX_ID_COL_NAME).collectAsList().stream()
                        .allMatch(r -> r.getLong(0) == TEST_TXID));
        assertTrue(
                records.select(DebeziumConstants.FLATTENED_LSN_COL_NAME).collectAsList().stream()
