...
public class DistributedTestDataSource extends AbstractBaseTestSource {
    ...
    public DistributedTestDataSource(
        ...
        this.numTestSourcePartitions =
                ConfigUtils.getIntWithAltKeys(props, SourceTestConfig.NUM_SOURCE_PARTITIONS_PROP);
    }
    ...
    protected InputBatch<JavaRDD<GenericRecord>> fetchNewData(
        ...
        String maxUniqueRecordsPerPartition =
                String.valueOf(Math.max(1, maxUniqueRecords / numTestSourcePartitions));
        newProps.setProperty(
                SourceTestConfig.MAX_UNIQUE_RECORDS_PROP.key(), maxUniqueRecordsPerPartition);
        int perPartitionSourceLimit = Math.max(1, (int) (sourceLimit / numTestSourcePartitions));
        JavaRDD<GenericRecord> avroRDD =
                sparkContext
                        .parallelize(
                                IntStream.range(0, numTestSourcePartitions).boxed().collect(Collectors.toList()),
                        ...
                        .mapPartitionsWithIndex(
                                (p, idx) -> {
                                    ...
                                    return fetchNextBatch(newProps, perPartitionSourceLimit, instantTime, p)
                                            .iterator();
                                },
                                true);
