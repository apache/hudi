...
public class TestDatePartitionPathSelector extends HoodieSparkClientTestHarness {
    ...
    /*
      ...
      */
    public List<StoragePath> createDatePartitionsWithFiles(
            List<StoragePath> leafDirs, boolean hiveStyle, String dateFormat) throws IOException {
    ...
    /**
      ...
      * @param leafDirs Collect list of leaf dirs. These will be the immediate parents of date based
      *     partitions.
      * @throws IOException
      */
    public void createParentDirsBeforeDatePartitions(
            StoragePath root, List<String> dirs, int depth, List<StoragePath> leafDirs)
            throws IOException {
        if (depth <= 0) {
            leafDirs.add(root);
            return;
    ...
    /*
      ...
      */
    private List<StoragePath> generateDatePartitionsUnder(
            StoragePath parent, boolean hiveStyle, String dateFormat) throws IOException {
    ...
    public void testPruneDatePartitionPaths(
        ...
        TypedProperties props =
                getProps(
                        ...
                        numPrevDaysToList,
                        currentDate);
        DatePartitionPathSelector pathSelector =
                new DatePartitionPathSelector(props, jsc.hadoopConfiguration());
        ...
        createParentDirsBeforeDatePartitions(
                root, generateRandomStrings(), totalDepthBeforeDatePartitions, leafDirs);
        createDatePartitionsWithFiles(leafDirs, isHiveStylePartition, dateFormat);

        List<String> paths =
                pathSelector.pruneDatePartitionPaths(
                        context,
