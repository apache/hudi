...
public class TestDatePartitionPathSelector extends HoodieSparkClientTestHarness {
   ...
   */
  public List<HoodieLocation> createDatePartitionsWithFiles(List<HoodieLocation> leafDirs, boolean hiveStyle, String dateFormat)
      throws IOException {
    List<HoodieLocation> allFiles = new ArrayList<>();
    for (HoodieLocation path : leafDirs) {
      List<HoodieLocation> datePartitions = generateDatePartitionsUnder(path, hiveStyle, dateFormat);
      for (HoodieLocation datePartition : datePartitions) {
   ...
   * @throws IOException
   */
  public void createParentDirsBeforeDatePartitions(HoodieLocation root, List<String> dirs,
                                                   int depth,
                                                   List<HoodieLocation> leafDirs)
      throws IOException {
    if (depth <= 0) {
      leafDirs.add(root);
      return;
    ...
    for (String s : dirs) {
      HoodieLocation subdir = new HoodieLocation(root, s);
   ...
   */
  private List<HoodieLocation> generateDatePartitionsUnder(HoodieLocation parent, boolean hiveStyle,
                                                           String dateFormat) throws IOException {
    DateTimeFormatter formatter = DateTimeFormatter.ofPattern(dateFormat);
    List<HoodieLocation> datePartitions = new ArrayList<>();
    String prefix = (hiveStyle ? "dt=" : "");
    for (int i = 0; i < 5; i++) {
      HoodieLocation child =
          new HoodieLocation(parent, prefix + formatter.format(totalDates.get(i)));
   ...
   */
  private List<HoodieLocation> createRandomFilesUnder(HoodieLocation path) throws IOException {
    List<HoodieLocation> resultFiles = new ArrayList<>();
    ...
    for (String fileName : fileNames) {
      ...
      HoodieLocation file = new HoodieLocation(path, fileName);
  ...
  public void testPruneDatePartitionPaths(
    ...
    HoodieLocation root = new HoodieLocation(getStringWithAltKeys(props, ROOT_INPUT_PATH));
    int totalDepthBeforeDatePartitions = props.getInteger(DATE_PARTITION_DEPTH.key()) - 1;

    // Create parent dir
    List<HoodieLocation> leafDirs = new ArrayList<>();
