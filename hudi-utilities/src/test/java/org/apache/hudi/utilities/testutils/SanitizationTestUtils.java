...
public class SanitizationTestUtils {
    ...
    protected static StructType getSchemaWithBadAvroNamingForStructType(boolean shouldSanitize) {
        StructType addressStruct =
                new StructType(
                        new StructField[] {
                            new StructField(
                                    sanitizeIfNeeded("@state.", shouldSanitize),
                                    DataTypes.StringType,
                                    true,
                                    Metadata.empty()),
                            new StructField(
                                    sanitizeIfNeeded("@@stree@t@", shouldSanitize),
                                    DataTypes.StringType,
                                    true,
                                    Metadata.empty()),
                            new StructField(
                                    sanitizeIfNeeded("8@_zip", shouldSanitize),
                                    DataTypes.LongType,
                                    true,
                                    Metadata.empty())
                        });
        ...
        StructType personStruct =
                new StructType(
                        new StructField[] {
                            new StructField(
                                    sanitizeIfNeeded("@_addr*$ess", shouldSanitize),
                                    addressStruct,
                                    true,
                                    Metadata.empty()),
                            new StructField(
                                    sanitizeIfNeeded("9name", shouldSanitize),
                                    DataTypes.StringType,
                                    true,
                                    Metadata.empty()),
                            new StructField(
                                    sanitizeIfNeeded("_occu9pation", shouldSanitize),
                                    DataTypes.StringType,
                                    true,
                                    Metadata.empty()),
                            new StructField(
                                    sanitizeIfNeeded("@plac.e.", shouldSanitize),
                                    DataTypes.StringType,
                                    true,
                                    Metadata.empty())
                        });
    ...
    protected static StructType getSchemaWithBadAvroNamingForArrayType(boolean shouldSanitize) {
        ...
        StructType personStruct =
                new StructType(
                        new StructField[] {
                            new StructField(
                                    sanitizeIfNeeded("@name", shouldSanitize),
                                    DataTypes.StringType,
                                    true,
                                    Metadata.empty()),
                            new StructField(
                                    sanitizeIfNeeded("@arr@", shouldSanitize),
                                    new ArrayType(addressStruct, true),
                                    true,
                                    Metadata.empty())
                        });
        return personStruct;
                ...
                new StructType(
                        new StructField[] {
                            new StructField(
                                    ...
                                    Metadata.empty()),
                            new StructField(
                                    sanitizeIfNeeded("@map9", shouldSanitize),
                                    new MapType(DataTypes.StringType, addressStruct, true),
                                    true,
                                    Metadata.empty()),
                        });
        return personStruct;
    ...
    public static Schema generateRenamedSchemaWithDefaultReplacement() {
        Schema addressSchema =
                SchemaBuilder.record("__Address")
                        .fields()
    ...
    public static Stream<Arguments> provideDataFiles() {
        return Stream.of(
                Arguments.of(
                        ...
                        getSchemaWithProperNaming()),
                Arguments.of(
                        "src/test/resources/data/avro_sanitization_bad_naming_in.json",
                ...
                Arguments.of(
                        "src/test/resources/data/avro_sanitization_bad_naming_nested_array_in.json",
                ...
                Arguments.of(
                        "src/test/resources/data/avro_sanitization_bad_naming_nested_map_in.json",
