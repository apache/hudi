...
public class MockConfigurationHotUpdateStrategy extends ConfigurationHotUpdateStrategy
        implements Serializable {
    private static final Logger LOG =
            LoggerFactory.getLogger(MockConfigurationHotUpdateStrategy.class);

    public MockConfigurationHotUpdateStrategy(
            HoodieDeltaStreamer.Config cfg, TypedProperties properties) {
    ...
    public Option<TypedProperties> updateProperties(TypedProperties currentProps) {
        if (currentProps.containsKey(UPSERT_PARALLELISM_VALUE.key())) {
            ...
            newProps.setProperty(
                    UPSERT_PARALLELISM_VALUE.key(), String.valueOf(upsertShuffleParallelism + 5));
            LOG.info(
                    "update {} from [{}] to [{}]",
                    UPSERT_PARALLELISM_VALUE.key(),
                    upsertShuffleParallelism,
                    upsertShuffleParallelism + 5);
