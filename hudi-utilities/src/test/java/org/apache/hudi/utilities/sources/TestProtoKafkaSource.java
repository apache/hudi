...
public class TestProtoKafkaSource extends BaseTestKafkaSource {
    ...
    protected TypedProperties createPropsForKafkaSource(
        ...
        props.setProperty(
                "hoodie.streamer.kafka.source.maxEvents",
        ...
        props.setProperty(
                ProtoClassBasedSchemaProviderConfig.PROTO_SCHEMA_CLASS_NAME.key(), Sample.class.getName());
        return props;
    }
    ...
    protected SourceFormatAdapter createSource(TypedProperties props) {
        ...
        Source protoKafkaSource =
                new ProtoKafkaSource(
                        props,
                        jsc(),
                        spark(),
                        metrics,
                        new DefaultStreamContext(schemaProvider, sourceProfile));
    ...
    public void testProtoKafkaSourceWithConfluentProtoDeserialization() {
        ...
        props.put(
                KAFKA_PROTO_VALUE_DESERIALIZER_CLASS.key(),
        ...
        props.setProperty(
                ProtoClassBasedSchemaProviderConfig.PROTO_SCHEMA_WRAPPED_PRIMITIVES_AS_RECORDS.key(),
                "true");
        ...
        ProtoKafkaSource protoKafkaSource =
        ...
        JavaRDD<Message> messagesRead =
                protoKafkaSource.fetchNext(Option.empty(), 1000).getBatch().get();
        assertEquals(
                messages.stream().map(this::protoToJson).collect(Collectors.toSet()),
                        ...
                        unused -> {
                            Sample.Builder builder =
                                    Sample.newBuilder()
                                            ...
                                            .setPrimitiveBytes(
                                                    ByteString.copyFrom(getUTF8Bytes(UUID.randomUUID().toString())));
                            ...
                            if (RANDOM.nextBoolean()) {
                                ...
                                builder
                                        .addAllRepeatedPrimitive(Arrays.asList(RANDOM.nextInt(), RANDOM.nextInt()))
                                        .putAllMapPrimitive(primitiveMap)
                                        .setNestedMessage(generateRandomNestedMessage())
                                        .addAllRepeatedMessage(
                                                Arrays.asList(generateRandomNestedMessage(), generateRandomNestedMessage()))
    ...
    protected void sendMessagesToKafka(String topic, int count, int numPartitions) {
        ...
        try (Producer<String, byte[]> producer = new KafkaProducer<>(getProducerProperties(false))) {
            for (int i = 0; i < messages.size(); i++) {
                ...
                producer.send(
                        new ProducerRecord<>(
                                topic, Integer.toString(i % numPartitions), messages.get(i).toByteArray()));
            }
        }
    }

    private void sendMessagesToKafkaWithConfluentSerializer(
            String topic, int numPartitions, List<Sample> messages) {
        try (Producer<String, Message> producer = new KafkaProducer<>(getProducerProperties(true))) {
            for (int i = 0; i < messages.size(); i++) {
                // use consistent keys to get even spread over partitions for test expectations
                producer.send(
                        new ProducerRecord<>(topic, Integer.toString(i % numPartitions), messages.get(i)));
            }
        }
    }
