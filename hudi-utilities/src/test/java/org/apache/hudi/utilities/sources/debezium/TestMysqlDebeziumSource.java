...
public class TestMysqlDebeziumSource extends TestAbstractDebeziumSource {
    ...
    private static final String MYSQL_GITHUB_SCHEMA =
            "{\"connect.name\": \"mysql.ghschema.gharchive.Envelope\",\n"
    ...
    protected GenericRecord generateMetaFields(GenericRecord rec) {
        ...
        GenericRecord sourceRecord =
                new GenericData.Record(schema.getField(DebeziumConstants.INCOMING_SOURCE_FIELD).schema());
    ...
    protected void validateMetaFields(Dataset<Row> records) {
        assertTrue(
                records.select(DebeziumConstants.FLATTENED_SHARD_NAME).collectAsList().stream()
                        .allMatch(r -> r.getString(0).equals(getIndexName())));
        assertTrue(
                records.select(DebeziumConstants.FLATTENED_TS_COL_NAME).collectAsList().stream()
                        .allMatch(r -> r.getLong(0) == TEST_TS_MS));
        assertTrue(
                records.select(DebeziumConstants.ADDED_SEQ_COL_NAME).collectAsList().stream()
