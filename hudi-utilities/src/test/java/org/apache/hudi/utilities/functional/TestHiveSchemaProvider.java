...
public class TestHiveSchemaProvider extends SparkClientFunctionalTestHarnessWithHiveSupport {
    ...
    public static void init() {
        ...
        PROPS.setProperty(
                "hoodie.streamer.schemaprovider.source.schema.hive.database", dbAndTableName.getLeft());
        PROPS.setProperty(
                "hoodie.streamer.schemaprovider.source.schema.hive.table", dbAndTableName.getRight());
    }
    ...
    public void testSourceSchema() throws Exception {
        try {
            ...
            Schema sourceSchema =
                    UtilHelpers.createSchemaProvider(HiveSchemaProvider.class.getName(), PROPS, jsc())
                            .getSourceSchema();

            Schema originalSchema =
                    new Schema.Parser()
                            .parse(
                                    UtilitiesTestBase.Helpers.readFile(
                                            "streamer-config/hive_schema_provider_source.avsc"));
    ...
    public void testTargetSchema() throws Exception {
        try {
            ...
            PROPS.setProperty(
                    "hoodie.streamer.schemaprovider.target.schema.hive.database", dbAndTableName.getLeft());
            PROPS.setProperty(
                    "hoodie.streamer.schemaprovider.target.schema.hive.table", dbAndTableName.getRight());
            createSchemaTable(SOURCE_SCHEMA_TABLE_NAME);
            ...
            Schema originalSchema =
                    new Schema.Parser()
                            .parse(
                                    UtilitiesTestBase.Helpers.readFile(
                                            "streamer-config/hive_schema_provider_target.avsc"));
    ...
    private void createSchemaTable(String fullName) throws IOException {
        ...
        String createTableSQL =
                UtilitiesTestBase.Helpers.readFile(String.format("streamer-config/%s.sql", fullName));
