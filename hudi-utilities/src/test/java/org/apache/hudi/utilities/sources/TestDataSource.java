...
public class TestDataSource extends AbstractBaseTestSource {
    ...
    protected InputBatch<JavaRDD<GenericRecord>> fetchNewData(
        ...
        if (sourceLimit <= 0 || returnEmptyBatch) {
            LOG.warn(
                    "Return no new data from Test Data source " + counter + ", source limit " + sourceLimit);
            return new InputBatch<>(Option.empty(), lastCheckpointStr.orElse(null));
        } else {
            LOG.warn(
                    "Returning valid data from Test Data source "
                            + counter
                            + ", source limit "
                            + sourceLimit);
        }
        ...
        List<GenericRecord> records =
                fetchNextBatch(props, (int) sourceLimit, instantTime, DEFAULT_PARTITION_NUM)
