...
public class TestErrorTableAwareChainedTransformer extends SparkClientFunctionalTestHarness {
    ...
    public void testForErrorTableConfig() {
        ...
        assertArrayEquals(
                new String[] {"foo", ERROR_TABLE_CURRUPT_RECORD_COL_NAME}, transformed.columns());
        assertEquals(
                2, transformed.filter(new Column(ERROR_TABLE_CURRUPT_RECORD_COL_NAME).isNotNull()).count());
    ...
    public void testForErrorRecordColumn() {
        ...
        properties.setProperty(ERROR_TABLE_ENABLED.key(), "true");
        ErrorTableAwareChainedTransformer transformer =
                new ErrorTableAwareChainedTransformer(Arrays.asList(t1, t2, t3));
        assertThrows(
    ...
    private Dataset<Row> getTestDataset() {
        StructType schema =
                DataTypes.createStructType(new StructField[] {createStructField("foo", StringType, false)});
    ...
    private Transformer getErrorEventHandlerTransformer() {
        return (jsc, sparkSession, dataset, properties) -> {
            boolean isErrorTableEnabledInTransformer =
                    properties.getBoolean(ERROR_TABLE_ENABLED.key(), ERROR_TABLE_ENABLED.defaultValue());
            if (isErrorTableEnabledInTransformer) {
                dataset =
                        dataset.withColumn(
                                ...
                                functions
                                        .when(
                                                new Column(ERROR_TABLE_CURRUPT_RECORD_COL_NAME).isNull(),
                                                functions.lit("true"))
    ...
    public void testErrorTableAwareChainedTransformerValidationFails(String transformerName) {
        assertThrows(
                ...
                () ->
                        new ErrorTableAwareChainedTransformer(
                                Arrays.asList(transformerName.split(",")), Option::empty));
    }
