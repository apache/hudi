...
public class TestPubsubMessagesFetcher {
    ...
    private static final String SUBSCRIPTION_NAME =
            ProjectSubscriptionName.format(PROJECT_ID, SUBSCRIPTION_ID);
    ...
    public void testFetchMessages() throws IOException {
        ...
        ReceivedMessage message1 =
                ReceivedMessage.newBuilder()
                        .setAckId("1")
                        .setMessage(PubsubMessage.newBuilder().setMessageId("msgId1").build())
                        .build();
        ReceivedMessage message2 =
                ReceivedMessage.newBuilder()
                        .setAckId("2")
                        .setMessage(PubsubMessage.newBuilder().setMessageId("msgId2").build())
                        .build();
        ReceivedMessage message3 =
                ReceivedMessage.newBuilder()
                        .setAckId("3")
                        .setMessage(PubsubMessage.newBuilder().setMessageId("msgId3").build())
                        .build();
        ...
        PubsubMessagesFetcher fetcher =
                new PubsubMessagesFetcher(
                        PROJECT_ID,
                        SUBSCRIPTION_ID,
                        SMALL_BATCH_SIZE,
                        MAX_MESSAGES_IN_REQUEST,
                        MAX_WAIT_TIME_IN_REQUEST,
        ...
        verify(mockPubsubQueueClient, times(3))
                .makePullRequest(mockSubscriber, SUBSCRIPTION_NAME, SMALL_BATCH_SIZE);
    }
    ...
    public void testSendAcks() throws IOException {
        doNothing().when(mockSubscriber).close();
        when(mockPubsubQueueClient.getSubscriber(any())).thenReturn(mockSubscriber);
        List<String> messageAcks =
                IntStream.range(0, 20).mapToObj(i -> "msg_" + i).collect(Collectors.toList());
        doNothing()
                .when(mockPubsubQueueClient)
                .makeAckRequest(eq(mockSubscriber), eq(SUBSCRIPTION_NAME), any());
        PubsubMessagesFetcher fetcher =
                new PubsubMessagesFetcher(
                        PROJECT_ID,
                        SUBSCRIPTION_ID,
        ...
        verify(mockPubsubQueueClient, times(2))
                .makeAckRequest(eq(mockSubscriber), eq(SUBSCRIPTION_NAME), any());
    }
}
