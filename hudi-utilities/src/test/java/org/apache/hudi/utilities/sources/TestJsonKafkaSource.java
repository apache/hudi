...
public class TestJsonKafkaSource extends BaseTestKafkaSource {
  ...
  static final URL SCHEMA_FILE_URL =
      TestJsonKafkaSource.class
          .getClassLoader()
          .getResource("streamer-config/source_short_trip_uber.avsc");
  ...
  protected TypedProperties createPropsForKafkaSource(
    ...
    return createPropsForJsonKafkaSource(
        testUtils.brokerAddress(), topic, maxEventsToReadFromKafkaSource, resetStrategy);
  }

  static TypedProperties createPropsForJsonKafkaSource(
  ...
  protected SourceFormatAdapter createSource(TypedProperties props) {
    return new SourceFormatAdapter(
        new JsonKafkaSource(
            props,
            jsc(),
            spark(),
    ...
    testUtils.sendMessages(
        ...
        jsonifyRecords(
            dataGenerator.generateInsertsAsPerSchema(
                "000", 1000, HoodieTestDataGenerator.SHORT_TRIP_SCHEMA)));
  ...
  public void testJsonKafkaSourceWithDefaultUpperCap() {
    ...
    testUtils.sendMessages(
        ...
        jsonifyRecords(
            dataGenerator.generateInsertsAsPerSchema(
                "001", 1000, HoodieTestDataGenerator.SHORT_TRIP_SCHEMA)));
  ...
  public void testJsonKafkaSourceWithConfigurableUpperCap() {
    ...
    InputBatch<JavaRDD<GenericRecord>> fetch6 =
        kafkaSource.fetchNewDataInAvroFormat(
            Option.of(fetch4.getCheckpointForNextBatch()), Long.MAX_VALUE);
  ...
  protected void sendMessagesToKafka(String topic, int count, int numPartitions) {
    ...
    testUtils.sendMessages(
        topic,
        jsonifyRecordsByPartitions(
            dataGenerator.generateInsertsAsPerSchema(
                "000", count, HoodieTestDataGenerator.SHORT_TRIP_SCHEMA),
            numPartitions));
  }
  ...
  void sendNullKafkaKeyMessagesToKafka(String topic, int count, int numPartitions) {
    ...
    testUtils.sendMessages(
        topic,
        jsonifyRecordsByPartitionsWithNullKafkaKey(
            dataGenerator.generateInsertsAsPerSchema(
                "000", count, HoodieTestDataGenerator.SHORT_TRIP_SCHEMA),
            numPartitions));
  }
  ...
  void sendJsonSafeMessagesToKafka(String topic, int count, int numPartitions) {
    try {
      ...
      for (int i = 0; i < count; i++) {
        ...
        keyValues[i] =
            new Tuple2<>(
                Integer.toString(i % numPartitions), OBJECT_MAPPER.writeValueAsString(node));
      }
  ...
  public void testErrorEventsForDataInRowFormat() throws IOException {
    ...
    SourceFormatAdapter kafkaSource =
        new SourceFormatAdapter(jsonSource, errorTableWriter, Option.of(props));
    assertEquals(
        1000,
        kafkaSource
    ...
    assertEquals(
        ...
        ((JavaRDD)
                errorTableWriter
                    .get()
                    .getErrorEvents(InProcessTimeGenerator.createNewInstantTime(), Option.empty())
                    .get())
            .count());
  }
  ...
  private BaseErrorTableWriter getAnonymousErrorTableWriter(TypedProperties props) {
    return new BaseErrorTableWriter<ErrorEvent<String>>(
        new HoodieDeltaStreamer.Config(),
        spark(),
        props,
  ...
  public void testAppendKafkaOffset() {
    ...
    Dataset<Row> dfWithOffsetInfoAndNullKafkaKey =
        kafkaSource
            ...
            .get()
            .cache();
    // total of 2 * numMessages are in the topic at this point, half with a key and half with a
    // null key. All should have the source offset.
    assertEquals(
        numMessages,
        dfWithOffsetInfoAndNullKafkaKey.toDF().filter("_hoodie_kafka_source_key is null").count());
    assertEquals(
        numMessages,
        dfWithOffsetInfoAndNullKafkaKey
            .toDF()
            .filter("_hoodie_kafka_source_key is not null")
            .count());
    assertEquals(
        numMessages * 2,
        dfWithOffsetInfoAndNullKafkaKey
            .toDF()
            .filter("_hoodie_kafka_source_offset is not null")
            .count());
