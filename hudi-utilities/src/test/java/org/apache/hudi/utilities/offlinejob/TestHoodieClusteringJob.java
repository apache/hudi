...
public class TestHoodieClusteringJob extends HoodieOfflineJobTestBase {
    ...
    public void testHoodieClusteringJobWithClean(boolean skipClean) throws Exception {
        ...
        hoodieCluster = init(tableBasePath, true, "scheduleAndExecute", false, skipClean);
        hoodieCluster.cluster(0);
    ...
    public void testPurgePendingInstants() throws Exception {
        ...
        // remove the completed instant from timeline and trigger purge of pending clustering
        // instant.
        HoodieInstant latestClusteringInstant =
                metaClient
                        .getActiveTimeline()
        ...
        String completedFilePath =
                tableBasePath + "/" + METAFOLDER_NAME + "/" + latestClusteringInstant.getFileName();
        deleteFileFromDfs(fs, completedFilePath);

        // trigger purge.
        hoodieCluster =
                getClusteringConfigForPurge(
                        tableBasePath, true, PURGE_PENDING_INSTANT, latestClusteringInstant.getTimestamp());
        hoodieCluster.cluster(0);
        ...
        for (int i = 0; i < fullPartitionPaths.length; i++) {
            fullPartitionPaths[i] =
                    String.format("%s/%s/*", tableBasePath, dataGen.getPartitionPaths()[i]);
        }
        assertEquals(
                0,
                HoodieClientTestUtils.read(jsc, tableBasePath, sqlContext, storage, fullPartitionPaths)
                        .filter("_hoodie_commit_time = " + latestClusteringInstant.getTimestamp())
                        .count(),
    ...
    private HoodieClusteringJob init(
            String tableBasePath,
            boolean runSchedule,
            String scheduleAndExecute,
            boolean isAutoClean,
            boolean skipClean) {
        HoodieClusteringJob.Config clusterConfig =
                buildHoodieClusteringUtilConfig(tableBasePath, runSchedule, scheduleAndExecute, skipClean);
        clusterConfig.configs.add(
                String.format("%s=%s", "hoodie.datasource.write.row.writer.enable", "false"));
        return new HoodieClusteringJob(jsc, clusterConfig);
    }

    private HoodieClusteringJob getClusteringConfigForPurge(
            String tableBasePath, boolean runSchedule, String scheduleAndExecute, String pendingInstant) {
        HoodieClusteringJob.Config clusterConfig =
                buildHoodieClusteringUtilConfig(tableBasePath, runSchedule, scheduleAndExecute, false);
        clusterConfig.configs.add(
                String.format("%s=%s", "hoodie.datasource.write.row.writer.enable", "false"));
    ...
    private HoodieClusteringJob.Config buildHoodieClusteringUtilConfig(
            String basePath, boolean runSchedule, String runningMode, boolean skipClean) {
        ...
        config.configs.add(
                String.format("%s=%s", HoodieClusteringConfig.INLINE_CLUSTERING_MAX_COMMITS.key(), 1));
        return config;
    }
