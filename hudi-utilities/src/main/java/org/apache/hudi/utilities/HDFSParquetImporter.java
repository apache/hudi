...
/**
  ...
  *
  * @see HoodieStreamer
  * @deprecated This utility is deprecated in 0.10.0 and will be removed in 0.11.0. Use {@link
  *     HoodieStreamer.Config#runBootstrap} instead.
  */
public class HDFSParquetImporter implements Serializable {
    ...
    public static void main(String[] args) {
        ...
        JavaSparkContext jssc =
                UtilHelpers.buildSparkContext(
                        "data-importer-" + cfg.tableName, cfg.sparkMaster, cfg.sparkMemory);
        try {
    ...
    public int dataImport(JavaSparkContext jsc, int retry) {
        ...
        this.props =
                cfg.propsFilePath == null
                        ...
                        : UtilHelpers.readConfig(fs.getConf(), new Path(cfg.propsFilePath), cfg.configs)
                                .getProps(true);
        ...
        try {
            ...
            SparkRDDWriteClient<HoodieRecordPayload> client =
                    UtilHelpers.createHoodieClient(
                            jsc, cfg.targetPath, schemaStr, cfg.parallelism, Option.empty(), props);
    ...
    protected JavaRDD<HoodieRecord<HoodieRecordPayload>> buildHoodieRecordsForImport(
        ...
        context.setJobStatus(
                this.getClass().getSimpleName(), "Build records for import: " + cfg.tableName);
    ...
    /**
      ...
      * @param <T> Type
      */
    protected <T extends HoodieRecordPayload> JavaRDD<WriteStatus> load(
        ...
        public void validate(String name, String value) {
            if (value == null || !validCommands.contains(value.toLowerCase())) {
                throw new ParameterException(
                        String.format(
                                "Invalid command: value:%s: supported commands:%s", value, validCommands));
            }
        }
    }
    ...
    public static class Config implements Serializable {
        ...
        @Parameter(
                names = {"--command", "-c"},
                description = "Write command Valid values are insert(default)/upsert/bulkinsert",
                validateValueWith = CommandValidator.class)
        public String command = "INSERT";

        @Parameter(
                names = {"--src-path", "-sp"},
                description = "Base path for the input table",
                required = true)
        public String srcPath = null;

        @Parameter(
                names = {"--target-path", "-tp"},
        ...
        @Parameter(
                names = {"--row-key-field", "-rk"},
                description = "Row key field name",
                required = true)
        public String rowKey = null;

        @Parameter(
                names = {"--partition-key-field", "-pk"},
        ...
        @Parameter(
                ...
                description = "Parallelism for hoodie insert(default)/upsert/bulkinsert",
                required = true)
        public int parallelism = 1;

        @Parameter(
                names = {"--schema-file", "-sf"},
                description = "path for Avro schema file",
                required = true)
