...
public abstract class DebeziumSource extends RowSource {
    ...
    private static final String OVERRIDE_CHECKPOINT_STRING =
            "hoodie.debezium.override.initial.checkpoint.key";
    ...
    public DebeziumSource(
        ...
        try {
            props.put(
                    NATIVE_KAFKA_VALUE_DESERIALIZER_PROP, Class.forName(deserializerClassName).getName());
        ...
        // Currently, debezium source requires Confluent/Kafka schema-registry to fetch the latest
        // schema.
    ...
    protected Pair<Option<Dataset<Row>>, String> fetchNextBatch(
        ...
        if (totalNewMsgs == 0) {
            // If there are no new messages, use empty dataframe with no schema. This is because the
            // schema from schema registry can only be considered
            ...
            try {
                String schemaStr =
                        schemaRegistryProvider.fetchSchemaFromRegistry(
                                getStringWithAltKeys(props, HoodieSchemaProviderConfig.SRC_SCHEMA_REGISTRY_URL));
                Dataset<Row> dataset = toDataset(offsetRanges, offsetGen, schemaStr);
                LOG.info(
                        String.format(
                                "Spark schema of Kafka Payload for topic %s:\n%s",
                                offsetGen.getTopicName(), dataset.schema().treeString()));
                LOG.info(
                        String.format("New checkpoint string: %s", CheckpointUtils.offsetsToStr(offsetRanges)));
                return Pair.of(
                        Option.of(dataset),
                        overrideCheckpointStr.isEmpty()
                                ? CheckpointUtils.offsetsToStr(offsetRanges)
                ...
                throw new HoodieReadFromSourceException(
                        "Fatal error reading and parsing incoming debezium event", e);
            }
        }
    }
    ...
    private Dataset<Row> toDataset(
            OffsetRange[] offsetRanges, KafkaOffsetGen offsetGen, String schemaStr) {
        ...
        if (deserializerClassName.equals(StringDeserializer.class.getName())) {
            kafkaData =
                    AvroConversionUtils.createDataFrame(
                            KafkaUtils.<String, String>createRDD(
                                            sparkContext,
        ...
        // Some required transformations to ensure debezium data types are converted to spark
        // supported types.
        return convertArrayColumnsToString(
    ...
    public static Dataset<Row> convertDateColumns(Dataset<Row> dataset, Schema schema) {
        if (schema.getField("before") != null) {
            List<String> dateFields =
                    schema.getField("before").schema().getTypes().get(1).getFields().stream()
                            .filter(
                                    field -> {
                                        if (field.schema().getType() == Type.UNION) {
                                            return field.schema().getTypes().stream()
                                                    .anyMatch(
                                                            schemaInUnion ->
                                                                    DATE_CONNECT_NAME.equals(
                                                                            schemaInUnion.getProp(CONNECT_NAME_KEY)));
                                        } else {
    ...
    /**
      * Utility function for converting columns to nullable. This is useful when required to make a
      * column nullable to match a nullable column from Debezium change events.
      *
      * @param sparkSession SparkSession object
      * @param dataset Dataframe to modify
      * @return Modified dataframe
      */
    private static Dataset<Row> convertColumnToNullable(
            SparkSession sparkSession, Dataset<Row> dataset) {
        List<String> columns = Arrays.asList(dataset.columns());
        StructField[] modifiedStructFields =
    ...
    /**
      * Converts Array types to String types because not all Debezium array columns are supported to be
      * converted to Spark array columns.
      *
      * @param dataset Dataframe to modify
      * @return Modified dataframe
      */
