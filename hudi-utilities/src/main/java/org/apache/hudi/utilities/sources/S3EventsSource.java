...
/**
  * This source provides capability to create the hudi table for S3 events metadata (eg. S3 put
  * events data). It will use the SQS for receiving the object key events. This can be useful to
  * check S3 files activity over time. The hudi table created by this source is consumed by {@link
  * S3EventsHoodieIncrSource} to apply changes to the hudi table corresponding to user data.
  */
public class S3EventsSource extends RowSource implements Closeable {
    ...
    /**
      ...
      * @param sourceLimit Limit on the size of data to fetch. For {@link S3EventsSource}, {@link
      *     S3SourceConfig#S3_SOURCE_QUEUE_MAX_MESSAGES_PER_BATCH} is used.
    ...
    public Pair<Option<Dataset<Row>>, String> fetchNextBatch(
        ...
        if (selectPathsWithLatestSqsMessage.getLeft().isEmpty()) {
            ...
            Dataset<String> eventRecords =
                    sparkSession.createDataset(selectPathsWithLatestSqsMessage.getLeft(), Encoders.STRING());
