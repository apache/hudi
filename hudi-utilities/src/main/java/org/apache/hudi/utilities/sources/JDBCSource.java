...
//      }
      ...
      properties.entrySet().stream().forEach(i-> System.out.println(properties.get(i)));
  ...
  protected Pair<Option<Dataset<Row>>, String> fetchNextBatch(Option<String> lastCkptStr, long sourceLimit) {
   // final String ppdQuery = "(select * from %s where %s >= TIMESTAMP(\"%s\")) rdbms_table";
  ...
  @NotNull
  private Pair<Option<Dataset<Row>>, String> incremental(String ppdQuery, Option<String> lastCheckpoint,
    ...
    return checkpoint(rowDataset, isIncremental);
  }

  @NotNull
  private Pair<Option<Dataset<Row>>, String> full(boolean isIncremental) {
    Dataset<Row> rowDataset = validatePropsAndGetDataFrameReader(sparkSession, props).load();
    return checkpoint(rowDataset, isIncremental);
  }

  @NotNull
  private Pair<Option<Dataset<Row>>, String> checkpoint(Dataset<Row> rowDataset, boolean isIncremental) {
    if (isIncremental) {
