...
/**
  * A schema provider that takes in a class name for a generated protobuf class that is on the
  * classpath.
  */
public class ProtoClassBasedSchemaProvider extends SchemaProvider {
    ...
    public ProtoClassBasedSchemaProvider(TypedProperties props, JavaSparkContext jssc) {
        ...
        try {
            schemaString =
                    ProtoConversionUtil.getAvroSchemaForMessageClass(
                                    ReflectionUtils.getClass(className), schemaConfig)
                            .toString();
        } catch (Exception e) {
            throw new HoodieSchemaException(
                    String.format("Error reading proto source schema for class: %s", className), e);
        }
    }
