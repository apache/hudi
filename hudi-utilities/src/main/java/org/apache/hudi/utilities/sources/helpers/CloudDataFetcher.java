...
/**
  * Connects to S3/GCS from Spark and downloads data from a given list of files. Assumes SparkContext
...
public class CloudDataFetcher implements Serializable {
    ...
    public Pair<Option<Dataset<Row>>, String> fetchPartitionedSource(
        ...
        boolean isSourceProfileSupplierAvailable =
                sourceProfileSupplier.isPresent() && sourceProfileSupplier.get().getSourceProfile() != null;
        if (isSourceProfileSupplierAvailable) {
            LOG.debug(
                    "Using source limit from source profile sourceLimitFromConfig {} sourceLimitFromProfile {}",
                    sourceLimit,
        ...
        LOG.info(
                "Adjusting end checkpoint:"
                        + queryInfo.getEndInstant()
                        + " based on sourceLimit :"
                        + sourceLimit);
        ...
        long bytesPerPartition =
                props.containsKey(SOURCE_MAX_BYTES_PER_PARTITION.key())
                        ...
                        : props.getLong(
                                PARQUET_MAX_FILE_SIZE.key(), Long.parseLong(PARQUET_MAX_FILE_SIZE.defaultValue()));
        if (isSourceProfileSupplierAvailable) {
            long bytesPerPartitionFromProfile =
                    (long) sourceProfileSupplier.get().getSourceProfile().getSourceSpecificContext();
            if (bytesPerPartitionFromProfile > 0) {
                LOG.debug(
                        "Using bytesPerPartition from source profile bytesPerPartitionFromConfig {} bytesPerPartitionFromProfile {}",
                        bytesPerPartition,
                        bytesPerPartitionFromProfile);
    ...
    private Option<Dataset<Row>> getCloudObjectDataDF(
            List<CloudObjectMetadata> cloudObjectMetadata,
        ...
        int numPartitions =
                (int) Math.max(Math.ceil(totalSizeWithHoodieMetaFields / bytesPerPartition), 1);
        metrics.updateStreamerSourceParallelism(numPartitions);
        return cloudObjectsSelectorCommon.loadAsDataset(
                sparkSession,
                cloudObjectMetadata,
                getFileFormat(props),
