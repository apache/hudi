...
public class HoodieCompactor {
    ...
    public static void main(String[] args) {
        ...
        if (cfg.help || args.length == 0) {
            ...
            throw new HoodieException(
                    "Fail to run compaction for " + cfg.tableName + ", return code: " + 1);
        }
        final JavaSparkContext jsc =
                UtilHelpers.buildSparkContext(
                        "compactor-" + cfg.tableName, cfg.sparkMaster, cfg.sparkMemory);
        int ret = new HoodieCompactor(jsc, cfg).compact(cfg.retry);
        if (ret != 0) {
            throw new HoodieException(
                    "Fail to run compaction for " + cfg.tableName + ", return code: " + ret);
        }
    ...
    public int compact(int retry) {
        ...
        // need to do validate in case that users call compact() directly without setting
        // cfg.runningMode
                        ...
                        () -> {
                            switch (cfg.runningMode.toLowerCase()) {
                                ...
                                default:
                                    {
                                        LOG.info(
                                                "Unsupported running mode ["
                                                        + cfg.runningMode
                                                        + "], quit the job directly");
                                        return -1;
                                    }
                            }
                        },
    ...
    private int doCompact(JavaSparkContext jsc) throws Exception {
        ...
        try (SparkRDDWriteClient<HoodieRecordPayload> client =
            ...
            // If no compaction instant is provided by --instant-time, find the earliest scheduled
            // compaction
            ...
            if (StringUtils.isNullOrEmpty(cfg.compactionInstantTime)) {
                ...
                if (firstCompactionInstant.isPresent()) {
                    ...
                    LOG.info(
                            "Found the earliest scheduled compaction instant which will be executed: "
            ...
            clean(client);
            return UtilHelpers.handleErrors(
                    compactionMetadata.getCommitMetadata().get(), cfg.compactionInstantTime);
        }
    }
    ...
    private Option<String> doSchedule(JavaSparkContext jsc) {
        try (SparkRDDWriteClient client =
                UtilHelpers.createHoodieClient(
                        jsc, cfg.basePath, "", cfg.parallelism, Option.of(cfg.strategyClassName), props)) {
