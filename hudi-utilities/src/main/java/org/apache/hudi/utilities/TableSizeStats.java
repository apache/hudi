...
/**
  * This class provides file size updates for the latest files that hudi is consuming. These stats
  * are at table level by default, but specifying --enable-partition-stats will also show stats at
  * the partition level. If a start date (--start-date parameter) and/or end date (--end-date
  * parameter) are specified, stats are based on files that were modified in the half-open interval
  * [start date (--start-date parameter), end date (--end-date parameter)). --num-days parameter can
  * be used to select data files over last --num-days. If --start-date is specified, --num-days will
  * be ignored. If none of the date parameters are set, stats will be computed over all data files of
  * all partitions in the table. Note that date filtering is carried out only if the partition name
  * has the format '[column name=]yyyy-M-d', '[column name=]yyyy/M/d'. <br>
  * <br>
  * The following stats are produced by this class: Number of files. Total table size. Minimum file
  * size Maximum file size Average file size Median file size p50 file size p90 file size p95 file
  * size p99 file size <br>
  * <br>
  * Sample spark-submit command: ./bin/spark-submit \ --class
  ...
  * --base-path <base-path> \ --num-days <number-of-days>
  */
public class TableSizeStats implements Serializable {
    ...
    public static class Config implements Serializable {
        ...
        @Parameter(
                names = {"--num-days", "-nd"},
                description = "Consider files modified within this many days.",
                required = false)
        public long numDays = 0;

        @Parameter(
                names = {"--start-date", "-sd"},
                description = "Consider files modified on or after this date.",
                required = false)
        public String startDate = null;

        @Parameter(
                names = {"--end-date", "-ed"},
                description = "Consider files modified before this date.",
                required = false)
        public String endDate = null;

        @Parameter(
                names = {"--enable-table-stats", "-fs"},
                description = "Show file-level stats.",
                required = false)
        public boolean tableStats = false;

        @Parameter(
                names = {"--enable-partition-stats", "-ps"},
                description = "Show partition-level stats.",
                required = false)
        public boolean partitionStats = false;

        @Parameter(
                names = {"--props-path", "-pp"},
                description = "Properties file containing base paths one per line",
                required = false)
    ...
    private void logTableStats(String basePath, LocalDate[] dateInterval) throws IOException {
        ...
        HoodieMetadataConfig metadataConfig =
                HoodieMetadataConfig.newBuilder().enable(isMetadataEnabled(basePath, jsc)).build();
        ...
        // As a sanity check, throw exception and exit early if date interval is specified, but the
        // first partition does not have
        // date.
        if (dateInterval != null && getPartitionDate(allPartitions.get(0)) == null) {
            throw new HoodieException(
                    "Cannot apply --start-date, --end-date, or --num-days when partition does not contain date. Interval: "
                            + Arrays.toString(dateInterval)
                            + ", Partition Name: "
                            + allPartitions.get(0));
        }
        ...
        allPartitions.forEach(
                partition -> {
                    ...
                    // 3. endDate is null (not specified) and partition date is equal to or after
                    // startDate.
                    // 4. startDate is null (not specified) and partition date is before endDate.
                    // 5. startDate and endDate are both specified and partition date lies in the
                    ...
                    if (partitionDate == null
                        ...
                        HoodieTableFileSystemView fileSystemView =
                                FileSystemViewManager.createInMemoryFileSystemView(
                                        new HoodieLocalEngineContext(storageConf), metaClientLocal, metadataConfig1);
                        ...
                        final Histogram partitionHistogram =
                                cfg.partitionStats && partition.trim().length() > 0
                                        ? new Histogram(new UniformReservoir(1_000_000))
                                        : null;
                        baseFiles.forEach(
                                baseFile -> {
                                    // Add file size to histogram since the file was modified within
                        ...
                        if (partitionHistogram != null) {
                            logStats(
                                    "Partition stats [name: "
                                            + partition
                                            + (partitionDate != null ? ", has date: yes" : "")
                                            + "]",
                                    partitionHistogram);
                        }
                    }
    ...
    private static List<String> getFilePaths(String propsPath, Configuration hadoopConf) {
        ...
        FileSystem fs =
                HadoopFSUtils.getFs(propsPath, Option.ofNullable(hadoopConf).orElseGet(Configuration::new));

        try (BufferedReader reader =
                new BufferedReader(
                        new InputStreamReader(fs.open(new Path(propsPath)), StandardCharsets.UTF_8))) {
    ...
    private static LocalDate[] getUserSpecifiedDateInterval(Config cfg) {
        ...
        // Set startDate to cfg.startDate if specified. cfg.startDate takes priority over
        // cfg.numDays if both are specified.
        ...
        if (startDate != null && endDate != null && !startDate.isBefore(endDate)) {
            throw new HoodieException(
                    "Starting date must be before ending date. Start Date: "
                            + startDate
                            + ", End Date: "
                            + endDate);
        }
    ...
    private static LocalDate getPartitionDate(String partition) {
        // Partition name should conform to date format if startDate and/or endDate are specified.
        // Otherwise, we don't
        ...
        try {
            ...
            LOG.error(
                    "Partition name {} must conform to date format if --start-date, --end-date, or --num-days are specified. ",
                    partition,
                    dtpe);
        }
