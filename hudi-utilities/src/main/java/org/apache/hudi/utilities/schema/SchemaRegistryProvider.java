...
/**
  ...
  *
  * <p>https://github.com/confluentinc/schema-registry
  */
public class SchemaRegistryProvider extends SchemaProvider {
    ...
    public Schema parseSchemaFromRegistry(String registryUrl) {
        ...
        try {
            String schemaConverter =
                    getStringWithAltKeys(config, HoodieSchemaProviderConfig.SCHEMA_CONVERTER, true);
            SchemaConverter converter =
                    !StringUtils.isNullOrEmpty(schemaConverter)
    ...
    /**
      * The method takes the provided url {@code registryUrl} and gets the schema from the schema
      * registry using that url. If the caller provides userInfo credentials in the url (e.g
      * "https://foo:bar@schemaregistry.org") then the credentials are extracted the url using the
      * Matcher and the extracted credentials are set on the request as an Authorization header.
      *
      * @param registryUrl
    ...
    public SchemaRegistryProvider(TypedProperties props, JavaSparkContext jssc) {
        ...
        checkRequiredConfigProperties(
                props, Collections.singletonList(HoodieSchemaProviderConfig.SRC_SCHEMA_REGISTRY_URL));
        this.srcSchemaRegistryUrl =
                getStringWithAltKeys(config, HoodieSchemaProviderConfig.SRC_SCHEMA_REGISTRY_URL);
        this.targetSchemaRegistryUrl =
                getStringWithAltKeys(
                        config, HoodieSchemaProviderConfig.TARGET_SCHEMA_REGISTRY_URL, srcSchemaRegistryUrl);
        ...
        try {
            ...
            throw new HoodieSchemaFetchException(
                    String.format(
                            ...
                            StringUtils.truncate(srcSchemaRegistryUrl, 10, 10)),
                    e);
        }
    }
        ...
        try {
            ...
            throw new HoodieSchemaFetchException(
                    String.format(
                            ...
                            StringUtils.truncate(targetSchemaRegistryUrl, 10, 10)),
                    e);
        }
    }
