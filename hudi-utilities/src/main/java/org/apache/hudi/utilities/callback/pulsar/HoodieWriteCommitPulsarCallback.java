...
public class HoodieWriteCommitPulsarCallback implements HoodieWriteCommitCallback, Closeable {
    ...
    public Producer<String> createProducer(HoodieConfig hoodieConfig) throws PulsarClientException {
        MessageRoutingMode routeMode =
                Enum.valueOf(MessageRoutingMode.class, PRODUCER_ROUTE_MODE.defaultValue());
        Duration sendTimeout = parseDuration(hoodieConfig.getString(PRODUCER_SEND_TIMEOUT));
        int pendingQueueSize = hoodieConfig.getInt(PRODUCER_PENDING_QUEUE_SIZE);
        int pendingSize = hoodieConfig.getInt(PRODUCER_PENDING_SIZE);
        boolean blockIfQueueFull = hoodieConfig.getBoolean(PRODUCER_BLOCK_QUEUE_FULL);
    ...
    public PulsarClient createClient(HoodieConfig hoodieConfig) throws PulsarClientException {
        ...
        Duration operationTimeout = parseDuration(hoodieConfig.getString(OPERATION_TIMEOUT));
        Duration connectionTimeout = parseDuration(hoodieConfig.getString(CONNECTION_TIMEOUT));
        Duration requestTimeout = parseDuration(hoodieConfig.getString(REQUEST_TIMEOUT));
        Duration keepAliveInterval = parseDuration(hoodieConfig.getString(KEEPALIVE_INTERVAL));
    ...
    /**
      * Validate whether both pulsar's brokerServiceUrl and topic are configured. Exception will be
      * thrown if anyone of them is not configured.
      */
    private void validatePulsarConfig() {
        ValidationUtils.checkArgument(
                !StringUtils.isNullOrEmpty(serviceUrl),
                String.format("Config %s can not be " + "null or empty", BROKER_SERVICE_URL.key()));
        ValidationUtils.checkArgument(
