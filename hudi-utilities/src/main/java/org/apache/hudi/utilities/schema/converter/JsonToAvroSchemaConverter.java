...
public class JsonToAvroSchemaConverter implements SchemaRegistryProvider.SchemaConverter {
    ...
    public String convert(String jsonSchema) throws IOException {
        ...
        if (jsonNode.hasNonNull("properties")) {
            avroRecord.set(
                    "fields", convertProperties(jsonNode.get("properties"), getRequired(jsonNode)));
        } else {
                                ...
                                avroFields.add(
                                        tryConvertNestedProperty(name, jsonProperties.get(name))
                                                ...
                                                .orElseGet(
                                                        () ->
                                                                convertProperty(
                                                                        name, jsonProperties.get(name), required.contains(name)))));
                        ...
                        .set(
                                ...
                                MAPPER
                                        ...
                                        .set(
                                                "fields",
                                                convertProperties(
                                                        jsonProperty.get("properties"), getRequired(jsonProperty))));
    ...
    private static JsonNode convertProperty(String name, JsonNode jsonProperty, boolean isRequired) {
        ...
        if (jsonProperty.hasNonNull("oneOf") || jsonProperty.hasNonNull("allOf")) {
            ...
            if (oneOfTypes.isPresent()) {
                oneOfTypes
                        .get()
                        .elements()
                        .forEachRemaining(e -> avroTypeSet.add(JSON_TO_AVRO_TYPE.get(e.get("type").asText())));
            }
            ...
            if (jsonType.isArray()) {
                jsonType
                        .elements()
                        .forEachRemaining(e -> avroTypeSet.add(JSON_TO_AVRO_TYPE.get(e.asText())));
            } else {
        ...
        avroNode.set(
                ...
                avroTypes.size() > 1
                        ? MAPPER
                                .createArrayNode()
                                .addAll(avroTypes.stream().map(TextNode::valueOf).collect(Collectors.toList()))
