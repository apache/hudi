...
/**
  * SQL Source that reads from any table, used mainly for backfill jobs which will process specific
  * partition dates.
  *
  ...
  *
  * <p>SQL Source is used for one time backfill scenarios, this won't update the
  * deltastreamer.checkpoint.key to the processed commit, instead it will fetch the latest successful
  * checkpoint key and set that value as this backfill commits checkpoint so that it won't interrupt
  * the regular incremental processing.
  *
  * <p>To fetch and use the latest incremental checkpoint, you need to also set this hoodie_conf for
  * deltastremer jobs:
  *
  * <p>hoodie.write.meta.key.prefixes = 'deltastreamer.checkpoint.key'
  *
  * <p>Also, users are expected to set --allow-commit-on-no-checkpoint-change while using this
  * SqlSource.
  */
public class SqlSource extends RowSource {
    ...
    public SqlSource(
        ...
        checkRequiredConfigProperties(props, Collections.singletonList(SqlSourceConfig.SOURCE_SQL));
