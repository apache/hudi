...
public class PubsubQueueClient {
    private static final String METRIC_FILTER_PATTERN =
            "metric.type=\"pubsub.googleapis.com/subscription/%s\" AND resource.label.subscription_id=\"%s\"";
    private static final String NUM_UNDELIVERED_MESSAGES = "num_undelivered_messages";

    public SubscriberStub getSubscriber(SubscriberStubSettings subscriberStubSettings)
            throws IOException {
        return GrpcSubscriberStub.create(subscriberStubSettings);
    }

    public PullResponse makePullRequest(
            SubscriberStub subscriber, String subscriptionName, int batchSize) throws IOException {
    ...
    public void makeAckRequest(
            SubscriberStub subscriber, String subscriptionName, List<String> messages) {
    ...
    public long getNumUnAckedMessages(String subscriptionId) throws IOException {
        try (MetricServiceClient metricServiceClient = MetricServiceClient.create()) {
            MetricServiceClient.ListTimeSeriesPagedResponse response =
                    metricServiceClient.listTimeSeries(
                            ListTimeSeriesRequest.newBuilder()
                                    ...
                                    .setFilter(
                                            String.format(
                                                    METRIC_FILTER_PATTERN, NUM_UNDELIVERED_MESSAGES, subscriptionId))
                                    .setInterval(
                                            TimeInterval.newBuilder()
                                                    .setStartTime(
                                                            Timestamps.fromSeconds(
                                                                    Instant.now().getEpochSecond() - TimeUnit.MINUTES.toSeconds(2)))
            ...
            return pointList.stream()
                    .findFirst()
                    .map(point -> point.getValue().getInt64Value())
