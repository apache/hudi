...
public class CloudStoreIngestionConfig {
    ...
    /** How many metadata messages to pull at a time. Also see {@link #DEFAULT_BATCH_SIZE}. */
    @Deprecated public static final String BATCH_SIZE_CONF = CloudSourceConfig.BATCH_SIZE_CONF.key();

    /**
      * Provide a reasonable setting to use for default batch size when fetching File Metadata as part
      * of Cloud Ingestion. If batch size is too big, two possible issues can happen: i)
      * Acknowledgement takes too long (given that Hudi needs to commit first). ii) In the case of
      * Google Cloud Pubsub: a) it will keep delivering the same message since it wasn't acknowledged
      * in time. b) The size of the request that acknowledges outstanding messages may exceed the
      * limit, which is 512KB as per Google's docs. See:
    ...
    /**
      * Whether to acknowledge Metadata messages during Cloud Ingestion or not. This is useful during
      * dev and testing. In Prod this should always be true. In case of Cloud Pubsub, not acknowledging
      * means Pubsub will keep redelivering the same messages.
      */
    @Deprecated public static final String ACK_MESSAGES = CloudSourceConfig.ACK_MESSAGES.key();
    ...
    /**
      * A JSON string passed to the Spark DataFrameReader while loading the dataset. Example:
    ...
    /**
      * Only match files with this extension. By default, this is the same as {@link
    ...
    /**
      * Format of the data file. By default, this will be the same as {@link
      * org.apache.hudi.utilities.config.HoodieIncrSourceConfig#SOURCE_FILE_FORMAT}.
      */
    @Deprecated public static final String DATAFILE_FORMAT = CloudSourceConfig.DATAFILE_FORMAT.key();
