...
public class MysqlDebeziumSource extends DebeziumSource {
    ...
    public MysqlDebeziumSource(
        ...
        sqlContext
                ...
                .register(
                        ...
                        (UDF2<String, Long, String>) MysqlDebeziumSource::generateUniqueSequence,
                        DataTypes.StringType);
    }

    /**
      * Debezium Kafka Payload has a nested structure (see
      * https://debezium.io/documentation/reference/1.4/connectors/mysql.html). This function flattens
      * this nested structure for the Mysql data, and also extracts a subset of Debezium metadata
      * fields.
      *
    ...
    protected Dataset<Row> processDataset(Dataset<Row> rowDataset) {
            ...
            Dataset<Row> insertedOrUpdatedData =
                    rowDataset
                            .selectExpr(
                                    ...
                                    String.format(
                                            "%s as %s",
                                            DebeziumConstants.INCOMING_SOURCE_NAME_FIELD,
                                            DebeziumConstants.FLATTENED_SHARD_NAME),
                                    String.format(
                                            "%s as %s",
                                            DebeziumConstants.INCOMING_SOURCE_TS_MS_FIELD,
                                            DebeziumConstants.FLATTENED_TS_COL_NAME),
                                    String.format(
                                            "%s as %s",
                                            DebeziumConstants.INCOMING_SOURCE_FILE_FIELD,
                                            DebeziumConstants.FLATTENED_FILE_COL_NAME),
                                    String.format(
                                            "%s as %s",
                                            DebeziumConstants.INCOMING_SOURCE_POS_FIELD,
                                            DebeziumConstants.FLATTENED_POS_COL_NAME),
                                    String.format(
                                            "%s as %s",
                                            DebeziumConstants.INCOMING_SOURCE_ROW_FIELD,
                                            DebeziumConstants.FLATTENED_ROW_COL_NAME),
                                    String.format("%s.*", DebeziumConstants.INCOMING_AFTER_FIELD))
                            .filter(
                                    rowDataset
                                            .col(DebeziumConstants.INCOMING_OP_FIELD)
                                            .notEqual(DebeziumConstants.DELETE_OP));

            Dataset<Row> deletedData =
                    rowDataset
                            .selectExpr(
                                    String.format(
                                            "%s as %s",
        ...
        return flattenedDataset.withColumn(
                DebeziumConstants.ADDED_SEQ_COL_NAME,
                callUDF(
                        generateUniqueSeqUdfFn,
                        flattenedDataset.col(DebeziumConstants.FLATTENED_FILE_COL_NAME),
