...
public class HoodieWriteCommitKafkaCallback implements HoodieWriteCommitCallback {
    ...
    /**
      * Method helps to create {@link KafkaProducer}. Here we set acks = all and retries = 3 by default
    ...
    public KafkaProducer<String, String> createProducer(HoodieConfig hoodieConfig) {
        ...
        kafkaProducerProps.setProperty(
                ProducerConfig.ACKS_CONFIG, hoodieConfig.getStringOrDefault(ACKS));
        // retries 3 times by default
        kafkaProducerProps.setProperty(
                ProducerConfig.RETRIES_CONFIG, hoodieConfig.getStringOrDefault(RETRIES));
        kafkaProducerProps.setProperty(
        ...
        LOG.debug(
                "Callback kafka producer init with configs: "
    ...
    /**
      * Method helps to create a {@link ProducerRecord}. To ensure the order of the callback messages,
      * we should guarantee that the callback message of the same table will goes to the same
      * partition. Therefore, if user does not specify the partition, we can use the table name as
      * {@link ProducerRecord} key.
      *
      * @param hoodieConfig Kafka configs
    ...
    private ProducerRecord<String, String> buildProducerRecord(
            HoodieConfig hoodieConfig, String callbackMsg) {
        String partition = hoodieConfig.getString(PARTITION);
        if (null != partition) {
            return new ProducerRecord<String, String>(
                    topic, Integer.valueOf(partition), hoodieConfig.getString(TBL_NAME), callbackMsg);
        } else {
            return new ProducerRecord<String, String>(
                    topic, hoodieConfig.getString(TBL_NAME), callbackMsg);
        }
    }

    /**
      * Validate whether both {@code ProducerConfig.BOOTSTRAP_SERVERS_CONFIG} and kafka topic are
      * configured. Exception will be thrown if anyone of them is not configured.
      */
    private void validateKafkaConfig() {
        ValidationUtils.checkArgument(
                !StringUtils.isNullOrEmpty(bootstrapServers),
                String.format("Config %s can not be " + "null or empty", BOOTSTRAP_SERVERS.key()));
        ValidationUtils.checkArgument(
                !StringUtils.isNullOrEmpty(topic),
                String.format("Config %s can not be null or empty", TOPIC.key()));
    }
    ...
    private static class ProducerSendCallback implements Callback {
        ...
        public void onCompletion(RecordMetadata metadata, Exception exception) {
            if (null != metadata) {
                LOG.info(
                        String.format(
                                "message offset=%s partition=%s timestamp=%s topic=%s",
