...
public class ProtoConversionUtil {
    ...
    /**
      * Creates an Avro {@link Schema} for the provided class. Assumes that the class is a protobuf
      * {@link Message}.
      *
      * @param clazz The protobuf class
      * @param schemaConfig configuration used to determine how to handle particular cases when
      *     converting from the proto schema
    ...
    /**
      * Creates an Avro {@link Schema} for the provided {@link Descriptors.Descriptor}. Intended for
      * use when the descriptor is provided by an external registry.
      *
      ...
      * @return An Avro schema
      */
    public static Schema getAvroSchemaForMessageDescriptor(
            Descriptors.Descriptor descriptor, SchemaConfig schemaConfig) {
        return new AvroSupport(schemaConfig).getSchema(descriptor);
    }

    /**
      * Converts the provided {@link Message} into an avro {@link GenericRecord} with the provided
      * schema.
      *
    ...
    public static class SchemaConfig {
        ...
        /**
          ...
          *
          * @param wrappedPrimitivesAsRecords if true, to treat wrapped primitives like record with a
          *     single "value" field. If false, treat them as a nullable field
          * @param maxRecursionDepth the number of times to unravel a recursive proto schema before
          *     spilling the rest to bytes
          * @param timestampsAsRecords if true convert {@link Timestamp} to a Record with a seconds and
          *     nanos field. If false, convert it to a long with the timestamp-mircos logical type.
          */
        public SchemaConfig(
                boolean wrappedPrimitivesAsRecords, int maxRecursionDepth, boolean timestampsAsRecords) {
        ...
        public static SchemaConfig fromProperties(TypedProperties props) {
            boolean wrappedPrimitivesAsRecords =
                    getBooleanWithAltKeys(props, PROTO_SCHEMA_WRAPPED_PRIMITIVES_AS_RECORDS);
            int maxRecursionDepth = getIntWithAltKeys(props, PROTO_SCHEMA_MAX_RECURSION_DEPTH);
            boolean timestampsAsRecords =
                    getBooleanWithAltKeys(props, PROTO_SCHEMA_TIMESTAMPS_AS_RECORDS);
            return new ProtoConversionUtil.SchemaConfig(
                    wrappedPrimitivesAsRecords, maxRecursionDepth, timestampsAsRecords);
        }
    ...
    /**
      * This class provides support for generating schemas and converting from proto to avro. We don't
      * directly use Avro's ProtobufData class so we can: 1. Customize how schemas are generated for
      * protobufs. We treat Enums as strings and provide an option to treat wrapped primitives like
      * {@link Int32Value} and {@link StringValue} as messages (default behavior) or as nullable
      * versions of those primitives. 2. Convert directly from a protobuf {@link Message} to a {@link
      * GenericRecord} while properly handling enums and wrapped primitives mentioned above.
      */
    private static class AvroSupport {
        private static final Schema STRING_SCHEMA = Schema.create(Schema.Type.STRING);
        private static final Schema NULL_SCHEMA = Schema.create(Schema.Type.NULL);
        // The max unsigned long value has 20 digits, so a decimal with precision 20 and scale 0 is
        // required to represent all possible values.
        // A byte array of length N can store at most floor(log_10(2^(8 × N - 1) - 1)) base 10
        // digits so we require N = 9.
        private static final Schema UNSIGNED_LONG_SCHEMA =
                LogicalTypes.decimal(20)
                        .addToSchema(Schema.createFixed("unsigned_long", null, "org.apache.hudi.protos", 9));
        ...
        private static final Schema RECURSION_OVERFLOW_SCHEMA =
                Schema.createRecord(
                        "recursion_overflow",
                        null,
                        "org.apache.hudi.proto",
                        false,
                        Arrays.asList(
                                new Schema.Field(OVERFLOW_DESCRIPTOR_FIELD_NAME, STRING_SCHEMA, null, ""),
                                new Schema.Field(
                                        OVERFLOW_BYTES_FIELD_NAME,
                                        Schema.create(Schema.Type.BYTES),
                                        null,
                                        getUTF8Bytes(""))));
        // A cache of the proto class name paired with whether wrapped primitives should be
        // flattened as the key and the generated avro schema as the value
        private static final Map<SchemaCacheKey, Schema> SCHEMA_CACHE = new ConcurrentHashMap<>();
        // A cache with a key as the pair target avro schema and the proto descriptor for the source
        // and the value as an array of proto field descriptors where the order matches the avro
        // ordering.
        // When converting from proto to avro, we want to be able to iterate over the fields in the
        // proto in the same order as they appear in the avro schema.
        private static final Map<Pair<Schema, Descriptors.Descriptor>, Descriptors.FieldDescriptor[]>
                FIELD_CACHE = new ConcurrentHashMap<>();
        private static final Set<Descriptors.Descriptor> WRAPPER_DESCRIPTORS_TO_TYPE =
        ...
        Schema getSchema(Class c) {
            return SCHEMA_CACHE.computeIfAbsent(
                    new SchemaCacheKey(c, wrappedPrimitivesAsRecords, maxRecursionDepth, timestampsAsRecords),
                    key -> {
                        try {
                            ...
                            if (c.isEnum()) {
                                ...
                                return getMessageSchema(
                                        castedDescriptor,
                                        new CopyOnWriteMap<>(),
                                        getNamespace(castedDescriptor.getFullName()));
                            }
                        } catch (Exception e) {
        ...
        /**
          * Translates a Proto Message descriptor into an Avro Schema. Does not cache since external
          * system may evolve the schema and that can result in a stale version of the avro schema.
          *
        ...
        Schema getSchema(Descriptors.Descriptor descriptor) {
            return getMessageSchema(
                    descriptor, new CopyOnWriteMap<>(), getNamespace(descriptor.getFullName()));
        }
        ...
        private Schema getEnumSchema(Descriptors.EnumDescriptor enumDescriptor) {
            ...
            return Schema.createEnum(
                    enumDescriptor.getName(), null, getNamespace(enumDescriptor.getFullName()), symbols);
        }
        ...
        /**
          ...
          *
          * @param descriptor the descriptor for the proto message
          * @param recursionDepths a map of the descriptor to the number of times it has been encountered
          *     in this depth first traversal of the schema. This is used to cap the number of times we
          *     recurse on a schema.
          * @param path a string prefixed with the namespace of the original message being translated to
          *     avro and containing the current dot separated path tracking progress through the schema.
          *     This value is used for a namespace when creating Avro records to avoid an error when
          *     reusing the same class name when unraveling a recursive schema.
          * @return an avro schema
          */
        private Schema getMessageSchema(
                Descriptors.Descriptor descriptor,
                CopyOnWriteMap<Descriptors.Descriptor, Integer> recursionDepths,
                String path) {
            ...
            // The current path is used as a namespace to avoid record name collisions within
            // recursive schemas
            ...
            for (Descriptors.FieldDescriptor f : descriptor.getFields()) {
                // each branch of the schema traversal requires its own recursion depth tracking so
                // copy the recursionDepths map
                fields.add(
                        new Schema.Field(
                                f.getName(),
                                getFieldSchema(f, new CopyOnWriteMap<>(recursionDepths), path),
                                null,
                                getDefault(f)));
            }
            ...
            switch (fieldDescriptor.getType()) {
                ...
                case MESSAGE:
                    ...
                    if (!wrappedPrimitivesAsRecords
                        ...
                        // all wrapper types have a single field, so we can get the first field in
                    ...
                    if (fieldDescriptor.isRepeated()) {
                        Schema elementSchema =
                                getMessageSchema(fieldDescriptor.getMessageType(), recursionDepths, updatedPath);
        ...
        private static Schema finalizeSchema(
                Schema schema, Descriptors.FieldDescriptor fieldDescriptor) {
        ...
        private static Descriptors.FieldDescriptor[] getOrderedFields(Schema schema, Message message) {
            ...
            return FIELD_CACHE.computeIfAbsent(
                    Pair.of(schema, descriptor),
                    key -> {
                        Descriptors.FieldDescriptor[] fields =
                                new Descriptors.FieldDescriptor[key.getLeft().getFields().size()];
        ...
        private static Object convertObject(Schema schema, Object value) {
            ...
            // if we've reached max recursion depth in the provided schema, write out message to
            // bytes
            if (RECURSION_OVERFLOW_SCHEMA.getFullName().equals(schema.getFullName())) {
                ...
                overflowRecord.put(
                        OVERFLOW_DESCRIPTOR_FIELD_NAME, messageValue.getDescriptorForType().getFullName());
            ...
            switch (schema.getType()) {
                ...
                case FIXED:
                    ...
                    if (unsignedLongValue instanceof UInt64Value) {
                        ...
                        throw new HoodieException(
                                "Unexpected Message type when converting as an unsigned long: "
                                        + unsignedLongValue.getClass().getName());
                    }
                    // convert the long to its unsigned value
                    return DECIMAL_CONVERSION.toFixed(
                            new BigDecimal(toUnsignedBigInteger((Long) unsignedLongValue)),
                            schema,
                        ...
                        if (LogicalTypes.timestampMicros().equals(schema.getLogicalType())) {
                            if (value instanceof Timestamp) {
                                ...
                                ((DynamicMessage) value)
                                        ...
                                        .forEach(
                                                (fieldDescriptor, fieldValue) -> {
                                                    if (fieldDescriptor
                                                            .getFullName()
                                                            .equals("google.protobuf.Timestamp.seconds")) {
                                                        builder.setSeconds((Long) fieldValue);
                                                    } else if (fieldDescriptor
                                                            .getFullName()
                                                            .equals("google.protobuf.Timestamp.nanos")) {
                                ...
                                throw new HoodieSchemaException(
                                        "Unexpected message type while handling timestamps: "
                ...
                case MAP:
                    ...
                    for (Map.Entry<Object, Object> entry : mapValue.entrySet()) {
                        mapCopy.put(
                                convertObject(STRING_SCHEMA, entry.getKey()),
                ...
                case RECORD:
                    ...
                    for (Schema.Field field : schema.getFields()) {
                        ...
                        if (field.schema().getType() == Schema.Type.UNION
                            ...
                            convertedValue =
                                    convertObject(field.schema(), messageValue.getField(fieldDescriptor));
                        }
                ...
                case UNION:
                    // Unions only occur for nullable fields when working with proto + avro and null
                    // is the first schema in the union
                    return convertObject(schema.getTypes().get(1), value);
                default:
                    throw new HoodieException(
                            "Proto to Avro conversion failed for schema \""
                                    + schema
                                    + "\" and value \""
                                    + value
                                    + "\"");
        ...
        private static class SchemaCacheKey {
            ...
            public boolean equals(Object o) {
                ...
                return wrappedPrimitivesAsRecords == that.wrappedPrimitivesAsRecords
                        && maxRecursionDepth == that.maxRecursionDepth
                        && timestampsAsRecords == that.timestampsAsRecords
            ...
            public int hashCode() {
                return Objects.hash(
                        className, wrappedPrimitivesAsRecords, maxRecursionDepth, timestampsAsRecords);
            }
        }
    }

    static BigInteger toUnsignedBigInteger(long input) {
        // if the value is less than the max unsigned, avoid doing conversion to avoid performance
        // impact
        if (input >= 0L) {
