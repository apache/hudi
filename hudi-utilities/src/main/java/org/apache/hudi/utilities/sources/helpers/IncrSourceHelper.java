...
public class IncrSourceHelper {
    ...
    /**
      ...
      * instead we use {@link HollowCommitHandling#BLOCK} to block processing data from going beyond
      * the hollow commits to avoid unintentional skip.
      *
      * <p>Users can set {@link DataSourceReadOptions#INCREMENTAL_READ_HANDLE_HOLLOW_COMMIT} to {@link
    ...
    public static HollowCommitHandling getHollowCommitHandleMode(TypedProperties props) {
        return HollowCommitHandling.valueOf(
                props.getString(
                        INCREMENTAL_READ_HANDLE_HOLLOW_COMMIT().key(), HollowCommitHandling.BLOCK.name()));
    }
    ...
    /**
      ...
      * @param missingCheckpointStrategy when begin instant is missing, allow reading based on missing
      *     checkpoint strategy
      ...
      * @param sourceLimitBasedBatching When sourceLimit based batching is used, we need to fetch the
      *     current commit as well, this flag is used to indicate that.
      ...
      */
    public static QueryInfo generateQueryInfo(
        ...
        final HoodieTimeline activeCommitTimeline =
                handleHollowCommitIfNeeded(completedCommitTimeline, srcMetaClient, handlingMode);
        Function<HoodieInstant, String> timestampForLastInstant =
                instant ->
                        handlingMode == HollowCommitHandling.USE_TRANSITION_TIME
                                ? instant.getCompletionTime()
                                : instant.getTimestamp();
        String beginInstantTime =
                beginInstant.orElseGet(
                        () -> {
                            if (missingCheckpointStrategy != null) {
                                if (missingCheckpointStrategy == MissingCheckpointStrategy.READ_LATEST) {
                                    ...
                                    return lastInstant
                                            .map(
                                                    hoodieInstant ->
                                                            getStrictlyLowerTimestamp(
                                                                    timestampForLastInstant.apply(hoodieInstant)))
                                ...
                                throw new IllegalArgumentException(
                                        "Missing begin instant for incremental pull. For reading from latest "
                                                + "committed instant set hoodie.streamer.source.hoodieincr.missing.checkpoint.strategy to a valid value");
                            }
                        });

        // When `beginInstantTime` is present, `previousInstantTime` is set to the completed commit
        // before `beginInstantTime` if that exists.
        // If there is no completed commit before `beginInstantTime`, e.g., `beginInstantTime` is
        // the first commit in the active timeline,
        ...
        if (!beginInstantTime.equals(DEFAULT_BEGIN_TIMESTAMP)) {
            ...
            if (previousInstant.isPresent()) {
                ...
                // if begin instant time matches first entry in active timeline, we can set previous
                // = beginInstantTime - 1
        ...
        if (missingCheckpointStrategy == MissingCheckpointStrategy.READ_LATEST
            ...
            // When we are in the upgrade code path from non-sourcelimit-based batching to
            // sourcelimit-based batching, we need to avoid fetching the commit
            // that is read already. Else we will have duplicates in append-only use case if we use
            // "findInstantsAfterOrEquals".
            // As soon as we have a new format of checkpoint and a key we will move to the new code
            // of fetching the current commit as well.
            if (sourceLimitBasedBatching && lastCheckpointKey.isPresent()) {
                nthInstant =
                        Option.fromJavaOptional(
                                activeCommitTimeline
                                        .findInstantsAfterOrEquals(beginInstantTime, numInstantsPerFetch)
                                        .getInstantsAsStream()
                                        .reduce((x, y) -> y));
            } else {
                nthInstant =
            ...
            // when MissingCheckpointStrategy is set to read everything until latest, trigger
            // snapshot query.
            Option<HoodieInstant> lastInstant = activeCommitTimeline.lastInstant();
            return new QueryInfo(
    ...
    public static Pair<CloudObjectIncrCheckpoint, Option<Dataset<Row>>>
        ...
        Option<String> concatenatedKey =
                lastCheckpoint.flatMap(checkpoint -> lastCheckpointKey.map(key -> checkpoint + key));

        // Filter until last checkpoint key
        if (concatenatedKey.isPresent()) {
            orderedDf =
                    orderedDf.withColumn(
                            "commit_key",
                            functions.concat(
                                    functions.col(queryInfo.getOrderColumn()),
                                    functions.col(queryInfo.getKeyColumn())));
            // Apply incremental filter
            orderedDf =
                    orderedDf
                            .filter(functions.col("commit_key").gt(concatenatedKey.get()))
                            .drop("commit_key");
            ...
            if (orderedDf.isEmpty()) {
                ...
                // If current checkpoint is c1#abc and queryInfo.getEndInstant() is c1, return
                // c1#abc.
        ...
        WindowSpec windowSpec =
                Window.orderBy(col(queryInfo.getOrderColumn()), col(queryInfo.getKeyColumn()));
        // Add the 'cumulativeSize' column with running sum of 'limitColumn'
        Dataset<Row> aggregatedData =
                orderedDf.withColumn(
                        CUMULATIVE_COLUMN_NAME, sum(col(queryInfo.getLimitColumn())).over(windowSpec));
        ...
        if (collectedRows.isEmpty()) {
            ...
            row =
                    aggregatedData
                            .select(queryInfo.getOrderColumn(), queryInfo.getKeyColumn(), CUMULATIVE_COLUMN_NAME)
                            .first();
        ...
        sourceData.unpersist();
        return Pair.of(
                new CloudObjectIncrCheckpoint(row.getString(0), row.getString(1)),
                Option.of(collectedRows));
    }

    /**
      * Determine the policy to choose if a checkpoint is missing (detected by the absence of a
      * beginInstant), during a run of a {@link HoodieIncrSource}.
      *
    ...
    public static MissingCheckpointStrategy getMissingCheckpointStrategy(TypedProperties props) {
        boolean readLatestOnMissingCkpt =
                getBooleanWithAltKeys(props, READ_LATEST_INSTANT_ON_MISSING_CKPT);
        ...
        if (containsConfigProperty(props, MISSING_CHECKPOINT_STRATEGY)) {
            return MissingCheckpointStrategy.valueOf(
                    getStringWithAltKeys(props, MISSING_CHECKPOINT_STRATEGY));
        }
