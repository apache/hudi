...
/**
  * A validator with spark-submit to compare information, such as partitions, file listing, index,
  * etc., between metadata table and filesystem.
  *
  * <p>There are seven validation tasks, that can be enabled independently through the following CLI
  * options: - `--validate-latest-file-slices`: validate the latest file slices for all partitions. -
  ...
  * `--validate-record-index-count`: validate the number of entries in the record index, which should
  * be equal to the number of record keys in the latest snapshot of the table. -
  * `--validate-record-index-content`: validate the content of the record index so that each record
  * key should have the correct location, and there is no additional or missing entry.
  *
  * <p>If the Hudi table is on the local file system, the base path passed to `--base-path` must have
  * "file:" prefix to avoid validation failure.
  *
  * <p>- Default : This validator will compare the results between metadata table and filesystem only
  * once.
  *
  * <p>Example command: ``` spark-submit \ --class
  * org.apache.hudi.utilities.HoodieMetadataTableValidator \ --master spark://xxxx:7077 \
  * --driver-memory 1g \ --executor-memory 1g \
  * $HUDI_DIR/packaging/hudi-utilities-bundle/target/hudi-utilities-bundle_2.11-0.13.0-SNAPSHOT.jar \
  * --base-path basePath \ --validate-latest-file-slices \ --validate-latest-base-files \
  ...
  * $HUDI_DIR/packaging/hudi-utilities-bundle/target/hudi-utilities-bundle_2.11-0.13.0-SNAPSHOT.jar \
  * --base-path basePath \ --validate-latest-file-slices \ --validate-latest-base-files \
  * --validate-all-file-groups \ --continuous \ --min-validate-interval-seconds 60 ```
  */
public class HoodieMetadataTableValidator implements Serializable {
    ...
    public HoodieMetadataTableValidator(JavaSparkContext jsc, Config cfg) {
        ...
        this.asyncMetadataTableValidateService =
                cfg.continuous ? Option.of(new AsyncMetadataTableValidateService()) : Option.empty();
        this.taskLabels = generateValidationTaskLabels();
    }

    /**
      * Returns list of Throwable which were encountered during validation. This method is useful when
      * ignoreFailed parameter is set to true.
      */
    ...
    /**
      * Returns true if there is a validation failure encountered during validation. This method is
      * useful when ignoreFailed parameter is set to true.
      */
    ...
    public static class Config implements Serializable {
        ...
        @Parameter(
                names = {"--skip-data-files-for-cleaning"},
                description = "Skip to compare the data files which are under deletion by cleaner",
                required = false)
        public boolean skipDataFilesForCleaning = false;

        @Parameter(
                names = {"--validate-latest-file-slices"},
                description = "Validate latest file slices for all partitions.",
                required = false)
        public boolean validateLatestFileSlices = false;

        @Parameter(
                names = {"--validate-latest-base-files"},
                description = "Validate latest base files for all partitions.",
                required = false)
        public boolean validateLatestBaseFiles = false;

        @Parameter(
                names = {"--validate-all-file-groups"},
                description = "Validate all file groups, and all file slices within file groups.",
                required = false)
        public boolean validateAllFileGroups = false;

        @Parameter(
                names = {"--validate-all-column-stats"},
                description = "Validate column stats for all columns in the schema",
                required = false)
        public boolean validateAllColumnStats = false;

        @Parameter(
                names = {"--validate-bloom-filters"},
                description = "Validate bloom filters of base files",
                required = false)
        public boolean validateBloomFilters = false;

        @Parameter(
                names = {"--validate-record-index-count"},
                description =
                        "Validate the number of entries in the record index, which should be equal "
                                + "to the number of record keys in the latest snapshot of the table",
                required = false)
        public boolean validateRecordIndexCount = false;

        @Parameter(
                names = {"--validate-record-index-content"},
                description =
                        "Validate the content of the record index so that each record key should "
                                + "have the correct location, and there is no additional or missing entry",
                required = false)
        public boolean validateRecordIndexContent = false;

        @Parameter(
                names = {"--num-record-index-error-samples"},
        ...
        @Parameter(
                ...
                description = "Parallelism for valuation",
                required = false)
        public int parallelism = 200;

        @Parameter(
                names = {"--record-index-parallelism", "-rpl"},
                description = "Parallelism for validating record index",
                required = false)
        public int recordIndexParallelism = 100;

        @Parameter(
                names = {"--ignore-failed", "-ig"},
                description = "Ignore metadata validate failure and continue.",
                required = false)
        ...
        public String toString() {
            return "MetadataTableValidatorConfig {\n"
                    ...
                    + validateLatestBaseFiles
                    + ", \n"
                    + "   --validate-all-file-groups "
                    + validateAllFileGroups
                    + ", \n"
                    + "   --validate-all-column-stats "
                    + validateAllColumnStats
                    + ", \n"
                    + "   --validate-bloom-filters "
                    + validateBloomFilters
                    + ", \n"
                    + "   --validate-record-index-count "
                    + validateRecordIndexCount
                    + ", \n"
                    + "   --validate-record-index-content "
                    + validateRecordIndexContent
                    + ", \n"
    ...
    public static void main(String[] args) {
        ...
        SparkConf sparkConf =
                UtilHelpers.buildSparkConf("Hoodie-Metadata-Table-Validator", cfg.sparkMaster);
        ...
        try {
            ...
            LOG.warn(
                    String.format(
                            "The Hudi data table is not found: [%s]. "
                                    + "Skipping the validation of the metadata table.",
                            cfg.basePath),
                    e);
    ...
    public boolean run() {
        ...
        try {
            ...
            if (cfg.continuous) {
                LOG.info(
                        " ****** do hoodie metadata table validation in CONTINUOUS mode - {} ******",
                        taskLabels);
            ...
            throw new HoodieException(
                    "Unable to do hoodie metadata table validation in " + cfg.basePath, e);
        } finally {
    ...
    private boolean doHoodieMetadataTableValidationOnce() {
        try {
            ...
            LOG.error(
                    "Metadata table validation failed to HoodieValidationException {} {}", taskLabels, e);
            if (!cfg.ignoreFailed) {
                throw e;
            }
            throwables.add(e);
    ...
    private void doHoodieMetadataTableValidationContinuous() {
        asyncMetadataTableValidateService.ifPresent(
                service -> {
                    service.start(null);
                    try {
    ...
    public boolean doMetadataTableValidation() {
        ...
        if (cfg.skipDataFilesForCleaning) {
            HoodieTimeline inflightCleaningTimeline =
                    metaClient.getActiveTimeline().getCleanerTimeline().filterInflights();

            baseFilesForCleaning =
                    inflightCleaningTimeline
                            .getInstantsAsStream()
                            .flatMap(
                                    instant -> {
                                        try {
                                            // convert inflight instant to requested and get clean
                                            // plan
                                            instant =
                                                    new HoodieInstant(
                                            ...
                                            return cleanerPlan.getFilePathsToBeDeletedPerPartition().values().stream()
                                                    .flatMap(
                                                            cleanerFileInfoList ->
                                                                    cleanerFileInfoList.stream()
                                                                            .map(fileInfo -> new Path(fileInfo.getFilePath()).getName()));
        ...
        if (allPartitions.isEmpty()) {
            LOG.warn(
                    "The result of getting all partitions is null or empty, skip current validation. {}",
                    taskLabels);
            return true;
        }
        ...
        try (HoodieMetadataValidationContext metadataTableBasedContext =
            ...
            List<Pair<Boolean, ? extends Exception>> result =
                    new ArrayList<>(
                            engineContext
                                    .parallelize(allPartitions, allPartitions.size())
                                    .map(
                                            partitionPath -> {
                                                try {
                                                    validateFilesInPartition(
                                                            ...
                                                            fsBasedContext,
                                                            partitionPath,
                                                            finalBaseFilesForCleaning);
                                                    LOG.info(
                                                            "Metadata table validation succeeded for partition {} (partition {})",
                                                            partitionPath,
                                                            taskLabels);
                                                    ...
                                                    LOG.error(
                                                            String.format(
                                                                    "Metadata table validation failed for partition %s due to HoodieValidationException (partition %s)",
                                                                    partitionPath, taskLabels),
                                                            e);
                                                    if (!cfg.ignoreFailed) {
                                                        throw e;
                                                    }
                                                    return Pair.of(
                                                            false,
                                                            new HoodieValidationException(
                                                                    e.getMessage() + " for partition: " + partitionPath, e));
                                                }
                                            })
                                    .collectAsList());
            ...
            try {
                ...
                LOG.error(
                        "Metadata table validation failed due to HoodieValidationException in record index validation for table: {} ",
                        cfg.basePath,
                        e);
                if (!cfg.ignoreFailed) {
                    throw e;
            ...
            for (Pair<Boolean, ? extends Exception> res : result) {
                ...
                if (res.getKey().equals(false)) {
                    LOG.error(
                            "Metadata Validation failed for table: "
                                    + cfg.basePath
                                    + " with error: "
                                    + res.getValue());
            ...
            LOG.warn(
                    "Error closing HoodieMetadataValidationContext, "
                            + "ignoring the error as the validation is successful.",
                    e);
            return true;
        }
    }
    ...
    /**
      * Check metadata is initialized and available to ready. If not we will log.warn and skip current
      * validation.
      */
        ...
        try {
            HoodieTableMetaClient mdtMetaClient =
                    HoodieTableMetaClient.builder()
                            .setConf(HadoopFSUtils.getStorageConfWithCopy(jsc.hadoopConfiguration()))
                            .setBasePath(
                                    new Path(cfg.basePath, HoodieTableMetaClient.METADATA_TABLE_FOLDER_PATH)
                                            .toString())
                            .setLoadActiveTimelineOnLoad(true)
                            .build();
            ...
            if (finishedInstants == 0) {
                if (metaClient.getCommitsTimeline().filterCompletedInstants().countInstants() == 0) {
                    LOG.info(
                            "There is no completed commit in both metadata table and corresponding data table: {}",
                            taskLabels);
                    return false;
                } else {
                    throw new HoodieValidationException(
                            "There is no completed instant for metadata table: " + cfg.basePath);
                }
            }
            return true;
        } catch (TableNotFoundException tbe) {
            // Suppress the TableNotFound exception if Metadata table is not available to read for
            // now
    ...
    @VisibleForTesting
    List<String> validatePartitions(
        ...
        List<String> allPartitionPathsFromFS =
                getPartitionsFromFileSystem(
                        engineContext, basePath, metaClient.getStorage(), completedTimeline);

        List<String> allPartitionPathsMeta =
                getPartitionsFromMDT(engineContext, basePath, metaClient.getStorage());
        ...
        if (allPartitionPathsFromFS.size() != allPartitionPathsMeta.size()
            ...
            if (additionalFromFS.isEmpty() && !additionalFromMDT.isEmpty()) {
                // there is a chance that when we polled MDT there could have been a new completed
                // commit which was not complete when we polled FS based
                // listing. let's rule that out.
                additionalFromMDT.forEach(
                        partitionFromDMT -> {
                            Option<String> partitionCreationTimeOpt =
                                    getPartitionCreationInstant(metaClient.getStorage(), basePath, partitionFromDMT);
                            ...
                            if (partitionCreationTimeOpt.isPresent()
                                ...
                                if (lastInstant.isPresent()
                                        && HoodieTimeline.compareTimestamps(
                                                partitionCreationTimeOpt.get(),
                                                GREATER_THAN,
                                                lastInstant.get().getTimestamp())) {
                                    LOG.warn(
                                            "Ignoring additional partition {}, as it was deduced to be part of a "
                ...
                // if there is no additional partitions from FS listing and only additional
                // partitions from MDT based listing is due to a new commit, we are good
            ...
            if (misMatch) {
                String message =
                        "Compare Partitions Failed! "
                                + " Additional partitions from FS, but missing from MDT : \""
                                + additionalFromFS
                                + "\" and additional partitions from MDT, but missing from FS listing : \""
                                + actualAdditionalPartitionsInMDT
    ...
    @VisibleForTesting
    Option<String> getPartitionCreationInstant(
            HoodieStorage storage, StoragePath basePath, String partition) {
    ...
    @VisibleForTesting
    List<String> getPartitionsFromMDT(
            HoodieEngineContext engineContext, StoragePath basePath, HoodieStorage storage) {
        return FSUtils.getAllPartitionPaths(engineContext, storage, basePath, true);
    }

    @VisibleForTesting
    List<String> getPartitionsFromFileSystem(
            HoodieEngineContext engineContext,
            StoragePath basePath,
            HoodieStorage storage,
            HoodieTimeline completedTimeline) {
        List<String> allPartitionPathsFromFS =
                FSUtils.getAllPartitionPaths(engineContext, storage, basePath, false);

        // ignore partitions created by uncommitted ingestion.
        return allPartitionPathsFromFS.stream()
                .parallel()
                .filter(
                        part -> {
                            HoodiePartitionMetadata hoodiePartitionMetadata =
                                    new HoodiePartitionMetadata(
                                            storage, FSUtils.constructAbsolutePath(basePath, part));
                            ...
                            if (instantOption.isPresent()) {
                                ...
                                // There are two cases where the created commit time is written to
                                // the partition metadata:
                                // (1) Commit C1 creates the partition and C1 succeeds, the
                                // partition metadata has C1 as
                                // the created commit time.
                                // (2) Commit C1 creates the partition, the partition metadata is
                                // written, and C1 fails
                                // during writing data files.  Next time, C2 adds new data to the
                                // same partition after C1
    ...
    /**
      * Compare the file listing and index data between metadata table and fileSystem. For now,
      * validate five kinds of apis: 1. HoodieMetadataFileSystemView::getLatestFileSlices 2.
      * HoodieMetadataFileSystemView::getLatestBaseFiles 3.
      * HoodieMetadataFileSystemView::getAllFileGroups and
      * HoodieMetadataFileSystemView::getAllFileSlices 4. HoodieBackedTableMetadata::getColumnStats 5.
      * HoodieBackedTableMetadata::getBloomFilters
      *
      * @param metadataTableBasedContext Validation context containing information based on metadata
      *     table
    ...
    private void validateFilesInPartition(
        ...
        if (cfg.validateLatestFileSlices) {
            validateLatestFileSlices(
                    metadataTableBasedContext, fsBasedContext, partitionPath, baseDataFilesForCleaning);
        }
    ...
    private void validateAllFileGroups(
        ...
        if (!baseDataFilesForCleaning.isEmpty()) {
            List<FileSlice> fileSlicesFromMeta =
                    metadataTableBasedContext.getSortedAllFileGroupList(partitionPath).stream()
                            .flatMap(HoodieFileGroup::getAllFileSlices)
                            .sorted(new FileSliceComparator())
                            .collect(Collectors.toList());
            List<FileSlice> fileSlicesFromFS =
                    fsBasedContext.getSortedAllFileGroupList(partitionPath).stream()
                            .flatMap(HoodieFileGroup::getAllFileSlices)
                            .sorted(new FileSliceComparator())
                            .collect(Collectors.toList());

            allFileSlicesFromMeta =
                    filterFileSliceBasedOnInflightCleaning(fileSlicesFromMeta, baseDataFilesForCleaning);
            allFileSlicesFromFS =
                    filterFileSliceBasedOnInflightCleaning(fileSlicesFromFS, baseDataFilesForCleaning);
        } else {
            allFileSlicesFromMeta =
                    metadataTableBasedContext.getSortedAllFileGroupList(partitionPath).stream()
                            .flatMap(HoodieFileGroup::getAllFileSlices)
        ...
        LOG.debug(
                "All file slices from metadata: {}. For partitions {}",
                allFileSlicesFromMeta,
                partitionPath);
        LOG.debug(
                "All file slices from direct listing: {}. For partitions {}",
                allFileSlicesFromFS,
                partitionPath);
        validateFileSlices(
                allFileSlicesFromMeta,
    ...
    private void validateLatestBaseFiles(
        ...
        if (!baseDataFilesForCleaning.isEmpty()) {
            latestFilesFromMetadata =
                    filterBaseFileBasedOnInflightCleaning(
                            metadataTableBasedContext.getSortedLatestBaseFileList(partitionPath),
                            baseDataFilesForCleaning);
            latestFilesFromFS =
                    filterBaseFileBasedOnInflightCleaning(
                            fsBasedContext.getSortedLatestBaseFileList(partitionPath), baseDataFilesForCleaning);
        } else {
            latestFilesFromMetadata =
                    metadataTableBasedContext.getSortedLatestBaseFileList(partitionPath);
        ...
        LOG.debug(
                "Latest base file from metadata: {}. For partitions {}",
                latestFilesFromMetadata,
                partitionPath);
        LOG.debug(
                "Latest base file from direct listing: {}. For partitions {}",
                latestFilesFromFS,
                partitionPath);
    ...
    private void validateLatestFileSlices(
        ...
        if (!baseDataFilesForCleaning.isEmpty()) {
            latestFileSlicesFromMetadataTable =
            ...
            latestFileSlicesFromFS =
                    filterFileSliceBasedOnInflightCleaning(
                            fsBasedContext.getSortedLatestFileSliceList(partitionPath), baseDataFilesForCleaning);
        } else {
            latestFileSlicesFromMetadataTable =
                    metadataTableBasedContext.getSortedLatestFileSliceList(partitionPath);
        ...
        LOG.debug(
                "Latest file list from metadata: {}. For partition {}",
                latestFileSlicesFromMetadataTable,
                partitionPath);
        LOG.debug(
                "Latest file list from direct listing: {}. For partition {}",
                latestFileSlicesFromFS,
                partitionPath);
    ...
    private List<FileSlice> filterFileSliceBasedOnInflightCleaning(
            List<FileSlice> sortedLatestFileSliceList, Set<String> baseDataFilesForCleaning) {
                ...
                .filter(
                        fileSlice -> {
                            if (!fileSlice.getBaseFile().isPresent()) {
                                ...
                                return !baseDataFilesForCleaning.contains(
    ...
    private List<HoodieBaseFile> filterBaseFileBasedOnInflightCleaning(
            List<HoodieBaseFile> sortedBaseFileList, Set<String> baseDataFilesForCleaning) {
    ...
    private void validateAllColumnStats(
        ...
        List<HoodieColumnRangeMetadata<Comparable>> metadataBasedColStats =
                metadataTableBasedContext.getSortedColumnStatsList(partitionPath, latestBaseFilenameList);
        List<HoodieColumnRangeMetadata<Comparable>> fsBasedColStats =
                fsBasedContext.getSortedColumnStatsList(partitionPath, latestBaseFilenameList);
    ...
    private void validateBloomFilters(
        ...
        List<String> latestBaseFilenameList =
                getLatestBaseFileNames(fsBasedContext, partitionPath, baseDataFilesForCleaning);
        List<BloomFilterData> metadataBasedBloomFilters =
                metadataTableBasedContext.getSortedBloomFilterList(partitionPath, latestBaseFilenameList);
        List<BloomFilterData> fsBasedBloomFilters =
                fsBasedContext.getSortedBloomFilterList(partitionPath, latestBaseFilenameList);

        validate(metadataBasedBloomFilters, fsBasedBloomFilters, partitionPath, "bloom filters");
    }

    private void validateRecordIndex(
            HoodieSparkEngineContext sparkEngineContext, HoodieTableMetaClient metaClient) {
        if (!metaClient
                .getTableConfig()
    ...
    private void validateRecordIndexCount(
            HoodieSparkEngineContext sparkEngineContext, HoodieTableMetaClient metaClient) {
        ...
        if (countKeyFromTable != countKeyFromRecordIndex) {
            String message =
                    String.format(
                            "Validation of record index count failed: %s entries from record index metadata, %s keys from the data table: %s",
                            countKeyFromRecordIndex, countKeyFromTable, cfg.basePath);
            LOG.error(message);
            throw new HoodieValidationException(message);
        } else {
            LOG.info(
                    "Validation of record index count succeeded: {} entries. Table: {}",
                    countKeyFromRecordIndex,
                    cfg.basePath);
        }
    }

    private void validateRecordIndexContent(
            HoodieSparkEngineContext sparkEngineContext, HoodieTableMetaClient metaClient) {
        String basePath = metaClient.getBasePath().toString();
        String latestCompletedCommit =
                metaClient
        ...
        Pair<Long, List<String>> result =
                keyToLocationOnFsRdd
                        .fullOuterJoin(keyToLocationFromRecordIndexRdd, cfg.recordIndexParallelism)
                        .map(
                                e -> {
                                    ...
                                    if (locationOnFs.isPresent() && locationFromRecordIndex.isPresent()) {
                                        if (locationOnFs.get().getLeft().equals(locationFromRecordIndex.get().getLeft())
                                                && locationOnFs
                                                        .get()
                                                        .getRight()
                                                        .equals(locationFromRecordIndex.get().getRight())) {
                                            return Pair.of(0L, errorSampleList);
                                        }
                                        errorSampleList.add(
                                                constructLocationInfoString(
                                                        recordKey, locationOnFs, locationFromRecordIndex));
        ...
        if (diffCount > 0) {
            ...
            LOG.info(
                    "Validation of record index content succeeded: {} entries. Table: {}",
                    countKey,
                    cfg.basePath);
        }
    }
    ...
    @VisibleForTesting
    JavaPairRDD<String, Pair<String, String>> getRecordLocationsFromFSBasedListing(
            HoodieSparkEngineContext sparkEngineContext, String basePath, String latestCompletedCommit) {
        return sparkEngineContext
                .getSqlContext()
                .read()
                .format("hudi")
                .option(DataSourceReadOptions.TIME_TRAVEL_AS_OF_INSTANT().key(), latestCompletedCommit)
                ...
                .mapToPair(
                        row ->
                                new Tuple2<>(
                                        row.getString(row.fieldIndex(RECORD_KEY_METADATA_FIELD)),
                                        Pair.of(
                                                row.getString(row.fieldIndex(PARTITION_PATH_METADATA_FIELD)),
                                                FSUtils.getFileId(row.getString(row.fieldIndex(FILENAME_METADATA_FIELD))))))
                .cache();
    }

    @VisibleForTesting
    JavaPairRDD<String, Pair<String, String>> getRecordLocationsFromRLI(
            HoodieSparkEngineContext sparkEngineContext, String basePath, String latestCompletedCommit) {
        return sparkEngineContext
                .getSqlContext()
                .read()
                ...
                .map(
                        row -> {
                            ...
                            if (HoodieTimeline.compareTimestamps(
                                    location.getInstantTime(), GREATER_THAN, latestCompletedCommit)) {
                ...
                .mapToPair(
                        tuple2 -> {
                            ...
                            return new Tuple2<>(
                                    rowAndLocation._1.getString(rowAndLocation._1.fieldIndex("key")),
                                    Pair.of(
                                            rowAndLocation._2.get().getPartitionPath(),
                                            rowAndLocation._2.get().getFileId()));
                        })
                .cache();
    }

    private String constructLocationInfoString(
            String recordKey,
    ...
    private List<String> getLatestBaseFileNames(
            HoodieMetadataValidationContext fsBasedContext,
            String partitionPath,
            Set<String> baseDataFilesForCleaning) {
        List<String> latestBaseFilenameList;
        if (!baseDataFilesForCleaning.isEmpty()) {
            ...
            latestBaseFilenameList =
                    filterBaseFileBasedOnInflightCleaning(sortedLatestBaseFileList, baseDataFilesForCleaning)
                            .stream()
                            .map(BaseFile::getFileName)
                            .collect(Collectors.toList());
        } else {
            latestBaseFilenameList =
                    fsBasedContext.getSortedLatestBaseFileList(partitionPath).stream()
                            .map(BaseFile::getFileName)
                            .collect(Collectors.toList());
        }
    ...
    private <T> void validate(
            List<T> infoListFromMetadataTable,
            List<T> infoListFromFS,
            String partitionPath,
            String label) {
        if (infoListFromMetadataTable.size() != infoListFromFS.size()
            ...
            LOG.info(
                    "Validation of {} succeeded for partition {} for table: {}",
                    label,
                    partitionPath,
                    cfg.basePath);
        }
    ...
    private void validateFileSlices(
            List<FileSlice> fileSliceListFromMetadataTable,
            List<FileSlice> fileSliceListFromFS,
            String partitionPath,
            HoodieTableMetaClient metaClient,
            String label) {
        ...
        if (fileSliceListFromMetadataTable.size() != fileSliceListFromFS.size()) {
            ...
            for (int i = 0; i < fileSliceListFromMetadataTable.size(); i++) {
                ...
                if (!areFileSliceCommittedLogFilesMatching(
                    ...
                    LOG.warn(
                            "There are uncommitted log files in the latest file slices but the committed log files match: {} {}",
                            fileSlice1,
                            fileSlice2);
                }
            }
    ...
    private boolean areFileSliceCommittedLogFilesMatching(
        ...
        if (hasCommittedLogFiles(storage, fs1LogPathSet, metaClient, committedFilesMap)) {
            LOG.error(
                    "The first file slice has committed log files that cause mismatching: {}; Different log files are: {}",
                    fs1,
                    fs1LogPathSet);
            return false;
        }
        if (hasCommittedLogFiles(storage, fs2LogPathSet, metaClient, committedFilesMap)) {
            LOG.error(
                    "The second file slice has committed log files that cause mismatching: {}; Different log files are: {}",
                    fs2,
                    fs2LogPathSet);
            return false;
        }
        ...
        for (String logFilePathStr : logFilePathSet) {
            ...
            try {
                ...
                if (readerSchema == null) {
                    LOG.warn(
                            "Cannot read schema from log file {}. Skip the check as it's likely being written by an inflight instant.",
                            logFilePathStr);
                    continue;
                }
                reader =
                        HoodieLogFormat.newReader(
                                storage, new HoodieLogFile(logFilePathStr), readerSchema, false);
                ...
                if (reader.hasNext()) {
                    ...
                    if (completedInstantsTimeline.containsInstant(instantTime)) {
                        ...
                        // Checking commit metadata only as log files can only be written by COMMIT
                        // or DELTA_COMMIT
                        ...
                        // In such a case, the log file can stay in the file system, but the
                        // metadata
                        // table does not contain the log file for file listing, which is an
                        // expected
                        // behavior.
                        ...
                        if (committedFilesMap.get(instantTime).contains(relativeLogFilePathStr)) {
                            LOG.warn(
                                    "Log file is committed in an instant in active timeline: instantTime={} {}",
                                    instantTime,
                                    logFilePathStr);
                            return true;
                        } else {
                            LOG.warn(
                                    "Log file is uncommitted in a completed instant, likely due to retry: instantTime={} {}",
                                    instantTime,
                                    logFilePathStr);
                        }
                        ...
                        LOG.warn(
                                "Log file is committed in an instant in archived timeline: instantTime={} {}",
                                instantTime,
                                logFilePathStr);
                        return true;
                        ...
                        LOG.warn(
                                "Log file is uncommitted because of an inflight instant: instantTime={} {}",
                                instantTime,
                                logFilePathStr);
                    } else {
                        ...
                        LOG.warn(
                                "Log file is uncommitted because the instant is after the start of the active timeline but absent or in requested in the active timeline: instantTime={} {}",
                                instantTime,
                                logFilePathStr);
                    }
                } else {
                ...
                LOG.warn(
                        String.format(
                                "Cannot read log file %s: %s. Skip the check as it's likely being written by an inflight instant.",
                                logFilePathStr, e.getMessage()),
                        e);
            } finally {
    ...
    private String getRelativePath(String basePath, String absoluteFilePath) {
        ...
        String absoluteFilePathStr =
                new StoragePath(absoluteFilePath).getPathWithoutSchemeAndAuthority().toString();

        if (!absoluteFilePathStr.startsWith(basePathStr)) {
            throw new IllegalArgumentException(
                    "File path does not belong to the base path! basePath="
                            + basePathStr
                            + " absoluteFilePathStr="
                            + absoluteFilePathStr);
        }
                            ...
                            () -> {
                                while (true) {
                                    try {
                                        ...
                                        if (toSleepMs > 0) {
                                            LOG.info(
                                                    "Last validate ran less than min validate interval: {} s, sleep: {} ms.",
                                        ...
                                        LOG.error(
                                                "Shutting down AsyncMetadataTableValidateService due to HoodieValidationException",
                                                e);
                                        if (!cfg.ignoreFailed) {
                                            throw e;
                                        }
    ...
    public static class HoodieFileGroupComparator
            implements Comparator<HoodieFileGroup>, Serializable {
    ...
    public static class HoodieColumnRangeMetadataComparator
        ...
        @Override
        public int compare(
                HoodieColumnRangeMetadata<Comparable> o1, HoodieColumnRangeMetadata<Comparable> o2) {
    ...
    /**
      ...
      *
      * <p>If metadata table is disabled, the APIs provide the information, e.g., file listing, index,
      * from the file system and base files. If metadata table is enabled, the APIs provide the
      * information from the metadata table. The same API is expected to return the same information
      * regardless of whether metadata table is enabled, which is verified in the {@link
    ...
    private static class HoodieMetadataValidationContext implements AutoCloseable, Serializable {
        ...
        public List<HoodieBaseFile> getSortedLatestBaseFileList(String partitionPath) {
            return fileSystemView
                    ...
                    .sorted(new HoodieBaseFileComparator())
                    .collect(Collectors.toList());
        }
        ...
        public List<HoodieFileGroup> getSortedAllFileGroupList(String partitionPath) {
            return fileSystemView
                    ...
                    .sorted(new HoodieFileGroupComparator())
                    .collect(Collectors.toList());
        }
            ...
            if (enableMetadataTable) {
                List<Pair<String, String>> partitionFileNameList =
                        baseFileNameList.stream()
                                .map(filename -> Pair.of(partitionPath, filename))
                                .collect(Collectors.toList());
                return allColumnNameList.stream()
                        .flatMap(
                                columnName ->
                                        tableMetadata.getColumnStats(partitionFileNameList, columnName).values()
                                                .stream()
        ...
        public List<BloomFilterData> getSortedBloomFilterList(
            ...
            if (enableMetadataTable) {
                ...
                return tableMetadata.getBloomFilters(partitionFileNameList).entrySet().stream()
                        .map(
                                entry ->
                                        BloomFilterData.builder()
                                                .setPartitionPath(entry.getKey().getKey())
                                                .setFilename(entry.getKey().getValue())
                                                .setBloomFilter(
                                                        ByteBuffer.wrap(getUTF8Bytes(entry.getValue().serializeToString())))
                                                .build())
                        .sorted()
                        .collect(Collectors.toList());
            } else {
        ...
        private Option<BloomFilterData> readBloomFilterFromFile(String partitionPath, String filename) {
            StoragePath path =
                    new StoragePath(
                            FSUtils.constructAbsolutePath(metaClient.getBasePath(), partitionPath).toString(),
                            filename);
            ...
            hoodieConfig.setValue(
                    ...
                    Boolean.toString(
                            ConfigUtils.getBooleanWithAltKeys(
                                    props, HoodieReaderConfig.USE_NATIVE_HFILE_READER)));
