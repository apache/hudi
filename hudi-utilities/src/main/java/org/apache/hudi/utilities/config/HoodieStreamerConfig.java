...
public class HoodieStreamerConfig extends HoodieConfig {
    ...
    public static final ConfigProperty<String> CHECKPOINT_PROVIDER_PATH =
            ConfigProperty.key(STREAMER_CONFIG_PREFIX + "checkpoint.provider.path")
                    .noDefaultValue()
    ...
    public static final ConfigProperty<Boolean> CHECKPOINT_FORCE_SKIP =
            ConfigProperty.key(STREAMER_CONFIG_PREFIX + "checkpoint.force.skip")
                    .defaultValue(false)
                    .withAlternatives(DELTA_STREAMER_CONFIG_PREFIX + "checkpoint.force.skip")
                    .markAdvanced()
                    .withDocumentation(
                            "Config to force to skip saving checkpoint in the commit metadata."
                                    + "It is typically used in one-time backfill scenarios, where checkpoints are not "
                                    + "to be persisted.");

    public static final ConfigProperty<String> KAFKA_TOPIC =
            ConfigProperty.key(STREAMER_CONFIG_PREFIX + "source.kafka.topic")
                    .noDefaultValue()
                    .withAlternatives(DELTA_STREAMER_CONFIG_PREFIX + "source.kafka.topic")
                    .withDocumentation(
                            "Kafka topic name. The config is specific to HoodieMultiTableStreamer");

    public static final ConfigProperty<String> KAFKA_APPEND_OFFSETS =
            ConfigProperty.key(STREAMER_CONFIG_PREFIX + "source.kafka.append.offsets")
                    .defaultValue("false")
                    .withAlternatives(DELTA_STREAMER_CONFIG_PREFIX + "source.kafka.append.offsets")
                    .markAdvanced()
                    .withDocumentation(
                            "When enabled, appends kafka offset info like source offset(_hoodie_kafka_source_offset), "
    ...
    public static final ConfigProperty<Boolean> SANITIZE_SCHEMA_FIELD_NAMES =
            ConfigProperty.key(STREAMER_CONFIG_PREFIX + "source.sanitize.invalid.schema.field.names")
                    .defaultValue(false)
                    .withAlternatives(
                            DELTA_STREAMER_CONFIG_PREFIX + "source.sanitize.invalid.schema.field.names")
                    .markAdvanced()
                    .withDocumentation(
    ...
    public static final ConfigProperty<String> SCHEMA_FIELD_NAME_INVALID_CHAR_MASK =
            ConfigProperty.key(STREAMER_CONFIG_PREFIX + "source.sanitize.invalid.char.mask")
                    .defaultValue("__")
                    .withAlternatives(DELTA_STREAMER_CONFIG_PREFIX + "source.sanitize.invalid.char.mask")
                    .markAdvanced()
                    .withDocumentation(
                            "Defines the character sequence that replaces invalid characters in schema field names if "
                                    + "hoodie.streamer.source.sanitize.invalid.schema.field.names is enabled.");

    public static final ConfigProperty<String> TABLES_TO_BE_INGESTED =
            ConfigProperty.key(INGESTION_PREFIX + "tablesToBeIngested")
                    .noDefaultValue()
    ...
    public static final ConfigProperty<String> TARGET_BASE_PATH =
            ConfigProperty.key(INGESTION_PREFIX + "targetBasePath")
                    .defaultValue("")
                    .withAlternatives(OLD_INGESTION_PREFIX + "targetBasePath")
                    .markAdvanced()
                    .withDocumentation(
                            "The path to which a particular table is ingested. The config is specific to HoodieMultiTableStreamer"
                                    + " and overrides path determined using option `--base-path-prefix` for a table. This config is ignored for a single"
                                    + " table streamer");

    public static final ConfigProperty<String> TRANSFORMER_CLASS =
            ConfigProperty.key(STREAMER_CONFIG_PREFIX + "transformer.class")
                    .noDefaultValue()
                    .withAlternatives(DELTA_STREAMER_CONFIG_PREFIX + "transformer.class")
                    .markAdvanced()
                    .sinceVersion("0.14.0")
                    .withDocumentation(
                            "Names of transformer classes to apply. The config is specific to HoodieMultiTableStreamer.");
    public static final ConfigProperty<Boolean> SAMPLE_WRITES_ENABLED =
            ConfigProperty.key(STREAMER_CONFIG_PREFIX + "sample.writes.enabled")
                    .defaultValue(false)
                    .withAlternatives(DELTA_STREAMER_CONFIG_PREFIX + "sample.writes.enabled")
                    .markAdvanced()
                    .sinceVersion("0.14.0")
                    .withDocumentation(
                            "Set this to true to sample from the first batch of records and write to the auxiliary path, before writing to the table."
                                    + "The sampled records are used to calculate the average record size. The relevant write client will have `"
    ...
    public static final ConfigProperty<Integer> SAMPLE_WRITES_SIZE =
            ConfigProperty.key(STREAMER_CONFIG_PREFIX + "sample.writes.size")
                    .defaultValue(5000)
                    .withAlternatives(DELTA_STREAMER_CONFIG_PREFIX + "sample.writes.size")
                    .markAdvanced()
                    .sinceVersion("0.14.0")
                    .withDocumentation(
                            "Number of records to sample from the first write. To improve the estimation's accuracy, "
    ...
    public static final ConfigProperty<Boolean> ROW_THROW_EXPLICIT_EXCEPTIONS =
            ConfigProperty.key(STREAMER_CONFIG_PREFIX + "row.throw.explicit.exceptions")
                    .defaultValue(false)
                    .markAdvanced()
                    .sinceVersion("0.15.0")
                    .withDocumentation(
                            "When enabled, the dataframe generated from reading source data is wrapped with an exception handler to explicitly surface exceptions.");
}
