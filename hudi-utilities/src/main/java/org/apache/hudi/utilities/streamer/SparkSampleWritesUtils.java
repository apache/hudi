...
/**
  * The utilities class is dedicated to estimating average record size by writing sample incoming
  * records to `.hoodie/.aux/.sample_writes/<instant time>/<epoch millis>` and reading the commit
  * metadata.
  *
  * <p>TODO handle sample_writes sub-path clean-up w.r.t. rollback and insert overwrite. (HUDI-6044)
  */
public class SparkSampleWritesUtils {

    private static final Logger LOG = LoggerFactory.getLogger(SparkSampleWritesUtils.class);

    public static Option<HoodieWriteConfig> getWriteConfigWithRecordSizeEstimate(
        ...
        try {
            ...
            LOG.error(
                    String.format(
                            "Not overwriting record size estimate for table %s due to error when doing sample writes.",
    ...
    private static Pair<Boolean, String> doSampleWrites(
            JavaSparkContext jsc,
            Option<JavaRDD<HoodieRecord>> recordsOpt,
            HoodieWriteConfig writeConfig,
            String instantTime)
        ...
        HoodieTableMetaClient.withPropertyBuilder()
                ...
                .setCDCEnabled(false)
                .initTable(
                        HadoopFSUtils.getStorageConfWithCopy(jsc.hadoopConfiguration()), sampleWritesBasePath);
        TypedProperties props = writeConfig.getProps();
        props.put(SAMPLE_WRITES_ENABLED.key(), "false");
        final HoodieWriteConfig sampleWriteConfig =
        ...
        try (SparkRDDWriteClient sampleWriteClient =
                new SparkRDDWriteClient(
                        new HoodieSparkEngineContext(jsc), sampleWriteConfig, Option.empty())) {
                    ...
                    .map(
                            records -> {
                                ...
                                if (writeStatusRDD.filter(WriteStatus::hasErrors).count() > 0) {
                                    LOG.error(
                                            "sample writes for table {} failed with errors.", writeConfig.getTableName());
                                                        ...
                                                        ws -> {
                                                            ...
                                                            ws.getErrors()
                                                                    .forEach(
                                                                            (key, throwable) ->
                                                                                    LOG.trace(
                                                                                            String.format("Error for key: %s", key), throwable));
                                                        });
                                    }
                                    return emptyRes;
                                } else {
    ...
    private static String getSampleWritesBasePath(
            JavaSparkContext jsc, HoodieWriteConfig writeConfig, String instantTime) throws IOException {
    ...
    private static long getAvgSizeFromSampleWrites(JavaSparkContext jsc, String sampleWritesBasePath)
            throws IOException {
        ...
        checkState(
                lastInstantOpt.isPresent(),
                "The only completed instant should be present in sample_writes table.");
        HoodieInstant instant = lastInstantOpt.get();
        HoodieCommitMetadata commitMetadata =
                HoodieCommitMetadata.fromBytes(
                        metaClient.getCommitTimeline().getInstantDetails(instant).get(),
