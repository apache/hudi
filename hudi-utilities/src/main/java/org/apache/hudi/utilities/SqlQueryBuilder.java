...
public class SqlQueryBuilder {
    ...
    public static SqlQueryBuilder select(String... columns) {
        if (columns == null || columns.length == 0) {
            throw new IllegalArgumentException(
                    "No columns provided with SELECT statement. Please mention column names or '*' to select all columns.");
        }
    ...
    public SqlQueryBuilder from(String... tables) {
        if (tables == null || tables.length == 0) {
            throw new IllegalArgumentException(
                    "No table name provided with FROM clause. Please provide a table name to select from.");
        }
    ...
    public SqlQueryBuilder join(String table) {
        if (StringUtils.isNullOrEmpty(table)) {
            throw new IllegalArgumentException(
                    "No table name provided with JOIN clause. Please provide a table name to join with.");
        }
    ...
    public SqlQueryBuilder where(String predicate) {
        if (StringUtils.isNullOrEmpty(predicate)) {
            throw new IllegalArgumentException(
                    "No predicate provided with WHERE clause. Please provide a predicate to filter records.");
        }
    ...
    /**
      * Appends an ORDER BY clause to a query. By default, records are ordered in ascending order by
      * the given column. To order in descending order use DESC after the column name, e.g.
      * queryBuilder.orderBy("update_time desc").
      *
    ...
    public SqlQueryBuilder orderBy(String... columns) {
        if (columns == null || columns.length == 0) {
            throw new IllegalArgumentException(
                    "No columns provided with ORDER BY clause. Please provide a column name to order records.");
        }
