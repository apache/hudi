...
/**
  * A {@link JsonKafkaSourcePostProcessor} help to extract fresh data from maxwell json string and
  * tag the record as delete or not.
  */
public class MaxwellJsonKafkaSourcePostProcessor extends JsonKafkaSourcePostProcessor {
    ...
    public MaxwellJsonKafkaSourcePostProcessor(TypedProperties props) {
        ...
        databaseRegex =
                Option.ofNullable(
                        getStringWithAltKeys(props, JsonKafkaPostProcessorConfig.DATABASE_NAME_REGEX, true));
    ...
    private String processDelete(JsonNode inputJson, ObjectNode result) {
        ...
        PreCombineFieldType preCombineFieldType =
                valueOf(
                        getStringWithAltKeys(
                                        this.props, JsonKafkaPostProcessorConfig.PRECOMBINE_FIELD_TYPE, true)
                                .toUpperCase(Locale.ROOT));

        // maxwell won't update the `update_time`(delete time) field of the record which is tagged
        // as delete. so if we
        // want to delete this record correctly, we should update its `update_time` to a time closer
        // to where the
        // delete operation actually occurred. here we use `ts` from maxwell json string as this
        // 'delete' time.
        ...
        if (!preCombineFieldType.equals(NON_TIMESTAMP)) {
            ...
            if (preCombineFieldType.equals(DATE_STRING)) {
                ...
                String timeFormat =
                        getStringWithAltKeys(
                                this.props, JsonKafkaPostProcessorConfig.PRECOMBINE_FIELD_FORMAT, true);
                ...
                throw new HoodieSourcePostProcessException(
                        "Unsupported preCombine time format " + preCombineFieldType);
            }
        }
        return result.toString();
    }
