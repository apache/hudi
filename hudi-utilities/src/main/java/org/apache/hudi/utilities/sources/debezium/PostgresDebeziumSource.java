...
public class PostgresDebeziumSource extends DebeziumSource {
    ...
    /**
      ...
      * https://debezium.io/documentation/reference/1.4/connectors/postgresql.html#postgresql-create-events).
      * This function flattens this nested structure for the Postgres data, and also extracts a subset
      * of Debezium metadata fields.
      *
    ...
    protected Dataset<Row> processDataset(Dataset<Row> rowDataset) {
        if (rowDataset.columns().length > 0) {
            // Pick selective debezium and postgres meta fields: pick the row values from before
            // field for delete record
            ...
            Dataset<Row> insertedOrUpdatedData =
                    rowDataset
                            .selectExpr(
                                    ...
                                    String.format(
                                            "%s as %s",
                                            DebeziumConstants.INCOMING_SOURCE_SCHEMA_FIELD,
                                            DebeziumConstants.FLATTENED_SCHEMA_NAME),
                                    String.format(
                                            "%s as %s",
                                    ...
                                    String.format(
                                            "%s as %s",
                                            DebeziumConstants.INCOMING_SOURCE_TXID_FIELD,
                                            DebeziumConstants.FLATTENED_TX_ID_COL_NAME),
                                    String.format(
                                            "%s as %s",
                                            DebeziumConstants.INCOMING_SOURCE_LSN_FIELD,
                                            DebeziumConstants.FLATTENED_LSN_COL_NAME),
                                    String.format(
                                            "%s as %s",
                                            DebeziumConstants.INCOMING_SOURCE_XMIN_FIELD,
                                            DebeziumConstants.FLATTENED_XMIN_COL_NAME),
