...
/**
  * A validator with spark-submit to ensure there are no dangling data files in the data table. No
  * data files found for commits prior to active timeline. No extra data files found for completed
  * commits more than whats present in commit metadata.
  *
  * <p>- Default : This validator will validate the data files only once.
  *
  * <p>Example command: ``` spark-submit \ --class org.apache.hudi.utilities.HoodieDataTableValidator
  * \ --master spark://xxxx:7077 \ --driver-memory 1g \ --executor-memory 1g \
  * $HUDI_DIR/hudi/packaging/hudi-utilities-bundle/target/hudi-utilities-bundle_2.11-0.11.0-SNAPSHOT.jar
  * \ --base-path basePath ```
  *
  * <p>Also You can set `--continuous` for long running this validator. And use
  * `--min-validate-interval-seconds` to control the validation frequency, default is 10 minutes.
  *
  * <p>Example command: ``` spark-submit \ --class org.apache.hudi.utilities.HoodieDataTableValidator
  * \ --master spark://xxxx:7077 \ --driver-memory 1g \ --executor-memory 1g \
  * $HUDI_DIR/hudi/packaging/hudi-utilities-bundle/target/hudi-utilities-bundle_2.11-0.11.0-SNAPSHOT.jar
  * \ --base-path basePath --continuous \ --min-validate-interval-seconds 60 ```
  */
public class HoodieDataTableValidator implements Serializable {
    ...
    public HoodieDataTableValidator(JavaSparkContext jsc, Config cfg) {
        ...
        this.asyncDataTableValidateService =
                cfg.continuous ? Option.of(new AsyncDataTableValidateService()) : Option.empty();
    }
    ...
    public static class Config implements Serializable {
        ...
        @Parameter(
                names = {"--min-validate-interval-seconds"},
                description =
                        "the min validate interval of each validate when set --continuous, default is 10 minutes.")
        public Integer minValidateIntervalSeconds = 10 * 60;

        @Parameter(
                names = {"--parallelism", "-pl"},
                description = "Parallelism for validation",
                required = false)
        public int parallelism = 200;

        @Parameter(
                names = {"--ignore-failed", "-ig"},
                description = "Ignore data table validate failure and continue.",
                required = false)
        ...
        @Parameter(
                ...
                description =
                        "Should HoodieWriteClient assume the data is partitioned by dates, i.e three levels from base path."
        ...
        @Parameter(
                names = {"--props"},
                description =
                        "path to properties file on localfs or dfs, with configurations for " + "hoodie client")
    ...
    public static void main(String[] args) {
        ...
        SparkConf sparkConf =
                UtilHelpers.buildSparkConf("Hoodie-Data-Table-Validator", cfg.sparkMaster);
    ...
    private void doHoodieDataTableValidationContinuous() {
        asyncDataTableValidateService.ifPresent(
                service -> {
                    service.start(null);
                    try {
    ...
    public void doDataTableValidation() {
        ...
        try {
            ...
            // verify that no data files present with commit time < earliest commit in active
            // timeline.
            if (metaClient.getActiveTimeline().firstInstant().isPresent()) {
                ...
                List<StoragePath> danglingFilePaths =
                        allDataFilePaths.stream()
                                .filter(
                                        path -> {
                                            String instantTime = FSUtils.getCommitTime(path.getName());
                                            return HoodieTimeline.compareTimestamps(
                                                    instantTime, HoodieTimeline.LESSER_THAN, earliestInstant);
                                        })
                                .collect(Collectors.toList());

                if (!danglingFilePaths.isEmpty() && danglingFilePaths.size() > 0) {
                    LOG.error(
                            "Data table validation failed due to dangling files count "
                                    + danglingFilePaths.size()
                                    + ", found before active timeline");
                ...
                // Verify that for every completed commit in active timeline, there are no extra
                // files found apart from what is present in
                // commit metadata.
                ...
                List<String> danglingFiles =
                        engineContext
                                .flatMap(
                                        hoodieInstants,
                                        instant -> {
                                            Option<Set<String>> filesFromTimeline =
                                                    RepairUtils.getBaseAndLogFilePathsFromTimeline(activeTimeline, instant);
                                            List<String> baseAndLogFilesFromFs =
                                                    instantToFilesMap.containsKey(instant.getTimestamp())
                                                            ? instantToFilesMap.get(instant.getTimestamp())
                ...
                if (!danglingFiles.isEmpty()) {
                    LOG.error(
                            "Data table validation failed due to extra files found for completed commits "
                                    + danglingFiles.size());
                    danglingFiles.forEach(entry -> LOG.error("Dangling file: " + entry.toString()));
                    finalResult = false;
                    if (!cfg.ignoreFailed) {
                        throw new HoodieValidationException(
                                "Data table validation failed due to dangling files " + danglingFiles.size());
                    }
                }
            }
        } catch (Exception e) {
            ...
            if (!cfg.ignoreFailed) {
                throw new HoodieValidationException(
                        "Data table validation failed due to " + e.getMessage(), e);
            }
        }
                            ...
                            () -> {
                                while (true) {
                                    try {
                                        ...
                                        doDataTableValidation();
                                        long toSleepMs =
                                                cfg.minValidateIntervalSeconds * 1000
                                                        - (System.currentTimeMillis() - start);

                                        if (toSleepMs > 0) {
                                            LOG.info(
                                                    "Last validate ran less than min validate interval: "
                                                            + cfg.minValidateIntervalSeconds
                                                            + " s, sleep: "
                                                            + toSleepMs
                                                            + " ms.");
                                        ...
                                        LOG.error(
                                                "Shutting down AsyncDataTableValidateService due to HoodieValidationException",
                                                e);
                                        if (!cfg.ignoreFailed) {
                                            throw e;
                                        }
