...
/**
  * A tool with spark-submit to repair Hudi table by finding and deleting dangling base and log
  * files.
  *
  * <p>You can run this tool with the following command: ``` spark-submit \ --class
  * org.apache.hudi.utilities.HoodieRepairTool \ --driver-memory 4g \ --executor-memory 1g \ --conf
  ...
  * --mode dry_run \ --base-path base_path \ --assume-date-partitioning ```
  *
  * <p>You can specify the running mode of the tool through `--mode`. There are three modes of the
  * {@link HoodieRepairTool}: - REPAIR ("repair"): repairs the table by removing dangling data and
  * log files not belonging to any commit. The removed files are going to be backed up at the backup
  * path provided, in case recovery is needed. In this mode, backup path is required through
  * `--backup-path`. You can also provide a range for repairing only the instants within the range,
  * through `--start-instant-time` and `--end-instant-time`. You can also specify only one of them.
  * If no range is provided, all instants are going to be repaired.
  *
  * <p>Example command: ``` spark-submit \ --class org.apache.hudi.utilities.HoodieRepairTool \
  * --driver-memory 4g \ --executor-memory 1g \ --conf
  * spark.serializer=org.apache.spark.serializer.KryoSerializer \ --conf
  * spark.sql.catalogImplementation=hive \ --conf
  * spark.sql.extensions=org.apache.spark.sql.hudi.HoodieSparkSessionExtension \
  * $HUDI_DIR/packaging/hudi-utilities-bundle/target/hudi-utilities-bundle_2.12-0.11.0-SNAPSHOT.jar \
  * --mode repair \ --base-path base_path \ --backup-path backup_path \ --start-instant-time ts1 \
  * --end-instant-time ts2 \ --assume-date-partitioning ```
  *
  * <p>- DRY_RUN ("dry_run"): only looks for dangling data and log files. You can also provide a
  * range for looking at only the instants within the range, through `--start-instant-time` and
  * `--end-instant-time`. You can also specify only one of them. If no range is provided, all
  * instants are going to be scanned.
  *
  * <p>Example command: ``` spark-submit \ --class org.apache.hudi.utilities.HoodieRepairTool \
  * --driver-memory 4g \ --executor-memory 1g \ --conf
  * spark.serializer=org.apache.spark.serializer.KryoSerializer \ --conf
  ...
  * spark.sql.extensions=org.apache.spark.sql.hudi.HoodieSparkSessionExtension \
  * $HUDI_DIR/packaging/hudi-utilities-bundle/target/hudi-utilities-bundle_2.12-0.11.0-SNAPSHOT.jar \
  * --mode dry_run \ --base-path base_path \ --start-instant-time ts1 \ --end-instant-time ts2 \
  * --assume-date-partitioning ```
  *
  * <p>- UNDO ("undo"): undoes the repair by copying back the files from backup directory to the
  * table base path. In this mode, backup path is required through `--backup-path`.
  *
  * <p>Example command: ``` spark-submit \ --class org.apache.hudi.utilities.HoodieRepairTool \
  * --driver-memory 4g \ --executor-memory 1g \ --conf
  * spark.serializer=org.apache.spark.serializer.KryoSerializer \ --conf
  ...
  * spark.sql.extensions=org.apache.spark.sql.hudi.HoodieSparkSessionExtension \
  * $HUDI_DIR/packaging/hudi-utilities-bundle/target/hudi-utilities-bundle_2.12-0.11.0-SNAPSHOT.jar \
  * --mode undo \ --base-path base_path \ --backup-path backup_path ```
  */
public class HoodieRepairTool {
    ...
    public boolean run() {
        ...
        if (startingInstantOption.isPresent() && endingInstantOption.isPresent()) {
            LOG.info(
                    String.format(
                            "Start repairing completed instants between %s and %s (inclusive)",
                            startingInstantOption.get(), endingInstantOption.get()));
        } else if (startingInstantOption.isPresent()) {
            LOG.info(
                    String.format(
                            "Start repairing completed instants from %s (inclusive)",
                            startingInstantOption.get()));
        } else if (endingInstantOption.isPresent()) {
            LOG.info(
                    String.format(
                            "Start repairing completed instants till %s (inclusive)", endingInstantOption.get()));
        } else {
        ...
        try {
            ...
            switch (mode) {
                case REPAIR:
                    LOG.info(
                            " ****** The repair tool is in REPAIR mode, dangling data and logs files "
                ...
                case DRY_RUN:
                    LOG.info(
                            " ****** The repair tool is in DRY_RUN mode, "
    ...
    public static void main(String[] args) {
        ...
        final JavaSparkContext jsc =
                UtilHelpers.buildSparkContext("hudi-table-repair", cfg.sparkMaster, cfg.sparkMemory);
        try {
    ...
    /**
      * Copies the list of files from source base path to destination base path. The destination file
      * path (base + relative) should not already exist.
      *
    ...
    static boolean copyFiles(
        ...
        List<Boolean> allResults =
                context
                        .parallelize(relativeFilePaths)
                        .mapPartitions(
                                iterator -> {
                                    ...
                                    iterator.forEachRemaining(
                                            filePath -> {
                                                ...
                                                try {
                                                    ...
                                                    LOG.error(
                                                            String.format(
                                                                    "Copying file fails: source [%s], destination [%s]",
                                                                    sourcePath, destPath));
                                                } finally {
                                                    results.add(success);
                                                }
    ...
    static List<String> listFilesFromBasePath(
        ...
        return HadoopFSUtils.getFileStatusAtLevel(context, fs, basePath, expectedLevel, parallelism)
                .stream()
                .filter(
                        fileStatus -> {
    ...
    /**
      ...
      */
    boolean doRepair(
            Option<String> startingInstantOption, Option<String> endingInstantOption, boolean isDryRun)
            throws IOException {
        ...
        Map<String, List<String>> instantToFilesMap =
                RepairUtils.tagInstantsOfBaseAndLogFiles(
                        metaClient.getBasePath().toString(), allFilesInPartitions);
        List<String> instantTimesToRepair =
                instantToFilesMap.keySet().stream()
                        .filter(
                                instant ->
                                        (!startingInstantOption.isPresent()
        ...
        List<ImmutablePair<String, List<String>>> instantFilesToRemove =
                context
                        .parallelize(instantTimesToRepair)
                        .map(
                                instantToRepair ->
                                        new ImmutablePair<>(
                                                instantToRepair,
    ...
    boolean undoRepair() throws IOException {
        ...
        List<String> relativeFilePaths =
                listFilesFromBasePath(context, backupPathStr, partitionLevels, cfg.parallelism).stream()
                        .map(
                                filePath ->
                                        HadoopFSUtils.getRelativePartitionPath(
                                                new Path(backupPathStr), new Path(filePath)))
                        .collect(Collectors.toList());
    ...
    /**
      * Verifies the backup path for repair. If there is no backup path configured, creates a new one
      * in temp folder. If the backup path already has files, throws an error to the user. If the
      * backup path is within the table base path, throws an error too.
      *
    ...
    /**
      * Verifies the backup path against table base path. If the backup path is within the table base
    ...
    int checkBackupPathAgainstBasePath() {
        ...
        if (cfg.backupPath.contains(cfg.basePath)) {
            LOG.error(
                    String.format(
                            "Cannot use backup path %s: it resides in the base path %s",
    ...
    private void printRepairInfo(
        ...
        LOG.warn(
                "Number of instants verified based on the base and log files: "
    ...
    public static class Config implements Serializable {
        ...
        @Parameter(
                names = {"--mode", "-m"},
                description =
                        "Set job mode: Set \"repair\" means repairing the table "
                                ...
                                + "Set \"undo\" means undoing the repair by copying back the files from backup directory",
                required = true)
        public String runningMode = null;

        @Parameter(
                names = {"--start-instant-time", "-si"},
                description = "Starting Instant time " + "for repair (inclusive)",
                required = false)
        public String startingInstantTime = null;

        @Parameter(
                names = {"--end-instant-time", "-ei"},
                description = "Ending Instant time " + "for repair (inclusive)",
                required = false)
        public String endingInstantTime = null;

        @Parameter(
                names = {"--backup-path", "-bp"},
                description = "Backup path for storing dangling data " + "and log files from the table",
                required = false)
        public String backupPath = null;

        @Parameter(
                names = {"--parallelism", "-pl"},
                description = "Parallelism for repair",
                required = false)
        ...
        @Parameter(
                names = {"--assume-date-partitioning", "-dp"},
                description = "whether the partition path " + "is date with three levels",
                required = false)
