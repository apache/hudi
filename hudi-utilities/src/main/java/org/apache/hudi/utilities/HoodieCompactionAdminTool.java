...
import org.apache.hudi.client.SparkCompactionAdminClient;
...
public class HoodieCompactionAdminTool {
  ...
  public void run(JavaSparkContext jsc) throws Exception {
    ...
    try (SparkCompactionAdminClient admin = new SparkCompactionAdminClient(new HoodieSparkEngineContext(jsc), cfg.basePath)) {
      ...
      switch (cfg.operation) {
        ...
        case UNSCHEDULE_PLAN:
          ...
          break;
        case REPAIR:
          List<BaseCompactionAdminClient.RenameOpResult> r3 = admin.repairCompaction(cfg.compactionInstantTime, cfg.parallelism, cfg.dryRun);
          if (cfg.printOutput) {
