...
public class HoodieCompactionAdminTool {
    ...
    public void run(JavaSparkContext jsc) throws Exception {
        HoodieTableMetaClient metaClient =
                HoodieTableMetaClient.builder()
                        ...
                        .build();
        try (CompactionAdminClient admin =
                new CompactionAdminClient(new HoodieSparkEngineContext(jsc), cfg.basePath)) {
            ...
            switch (cfg.operation) {
                ...
                case UNSCHEDULE_PLAN:
                    List<RenameOpResult> r2 =
                            admin.unscheduleCompactionPlan(
                                    cfg.compactionInstantTime, cfg.skipValidation, cfg.parallelism, cfg.dryRun);
                    if (cfg.printOutput) {
    ...
    public static class Config implements Serializable {
        ...
        @Parameter(
                names = {"--instant-time", "-in"},
                description = "Compaction Instant time",
                required = false)
        public String compactionInstantTime = null;

        @Parameter(
                names = {"--partition-path", "-pp"},
        ...
        @Parameter(
                names = {"--skip-validation", "-sv"},
                description = "Skip Validation",
                required = false)
        public boolean skipValidation = false;

        @Parameter(
                names = {"--output-path", "-ot"},
                description = "Output Path",
                required = false)
        public String outputPath = null;

        @Parameter(
                names = {"--print-output", "-pt"},
