...
@ConfigClassProperty(name = "Cloud Source Configs",
    ...
    description = "Configurations controlling the behavior of cloud source in Deltastreamer.")
public class CloudSourceConfig extends HoodieConfig {
  public static final ConfigProperty<Integer> BATCH_SIZE = ConfigProperty
      ...
      .defaultValue(10)
      .withDocumentation("Number of metadata messages to pull at a time. A reasonable setting "
          + "should be provided to use for default batch size when fetching File Metadata as "
          + "part of Cloud Ingestion. If batch size is too big, two possible issues can happen: "
          + "i) Acknowledgement takes too long (given that Hudi needs to commit first). "
          + "ii) In the case of Google Cloud Pubsub: a) it will keep delivering the same message "
          + "since it wasn't acked in time. b) The size of the request that acks outstanding "
          + "messages may exceed the limit, which is 512KB as per Google's docs. See: "
  ...
  public static final ConfigProperty<String> SPARK_DATASOURCE_OPTIONS = ConfigProperty
      ...
      .withDocumentation("A JSON string passed to the Spark DataFrameReader while loading the dataset. "
          ...
          + "Delta streamer conf ex. --hoodie-conf hoodie.deltastreamer.source.s3incr.spark.datasource.options={\"header\":\"true\",\"encoding\":\"UTF-8\"}");
