...
/**
  ...
  *
  * <p>Internally, we use Spark to read CSV files thus any limitation of Spark CSV also applies here
  * (e.g., limited support for nested schema).
  *
  * <p>You can set the CSV-specific configs in the format of hoodie.streamer.csv.* that are Spark
  * compatible to deal with CSV files in Hudi. The supported options are:
  *
  * <p>"sep", "encoding", "quote", "escape", "charToEscapeQuoteEscaping", "comment", "header",
  * "enforceSchema", "inferSchema", "samplingRatio", "ignoreLeadingWhiteSpace",
  * "ignoreTrailingWhiteSpace", "nullValue", "emptyValue", "nanValue", "positiveInf", "negativeInf",
  * "dateFormat", "timestampFormat", "maxColumns", "maxCharsPerColumn", "mode",
  * "columnNameOfCorruptRecord", "multiLine"
  *
  * <p>Detailed information of these CSV options can be found at:
  * https://spark.apache.org/docs/latest/api/java/org/apache/spark/sql/DataFrameReader.html#csv-scala.collection.Seq-
  *
  * <p>If the source Avro schema is provided through the {@link
  * org.apache.hudi.utilities.schema.FilebasedSchemaProvider} using
  * "hoodie.streamer.schemaprovider.source.schema.file" config, the schema is passed to the CSV
  * reader without inferring the schema from the CSV file.
  */
public class CsvDFSSource extends RowSource {
    ...
    public CsvDFSSource(
        ...
        this.pathSelector =
                DFSPathSelector.createSourceSelector(props, sparkContext.hadoopConfiguration());
        if (schemaProvider != null) {
            sourceSchema =
                    (StructType) SchemaConverters.toSqlType(schemaProvider.getSourceSchema()).dataType();
        } else {
            sourceSchema = null;
        }
    }
    ...
    protected Pair<Option<Dataset<Row>>, String> fetchNextBatch(
        ...
        return Pair.of(
                fromFiles(selPathsWithMaxModificationTime.getLeft()),
