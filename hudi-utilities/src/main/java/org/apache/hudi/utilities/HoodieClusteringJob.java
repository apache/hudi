...
public class HoodieClusteringJob {
    ...
    public HoodieClusteringJob(JavaSparkContext jsc, Config cfg) {
        this(
                jsc,
                cfg,
                UtilHelpers.buildProperties(jsc.hadoopConfiguration(), cfg.propsFilePath, cfg.configs),
                UtilHelpers.createMetaClient(jsc, cfg.basePath, true));
    }

    public HoodieClusteringJob(
            JavaSparkContext jsc, Config cfg, TypedProperties props, HoodieTableMetaClient metaClient) {
        this.cfg = cfg;
        this.jsc = jsc;
        this.props = props;
    ...
    public static class Config implements Serializable {
        ...
        @Parameter(
                names = {"--skip-clean", "-sc"},
                description = "do not trigger clean after compaction",
                required = false)
    ...
    private static void validateRunningMode(Config cfg) {
        ...
        // If we remove --schedule option in the future we need to change runningMode default value
        // to EXECUTE
            ...
            if (cfg.retryLastFailedClusteringJob) {
                ...
                if (lastClusterOpt.isPresent()) {
                    ...
                    if (clusteringStartTime.getTime() + cfg.maxProcessingTimeMs
                        ...
                        // if there has failed clustering, then we will use the failed clustering
                        // instant-time to trigger next clustering action which will rollback and
                        // clustering.
                        LOG.info(
