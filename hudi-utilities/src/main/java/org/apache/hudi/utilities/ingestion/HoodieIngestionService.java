...
public abstract class HoodieIngestionService extends HoodieAsyncService {
    ...
    /**
      ...
      *
      * <p>Depends on the ingestion mode, this method will
    ...
    /**
      ...
      *
      * <p>In continuous mode, this will be executed in a loop with sleeps in between.
      */
    public abstract void ingestOnce();

    /**
      * To determine if shutdown should be requested to allow gracefully terminate the ingestion in
      * continuous mode.
      *
      * <p>Subclasses should implement the logic to make the decision. If the shutdown condition is
      * met, the implementation should call {@link #shutdown(boolean)} to indicate the request.
      *
    ...
    protected void sleepBeforeNextIngestion(long ingestionStartEpochMillis) {
        try {
            ...
            if (sleepMs > 0) {
                LOG.info(
                        String.format(
                                "Last ingestion took less than min sync interval: %d s; sleep for %.2f s",
                                minSyncInternalSeconds, sleepMs / 1000.0));
                Thread.sleep(sleepMs);
            }
        } catch (InterruptedException e) {
            throw new HoodieIngestionException(
                    "Ingestion service (continuous mode) was interrupted during sleep.", e);
        }
    }
    ...
    /**
      ...
      *
      * <p>For continuous mode, this is invoked once after exiting the ingestion loop.
      */
    ...
    public static class HoodieIngestionConfig extends HoodieConfig {
        ...
        public static final ConfigProperty<Boolean> INGESTION_IS_CONTINUOUS =
                ConfigProperty.key("hoodie.utilities.ingestion.is.continuous")
                        .defaultValue(false)
                        .markAdvanced()
                        .withDocumentation("Indicate if the ingestion runs in a continuous loop.");

        public static final ConfigProperty<Integer> INGESTION_MIN_SYNC_INTERNAL_SECONDS =
                ConfigProperty.key("hoodie.utilities.ingestion.min.sync.internal.seconds")
                        .defaultValue(0)
                        .markAdvanced()
        ...
        public static class Builder {
            ...
            public Builder withMinSyncInternalSeconds(int minSyncInternalSeconds) {
                this.ingestionConfig.setValue(
                        INGESTION_MIN_SYNC_INTERNAL_SECONDS, String.valueOf(minSyncInternalSeconds));
                return this;
            }
