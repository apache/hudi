...
/**
  * Generic helper methods to fetch from Cloud Storage during incremental fetch from cloud storage
  * buckets. NOTE: DO NOT use any implementation specific classes here. This class is supposed to
  * across S3EventsSource, GcsEventsSource etc...so you can't assume the classes for your specific
  * implementation will be available here.
  */
public class CloudObjectsSelectorCommon {
    ...
    /**
      * Return a function that extracts filepaths from a list of Rows. Here Row is assumed to have the
      * schema [bucket_name, filepath_relative_to_bucket, object_size]
      *
      * @param storageUrlSchemePrefix Eg: s3:// or gs://. The storage-provider-specific prefix to use
      *     within the URL.
    ...
    public static MapPartitionsFunction<Row, CloudObjectMetadata> getCloudObjectMetadataPerPartition(
        ...
        return rows -> {
            ...
            rows.forEachRemaining(
                    row -> {
                        Option<String> filePathUrl =
                                getUrlForFile(row, storageUrlSchemePrefix, storageConf, checkIfExists);
                        filePathUrl.ifPresent(
                                url -> {
                                    ...
                                    if (obj instanceof String) {
                                        ...
                                        throw new HoodieIOException(
                                                "unexpected object size's type in Cloud storage events: " + obj.getClass());
                                    }
    ...
    /**
      * Construct a full qualified URL string to a cloud file from a given Row. Optionally check if the
      * file exists. Here Row is assumed to have the schema [bucket_name, filepath_relative_to_bucket].
      * The checkIfExists logic assumes that the relevant impl classes for the storageUrlSchemePrefix
      * are already present on the classpath!
      *
      * @param storageUrlSchemePrefix Eg: s3:// or gs://. The storage-provider-specific prefix to use
      *     within the URL.
      */
    private static Option<String> getUrlForFile(
            Row row,
            String storageUrlSchemePrefix,
            StorageConfiguration<Configuration> storageConf,
            boolean checkIfExists) {
        ...
        try {
            ...
            boolean exists =
                    checkIfFileExists(storageUrlSchemePrefix, bucket, filePathUrl, configuration);
            ...
            throw new HoodieException(
                    String.format("Failed to generate path to cloud file %s", filePath), exception);
        }
    }
    ...
    public static String generateFilter(Type type, TypedProperties props) {
        ...
        if (type.equals(Type.S3)) {
            ...
            selectRelativePathPrefix =
                    selectRelativePathPrefix.or(() -> getPropVal(props, S3_KEY_PREFIX));
            ignoreRelativePathPrefix =
                    ignoreRelativePathPrefix.or(() -> getPropVal(props, S3_IGNORE_KEY_PREFIX));
            ignoreRelativePathSubStr =
                    ignoreRelativePathSubStr.or(() -> getPropVal(props, S3_IGNORE_KEY_SUBSTRING));
        } else {
        ...
        if (selectRelativePathPrefix.isPresent()) {
            filter
                    .append(SPACE_DELIMTER)
                    .append(String.format("and %s like '%s%%'", objectKey, selectRelativePathPrefix.get()));
        }
        if (ignoreRelativePathPrefix.isPresent()) {
            filter
                    .append(SPACE_DELIMTER)
                    .append(
                            String.format("and %s not like '%s%%'", objectKey, ignoreRelativePathPrefix.get()));
        }
        if (ignoreRelativePathSubStr.isPresent()) {
            filter
                    .append(SPACE_DELIMTER)
                    .append(
                            String.format("and %s not like '%%%s%%'", objectKey, ignoreRelativePathSubStr.get()));
        }

        // Match files with a given extension, or use the fileFormat as the default.
        getPropVal(props, CLOUD_DATAFILE_EXTENSION)
                .or(() -> Option.of(fileFormat))
                .map(
                        val ->
                                filter
                                        .append(SPACE_DELIMTER)
                                        .append(String.format("and %s like '%%%s'", objectKey, val)));

        return filter.toString();
    }

    /**
      * @param cloudObjectMetadataDF a Dataset that contains metadata of S3/GCS objects. Assumed to be
      *     a persisted form of a Cloud Storage SQS/PubSub Notification event.
    ...
    public static List<CloudObjectMetadata> getObjectMetadata(
        ...
        if (type == Type.GCS) {
            return cloudObjectMetadataDF
                    ...
                    .distinct()
                    .mapPartitions(
                            getCloudObjectMetadataPerPartition(GCS_PREFIX, storageConf, checkIfExists),
            ...
            return cloudObjectMetadataDF
                    .select(
                            CloudObjectsSelectorCommon.S3_BUCKET_NAME,
                            CloudObjectsSelectorCommon.S3_OBJECT_KEY,
                            CloudObjectsSelectorCommon.S3_OBJECT_SIZE)
                    .distinct()
                    .mapPartitions(
                            getCloudObjectMetadataPerPartition(s3Prefix, storageConf, checkIfExists),
                            Encoders.kryo(CloudObjectMetadata.class))
                    .collectAsList();
        }
    ...
    public Option<Dataset<Row>> loadAsDataset(
            SparkSession spark,
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug(
                    "Extracted distinct files "
                            + cloudObjectMetadata.size()
                            + " and some samples "
                            + cloudObjectMetadata.stream()
        ...
        String datasourceOpts =
                getStringWithAltKeys(properties, CloudSourceConfig.SPARK_DATASOURCE_OPTIONS, true);
        ...
        if (StringUtils.isNullOrEmpty(datasourceOpts)) {
            ...
            datasourceOpts =
                    getStringWithAltKeys(
                            properties, S3EventsHoodieIncrSourceConfig.SPARK_DATASOURCE_OPTIONS, true);
        }
        if (StringUtils.nonEmpty(datasourceOpts)) {
            ...
            try {
                ...
                throw new HoodieException(
                        String.format("Failed to parse sparkOptions: %s", datasourceOpts), e);
            }
        ...
        boolean isCommaSeparatedPathFormat =
                properties.getBoolean(SPARK_DATASOURCE_READER_COMMA_SEPARATED_PATH_FORMAT.key(), false);
        ...
        if (containsConfigProperty(properties, PATH_BASED_PARTITION_FIELDS)) {
            String[] partitionKeysToAdd =
                    getStringWithAltKeys(properties, PATH_BASED_PARTITION_FIELDS).split(",");
            // Add partition column for all path-based partition keys. If key is not present in
            // path, the value will be null.
            for (String partitionKey : partitionKeysToAdd) {
                ...
                dataset =
                        dataset.withColumn(
                                partitionKey,
                                split(split(input_file_name(), partitionPathPattern).getItem(1), "/").getItem(0));
            }
        }
    ...
    private static Dataset<Row> coalesceOrRepartition(Dataset dataset, int numPartitions) {
        ...
        LOG.info(
                String.format(
                        "existing number of partitions=%d, required number of partitions=%d",
                        existingNumPartitions, numPartitions));
    ...
    private static Option<String> getPropVal(
            TypedProperties props, ConfigProperty<String> configProperty) {
