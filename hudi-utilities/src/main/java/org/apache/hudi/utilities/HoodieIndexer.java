...
/**
  ...
  *
  * <p>Example command (assuming indexer.properties contains related index configs, see {@link
  * org.apache.hudi.common.config.HoodieMetadataConfig} for configs):
  *
  * <p>spark-submit \ --class org.apache.hudi.utilities.HoodieIndexer \
  * /path/to/hudi/packaging/hudi-utilities-bundle/target/hudi-utilities-bundle_2.11-0.11.0-SNAPSHOT.jar
  * \ --props /path/to/indexer.properties \ --mode scheduleAndExecute \ --base-path
  * /tmp/hudi_trips_cow \ --table-name hudi_trips_cow \ --index-types COLUMN_STATS \ --parallelism 1
  * \ --spark-memory 1g
  *
  * <p>A sample indexer.properties file:
  *
  * <p>hoodie.metadata.index.async=true hoodie.metadata.index.column.stats.enable=true
...
public class HoodieIndexer {
    ...
    public HoodieIndexer(JavaSparkContext jsc, HoodieIndexer.Config cfg) {
        ...
        this.jsc = jsc;
        this.props =
                isNullOrEmpty(cfg.propsFilePath)
    ...
    public static class Config implements Serializable {
        ...
        @Parameter(
                ...
                description = "Indexing Instant time")
        ...
        @Parameter(
                names = {"--index-types", "-ixt"},
                description = "Comma-separated index types to be built, e.g. BLOOM_FILTERS,COLUMN_STATS",
                required = true)
        ...
        @Parameter(
                names = {"--mode", "-m"},
                description =
                        "Set job mode: Set \"schedule\" to generate an indexing plan; "
        ...
        @Parameter(
                names = {"--props"},
                description =
                        "path to properties file on localfs or dfs, with configurations for hoodie client for indexing")
    ...
    public static void main(String[] args) {
        ...
        final JavaSparkContext jsc =
                UtilHelpers.buildSparkContext(
                        "indexing-" + cfg.tableName, cfg.sparkMaster, cfg.sparkMemory);
        HoodieIndexer indexer = new HoodieIndexer(jsc, cfg);
        int result = indexer.start(cfg.retry);
        String resultMsg =
                String.format(
                        "Indexing with basePath: %s, tableName: %s, runningMode: %s",
    ...
    public int start(int retry) {
        ...
        if (!props.getBoolean(HoodieMetadataConfig.ENABLE.key())) {
            LOG.error(
                    String.format(
                            "Metadata is not enabled. Please set %s to true.",
        ...
        initializedMetadataPartitions.forEach(
                p -> {
    ...
    private Option<String> doSchedule(SparkRDDWriteClient<HoodieRecordPayload> client) {
        ...
        List<MetadataPartitionType> partitionTypes =
                getRequestedPartitionTypes(cfg.indexTypes, Option.of(metadataConfig));
        checkArgument(
                partitionTypes.size() == 1, "Currently, only one index type can be scheduled at a time.");
        if (!isMetadataInitialized() && !partitionTypes.contains(MetadataPartitionType.FILES)) {
            throw new HoodieException(
                    "Metadata table is not yet initialized. Initialize FILES partition before any other partition "
        ...
        Option<String> indexingInstant =
                client.scheduleIndexing(partitionTypes, Collections.emptyList());
        ...
        try (SparkRDDWriteClient<HoodieRecordPayload> client =
            ...
            if (isNullOrEmpty(cfg.indexInstantTime)) {
                ...
                Option<HoodieInstant> earliestPendingIndexInstant =
                        metaClient.getActiveTimeline().filterPendingIndexTimeline().firstInstant();
                if (earliestPendingIndexInstant.isPresent()) {
                    cfg.indexInstantTime = earliestPendingIndexInstant.get().getTimestamp();
                    LOG.info(
                            "Found the earliest scheduled indexing instant which will be executed: "
    ...
    private int dropIndex(JavaSparkContext jsc) throws Exception {
        List<MetadataPartitionType> partitionTypes =
                getRequestedPartitionTypes(cfg.indexTypes, Option.empty());
        String schemaStr = UtilHelpers.getSchemaFromLatestInstant(metaClient);
        try (SparkRDDWriteClient<HoodieRecordPayload> client =
                UtilHelpers.createHoodieClient(
                        jsc, cfg.basePath, schemaStr, cfg.parallelism, Option.empty(), props)) {
    ...
    private boolean handleResponse(Option<HoodieIndexCommitMetadata> commitMetadata) {
        ...
        LOG.info(
                String.format(
                        "Indexing complete for partitions: %s",
    ...
    boolean isIndexBuiltForAllRequestedTypes(List<HoodieIndexPartitionInfo> indexPartitionInfos) {
        Set<String> indexedPartitions =
                indexPartitionInfos.stream()
                        .map(HoodieIndexPartitionInfo::getMetadataPartitionPath)
                        .collect(Collectors.toSet());
        Set<String> requestedPartitions =
                getRequestedPartitionTypes(cfg.indexTypes, Option.empty()).stream()
                        .map(MetadataPartitionType::getPartitionPath)
                        .collect(Collectors.toSet());
    ...
    List<MetadataPartitionType> getRequestedPartitionTypes(
            String indexTypes, Option<HoodieMetadataConfig> metadataConfig) {
