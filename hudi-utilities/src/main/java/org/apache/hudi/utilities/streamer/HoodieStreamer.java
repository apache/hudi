...
/**
  ...
  * it to the target table. Does not maintain any state, queries at runtime to see how far behind the
  * target table is from the source table. This can be overridden to force sync from a timestamp.
  *
  * <p>In continuous mode, DeltaStreamer runs in loop-mode going through the below operations (a)
  * pull-from-source (b) write-to-sink (c) Schedule Compactions if needed (d) Conditionally Sync to
  * Hive each cycle. For MOR table with continuous mode enabled, a separate compactor thread is
  * allocated to execute compactions
  */
public class HoodieStreamer implements Serializable {
    ...
    private static final List<String> DEFAULT_SENSITIVE_CONFIG_KEYS =
            Arrays.asList(HoodieWriteConfig.SENSITIVE_CONFIG_KEYS_FILTER.defaultValue().split(","));
    ...
    public HoodieStreamer(Config cfg, JavaSparkContext jssc) throws IOException {
        this(
                cfg,
                jssc,
                HadoopFSUtils.getFs(cfg.targetBasePath, jssc.hadoopConfiguration()),
                jssc.hadoopConfiguration(),
                Option.empty());
    }

    public HoodieStreamer(Config cfg, JavaSparkContext jssc, Option<TypedProperties> props)
            throws IOException {
        this(
                cfg,
                jssc,
    ...
    public HoodieStreamer(
        ...
        if (cfg.initialCheckpointProvider != null && cfg.checkpoint == null) {
            InitialCheckPointProvider checkPointProvider =
                    UtilHelpers.createInitialCheckpointProvider(
                            cfg.initialCheckpointProvider, this.properties);
    ...
    private static TypedProperties combineProperties(
            Config cfg, Option<TypedProperties> propsOverride, Configuration hadoopConf) {
        ...
        if (propsOverride.isPresent()) {
            ...
            hoodieConfig.setAll(
                    readConfig(hadoopConf, new Path(cfg.propsFilePath), cfg.configs).getProps());
        }
        ...
        // we need auto adjustment enabled for deltastreamer since async table services are feasible
        // within the same JVM.
    ...
    public static class Config implements Serializable {
        public static final String DEFAULT_DFS_SOURCE_PROPERTIES =
                "file://"
        ...
        @Parameter(
                ...
                description = "File format for the base files. PARQUET (or) HFILE",
                required = false)
        ...
        @Parameter(
                names = {"--schemaprovider-class"},
                description =
                        "subclass of org.apache.hudi.utilities.schema"
        ...
        @Parameter(
                ...
                description =
                        "A subclass or a list of subclasses of org.apache.hudi.utilities.transform.Transformer"
        ...
        @Parameter(
                names = {"--enable-hive-sync"},
                description = "Enable syncing to hive")
        public Boolean enableHiveSync = false;

        @Parameter(
                names = {"--enable-sync"},
                description = "Enable syncing meta")
        public Boolean enableMetaSync = false;

        @Parameter(
                names = {"--force-empty-sync"},
                description = "Force syncing meta even on empty commit")
        public Boolean forceEmptyMetaSync = false;

        @Parameter(
                names = {"--sync-tool-classes"},
                description = "Meta sync client tool, using comma to separate multi tools")
        public String syncClientToolClassNames = HiveSyncTool.class.getName();

        @Parameter(
                names = {"--max-pending-compactions"},
                description =
                        "Maximum number of outstanding inflight/requested compactions. Delta Sync will not happen unless"
                                + "outstanding compactions is less than this number")
        public Integer maxPendingCompactions = 5;

        @Parameter(
                names = {"--max-pending-clustering"},
                description =
                        "Maximum number of outstanding inflight/requested clustering. Delta Sync will not happen unless"
        ...
        @Parameter(
                names = {"--continuous"},
                description =
                        "Hudi Streamer runs in continuous mode running"
                                + " source-fetch -> Transform -> Hudi Write in loop")
        public Boolean continuousMode = false;

        @Parameter(
                names = {"--min-sync-interval-seconds"},
        ...
        @Parameter(
                names = {"--spark-master"},
                description =
                        "spark master to use, if not defined inherits from your environment taking into "
        ...
        @Parameter(
                names = {"--delta-sync-scheduling-weight"},
                description =
        ...
        @Parameter(
                names = {"--compact-scheduling-weight"},
                description =
        ...
        @Parameter(
                names = {"--delta-sync-scheduling-minshare"},
                description =
        ...
        @Parameter(
                names = {"--compact-scheduling-minshare"},
                description =
        ...
        @Parameter(
                names = {"--checkpoint"},
                description = "Resume Hudi Streamer from this checkpoint.")
        public String checkpoint = null;

        @Parameter(
                names = {"--initial-checkpoint-provider"},
                description =
                        "subclass of "
        ...
        @Parameter(
                names = {"--run-bootstrap"},
                description = "Run bootstrap if bootstrap index is not found")
        public Boolean runBootstrap = false;

        @Parameter(
                names = {"--bootstrap-overwrite"},
                description = "Overwrite existing target table, default false")
        public Boolean bootstrapOverwrite = false;

        @Parameter(
                names = {"--bootstrap-index-class"},
                description = "subclass of BootstrapIndex")
        public String bootstrapIndexClass = HFileBootstrapIndex.class.getName();

        @Parameter(
                names = {"--retry-on-source-failures"},
                description = "Retry on any source failures")
        public Boolean retryOnSourceFailures = false;

        @Parameter(
                names = {"--retry-interval-seconds"},
                description =
                        "the retry interval for source failures if --retry-on-source-failures is enabled")
        public Integer retryIntervalSecs = 30;

        @Parameter(
                names = {"--max-retry-count"},
                description = "the max retry count if --retry-on-source-failures is enabled")
        public Integer maxRetryCount = 3;

        @Parameter(
                names = {"--allow-commit-on-no-checkpoint-change"},
                description =
        ...
        @Parameter(
                names = {"--retry-last-pending-inline-clustering", "-rc"},
                description = "Retry last pending inline clustering plan before writing to sink.")
        public Boolean retryLastPendingInlineClusteringJob = false;

        @Parameter(
                names = {"--retry-last-pending-inline-compaction"},
                description = "Retry last pending inline compaction plan before writing to sink.")
        public Boolean retryLastPendingInlineCompactionJob = false;

        @Parameter(
                names = {"--cluster-scheduling-weight"},
                description =
        ...
        @Parameter(
                names = {"--cluster-scheduling-minshare"},
                description =
        ...
        @Parameter(
                names = {"--post-write-termination-strategy-class"},
                description =
                        "Post writer termination strategy class to gracefully shutdown deltastreamer in continuous mode")
        public String postWriteTerminationStrategyClass = "";

        @Parameter(
                names = {"--ingestion-metrics-class"},
                description = "Ingestion metrics class for reporting metrics during ingestion lifecycles.")
        public String ingestionMetricsClass = HoodieStreamerMetrics.class.getCanonicalName();

        @Parameter(
                names = {"--config-hot-update-strategy-class"},
                description = "Configuration hot update in continuous mode")
        public String configHotUpdateStrategyClass = "";

        @Parameter(
                names = {"--ignore-checkpoint"},
                description =
                        "Set this config with a unique value, recommend using a timestamp value or UUID."
        ...
        public int hashCode() {
            return Objects.hash(
                    ...
                    enableMetaSync,
                    forceEmptyMetaSync,
                    syncClientToolClassNames,
                    maxPendingCompactions,
                    maxPendingClustering,
                    continuousMode,
                    minSyncIntervalSeconds,
                    sparkMaster,
                    commitOnErrors,
                    deltaSyncSchedulingWeight,
                    compactSchedulingWeight,
                    clusterSchedulingWeight,
                    deltaSyncSchedulingMinShare,
                    compactSchedulingMinShare,
                    clusterSchedulingMinShare,
                    forceDisableCompaction,
                    checkpoint,
                    initialCheckpointProvider,
                    ingestionMetricsClass,
                    help);
        }
    ...
    public static String toSortedTruncatedString(TypedProperties props) {
        List<String> sensitiveConfigList =
                props.getStringList(
                        HoodieWriteConfig.SENSITIVE_CONFIG_KEYS_FILTER.key(),
                        ",",
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (StringUtils.isNullOrEmpty(cfg.sparkMaster)) {
            jssc =
                    UtilHelpers.buildSparkContext("streamer-" + cfg.targetTableName, additionalSparkConfigs);
        } else {
            jssc =
                    UtilHelpers.buildSparkContext(
                            "streamer-" + cfg.targetTableName, cfg.sparkMaster, additionalSparkConfigs);
        }
        if (cfg.enableHiveSync) {
            LOG.warn(
                    "--enable-hive-sync will be deprecated in a future release; please use --enable-sync instead for Hive syncing");
        }
    ...
    public static class StreamSyncService extends HoodieIngestionService {
        ...
        public StreamSyncService(
                ...
                throws IOException {
            super(
                    HoodieIngestionConfig.newBuilder()
                            .isContinuous(cfg.continuousMode)
                            .withMinSyncInternalSeconds(cfg.minSyncIntervalSeconds)
                            .build());
            this.cfg = cfg;
            ...
            this.postWriteTerminationStrategy =
                    StringUtils.isNullOrEmpty(cfg.postWriteTerminationStrategyClass)
                            ...
                            : TerminationStrategyUtils.createPostWriteTerminationStrategy(
                                    properties.get(), cfg.postWriteTerminationStrategyClass);
            this.configurationHotUpdateStrategyOpt =
                    StringUtils.isNullOrEmpty(cfg.configHotUpdateStrategyClass)
                            ? Option.empty()
                            : ConfigurationHotUpdateStrategyUtils.createConfigurationHotUpdateStrategy(
                                    cfg.configHotUpdateStrategyClass, cfg, properties.get());
            ...
            if (this.storage.exists(new StoragePath(cfg.targetBasePath))) {
                try {
                    ...
                    checkArgument(
                            tableType.equals(HoodieTableType.valueOf(cfg.tableType)),
                            "Hoodie table is of type "
                                    + tableType
            ...
            checkArgument(
                    !cfg.filterDupes || cfg.operation != WriteOperationType.UPSERT,
            ...
            this.schemaProvider =
                    UtilHelpers.wrapSchemaProviderWithPostProcessor(
                            UtilHelpers.createSchemaProvider(
                                    cfg.schemaProviderClassName, props, hoodieSparkContext.jsc()),
                            props,
                    ...
                    CompletableFuture.supplyAsync(
                            () -> {
                                ...
                                if (cfg.isAsyncCompactionEnabled()) {
                                    ...
                                    hoodieSparkContext.setProperty(
                                            EngineProperty.DELTASYNC_POOL_NAME, STREAMSYNC_POOL_NAME);
                                }
                                ...
                                try {
                                    while (!isShutdownRequested()) {
                                        try {
                                            ...
                                            // Send a heartbeat metrics event to track the active
                                            // ingestion job for this table.
                                            streamSync.getMetrics().updateStreamerHeartbeatTimestamp(start);
                                            // check if deltastreamer need to update the
                                            // configuration before the sync
                                            if (configurationHotUpdateStrategyOpt.isPresent()) {
                                                Option<TypedProperties> newProps =
                                                        configurationHotUpdateStrategyOpt.get().updateProperties(props);
                                                if (newProps.isPresent()) {
                                                    this.props = newProps.get();
                                                    // reinit the DeltaSync only when the props
                                                    // updated
                                            ...
                                            if (scheduledCompactionInstantAndRDD.isPresent()
                                                ...
                                                asyncCompactService
                                                        .get()
                                                        .waitTillPendingAsyncServiceInstantsReducesTo(
                                                                cfg.maxPendingCompactions);
                                                if (asyncCompactService.get().hasError()) {
                                                    error = true;
                                                    throw new HoodieException(
                                                            "Async compaction failed.  Shutting down Delta Sync...");
                                                }
                                            }
                                            if (clusteringConfig.isAsyncClusteringEnabled()) {
                                                ...
                                                if (clusteringInstant.isPresent()) {
                                                    LOG.info(
                                                            "Scheduled async clustering for instant: " + clusteringInstant.get());
                                                    ...
                                                    asyncClusteringService
                                                            .get()
                                                            .waitTillPendingAsyncServiceInstantsReducesTo(
                                                                    cfg.maxPendingClustering);
                                                    if (asyncClusteringService.get().hasError()) {
                                                        error = true;
                                                        throw new HoodieException(
                                                                "Async clustering failed.  Shutting down Delta Sync...");
                                                    }
                                                }
                                            }
                                            ...
                                            Option<HoodieData<WriteStatus>> lastWriteStatuses =
                                                    Option.ofNullable(
                                                            scheduledCompactionInstantAndRDD.isPresent()
                                                                    ? HoodieJavaRDD.of(
                                                                            scheduledCompactionInstantAndRDD.get().getRight())
                                                                    : null);
        ...
        private void handleUpsertException(HoodieUpsertException ue) {
            if (ue.getCause() instanceof HoodieClusteringUpdateException) {
                LOG.warn(
                        "Write rejected due to conflicts with pending clustering operation. Going to retry after 1 min with the hope "
                                + "that clustering will complete by then.",
                        ue);
                try {
                    Thread.sleep(60000); // Intentionally not using cfg.minSyncIntervalSeconds, since it
                    // could be too high or it could be 0.
                    // Once the Hudi Streamer gets past this clustering update exception, regular
                    // syncs will honor cfg.minSyncIntervalSeconds.
        ...
        public void ingestOnce() {
            try {
                ...
                throw new HoodieIngestionException(
                        String.format("Ingestion via %s failed with exception.", this.getClass()), e);
            } finally {
                close();
            }
        }
        ...
        protected boolean requestShutdownIfNeeded(Option<HoodieData<WriteStatus>> lastWriteStatuses) {
            Option<JavaRDD<WriteStatus>> lastWriteStatusRDD =
                    Option.ofNullable(
                            lastWriteStatuses.isPresent()
                                    ? HoodieJavaRDD.getJavaRDD(lastWriteStatuses.get())
                                    : null);
        ...
        protected Boolean onInitializingWriteClient(SparkRDDWriteClient writeClient) {
            if (cfg.isAsyncCompactionEnabled()) {
                if (asyncCompactService.isPresent()) {
                    ...
                    asyncCompactService =
                            Option.ofNullable(new SparkAsyncCompactService(hoodieSparkContext, writeClient));
                    ...
                    try {
                        asyncCompactService
                                .get()
                                .waitTillPendingAsyncServiceInstantsReducesTo(cfg.maxPendingCompactions);
                        if (asyncCompactService.get().hasError()) {
                            throw new HoodieException(
                                    "Async compaction failed during write client initialization.");
                        }
            ...
            if (HoodieClusteringConfig.from(props).isAsyncClusteringEnabled()) {
                if (asyncClusteringService.isPresent()) {
                    ...
                    try {
                        asyncClusteringService
                                .get()
                                .waitTillPendingAsyncServiceInstantsReducesTo(cfg.maxPendingClustering);
                        if (asyncClusteringService.get().hasError()) {
                            throw new HoodieException(
                                    "Async clustering failed during write client initialization.");
                        }
                    } catch (InterruptedException e) {
