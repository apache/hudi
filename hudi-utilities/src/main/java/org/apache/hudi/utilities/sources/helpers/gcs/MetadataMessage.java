...
/**
  * Wraps a PubsubMessage assuming it's from Cloud Storage Pubsub Notifications (CSPN), and adds
  * relevant helper methods. For details of CSPN messages see:
...
public class MetadataMessage {
    ...
    /**
      * Whether a message is valid to be ingested and stored by this Metadata puller. Ref:
    ...
    public MessageValidity shouldBeProcessed() {
        if (!isNewFileCreation()) {
            return new MessageValidity(
                    DO_SKIP, "eventType: " + getEventType() + ". Not a file creation message.");
        }
        ...
        if (isOverwriteOfExistingFile()) {
            return new MessageValidity(
                    ...
                    "eventType: "
                            + getEventType()
                            + ". Overwrite of existing objectId: "
                            + getObjectId()
                            + " with generation number: "
                            + getOverwroteGeneration());
        }
    ...
    /**
      * Whether message represents an overwrite of an existing file. Ref:
    ...
    /**
      * Returns true if message corresponds to new file creation, false if not. Ref:
      * https://cloud.google.com/storage/docs/pubsub-notifications#events
      */
