...
public class DatePartitionPathSelector extends DFSPathSelector {
    ...
    public static class Config {
        ...
        public static final int DEFAULT_DATE_PARTITION_DEPTH =
                DatePartitionPathSelectorConfig.DATE_PARTITION_DEPTH
                        .defaultValue(); // Implies no (date) partition
    ...
    public Pair<Option<String>, String> getNextFilePathsAndMaxModificationTime(
        ...
        LocalDate currentDate =
                LocalDate.parse(
                        getStringWithAltKeys(
                                props, DatePartitionPathSelectorConfig.CURRENT_DATE, LocalDate.now().toString()));
        ...
        List<String> prunedPartitionPaths =
                pruneDatePartitionPaths(
                        context, fs, getStringWithAltKeys(props, ROOT_INPUT_PATH), currentDate);

        List<FileStatus> eligibleFiles =
                context.flatMap(
        ...
        List<FileStatus> sortedEligibleFiles =
    ...
    public List<String> pruneDatePartitionPaths(
            HoodieSparkEngineContext context, FileSystem fs, String rootPath, LocalDate currentDate) {
        ...
        return context
                .getJavaSparkContext()
                .parallelize(partitionPaths, partitionsListParallelism)
                .filter(
                        s -> {
