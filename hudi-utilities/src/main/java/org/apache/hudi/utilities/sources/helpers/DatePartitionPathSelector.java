...
public class DatePartitionPathSelector extends DFSPathSelector {
  ...
  public Pair<Option<String>, String> getNextFilePathsAndMaxModificationTime(JavaSparkContext sparkContext,
    ...
    List<HoodieFileStatus> eligibleFiles = context.flatMap(prunedPartitionPaths,
        path -> {
          HoodieStorage storage = HoodieStorageUtils.getHoodieStorage(path, serializedConf.get());
          return listEligibleFiles(storage, new HoodieLocation(path), lastCheckpointTime).stream();
        }, partitionsListParallelism);
    // sort them by modification time ascending.
    List<HoodieFileStatus> sortedEligibleFiles = eligibleFiles.stream()
        .sorted(Comparator.comparingLong(HoodieFileStatus::getModificationTime))
        .collect(Collectors.toList());

    // Filter based on checkpoint & input size, if needed
    long currentBytes = 0;
    long newCheckpointTime = lastCheckpointTime;
    List<HoodieFileStatus> filteredFiles = new ArrayList<>();
    for (HoodieFileStatus f : sortedEligibleFiles) {
    ...
    // read the files out.
    String pathStr =
        filteredFiles.stream().map(f -> f.getLocation().toString())
