...
/**
  ...
  *
  * <p>File-based SQL Source is used for one time backfill scenarios, this won't update the
...
public class SqlFileBasedSource extends RowSource {
    ...
    protected Pair<Option<Dataset<Row>>, String> fetchNextBatch(
        ...
        final FileSystem fs =
                HadoopFSUtils.getFs(sourceSqlFile, sparkContext.hadoopConfiguration(), true);
        try {
            ...
            return Pair.of(
                    Option.of(rows),
                    shouldEmitCheckPoint ? String.valueOf(System.currentTimeMillis()) : null);
