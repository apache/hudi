...
/**
  * An implementation of {@link SchemaPostProcessor} which will add a column named
  * "_hoodie_is_deleted" to the end of a given schema.
  */
        ...
        for (Schema.Field sourceField : sourceFields) {
            targetFields.add(
                    new Schema.Field(
                            ...
                            sourceField.schema(),
                            sourceField.doc(),
                            sourceField.defaultVal()));
        }
        // add _hoodie_is_deleted column
        targetFields.add(
                new Schema.Field(
                        HoodieRecord.HOODIE_IS_DELETED_FIELD, Schema.create(Schema.Type.BOOLEAN), null, false));

        return Schema.createRecord(
                schema.getName(), schema.getDoc(), schema.getNamespace(), false, targetFields);
    }
}
