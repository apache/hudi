...
/**
  ...
  *
  * <p>You can dry run this tool with the following command to look and print for the table
  * partitions and corresponding data files which will be deleted. ``` spark-submit \ --class
  * org.apache.hudi.utilities.HoodieDropPartitionsTool \ --packages
  * org.apache.spark:spark-avro_2.11:2.4.4 \ --master local[*] --driver-memory 1g \ --executor-memory
  * 1g \
  * $HUDI_DIR/hudi/packaging/hudi-utilities-bundle/target/hudi-utilities-bundle_2.11-0.11.0-SNAPSHOT.jar
  * \ --base-path basePath \ --table-name tableName \ --mode dry_run \ --partitions
  * partition1,partition2 ```
  *
  * <p>You can delete the table partitions with '--mode delete'
  *
  * <p>- DELETE ("delete"): This tool will mask/tombstone these partitions and corresponding data
  * files and let cleaner delete these files later. - Also you can set --sync-hive-meta to sync
  * current drop partition into hive
  *
  * <p>Example command: ``` spark-submit \ --class org.apache.hudi.utilities.HoodieDropPartitionsTool
  * \ --packages org.apache.spark:spark-avro_2.11:2.4.4 \ --master local[*] --driver-memory 1g \
  * --executor-memory 1g \
  * $HUDI_DIR/hudi/packaging/hudi-utilities-bundle/target/hudi-utilities-bundle_2.11-0.11.0-SNAPSHOT.jar
  * \ --base-path basePath \ --table-name tableName \ --mode delete \ --partitions
  * partition1,partition2 ```
  *
  * <p>Also you can use --help to find more configs to use.
  */
public class HoodieDropPartitionsTool implements Serializable {
    ...
    public enum Mode {
        // Mask/Tombstone these partitions and corresponding data files and let cleaner delete these
        // files later.
        DELETE,
        // Dry run by looking for the table partitions and corresponding data files which will be
        // deleted.
        DRY_RUN
    }
    ...
    public static class Config implements Serializable {
        ...
        @Parameter(
                ...
                description =
                        "Set job mode: "
                                ...
                                + "Set \"dry_run\" means only looking for the table partitions will be deleted and corresponding data files.",
                required = true)
        ...
        @Parameter(
                names = {"--partitions", "-p"},
                description = "Comma separated list of partitions to delete.",
                required = true)
        public String partitions = null;

        @Parameter(
                names = {"--parallelism", "-pl"},
                description = "Parallelism for hoodie insert/upsert/delete",
                required = false)
        public int parallelism = 1500;

        @Parameter(
                names = {"--instant-time", "-it"},
                description = "instant time for delete table partitions operation.",
                required = false)
        public String instantTime = null;

        @Parameter(
                names = {"--sync-hive-meta", "-sync"},
                description = "Sync information to HMS.",
                required = false)
        public boolean syncToHive = false;

        @Parameter(
                names = {"--hive-database", "-db"},
                description = "Database to sync to.",
                required = false)
        public String hiveDataBase = null;

        @Parameter(
                names = {"--hive-table-name"},
                description = "Table to sync to.",
                required = false)
        public String hiveTableName = null;

        @Parameter(
                names = {"--hive-user-name", "-user"},
                description = "hive user name to use.",
                required = false)
        public String hiveUserName = "hive";

        @Parameter(
                names = {"--hive-pass-word", "-pass"},
                description = "hive password to use.",
                required = false)
        public String hivePassWord = "hive";

        @Parameter(
                names = {"--hive-jdbc-url", "-jdbc"},
                description = "hive url to use.",
                required = false)
        public String hiveURL = "jdbc:hive2://localhost:10000";

        @Parameter(
                names = {"--hive-partition-field"},
                description =
                        "Comma separated list of field in the hive table to use for determining hive partition columns.",
                required = false)
        public String hivePartitionsField = "";

        @Parameter(
                names = {"--hive-sync-use-jdbc"},
                description = "Use JDBC when hive synchronization.",
                required = false)
        public boolean hiveUseJdbc = true;

        @Parameter(
                names = {"--hive-metastore-uris"},
                description = "hive meta store uris to use.",
                required = false)
        public String hiveHMSUris = null;

        @Parameter(
                names = {"--hive-sync-mode"},
                description = "Mode to choose for Hive ops. Valid values are hms, jdbc and hiveql.",
                required = false)
        public String hiveSyncMode = "hms";

        @Parameter(
                names = {"--hive-sync-ignore-exception"},
                description = "Ignore hive sync exception.",
                required = false)
        public boolean hiveSyncIgnoreException = false;

        @Parameter(
                names = {"--hive-partition-value-extractor-class"},
                description =
                        "Class which implements PartitionValueExtractor to extract the partition values,"
        ...
        public String toString() {
            return "HoodieDropPartitionsToolConfig {\n"
                    ...
                    + hiveHMSUris
                    + ", \n"
                    + "   --hive-sync-ignore-exception "
                    + hiveSyncIgnoreException
                    + ", \n"
                    + "   --hive-partition-value-extractor-class "
            ...
            if (StringUtils.isNullOrEmpty(cfg.instantTime)) {
                TimeGenerator timeGenerator =
                        TimeGenerators.getTimeGenerator(
                                HoodieTimeGeneratorConfig.defaultConfig(cfg.basePath),
    ...
    public void dryRun() {
        try (SparkRDDWriteClient<HoodieRecordPayload> client =
                UtilHelpers.createHoodieClient(
                        jsc, cfg.basePath, "", cfg.parallelism, Option.empty(), props)) {
            ...
            Map<String, List<String>> partitionToReplaceFileIds =
                    jsc.parallelize(parts, parts.size())
                            .distinct()
                            .mapToPair(
                                    partitionPath ->
                                            new Tuple2<>(
    ...
    private void verifyHiveConfigs() {
        ValidationUtils.checkArgument(
                !StringUtils.isNullOrEmpty(cfg.hiveDataBase),
                "Hive database name couldn't be null or empty when enable sync meta, please set --hive-database/-db.");
        ValidationUtils.checkArgument(
                !StringUtils.isNullOrEmpty(cfg.hiveTableName),
                "Hive table name couldn't be null or empty when enable sync meta, please set --hive-table-name/-tn.");
    }

    private void syncHive(HiveSyncConfig hiveSyncConfig) {
        LOG.info(
                "Syncing target hoodie table with hive table("
    ...
    private void printDeleteFilesInfo(Map<String, List<String>> partitionToReplaceFileIds) {
        ...
        for (Map.Entry<String, List<String>> entry : partitionToReplaceFileIds.entrySet()) {
            LOG.info(
                    String.format(
                            "Partitions : %s, corresponding data file IDs : %s",
