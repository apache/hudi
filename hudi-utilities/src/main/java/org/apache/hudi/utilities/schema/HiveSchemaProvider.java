...
public class HiveSchemaProvider extends SchemaProvider {
    ...
    public HiveSchemaProvider(TypedProperties props, JavaSparkContext jssc) {
        ...
        checkRequiredConfigProperties(
                props, Collections.singletonList(HiveSchemaProviderConfig.SOURCE_SCHEMA_TABLE));
        String sourceSchemaDatabaseName =
                getStringWithAltKeys(props, HiveSchemaProviderConfig.SOURCE_SCHEMA_DATABASE, true);
        String sourceSchemaTableName =
                getStringWithAltKeys(props, HiveSchemaProviderConfig.SOURCE_SCHEMA_TABLE);
        SparkSession spark =
                SparkSession.builder().config(jssc.getConf()).enableHiveSupport().getOrCreate();

        // source schema
        try {
            TableIdentifier sourceSchemaTable =
                    new TableIdentifier(sourceSchemaTableName, scala.Option.apply(sourceSchemaDatabaseName));
            ...
            this.sourceSchema =
                    AvroConversionUtils.convertStructTypeToAvroSchema(
                            sourceSchema, sourceSchemaTableName, "hoodie." + sourceSchemaDatabaseName);
        } catch (NoSuchTableException | NoSuchDatabaseException e) {
            throw new HoodieSchemaFetchException(
                    String.format(
                            "Can't find Hive table: %s.%s", sourceSchemaDatabaseName, sourceSchemaTableName),
                    e);
        }

        // target schema
        if (containsConfigProperty(props, HiveSchemaProviderConfig.TARGET_SCHEMA_TABLE)) {
            String targetSchemaDatabaseName =
                    getStringWithAltKeys(props, HiveSchemaProviderConfig.TARGET_SCHEMA_DATABASE, true);
            String targetSchemaTableName =
                    getStringWithAltKeys(props, HiveSchemaProviderConfig.TARGET_SCHEMA_TABLE);
            try {
                TableIdentifier targetSchemaTable =
                        new TableIdentifier(
                                targetSchemaTableName, scala.Option.apply(targetSchemaDatabaseName));
                StructType targetSchema =
                        spark.sessionState().catalog().getTableMetadata(targetSchemaTable).schema();
                this.targetSchema =
                        AvroConversionUtils.convertStructTypeToAvroSchema(
                                targetSchema, targetSchemaTableName, "hoodie." + targetSchemaDatabaseName);
            } catch (NoSuchDatabaseException | NoSuchTableException e) {
                throw new HoodieSchemaFetchException(
                        String.format(
                                "Can't find Hive table: %s.%s", targetSchemaDatabaseName, targetSchemaTableName),
                        e);
            }
        }
    }
