...
public class DFSPathSelector implements Serializable {
    ...
    /**
      * Factory method for creating custom DFSPathSelector. Default selector to use is {@link
      * DFSPathSelector}
      */
        ...
        try {
            ...
            for (FileStatus f : eligibleFiles) {
                if (currentBytes + f.getLen() >= sourceLimit
                    ...
                    // so that some files with the same modification time won't be skipped in next
                    // read
                    break;
                }
    ...
    protected List<FileStatus> listEligibleFiles(FileSystem fs, Path path, long lastCheckpointTime)
            throws IOException {
