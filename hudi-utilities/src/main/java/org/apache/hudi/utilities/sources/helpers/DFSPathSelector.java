...
public class DFSPathSelector implements Serializable {
  ...
  public DFSPathSelector(TypedProperties props, Configuration hadoopConf) {
    ...
    this.props = props;
    this.storage = HoodieStorageUtils.getHoodieStorage(
  ...
  protected List<StoragePathInfo> listEligibleFiles(HoodieStorage storage, StoragePath path,
    ...
    List<StoragePathInfo> statuses = storage.listDirectEntries(path, file ->
    ...
    for (StoragePathInfo status : statuses) {
      if (status.isDirectory()) {
        ...
        // avoid infinite loop
        // if (!status.isSymlink()) {
        res.addAll(listEligibleFiles(storage, status.getPath(), lastCheckpointTime));
