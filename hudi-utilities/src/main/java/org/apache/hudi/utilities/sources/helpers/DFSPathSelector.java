...
public class DFSPathSelector implements Serializable {
  ...
  public Pair<Option<String>, String> getNextFilePathsAndMaxModificationTime(Option<String> lastCheckpointStr,
    ...
    try {
      ...
      List<StoragePathInfo> eligibleFiles = listEligibleFiles(
          storage, new StoragePath(getStringWithAltKeys(props, DFSPathSelectorConfig.ROOT_INPUT_PATH)), lastCheckpointTime);
  ...
  protected List<StoragePathInfo> listEligibleFiles(HoodieStorage storage,
    ...
    for (StoragePathInfo pathInfo : pathInfos) {
      if (!visited.add(pathInfo)) {
        // visited this pathInfo, skip this path
        return new ArrayList<>();
      }

      if (pathInfo.isDirectory()) {
        res.addAll(listEligibleFiles(storage, pathInfo.getPath(), lastCheckpointTime, visited));
