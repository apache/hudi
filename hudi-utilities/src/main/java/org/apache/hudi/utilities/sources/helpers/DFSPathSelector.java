...
public class DFSPathSelector implements Serializable {
   ...
   * Configs supported.
   * <p>
   * Please use {@link DFSPathSelectorConfig} instead.
   */
  @Deprecated
  ...
  public DFSPathSelector(TypedProperties props, Configuration hadoopConf) {
    DataSourceUtils.checkRequiredProperties(
        props, Collections.singletonList(DFSPathSelectorConfig.ROOT_INPUT_PATH.key()));
    this.props = props;
  ...
  public Pair<Option<String>, String> getNextFilePathsAndMaxModificationTime(Option<String> lastCheckpointStr,
    ...
    try {
      ...
      log.info("Root path => " + props.getString(DFSPathSelectorConfig.ROOT_INPUT_PATH.key())
          + " source limit => " + sourceLimit);
      long lastCheckpointTime = lastCheckpointStr.map(Long::parseLong).orElse(Long.MIN_VALUE);
      List<FileStatus> eligibleFiles = listEligibleFiles(
          fs, new Path(props.getString(DFSPathSelectorConfig.ROOT_INPUT_PATH.key())), lastCheckpointTime);
