...
/**
  * An incremental source that detects new data in a source table containing metadata about GCS
  * files, downloads the actual content of these files from GCS and stores them as records into a
  * destination table.
  *
  * <p>You should set spark.driver.extraClassPath in spark-defaults.conf to look like below WITHOUT
  * THE NEWLINES (or give the equivalent as CLI options if in cluster mode): (mysql-connector at the
  * end is only needed if Hive Sync is enabled and Mysql is used for Hive Metastore).
  *
  * <p>absolute_path_to/protobuf-java-3.21.1.jar:absolute_path_to/failureaccess-1.0.1.jar:
  * absolute_path_to/31.1-jre/guava-31.1-jre.jar: absolute_path_to/mysql-connector-java-8.0.30.jar
  *
  * <p>This class can be invoked via spark-submit as follows. There's a bunch of optional hive sync
  * flags at the end. $ bin/spark-submit \ --packages com.google.cloud:google-cloud-pubsub:1.120.0 \
  * --packages com.google.cloud.bigdataoss:gcs-connector:hadoop2-2.2.7 \ --driver-memory 4g \
  * --executor-memory 4g \ --class org.apache.hudi.utilities.streamer.HoodieStreamer \
  * absolute_path_to/hudi-utilities-bundle_2.12-0.13.0-SNAPSHOT.jar \ --source-class
  * org.apache.hudi.utilities.sources.GcsEventsHoodieIncrSource \ --op INSERT \ --hoodie-conf
  * hoodie.streamer.source.hoodieincr.file.format="parquet" \ --hoodie-conf
  * hoodie.streamer.source.cloud.data.select.file.extension="jsonl" \ --hoodie-conf
  * hoodie.streamer.source.cloud.data.datafile.format="json" \ --hoodie-conf
  * hoodie.streamer.source.cloud.data.select.relpath.prefix="country" \ --hoodie-conf
  * hoodie.streamer.source.cloud.data.ignore.relpath.prefix="blah" \ --hoodie-conf
  * hoodie.streamer.source.cloud.data.ignore.relpath.substring="blah" \ --hoodie-conf
  * hoodie.datasource.write.recordkey.field=id \ --hoodie-conf
  * hoodie.datasource.write.partitionpath.field= \ --filter-dupes \ --hoodie-conf
  * hoodie.datasource.write.insert.drop.duplicates=true \ --hoodie-conf
  * hoodie.combine.before.insert=true \ --source-ordering-field id \ --table-type COPY_ON_WRITE \
  * --target-base-path file:\/\/\/absolute_path_to/data-gcs \ --target-table gcs_data \ --continuous
  * \ --source-limit 100 \ --min-sync-interval-seconds 60 \ --hoodie-conf
  * hoodie.streamer.source.hoodieincr.path=file:\/\/\/absolute_path_to/meta-gcs \ --hoodie-conf
  * hoodie.streamer.source.hoodieincr.missing.checkpoint.strategy=READ_UPTO_LATEST_COMMIT \
  * --enable-hive-sync \ --hoodie-conf hoodie.datasource.hive_sync.database=default \ --hoodie-conf
  * hoodie.datasource.hive_sync.table=gcs_data
  */
public class GcsEventsHoodieIncrSource extends HoodieIncrSource {
    ...
    public Pair<Option<Dataset<Row>>, String> fetchNextBatch(
            Option<String> lastCheckpoint, long sourceLimit) {
        ...
        if (isNullOrEmpty(cloudObjectIncrCheckpoint.getKey())
            ...
            LOG.info(
                    "Source of file names is empty. Returning empty result and endInstant: "
