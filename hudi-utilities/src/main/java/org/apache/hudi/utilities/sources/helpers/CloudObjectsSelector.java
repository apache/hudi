...
/**
  * This class has methods for processing cloud objects. It currently supports only AWS S3 objects
  * and AWS SQS queue.
  */
public class CloudObjectsSelector {
    ...
    public CloudObjectsSelector(TypedProperties props) {
        checkRequiredConfigProperties(
                props,
                Arrays.asList(S3SourceConfig.S3_SOURCE_QUEUE_URL, S3SourceConfig.S3_SOURCE_QUEUE_REGION));
        this.props = props;
        ...
        this.maxMessagePerBatch =
                getIntWithAltKeys(props, S3SourceConfig.S3_SOURCE_QUEUE_MAX_MESSAGES_PER_BATCH);
        this.maxMessagesPerRequest =
                getIntWithAltKeys(props, S3SourceConfig.S3_SOURCE_QUEUE_MAX_MESSAGES_PER_REQUEST);
        this.visibilityTimeout =
                getIntWithAltKeys(props, S3SourceConfig.S3_SOURCE_QUEUE_VISIBILITY_TIMEOUT);
    }
    ...
    protected Map<String, String> getSqsQueueAttributes(SqsClient sqsClient, String queueUrl) {
        GetQueueAttributesResponse queueAttributesResult =
    ...
    protected void deleteBatchOfMessages(
            SqsClient sqs, String queueUrl, List<Message> messagesToBeDeleted) {
    ...
    public void deleteProcessedMessages(
            SqsClient sqs, String queueUrl, List<Message> processedMessages) {
    ...
    public static class Config {
        ...
        /**
          * {@link #S3_SOURCE_QUEUE_REGION} is the case-sensitive region name of the cloud provider for
          * the queue. For example, "us-east-1".
          */
        @Deprecated
        public static final String S3_SOURCE_QUEUE_REGION = S3SourceConfig.S3_SOURCE_QUEUE_REGION.key();

        /**
          * {@link #S3_SOURCE_QUEUE_FS} is file system corresponding to queue. For example, for AWS SQS
          * it is s3/s3a.
          */
        @Deprecated
        ...
        public static final String S3_QUEUE_LONG_POLL_WAIT =
                S3SourceConfig.S3_QUEUE_LONG_POLL_WAIT.key();

        /**
          * {@link #S3_SOURCE_QUEUE_MAX_MESSAGES_PER_BATCH} is max messages for each batch of Hudi
          * Streamer run. Source will process these maximum number of message at a time.
          */
        @Deprecated
        public static final String S3_SOURCE_QUEUE_MAX_MESSAGES_PER_BATCH =
                S3SourceConfig.S3_SOURCE_QUEUE_MAX_MESSAGES_PER_BATCH.key();
        ...
        public static final String S3_SOURCE_QUEUE_MAX_MESSAGES_PER_REQUEST =
                S3SourceConfig.S3_SOURCE_QUEUE_MAX_MESSAGES_PER_REQUEST.key();

        /**
          * {@link #S3_SOURCE_QUEUE_VISIBILITY_TIMEOUT} is visibility timeout for messages in queue.
          * After we consume the message, queue will move the consumed messages to in-flight state, these
          * messages can't be consumed again by source for this timeout period.
          */
        @Deprecated
        public static final String S3_SOURCE_QUEUE_VISIBILITY_TIMEOUT =
                S3SourceConfig.S3_SOURCE_QUEUE_VISIBILITY_TIMEOUT.key();
