...
public class CloudObjectsSelector {
  ...
  public CloudObjectsSelector(TypedProperties props) {
    DataSourceUtils.checkRequiredProperties(props,
        Arrays.asList(S3SourceConfig.S3_SOURCE_QUEUE_URL.key(), S3SourceConfig.S3_SOURCE_QUEUE_REGION.key()));
    this.props = props;
    ...
    this.fsName = props.getString(S3SourceConfig.S3_SOURCE_QUEUE_FS.key(),
        S3SourceConfig.S3_SOURCE_QUEUE_FS.defaultValue()).toLowerCase();
    this.longPollWait = props.getInteger(S3SourceConfig.S3_QUEUE_LONG_POLL_WAIT.key(),
        S3SourceConfig.S3_QUEUE_LONG_POLL_WAIT.defaultValue());
    this.maxMessagePerBatch = props.getInteger(S3SourceConfig.S3_SOURCE_QUEUE_MAX_MESSAGES_PER_BATCH.key(),
        S3SourceConfig.S3_SOURCE_QUEUE_MAX_MESSAGES_PER_BATCH.defaultValue());
    this.maxMessagesPerRequest = props.getInteger(S3SourceConfig.S3_SOURCE_QUEUE_MAX_MESSAGES_PER_REQUEST.key(),
        S3SourceConfig.S3_SOURCE_QUEUE_MAX_MESSAGES_PER_REQUEST.defaultValue());
    this.visibilityTimeout = props.getInteger(S3SourceConfig.S3_SOURCE_QUEUE_VISIBILITY_TIMEOUT.key(),
        S3SourceConfig.S3_SOURCE_QUEUE_VISIBILITY_TIMEOUT.defaultValue());
  }
  ...
  /**
   * Configs supported.
   * Please use {@link S3SourceConfig} instead
   */
  @Deprecated
