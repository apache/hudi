...
/**
  * Convert a variety of datum into Avro GenericRecords. Has a bunch of lazy fields to circumvent
  * issues around serializing these objects from driver to executors
  */
public class AvroConvertor implements Serializable {
    ...
    public AvroConvertor(String schemaStr) {
        this(
                schemaStr,
                SANITIZE_SCHEMA_FIELD_NAMES.defaultValue(),
        ...
        try {
            ...
            if (json != null) {
                ...
                throw new HoodieSchemaException(
                        "Failed to convert schema from json to avro. Schema string was null.", e);
            }
        }
    }
    ...
    public GenericRecord fromAvroBinary(byte[] avroBinary) {
        try {
            ...
            if (avroBinary != null) {
                throw new HoodieSchemaException(
                        "Failed to get avro schema from avro binary: " + Arrays.toString(avroBinary), e);
            } else {
                throw new HoodieSchemaException(
                        "Failed to get avro schema from avro binary. Binary is null", e);
            }
        }
    }
