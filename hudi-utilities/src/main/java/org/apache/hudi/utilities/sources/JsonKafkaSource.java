...
public class JsonKafkaSource extends KafkaSource<JavaRDD<String>> {
    ...
    protected JavaRDD<String> maybeAppendKafkaOffsets(
            JavaRDD<ConsumerRecord<Object, Object>> kafkaRDD) {
        if (this.shouldAddOffsets) {
            return kafkaRDD.mapPartitions(
                    partitionIterator -> {
                        ObjectMapper objectMapper = new ObjectMapper();
                        return new CloseableMappingIterator<>(
                                ClosableIterator.wrap(partitionIterator),
                                consumerRecord -> {
    ...
    private JavaRDD<String> postProcess(JavaRDD<String> jsonStringRDD) {
        ...
        try {
            processor =
                    UtilHelpers.createJsonKafkaSourcePostProcessor(postProcessorClassName, this.props);
        } catch (IOException e) {
