...
/**
  * Adapts data-format provided by the source to the data-format required by the client
  * (DeltaStreamer).
  */
public class SourceFormatAdapter implements Closeable {
    ...
    public SourceFormatAdapter(
            Source source, Option<BaseErrorTableWriter> errorTableWriter, Option<TypedProperties> props) {
        this.source = source;
        ...
        if (props.isPresent()) {
            ...
            this.wrapWithException =
                    ConfigUtils.getBooleanWithAltKeys(props.get(), ROW_THROW_EXPLICIT_EXCEPTIONS);
        }
    ...
    /**
      * transform input rdd of json string to generic records with support for adding error events to
      * error table
      *
      * @param inputBatch
      * @return
    ...
    private JavaRDD<GenericRecord> transformJsonToGenericRdd(InputBatch<JavaRDD<String>> inputBatch) {
        MercifulJsonConverter.clearCache(
                inputBatch.getSchemaProvider().getSourceSchema().getFullName());
        AvroConvertor convertor =
                new AvroConvertor(
                        inputBatch.getSchemaProvider().getSourceSchema(),
                        isFieldNameSanitizingEnabled(),
                        getInvalidCharMask());
        return inputBatch
                .getBatch()
                .map(
    ...
    /**
      ...
      * @return
      */
    public Option<Dataset<Row>> processErrorEvents(
            Option<Dataset<Row>> eventsRow, ErrorEvent.ErrorReason errorReason) {
        return eventsRow.map(
                dataset -> {
                    if (errorTableWriter.isPresent()
                        ...
                        errorTableWriter
                                ...
                                .addErrorEvents(
                                        dataset
                                                .filter(new Column(ERROR_TABLE_CURRUPT_RECORD_COL_NAME).isNotNull())
                                                .select(new Column(ERROR_TABLE_CURRUPT_RECORD_COL_NAME))
                                                .toJavaRDD()
                                                .map(ev -> new ErrorEvent<>(ev.getString(0), errorReason)));
                        return dataset
                                .filter(new Column(ERROR_TABLE_CURRUPT_RECORD_COL_NAME).isNull())
                                .drop(ERROR_TABLE_CURRUPT_RECORD_COL_NAME);
                    }
                    return dataset;
                });
    }
    ...
    /**
      * Fetch new data in avro format. If the source provides data in different format, they are
      * translated to Avro format
      */
    public InputBatch<JavaRDD<GenericRecord>> fetchNewDataInAvroFormat(
        ...
        switch (source.getSourceType()) {
            ...
            case JSON:
                {
                    // sanitizing is done inside the convertor in transformJsonToGenericRdd if
                    // enabled
                    InputBatch<JavaRDD<String>> r =
                            ((Source<JavaRDD<String>>) source).fetchNext(lastCkptStr, sourceLimit);
                    JavaRDD<GenericRecord> eventsRdd = transformJsonToGenericRdd(r);
                    return new InputBatch<>(
                            Option.ofNullable(eventsRdd), r.getCheckpointForNextBatch(), r.getSchemaProvider());
                }
            case ROW:
                {
                    // we do the sanitizing here if enabled
                    InputBatch<Dataset<Row>> r =
                            ((Source<Dataset<Row>>) source).fetchNext(lastCkptStr, sourceLimit);
                    return new InputBatch<>(
                            Option.ofNullable(
                                    r.getBatch()
                                            .map(
                                                    rdd -> {
                                                        ...
                                                        return (originalProvider instanceof FilebasedSchemaProvider
                                                                ...
                                                                // If the source schema is specified
                                                                // through Avro schema,
                                                                // pass in the schema for the
                                                                // Row-to-Avro conversion
                                                                // to avoid nullability mismatch
                                                                // between Avro schema and Row
                                                                // schema
            ...
            case PROTO:
                {
                    ...
                    InputBatch<JavaRDD<Message>> r =
                            ((Source<JavaRDD<Message>>) source).fetchNext(lastCkptStr, sourceLimit);
                    AvroConvertor convertor = new AvroConvertor(r.getSchemaProvider().getSourceSchema());
                    return new InputBatch<>(
                            Option.ofNullable(
                                    r.getBatch().map(rdd -> rdd.map(convertor::fromProtoMessage)).orElse(null)),
                            r.getCheckpointForNextBatch(),
                            r.getSchemaProvider());
                }
            default:
                                ...
                                .map(
                                        rdd ->
                                                AvroConversionUtils.createDataFrame(
                                                        JavaRDD.toRDD(rdd), sourceSchema.toString(), source.getSparkSession()))
                                .orElse(null)),
                r.getCheckpointForNextBatch(),
                r.getSchemaProvider());
    }
    ...
    /**
      * Fetch new data in row format. If the source provides data in different format, they are
      * translated to Row format
      */
    public InputBatch<Dataset<Row>> fetchNewDataInRowFormat(
            Option<String> lastCkptStr, long sourceLimit) {
        switch (source.getSourceType()) {
            case ROW:
                // we do the sanitizing here if enabled
            ...
            case JSON:
                {
                    if (isFieldNameSanitizingEnabled()) {
                        // leverage the json -> avro sanitizing. TODO([HUDI-5829]) Optimize by
                        // sanitizing during direct conversion
                        InputBatch<JavaRDD<GenericRecord>> r =
                                fetchNewDataInAvroFormat(lastCkptStr, sourceLimit);
                        return avroDataInRowFormat(r);
                    }
                    InputBatch<JavaRDD<String>> r =
                            ((Source<JavaRDD<String>>) source).fetchNext(lastCkptStr, sourceLimit);
                    ...
                    if (errorTableWriter.isPresent()) {
                        ...
                        // Any records which spark is unable to read successfully are transferred to
                        // the column
                        // configured via this option. The column is then used to trigger error
                        // events.
                        StructType dataType =
                                        ...
                                        .map(
                                                rdd ->
                                                        source
                                                                ...
                                                                .json(rdd));
                        Option<Dataset<Row>> eventsDataset =
                                processErrorEvents(
                                        dataset, ErrorEvent.ErrorReason.JSON_ROW_DESERIALIZATION_FAILURE);
                        return new InputBatch<>(
                                eventsDataset, r.getCheckpointForNextBatch(), r.getSchemaProvider());
                    } else {
                        ...
                        return new InputBatch<>(
                                Option.ofNullable(
                                        r.getBatch()
                                                .map(
                                                        rdd ->
                                                                HoodieSparkUtils.maybeWrapDataFrameWithException(
                                                                        source.getSparkSession().read().schema(dataType).json(rdd),
                                                                        SchemaCompatibilityException.class.getName(),
                                                                        "Schema does not match json data",
                                                                        wrapWithException))
                                                .orElse(null)),
                                r.getCheckpointForNextBatch(),
                                r.getSchemaProvider());
        ...
        if (source instanceof Closeable) {
            try {
                ...
                throw new HoodieIOException(
                        String.format("Failed to shutdown the source (%s)", source.getClass().getName()), e);
            }
        }
    }
}
