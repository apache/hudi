...
public class ProtoClassBasedSchemaProviderConfig extends HoodieConfig {
    private static final String PROTO_SCHEMA_PROVIDER_PREFIX =
            SCHEMAPROVIDER_CONFIG_PREFIX + "proto.";
    private static final String OLD_PROTO_SCHEMA_PROVIDER_PREFIX =
            OLD_SCHEMAPROVIDER_CONFIG_PREFIX + "proto.";
    ...
    public static final ConfigProperty<Boolean> PROTO_SCHEMA_WRAPPED_PRIMITIVES_AS_RECORDS =
            ConfigProperty.key(PROTO_SCHEMA_PROVIDER_PREFIX + "flatten.wrappers")
                    ...
                    .sinceVersion("0.13.0")
                    .withDocumentation(
                            "When set to true wrapped primitives like Int64Value are translated to a record with a single 'value' field. By default, the value is false and the wrapped primitives are "
    ...
    public static final ConfigProperty<Boolean> PROTO_SCHEMA_TIMESTAMPS_AS_RECORDS =
            ConfigProperty.key(PROTO_SCHEMA_PROVIDER_PREFIX + "timestamps.as.records")
                    ...
                    .sinceVersion("0.13.0")
                    .withDocumentation(
                            "When set to true Timestamp fields are translated to a record with a seconds and nanos field. By default, the value is false and the timestamp is converted to a long with "
                                    + "the timestamp-micros logical type");

    public static final ConfigProperty<Integer> PROTO_SCHEMA_MAX_RECURSION_DEPTH =
            ConfigProperty.key(PROTO_SCHEMA_PROVIDER_PREFIX + "max.recursion.depth")
                    .defaultValue(5)
                    .withAlternatives(OLD_PROTO_SCHEMA_PROVIDER_PREFIX + "max.recursion.depth")
                    .markAdvanced()
                    .sinceVersion("0.13.0")
                    .withDocumentation(
                            "The max depth to unravel the Proto schema when translating into an Avro schema. Setting this depth allows the user to convert a schema that is recursive in proto into "
