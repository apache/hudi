...
public class ChainedTransformer implements Transformer {
    ...
    /**
      * Creates a chained transformer using the input transformer class names. Refer {@link
      * HoodieStreamer.Config#transformerClassNames} for more information on how the transformers can
      * be configured.
      *
      * @param sourceSchemaSupplier Supplies the schema (if schema provider is present) for the dataset
    ...
    public ChainedTransformer(
            List<String> configuredTransformers, Supplier<Option<Schema>> sourceSchemaSupplier) {
        ...
        for (String configuredTransformer : configuredTransformers) {
            if (!configuredTransformer.contains(ID_TRANSFORMER_CLASS_NAME_DELIMITER)) {
                ...
                if (splits.length > 2) {
                    throw new HoodieTransformPlanException(
                            "There should only be one colon in a configured transformer");
                }
                String id = splits[0];
    ...
    public List<String> getTransformersNames() {
        return transformers.stream()
                .map(t -> t.getTransformer().getClass().getName())
    ...
    public Dataset<Row> apply(
        ...
        for (TransformerInfo transformerInfo : transformers) {
            ...
            dataset =
                    transformer.apply(
                            jsc, sparkSession, dataset, transformerInfo.getProperties(properties, transformers));
        }
        return dataset;
    }

    private void validateIdentifier(
            String id, Set<String> identifiers, String configuredTransformer) {
        ...
        if (identifiers.contains(id)) {
            throw new HoodieTransformPlanException(
                    String.format(
                            "Duplicate identifier %s found for transformer %s", id, configuredTransformer));
        } else {
            identifiers.add(id);
        }
    }

    private StructType getExpectedTransformedSchema(
            TransformerInfo transformerInfo,
            JavaSparkContext jsc,
            SparkSession sparkSession,
            Option<StructType> incomingStructOpt,
            Option<Dataset<Row>> rowDatasetOpt,
            TypedProperties properties) {
        Option<Schema> sourceSchemaOpt = sourceSchemaSupplier.get();
        if (!sourceSchemaOpt.isPresent() && !rowDatasetOpt.isPresent()) {
            throw new HoodieTransformPlanException(
                    "Either source schema or source dataset should be available to fetch the schema");
        }
        StructType incomingStruct =
                incomingStructOpt.orElseGet(
                        () ->
                                sourceSchemaOpt.isPresent()
                                        ? AvroConversionUtils.convertAvroSchemaToStructType(sourceSchemaOpt.get())
                                        : rowDatasetOpt.get().schema());
        return transformerInfo
                .getTransformer()
                .transformedSchema(jsc, sparkSession, incomingStruct, properties)
                .asNullable();
    }

    @Override
    public StructType transformedSchema(
            JavaSparkContext jsc,
            SparkSession sparkSession,
            StructType incomingStruct,
            TypedProperties properties) {
        ...
        for (TransformerInfo transformerInfo : transformers) {
            expectedTargetStructOpt =
                    Option.of(
                            getExpectedTransformedSchema(
                                    transformerInfo,
                                    jsc,
                                    sparkSession,
                                    expectedTargetStructOpt,
    ...
    protected static class TransformerInfo {
        ...
        protected TypedProperties getProperties(
                TypedProperties properties, List<TransformerInfo> transformers) {
            Set<String> transformerIds =
                    transformers.stream()
                            .map(transformerInfo -> transformerInfo.idOpt.orElse(null))
                            .filter(Objects::nonNull)
                            .collect(Collectors.toSet());
            TypedProperties transformerProps = properties;
            if (idOpt.isPresent()) {
                // Transformer specific property keys end with the id associated with the
                // transformer.
                ...
                for (Map.Entry<Object, Object> entry : properties.entrySet()) {
                    ...
                    if (keyId.equals(id)) {
                        overrideKeysMap.put(
                                key.substring(0, key.length() - (id.length() + 1)), entry.getValue());
                    }
