...
public class KafkaOffsetPostProcessor extends SchemaPostProcessor {
    ...
    public Schema processSchema(Schema schema) {
        // this method adds kafka offset fields namely source offset, partition, timestamp and kafka
        // message key to the schema of the batch.
        List<Schema.Field> fieldList = schema.getFields();
        Set<String> fieldNames = fieldList.stream().map(Schema.Field::name).collect(Collectors.toSet());
        // if the source schema already contains the kafka offset fields, then return the schema as
        // is.
        if (fieldNames.containsAll(
                Arrays.asList(
        ...
        try {
            List<Schema.Field> newFieldList =
                    fieldList.stream()
                            .map(f -> new Schema.Field(f.name(), f.schema(), f.doc(), f.defaultVal()))
                            .collect(Collectors.toList());
            // handle case where source schema provider may have already set 1 or more of these
            // fields
            if (!fieldNames.contains(KAFKA_SOURCE_OFFSET_COLUMN)) {
                newFieldList.add(
                        new Schema.Field(
                                KAFKA_SOURCE_OFFSET_COLUMN, Schema.create(Schema.Type.LONG), "offset column", 0));
            }
            ...
            throw new HoodieSchemaException(
                    "Kafka offset post processor failed with schema: " + schema, e);
        }
    }
}
