...
public class HoodieWithTimelineServer implements Serializable {
    ...
    public static class Config implements Serializable {
        ...
        @Parameter(
                names = {"--delay-secs", "-d"},
                description = "Delay(sec) before client connects")
    ...
    public static void main(String[] args) throws Exception {
        ...
        service.run(
                UtilHelpers.buildSparkContext("client-server-hoodie", cfg.sparkMaster, cfg.sparkMemory));
    }
    ...
    public void run(JavaSparkContext jsc) throws UnknownHostException {
        ...
        List<String> gotMessages =
                context.map(messages, msg -> sendRequest(driverHost, cfg.serverPort), messages.size());
    ...
    public String sendRequest(String driverHost, int port) {
        ...
        try (CloseableHttpClient client = HttpClientBuilder.create().build()) {
            ...
            System.out.println(
                    "Response Code from(" + url + ") : " + response.getStatusLine().getStatusCode());
