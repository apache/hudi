...
public class HoodieDataTableUtils {
    ...
    static List<StoragePath> getBaseAndLogFilePathsFromFileSystem(
            HoodieTableMetadata tableMetadata, String basePath) throws IOException {
        List<String> allPartitionPaths =
                tableMetadata.getAllPartitionPaths().stream()
                        .map(partitionPath -> FSUtils.constructAbsolutePath(basePath, partitionPath).toString())
