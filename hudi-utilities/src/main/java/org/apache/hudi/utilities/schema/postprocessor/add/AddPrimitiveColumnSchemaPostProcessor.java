...
/**
  * A {@link SchemaPostProcessor} used to add a new column of primitive types to given schema. Only
  * supports adding one column at a time.
  *
  * <p>The new column will be appended to the end.
  *
  * <p>TODO support complex types.
  */
public class AddPrimitiveColumnSchemaPostProcessor extends SchemaPostProcessor {
    ...
    public Schema processSchema(Schema schema) {
        String newColumnName =
                getStringWithAltKeys(this.config, SCHEMA_POST_PROCESSOR_ADD_COLUMN_NAME_PROP);

        if (schema.getField(newColumnName) != null) {
            throw new HoodieSchemaPostProcessException(
                    String.format("Column %s already exist!", newColumnName));
        }
    ...
    private Schema.Field buildNewColumn() {
        ...
        String columnName =
                getStringWithAltKeys(this.config, SCHEMA_POST_PROCESSOR_ADD_COLUMN_NAME_PROP);
        String type =
                getStringWithAltKeys(this.config, SCHEMA_POST_PROCESSOR_ADD_COLUMN_TYPE_PROP)
                        .toUpperCase(Locale.ROOT);
        String doc = getStringWithAltKeys(this.config, SCHEMA_POST_PROCESSOR_ADD_COLUMN_DOC_PROP, true);
        Option<Object> defaultValue =
                getRawValueWithAltKeys(this.config, SCHEMA_POST_PROCESSOR_ADD_COLUMN_DEFAULT_PROP);

        boolean nullable =
                getBooleanWithAltKeys(this.config, SCHEMA_POST_PROCESSOR_ADD_COLUMN_NULLABLE_PROP);
        ...
        return new Schema.Field(
                columnName, newSchema, doc, defaultValue.isPresent() ? defaultValue.get() : null);
    }
