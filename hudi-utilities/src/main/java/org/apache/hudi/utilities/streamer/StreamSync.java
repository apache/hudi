...
public class StreamSync implements Serializable, Closeable {
    ...
    /**
      ...
      *
      * <p>NOTE: These properties are already consolidated w/ CLI provided config-overrides
      */
    ...
    // all commits timeline, including all (commits, delta commits, compaction, clean, savepoint,
    // rollback, replace commits, index)
    ...
    /**
      * DeltaSync will explicitly manage embedded timeline server so that they can be reused across
      * Write Client instantiations.
      */
    ...
    StreamSync(
            ...
            SchemaProvider userProvidedSchemaProvider,
            Option<BaseErrorTableWriter> errorTableWriter,
            SourceFormatAdapter formatAdapter,
    ...
    public StreamSync(
        ...
        this(
                ...
                sparkSession,
                props,
                new HoodieSparkEngineContext(jssc),
                fs,
                conf,
    ...
    public StreamSync(
        ...
        if (props.getBoolean(ERROR_TABLE_ENABLED.key(), ERROR_TABLE_ENABLED.defaultValue())) {
            this.errorTableWriter =
                    ErrorTableUtils.getErrorTableWriter(cfg, sparkSession, props, hoodieSparkContext, fs);
        ...
        Supplier<Option<Schema>> schemaSupplier =
                schemaProvider == null
                        ? Option::empty
                        : () -> Option.ofNullable(schemaProvider.getSourceSchema());
        this.transformer =
                UtilHelpers.createTransformer(
                        Option.ofNullable(cfg.transformerClassNames),
                        schemaSupplier,
                        this.errorTableWriter.isPresent());
        // enable row writer only when operation is BULK_INSERT, and source is ROW type and if row
        // writer is not explicitly disabled.
        this.useRowWriter =
        ...
        if (storage.exists(new StoragePath(cfg.targetBasePath))) {
            try {
                ...
                switch (meta.getTableType()) {
                    case COPY_ON_WRITE:
                    case MERGE_ON_READ:
                        // we can use getCommitsTimeline for both COW and MOR here, because for COW
                        // there is no deltacommit
                ...
                if (e.getMessage().contains("Could not load Hoodie properties")
                    ...
                    String basePathWithForwardSlash =
                            cfg.targetBasePath.endsWith("/")
                                    ? cfg.targetBasePath
                                    : String.format("%s/", cfg.targetBasePath);
                    String pathToHoodieProps =
                            String.format(
                                    "%s%s/%s",
                                    basePathWithForwardSlash,
                                    HoodieTableMetaClient.METAFOLDER_NAME,
                                    HoodieTableConfig.HOODIE_PROPERTIES_FILE);
                    String pathToHoodiePropsBackup =
                            String.format(
                                    "%s%s/%s",
                                    basePathWithForwardSlash,
                                    HoodieTableMetaClient.METAFOLDER_NAME,
                    ...
                    if (!hoodiePropertiesExists) {
                        ...
                        // reload the timeline from metaClient and validate that its empty table. If
                        // there are any instants found, then we should fail the pipeline, bcoz
    ...
    private void initializeEmptyTable() throws IOException {
        ...
        HoodieTableMetaClient.withPropertyBuilder()
                ...
                .setCDCEnabled(
                        props.getBoolean(
                                HoodieTableConfig.CDC_ENABLED.key(), HoodieTableConfig.CDC_ENABLED.defaultValue()))
    ...
    public Pair<Option<String>, JavaRDD<WriteStatus>> syncOnce() throws IOException {
        ...
        if (inputBatch != null) {
            ...
            // this is the first input batch. If schemaProvider not set, use it and register Avro
            // Schema and start
            // compactor
            if (writeClient == null) {
                ...
                if ((newSourceSchema != null && !processedSchema.isSchemaPresent(newSourceSchema))
                    ...
                    String sourceStr =
                            newSourceSchema == null ? NULL_PLACEHOLDER : newSourceSchema.toString(true);
                    String targetStr =
                            newTargetSchema == null ? NULL_PLACEHOLDER : newTargetSchema.toString(true);
            ...
            if (cfg.retryLastPendingInlineCompactionJob
                ...
                if (pendingCompactionInstant.isPresent()) {
                    ...
                    writeClient.commitCompaction(
                            pendingCompactionInstant.get(),
                            writeMetadata.getCommitMetadata().get(),
                            Option.empty());
                    refreshTimeline();
                    reInitWriteClient(
                            schemaProvider.getSourceSchema(), schemaProvider.getTargetSchema(), null);
                }
    ...
    private Option<String> getLastPendingClusteringInstant(Option<HoodieTimeline> commitTimelineOpt) {
        if (commitTimelineOpt.isPresent()) {
            ...
            return pendingClusteringInstant.isPresent()
                    ? Option.of(pendingClusteringInstant.get().getTimestamp())
                    : Option.empty();
        }
        return Option.empty();
    }
    ...
    private Option<String> getLastPendingCompactionInstant(Option<HoodieTimeline> commitTimelineOpt) {
        if (commitTimelineOpt.isPresent()) {
            ...
            return pendingCompactionInstant.isPresent()
                    ? Option.of(pendingCompactionInstant.get().getTimestamp())
                    : Option.empty();
        }
        return Option.empty();
    }
    ...
    /**
      ...
      *
      * @return Pair<InputBatch and Boolean> Input data read from upstream source, and boolean is true
      *     if empty.
      * @throws Exception in case of any Exception
      */
    public InputBatch readFromSource(String instantTime, HoodieTableMetaClient metaClient)
            throws IOException {
        ...
        while (curRetryCount++ < maxRetryCount && sourceDataToSync == null) {
            try {
                ...
                try {
                    LOG.error(
                            "Exception thrown while fetching data from source. Msg : "
                                    + e.getMessage()
                                    + ", class : "
                                    + e.getClass()
                                    + ", cause : "
                    ...
                    LOG.error(
                            "Sleeping for "
                                    + (cfg.retryIntervalSecs)
                                    + " before retrying again. Current retry count "
                                    + curRetryCount
                                    + ", max retry count "
                                    + cfg.maxRetryCount);
                    ...
                    LOG.error(
                            "Ignoring InterruptedException while waiting to retry on source failure "
                                    + e.getMessage());
                }
            }
        }
    ...
    private InputBatch fetchFromSourceAndPrepareRecords(
            Option<String> resumeCheckpointStr, String instantTime, HoodieTableMetaClient metaClient) {
        hoodieSparkContext.setJobStatus(
                this.getClass().getSimpleName(), "Fetching next batch: " + cfg.targetTableName);
        ...
        if (recordType == HoodieRecordType.SPARK
                ...
                && HoodieLogBlockType.fromId(
                                props.getProperty(HoodieStorageConfig.LOGFILE_DATA_BLOCK_FORMAT.key(), "avro"))
        ...
        if (!cfg.allowCommitOnNoCheckpointChange
                && Objects.equals(checkpointStr, resumeCheckpointStr.orElse(null))) {
            LOG.info(
                    "No new data, source checkpoint has not changed. Nothing to commit. Old checkpoint=("
                            + resumeCheckpointStr
                            + "). New Checkpoint=("
                            + checkpointStr
                            + ")");
            String commitActionType =
                    CommitUtils.getCommitActionType(cfg.operation, HoodieTableType.valueOf(cfg.tableType));
        ...
        hoodieSparkContext.setJobStatus(
                this.getClass().getSimpleName(), "Checking if input is empty: " + cfg.targetTableName);
    ...
    /**
      * Fetch data from source, apply transformations if any, align with schema from schema provider if
      * need be and return the input batch.
      *
      * @param resumeCheckpointStr checkpoint to resume from source.
      * @return {@link InputBatch} containing the new batch of data from source along with new
      *     checkpoint and schema provider instance to use.
      */
    @VisibleForTesting
    InputBatch fetchNextBatchFromSource(
            Option<String> resumeCheckpointStr, HoodieTableMetaClient metaClient) {
        ...
        if (transformer.isPresent()) {
            // Transformation is needed. Fetch New rows in Row Format, apply transformation and then
            // convert them
            ...
            transformed =
                    formatAdapter.processErrorEvents(
                            transformed, ErrorEvent.ErrorReason.CUSTOM_TRANSFORMER_FAILURE);
            ...
            if (this.userProvidedSchemaProvider != null
                ...
                if (useRowWriter) {
                    ...
                    if (errorTableWriter.isPresent()
                        ...
                        // If the above conditions are met, trigger error events for the rows whose
                        // conversion to
                        // avro records fails.
                        avroRDDOptional =
                                transformed.map(
                                        rowDataset -> {
                                            Tuple2<RDD<GenericRecord>, RDD<String>> safeCreateRDDs =
                                                    HoodieSparkUtils.safeCreateRDD(
                                                            rowDataset,
                        ...
                        avroRDDOptional =
                                transformed.map(
                                        rowDataset ->
                                                getTransformedRDD(
                                                        rowDataset, reconcileSchema, finalSchemaProvider.getTargetSchema()));
                    }
                }
            } else {
                                ...
                                .map(
                                        df ->
                                                AvroConversionUtils.convertStructTypeToAvroSchema(
                                                        df.schema(), getAvroRecordQualifiedName(cfg.targetTableName)))
                                .orElseGet(dataAndCheckpoint.getSchemaProvider()::getTargetSchema);
                schemaProvider =
                        getDeducedSchemaProvider(
                                incomingSchema, dataAndCheckpoint.getSchemaProvider(), metaClient);
                ...
                if (useRowWriter) {
                    ...
                    avroRDDOptional =
                            transformed.map(
                                    t ->
                                            getTransformedRDD(t, reconcileSchema, finalSchemaProvider.getTargetSchema()));
                }
            }
        } else {
            if (useRowWriter) {
                ...
                inputBatchForWriter =
                        new InputBatch<>(
                                inputBatchNeedsDeduceSchema.getBatch(),
                                inputBatchNeedsDeduceSchema.getCheckpointForNextBatch(),
                                getDeducedSchemaProvider(
                                        inputBatchNeedsDeduceSchema.getSchemaProvider().getTargetSchema(),
                                        inputBatchNeedsDeduceSchema.getSchemaProvider(),
                                        metaClient));
            } else {
                ...
                schemaProvider =
                        getDeducedSchemaProvider(
                                dataAndCheckpoint.getSchemaProvider().getTargetSchema(),
    ...
    /**
      * Apply schema reconcile and schema evolution rules(schema on read) and generate new target
      * schema provider.
      *
      ...
      */
    @VisibleForTesting
    SchemaProvider getDeducedSchemaProvider(
            Schema incomingSchema,
            SchemaProvider sourceSchemaProvider,
            HoodieTableMetaClient metaClient) {
        Option<Schema> latestTableSchemaOpt =
                UtilHelpers.getLatestTableSchema(
        ...
        return new DelegatingSchemaProvider(
                props,
                hoodieSparkContext.jsc(),
    ...
    private JavaRDD<GenericRecord> getTransformedRDD(
            Dataset<Row> rowDataset, boolean reconcileSchema, Schema readerSchema) {
    ...
    /**
      * Process previous commit metadata and checkpoint configs set by user to determine the checkpoint
      * to resume from.
      *
      ...
      */
    @VisibleForTesting
    Option<String> getCheckpointToResume(Option<HoodieTimeline> commitsTimelineOpt)
            throws IOException {
        Option<String> resumeCheckpointStr = Option.empty();
        // try get checkpoint from commits(including commit and deltacommit)
        // in COW migrating to MOR case, the first batch of the deltastreamer will lost the
        // checkpoint from COW table, cause the dataloss
        ...
        // if changing from mor to cow, before changing we must do a full compaction, so we can only
        // consider .commit in such case
        ...
        if (lastCommit.isPresent()) {
            // if previous commit metadata did not have the checkpoint key, try traversing previous
            // commits until we find one.
            ...
            if (commitMetadataOption.isPresent()) {
                ...
                LOG.debug(
                        "Checkpoint reset from metadata: " + commitMetadata.getMetadata(CHECKPOINT_RESET_KEY));
                if (cfg.ignoreCheckpoint != null
                        && (StringUtils.isNullOrEmpty(commitMetadata.getMetadata(CHECKPOINT_IGNORE_KEY))
                                || !cfg.ignoreCheckpoint.equals(
                ...
                } else if (HoodieTimeline.compareTimestamps(
                        HoodieTimeline.FULL_BOOTSTRAP_INSTANT_TS,
                        HoodieTimeline.LESSER_THAN,
                        lastCommit.get().getTimestamp())) {
                ...
                if (!StringUtils.isNullOrEmpty(commitMetadata.getMetadata(CHECKPOINT_RESET_KEY))) {
                    ...
                    != null) { // getLatestCommitMetadataWithValidCheckpointInfo(commitTimelineOpt.get()) will
    ...
    protected Option<Pair<String, HoodieCommitMetadata>>
            getLatestInstantAndCommitMetadataWithValidCheckpointInfo(HoodieTimeline timeline)
                    throws IOException {
                        ...
                        .map(
                                instant -> {
                                    try {
                                        ...
                                        throw new HoodieIOException(
                                                "Failed to parse HoodieCommitMetadata for " + instant.toString(), e);
                                    }
                                })
    ...
    protected Option<HoodieCommitMetadata> getLatestCommitMetadataWithValidCheckpointInfo(
            HoodieTimeline timeline) throws IOException {
        return getLatestInstantAndCommitMetadataWithValidCheckpointInfo(timeline)
                .map(pair -> pair.getRight());
    }

    protected Option<String> getLatestInstantWithValidCheckpointInfo(
            Option<HoodieTimeline> timelineOpt) {
        return timelineOpt
                .map(
                        timeline -> {
                            try {
                                ...
                                throw new HoodieIOException(
                                        "failed to get latest instant with ValidCheckpointInfo", e);
                            }
                        })
                .orElse(Option.empty());
    }
    ...
    private HoodieWriteConfig prepareHoodieConfigForRowWriter(Schema writerSchema) {
        ...
        return HoodieSparkSqlWriter.getBulkInsertRowConfig(
                writerSchema != InputBatch.NULL_SCHEMA ? Option.of(writerSchema) : Option.empty(),
                hoodieConfig,
                cfg.targetBasePath,
                cfg.targetTableName);
    }
    ...
    /**
      ...
      */
    private Pair<Option<String>, JavaRDD<WriteStatus>> writeToSinkAndDoMetaSync(
            String instantTime,
        ...
        LOG.info(
                String.format(
                        "instantTime=%s, totalRecords=%d, totalErrorRecords=%d, totalSuccessfulRecords=%d",
        ...
        if (!hasErrors || cfg.commitOnErrors) {
                ...
                if (!errorTableSuccess) {
                    switch (errorWriteFailureStrategy) {
                        ...
                        case LOG_ERROR:
                            ...
                            break;
                        default:
                            throw new HoodieStreamerWriteException(
                                    "Write failure strategy not implemented for " + errorWriteFailureStrategy);
                    }
                }
            }
            boolean success =
            ...
            if (success) {
                ...
                if ((totalSuccessfulRecords > 0) || cfg.forceEmptyMetaSync) {
                    ...
                    LOG.info(
                            String.format(
                                    "Not running metaSync totalSuccessfulRecords=%d", totalSuccessfulRecords));
                }
            } else {
                LOG.info("Commit " + instantTime + " failed!");
            ...
            LOG.error(
                    "Delta Sync found errors when writing. Errors/Total="
                            + totalErrorRecords
                            + "/"
                            + totalRecords);
                    ...
                    .forEach(
                            ws -> {
                                ...
                                if (ws.getErrors().size() > 0) {
                                    ws.getErrors()
                                            .forEach((key, value) -> LOG.trace("Error for key:" + key + " is " + value));
                                }
                            });
    ...
    /**
      ...
      *
      * <p>Exception will be thrown if it failed in 2 tries.
      *
        ...
        if (useRowWriter) {
            Dataset<Row> df =
                    (Dataset<Row>)
                            inputBatch
                                    .getBatch()
                                    .orElseGet(() -> hoodieSparkContext.getSqlContext().emptyDataFrame());
            HoodieWriteConfig hoodieWriteConfig =
                    prepareHoodieConfigForRowWriter(inputBatch.getSchemaProvider().getTargetSchema());
            BaseDatasetBulkInsertCommitActionExecutor executor =
                    new HoodieStreamerDatasetBulkInsertCommitActionExecutor(
                            hoodieWriteConfig, writeClient, instantTime);
            writeClientWriteResult =
                    new WriteClientWriteResult(
                            executor
                                    .execute(df, !HoodieStreamerUtils.getPartitionColumns(props).isEmpty())
                                    .getWriteStatuses());
        } else {
            ...
            if (cfg.filterDupes) {
                records =
                        DataSourceUtils.dropDuplicates(
                                hoodieSparkContext.jsc(), records, writeClient.getConfig());
            }
            ...
            switch (cfg.operation) {
                case INSERT:
                    writeClientWriteResult =
                            new WriteClientWriteResult(writeClient.insert(records, instantTime));
                    break;
                case UPSERT:
                    writeClientWriteResult =
                            new WriteClientWriteResult(writeClient.upsert(records, instantTime));
                    break;
                case BULK_INSERT:
                    writeClientWriteResult =
                            new WriteClientWriteResult(
                                    writeClient.bulkInsert(
                                            records,
                                            instantTime,
                ...
                case INSERT_OVERWRITE:
                    ...
                    writeClientWriteResult.setPartitionToReplacedFileIds(
                ...
                case DELETE_PARTITION:
                    List<String> partitions =
                            records.map(record -> record.getPartitionPath()).distinct().collect();
    ...
    public void runMetaSync() {
        List<String> syncClientToolClasses =
                Arrays.stream(cfg.syncClientToolClassNames.split(","))
                        .distinct()
        ...
        if (cfg.enableMetaSync) {
            ...
            FileSystem fs =
                    HadoopFSUtils.getFs(cfg.targetBasePath, hoodieSparkContext.hadoopConfiguration());
            ...
            for (String impl : syncClientToolClasses) {
                ...
                try {
                    SyncUtilHelpers.runHoodieMetaSync(
                            impl.trim(), metaProps, conf, fs, cfg.targetBasePath, cfg.baseFileFormat);
    ...
    private void logMetaSync(
            String impl,
            Timer.Context syncContext,
            Map<String, HoodieException> failedMetaSyncs,
            Option<HoodieMetaSyncException> metaSyncException) {
        ...
        if (metaSyncException.isPresent()) {
            LOG.error(
                    "[MetaSync] SyncTool class {} failed with exception {} {}",
                    impl.trim(),
                    metaSyncException.get(),
                    timeString);
    ...
    /**
      * Note that depending on configs and source-type, schemaProvider could either be eagerly or
      * lazily created. SchemaProvider creation is a precursor to HoodieWriteClient and AsyncCompactor
      * creation. This method takes care of this constraint.
      */
    ...
    private void reInitWriteClient(
            Schema sourceSchema, Schema targetSchema, Option<JavaRDD<HoodieRecord>> recordsOpt)
            throws IOException {
        LOG.info("Setting up new Hoodie Write Client");
        if (HoodieStreamerUtils.isDropPartitionColumns(props)) {
            targetSchema =
                    HoodieAvroUtils.removeFields(
                            targetSchema, HoodieStreamerUtils.getPartitionColumns(props));
        }
        ...
        final HoodieWriteConfig writeConfig =
                SparkSampleWritesUtils.getWriteConfigWithRecordSizeEstimate(
                                hoodieSparkContext.jsc(), recordsOpt, initialWriteConfig)
                        .orElse(initialWriteConfig);

        if (writeConfig.isEmbeddedTimelineServerEnabled()) {
            if (!embeddedTimelineService.isPresent()) {
                embeddedTimelineService =
                        EmbeddedTimelineServerHelper.createEmbeddedTimelineService(
                                hoodieSparkContext, writeConfig);
            } else {
                EmbeddedTimelineServerHelper.updateWriteConfigWithTimelineServer(
                        embeddedTimelineService.get(), writeConfig);
            }
        }
        ...
        writeClient =
                new SparkRDDWriteClient<>(hoodieSparkContext, writeConfig, embeddedTimelineService);
    ...
    /**
      ...
      *
      * @param schema initial writer schema. If null or Avro Null type, the schema will be fetched from
      *     previous commit metadata for the table.
      * @param requireSchemaInConfig whether the schema should be present in the config. This is an
      *     optimization to avoid fetching schema from previous commits if not needed.
      * @return Pair of HoodieWriteConfig and writer schema.
      */
    private Pair<HoodieWriteConfig, Schema> getHoodieClientConfigAndWriterSchema(
            Schema schema, boolean requireSchemaInConfig) {
        ...
        // If schema is required in the config, we need to handle the case where the target schema
        // is null and should be fetched from previous commits
        ...
        ValidationUtils.checkArgument(
                config.inlineCompactionEnabled() == cfg.isInlineCompactionEnabled(),
                String.format(
                        "%s should be set to %s", INLINE_COMPACT.key(), cfg.isInlineCompactionEnabled()));
        ValidationUtils.checkArgument(
                config.inlineClusteringEnabled() == clusteringConfig.isInlineClusteringEnabled(),
                String.format(
                        "%s should be set to %s",
                        INLINE_CLUSTERING.key(), clusteringConfig.isInlineClusteringEnabled()));
        ValidationUtils.checkArgument(
                config.isAsyncClusteringEnabled() == clusteringConfig.isAsyncClusteringEnabled(),
                String.format(
                        "%s should be set to %s",
                        ASYNC_CLUSTERING_ENABLE.key(), clusteringConfig.isAsyncClusteringEnabled()));
        ValidationUtils.checkArgument(
                !config.shouldAutoCommit(),
                String.format("%s should be set to %s", AUTO_COMMIT_ENABLE.key(), autoCommit));
        ValidationUtils.checkArgument(
                config.shouldCombineBeforeInsert() == cfg.filterDupes,
        ...
        try {
            ...
            if (targetSchema == null
                    || (SchemaCompatibility.checkReaderWriterCompatibility(
                                                    targetSchema, InputBatch.NULL_SCHEMA)
                                            .getType()
                                    == SchemaCompatibility.SchemaCompatibilityType.COMPATIBLE
                            && SchemaCompatibility.checkReaderWriterCompatibility(
                                                    InputBatch.NULL_SCHEMA, targetSchema)
                                            .getType()
                                    == SchemaCompatibility.SchemaCompatibilityType.COMPATIBLE)) {
    ...
    private void registerAvroSchemas(Schema sourceSchema, Schema targetSchema) {
        ...
        if (!schemas.isEmpty()) {
            ...
            hoodieSparkContext
                    ...
                    .sc()
                    .getConf()
                    .registerAvroSchemas(JavaScalaConverters.convertJavaListToScalaList(schemas).toList());
        }
    }
    ...
    /**
      * Schedule clustering. Called from {@link HoodieStreamer} when async clustering is enabled.
      *
    ...
    class WriteClientWriteResult {
        ...
        public void setPartitionToReplacedFileIds(
                Map<String, List<String>> partitionToReplacedFileIds) {
