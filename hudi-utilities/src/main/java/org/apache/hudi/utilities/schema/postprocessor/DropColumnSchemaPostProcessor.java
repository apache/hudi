...
/**
  ...
  *
  * <p>Multiple columns are separated by commas. For example:
  *
  * <p>properties.put("hoodie.streamer.schemaprovider.schema_post_processor.delete.columns",
  * "column1,column2").
  */
public class DropColumnSchemaPostProcessor extends SchemaPostProcessor {
    ...
    public Schema processSchema(Schema schema) {
        ...
        if (StringUtils.isNullOrEmpty(columnToDeleteStr)) {
            LOG.warn(
                    String.format(
                            "Param %s is null or empty, return original schema",
                            SchemaProviderPostProcessorConfig.DELETE_COLUMN_POST_PROCESSOR_COLUMN.key()));
        }

        // convert field to lowerCase for compare purpose
        Set<String> columnsToDelete =
                Arrays.stream(columnToDeleteStr.split(","))
        ...
        if (targetFields.isEmpty()) {
            throw new HoodieSchemaPostProcessException(
                    "Target schema is empty, you can not remove all columns!");
        }
