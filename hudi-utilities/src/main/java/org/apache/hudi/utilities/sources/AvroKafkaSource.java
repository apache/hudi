...
public class AvroKafkaSource extends KafkaSource<JavaRDD<GenericRecord>> {
    ...
    public AvroKafkaSource(
        ...
        super(
                ...
                new DefaultStreamContext(
                        UtilHelpers.getSchemaProviderForKafkaSource(
                                streamContext.getSchemaProvider(), properties, sparkContext),
        ...
        if (deserializerClassName.equals(ByteArrayDeserializer.class.getName())) {
            if (schemaProvider == null) {
                throw new HoodieReadFromSourceException(
                        "Please provide a valid schema provider class when use ByteArrayDeserializer!");
            }
            ...
            kafkaRDD =
                    KafkaUtils.<String, byte[]>createRDD(
                                    sparkContext,
    ...
    protected JavaRDD<GenericRecord> maybeAppendKafkaOffsets(
            JavaRDD<ConsumerRecord<Object, Object>> kafkaRDD) {
    ...
    private void configureSchemaDeserializer() {
        if (schemaProvider == null) {
            throw new HoodieReadFromSourceException(
                    "SchemaProvider has to be set to use KafkaAvroSchemaDeserializer");
        }
