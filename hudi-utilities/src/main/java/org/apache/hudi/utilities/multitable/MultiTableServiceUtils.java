...
public class MultiTableServiceUtils {
    ...
    public static List<String> getTablesToBeServedFromProps(
            JavaSparkContext jsc, TypedProperties properties) {
    ...
    public static List<String> findHoodieTablesUnderPath(JavaSparkContext jsc, String pathStr) {
        ...
        while (!pathsToList.isEmpty()) {
            ...
            // if current dictionary contains meta folder(.hoodie), add it to result. Otherwise, add
            // it to queue
            List<FileStatus> dirs =
                    dirToFileListing.stream()
                            .flatMap(Arrays::stream)
                            .filter(FileStatus::isDirectory)
                            .collect(Collectors.toList());

            if (!dirs.isEmpty()) {
                List<Pair<FileStatus, DirType>> dirResults =
                        engineContext.map(
                                dirs,
                                fileStatus -> {
    ...
    public static TableServicePipeline buildTableServicePipeline(
        ...
        TypedProperties propsWithTableConfig =
                new TypedProperties(metaClient.getTableConfig().getProps());
