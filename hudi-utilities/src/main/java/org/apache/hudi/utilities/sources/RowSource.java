...
public abstract class RowSource extends Source<Dataset<Row>> {
    ...
    protected abstract Pair<Option<Dataset<Row>>, String> fetchNextBatch(
            Option<String> lastCkptStr, long sourceLimit);

    @Override
    protected final InputBatch<Dataset<Row>> fetchNewData(
            Option<String> lastCkptStr, long sourceLimit) {
                ...
                .map(
                        dsr -> {
                            ...
                            SchemaProvider rowSchemaProvider =
                                    UtilHelpers.createRowBasedSchemaProvider(
                                            sanitizedRows.schema(), props, sparkContext);
