...
/**
  * Avro schemas have restrictions (https://avro.apache.org/docs/current/spec.html#names) on field
  * names that other schema formats do not have. This class provides utilities to help with
  * sanitizing
  */
public class SanitizationUtils {
    ...
    public static String getInvalidCharMask(TypedProperties props) {
        return getStringWithAltKeys(
                props, HoodieStreamerConfig.SCHEMA_FIELD_NAME_INVALID_CHAR_MASK, true);
    }
    ...
    private static StructType sanitizeStructTypeForAvro(
            StructType structType, String invalidCharMask) {
        ...
        for (StructField s : structFields) {
            ...
            StructField structFieldCopy =
                    new StructField(
                            HoodieAvroUtils.sanitizeName(s.name(), invalidCharMask),
                            currFieldDataTypeSanitized,
                            s.nullable(),
                            s.metadata());
    ...
    public static Dataset<Row> sanitizeColumnNamesForAvro(
            Dataset<Row> inputDataset, String invalidCharMask) {
        ...
        for (StructField sf : inputFields) {
            ...
            if (!sanitizedFieldDataType.equals(sf.dataType())) {
                // Sanitizing column names for nested types can be thought of as going from one
                // schema to another
                // which are structurally similar except for actual column names itself. So casting
                // is safe and sufficient.
                targetDataset =
                        targetDataset.withColumn(
                                sf.name(), targetDataset.col(sf.name()).cast(sanitizedFieldDataType));
            }
    ...
    public static Dataset<Row> sanitizeColumnNamesForAvro(
            Dataset<Row> inputDataset, TypedProperties props) {
        return shouldSanitize(props)
                ? sanitizeColumnNamesForAvro(inputDataset, getInvalidCharMask(props))
                : inputDataset;
    }
    ...
    /*
      ...
      */
    public static Schema parseAvroSchema(
            String schemaStr, boolean shouldSanitize, String invalidCharMask) {
        try {
    ...
    /**
      * Parse map for sanitizing. If we have a string in the map, and it is an avro field name key,
      * then we sanitize the name. Otherwise, we keep recursively going through the schema.
      *
      * @param src - deserialized schema
      * @param invalidCharMask - mask to replace invalid characters with
      */
                ...
                .map(
                        kv -> {
                            if (kv.getValue() instanceof List) {
                                return Pair.of(
                                        kv.getKey(), transformList((List<Object>) kv.getValue(), invalidCharMask));
                            } else if (kv.getValue() instanceof Map) {
                                return Pair.of(
                                        kv.getKey(),
                                        transformMap((Map<String, Object>) kv.getValue(), invalidCharMask));
                            } else if (kv.getValue() instanceof String) {
                                String currentStrValue = (String) kv.getValue();
                                if (kv.getKey().equals(AVRO_FIELD_NAME_KEY)) {
                                    return Pair.of(
                                            kv.getKey(), HoodieAvroUtils.sanitizeName(currentStrValue, invalidCharMask));
                                }
    ...
    /**
      * Sanitizes illegal field names in the schema using recursive calls to transformMap and
      * transformList
      */
    private static Option<Schema> parseSanitizedAvroSchemaNoThrow(
            String schemaStr, String invalidCharMask) {
        try {
