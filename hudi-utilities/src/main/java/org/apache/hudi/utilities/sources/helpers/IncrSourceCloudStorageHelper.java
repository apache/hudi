...
public class IncrSourceCloudStorageHelper {
  ...
  private static DataFrameReader getDataFrameReader(SparkSession spark, TypedProperties props, String fileFormat) {
    ...
    if (isNullOrEmpty(props.getString(CloudSourceConfig.SPARK_DATASOURCE_OPTIONS.key(), null))) {
    ...
    try {
      sparkOptionsMap = mapper.readValue(
          props.getString(CloudSourceConfig.SPARK_DATASOURCE_OPTIONS.key()), Map.class);
    } catch (IOException e) {
      throw new HoodieException(String.format("Failed to parse sparkOptions: %s",
          props.getString(CloudSourceConfig.SPARK_DATASOURCE_OPTIONS.key())), e);
    }
