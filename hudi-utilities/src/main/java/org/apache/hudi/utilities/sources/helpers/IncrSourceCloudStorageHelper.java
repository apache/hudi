...
/**
  * Helper methods for when the incremental source is fetching from Cloud Storage, like AWS S3
  * buckets or GCS.
  */
public class IncrSourceCloudStorageHelper {
    ...
    /**
      ...
      */
    public static Option<Dataset<Row>> fetchFileData(
            SparkSession spark, List<String> filepaths, TypedProperties props, String fileFormat) {
    ...
    private static DataFrameReader getDataFrameReader(
            SparkSession spark, TypedProperties props, String fileFormat) {
        ...
        try {
            sparkOptionsMap =
                    mapper.readValue(getStringWithAltKeys(props, SPARK_DATASOURCE_OPTIONS), Map.class);
        } catch (IOException e) {
            throw new HoodieException(
                    String.format(
                            "Failed to parse sparkOptions: %s",
                            getStringWithAltKeys(props, SPARK_DATASOURCE_OPTIONS)),
                    e);
        }
