...
public class HoodieStreamerUtils {
    ...
    /**
      * Generates HoodieRecords for the avro data read from source. Takes care of dropping columns,
      * precombine, auto key generation. Both AVRO and SPARK record types are supported.
      */
    public static Option<JavaRDD<HoodieRecord>> createHoodieRecords(
        ...
        boolean shouldErrorTable =
                errorTableWriter.isPresent()
                        && props.getBoolean(
                                ERROR_ENABLE_VALIDATE_RECORD_CREATION.key(),
                                ERROR_ENABLE_VALIDATE_RECORD_CREATION.defaultValue());
        boolean useConsistentLogicalTimestamp =
        ...
        return avroRDDOptional.map(
                avroRDD -> {
                    SerializableSchema avroSchema = new SerializableSchema(schemaProvider.getTargetSchema());
                    SerializableSchema processedAvroSchema =
                            new SerializableSchema(
                                    isDropPartitionColumns(props)
                                            ? HoodieAvroUtils.removeMetadataFields(avroSchema.get())
                                            : avroSchema.get());
                                        ...
                                        (FlatMapFunction<Iterator<GenericRecord>, Either<HoodieRecord, String>>)
                                                genericRecordIterator -> {
                                                    if (autoGenerateRecordKeys) {
                                                        props.setProperty(
                                                                KeyGenUtils.RECORD_KEY_GEN_PARTITION_ID_CONFIG,
                                                                String.valueOf(TaskContext.getPartitionId()));
                                                        props.setProperty(
                                                                KeyGenUtils.RECORD_KEY_GEN_INSTANT_TIME_CONFIG, instantTime);
                                                    }
                                                    BuiltinKeyGenerator builtinKeyGenerator =
                                                            (BuiltinKeyGenerator)
                                                                    HoodieSparkKeyGeneratorFactory.createKeyGenerator(props);
                                                    return new CloseableMappingIterator<>(
                                                            ClosableIterator.wrap(genericRecordIterator),
                                                            genRec -> {
                                                                try {
                                                                    HoodieKey hoodieKey =
                                                                            new HoodieKey(
                                                                                    builtinKeyGenerator.getRecordKey(genRec),
                                                                                    builtinKeyGenerator.getPartitionPath(genRec));
                                                                    GenericRecord gr =
                                                                            isDropPartitionColumns(props)
                                                                                    ? HoodieAvroUtils.removeFields(genRec, partitionColumns)
                                                                                    : genRec;
                                                                    ...
                                                                    return generateErrorRecordOrThrowException(
                                                                            genRec, e, shouldErrorTable);
                                                                }
                                                            });
                                                });
                                        ...
                                        itr -> {
                                            ...
                                            StructType baseStructType =
                                                    AvroConversionUtils.convertAvroSchemaToStructType(
                                                            processedAvroSchema.get());
                                            StructType targetStructType =
                                                    isDropPartitionColumns(props)
                                                            ? AvroConversionUtils.convertAvroSchemaToStructType(
                                                                    HoodieAvroUtils.removeFields(
                                                                            processedAvroSchema.get(), partitionColumns))
                                                            : baseStructType;
                                            HoodieAvroDeserializer deserializer =
                                                    SparkAdapterSupport$.MODULE$
                                                            .sparkAdapter()
                                                            .createAvroDeserializer(processedAvroSchema.get(), baseStructType);

                                            return new CloseableMappingIterator<>(
                                                    ClosableIterator.wrap(itr),
                                                    rec -> {
                                                        InternalRow row = (InternalRow) deserializer.deserialize(rec).get();
                                                        try {
                                                            String recordKey =
                                                                    builtinKeyGenerator.getRecordKey(row, baseStructType).toString();
                                                            String partitionPath =
                                                                    builtinKeyGenerator
                                                                            .getPartitionPath(row, baseStructType)
                                                                            .toString();
                                                            return Either.left(
                                                                    new HoodieSparkRecord(
                                                                            new HoodieKey(recordKey, partitionPath),
                                                                            HoodieInternalRowUtils.getCachedUnsafeProjection(
                                                                                            baseStructType, targetStructType)
                                                                                    .apply(row),
                                                                            targetStructType,
                                                                            false));
                                                        } catch (Exception e) {
                        ...
                        errorTableWriter
                                ...
                                .addErrorEvents(
                                        records
                                                ...
                                                .map(
                                                        evStr ->
                                                                new ErrorEvent<>(evStr, ErrorEvent.ErrorReason.RECORD_CREATION)));
                    }
    ...
    /**
      ...
      */
    private static Either<HoodieRecord, String> generateErrorRecordOrThrowException(
            GenericRecord genRec, Exception e, boolean shouldErrorTable) {
        if (!shouldErrorTable) {
    ...
    /**
      * Set based on hoodie.datasource.write.drop.partition.columns config. When set to true, will not
      * write the partition columns into the table.
      */
