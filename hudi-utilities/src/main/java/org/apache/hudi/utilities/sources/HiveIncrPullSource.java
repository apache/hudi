...
/**
  * Source to read deltas produced by {@link HiveIncrementalPuller}, commit by commit and apply to
  * the target table
  *
  * <p>The general idea here is to have commits sync across the data pipeline.
  *
  * <p>[Source Tables(s)] ====> HiveIncrementalScanner ==> incrPullRootPath ==> targetTable
  * {c1,c2,c3,...} {c1,c2,c3,...} {c1,c2,c3,...}
  *
  * <p>This produces beautiful causality, that makes data issues in ETLs very easy to debug
  */
public class HiveIncrPullSource extends AvroSource {
    ...
    static class Config {
        @Deprecated
        private static final String ROOT_INPUT_PATH_PROP =
                HiveIncrPullSourceConfig.ROOT_INPUT_PATH.key();
    }

    public HiveIncrPullSource(
        ...
        checkRequiredConfigProperties(
                props, Collections.singletonList(HiveIncrPullSourceConfig.ROOT_INPUT_PATH));
        ...
        try {
            ...
            if (!commitToPull.isPresent()) {
                return new InputBatch<>(
                        Option.empty(), lastCheckpointStr.isPresent() ? lastCheckpointStr.get() : "");
            }

            // read the files out.
            List<FileStatus> commitDeltaFiles =
                    Arrays.asList(fs.listStatus(new Path(incrPullRootPath, commitToPull.get())));
            String pathStr =
                    commitDeltaFiles.stream()
            ...
            return new InputBatch<>(
                    Option.of(avroRDD.keys().map(r -> ((GenericRecord) r.datum()))),
                    String.valueOf(commitToPull.get()));
        } catch (Exception e) {
            throw new HoodieReadFromSourceException(
                    "Unable to read from source from checkpoint: " + lastCheckpointStr, e);
        }
    }
}
