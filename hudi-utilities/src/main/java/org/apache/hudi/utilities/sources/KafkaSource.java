...
public abstract class KafkaSource<T> extends Source<T> {
    ...
    protected InputBatch<T> fetchNewData(Option<String> lastCheckpointStr, long sourceLimit) {
        try {
            return toInputBatch(
                    getOffsetRanges(
                            props, sourceProfileSupplier, offsetGen, metrics, lastCheckpointStr, sourceLimit));
    ...
    public static OffsetRange[] getOffsetRanges(
            TypedProperties props,
        ...
        if (sourceProfileSupplier.isPresent()
            ...
            LOG.info(
                    "About to read maxEventsInSyncRound {} of size {} bytes in {} partitions from Kafka for topic {} with offsetRanges {}",
                    kafkaSourceProfile.getSourceSpecificContext(),
                    kafkaSourceProfile.getMaxSourceBytes(),
                    kafkaSourceProfile.getSourcePartitions(),
            ...
            int minPartitions =
                    (int) getLongWithAltKeys(props, KafkaSourceConfig.KAFKA_SOURCE_MIN_PARTITIONS);
            ...
            LOG.info(
                    "About to read sourceLimit {} in {} spark partitions from kafka for topic {} with offset ranges {}",
                    sourceLimit,
                    minPartitions,
                    offsetGen.getTopicName(),
    ...
    private InputBatch<T> toInputBatch(OffsetRange[] offsetRanges) {
        ...
        if (totalNewMsgs <= 0) {
            ...
            return new InputBatch<>(
                    Option.empty(), KafkaOffsetGen.CheckpointUtils.offsetsToStr(offsetRanges));
        }
        ...
        return new InputBatch<>(
                Option.of(newBatch), KafkaOffsetGen.CheckpointUtils.offsetsToStr(offsetRanges));
    }
