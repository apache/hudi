...
public class PulsarSource extends RowSource implements Closeable {
    ...
    private static final String[] PULSAR_META_FIELDS =
            new String[] {
                "__key", "__topic", "__messageId", "__publishTime", "__eventTime", "__messageProperties"
            };
    ...
    public PulsarSource(
        ...
        checkRequiredConfigProperties(
                props, Arrays.asList(PULSAR_SOURCE_TOPIC_NAME, PULSAR_SOURCE_SERVICE_ENDPOINT_URL));

        // Converting to a descriptor allows us to canonicalize the topic's name properly
        this.topicName =
                TopicName.get(getStringWithAltKeys(props, PULSAR_SOURCE_TOPIC_NAME)).toString();
    ...
    private Pair<MessageId, MessageId> computeOffsets(
            Option<String> lastCheckpointStrOpt, long sourceLimit) {
        ...
        if (endingOffset.compareTo(startingOffset) < 0) {
            String message =
                    String.format(
                            "Ending offset (%s) is preceding starting offset (%s) for '%s'",
                ...
                .orElseGet(
                        () -> {
                            PulsarSourceConfig.OffsetAutoResetStrategy autoResetStrategy =
                                    PulsarSourceConfig.OffsetAutoResetStrategy.valueOf(
    ...
    private void ackOffset(MessageId latestConsumedOffset) {
        try {
            ...
            LOG.error(
                    String.format(
                            "Failed to ack messageId (%s) for topic '%s'", latestConsumedOffset, topicName),
                    e);
    ...
    private Consumer<byte[]> subscribeToTopic() {
        try {
            ...
            String subscriptionId =
                    String.format(HUDI_PULSAR_CONSUMER_ID_FORMAT, System.currentTimeMillis());
            return pulsarClient
                    .get()
                    .newConsumer()
                    .topic(topicName)
    ...
    private PulsarClient initPulsarClient() {
        try {
            return PulsarClient.builder().serviceUrl(serviceEndpointURL).build();
        } catch (PulsarClientException e) {
            LOG.error(
                    String.format("Failed to init Pulsar client connecting to '%s'", serviceEndpointURL), e);
