...
public class ProtoKafkaSource extends KafkaSource<JavaRDD<Message>> {
    ...
    public ProtoKafkaSource(
        ...
        this.deserializerName =
                ConfigUtils.getStringWithAltKeys(
                        props, KafkaSourceConfig.KAFKA_PROTO_VALUE_DESERIALIZER_CLASS, true);
        if (!deserializerName.equals(ByteArrayDeserializer.class.getName())
            ...
            throw new HoodieReadFromSourceException(
                    "Only ByteArrayDeserializer and KafkaProtobufDeserializer are supported for ProtoKafkaSource");
        }
        if (deserializerName.equals(ByteArrayDeserializer.class.getName())) {
    ...
    protected JavaRDD<Message> toBatch(OffsetRange[] offsetRanges) {
        if (deserializerName.equals(ByteArrayDeserializer.class.getName())) {
            ...
            return KafkaUtils.<String, byte[]>createRDD(
                            sparkContext,
                            offsetGen.getKafkaParams(),
                            offsetRanges,
                            LocationStrategies.PreferConsistent())
                    .map(obj -> deserializer.parse(obj.value()));
        } else {
            return KafkaUtils.<String, Message>createRDD(
                            sparkContext,
                            offsetGen.getKafkaParams(),
                            offsetRanges,
                            LocationStrategies.PreferConsistent())
        ...
        private Method getParseMethod() {
            if (parseMethod == null) {
                try {
                    ...
                    throw new HoodieReadFromSourceException(
                            "Unable to get proto parsing method from specified class: " + className, ex);
                }
            }
            return parseMethod;
        }
