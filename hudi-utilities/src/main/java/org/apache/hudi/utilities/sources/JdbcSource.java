...
public class JdbcSource extends RowSource {
    ...
    /**
      ...
      */
    private static DataFrameReader validatePropsAndGetDataFrameReader(
            final SparkSession session, final TypedProperties properties) throws HoodieException {
        ...
        try {
            ...
            if (containsConfigProperty(properties, JdbcSourceConfig.PASSWORD)) {
                ...
                dataFrameReader =
                    ...
                    && !StringUtils.isNullOrEmpty(
                            getStringWithAltKeys(properties, JdbcSourceConfig.PASSWORD_FILE))) {
                LOG.info(
                        String.format(
                                "Reading JDBC password from password file %s",
                                getStringWithAltKeys(properties, JdbcSourceConfig.PASSWORD_FILE)));
                FileSystem fileSystem = FileSystem.get(session.sparkContext().hadoopConfiguration());
                passwordFileStream =
                        fileSystem.open(
                                new Path(getStringWithAltKeys(properties, JdbcSourceConfig.PASSWORD_FILE)));
                ...
                throw new IllegalArgumentException(
                        String.format(
                                "JDBCSource needs either a %s or %s to connect to RDBMS " + "datasource",
                                JdbcSourceConfig.PASSWORD_FILE.key(), JdbcSourceConfig.PASSWORD.key()));
            }
            ...
            if (getBooleanWithAltKeys(properties, JdbcSourceConfig.IS_INCREMENTAL)) {
                checkRequiredConfigProperties(
                        properties, Collections.singletonList(JdbcSourceConfig.INCREMENTAL_COLUMN));
            }
    ...
    /**
      * Accepts spark JDBC options from the user in terms of EXTRA_OPTIONS adds them to {@link
      * DataFrameReader} Example: In a normal spark code you would do something like:
      * session.read.format('jdbc') .option(fetchSize,1000) .option(timestampFormat,"yyyy-mm-dd
      * hh:mm:ss")
      *
      * <p>The way to pass these properties to HUDI is through the config file. Any property starting
      * with hoodie.streamer.jdbc.extra.options. will be added.
      *
      * <p>Example: hoodie.streamer.jdbc.extra.options.fetchSize=100
      ...
      */
    private static void addExtraJdbcOptions(
            TypedProperties properties, DataFrameReader dataFrameReader) {
    ...
    protected Pair<Option<Dataset<Row>>, String> fetchNextBatch(
            Option<String> lastCkptStr, long sourceLimit) throws HoodieException {
        try {
            checkRequiredConfigProperties(
                    props,
                    Arrays.asList(
                            JdbcSourceConfig.URL,
                            JdbcSourceConfig.DRIVER_CLASS,
                            JdbcSourceConfig.USER,
                            JdbcSourceConfig.RDBMS_TABLE_NAME,
                            JdbcSourceConfig.IS_INCREMENTAL));
            ...
            throw new HoodieException(
                    "Error fetching next batch from JDBC source. Last checkpoint: "
                            + lastCkptStr.orElse(null),
                    e);
        }
    }

    /**
      * Decide to do a full RDBMS table scan or an incremental scan based on the lastCkptStr. If
      * previous checkpoint value exists then we do an incremental scan with a PPD query or else we do
      * a full scan. In certain cases where the incremental query fails, we fallback to a full scan.
      *
    ...
    private Pair<Option<Dataset<Row>>, String> fetch(Option<String> lastCkptStr, long sourceLimit) {
        ...
        Pair<Option<Dataset<Row>>, String> pair =
                Pair.of(Option.of(dataset), checkpoint(dataset, isIncremental, lastCkptStr));
        dataset.unpersist();
        return pair;
    }
    ...
    /**
      ...
      *
      * @param lastCheckpoint Last checkpoint. Note that the records fetched will be exclusive of the
      *     last checkpoint (i.e. incremental column value > lastCheckpoint).
    ...
    private Dataset<Row> incrementalFetch(Option<String> lastCheckpoint, long sourceLimit) {
        try {
            ...
            if (sourceLimit > 0) {
                URI jdbcURI =
                        URI.create(
                                getStringWithAltKeys(props, JdbcSourceConfig.URL)
                                        .substring(URI_JDBC_PREFIX.length()));
                if (DB_LIMIT_CLAUSE.contains(jdbcURI.getScheme())) {
                    queryBuilder
                            .orderBy(getStringWithAltKeys(props, JdbcSourceConfig.INCREMENTAL_COLUMN))
                            .limit(sourceLimit);
                }
            }
            ...
            LOG.info(
                    String.format(
                            "Referenced last checkpoint and prepared new predicate pushdown query for jdbc pull %s",
                            query));
            return validatePropsAndGetDataFrameReader(sparkSession, props)
                    .option(Config.RDBMS_TABLE_PROP, query)
                    .load();
        } catch (Exception e) {
            LOG.error(
                    "Error while performing an incremental fetch. Not all database support the PPD query we generate to do an incremental scan",
                    e);
    ...
    private String checkpoint(
            Dataset<Row> rowDataset, boolean isIncremental, Option<String> lastCkptStr) {
        try {
            if (isIncremental) {
                Column incrementalColumn =
                        rowDataset.col(getStringWithAltKeys(props, JdbcSourceConfig.INCREMENTAL_COLUMN));
                final String max =
                        rowDataset
                ...
                return lastCkptStr.isPresent() && !StringUtils.isNullOrEmpty(lastCkptStr.get())
                        ? lastCkptStr.get()
            ...
            throw new HoodieReadFromSourceException(
                    "Failed to checkpoint. Last checkpoint: " + lastCkptStr.orElse(null), e);
        }
    }
