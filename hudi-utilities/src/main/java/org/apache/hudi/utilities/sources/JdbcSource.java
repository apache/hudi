...
public class JdbcSource extends RowSource {
  ...
  private static DataFrameReader validatePropsAndGetDataFrameReader(final SparkSession session,
    ...
    try {
      ...
      dataFrameReader = dataFrameReader.option(JdbcSourceConfig.URL_PROP, properties.getString(JdbcSourceConfig.URL.key()));
      dataFrameReader = dataFrameReader.option(JdbcSourceConfig.USER_PROP, properties.getString(JdbcSourceConfig.USER.key()));
      dataFrameReader = dataFrameReader.option(JdbcSourceConfig.DRIVER_PROP, properties.getString(JdbcSourceConfig.DRIVER_CLASS.key()));
      dataFrameReader = dataFrameReader
          .option(JdbcSourceConfig.RDBMS_TABLE_PROP, properties.getString(JdbcSourceConfig.RDBMS_TABLE_NAME.key()));

      if (properties.containsKey(JdbcSourceConfig.PASSWORD.key())) {
        LOG.info("Reading JDBC password from properties file....");
        dataFrameReader = dataFrameReader.option(JdbcSourceConfig.PASSWORD_PROP, properties.getString(JdbcSourceConfig.PASSWORD.key()));
        ...
        LOG.info(String.format("Reading JDBC password from password file %s",
            properties.getString(JdbcSourceConfig.PASSWORD_FILE.key())));
        ...
        dataFrameReader = dataFrameReader.option(JdbcSourceConfig.PASSWORD_PROP, new String(bytes));
      } else {
  ...
  protected Pair<Option<Dataset<Row>>, String> fetchNextBatch(Option<String> lastCkptStr, long sourceLimit) throws HoodieException {
    try {
      DataSourceUtils.checkRequiredProperties(props, Arrays.asList(
          JdbcSourceConfig.URL.key(), JdbcSourceConfig.DRIVER_CLASS.key(),
  ...
  private Dataset<Row> incrementalFetch(Option<String> lastCheckpoint, long sourceLimit) {
    try {
      ...
      return validatePropsAndGetDataFrameReader(sparkSession, props).option(JdbcSourceConfig.RDBMS_TABLE_PROP, query).load();
    } catch (Exception e) {
