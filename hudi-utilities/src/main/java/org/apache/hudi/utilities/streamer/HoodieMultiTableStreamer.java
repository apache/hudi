...
/**
  * Wrapper over HoodieStreamer.java class. Helps with ingesting incremental data into hoodie
  * datasets for multiple tables. Supports COPY_ON_WRITE and MERGE_ON_READ storage types.
  */
public class HoodieMultiTableStreamer {
    ...
    public HoodieMultiTableStreamer(Config config, JavaSparkContext jssc) throws IOException {
        ...
        ValidationUtils.checkArgument(
                !config.filterDupes || config.operation != WriteOperationType.UPSERT,
        ...
        configFolder =
                configFolder.charAt(configFolder.length() - 1) == '/'
                        ? configFolder.substring(0, configFolder.length() - 1)
                        : configFolder;
        checkIfPropsFileAndConfigFolderExist(commonPropsFile, configFolder, fs);
        TypedProperties commonProperties =
                UtilHelpers.readConfig(fs.getConf(), new Path(commonPropsFile), new ArrayList<String>())
                        .getProps();
        // get the tables to be ingested and their corresponding config files from this properties
        // instance
        populateTableExecutionContextList(commonProperties, configFolder, fs, config);
    }

    private void checkIfPropsFileAndConfigFolderExist(
            String commonPropsFile, String configFolder, FileSystem fs) throws IOException {
    ...
    private void checkIfTableConfigFileExists(
            String configFolder, FileSystem fs, String configFilePath) throws IOException {
    ...
    private void populateTableExecutionContextList(
            TypedProperties properties, String configFolder, FileSystem fs, Config config)
            throws IOException {
        ...
        for (String table : tablesToBeIngested) {
            ...
            String configProp =
                    HoodieStreamerConfig.INGESTION_PREFIX
                            + database
                            + Constants.DELIMITER
                            + currentTable
                            + Constants.INGESTION_CONFIG_SUFFIX;
            String oldConfigProp =
                    HoodieStreamerConfig.OLD_INGESTION_PREFIX
                            + database
                            + Constants.DELIMITER
                            + currentTable
                            + Constants.INGESTION_CONFIG_SUFFIX;
            ...
            TypedProperties tableProperties =
                    UtilHelpers.readConfig(fs.getConf(), new Path(configFilePath), new ArrayList<>())
                            .getProps();
            properties.forEach(
                    (k, v) -> {
            ...
            String overriddenTargetBasePath =
                    getStringWithAltKeys(tableProperties, HoodieStreamerConfig.TARGET_BASE_PATH, true);
            cfg.targetBasePath =
                    StringUtils.isNullOrEmpty(overriddenTargetBasePath)
                            ? targetBasePath
                            : overriddenTargetBasePath;
            if (cfg.enableMetaSync
                    && StringUtils.isNullOrEmpty(
                            tableProperties.getString(HoodieSyncConfig.META_SYNC_TABLE_NAME.key(), ""))) {
    ...
    private void populateTransformerProps(
            HoodieStreamer.Config cfg, TypedProperties typedProperties) {
    ...
    private List<String> getTablesToBeIngested(TypedProperties properties) {
        String combinedTablesString =
                getStringWithAltKeys(properties, HoodieStreamerConfig.TABLES_TO_BE_INGESTED);
    ...
    private void populateSchemaProviderProps(
            HoodieStreamer.Config cfg, TypedProperties typedProperties) {
    ...
    private void populateTargetRegistryProp(TypedProperties typedProperties) {
        String schemaRegistryTargetUrl =
                getStringWithAltKeys(typedProperties, TARGET_SCHEMA_REGISTRY_URL, true);
    ...
    private void populateSourceRegistryProp(TypedProperties typedProperties) {
        String schemaRegistrySourceUrl =
                getStringWithAltKeys(typedProperties, SRC_SCHEMA_REGISTRY_URL, true);
        if (StringUtils.isNullOrEmpty(schemaRegistrySourceUrl)) {
            ...
            if (StringUtils.isNullOrEmpty(schemaRegistrySuffix)) {
                sourceSchemaRegistrySuffix =
                        getStringWithAltKeys(typedProperties, SCHEMA_REGISTRY_SOURCE_URL_SUFFIX);
            } else {
                sourceSchemaRegistrySuffix = schemaRegistrySuffix;
            }
            typedProperties.setProperty(
                    SRC_SCHEMA_REGISTRY_URL.key(),
                    schemaRegistryBaseUrl
                            + getStringWithAltKeys(typedProperties, HoodieStreamerConfig.KAFKA_TOPIC)
                            + sourceSchemaRegistrySuffix);
        }
    ...
    public static class Helpers {
        ...
        static String getDefaultConfigFilePath(
                String configFolder, String database, String currentTable) {
            return configFolder
                    + Constants.FILE_DELIMITER
                    + database
                    + Constants.UNDERSCORE
                    + currentTable
                    + Constants.DEFAULT_CONFIG_FILE_NAME_SUFFIX;
        }
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (config.targetTableName != null) {
            logger.warn(
                    String.format(
                            "--target-table is deprecated and will be removed in a future release due to it's useless;"
                                    + " please use %s to configure multiple target tables",
                            HoodieStreamerConfig.TABLES_TO_BE_INGESTED.key()));
        }

        JavaSparkContext jssc =
                UtilHelpers.buildSparkContext("multi-table-streamer", Constants.LOCAL_SPARK_MASTER);
        try {
    ...
    public static class Config implements Serializable {
        ...
        @Parameter(
                names = {"--config-folder"},
                description = "Path to folder which contains all the properties file",
                required = true)
    ...
    private static String resetTarget(Config configuration, String database, String tableName) {
        ...
        basePathPrefix =
                basePathPrefix.charAt(basePathPrefix.length() - 1) == '/'
                        ? basePathPrefix.substring(0, basePathPrefix.length() - 1)
                        : basePathPrefix;
        String targetBasePath =
                basePathPrefix + Constants.FILE_DELIMITER + database + Constants.FILE_DELIMITER + tableName;
    ...
    public void sync() {
        for (TableExecutionContext context : tableExecutionContexts) {
            try {
                new HoodieStreamer(context.getConfig(), jssc, Option.ofNullable(context.getProperties()))
                        .sync();
                successTables.add(Helpers.getTableWithDatabase(context));
            } catch (Exception e) {
                logger.error(
                        "error while running MultiTableDeltaStreamer for table: " + context.getTableName(), e);
