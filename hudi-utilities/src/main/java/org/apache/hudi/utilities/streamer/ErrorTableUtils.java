...
public final class ErrorTableUtils {
    public static Option<BaseErrorTableWriter> getErrorTableWriter(
        ...
        ValidationUtils.checkState(
                !StringUtils.isNullOrEmpty(errorTableWriterClass),
                "Missing error table config " + ERROR_TABLE_WRITE_CLASS);

        Class<?>[] argClassArr =
                new Class[] {
                    HoodieStreamer.Config.class,
                    SparkSession.class,
        ...
        String errMsg =
                "Unable to instantiate ErrorTableWriter with arguments type "
    ...
    public static HoodieErrorTableConfig.ErrorWriteFailureStrategy getErrorWriteFailureStrategy(
        ...
        String writeFailureStrategy =
                props.getString(
                        ERROR_TABLE_WRITE_FAILURE_STRATEGY.key(),
    ...
    public static void validate(Dataset<Row> dataset) {
        if (!isErrorTableCorruptRecordColumnPresent(dataset)) {
            throw new HoodieValidationException(
                    String.format(
                            "Invalid condition, columnName=%s "
                                    + "is not present in transformer "
                                    + "output schema",
    ...
    private static boolean isErrorTableCorruptRecordColumnPresent(Dataset<Row> dataset) {
        return Arrays.stream(dataset.columns())
                .anyMatch(col -> col.equals(ERROR_TABLE_CURRUPT_RECORD_COL_NAME));
    }
}
