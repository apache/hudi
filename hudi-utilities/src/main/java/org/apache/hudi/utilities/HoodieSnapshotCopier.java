...
public class HoodieSnapshotCopier implements Serializable {
    ...
    static class Config implements Serializable {
        ...
        @Parameter(
                names = {"--base-path", "-bp"},
                description = "Hoodie table base path",
                required = true)
        String basePath = null;

        @Parameter(
                names = {"--output-path", "-op"},
                description = "The snapshot output path",
                required = true)
    ...
    public void snapshot(
        ...
        if (partitions.size() > 0) {
            ...
            if (fs.exists(outputPath)) {
                LOG.warn(
                        String.format(
                                "The output path %s targetBasePath already exists, deleting", outputPath));
                fs.delete(new Path(outputDir), true);
            }

            context.setJobStatus(this.getClass().getSimpleName(), "Creating a snapshot: " + baseDir);

            List<Tuple2<String, String>> filesToCopy =
                    context.flatMap(
                            partitions,
                            partition -> {
                                ...
                                StoragePath partitionMetaFile =
                                        HoodiePartitionMetadata.getPartitionMetafilePath(
                                                        storage1, FSUtils.constructAbsolutePath(baseDir, partition))
                                                .get();
            ...
            context.foreach(
                    ...
                    tuple -> {
                        ...
                        FileSystem ifs =
                                HadoopFSUtils.getFs(baseDir, storageConf.unwrapCopyAs(Configuration.class));
            ...
            FileStatus[] commitFilesToCopy =
                    Arrays.stream(
                                    fs.listStatus(new Path(baseDir + "/" + HoodieTableMetaClient.METAFOLDER_NAME)))
                            .filter(
                                    fileStatus -> {
                                        ...
                                        if (path.getName().equals(HoodieTableConfig.HOODIE_PROPERTIES_FILE)) {
                                            ...
                                            return HoodieTimeline.compareTimestamps(
                                                    instantTime, HoodieTimeline.LESSER_THAN_OR_EQUALS, latestCommitTimestamp);
                                        }
                                    })
            ...
            for (FileStatus commitStatus : commitFilesToCopy) {
                ...
                if (fs.exists(targetFilePath)) {
                    LOG.error(
                            String.format(
                                    "The target output commit file (%s targetBasePath) already exists.",
                                    targetFilePath));
                }
    ...
    public static void main(String[] args) throws IOException {
        ...
        LOG.info(
                String.format(
                        "Snapshot hoodie table from %s (source) to %s (target)", cfg.basePath, cfg.outputPath));
