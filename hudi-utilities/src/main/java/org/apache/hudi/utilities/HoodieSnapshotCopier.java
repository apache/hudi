  ...
  public void snapshot(JavaSparkContext jsc, String baseDir, final String outputDir,
    ...
    if (partitions.size() > 0) {
      ...
      List<Tuple2<String, String>> filesToCopy = context.flatMap(partitions, partition -> {
        ...
        HoodieStorage storage1 = HoodieStorageUtils.getHoodieStorage(baseDir, serConf.newCopy());
        ...
        HoodieLocation partitionMetaFile = HoodiePartitionMetadata.getPartitionMetafilePath(storage1,
      ...
      context.foreach(filesToCopy, tuple -> {
        ...
        Path toPartitionPath = FSUtils.getPartitionPathInPath(outputDir, partition);
