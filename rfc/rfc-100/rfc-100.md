...
At the end of milestone 1, the user will be able to store blobs as references to files and read them back through Spark dataframes or SQL. If the blob is marked as managed, the cleaner will clean it up when all references to that blob are removed.
