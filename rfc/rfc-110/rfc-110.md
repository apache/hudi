...
and predicates, so Phase 1 performs real indexed reads rather than always taking
...
5. Analyze Hudi records and build bounded-memory sorted runs using the selected
   implementation.
...
Five versions are independent and explicit:
...
4. analyzer implementation/fingerprint version; and
5. Spark predicate and Hudi RS query-object API version.
...
- Finalize Avro descriptors, analyzer canonicalization, query objects, and the
  language-neutral index-file format.
...
- Add the pure-Java index-file builder, reader, and analyzer conformance suite.
