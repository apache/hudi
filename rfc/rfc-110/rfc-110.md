...
A new `TextSearchIndexer` table service builds immutable index files from
completed Hudi file slices. MDT stores the descriptors for those files, not the
term dictionaries and posting lists themselves. At query time, Hudi uses an
index segment only when it exactly covers the file slice in the requested
snapshot. Other slices are read through the normal Hudi path, so index
maintenance can run separately from ingestion while complete queries still see
the full snapshot.
...
1. MDT records control which auxiliary text-index files are visible; MDT file
   groups do not store the text-index contents.
2. Coverage is recorded per data table file slice and data instant.
3. Coverage is invalidated by an exact file-slice identity mismatch, without an
   inline text-index metadata update from the ingestion writer.
4. A slice without usable coverage is read through the normal Hudi path.
5. Published text-index files are immutable.
6. Ranking does not depend on segment layout.
7. MDT records and text-index files are versioned independently.

### Goals

- Match token, boolean, prefix, fuzzy, phrase, and multi-column queries.
- Support copy-on-write (COW) and merge-on-read (MOR) tables.
- Build asynchronously and incrementally through a separately scheduled MDT
  table service for bootstrap, catch-up, consolidation, rebuild, repair, and
  cleanup.
...
| `TextSearchIndexer` | The separately scheduled MDT table service that bootstraps the index and performs incremental catch-up, consolidation, rebuild, repair, and cleanup. |
...
| Fallback scan set | Data table file slices without compatible index coverage for the pinned snapshot, including changed slices awaiting catch-up and slices affected by bootstrap, rebuild, or recovery. |
...
API. `CREATE INDEX` stores the definition and schedules the separately executed
table service; it does not build the index inside the ingestion job. A Hudi RS
`create_text_index` API can be added when Hudi RS supports the required MDT
writes.

### Hudi architecture

`CREATE INDEX` writes a `HoodieIndexDefinition` and creates the dynamic MDT
partition `text_index_<name>`. The separately scheduled `TextSearchIndexer`
reads merged Hudi file slices, writes immutable index files to auxiliary
storage, and publishes their descriptors and coverage to MDT. A `SEGMENT`
record describes an auxiliary file set; it does not contain the segment data.
...
Ingestion does not update the text index. A new MOR log, COW replacement,
compaction, or clustering result changes the identity of the affected slice.
Until the table service catches up, the planner reads that slice normally. The
identity mismatch itself invalidates coverage; the writer does not need to
...
flowchart LR
    ...
    INDEXER["TextSearchIndexer<br/>bootstrap and incremental maintenance"]
...
expression-index code. `IndexerFactory` creates a `TextSearchIndexer` using the
separately scheduled `BaseIndexer` lifecycle.
...
incremental catch-up enumerate file slices from a pinned table snapshot and read
their merged records directly, so index construction does not use RLI either.
...
statistics. Catch-up can be scheduled by elapsed time, changed-slice count, or
...
Main Hudi owns Spark SQL planning, the MDT table service, the Java JNI adapter
and loader, `HoodieIndexDefinition`, `HoodieTextIndexInfo`, and the file-format
and JNI specifications. During incubation, it also contains the Rust crate and
conformance tests. After migration, Hudi RS owns the Rust source, native
artifact publication, and Python bindings; main Hudi consumes a pinned artifact.
...
`CREATE INDEX` schedules `TextSearchIndexer` as a separate table service.
Bootstrap and incremental catch-up use the same steps:

1. Start a text-index table-service instant and heartbeat for a unique build
   UUID.
2. Pin a completed data-table instant and matching MDT snapshot.
3. Enumerate file slices and select those without exact compatible coverage.
4. Use Hudi's merged reader to send stable record keys, indexed text,
   file-slice ordinals, and optional row-position hints to the native builder.
5. Build bounded-memory sorted runs and write immutable text-index files to
   UUID-scoped auxiliary paths.
6. Finalize checksums, statistics, and exact file-slice descriptors.
7. Revalidate the pinned instant, index definition, analyzer, and every final
   file.
8. Commit `SEGMENT`, `COVERAGE`, and `HEAD` records to MDT through the indexing
   transaction.

Bootstrap changes `HEAD` to `ACTIVE` after covering the pinned snapshot. Later
catch-up commits advance its publication instant.

The data writer neither waits for this service nor loads the native builder. A
data commit may make slices uncovered, and complete queries scan them until a
later table-service commit restores coverage. The exact identity check prevents
an old segment from being used for a changed slice.
...
`S`. Catch-up may group slices from several file groups into target-sized
segments. It publishes only after validating every file and descriptor in the
build.

The table-service heartbeat protects unpublished files from orphan cleanup.
...
If `DROP INDEX`, a definition change, or overlapping maintenance wins the race,
the build reloads current state and retries or aborts.
...
```mermaid
sequenceDiagram
    participant I as TextSearchIndexer
    ...
    I->>D: Pin completed data instant S
    ...
    I->>D: Revalidate S and index definition
    I->>S: Verify every final file and checksum
    I->>M: OCC commit HEAD, SEGMENT, and COVERAGE
    alt Validation or OCC fails
        M-->>I: Reject stale publication
        I->>I: Reload state, revalidate, and retry or abort
    else Publication succeeds
        M-->>I: MDT maintenance commit completes
    end
...
The table service consolidates small segments based on segment count, total
bytes, and the share of slices that no longer match a retained snapshot. It
copies still-covered records into new immutable files and swaps descriptors in
one MDT maintenance commit. Tombstoned segments remain available to readers
pinned before that commit.
...
unreachable. Unpublished table-service files also require an expired heartbeat
and the orphan grace period. `DROP INDEX` removes visibility first and deletes
...
This can dominate latency during bootstrap, catch-up, rebuild, or recovery, so
...
work, and skipped posting blocks. Table-service metrics include analyzed bytes,
...
- Add `TEXT_INDEX`, `TextSearchIndexer`, MDT records, separately scheduled
  bootstrap, and incremental catch-up for changed file slices.
...
they may simply leave changed slices uncovered until catch-up. Dropping an index
...
- COW and MOR bootstrap and catch-up after inserts, updates, deletes, log writes,
  compaction, clustering, rollback, restore, clean, drop, and recreate.
...
- Orphan, partial upload, retry, duplicate build, and tombstone cleanup cases,
  including a slow live table-service build protected by heartbeats, an expired
  build becoming collectable, and a missing final file aborting publication.
- Ingestion racing with bootstrap or catch-up publication, maintenance operations
...
- Table-service throughput, peak JVM/native/Hudi RS memory, JNI batch overhead,
...
- Complete-query latency across bootstrap coverage, catch-up delay, MOR log
...
production measurements show that fallback scans and scheduled catch-up cannot
...
key, while a separately scheduled table service would consolidate long chains
into replacement base segments. Compaction and clustering would publish
replacement coverage for their rewritten slices.
...
- measured fallback latency or catch-up lag violates a concrete search
  freshness objective;
...
If scheduled catch-up meets the freshness target and fallback remains
