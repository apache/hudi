...
The design keeps Hudi responsible for record materialization, time travel,
rollback, cleaning, and access filters. The text index accelerates discovery of
matching Hudi records without replacing the table's normal read path. This is
the central value of the RFC: applications gain search-oriented access while
retaining Hudi's table semantics and operational model.
...
The proposal uses established inverted-index and relevance-ranking techniques,
but defines them as a Hudi index: Hudi owns index creation, file-slice coverage,
publication, query planning, and cleanup. The references provide background on
full-text search; they are not dependencies or API specifications for this
design.
