...
Python API. Index creation and maintenance use Hudi's metadata table (MDT)
indexing lifecycle.

`CREATE INDEX` stores the text index definition in `HoodieIndexDefinition`. A
`TextSearchIndexer` table service reads Hudi file slices, builds immutable
text-index files, and publishes their descriptors and file-slice coverage in a
dynamic MDT partition. During query planning, exact coverage determines which
file slices use the text index and which use the normal Hudi reader.

This RFC defines the user APIs, MDT records, text-index file format, build and
query flows, lifecycle operations, and implementation plan.

## Background

Hudi indexes currently answer questions such as which files might contain a
record key or a value range. A text index analyzes a string column into terms,
maps each term to matching Hudi record keys, optionally records token positions
for phrase matching, and stores statistics for ranked search.

A posting is one entry connecting a term to a matching Hudi record. A posting
list is the ordered collection of those entries for one term. A text-index file
...
until it catches up. A Hudi text query instead checks index coverage against the
selected data file slices and scans any changed slice, so it does not read stale
postings for the deleted record.

The same lifecycle enables time-travel search. A query `AS OF` an older instant
selects text-index files for that snapshot when retained and falls back to the
retained table files otherwise.
...
Established text indexes use a term dictionary, posting lists, token positions,
corpus statistics, and immutable segments. This RFC uses those standard data
structures, then defines how Hudi builds, publishes, selects, and cleans them.
The references provide background on inverted indexes and full-text query APIs;
they are not dependencies or API specifications for this design.

### Design principles

1. Text-index visibility follows Hudi data and MDT instants.
2. Every accelerated data table file slice has an exact coverage record.
3. Uncovered or incompatible file slices use the normal Hudi reader.
4. Published text-index files are immutable and visible only through MDT records.
5. Ranking is independent of how the index is physically partitioned.
6. Persisted MDT records and text-index files are explicitly versioned.
...
In this RFC, **SQL** means Spark SQL and the **direct API** means Hudi RS Python.
Both expose the same text-query behavior. The engine and language integration
are described after the Hudi storage and query design.
...
requires a new `HoodieIndexVersion`. The text-index file format version and
analyzer version remain separate because either can evolve without changing the
Hudi index definition.
...
not imply relevance order. Ranked top-k is a separate direct-search API.
...
For search applications, a fluent API exposes ranked top-k and an explicit
score:
...
Spark SQL and `read_snapshot` return every matching row. `search_text` returns
BM25-ranked rows with `_score`.

### Hudi architecture

The text index uses existing Hudi components as follows:

1. `CREATE INDEX` writes a `HoodieIndexDefinition` and creates a dynamic
   `text_index_<name>` MDT partition.
2. `TextSearchIndexer`, implemented as a Hudi MDT table service, reads merged
   records from selected data table file slices.
3. The indexer writes immutable text-index files and publishes `SEGMENT`,
   `COVERAGE`, and `HEAD` records in one MDT commit.
4. A query pins data and MDT snapshots, enumerates the eligible data file
   slices, and resolves exact coverage through MDT point lookups.
5. Covered slices use text-index postings. Other slices use the normal Hudi
   reader. Both paths produce Hudi record addresses that are materialized by
   partition path and file ID.

The build and publication path is:
...
The SQL definition is stored in `.hoodie/.index/index.json`. Text-index files
live under an MDT-owned auxiliary namespace. Readers do not determine visibility
by listing that namespace; they use descriptors visible in the selected MDT
snapshot.
...
UUID, text-index file format version, file-slice identities, file-slice-ordinal mapping,
...
A future external text-index storage tier may be configured, but every path must be scoped
...
Every field is defined by the text-index file format in this RFC, so upgrading a
Java or Rust dependency cannot silently change files.
...
and the text-index file format. The Rust crate is the only complete
...
crate beside the Spark and MDT integration allows the native API, text-index
file format, and lifecycle to evolve atomically without coordinating unreleased
changes across two repositories. After the native interface meets the migration
criteria below, the crate moves to Hudi RS. Main Hudi then replaces its source
dependency with a pinned Hudi RS native artifact while preserving the
Java-facing API and text-index file format:
...
| Text-index file format and JNI specifications | No separate Java engine | Python query objects and bindings |
...
The compatibility boundaries are the analyzer fingerprint, query AST semantics,
text-index file format version, segment descriptor schema, record-address
encoding, completeness rules, and JNI ABI version. A segment written before
migration must remain readable afterward without rebuilding.
...
main Hudi keeps the same loader interface and pins a compatible Hudi RS release.
...
Visibility begins at step 12. An indexer retry may reuse a text-index file set
...
verifies that all final text-index files still match their recorded lengths and
...
configuration. The expected index-definition version and dynamic MDT partition
state are publication preconditions, so `DROP INDEX` or a definition change
conflicts with an in-flight publication. Every OCC retry restarts at step 10:
the indexer reloads the latest data and MDT state, revalidates `S`, the index
definition, analyzer fingerprint, partition state, and final text-index files,
then merges candidate segments in data-instant order. If any precondition no
longer holds, the build aborts, stops its heartbeat, and leaves its unpublished
files for orphan cleanup.

`HEAD` advancement is monotonic: an older build may remain a time-travel
candidate but cannot replace a newer head or discard newer coverage. These rules
cover concurrent data writers, index creation or deletion, and concurrent
text-index table services without introducing a separate lock protocol.
...
If text-index writing fails, no MDT descriptor is committed and readers cannot
discover the orphan. If the MDT commit fails, retry validation may reuse the
text-index files; otherwise the orphan cleaner removes them after the heartbeat
expires and the grace interval elapses.
...
requires the same analyzer fingerprint, writes new text-index files, publishes
...
The cleaner deletes tombstoned text-index files only after both data and MDT
retention guarantee that no supported query can see the descriptor. Unpublished
text-index files are deleted only when their build has no active heartbeat and
the separate grace period has elapsed. `DROP INDEX` first removes visibility
and emits tombstones, then cleans asynchronously.
...
text-index file format does not depend on a particular companion index.
...
segment count, coverage ratio, index catch-up lag, fallback-scan bytes, index
...
Readers may accept older text-index file format versions through dedicated decoders. Writers
...
### Phase 0: design and native interfaces
...
the feature leaves table data readable. Text-index file format upgrades are
side-by-side rebuilds followed by descriptor switchover and cleanup.

## Test Plan

### Text-index file format, JNI, and migration compatibility tests
