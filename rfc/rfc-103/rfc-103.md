...
- **Layout sorted by keys within each file group, that can be faster for point access**

## Goal

This RFC proposes applying LSM-inspired principles (**sorted writes + tiered N-way merges**) to improve the data organization protocol for **Hudi MOR tables**, and favoring **Parquet** over **Avro** as the on-disk format for individual log files, to achieve:
