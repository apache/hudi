...
The layout should be enforced by a table property, for e.g. `hoodie.table.lsm_tree.layout.enabled=true` (default `false`):

- The config is not allowed to be set to `true` for an existing table
- The config is allowed to be set to `false` for an existing table
...
- `hoodie.table.lsm_tree.layout.enabled=true`
