...
| `M\|<generation>` | generations | Generation state, vector schema, bootstrap baseline, chunk counts/checksums, and quantizer metadata. |
...
M|<gen>           -> generation metadata + vector schema + chunk integrity metadata
...
completed data writes in timeline order outside the coordination lock. Each replay writes the
new generation's deltas and source-instant marker atomically. When the gap is bounded,
...
The Record Level Index (RLI) is a hard prerequisite: `CREATE INDEX` fails unless RLI is
active, and RLI cannot be disabled or dropped while a vector index exists. Posting locators
are hints; finalist keys are validated by batched RLI lookup on the pinned snapshot. Moved
keys are re-resolved and deleted keys are removed.
...
a marker and no earlier data-write instant remains inflight. At planning, the effective
frontier is derived from the active data-write timeline and marker coverage; it is contiguous,
never the maximum marker observed. A frontier behind the pinned data-write instant invokes
`FAIL`, `WARN`, or exact `FALLBACK`, and catch-up replay repairs the gap by writing missing
deltas and source-instant markers atomically.

A rollback needs no marker mutation rule: frontier calculation reads the active data-write
timeline, where rolled-back instants are absent. Thus an unaware or legacy writer cannot
falsely certify freshness; it leaves an observable, repairable gap.
...
- Unit: posting-block round-trip, delta supersession, shard/routing-version stability,
  marker-frontier contiguity (including empty updates and inflight gaps), candidate refill,
  and two-pass pruning correctness.
...
- Integration: ANN vs brute-force quality; upsert, delete, compaction, clustering, rollback,
  and restore; feature-aware and unaware writers; stale `FAIL/WARN/FALLBACK`; catch-up and
  multi-writer generation activation; cleaner retention.
