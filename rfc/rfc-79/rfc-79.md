...
Note that the reason (1b-d) needs to be within a transaciton is in order to handle the case where a concurrent writer executing a table service plan might start the heartbeat at the same time and about to perform the heartbeating-based guard steps mentioned above. 
