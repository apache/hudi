...
The reason this approach cannot allow re-executing an inflight cancellable plan is due to write conflict resolution. If a failed cancellable table service plan appears in the timeline as inflight, it is expensive to determine wether the write failed due to being blocked by a conflicting write or due to a transient failure (without re-executing the entire plan again). In order to simplify this flow, this approach assumes that a failed plan might have failed due to a write conflict and therefore should not be re-attempted.
