...
![cancel table service lifecycle (1)](https://github.com/user-attachments/assets/30e5206a-75c4-4ef3-993a-707415034145)

Note that the table service execution does not need to be responsible for transitioning a requested cancel action to ".cancel". This is since clean will anyway be required to execute any pending cancellation plan, and can execute it to completion even if the plan it is targeting has already been rolled back and aborted.

This approach resolves both drawbacks of approach (A), due to leaving the plan metadata file in the timeline and enabling table service writers to infer wether it can re-execute the plan again or should not bother (due to write conflict or cancellation). Despite this, it has the below drawback over (A)
