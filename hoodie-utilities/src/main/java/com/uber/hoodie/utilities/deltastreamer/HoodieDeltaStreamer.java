...
public class HoodieDeltaStreamer implements Serializable {
            ...
            HoodieWriteConfig.newBuilder().withPath(cfg.targetBasePath)
                    ...
                    .withCompactionConfig(HoodieCompactionConfig.newBuilder()
                            ...
                            .withInlineCompaction(HoodieTableType.valueOf(cfg.storageType)
                                    == HoodieTableType.MERGE_ON_READ).build())
  ...
  public static class Config extends AbstractCommandConfig {
    ...
    @Parameter(names = {"--props"}, description = "path to properties file on localfs or dfs, with configurations for "
            + "hoodie client, schema provider, key generator and data source. For hoodie client props, sane defaults"
            + " are used, but recommend use to provide basic things like metrics endpoints, hive configs etc. "
            + "For sources, refer to individual classes, for supported properties.")
    ...
    @Parameter(names = {"--source-ordering-field"}, description = "Field within source record to decide how"
            + " to break ties between records with same key in input data. Default: "
            + "'ts' holding unix timestamp of record")
    ...
    @Parameter(names = {"--key-generator-class"}, description = "Subclass of com.uber.hoodie.KeyGenerator "
            ...
            + "provided field names as recordkey & partitionpath. Nested fields specified "
            +  "via dot notation, e.g: a.b.c)")
    ...
    @Parameter(names = {"--schemaprovider-class"}, description = "subclass of com.uber.hoodie.utilities.schema"
            ...
            + "Source (See com.uber.hoodie.utilities.sources.Source) implementation can implement "
            + "their own SchemaProvider. For Sources that return Dataset<Row>, the schema is obtained implicitly. "
    ...
    @Parameter(names = {"--transformer-class"},
            description = "subclass of com.uber.hoodie.utilities.transform.Transformer"
                    + ". Allows transforming raw source dataset to a target dataset (conforming to target schema) "
                    + "before writing. Default : Not set. "
                    + "E:g - com.uber.hoodie.utilities.transform.SqlQueryBasedTransformer (which allows"
