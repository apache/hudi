...
public class TestCloudWatchReporter {
    ...
    @Mock CloudWatchAsyncClient cloudWatchAsync;

    @Mock CompletableFuture<PutMetricDataResponse> cloudWatchFuture;

    @Captor ArgumentCaptor<PutMetricDataRequest> putMetricDataRequestCaptor;
    ...
    public void setup() {
        reporter =
                CloudWatchReporter.forRegistry(metricRegistry)
                        .namespace(NAMESPACE)
                        .prefixedWith(PREFIX)
        ...
        Mockito.when(cloudWatchAsync.putMetricData((PutMetricDataRequest) ArgumentMatchers.any()))
                .thenReturn(cloudWatchFuture);
    }
    ...
    public void testReporter() {
        ...
        // Since there are 6 metrics in total, and max datums per request is 2 we would expect 3
        // calls to CloudWatch
        // with 2 datums in each
        Mockito.verify(cloudWatchAsync, Mockito.times(3))
                .putMetricData(putMetricDataRequestCaptor.capture());
    ...
    private void assertDimensions(List<Dimension> actualDimensions, String metricTypeDimensionVal) {
        ...
        Dimension expectedTableNameDimension =
                Dimension.builder().name(DIMENSION_TABLE_NAME_KEY).value(TABLE_NAME).build();
        Dimension expectedMetricTypeDimension =
                Dimension.builder().name(DIMENSION_METRIC_TYPE_KEY).value(metricTypeDimensionVal).build();
