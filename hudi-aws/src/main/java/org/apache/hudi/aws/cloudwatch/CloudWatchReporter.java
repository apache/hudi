...
/**
  * A reporter for publishing metrics to Amazon CloudWatch. It is responsible for collecting,
  * converting DropWizard metrics to CloudWatch metrics and composing metrics payload.
  */
public class CloudWatchReporter extends ScheduledReporter {
    ...
    private List<Dimension> getDimensions(String tableName, String metricType) {
        ...
        dimensions.add(Dimension.builder().name(DIMENSION_TABLE_NAME_KEY).value(tableName).build());
        dimensions.add(Dimension.builder().name(DIMENSION_METRIC_TYPE_KEY).value(metricType).build());
        return dimensions;
    }
