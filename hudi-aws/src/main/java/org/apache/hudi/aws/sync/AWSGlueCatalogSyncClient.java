...
/**
  * This class implements all the AWS APIs to enable syncing of a Hudi Table with the AWS Glue Data
  * Catalog (https://docs.aws.amazon.com/glue/latest/dg/populate-data-catalog.html). @Experimental
  */
public class AWSGlueCatalogSyncClient extends HoodieSyncClient {
    ...
    /**
      * athena v2/v3 table property see https://docs.aws.amazon.com/athena/latest/ug/querying-hudi.html
      */
    ...
    public AWSGlueCatalogSyncClient(HiveSyncConfig config) {
        super(config);
        try {
            GlueAsyncClientBuilder awsGlueBuilder =
                    GlueAsyncClient.builder()
                            .credentialsProvider(
                                    HoodieAWSCredentialsProviderFactory.getAwsCredentialsProvider(config.getProps()));
            awsGlueBuilder =
                    config.getString(AWS_GLUE_ENDPOINT) == null
                            ? awsGlueBuilder
                            : awsGlueBuilder.endpointOverride(new URI(config.getString(AWS_GLUE_ENDPOINT)));
            awsGlueBuilder =
                    config.getString(AWS_GLUE_REGION) == null
                            ? awsGlueBuilder
    ...
    private List<Partition> getPartitionsSegment(Segment segment, String tableName) {
        try {
            ...
            throw new HoodieGlueSyncException(
    ...
    public List<Partition> getAllPartitions(String tableName) {
        ExecutorService executorService =
                Executors.newFixedThreadPool(
                        ...
                        new CustomizedThreadFactory("glue-sync-all-partitions", true));
        try {
            List<Segment> segments = new ArrayList<>();
            for (int i = 0; i < allPartitionsReadParallelism; i++) {
                segments.add(
                        Segment.builder().segmentNumber(i).totalSegments(allPartitionsReadParallelism).build());
            }
            List<Future<List<Partition>>> futures =
                    segments.stream()
                            .map(
                                    segment ->
                                            executorService.submit(() -> this.getPartitionsSegment(segment, tableName)))
                            .collect(Collectors.toList());
    ...
    public List<Partition> getPartitionsFromList(String tableName, List<String> partitionList) {
        ...
        List<List<String>> batches =
                CollectionUtils.batches(partitionList, MAX_PARTITIONS_PER_READ_REQUEST);
        ExecutorService executorService =
                Executors.newFixedThreadPool(
        ...
        try {
            List<Future<List<Partition>>> futures =
                    batches.stream()
                            .map(
                                    batch ->
                                            executorService.submit(() -> this.getChangedPartitions(batch, tableName)))
                            .collect(Collectors.toList());
            ...
            throw new HoodieGlueSyncException(
                    "Failed to get all partitions for table " + tableId(this.databaseName, tableName), e);
        } finally {
            executorService.shutdownNow();
            LOG.info(
                    "Took {} ms to get {} partitions for table {}",
                    timer.endTimer(),
                    partitionList.size(),
    ...
    private List<Partition> getChangedPartitions(List<String> changedPartitions, String tableName)
                        ...
                        .map(
                                str ->
                                        PartitionValueList.builder()
                                                .values(partitionValueExtractor.extractPartitionValuesInPath(str))
                                                .build())
                        .collect(Collectors.toList());
    ...
    public void addPartitionsToTable(String tableName, List<String> partitionsToAdd) {
        ...
        try {
            ...
            parallelizeChange(
                    ...
                    partitions -> this.addPartitionsToTableInternal(table, partitions),
                    MAX_PARTITIONS_PER_CHANGE_REQUEST);
        } finally {
            LOG.info(
                    "Added {} partitions to table {} in {} ms",
    ...
    private <T> void parallelizeChange(
            List<T> items, int parallelism, Consumer<List<T>> consumer, int sliceSize) {
        List<List<T>> batches = CollectionUtils.batches(items, sliceSize);
        ExecutorService executorService =
                Executors.newFixedThreadPool(
                        Math.min(parallelism, batches.size()), new CustomizedThreadFactory("glue-sync", true));
        try {
            List<Future<?>> futures =
                    batches.stream()
                            .map(
    ...
    private void addPartitionsToTableInternal(Table table, List<String> partitionsToAdd) {
        try {
            ...
            throw new HoodieGlueSyncException(
                    "Fail to add partitions to " + tableId(databaseName, table.name()), e);
        }
    }
    ...
    public void updatePartitionsToTable(String tableName, List<String> changedPartitions) {
        ...
        try {
            ...
            parallelizeChange(
                    changedPartitions,
                    this.changeParallelism,
                    partitions -> this.updatePartitionsToTableInternal(table, partitions),
                    MAX_PARTITIONS_PER_CHANGE_REQUEST);
        } finally {
            LOG.info(
                    "Updated {} partitions to table {} in {} ms",
        ...
        try {
            ...
            BatchUpdatePartitionRequest request =
                    BatchUpdatePartitionRequest.builder()
                            .databaseName(databaseName)
                            .tableName(table.name())
                            .entries(updatePartitionEntries)
                            .build();
            ...
            throw new HoodieGlueSyncException(
                    "Fail to update partitions to " + tableId(databaseName, table.name()), e);
        }
    }
    ...
    public void dropPartitions(String tableName, List<String> partitionsToDrop) {
        ...
        try {
            ...
            parallelizeChange(
                    partitionsToDrop,
                    this.changeParallelism,
                    partitions -> this.dropPartitionsInternal(tableName, partitions),
                    MAX_DELETE_PARTITIONS_PER_REQUEST);
        } finally {
            LOG.info(
                    "Deleted {} partitions to table {} in {} ms",
                    partitionsToDrop.size(),
                    tableId(this.databaseName, tableName),
                    timer.endTimer());
        }
    }
        ...
        try {
            ...
            BatchDeletePartitionRequest batchDeletePartitionRequest =
                    BatchDeletePartitionRequest.builder()
                            ...
                            .build();
            CompletableFuture<BatchDeletePartitionResponse> future =
                    awsGlue.batchDeletePartition(batchDeletePartitionRequest);
            ...
            throw new HoodieGlueSyncException(
                    "Fail to drop partitions to " + tableId(databaseName, tableName), e);
        }
    }
    ...
    public boolean updateTableProperties(String tableName, Map<String, String> tableProperties) {
        try {
            ...
            return updateTableParameters(
                    awsGlue, databaseName, tableName, tableProperties, skipTableArchive);
        } catch (Exception e) {
            throw new HoodieGlueSyncException(
                    "Fail to update properties for table " + tableId(databaseName, tableName), e);
        }
    }
    ...
    public List<FieldSchema> getStorageFieldSchemas() {
        try {
            return new TableSchemaResolver(metaClient)
                    .getTableAvroSchema(true).getFields().stream()
    ...
    public boolean updateTableComments(
        ...
        Map<String, Option<String>> commentsMap =
                fromStorage.stream()
                        .collect(Collectors.toMap(FieldSchema::getName, FieldSchema::getComment));
    ...
    public void updateTableSchema(String tableName, MessageType newSchema) {
        // ToDo Cascade is set in Hive meta sync, but need to investigate how to configure it for
        // Glue meta
        boolean cascade = config.getSplitStrings(META_SYNC_PARTITION_FIELDS).size() > 0;
        try {
            Table table = getTable(awsGlue, databaseName, tableName);
            Map<String, String> newSchemaMap =
                    parquetSchemaToMapSchema(
                            newSchema, config.getBoolean(HIVE_SUPPORT_TIMESTAMP_TYPE), false);
            ...
            throw new HoodieGlueSyncException(
                    "Fail to update definition for table " + tableId(databaseName, tableName), e);
        }
    }
    ...
    public void createOrReplaceTable(
        ...
        try {
            ...
            throw new HoodieGlueSyncException(
                    "Fail to recreate the table" + tableId(databaseName, tableName), e);
        }
    }
    ...
    public void createTable(
        ...
        try {
            Map<String, String> mapSchema =
                    parquetSchemaToMapSchema(
                            storageSchema, config.getBoolean(HIVE_SUPPORT_TIMESTAMP_TYPE), false);
    ...
    /**
      ...
      *
      * <p>reference doc for partition indexes:
        ...
        if (!config.getBooleanOrDefault(META_SYNC_PARTITION_INDEX_FIELDS_ENABLE)) {
            ...
            for (PartitionIndexDescriptor idsToDelete : existingIdxsResp.partitionIndexDescriptorList()) {
                ...
                DeletePartitionIndexRequest idxToDelete =
                        DeletePartitionIndexRequest.builder()
                                .databaseName(databaseName)
                                .tableName(tableName)
                                .indexName(idsToDelete.indexName())
                                .build();
            ...
            for (PartitionIndexDescriptor existingIdx : existingIdxsResp.partitionIndexDescriptorList()) {
                List<String> idxColumns =
                        existingIdx.keys().stream().map(key -> key.name()).collect(Collectors.toList());
                ...
                if (toBeRemoved) {
                    ...
                    DeletePartitionIndexRequest idxToDelete =
                            DeletePartitionIndexRequest.builder()
                                    .databaseName(databaseName)
                                    .tableName(tableName)
                                    .indexName(existingIdx.indexName())
                                    .build();
            ...
            for (List<String> neededIdx : partitionsIndexNeeded) {
                ...
                if (toBeCreated) {
                    ...
                    PartitionIndex newIdx =
                            PartitionIndex.builder().indexName(newIdxName).keys(neededIdx).build();
    ...
    protected List<List<String>> parsePartitionsIndexConfig() {
        ...
        List<List<String>> indexes =
                Arrays.stream(rawPartitionIndex.split(","))
                        .map(idx -> Arrays.stream(idx.split(";")).collect(Collectors.toList()))
                        .collect(Collectors.toList());
    ...
    public Boolean getPartitionIndexEnable(String tableName) {
        try {
            ...
            throw new HoodieGlueSyncException(
                    "Fail to get parameter "
                            + GLUE_PARTITION_INDEX_ENABLE
                            + " time for "
                            + tableId(databaseName, tableName),
                    e);
        }
    ...
    public Map<String, String> getMetastoreSchema(String tableName) {
        try {
            // GlueMetastoreClient returns partition keys separate from Columns, hence get both and
            // merge to
            // get the Schema of the table.
            Table table = getTable(awsGlue, databaseName, tableName);
            Map<String, String> partitionKeysMap =
                    table.partitionKeys().stream()
                            .collect(Collectors.toMap(Column::name, f -> f.type().toUpperCase()));
            ...
            throw new HoodieGlueSyncException(
                    "Fail to get schema for table " + tableId(databaseName, tableName), e);
        }
    }
    ...
    public boolean tableExists(String tableName) {
        ...
        try {
            ...
            if (e.getCause() instanceof EntityNotFoundException) {
                ...
                throw new HoodieGlueSyncException(
                        "Fail to get table: " + tableId(databaseName, tableName), e);
            }
        } catch (Exception e) {
            throw new HoodieGlueSyncException(
                    "Fail to get table: " + tableId(databaseName, tableName), e);
    ...
    public Option<String> getLastCommitTimeSynced(String tableName) {
        try {
            ...
            throw new HoodieGlueSyncException(
                    "Fail to get last sync commit time for " + tableId(databaseName, tableName), e);
        }
    }
    ...
    public void updateLastCommitTimeSynced(String tableName) {
        ...
        try {
            ...
            throw new HoodieGlueSyncException(
                    "Fail to update last sync commit time for " + tableId(databaseName, tableName), e);
        }
        try {
    ...
    public void dropTable(String tableName) {
        ...
        try {
            ...
            throw new HoodieGlueSyncException(
                    "Failed to delete table " + tableId(databaseName, tableName), e);
        }
    }
    ...
    public List<FieldSchema> getMetastoreFieldSchemas(String tableName) {
        try {
            ...
            throw new HoodieGlueSyncException(
                    "Failed to get field schemas from metastore for table : "
                            + tableId(databaseName, tableName),
                    e);
        }
    }
    ...
    private List<FieldSchema> getFieldSchemas(List<Column> columns) {
        return columns.stream()
                .map(column -> new FieldSchema(column.name(), column.type(), column.comment()))
                .collect(Collectors.toList());
    }
    ...
    public String generatePushDownFilter(
        ...
        return new GluePartitionFilterGenerator()
                .generatePushDownFilter(writtenPartitions, partitionFields, (HiveSyncConfig) config);
    }
    ...
    public boolean updateSerdeProperties(
        ...
        try {
            ...
            if (existingTableStorageDescriptor != null
                ...
                boolean different =
                        serdeProperties.entrySet().stream()
                                .anyMatch(
                                        e ->
                ...
                if (!different) {
                    LOG.debug(
                            "Table "
                                    + tableName
                                    + " serdeProperties already up to date, skip update serde properties.");
                    return false;
                }
            }
            ...
            StorageDescriptor storageDescriptor =
                    table.storageDescriptor().toBuilder().serdeInfo(newSerdeInfo).build();
    ...
    public String getTableLocation(String tableName) {
        try {
            ...
            throw new HoodieGlueSyncException(
                    "Fail to get base path for the table " + tableId(databaseName, tableName), e);
        }
    }
    ...
    private static Table getTable(GlueAsyncClient awsGlue, String databaseName, String tableName)
            throws HoodieGlueSyncException {
        GetTableRequest request =
                GetTableRequest.builder().databaseName(databaseName).name(tableName).build();
        try {
    ...
    private static boolean updateTableParameters(
            GlueAsyncClient awsGlue,
            String databaseName,
            String tableName,
            Map<String, String> updatingParams,
        ...
        try {
            ...
            throw new HoodieGlueSyncException(
                    "Fail to update params for table "
                            + tableId(databaseName, tableName)
                            + ": "
                            + updatingParams,
                    e);
