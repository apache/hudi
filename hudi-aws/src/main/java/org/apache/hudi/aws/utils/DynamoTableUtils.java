...
/**
  ...
  *
  * <p>Utility methods for working with DynamoDB tables.
  *
...
public class DynamoTableUtils {
    ...
    /**
      * Waits up to 10 minutes for a specified DynamoDB table to resolve, indicating that it exists. If
      * the table doesn't return a result after this time, a SdkClientException is thrown.
      *
      * @param dynamo The DynamoDB client to use to make requests.
      * @param tableName The name of the table being resolved.
      * @throws SdkClientException If the specified table does not resolve before this method times out
      *     and stops polling.
    ...
    /**
      * Waits up to a specified amount of time for a specified DynamoDB table to resolve, indicating
      * that it exists. If the table doesn't return a result after this time, a SdkClientException is
      * thrown.
      *
      * @param dynamo The DynamoDB client to use to make requests.
      * @param tableName The name of the table being resolved.
    ...
    public static void waitUntilExists(
            final DynamoDbClient dynamo, final String tableName, final int timeout, final int interval)
            throws InterruptedException {
        TableDescription table = waitForTableDescription(dynamo, tableName, null, timeout, interval);

        if (table == null) {
            throw SdkClientException.builder()
                    .message("Table " + tableName + " never returned a result")
                    .build();
        }
    }

    /**
      * Waits up to 10 minutes for a specified DynamoDB table to move into the <code>ACTIVE</code>
      * state. If the table does not exist or does not transition to the <code>ACTIVE</code> state
      * after this time, then SdkClientException is thrown.
      *
      * @param dynamo The DynamoDB client to use to make requests.
    ...
    /**
      ...
      * </code> state after this time, then a SdkClientException is thrown.
      *
      * @param dynamo The DynamoDB client to use to make requests.
      * @param tableName The name of the table whose status is being checked.
      * @param timeout The maximum number of milliseconds to wait.
    ...
    public static boolean createTableIfNotExists(
            final DynamoDbClient dynamo, final CreateTableRequest createTableRequest) {
        try {
    ...
    /**
      ...
      */
    public static boolean deleteTableIfExists(
            final DynamoDbClient dynamo, final DeleteTableRequest deleteTableRequest) {
        try {
