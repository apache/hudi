...
public class DynamoDBBasedLockProvider implements LockProvider<LockItem>, Serializable {
    ...
    public DynamoDBBasedLockProvider(
    ...
    public DynamoDBBasedLockProvider(
        ...
        this.dynamoDBLockConfiguration =
                DynamoDbBasedLockConfig.newBuilder().fromProperties(lockConfiguration.getConfig()).build();
        this.tableName =
        ...
        long leaseDuration =
                dynamoDBLockConfiguration.getInt(
                        DynamoDbBasedLockConfig.LOCK_ACQUIRE_WAIT_TIMEOUT_MS_PROP_KEY);
        if (dynamoDB == null) {
    ...
    public boolean tryLock(long time, TimeUnit unit) {
        ...
        try {
            lock =
                    client.acquireLock(
                            AcquireLockOptions.builder(dynamoDBPartitionKey)
            ...
            throw new HoodieLockException(
                    generateLogStatement(LockState.FAILED_TO_ACQUIRE, generateLogSuffixString()), e);
    ...
    public void unlock() {
        try {
            ...
            throw new HoodieLockException(
                    generateLogStatement(LockState.FAILED_TO_RELEASE, generateLogSuffixString()), e);
        }
    }
    ...
    private DynamoDbClient getDynamoDBClient() {
        ...
        String endpointURL =
                this.dynamoDBLockConfiguration.contains(DynamoDbBasedLockConfig.DYNAMODB_ENDPOINT_URL.key())
                        ? this.dynamoDBLockConfiguration.getString(
                                DynamoDbBasedLockConfig.DYNAMODB_ENDPOINT_URL)
        ...
        return DynamoDbClient.builder()
                ...
                .credentialsProvider(
                        HoodieAWSCredentialsProviderFactory.getAwsCredentialsProvider(
                                dynamoDBLockConfiguration.getProps()))
                .build();
    }
    ...
    private void createLockTableInDynamoDB(DynamoDbClient dynamoDB, String tableName) {
        ...
        KeySchemaElement partitionKeyElement =
        ...
        if (billingMode.equals(BillingMode.PROVISIONED.name())) {
            createTableRequestBuilder.provisionedThroughput(
                    ProvisionedThroughput.builder()
                            .readCapacityUnits(
                                    dynamoDBLockConfiguration.getLong(
                                            DynamoDbBasedLockConfig.DYNAMODB_LOCK_READ_CAPACITY))
                            .writeCapacityUnits(
                                    dynamoDBLockConfiguration.getLong(
                                            DynamoDbBasedLockConfig.DYNAMODB_LOCK_WRITE_CAPACITY))
                            .build());
        }
        ...
        try {
            ...
            DynamoTableUtils.waitUntilActive(
                    dynamoDB,
                    tableName,
                    dynamoDBLockConfiguration.getInt(
                            DynamoDbBasedLockConfig.DYNAMODB_LOCK_TABLE_CREATION_TIMEOUT),
                    20 * 1000);
            ...
            throw new HoodieLockException(
                    "Thread interrupted while waiting for dynamoDB table to turn active", e);
        }
    ...
    private String generateLogSuffixString() {
        return StringUtils.join(
                "DynamoDb table = ", tableName, ", partition key = ", dynamoDBPartitionKey);
    }
