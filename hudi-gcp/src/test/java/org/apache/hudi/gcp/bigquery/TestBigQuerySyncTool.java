...
public class TestBigQuerySyncTool {
    ...
    private static final String TEST_TABLE_BASE_PATH =
            "gs://test-bucket/test-lake/test-db/test_table";
    ...
    void missingDatasetCausesFailure() {
        ...
        BigQuerySyncTool tool =
                new BigQuerySyncTool(
                        properties,
                        mockManifestFileWriter,
                        mockBqSyncClient,
                        mockMetaClient,
                        mockBqSchemaResolver);
    ...
    void useBQManifestFile_newTablePartitioned() {
        ...
        when(mockManifestFileWriter.getManifestSourceUri(true))
                .thenReturn(manifestPath.toUri().getPath());
        when(mockBqSchemaResolver.getTableSchema(any(), eq(Arrays.asList("datestr", "type"))))
                .thenReturn(schema);
        BigQuerySyncTool tool =
                new BigQuerySyncTool(
                        properties,
        ...
        verify(mockBqSyncClient)
                .createOrUpdateTableUsingBqManifestFile(
                        TEST_TABLE, manifestPath.toUri().getPath(), prefix, schema);
    ...
    void useBQManifestFile_newTableNonPartitioned() {
        ...
        when(mockManifestFileWriter.getManifestSourceUri(true))
                .thenReturn(manifestPath.toUri().getPath());
        when(mockBqSchemaResolver.getTableSchema(any(), eq(Collections.emptyList())))
                .thenReturn(schema);
        BigQuerySyncTool tool =
                new BigQuerySyncTool(
                        properties,
        ...
        verify(mockBqSyncClient)
                .createOrUpdateTableUsingBqManifestFile(
                        TEST_TABLE, manifestPath.toUri().getPath(), null, schema);
        verify(mockManifestFileWriter).writeManifestFile(true);
    }
