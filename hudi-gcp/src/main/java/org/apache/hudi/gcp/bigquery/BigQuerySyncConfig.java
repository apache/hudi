...
public class BigQuerySyncConfig extends HoodieSyncConfig implements Serializable {
    ...
    public static final ConfigProperty<String> BIGQUERY_SYNC_PROJECT_ID =
            ConfigProperty.key("hoodie.gcp.bigquery.sync.project_id")
                    .noDefaultValue()
                    .markAdvanced()
    ...
    public static final ConfigProperty<String> BIGQUERY_SYNC_DATASET_NAME =
            ConfigProperty.key("hoodie.gcp.bigquery.sync.dataset_name")
                    .noDefaultValue()
    ...
    public static final ConfigProperty<String> BIGQUERY_SYNC_DATASET_LOCATION =
            ConfigProperty.key("hoodie.gcp.bigquery.sync.dataset_location")
                    .noDefaultValue()
                    .markAdvanced()
    ...
    public static final ConfigProperty<String> BIGQUERY_SYNC_TABLE_NAME =
            ConfigProperty.key("hoodie.gcp.bigquery.sync.table_name")
                    .noDefaultValue()
                    .withInferFunction(
                            cfg ->
    ...
    public static final ConfigProperty<Boolean> BIGQUERY_SYNC_USE_BQ_MANIFEST_FILE =
            ConfigProperty.key("hoodie.gcp.bigquery.sync.use_bq_manifest_file")
                    .defaultValue(false)
                    .markAdvanced()
                    .sinceVersion("0.14.0")
                    .withDocumentation(
                            "If true, generate a manifest file with data file absolute paths and use BigQuery manifest file support to "
    ...
    public static final ConfigProperty<String> BIGQUERY_SYNC_SOURCE_URI =
            ConfigProperty.key("hoodie.gcp.bigquery.sync.source_uri")
                    .noDefaultValue()
                    .markAdvanced()
    ...
    public static final ConfigProperty<String> BIGQUERY_SYNC_SOURCE_URI_PREFIX =
            ConfigProperty.key("hoodie.gcp.bigquery.sync.source_uri_prefix")
                    .noDefaultValue()
                    .markAdvanced()
    ...
    public static final ConfigProperty<String> BIGQUERY_SYNC_PARTITION_FIELDS =
            ConfigProperty.key("hoodie.gcp.bigquery.sync.partition_fields")
                    .noDefaultValue()
                    .withInferFunction(
                            cfg ->
    ...
    public static final ConfigProperty<Boolean> BIGQUERY_SYNC_USE_FILE_LISTING_FROM_METADATA =
            ConfigProperty.key("hoodie.gcp.bigquery.sync.use_file_listing_from_metadata")
    ...
    public static final ConfigProperty<Boolean> BIGQUERY_SYNC_REQUIRE_PARTITION_FILTER =
            ConfigProperty.key("hoodie.gcp.bigquery.sync.require_partition_filter")
                    .defaultValue(false)
                    .sinceVersion("0.14.1")
                    .markAdvanced()
                    .withDocumentation(
                            "If true, configure table to require a partition filter to be specified when querying the table");

    public static final ConfigProperty<String> BIGQUERY_SYNC_BIG_LAKE_CONNECTION_ID =
            ConfigProperty.key("hoodie.gcp.bigquery.sync.big_lake_connection_id")
                    .noDefaultValue()
                    .sinceVersion("0.14.1")
                    .markAdvanced()
    ...
    public static class BigQuerySyncConfigParams {
        ...
        @Parameter(
                names = {"--project-id"},
                description = "Name of the target project in BigQuery",
                required = true)
        public String projectId;

        @Parameter(
                names = {"--dataset-name"},
                description = "Name of the target dataset in BigQuery",
                required = true)
        public String datasetName;

        @Parameter(
                names = {"--dataset-location"},
                description = "Location of the target dataset in BigQuery",
                required = true)
        public String datasetLocation;

        @Parameter(
                names = {"--use-bq-manifest-file"},
                description =
                        "If true, generate a manifest file with data file absolute paths and use "
        ...
        @Parameter(
                names = {"--source-uri"},
                description = "Name of the source uri gcs path of the table",
                required = true)
        public String sourceUri;

        @Parameter(
                names = {"--source-uri-prefix"},
                description = "Name of the source uri gcs path prefix of the table",
                required = false)
        public String sourceUriPrefix;

        @Parameter(
                names = {"--big-lake-connection-id"},
                description =
                        "The Big Lake connection ID to use when creating the table if using the manifest file approach.")
        public String bigLakeConnectionId;

        @Parameter(
                names = {"--require-partition-filter"},
                description =
                        "If true, configure table to require a partition filter to be specified when querying the table")
        ...
        public TypedProperties toProps() {
            ...
            props.setPropertyIfNonNull(
                    BIGQUERY_SYNC_PARTITION_FIELDS.key(),
                    StringUtils.join(",", hoodieSyncConfigParams.partitionFields));
            props.setPropertyIfNonNull(
                    BIGQUERY_SYNC_USE_FILE_LISTING_FROM_METADATA.key(),
            ...
            props.setPropertyIfNonNull(
                    BIGQUERY_SYNC_REQUIRE_PARTITION_FILTER.key(), requirePartitionFilter);
            return props;
        }
    }
}
