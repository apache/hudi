...
/**
  ...
  * BigQuerySyncTool.syncHoodieTable(BigQuerySyncConfig) or as a command line java -cp
  * hoodie-hive.jar BigQuerySyncTool [args]
  *
  * <p>This utility will get the schema from the latest commit and will sync big query table
  * schema. @Experimental
  */
public class BigQuerySyncTool extends HoodieSyncTool {
    ...
    public BigQuerySyncTool(Properties props) {
        ...
        // reuse existing meta client if not provided (only test cases will provide their own meta
        // client)
    ...
    BigQuerySyncTool(
            ...
            HoodieBigQuerySyncClient bigQuerySyncClient,
    ...
    private static ManifestFileWriter buildManifestFileWriterFromConfig(
            HoodieTableMetaClient metaClient, BigQuerySyncConfig config) {
        return ManifestFileWriter.builder()
                .setMetaClient(metaClient)
                .setUseFileListingFromMetadata(
                        config.getBoolean(BIGQUERY_SYNC_USE_FILE_LISTING_FROM_METADATA))
                .build();
    }
        ...
        switch (bqSyncClient.getTableType()) {
            ...
            default:
                throw new UnsupportedOperationException(
                        bqSyncClient.getTableType() + " table type is not supported yet.");
        }
    }
    ...
    private void syncTable(HoodieBigQuerySyncClient bqSyncClient) {
        ...
        if (!bqSyncClient.datasetExists()) {
            throw new HoodieBigQuerySyncException(
                    "Dataset not found: " + config.getString(BIGQUERY_SYNC_DATASET_NAME));
        }
        ...
        if (config.getBoolean(BIGQUERY_SYNC_USE_BQ_MANIFEST_FILE)) {
            ...
            // if table exists but is not yet using manifest file or needs to be recreated with the
            // big-lake connection ID, update it to use manifest file
        ...
        if (!tableExists(bqSyncClient, manifestTableName)) {
            bqSyncClient.createManifestTable(
                    manifestTableName, manifestFileWriter.getManifestSourceUri(false));
    ...
    public static void main(String[] args) {
        final BigQuerySyncConfig.BigQuerySyncConfigParams params =
