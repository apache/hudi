...
public class BigQuerySchemaResolver {
    private static final BigQuerySchemaResolver INSTANCE =
            new BigQuerySchemaResolver(TableSchemaResolver::new);

    private final Function<HoodieTableMetaClient, TableSchemaResolver> tableSchemaResolverSupplier;

    @VisibleForTesting
    BigQuerySchemaResolver(
            Function<HoodieTableMetaClient, TableSchemaResolver> tableSchemaResolverSupplier) {
    ...
    /**
      * Get the BigQuery schema for the table. If the BigQuery table is configured with partitioning,
      * the caller must pass in the partition fields so that they are not returned in the schema. If
      * the partition fields are in the schema, it will cause an error when querying the table since
      * BigQuery will treat it as a duplicate column.
      *
    ...
    /**
      * Converts a BigQuery schema to the string representation used in the BigQuery SQL command to
      * create the manifest based table.
      *
        ...
        switch (fieldSchema.getType()) {
            ...
            case LONG:
                ...
                if (logicalType == null) {
                    ...
                    // Due to older avro support, we need to use strings for local timestamp logical
                    // types
            ...
            case FIXED:
                ...
                if (bytesLogicalType == null) {
                    ...
                    throw new IllegalArgumentException(
                            "Unexpected logical type in schema: " + bytesLogicalType);
                }
                break;
            case RECORD:
                return Field.newBuilder(
                                name, StandardSQLTypeName.STRUCT, FieldList.of(getFields(fieldSchema)))
                        .setMode(fieldMode)
                        .build();
            case ARRAY:
                Field arrayField = getField(fieldSchema.getElementType(), "array", true);
                return Field.newBuilder(name, arrayField.getType(), arrayField.getSubFields())
            ...
            case MAP:
                ...
                Field keyValueField =
                        Field.newBuilder("key_value", StandardSQLTypeName.STRUCT, keyField, valueField)
                                .setMode(Field.Mode.REPEATED)
                                .build();
                return Field.newBuilder(name, StandardSQLTypeName.STRUCT, keyValueField)
            ...
            case UNION:
                ...
                org.apache.avro.Schema fieldSchemaFromUnion =
                        subTypes.get(0).getType() == org.apache.avro.Schema.Type.NULL
                                ? subTypes.get(1)
                                : subTypes.get(0);
                nullable = true;
