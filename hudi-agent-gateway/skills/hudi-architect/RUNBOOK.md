...
- **Multi-writer concurrency** — if you have multiple independent jobs writing one table (e.g. a streamer plus a nightly GDPR-deletion job), the agent states the hard requirement (OCC + one identically-configured lock provider across *all* writers; `SINGLE_WRITER` in that deployment risks silent corruption) but does not pick a lock provider or model conflict behavior. Resolve lock configuration from the [Hudi concurrency docs](https://hudi.apache.org/docs/concurrency_control) before go-live. The one case it handles concretely: MOR + Spark DataSource with a standalone compactor, where it emits the OCC skeleton and warns you.
- **Catalog / metastore sync** (Hive Metastore, AWS Glue, `hoodie.datasource.hive_sync.*`) — not covered. If Trino/Athena/Presto or any catalog-based engine reads your table, configure sync separately per the [Hudi syncing docs](https://hudi.apache.org/docs/syncing_metastore).
