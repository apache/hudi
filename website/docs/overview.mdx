...
- [Unstructured Data Quick Start Guide](unstructured-data-quick-start-guide.md) – store embeddings (`VECTOR`) and raw bytes (`BLOB`) and run similarity search with `hudi_vector_search`
...
Hudi supports the following types and file formats for unstructured and semi-structured data, in addition to standard SQL types and Parquet/ORC:

- [`VECTOR(dim[, elementType])`](sql_ddl.md#vector): fixed-dimension embedding column with the [`hudi_vector_search`](sql_queries.md#vector-similarity-search) TVF.
- [`BLOB`](sql_ddl.md#blob): binary column with `INLINE` or `OUT_OF_LINE` storage and the [`read_blob()`](sql_queries.md#reading-blob-columns) function.
- [`VARIANT`](sql_ddl.md#variant): semi-structured (JSON-like) column with optional [shredding](schema_evolution.md#variant-shredding).
- [Lance file format](storage_layouts.md#lance-base-file-format): pluggable base file format (Spark-only).
