...
- [Unstructured Data Quick Start](unstructured-data-quick-start.md) – store embeddings (`VECTOR`) and raw bytes (`BLOB`) and run similarity search with `hudi_vector_search`
...
- **[`VECTOR(dim[, elementType])`](vector_search.md)** — fixed-dimension embedding column with the `hudi_vector_search` TVF for similarity search.
- **[`BLOB`](blob_unstructured_data.md)** — binary column with `INLINE` or `OUT_OF_LINE` storage modes and a `read_blob()` SQL function.
- **[`VARIANT`](variant_type.md)** — semi-structured (JSON-like) column with optional shredding.
- **[Lance file format](lance_file_format.md)** — pluggable base file format (Spark-only).
