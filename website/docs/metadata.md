...
2. `column_stats` index is not yet supported for [complex types](https://avro.apache.org/docs/1.11.1/specification/#complex-types).
3. `partition_stats` index is not yet supported for complex and [logical types](https://avro.apache.org/docs/1.11.1/specification/#logical-types).
4. By default, only first 32 columns are indexed. To index more columns, set `hoodie.metadata.index.column.stats.max.columns.to.index` to the desired value.
:::
