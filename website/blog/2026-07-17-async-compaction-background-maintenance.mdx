...
PostgreSQL periodically runs **VACUUM** to reclaim obsolete row versions created by MVCC. LSM-tree based systems such as [RocksDB](https://github.com/facebook/rocksdb/wiki/Universal-Compaction), [LevelDB](https://github.com/google/leveldb/blob/main/doc/impl.md#compactions), Cassandra, and ScyllaDB continuously compact SSTables to reduce read amplification and reclaim space. SQL Server and Oracle rebuild indexes online, while checkpointing mechanisms asynchronously persist in-memory state without delaying foreground transactions.
...
Apache Iceberg and Delta Lake naturally extended the strengths of immutable data lakes. Immutable files, snapshot isolation, and file replacement became the primary mechanisms for coordinating updates while preserving compatibility with cloud object storage. This approach provided a simple and elegant foundation for the analytical workloads of the time.
