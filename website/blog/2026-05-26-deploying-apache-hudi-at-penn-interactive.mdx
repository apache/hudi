...
excerpt: "How Penn Interactive, the online sports betting arm of Penn Entertainment, modernized its data platform using Apache Hudi HudiStreamer to ingest CDC from hundreds of Kafka topics, orchestrate 100+ concurrent jobs with Argo Workflows, and deliver near real-time data freshness across thousands of tables."
...
[Penn Interactive](https://www.pennentertainment.com) and The Score merged in 2022 under Penn Entertainment, combining sports betting and sports media for customers across the U.S. and Canada. Known for apps like Barstool Sportsbook and The Score Bet, the company operates at the forefront of the online and retail sports betting industry.
...
1. **Raw Layer:** A mirror of the source data, stored in S3 using the Parquet file format and written efficiently by HudiStreamer.
...
## The Solution: Apache Hudi and HudiStreamer
...
HudiStreamer became the core database replication tool. It natively processes CDC messages streamed through Kafka topics, applying the exact changes to the Hudi tables to maintain synchronization. The team leveraged its capabilities for:
...
* **Scalability:** Managing over a hundred concurrent HudiStreamer jobs, consuming from nearly a thousand topics, by optimizing resource use through table grouping.
...
Early experiments with homegrown solutions and traditional streaming methods highlighted major pain points such as manual checkpointing, batch sizing, and scalability concerns. The combination of Debezium, Kafka, and Apache Hudi HudiStreamer was ultimately chosen, as HudiStreamer provided built-in solutions for these complex operational challenges.

<img src="/assets/images/blog/2026-05-26-penn-interactive/img4.png" alt="Proof of Concept" width="800"/>

The data platform's CDC pipeline relies on Debezium connectors to capture changes from PostgreSQL databases, streaming them into Kafka topics hosted on Confluent Cloud. HudiStreamer consumes these messages, applies the updates to Hudi tables in S3, and integrates them into the Redshift query layer.
...
The team established a workflow for Disaster Recovery for scenarios where Kafka logs have expired. They developed a process to temporarily pause the streaming job and kick off a dedicated HudiStreamer job with a JDBC source. This job directly upserts missing data from the source, quickly patching the data gap before the streaming job is resumed.

<img src="/assets/images/blog/2026-05-26-penn-interactive/img6.png" alt="HudiStreamer Job" width="800"/>
