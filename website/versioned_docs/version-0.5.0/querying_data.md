...
Conceptually, Hudi stores data physically once on DFS, while providing 3 logical views on top, as explained [before](/docs/concepts#views). 
...
Specifically, there are two Hive tables named off [table name](/docs/configurations#TABLE_NAME_OPT_KEY) passed during write. 
...
joined with other tables (datasets/dimensions), to [write out deltas](/docs/writing_data) to a target Hudi dataset. Incremental view is realized by querying one of the tables above, 
...
in its [aux jars path](https://www.cloudera.com/documentation/enterprise/5-6-x/topics/cm_mc_hive_udf#concept_nc3_mms_lr). This will ensure the input format 
