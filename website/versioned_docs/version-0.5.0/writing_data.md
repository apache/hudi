...
speeding up large Spark jobs via upserts using the [Hudi datasource](#datasource-writer). Such datasets can then be [queried](/docs/querying_data) using various query engines.
...
In some cases, you may want to migrate your existing dataset into Hudi beforehand. Please refer to [migration guide](/docs/migration_guide). 
...
Here are some ways to efficiently manage the storage of your Hudi datasets.
 ...
 - For workloads with heavy updates, the [merge-on-read storage](/docs/concepts#merge-on-read-storage) provides a nice mechanism for ingesting quickly into smaller files and then later merging them into larger base files via compaction.
