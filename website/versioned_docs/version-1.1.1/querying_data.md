...
:::danger
This page is no longer maintained. Please refer to Hudi [SQL DDL](sql_ddl.md), [SQL DML](sql_dml.md), [SQL Queries](sql_queries.md) and [Procedures](procedures.md) for the latest documentation.
:::

Conceptually, Hudi stores data physically once on DFS, while providing 3 different ways of querying, as explained [before](concepts.md#query-types). 
...
See the [Spark Quick Start](quick-start-guide.md) for more examples of Spark datasource reading queries. 
...
For examples, refer to [Incremental Queries](quick-start-guide.md#incremental-query) in the Spark quickstart. 
Please refer to [configurations](configurations.md#SPARK_DATASOURCE) section, to view all datasource options.
