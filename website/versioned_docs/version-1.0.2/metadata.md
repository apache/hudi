...
To try out these features, refer to the [SQL guide](sql_ddl.md#create-partition-stats-index).
...
[here](configurations.md#Metadata-Configs).
...
must be configured. Please refer to [concurrency control](concurrency_control.md) for more details. 

Before enabling metadata table for the first time, all writers on the same table must and table services must be stopped.
If your current deployment model is [multi-writer](concurrency_control.md#full-on-multi-writer--async-table-services) along with a lock 
