...
will rollover to the new file handle. Finally, `the number of files` >= [`write.bucket_assign.tasks`](/docs/1.0.0/configurations#writebucket_assigntasks).
:::
...
| `write.tasks`  |  `false`  | `4` | The parallelism of `bulk_insert`, `the number of files` >= [`write.bucket_assign.tasks`](/docs/1.0.0/configurations#writebucket_assigntasks) |
...
the compaction options: [`compaction.delta_commits`](/docs/1.0.0/ingestion_flink#compaction) and [`compaction.delta_seconds`](/docs/1.0.0/ingestion_flink#compaction).
:::
