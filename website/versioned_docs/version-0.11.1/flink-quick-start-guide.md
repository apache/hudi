...
- **Configuration** : For [Global Configuration](/docs/0.11.1/flink_configuration#global-configurations), sets up through `$FLINK_HOME/conf/flink-conf.yaml`. For per job configuration, sets up through [Table Option](/docs/0.11.1/flink_configuration#table-options).
- **Writing Data** : Flink supports different modes for writing, such as [CDC Ingestion](/docs/0.11.1/hoodie_deltastreamer#cdc-ingestion), [Bulk Insert](/docs/0.11.1/hoodie_deltastreamer#bulk-insert), [Index Bootstrap](/docs/0.11.1/hoodie_deltastreamer#index-bootstrap), [Changelog Mode](/docs/0.11.1/hoodie_deltastreamer#changelog-mode) and [Append Mode](/docs/0.11.1/hoodie_deltastreamer#append-mode).
- **Querying Data** : Flink supports different modes for reading, such as [Streaming Query](/docs/0.11.1/hoodie_deltastreamer#streaming-query) and [Incremental Query](/docs/0.11.1/hoodie_deltastreamer#incremental-query).
- **Tuning** : For write/read tasks, this guide gives some tuning suggestions, such as [Memory Optimization](/docs/0.11.1/flink_configuration#memory-optimization) and [Write Rate Limit](/docs/0.11.1/flink_configuration#write-rate-limit).
- **Optimization**: Offline compaction is supported [Offline Compaction](/docs/compaction#flink-offline-compaction).
- **Query Engines**: Besides Flink, many other engines are integrated: [Hive Query](/docs/syncing_metastore#flink-setup), [Presto Query](/docs/0.11.1/query_engine_setup#prestodb).
...
- in `$FLINK_HOME/conf/flink-conf.yaml`, [add other global configurations according to the characteristics of your task](/docs/0.11.1/flink_configuration#global-configurations)
...
Check out the [Flink Setup](/docs/0.11.1/flink_configuration) how-to page for deeper dive into configuration settings. 
