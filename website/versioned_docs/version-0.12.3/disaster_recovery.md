...
:::danger Important:
If you're facing `java.lang.IllegalArgumentException: For input string: "null"` exception, it means that you may need to
manually set the `LEGACY_PARQUET_NANOS_AS_LONG` to `false` i.e. add `--conf 'spark.hadoop.spark.sql.legacy.parquet.nanosAsLong=false'`
to your spark configuration while starting the spark session. For more information, read [here](https://github.com/apache/hudi/issues/8061).
:::
