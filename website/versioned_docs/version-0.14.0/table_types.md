...
- [Copy On Write](#copy-on-write-table) : Stores data using columnar file formats (e.g parquet) exclusively. Updates simply version & rewrite the files by performing a synchronous merge during write.
...
| Update cost (I/O)   | Higher <br/>(rewrite entire parquet) | Lower <br/>(append to delta log)              |
| Parquet File Size   | Smaller <br/>(high update(I/0) cost) | Larger <br/>(low update cost)                 |
| Write Amplification | Higher                               | Lower <br/>(depending on compaction strategy) |
...
| Query Latency | Higher <br/>(merge base / columnar file + row based delta / log files) | Lower <br/>(raw base / columnar file performance) |
...
| Config Name                  | Default                  | Description                                                                                                                                                                                                                                                                                                                                                                  |
|------------------------------|--------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| hoodie.datasource.query.type | snapshot <br/>(Optional) | Decides how data files need to be read, in 1) Snapshot mode (obtain latest view, based on row &amp; columnar data); 2) incremental mode (new data since an instantTime). If `cdc.enabled` is set incremental queries on cdc data are possible; 3) Read Optimized mode (obtain latest view, based on columnar data) .Default: snapshot<br /><br /> `Config Param: QUERY_TYPE` |
| read.start-commit            | N/A <br/>**(Required)**  | Required in case of incremental queries. Start commit instant for reading, the commit time format should be 'yyyyMMddHHmmss', by default reading from the latest instant for streaming read<br /><br /> `Config Param: READ_START_COMMIT`                                                                                                                                    |
| read.end-commit              | N/A <br/>**(Required)**  | Used in the context of incremental queries. End commit instant for reading, the commit time format should be 'yyyyMMddHHmmss'<br /><br /> `Config Param: READ_END_COMMIT`                                                                                                                                                                                                    |
