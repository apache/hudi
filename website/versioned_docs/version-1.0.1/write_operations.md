...
1. [Deduping](/docs/1.0.1/configurations#hoodiecombinebeforeinsert) : First your input records may have duplicate keys within the same batch and duplicates need to be combined or reduced by key.
2. [Index Lookup](indexes.md) : Next, an index lookup is performed to try and match the input records to identify which file groups they belong to.
3. [File Sizing](file_sizing.md): Then, based on the average size of previous commits, Hudi will make a plan to add enough records to a small file to get it close to the configured maximum limit.
4. [Partitioning](storage_layouts.md): We now arrive at partitioning where we decide what file groups certain updates and inserts will be placed in or if new file groups will be created
