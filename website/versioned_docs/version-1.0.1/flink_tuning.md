...
:::note
Turn off online compaction by setting `compaction.async.enabled` = `false`, but we still recommend turning on `compaction.schedule.enable` for the writing job. You can then execute the compaction plan by [`offline compaction`](/docs/1.0.1/flink_tuning#offline-compaction).
:::
