...
- The [small file handling feature](/docs/1.0.1/configurations/#hoodieparquetsmallfilelimit) in Hudi, profiles incoming workload
  and distributes inserts to existing file groups instead of creating new file groups, which can lead to small files.
- Cleaner can be [configured](/docs/1.0.1/configurations#hoodiecleanercommitsretained) to clean up older file slices, more or less aggressively depending on maximum time for queries to run & lookback needed for incremental pull
- User can also tune the size of the [base/parquet file](/docs/1.0.1/configurations#hoodieparquetmaxfilesize), [log files](/docs/1.0.1/configurations#hoodielogfilemaxsize) & expected [compression ratio](/docs/1.0.1/configurations#hoodieparquetcompressionratio),
...
- Intelligently tuning the [bulk insert parallelism](/docs/1.0.1/configurations#hoodiebulkinsertshuffleparallelism), can again in nicely sized initial file groups. It is in fact critical to get this right, since the file groups
