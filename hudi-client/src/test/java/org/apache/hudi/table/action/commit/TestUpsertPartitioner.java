...
public class TestUpsertPartitioner extends HoodieClientTestHarness {
  ...
  private UpsertPartitioner<TestRawTripPayload> getUpsertPartitioner(
      int smallFileSize,
      int numInserts,
      int numUpdates,
      int fileSize,
    ...
    HoodieCopyOnWriteTable<TestRawTripPayload> table = (HoodieCopyOnWriteTable<TestRawTripPayload>) HoodieTable.create(metaClient, config, jsc);

    HoodieTestDataGenerator dataGenerator = new HoodieTestDataGenerator(new String[] {testPartitionPath});
    List<HoodieRecord<TestRawTripPayload>> insertRecords = dataGenerator.generateInserts("001", numInserts);
    List<HoodieRecord<TestRawTripPayload>> updateRecords = dataGenerator.generateUpdates("001", numUpdates);
    for (HoodieRecord<TestRawTripPayload> updateRec : updateRecords) {
      updateRec.unseal();
      updateRec.setCurrentLocation(new HoodieRecordLocation("001", "file1"));
      updateRec.seal();
    }
    List<HoodieRecord<TestRawTripPayload>> records = new ArrayList<>();
    records.addAll(insertRecords);
    records.addAll(updateRecords);
    WorkloadProfile<TestRawTripPayload> profile = new WorkloadProfile<>(jsc.parallelize(records));
    UpsertPartitioner<TestRawTripPayload> partitioner = new UpsertPartitioner<>(profile, jsc, table, config);
  ...
  public void testUpsertPartitioner() throws Exception {
    ...
    UpsertPartitioner<TestRawTripPayload> partitioner = getUpsertPartitioner(0, 200, 100, 1024, testPartitionPath, false);
  ...
  public void testUpsertPartitionerWithSmallInsertHandling() throws Exception {
    ...
    // smallest file
    UpsertPartitioner<TestRawTripPayload> partitioner = getUpsertPartitioner(1000 * 1024, 400, 100, 800 * 1024, testPartitionPath, false);
    List<InsertBucket> insertBuckets = partitioner.getInsertBuckets(testPartitionPath);
