...
/**
  * Implementation of a very heavily read-optimized Hoodie Table where, all data is stored in base
  * files, with zero read amplification.
  *
  * <p>INSERTS - Produce new files, block aligned to desired size (or) Merge with the smallest
  * existing file, to expand it
  *
  * <p>UPDATES - Produce a new version of the file, just replacing the updated records with new
  * values
  */
public class HoodieFlinkCopyOnWriteTable<T> extends HoodieFlinkTable<T>
    ...
    public HoodieFlinkCopyOnWriteTable(
    ...
    /**
      ...
      *
      * <p>Specifies the write handle explicitly in order to have fine-grained control with the
      * underneath file.
      *
    ...
    public HoodieWriteMetadata<List<WriteStatus>> upsert(
        ...
        return new FlinkUpsertCommitActionExecutor<>(
                        context, writeHandle, config, this, instantTime, records)
                .execute();
    }
    ...
    public HoodieWriteMetadata<List<WriteStatus>> insert(
            ...
            String instantTime,
            List<HoodieRecord<T>> records) {
        return new FlinkInsertCommitActionExecutor<>(
                        context, writeHandle, config, this, instantTime, records)
                .execute();
    }
    ...
    public HoodieWriteMetadata<List<WriteStatus>> delete(
            ...
            String instantTime,
            List<HoodieKey> keys) {
        return new FlinkDeleteCommitActionExecutor<>(
                        context, writeHandle, config, this, instantTime, keys)
                .execute();
    }
    ...
    public HoodieWriteMetadata<List<WriteStatus>> deletePrepped(
        ...
        return new FlinkDeletePreppedCommitActionExecutor<>(
                        context, writeHandle, config, this, instantTime, preppedRecords)
                .execute();
    }
    ...
    public HoodieWriteMetadata<List<WriteStatus>> upsertPrepped(
            ...
            String instantTime,
            List<HoodieRecord<T>> preppedRecords) {
        return new FlinkUpsertPreppedCommitActionExecutor<>(
                        context, writeHandle, config, this, instantTime, preppedRecords)
                .execute();
    }
    ...
    public HoodieWriteMetadata<List<WriteStatus>> bulkInsertPrepped(
            ...
            String instantTime,
            List<HoodieRecord<T>> preppedRecords) {
        return new FlinkBulkInsertPreppedCommitActionExecutor<>(
                        context, writeHandle, config, this, instantTime, preppedRecords)
                .execute();
    }
    ...
    public HoodieWriteMetadata<List<WriteStatus>> insertOverwrite(
            ...
            String instantTime,
            List<HoodieRecord<T>> records) {
        return new FlinkInsertOverwriteCommitActionExecutor(
                        context, writeHandle, config, this, instantTime, records)
                .execute();
    }
    ...
    public HoodieWriteMetadata<List<WriteStatus>> insertOverwriteTable(
            ...
            String instantTime,
            List<HoodieRecord<T>> records) {
        return new FlinkInsertOverwriteTableCommitActionExecutor(
                        context, writeHandle, config, this, instantTime, records)
                .execute();
    }
    ...
    @Override
    public HoodieWriteMetadata<List<WriteStatus>> upsert(
            HoodieEngineContext context, String instantTime, List<HoodieRecord<T>> records) {
    ...
    @Override
    public HoodieWriteMetadata<List<WriteStatus>> bulkInsert(
            HoodieEngineContext context,
            String instantTime,
            List<HoodieRecord<T>> records,
    ...
    @Override
    public HoodieWriteMetadata<List<WriteStatus>> delete(
            HoodieEngineContext context, String instantTime, List<HoodieKey> keys) {
        throw new HoodieNotSupportedException("This method should not be invoked");
    }

    @Override
    public HoodieWriteMetadata<List<WriteStatus>> deletePrepped(
            HoodieEngineContext context, String instantTime, List<HoodieRecord<T>> preppedRecords) {
        throw new HoodieNotSupportedException("This method should not be invoked");
    }

    @Override
    public HoodieWriteMetadata<List<WriteStatus>> deletePartitions(
            HoodieEngineContext context, String instantTime, List<String> partitions) {
        return new FlinkDeletePartitionCommitActionExecutor<>(
                        context, config, this, instantTime, partitions)
                .execute();
    }
    ...
    public Option<HoodieClusteringPlan> scheduleClustering(
        ...
        return new ClusteringPlanActionExecutor<>(context, config, this, instantTime, extraMetadata)
                .execute();
    }

    @Override
    public HoodieWriteMetadata<List<WriteStatus>> cluster(
            final HoodieEngineContext context, final String clusteringInstantTime) {
        throw new HoodieNotSupportedException(
                "Clustering is not supported on a Flink CopyOnWrite table");
    }

    @Override
    public HoodieBootstrapWriteMetadata<List<WriteStatus>> bootstrap(
    ...
    /**
      ...
      */
    @Override
    public Option<HoodieCleanerPlan> scheduleCleaning(
            HoodieEngineContext context, String instantTime, Option<Map<String, String>> extraMetadata) {
        return new CleanPlanActionExecutor(context, config, this, instantTime, extraMetadata).execute();
    }

    @Override
    public Option<HoodieRollbackPlan> scheduleRollback(
            HoodieEngineContext context,
            String instantTime,
            ...
            boolean isRestore) {
        return new BaseRollbackPlanActionExecutor(
                        context,
                        config,
                        this,
                        instantTime,
    ...
    @Override
    public HoodieRollbackMetadata rollback(
            HoodieEngineContext context,
        ...
        return new CopyOnWriteRollbackActionExecutor(
                        context, config, this, rollbackInstantTime, commitInstant, deleteInstants, skipLocking)
                .execute();
    }

    @Override
    public Option<HoodieIndexPlan> scheduleIndexing(
            HoodieEngineContext context,
            String indexInstantTime,
            List<MetadataPartitionType> partitionsToIndex,
            List<String> partitionPaths) {
        throw new HoodieNotSupportedException(
                "Metadata indexing is not supported for a Flink table yet.");
    }

    @Override
    public Option<HoodieIndexCommitMetadata> index(
    ...
    @Override
    public HoodieSavepointMetadata savepoint(
    ...
    @Override
    public Option<HoodieRestorePlan> scheduleRestore(
            HoodieEngineContext context,
    ...
    @Override
    public HoodieWriteMetadata<List<WriteStatus>> managePartitionTTL(
    ...
    @Override
    public HoodieRestoreMetadata restore(
            HoodieEngineContext context,
            String restoreInstantTimestamp,
            String savepointToRestoreTimestamp) {
    ...
    public Iterator<List<WriteStatus>> handleUpdate(
        ...
        HoodieMergeHandle upsertHandle =
                getUpdateHandle(instantTime, partitionPath, fileId, keyToNewRecords, oldDataFile);
        ...
        if (!config.populateMetaFields()) {
            try {
                ...
                throw new HoodieIOException(
                        "Only BaseKeyGenerator (or any key generator that extends from BaseKeyGenerator) are supported when meta "
                                + "columns are disabled. Please choose the right key generator if you wish to disable meta fields.",
                        e);
            }
        }
    ...
    public Iterator<List<WriteStatus>> handleInsert(
        ...
        HoodieCreateHandle<?, ?, ?, ?> createHandle =
                new HoodieCreateHandle(
                        config, instantTime, this, partitionPath, fileId, recordMap, taskContextSupplier);
        createHandle.write();
