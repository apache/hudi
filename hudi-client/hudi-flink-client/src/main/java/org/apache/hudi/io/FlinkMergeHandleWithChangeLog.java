...
/**
  ...
  *
  * <p>The cdc about logic is copied from {@link HoodieMergeHandleWithChangeLog}, we should refactor
  * it out when there are good abstractions.
  */
public class FlinkMergeHandleWithChangeLog<T, I, K, O> extends FlinkMergeHandle<T, I, K, O> {
    ...
    public FlinkMergeHandleWithChangeLog(
    ...
    protected boolean writeUpdateRecord(
        ...
        final boolean result =
                super.writeUpdateRecord(newRecord, oldRecord, combineRecordOpt, writerSchema);
        if (result) {
            boolean isDelete = HoodieOperation.isDelete(newRecord.getOperation());
            Option<IndexedRecord> avroRecordOpt =
                    savedCombineRecordOp.flatMap(
                            r -> toAvroRecord(r, writerSchema, config.getPayloadConfig().getProps()));
            cdcLogger.put(
                    newRecord,
                    (GenericRecord) oldRecord.getData(),
                    isDelete ? Option.empty() : avroRecordOpt);
        }
        return result;
    }
    ...
    protected void writeInsertRecord(HoodieRecord<T> newRecord) throws IOException {
        ...
        if (!HoodieOperation.isDelete(newRecord.getOperation())) {
            cdcLogger.put(
                    ...
                    null,
                    savedRecord
                            .toIndexedRecord(schema, config.getPayloadConfig().getProps())
