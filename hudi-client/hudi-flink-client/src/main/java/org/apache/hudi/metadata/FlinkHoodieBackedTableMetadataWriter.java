...
public class FlinkHoodieBackedTableMetadataWriter
    ...
    public static HoodieTableMetadataWriter create(
            StorageConfiguration<?> conf, HoodieWriteConfig writeConfig, HoodieEngineContext context) {
        return new FlinkHoodieBackedTableMetadataWriter(
                conf, writeConfig, EAGER, context, Option.empty());
    }
            ...
            this.metrics =
                    Option.of(
                            new HoodieMetadataMetrics(
                                    metadataWriteConfig.getMetricsConfig(), dataMetaClient.getStorage()));
        } else {
    ...
    @Override
    protected List<HoodieRecord> convertHoodieDataToEngineSpecificData(
    ...
    protected void bulkCommit(
            ...
            int fileGroupCount) {
        commitInternal(
                instantTime, Collections.singletonMap(partitionType, records), true, Option.empty());
    }
    ...
    protected void commitInternal(
        ...
        ValidationUtils.checkState(
                metadataMetaClient != null, "Metadata table is not fully initialized yet.");
        ...
        BaseHoodieWriteClient<?, List<HoodieRecord>, ?, List<WriteStatus>> writeClient =
                (BaseHoodieWriteClient<?, List<HoodieRecord>, ?, List<WriteStatus>>) getWriteClient();
        ...
        if (!metadataMetaClient.getActiveTimeline().containsInstant(instantTime)) {
            ...
            // for e.g., let's say compaction c1 on 1st attempt succeeded in metadata table and
            // failed before committing to datatable.
            // when retried again, data table will first rollback pending compaction. these will be
            // applied to metadata table, but all changes
            // are upserts to metadata table and so only a new delta commit will be created.
            // once rollback is complete in datatable, compaction will be retried again, which will
            // eventually hit this code block where the respective commit is
            // already part of completed commit. So, we have to manually rollback the completed
            // instant and proceed.
            Option<HoodieInstant> alreadyCompletedInstant =
                    metadataMetaClient
                            .getActiveTimeline()
                            .filterCompletedInstants()
                            .filter(entry -> entry.getTimestamp().equals(instantTime))
                            .lastInstant();
            LOG.info(
                    String.format(
                            "%s completed commit at %s being applied to MDT.",
            ...
            if (!writeClient.rollback(instantTime)) {
                throw new HoodieMetadataException(
                        "Failed to rollback deltacommit at " + instantTime + " from MDT");
            }
        ...
        List<WriteStatus> statuses =
                isInitializing
                        ? writeClient.bulkInsertPreppedRecords(
                                preppedRecordList, instantTime, bulkInsertPartitioner)
                        : writeClient.upsertPreppedRecords(preppedRecordList, instantTime);
        // flink does not support auto-commit yet, also the auto commit logic is not complete as
        // BaseHoodieWriteClient now.
        writeClient.commit(
                instantTime,
                statuses,
                Option.empty(),
    ...
    public void deletePartitions(String instantTime, List<MetadataPartitionType> partitions) {
        throw new HoodieNotSupportedException(
                "Dropping metadata index not supported for Flink metadata table yet.");
    }
    ...
    protected void preWrite(String instantTime) {
        metadataMetaClient
                .getActiveTimeline()
                .transitionRequestedToInflight(HoodieActiveTimeline.DELTA_COMMIT_ACTION, instantTime);
    }

    @Override
    protected HoodieData<HoodieRecord> getFunctionalIndexRecords(
            ...
            StorageConfiguration<?> storageConf) {
        throw new HoodieNotSupportedException(
                "Flink metadata table does not support functional index yet.");
    }

    @Override
    protected HoodieTable getHoodieTable(
    ...
    @Override
    public HoodieData<HoodieRecord> getDeletedSecondaryRecordMapping(
            HoodieEngineContext engineContext,
            Map<String, String> recordKeySecondaryKeyMap,
            HoodieIndexDefinition indexDefinition) {
        throw new HoodieNotSupportedException(
                "Flink metadata table does not support secondary index yet.");
    }
}
