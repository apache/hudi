...
public class HoodieFlinkTableServiceClient<T>
        extends BaseHoodieTableServiceClient<
                List<HoodieRecord<T>>, List<WriteStatus>, List<WriteStatus>> {

    private static final Logger LOG = LoggerFactory.getLogger(HoodieFlinkTableServiceClient.class);

    protected HoodieFlinkTableServiceClient(
    ...
    protected void completeCompaction(
        ...
        try {
            ...
            // Do not do any conflict resolution here as we do with regular writes. We take the lock
            // here to ensure all writes to metadata table happens within a
            // single lock (single writer). Because more than one write to metadata table will
            // result in conflicts since all of them updates the same partition.
        ...
        if (compactionTimer != null) {
            ...
            try {
                metrics.updateCommitMetrics(
                        HoodieActiveTimeline.parseDateFromInstantTime(compactionCommitTime).getTime(),
                        durationInMs,
                        metadata,
                        HoodieActiveTimeline.COMPACTION_ACTION);
            } catch (ParseException e) {
                throw new HoodieCommitException(
                        "Commit time is not of valid format. Failed to commit compaction "
                                + config.getBasePath()
                                + " at time "
    ...
    protected void completeClustering(
        ...
        this.context.setJobStatus(
                this.getClass().getSimpleName(), "Collect clustering write status and commit clustering");
        ...
        try {
            ...
            // commit to data table after committing to metadata table.
            // We take the lock here to ensure all writes to metadata table happens within a single
            // lock (single writer).
            // Because more than one write to metadata table will result in conflicts since all of
            // them updates the same partition.
            writeTableMetadata(
                    table, clusteringCommitTime, metadata, writeStatuses.orElseGet(context::emptyHoodieData));
        ...
        if (clusteringTimer != null) {
            ...
            try {
                metrics.updateCommitMetrics(
                        HoodieActiveTimeline.parseDateFromInstantTime(clusteringCommitTime).getTime(),
                        durationInMs,
                        metadata,
    ...
    public HoodieWriteMetadata<List<WriteStatus>> cluster(
            String clusteringInstant, boolean shouldComplete) {
        return null;
    }

    @Override
    protected void validateClusteringCommit(
            HoodieWriteMetadata<List<WriteStatus>> clusteringMetadata,
            String clusteringCommitTime,
            HoodieTable table) {
    ...
    @Override
    protected HoodieWriteMetadata<List<WriteStatus>> convertToOutputMetadata(
            HoodieWriteMetadata<List<WriteStatus>> writeMetadata) {
        return writeMetadata;
    }

    @Override
    protected HoodieData<WriteStatus> convertToWriteStatus(
            HoodieWriteMetadata<List<WriteStatus>> writeMetadata) {
        return HoodieListData.eager(writeMetadata.getWriteStatuses());
    }

    @Override
    protected HoodieTable<T, List<HoodieRecord<T>>, List<HoodieKey>, List<WriteStatus>> createTable(
    ...
    /**
      * Initialize the table metadata writer, for e.g, bootstrap the metadata table from the filesystem
        ...
        if (config.isMetadataTableEnabled()) {
            ...
            try {
                ...
                try (HoodieBackedTableMetadataWriter metadataWriter =
                        initMetadataWriter(latestPendingInstant)) {
