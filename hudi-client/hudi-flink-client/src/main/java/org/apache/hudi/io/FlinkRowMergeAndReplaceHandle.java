...
/**
 * A {@link HoodieRowMergeHandle} that supports MERGE write incrementally(small data buffers).
 *
 ...
 */
public class FlinkRowMergeAndReplaceHandle<T, I, K, O>
    extends HoodieRowMergeHandle<T, I, K, O>
    implements MiniBatchHandle {

  private static final Logger LOG = LoggerFactory.getLogger(FlinkRowMergeAndReplaceHandle.class);
  ...
  public FlinkRowMergeAndReplaceHandle(HoodieWriteConfig config, String instantTime, HoodieTable<T, I, K, O> hoodieTable,
  ...
  protected void makeOldAndNewFilePaths(String partitionPath, String oldFileName, String newFileName) {
    ...
    try {
      int rollNumber = 0;
      while (storage.exists(targetFilePath)) {
        StoragePath oldPath = targetFilePath;
        newFileName = newFileNameWithRollover(rollNumber++);
        targetFilePath = makeNewFilePath(partitionPath, newFileName);
        LOG.warn("Duplicate write for MERGE and REPLACE handle with path: " + oldPath + ", rolls over to new path: " + targetFilePath);
      }
    } catch (IOException e) {
      throw new HoodieException("Checking existing path for merge and replace handle error: " + targetFilePath.toString(), e);
    }
  }
  ...
  public void finalizeWrite() {
    ...
    try {
      storage.rename(targetFilePath, oldFilePath);
    } catch (IOException e) {
      throw new HoodieIOException("Error while renaming the temporary rollover file: "
          + targetFilePath + " to old base file name: " + oldFilePath, e);
    }
  }
  ...
  public void closeGracefully() {
    ...
    try {
      ...
      try {
        storage.deleteFile(targetFilePath);
        LOG.info("Deleting the intermediate MERGE and REPLACE data file: " + targetFilePath + " success!");
      } catch (IOException e) {
        // logging a warning and ignore the exception.
        LOG.warn("Deleting the intermediate MERGE and REPLACE data file: " + targetFilePath + " failed", e);
      }
    }
  }
