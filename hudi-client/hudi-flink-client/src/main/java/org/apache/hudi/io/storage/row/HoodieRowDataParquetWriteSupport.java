...
public class HoodieRowDataParquetWriteSupport extends RowDataParquetWriteSupport {
    ...
    public HoodieRowDataParquetWriteSupport(
            Configuration conf, RowType rowType, BloomFilter bloomFilter) {
        super(rowType);
        this.hadoopConf = new Configuration(conf);
        this.bloomFilterWriteSupportOpt =
                Option.ofNullable(bloomFilter).map(HoodieBloomFilterRowDataWriteSupport::new);
    }
