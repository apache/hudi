...
public class FlinkWriteHandleFactory {
    ...
    public static <T, I, K, O> Factory<T, I, K, O> getFactory(
            HoodieTableConfig tableConfig, HoodieWriteConfig writeConfig, boolean overwrite) {
        if (overwrite) {
    ...
    /**
      * Base clazz for commit write handle factory, it encapsulates the handle switching logic: INSERT
      * OR UPSERT.
      */
    private abstract static class BaseCommitWriteHandleFactory<T, I, K, O>
        ...
        public HoodieWriteHandle<?, ?, ?, ?> create(
            ...
            if (writePath != null) {
                HoodieWriteHandle<?, ?, ?, ?> writeHandle =
                        createReplaceHandle(
                                ...
                                partitionPath,
                                fileID,
                                convertToStoragePath(writePath));
                bucketToHandles.put(
                        fileID,
                        new Path(
                                ((MiniBatchHandle) writeHandle)
                                        .getWritePath()
                                        .toUri())); // override with new replace handle
                return writeHandle;
            }
            ...
            if (loc.getInstantTime().equals("I")) {
                writeHandle =
                        new FlinkCreateHandle<>(
                                config, instantTime, table, partitionPath, fileID, table.getTaskContextSupplier());
            } else {
                writeHandle =
                        createMergeHandle(config, instantTime, table, recordItr, partitionPath, fileID);
            }
    ...
    private static class CommitWriteHandleFactory<T, I, K, O>
        ...
        private static final CommitWriteHandleFactory<?, ?, ?, ?> INSTANCE =
                new CommitWriteHandleFactory<>();
        ...
        protected HoodieWriteHandle<?, ?, ?, ?> createReplaceHandle(
                ...
                String fileId,
                StoragePath basePath) {
            return new FlinkMergeAndReplaceHandle<>(
                    config,
                    instantTime,
                    table,
                    recordItr,
    ...
    private static class ClusterWriteHandleFactory<T, I, K, O>
            extends BaseCommitWriteHandleFactory<T, I, K, O> {
        private static final ClusterWriteHandleFactory<?, ?, ?, ?> INSTANCE =
                new ClusterWriteHandleFactory<>();
        ...
        protected HoodieWriteHandle<?, ?, ?, ?> createReplaceHandle(
                ...
                String fileId,
                StoragePath basePath) {
            return new FlinkConcatAndReplaceHandle<>(
                    config,
                    instantTime,
                    table,
                    recordItr,
    ...
    private static class CdcWriteHandleFactory<T, I, K, O>
        ...
        protected HoodieWriteHandle<?, ?, ?, ?> createReplaceHandle(
                ...
                String fileId,
                StoragePath basePath) {
            return new FlinkMergeAndReplaceHandleWithChangeLog<>(
                    config,
                    instantTime,
                    table,
                    recordItr,
        ...
        protected HoodieWriteHandle<?, ?, ?, ?> createMergeHandle(
                ...
                String partitionPath,
                String fileId) {
            return new FlinkMergeHandleWithChangeLog<>(
                    config,
                    instantTime,
                    table,
                    recordItr,
