...
public class ParquetRowDataWriter {
  ...
  public ParquetRowDataWriter(
    ...
    for (int i = 0; i < rowType.getFieldCount(); i++) {
      ...
      HoodieSchema fieldSchema = hoodieSchema.getField(fieldName).map(HoodieSchemaField::schema)
  ...
  private FieldWriter createWriter(LogicalType t, HoodieSchema hoodieSchema) {
    ...
    switch (t.getTypeRoot()) {
      ...
      case ARRAY:
        if (resolvedSchema != null && resolvedSchema.getType() == HoodieSchemaType.VECTOR) {
          return new VectorWriter((HoodieSchema.Vector) resolvedSchema);
        }
        ArrayType arrayType = (ArrayType) t;
        LogicalType elementType = arrayType.getElementType();
        FieldWriter elementWriter = createWriter(elementType, resolvedSchema == null ? null : resolvedSchema.getElementType());
        return new ArrayWriter(elementWriter);
      case MAP:
        MapType mapType = (MapType) t;
        LogicalType keyType = mapType.getKeyType();
        LogicalType valueType = mapType.getValueType();
        FieldWriter keyWriter = createWriter(keyType, resolvedSchema.getKeyType());
        FieldWriter valueWriter = createWriter(valueType, resolvedSchema.getValueType());
        return new MapWriter(keyWriter, valueWriter);
      case ROW:
        RowType rowType = (RowType) t;
        ValidationUtils.checkArgument(resolvedSchema.getType() == HoodieSchemaType.RECORD, "Hoodie schema should be RECORD type.");
