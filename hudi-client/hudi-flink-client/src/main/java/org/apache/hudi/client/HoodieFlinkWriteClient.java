...
/**
  ...
  *
  * <p>The client is used both on driver (for starting/committing transactions) and executor (for
  * writing dataset).
  *
...
public class HoodieFlinkWriteClient<T>
    ...
    /**
      * FileID to write handle mapping in order to record the write handles for each file group, so
      * that we can append the mini-batch data buffer incrementally.
      */
    ...
    public HoodieFlinkWriteClient(HoodieEngineContext context, HoodieWriteConfig writeConfig) {
        ...
        this.tableServiceClient =
                new HoodieFlinkTableServiceClient<>(context, writeConfig, getTimelineServer());
    }
    ...
    public boolean commit(
        ...
        List<HoodieWriteStat> merged =
                writeStats.stream()
                        .collect(
                                Collectors.groupingBy(
                                        writeStat -> writeStat.getPartitionPath() + writeStat.getPath()))
                        .values()
                        .stream()
        ...
        return commitStats(
                instantTime,
                HoodieListData.eager(writeStatuses),
                merged,
                extraMetadata,
                commitActionType,
    ...
    public List<HoodieRecord<T>> filterExists(List<HoodieRecord<T>> hoodieRecords) {
        ...
        List<HoodieRecord<T>> recordsWithLocation =
                getIndex().tagLocation(HoodieListData.eager(hoodieRecords), context, table).collectAsList();
        metrics.updateIndexMetrics(
                LOOKUP_STR, metrics.getDurationInMs(indexTimer == null ? 0L : indexTimer.stop()));
        return recordsWithLocation.stream()
                .filter(v1 -> !v1.isCurrentLocationKnown())
                .collect(Collectors.toList());
    }
    ...
    public List<WriteStatus> upsert(List<HoodieRecord<T>> records, String instantTime) {
        ...
        try (AutoCloseableWriteHandle closeableHandle =
                new AutoCloseableWriteHandle(records, instantTime, table)) {
            result =
                    ((HoodieFlinkTable<T>) table)
                            .upsert(context, closeableHandle.getWriteHandle(), instantTime, records);
        }
    ...
    @Override
    public List<WriteStatus> upsertPreppedRecords(
            List<HoodieRecord<T>> preppedRecords, String instantTime) {
        ...
        Map<String, List<HoodieRecord<T>>> preppedRecordsByFileId =
                preppedRecords.stream()
                        .parallel()
                        .collect(Collectors.groupingBy(r -> r.getCurrentLocation().getFileId()));
        return preppedRecordsByFileId.values().stream()
                .parallel()
                .map(
                        records -> {
                            HoodieWriteMetadata<List<WriteStatus>> result;
                            try (AutoCloseableWriteHandle closeableHandle =
                                ...
                                result =
                                        ((HoodieFlinkTable<T>) table)
                                                .upsertPrepped(
                                                        context, closeableHandle.getWriteHandle(), instantTime, records);
                            }
                            return postWrite(result, instantTime, table);
                        })
    ...
    public List<WriteStatus> insert(List<HoodieRecord<T>> records, String instantTime) {
        ...
        try (AutoCloseableWriteHandle closeableHandle =
            ...
            result =
                    ((HoodieFlinkTable<T>) table)
                            .insert(context, closeableHandle.getWriteHandle(), instantTime, records);
        }
        if (result.getIndexLookupDuration().isPresent()) {
            metrics.updateIndexMetrics(LOOKUP_STR, result.getIndexLookupDuration().get().toMillis());
        }
    ...
    /**
      * Removes all existing records from the partitions affected and inserts the given HoodieRecords,
      * into the table.
      *
    ...
    public List<WriteStatus> insertOverwrite(
        ...
        HoodieWriteMetadata<List<WriteStatus>> result;
        try (AutoCloseableWriteHandle closeableHandle =
                new AutoCloseableWriteHandle(records, instantTime, table, true)) {
            result =
                    ((HoodieFlinkTable<T>) table)
                            .insertOverwrite(context, closeableHandle.getWriteHandle(), instantTime, records);
        }
        return postWrite(result, instantTime, table);
    }

    /**
      * Removes all existing records of the Hoodie table and inserts the given HoodieRecords, into the
      * table.
      *
      * @param records HoodieRecords to insert
      * @param instantTime Instant time of the commit
    ...
    public List<WriteStatus> insertOverwriteTable(
            List<HoodieRecord<T>> records, final String instantTime) {
        HoodieTable table =
                initTable(WriteOperationType.INSERT_OVERWRITE_TABLE, Option.ofNullable(instantTime));
        table.validateInsertSchema();
    ...
    @Override
    public List<WriteStatus> insertPreppedRecords(
            List<HoodieRecord<T>> preppedRecords, String instantTime) {
    ...
    @Override
    public List<WriteStatus> bulkInsert(
    ...
    @Override
    public List<WriteStatus> bulkInsertPreppedRecords(
    ...
    @Override
    public List<WriteStatus> deletePrepped(
            List<HoodieRecord<T>> preppedRecords, final String instantTime) {
        HoodieTable<T, List<HoodieRecord<T>>, List<HoodieKey>, List<WriteStatus>> table =
    ...
    /**
      * Refresh the last transaction metadata, should be called before the Driver starts a new
      * transaction.
      */
    ...
    /**
      ...
      *
      * <p>The Flink write client is designed to write data set as buckets but cleaning action should
      * trigger after all the write actions within a checkpoint finish.
      */
    ...
    @Override
    public List<WriteStatus> postWrite(
            HoodieWriteMetadata<List<WriteStatus>> result, String instantTime, HoodieTable hoodieTable) {
        if (result.getIndexLookupDuration().isPresent()) {
            metrics.updateIndexMetrics(
                    getOperationType().name(), result.getIndexUpdateDuration().get().toMillis());
        }
    ...
    @Override
    protected HoodieWriteMetadata<List<WriteStatus>> compact(
    ...
    public HoodieWriteMetadata<List<WriteStatus>> cluster(
            final String clusteringInstant, final boolean shouldComplete) {
    ...
    private void completeClustering(
        ...
        ((HoodieFlinkTableServiceClient<T>) tableServiceClient)
                .completeClustering(metadata, table, clusteringCommitTime, writeStatuses);
    }
    ...
    protected void doInitTable(
        ...
        // flink executes the upgrade/downgrade once when initializing the first instant on start
        // up,
    ...
    public void completeTableService(
        ...
        switch (tableServiceType) {
            case CLUSTER:
                completeClustering(
                        (HoodieReplaceCommitMetadata) metadata, table, commitInstant, writeStatuses);
                break;
            case COMPACT:
    ...
    /**
      ...
      *
      * <p>This action should only be executed once for each commit. The modification of the table
    ...
    /**
      * Clean the write handles within a checkpoint interval. All the handles should have been closed
      * already.
      */
    ...
    private HoodieWriteHandle<?, ?, ?, ?> getOrCreateWriteHandle(
        ...
        FlinkWriteHandleFactory.Factory<T, List<HoodieRecord<T>>, List<HoodieKey>, List<WriteStatus>>
                writeHandleFactory =
                        FlinkWriteHandleFactory.getFactory(
                                table.getMetaClient().getTableConfig(), config, overwrite);
        return writeHandleFactory.create(
                this.bucketToHandles, record, config, instantTime, table, recordItr);
    }
    ...
    public Map<String, List<String>> getPartitionToReplacedFileIds(
            WriteOperationType writeOperationType, List<WriteStatus> writeStatuses) {
        HoodieFlinkTable<T> table = getHoodieTable();
        switch (writeOperationType) {
            case INSERT_OVERWRITE:
                return writeStatuses.stream()
                        .map(status -> status.getStat().getPartitionPath())
                        .distinct()
                        .collect(
                                Collectors.toMap(
                                        partition -> partition,
            ...
            case INSERT_OVERWRITE_TABLE:
                ...
                if (partitionPaths != null && partitionPaths.size() > 0) {
                    context.setJobStatus(
                            ...
                            "Getting ExistingFileIds of all partitions: " + config.getTableName());
        ...
        AutoCloseableWriteHandle(
            ...
            this.writeHandle =
                    getOrCreateWriteHandle(
                            records.get(0), getConfig(), instantTime, table, records.listIterator(), overwrite);
        }
