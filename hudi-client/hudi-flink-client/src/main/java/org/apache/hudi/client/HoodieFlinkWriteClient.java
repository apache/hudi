...
public class HoodieFlinkWriteClient<T extends HoodieRecordPayload> extends
  ...
  protected List<WriteStatus> compact(String compactionInstantTime, boolean shouldComplete) {
    ...
    try {
      HoodieFlinkCopyOnWriteTable flinkCopyOnWriteTable = new HoodieFlinkCopyOnWriteTable(
          getConfig(), getEngineContext(), getHoodieTable().getMetaClient());
      HoodieWriteMetadata<HoodieData<WriteStatus>> writeMetadata =
          CompactHelpers.newInstance().compact(context, getHoodieTable(), flinkCopyOnWriteTable,
              config, compactionInstantTime, this, new FlinkTaskContextSupplier(null));
      List<WriteStatus> writeStatuses =
