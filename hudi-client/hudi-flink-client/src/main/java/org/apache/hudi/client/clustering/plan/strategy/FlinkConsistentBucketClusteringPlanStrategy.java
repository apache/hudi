...
public class FlinkConsistentBucketClusteringPlanStrategy<T extends HoodieRecordPayload<T>>
        extends BaseConsistentHashingBucketClusteringPlanStrategy<
                T, List<HoodieRecord<T>>, List<HoodieKey>, List<WriteStatus>> {
    ...
    protected boolean isBucketClusteringMergeEnabled() {
        // Flink would not generate merge plan because it would cause multiple write sub tasks write
    ...
    protected boolean isBucketClusteringSortEnabled() {
        // Flink would not generate sort clustering plans for buckets that are not involved in merge
        // or split to avoid unnecessary clustering costs.
        return false;
    }
}
