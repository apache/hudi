...
/**
  ...
  * with a bucket ID (partition path + fileID) in streaming way. All the records consumed by this
  * executor should be tagged with bucket IDs and belong to one data bucket.
  *
  * <p>These bucket IDs make it possible to shuffle the records first by the bucket ID (see
  * org.apache.hudi.operator.partitioner.BucketAssignerFunction), and this executor only needs to
  * handle the data buffer that belongs to one data bucket once at a time. So there is no need to
...
public abstract class BaseFlinkCommitActionExecutor<T>
    ...
    public BaseFlinkCommitActionExecutor(
    ...
    public HoodieWriteMetadata<List<WriteStatus>> execute(List<HoodieRecord<T>> inputRecords) {
        ...
        final BucketType bucketType =
                record.getCurrentLocation().getInstantTime().equals("I")
                        ? BucketType.INSERT
                        : BucketType.UPDATE;
        handleUpsertPartition(partitionPath, fileId, bucketType, inputRecords.iterator())
    ...
    protected void setUpWriteMetadata(
            List<WriteStatus> statuses, HoodieWriteMetadata<List<WriteStatus>> result) {
    ...
    protected Iterator<List<WriteStatus>> handleUpsertPartition(
            String partitionPath, String fileIdHint, BucketType bucketType, Iterator recordItr) {
        try {
    ...
    public Iterator<List<WriteStatus>> handleInsert(String idPfx, Iterator<HoodieRecord<T>> recordItr)
        ...
        return new FlinkLazyInsertIterable<>(
                recordItr,
                true,
                config,
                instantTime,
                ...
                idPfx,
                taskContextSupplier,
                new ExplicitWriteHandleFactory<>(writeHandle));
    }
}
