...
/**
  ...
  *
  * <p>Reference org.apache.flink.formats.parquet.utils.ParquetSchemaConverter to support timestamp
  * of INT64 8 bytes.
  */
                ...
                switch (originalType) {
                            ...
                            if (elementType.getFieldCount() > 1) {
                                ...
                                if (internalType.isPrimitive()) {
                                    ...
                                    if (tupleGroup.getFieldCount() == 1
                                            && tupleGroup.getFields().get(0).isRepetition(Type.Repetition.REQUIRED)) {
                                        typeInfo =
                                                ObjectArrayTypeInfo.getInfoFor(convertParquetTypeToTypeInfo(internalType));
                                    } else {
                                        throw new UnsupportedOperationException(
                                                String.format(
                                                        "Unrecgonized List schema [%s] according to Parquet" + " standard",
                    ...
                    case MAP:
                        ...
                        return new MapTypeInfo<>(
                                BasicTypeInfo.STRING_TYPE_INFO, convertParquetTypeToTypeInfo(valueType));
                    default:
                        throw new UnsupportedOperationException("Unsupported schema: " + fieldType);
                }
            } else {
    ...
    private static Type convertField(
        ...
        if (typeInfo instanceof BasicTypeInfo) {
            ...
            if (basicTypeInfo.equals(BasicTypeInfo.BIG_DEC_TYPE_INFO)
                ...
                fieldType =
                        Types.primitive(PrimitiveType.PrimitiveTypeName.DOUBLE, repetition).named(fieldName);
            } else if (basicTypeInfo.equals(BasicTypeInfo.FLOAT_TYPE_INFO)) {
                fieldType =
                        Types.primitive(PrimitiveType.PrimitiveTypeName.FLOAT, repetition).named(fieldName);
                ...
                fieldType =
                        Types.primitive(PrimitiveType.PrimitiveTypeName.BOOLEAN, repetition).named(fieldName);
            ...
            if (legacyMode) {
                ...
                fieldType =
                        Types.buildGroup(repetition).addField(listGroup).as(OriginalType.LIST).named(fieldName);
            } else {
        ...
        switch (type.getTypeRoot()) {
            ...
            case TIMESTAMP_WITH_LOCAL_TIME_ZONE:
                ...
                if (localZonedTimestampType.getPrecision() == 3
                    ...
                    TimeUnit timeunit =
                            localZonedTimestampType.getPrecision() == 3 ? TimeUnit.MILLIS : TimeUnit.MICROS;
                    return Types.primitive(PrimitiveType.PrimitiveTypeName.INT64, repetition)
