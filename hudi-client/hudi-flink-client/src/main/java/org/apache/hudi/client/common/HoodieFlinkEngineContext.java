...
public class HoodieFlinkEngineContext extends HoodieEngineContext {
    ...
    private HoodieFlinkEngineContext() {
        this(
                HadoopFSUtils.getStorageConf(FlinkClientUtil.getHadoopConf()),
                new DefaultTaskContextSupplier());
    }
    ...
    public HoodieFlinkEngineContext(
            StorageConfiguration<?> storageConf, TaskContextSupplier taskContextSupplier) {
    ...
    public <I, K, V> List<V> mapToPairAndReduceByKey(
        ...
        return data.stream()
                .parallel()
                .map(throwingMapToPairWrapper(mapToPairFunc))
                ...
                .map(
                        list ->
                                list.stream()
                                        .map(e -> e.getValue())
                                        .reduce(throwingReduceWrapper(reduceFunc))
                                        .orElse(null))
    ...
    public <I, O> List<O> flatMap(
        ...
        return data.stream()
                .parallel()
                .flatMap(throwingFlatMapWrapper(func))
                .collect(Collectors.toList());
    }
    ...
    @Override
    public <I, K, V> Map<K, V> mapToPair(
            List<I> data, SerializablePairFunction<I, K, V> func, Integer parallelism) {
        return data.stream()
                .parallel()
                .map(throwingMapToPairWrapper(func))
