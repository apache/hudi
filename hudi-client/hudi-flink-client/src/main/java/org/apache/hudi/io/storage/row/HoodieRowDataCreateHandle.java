...
public class HoodieRowDataCreateHandle implements Serializable {
    ...
    public HoodieRowDataCreateHandle(
            HoodieTable table,
        ...
        try {
            ...
            createMarkerFile(
                    ...
                    FSUtils.makeBaseFileName(
                            this.instantTime, getWriteToken(), this.fileId, table.getBaseFileExtension()));
    ...
    /**
      * Writes an {@link RowData} to the underlying {@link HoodieRowDataFileWriter}. Before writing,
      * value for meta columns are computed as required and wrapped in {@link HoodieRowData}. {@link
      * HoodieRowData} is what gets written to HoodieRowDataFileWriter.
      *
            ...
            String seqId =
                    preserveHoodieMetadata
                            ...
                            : HoodieRecord.generateSequenceId(
                                    instantTime, taskPartitionId, SEQGEN.getAndIncrement());
            String commitInstant =
                    preserveHoodieMetadata
                            ? record.getString(HoodieRecord.COMMIT_TIME_METADATA_FIELD_ORD).toString()
                            : instantTime;
            RowData rowData =
                    HoodieRowDataCreation.create(
                            commitInstant,
                            seqId,
                            recordKey,
                            partitionPath,
            ...
            try {
                ...
                HoodieRecordDelegate recordDelegate =
                        writeStatus.isTrackingSuccessfulWrites()
                                ? HoodieRecordDelegate.create(recordKey, partitionPath, null, newRecordLocation)
                                : null;
    ...
    /**
      * Closes the {@link HoodieRowDataCreateHandle} and returns an instance of {@link WriteStatus}
    ...
    private Path makeNewPath(String partitionPath) {
        StoragePath path = FSUtils.constructAbsolutePath(writeConfig.getBasePath(), partitionPath);
        try {
        ...
        return new Path(
                path.toString(),
                FSUtils.makeBaseFileName(
                        instantTime, getWriteToken(), fileId, table.getBaseFileExtension()));
    }
