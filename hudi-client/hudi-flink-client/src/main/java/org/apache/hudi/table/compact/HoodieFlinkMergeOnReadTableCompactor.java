...
public class HoodieFlinkMergeOnReadTableCompactor<T extends HoodieRecordPayload> implements HoodieCompactor<T, List<HoodieRecord<T>>, List<HoodieKey>, List<WriteStatus>> {
  ...
  private LongCounter totalLogFiles;
  // Accumulator to keep track of total log file slices for a table
  private LongCounter totalFileSlices;
  ...
  public List<WriteStatus> compact(HoodieEngineContext context, HoodieCompactionPlan compactionPlan,
    ...
    return context.map(operations, s -> compact(table, metaClient, config, s, compactionInstantTime, (HoodieFlinkEngineContext)context),
        0).stream().flatMap(List::stream).collect(Collectors.toList());

  }

  private List<WriteStatus> compact(HoodieFlinkCopyOnWriteTable hoodieCopyOnWriteTable, HoodieTableMetaClient metaClient,
                                    HoodieWriteConfig config, CompactionOperation operation, String instantTime, HoodieFlinkEngineContext context) throws IOException {
    ...
    long maxMemoryPerCompaction = IOUtils.getMaxMemoryPerCompaction(context.getTaskContextSupplier(), config.getProps());
  ...
  public HoodieCompactionPlan generateCompactionPlan(HoodieEngineContext context,
    ...
    RuntimeContext runtimeContext = ((HoodieFlinkEngineContext)context).getRuntimeContext();
    totalLogFiles = new LongCounter();
    totalFileSlices = new LongCounter();
    runtimeContext.addAccumulator("totalLogFiles", totalLogFiles);
    runtimeContext.addAccumulator("totalFileSlices", totalFileSlices);
    ...
    LOG.info("Total number of latest files slices " + totalFileSlices.getLocalValue());
    LOG.info("Total number of log files " + totalLogFiles.getLocalValue());
    LOG.info("Total number of file slices " + totalFileSlices.getLocalValue());
