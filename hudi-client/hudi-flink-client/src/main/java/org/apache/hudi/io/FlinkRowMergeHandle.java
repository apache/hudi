...
public class FlinkRowMergeHandle<T, I, K, O>
  ...
  protected void makeOldAndNewFilePaths(String partitionPath, String oldFileName, String newFileName) {
    ...
    try {
      ...
      while (storage.exists(targetFilePath)) {
        ...
        if (storage.getPathInfo(targetFilePath).getLength() <= 0) {
        ...
        LOG.warn("Duplicate write for MERGE bucket with path: " + oldFilePath
            + ", rolls over to new path: " + targetFilePath);
      }
    } catch (IOException e) {
  ...
  public void finalizeWrite() {
    ...
    try {
      storage.rename(targetFilePath, desiredPath);
    } catch (IOException e) {
      throw new HoodieIOException(
          "Error when rename the temporary roll file: " + targetFilePath + " to: " + desiredPath, e);
    }
  }
