...
public class ScheduleCompactionsActionExecutor<T extends HoodieRecordPayload, I, K, O> extends BaseActionExecutor<T, I, K, O, Option<List<HoodieCompactionPlan>>> {

    private static final Logger LOG = LogManager.getLogger(ScheduleCompactionActionExecutor.class);

    private final Option<Map<String, String>> extraMetadata;
    private final HoodieFlinkMergeOnReadTableCompactor compactor;

    public ScheduleCompactionsActionExecutor(HoodieEngineContext context,
    ...
    @Override
    public Option<List<HoodieCompactionPlan>> execute() {
        ...
        List<HoodieCompactionPlan> plans = scheduleCompaction();
        AtomicLong num = new AtomicLong(0);
        plans.stream().forEach(plan -> {
            if (plan != null && (plan.getOperations() != null) && (!plan.getOperations().isEmpty())) {
                extraMetadata.ifPresent(plan::setExtraMetadata);
                HoodieInstant compactionInstant =
                        new HoodieInstant(HoodieInstant.State.REQUESTED, HoodieTimeline.COMPACTION_ACTION, String.valueOf(Long.valueOf(instantTime) + num.getAndAdd(1)));
                try {
        ...
        return Option.of(plans);
    }

    private List<HoodieCompactionPlan> scheduleCompaction() {
        ...
        if (compactable) {
            ...
            try {
                ...
                return compactor.generateCompactionPlans(context, table, config, instantTime, fgInPendingCompactionAndClustering);
            } catch (IOException e) {
                throw new HoodieCompactionException("Could not schedule compaction " + config.getBasePath(), e);
            }
        }
        return new ArrayList<HoodieCompactionPlan>();
    }
