...
/**
  ...
  *
  * <p>For a new data buffer, it initializes and set up the next file path to write, and closes the
  * file path when the data buffer write finish. When next data buffer write starts, it rolls over to
  * another new file. If all the data buffers write finish for a checkpoint round, it renames the
  * last new file path as the desired file name (name with the expected file ID).
  *
  * @see FlinkMergeAndReplaceHandle
  */
public class FlinkMergeHandle<T, I, K, O> extends HoodieMergeHandle<T, I, K, O>
    ...
    private void deleteInvalidDataFile(long lastAttemptId) {
        ...
        try {
            ...
            throw new HoodieException(
                    "Error while deleting the MERGE base file due to task retry: " + lastDataFileName, e);
        }
    }
    ...
    protected void makeOldAndNewFilePaths(
        ...
        // If the data file already exists, it means the write task write merge data bucket multiple
        // times
        ...
        try {
            ...
            while (storage.exists(newFilePath)) {
                ...
                LOG.warn(
                        "Duplicate write for MERGE bucket with path: "
                                + oldFilePath
