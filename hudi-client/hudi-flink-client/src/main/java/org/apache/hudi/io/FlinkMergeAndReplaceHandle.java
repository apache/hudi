...
public class FlinkMergeAndReplaceHandle<T, I, K, O>
  ...
  private void deleteInvalidDataFile(long lastAttemptId) {
    ...
    final StoragePath location = makeNewFilePath(partitionPath, lastDataFileName);
    try {
  ...
  protected void makeOldAndNewFilePaths(String partitionPath, String oldFileName, String newFileName) {
    ...
    try {
      ...
      while (storage.exists(newFileLocation)) {
        StoragePath oldPath = newFileLocation;
      ...
      throw new HoodieException("Checking existing path for merge and replace handle error: " + newFileLocation, e);
    }
  }
  ...
  protected void setWriteStatusPath() {
    ...
    writeStatus.getStat().setPath(new StoragePath(config.getBasePath()), oldFileLocation);
  }
  ...
  public void finalizeWrite() {
    ...
    try {
      storage.deleteFile(oldFileLocation);
    } catch (IOException e) {
      throw new HoodieIOException("Error while cleaning the old base file: " + oldFileLocation, e);
    }
    try {
      storage.rename(newFileLocation, oldFileLocation);
    } catch (IOException e) {
      throw new HoodieIOException("Error while renaming the temporary rollover file: "
          + newFileLocation + " to old base file name: " + oldFileLocation, e);
    }
  }
  ...
  public void closeGracefully() {
    ...
    try {
      ...
      try {
        ...
        LOG.info("Deleting the intermediate MERGE and REPLACE data file: " + newFileLocation + " success!");
      } catch (IOException e) {
        // logging a warning and ignore the exception.
        LOG.warn("Deleting the intermediate MERGE and REPLACE data file: " + newFileLocation + " failed", e);
      }
    }
  }
