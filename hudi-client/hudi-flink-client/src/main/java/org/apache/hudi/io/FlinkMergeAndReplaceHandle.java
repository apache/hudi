...
/**
  ...
  *
  * <p>This handle is needed from the second mini-batch write for COW data bucket when the data
  * bucket is written using multiple mini-batches.
  *
  * <p>For the incremental data buffer, it initializes and sets up the next file path to write, then
  * closes the file and rename to the old file name, behaves like the new data buffer are appended to
  * the old file.
  */
public class FlinkMergeAndReplaceHandle<T, I, K, O> extends HoodieMergeHandle<T, I, K, O>
    ...
    public FlinkMergeAndReplaceHandle(
    ...
    /**
      * The flink checkpoints start in sequence and asynchronously, when one write task finish the
      * checkpoint(A) (thus the fs view got the written data files some of which may be invalid), it
      * goes on with the next round checkpoint(B) write immediately, if it tries to reuse the last
      * small data bucket(small file) of an invalid data file, finally, when the coordinator receives
      * the checkpoint success event of checkpoint(A), the invalid data file would be cleaned, and this
      * merger got a FileNotFoundException when it close the write file handle.
      *
      * <p>To solve, deletes the invalid data file eagerly so that the invalid file small bucket would
      * never be reused.
      *
      * @param lastAttemptId The last attempt ID
      */
    private void deleteInvalidDataFile(long lastAttemptId) {
        final String lastWriteToken =
                FSUtils.makeWriteToken(getPartitionId(), getStageId(), lastAttemptId);
        final String lastDataFileName =
                FSUtils.makeBaseFileName(
                        instantTime, lastWriteToken, this.fileId, hoodieTable.getBaseFileExtension());
        final StoragePath path = makeNewFilePath(partitionPath, lastDataFileName);
        try {
            if (storage.exists(path)) {
                LOG.info(
                        "Deleting invalid MERGE and REPLACE base file due to task retry: " + lastDataFileName);
                storage.deleteFile(path);
            }
        } catch (IOException e) {
            throw new HoodieException(
                    "Error while deleting the MERGE and REPLACE base file due to task retry: "
                            + lastDataFileName,
                    e);
        }
    }
    ...
    protected void makeOldAndNewFilePaths(
        ...
        try {
            ...
            while (storage.exists(newFilePath)) {
                ...
                LOG.warn(
                        "Duplicate write for MERGE and REPLACE handle with path: "
                                + oldPath
                                + ", rolls over to new path: "
                                + newFilePath);
            }
        } catch (IOException e) {
            throw new HoodieException(
                    "Checking existing path for merge and replace handle error: " + newFilePath, e);
        }
    }
    ...
    boolean needsUpdateLocation() {
        // No need to update location for Flink hoodie records because all the records are
        // pre-tagged
    ...
    public void finalizeWrite() {
        ...
        try {
            ...
            throw new HoodieIOException(
                    "Error while renaming the temporary rollover file: "
                            + newFilePath
                            + " to old base file name: "
                            + oldFilePath,
                    e);
        }
    }
    ...
    public void closeGracefully() {
        ...
        try {
            ...
            try {
                ...
                LOG.info(
                        "Deleting the intermediate MERGE and REPLACE data file: " + newFilePath + " success!");
            } catch (IOException e) {
                // logging a warning and ignore the exception.
                LOG.warn(
                        "Deleting the intermediate MERGE and REPLACE data file: " + newFilePath + " failed", e);
            }
        }
    }
