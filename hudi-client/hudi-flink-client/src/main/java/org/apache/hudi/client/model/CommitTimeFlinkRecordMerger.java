...
public class CommitTimeFlinkRecordMerger extends HoodieFlinkRecordMerger {
  ...
  public Option<Pair<HoodieRecord, Schema>> merge(
    ...
    if (oldSchema.getField(HoodieRecord.COMMIT_TIME_METADATA_FIELD) != null
        && newSchema.getField(HoodieRecord.COMMIT_TIME_METADATA_FIELD) != null) {
      Comparable olderFlinkCommitTime = (Comparable) older.getColumnValues(oldSchema,
          new String[] {HoodieRecord.COMMIT_TIME_METADATA_FIELD}, false)[0];
      Comparable newerFlinkCommitTime = (Comparable) newer.getColumnValues(newSchema,
          new String[] {HoodieRecord.COMMIT_TIME_METADATA_FIELD}, false)[0];

      if (olderFlinkCommitTime.compareTo(newerFlinkCommitTime) > 0) {
        return Option.of(Pair.of(older, oldSchema));
      } else if (olderFlinkCommitTime.compareTo(newerFlinkCommitTime) < 0) {
