...
public class HoodieFlinkWriteableTestTable extends HoodieWriteableTestTable {
    ...
    private HoodieFlinkWriteableTestTable(
            String basePath,
    ...
    public static HoodieFlinkWriteableTestTable of(
            HoodieTableMetaClient metaClient, Schema schema, BloomFilter filter) {
        return new HoodieFlinkWriteableTestTable(
    ...
    public HoodieFlinkWriteableTestTable withInserts(String partition, String fileId)
            throws Exception {
        return withInserts(partition, fileId, new HoodieRecord[0]);
    }

    public HoodieFlinkWriteableTestTable withInserts(
    ...
    public HoodieFlinkWriteableTestTable withInserts(
        ...
        withInserts(
                partition, fileId, records, new org.apache.hudi.client.FlinkTaskContextSupplier(null));
        return this;
    }

    public Map<String, List<HoodieLogFile>> withLogAppends(List<HoodieRecord> records)
            throws Exception {
        Map<String, List<HoodieLogFile>> partitionToLogfilesMap = new HashMap<>();
        for (List<HoodieRecord> groupedRecords :
                records.stream()
    ...
    private Pair<String, HoodieLogFile> appendRecordsToLogFile(List<HoodieRecord> groupedRecords)
            throws Exception {
        ...
        try (HoodieLogFormat.Writer logWriter =
                HoodieLogFormat.newWriterBuilder()
                        ...
                        .withDeltaCommit(location.getInstantTime())
                        .withStorage(storage)
                        .build()) {
