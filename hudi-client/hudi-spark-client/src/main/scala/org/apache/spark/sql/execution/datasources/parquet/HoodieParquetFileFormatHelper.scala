...
object HoodieParquetFileFormatHelper {
  ...
  def generateUnsafeProjection(fullSchema: Seq[Attribute],
    ...
    val specialCastCache = scala.collection.mutable.HashMap.empty[(DataType, DataType), Boolean]

    def hasSpecialCast(src: DataType, dst: DataType): Boolean = {
      specialCastCache.getOrElseUpdate((src, dst), {
        (src, dst) match {
          ...
          case (StructType(srcFields), StructType(dstFields)) =>
            srcFields.zip(dstFields).exists { case (sf, df) => hasSpecialCast(sf.dataType, df.dataType) }
          case (ArrayType(sElem, _), ArrayType(dElem, _)) =>
            hasSpecialCast(sElem, dElem)
          case (MapType(sKey, sVal, _), MapType(dKey, dVal, _)) =>
            hasSpecialCast(sKey, dKey) || hasSpecialCast(sVal, dVal)
          case _ => false
        }
      })
    }

    def repairSpecialCasts(expr: Expression, srcType: DataType, dstType: DataType): Expression = {
      ...
      (srcType, dstType) match {
        case (FloatType, DoubleType) =>
          ...
          Cast(toStr, DoubleType, if (needTimeZone) timeZoneId else None)
        ...
        case (s: StructType, d: StructType) if hasSpecialCast(s, d) =>
          val structFields = s.fields.zip(d.fields).zipWithIndex.map {
            case ((srcField, dstField), i) =>
              ...
              repairSpecialCasts(child, srcField.dataType, dstField.dataType)
          }
          CreateNamedStruct(d.fields.zip(structFields).flatMap {
            case (f, c) => Seq(Literal(f.name), c)
          })

        case (ArrayType(sElementType, containsNull), ArrayType(dElementType, _)) if hasSpecialCast(sElementType, dElementType) =>
          val lambdaVar = NamedLambdaVariable("x", sElementType, containsNull)
          val body = repairSpecialCasts(lambdaVar, sElementType, dElementType)
          val func = LambdaFunction(body, Seq(lambdaVar))
          ArrayTransform(expr, func)

        case (MapType(sKeyType, sValType, vnull), MapType(dKeyType, dValType, _)) if hasSpecialCast(sKeyType, dKeyType) || hasSpecialCast(sValType, dValType) =>
          ...
          val newKey = repairSpecialCasts(GetStructField(kv, 0), sKeyType, dKeyType)
          val newVal = repairSpecialCasts(GetStructField(kv, 1), sValType, dValType)
    ...
    if (typeChangeInfos.isEmpty) {
      ...
      val castSchema = newFullSchema.zipWithIndex.map { case (attr, i) =>
        if (typeChangeInfos.containsKey(i)) {
          ...
          repairSpecialCasts(attr, srcType, dstType)
        } else attr
      }
