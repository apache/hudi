...
class SparkFileFormatInternalRowReaderContext(readerMaps: mutable.Map[Long, PartitionedFile => Iterator[InternalRow]]) extends BaseSparkInternalRowReaderContext {
  ...
  override def getFileRecordIterator(fileLocation: StoragePath,
                                     start: Long,
                                     length: Long,
                                     dataSchema: Schema,
    ...
    val fileInfo = sparkAdapter.getSparkPartitionedFileUtils
      .createPartitionedFile(InternalRow.empty, fileLocation, start, length)
    if (FSUtils.isLogFile(fileLocation)) {
      ...
      new CloseableMappingIterator[InternalRow, UnsafeRow](
        sparkFileReaderFactory.newParquetFileReader(conf, fileLocation).asInstanceOf[HoodieSparkParquetReader]
