...
import org.apache.hudi.table.action.commit.{BulkInsertDataInternalWriterHelper, ConsistentBucketBulkInsertDataInternalWriterHelper}
...
trait HoodieDatasetBulkInsertHelper {
  /**
  ...
  private def dedupeRows(rdd: RDD[InternalRow], schema: StructType, preCombineFieldRef: String, isGlobalIndex: Boolean, targetParallelism: Int): RDD[InternalRow] = {
    ...
    rdd.map { row =>
      ...
      .reduceByKey((oneRow, otherRow) => {
        ...
        if (compareRow(onePreCombineVal, otherPreCombineVal) >= 0) {
          oneRow
        } else {
          otherRow
        }
  ...
  protected def compareRow(a: Comparable[AnyRef], b: Comparable[AnyRef]): Int = {
    a.compareTo(b.asInstanceOf[AnyRef])
  }

  protected def deduceShuffleParallelism(input: DataFrame, configuredParallelism: Int): Int

  protected def getPartitionPathFields(config: HoodieWriteConfig): mutable.Seq[String] = {
