...
public final class SparkHoodieIndexFactory {
    public static HoodieIndex createIndex(HoodieWriteConfig config) {
        boolean sqlMergeIntoPrepped =
                config.getProps().getBoolean(HoodieWriteConfig.SPARK_SQL_MERGE_INTO_PREPPED_KEY, false);
        ...
        switch (config.getIndexType()) {
            ...
            case SIMPLE:
                return new HoodieSimpleIndex(
                        config, HoodieSparkKeyGeneratorFactory.createBaseKeyGenerator(config));
            case GLOBAL_SIMPLE:
                return new HoodieGlobalSimpleIndex(
                        config, HoodieSparkKeyGeneratorFactory.createBaseKeyGenerator(config));
            case BUCKET:
                switch (config.getBucketIndexEngineType()) {
                    ...
                    default:
                        throw new HoodieIndexException(
                                "Unknown bucket index engine type: " + config.getBucketIndexEngineType());
                }
            case RECORD_INDEX:
