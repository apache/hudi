...
public class SparkBucketIndexPartitioner<T> extends SparkHoodiePartitioner<T> {
    ...
    /**
      * Helps get the RDD partition id, partition id is partition offset + bucket id. The partition
      * offset is a multiple of the bucket num.
      */
    ...
    /**
      * Partition path and file groups in it pair. Decide the file group an incoming update should go
      * to.
      */
    ...
    public SparkBucketIndexPartitioner(
        ...
        this.isOverwrite =
                INSERT_OVERWRITE.equals(operationType) || INSERT_OVERWRITE_TABLE.equals(operationType);
    ...
    public BucketInfo getBucketInfo(int bucketNumber) {
        ...
        if (isOverwrite) {
            return new BucketInfo(
                    BucketType.INSERT, BucketIdentifier.newBucketFileIdPrefix(bucketId), partitionPath);
        }
        Option<String> fileIdOption =
                Option.fromJavaOptional(
                        updatePartitionPathFileIds.getOrDefault(partitionPath, Collections.emptySet()).stream()
        ...
        if (fileIdOption.isPresent()) {
            ...
            BucketType bucketType =
                    isNonBlockingConcurrencyControl ? BucketType.UPDATE : BucketType.INSERT;
            String fileIdPrefix =
                    BucketIdentifier.newBucketFileIdPrefix(bucketId, isNonBlockingConcurrencyControl);
