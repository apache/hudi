...
public class UpsertPartitioner<T> extends SparkHoodiePartitioner<T> {
    ...
    public UpsertPartitioner(
        ...
        long totalInserts =
                profile.getInputPartitionPathStatMap().values().stream()
                        .mapToLong(stat -> stat.getNumInserts())
                        .sum();
        if (!WriteOperationType.isPreppedWriteOperation(operationType)
                || totalInserts > 0) { // skip if its prepped write operation. or if totalInserts = 0.
            assignInserts(profile, context);
        }

        LOG.info(
                "Total Buckets: {}, bucketInfoMap size: {}, partitionPathToInsertBucketInfos size: {}, updateLocationToBucket size: {}",
                totalBuckets,
                bucketInfoMap.size(),
                partitionPathToInsertBucketInfos.size(),
                updateLocationToBucket.size());
        ...
        for (Map.Entry<String, WorkloadStat> partitionStat : partitionStatEntries) {
            WorkloadStat outputWorkloadStats =
                    profile
                            ...
                            .getOrDefault(partitionStat.getKey(), new WorkloadStat());
            for (Map.Entry<String, Pair<String, Long>> updateLocEntry :
                ...
                if (profile.hasOutputWorkLoadStats()) {
                    HoodieRecordLocation hoodieRecordLocation =
                            new HoodieRecordLocation(updateLocEntry.getValue().getKey(), updateLocEntry.getKey());
                    outputWorkloadStats.addUpdates(
                            hoodieRecordLocation, updateLocEntry.getValue().getValue());
                }
            }
            if (profile.hasOutputWorkLoadStats()) {
    ...
    private Map<String, Set<String>> getPartitionPathToPendingClusteringFileGroupsId() {
        Map<String, Set<String>> partitionPathToInPendingClusteringFileId =
                table
                        ...
                        .map(
                                fileGroupIdAndInstantPair ->
                                        Pair.of(
                                                fileGroupIdAndInstantPair.getKey().getPartitionPath(),
    ...
    /**
      ...
      */
    private List<SmallFile> filterSmallFilesInClustering(
            final Set<String> pendingClusteringFileGroupsId, final List<SmallFile> smallFiles) {
        if (!pendingClusteringFileGroupsId.isEmpty()) {
            return smallFiles.stream()
                    .filter(
                            smallFile -> !pendingClusteringFileGroupsId.contains(smallFile.location.getFileId()))
    ...
    private void assignInserts(WorkloadProfile profile, HoodieEngineContext context) {
        ...
        long averageRecordSize =
                AverageRecordSizeUtils.averageBytesPerRecord(
                        table
                                .getMetaClient()
                                .getActiveTimeline()
        ...
        for (String partitionPath : partitionPaths) {
            ...
            WorkloadStat outputWorkloadStats =
                    profile.getOutputPartitionPathStatMap().getOrDefault(partitionPath, new WorkloadStat());
            if (pStat.getNumInserts() > 0) {

                List<SmallFile> smallFiles =
                        filterSmallFilesInClustering(
                                partitionPathToPendingClusteringFileGroupsId.getOrDefault(
                                        partitionPath, Collections.emptySet()),
                                partitionSmallFilesMap.getOrDefault(partitionPath, Collections.emptyList()));

                this.smallFiles.addAll(smallFiles);

                LOG.info(
                        "For partitionPath : {} Total Small Files => {}", partitionPath, smallFiles.size());
                ...
                for (SmallFile smallFile : smallFiles) {
                    ...
                    if (recordsToAppend > 0) {
                        ...
                        if (updateLocationToBucket.containsKey(smallFile.location.getFileId())) {
                            ...
                            LOG.debug(
                                    "Assigning " + recordsToAppend + " inserts to existing update bucket " + bucket);
                        } else {
                ...
                if (totalUnassignedInserts > 0) {
                    ...
                    if (config.shouldAutoTuneInsertSplits()) {
                        insertRecordsPerBucket =
                                (int) Math.ceil((1.0 * config.getParquetMaxFileSize()) / averageRecordSize);
                    }

                    int insertBuckets =
                            (int) Math.ceil((1.0 * totalUnassignedInserts) / insertRecordsPerBucket);
                    LOG.info(
                    ...
                    for (int b = 0; b < insertBuckets; b++) {
                        ...
                        if (b < insertBuckets - 1) {
                            ...
                            recordsPerBucket.add(
                                    totalUnassignedInserts - (insertBuckets - 1) * insertRecordsPerBucket);
                        }
                        BucketInfo bucketInfo =
                                new BucketInfo(BucketType.INSERT, FSUtils.createNewFileIdPfx(), partitionPath);
                        bucketInfoMap.put(totalBuckets, bucketInfo);
                        if (profile.hasOutputWorkLoadStats()) {
                            outputWorkloadStats.addInserts(
                                    new HoodieRecordLocation(
                                            HoodieWriteStat.NULL_COMMIT, bucketInfo.getFileIdPrefix()),
                                    recordsPerBucket.get(recordsPerBucket.size() - 1));
                        }
                        totalBuckets++;
                    }
                }
    ...
    private Map<String, List<SmallFile>> getSmallFilesForPartitions(
            List<String> partitionPaths, HoodieEngineContext context) {
        ...
        if (partitionPaths != null && partitionPaths.size() > 0) {
            context.setJobStatus(
                    ...
                    "Getting small files from partitions: " + config.getTableName());
            JavaRDD<String> partitionPathRdds = jsc.parallelize(partitionPaths, partitionPaths.size());
            partitionSmallFilesMap =
                    partitionPathRdds
                            .mapToPair(
                                    (PairFunction<String, String, List<SmallFile>>)
                                            partitionPath -> new Tuple2<>(partitionPath, getSmallFiles(partitionPath)))
                            .collectAsMap();
        }
    ...
    public int getPartition(Object key) {
        ...
        if (keyLocation._2().isPresent()) {
            ...
            final long hashOfKey =
                    NumericUtils.getMessageDigestHash("MD5", keyLocation._1().getRecordKey());
            final double r = 1.0 * Math.floorMod(hashOfKey, totalInserts) / totalInserts;

            int index =
                    Collections.binarySearch(
                            targetBuckets, new InsertBucketCumulativeWeightPair(new InsertBucket(), r));
