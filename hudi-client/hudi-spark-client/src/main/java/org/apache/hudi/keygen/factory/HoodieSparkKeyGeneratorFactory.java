...
/**
  ...
  *
  * <p>This factory will try {@link HoodieWriteConfig#KEYGENERATOR_CLASS_NAME} firstly, this ensures
  * the class prop will not be overwritten by {@link KeyGeneratorType}
  */
public class HoodieSparkKeyGeneratorFactory {
    ...
    static {
        COMMON_TO_SPARK_KEYGENERATOR.put(
    ...
    public static KeyGenerator createKeyGenerator(String keyGeneratorClass, TypedProperties props) {
        boolean autoRecordKeyGen =
                KeyGenUtils.isAutoGeneratedRecordKeysEnabled(props)
                        // Need to prevent overwriting the keygen for spark sql merge into because
                        // we need to extract
                        // the recordkey from the meta cols if it exists. Sql keygen will use pkless
                        // keygen if needed.
    ...
    public static Option<BaseKeyGenerator> createBaseKeyGenerator(HoodieWriteConfig writeConfig) {
        if (!writeConfig.populateMetaFields()) {
            try {
                ...
                return Option.of(
                        (BaseKeyGenerator) HoodieSparkKeyGeneratorFactory.createKeyGenerator(typedProperties));
            } catch (ClassCastException cce) {
                throw new HoodieException(
                        "Only BaseKeyGenerators are supported when meta columns are disabled ", cce);
            }
        } else {
            return Option.empty();
        }
    ...
    public static Option<BuiltinKeyGenerator> getKeyGenerator(Properties properties) {
        ...
        if (Option.ofNullable(properties.get(HoodieWriteConfig.KEYGENERATOR_CLASS_NAME.key()))
                .map(v -> v.equals(NonpartitionedKeyGenerator.class.getName()))
                .orElse(false)) {
            ...
            try {
                ...
                throw new HoodieIOException(
                        "Only those key generators implementing BuiltInKeyGenerator interface is supported with virtual keys");
            }
        }
    }

    /**
      * Infers the key generator type based on the record key and partition fields. If neither of the
      * record key and partition fields are set, the default type is returned.
      *
    ...
    public static String getKeyGeneratorClassName(TypedProperties props) {
        ...
        if (StringUtils.isNullOrEmpty(keyGeneratorClass)) {
            ...
            if (StringUtils.isNullOrEmpty(keyGeneratorType)) {
                ...
                LOG.info(
                        "The value of {} is empty; inferred to be {}",
                        KEYGENERATOR_TYPE.key(),
                ...
                try {
                    ...
                    throw new HoodieKeyGeneratorException(
                            "Unsupported keyGenerator Type " + keyGeneratorType);
                }
            }
