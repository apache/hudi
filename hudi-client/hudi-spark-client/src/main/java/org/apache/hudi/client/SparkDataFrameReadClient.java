 ...
 */
public class SparkDataFrameReadClient<T> implements Serializable {
  ...
  private final transient StorageConfiguration hadoopConf;

  /**
   * @param basePath path to Hoodie table
   */
  public SparkDataFrameReadClient(HoodieSparkEngineContext context, String basePath) {
   ...
   * @param sqlContext
   */
  public SparkDataFrameReadClient(HoodieSparkEngineContext context, String basePath, SQLContext sqlContext) {
   ...
   */
  public SparkDataFrameReadClient(HoodieSparkEngineContext context, String basePath, SQLContext sqlContext, HoodieIndex.IndexType indexType) {
    this(context, HoodieWriteConfig.newBuilder().withPath(basePath)
   ...
   */
  public SparkDataFrameReadClient(HoodieSparkEngineContext context, HoodieWriteConfig clientConfig) {
    this.context = context;
    this.hadoopConf = getStorageConf();
  ...
  public Dataset<HoodieRecord<T>> tagLocation(Dataset<HoodieRecord<T>> hoodieRecords) throws HoodieIndexException {
    return HoodieJavaDataFrame.getDataFrame(
        index.tagLocation(HoodieJavaDataFrame.of(hoodieRecords), context, hoodieTable));
  }
