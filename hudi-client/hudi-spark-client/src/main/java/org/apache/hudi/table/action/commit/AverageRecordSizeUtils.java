...
public class AverageRecordSizeUtils {
    ...
    /**
      ...
      */
    static long averageBytesPerRecord(
                ...
                try {
                    ...
                    if (instant.getAction().equals(COMMIT_ACTION)
                        ...
                        commitMetadata.getWriteStats().stream()
                                .filter(
                                        hoodieWriteStat ->
                                                FSUtils.isBaseFile(new StoragePath(hoodieWriteStat.getPath())))
                                .forEach(
                                        hoodieWriteStat -> {
                        ...
                        if (totalBytesWritten.get() > fileSizeThreshold && totalRecordsWritten.get() > 0) {
                            avgSize =
                                    (long) Math.ceil((1.0 * totalBytesWritten.get()) / totalRecordsWritten.get());
                            break;
                        }
                    }
                } catch (Throwable t) {
