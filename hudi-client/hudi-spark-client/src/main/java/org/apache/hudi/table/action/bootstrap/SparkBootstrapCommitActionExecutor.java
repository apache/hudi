...
public class SparkBootstrapCommitActionExecutor<T>
        extends BaseCommitActionExecutor<
                ...
                HoodieBootstrapWriteMetadata<HoodieData<WriteStatus>>> {
    ...
    public SparkBootstrapCommitActionExecutor(
        ...
        bootstrapSourceFileSystem =
                HadoopFSUtils.getFs(config.getBootstrapSourceBasePath(), hadoopConf);
    }

    private void validate() {
        checkArgument(
                config.getBootstrapSourceBasePath() != null, "Ensure Bootstrap Source Path is set");
        checkArgument(
                config.getBootstrapModeSelectorClass() != null,
    ...
    protected Option<HoodieWriteMetadata<HoodieData<WriteStatus>>> metadataBootstrap(
        ...
        metaClient
                ...
                .createNewInstant(
                        new HoodieInstant(
                                State.REQUESTED, metaClient.getCommitActionType(), bootstrapInstantTime));
        ...
        HoodieData<BootstrapWriteStatus> bootstrapWriteStatuses =
    ...
    private void updateIndexAndCommitIfNeeded(
    ...
    protected void commit(HoodieWriteMetadata<HoodieData<WriteStatus>> result) {
        // Perform bootstrap index write and then commit. Make sure both record-key and
        // bootstrap-index
        ...
        try (BootstrapIndex.IndexWriter indexWriter =
            ...
            LOG.info(
                    "Starting to write bootstrap index for source "
                            + config.getBootstrapSourceBasePath()
                            + " in table "
                            + config.getBasePath());
            indexWriter.begin();
            bootstrapSourceAndStats.forEach(
                    (key, value) ->
                            indexWriter.appendNextPartition(
                                    key, value.stream().map(Pair::getKey).collect(Collectors.toList())));
            indexWriter.finish();
            LOG.info(
                    "Finished writing bootstrap index for source "
                            + config.getBootstrapSourceBasePath()
                            + " in table "
                            + config.getBasePath());
        }
        commit(
                ...
                bootstrapSourceAndStats.values().stream()
                        .flatMap(f -> f.stream().map(Pair::getValue))
    ...
    protected Option<HoodieWriteMetadata<HoodieData<WriteStatus>>> fullBootstrap(
        ...
        JavaRDD<HoodieRecord> inputRecordsRDD =
                (JavaRDD<HoodieRecord>)
                        inputProvider.generateInputRecords(
                                "bootstrap_source",
                                config.getBootstrapSourceBasePath(),
                                partitionFilesList,
                                config);
    ...
    protected BaseSparkCommitActionExecutor<T> getBulkInsertActionExecutor(
            HoodieData<HoodieRecord> inputRecordsRDD) {
    ...
    private Map<BootstrapMode, List<Pair<String, List<HoodieFileStatus>>>>
        ...
        HoodieBootstrapSchemaProvider sourceSchemaProvider =
                new HoodieSparkBootstrapSchemaProvider(config);
        ...
        Map<String, List<HoodieFileStatus>> partitionToFiles =
                folders.stream().collect(Collectors.toMap(Pair::getKey, Pair::getValue));
                ...
                .map(
                        e ->
                                Pair.of(
                                        e.getKey(),
                                        e.getValue().stream()
                                                .map(p -> Pair.of(p, partitionToFiles.get(p)))
                                                .collect(Collectors.toList())))
                .collect(Collectors.toMap(Pair::getKey, Pair::getValue));
    }

    private HoodieData<BootstrapWriteStatus> runMetadataBootstrap(
        ...
        List<Pair<String, Pair<String, HoodieFileStatus>>> bootstrapPaths =
                partitions.stream()
                        .flatMap(
                                p -> {
                                    String translatedPartitionPath =
                                            translator.getBootstrapTranslatedPath(p.getKey());
                                    return p.getValue().stream()
                                            .map(f -> Pair.of(p.getKey(), Pair.of(translatedPartitionPath, f)));
                                })
                        .collect(Collectors.toList());

        context.setJobStatus(
                this.getClass().getSimpleName(),
                "Run metadata-only bootstrap operation: " + config.getTableName());
        return context
                .parallelize(
                        bootstrapPaths, Math.min(bootstrapPaths.size(), config.getBootstrapParallelism()))
                .map(
                        partitionFsPair ->
                                getMetadataHandler(config, table, partitionFsPair.getRight().getRight())
                                        .runMetadataBootstrap(
                                                partitionFsPair.getLeft(),
                                                partitionFsPair.getRight().getLeft(),
                                                keyGenerator));
    }
