...
public class SparkHoodieHBaseIndex extends HoodieIndex<Object, Object> {
    ...
    /**
      * multiPutBatchSize will be computed and re-set in updateLocation if {@link
      * HoodieHBaseIndexConfig#PUT_BATCH_SIZE_AUTO_COMPUTE} is set to true.
      */
    ...
    public HBaseIndexQPSResourceAllocator createQPSResourceAllocator(HoodieWriteConfig config) {
        try {
            ...
            return (HBaseIndexQPSResourceAllocator)
                    ReflectionUtils.loadClass(config.getHBaseQPSResourceAllocatorClass(), config);
        } catch (Exception e) {
    ...
    /**
      * Since we are sharing the HBaseConnection across tasks in a JVM, make sure the HBaseConnection
      * is closed when JVM exits.
      */
    ...
    private Get generateStatement(String key) throws IOException {
        return new Get(getUTF8Bytes(getHBaseKey(key)))
                .readVersions(1)
    ...
    private <R>
            Function2<Integer, Iterator<HoodieRecord<R>>, Iterator<HoodieRecord<R>>> locationTagFunction(
                    HoodieTableMetaClient metaClient) {

        // `multiGetBatchSize` is intended to be a batch per 100ms. To create a rate limiter that
        // measures
            ...
            try (HTable hTable = (HTable) hbaseConnection.getTable(TableName.valueOf(tableName))) {
                ...
                while (hoodieRecordIterator.hasNext()) {
                    ...
                    for (Result result : results) {
                        ...
                        String commitTs =
                                fromUTF8Bytes(result.getValue(SYSTEM_COLUMN_FAMILY, COMMIT_TS_COLUMN));
                        String fileId = fromUTF8Bytes(result.getValue(SYSTEM_COLUMN_FAMILY, FILE_NAME_COLUMN));
                        String partitionPath =
                                fromUTF8Bytes(result.getValue(SYSTEM_COLUMN_FAMILY, PARTITION_PATH_COLUMN));
                        ...
                        if (updatePartitionPath && !partitionPath.equals(currentRecord.getPartitionPath())) {
                            ...
                            currentRecord =
                                    new HoodieAvroRecord(
                                            new HoodieKey(currentRecord.getRecordKey(), currentRecord.getPartitionPath()),
            ...
            try (BufferedMutator mutator =
                ...
                while (statusIterator.hasNext()) {
                    ...
                    try {
                        ...
                        // Create a rate limiter that allows `multiPutBatchSize` operations per
                        // second
                            ...
                            if (!writeStatus.isErrored(recordDelegate.getHoodieKey())) {
                                ...
                                if (loc.isPresent()) {
                                    ...
                                    put.addColumn(
                                            ...
                                            COMMIT_TS_COLUMN,
                                            getUTF8Bytes(loc.get().getInstantTime()));
                                    put.addColumn(
                                            SYSTEM_COLUMN_FAMILY, FILE_NAME_COLUMN, getUTF8Bytes(loc.get().getFileId()));
                                    put.addColumn(
                                            SYSTEM_COLUMN_FAMILY,
                                            PARTITION_PATH_COLUMN,
                                            getUTF8Bytes(recordDelegate.getPartitionPath()));
                                    mutations.add(put);
                                } else {
                                    // Delete existing index for a deleted record
                                    Delete delete =
                                            new Delete(getUTF8Bytes(getHBaseKey(recordDelegate.getRecordKey())));
    ...
    private void doMutations(BufferedMutator mutator, List<Mutation> mutations, RateLimiter limiter)
            throws IOException {
        if (mutations.isEmpty()) {
            return;
        }
        // report number of operations to account per second with rate limiter.
        // If #limiter.getRate() operations are acquired within 1 second, ratelimiter will limit the
        // rest of calls
    ...
    Map<String, Integer> mapFileWithInsertsToUniquePartition(JavaRDD<WriteStatus> writeStatusRDD) {
        ...
        final List<String> fileIds =
                writeStatusRDD
                        .filter(w -> w.getStat().getNumInserts() > 0)
    ...
    public HoodieData<WriteStatus> updateLocation(
            HoodieData<WriteStatus> writeStatus, HoodieEngineContext context, HoodieTable hoodieTable) {
        ...
        final Map<String, Integer> fileIdPartitionMap =
                mapFileWithInsertsToUniquePartition(writeStatusRDD);
        JavaRDD<WriteStatus> partitionedRDD =
                this.numWriteStatusWithInserts == 0
                        ? writeStatusRDD
                        : writeStatusRDD
                                .mapToPair(w -> new Tuple2<>(w.getFileId(), w))
                                .partitionBy(
                                        new WriteStatusPartitioner(fileIdPartitionMap, this.numWriteStatusWithInserts))
                                .map(Tuple2::_2);
        JavaSparkContext jsc = HoodieSparkEngineContext.getSparkContext(context);
        acquireQPSResourcesAndSetBatchSize(desiredQPSFraction, jsc);
        JavaRDD<WriteStatus> writeStatusJavaRDD =
                partitionedRDD.mapPartitionsWithIndex(updateLocationFunction(), true);
        // caching the index updated status RDD
        writeStatusJavaRDD =
                writeStatusJavaRDD.persist(SparkMemoryUtils.getWriteStatusStorageLevel(config.getProps()));
        ...
        if (config.getHbaseIndexPutBatchSizeAutoCompute()) {
            ...
            */
            ...
            final float desiredQPSFraction =
                    this.hBaseIndexQPSResourceAllocator.calculateQPSFractionForPutsTime(
    ...
    private void acquireQPSResourcesAndSetBatchSize(
            final Option<Float> desiredQPSFraction, final JavaSparkContext jsc) {
        if (config.getHbaseIndexPutBatchSizeAutoCompute()) {
            ...
            if (conf.getBoolean(DEFAULT_SPARK_DYNAMIC_ALLOCATION_ENABLED_CONFIG_NAME, false)) {
                maxExecutors =
                        Math.max(
                                maxExecutors,
                                conf.getInt(DEFAULT_SPARK_DYNAMIC_ALLOCATION_MAX_EXECUTORS_CONFIG_NAME, 1));
            }
            final float availableQpsFraction =
                    this.hBaseIndexQPSResourceAllocator.acquireQPSResources(
                            desiredQPSFraction.get(), this.totalNumInserts);
            LOG.info("Allocated QPS Fraction :" + availableQpsFraction);
            multiPutBatchSize =
                    putBatchSizeCalculator.getBatchSize(
    ...
    Tuple2<Long, Integer> getHBasePutAccessParallelism(final JavaRDD<WriteStatus> writeStatusRDD) {
        ...
        return insertOnlyWriteStatusRDD.fold(
                new Tuple2<>(0L, 0), (w, c) -> new Tuple2<>(w._1 + c._1, w._2 + c._2));
    }
    ...
    public static class HBasePutBatchSizeCalculator implements Serializable {
        ...
        /**
          * Calculate putBatch size so that sum of requests across multiple jobs in a second does not
          * exceed maxQpsPerRegionServer for each Region Server. Multiplying qpsFraction to reduce the
          * aggregate load on common RS across topics. Assumption here is that all tables have regions
          * across all RS, which is not necessarily true for smaller tables. So, they end up getting a
          * smaller share of QPS than they deserve, but it might be ok.
          *
          * <p>Example: int putBatchSize = batchSizeCalculator.getBatchSize(10, 16667, 1200, 200, 100,
          * 0.1f)
          *
          * <p>Expected batchSize is 8 because in that case, total request sent to a Region Server in one
          * second is:
          *
          * <p>8 (batchSize) * 200 (parallelism) * 10 (maxReqsInOneSecond) * 10 (numRegionServers) * 0.1
          * (qpsFraction)) => 16000. We assume requests get distributed to Region Servers uniformly, so
          * each RS gets 1600 requests which happens to be 10% of 16667 (maxQPSPerRegionServer), as
          * expected.
          *
          * <p>Assumptions made here
          * <li>In a batch, writes get evenly distributed to each RS for that table. Since we do writes
          *     only in the case of inserts and not updates, for this assumption to fail, inserts would
          *     have to be skewed towards few RS, likelihood of which is less if Hbase table is pre-split
          *     and rowKeys are UUIDs (random strings). If this assumption fails, then it is possible for
          *     some RS to receive more than maxQpsPerRegionServer QPS, but for simplicity, we are going
          *     ahead with this model, since this is meant to be a lightweight distributed throttling
          *     mechanism without maintaining a global context. So if this assumption breaks, we are
          *     hoping the HBase Master relocates hot-spot regions to new Region Servers.
          * <li>For Region Server stability, throttling at a second level granularity is fine. Although,
          *     within a second, the sum of queries might be within maxQpsPerRegionServer, there could be
          *     peaks at some sub second intervals. So, the assumption is that these peaks are tolerated
          *     by the Region Server (which at max can be maxQpsPerRegionServer).
          */
        public int getBatchSize(
                int numRegionServersForTable,
                int maxQpsPerRegionServer,
                int numTasksDuringPut,
                int maxExecutors,
                float qpsFraction) {
            ...
            int multiPutBatchSizePerSecPerTask =
                    Math.max(1, (int) Math.ceil((double) maxReqPerSec / maxParallelPutsTask));
            ...
            LOG.info(
                    "HbaseIndexThrottling: multiPutBatchSizePerSecPerTask :"
                            + multiPutBatchSizePerSecPerTask);
            return multiPutBatchSizePerSecPerTask;
        }

        public int getMaxReqPerSec(
                int numRegionServersForTable, int maxQpsPerRegionServer, float qpsFraction) {
    ...
    private Integer getNumRegionServersAliveForTable() {
        ...
        if (numRegionServersForTable == null) {
            try (Connection conn = getHBaseConnection()) {
                ...
                numRegionServersForTable =
                        Math.toIntExact(
                                regionLocator.getAllRegionLocations().stream()
                                        .map(HRegionLocation::getServerName)
                                        .distinct()
                                        .count());
            ...
            while (scannerIterator.hasNext()) {
                ...
                for (int i = 0; i < lastVersionResults.length; i++) {
                    ...
                    if (null != lastVersionResult.getRow()) {
                        String oldPath =
                                new String(lastVersionResult.getValue(SYSTEM_COLUMN_FAMILY, PARTITION_PATH_COLUMN));
                        String nowPath =
                                new String(
                        ...
                        if (!oldPath.equals(nowPath) || rollbackSync) {
                            ...
                            put.addColumn(
                                    SYSTEM_COLUMN_FAMILY,
                                    COMMIT_TS_COLUMN,
                                    lastVersionResult.getValue(SYSTEM_COLUMN_FAMILY, COMMIT_TS_COLUMN));
                            put.addColumn(
                                    SYSTEM_COLUMN_FAMILY,
                                    FILE_NAME_COLUMN,
                                    lastVersionResult.getValue(SYSTEM_COLUMN_FAMILY, FILE_NAME_COLUMN));
                            put.addColumn(
                                    SYSTEM_COLUMN_FAMILY,
                                    PARTITION_PATH_COLUMN,
    ...
    /**
      * Partitions each WriteStatus with inserts into a unique single partition. WriteStatus without
      * inserts will be assigned to random partitions. This partitioner will be useful to utilize max
      * parallelism with spark operations that are based on inserts in each WriteStatus.
      */
    public static class WriteStatusPartitioner extends Partitioner {
        private int totalPartitions;
        final Map<String, Integer> fileIdPartitionMap;

        public WriteStatusPartitioner(
                final Map<String, Integer> fileIdPartitionMap, final int totalPartitions) {
        ...
        public int getPartition(Object key) {
            ...
            if (!fileIdPartitionMap.containsKey(fileId)) {
                LOG.info(
                        "This writestatus(fileId: "
                                + fileId
                                + ") is not mapped because it doesn't have any inserts. "
                                + "In this case, we can assign a random partition to this WriteStatus.");
                // Assign random spark partition for the `WriteStatus` that has no inserts. For a
                // spark operation that depends
                // on number of inserts, there won't be any performance penalty in packing these
                // WriteStatus'es together.
