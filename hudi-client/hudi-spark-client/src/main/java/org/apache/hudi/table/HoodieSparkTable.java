...
public abstract class HoodieSparkTable<T>
    ...
    public static <T> HoodieSparkTable<T> create(
    ...
    protected Option<HoodieTableMetadataWriter> getMetadataWriter(
        ...
        if (config.isMetadataTableEnabled()) {
            // if any partition is deleted, we need to reload the metadata table writer so that new
            // table configs are picked up
            // to reflect the delete mdt partitions.
            deleteMetadataIndexIfNecessary();

            // Create the metadata table writer. First time after the upgrade this creation might
            // trigger
            ...
            try {
                if (isMetadataTableExists
                        || metaClient
                                .getStorage()
                                .exists(HoodieTableMetadata.getMetadataTableBasePath(metaClient.getBasePath()))) {
            ...
            // if metadata is not enabled in the write config, we should try and delete it (if
            // present)
        ...
        if (upsertHandle.getOldFilePath() == null) {
            ...
            if (upsertHandle.baseFileForMerge().getBootstrapBaseFile().isPresent()) {
                ...
                Object[] partitionValues =
                        SparkPartitionUtils.getPartitionFieldVals(
                                partitionFields,
