...
public abstract class SqlQueryPreCommitValidator<T, I, K, O extends HoodieData<WriteStatus>>
    ...
    public SqlQueryPreCommitValidator(
            HoodieSparkTable<T> table, HoodieEngineContext engineContext, HoodieWriteConfig config) {
        super(table, engineContext, config);
    }

    /**
      * Takes input datasets 1) before commit started and 2) with inflight commit. Perform required
      * validation and throw error if validation fails
      */
    @Override
    public void validateRecordsBeforeAndAfter(
            Dataset<Row> before, Dataset<Row> after, final Set<String> partitionsAffected) {
        ...
        Arrays.asList(queries)
                ...
                .forEach(
                        query ->
                                validateUsingQuery(
                                        query,
                                        hoodieTableBeforeCurrentCommit,
                                        hoodieTableWithInflightCommit,
                                        sqlContext));
    }
    ...
    protected String[] getQueriesToRun() {
        ...
        if (StringUtils.isNullOrEmpty(sqlQueriesConfigured)) {
            throw new HoodieValidationException(
                    "Sql validator configured incorrectly. expecting at least one query. Found 0 queries in "
    ...
    protected abstract void validateUsingQuery(
