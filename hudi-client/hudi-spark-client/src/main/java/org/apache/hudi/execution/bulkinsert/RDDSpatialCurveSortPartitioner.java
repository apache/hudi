...
/**
  * A partitioner that does spatial curve optimization sorting based on specified column values for
  * each RDD partition. support z-curve optimization, hilbert will come soon.
  *
        ...
        if (recordType == HoodieRecordType.AVRO) {
            JavaRDD<GenericRecord> genericRecordsRDD =
                    records.map(
                            f ->
                                    (GenericRecord)
                                            f.toIndexedRecord(schema.get(), new Properties()).get().getData());
                    ...
                    .map(
                            record -> {
                                ...
                                HoodieRecord hoodieRecord =
                                        new HoodieAvroRecord(hoodieKey, new RewriteAvroPayload(record));
                                return hoodieRecord;
                            });
        } else if (recordType == HoodieRecordType.SPARK) {
            StructType structType = HoodieInternalRowUtils.getCachedSchema(schema.get());
            Dataset<Row> sourceDataset =
                    SparkConversionUtils.createDataFrame(
                            records.rdd(), sparkEngineContext.getSqlContext().sparkSession(), structType);
            Dataset<Row> sortedDataset = reorder(sourceDataset, outputSparkPartitions);
            ...
            return sortedDataset
                    ...
                    .map(
                            internalRow -> {
                                ...
                                String partition =
                                        internalRow.getString(
                                                HoodieMetadataField.PARTITION_PATH_METADATA_FIELD.ordinal());
                                HoodieKey hoodieKey = new HoodieKey(key, partition);
                                return (HoodieRecord)
                                        new HoodieSparkRecord(hoodieKey, internalRow, structType, false);
                            });
        } else {
