...
/**
  * Implementation of the function that probing Bloom Filters of individual files verifying whether
  * particular record key could be stored in the latest file-slice of the file-group identified by
  * the {@link HoodieFileGroupId}
  */
public class HoodieMetadataBloomFilterProbingFunction
        implements PairFlatMapFunction<
                Iterator<Tuple2<HoodieFileGroupId, String>>,
                HoodieFileGroupId,
                HoodieBloomFilterProbingResult> {

    private static final Logger LOG =
            LoggerFactory.getLogger(HoodieMetadataBloomFilterProbingFunction.class);
    ...
    /**
      ...
      * broadcast-ed instance of {@link HoodieBackedTableMetadata} internally, instead of one being
      * serialized and deserialized for _every_ task individually
      *
      * <p>NOTE: We pass in broadcasted {@link HoodieTableFileSystemView} to make sure it's
      * materialized on executor once
      */
    public HoodieMetadataBloomFilterProbingFunction(
            Broadcast<HoodieTableFileSystemView> baseFileOnlyViewBroadcast, HoodieTable hoodieTable) {
        this.baseFileOnlyViewBroadcast = baseFileOnlyViewBroadcast;
        this.hoodieTable = hoodieTable;
    }

    @Override
    public Iterator<Tuple2<HoodieFileGroupId, HoodieBloomFilterProbingResult>> call(
            Iterator<Tuple2<HoodieFileGroupId, String>> tuple2Iterator) throws Exception {
    ...
    private class BloomIndexLazyKeyCheckIterator
            extends LazyIterableIterator<
                    Tuple2<HoodieFileGroupId, String>,
                    Iterator<Tuple2<HoodieFileGroupId, HoodieBloomFilterProbingResult>>> {

        public BloomIndexLazyKeyCheckIterator(
                Iterator<Tuple2<HoodieFileGroupId, String>> tuple2Iterator) {
        ...
        protected Iterator<Tuple2<HoodieFileGroupId, HoodieBloomFilterProbingResult>> computeNext() {
            ...
            while (inputItr.hasNext()) {
                ...
                if (!fileIDBaseFileMap.containsKey(fileId)) {
                    ...
                    if (!baseFile.isPresent()) {
                        throw new HoodieIndexException(
                                "Failed to find the base file for partition: "
                                        + partitionPath
                                        + ", fileId: "
                                        + fileId);
                    }
                ...
                fileToKeysMap
                        .computeIfAbsent(
                                Pair.of(partitionPath, fileIDBaseFileMap.get(fileId)), k -> new ArrayList<>())
                        .add(new HoodieKey(entry._2, partitionPath));
            ...
            List<Pair<String, String>> partitionNameFileNameList =
                    fileToKeysMap.keySet().stream()
                            .map(pair -> Pair.of(pair.getLeft(), pair.getRight().getFileName()))
                    ...
                    .map(
                            entry -> {
                                ...
                                if (!fileToBloomFilterMap.containsKey(partitionPathFileNamePair)) {
                                    throw new HoodieIndexException(
                                            "Failed to get the bloom filter for " + partitionPathFileNamePair);
                                }
                                ...
                                LOG.debug(
                                        String.format(
                                                "Total records (%d), bloom filter candidates (%d)",
