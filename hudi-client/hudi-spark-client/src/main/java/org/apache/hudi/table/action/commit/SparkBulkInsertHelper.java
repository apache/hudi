...
public class SparkBulkInsertHelper<T, R> extends BaseBulkInsertHelper<T, JavaRDD<HoodieRecord<T>>,
  ...
  public static <T, R> SparkBulkInsertHelper<T, R> newInstance() {
  ...
  public HoodieWriteMetadata<JavaRDD<WriteStatus>> bulkInsert(final JavaRDD<HoodieRecord<T>> inputRecords,
    ...
    //update index
    ((BaseSparkCommitActionExecutor<T>) executor).updateIndexAndCommitIfNeeded(writeStatuses, result);
    return result;
  }
  ...
  public JavaRDD<WriteStatus> bulkInsert(JavaRDD<HoodieRecord<T>> inputRecords,
                                         ...
                                         int parallelism,
                                         WriteHandleFactory<T, JavaRDD<HoodieRecord<T>>, JavaRDD<HoodieKey>, JavaRDD<WriteStatus>> writeHandleFactory) {

    // De-dupe/merge if needed
    JavaRDD<HoodieRecord<T>> dedupedRecords = inputRecords;

    if (performDedupe) {
      dedupedRecords = SparkWriteHelper.<T, R>newInstance().combineOnCondition(config.shouldCombineBeforeInsert(), inputRecords,
          parallelism, table);
    }

    final JavaRDD<HoodieRecord<T>> repartitionedRecords;
    BulkInsertPartitioner<JavaRDD<HoodieRecord<T>>> partitioner = userDefinedBulkInsertPartitioner.isPresent()
