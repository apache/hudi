...
public class SparkBulkInsertHelper<T, R>
    ...
    private static class BulkInsertHelperHolder {
        private static final SparkBulkInsertHelper HOODIE_BULK_INSERT_HELPER =
                new SparkBulkInsertHelper<>();
    }
    ...
    public HoodieWriteMetadata<HoodieData<WriteStatus>> bulkInsert(
        ...
        table
                ...
                .transitionRequestedToInflight(
                        new HoodieInstant(
                                HoodieInstant.State.REQUESTED, executor.getCommitActionType(), instantTime),
                        Option.empty(),
    ...
    /**
      * Do bulk insert using WriteHandleFactory from the partitioner (i.e.,
    ...
    public HoodieData<WriteStatus> bulkInsert(
        ...
        // only JavaRDD is supported for Spark partitioner, but it is not enforced by
                ...
                HoodieJavaRDD.of(
                        (JavaRDD<HoodieRecord<T>>)
                                partitioner.repartitionRecords(
                                        HoodieJavaRDD.getJavaRDD(dedupedRecords), targetParallelism));

        JavaRDD<WriteStatus> writeStatusRDD =
                HoodieJavaRDD.getJavaRDD(repartitionedRecords)
                        .mapPartitionsWithIndex(
                                new BulkInsertMapFunction<>(
                                        instantTime,
