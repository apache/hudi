...
import org.apache.hudi.common.model.{HoodieAvroIndexedRecord, HoodieRecord, HoodieRecordMerger}
import org.apache.hudi.common.util.{BinaryUtil, ConfigUtils, StringUtils, ValidationUtils, collection, Option => HOption}
...
import org.apache.hudi.SparkExpressionMerger.{PAYLOAD_DELETE_CONDITION, PAYLOAD_INSERT_CONDITION_AND_ASSIGNMENTS, PAYLOAD_UPDATE_CONDITION_AND_ASSIGNMENTS, getAvroDeserializerFor, getAvroSerializerFor, getEvaluator, getMergedSchema, getWriterSchema, needUpdatingPersistedRecord}
...
/**
 * Description from original payload class: ExpressionPayload.
 *
 ...
 */
class SparkExpressionMerger extends HoodieRecordMerger with Logging {
  override def merge(older: HoodieRecord[_],
                     oldSchema: Schema,
                     newer: HoodieRecord[_],
                     newSchema: Schema,
                     props: TypedProperties): HOption[collection.Pair[HoodieRecord[_], Schema]] = {
    val oldRecord: HoodieAvroIndexedRecord = older.toIndexedRecord(oldSchema, props).get()
    val newRecord: HoodieAvroIndexedRecord = newer.toIndexedRecord(newSchema, props).get()
    val oldIndexedRecord: IndexedRecord = oldRecord.getData
    val newIndexedRecord: IndexedRecord = newRecord.getData
    val mergedRecord = joinRecord(oldIndexedRecord, newIndexedRecord, props)

    val processedMergedRecord = processMatchedRecord(
      ConvertibleRecord(mergedRecord), Some(newIndexedRecord), props)
    if (processedMergedRecord.isEmpty) {
      HOption.empty()
    } else {
      HOption.of(collection.Pair.of(
        new HoodieAvroIndexedRecord(processedMergedRecord.get()),
        mergedRecord.getSchema))
    }
  }
   ...
   * delete-condition, if matched then return a delete record.
   *
  ...
  private def processMatchedRecord(inputRecord: ConvertibleRecord,
    ...
    val updateConditionAndAssignmentsText =
      properties.get(PAYLOAD_UPDATE_CONDITION_AND_ASSIGNMENTS)

    checkState(updateConditionAndAssignmentsText != null,
      s"${PAYLOAD_UPDATE_CONDITION_AND_ASSIGNMENTS} have to be set")
    ...
    if (resultRecordOpt == null) {
      // Process delete
      val deleteConditionText = properties.get(PAYLOAD_DELETE_CONDITION)
  ...
  private def processNotMatchedRecord(inputRecord: ConvertibleRecord, properties: Properties): HOption[IndexedRecord] = {
    val insertConditionAndAssignmentsText: String =
      properties.get(PAYLOAD_INSERT_CONDITION_AND_ASSIGNMENTS).toString
    // Get the evaluator for each condition and insert assignment.
    val insertConditionAndAssignments = getEvaluator(insertConditionAndAssignmentsText, inputRecord.asAvro.getSchema)
    var resultRecordOpt: HOption[IndexedRecord] = null
   ...
   * SPARK, AVRO, FLINK
   */
  override def getRecordType: HoodieRecord.HoodieRecordType = {
...
object SparkExpressionMerger {
   ...
   */
  private val PAYLOAD_SHOULD_VALIDATE_COMBINED_SCHEMA = "hoodie.payload.combined.schema.validate"
  ...
  /**
   * Determines if the persisted record should be updated based on ordering field comparison.
   * - Returns true if the ordering field is not configured.
   * - Returns true if the persisted record's ordering value is null.
   * - Returns true if the incoming record is newer (ordering value is greater or equal).
   * - Returns false if the incoming record is older.
   *
   * NOTE: Deletes sent via EmptyHoodieRecordPayload and/or Delete operation type do not hit this code path
   * and need to be dealt with separately.
   */
  protected def needUpdatingPersistedRecord(currentValue: IndexedRecord,
                                            incomingRecord: IndexedRecord,
                                            properties: Properties): Boolean = {
    val orderField = ConfigUtils.getOrderingField(properties)
    if (orderField == null) return true
    ...
    val persistedOrderingVal = Option(HoodieAvroUtils.getNestedFieldVal(
      currentValue.asInstanceOf[GenericRecord], orderField, true, consistentLogicalTimestampEnabled)
    ).map(_.asInstanceOf[Comparable[Any]])

    val incomingOrderingVal = Option(HoodieAvroUtils.getNestedFieldVal(
      incomingRecord.asInstanceOf[GenericRecord], orderField, true, consistentLogicalTimestampEnabled)
    ).map(_.asInstanceOf[Comparable[Any]])

    (persistedOrderingVal, incomingOrderingVal) match {
      case (None, _) => true // No persisted value to compare
      case (_, None) => false // Incoming value has no valid ordering
      case (Some(p), Some(i)) => p.compareTo(i) <= 0
    }
  }
}
