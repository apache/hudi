...
public class CustomKeyGenerator extends BuiltinKeyGenerator {
    ...
    public CustomKeyGenerator(TypedProperties props) {
        // NOTE: We have to strip partition-path configuration, since it could only be interpreted
        // by
        //       this key-gen
        ...
        this.partitionPathFields =
                partitionPathFields == null
                        ? Collections.emptyList()
                        : Arrays.stream(partitionPathFields.split(","))
                                .map(String::trim)
                                .collect(Collectors.toList());
    ...
    private static List<BuiltinKeyGenerator> getPartitionKeyGenerators(
                                ...
                                switch (keyType) {
                                    ...
                                    case TIMESTAMP:
                                        try {
                                            ...
                                            throw new HoodieKeyGeneratorException(
                                                    "Unable to initialise TimestampBasedKeyGenerator class", ioe);
                                        }
                                    default:
                                        throw new HoodieKeyGeneratorException(
    ...
    public UTF8String getPartitionPath(InternalRow row, StructType schema) {
        return UTF8String.fromString(
                getPartitionPath(Option.empty(), Option.empty(), Option.of(Pair.of(row, schema))));
    }

    private String getPartitionPath(
            Option<GenericRecord> record,
            Option<Row> row,
            Option<Pair<InternalRow, StructType>> internalRowStructTypePair) {
        ...
        for (int i = 0; i < partitionKeyGenerators.size(); i++) {
            ...
            if (record.isPresent()) {
                ...
                partitionPath.append(
                        keyGenerator.getPartitionPath(
                                internalRowStructTypePair.get().getKey(),
