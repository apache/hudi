...
/**
  * Update strategy for (consistent hashing) bucket index If updates to file groups that are under
  * clustering are identified, then generate two same records for each update, routing to both old
  * and new file groups
  */
public class SparkConsistentBucketDuplicateUpdateStrategy<T extends HoodieRecordPayload<T>>
    ...
    public SparkConsistentBucketDuplicateUpdateStrategy(
    ...
    @Override
    public Pair<HoodieData<HoodieRecord<T>>, Set<HoodieFileGroupId>> handleUpdate(
            HoodieData<HoodieRecord<T>> taggedRecordsRDD) {
        if (fileGroupsInPendingClustering.isEmpty()) {
            return Pair.of(taggedRecordsRDD, Collections.emptySet());
        }

        HoodieData<HoodieRecord<T>> filteredRecordsRDD =
                taggedRecordsRDD.filter(
                        r -> {
                            checkState(r.getCurrentLocation() != null);
                            return fileGroupsInPendingClustering.contains(
                                    new HoodieFileGroupId(r.getPartitionPath(), r.getCurrentLocation().getFileId()));
                        });
        ...
        final Set<String> partitions =
                new HashSet<>(
                        filteredRecordsRDD.map(HoodieRecord::getPartitionPath).distinct().collectAsList());
        ...
        HoodieData<HoodieRecord<T>> redirectedRecordsRDD =
                filteredRecordsRDD.map(
                        r -> {
                            ...
                            return tagAsNewRecordIfNeeded(
                                    new HoodieAvroRecord(r.getKey(), r.getData(), r.getOperation()),
                                    Option.ofNullable(
                                            new HoodieRecordLocation(
                                                    identifierPair.getKey(),
