...
/**
  ...
  *
  * <p>- For physically partitioned table, repartition the input records based on the partition path,
  * and sort records within Spark partitions, limiting the shuffle parallelism to specified
  * `outputSparkPartitions`
  *
  * <p>- For physically non-partitioned table, simply does coalesce for the input records with
  * `outputSparkPartitions`
  *
  * <p>Corresponding to the {@code BulkInsertSortMode.PARTITION_PATH_REPARTITION_AND_SORT} mode.
  *
...
public class PartitionPathRepartitionAndSortPartitioner<T extends HoodieRecordPayload>
    ...
    public PartitionPathRepartitionAndSortPartitioner(
            boolean isTablePartitioned, HoodieWriteConfig config) {
    ...
    public JavaRDD<HoodieRecord<T>> repartitionRecords(
        ...
        if (isTablePartitioned) {
            PartitionPathRDDPartitioner partitioner =
                    new PartitionPathRDDPartitioner(
