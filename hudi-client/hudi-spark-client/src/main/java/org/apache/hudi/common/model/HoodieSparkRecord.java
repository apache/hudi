...
/**
  ...
  *
  * <p>NOTE: [[HoodieSparkRecord]] is expected to hold either [[UnsafeRow]] or [[HoodieInternalRow]]:
  *
  * <ul>
  *   <li>[[UnsafeRow]] is held to make sure a) we don't deserialize raw bytes payload into JVM types
  *       unnecessarily, b) we don't incur penalty of ser/de during shuffling, c) we don't add strain
  *       on GC
  *   <li>[[HoodieInternalRow]] is held in cases when underlying [[UnsafeRow]]'s metadata fields need
  *       to be updated (ie serving as an overlay layer on top of [[UnsafeRow]])
  * </ul>
  */
public class HoodieSparkRecord extends HoodieRecord<InternalRow> {
    ...
    /**
      * NOTE: {@code HoodieSparkRecord} is holding the schema only in cases when it would have to
      * execute {@link UnsafeProjection} so that the {@link InternalRow} it's holding to could be
      * projected into {@link UnsafeRow} and be efficiently serialized subsequently (by Kryo)
      */
    ...
    private HoodieSparkRecord(
            HoodieKey key, InternalRow data, StructType schema, HoodieOperation operation, boolean copy) {
    ...
    public String getRecordKey(Schema recordSchema, Option<BaseKeyGenerator> keyGeneratorOpt) {
        ...
        return keyGeneratorOpt.isPresent()
                ? ((SparkKeyGeneratorInterface) keyGeneratorOpt.get())
                        .getRecordKey(data, structType)
                        .toString()
    ...
    public Object[] getColumnValues(
            Schema recordSchema, String[] columns, boolean consistentLogicalTimestampEnabled) {
        StructType structType = HoodieInternalRowUtils.getCachedSchema(recordSchema);
    ...
    public HoodieRecord joinWith(HoodieRecord other, Schema targetSchema) {
        ...
        UnsafeProjection projection = getCachedUnsafeProjection(targetStructType, targetStructType);
        return new HoodieSparkRecord(
                getKey(),
                projection.apply(mergeRow),
                targetStructType,
                getOperation(),
    ...
    @Override
    public HoodieRecord prependMetaFields(
            Schema recordSchema, Schema targetSchema, MetadataValues metadataValues, Properties props) {
        StructType structType = HoodieInternalRowUtils.getCachedSchema(recordSchema);
        StructType targetStructType = HoodieInternalRowUtils.getCachedSchema(targetSchema);
    ...
    @Override
    public HoodieRecord rewriteRecordWithNewSchema(
            Schema recordSchema, Properties props, Schema newSchema, Map<String, String> renameCols) {
        StructType structType = HoodieInternalRowUtils.getCachedSchema(recordSchema);
    ...
    @Override
    public HoodieRecord truncateRecordKey(
            Schema recordSchema, Properties props, String keyFieldName) {
        StructType structType = HoodieInternalRowUtils.getCachedSchema(recordSchema);
        int pos = structType.fieldIndex(keyFieldName);
    ...
    public boolean isDelete(Schema recordSchema, Properties props) throws IOException {
        ...
        Object deleteMarker =
                data.get(recordSchema.getField(HoodieRecord.HOODIE_IS_DELETED_FIELD).pos(), BooleanType);
    ...
    public HoodieRecord wrapIntoHoodieRecordPayloadWithParams(
        ...
        if (populateMetaFields) {
            ...
            return convertToHoodieSparkRecord(
                    structType,
                    this,
            ...
            return convertToHoodieSparkRecord(
                    structType, this, withOperation, partitionNameOp, structTypeWithoutMetaFields);
        }
    }

    @Override
    public HoodieRecord wrapIntoHoodieRecordPayloadWithKeyGen(
            Schema recordSchema, Properties props, Option<BaseKeyGenerator> keyGen) {
        StructType structType = HoodieInternalRowUtils.getCachedSchema(recordSchema);
        String key;
        String partition;
        boolean populateMetaFields =
                Boolean.parseBoolean(
                        props
                                .getOrDefault(
                                        POPULATE_META_FIELDS.key(), POPULATE_META_FIELDS.defaultValue().toString())
                                .toString());
        if (!populateMetaFields && keyGen.isPresent()) {
            ...
            partition =
                    data.get(HoodieMetadataField.PARTITION_PATH_METADATA_FIELD.ordinal(), StringType)
                            .toString();
        }
        HoodieKey hoodieKey = new HoodieKey(key, partition);
        return new HoodieSparkRecord(
                hoodieKey, data, structType, getOperation(), this.currentLocation, this.newLocation, copy);
    }
    ...
    /**
      * NOTE: This method is declared final to make sure there's no polymorphism and therefore JIT
      * compiler could perform more aggressive optimizations
      */
    @Override
    ...
    private static HoodieInternalRow wrapIntoUpdatableOverlay(
            InternalRow data, StructType structType) {
    ...
    private static void updateMetadataValuesInternal(
            HoodieInternalRow updatableRow, MetadataValues metadataValues) {
    ...
    private static HoodieRecord<InternalRow> convertToHoodieSparkRecord(
            StructType structType, HoodieSparkRecord record, boolean withOperationField) {
        return convertToHoodieSparkRecord(
                structType,
                record,
    ...
    private static HoodieRecord<InternalRow> convertToHoodieSparkRecord(
            ...
            Option<StructType> structTypeWithoutMetaFields) {
        final String recKey =
                getValue(structType, recordKeyPartitionPathFieldPair.getKey(), record.data).toString();
        final String partitionPath =
                (partitionName.isPresent()
                        ? partitionName.get()
                        : getValue(structType, recordKeyPartitionPathFieldPair.getRight(), record.data)
                                .toString());

        HoodieOperation operation =
                withOperationField
                        ? HoodieOperation.fromName(
                                record.data.getString(structType.fieldIndex(HoodieRecord.OPERATION_METADATA_FIELD)))
                        : null;

        if (structTypeWithoutMetaFields.isPresent()) {
            StructType structTypeNoMetaFields = structTypeWithoutMetaFields.get();
            UnsafeRow rowNoMetaFields =
                    getCachedUnsafeProjection(structType, structTypeNoMetaFields).apply(record.data);
            return new HoodieSparkRecord(
                    new HoodieKey(recKey, partitionPath),
                    rowNoMetaFields,
                    structTypeNoMetaFields,
                    operation,
                    record.copy);
        }

        return new HoodieSparkRecord(
                new HoodieKey(recKey, partitionPath), record.data, structType, operation, record.copy);
    }
    ...
    private static Object getValue(StructType structType, String fieldName, InternalRow row) {
        ...
        if (cachedNestedFieldPath.isDefined()) {
            ...
            throw new HoodieException(
                    String.format("Field at %s is not present in %s", fieldName, structType));
        }
    }
}
