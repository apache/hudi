...
public abstract class SpatialCurveSortPartitionerBase<T> implements BulkInsertPartitioner<T> {
    ...
    public SpatialCurveSortPartitionerBase(
        ...
        if (orderByColumns != null) {
            this.orderByColumns =
                    Arrays.stream(orderByColumns.split(",")).map(String::trim).toArray(String[]::new);
        } else {
    ...
    protected Dataset<Row> reorder(Dataset<Row> dataset, int numOutputGroups) {
        ...
        switch (curveCompositionStrategyType) {
            case DIRECT:
                return SpaceCurveSortingHelper.orderDataFrameByMappingValues(
                        dataset, layoutOptStrategy, orderedCols, numOutputGroups);
            case SAMPLE:
                return SpaceCurveSortingHelper.orderDataFrameBySamplingValues(
                        dataset, layoutOptStrategy, orderedCols, numOutputGroups);
            default:
                throw new UnsupportedOperationException(
                        String.format(
                                "Unsupported space-curve curve building strategy (%s)",
                                curveCompositionStrategyType));
        }
    }
