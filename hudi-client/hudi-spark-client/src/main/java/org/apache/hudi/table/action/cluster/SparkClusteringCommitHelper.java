...
public class SparkClusteringCommitHelper<T extends HoodieRecordPayload<T>> extends SparkCommitHelper<T> {

  private final HoodieClusteringPlan clusteringPlan;

  public SparkClusteringCommitHelper(
      HoodieSparkEngineContext context, HoodieWriteConfig config, HoodieTable table,
      String instantTime) {
    ...
    this.clusteringPlan = ClusteringUtils.getClusteringPlan(
        ...
        .map(Pair::getRight).orElseThrow(
            () -> new HoodieClusteringException("Unable to read clustering plan for instant: " + instantTime));
  }
