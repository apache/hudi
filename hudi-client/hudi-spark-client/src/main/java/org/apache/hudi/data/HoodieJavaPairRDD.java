...
public class HoodieJavaPairRDD<K, V> implements HoodiePairData<K, V> {
    ...
    /**
      * @param hoodiePairData {@link HoodieJavaPairRDD <K, V>} instance containing the {@link
      *     JavaPairRDD} of pairs.
      * @param <K> type of key.
      * @param <V> type of value.
    ...
    @Override
    public HoodiePairData<K, V> reduceByKey(
            SerializableBiFunction<V, V, V> combiner, int parallelism) {
    ...
    public <O> HoodieData<O> map(SerializableFunction<Pair<K, V>, O> func) {
        return HoodieJavaRDD.of(
                pairRDDData.map(tuple -> func.apply(new ImmutablePair<>(tuple._1, tuple._2))));
    }
    ...
    public <W> HoodiePairData<K, Pair<V, Option<W>>> leftOuterJoin(HoodiePairData<K, W> other) {
        return HoodieJavaPairRDD.of(
                JavaPairRDD.fromJavaRDD(
                        pairRDDData
                                .leftOuterJoin(HoodieJavaPairRDD.getJavaPairRDD(other))
                                .map(
                                        tuple ->
                                                new Tuple2<>(
                                                        tuple._1,
                                                        new ImmutablePair<>(
                                                                tuple._2._1, Option.ofNullable(tuple._2._2.orElse(null)))))));
    }
