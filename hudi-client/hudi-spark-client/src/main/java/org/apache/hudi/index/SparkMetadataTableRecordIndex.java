...
public class SparkMetadataTableRecordIndex extends HoodieIndex<Object, Object> {
    ...
    // This should be a global index like record index so that the behavior of tagging across
    // partitions is not changed.
    ...
    public <R> HoodieData<HoodieRecord<R>> tagLocation(
            HoodieData<HoodieRecord<R>> records, HoodieEngineContext context, HoodieTable hoodieTable)
        ...
        try {
            ...
            fileGroupSize =
                    hoodieTable
                            .getMetadataTable()
                            .getNumFileGroupsForPartition(MetadataPartitionType.RECORD_INDEX);
            ValidationUtils.checkState(
                    fileGroupSize > 0, "Record index should have at least one file group");
        } catch (TableNotFoundException | IllegalStateException e) {
            // This means that record index has not been initialized.
            LOG.warn(
                    String.format(
                            "Record index not initialized so falling back to %s for tagging records",
                            FALLBACK_INDEX_TYPE.name()));

            // Fallback to another index so that tagLocation is still accurate and there are no
            // duplicates.
            HoodieWriteConfig otherConfig =
                    HoodieWriteConfig.newBuilder()
                            .withProperties(config.getProps())
                            .withIndexConfig(
                                    HoodieIndexConfig.newBuilder().withIndexType(FALLBACK_INDEX_TYPE).build())
                            .build();
        ...
        if (config.getRecordIndexUseCaching()) {
            records.persist(
                    new HoodieConfig(config.getProps())
                            .getString(HoodieIndexConfig.RECORD_INDEX_INPUT_STORAGE_LEVEL_VALUE));
        }

        // Partition the record keys to lookup such that each partition looks up one record index
        // shard
        JavaRDD<String> partitionedKeyRDD =
        ...
        HoodiePairData<String, HoodieRecordGlobalLocation> keyAndExistingLocations =
                HoodieJavaPairRDD.of(
                        partitionedKeyRDD.mapPartitionsToPair(
                                new RecordIndexFileGroupLookupFunction(hoodieTable)));
        ...
        // The number of partitions in the taggedRecords is expected to the maximum of the
        // partitions in
    ...
    public HoodieData<WriteStatus> updateLocation(
            HoodieData<WriteStatus> writeStatuses, HoodieEngineContext context, HoodieTable hoodieTable) {
        // This is a no-op as metadata record index updates are automatically maintained within the
        // metadata table.
    ...
    public boolean rollbackCommit(String instantTime) {
        // Only those deltacommits which have a valid completed commit on the dataset are read.
        // Since, the instantTime
        // is being rolled back on the dataset, we will anyway not load the records from the
        // deltacommit.
        //
        // In other words, there is no need to rollback anything here as the MDT rollback mechanism
    ...
    private static class RecordIndexFileGroupLookupFunction
        ...
        @Override
        public Iterator<Tuple2<String, HoodieRecordGlobalLocation>> call(
                Iterator<String> recordKeyIterator) {
    ...
    /**
      ...
      *
      * <p>NOTE: This is a workaround for SPARK-39391, which moved the PartitionIdPassthrough from
