...
public class HoodieSparkRecordMerger implements HoodieRecordMerger {
    ...
    public Option<Pair<HoodieRecord, Schema>> merge(
        ...
        if (older.getOrderingValue(oldSchema, props).compareTo(newer.getOrderingValue(newSchema, props))
                > 0) {
    ...
    @Override
    public Option<Pair<HoodieRecord, Schema>> partialMerge(
            HoodieRecord older,
            Schema oldSchema,
            HoodieRecord newer,
            Schema newSchema,
        ...
        if (older.getOrderingValue(oldSchema, props).compareTo(newer.getOrderingValue(newSchema, props))
                > 0) {
            return Option.of(
                    SparkRecordMergingUtils.mergePartialRecords(
