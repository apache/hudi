...
public class HoodieSparkCompactor<T> extends BaseCompactor<T,
  ...
  public void compact(HoodieInstant instant) {
    ...
    HoodieWriteMetadata<JavaRDD<WriteStatus>> compactionMetadata = writeClient.compact(instant.requestedTime());
    ...
    // Commit compaction
    writeClient.commitCompaction(instant.requestedTime(), compactionMetadata.getCommitMetadata().get(), Option.empty());
  }
}
