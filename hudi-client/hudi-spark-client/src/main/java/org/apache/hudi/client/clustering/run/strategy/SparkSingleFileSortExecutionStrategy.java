...
/**
  * This strategy is similar to {@link SparkSortAndSizeExecutionStrategy} with the difference being
  * that there should be only one large file group per clustering group.
  */
public class SparkSingleFileSortExecutionStrategy<T> extends MultipleSparkJobExecutionStrategy<T> {

    private static final Logger LOG =
            LoggerFactory.getLogger(SparkSingleFileSortExecutionStrategy.class);

    public SparkSingleFileSortExecutionStrategy(
            HoodieTable table, HoodieEngineContext engineContext, HoodieWriteConfig writeConfig) {
        super(table, engineContext, writeConfig);
    }

    @Override
    public HoodieData<WriteStatus> performClusteringWithRecordsAsRow(
        ...
        if (numOutputGroups != 1 || fileGroupIdList.size() != 1) {
            throw new HoodieClusteringException(
                    "Expect only one file group for strategy: " + getClass().getName());
        }
        LOG.info(
                "Starting clustering for a group, parallelism:"
                        + numOutputGroups
                        + " commit:"
                        + instantTime);
        ...
        // Since clustering will write to single file group using HoodieUnboundedCreateHandle, set
        // max file size to a large value.
    ...
    @Override
    public HoodieData<WriteStatus> performClusteringWithRecordsRDD(
        ...
        return (HoodieData<WriteStatus>)
                SparkBulkInsertHelper.newInstance()
                        .bulkInsert(
                                ...
                                newConfig,
                                false,
                                getRDDPartitioner(strategyParams, schema),
                                true,
                                numOutputGroups,
                                new SingleFileHandleCreateFactory(
                                        fileGroupIdList.get(0).getFileId(), shouldPreserveHoodieMetadata));
    }
}
