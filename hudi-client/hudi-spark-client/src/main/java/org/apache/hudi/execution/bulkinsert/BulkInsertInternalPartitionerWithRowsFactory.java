...
/**
  * A factory to generate built-in partitioner to repartition input Rows into at least expected
...
public abstract class BulkInsertInternalPartitionerWithRowsFactory {
    ...
    public static BulkInsertPartitioner<Dataset<Row>> get(
            HoodieWriteConfig config, boolean isTablePartitioned) {
        return get(config, isTablePartitioned, false);
    }

    public static BulkInsertPartitioner<Dataset<Row>> get(
            HoodieWriteConfig config, boolean isTablePartitioned, boolean enforceNumOutputPartitions) {
        ...
        switch (sortMode) {
            ...
            default:
                throw new UnsupportedOperationException(
                        "The bulk insert sort mode \"" + sortMode.name() + "\" is not supported.");
        }
    }
}
