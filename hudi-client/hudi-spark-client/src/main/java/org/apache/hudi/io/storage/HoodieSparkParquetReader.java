...
public class HoodieSparkParquetReader implements HoodieSparkFileReader {
    ...
    @Override
    public ClosableIterator<HoodieRecord<InternalRow>> getRecordIterator(
            Schema readerSchema, Schema requestedSchema) throws IOException {
        ...
        return new CloseableMappingIterator<>(
                iterator,
                data -> {
                    // NOTE: We have to do [[UnsafeProjection]] of incoming [[InternalRow]] to
                    // convert
    ...
    public ClosableIterator<InternalRow> getInternalRowIterator(
            Schema readerSchema, Schema requestedSchema) throws IOException {
        ...
        storage
                .getConf()
                .set(ParquetReadSupport.SPARK_ROW_REQUESTED_SCHEMA(), requestedStructType.json());
        storage
                .getConf()
                .set(
        ...
        ParquetReader<InternalRow> reader =
                ParquetReader.<InternalRow>builder(
                                (ReadSupport) new ParquetReadSupport(), new Path(path.toUri()))
    ...
    public Schema getSchema() {
        ...
        StructType structType =
                new ParquetToSparkSchemaConverter(storage.getConf().unwrapAs(Configuration.class))
                        .convert(messageType);
