...
public class SparkMetadataWriterUtils {
    ...
    public static HoodieJavaRDD<HoodieRecord> getFunctionalIndexRecordsUsingColumnStats(
        ...
        return HoodieJavaRDD.of(
                createColumnStatsRecords(partition, columnRangeMetadataList, false)
    ...
    private static void buildBloomFilterMetadata(
        ...
        bloomFilterMetadataList.add(
                createBloomFilterMetadataRecord(
                        partitionName,
                        filePath.toString(),
                        instantTime,
    ...
    private static Dataset<Row> readRecordsAsRow(
            StoragePath[] paths, SQLContext sqlContext, HoodieTableMetaClient metaClient, Schema schema) {
    ...
    private static <T extends Comparable<T>>
            HoodieColumnRangeMetadata<Comparable> computeColumnRangeMetadata(
                    Dataset<Row> rowDataset, String columnName, String filePath, long fileSize) {
        long totalSize = fileSize;
        // Get nullCount, minValue, and maxValue
        Dataset<Row> aggregated =
                rowDataset.agg(
                        functions
                                .count(functions.when(functions.col(columnName).isNull(), 1))
                                .alias("nullCount"),
        ...
        // Total uncompressed size is harder to get directly. This is just an approximation to
        // maintain the order.
