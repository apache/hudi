...
public class ConsistentBucketBulkInsertDataInternalWriterHelper
    ...
    public ConsistentBucketBulkInsertDataInternalWriterHelper(
    ...
    private HoodieRowCreateHandle getBucketRowCreateHandle(String partitionPath, String recordKey) {
        ...
        ValidationUtils.checkArgument(
                node.getTag() != ConsistentHashingNode.NodeTag.NORMAL
                        || hoodieTable
                                .getFileSystemView()
    ...
    private ConsistentBucketIdentifier getBucketIdentifier(String partition) {
        ...
        if (fileGroupsInPendingClustering.stream()
                .anyMatch(f -> f.getPartitionPath().equals(partition))) {
            Pair<String, ConsistentBucketIdentifier> bucketIdentifierPair =
                    ConsistentHashingUpdateStrategyUtils.constructPartitionToIdentifier(
                                    Collections.singleton(partition), hoodieTable)
                            .get(partition);
            ...
            HoodieConsistentHashingMetadata metadata =
                    ConsistentBucketIndexUtils.loadOrCreateMetadata(
                            hoodieTable, String.valueOf(partition), bucketNum);
