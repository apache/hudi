...
public class HoodieSparkFileWriterFactory extends HoodieFileWriterFactory {
    ...
    protected HoodieFileWriter newParquetFileWriter(
        ...
        HoodieRowParquetWriteSupport writeSupport =
                getHoodieRowParquetWriteSupport(
                        storage.getConf(), schema, config, enableBloomFilter(populateMetaFields, config));
        HoodieRowParquetConfig parquetConfig =
                new HoodieRowParquetConfig(
                        writeSupport,
        ...
        return new HoodieSparkParquetWriter(
    ...
    protected HoodieFileWriter newParquetFileWriter(
        ...
        HoodieRowParquetWriteSupport writeSupport =
                getHoodieRowParquetWriteSupport(storage.getConf(), schema, config, enableBloomFilter);
        String compressionCodecName =
                config.getStringOrDefault(HoodieStorageConfig.PARQUET_COMPRESSION_CODEC_NAME);
        // Support PARQUET_COMPRESSION_CODEC_NAME is ""
        if (compressionCodecName.isEmpty()) {
        ...
        HoodieRowParquetConfig parquetConfig =
                new HoodieRowParquetConfig(
                        ...
                        config.getBooleanOrDefault(HoodieStorageConfig.PARQUET_DICTIONARY_ENABLED));
        parquetConfig.getHadoopConf().addResource(writeSupport.getHadoopConf());
        return new HoodieSparkParquetStreamWriter(
                new FSDataOutputStream(outputStream, null), parquetConfig);
    }
    ...
    private static HoodieRowParquetWriteSupport getHoodieRowParquetWriteSupport(
            StorageConfiguration<?> conf, Schema schema, HoodieConfig config, boolean enableBloomFilter) {
        Option<BloomFilter> filter =
                enableBloomFilter ? Option.of(createBloomFilter(config)) : Option.empty();
        StructType structType = HoodieInternalRowUtils.getCachedSchema(schema);
        return HoodieRowParquetWriteSupport.getHoodieRowParquetWriteSupport(
                conf.unwrapAs(Configuration.class), structType, filter, config);
    }
}
