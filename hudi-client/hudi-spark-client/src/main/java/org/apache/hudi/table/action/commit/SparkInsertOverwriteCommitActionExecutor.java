...
public class SparkInsertOverwriteCommitActionExecutor<T> extends BaseSparkCommitActionExecutor<T> {
    ...
    public SparkInsertOverwriteCommitActionExecutor(
            HoodieEngineContext context,
            HoodieWriteConfig config,
            HoodieTable table,
            String instantTime,
            HoodieData<HoodieRecord<T>> inputRecordsRDD) {
    ...
    public HoodieWriteMetadata<HoodieData<WriteStatus>> execute() {
        return HoodieWriteHelper.newInstance()
                .write(
                        instantTime,
                        inputRecordsRDD,
                        context,
    ...
    protected Partitioner getPartitioner(WorkloadProfile profile) {
        return table
                .getStorageLayout()
                .layoutPartitionerClass()
                .map(c -> getLayoutPartitioner(profile, c))
                .orElseGet(
                        () ->
                                new SparkInsertOverwritePartitioner(
                                        profile, context, table, config, operationType));
    }
    ...
    protected Map<String, List<String>> getPartitionToReplacedFileIds(
        ...
        if (StringUtils.nonEmpty(staticOverwritePartition)) {
            ...
            context.setJobStatus(
                    this.getClass().getSimpleName(), "Getting ExistingFileIds of matching static partitions");
            return HoodieJavaPairRDD.getJavaPairRDD(
                            context
                                    .parallelize(partitionPaths, partitionPaths.size())
                                    .mapToPair(
                                            partitionPath ->
        ...
        switch (btype) {
            ...
            default:
                throw new AssertionError(
                        "Expect INSERT bucketType for insert overwrite, please correct the logical of "
