...
class ParquetBootstrapMetadataHandler extends BaseBootstrapMetadataHandler {
    ...
    public ParquetBootstrapMetadataHandler(
            HoodieWriteConfig config, HoodieTable table, HoodieFileStatus srcFileStatus) {
    ...
    Schema getAvroSchema(StoragePath sourceFilePath) throws IOException {
        ParquetMetadata readFooter =
                ParquetFileReader.readFooter(
                        (Configuration) table.getStorageConf().unwrap(),
    ...
    protected void executeBootstrap(
            HoodieBootstrapHandle<?, ?, ?, ?> bootstrapHandle,
        ...
        HoodieFileReader reader =
                getHoodieSparkIOFactory(table.getStorage())
        ...
        try {
            ...
            executor =
                    ExecutorFactory.create(
                            config,
                            recordIterator,
                            new BootstrapRecordConsumer(bootstrapHandle),
                            transformer,
    ...
    private HoodieRecord createNewMetadataBootstrapRecord(
            String recordKey, String partitionPath, HoodieRecord.HoodieRecordType recordType) {
        ...
        switch (recordType) {
            ...
            case SPARK:
                ...
                GenericInternalRow row =
                        new GenericInternalRow(METADATA_BOOTSTRAP_RECORD_SCHEMA.getFields().size());
            ...
            default:
                throw new UnsupportedOperationException(
                        String.format("Record type %s is not supported yet!", recordType));
        }
    }
}
