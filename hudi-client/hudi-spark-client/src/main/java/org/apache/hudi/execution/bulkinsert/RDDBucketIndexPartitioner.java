...
public abstract class RDDBucketIndexPartitioner<T>
    ...
    public RDDBucketIndexPartitioner(
            HoodieTable table, String sortString, boolean preserveHoodieMetadata) {
        super(table, sortString, preserveHoodieMetadata);
    }

    /**
      * Execute partition using the given partitioner. If sorting is required, will do it within each
      * data partition: - if sortColumnNames is specified, apply sort to the column (the behaviour is
      * the same as `RDDCustomColumnsSortPartitioner`) - if table requires sort or BulkInsertSortMode
      * is not None, then sort by record key within partition. By default, do partition only.
      *
      * @param records
      * @param partitioner a default partition that accepts `HoodieKey` as the partition key
      * @return
      */
    public JavaRDD<HoodieRecord<T>> doPartition(
            JavaRDD<HoodieRecord<T>> records, Partitioner partitioner) {
    ...
    /**
      ...
      * @return
      */
    private JavaRDD<HoodieRecord<T>> doPartitionAndCustomColumnSort(
            JavaRDD<HoodieRecord<T>> records, Partitioner partitioner) {
        final String[] sortColumns = sortColumnNames;
        final SerializableSchema schema =
                new SerializableSchema(
                        HoodieAvroUtils.addMetadataFields(
                                (new Schema.Parser().parse(table.getConfig().getSchema()))));
        Comparator<HoodieRecord<T>> comparator =
                (Comparator<HoodieRecord<T>> & Serializable)
                        (t1, t2) -> {
                            FlatLists.ComparableList obj1 =
                                    FlatLists.ofComparableArray(
                                            t1.getColumnValues(
                                                    schema.get(), sortColumns, consistentLogicalTimestampEnabled));
    ...
    /**
      ...
      * @return
      */
    private JavaRDD<HoodieRecord<T>> doPartitionAndSortByRecordKey(
            JavaRDD<HoodieRecord<T>> records, Partitioner partitioner) {
        if (table.getConfig().getBulkInsertSortMode() == BulkInsertSortMode.GLOBAL_SORT) {
            LOG.warn(
                    "Bucket index does not support global sort mode, the sort will only be done within each data partition");
        }

        Comparator<HoodieKey> comparator =
                (Comparator<HoodieKey> & Serializable)
