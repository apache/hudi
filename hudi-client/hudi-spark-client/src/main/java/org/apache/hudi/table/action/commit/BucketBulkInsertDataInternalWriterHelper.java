...
public class BucketBulkInsertDataInternalWriterHelper extends BulkInsertDataInternalWriterHelper {
    ...
    public BucketBulkInsertDataInternalWriterHelper(
    ...
    public void write(InternalRow row) throws IOException {
        try {
            ...
            int bucketId =
                    BucketIdentifier.getBucketId(String.valueOf(recordKey), indexKeyFields, bucketNum);
            if (lastFileId == null
                    || !Objects.equals(lastFileId.getKey(), partitionPath)
                    || !Objects.equals(lastFileId.getValue(), bucketId)) {
                // NOTE: It's crucial to make a copy here, since [[UTF8String]] could be pointing
                // into
    ...
    protected UTF8String extractRecordKey(InternalRow row) {
        if (populateMetaFields) {
            // In case meta-fields are materialized w/in the table itself, we can just simply
            // extract
    ...
    protected HoodieRowCreateHandle getBucketRowCreateHandle(
            Pair<UTF8String, Integer> fileId, int bucketId) throws Exception {
