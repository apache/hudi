...
public class SimpleKeyGenerator extends BuiltinKeyGenerator {
    ...
    SimpleKeyGenerator(
            TypedProperties props, Option<String> recordKeyField, String partitionPathField) {
        super(props);
        ...
        this.simpleAvroKeyGenerator =
                new SimpleAvroKeyGenerator(props, recordKeyField, partitionPathField);
    }
    ...
    private static void validatePartitionPath(String partitionPathField) {
        checkArgument(
                partitionPathField == null || !partitionPathField.isEmpty(),
                "Partition-path field has to be non-empty!");
        checkArgument(
                partitionPathField == null || !partitionPathField.contains(FIELDS_SEP),
                String.format(
                        "Single partition-path field is expected; provided (%s)", partitionPathField));
    }

    private void validateRecordKey(Option<String> recordKeyField) {
        checkArgument(
                !recordKeyField.isPresent() || !recordKeyField.get().isEmpty(),
                "Record key field has to be non-empty!");
        checkArgument(
                !recordKeyField.isPresent() || !recordKeyField.get().contains(FIELDS_SEP),
