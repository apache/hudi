...
import static org.apache.hudi.metadata.index.columnstats.ColumnStatsIndexer.isColumnTypeSupported;
import static org.apache.hudi.metadata.index.partitionstats.PartitionStatsIndexer.generateKeyPrefixes;
...
public class SparkExpressionIndexRecordGenerator implements ExpressionIndexRecordGenerator {
  ...
  public HoodieData<HoodieRecord> generate(
    ...
    HoodieSparkExpressionIndex.ExpressionIndexComputationMetadata expressionIndexComputationMetadata =
        getExprIndexRecords(filesToIndex, indexDefinition,
  ...
  @Override
  public HoodieData<HoodieRecord> updateFromCommitMetadata(
      HoodieTableMetaClient dataTableMetaClient,
      HoodieTableMetadata tableMetadata,
      HoodieCommitMetadata commitMetadata,
      String indexPartition, String instantTime) {
    ...
    if (isExprIndexUsingColumnStats) {
      ...
      HoodiePairData<String, HoodieColumnRangeMetadata<Comparable>> exprIndexPartitionStatUpdates =
          getExpressionIndexPartitionStatUpdates(
                  commitMetadata, indexPartition, engineContext, tableMetadata,
    ...
    int parallelism = Math.min(partitionFilePathPairs.size(), dataTableWriteConfig.getMetadataConfig().getExpressionIndexParallelism());
    ...
    HoodieSparkExpressionIndex.ExpressionIndexComputationMetadata expressionIndexComputationMetadata =
        getExprIndexRecords(
            partitionFilePathPairs, indexDefinition, dataTableMetaClient, parallelism, readerSchema,
            instantTime, engineContext, dataTableWriteConfig,
            // TODO(yihua): revisit metadata config here
            metadataTableWriteConfig, partitionRecordsFunctionOpt);
  ...
  public static HoodiePairData<String, List<HoodieColumnRangeMetadata<Comparable>>> getExpressionIndexPartitionStatUpdates(HoodieCommitMetadata commitMetadata, String indexPartition,
    ...
    try {
      ...
      return engineContext.parallelize(partitionPaths, parallelism).mapToPair(partitionName -> {
        ...
        Set<String> fileNames = getPartitionLatestFileSlicesIncludingInflight(dataMetaClient, Option.of(fileSystemView), partitionName).stream()
        ...
        List<HoodieColumnRangeMetadata<Comparable>> partitionColumnMetadata =
            tableMetadata.getRecordsByKeyPrefixes(generateKeyPrefixes(validColumnsToIndex, partitionName), indexPartition, false)
   ...
   * @param partitionRecordsFunctionOpt Function used to generate partition stat records for the EI.
   *                                    It takes the column range metadata generated for the provided partition files as input
   *                                    and uses those to generate the final partition stats
   * @return ExpressionIndexComputationMetadata containing both EI column stat records
   * and partition stat records if partitionRecordsFunctionOpt is provided
   */
  public static HoodieSparkExpressionIndex.ExpressionIndexComputationMetadata getExprIndexRecords(
      List<ExpressionIndexRecordGenerator.FileToIndex> filesToIndex,
    ...
    HoodieData<Row> rowData = sparkEngineContext.parallelize(filesToIndex, parallelism)
        .flatMap((SerializableFunction<FileToIndex, Iterator<Row>>) entry ->
  ...
  private static Iterator<Row> getExpressionIndexRecordsIterator(HoodieTableMetaClient metaClient, Schema readerSchema,
                                                                 HoodieWriteConfig dataWriteConfig,
                                                                 ExpressionIndexRecordGenerator.FileToIndex fileToIndex,
                                                                 SQLContext sqlContext) {
    String partition = fileToIndex.partition();
    String filePath = fileToIndex.path();
    String relativeFilePath = FSUtils.getRelativePartitionPath(
        metaClient.getBasePath(), new StoragePath(filePath));
    List<Row> rowsForFilePath = readRecordsAsRows(
        new StoragePath[] {new StoragePath(filePath)},
        sqlContext, metaClient, readerSchema, dataWriteConfig,
        FSUtils.isBaseFile(new StoragePath(filePath.substring(filePath.lastIndexOf("/") + 1))));
    List<Row> rowsWithIndexMetadata = getRowsWithExpressionIndexMetadata(
        rowsForFilePath, partition, relativeFilePath, fileToIndex.size());
  ...
  public static HoodieSparkExpressionIndex.ExpressionIndexComputationMetadata getExpressionIndexRecordsUsingColumnStats(
      Dataset<Row> dataset, HoodieExpressionIndex<Column, Column> expressionIndex,
    ...
    Dataset<Row> columnRangeMetadataDataset = dataset
        .select(columnToIndex, getExpressionIndexColumnNames())
        .groupBy(getExpressionIndexColumns())
    ...
    HoodiePairData<String, HoodieColumnRangeMetadata<Comparable>> rangeMetadataHoodieJavaRDD = HoodieJavaRDD.of(columnRangeMetadataDataset.javaRDD())
        .flatMapToPair((SerializableFunction<Row, Iterator<? extends Pair<String, HoodieColumnRangeMetadata<Comparable>>>>)
            row -> {
              int baseAggregatePosition = getExpressionIndexColumnNames().length;
    ...
    return partitionRecordsFunctionOpt.isPresent()
        ? new HoodieSparkExpressionIndex.ExpressionIndexComputationMetadata(colStatRecords, partitionStatRecordsOpt)
        : new HoodieSparkExpressionIndex.ExpressionIndexComputationMetadata(colStatRecords);
  }

  public static HoodieSparkExpressionIndex.ExpressionIndexComputationMetadata getExpressionIndexRecordsUsingBloomFilter(Dataset<Row> dataset, String columnToIndex,
    ...
    Dataset<HoodieRecord> bloomFilterRecords = dataset.select(columnToIndex, getExpressionIndexColumnNames())
