...
/**
  ...
  *
  * <p>- For physically partitioned table, repartition the input rows based on the partition path,
  * limiting the shuffle parallelism to specified `outputSparkPartitions`
  *
  * <p>- For physically non-partitioned table, simply does coalesce for the input rows with
  * `outputSparkPartitions`
  *
  * <p>Corresponding to the {@code BulkInsertSortMode.PARTITION_PATH_REPARTITION} mode.
  */
public class PartitionPathRepartitionPartitionerWithRows
    ...
    public PartitionPathRepartitionPartitionerWithRows(
    ...
    public Dataset<Row> repartitionRecords(Dataset<Row> rows, int outputSparkPartitions) {
        if (!shouldPopulateMetaFields) {
            throw new HoodieException(
                    PARTITION_PATH_REPARTITION.name() + " mode requires meta-fields to be enabled");
        }

        if (isTablePartitioned) {
            return rows.repartition(
                    outputSparkPartitions, new Column(HoodieRecord.PARTITION_PATH_METADATA_FIELD));
        }
