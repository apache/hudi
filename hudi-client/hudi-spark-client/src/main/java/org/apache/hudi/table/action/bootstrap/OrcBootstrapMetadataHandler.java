...
class OrcBootstrapMetadataHandler extends BaseBootstrapMetadataHandler {
    ...
    public OrcBootstrapMetadataHandler(
    ...
    void executeBootstrap(
        ...
        RecordReader reader =
                orcReader.rows(
                        new Reader.Options((Configuration) table.getStorageConf().unwrap()).schema(orcSchema));
        try {
            executor =
                    ExecutorFactory.create(
                            config,
                            new OrcReaderIterator<GenericRecord>(reader, avroSchema, orcSchema),
                            ...
                            inp -> {
                                ...
                                HoodieRecord rec =
                                        new HoodieAvroRecord(new HoodieKey(recKey, partitionPath), payload);
                                return rec;
                            },
