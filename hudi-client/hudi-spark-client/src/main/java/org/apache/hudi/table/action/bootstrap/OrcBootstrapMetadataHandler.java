...
class OrcBootstrapMetadataHandler extends BaseBootstrapMetadataHandler {
  ...
  @Override
  Schema getAvroSchema(StoragePath sourceFileLocation) throws IOException {
    Reader orcReader = OrcFile.createReader(
        new Path(sourceFileLocation.toUri()), OrcFile.readerOptions(table.getHadoopConf()));
  ...
  void executeBootstrap(HoodieBootstrapHandle<?, ?, ?, ?> bootstrapHandle,
                        StoragePath sourceFileLocation, KeyGeneratorInterface keyGenerator,
