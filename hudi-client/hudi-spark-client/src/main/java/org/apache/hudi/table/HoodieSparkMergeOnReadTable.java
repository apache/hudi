...
/**
  * Implementation of a more real-time Hoodie Table the provides tradeoffs on read and write
  * cost/amplification.
  *
  * <p>INSERTS - Same as HoodieCopyOnWriteTable - Produce new files, block aligned to desired size
  * (or) Merge with the smallest existing file, to expand it
  *
  * <p>UPDATES - Appends the changes to a rolling log file maintained per file Id. Compaction merges
  * the log file into the base file.
  *
  * <p>WARNING - MOR table type does not support nested rollbacks, every rollback must be followed by
  * an attempted commit action
  */
public class HoodieSparkMergeOnReadTable<T> extends HoodieSparkCopyOnWriteTable<T>
    ...
    public HoodieWriteMetadata<HoodieData<WriteStatus>> upsert(
        ...
        return new SparkUpsertDeltaCommitActionExecutor<>(
    ...
    public HoodieWriteMetadata<HoodieData<WriteStatus>> insert(
            HoodieEngineContext context, String instantTime, HoodieData<HoodieRecord<T>> records) {
        return new SparkInsertDeltaCommitActionExecutor<>(
                        (HoodieSparkEngineContext) context, config, this, instantTime, records)
                .execute();
    }
    ...
    public HoodieWriteMetadata<HoodieData<WriteStatus>> delete(
        ...
        return new SparkDeleteDeltaCommitActionExecutor<>(
    ...
    public HoodieWriteMetadata<HoodieData<WriteStatus>> deletePrepped(
        ...
        return new SparkDeletePreppedDeltaCommitActionExecutor<>(
    ...
    public HoodieWriteMetadata<HoodieData<WriteStatus>> upsertPrepped(
            HoodieEngineContext context, String instantTime, HoodieData<HoodieRecord<T>> preppedRecords) {
        return new SparkUpsertPreppedDeltaCommitActionExecutor<>(
                        (HoodieSparkEngineContext) context, config, this, instantTime, preppedRecords)
                .execute();
    }
    ...
    public HoodieWriteMetadata<HoodieData<WriteStatus>> insertPrepped(
            HoodieEngineContext context, String instantTime, HoodieData<HoodieRecord<T>> preppedRecords) {
        return new SparkInsertPreppedDeltaCommitActionExecutor<>(
                        (HoodieSparkEngineContext) context, config, this, instantTime, preppedRecords)
                .execute();
    }
    ...
    public HoodieWriteMetadata<HoodieData<WriteStatus>> bulkInsertPrepped(
            ...
            Option<BulkInsertPartitioner> userDefinedBulkInsertPartitioner) {
        return new SparkBulkInsertPreppedDeltaCommitActionExecutor(
                        (HoodieSparkEngineContext) context,
                        config,
                        this,
                        instantTime,
    ...
    public HoodieWriteMetadata<HoodieData<WriteStatus>> compact(
        ...
        RunCompactionActionExecutor<T> compactionExecutor =
    ...
    public HoodieBootstrapWriteMetadata<HoodieData<WriteStatus>> bootstrap(
        ...
        return new SparkBootstrapDeltaCommitActionExecutor<>(
    ...
    public void rollbackBootstrap(HoodieEngineContext context, String instantTime) {
        ...
        new RestorePlanActionExecutor<>(
                ...
                .execute();
        new MergeOnReadRestoreActionExecutor<>(
                        context, config, this, instantTime, HoodieTimeline.INIT_INSTANT_TS)
                .execute();
    }
