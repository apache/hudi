...
/**
  * Util class to merge records that may contain partial updates. This can be used by any Spark
  * {@link HoodieRecordMerger} implementation.
  */
public class SparkRecordMergingUtils {
    private static final Map<Schema, Map<Integer, StructField>> FIELD_ID_TO_FIELD_MAPPING_CACHE =
            new ConcurrentHashMap<>();
    private static final Map<Schema, Map<String, Integer>> FIELD_NAME_TO_ID_MAPPING_CACHE =
            new ConcurrentHashMap<>();
    private static final Map<
                    Pair<Pair<Schema, Schema>, Schema>,
    ...
    /**
      ...
      *
      * <p>For example, the reader schema is {[ {"name":"id", "type":"string"}, {"name":"ts",
      * "type":"long"}, {"name":"name", "type":"string"}, {"name":"price", "type":"double"},
      * {"name":"tags", "type":"string"} ]} The older and newer records can be (omitting Hudi meta
      * fields):
      *
      * <p>(1) older (complete record update): id | ts | name | price | tags 1 | 10 | apple | 2.3 |
      * fruit
      *
      * <p>newer (partial record update): ts | price 16 | 2.8
      *
      * <p>The merging result is (updated values from newer replaces the ones in the older):
      *
      * <p>id | ts | name | price | tags 1 | 16 | apple | 2.8 | fruit
      *
      * <p>(2) older (partial record update): ts | price 10 | 2.8
      *
      * <p>newer (partial record update): ts | tag 16 | fruit,juicy
      *
      * <p>The merging result is (two partial updates are merged together, and values of overlapped
      * fields come from the newer):
      *
      * <p>ts | price | tags 16 | 2.8 | fruit,juicy
      *
      ...
      */
    public static Pair<HoodieRecord, Schema> mergePartialRecords(
            HoodieSparkRecord older,
            Schema oldSchema,
        ...
        if (isNewerPartial) {
            ...
            for (int fieldId = 0; fieldId < mergedIdToFieldMapping.size(); fieldId++) {
                ...
                if (ordInPartialUpdate != null) {
                    ...
                    // The field does not exist in the newer record; picks the value from older
                    // record
                    values.add(
                            oldRow.get(oldNameToIdMapping.get(structField.name()), structField.dataType()));
                }
            }
            InternalRow mergedRow = new GenericInternalRow(values.toArray());

            HoodieSparkRecord mergedSparkRecord =
                    new HoodieSparkRecord(mergedRow, mergedSchemaPair.getRight().getLeft());
    ...
    public static Map<Integer, StructField> getCachedFieldIdToFieldMapping(Schema avroSchema) {
        return FIELD_ID_TO_FIELD_MAPPING_CACHE.computeIfAbsent(
                avroSchema,
                schema -> {
    ...
    public static Map<String, Integer> getCachedFieldNameToIdMapping(Schema avroSchema) {
        return FIELD_NAME_TO_ID_MAPPING_CACHE.computeIfAbsent(
                avroSchema,
                schema -> {
                    StructType structType = HoodieInternalRowUtils.getCachedSchema(schema);
    ...
    /**
      * Merges the two schemas so the merged schema contains all the fields from the two schemas, with
      * the same ordering of fields based on the provided reader schema.
      *
      * @param oldSchema Old schema.
      * @param newSchema New schema.
      * @param readerSchema Reader schema containing all the fields to read.
      * @return The ID to {@link StructField} instance mapping of the merged schema, and the {@link
      *     StructType} and Avro schema of the merged schema.
      */
    public static Pair<Map<Integer, StructField>, Pair<StructType, Schema>> getCachedMergedSchema(
            Schema oldSchema, Schema newSchema, Schema readerSchema) {
        return MERGED_SCHEMA_CACHE.computeIfAbsent(
                Pair.of(Pair.of(oldSchema, newSchema), readerSchema),
                schemaPair -> {
