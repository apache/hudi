...
/**
  ...
  *
  * <p>Each bucket incurs the following cost
  *
  * <pre>
  ...
  * </pre>
  *
  * The partitioner performs a two phase bin packing algorithm, to pack enough work into each bucket
  * such that cost of (1) & (3) is amortized. Also, avoids any skews in the sort based approach, by
  * directly partitioning by the file to be checked against and ensuring each partition has similar
  * number of buckets. Performance tests show that this approach could bound the amount of skew to
  * std_dev(numberOfBucketsPerPartition) * cost of (3), lower than sort partitioning.
  *
  * <p>Approach has two goals :
  *
  * <pre>
...
public class BucketizedBloomCheckPartitioner extends Partitioner {
    ...
    public BucketizedBloomCheckPartitioner(
        ...
        fileGroupToComparisons.forEach(
                (f, c) -> bucketsPerFileGroup.put(f, (int) Math.ceil((c * 1.0) / keysPerBucket)));
        ...
        // PHASE 1 : start filling upto minimum number of buckets into partitions, taking all but
        // one bucket from each file
        // This tries to first optimize for goal 1 above, with knowledge that each partition needs a
        // certain minimum number
        // of buckets and assigns buckets in the same order as file groups. If we were to simply
        // round robin, then buckets
        // for a file group is more or less guaranteed to be placed on different partitions all the
        // time.
        int minBucketsPerPartition = Math.max((int) Math.floor((1.0 * totalBuckets) / partitions), 1);
        LOG.info(
                String.format(
                        "TotalBuckets %d, min_buckets/partition %d", totalBuckets, minBucketsPerPartition));
        ...
        for (Map.Entry<HoodieFileGroupId, Integer> e : bucketsPerFileGroup.entrySet()) {
            for (int b = 0; b < Math.max(1, e.getValue() - 1); b++) {
                ...
                AtomicInteger cnt =
                        bucketsFilledPerFileGroup.getOrDefault(e.getKey(), new AtomicInteger(0));
                cnt.incrementAndGet();
                bucketsFilledPerFileGroup.put(e.getKey(), cnt);

                // mark this partition against the file group
                List<Integer> partitionList =
                        this.fileGroupToPartitions.getOrDefault(e.getKey(), new ArrayList<>());
        ...
        // PHASE 2 : for remaining unassigned buckets, round robin over partitions once. Since we
        // withheld 1 bucket from
        // each file group uniformly, this remaining is also an uniform mix across file groups. We
        // just round robin to
        // optimize for goal 2.
        for (Map.Entry<HoodieFileGroupId, Integer> e : bucketsPerFileGroup.entrySet()) {
