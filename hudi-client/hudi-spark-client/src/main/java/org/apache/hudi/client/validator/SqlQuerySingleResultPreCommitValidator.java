...
/**
  * Validator to run sql queries on new table state and expects a single result. If the result does
  * not match expected result, throw validation error.
  *
  * <p>Example configuration: "query1#expectedResult1;query2#expectedResult2;"
  */
public class SqlQuerySingleResultPreCommitValidator<T, I, K, O extends HoodieData<WriteStatus>>
    ...
    private static final Logger LOG =
            LoggerFactory.getLogger(SqlQuerySingleResultPreCommitValidator.class);

    public SqlQuerySingleResultPreCommitValidator(
    ...
    protected void validateUsingQuery(
        ...
        List<Row> newRows =
                executeSqlQuery(sqlContext, queryToRun, newTableSnapshot, "new state").collectAsList();
        if (newRows.size() != 1 && newRows.get(0).size() != 1) {
            throw new HoodieValidationException(
                    "Invalid query result. expect single value for '" + query + "'");
        }
        Object result = newRows.get(0).apply(0);
        if (result == null || !expectedResult.equals(result.toString())) {
            LOG.error(
                    "Mismatch query result. Expected: "
                            + expectedResult
                            + " got "
                            + result
                            + " on Query: "
