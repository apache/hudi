...
public abstract class BaseSparkCommitActionExecutor<T>
    ...
    public BaseSparkCommitActionExecutor(
    ...
    private HoodieData<HoodieRecord<T>> clusteringHandleUpdate(
            HoodieData<HoodieRecord<T>> inputRecords) {
        context.setJobStatus(
                this.getClass().getSimpleName(),
                "Handling updates which are under clustering: " + config.getTableName());
        ...
        UpdateStrategy<T, HoodieData<HoodieRecord<T>>> updateStrategy =
                (UpdateStrategy<T, HoodieData<HoodieRecord<T>>>)
                        ReflectionUtils.loadClass(
                                config.getClusteringUpdatesStrategyClass(),
        ...
        Pair<HoodieData<HoodieRecord<T>>, Set<HoodieFileGroupId>>
                recordsAndPendingClusteringFileGroups = updateStrategy.handleUpdate(inputRecords);
        ...
        // there are file groups pending clustering and receiving updates, so rollback the pending
        // clustering instants
        // there could be race condition, for example, if the clustering completes after instants
        // are fetched but before rollback completed
        if (config.isRollbackPendingClustering()) {
            Set<HoodieInstant> pendingClusteringInstantsToRollback =
                    getAllFileGroupsInPendingClusteringPlans(table.getMetaClient()).entrySet().stream()
                            ...
                            .collect(Collectors.toSet());
            pendingClusteringInstantsToRollback.forEach(
                    instant -> {
    ...
    public HoodieWriteMetadata<HoodieData<WriteStatus>> execute(
        ...
        if (inputRDD.getStorageLevel() == StorageLevel.NONE()) {
            HoodieJavaRDD.of(inputRDD)
                    .persist(
                            config.getTaggedRecordStorageLevel(),
                            context,
                            HoodieDataCacheKey.of(config.getBasePath(), instantTime));
        } else {
        ...
        context.setJobStatus(
                this.getClass().getSimpleName(), "Building workload profile:" + config.getTableName());
        HoodieTimer sourceReadAndIndexTimer =
                HoodieTimer.start(); // time taken from dedup -> tag location -> building workload
        // profile
        WorkloadProfile workloadProfile =
                new WorkloadProfile(
                        buildProfile(inputRecordsWithClusteringUpdate),
                        operationType,
        ...
        context.setJobStatus(
                this.getClass().getSimpleName(),
                "Doing partition and writing data: " + config.getTableName());
    ...
    private Pair<HashMap<String, WorkloadStat>, WorkloadStat> buildProfile(
            HoodieData<HoodieRecord<T>> inputRecords) {
        ...
        // count the number of both inserts and updates in each partition, update the counts to
        // workLoadStats
        for (Map.Entry<Tuple2<String, Option<HoodieRecordLocation>>, Long> e :
    ...
    private HoodieData<WriteStatus> mapPartitionsAsRDD(
            HoodieData<HoodieRecord<T>> dedupedRecords, Partitioner partitioner) {
        JavaPairRDD<Tuple2<HoodieKey, Option<HoodieRecordLocation>>, HoodieRecord<T>> mappedRDD =
                HoodieJavaPairRDD.getJavaPairRDD(
                        dedupedRecords.mapToPair(
                                record ->
                                        Pair.of(
                                                new Tuple2<>(
        ...
        if (table.requireSortedRecords()) {
            // Partition and sort within each partition as a single step. This is faster than
            // partitioning first and then
            // applying a sort.
    ...
    protected HoodieData<WriteStatus> updateIndex(
            HoodieData<WriteStatus> writeStatuses, HoodieWriteMetadata<HoodieData<WriteStatus>> result) {
        // cache writeStatusRDD before updating index, so that all actions before this are not
        // triggered again for future
        // RDD actions that are performed after updating the index.
        writeStatuses.persist(
    ...
    protected void updateIndexAndCommitIfNeeded(
            HoodieData<WriteStatus> writeStatusRDD, HoodieWriteMetadata<HoodieData<WriteStatus>> result) {
    ...
    protected void commit(HoodieWriteMetadata<HoodieData<WriteStatus>> result) {
        context.setJobStatus(
                this.getClass().getSimpleName(), "Commit write status collect: " + config.getTableName());
        commit(
    ...
    protected Map<String, List<String>> getPartitionToReplacedFileIds(
            HoodieWriteMetadata<HoodieData<WriteStatus>> writeStatuses) {
    ...
    public Iterator<List<WriteStatus>> handleUpdate(
        ...
        // Pre-check: if the old file does not exist (which may happen in bucket index case),
        // fallback to insert
        ...
        return (Partitioner)
                ReflectionUtils.loadClass(
                        ...
                        new Class[] {
                            ...
                            HoodieEngineContext.class,
                            HoodieTable.class,
                            HoodieWriteConfig.class
                        },
                        profile,
                        context,
                        table,
    ...
    @Override
    protected void runPrecommitValidators(
