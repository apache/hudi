...
public class SparkInternalSchemaConverter {
    ...
    public static Type buildTypeFromStructType(
            DataType sparkType, Boolean firstVisitRoot, AtomicInteger nextId) {
        if (sparkType instanceof StructType) {
            ...
            return Types.ArrayType.get(
                    elementId, array.containsNull(), buildTypeFromStructType(et, false, nextId));
            ...
            return Types.DecimalType.get(
                    ((DecimalType) sparkType).precision(), ((DecimalType) sparkType).scale());
        } else if (sparkType instanceof BinaryType) {
            return Types.BinaryType.get();
        } else {
            throw new UnsupportedOperationException(
                    String.format("Not a supported type: %s", sparkType.catalogString()));
        }
    }

    /**
      * Convert Spark schema to Hudi internal schema, and prune fields. Fields without IDs are kept and
      * assigned fallback IDs.
      *
    ...
    public static InternalSchema convertAndPruneStructTypeToInternalSchema(
            StructType sparkSchema, InternalSchema originSchema) {
    ...
    private static void collectColNamesFromStructType(
            DataType sparkType, Deque<String> fieldNames, List<String> resultSet) {
        if (sparkType instanceof StructType) {
            StructField[] fields = ((StructType) sparkType).fields();
            for (StructField f : fields) {
    ...
    private static void addFullName(
            DataType sparkType, String name, Deque<String> fieldNames, List<String> resultSet) {
        if (!(sparkType instanceof StructType)
    ...
    public static Map<Integer, Pair<DataType, DataType>> collectTypeChangedCols(
            InternalSchema schema, InternalSchema other) {
        return InternalSchemaUtils.collectTypeChangedCols(schema, other).entrySet().stream()
                .collect(
                        Collectors.toMap(
                                e -> e.getKey(),
                                e ->
                                        Pair.of(
                                                constructSparkSchemaFromType(e.getValue().getLeft()),
    ...
    private static DataType constructSparkSchemaFromType(Type type) {
        switch (type.typeId()) {
            case RECORD:
                ...
                for (Types.Field f : fields) {
                    ...
                    StructField structField =
                            StructField.apply(f.name(), dataType, f.isOptional(), Metadata.empty());
            ...
            case TIME:
                throw new UnsupportedOperationException(
                        String.format("cannot convert %s type to Spark", type));
            case TIMESTAMP:
            ...
            default:
                throw new UnsupportedOperationException(
                        String.format("cannot convert unknown type: %s to Spark", type));
        }
    }

    /**
      * Convert Int/long type to other Type. Now only support int/long ->
      * long/float/double/string/Decimal TODO: support more types
      */
    private static boolean convertIntLongType(
            WritableColumnVector oldV, WritableColumnVector newV, DataType newType, int len) {
        boolean isInt = oldV.dataType() instanceof IntegerType;
        if (newType instanceof LongType
                || newType instanceof FloatType
                || newType instanceof DoubleType
            ...
            for (int i = 0; i < len; i++) {
                ...
                if (newType instanceof LongType) {
                    ...
                    oldDecimal.changePrecision(
                            ((DecimalType) newType).precision(), ((DecimalType) newType).scale());
    ...
    /**
      * Convert float type to other Type. Now only support float -> double/String/Decimal TODO: support
      * more types
      */
    private static boolean convertFloatType(
            WritableColumnVector oldV, WritableColumnVector newV, DataType newType, int len) {
    ...
    /**
      * Convert double type to other Type. Now only support Double -> Decimal/String TODO: support more
      * types
      */
    private static boolean convertDoubleType(
            WritableColumnVector oldV, WritableColumnVector newV, DataType newType, int len) {
    ...
    /**
      * Convert decimal type to other Type. Now only support Decimal -> Decimal/String TODO: support
      * more types
      */
    private static boolean convertDecimalType(
            WritableColumnVector oldV, WritableColumnVector newV, DataType newType, int len) {
        ...
        if (newType instanceof DecimalType || newType instanceof StringType) {
            for (int i = 0; i < len; i++) {
                ...
                Decimal oldDecimal =
                        oldV.getDecimal(
                                i, ((DecimalType) oldType).precision(), ((DecimalType) oldType).scale());
                if (newType instanceof DecimalType) {
                    oldDecimal.changePrecision(
                            ((DecimalType) newType).precision(), ((DecimalType) newType).scale());
                    newV.putDecimal(i, oldDecimal, ((DecimalType) newType).precision());
    ...
    /** Convert date type to other Type. Now only support Date -> String TODO: support more types */
    private static boolean convertDateType(
            WritableColumnVector oldV, WritableColumnVector newV, DataType newType, int len) {
        if (newType instanceof StringType) {
            for (int i = 0; i < len; i++) {
                ...
                String res =
                        org.apache.spark.sql.catalyst.util.DateTimeUtils.toJavaDate(oldV.getInt(i)).toString();
    ...
    /**
      * Convert String type to other Type. Now only support String -> Decimal/Date. Notice: This
      * convert maybe failed!!! TODO: support more types
      */
    private static boolean convertStringType(
            WritableColumnVector oldV, WritableColumnVector newV, DataType newType, int len) {
                ...
                if (newType instanceof DateType) {
                    int days =
                            org.apache.spark.sql.catalyst.util.DateTimeUtils.fromJavaDate(
                                    Date.valueOf(oldV.getUTF8String(i).toString()));
                    newV.putInt(i, days);
                } else if (newType instanceof DecimalType) {
                    DecimalType decimalType = (DecimalType) newType;
                    java.math.BigDecimal bigDecimal =
                            new java.math.BigDecimal(oldV.getUTF8String(i).toString().trim());
    ...
    public static boolean convertColumnVectorType(
            WritableColumnVector oldV, WritableColumnVector newV, int len) {
