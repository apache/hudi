...
public class TimestampBasedKeyGenerator extends SimpleKeyGenerator {
    ...
    TimestampBasedKeyGenerator(
        ...
        timestampBasedAvroKeyGenerator =
                new TimestampBasedAvroKeyGenerator(config, recordKeyField, partitionPathField);
    }
    ...
    public String getRecordKey(Row row) {
        ...
        return combineRecordKey(
                getRecordKeyFieldNames(), Arrays.asList(rowAccessor.getRecordKeyParts(row)));
    }

    @Override
    public UTF8String getRecordKey(InternalRow internalRow, StructType schema) {
        tryInitRowAccessor(schema);
        return combineRecordKeyUnsafe(
                getRecordKeyFieldNames(), Arrays.asList(rowAccessor.getRecordKeyParts(internalRow)));
    }
    ...
    private String getFormattedPartitionPath(Object partitionPathPart) {
        ...
        if (partitionPathPart == null
                || Objects.equals(partitionPathPart, HUDI_DEFAULT_PARTITION_PATH)) {
        ...
        try {
            ...
            throw new HoodieKeyGeneratorException(
                    String.format("Failed to properly format partition-path (%s)", fieldVal), e);
        }
    }
}
