...
public class SparkCommitHelper<T extends HoodieRecordPayload<T>> extends BaseCommitHelper<T> {

  private static final Logger LOG = LogManager.getLogger(SparkCommitHelper.class);
  ...
  private void initKeyGenIfNeeded(boolean populateMetaFields) {
    if (!populateMetaFields) {
      try {
        keyGeneratorOpt = Option.of((BaseKeyGenerator) HoodieSparkKeyGeneratorFactory
            .createKeyGenerator(new TypedProperties(config.getProps())));
      } catch (IOException e) {
  ...
  private JavaRDD<HoodieRecord<T>> clusteringHandleUpdate(JavaRDD<HoodieRecord<T>> inputRecordsRDD) {
    if (config.isClusteringEnabled()) {
      Set<HoodieFileGroupId> fileGroupsInPendingClustering =
          table.getFileSystemView().getFileGroupsInPendingClustering().map(
              entry -> entry.getKey()).collect(Collectors.toSet());
  ...
  public HoodieWriteMetadata<HoodieData<WriteStatus>> execute(HoodieData<HoodieRecord<T>> inputRecords) {
    JavaRDD<HoodieRecord<T>> inputRecordsRDD = getRdd(inputRecords);
  ...
  private JavaRDD<HoodieRecord<T>> partition(
      JavaRDD<HoodieRecord<T>> dedupedRecords, Partitioner partitioner) {
    ...
    if (table.requireSortedRecords()) {
      ...
      Comparator<Tuple2<HoodieKey, Option<HoodieRecordLocation>>> comparator =
          (Comparator<Tuple2<HoodieKey, Option<HoodieRecordLocation>>> & Serializable) (t1, t2) -> {
  ...
  public HoodieData<WriteStatus> updateIndex(
      JavaRDD<WriteStatus> writeStatusRDD, HoodieWriteMetadata result) {
    ...
    HoodieData<WriteStatus> statuses = SparkHoodieRDDData.of((JavaRDD<WriteStatus>)
        table.getIndex().updateLocation(writeStatusRDD, context, table));
  ...
  public void updateIndexAndCommitIfNeeded(
      JavaRDD<WriteStatus> writeStatusRDD, HoodieWriteMetadata<HoodieData<WriteStatus>> result) {
  ...
  protected BaseMergeHelper getMergeHelper() {
    return SparkMergeHelper.newInstance();
  }
