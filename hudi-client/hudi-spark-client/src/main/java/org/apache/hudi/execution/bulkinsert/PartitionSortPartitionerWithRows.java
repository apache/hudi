...
/**
  * A built-in partitioner that does local sorting for each spark partitions after coalesce for bulk
    ...
    public Dataset<Row> repartitionRecords(Dataset<Row> rows, int outputSparkPartitions) {
        ...
        return rows.coalesce(outputSparkPartitions)
                .sortWithinPartitions(
                        HoodieRecord.PARTITION_PATH_METADATA_FIELD, HoodieRecord.RECORD_KEY_METADATA_FIELD);
    }
