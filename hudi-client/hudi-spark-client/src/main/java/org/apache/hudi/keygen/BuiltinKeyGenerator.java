...
/**
  ...
  *
  * <p>NOTE: By default it implements all the methods of {@link SparkKeyGeneratorInterface}, which by
  * default however fallback to Avro implementation. For maximum performance (to avoid conversion
  * from Spark's internal data-types to Avro) you should override these methods in your
  * implementation.
  *
  * <p>TODO rename to AvroFallbackBaseKeyGenerator
  */
@ThreadSafe
public abstract class BuiltinKeyGenerator extends BaseKeyGenerator
    ...
    public String getRecordKey(Row row) {
        ...
        // NOTE: This implementation has considerable computational overhead and has to be
        // overridden
        //       to provide for optimal performance on Spark. This implementation provided
        // exclusively
    ...
    /**
      ...
      * optimizations, like inlining)
      */
    protected final UTF8String combineRecordKeyUnsafe(
            List<String> fieldNames, List<Object> recordKeyParts) {
        return combineRecordKeyInternal(
    ...
    private <S> S combineCompositeRecordKeyInternal(
        ...
        for (int i = 0; i < recordKeyParts.length; ++i) {
            ...
            // This check is to validate that overall composite-key has at least one non-null,
            // non-empty
            // segment
        ...
        if (hasNonNullNonEmptyPart) {
            ...
            throw new HoodieKeyException(
                    String.format("All of the values for (%s) were either null or empty", recordKeyFields));
        }
    }
    ...
    private StringPartitionPathFormatter getStringPartitionPathFormatter() {
        if (stringPartitionPathFormatter == null) {
            synchronized (this) {
                if (stringPartitionPathFormatter == null) {
                    this.stringPartitionPathFormatter =
                            new StringPartitionPathFormatter(
                                    StringPartitionPathFormatter.JavaStringBuilder::new,
    ...
    private UTF8StringPartitionPathFormatter getUTF8StringPartitionPathFormatter() {
        if (utf8StringPartitionPathFormatter == null) {
            synchronized (this) {
                if (utf8StringPartitionPathFormatter == null) {
                    this.utf8StringPartitionPathFormatter =
                            new UTF8StringPartitionPathFormatter(
                                    UTF8StringPartitionPathFormatter.UTF8StringBuilder::new,
                                    hiveStylePartitioning,
                                    encodePartitionPath);
                }
    ...
    private static boolean isNullOrEmptyCompositeKeyPartUTF8(UTF8String keyPart) {
        ...
        //       for performance reasons (it relies on the fact that we're using internalized
        //       constants)
        return keyPart == NULL_RECORD_KEY_PLACEHOLDER_UTF8
                || keyPart == EMPTY_RECORD_KEY_PLACEHOLDER_UTF8;
    }

    /**
      * Converts provided (raw) value extracted from the {@link InternalRow} object into a
      * deserialized, JVM native format (for ex, converting {@code Long} into {@link Instant}, {@code
      * Integer} to {@link LocalDate}, etc)
      *
      * <p>This method allows to avoid costly full-row deserialization sequence. Note, that this method
      * should be maintained in sync w/
      *
      * <ol>
      *   <li>{@code RowEncoder#deserializerFor}, as well as
      *   <li>{@code HoodieAvroUtils#convertValueForAvroLogicalTypes}
      * </ol>
      *
    ...
    protected static class SparkRowConverter {
        ...
        SparkRowConverter(StructType schema) {
            ...
            this.avroConverter =
                    AvroConversionUtils.createConverterToAvro(schema, STRUCT_NAME, NAMESPACE);
        }
    ...
    protected class SparkRowAccessor {
        ...
        SparkRowAccessor(StructType schema) {
            ...
            this.partitionPathFieldsPaths =
                    resolveNestedFieldPaths(getPartitionPathFields(), schema, true);
        }
        ...
        private Object[] getNestedFieldValues(
                Row row, HoodieUnsafeRowUtils.NestedFieldPath[] nestedFieldsPaths) {
            Object[] nestedFieldValues = new Object[nestedFieldsPaths.length];
            for (int i = 0; i < nestedFieldsPaths.length; ++i) {
                nestedFieldValues[i] =
                        HoodieUnsafeRowUtils$.MODULE$.getNestedRowValue(row, nestedFieldsPaths[i]);
            }
            return nestedFieldValues;
        }

        private Object[] getNestedFieldValues(
                InternalRow row, HoodieUnsafeRowUtils.NestedFieldPath[] nestedFieldsPaths) {
            Object[] nestedFieldValues = new Object[nestedFieldsPaths.length];
            for (int i = 0; i < nestedFieldsPaths.length; ++i) {
                Object rawValue =
                        HoodieUnsafeRowUtils$.MODULE$.getNestedInternalRowValue(row, nestedFieldsPaths[i]);
        ...
        private HoodieUnsafeRowUtils.NestedFieldPath[] resolveNestedFieldPaths(
                List<String> fieldPaths, StructType schema, boolean returnNull) {
            try {
                return fieldPaths.stream()
                        .map(
                                fieldPath ->
                                        HoodieUnsafeRowUtils$.MODULE$.composeNestedFieldPath(schema, fieldPath).get())
                ...
                LOG.error(
                        String.format(
                                "Failed to resolve nested field-paths (%s) in schema (%s)", fieldPaths, schema),
                        e);
