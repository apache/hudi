...
public class HoodieSparkFunctionalIndex
        implements HoodieFunctionalIndex<Column, Column>, Serializable {
    ...
    /**
      * Map of Spark functions to their implementations. NOTE: This is not an exhaustive list of
      * spark-sql functions. Only the common date/timestamp and string functions have been added. Add
      * more functions as needed. However, keep the key should match the exact spark-sql function name
      * in lowercase.
      */
                    ...
                    Pair.of(
                            ...
                            (columns, options) -> {
                                ...
                                return functions.substring(
                                        columns.get(0),
                                        Integer.parseInt(options.get("pos")),
                                        Integer.parseInt(options.get("len")));
                            }),
                    Pair.of(
                            SPARK_LOWER,
                            (columns, options) -> {
                    ...
                    Pair.of(
                            ...
                            (columns, options) -> {
                                ...
                                return functions.regexp_replace(
                                        columns.get(0), options.get("pattern"), options.get("replacement"));
                            }),
                    Pair.of(
                            ...
                            (columns, options) -> {
                                ...
                                return functions.regexp_extract(
                                        columns.get(0), options.get("pattern"), Integer.parseInt(options.get("idx")));
                            }),
                    Pair.of(
                            SPARK_SPLIT,
                            (columns, options) -> {
    ...
    public HoodieSparkFunctionalIndex(
            ...
            List<String> orderedSourceFields,
    ...
    public Column apply(List<Column> orderedSourceValues) {
        if (orderedSourceValues.size() != orderedSourceFields.size()) {
            throw new IllegalArgumentException(
                    "Mismatch in number of source values and fields in the expression");
        }
