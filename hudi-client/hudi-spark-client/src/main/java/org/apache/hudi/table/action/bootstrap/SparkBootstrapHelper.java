...
public class SparkBootstrapHelper<T extends HoodieRecordPayload<T>> extends BaseBootstrapHelper<T> {

  private static final Logger LOG = LogManager.getLogger(SparkBootstrapHelper.class);

  protected SparkBootstrapHelper() {
  }

  private static class BootstrapHelperHolder {
    private static final SparkBootstrapHelper SPARK_BOOTSTRAP_HELPER = new SparkBootstrapHelper();
  }

  public static SparkBootstrapHelper newInstance() {
    return BootstrapHelperHolder.SPARK_BOOTSTRAP_HELPER;
  }
  ...
  public HoodieBootstrapWriteMetadata<HoodieData<WriteStatus>> execute(
    ...
    try {
      ...
      Map<BootstrapMode, List<Pair<String, List<HoodieFileStatus>>>> partitionSelections =
          listAndProcessSourcePartitions(context, table, config, (SparkBootstrapCommitHelper<T>) commitHelper);

      // First run metadata bootstrap which will auto commit
      Option<HoodieWriteMetadata<HoodieData<WriteStatus>>> metadataResult =
          metadataBootstrap(context, table, config, commitHelper, partitionSelections.get(BootstrapMode.METADATA_ONLY));
      ...
      Option<HoodieWriteMetadata<HoodieData<WriteStatus>>> fullBootstrapResult =
          fullBootstrap(context, table, config, extraMetadata, commitHelper.getSchemaToStoreInCommit(),
              partitionSelections.get(BootstrapMode.FULL_RECORD));
  ...
  protected Option<HoodieWriteMetadata<HoodieData<WriteStatus>>> metadataBootstrap(
    ...
    metaClient.getActiveTimeline().createNewInstant(
        new HoodieInstant(HoodieInstant.State.REQUESTED, metaClient.getCommitActionType(),
    ...
    JavaRDD<BootstrapWriteStatus> bootstrapWriteStatuses =
        runMetadataBootstrap(context, table, config, partitionFilesList);

    HoodieWriteMetadata<HoodieData<WriteStatus>> result = new HoodieWriteMetadata<>();
    updateIndexAndCommitIfNeeded(context, table, config, commitHelper, bootstrapWriteStatuses.map(w -> w), result);
    return Option.of(result);
  }

  private void updateIndexAndCommitIfNeeded(HoodieEngineContext context,
                                            HoodieTable table,
                                            HoodieWriteConfig config,
                                            BaseCommitHelper<T> commitHelper,
    ...
    JavaRDD<WriteStatus> statuses = (JavaRDD<WriteStatus>) table.getIndex().updateLocation(writeStatusRDD, context,
        table);
    result.setIndexUpdateDuration(Duration.between(indexStartTime, Instant.now()));
    result.setWriteStatuses(SparkHoodieRDDData.of(statuses));
    commitHelper.commitOnAutoCommit(result);
  }

  private void validate(HoodieWriteConfig config) {
   ...
   * @param partitionFilesList List of partitions and files within that partitions
   */
  protected Option<HoodieWriteMetadata<HoodieData<WriteStatus>>> fullBootstrap(
      HoodieEngineContext context,
      HoodieTable table,
      HoodieWriteConfig config,
      Option<Map<String, String>> extraMetadata,
    ...
    JavaRDD<HoodieRecord<T>> inputRecordsRDD =
        (JavaRDD<HoodieRecord<T>>) inputProvider.generateInputRecords("bootstrap_source", config.getBootstrapSourceBasePath(),
            partitionFilesList);
    ...
    return Option.of(getBulkInsertActionExecutor(context, table, config, extraMetadata, bootstrapSchema, inputRecordsRDD).execute());
  }
  ...
  protected BulkInsertCommitActionExecutor<T> getBulkInsertActionExecutor(
    ...
    return new BulkInsertCommitActionExecutor<T>(
        ...
        new SparkCommitHelper<>((HoodieSparkEngineContext) context, bulkInsertConfig, table,
            HoodieTimeline.FULL_BOOTSTRAP_INSTANT_TS, WriteOperationType.BULK_INSERT),
        SparkBulkInsertHelper.newInstance());
  }

  private BootstrapWriteStatus handleMetadataBootstrap(HoodieTable table, HoodieWriteConfig config,
                                                       String srcPartitionPath, String partitionPath,
  ...
  private Map<BootstrapMode, List<Pair<String, List<HoodieFileStatus>>>> listAndProcessSourcePartitions(
      ...
      HoodieTable table,
      HoodieWriteConfig config,
      SparkBootstrapCommitHelper commitHelper) throws IOException {
    FileSystem bootstrapSourceFileSystem =
        FSUtils.getFs(config.getBootstrapSourceBasePath(), context.getHadoopConf().get());
    ...
    String bootstrapSchema = sourceSchemaProvider.getBootstrapSchema(context, folders).toString();
    commitHelper.setBootstrapSchema(bootstrapSchema);
  ...
  private JavaRDD<BootstrapWriteStatus> runMetadataBootstrap(
      HoodieEngineContext context,
      HoodieTable table,
      HoodieWriteConfig config,
    ...
    return jsc.parallelize(bootstrapPaths, config.getBootstrapParallelism())
        .map(partitionFsPair -> handleMetadataBootstrap(table, config, partitionFsPair.getLeft(), partitionFsPair.getRight().getLeft(),
