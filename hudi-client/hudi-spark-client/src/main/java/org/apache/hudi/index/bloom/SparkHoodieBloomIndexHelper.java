...
public class SparkHoodieBloomIndexHelper extends BaseHoodieBloomIndexHelper {
  ...
  public HoodiePairData<HoodieKey, HoodieRecordLocation> findMatchingFilesForRecordKeys(
    ...
    LOG.info(
        String.format(
            "Input parallelism: %d, Index parallelism: %d", inputParallelism, targetParallelism));
    ...
    if (config.getBloomIndexUseMetadata()
      ...
      //    - (G1) All requests to MT are made in batch (ie we're trying to fetch all the
      // values
      //      for corresponding keys at once)
      //    - (G2) Each task reads no more than just _one_ file-group from the MT Bloom
      // Filters
      //    partition
      //
      // Ta achieve G2, following invariant have to be maintained: Spark partitions have to be
      // affine w/ Metadata Table's file-groups, meaning that each Spark partition holds
      // records
      // belonging to one and only file-group in MT Bloom Filters partition. To provide for
      // that
      // we need to make sure
      //    - Spark's used [[Partitioner]] employs same hashing function as Metadata Table (as
      // well
      //      as being applied to the same keys as the MT one)
      //    - Make sure that # of partitions is congruent to the # of file-groups (ie number
      // of Spark
      ...
      //    partition). In that case Spark partitions for which `hash(key) % N` will be either
      // 0
      ...
      AffineBloomIndexFileGroupPartitioner partitioner =
          new AffineBloomIndexFileGroupPartitioner(
              baseFileOnlyViewBroadcast, adjustedTargetParallelism);

      // First, we need to repartition and sort records using
      // [[AffineBloomIndexFileGroupPartitioner]]
      // to make sure every Spark task accesses no more than just a single file-group in MT
      // (allows
      // us to achieve G2).
      //
      // NOTE: Sorting records w/in individual partitions is required to make sure that we
      // cluster
      ...
      keyLookupResultRDD =
          fileComparisonsRDD
              ...
              .mapPartitionsToPair(
                  new HoodieMetadataBloomFilterProbingFunction(
                      baseFileOnlyViewBroadcast, hoodieTable))
              // Second, we use [[HoodieFileProbingFunction]] to open actual file and
              // check whether it
              // contains the records with candidate keys that were filtered in by the
              // Bloom Filter
              .mapPartitions(
                  new HoodieFileProbingFunction(baseFileOnlyViewBroadcast, storageConf), true);
      ...
      Partitioner partitioner =
          new BucketizedBloomCheckPartitioner(
              targetParallelism, comparisonsPerFileGroup, config.getBloomIndexKeysPerBucket());

      keyLookupResultRDD =
          fileComparisonsRDD
              .mapToPair(t -> new Tuple2<>(Pair.of(t._1, t._2), t))
              .repartitionAndSortWithinPartitions(partitioner)
              .map(Tuple2::_2)
      ...
      keyLookupResultRDD =
          fileComparisonsRDD
              .sortByKey(true, targetParallelism)
              .mapPartitions(new HoodieSparkBloomIndexCheckFunction(hoodieTable, config), true);
    }
            ...
            .flatMapToPair(
                lookupResult ->
                    lookupResult.getMatchingRecordKeysAndPositions().stream()
                        .map(
                            recordKeyAndPosition ->
                                new Tuple2<>(
                                    new HoodieKey(
                                        recordKeyAndPosition.getLeft(),
                                        lookupResult.getPartitionPath()),
                                    new HoodieRecordLocation(
                                        lookupResult.getBaseInstantTime(),
  ...
  private Map<HoodieFileGroupId, Long> computeComparisonsPerFileGroup(
    ...
    if (config.getBloomIndexPruneByRanges()) {
      ...
      context.setJobStatus(
          ...
          "Compute all comparisons needed between records and files: " + config.getTableName());
      fileToComparisons = fileComparisonsRDD.countByKey();
    } else {
      fileToComparisons = new HashMap<>();
      partitionToFileInfo.forEach(
          (partitionPath, fileInfos) -> {
  ...
  private static HoodieTableFileSystemView getBaseFileOnlyView(
      HoodieTable<?, ?, ?, ?> hoodieTable, Collection<String> partitionPaths) {
    try {
      List<String> fullPartitionPaths =
          partitionPaths.stream()
              .map(
                  partitionPath ->
                      String.format(
                          "%s/%s", hoodieTable.getMetaClient().getBasePath(), partitionPath))
              .collect(Collectors.toList());
      ...
      return new HoodieTableFileSystemView(
          hoodieTable.getMetaClient(), hoodieTable.getActiveTimeline(), allFiles);
    } catch (IOException e) {
  ...
  static class AffineBloomIndexFileGroupPartitioner extends Partitioner {
    ...
    AffineBloomIndexFileGroupPartitioner(
        Broadcast<HoodieTableFileSystemView> baseFileOnlyViewBroadcast, int targetPartitions) {
    ...
    public int getPartition(Object key) {
      ...
      // NOTE: This is a workaround to alleviate performance impact of needing to process
      // whole
      //       partition for every file-group being looked up.
      //       See HUDI-5619 for more details
      String baseFileName =
          cachedLatestBaseFileNames
              .computeIfAbsent(
                  partitionPath,
                  ignored ->
      ...
      if (baseFileName == null) {
        throw new HoodieException(
            String.format(
                "File from file-group (%s) not found in partition path (%s)",
                fileGroupId, partitionPath));
      }

      String bloomIndexEncodedKey =
          getBloomFilterIndexKey(
              new PartitionIndexID(
                  HoodieTableMetadataUtil.getBloomFilterIndexPartitionIdentifier(partitionPath)),
  ...
  public static class HoodieSparkBloomIndexCheckFunction
      ...
      implements FlatMapFunction<
          Iterator<Tuple2<HoodieFileGroupId, String>>, List<HoodieKeyLookupResult>> {
    ...
    @Override
    public Iterator<List<HoodieKeyLookupResult>> call(
        Iterator<Tuple2<HoodieFileGroupId, String>> fileGroupIdRecordKeyPairIterator) {
