...
public class SpaceCurveSortingHelper {
    ...
    /**
      * Orders provided {@link Dataset} by mapping values of the provided list of columns {@code
      * orderByCols} onto a specified space curve (Z-curve, Hilbert, etc)
      *
      * <p>NOTE: Only support base data-types:
      * long,int,short,double,float,string,timestamp,decimal,date,byte. This method is more effective
      * than {@link #orderDataFrameBySamplingValues} leveraging data sampling instead of direct mapping
      *
    ...
    public static Dataset<Row> orderDataFrameByMappingValues(
        ...
        List<String> checkCols =
                orderByCols.stream().filter(columnsMap::containsKey).collect(Collectors.toList());

        if (orderByCols.size() != checkCols.size()) {
            LOG.error(
                    String.format(
                            "Trying to ordering over a column(s) not present in the schema (%s); skipping",
                            CollectionUtils.diff(orderByCols, checkCols)));
            return df;
        }
        ...
        if (orderByCols.size() == 1) {
            ...
            LOG.debug(
                    String.format(
                            "Single column to order by (%s), skipping space-curve ordering", orderByColName));
        ...
        Map<Integer, StructField> fieldMap =
                orderByCols.stream()
                        .collect(
                                Collectors.toMap(
                                        e -> Arrays.asList(df.schema().fields()).indexOf(columnsMap.get(e)),
                                        columnsMap::get));
        ...
        switch (layoutOptStrategy) {
            ...
            case HILBERT:
                sortedRDD =
                        createHilbertSortedRDD(df.toJavaRDD(), fieldMap, fieldNum, targetPartitionCount);
                break;
            default:
                throw new UnsupportedOperationException(
                        String.format("Not supported layout-optimization strategy (%s)", layoutOptStrategy));
        }
    ...
    private static JavaRDD<Row> createZCurveSortedRDD(
            JavaRDD<Row> originRDD, Map<Integer, StructField> fieldMap, int fieldNum, int fileNum) {
        return originRDD
                .map(
                        row -> {
                            byte[][] zBytes =
    ...
    private static JavaRDD<Row> createHilbertSortedRDD(
            JavaRDD<Row> originRDD, Map<Integer, StructField> fieldMap, int fieldNum, int fileNum) {
        // NOTE: Here {@code mapPartitions} is used to make sure Hilbert curve instance is
        // initialized
                        ...
                        rows -> {
                            ...
                            return new Iterator<Row>() {
                                ...
                                public Row next() {
                                    ...
                                    // Map N-dimensional coordinates into position on the Hilbert
                                    // curve
    ...
    private static byte[] mapColumnValueTo8Bytes(Row row, int index, DataType dataType) {
        if (dataType instanceof LongType) {
            ...
            return BinaryUtil.doubleTo8Byte(
                    row.isNullAt(index) ? Double.MAX_VALUE : row.getDouble(index));
            ...
            return BinaryUtil.longTo8Byte(
                    row.isNullAt(index) ? Long.MAX_VALUE : row.getDate(index).getTime());
        } else if (dataType instanceof TimestampType) {
            return BinaryUtil.longTo8Byte(
                    row.isNullAt(index) ? Long.MAX_VALUE : row.getTimestamp(index).getTime());
            ...
            return BinaryUtil.longTo8Byte(
                    row.isNullAt(index) ? Long.MAX_VALUE : row.getDecimal(index).longValue());
        } else if (dataType instanceof BooleanType) {
            boolean value = row.isNullAt(index) ? false : row.getBoolean(index);
            return BinaryUtil.intTo8Byte(value ? 1 : 0);
        } else if (dataType instanceof BinaryType) {
            return BinaryUtil.paddingTo8Byte(
                    row.isNullAt(index) ? new byte[] {0} : (byte[]) row.get(index));
        }

        throw new UnsupportedOperationException(
                String.format("Unsupported data-type (%s)", dataType.typeName()));
    }
    ...
    public static Dataset<Row> orderDataFrameBySamplingValues(
        ...
        return RangeSampleSort$.MODULE$.sortDataFrameBySample(
                df,
                layoutOptStrategy,
                JavaScalaConverters.convertJavaListToScalaList(orderByCols),
