...
/**
  * UpsertPartitioner for MergeOnRead table type, this allows auto correction of small parquet files
  * to larger ones without the need for an index in the logFile.
  */
public class SparkUpsertDeltaCommitPartitioner<T> extends UpsertPartitioner<T> {

    public SparkUpsertDeltaCommitPartitioner(
    ...
    protected List<SmallFile> getSmallFiles(String partitionPath) {
        ...
        List<FileSlice> smallFileSlicesCandidates =
                getSmallFileCandidates(partitionPath, latestCommitTime);
    ...
    @Nonnull
    private List<FileSlice> getSmallFileCandidates(
            String partitionPath, HoodieInstant latestCommitInstant) {
        // If we can index log files, we can add more inserts to log files for fileIds NOT including
        // those under
        // pending compaction
        ...
        // If we cannot index log files, then we choose the smallest parquet file in the partition
        // and add inserts to
        ...
        return table
                ...
                .filter(
                        fileSlice ->
                                // NOTE: We can not pad slices with existing log-files w/o
                                // compacting these,
                                //       hence skipping
    ...
    private long getTotalFileSize(FileSlice fileSlice) {
        if (!fileSlice.getBaseFile().isPresent()) {
            return convertLogFilesSizeToExpectedParquetSize(
