...
public class HoodieRowCreateHandle implements Serializable {
    ...
    public HoodieRowCreateHandle(
        ...
        String fileName =
                FSUtils.makeBaseFileName(
                        instantTime, writeToken, this.fileId, table.getBaseFileExtension());
        ...
        this.writeStatus =
                new WriteStatus(
                        table.shouldTrackSuccessRecords(), writeConfig.getWriteStatusFailureFraction());
        ...
        try {
            ...
            this.fileWriter =
                    HoodieInternalRowFileWriterFactory.getInternalRowFileWriter(
                            path, table, writeConfig, structType);
        } catch (IOException e) {
    ...
    /**
      * Writes an {@link InternalRow} to the underlying HoodieInternalRowFileWriter. Before writing,
      * value for meta columns are computed as required and wrapped in {@link HoodieInternalRow}.
      * {@link HoodieInternalRow} is what gets written to HoodieInternalRowFileWriter.
      *
    ...
    private void writeRow(InternalRow row) {
        try {
            ...
            //          - Conversions b/w Spark internal types and JVM native ones (like
            // [[UTF8String]]
            //          and [[String]])
            //          - Repeated computations (for ex, converting file-path to [[UTF8String]] over
            // and
            //          over again)
            ...
            UTF8String writeCommitTime =
                    shouldPreserveHoodieMetadata
                            ? row.getUTF8String(HoodieRecord.COMMIT_TIME_METADATA_FIELD_ORD)
                            : commitTime;

            InternalRow updatedRow =
                    new HoodieInternalRow(
                            writeCommitTime, seqId, recordKey, partitionPath, fileName, row, true);
            try {
                fileWriter.writeRow(recordKey, updatedRow);
                // NOTE: To avoid conversion on the hot-path we only convert [[UTF8String]] into
                // [[String]]
                ...
                HoodieRecordDelegate recordDelegate =
                        writeStatus.isTrackingSuccessfulWrites()
                                ? HoodieRecordDelegate.create(
                                        recordKey.toString(), partitionPath.toString(), null, newRecordLocation)
                                : null;
    ...
    private void writeRowNoMetaFields(InternalRow row) {
        try {
            ...
            //      meta-fields would fail if any record will, while when writing w/ meta-fields it
            // won't)
    ...
    /**
      * Closes the {@link HoodieRowCreateHandle} and returns an instance of {@link WriteStatus}
      * containing the stats and status of the writes to this handle.
      *
    ...
    private static StoragePath makeNewPath(
            HoodieStorage storage, String partitionPath, String fileName, HoodieWriteConfig writeConfig) {
