...
/**
  * Spark base update strategy, write records to the file groups which are in clustering need to
  * check. Spark relate implementations should extend this base class.
  */
public abstract class BaseSparkUpdateStrategy<T>
        extends UpdateStrategy<T, HoodieData<HoodieRecord<T>>> {

    public BaseSparkUpdateStrategy(
    ...
    /**
      ...
      */
    protected List<HoodieFileGroupId> getGroupIdsWithUpdate(
                ...
                .map(
                        record ->
                                new HoodieFileGroupId(
                                        record.getPartitionPath(), record.getCurrentLocation().getFileId()))
                .distinct()
                .collectAsList();
    }
}
