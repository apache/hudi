...
public class SparkValidatorUtils {
    ...
    /**
      ...
      *
      * <p>Throw error if there are validation failures.
      */
    public static void runValidators(
        ...
        if (StringUtils.isNullOrEmpty(config.getPreCommitValidators())) {
            ...
            if (!writeMetadata.getWriteStats().isPresent()) {
                writeMetadata.setWriteStats(
                        writeMetadata.getWriteStatuses().map(WriteStatus::getStat).collectAsList());
            }
            ...
            // Refresh timeline to ensure validator sees the any other operations done on timeline
            // (async operations such as other clustering/compaction/rollback)
            table.getMetaClient().reloadActiveTimeline();
            Dataset<Row> afterState =
                    getRecordsFromPendingCommits(
                            sqlContext, partitionsModified, writeMetadata, table, instantTime);
            Dataset<Row> beforeState =
                    getRecordsFromCommittedFiles(sqlContext, partitionsModified, table, afterState.schema());
            ...
            boolean allSuccess =
                    validators
                            .map(v -> runValidatorAsync(v, writeMetadata, beforeState, afterState, instantTime))
    ...
    /**
      * Run validators in a separate thread pool for parallelism. Each of validator can submit a
      * distributed spark job if needed.
      */
    private static CompletableFuture<Boolean> runValidatorAsync(
            SparkPreCommitValidator validator,
            HoodieWriteMetadata<HoodieData<WriteStatus>> writeMetadata,
            Dataset<Row> beforeState,
            Dataset<Row> afterState,
            String instantTime) {
    ...
    /**
      * Get records from partitions modified as a dataset. Note that this only works for COW tables.
      *
      ...
      */
    public static Dataset<Row> getRecordsFromCommittedFiles(
            SQLContext sqlContext,
            Set<String> partitionsAffected,
            HoodieTable table,
            StructType newStructTypeSchema) {
        List<String> committedFiles =
                partitionsAffected.stream()
                        .flatMap(
                                partition ->
                                        table
                                                .getBaseFileOnlyView()
        ...
        if (committedFiles.isEmpty()) {
            try {
                ...
                LOG.warn(
                        "Use the schema from after state (current transaction) to create the empty Spark "
                                + "dataframe: "
                                + newStructTypeSchema);
    ...
    public static Dataset<Row> readRecordsForBaseFiles(
            SQLContext sqlContext, List<String> baseFilePaths) {
        return sqlContext
                .read()
                .parquet(JavaScalaConverters.<String>convertJavaListToScalaSeq(baseFilePaths));
    }

    /**
      * Get reads from partitions modified including any inflight commits. Note that this only works
      * for COW tables
      */
    public static Dataset<Row> getRecordsFromPendingCommits(
            SQLContext sqlContext,
            Set<String> partitionsAffected,
            HoodieWriteMetadata<HoodieData<WriteStatus>> writeMetadata,
            HoodieTable table,
        ...
        HoodieTablePreCommitFileSystemView fsView =
                new HoodieTablePreCommitFileSystemView(
                        table.getMetaClient(),
                        table.getHoodieView(),
