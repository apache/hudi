...
/**
  * Hudi internal implementation of the {@link InternalRow} allowing to extend arbitrary {@link
  * InternalRow} overlaying Hudi-internal meta-fields on top of it.
  *
  * <p>Capable of overlaying meta-fields in both cases: whether original {@link #sourceRow} contains
  * meta columns or not. This allows to handle following use-cases allowing to avoid any manipulation
  * (reshuffling) of the source row, by simply creating new instance of {@link HoodieInternalRow}
  * with all the meta-values provided
  *
  * <ul>
  *   <li>When meta-fields need to be prepended to the source {@link InternalRow}
  *   <li>When meta-fields need to be updated w/in the source {@link InternalRow} ({@link
  *       org.apache.spark.sql.catalyst.expressions.UnsafeRow} currently does not allow in-place
  *       updates due to its memory layout)
  * </ul>
  */
public class HoodieInternalRow extends InternalRow {
    ...
    /**
      ...
      *
      * <p>NOTE: {@code HoodieInternalRow} *always* overlays its own meta-fields even in case when
      * source row also contains them, to make sure these fields are mutable and can be updated (for
      * ex, {@link UnsafeRow} doesn't support mutations due to its memory layout, as it persists field
      * offsets)
      */
    ...
    public HoodieInternalRow(
            UTF8String commitTime,
        ...
        this.metaFields =
                new UTF8String[] {commitTime, commitSeqNumber, recordKey, partitionPath, fileName};
    ...
    public HoodieInternalRow(
            UTF8String[] metaFields, InternalRow sourceRow, boolean sourceContainsMetaFields) {
        ...
        if (ordinal < metaFields.length) {
            if (value instanceof UTF8String) {
                ...
                throw new IllegalArgumentException(
                        String.format(
                                "Could not update the row at (%d) with value of type (%s), either UTF8String or String are expected",
                                ordinal, value.getClass().getSimpleName()));
            }
        } else {
    ...
    private void validateMetaFieldDataType(DataType dataType) {
        if (!dataType.sameType(StringType$.MODULE$)) {
            throw new ClassCastException(
                    String.format(
                            "Can not cast meta-field of type UTF8String to %s", dataType.simpleString()));
        }
    }
    ...
    private void ruleOutMetaFieldsAccess(int ordinal, Class<?> expectedDataType) {
        if (ordinal < metaFields.length) {
            throw new ClassCastException(
                    String.format(
                            "Can not cast meta-field of type UTF8String at (%d) as %s",
                            ordinal, expectedDataType.getName()));
        }
    }
}
