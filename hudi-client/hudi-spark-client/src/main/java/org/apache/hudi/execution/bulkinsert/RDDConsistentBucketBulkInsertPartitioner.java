...
public class RDDConsistentBucketBulkInsertPartitioner<T> extends RDDBucketIndexPartitioner<T>
        implements ConsistentHashingBucketInsertPartitioner {

    private final Map<String, List<ConsistentHashingNode>> hashingChildrenNodes;

    public RDDConsistentBucketBulkInsertPartitioner(
            HoodieTable table, Map<String, String> strategyParams, boolean preserveHoodieMetadata) {
        super(
                table,
    ...
    public RDDConsistentBucketBulkInsertPartitioner(HoodieTable table) {
        ...
        ValidationUtils.checkArgument(
                table.getIndex() instanceof HoodieSparkConsistentBucketIndex,
                "RDDConsistentBucketPartitioner can only be used together with consistent hashing bucket index");
    }

    /**
      * Repartition the records to conform the bucket index storage layout constraints. Specifically,
      * partition the records based on consistent bucket index, which is computed using hashing
      * metadata and records' key.
      *
      * @param records Input Hoodie records
      * @param outputSparkPartitions Not used, the actual parallelism is determined by the bucket
      *     number
    ...
    public JavaRDD<HoodieRecord<T>> repartitionRecords(
        ...
        Map<String, Map<String, Integer>> partitionToFileIdPfxIdxMap =
                generateFileIdPfx(partitionToIdentifier);
        return doPartition(
                records,
                new Partitioner() {
                    @Override
    ...
    public void addHashingChildrenNodes(String partition, List<ConsistentHashingNode> nodes) {
        ValidationUtils.checkState(
                nodes.stream().noneMatch(n -> n.getTag() == ConsistentHashingNode.NodeTag.NORMAL),
    ...
    private ConsistentBucketIdentifier getBucketIdentifier(String partition) {
        ...
        HoodieConsistentHashingMetadata metadata =
                ConsistentBucketIndexUtils.loadOrCreateMetadata(
                        this.table, partition, index.getNumBuckets());
    ...
    /**
      * Initialize hashing metadata of input records. The metadata of all related partitions will be
      * loaded, and the mapping from partition to its bucket identifier is constructed.
      */
    private Map<String, ConsistentBucketIdentifier> initializeBucketIdentifier(
    ...
    /**
      ...
      * idx) mapping - doAppend: represents if the Nth data partition should use AppendHandler
      *
      * @param partitionToIdentifier Mapping from table partition to bucket identifier
      */
    private Map<String, Map<String, Integer>> generateFileIdPfx(
            Map<String, ConsistentBucketIdentifier> partitionToIdentifier) {
        Map<String, Map<String, Integer>> partitionToFileIdPfxIdxMap =
        ...
        for (ConsistentBucketIdentifier identifier : partitionToIdentifier.values()) {
            fileIdPfxList.addAll(
                    identifier.getNodes().stream()
            ...
            if (identifier.getMetadata().isFirstCreated()) {
                // Create new file group when the hashing metadata is new (i.e., first write to the
                // partition)
                ...
                doAppend.addAll(
                        identifier.getNodes().stream()
                                .map(n -> n.getTag() == ConsistentHashingNode.NodeTag.NORMAL)
                                .collect(Collectors.toList()));
            }
        ...
        ValidationUtils.checkState(
                fileIdPfxList.size()
                        == partitionToIdentifier.values().stream()
                                .mapToInt(ConsistentBucketIdentifier::getNumBuckets)
                                .sum(),
                        ...
                        writeHandleFactory ->
                                new WriteHandleFactory() {
                                    ...
                                    public HoodieWriteHandle create(
                                        ...
                                        // Ensure we do not create append handle for consistent
                                        // hashing bulk_insert, align with
