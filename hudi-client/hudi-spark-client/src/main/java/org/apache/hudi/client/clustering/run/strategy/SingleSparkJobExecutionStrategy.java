...
/**
  * Clustering strategy to submit single spark jobs. MultipleSparkJobExecution strategy is not ideal
  * for use cases that require large number of clustering groups
  */
public abstract class SingleSparkJobExecutionStrategy<T>
    ...
    public SingleSparkJobExecutionStrategy(
    ...
    public HoodieWriteMetadata<HoodieData<WriteStatus>> performClustering(
        ...
        JavaRDD<WriteStatus> writeStatusRDD =
                groupInfoJavaRDD.mapPartitions(
                        clusteringOps -> {
                            ...
                            List<ClusteringGroupInfo> groupsInPartition =
                                    StreamSupport.stream(clusteringOpsIterable.spliterator(), false)
                                            .collect(Collectors.toList());
                            return groupsInPartition.stream()
                                    .flatMap(
                                            clusteringOp ->
                                                    runClusteringForGroup(
                                                            clusteringOp,
                                                            clusteringPlan.getStrategy().getStrategyParams(),
                                                            Option.ofNullable(clusteringPlan.getPreserveHoodieMetadata())
                                                                    .orElse(false),
                                                            serializableSchema,
                                                            taskContextSupplier,
                                                            instantTime))
    ...
    private Stream<WriteStatus> runClusteringForGroup(
        ...
        List<HoodieFileGroupId> inputFileIds =
                clusteringOps.getOperations().stream()
                        .map(op -> new HoodieFileGroupId(op.getPartitionPath(), op.getFileId()))
                        .collect(Collectors.toList());

        Iterator<HoodieRecord<T>> inputRecords =
                readRecordsForGroupBaseFiles(clusteringOps.getOperations());
        Iterator<List<WriteStatus>> writeStatuses =
                performClusteringWithRecordsIterator(
                        inputRecords,
                        clusteringOps.getNumOutputGroups(),
                        instantTime,
                        strategyParams,
                        schema.get(),
                        inputFileIds,
    ...
    /**
      ...
      */
    public abstract Iterator<List<WriteStatus>> performClusteringWithRecordsIterator(
            final Iterator<HoodieRecord<T>> records,
    ...
    private Iterator<HoodieRecord<T>> readRecordsForGroupBaseFiles(
            List<ClusteringOperation> clusteringOps) {
        List<Iterator<HoodieRecord<T>>> iteratorsForPartition =
                                            ...
                                            () -> {
                                                try {
                                                    HoodieFileReader baseFileReader =
                                                            getHoodieSparkIOFactory(getHoodieTable().getStorage())
                                                                    .getReaderFactory(recordType)
                                                                    .getFileReader(
                                                                            writeConfig, new StoragePath(clusteringOp.getDataFilePath()));
