...
/**
  * Implementation of the function probing filtered in candidate keys provided in {@link
  * HoodieBloomFilterProbingResult} w/in corresponding files identified by {@link HoodieFileGroupId}
...
public class HoodieFileProbingFunction
    ...
    public Iterator<List<HoodieKeyLookupResult>> call(
            Iterator<Tuple2<HoodieFileGroupId, HoodieBloomFilterProbingResult>> tuple2Iterator)
            throws Exception {
    ...
    private class BloomIndexLazyKeyCheckIterator
            extends LazyIterableIterator<
                    Tuple2<HoodieFileGroupId, HoodieBloomFilterProbingResult>, List<HoodieKeyLookupResult>> {
        ...
        protected List<HoodieKeyLookupResult> computeNext() {
            ...
            final Map<Pair<String, HoodieBaseFile>, HoodieBloomFilterProbingResult> fileToLookupResults =
                    new HashMap<>();
            ...
            while (inputItr.hasNext()) {
                ...
                fileToLookupResults.putIfAbsent(
                        Pair.of(partitionPath, fileIDBaseFileMap.get(fileId)), entry._2);
                            ...
                            entry -> {
                                ...
                                List<Pair<String, Long>> matchingKeysAndPositions =
                                        HoodieIndexUtils.filterKeysFromFile(
                                                dataFile.getStoragePath(),
                                                candidateRecordKeys,
                                                HoodieStorageUtils.getStorage(dataFile.getStoragePath(), storageConf));

                                LOG.debug(
                                        String.format(
                                                "Bloom filter candidates (%d) / false positives (%d), actual matches (%d)",
                                                candidateRecordKeys.size(),
                                                candidateRecordKeys.size() - matchingKeysAndPositions.size(),
                                                matchingKeysAndPositions.size()));

                                return new HoodieKeyLookupResult(
                                        fileId, partitionPath, dataFile.getCommitTime(), matchingKeysAndPositions);
                            })
