...
public class SparkReleaseResources {
    ...
    /**
      * Called after each write commit, compaction commit and clustering commit to unpersist all RDDs
      * persisted or cached per table.
      *
    ...
    public static void releaseCachedData(
            HoodieEngineContext context, HoodieWriteConfig config, String basePath, String instantTime) {
        // If we do not explicitly release the resource, spark will automatically manage the
        // resource and clean it up automatically
        ...
        if (config.areReleaseResourceEnabled()) {
            ...
            List<Integer> allDataIds =
                    new ArrayList<>(
                            sparkEngineContext.removeCachedDataIds(
                                    HoodieData.HoodieDataCacheKey.of(basePath, instantTime)));
            if (config.isMetadataTableEnabled()) {
                String metadataTableBasePath = HoodieTableMetadata.getMetadataTableBasePath(basePath);
                allDataIds.addAll(
                        sparkEngineContext.removeCachedDataIds(
                                HoodieData.HoodieDataCacheKey.of(metadataTableBasePath, instantTime)));
            }
