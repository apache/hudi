...
/**
  * Clustering Strategy based on following. 1) Spark execution engine. 2) Uses bulk_insert to write
  * data into new files.
  */
public class SparkSortAndSizeExecutionStrategy<T> extends MultipleSparkJobExecutionStrategy<T> {
    private static final Logger LOG =
            LoggerFactory.getLogger(SparkSortAndSizeExecutionStrategy.class);

    public SparkSortAndSizeExecutionStrategy(
    ...
    public HoodieData<WriteStatus> performClusteringWithRecordsRDD(
            final HoodieData<HoodieRecord<T>> inputRecords,
        ...
        return (HoodieData<WriteStatus>)
                SparkBulkInsertHelper.newInstance()
                        .bulkInsert(
                                ...
                                true,
                                numOutputGroups,
                                new CreateHandleFactory(shouldPreserveHoodieMetadata));
    }
}
