...
/**
  * Validator to run sql query and compare table state 1) before new commit started. 2) current
  * inflight commit (if successful).
  *
  * <p>Expects query results do not match.
  */
public class SqlQueryInequalityPreCommitValidator<T, I, K, O extends HoodieData<WriteStatus>>
    ...
    private static final Logger LOG =
            LoggerFactory.getLogger(SqlQueryInequalityPreCommitValidator.class);

    public SqlQueryInequalityPreCommitValidator(
            HoodieSparkTable<T> table, HoodieEngineContext engineContext, HoodieWriteConfig config) {
    ...
    protected void validateUsingQuery(
            String query, String prevTableSnapshot, String newTableSnapshot, SQLContext sqlContext) {
        Dataset<Row> prevRows =
                executeSqlQuery(sqlContext, query, prevTableSnapshot, "previous state").cache();
        LOG.info("Total rows in prevRows " + prevRows.count());
        Dataset<Row> newRows =
                executeSqlQuery(sqlContext, query, newTableSnapshot, "new state").cache();
        ...
        if (areDatasetsEqual) {
            ...
            System.out.println(
                    "Expected query results to be different, but they are same. Result (sample records only):");
            prevRows.show();
