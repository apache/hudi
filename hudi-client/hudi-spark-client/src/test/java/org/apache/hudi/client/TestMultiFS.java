...
public class TestMultiFS extends HoodieSparkClientTestHarness {
    ...
    public void readLocalWriteHDFS() throws Exception {
        ...
        HoodieTableMetaClient.withPropertyBuilder()
                ...
                .setPayloadClass(HoodieAvroPayload.class)
                .setRecordKeyFields(
                        localConfig.getProps().getProperty(KeyGeneratorOptions.RECORDKEY_FIELD_NAME.key()))
                .setPartitionFields(
                        localConfig.getProps().getProperty(KeyGeneratorOptions.PARTITIONPATH_FIELD_NAME.key()))
        ...
        try (SparkRDDWriteClient hdfsWriteClient = getHoodieWriteClient(cfg);
            ...
            HoodieTableMetaClient metaClient =
                    HoodieTestUtils.createMetaClient(HadoopFSUtils.getStorageConf(fs.getConf()), dfsBasePath);
            HoodieTimeline timeline = new HoodieActiveTimeline(metaClient).getCommitAndReplaceTimeline();
            Dataset<Row> readRecords =
                    HoodieClientTestUtils.readCommit(dfsBasePath, sqlContext, timeline, readCommitTime);
            ...
            metaClient =
                    HoodieTestUtils.createMetaClient(new HadoopStorageConfiguration(fs.getConf()), tablePath);
