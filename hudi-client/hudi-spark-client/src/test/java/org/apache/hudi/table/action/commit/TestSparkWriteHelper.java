...
public class TestSparkWriteHelper extends TestWriterHelperBase<HoodieData<HoodieRecord>> {
    ...
    @Override
    public HoodieData<HoodieRecord> getInputRecords(
            List<HoodieRecord> recordList, int numPartitions) {
    ...
    public void testCombineParallelism(boolean shouldCombine, int configuredShuffleParallelism) {
        ...
        inputRecords =
                getInputRecords(dataGen.generateInserts("20230915000000000", 10), inputParallelism);
        HoodieData<HoodieRecord> outputRecords =
