...
public class TestRDDSimpleBucketBulkInsertPartitioner extends HoodieSparkClientTestHarness {
    ...
    public void testSimpleBucketPartitioner(String tableType, boolean partitionSort)
            throws IOException {
        HoodieTestUtils.init(
                HoodieTestUtils.getDefaultStorageConf(), basePath, HoodieTableType.valueOf(tableType));
        int bucketNum = 10;
        ...
        config.setValue(
                HoodieIndexConfig.BUCKET_INDEX_ENGINE_TYPE,
        ...
        if (partitionSort) {
            config.setValue(
                    HoodieWriteConfig.BULK_INSERT_SORT_MODE, BulkInsertSortMode.PARTITION_SORT.name());
        }
        ...
        assertEquals(
                bucketNum * javaRDD.map(HoodieRecord::getPartitionPath).distinct().count(),
        ...
        List<WriteStatus> writeStatues =
                getHoodieWriteClient(config).bulkInsert(HoodieJavaRDD.getJavaRDD(javaRDD), "0").collect();
        ...
        List<WriteStatus> writeStatues2 =
                getHoodieWriteClient(config).bulkInsert(HoodieJavaRDD.getJavaRDD(javaRDD), "1").collect();
        writeStatues2.forEach(
                ws ->
                        assertEquals(
                                ws.getTotalRecords(), writeStatuesMap.get(ws.getFileId()).getTotalRecords()));
    }
