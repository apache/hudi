...
public class TestUpdateSchemaEvolution extends HoodieSparkClientTestHarness
    ...
    private WriteStatus prepareFirstRecordCommit(List<String> recordsStrs) throws IOException {
        ...
        final List<WriteStatus> statuses =
                jsc.parallelize(Arrays.asList(1))
                        .map(
                                x -> {
                                    ...
                                    Map<String, HoodieRecord> insertRecordMap =
                                            insertRecords.stream()
                                                    .collect(Collectors.toMap(r -> r.getRecordKey(), Function.identity()));
                                    HoodieWriteHandle<?, ?, ?, ?> createHandle =
                                            new CreateHandleFactory<>(false)
                                                    .create(
                                                            config,
                                                            "100",
                                                            table,
                                                            insertRecords.get(0).getPartitionPath(),
                                                            "f1-0",
                                                            supplier);
                                    ...
                                    () -> {
                                        HoodieMergeHandle mergeHandle =
                                                new HoodieMergeHandle(
                                                        updateTable.getConfig(),
                                                        "101",
                                                        updateTable,
                                                        updateRecords.iterator(),
                                                        updateRecords.get(0).getPartitionPath(),
                                        ...
                                        List<GenericRecord> oldRecords =
                                                HoodieIOFactory.getIOFactory(updateTable.getStorage())
                                                        ...
                                                        .readAvroRecords(
                                                                updateTable.getStorage(),
                                                                new StoragePath(
                                                                        updateTable.getConfig().getBasePath()
                                                                                + "/"
    ...
    private List<HoodieRecord> buildUpdateRecords(String recordStr, String insertFileId)
            throws IOException {
    ...
    public void testSchemaEvolutionOnUpdateSuccessWithAddColumnHaveDefault() throws Exception {
        ...
        String assertMsg =
                "UpdateFunction could not read records written with exampleSchema.avsc using the "
    ...
    public void testSchemaEvolutionOnUpdateSuccessWithChangeColumnOrder() throws Exception {
        ...
        final HoodieWriteConfig config =
                makeHoodieClientConfig("/exampleEvolvedSchemaChangeOrder.avsc");
        final HoodieSparkTable table = HoodieSparkTable.create(config, context);
        String recordStr =
                "{\"_row_key\":\"8eb5b87a-1feh-4edd-87b4-6ec96dc405a0\","
    ...
    public void testSchemaEvolutionOnUpdateMisMatchWithDeleteColumn() throws Exception {
        ...
        // Evolved schema does not have guarantee on preserving the original field ordering
        final HoodieWriteConfig config =
                makeHoodieClientConfig("/exampleEvolvedSchemaDeleteColumn.avsc");
        final HoodieSparkTable table = HoodieSparkTable.create(config, context);
        String recordStr =
                "{\"_row_key\":\"8eb5b87a-1feh-4edd-87b4-6ec96dc405a0\","
        ...
        assertSchemaEvolutionOnUpdateResult(
                insertResult, table, updateRecords, assertMsg, true, InvalidRecordException.class);
    }
    ...
    public void testSchemaEvolutionOnUpdateMisMatchWithAddColumnNotHaveDefault() throws Exception {
        ...
        // Evolved schema does not have guarantee on preserving the original field ordering
        final HoodieWriteConfig config =
                makeHoodieClientConfig("/exampleEvolvedSchemaColumnRequire.avsc");
        final HoodieSparkTable table = HoodieSparkTable.create(config, context);
        String recordStr =
                "{\"_row_key\":\"8eb5b87a-1feh-4edd-87b4-6ec96dc405a0\","
                        + "\"time\":\"2016-01-31T03:16:41.415Z\",\"number\":12,\"added_field\":1}";
        ...
        assertSchemaEvolutionOnUpdateResult(
                insertResult, table, updateRecords, assertMsg, true, HoodieUpsertException.class);
    }
    ...
    public void testSchemaEvolutionOnUpdateMisMatchWithChangeColumnType() throws Exception {
        ...
        assertSchemaEvolutionOnUpdateResult(
                insertResult, table, updateRecords, assertMsg, true, ParquetDecodingException.class);
    }
