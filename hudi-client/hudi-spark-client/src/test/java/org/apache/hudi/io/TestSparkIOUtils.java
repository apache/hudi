...
public class TestSparkIOUtils {
    ...
    public void testMaxMemoryPerPartitionMergeWithMaxSizeDefined() {
        ...
        HoodieMemoryConfig memoryConfig =
                HoodieMemoryConfig.newBuilder()
                        .withMaxMemoryMaxSize(mergeMaxSize, compactionMaxSize)
                        .build();
        HoodieWriteConfig config =
                HoodieWriteConfig.newBuilder().withPath(path).withMemoryConfig(memoryConfig).build();
    ...
    public void testMaxMemoryPerPartitionMergeInDefault() {
        ...
        String compactionFraction =
                config
                        .getProps()
                        .getProperty(
                                MAX_MEMORY_FRACTION_FOR_COMPACTION.key(),
                                MAX_MEMORY_FRACTION_FOR_COMPACTION.defaultValue());
        long compactionMaxSize =
                IOUtils.getMaxMemoryAllowedForMerge(contextSupplier, compactionFraction);

        String mergeFraction =
                config
                        .getProps()
                        .getProperty(
                                MAX_MEMORY_FRACTION_FOR_MERGE.key(), MAX_MEMORY_FRACTION_FOR_MERGE.defaultValue());
