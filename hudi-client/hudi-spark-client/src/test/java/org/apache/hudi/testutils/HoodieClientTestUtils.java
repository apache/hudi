...
public class HoodieClientTestUtils {
    ...
    /**
      ...
      *
      * <p>The following properties may be set to customize the Spark context: SPARK_EVLOG_DIR: Local
      * directory where event logs should be saved. This allows viewing the logs with
      * spark-history-server.
      *
      * @note When running the tests using maven, use the following syntax to set a property: mvn
      *     -DSPARK_XXX=yyy ...
    ...
    public static Dataset<Row> readCommit(
            String basePath, SQLContext sqlContext, HoodieTimeline commitTimeline, String instantTime) {
    ...
    public static Dataset<Row> readCommit(
        ...
        try {
            ...
            if (HoodieTableConfig.BASE_FILE_FORMAT.defaultValue().equals(HoodieFileFormat.PARQUET)) {
                unFilteredRows =
                        sqlContext.read().parquet(paths.values().toArray(new String[paths.size()]));
            ...
            if (unFilteredRows != null) {
                if (filterByCommitTime) {
                    return unFilteredRows.filter(
                            String.format("%s ='%s'", HoodieRecord.COMMIT_TIME_METADATA_FIELD, instantTime));
                } else {
    ...
    public static long countRecordsOptionallySince(
            JavaSparkContext jsc,
            String basePath,
            SQLContext sqlContext,
            HoodieTimeline commitTimeline,
            Option<String> lastCommitTimeOpt) {
        ...
        try {
            ...
            if (paths[0].endsWith(HoodieFileFormat.PARQUET.getFileExtension())) {
                ...
                if (lastCommitTimeOpt.isPresent()) {
                    return rows.filter(
                                    String.format(
                                            "%s >'%s'", HoodieRecord.COMMIT_TIME_METADATA_FIELD, lastCommitTimeOpt.get()))
                            .count();
                } else {
                    return rows.count();
                }
    ...
    public static List<HoodieBaseFile> getLatestBaseFiles(
        ...
        try {
            ...
            for (String path : paths) {
                BaseFileOnlyView fileSystemView =
            ...
            TimelineService timelineService =
                    new TimelineService(
                            ...
                            FileSystemViewManager.createViewManager(
                                    context, config.getViewStorageConfig(), config.getCommonConfig()));
    ...
    public static HoodieTableMetaClient createMetaClient(JavaSparkContext jsc, String basePath) {
        return HoodieTestUtils.createMetaClient(
                new HadoopStorageConfiguration(jsc.hadoopConfiguration()), basePath);
    }
    ...
    public static HoodieTableMetaClient createMetaClient(SparkSession spark, String basePath) {
        return HoodieTestUtils.createMetaClient(
                new HadoopStorageConfiguration(spark.sessionState().newHadoopConf()), basePath);
    }

    private static Option<HoodieCommitMetadata> getCommitMetadataForInstant(
