 ...
 */
public class TestCleanActionExecutor extends SparkClientFunctionalTestHarness {
  ...
  void setUp() {
    ...
    fs = mock(HoodieWrapperFileSystem.class);
    when(metaClient.getFs()).thenReturn(fs);
    when(fs.getConf()).thenReturn(jsc().hadoopConfiguration());
    context = new HoodieLocalEngineContext(jsc().hadoopConfiguration());
  }
  ...
  void testPartialCleanFailure(CleanFailureType failureType) throws IOException {
    ...
    FileSystem localFs = new Path(baseFile.getPath()).getFileSystem(jsc().hadoopConfiguration());
