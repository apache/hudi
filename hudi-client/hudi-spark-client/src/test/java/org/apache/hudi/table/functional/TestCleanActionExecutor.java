...
public class TestCleanActionExecutor {
    ...
    void testPartialCleanFailure(CleanFailureType failureType) throws IOException {
        ...
        HoodieBaseFile baseFile =
                new HoodieBaseFile(String.format("/tmp/base/%s_1-0-1_%s.parquet", fileGroup, "001"));
        ...
        if (failureType == CleanFailureType.TRUE_ON_DELETE) {
            ...
            when(fs.delete(filePath, false))
                    .thenThrow(new FileNotFoundException("throwing file not found exception"));
        } else {
            // run time exception
            when(fs.delete(filePath, false))
                    .thenThrow(new RuntimeException("throwing run time exception"));
        }

        Map<String, List<HoodieCleanFileInfo>> partitionCleanFileInfoMap = new HashMap<>();
        List<HoodieCleanFileInfo> cleanFileInfos =
                Collections.singletonList(new HoodieCleanFileInfo(baseFile.getPath(), false));
        ...
        HoodieInstant cleanInstant =
                new HoodieInstant(HoodieInstant.State.REQUESTED, HoodieTimeline.CLEAN_ACTION, "002");
        ...
        when(activeTimeline.getInstantDetails(cleanInstant))
                .thenReturn(TimelineMetadataUtils.serializeCleanerPlan(cleanerPlan));
        when(activeTimeline.readCleanerInfoAsBytes(cleanInstant))
                .thenReturn(TimelineMetadataUtils.serializeCleanerPlan(cleanerPlan));
        ...
        when(inflightsAndRequestedTimeline.getInstants())
                .thenReturn(Collections.singletonList(cleanInstant));
        when(activeTimeline.transitionCleanRequestedToInflight(any(), any()))
                .thenReturn(
                        new HoodieInstant(HoodieInstant.State.INFLIGHT, HoodieTimeline.CLEAN_ACTION, "002"));
        when(mockHoodieTable.getMetadataWriter("002")).thenReturn(Option.empty());

        CleanActionExecutor cleanActionExecutor =
                new CleanActionExecutor(context, config, mockHoodieTable, "002");
        if (failureType == CleanFailureType.TRUE_ON_DELETE) {
