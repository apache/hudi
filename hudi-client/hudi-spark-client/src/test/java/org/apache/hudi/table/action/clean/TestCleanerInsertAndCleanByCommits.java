...
public class TestCleanerInsertAndCleanByCommits extends SparkClientFunctionalTestHarness {
    ...
    private static final Logger LOG =
            LoggerFactory.getLogger(TestCleanerInsertAndCleanByCommits.class);
    ...
    public void testInsertAndCleanByCommits(boolean isAsync) throws Exception {
        testInsertAndCleanByCommits(
                SparkRDDWriteClient::insert, SparkRDDWriteClient::upsert, false, isAsync);
    }
    ...
    public void testBulkInsertAndCleanByCommits() throws Exception {
        testInsertAndCleanByCommits(
                SparkRDDWriteClient::bulkInsert, SparkRDDWriteClient::upsert, false, false);
    }
    ...
    private void testInsertAndCleanByCommits(
            ...
            Function3<JavaRDD<WriteStatus>, SparkRDDWriteClient, JavaRDD<HoodieRecord>, String> upsertFn,
        ...
        try (final SparkRDDWriteClient client = getHoodieWriteClient(cfg)) {
            ...
            final Function2<List<HoodieRecord>, String, Integer> recordInsertGenWrappedFunction =
                    isPreppedAPI
                            ? wrapRecordsGenFunctionForPreppedCalls(
                                    basePath(), storageConf(), context(), cfg, dataGen::generateInserts)
            ...
            for (int i = 0; i < 8; i++) {
                ...
                List<WriteStatus> statuses =
                        upsertFn
                                .apply(client, jsc().parallelize(records, PARALLELISM), newCommitTime)
                                .collect();
    ...
    private void validateFilesAfterCleaning(
        ...
        // This corresponds to the `earliestCommitToRetain` in {@code
        // CleanPlanner::getFilesToCleanKeepingLatestCommits}
        Option<HoodieInstant> earliestRetainedCommit =
                commitsTimeline.nthFromLastInstant(maxCommitsToRetain - 1);
        ...
        for (HoodieInstant instant :
                commitsTimeline.getReverseOrderedInstants().collect(Collectors.toList())) {
            List<HoodieWriteStat> hoodieWriteStatList =
                    commitWriteStatsMap.computeIfAbsent(
                                ...
                                try {
                                    return HoodieCommitMetadata.fromBytes(
                                                    timeline
                                                            .getInstantDetails(
                                                                    timeline
                                                                            .filter(inst -> inst.getTimestamp().equals(newInstant))
                                                                            .firstInstant()
                                                                            .get())
                                                            .get(),
