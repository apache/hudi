...
class TestRangeSampleSort extends HoodieClientTestBase {
    ...
    void sortDataFrameBySampleSupportAllTypes() {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            Assertions.assertDoesNotThrow(
                    () ->
                            RangeSampleSort$.MODULE$.sortDataFrameBySampleSupportAllTypes(
                                    df.limit(limit),
                                    JavaScalaConverters.convertJavaListToScalaSeq(Arrays.asList("id", "content")),
                                    1),
    ...
    void sortDataFrameBySample() {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            Assertions.assertDoesNotThrow(
                    () ->
                            RangeSampleSort$.MODULE$.sortDataFrameBySample(
                                    df.limit(limit),
                                    layoutOptStrategy,
                                    JavaScalaConverters.convertJavaListToScalaSeq(Arrays.asList("id", "content")),
                                    1),
