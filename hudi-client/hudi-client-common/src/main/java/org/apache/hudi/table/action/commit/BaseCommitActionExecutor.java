...
public abstract class BaseCommitActionExecutor<T, I, K, O, R>
    ...
    public BaseCommitActionExecutor(
        ...
        // TODO : Remove this once we refactor and move out autoCommit method from here, since the
        // TxnManager is held in {@link BaseHoodieWriteClient}.
        this.txnManagerOption =
                config.shouldAutoCommit()
        ...
        if (this.txnManagerOption.isPresent() && this.txnManagerOption.get().isLockRequired()) {
            // these txn metadata are only needed for auto commit when optimistic concurrent control
            // is also enabled
    ...
    /**
      ...
      * when performing rollback for MOR tables. Only updates are recorded in the workload profile
            ...
            profile
                    ...
                    .forEach(
                            path -> {
                                ...
                                Map<String, Pair<String, Long>> insertLocationMap =
                                        partitionStat.getInsertLocationToCount();
                                Stream.concat(
                                                updateLocationMap.keySet().stream(), insertLocationMap.keySet().stream())
                                        .distinct()
                                        .forEach(
                                                fileId -> {
                                                    ...
                                                    writeStat.setPrevCommit(
                                                            updateLocation != null
                                                                    ? updateLocation.getKey()
                                                                    : insertLocation.getKey());
    ...
    /**
      * Check if any validators are configured and run those validations. If any of the validations
      * fail, throws HoodieValidationException.
      */
    ...
    protected void autoCommit(HoodieWriteMetadata<O> result) {
        final Option<HoodieInstant> inflightInstant =
                Option.of(new HoodieInstant(State.INFLIGHT, getCommitActionType(), instantTime));
        ValidationUtils.checkState(
                this.txnManagerOption.isPresent(), "The transaction manager has not been initialized");
        TransactionManager txnManager = this.txnManagerOption.get();
        txnManager.beginTransaction(
                inflightInstant,
                lastCompletedTxn.isPresent()
                        ? Option.of(lastCompletedTxn.get().getLeft())
                        : Option.empty());
        try {
            setCommitMetadata(result);
            // reload active timeline so as to get all updates after current transaction have
            // started. hence setting last arg to true.
    ...
    protected void finalizeWrite(
            String instantTime, List<HoodieWriteStat> stats, HoodieWriteMetadata<O> result) {
        try {
    ...
    protected abstract Iterator<List<WriteStatus>> handleInsert(
            String idPfx, Iterator<HoodieRecord<T>> recordItr) throws Exception;

    protected abstract Iterator<List<WriteStatus>> handleUpdate(
            String partitionPath, String fileId, Iterator<HoodieRecord<T>> recordItr) throws IOException;

    protected HoodieWriteMetadata<HoodieData<WriteStatus>> executeClustering(
            HoodieClusteringPlan clusteringPlan) {
        context.setJobStatus(
                this.getClass().getSimpleName(), "Clustering records for " + config.getTableName());
        ...
        HoodieWriteMetadata<HoodieData<WriteStatus>> writeMetadata =
                ((ClusteringExecutionStrategy<
                                        T, HoodieData<HoodieRecord<T>>, HoodieData<HoodieKey>, HoodieData<WriteStatus>>)
                                ReflectionUtils.loadClass(
                                        config.getClusteringExecutionStrategyClass(),
                                        new Class<?>[] {
                                            HoodieTable.class, HoodieEngineContext.class, HoodieWriteConfig.class
                                        },
                                        table,
                                        context,
                                        config))
        ...
        statuses.persist(
                config.getString(WRITE_STATUS_STORAGE_LEVEL_VALUE),
                context,
                HoodieData.HoodieDataCacheKey.of(config.getBasePath(), instantTime));
        // triggers clustering.
        writeMetadata.setWriteStats(statuses.map(WriteStatus::getStat).collectAsList());
        writeMetadata.setPartitionToReplaceFileIds(
                getPartitionToReplacedFileIds(clusteringPlan, writeMetadata));
    ...
    private HoodieData<WriteStatus> updateIndex(
            HoodieData<WriteStatus> writeStatuses, HoodieWriteMetadata<HoodieData<WriteStatus>> result) {
    ...
    private Map<String, List<String>> getPartitionToReplacedFileIds(
            HoodieClusteringPlan clusteringPlan,
            HoodieWriteMetadata<HoodieData<WriteStatus>> writeMetadata) {
        ...
        return ClusteringUtils.getFileGroupsFromClusteringPlan(clusteringPlan)
                ...
                .collect(
                        Collectors.groupingBy(
                                HoodieFileGroupId::getPartitionPath,
                                Collectors.mapping(HoodieFileGroupId::getFileId, Collectors.toList())));
    }

    /**
      * Validate actions taken by clustering. In the first implementation, we validate at least one new
      * file is written. But we can extend this to add more validation. E.g. number of records read =
      * number of records written etc. We can also make these validations in BaseCommitActionExecutor
      * to reuse pre-commit hooks for multiple actions.
      */
    private void validateWriteResult(
        ...
        if (writeMetadata.getWriteStatuses().isEmpty()) {
            throw new HoodieClusteringException(
                    "Clustering plan produced 0 WriteStatus for "
                            + instantTime
                            + " #groups: "
                            + clusteringPlan.getInputGroups().size()
                            + " expected at least "
                            + clusteringPlan.getInputGroups().stream()
                                    .mapToInt(HoodieClusteringGroup::getNumOutputFileGroups)
                                    .sum()
                            + " write statuses");
        }
    }
