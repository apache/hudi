...
public abstract class BaseConsistentHashingBucketClusteringPlanStrategy<
                T extends HoodieRecordPayload, I, K, O>
    ...
    /**
      * TODO maybe add force config to schedule the clustering. It could allow clustering on partitions
      * that are not doing write operation. Block clustering if there is any ongoing concurrent writers
      *
    ...
    public boolean checkPrecondition() {
        ...
        if (!timeline.empty()) {
            LOG.warn(
                    "When using consistent bucket, clustering cannot be scheduled async if there are concurrent writers. "
                            + "Writer instant: {}.",
    ...
    protected Stream<HoodieClusteringGroup> buildClusteringGroupsForPartition(
        ...
        ValidationUtils.checkArgument(
                metadata.isPresent(), "Metadata is empty for partition: " + partitionPath);
        ...
        if (isBucketClusteringMergeEnabled()) {
            ...
            int mergeSlot =
                    identifier.getNumBuckets()
                            - getWriteConfig().getBucketIndexMinNumBuckets()
                            + splitResult.getMiddle();
    ...
    /**
      * Whether generate regular sort clustering plans for buckets that are not involved in merge or
      * split.
      *
      * @return true if generate regular sort clustering plans for buckets that are not involved in
      *     merge or split, false otherwise.
      */
    ...
    /**
      * Generate candidate clustering file slices of the given partition. If there is inflight /
      * requested clustering working on the partition, then return empty list to ensure serialized
      * update to the hashing metadata.
      *
    ...
    protected Stream<FileSlice> getFileSlicesEligibleForClustering(String partition) {
        ...
        boolean isPartitionInClustering =
                fileSystemView
                        ...
                        .anyMatch(p -> p.getLeft().getPartitionPath().equals(partition));
    ...
    /**
      * Generate clustering groups according to split rules. Currently, we always split bucket into two
      * sub-buckets.
      *
      * @param identifier bucket identifier
      * @param fileSlices file slice candidate to be built as split clustering groups
      * @param splitSlot number of new bucket allowed to produce, in order to constrain the upper bound
      *     of the total number of bucket
      * @return list of clustering group, number of new buckets generated, remaining file slice (that
      *     does not split)
      */
        ...
        for (FileSlice fs : fileSlices) {
            ...
            retGroup.add(
                    HoodieClusteringGroup.newBuilder()
                            ...
                            .setExtraMetadata(
                                    constructExtraMetadata(
                                            fs.getPartitionPath(), nodes.get(), identifier.getMetadata().getSeqNo()))
                            .build());
        }
    ...
    /**
      ...
      * @param mergeSlot number of bucket allowed to be merged, in order to guarantee the lower bound
      *     of the total number of bucket
      * @return list of clustering group, number of buckets merged (removed), remaining file slice
      *     (that does not be merged)
      */
    protected Triple<List<HoodieClusteringGroup>, Integer, List<FileSlice>> buildMergeClusteringGroup(
        ...
        fileSlices.sort(
                Comparator.comparingInt(a -> identifier.getBucketByFileId(a.getFileId()).getValue()));
        // In each round, we check if the ith file slice can be merged with its predecessors and
        // successors
        for (int i = 0; i < fileSlices.size(); ++i) {
            ...
            for (int k = 0; k < 2; ++k) {
                ...
                do {
                    int nextIdx =
                            forward
                                    ? (rangeIdx[k] + 1 < fileSlices.size() ? rangeIdx[k] + 1 : 0)
                                    : (rangeIdx[k] >= 1 ? rangeIdx[k] - 1 : fileSlices.size() - 1);
                    ConsistentHashingNode bucketOfNextFile =
                            identifier.getBucketByFileId(fileSlices.get(nextIdx).getFileId());
                    ConsistentHashingNode nextBucket =
                            forward
                    ...
                    /**
                      * Merge condition: 1. there is still slot to merge bucket 2. the previous file slices is
                      * not merged 3. the previous file slice and current file slice are neighbour in the hash
                      * ring 4. Both the total file size up to now and the previous file slice size are smaller
                      * than merge size threshold
                      */
                    if (remainingMergeSlot == 0
                            || added[nextIdx]
                            || !isNeighbour
                            || totalSize > mergeSize
                            || fileSlices.get(nextIdx).getTotalFileSize() > mergeSize
                    ...
                    HoodieClusteringGroup.newBuilder()
                            ...
                            .setExtraMetadata(
                                    constructExtraMetadata(
                                            ...
                                            identifier.mergeBucket(
                                                    fs.stream().map(FileSlice::getFileId).collect(Collectors.toList())),
                                            identifier.getMetadata().getSeqNo()))
                            .build());
        }

        // Collect file slices that are not involved in merge
        List<FileSlice> fsUntouched =
                IntStream.range(0, fileSlices.size())
                        .filter(i -> !added[i])
                        .mapToObj(fileSlices::get)
                        .collect(Collectors.toList());
    ...
    private Map<String, String> constructExtraMetadata(
            String partition, List<ConsistentHashingNode> nodes, int seqNo) {
        ...
        try {
            ...
            throw new HoodieClusteringException(
                    "Failed to construct extra metadata, partition: " + partition + ", nodes:" + nodes);
        }
    ...
    private long getSplitSize() {
        ...
        return (long)
                (getWriteConfig().getMaxFileSize(format) * getWriteConfig().getBucketSplitThreshold());
    }

    private long getMergeSize() {
        HoodieFileFormat format = getHoodieTable().getBaseFileFormat();
        return (long)
                (getWriteConfig().getMaxFileSize(format) * getWriteConfig().getBucketMergeThreshold());
    }
}
