...
public class ExecutorFactory {
    ...
    public static <I, O, E> HoodieExecutor<E> create(
    ...
    public static <I, O, E> HoodieExecutor<E> create(
        ...
        switch (executorType) {
            case BOUNDED_IN_MEMORY:
                return new BoundedInMemoryExecutor<>(
                        config.getWriteBufferLimitBytes(),
                        config.getWriteBufferRecordSamplingRate(),
                        config.getWriteBufferRecordCacheLimit(),
                        inputItr,
                        consumer,
                        transformFunction,
                        preExecuteRunnable);
            case DISRUPTOR:
                return new DisruptorExecutor<>(
                        config.getWriteExecutorDisruptorWriteBufferLimitBytes(),
                        inputItr,
                        consumer,
                        transformFunction,
                        config.getWriteExecutorDisruptorWaitStrategy(),
                        preExecuteRunnable);
            case SIMPLE:
    ...
    /**
      * Checks whether configured {@link HoodieExecutor} buffer records (for ex, by holding them in the
      * queue)
      */
