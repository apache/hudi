...
public class HoodieBloomIndex extends HoodieIndex<Object, Object> {
    ...
    public <R> HoodieData<HoodieRecord<R>> tagLocation(
        ...
        // Step 3: Tag the incoming records, as inserts or updates, by joining with existing record
        // keys
    ...
    private HoodiePairData<HoodieKey, HoodieRecordLocation> lookupIndex(
        ...
        List<Pair<String, BloomIndexFileInfo>> fileInfoList =
                getBloomIndexFileInfoForPartitions(context, hoodieTable, affectedPartitionPathList);
        final Map<String, List<BloomIndexFileInfo>> partitionToFileInfo =
                fileInfoList.stream().collect(groupingBy(Pair::getLeft, mapping(Pair::getRight, toList())));

        // Step 3: Obtain a HoodieData, for each incoming record, that already exists, with the file
        // id,
        // that contains it.
        ...
        return bloomIndexHelper.findMatchingFilesForRecordKeys(
                config,
                context,
                hoodieTable,
    ...
    private List<Pair<String, BloomIndexFileInfo>> getBloomIndexFileInfoForPartitions(
        ...
        if (config.getBloomIndexPruneByRanges()) {
            // load column ranges from metadata index if column stats index is enabled and
            // column_stats metadata partition is available
    ...
    List<Pair<String, BloomIndexFileInfo>> loadColumnRangesFromFiles(
        ...
        List<Pair<String, Pair<String, HoodieBaseFile>>> partitionPathFileIDList =
                getLatestBaseFilesForAllPartitions(partitions, context, hoodieTable).stream()
                        .map(
                                pair ->
                                        Pair.of(pair.getKey(), Pair.of(pair.getValue().getFileId(), pair.getValue())))
                        .collect(toList());

        context.setJobStatus(
                this.getClass().getName(),
                "Obtain key ranges for file slices (range pruning=on): " + config.getTableName());
        return context.map(
                partitionPathFileIDList,
                pf -> {
                    try {
                        HoodieRangeInfoHandle rangeInfoHandle =
                                new HoodieRangeInfoHandle(
                                        config, hoodieTable, Pair.of(pf.getKey(), pf.getValue().getKey()));
                        String[] minMaxKeys = rangeInfoHandle.getMinMaxKeys(pf.getValue().getValue());
                        return Pair.of(
                                pf.getKey(),
                                new BloomIndexFileInfo(pf.getValue().getKey(), minMaxKeys[0], minMaxKeys[1]));
    ...
    private List<Pair<String, BloomIndexFileInfo>> getFileInfoForLatestBaseFiles(
        ...
        return partitionPathFileIDList.stream()
                .map(pf -> Pair.of(pf.getKey(), new BloomIndexFileInfo(pf.getValue())))
                .collect(toList());
    }
    ...
    protected List<Pair<String, BloomIndexFileInfo>> loadColumnRangesFromMetaIndex(
        ...
        context.setJobStatus(
                this.getClass().getName(),
                "Load meta index key ranges for file slices: " + config.getTableName());
        ...
        List<Pair<String, String>> partitionFileNameList =
                new ArrayList<>(baseFilesForAllPartitions.size());
        Map<Pair<String, String>, String> partitionAndFileNameToFileId =
                new HashMap<>(baseFilesForAllPartitions.size(), 1);
        baseFilesForAllPartitions.forEach(
                pair -> {
                    Pair<String, String> partitionAndFileName =
                            Pair.of(pair.getKey(), pair.getValue().getFileName());
    ...
    /**
      ...
      * record's key needs to be checked. For tables, where the keys have a definite insert order (e.g:
      * timestamp as prefix), the number of files to be compared gets cut down a lot from range
      * pruning.
      *
      * <p>Sub-partition to ensure the records can be looked up against files & also prune
      * file<=>record comparisons based on recordKey ranges in the index info.
      */
    HoodiePairData<HoodieFileGroupId, String> explodeRecordsWithFileComparisons(
        ...
        return partitionRecordKeyPairs
                .map(
                        partitionRecordKeyPair -> {
    ...
    protected <R> HoodieData<HoodieRecord<R>> tagLocationBacktoRecords(
        ...
        // Here as the records might have more data than keyFilenamePairs (some row keys' fileId is
        // null),
        // so we do left outer join.
        return keyRecordPairs
                .leftOuterJoin(keyFilenamePair)
                .values()
                .map(
                        v ->
                                HoodieIndexUtils.tagAsNewRecordIfNeeded(
                                        v.getLeft(), Option.ofNullable(v.getRight().orElse(null))));
    }

    @Override
    public HoodieData<WriteStatus> updateLocation(
            HoodieData<WriteStatus> writeStatusData,
            HoodieEngineContext context,
            HoodieTable hoodieTable) {
