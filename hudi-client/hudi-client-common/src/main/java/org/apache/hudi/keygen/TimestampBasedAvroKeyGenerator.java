...
/**
  * Avro Key generator, that relies on timestamps for partitioning field. Still picks record key by
  * name.
  */
public class TimestampBasedAvroKeyGenerator extends SimpleAvroKeyGenerator {
    ...
    TimestampBasedAvroKeyGenerator(TypedProperties config, String partitionPathField)
            throws IOException {
        this(config, null, partitionPathField);
    }

    TimestampBasedAvroKeyGenerator(
            TypedProperties config, String recordKeyField, String partitionPathField) throws IOException {
        ...
        switch (this.timestampType) {
            ...
            case SCALAR:
                String timeUnitStr =
                        getStringWithAltKeys(config, INPUT_TIME_UNIT, TimeUnit.SECONDS.toString());
        ...
        this.encodePartitionPath =
                config.getBoolean(
                        KeyGeneratorOptions.URL_ENCODE_PARTITIONING.key(),
    ...
    public String getPartitionPath(GenericRecord record) {
        Object partitionVal =
                HoodieAvroUtils.getNestedFieldVal(
                        record, getPartitionPathFields().get(0), true, isConsistentLogicalTimestampEnabled());
        ...
        try {
            ...
            throw new HoodieKeyGeneratorException(
                    "Unable to parse input partition field: " + partitionVal, e);
        }
    }
    ...
    public Object getDefaultPartitionVal() {
        ...
        if (timestampType == TimestampType.DATE_STRING || timestampType == TimestampType.MIXED) {
            // since partitionVal is null, we can set a default value of any format as
            // TIMESTAMP_INPUT_DATE_FORMAT_PROP
            // configured, here we take the first.
            // {Config.TIMESTAMP_INPUT_DATE_FORMAT_PROP} won't be null, it has been checked in the
            // initialization process of
            // inputFormatter
            String delimiter = parser.getConfigInputDateFormatDelimiter();
            String format =
                    getStringWithAltKeys(config, TIMESTAMP_INPUT_DATE_FORMAT, true).split(delimiter)[0];
            ...
            if (null != inputDateTimeZone) {
                ...
                return new DateTime(result, DateTimeZone.forTimeZone(TimeZone.getTimeZone("GMT")))
                        .toString(format);
            }
        }
        return result;
    ...
    public String getPartitionPath(Object partitionVal) {
        ...
        if (partitionVal instanceof Double) {
            ...
            if (!inputFormatter.isPresent()) {
                throw new HoodieException(
                        "Missing input formatter. Ensure "
        ...
        return hiveStylePartitioning
                ? getPartitionPathFields().get(0) + "=" + partitionPath
                : partitionPath;
    }
    ...
    private long convertLongTimeToMillis(Long partitionVal) {
        if (timeUnit == null) {
            ...
            throw new RuntimeException(
                    INPUT_TIME_UNIT.key() + " is not specified but scalar it supplied as time value");
        }
