...
/**
  * This strategy is used for direct marker writers, trying to do early conflict detection. It will
  * use fileSystem api like list and exist directly to check if there is any marker file conflict,
  * without any locking.
  */
public class SimpleDirectMarkerBasedDetectionStrategy extends DirectMarkerBasedDetectionStrategy {

    private static final Logger LOG =
            LoggerFactory.getLogger(SimpleDirectMarkerBasedDetectionStrategy.class);
    ...
    public SimpleDirectMarkerBasedDetectionStrategy(
        ...
        this.completedCommitInstants =
                new HashSet<>(activeTimeline.getCommitsTimeline().filterCompletedInstants().getInstants());
    ...
    public boolean hasMarkerConflict() {
        try {
            ...
            throw new HoodieIOException(
                    "Exception occurs during create marker file in eager conflict detection mode.", e);
        }
    }
