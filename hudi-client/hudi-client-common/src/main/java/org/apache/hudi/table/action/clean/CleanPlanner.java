...
/**
  ...
  *
  * <p>1) It provides sufficient time for existing queries running on older versions, to close
  *
  * <p>2) It bounds the growth of the files in the file system
  */
public class CleanPlanner<T, I, K, O> implements Serializable {
    ...
    public CleanPlanner(
                                ...
                                entry ->
                                        Pair.of(
                                                new HoodieFileGroupId(
                                                        entry.getValue().getPartitionPath(), entry.getValue().getFileId()),
                                                entry.getValue()))
        ...
        // collect savepointed timestamps to assist with incremental cleaning. For non-partitioned
        // and metadata table, we may not need this.
        this.savepointedTimestamps =
    ...
    private HoodieSavepointMetadata getSavepointMetadata(String savepointTimestamp) {
        ...
        HoodieInstant instant =
                new HoodieInstant(false, HoodieTimeline.SAVEPOINT_ACTION, savepointTimestamp);
        try {
            ...
            throw new HoodieSavepointException(
                    "Could not get savepointed data files for savepoint " + savepointTimestamp, e);
        }
    }
    ...
    /**
      ...
      */
    public List<String> getPartitionPathsToClean(Option<HoodieInstant> earliestRetainedInstant)
            throws IOException {
    ...
    /**
      ...
      * @throws IOException
      */
    private List<String> getPartitionPathsForCleanByCommits(Option<HoodieInstant> instantToRetain)
            throws IOException {
                ...
                if (hoodieTable.getActiveTimeline().isEmpty(lastClean.get())) {
                    ...
                    HoodieCleanMetadata cleanMetadata =
                            TimelineMetadataUtils.deserializeHoodieCleanMetadata(
                                    hoodieTable.getActiveTimeline().getInstantDetails(lastClean.get()).get());
                    if ((cleanMetadata.getEarliestCommitToRetain() != null)
                            && (cleanMetadata.getEarliestCommitToRetain().length() > 0)
                            && !hoodieTable
                                    .getActiveTimeline()
                                    .getCommitsTimeline()
                                    .isBeforeTimelineStarts(cleanMetadata.getEarliestCommitToRetain())) {
    ...
    /**
      ...
      * @return
      */
    private List<String> getPartitionPathsForIncrementalCleaning(
            HoodieCleanMetadata cleanMetadata, Option<HoodieInstant> newInstantToRetain) {
        boolean isSavepointDeleted = isAnySavepointDeleted(cleanMetadata);
        if (isSavepointDeleted) {
            LOG.info(
                    "Since savepoints have been removed compared to previous clean, triggering clean planning for all partitions");
            return getPartitionPathsForFullCleaning();
        } else {
            LOG.info(
                    "Incremental Cleaning mode is enabled. Looking up partition-paths that have since changed "
            ...
            return hoodieTable
                    ...
                    .flatMap(this::getPartitionsForInstants)
                    .distinct()
    ...
    private boolean isAnySavepointDeleted(HoodieCleanMetadata cleanMetadata) {
        List<String> savepointedTimestampsFromLastClean =
                cleanMetadata.getExtraMetadata() == null
                        ? Collections.emptyList()
                        : Arrays.stream(
                                        cleanMetadata
                                                .getExtraMetadata()
                                                .getOrDefault(SAVEPOINTED_TIMESTAMPS, StringUtils.EMPTY_STRING)
                                                .split(","))
                                .filter(partition -> !StringUtils.isNullOrEmpty(partition))
                                .collect(Collectors.toList());
    ...
    /**
      ...
      */
    private Pair<Boolean, List<CleanFileInfo>> getFilesToCleanKeepingLatestVersions(
        ...
        // In this scenario, we will assume that once replaced a file group automatically becomes
        // eligible for cleaning completely
        // In other words, the file versions only apply to the active file groups.
        deletePaths.addAll(
                getReplacedFilesEligibleToClean(savepointedFiles, partitionPath, Option.empty()));
    ...
    private Pair<Boolean, List<CleanFileInfo>> getFilesToCleanKeepingLatestCommits(
            String partitionPath, Option<HoodieInstant> earliestCommitToRetain) {
        return getFilesToCleanKeepingLatestCommits(
                partitionPath,
    ...
    /**
      ...
      *
      * <p>- Leaves the latest version of the file untouched - For older versions, - It leaves all the
      * commits untouched which has occurred in last <code>config.getCleanerCommitsRetained()
      * </code> commits - It leaves ONE commit before this window. We assume that the max(query
      * execution time) == commit_batch_time * config.getCleanerCommitsRetained(). This is 5 hours by
      * default (assuming ingestion is running every 30 minutes). This is essential to leave the file
      * used by the query that is running for the max time.
      *
      * <p>This provides the effect of having lookback into all changes that happened in the last X
      * commits. (eg: if you retain 10 commits, and commit batch time is 30 mins, then you have 5 hrs
      * of lookback)
      *
      * <p>This policy is the default.
      *
      * @return A {@link Pair} whose left is boolean indicating whether partition itself needs to be
      *     deleted, and right is a list of {@link CleanFileInfo} about the files in the partition that
      *     needs to be deleted.
      */
    private Pair<Boolean, List<CleanFileInfo>> getFilesToCleanKeepingLatestCommits(
            String partitionPath,
        ...
        if (policy != HoodieCleaningPolicy.KEEP_LATEST_COMMITS
                && policy != HoodieCleaningPolicy.KEEP_LATEST_BY_HOURS) {
            throw new IllegalArgumentException(
                    "getFilesToCleanKeepingLatestCommits can only be used for KEEP_LATEST_COMMITS or KEEP_LATEST_BY_HOURS");
        }
        ...
        if (commitTimeline.countInstants() > commitsRetained) {
            ...
            deletePaths.addAll(
                    getReplacedFilesEligibleToClean(savepointedFiles, partitionPath, earliestCommitToRetain));
            // add active files
            List<HoodieFileGroup> fileGroups =
                    fileSystemView.getAllFileGroupsStateless(partitionPath).collect(Collectors.toList());
            for (HoodieFileGroup fileGroup : fileGroups) {
                ...
                // Ensure there are more than 1 version of the file (we only clean old files from
                // updates)
                ...
                for (FileSlice aSlice : fileSliceList) {
                    ...
                    // Do not delete the latest commit and also the last commit before the earliest
                    // commit we
                    // are retaining
                    // The window of commit retain == max query run time. So a query could be
                    // running which
                    ...
                    if (!isFileSliceNeededForPendingMajorOrMinorCompaction(aSlice)
                            && HoodieTimeline.compareTimestamps(
                                    earliestInstant.getTimestamp(), HoodieTimeline.GREATER_THAN, fileCommitTime)) {
                        // this is a commit, that should be cleaned.
                        aFile.ifPresent(
                                hoodieDataFile -> {
                                    deletePaths.add(new CleanFileInfo(hoodieDataFile.getPath(), false));
                                    if (hoodieDataFile.getBootstrapBaseFile().isPresent()
                                            && config.shouldCleanBootstrapBaseFile()) {
                                        deletePaths.add(
                                                new CleanFileInfo(
                                                        hoodieDataFile.getBootstrapBaseFile().get().getPath(), true));
                                    }
                                });
                        ...
                        deletePaths.addAll(
                                aSlice
                                        .getLogFiles()
                                        .map(lf -> new CleanFileInfo(lf.getPath().toString(), false))
                                        .collect(Collectors.toList()));
                    }
                }
            }
    ...
    /**
      * Returns whether there are uncommitted data files under the given partition, the pending files
      * are generated by the inflight instants and maybe ready to commit, the partition can not be
      * deleted as a whole if any pending file exists.
      *
      * <p>IMPORTANT: {@code fsView.getAllFileGroups} does not return pending file groups for metadata
      * table, file listing must be used instead.
      */
    private boolean hasPendingFiles(String partitionPath) {
        try {
            ...
            StoragePath fullPartitionPath =
                    new StoragePath(hoodieTable.getMetaClient().getBasePath(), partitionPath);
            fsView.addFilesToView(
                    partitionPath,
                    FSUtils.getAllDataFilesInPartition(hoodieTable.getStorage(), fullPartitionPath));
            // use #getAllFileGroups(partitionPath) instead of #getAllFileGroups() to exclude the
            ...
            LOG.warn(
                    "Error while checking the pending files under partition: "
                            + partitionPath
    ...
    /**
      * This method finds the files to be cleaned based on the number of hours. If {@code
      * config.getCleanerHoursRetained()} is set to 5, all the files with commit time earlier than 5
      * hours will be removed. Also the latest file for any file group is retained. This policy gives
      * much more flexibility to users for retaining data for running incremental queries as compared
      * to KEEP_LATEST_COMMITS cleaning policy. The default number of hours is 5.
      *
      ...
      */
    private Pair<Boolean, List<CleanFileInfo>> getFilesToCleanKeepingLatestHours(
            String partitionPath, Option<HoodieInstant> earliestCommitToRetain) {
        return getFilesToCleanKeepingLatestCommits(
                partitionPath, 0, earliestCommitToRetain, HoodieCleaningPolicy.KEEP_LATEST_BY_HOURS);
    }

    private List<CleanFileInfo> getReplacedFilesEligibleToClean(
            List<String> savepointedFiles,
            String partitionPath,
            Option<HoodieInstant> earliestCommitToRetain) {
        ...
        if (earliestCommitToRetain.isPresent()) {
            replacedGroups =
                    fileSystemView.getReplacedFileGroupsBefore(
                            earliestCommitToRetain.get().getTimestamp(), partitionPath);
        } else {
        ...
        return replacedGroups
                ...
                // do not delete savepointed files  (archival will make sure corresponding
                // replacecommit file is not deleted)
    ...
    private String getLatestVersionBeforeCommit(
            List<FileSlice> fileSliceList, HoodieInstant instantTime) {
        for (FileSlice file : fileSliceList) {
            String fileCommitTime = file.getBaseInstantTime();
            if (HoodieTimeline.compareTimestamps(
                    instantTime.getTimestamp(), HoodieTimeline.GREATER_THAN, fileCommitTime)) {
                // fileList is sorted on the reverse, so the first commit we find <= instantTime is
                // the
                // one we want
    ...
    public Pair<Boolean, List<CleanFileInfo>> getDeletePaths(
            String partitionPath, Option<HoodieInstant> earliestCommitToRetain) {
        ...
        LOG.info(
                deletePaths.getValue().size()
                        + " patterns used to delete in partition path:"
                        + partitionPath);
    ...
    private boolean isFileSliceNeededForPendingMajorOrMinorCompaction(FileSlice fileSlice) {
        return isFileSliceNeededForPendingCompaction(fileSlice)
                || isFileSliceNeededForPendingLogCompaction(fileSlice);
    }
    ...
    private boolean noSubsequentReplaceCommit(String earliestCommitToRetain, String partitionPath) {
        return !fileSystemView
                .getReplacedFileGroupsAfterOrOn(earliestCommitToRetain, partitionPath)
                .findAny()
                .isPresent();
    }
}
