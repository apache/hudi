...
/**
  * Abstract class taking care of holding common member variables (FileSystem, SparkContext,
  * HoodieConfigs) Also, manages embedded timeline-server if enabled.
  */
public abstract class BaseHoodieClient implements Serializable, AutoCloseable {
    ...
    /**
      * Timeline Server has the same lifetime as that of Client. Any operations done on the same
      * timeline service will be able to take advantage of the cached file-system view. New completed
      * actions will be synced automatically in an incremental fashion.
      */
    ...
    protected BaseHoodieClient(
        ...
        this.storage =
                HoodieStorageUtils.getStorage(
                        clientConfig.getBasePath(), HadoopFSUtils.getStorageConf(hadoopConf));
        ...
        if (config.isEmbeddedTimelineServerEnabled()) {
            if (!timelineServer.isPresent()) {
                ...
                try {
                    ...
                    LOG.warn(
                            "Unable to start timeline service. Proceeding as if embedded server is disabled", e);
    ...
    private void runClientInitCallbacks() {
        ...
        Arrays.stream(callbackClassNames.split(","))
                .forEach(
                        callbackClass -> {
    ...
    protected HoodieTableMetaClient createMetaClient(boolean loadActiveTimelineOnLoad) {
        return HoodieTableMetaClient.builder()
                ...
                .setMetaserverConfig(config.getProps())
                .build();
    }
    ...
    /**
      ...
      *
      * @param table A hoodie table instance created after transaction starts so that the latest
      *     commits and files are captured.
      ...
      */
    protected void resolveWriteConflict(
            HoodieTable table,
    ...
    /**
      ...
      */
    protected void writeTableMetadata(
            HoodieTable table,
            String instantTime,
            HoodieCommitMetadata metadata,
            HoodieData<WriteStatus> writeStatuses) {
        context.setJobStatus(
                this.getClass().getSimpleName(), "Committing to metadata table: " + config.getTableName());
