...
/**
  * Interval Tree based index look up. Builds an {@link KeyRangeLookupTree} for every partition and
  * uses it to search for matching index files for any given recordKey that needs to be looked up.
  */
class IntervalTreeBasedIndexFileFilter implements IndexFileFilter {
    ...
    IntervalTreeBasedIndexFileFilter(
        ...
        partitionToFileIndexInfo.forEach(
                (partition, bloomIndexFiles) -> {
                    // Note that the interval tree implementation doesn't have auto-balancing to
                    // ensure logN search time.
                    // So, we are shuffling the input here hoping the tree will not have any
                    // skewness. If not, the tree could be
                    ...
                    bloomIndexFiles.forEach(
                            indexFileInfo -> {
                                if (indexFileInfo.hasKeyRanges()) {
                                    lookUpTree.insert(
                                            new KeyRangeNode(
                                                    indexFileInfo.getMinRecordKey(),
                                                    indexFileInfo.getMaxRecordKey(),
