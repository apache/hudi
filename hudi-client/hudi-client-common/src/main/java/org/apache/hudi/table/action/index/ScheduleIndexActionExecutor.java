...
/**
  ...
  * <li>1. Fetch last completed instant on data timeline. 2. Write the index plan to the
  *     <instant>.index.requested. 3. Initialize file groups for the enabled partition types within a
  *     transaction.
  */
public class ScheduleIndexActionExecutor<T, I, K, O>
    ...
    public ScheduleIndexActionExecutor(
    ...
    public Option<HoodieIndexPlan> execute() {
        ...
        if (!requestedPartitions.isEmpty()) {
            LOG.warn(
                    String.format(
                            "Following partitions already exist or inflight: %s. Going to schedule indexing of only these partitions: %s",
        ...
        List<MetadataPartitionType> finalPartitionsToIndex =
                partitionIndexTypes.stream()
                        .filter(p -> requestedPartitions.contains(p.getPartitionPath()))
        ...
        try {
            ...
            if (indexUptoInstant.isPresent()) {
                ...
                HoodieIndexPlan indexPlan =
                        new HoodieIndexPlan(LATEST_INDEX_PLAN_VERSION, indexPartitionInfos);
                // update data timeline with requested instant
                table
                        .getActiveTimeline()
                        .saveToPendingIndexAction(
                                indexInstant, TimelineMetadataUtils.serializeIndexPlan(indexPlan));
    ...
    private HoodieIndexPartitionInfo buildIndexPartitionInfo(
            MetadataPartitionType partitionType, HoodieInstant indexUptoInstant) {
        ...
        return new HoodieIndexPartitionInfo(
                LATEST_INDEX_PLAN_VERSION,
                partitionName,
                indexUptoInstant.getTimestamp(),
    ...
    private void abort(HoodieInstant indexInstant) {
        ...
        partitionIndexTypes.forEach(
                partitionType -> {
                    if (metadataPartitionExists(
                            table.getMetaClient().getBasePath(), context, partitionType.getPartitionPath())) {
