...
/**
  * Look up tree implemented as interval trees to search for any given key in (N logN) time
  * complexity.
  */
class KeyRangeLookupTree implements Serializable {
    ...
    /**
      * Inserts a new {@link KeyRangeNode} to this look up tree.
      *
      * <p>If no root exists, make {@code newNode} as the root and return the new root.
      *
      * <p>If current root and newNode matches with min record key and max record key, merge two nodes.
      * In other words, add files from {@code newNode} to current root. Return current root.
      *
      * <p>If current root is < newNode if current root has no right sub tree update current root's
      * right sub tree max and min set newNode as right sub tree else update root's right sub tree min
      * and max with newNode's min and max record key as applicable recursively call insert() with
      * root's right subtree as new root
      *
      * <p>else // current root is >= newNode if current root has no left sub tree update current
      * root's left sub tree max and min set newNode as left sub tree else update root's left sub tree
      * min and max with newNode's min and max record key as applicable recursively call insert() with
      * root's left subtree as new root
      *
    ...
    /**
      ...
      * @param lookupKey the key to be searched for
      */
    private void getMatchingIndexFiles(
            KeyRangeNode root, String lookupKey, Set<String> matchingFileNameSet) {
        if (root == null) {
            return;
        }

        if (root.getMinRecordKey().compareTo(lookupKey) <= 0
                && lookupKey.compareTo(root.getMaxRecordKey()) <= 0) {
            matchingFileNameSet.addAll(root.getFileNameList());
        }

        if (root.getLeftSubTreeMax() != null
                && root.getLeftSubTreeMin().compareTo(lookupKey) <= 0
                && lookupKey.compareTo(root.getLeftSubTreeMax()) <= 0) {
            getMatchingIndexFiles(root.getLeft(), lookupKey, matchingFileNameSet);
        }

        if (root.getRightSubTreeMax() != null
                && root.getRightSubTreeMin().compareTo(lookupKey) <= 0
