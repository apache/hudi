...
public abstract class BucketIndexBulkInsertPartitioner<T> implements BulkInsertPartitioner<T> {
  ...
  public BucketIndexBulkInsertPartitioner(HoodieTable table, String sortString, boolean preserveHoodieMetadata) {
    ...
    // Multiple bulk inserts into COW using bucket bulk insert partitioner is restricted, otherwise AppendHandleFactory will produce MOR log files
  ...
  public Option<WriteHandleFactory> getWriteHandleFactory(int idx) {
    if (!doAppend.get(idx)) {
      ...
      throw new HoodieNotSupportedException("Multiple bulk inserts into COW with simple bucket without Spark native `Row` representation "
          + "is not supported, please, use upsert operation or overwrite mode (already written data will be lost) instead");
    }
  }
