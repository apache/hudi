...
/**
  * This class is used to hold all information used to identify how to resolve conflicts between
  * instants. Since we interchange payload types between AVRO specific records and POJO's, this
  * object serves as a common payload to manage these conversions.
  */
public class ConcurrentOperation {
    ...
    public ConcurrentOperation(HoodieInstant instant, HoodieTableMetaClient metaClient)
            throws IOException {
        // Replace compaction.inflight to compaction.request since inflight does not contain
        // compaction plan.
        if (instant.getAction().equals(COMPACTION_ACTION)
                && instant.getState().equals(HoodieInstant.State.INFLIGHT)) {
            instant =
                    new HoodieInstant(
                            HoodieInstant.State.REQUESTED, COMPACTION_ACTION, instant.getTimestamp());
        }
        this.metadataWrapper =
                new HoodieMetadataWrapper(MetadataConversionUtils.createMetaWrapper(instant, metaClient));
    ...
    private void init(HoodieInstant instant) {
        if (this.metadataWrapper.isAvroMetadata()) {
            switch (getInstantActionType()) {
                case COMPACTION_ACTION:
                    ...
                    this.mutatedPartitionAndFileIds =
                            this.metadataWrapper.getMetadataFromTimeline().getHoodieCompactionPlan()
                ...
                case REPLACE_COMMIT_ACTION:
                    if (instant.isCompleted()) {
                        ...
                        this.mutatedPartitionAndFileIds.addAll(
                                CommitUtils.flattenPartitionToReplaceFileIds(partitionToReplaceFileIds));
                        this.operationType =
                                WriteOperationType.fromValue(
                                        this.metadataWrapper
                                                .getMetadataFromTimeline()
                        ...
                        // for requested replacecommit, clustering will generate a plan and
                        // HoodieRequestedReplaceMetadata will not be empty, but
                        // insert_overwrite/insert_overwrite_table could have empty content
                        // for inflight replacecommit, clustering will have no content in metadata,
                        // but insert_overwrite/insert_overwrite_table will have some commit
                        // metadata
                        HoodieRequestedReplaceMetadata requestedReplaceMetadata =
                                this.metadataWrapper.getMetadataFromTimeline().getHoodieRequestedReplaceMetadata();
                        org.apache.hudi.avro.model.HoodieCommitMetadata inflightCommitMetadata =
                                this.metadataWrapper.getMetadataFromTimeline().getHoodieInflightReplaceMetadata();
                        if (instant.isRequested()) {
                            // for insert_overwrite/insert_overwrite_table clusteringPlan will be
                            // empty
                            ...
                            if (inflightCommitMetadata != null) {
                                this.mutatedPartitionAndFileIds =
                                        getPartitionAndFileIdWithoutSuffixFromSpecificRecord(
                                                inflightCommitMetadata.getPartitionToWriteStats());
                                this.operationType =
                                        WriteOperationType.fromValue(
                                                this.metadataWrapper
                                                        .getMetadataFromTimeline()
                                ...
                                // inflight replacecommit metadata is empty due to clustering, read
                                // fileIds from requested replacecommit
                                this.mutatedPartitionAndFileIds =
                                        getPartitionAndFileIdsFromRequestedReplaceMetadata(requestedReplaceMetadata);
                                this.operationType = WriteOperationType.CLUSTER;
                            }
                            // NOTE: it cannot be the case that instant is inflight, and both the
                            // requested and inflight replacecommit metadata are empty
                        }
                    }
                    break;
                default:
            ...
            switch (getInstantActionType()) {
                    // the enumerations should be kept in sync with the timeline metadata fetching
                    // code path,
                ...
                case LOG_COMPACTION_ACTION:
                    this.mutatedPartitionAndFileIds =
                            CommitUtils.getPartitionAndFileIdWithoutSuffix(
                                    this.metadataWrapper.getCommitMetadata().getPartitionToWriteStats());
                    this.operationType = this.metadataWrapper.getCommitMetadata().getOperationType();
                    if (this.operationType.equals(WriteOperationType.CLUSTER)
                            || WriteOperationType.isOverwrite(this.operationType)) {
                        HoodieReplaceCommitMetadata replaceCommitMetadata =
                                (HoodieReplaceCommitMetadata) this.metadataWrapper.getCommitMetadata();
                        mutatedPartitionAndFileIds.addAll(
    ...
    private static Set<Pair<String, String>> getPartitionAndFileIdsFromRequestedReplaceMetadata(
            HoodieRequestedReplaceMetadata requestedReplaceMetadata) {
        return requestedReplaceMetadata.getClusteringPlan().getInputGroups().stream()
    ...
    public String toString() {
        return "{"
                + "actionType="
                + this.getInstantActionType()
                + ", instantTime="
                + this.getInstantTimestamp()
                + ", actionState="
                + this.getInstantActionState()
                + '\''
                + '}';
    }
}
