...
public abstract class HoodieIndex<I, O> implements Serializable {
    ...
    public I tagLocation(I records, HoodieEngineContext context, HoodieTable hoodieTable)
    ...
    @Deprecated
    @PublicAPIMethod(maturity = ApiMaturityLevel.DEPRECATED)
    public O updateLocation(O writeStatuses, HoodieEngineContext context, HoodieTable hoodieTable)
            throws HoodieIndexException {
        throw new HoodieNotSupportedException("Deprecated API should not be called");
    }
    ...
    /**
      * Each index type should implement its own logic to release any resources acquired during the
      * process.
      */
    public void close() {}

    @EnumDescription(
            "Determines how input records are indexed, i.e., looked up based on the key "
                    + "for the location in the existing table. Default is SIMPLE on Spark engine, and INMEMORY "
                    + "on Flink and Java engines.")
    public enum IndexType {
        @EnumFieldDescription(
                "uses an external managed Apache HBase table to store record key to "
                        + "location mapping. HBase index is a global index, enforcing key uniqueness across all "
                        + "partitions in the table.")
        HBASE,

        @EnumFieldDescription(
                "Uses in-memory hashmap in Spark and Java engine and Flink in-memory "
        ...
        @EnumFieldDescription(
                "locates the file group containing the record fast by using bucket "
        ...
        @EnumFieldDescription(
                "Index which saves the record key to location mappings in the "
    ...
    @EnumDescription(
            "Determines the type of bucketing or hashing to use when `hoodie.index.type`"
                    + " is set to `BUCKET`.")
    public enum BucketIndexEngineType {
        @EnumFieldDescription(
                "Uses a fixed number of buckets for file groups which cannot shrink or "
                        + "expand. This works for both COW and MOR tables.")
        SIMPLE,

        @EnumFieldDescription(
                "Supports dynamic number of buckets with bucket resizing to properly "
