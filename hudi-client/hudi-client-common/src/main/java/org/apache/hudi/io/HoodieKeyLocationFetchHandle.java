...
/**
  * {@link HoodieRecordLocation} fetch handle for all records from {@link HoodieBaseFile} of
  * interest.
  *
  * @param <T>
  */
public class HoodieKeyLocationFetchHandle<T, I, K, O> extends HoodieReadHandle<T, I, K, O> {
    ...
    public HoodieKeyLocationFetchHandle(
        ...
        super(
                ...
                hoodieTable,
                Pair.of(
                        partitionPathBaseFilePair.getLeft(), partitionPathBaseFilePair.getRight().getFileId()));
    ...
    private List<Pair<HoodieKey, Long>> fetchRecordKeysWithPositions(HoodieBaseFile baseFile) {
        ...
        if (keyGeneratorOpt.isPresent()) {
            return fileFormatUtils.fetchRecordKeysWithPositions(
                    hoodieTable.getStorage(), baseFile.getStoragePath(), keyGeneratorOpt);
        } else {
            return fileFormatUtils.fetchRecordKeysWithPositions(
