...
public abstract class ClusteringPlanStrategy<T, I, K, O> implements Serializable {
    ...
    /**
      * Check if the given class is deprecated. If it is, then try to convert it to suitable one and
      * update the write config accordingly.
      *
    ...
    public static String checkAndGetClusteringPlanStrategy(HoodieWriteConfig config) {
        ...
        String logStr =
                "The clustering plan '%s' is deprecated. Please set the plan as '%s' and set '%s' as '%s' to achieve the same behaviour";
    ...
    /**
      * Generate metadata for grouping eligible files and create a plan. Note that data is not moved
      * around as part of this step.
      *
      * <p>If there is no data available to cluster, return None.
      */
    public abstract Option<HoodieClusteringPlan> generateClusteringPlan();

    /**
      * Check if the clustering can proceed. If not (i.e., return false), the PlanStrategy will
      * generate an empty plan to stop the scheduling.
      */
    ...
    /**
      * Return file slices eligible for clustering. FileIds in pending clustering/compaction are not
      * eligible for clustering.
      */
    protected Stream<FileSlice> getFileSlicesEligibleForClustering(String partition) {
        SyncableFileSystemView fileSystemView =
                (SyncableFileSystemView) getHoodieTable().getSliceView();
        ...
        fgIdsInPendingCompactionLogCompactionAndClustering.addAll(
                fileSystemView
        ...
        return hoodieTable
                .getSliceView()
                .getLatestFileSlicesStateless(partition)
                // file ids already in clustering are not eligible
                .filter(
                        slice ->
                                !fgIdsInPendingCompactionLogCompactionAndClustering.contains(
                                        slice.getFileGroupId()));
    }

    /**
      * Get parameters specific to strategy. These parameters are passed from 'schedule clustering'
      * step to 'execute clustering' step. 'execute clustering' step is typically async. So these
      * params help with passing any required context from schedule to run step.
      */
                ...
                .map(
                        slice ->
                                new HoodieSliceInfo()
                                        ...
                                        .setDataFilePath(
                                                slice.getBaseFile().map(BaseFile::getPath).orElse(StringUtils.EMPTY_STRING))
                                        .setDeltaFilePaths(
                                                slice
                                                        .getLogFiles()
    ...
    protected Map<String, Double> buildMetrics(List<FileSlice> fileSlices) {
        ...
        FileSliceMetricUtils.addFileSliceCommonMetrics(
                fileSlices, metrics, getWriteConfig().getParquetMaxFileSize());
        return metrics;
    }
