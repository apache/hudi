...
/**
  * Indexing check runs for instants that completed after the base instant (in the index plan). It
  * will check if these later instants have logged updates to metadata table or not. If not, then it
  * will do the update. If a later instant is inflight, it will wait until it is completed or the
  * task times out.
  */
public abstract class AbstractIndexingCatchupTask implements IndexingCatchupTask {
    ...
    public AbstractIndexingCatchupTask(
    ...
    public void run() {
        for (HoodieInstant instant : instantsToIndex) {
            ...
            // if instant completed, ensure that there was metadata commit, else update metadata for
            ...
            if (COMPLETED.equals(instant.getState())) {
                ...
                try {
                    // we need take a lock here as inflight writer could also try to update the
                    // timeline
                    ...
                    throw new HoodieIndexException(
                            String.format("Could not update metadata partition for instant: %s", instant), e);
                } finally {
    ...
    /**
      ...
      * i.e. for commit/deltacommit/compaction/replacecommit and not for clean/restore/rollback
      * actions.
      *
    ...
    /**
      * For the given instant, this method checks if it is already caught up or not. If not, it waits
      * until the instant is completed.
      *
    ...
    HoodieInstant awaitInstantCaughtUp(HoodieInstant instant) {
        ...
        if (!instant.isCompleted()) {
            try {
                ...
                throw new HoodieIndexException(
                        String.format(
                                "Thread interrupted while running indexing check for instant: %s", instant),
                        e);
            }
        }
        return instant;
