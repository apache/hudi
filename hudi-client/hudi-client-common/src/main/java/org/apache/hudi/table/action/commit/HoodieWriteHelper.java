...
public class HoodieWriteHelper<T, R>
        extends BaseWriteHelper<
                T, HoodieData<HoodieRecord<T>>, HoodieData<HoodieKey>, HoodieData<WriteStatus>, R> {
    ...
    protected HoodieData<HoodieRecord<T>> tag(
            ...
            HoodieTable<T, HoodieData<HoodieRecord<T>>, HoodieData<HoodieKey>, HoodieData<WriteStatus>>
                    table) {
        ...
        return records
                ...
                .reduceByKey(
                        (rec1, rec2) -> {
                            ...
                            try {
                                reducedRecord =
                                        merger.merge(rec1, schema.get(), rec2, schema.get(), props).get().getLeft();
                            } catch (IOException e) {
