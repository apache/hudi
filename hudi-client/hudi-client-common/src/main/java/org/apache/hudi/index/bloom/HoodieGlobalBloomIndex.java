...
/**
  * This filter will only work with hoodie table since it will only load partitions with
  * .hoodie_partition_metadata file in it.
  */
public class HoodieGlobalBloomIndex extends HoodieBloomIndex {
    public HoodieGlobalBloomIndex(
            HoodieWriteConfig config, BaseHoodieBloomIndexHelper bloomIndexHelper) {
    ...
    List<Pair<String, BloomIndexFileInfo>> loadColumnRangesFromFiles(
        ...
        List<String> allPartitionPaths =
                FSUtils.getAllPartitionPaths(
                        context, metaClient.getStorage(), config.getMetadataConfig(), metaClient.getBasePath());
    ...
    /**
      ...
      * file<=>record comparisons based on recordKey ranges in the index info. the partition path of
      * the incoming record (partitionRecordKeyPairs._2()) will be ignored since the search scope
    ...
    protected <R> HoodieData<HoodieRecord<R>> tagLocationBacktoRecords(
        ...
        HoodiePairData<String, HoodieRecordGlobalLocation> keyAndExistingLocations =
                keyLocationPairs.mapToPair(
                        p ->
                                Pair.of(
                                        p.getLeft().getRecordKey(),
                                        HoodieRecordGlobalLocation.fromLocal(
                                                p.getLeft().getPartitionPath(), p.getRight())));
