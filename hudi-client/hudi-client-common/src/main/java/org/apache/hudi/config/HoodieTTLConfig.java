...
public class HoodieTTLConfig extends HoodieConfig {
    ...
    public static final ConfigProperty<Boolean> INLINE_PARTITION_TTL =
            ConfigProperty.key("hoodie.partition.ttl.inline")
                    .defaultValue(false)
                    .sinceVersion("1.0.0")
                    .markAdvanced()
                    .withDocumentation(
                            "When enabled, the partition ttl management service is invoked immediately after each commit, "
                                    + "to delete exipired partitions");

    public static final ConfigProperty<String> PARTITION_TTL_STRATEGY_CLASS_NAME =
            ConfigProperty.key("hoodie.partition.ttl.strategy.class")
                    .noDefaultValue()
                    .sinceVersion("1.0.0")
                    .markAdvanced()
                    .withDocumentation(
                            "Config to provide a strategy class (subclass of PartitionTTLStrategy) to get the expired partitions");

    public static final ConfigProperty<String> PARTITION_TTL_STRATEGY_TYPE =
            ConfigProperty.key("hoodie.partition.ttl.management.strategy.type")
                    .defaultValue(PartitionTTLStrategyType.KEEP_BY_TIME.name())
                    .sinceVersion("1.0.0")
                    .markAdvanced()
                    .withDocumentation(
                            "Partition ttl management strategy type to determine the strategy class");

    public static final ConfigProperty<Integer> DAYS_RETAIN =
            ConfigProperty.key(PARTITION_TTL_STRATEGY_PARAM_PREFIX + "days.retain")
                    .defaultValue(-1)
                    .sinceVersion("1.0.0")
                    .markAdvanced()
    ...
    public static final ConfigProperty<String> PARTITION_SELECTED =
            ConfigProperty.key(PARTITION_TTL_STRATEGY_PARAM_PREFIX + "partition.selected")
                    .noDefaultValue()
                    .markAdvanced()
                    .sinceVersion("1.0.0")
    ...
    public static final ConfigProperty<Integer> MAX_PARTITION_TO_DELETE =
            ConfigProperty.key(PARTITION_TTL_STRATEGY_PARAM_PREFIX + "max.delete.partitions")
                    .defaultValue(1000)
                    .markAdvanced()
                    .sinceVersion("1.0.0")
