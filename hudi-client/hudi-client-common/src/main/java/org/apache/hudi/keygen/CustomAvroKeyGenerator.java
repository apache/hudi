...
/**
  * This is a generic implementation of KeyGenerator where users can configure record key as a single
  * field or a combination of fields. Similarly partition path can be configured to have multiple
  ...
  *
  * <p>properties.put("hoodie.datasource.write.partitionpath.field",
  * "field1:PartitionKeyType1,field2:PartitionKeyType2").
  *
  * <p>The complete partition path is created as <value for field1 basis PartitionKeyType1>/<value
  * for field2 basis PartitionKeyType2> and so on.
  *
  * <p>Few points to consider: 1. If you want to customize some partition path field on a timestamp
  * basis, you can use field1:timestampBased 2. If you simply want to have the value of your
  * configured field in the partition path, use field1:simple 3. If you want your table to be non
  * partitioned, simply leave it as blank.
  *
  * <p>RecordKey is internally generated using either SimpleKeyGenerator or ComplexKeyGenerator.
  */
public class CustomAvroKeyGenerator extends BaseKeyGenerator {
    ...
    public CustomAvroKeyGenerator(TypedProperties props) {
        ...
        this.recordKeyFields =
                Option.ofNullable(props.getString(KeyGeneratorOptions.RECORDKEY_FIELD_NAME.key(), null))
                        .map(
        ...
        this.partitionPathFields =
                Arrays.stream(
                                props.getString(KeyGeneratorOptions.PARTITIONPATH_FIELD_NAME.key()).split(","))
                        .map(String::trim)
                        .collect(Collectors.toList());
        this.recordKeyGenerator =
                getRecordKeyFieldNames().size() == 1
                        ? new SimpleAvroKeyGenerator(config)
                        : new ComplexAvroKeyGenerator(config);
        this.partitionKeyGenerators = getPartitionKeyGenerators(this.partitionPathFields, config);
    }

    private static List<BaseKeyGenerator> getPartitionKeyGenerators(
            List<String> partitionPathFields, TypedProperties config) {
                                    ...
                                    case TIMESTAMP:
                                        try {
                                            ...
                                            throw new HoodieKeyGeneratorException(
                                                    "Unable to initialise TimestampBasedKeyGenerator class", e);
                                        }
                                    default:
                                        throw new HoodieKeyGeneratorException(
                                                "Please provide valid PartitionKeyType with fields! You provided: "
                                                        + keyType);
                                }
                            })
                    .collect(Collectors.toList());
