...
public class ConsistentBucketIndexUtils {
    ...
    /**
      * Loads hashing metadata of the given partition, if it does not exist, creates a new one (also
      * persist it into storage).
      *
      * <p>NOTE: When creating a new hashing metadata, the content will always be the same for the same
      * partition. It means when multiple writer are trying to initialize metadata for the same
      * partition, no lock or synchronization is necessary as they are creating the file with the same
      * content.
      *
      ...
      */
    public static HoodieConsistentHashingMetadata loadOrCreateMetadata(
            HoodieTable table, String partition, int numBuckets) {
        ...
        HoodieConsistentHashingMetadata metadata =
                new HoodieConsistentHashingMetadata(partition, numBuckets);
        if (saveMetadata(table, metadata, false)) {
            return metadata;
        }

        // The creation failed, so try load metadata again. Concurrent creation of metadata should
        // have succeeded.
        // Note: the consistent problem of cloud storage is handled internal in the
        // HoodieWrapperFileSystem, i.e., ConsistentGuard
        metadataOption = loadMetadata(table, partition);
        ValidationUtils.checkState(
                metadataOption.isPresent(), "Failed to load or create metadata, partition: " + partition);
        return metadataOption.get();
    }
    ...
    /**
      ...
      */
    public static Option<HoodieConsistentHashingMetadata> loadMetadata(
            HoodieTable table, String partition) {
        HoodieTableMetaClient metaClient = table.getMetaClient();
        Path metadataPath =
                HadoopFSUtils.constructAbsolutePathInHadoopPath(
                        metaClient.getHashingMetadataPath(), partition);
        Path partitionPath =
                HadoopFSUtils.constructAbsolutePathInHadoopPath(
                        metaClient.getBasePath().toString(), partition);
        try {
            Predicate<FileStatus> hashingMetaCommitFilePredicate =
                    fileStatus -> {
                        String filename = fileStatus.getPath().getName();
                        return filename.contains(
                                HoodieConsistentHashingMetadata.HASHING_METADATA_COMMIT_FILE_SUFFIX);
                    };
            Predicate<FileStatus> hashingMetadataFilePredicate =
                    fileStatus -> {
                        String filename = fileStatus.getPath().getName();
            ...
            final TreeSet<String> commitMetaTss =
                    Arrays.stream(metaFiles)
                            .filter(hashingMetaCommitFilePredicate)
                            .map(
                                    commitFile ->
                                            HoodieConsistentHashingMetadata.getTimestampFromFile(
                                                    commitFile.getPath().getName()))
                            .sorted()
            ...
            FileStatus maxMetadataFile =
                    hashingMetaFiles.length > 0 ? hashingMetaFiles[hashingMetaFiles.length - 1] : null;
            // If single file present in metadata and if its default file return it
            if (maxMetadataFile != null
                    && HoodieConsistentHashingMetadata.getTimestampFromFile(
                                    maxMetadataFile.getPath().getName())
                            .equals(HoodieTimeline.INIT_INSTANT_TS)) {
            ...
            if (maxCommitMetaFileTs != null
                    && maxMetadataFile != null
                    && maxCommitMetaFileTs.equals(
                            HoodieConsistentHashingMetadata.getTimestampFromFile(
                                    maxMetadataFile.getPath().getName()))) {
                return loadMetadataFromGivenFile(table, maxMetadataFile);
            }
            ...
            Arrays.stream(hashingMetaFiles)
                    .forEach(
                            hashingMetaFile -> {
                                ...
                                if (maxCommitMetaFileTs != null && timestamp.compareTo(maxCommitMetaFileTs) <= 0) {
                                    // only fix the metadata with greater timestamp than max
                                    // committed timestamp
                                    return;
                                }
                                    ...
                                    if (!commitMetaTss.contains(timestamp)) {
                                        try {
                                            ...
                                            throw new HoodieIOException(
                                                    "Exception while creating marker file "
                                                            + path.getName()
                                                            + " for partition "
                                                            + partition,
                                                    e);
    ...
    public static boolean saveMetadata(
            HoodieTable table, HoodieConsistentHashingMetadata metadata, boolean overwrite) {
    ...
    /**
      * * Creates commit marker corresponding to hashing metadata file after post commit clustering
      * operation.
      *
      * @param table Hoodie table
      ...
      * @throws IOException
      */
    private static void createCommitMarker(HoodieTable table, Path fileStatus, Path partitionPath)
            throws IOException {
        HoodieStorage storage = table.getStorage();
        StoragePath fullPath =
                new StoragePath(
                        convertToStoragePath(partitionPath),
                        getTimestampFromFile(fileStatus.getName()) + HASHING_METADATA_COMMIT_FILE_SUFFIX);
        if (storage.exists(fullPath)) {
            return;
        }
        // prevent exception from race condition. We are ok with the file being created in another
        // thread, so we should
        // check for the marker after catching the exception and we don't need to fail if the file
        // exists
        try {
            FileIOUtils.createFileInPath(
                    storage, fullPath, Option.of(getUTF8Bytes(StringUtils.EMPTY_STRING)));
    ...
    /**
      ...
      */
    private static Option<HoodieConsistentHashingMetadata> loadMetadataFromGivenFile(
            HoodieTable table, FileStatus metaFile) {
        if (metaFile == null) {
            return Option.empty();
        }
        try (InputStream is = table.getStorage().open(convertToStoragePath(metaFile.getPath()))) {
            ...
            LOG.error(
                    "Error when loading hashing metadata, for path: " + metaFile.getPath().getName(), e);
            throw new HoodieIndexException("Error while loading hashing metadata", e);
        }
    }
    ...
    /**
      ...
      *
      * <p>If particular hashing metadata file doesn't have commit marker then there could be a case
      * where clustering is done but post commit marker creation operation failed. In this case this
      * method will check file group id from consistent hashing metadata against storage base file
      * group ids. if one of the file group matches then we can conclude that this is the latest
      * metadata file.
      *
      * <p>Note : we will end up calling this method if there is no marker file and no replace commit
      * on active timeline, if replace commit is not present on active timeline that means old file
      * group id's before clustering operation got cleaned and only new file group id's of current
      * clustering operation are present on the disk.
      *
      * @param table Hoodie table
      ...
      */
    private static boolean recommitMetadataFile(
            HoodieTable table, FileStatus metaFile, String partition) {
        Path partitionPath =
                new Path(
        ...
        Option<HoodieConsistentHashingMetadata> hoodieConsistentHashingMetadataOption =
                loadMetadataFromGivenFile(table, metaFile);
        ...
        Predicate<String> hoodieFileGroupIdPredicate =
                hoodieBaseFile ->
                        hoodieConsistentHashingMetadata.getNodes().stream()
                                .anyMatch(node -> node.getFileIdPrefix().equals(hoodieBaseFile));
        if (table
                .getBaseFileOnlyView()
                .getLatestBaseFiles(partition)
                .map(fileIdPrefix -> FSUtils.getFileIdPfxFromFileId(fileIdPrefix.getFileId()))
                .anyMatch(hoodieFileGroupIdPredicate)) {
            try {
    ...
    /**
      * Initialize fileIdPfx for each data partition. Specifically, the following fields is
      * constructed: - fileIdPfxList: the Nth element corresponds to the Nth data partition, indicating
      * its fileIdPfx - partitionToFileIdPfxIdxMap (return value): (table partition) -> (fileIdPfx ->
      * idx) mapping
      *
      * @param partitionToIdentifier Mapping from table partition to bucket identifier
      */
    public static Map<String, Map<String, Integer>> generatePartitionToFileIdPfxIdxMap(
            Map<String, ConsistentBucketIdentifier> partitionToIdentifier) {
        Map<String, Map<String, Integer>> partitionToFileIdPfxIdxMap =
                new HashMap(partitionToIdentifier.size() * 2);
        int count = 0;
