...
public class ConsistentBucketIndexUtils {
  ...
  public static Option<HoodieConsistentHashingMetadata> loadMetadata(HoodieTable table, String partition) {
    ...
    Path metadataPath = FSUtils.getPartitionPathInPath(metaClient.getHashingMetadataPath(), partition);
    Path partitionPath = FSUtils.getPartitionPathInPath(metaClient.getBasePathV2().toString(), partition);
    try {
      ...
      final FileStatus[] metaFiles =
          ((FileSystem) metaClient.getHoodieStorage().getFileSystem()).listStatus(metadataPath);
  ...
  public static boolean saveMetadata(HoodieTable table, HoodieConsistentHashingMetadata metadata, boolean overwrite) {
    ...
    Path dir =
        FSUtils.getPartitionPathInPath(table.getMetaClient().getHashingMetadataPath(), metadata.getPartitionPath());
  ...
  private static void createCommitMarker(HoodieTable table, Path fileStatus, Path partitionPath) throws IOException {
    ...
    FileIOUtils.createFileInLocation(storage, fullPath, Option.of(getUTF8Bytes(StringUtils.EMPTY_STRING)));
  }
  ...
  private static Option<HoodieConsistentHashingMetadata> loadMetadataFromGivenFile(HoodieTable table, FileStatus metaFile) {
    try {
      ...
      byte[] content = FileIOUtils.readAsByteArray(table.getMetaClient().getHoodieStorage().open(
          new HoodieLocation(metaFile.getPath().toUri())));
