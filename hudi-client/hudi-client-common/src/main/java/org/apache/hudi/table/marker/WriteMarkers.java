...
/**
  ...
  *
  * <p>This abstract class provides abstract methods of different marker operations, so that
...
public abstract class WriteMarkers implements Serializable {
    ...
    /**
      * Creates a marker without checking if the marker already exists. This can invoke marker-based
      * early conflict detection when enabled for multi-writers.
      *
      * @param partitionPath partition path in the table
      * @param fileName file name
      * @param type write IO type
      ...
      * @return the marker path.
      */
    public Option<StoragePath> create(
        ...
        if (writeConfig.getWriteConcurrencyMode().isOptimisticConcurrencyControl()
            ...
            // TODO If current is compact or clustering then create marker directly without early
            // conflict detection.
            // Need to support early conflict detection between table service and common writers.
            // ok to use filterPendingReplaceTimeline().containsInstant because early conflict
            // detection is not relevant for insert overwrite as well
    ...
    /**
      * Creates a marker if the marker does not exist. This can invoke marker-based early conflict
      * detection when enabled for multi-writers.
      *
      * @param partitionPath partition path in the table
      * @param fileName file name
      * @param type write IO type
    ...
    public void quietDeleteMarkerDir(HoodieEngineContext context, int parallelism) {
        try {
            context.setJobStatus(
                    this.getClass().getSimpleName(), "Deleting marker directory: " + basePath);
    ...
    /**
      ...
      * @throws IOException
      */
    public abstract Set<String> createdAndMergedDataPaths(
    ...
    /**
      ...
      */
    abstract Option<StoragePath> create(
            String partitionPath, String fileName, IOType type, boolean checkIfExists);

    /**
      * Creates a marker with early conflict detection for multi-writers. If conflict is detected, an
      * exception is thrown to fail the write operation.
      *
      * @param partitionPath partition path in the table.
      * @param fileName file name.
      * @param type write IO type.
      ...
      * @return the marker path or empty option if already exists and {@code checkIfExists} is true.
      */
    public abstract Option<StoragePath> createWithEarlyConflictDetection(
            String partitionPath,
            String fileName,
            IOType type,
