...
public class HoodieCreateHandle<T, I, K, O> extends HoodieWriteHandle<T, I, K, O> {
        ...
        try {
            ...
            createMarkerFile(
                    ...
                    FSUtils.makeBaseFileName(
                            this.instantTime, this.writeToken, this.fileId, hoodieTable.getBaseFileExtension()));
            this.fileWriter =
    ...
    public void write() {
        ...
        if (hoodieTable.requireSortedRecords()) {
            // Sorting the keys limits the amount of extra memory required for writing sorted
            // records
    ...
    public List<WriteStatus> close() {
        LOG.info(
                "Closing the file "
                        ...
                        + " as we are done with all the records "
                        + recordsWritten);
        try {
            if (isClosed()) {
            ...
            LOG.info(
                    String.format(
                            "CreateHandle for partitionPath %s fileID %s, took %d ms.",
