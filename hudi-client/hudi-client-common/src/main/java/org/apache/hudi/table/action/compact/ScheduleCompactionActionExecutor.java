...
public class ScheduleCompactionActionExecutor<T, I, K, O>
    ...
    public ScheduleCompactionActionExecutor(
    ...
    private void initPlanGenerator(
            HoodieEngineContext context, HoodieWriteConfig config, HoodieTable<T, I, K, O> table) {
                ...
                if (operationType.equals(WriteOperationType.COMPACT)) {
                    ...
                    table
                            ...
                            .saveToCompactionRequested(
                                    compactionInstant, TimelineMetadataUtils.serializeCompactionPlan(plan));
                } else {
                    ...
                    table
                            ...
                            .saveToLogCompactionRequested(
                                    logCompactionInstant, TimelineMetadataUtils.serializeCompactionPlan(plan));
                }
        ...
        if (compactable) {
            ...
            try {
                context.setJobStatus(
                        this.getClass().getSimpleName(), "Compaction: generating compaction plan");
                return planGenerator.generateCompactionPlan(instantTime);
            } catch (IOException e) {
                throw new HoodieCompactionException(
                        "Could not schedule compaction " + config.getBasePath(), e);
            }
        }
        ...
        switch (compactionTriggerStrategy) {
            case NUM_COMMITS:
                ...
                if (compactable) {
                    LOG.info(
                            String.format(
                                    "The delta commits >= %s, trigger compaction scheduler.",
            ...
            case NUM_COMMITS_AFTER_LAST_REQUEST:
                ...
                if (compactable) {
                    LOG.info(
                            String.format(
                                    "The delta commits >= %s since the last compaction request, trigger compaction scheduler.",
                                    inlineCompactDeltaCommitMax));
                }
                break;
            case TIME_ELAPSED:
                ...
                if (compactable) {
                    LOG.info(
                            String.format(
                                    "The elapsed time >=%ss, trigger compaction scheduler.",
                                    inlineCompactDeltaSecondsMax));
                }
                break;
            case NUM_OR_TIME:
                compactable =
                        inlineCompactDeltaCommitMax <= latestDeltaCommitInfo.getLeft()
                                || inlineCompactDeltaSecondsMax
                ...
                if (compactable) {
                    LOG.info(
                            String.format(
                                    "The delta commits >= %s or elapsed_time >=%ss, trigger compaction scheduler.",
                                    inlineCompactDeltaCommitMax, inlineCompactDeltaSecondsMax));
                }
                break;
            case NUM_AND_TIME:
                compactable =
                ...
                if (compactable) {
                    LOG.info(
                            String.format(
                                    "The delta commits >= %s and elapsed_time >=%ss, trigger compaction scheduler.",
                                    inlineCompactDeltaCommitMax, inlineCompactDeltaSecondsMax));
                }
                break;
            default:
                throw new HoodieCompactionException(
                        "Unsupported compaction trigger strategy: " + config.getInlineCompactTriggerStrategy());
        }
        return compactable;
    }

    private Long parsedToSeconds(String time) {
        return HoodieActiveTimeline.parseDateFromInstantTimeSafely(time)
                        .orElseThrow(() -> new HoodieCompactionException("Failed to parse timestamp " + time))
                        .getTime()
                / 1000;
    }
}
