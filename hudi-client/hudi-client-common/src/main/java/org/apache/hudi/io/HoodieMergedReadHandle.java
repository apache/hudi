...
public class HoodieMergedReadHandle<T, I, K, O> extends HoodieReadHandle<T, I, K, O> {
    ...
    public List<HoodieRecord<T>> getMergedRecords() {
        ...
        checkState(
                nonEmpty(instantTime),
                String.format("Expected a valid instant time but got `%s`", instantTime));
        final FileSlice fileSlice = fileSliceOpt.get();
        String baseFileInstantTime = fileSlice.getBaseFile().get().getCommitTime();
        final HoodieRecordLocation currentLocation =
                new HoodieRecordLocation(baseFileInstantTime, fileSlice.getFileId());
        ...
        try {
            ...
            doMergedRead(baseFileReader, logRecordScanner)
                    .forEach(
                            r -> {
                                r.unseal();
    ...
    private HoodieMergedLogRecordScanner getLogRecordScanner(FileSlice fileSlice) {
        ...
        return HoodieMergedLogRecordScanner.newBuilder()
                ...
                .withMaxMemorySizeInBytes(
                        IOUtils.getMaxMemoryPerCompaction(hoodieTable.getTaskContextSupplier(), config))
    ...
    private List<HoodieRecord<T>> doMergedRead(
            Option<HoodieFileReader> baseFileReaderOpt, HoodieMergedLogRecordScanner logRecordScanner)
            throws IOException {
        List<HoodieRecord<T>> mergedRecords = new ArrayList<>();
        ...
        if (baseFileReaderOpt.isPresent()) {
            ...
            Option<Pair<String, String>> simpleKeyGenFieldsOpt =
                    tableConfig.populateMetaFields()
                            ...
                            : Option.of(
                                    Pair.of(
                                            tableConfig.getRecordKeyFieldProp(), tableConfig.getPartitionFieldProp()));
                ...
                HoodieRecord<T> record =
                        baseFileItr
                                ...
                                .wrapIntoHoodieRecordPayloadWithParams(
                                        ...
                                        logRecordScanner.getPartitionNameOverride(),
                                        false,
                                        Option.empty());
