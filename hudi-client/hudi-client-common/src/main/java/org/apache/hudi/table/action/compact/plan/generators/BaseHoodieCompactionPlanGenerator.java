...
public abstract class BaseHoodieCompactionPlanGenerator<T extends HoodieRecordPayload, I, K, O>
    ...
    private static final Logger LOG =
            LoggerFactory.getLogger(BaseHoodieCompactionPlanGenerator.class);

    protected final HoodieTable<T, I, K, O> hoodieTable;
    protected final HoodieWriteConfig writeConfig;
    protected final transient HoodieEngineContext engineContext;

    public BaseHoodieCompactionPlanGenerator(
    ...
    public HoodieCompactionPlan generateCompactionPlan(String compactionInstant) throws IOException {
        ...
        engineContext.setJobStatus(
                ...
                "Looking for files to compact: " + writeConfig.getTableName());

        SyncableFileSystemView fileSystemView =
                (SyncableFileSystemView) this.hoodieTable.getSliceView();
        ...
        fgIdsInPendingCompactionAndClustering.addAll(
                fileSystemView
                                                                            ...
                                                                            .filter(
                                                                                    logFile ->
                                                                                            completionTimeQueryView.isCompletedBefore(
                                                                                                    compactionInstant, logFile.getDeltaCommitTime()))
                                                                            .sorted(HoodieLogFile.getLogFileComparator())
                                                                            .collect(toList());
                                                            totalLogFiles.add(logFiles.size());
                                                            totalFileSlices.add(1L);
                                                            // Avro generated classes are not
                                                            // inheriting Serializable. Using
                                                            // CompactionOperation POJO
                                                            // for Map operations and collecting
                                                            // them finally in Avro generated
                                                            // classes for storing
                                                            // into meta files.
    ...
    protected abstract HoodieCompactionPlan getCompactionPlan(
