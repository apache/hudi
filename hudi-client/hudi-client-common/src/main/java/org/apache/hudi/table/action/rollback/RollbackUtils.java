...
public class RollbackUtils {
  ...
  static List<HoodieRollbackRequest> groupRollbackRequests(List<HoodieRollbackRequest> rollbackRequests) {
    return groupRollbackRequests(rollbackRequests, e -> e,
  ...
  static List<SerializableHoodieRollbackRequest> groupSerializableRollbackRequests(
      List<SerializableHoodieRollbackRequest> rollbackRequests) {
    return groupRollbackRequests(rollbackRequests, SerializableHoodieRollbackRequest::new,
   ...
   */
  static <T> List<T> groupRollbackRequests(List<T> rollbackRequests,
    ...
    return requestMap.entrySet().stream().flatMap(entry -> {
      ...
      if (!logBlocksToBeDeleted.isEmpty() && !rollbackRequestList.isEmpty()) {
        newRequestList.add(createRequestFunc.apply(HoodieRollbackRequest.newBuilder()
            ...
            .setLatestBaseInstant(getLatestBaseInstant.apply(rollbackRequestList.stream().findFirst().get()))
