...
/**
  * Represents a node in the {@link KeyRangeLookupTree}. Holds information pertaining to a single
  * index file, viz file name, min record key and max record key.
  */
class KeyRangeNode implements Comparable<KeyRangeNode>, Serializable {
    ...
    public String toString() {
        return "KeyRangeNode{minRecordKey='"
                + minRecordKey
                + '\''
                + ", maxRecordKey='"
                + maxRecordKey
    ...
    /**
      ...
      * @return the result of comparison. 0 if both min and max are equal in both. 1 if this {@link
      *     KeyRangeNode} is greater than the {@code that} keyRangeNode. -1 if {@code that}
      *     keyRangeNode is greater than this {@link KeyRangeNode}
      */
    @Override
