...
public enum ClusteringPlanPartitionFilterMode {
    @EnumFieldDescription(
            "Do not filter partitions. The clustering plan will include all partitions that have clustering candidates.")
    NONE,

    @EnumFieldDescription(
            "This filter assumes that your data is partitioned by date. The clustering plan will only include partitions "
    ...
    @EnumFieldDescription(
            "The clustering plan will include only partition paths with names that sort within the inclusive range "
                    + "[`hoodie.clustering.plan.strategy.cluster.begin.partition`, `hoodie.clustering.plan.strategy.cluster.end.partition`].")
    SELECTED_PARTITIONS,

    @EnumFieldDescription(
            "To determine the partitions in the clustering plan, the eligible partitions will be sorted in ascending order. "
