...
/**
  * Base rollback plan action executor to assist in scheduling rollback requests. This phase
  * serialized {@link HoodieRollbackPlan} to rollback.requested instant.
  */
public class BaseRollbackPlanActionExecutor<T, I, K, O>
    ...
    public BaseRollbackPlanActionExecutor(
        ...
        this.shouldRollbackUsingMarkers =
                shouldRollbackUsingMarkers && !instantToRollback.isCompleted();
    ...
    protected Option<HoodieRollbackPlan> requestRollback(String startRollbackTime) {
        final HoodieInstant rollbackInstant =
                new HoodieInstant(
                        HoodieInstant.State.REQUESTED, HoodieTimeline.ROLLBACK_ACTION, startRollbackTime);
        try {
            ...
            HoodieRollbackPlan rollbackPlan =
                    new HoodieRollbackPlan(
                            new HoodieInstantInfo(
                                    instantToRollback.getTimestamp(), instantToRollback.getAction()),
                            rollbackRequests,
            ...
            if (!skipTimelinePublish) {
                if (table
                        ...
                        .containsInstant(rollbackInstant.getTimestamp())) {
                    LOG.warn(
                            "Request Rollback found with instant time "
                                    + rollbackInstant
                                    + ", hence skipping scheduling rollback");
                } else {
                    table
                            .getActiveTimeline()
                            .saveToRollbackRequested(
                                    rollbackInstant, TimelineMetadataUtils.serializeRollbackPlan(rollbackPlan));
