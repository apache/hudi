...
/**
  * Async Compactor Service that runs in separate thread. Currently, only one compactor is allowed to
  * run at any time.
  */
public abstract class AsyncCompactService extends HoodieAsyncTableService {
    ...
    public AsyncCompactService(
            HoodieEngineContext context, BaseHoodieWriteClient client, boolean runInDaemonMode) {
                                                ...
                                                CompletableFuture.supplyAsync(
                                                        () -> {
                                                            try {
                                                                ...
                                                                // compaction
                                                                LOG.info(
                                                                        "Setting pool name for compaction to " + COMPACT_POOL_NAME);
                                                                context.setProperty(
                                                                        EngineProperty.COMPACTION_POOL_NAME, COMPACT_POOL_NAME);
                                                                ...
                                                                hasError = true;
                                                                LOG.warn(
                                                                        "Compactor executor thread got interrupted exception. Stopping",
                                                                        ie);
                                                            } catch (IOException e) {
                                                                hasError = true;
