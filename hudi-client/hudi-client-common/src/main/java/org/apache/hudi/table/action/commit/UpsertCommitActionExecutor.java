...
public class UpsertCommitActionExecutor<T extends HoodieRecordPayload<T>>
  ...
  public UpsertCommitActionExecutor(HoodieEngineContext context, HoodieWriteConfig config,
                                    HoodieTable table,
                                    String instantTime, HoodieData<HoodieRecord<T>> inputRecords,
                                    BaseCommitHelper<T> commitHelper, BaseWriteHelper<T> writeHelper) {
    super(context, config, table, instantTime, WriteOperationType.UPSERT, commitHelper);
  ...
  public HoodieWriteMetadata<HoodieData<WriteStatus>> execute() {
    return writeHelper.write(instantTime, inputRecords, context, table,
        config.shouldCombineBeforeUpsert(), config.getUpsertShuffleParallelism(), commitHelper, true);
  }
}
