...
public class HoodieKeyLookupHandle<T, I, K, O> extends HoodieReadHandle<T, I, K, O> {
    ...
    private BloomFilter getBloomFilter() {
        ...
        try {
            if (config.getBloomIndexUseMetadata()
                ...
                bloomFilter =
                        hoodieTable
                                .getMetadataTable()
                                .getBloomFilter(
                                        partitionPathFileIDPair.getLeft(), partitionPathFileIDPair.getRight())
                                .orElseThrow(
                                        () ->
                                                new HoodieIndexException(
                                                        "BloomFilter missing for " + partitionPathFileIDPair.getRight()));
            } else {
            ...
            throw new HoodieIndexException(
                    String.format("Error reading bloom filter from %s", getPartitionPathFileIDPair()), e);
        }
        LOG.info(
                String.format(
                        "Read bloom filter from %s in %d ms", partitionPathFileIDPair, timer.endTimer()));
        return bloomFilter;
    }
    ...
    public void addKey(String recordKey) {
        ...
        if (bloomFilter.mightContain(recordKey)) {
            if (LOG.isDebugEnabled()) {
                LOG.debug(
                        "Record key " + recordKey + " matches bloom filter in  " + partitionPathFileIDPair);
            }
    ...
    /**
      * Of all the keys, that were added, return a list of keys that were actually found in the file
      * group.
      */
    public HoodieKeyLookupResult getLookupResult() {
        if (LOG.isDebugEnabled()) {
            LOG.debug(
                    "#The candidate row keys for " + partitionPathFileIDPair + " => " + candidateRecordKeys);
        }
        ...
        LOG.info(
                String.format(
                        "Total records (%d), bloom filter candidates (%d)/fp(%d), actual matches (%d)",
                        totalKeysChecked,
