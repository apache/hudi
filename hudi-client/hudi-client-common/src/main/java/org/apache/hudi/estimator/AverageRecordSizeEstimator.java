    ...
    try {
      if (!commitTimeline.empty()) {
        ...
        Stream<HoodieInstant> filteredInstants = commitTimeline.filterCompletedInstants()
            ...
            .limit(maxCommits);
        filteredInstants.parallel()
            .forEach(instant -> {
              HoodieCommitMetadata commitMetadata;
              try {
                commitMetadata = commitTimeline.deserializeInstantContent(instant, HoodieCommitMetadata.class);
                if (instant.getAction().equals(DELTA_COMMIT_ACTION)) {
                  ...
                  commitMetadata.getWriteStats().stream().parallel()
                      ...
                      .forEach(hoodieWriteStat -> {
                        averageRecordSizeStats.updateStats(hoodieWriteStat.getTotalWriteBytes(), hoodieWriteStat.getNumWrites());
                      });
                } else {
