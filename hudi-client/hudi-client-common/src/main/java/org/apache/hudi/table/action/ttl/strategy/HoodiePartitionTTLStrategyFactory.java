...
/**
  ...
  *
  * <p>This factory will try {@link HoodieTTLConfig#PARTITION_TTL_STRATEGY_CLASS_NAME} firstly, this
  * ensures the class prop will not be overwritten by {@link PartitionTTLStrategyType}
  */
public class HoodiePartitionTTLStrategyFactory {
        ...
        try {
            return (PartitionTTLStrategy)
                    ReflectionUtils.loadClass(
                            strategyClassName,
                            new Class<?>[] {HoodieTable.class, String.class},
                            hoodieTable,
                            instantTime);
        } catch (Throwable e) {
            throw new IOException(
                    "Could not load partition ttl management strategy class " + strategyClassName, e);
        }
    }

    private static String getPartitionTTLStrategyClassName(TypedProperties props) {
        String strategyClassName = props.getString(PARTITION_TTL_STRATEGY_CLASS_NAME.key(), null);
        if (StringUtils.isNullOrEmpty(strategyClassName)) {
            String strategyType =
                    props.getString(
                            PARTITION_TTL_STRATEGY_TYPE.key(), PARTITION_TTL_STRATEGY_TYPE.defaultValue());
    ...
    /**
      ...
      * @return The partition ttl management strategy class name based on the {@link
