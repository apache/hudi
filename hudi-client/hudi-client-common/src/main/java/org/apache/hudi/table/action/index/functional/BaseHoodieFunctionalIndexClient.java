...
public abstract class BaseHoodieFunctionalIndexClient {
    ...
    /**
      * Register a functional index. Index definitions are stored in user-specified path or, by
      * default, in .hoodie/.index_defs/index.json. For the first time, the index definition file will
      * be created if not exists. For the second time, the index definition file will be updated if
      * exists. Table Config is updated if necessary.
      */
    public void register(
        ...
        if (!metaClient.getTableConfig().getProps().containsKey(HoodieTableConfig.INDEX_DEFINITION_PATH)
