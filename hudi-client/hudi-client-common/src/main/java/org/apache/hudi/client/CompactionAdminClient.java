...
public class CompactionAdminClient extends BaseHoodieClient {
  ...
  public List<RenameOpResult> unscheduleCompactionPlan(String compactionInstant, boolean skipValidation,
    ...
    if (!dryRun && allSuccess.isPresent() && allSuccess.get()) {
      ...
      HoodieLocation inflightPath = new HoodieLocation(metaClient.getMetaPath(), inflight.getFileName());
      if (metaClient.getHoodieStorage().exists(inflightPath)) {
      ...
      boolean deleted = metaClient.getHoodieStorage().delete(
          new HoodieLocation(metaClient.getMetaPath(), instant.getFileName()), false);
  ...
  protected static List<Pair<HoodieLogFile, HoodieLogFile>> getRenamingActionsToAlignWithCompactionOperation(
    ...
    return logFilesToBeMoved.stream().map(lf -> {
      ...
      HoodieLogFile newLogFile = new HoodieLogFile(new HoodieLocation(lf.getLocation().getParent(),
  ...
  protected static void renameLogFile(HoodieTableMetaClient metaClient, HoodieLogFile oldLogFile,
    ...
    List<HoodieFileInfo> statuses = metaClient.getHoodieStorage().listFiles(oldLogFile.getLocation());
    ValidationUtils.checkArgument(statuses.size() == 1, "Only one status must be present");
    ValidationUtils.checkArgument(statuses.get(0).isFile(), "Source File must exist");
    ValidationUtils.checkArgument(oldLogFile.getLocation().getParent().equals(newLogFile.getLocation().getParent()),
        "Log file must only be moved within the parent directory");
    metaClient.getHoodieStorage().rename(oldLogFile.getLocation(), newLogFile.getLocation());
  }
      ...
      if (lastInstant.isPresent()) {
        ...
        if (fileSliceOptional.isPresent()) {
          ...
          if (operation.getDataFileName().isPresent()) {
            String expPath = metaClient.getHoodieStorage()
                .getFileInfo(
                    new HoodieLocation(FSUtils.getPartitionPath(metaClient.getBasePath(), operation.getPartitionPath()).toString(),
          ...
          Set<HoodieLogFile> logFilesInCompactionOp = operation.getDeltaFileNames().stream().map(dp -> {
            try {
              List<HoodieFileInfo> fileStatuses = metaClient.getHoodieStorage()
                  .listFiles(new HoodieLocation(
                      FSUtils.getPartitionPath(metaClient.getBasePath(), operation.getPartitionPath()).toString(), dp));
              ValidationUtils.checkArgument(fileStatuses.size() == 1, "Expect only 1 file-status");
              return new HoodieLogFile(fileStatuses.get(0));
    ...
    if (renameActions.isEmpty()) {
      ...
      if (!dryRun) {
        ...
        return context.map(renameActions, lfPair -> {
          try {
            LOG.info("RENAME " + lfPair.getLeft().getLocation() + " => " + lfPair.getRight().getLocation());
  ...
  public List<Pair<HoodieLogFile, HoodieLogFile>> getRenamingActionsForUnschedulingCompactionOperation(
    ...
    String parentPath = fileSliceForCompaction.getBaseFile().map(df -> new Path(df.getPath()).getParent().toString())
        .orElse(fileSliceForCompaction.getLogFiles().findFirst().map(lf -> lf.getLocation().getParent().toString()).get());
    for (HoodieLogFile toRepair : logFilesToRepair) {
      int version = maxUsedVersion + 1;
      HoodieLogFile newLf = new HoodieLogFile(new HoodieLocation(parentPath, FSUtils.makeLogFileName(operation.getFileId(),
  ...
  public static class RenameOpResult extends OperationResult<RenameInfo> {
    ...
    public RenameOpResult(Pair<HoodieLogFile, HoodieLogFile> op, boolean success, Option<Exception> exception) {
      super(
          new RenameInfo(op.getKey().getFileId(), op.getKey().getLocation().toString(), op.getRight().getLocation().toString()),
          success, exception);
    }
