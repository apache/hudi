        ...
        switch (markerType) {
            ...
            case TIMELINE_SERVER_BASED:
                if (!table.getConfig().isEmbeddedTimelineServerEnabled()) {
                    LOG.warn(
                            "Timeline-server-based markers are configured as the marker type "
                ...
                if (StorageSchemes.HDFS
                        .getScheme()
                        .equals(
                                HadoopFSUtils.getFs(
                                                basePath,
                                                table.getContext().getStorageConf().unwrapCopyAs(Configuration.class))
                                        .getScheme())) {
                    LOG.warn(
                            "Timeline-server-based markers are not supported for HDFS: "
                                    + "base path "
                                    + basePath
            ...
            default:
                throw new HoodieException(
                        "The marker type \"" + markerType.name() + "\" is not supported.");
        }
    }
}
