...
public class TwoToOneDowngradeHandler implements DowngradeHandler {
        ...
        for (HoodieInstant inflightInstant : commits) {
            ...
            try {
                ...
                throw new HoodieException(
                        "Converting marker files to DIRECT style failed during downgrade", e);
            }
        }
    ...
    /**
      ...
      * i.e., one marker file per data file, without MARKERS.type file. This needs to be idempotent. 1.
      * read all markers from timeline server based marker files 2. create direct style markers 3.
      * delete marker type file 4. delete timeline server based marker files
      *
      ...
      */
    private void convertToDirectMarkers(
        ...
        HoodieStorage storage =
                HoodieStorageUtils.getStorage(markerDir, context.getStorageConf().newInstance());
        ...
        if (markerTypeOption.isPresent()) {
            switch (markerTypeOption.get()) {
                ...
                default:
                    throw new HoodieException(
                            "The marker type \""
                                    + markerTypeOption.get().name()
                                    + "\" is not supported for rollback.");
            }
        } else {
            if (storage.exists(new StoragePath(markerDir))) {
                // In case of partial failures during downgrade, there is a chance that marker type
                // file was deleted,
    ...
    private void deleteTimelineBasedMarkerFiles(
            HoodieEngineContext context, String markerDir, HoodieStorage storage, int parallelism)
            throws IOException {
        // Deletes timeline based marker files if any.
        Predicate<StoragePathInfo> prefixFilter =
                fileStatus -> fileStatus.getPath().getName().startsWith(MARKERS_FILENAME_PREFIX);
        FSUtils.parallelizeSubPathProcess(
                context,
                storage,
                new StoragePath(markerDir),
                parallelism,
                prefixFilter,
                pairOfSubPathAndConf ->
                        FSUtils.deleteSubPath(
                                pairOfSubPathAndConf.getKey(), pairOfSubPathAndConf.getValue(), false));
    }
}
