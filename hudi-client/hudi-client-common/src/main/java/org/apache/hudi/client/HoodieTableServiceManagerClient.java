...
public class HoodieTableServiceManagerClient {
    ...
    public HoodieTableServiceManagerClient(
            HoodieTableMetaClient metaClient, HoodieTableServiceManagerConfig config) {
    ...
    private String executeRequest(String requestPath, Map<String, String> queryParameters)
            throws IOException {
        ...
        RetryHelper<String, IOException> retryHelper =
                new RetryHelper<>(
                        connectionRetryDelay, requestRetryLimit, connectionRetryDelay, RETRY_EXCEPTIONS);
        return retryHelper
                .tryWith(
                        () ->
                                Request.Get(url)
    ...
    public Option<String> executeClustering() {
        try {
            ...
            String instantRange =
                    StringUtils.join(
                            ClusteringUtils.getPendingClusteringInstantTimes(metaClient).stream()
                                    .map(HoodieInstant::getTimestamp)
                                    .toArray(String[]::new),
                            ",");
    ...
    private Map<String, String> getDefaultParams(Action action, String instantRange) {
        return getParamsWithAdditionalParams(
                ...
                new String[] {
                    ...
                    tableName,
                    instantRange,
                    config.getDeployUsername(),
                    config.getDeployQueue(),
                    config.getDeployResources(),
                    String.valueOf(config.getDeployParallelism()),
                    config.getDeployExtraParams(),
                    config.getDeployExecutionEngine()
                });
    }
}
