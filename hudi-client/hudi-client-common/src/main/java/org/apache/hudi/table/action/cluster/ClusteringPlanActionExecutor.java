    ...
    protected Option<HoodieClusteringPlan> createClusteringPlan() {
        ...
        int commitsSinceLastClustering =
                table
                        ...
                        .findInstantsAfter(
                                lastClusteringInstant.map(HoodieInstant::getTimestamp).orElse("0"),
                                Integer.MAX_VALUE)
                        .countInstants();
        ...
        if (config.inlineClusteringEnabled()
            ...
            LOG.warn(
                    "Not scheduling inline clustering as only "
                            + commitsSinceLastClustering
                            + " commits was found since last clustering "
                            + lastClusteringInstant
                            + ". Waiting for "
                            + config.getInlineClusterMaxCommits());
            return Option.empty();
        }

        if ((config.isAsyncClusteringEnabled() || config.scheduleInlineClustering())
                && config.getAsyncClusterMaxCommits() > commitsSinceLastClustering) {
            LOG.warn(
                    "Not scheduling async clustering as only "
                            + commitsSinceLastClustering
                            + " commits was found since last clustering "
                            + lastClusteringInstant
                            + ". Waiting for "
