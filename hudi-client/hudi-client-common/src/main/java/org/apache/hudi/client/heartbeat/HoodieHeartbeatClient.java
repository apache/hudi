...
/**
  * This class creates heartbeat for hudi client. This heartbeat is used to ascertain whether the
  * running job is or not. NOTE: Due to CPU contention on the driver/client node, the heartbeats
  * could be delayed, hence it's important to set the value high enough to avoid that possibility.
  */
@NotThreadSafe
public class HoodieHeartbeatClient implements AutoCloseable, Serializable {
    ...
    public HoodieHeartbeatClient(
            HoodieStorage storage,
            String basePath,
            Long heartbeatIntervalInMs,
            Integer numTolerableHeartbeatMisses) {
        ValidationUtils.checkArgument(
                heartbeatIntervalInMs >= 1000, "Cannot set heartbeat lower than 1 second");
        ...
        this.maxAllowableHeartbeatIntervalInMs =
                this.heartbeatIntervalInMs * numTolerableHeartbeatMisses;
    ...
    /**
      * Start a new heartbeat for the specified instant. If there is already one running, this will be
      * a NO_OP
      *
    ...
    public void start(String instantTime) {
        ...
        ValidationUtils.checkArgument(
                heartbeat == null || !heartbeat.isHeartbeatStopped(),
    ...
    public void stopHeartbeatTimers() throws HoodieException {
        instantToHeartbeatMap.values().stream()
                .filter(this::isHeartbeatStarted)
                .forEach(this::stopHeartbeatTimer);
    }
    ...
    public static Boolean heartbeatExists(HoodieStorage storage, String basePath, String instantTime)
            throws IOException {
        StoragePath heartbeatFilePath =
                new StoragePath(HoodieTableMetaClient.getHeartbeatFolderPath(basePath), instantTime);
    ...
    public boolean isHeartbeatExpired(String instantTime) throws IOException {
        ...
        if (currentTime - lastHeartbeatForWriter.getLastHeartbeatTime()
                > this.maxAllowableHeartbeatIntervalInMs) {
            LOG.warn(
                    "Heartbeat expired, currentTime = "
                            + currentTime
                            + ", last heartbeat = "
                            + lastHeartbeatForWriter
                            + ", heartbeat interval = "
                            + this.heartbeatIntervalInMs);
            return true;
        }
    ...
    private void updateHeartbeat(String instantTime) throws HoodieHeartbeatException {
        try {
            ...
            if (heartbeat.getLastHeartbeatTime() != null && isHeartbeatExpired(instantTime)) {
                LOG.error(
                        "Aborting, missed generating heartbeat within allowable interval "
                                + this.maxAllowableHeartbeatIntervalInMs);
                // Since TimerTask allows only java.lang.Runnable, cannot throw an exception and
                // bubble to the caller thread, hence
            ...
            if (isHeartbeatStopped) {
                LOG.warn(
                        String.format(
                                "update heart beat failed, because the instant time %s was stopped ? : %s",
                                instantTime, isHeartbeatStopped));
                return;
            }
            throw new HoodieHeartbeatException(
                    "Unable to generate heartbeat for instant " + instantTime, io);
        }
    }
