...
public class SavepointActionExecutor<T, I, K, O>
    ...
    public SavepointActionExecutor(
    ...
    public HoodieSavepointMetadata execute() {
        ...
        try {
            // Check the last commit that was not cleaned and check if savepoint time is > that
            // commit
                                            ...
                                            if (instant.isCompleted()) {
                                                ...
                                                return deserializeCleanerPlan(
                                                                table
                                                                        ...
                                                                        .getInstantDetails(
                                                                                new HoodieInstant(
                                                                                        REQUESTED, instant.getAction(), instant.getTimestamp()))
                                                                        .get())
            ...
            ValidationUtils.checkArgument(
                    HoodieTimeline.compareTimestamps(
                            instantTime, HoodieTimeline.GREATER_THAN_OR_EQUALS, lastCommitRetained),
                    "Could not savepoint commit "
                            + instantTime
                            + " as this is beyond the lookup window "
                            + lastCommitRetained);

            context.setJobStatus(
                    this.getClass().getSimpleName(),
                    "Collecting latest files for savepoint "
                            + instantTime
                            + " "
            ...
            // NOTE: for performance, we have to use different logic here for listing the latest
            // files
            ...
            table
                    .getActiveTimeline()
                    .createNewInstant(new HoodieInstant(true, HoodieTimeline.SAVEPOINT_ACTION, instantTime));
            table
                    .getActiveTimeline()
                    .saveAsComplete(
