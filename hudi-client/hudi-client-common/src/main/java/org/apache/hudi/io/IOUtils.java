...
public class IOUtils {
    /**
      ...
      * running on an executor and each task maintains a spillable map. user.available.memory =
      * executor.memory * (1 - memory.fraction) spillable.available.memory = user.available.memory *
      * hoodie.memory.fraction / executor.cores. Anytime the engine memory fractions/total memory is
      * changed, the memory used for spillable map changes accordingly.
      */
    public static long getMaxMemoryAllowedForMerge(
            TaskContextSupplier context, String maxMemoryFraction) {
    ...
    public static long getMaxMemoryPerPartitionMerge(
            TaskContextSupplier context, HoodieConfig hoodieConfig) {
    ...
    public static long getMaxMemoryPerCompaction(
            TaskContextSupplier context, HoodieConfig hoodieConfig) {
