...
/**
  * UpgradeHandler to assist in upgrading {@link org.apache.hudi.table.HoodieTable} from version 3 to
  * 4.
  */
public class ThreeToFourUpgradeHandler implements UpgradeHandler {
    ...
    public Map<ConfigProperty, String> upgrade(
        ...
        tablePropsToAdd.put(
                TABLE_CHECKSUM, String.valueOf(HoodieTableConfig.generateChecksum(config.getProps())));
        ...
        if (config.isMetadataTableEnabled()
                && metadataPartitionExists(
                        config.getBasePath(), context, MetadataPartitionType.FILES.getPartitionPath())) {
            tablePropsToAdd.put(
                    TABLE_METADATA_PARTITIONS, MetadataPartitionType.FILES.getPartitionPath());
        }
