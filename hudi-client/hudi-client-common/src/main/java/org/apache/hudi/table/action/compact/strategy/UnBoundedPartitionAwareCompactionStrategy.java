...
/**
  * UnBoundedPartitionAwareCompactionStrategy is a custom UnBounded Strategy. This will filter all
  * the partitions that are eligible to be compacted by a {@link
  * BoundedPartitionAwareCompactionStrategy} and return the result. This is done so that a long
  * running UnBoundedPartitionAwareCompactionStrategy does not step over partitions in a shorter
  * running BoundedPartitionAwareCompactionStrategy. Essentially, this is an inverse of the
...
public class UnBoundedPartitionAwareCompactionStrategy extends CompactionStrategy {
    ...
    public List<HoodieCompactionOperation> orderAndFilter(
            ...
            final List<HoodieCompactionOperation> operations,
            final List<HoodieCompactionPlan> pendingCompactionWorkloads) {
        ...
        List<HoodieCompactionOperation> operationsToExclude =
                boundedPartitionAwareCompactionStrategy.orderAndFilter(
                        config, operations, pendingCompactionWorkloads);
