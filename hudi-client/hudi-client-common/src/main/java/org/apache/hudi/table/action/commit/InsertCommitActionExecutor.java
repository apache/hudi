...
public class InsertCommitActionExecutor<T extends HoodieRecordPayload<T>>
  ...
  private final BaseWriteHelper<T> writeHelper;
  private HoodieData<HoodieRecord<T>> inputRecords;

  public InsertCommitActionExecutor(
      ...
      BaseCommitHelper<T> commitHelper, BaseWriteHelper<T> writeHelper) {
  ...
  public HoodieWriteMetadata<HoodieData<WriteStatus>> execute() {
    return writeHelper.write(instantTime, inputRecords, context, table,
        config.shouldCombineBeforeInsert(), config.getInsertShuffleParallelism(), commitHelper, false);
  }
}
