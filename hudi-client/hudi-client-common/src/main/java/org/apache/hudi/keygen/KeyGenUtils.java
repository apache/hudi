...
public class KeyGenUtils {
    ...
    /**
      ...
      *
      * <p>(1) partition field is empty: {@link KeyGeneratorType#NON_PARTITION}; (2) Only one partition
      * field and one record key field: {@link KeyGeneratorType#SIMPLE}; (3) More than one partition
      * and/or record key fields: {@link KeyGeneratorType#COMPLEX}.
      *
    ...
    /**
      ...
      */
    public static String getRecordKeyFromGenericRecord(
            GenericRecord genericRecord, Option<BaseKeyGenerator> keyGeneratorOpt) {
        return keyGeneratorOpt.isPresent()
                ? keyGeneratorOpt.get().getRecordKey(genericRecord)
                : genericRecord.get(HoodieRecord.RECORD_KEY_METADATA_FIELD).toString();
    }
    ...
    /**
      ...
      */
    public static String getPartitionPathFromGenericRecord(
            GenericRecord genericRecord, Option<BaseKeyGenerator> keyGeneratorOpt) {
        return keyGeneratorOpt.isPresent()
    ...
    /**
      * Extracts the record key fields in strings out of the given record key, this is the reverse
      * operation of {@link #getRecordKey(GenericRecord, String, boolean)}.
      *
    ...
    public static String[] extractRecordKeysByFields(String recordKey, List<String> fields) {
        ...
        return Arrays.stream(fieldKV)
                .map(kv -> kv.split(DEFAULT_COMPOSITE_KEY_FILED_VALUE, 2))
        ...
        for (int i = 0; i < recordKeyFields.size(); i++) {
            ...
            String recordKeyValue =
                    HoodieAvroUtils.getNestedFieldValAsString(
                            record, recordKeyField, true, consistentLogicalTimestampEnabled);
            if (recordKeyValue == null) {
                recordKey
                        .append(recordKeyField)
                        .append(DEFAULT_COMPOSITE_KEY_FILED_VALUE)
    ...
    public static String getRecordPartitionPath(
        ...
        for (int i = 0; i < partitionPathFields.size(); i++) {
            ...
            String fieldVal =
                    HoodieAvroUtils.getNestedFieldValAsString(
                            record, partitionPathField, true, consistentLogicalTimestampEnabled);
    ...
    public static String getRecordKey(
            GenericRecord record, String recordKeyField, boolean consistentLogicalTimestampEnabled) {
        String recordKey =
                HoodieAvroUtils.getNestedFieldValAsString(
                        record, recordKeyField, true, consistentLogicalTimestampEnabled);
    ...
    /**
      ...
      */
    public static BaseHoodieDateTimeParser createDateTimeParser(
            TypedProperties props, String parserClass) throws IOException {
        try {
    ...
    /**
      ...
      *
      * <p>This method is for user-defined classes. To create hudi's built-in key generators, please
      * set proper {@link org.apache.hudi.keygen.constant.KeyGeneratorType} conf, and use the relevant
      * factory, see {@link org.apache.hudi.keygen.factory.HoodieAvroKeyGeneratorFactory}.
      */
    public static KeyGenerator createKeyGeneratorByClassName(TypedProperties props)
            throws IOException {
    ...
    public static boolean isAutoGeneratedRecordKeysEnabled(TypedProperties props) {
        return !props.containsKey(KeyGeneratorOptions.RECORDKEY_FIELD_NAME.key())
                || props
                        .getProperty(KeyGeneratorOptions.RECORDKEY_FIELD_NAME.key())
                        .equals(StringUtils.EMPTY_STRING);
        // spark-sql sets record key config to empty string for update, and couple of other
        // statements.
    }
}
