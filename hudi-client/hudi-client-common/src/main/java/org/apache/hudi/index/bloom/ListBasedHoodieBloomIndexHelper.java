...
public class ListBasedHoodieBloomIndexHelper extends BaseHoodieBloomIndexHelper {
    ...
    private static final ListBasedHoodieBloomIndexHelper SINGLETON_INSTANCE =
            new ListBasedHoodieBloomIndexHelper();
    ...
    public HoodiePairData<HoodieKey, HoodieRecordLocation> findMatchingFilesForRecordKeys(
        ...
        List<HoodieKeyLookupResult> keyLookupResults =
                CollectionUtils.toStream(
                                new HoodieBloomIndexCheckFunction<Pair<HoodieFileGroupId, String>>(
                                                hoodieTable, config, Pair::getLeft, Pair::getRight)
        ...
        return context
                .parallelize(keyLookupResults)
                .flatMap(
                        lookupResult ->
                                lookupResult.getMatchingRecordKeysAndPositions().stream()
                                        .map(recordKey -> new ImmutablePair<>(lookupResult, recordKey))
                                        .iterator())
                .mapToPair(
                        pair -> {
                            ...
                            return new ImmutablePair<>(
                                    ...
                                    new HoodieRecordLocation(
                                            lookupResult.getBaseInstantTime(), lookupResult.getFileId(), recordPosition));
                        });
    }
}
