...
public class HoodieMergeHandleFactory {
    ...
    public static <T, I, K, O> HoodieMergeHandle<T, I, K, O> create(
        ...
        LOG.info(
                "Create update handle for fileId {} and partition path {} at commit {}",
                fileId,
                partitionPath,
                instantTime);
        if (table.requireSortedRecords()) {
            if (table.getMetaClient().getTableConfig().isCDCEnabled()) {
                return new HoodieSortedMergeHandleWithChangeLog<>(
                        writeConfig,
                        instantTime,
                        table,
                        recordItr,
                ...
                return new HoodieSortedMergeHandle<>(
                        writeConfig,
                        instantTime,
                        table,
                        recordItr,
            ...
            if (table.getMetaClient().getTableConfig().isCDCEnabled()) {
                return new HoodieMergeHandleWithChangeLog<>(
                        writeConfig,
                        instantTime,
                        table,
                        recordItr,
    ...
    public static <T, I, K, O> HoodieMergeHandle<T, I, K, O> create(
            ...
            Option<BaseKeyGenerator> keyGeneratorOpt) {
        LOG.info(
                "Get updateHandle for fileId {} and partitionPath {} at commit {}",
                fileId,
                partitionPath,
                instantTime);
        if (table.requireSortedRecords()) {
            ...
            // IMPORTANT: only index type that yields pure log files need to enable the cdc log
            // files for compaction,
            // index type such as the BLOOM does not need this because it would do delta merge for
            // inserts and generates log for updates,
