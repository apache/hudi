...
/**
  * Consistent hashing bucket index implementation, with auto-adjust bucket number. NOTE: bucket
  * resizing is triggered by clustering.
  */
public class HoodieConsistentBucketIndex extends HoodieBucketIndex {
    ...
    public HoodieData<WriteStatus> updateLocation(
        ...
        throw new HoodieIndexException(
                "Consistent hashing index does not support update location without the instant parameter");
    }

    /**
      * Do nothing. A failed write may create a hashing metadata for a partition. In this case, we
      * still do nothing when rolling back the failed write. Because the hashing metadata created by a
      * writer must have 00000000000000 timestamp and can be viewed as the initialization of a
      * partition rather than as a part of the failed write.
      */
    @Override
    ...
    public <R> HoodieData<HoodieRecord<R>> tagLocation(
        ...
        ConsistentBucketIndexLocationMapper mapper =
                new ConsistentBucketIndexLocationMapper(hoodieTable, partitions);
    ...
    public class ConsistentBucketIndexLocationMapper implements Serializable {
                                    ...
                                    Collectors.toMap(
                                            ...
                                            p -> {
                                                HoodieConsistentHashingMetadata metadata =
                                                        ConsistentBucketIndexUtils.loadOrCreateMetadata(
                                                                table, p, getNumBuckets());
        ...
        public Option<HoodieRecordLocation> getRecordLocation(HoodieKey key) {
            ...
            if (!StringUtils.isNullOrEmpty(node.getFileIdPrefix())) {
                ...
                // We add suffix 0 here to the file uuid, following the naming convention, i.e.,
                // fileId = [uuid]_[numWrites]
                return Option.of(
                        new HoodieRecordLocation(null, FSUtils.createNewFileId(node.getFileIdPrefix(), 0)));
            }

            LOG.error(
                    "Consistent hashing node has no file group, partition: {}, meta: {}, record_key: {}",
                    partitionPath,
