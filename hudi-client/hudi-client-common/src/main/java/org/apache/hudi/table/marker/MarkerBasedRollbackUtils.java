...
public class MarkerBasedRollbackUtils {
    ...
    /**
      ...
      * @throws IOException
      */
    public static List<String> getAllMarkerPaths(
            HoodieTable table, HoodieEngineContext context, String instant, int parallelism)
            throws IOException {
        ...
        if (!markerTypeOption.isPresent()) {
            ...
            try {
                ...
                LOG.warn(
                        String.format(
                                "%s not present and %s marker failed with error: %s. So, falling back to %s marker",
    ...
    private static List<String> getTimelineServerBasedMarkers(
            HoodieEngineContext context, int parallelism, String markerDir, HoodieStorage storage) {
