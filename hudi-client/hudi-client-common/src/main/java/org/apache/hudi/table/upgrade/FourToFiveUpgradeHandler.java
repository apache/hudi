...
public class FourToFiveUpgradeHandler implements UpgradeHandler {
    ...
    public Map<ConfigProperty, String> upgrade(
        ...
        try {
            ...
            if (!config.doSkipDefaultPartitionValidation() && hasDefaultPartitionPath(config, table)) {
                LOG.error(
                        String.format(
                                "\"%s\" partition detected. From 0.12, we are changing the default partition in hudi to %s "
                ...
                throw new HoodieException(
                        String.format(
                                "Old deprecated \"%s\" partition found in hudi table. This needs a migration step before we can upgrade ",
                                DEPRECATED_DEFAULT_PARTITION_PATH));
            }
    ...
    private boolean hasDefaultPartitionPath(HoodieWriteConfig config, HoodieTable table)
            throws IOException {
        ...
        FileSystem fs =
                new Path(config.getBasePath())
                        .getFileSystem((Configuration) table.getStorageConf().unwrap());
