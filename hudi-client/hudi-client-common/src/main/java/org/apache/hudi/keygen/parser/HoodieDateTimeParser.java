...
public class HoodieDateTimeParser extends BaseHoodieDateTimeParser {
    ...
    private DateTimeFormatter getInputDateFormatter() {
        if (this.configInputDateFormatList.isEmpty()) {
            throw new IllegalArgumentException(
                    TIMESTAMP_INPUT_DATE_FORMAT.key() + " configuration is required");
        }
    ...
    public Option<DateTimeFormatter> getInputFormatter() {
        TimestampType timestampType =
                TimestampType.valueOf(getStringWithAltKeys(config, TIMESTAMP_TYPE_FIELD));
        if (timestampType == TimestampType.DATE_STRING || timestampType == TimestampType.MIXED) {
            checkRequiredConfigProperties(config, Collections.singletonList(TIMESTAMP_INPUT_DATE_FORMAT));
            this.configInputDateFormatList =
                    getStringWithAltKeys(config, TIMESTAMP_INPUT_DATE_FORMAT, true);
    ...
    public DateTimeZone getInputDateTimeZone() {
        ...
        return !inputTimeZone.trim().isEmpty()
                ? DateTimeZone.forTimeZone(TimeZone.getTimeZone(inputTimeZone))
                : null;
    }
    ...
    public DateTimeZone getOutputDateTimeZone() {
        ...
        return !outputTimeZone.trim().isEmpty()
                ? DateTimeZone.forTimeZone(TimeZone.getTimeZone(outputTimeZone))
                : null;
    }
}
