...
public class HoodieSimpleIndex extends HoodieIndex<Object, Object> {
    ...
    protected <R> HoodieData<HoodieRecord<R>> tagLocationInternal(
            HoodieData<HoodieRecord<R>> inputRecords,
            HoodieEngineContext context,
            HoodieTable hoodieTable) {
        if (config.getSimpleIndexUseCaching()) {
            inputRecords.persist(
        ...
        int fetchParallelism =
                configuredSimpleIndexParallelism > 0
                        ? configuredSimpleIndexParallelism
                        : deduceNumParallelism;
        ...
        HoodiePairData<HoodieKey, HoodieRecordLocation> existingLocationsOnTable =
                fetchRecordLocationsForAffectedPartitions(
                        keyedInputRecords.keys(), context, hoodieTable, fetchParallelism);
    ...
    protected HoodiePairData<HoodieKey, HoodieRecordLocation>
            fetchRecordLocationsForAffectedPartitions(
                    HoodieData<HoodieKey> hoodieKeys,
                    HoodieEngineContext context,
                    HoodieTable hoodieTable,
                    int parallelism) {
        List<String> affectedPartitionPathList =
                hoodieKeys
                        ...
                        .distinct(hoodieKeys.deduceNumPartitions())
                        .collectAsList();
    ...
    protected HoodiePairData<HoodieKey, HoodieRecordLocation> fetchRecordLocations(
        ...
        return context
                .parallelize(baseFiles, fetchParallelism)
                .flatMap(
                        partitionPathBaseFile ->
                                new HoodieKeyLocationFetchHandle(
                                                config, hoodieTable, partitionPathBaseFile, keyGeneratorOpt)
                                        .locations()
                                        .iterator())
