...
public class ZeroToOneUpgradeHandler implements UpgradeHandler {
    ...
    public Map<ConfigProperty, String> upgrade(
        ...
        if (!commits.isEmpty() && instantTime != null) {
            // ignore the latest inflight commit since a new commit would have been started, and we
            // need to fix any pending commits from previous launch
    ...
    /**
      * Recreate markers in new format. Step1: Delete existing markers Step2: Collect all rollback file
      * info. Step3: recreate markers for all interested files.
      *
      ...
      */
    protected void recreateMarkers(
        ...
        try {
            ...
            if (commitInstantOpt.isPresent()) {
                ...
                WriteMarkers writeMarkers =
                        WriteMarkersFactory.get(MarkerType.DIRECT, table, commitInstantTime);
                ...
                for (HoodieRollbackStat rollbackStat : rollbackStats) {
                    for (String path : rollbackStat.getSuccessDeleteFiles()) {
                        ...
                        // not feasible to differentiate MERGE from CREATE. hence creating with
                        // MERGE IOType for all base files.
                    ...
                    for (StoragePathInfo pathInfo : rollbackStat.getCommandBlocksCount().keySet()) {
                        writeMarkers.create(
                                ...
                                getFileNameForMarkerFromLogFile(pathInfo.getPath().toString(), table),
                                IOType.APPEND);
                    }
                }
            }
            ...
            throw new HoodieRollbackException(
                    "Exception thrown while upgrading Hoodie Table from version 0 to 1", e);
        }
    }
    ...
    List<HoodieRollbackStat> getListBasedRollBackStats(
        ...
        List<HoodieRollbackRequest> hoodieRollbackRequests =
                new ListingBasedRollbackStrategy(
                                table, context, table.getConfig(), commitInstantOpt.get().getTimestamp(), false)
    ...
    /**
      * Curates file name for marker from existing log file path. log file format :
      * partitionPath/.fileid_baseInstant.log.writeToken marker file format :
    ...
    private static String getFileNameForMarkerFromLogFile(
            String logFilePath, HoodieTable<?, ?, ?, ?> table) {
