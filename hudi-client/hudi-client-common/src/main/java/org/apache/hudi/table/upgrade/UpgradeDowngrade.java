...
public class UpgradeDowngrade {
    ...
    public UpgradeDowngrade(
        ...
        this.updatedPropsFilePath =
                new Path(metaClient.getMetaPath().toString(), HOODIE_UPDATED_PROPERTY_FILE);
        this.propsFilePath =
    ...
    /**
      ...
      *
      * <p>Starting from version 0.6.0, this upgrade/downgrade step will be added in all write paths.
      *
      * <p>Essentially, if a dataset was created using an previous table version in an older release,
      * and Hoodie version was upgraded to a new release with new table version supported, Hoodie table
      * version gets bumped to the new version and there are some upgrade steps need to be executed
      * before doing any writes.
      *
      * <p>Similarly, if a dataset was created using an newer table version in an newer release, and
      * then hoodie was downgraded to an older release or to older Hoodie table version, then some
      * downgrade steps need to be executed before proceeding w/ any writes.
      *
      * <p>Below shows the table version corresponding to the Hudi release: Hudi release -> table
      * version pre 0.6.0 -> v0 0.6.0 to 0.8.0 -> v1 0.9.0 -> v2 0.10.0 -> v3 0.11.0 -> v4 0.12.0 to
      * 0.13.0 -> v5 0.14.0 to current -> v6
      *
      * <p>On a high level, these are the steps performed
      *
      * <p>Step1 : Understand current hoodie table version and table version from hoodie.properties
      * file Step2 : Delete any left over .updated from previous upgrade/downgrade Step3 : If version
      * are different, perform upgrade/downgrade. Step4 : Copy hoodie.properties ->
      * hoodie.properties.updated with the version updated Step6 : Rename hoodie.properties.updated to
      * hoodie.properties
      *
    ...
    public void run(HoodieTableVersion toVersion, String instantTime) {
        ...
        if (toVersion.versionCode() >= HoodieTableVersion.FOUR.versionCode()) {
            ...
            try {
                ...
                LOG.warn(
                        "Unable to upgrade or downgrade the metadata table to version "
                                + toVersion
        ...
        // Reload the meta client to get the latest table config (which could have been updated due
        // to metadata table)
    ...
    protected Map<ConfigProperty, String> upgrade(
            HoodieTableVersion fromVersion, HoodieTableVersion toVersion, String instantTime) {
        if (fromVersion == HoodieTableVersion.ZERO && toVersion == HoodieTableVersion.ONE) {
            ...
            throw new HoodieUpgradeDowngradeException(
                    fromVersion.versionCode(), toVersion.versionCode(), true);
        }
    }

    protected Map<ConfigProperty, String> downgrade(
            HoodieTableVersion fromVersion, HoodieTableVersion toVersion, String instantTime) {
        if (fromVersion == HoodieTableVersion.ONE && toVersion == HoodieTableVersion.ZERO) {
            return new OneToZeroDowngradeHandler()
                    .downgrade(config, context, instantTime, upgradeDowngradeHelper);
