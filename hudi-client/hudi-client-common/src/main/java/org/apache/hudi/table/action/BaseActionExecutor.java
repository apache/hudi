...
public abstract class BaseActionExecutor<T, I, K, O, R> implements Serializable {
  ...
  protected final void disableWriteRecordPositionsWithPendingCompaction() {
    // When there is a pending compaction, the new base files to be generated by compaction
    // is not available during this transaction. Given the log files in MOR from this transaction
    // can be attached to the base file generated by the compaction in the latest file slice,
    // the accurate record positions may not be derived. So disabling
    // {@link WRITE_RECORD_POSITIONS} in this case.
    if (!table.getActiveTimeline().filterPendingCompactionTimeline().empty()) {
