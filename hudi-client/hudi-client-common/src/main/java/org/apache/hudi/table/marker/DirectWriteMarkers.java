...
public class DirectWriteMarkers extends WriteMarkers {
  ...
  public DirectWriteMarkers(HoodieTable table, String instantTime) {
    this(table.getMetaClient().getHoodieStorage(),
  ...
  public Set<String> createdAndMergedDataPaths(HoodieEngineContext context, int parallelism) throws IOException {
    ...
    List<HoodieFileStatus> topLevelStatuses = storage.listDirectEntries(markerDirPath);
    List<String> subDirectories = new ArrayList<>();
    for (HoodieFileStatus topLevelStatus: topLevelStatuses) {
      if (topLevelStatus.isFile()) {
        String pathStr = topLevelStatus.getLocation().toString();
        ...
        subDirectories.add(topLevelStatus.getLocation().toString());
      }
    }
  ...
  public Set<String> allMarkerFilePaths() throws IOException {
    ...
    if (doesMarkerDirExist()) {
      FSUtils.processFiles(storage, markerDirPath.toString(), fileStatus -> {
        markerFiles.add(MarkerUtils.stripMarkerFolderPrefix(fileStatus.getLocation().toString(), basePath, instantTime));
        return true;
      }, false);
    }
    return markerFiles;
   ...
   */
  public Option<HoodieLocation> create(String markerName) {
    return create(new HoodieLocation(markerDirPath, markerName), true);
  }

  @Override
  protected Option<HoodieLocation> create(String partitionPath, String fileName, IOType type, boolean checkIfExists) {
    return create(getMarkerPath(partitionPath, fileName, type), checkIfExists);
  }

  @Override
  public Option<HoodieLocation> createWithEarlyConflictDetection(String partitionPath, String dataFileName, IOType type, boolean checkIfExists,
  ...
  private Option<HoodieLocation> create(HoodieLocation markerPath, boolean checkIfExists) {
    HoodieTimer timer = HoodieTimer.start();
    HoodieLocation dirPath = markerPath.getParent();
    try {
