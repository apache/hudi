...
/**
  * Marker operations of directly accessing the file system to create and delete marker files. Each
  * data file has a corresponding marker file.
  */
public class DirectWriteMarkers extends WriteMarkers {
    ...
    public DirectWriteMarkers(
            HoodieStorage storage, String basePath, String markerFolderPath, String instantTime) {
    ...
    public Set<String> createdAndMergedDataPaths(HoodieEngineContext context, int parallelism)
        ...
        if (subDirectories.size() > 0) {
            ...
            context.setJobStatus(
                    this.getClass().getSimpleName(), "Obtaining marker files for all created, merged paths");
            dataFiles.addAll(
                    context.flatMap(
                            subDirectories,
                            directory -> {
                                ...
                                FileSystem fileSystem =
                                        HadoopFSUtils.getFs(path, storageConf.unwrapAs(Configuration.class));
    ...
    public Set<String> allMarkerFilePaths() throws IOException {
        ...
        if (doesMarkerDirExist()) {
            FSUtils.processFiles(
                    ...
                    fileStatus -> {
                        markerFiles.add(
                                MarkerUtils.stripMarkerFolderPrefix(
                                        fileStatus.getPath().toString(), basePath, instantTime));
                        return true;
                    },
                    false);
        }
