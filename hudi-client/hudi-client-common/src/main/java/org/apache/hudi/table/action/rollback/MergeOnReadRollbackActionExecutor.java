...
public class MergeOnReadRollbackActionExecutor<T, I, K, O>
        extends BaseRollbackActionExecutor<T, I, K, O> {

    private static final Logger LOG =
            LoggerFactory.getLogger(MergeOnReadRollbackActionExecutor.class);

    public MergeOnReadRollbackActionExecutor(
    ...
    protected List<HoodieRollbackStat> executeRollback(HoodieRollbackPlan hoodieRollbackPlan) {
        ...
        // At the moment, MOR table type does not support bulk nested rollbacks. Nested rollbacks is
        // an experimental
        // feature that is expensive. To perform nested rollbacks, initiate multiple requests of
        // client.rollback
        // (commitToRollback).
        // NOTE {@link HoodieCompactionConfig#withCompactionLazyBlockReadEnabled} needs to be set to
        // TRUE. This is
        // required to avoid OOM when merging multiple LogBlocks performed during nested rollbacks.

        // For Requested State (like failure during index lookup), there is nothing to do rollback
        // other than
