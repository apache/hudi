...
public final class RepairUtils {
    /**
      ...
      * @return A {@link Map} of instant time in {@link String} to a {@link List} of relative file
      *     paths.
      */
    public static Map<String, List<String>> tagInstantsOfBaseAndLogFiles(
        ...
        allPaths.forEach(
                path -> {
                    ...
                    instantToFilesMap
                            .get(instantTime)
                            .add(FSUtils.getRelativePartitionPath(new StoragePath(basePath), path));
                });
        return instantToFilesMap;
    }

    /**
      * Gets the base and log file paths written for a given instant from the timeline. This reads the
      * details of the instant metadata.
      *
      * @param timeline {@link HoodieTimeline} instance, can be active or archived timeline.
      * @param instant Instant for lookup.
      * @return A {@link Option} of {@link Set} of relative file paths to base path if the instant
      *     action is supported; empty {@link Option} otherwise.
    ...
    public static Option<Set<String>> getBaseAndLogFilePathsFromTimeline(
        ...
        if (!instant.isCompleted()) {
            throw new HoodieException(
                    "Cannot get base and log file paths from "
                            + "instant not completed: "
        ...
        switch (instant.getAction()) {
            ...
            case REPLACE_COMMIT_ACTION:
                ...
                return Option.of(
                        replaceCommitMetadata.getPartitionToWriteStats().values().stream()
                                .flatMap(List::stream)
                                .map(HoodieWriteStat::getPath)
                                .collect(Collectors.toSet()));
            default:
    ...
    public static List<String> findInstantFilesToRemove(
        ...
        if (!activeTimeline
                .filter(instant -> instant.getTimestamp().equals(instantToRepair) && !instant.isCompleted())
                .empty()) {
        ...
        try {
            ...
            Option<HoodieInstant> instantOption =
                    activeTimeline
                            ...
                            .filter(instant -> instant.getTimestamp().equals(instantToRepair))
                            .firstInstant();
            if (instantOption.isPresent()) {
                // Completed instant in active timeline
                doesInstantExist = true;
                filesFromTimeline =
                        RepairUtils.getBaseAndLogFilePathsFromTimeline(activeTimeline, instantOption.get());
            } else {
                instantOption =
                        archivedTimeline
                                .filterCompletedInstants()
                ...
                if (instantOption.isPresent()) {
                    ...
                    doesInstantExist = true;
                    filesFromTimeline =
                            RepairUtils.getBaseAndLogFilePathsFromTimeline(archivedTimeline, instantOption.get());
                }
            }
            ...
            if (doesInstantExist) {
                ...
                // The instant does not exist in the whole timeline (neither completed nor
                // requested/inflight),
