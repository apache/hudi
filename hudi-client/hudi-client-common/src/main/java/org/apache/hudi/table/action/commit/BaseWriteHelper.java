...
/**
 * Base class helps to execute write.
 *
...
public abstract class BaseWriteHelper<T extends HoodieRecordPayload, I, K, O, P> {
  ...
  private I tag(I dedupedRecords, HoodieEngineContext context, BaseHoodieTable<T, I, K, O, P> table) {
    // perform index loop up to get existing location of records
    return table.getIndex().tagLocation(dedupedRecords, context, table);
  }

  public I combineOnCondition(
      boolean condition, I records, int parallelism, BaseHoodieTable<T, I, K, O, P> table) {
   ...
   */
  public I deduplicateRecords(I records, BaseHoodieTable<T, I, K, O, P> table, int parallelism) {
    return deduplicateRecords(records, table.getIndex(), parallelism);
  }

  public abstract I deduplicateRecords(I records, HoodieIndex<T, I, K, O, P> index, int parallelism);
