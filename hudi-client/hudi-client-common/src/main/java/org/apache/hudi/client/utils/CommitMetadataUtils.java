...
public class CommitMetadataUtils {
   ...
   * We need to add these to CommitMetadata so that it will be synced to MDT and the commit metadata serialized to timeline is inline with whats in stoage.
   */
  public static Pair<HoodieCommitMetadata, List<HoodieWriteStat>> reconcileMetadataForAdditionalLogFilesOnStorage(HoodieTable table, String commitActionType, String instantTime,
    ...
    List<HoodieWriteStat> additionalWriteStats = new ArrayList<>();
      ...
      if (!logFilesMarkerPath.isEmpty()) {
        ...
        for (Pair<String, List<HoodieWriteStat>> partitionDeltaStats : additionalLogFileWriteStat) {
          ...
          partitionDeltaStats.getValue().forEach(ws ->  {
            ...
            additionalWriteStats.add(ws);
          });
        }
      }
    }
    return Pair.of(commitMetadata, additionalWriteStats);
  }
  ...
  private static HoodiePairData<String, Map<String, List<String>>> getPartitionToFileIdToMissingLogFileMap(String basePathStr, Set<String> logFilesMarkerPath, HoodieEngineContext context,
    ...
    HoodiePairData<String, Map<String, List<String>>> partitionPathToFileIdAndLogFileList = partitionPathLogFilePair
        ...
        .reduceByKey((SerializableBiFunction<List<String>, List<String>, List<String>>) (strings, strings2) -> {
          ...
          // for each hudi partition, collect list missing log files grouped by fileId and return the same.
