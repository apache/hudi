...
public abstract class BaseHoodieWriteClient<T, I, K, O> extends BaseHoodieClient
    ...
    public boolean commit(
            String instantTime, O writeStatuses, Option<Map<String, String>> extraMetadata) {
    ...
    public boolean commitStats(
        ...
        HoodieInstant inflightInstant =
                new HoodieInstant(State.INFLIGHT, commitActionType, instantTime);
        HeartbeatUtils.abortIfHeartbeatExpired(instantTime, table, heartbeatClient, config);
        this.txnManager.beginTransaction(
                Option.of(inflightInstant),
        ...
        try {
            ...
            throw new HoodieCommitException(
                    "Failed to complete commit " + config.getBasePath() + " at time " + instantTime, e);
        } finally {
        ...
        // We don't want to fail the commit if hoodie.fail.writes.on.inline.table.service.exception
        // is false. We catch warn if false
        try {
            // do this outside of lock since compaction, clustering can be time taking and we don't
            // need a lock for the entire execution period
            ...
            LOG.warn(
                    "Inline compaction or clustering failed with exception: "
                            + e.getMessage()
    ...
    private void saveInternalSchema(
        ...
        FileBasedInternalSchemaStorageManager schemasManager =
                new FileBasedInternalSchemaStorageManager(table.getMetaClient());
        if (!historySchemaStr.isEmpty()
            ...
            Schema avroSchema =
                    HoodieAvroUtils.createHoodieWriteSchema(
                            config.getSchema(), config.allowOperationMetadataField());
            if (historySchemaStr.isEmpty()) {
                internalSchema =
                        SerDeHelper.fromJson(config.getInternalSchema())
                                .orElseGet(() -> AvroInternalSchemaConverter.convert(avroSchema));
                internalSchema.setSchemaId(Long.parseLong(instantTime));
            } else {
                internalSchema =
                        InternalSchemaUtils.searchSchema(
                                Long.parseLong(instantTime), SerDeHelper.parseSchemas(historySchemaStr));
            }
            ...
            if (evolvedSchema.equals(internalSchema)) {
                ...
                schemasManager.persistHistorySchemaStr(
                        instantTime,
                        historySchemaStr.isEmpty()
                                ? SerDeHelper.inheritSchemas(evolvedSchema, "")
                                : historySchemaStr);
            } else {
                ...
                schemasManager.persistHistorySchemaStr(
                        instantTime, SerDeHelper.inheritSchemas(evolvedSchema, historySchemaStr));
            }
            // update SCHEMA_KEY
            metadata.addMetadata(
                    SCHEMA_KEY,
                    AvroInternalSchemaConverter.convert(evolvedSchema, avroSchema.getFullName()).toString());
        }
    }

    protected abstract HoodieTable<T, I, K, O> createTable(HoodieWriteConfig config);

    protected abstract HoodieTable<T, I, K, O> createTable(
            HoodieWriteConfig config, HoodieTableMetaClient metaClient);
    ...
    void emitCommitMetrics(String instantTime, HoodieCommitMetadata metadata, String actionType) {
        if (writeTimer != null) {
            ...
            // e.g. INIT_INSTANT_TS, METADATA_BOOTSTRAP_INSTANT_TS and FULL_BOOTSTRAP_INSTANT_TS in
            // HoodieTimeline
            HoodieActiveTimeline.parseDateFromInstantTimeSafely(instantTime)
                    .ifPresent(
                            parsedInstant ->
                                    metrics.updateCommitMetrics(
                                            parsedInstant.getTime(), durationInMs, metadata, actionType));
            writeTimer = null;
        }
    }
    ...
    protected void preCommit(HoodieInstant inflightInstant, HoodieCommitMetadata metadata) {
        ...
        // Important to create this after the lock to ensure the latest commits show up in the
        // timeline without need for reload
        HoodieTable table = createTable(config);
    ...
    /**
      ...
      *
      * <p>This implementation requires that the input records are already tagged, and de-duped if
      * needed.
      *
    ...
    /**
      ...
      * writes.
      *
      * <p>This implementation skips the index check and is able to leverage benefits such as small
    ...
    /**
      ...
      *
      * <p>This implementation skips the index check, skips de-duping and is able to leverage benefits
    ...
    /**
      ...
      * Hoodie).
      *
      * <p>This implementation uses sortBy (which does range partitioning based on reservoir sampling)
      * and attempts to control the numbers of files with less memory compared to the {@link
      * BaseHoodieWriteClient#insert(I, String)}
      *
      * @param records HoodieRecords to insert
      * @param instantTime Instant time of the commit
      * @return Collection of WriteStatus to inspect errors and counts
    ...
    /**
      ...
      * <p>This implementation uses sortBy (which does range partitioning based on reservoir sampling)
      * and attempts to control the numbers of files with less memory compared to the {@link
      * BaseHoodieWriteClient#insert(I, String)}. Optionally it allows users to specify their own
      * partitioner. If specified then it will be used for repartitioning records. See {@link
      * BulkInsertPartitioner}.
      *
      * @param records HoodieRecords to insert
      * @param instantTime Instant time of the commit
      * @param userDefinedBulkInsertPartitioner If specified then it will be used to partition input
      *     records before they are inserted into hoodie.
      * @return Collection of WriteStatus to inspect errors and counts
      */
    public abstract O bulkInsert(
            I records,
    ...
    /**
      * Deletes a list of {@link HoodieKey}s from the Hoodie table, at the supplied instantTime {@link
      * HoodieKey}s will be de-duped and non-existent keys will be removed before deleting.
      *
    ...
    /**
      ...
      * org.apache.hudi.common.model.HoodieRecordLocation} specified in preppedRecords.
      *
    ...
    public void preWrite(
            String instantTime, WriteOperationType writeOperationType, HoodieTableMetaClient metaClient) {
        setOperationType(writeOperationType);
        this.lastCompletedTxnAndMetadata =
                txnManager.isLockRequired()
        ...
        tableServiceClient.setPendingInflightAndRequestedInstants(
                this.pendingInflightAndRequestedInstants);
    ...
    public O postWrite(HoodieWriteMetadata<O> result, String instantTime, HoodieTable hoodieTable) {
        if (result.isCommitted()) {
            ...
            if (result.getFinalizeDuration().isPresent()) {
                metrics.updateFinalizeWriteMetrics(
                        result.getFinalizeDuration().get().toMillis(), result.getWriteStats().get().size());
            }
    ...
    protected void postCommit(
        ...
        try {
            context.setJobStatus(
                    ...
                    "Cleaning up marker directories for commit "
                            + instantTime
                            + " in table "
                            + config.getTableName());
    ...
    /**
      * Triggers cleaning and archival for the table of interest. This method is called outside of
      * locks. So, internal callers should ensure they acquire lock whereever applicable.
      *
    ...
    protected void runTableServicesInline(
            HoodieTable table, HoodieCommitMetadata metadata, Option<Map<String, String>> extraMetadata) {
    ...
    protected void autoCleanOnCommit() {
        ...
        if (config.isAsyncClean()) {
            ...
            // Do not reuse instantTime for clean as metadata table requires all changes to have
            // unique instant timestamps.
            clean();
        }
    }
    ...
    /**
      * Savepoint a specific commit instant time. Latest version of data files as of the passed in
      * instantTime will be referenced in the savepoint and will never be cleaned. The savepointed
      * commit will never be rolledback or archived.
      *
      * <p>This gives an option to rollback the state to the savepoint anytime. Savepoint needs to be
      * manually created and deleted.
      *
      * <p>Savepoint should be on a commit that could not have been cleaned.
      *
    ...
    public void restoreToSavepoint() {
        ...
        if (savePointTimeline.empty()) {
            throw new HoodieSavepointException(
                    "Could not restore to savepoint. Savepoint timeline is empty");
        }
    ...
    /**
      ...
      *
      * <p>WARNING: This rolls back recent commits and deleted data files and also pending compactions
      * after savepoint time. Queries accessing the files will mostly fail. This is expected to be a
      * manual operation and no concurrent write or compaction is expected to be running
      *
    ...
    public void restoreToSavepoint(String savepointTime) {
        ...
        if (initializeMetadataTableIfNecessary) {
            try {
                // Delete metadata table directly when users trigger savepoint rollback if mdt
                // existed and if the savePointTime is beforeTimelineStarts
                // or before the oldest compaction on MDT.
                // We cannot restore to before the oldest compaction on MDT as we don't have the
                // basefiles before that time.
                HoodieTableMetaClient mdtMetaClient =
                        HoodieTableMetaClient.builder()
                                .setConf(HadoopFSUtils.getStorageConfWithCopy(hadoopConf))
                                .setBasePath(getMetadataTableBasePath(config.getBasePath()))
                                .build();
                ...
                if (oldestMdtCompaction.isPresent()) {
                    if (HoodieTimeline.LESSER_THAN_OR_EQUALS.test(
                            savepointTime, oldestMdtCompaction.get().getTimestamp())) {
                        LOG.warn(
                                String.format(
                                        "Deleting MDT during restore to %s as the savepoint is older than oldest compaction %s on MDT",
                                        savepointTime, oldestMdtCompaction.get().getTimestamp()));
                        deleteMDT = true;
                    }
                }

                // The instant required to sync rollback to MDT has been archived and the mdt
                // syncing will be failed
                // So that we need to delete the whole MDT here.
                if (!deleteMDT) {
                    HoodieInstant syncedInstant =
                            new HoodieInstant(false, HoodieTimeline.DELTA_COMMIT_ACTION, savepointTime);
                    if (mdtMetaClient
                            .getCommitsTimeline()
                            .isBeforeTimelineStarts(syncedInstant.getTimestamp())) {
                        LOG.warn(
                                String.format(
                                        "Deleting MDT during restore to %s as the savepoint is older than the MDT timeline %s",
                                        savepointTime,
                ...
                if (deleteMDT) {
                    ...
                    // rollbackToSavepoint action will try to bootstrap MDT at first but sync to MDT
                    // will fail at the current scenario.
        ...
        HoodieTable<T, I, K, O> table =
                initTable(WriteOperationType.UNKNOWN, Option.empty(), initializeMetadataTableIfNecessary);
    ...
    public boolean rollback(final String commitInstantTime) throws HoodieRollbackException {
        ...
        Option<HoodiePendingRollbackInfo> pendingRollbackInfo =
                tableServiceClient.getPendingRollbackInfo(table.getMetaClient(), commitInstantTime);
        return tableServiceClient.rollback(commitInstantTime, pendingRollbackInfo, false);
    }

    @Deprecated
    public boolean rollback(final String commitInstantTime, String rollbackInstantTimestamp)
            throws HoodieRollbackException {
        HoodieTable<T, I, K, O> table = initTable(WriteOperationType.UNKNOWN, Option.empty());
        Option<HoodiePendingRollbackInfo> pendingRollbackInfo =
                tableServiceClient.getPendingRollbackInfo(table.getMetaClient(), commitInstantTime);
        return tableServiceClient.rollback(
                commitInstantTime, pendingRollbackInfo, rollbackInstantTimestamp, false);
    }

    /**
      * NOTE : This action requires all writers (ingest and compact) to a table to be stopped before
      * proceeding. Revert the (inflight/committed) record changes for all commits after the provided
      * instant time.
      *
      * @param savepointToRestoreTimestamp savepoint instant time to which restoration is requested
      */
    public HoodieRestoreMetadata restoreToInstant(
            final String savepointToRestoreTimestamp, boolean initialMetadataTableIfNecessary)
            throws HoodieRestoreException {
        LOG.info("Begin restore to instant " + savepointToRestoreTimestamp);
        Timer.Context timerContext = metrics.getRollbackCtx();
        try {
            HoodieTable<T, I, K, O> table =
                    initTable(WriteOperationType.UNKNOWN, Option.empty(), initialMetadataTableIfNecessary);
            Pair<String, Option<HoodieRestorePlan>> timestampAndRestorePlan =
                    scheduleAndGetRestorePlan(savepointToRestoreTimestamp, table);
            ...
            if (restorePlanOption.isPresent()) {
                ...
                throw new HoodieRestoreException(
                        "Failed to restore "
                                + config.getBasePath()
                                + " to commit "
    ...
    /**
      * Check if there is a failed restore with the same savepointToRestoreTimestamp. Reusing the
      * commit instead of creating a new one will prevent causing some issues with the metadata table.
      */
    private Pair<String, Option<HoodieRestorePlan>> scheduleAndGetRestorePlan(
            final String savepointToRestoreTimestamp, HoodieTable<T, I, K, O> table) throws IOException {
        ...
        if (failedRestore.isPresent()
                && savepointToRestoreTimestamp.equals(
                        RestoreUtils.getSavepointToRestoreTimestamp(table, failedRestore.get()))) {
            return Pair.of(
                    failedRestore.get().getTimestamp(),
                    Option.of(RestoreUtils.getRestorePlan(table.getMetaClient(), failedRestore.get())));
        }
        final String restoreInstantTimestamp = createNewInstantTime();
        return Pair.of(
                restoreInstantTimestamp,
                table.scheduleRestore(context, restoreInstantTimestamp, savepointToRestoreTimestamp));
    }
    ...
    /**
      ...
      */
    @Deprecated
    public HoodieCleanMetadata clean(String cleanInstantTime, boolean skipLocking)
    ...
    /**
      * Clean up any stale/old files/data lying around (either on file storage or index storage) based
      * on the configurations and CleaningPolicy used. (typically files that no longer can be used by a
      * running query can be cleaned). This API provides the flexibility to schedule clean instant
      * asynchronously via {@link BaseHoodieWriteClient#scheduleTableService(String, Option,
      * TableServiceType)} and disable inline scheduling of clean.
      *
      * @param cleanInstantTime instant time for clean.
    ...
    public HoodieCleanMetadata clean(
            String cleanInstantTime, boolean scheduleInline, boolean skipLocking)
            throws HoodieIOException {
    ...
    /**
      * Triggers clean for the table. This refers to Clean up any stale/old files/data lying around
      * (either on file storage or index storage) based on the * configurations and CleaningPolicy
      * used.
      *
      * @param skipLocking if this is triggered by another parent transaction, locking can be skipped.
      * @return instance of {@link HoodieCleanMetadata}.
    ...
    /**
      * Trigger archival for the table. This ensures that the number of commits do not explode and keep
      * increasing unbounded over time.
      *
      * @param table table to commit on.
      */
    ...
    /**
      ...
      * (insert/update/delete/insert_overwrite/insert_overwrite_table) with specified action.
      */
    public String startCommit(String actionType, HoodieTableMetaClient metaClient) {
        CleanerUtils.rollbackFailedWrites(
    ...
    /**
      * Provides a new commit time for a write operation
      * (insert/update/delete/insert_overwrite/insert_overwrite_table) without specified action.
      *
    ...
    /**
      ...
      */
    private void startCommitWithTime(
            String instantTime, String actionType, HoodieTableMetaClient metaClient) {
        CleanerUtils.rollbackFailedWrites(
                config.getFailedWritesCleanPolicy(),
                HoodieTimeline.COMMIT_ACTION,
                () -> tableServiceClient.rollbackFailedWrites());
    ...
    private void startCommit(
        ...
        ValidationUtils.checkArgument(
                inflightRestoreTimeline.countInstants() == 0,
                "Found pending restore in active timeline. Please complete the restore fully before proceeding. As of now, "
                        ...
                        + Arrays.toString(
                                inflightRestoreTimeline
                                        .getInstantsAsStream()
                                        .map(instant -> instant.getTimestamp())
        ...
        if (actionType.equals(HoodieTimeline.REPLACE_COMMIT_ACTION)) {
            ...
            metaClient
                    .getActiveTimeline()
                    .createNewInstant(
                            new HoodieInstant(HoodieInstant.State.REQUESTED, actionType, instantTime));
        }
    }
    ...
    /**
      ...
      */
    public Option<String> scheduleCompaction(Option<Map<String, String>> extraMetadata)
            throws HoodieIOException {
        String instantTime = createNewInstantTime();
    ...
    /**
      ...
      * @param extraMetadata Extra Metadata to be stored
      */
    public boolean scheduleCompactionAtInstant(
            String instantTime, Option<Map<String, String>> extraMetadata) throws HoodieIOException {
    ...
    /**
      ...
      */
    public Option<String> scheduleIndexing(
            List<MetadataPartitionType> partitionTypes, List<String> partitionPaths) {
        String instantTime = createNewInstantTime();
        Option<HoodieIndexPlan> indexPlan =
                createTable(config).scheduleIndexing(context, instantTime, partitionTypes, partitionPaths);
    ...
    public void dropIndex(List<MetadataPartitionType> partitionTypes) {
        ...
        HoodieInstant ownerInstant =
                new HoodieInstant(true, HoodieTimeline.INDEXING_ACTION, dropInstant);
        this.txnManager.beginTransaction(Option.of(ownerInstant), Option.empty());
        try {
            context.setJobStatus(
                    this.getClass().getSimpleName(),
                    "Dropping partitions from metadata table: " + config.getTableName());
    ...
    public HoodieWriteMetadata<O> cluster(String clusteringInstantTime) {
        if (shouldDelegateToTableServiceManager(config, ActionType.replacecommit)) {
            throw new UnsupportedOperationException(
                    "Clustering should be delegated to table service manager instead of direct run.");
        }
    ...
    public HoodieWriteMetadata<O> compact(String compactionInstantTime) {
        if (shouldDelegateToTableServiceManager(config, ActionType.compaction)) {
            throw new UnsupportedOperationException(
                    "Compaction should be delegated to table service manager instead of direct run.");
        }
        return compact(compactionInstantTime, config.shouldAutoCommit());
    }

    /**
      * Commit a compaction operation. Allow passing additional meta-data to be stored in commit
      * instant file.
      *
      * @param compactionInstantTime Compaction Instant Time
    ...
    /**
      ...
      * @param extraMetadata Extra Metadata to be stored
      */
    public Option<String> scheduleLogCompaction(Option<Map<String, String>> extraMetadata)
            throws HoodieIOException {
        String instantTime = createNewInstantTime();
        return scheduleLogCompactionAtInstant(instantTime, extraMetadata)
                ? Option.of(instantTime)
                : Option.empty();
    }
    ...
    /**
      ...
      * @param extraMetadata Extra Metadata to be stored
      */
    public boolean scheduleLogCompactionAtInstant(
            String instantTime, Option<Map<String, String>> extraMetadata) throws HoodieIOException {
        return scheduleTableService(instantTime, extraMetadata, TableServiceType.LOG_COMPACT)
                .isPresent();
    }
    ...
    /**
      * Commit a log compaction operation. Allow passing additional meta-data to be stored in commit
      * instant file.
      *
      * @param logCompactionInstantTime Log Compaction Instant Time
      * @param metadata All the metadata that gets stored along with a commit
      * @param extraMetadata Extra Metadata to be stored
      */
    public void commitLogCompaction(
    ...
    protected void completeLogCompaction(
            HoodieCommitMetadata metadata, HoodieTable table, String logCompactionCommitTime) {
        tableServiceClient.completeLogCompaction(metadata, table, logCompactionCommitTime);
    }

    /**
      * Ensures compaction instant is in expected state and performs Compaction for the workload stored
      * in instant-time.
      *
      * @param compactionInstantTime Compaction Instant Time
    ...
    /**
      * Ensures compaction instant is in expected state and performs Log Compaction for the workload
      * stored in instant-time.s
      *
      * @param logCompactionInstantTime Compaction Instant Time
      * @return Collection of Write Status
      */
    protected HoodieWriteMetadata<O> logCompact(
            String logCompactionInstantTime, boolean shouldComplete) {
        HoodieTable table = createTable(config);
    ...
    /**
      ...
      * @param extraMetadata Extra Metadata to be stored
      */
    public Option<String> scheduleClustering(Option<Map<String, String>> extraMetadata)
            throws HoodieIOException {
        String instantTime = createNewInstantTime();
        return scheduleClusteringAtInstant(instantTime, extraMetadata)
                ? Option.of(instantTime)
                : Option.empty();
    }
    ...
    /**
      ...
      * @param extraMetadata Extra Metadata to be stored
      */
    public boolean scheduleClusteringAtInstant(
            String instantTime, Option<Map<String, String>> extraMetadata) throws HoodieIOException {
    ...
    /**
      ...
      * @param extraMetadata Extra Metadata to be stored
      */
    protected boolean scheduleCleaningAtInstant(
            String instantTime, Option<Map<String, String>> extraMetadata) throws HoodieIOException {
        return scheduleTableService(instantTime, extraMetadata, TableServiceType.CLEAN).isPresent();
    }

    /**
      * Ensures clustering instant is in expected state and performs clustering for the plan stored in
      * metadata.
      *
    ...
    /**
      ...
      */
    public Option<String> scheduleTableService(
    ...
    /**
      * Performs necessary bootstrapping operations (for ex, validating whether Metadata Table has to
      * be bootstrapped).
      *
      * <p>NOTE: THIS OPERATION IS EXECUTED UNDER LOCK, THEREFORE SHOULD AVOID ANY OPERATIONS NOT
      * REQUIRING EXTERNAL SYNCHRONIZATION
      *
      * @param metaClient instance of {@link HoodieTableMetaClient}
      * @param instantTime current inflight instant time
      */
    protected void doInitTable(
    ...
    protected final HoodieTable initTable(
            WriteOperationType operationType, Option<String> instantTime, boolean initMetadataTable) {
    ...
    /**
      ...
      * operations such as:
      *
      * <p>NOTE: This method is engine-agnostic and SHOULD NOT be overloaded, please check on {@link
      * #doInitTable(WriteOperationType, HoodieTableMetaClient, Option)} instead
      *
      * <ul>
      *   <li>Checking whether upgrade/downgrade is required
      *   <li>Bootstrapping Metadata Table (if required)
      *   <li>Initializing metrics contexts
      * </ul>
      */
    ...
    public void close() {
        ...
        super.close();
        // Calling this here releases any resources used by your index, so make sure to finish any
        // related operations
        // before this point
        this.index.close();
    ...
    /**
      * Upgrades the hoodie table if need be when moving to a new Hudi version. This method is called
      * within a lock. Try to avoid double locking from within this method.
      *
    ...
    protected void tryUpgrade(HoodieTableMetaClient metaClient, Option<String> instantTime) {
        ...
        if (upgradeDowngrade.needsUpgradeOrDowngrade(HoodieTableVersion.current())) {
            ...
            List<String> instantsToRollback =
                    tableServiceClient.getInstantsToRollback(
                            metaClient, HoodieFailedWritesCleaningPolicy.EAGER, instantTime);
    ...
    /**
      ...
      *
      * @param colName col name to be added. if we want to add col to a nested filed, the fullName
      *     should be specified
      * @param schema col type to be added.
      * @param doc col doc to be added.
      * @param position col position to be added
      * @param positionType col position change type. now support three change types:
      *     first/after/before
      */
    public void addColumn(
        ...
        InternalSchema newSchema =
                new InternalSchemaChangeApplier(pair.getLeft())
                        .applyAddChange(
                                colName,
                                AvroInternalSchemaConverter.convertToField(schema),
                                doc,
                                position,
                                positionType);
    ...
    public void addColumn(String colName, Schema schema) {
        addColumn(
                ...
                null,
                "",
                TableChange.ColumnPositionChange.ColumnPositionType.NO_OPERATION);
    }

    /**
      * delete columns to table.
      *
      * @param colNames col name to be deleted. if we want to delete col from a nested filed, the
      *     fullName should be specified
      */
    public void deleteColumns(String... colNames) {
        Pair<InternalSchema, HoodieTableMetaClient> pair = getInternalSchemaAndMetaClient();
        InternalSchema newSchema =
                new InternalSchemaChangeApplier(pair.getLeft()).applyDeleteChange(colNames);
        commitTableChange(newSchema, pair.getRight());
    }
    ...
    /**
      ...
      *
      * @param colName col name to be renamed. if we want to rename col from a nested filed, the
      *     fullName should be specified
    ...
    public void renameColumn(String colName, String newName) {
        Pair<InternalSchema, HoodieTableMetaClient> pair = getInternalSchemaAndMetaClient();
        InternalSchema newSchema =
                new InternalSchemaChangeApplier(pair.getLeft()).applyRenameChange(colName, newName);
        commitTableChange(newSchema, pair.getRight());
    }
    ...
    /**
      ...
      *
      * @param colName col name to be changed. if we want to change col from a nested filed, the
      *     fullName should be specified
      * @param nullable .
      */
    public void updateColumnNullability(String colName, boolean nullable) {
        ...
        InternalSchema newSchema =
                new InternalSchemaChangeApplier(pair.getLeft())
                        .applyColumnNullabilityChange(colName, nullable);
        commitTableChange(newSchema, pair.getRight());
    }

    /**
      * update col Type for hudi table. only support update primitive type to primitive type. cannot
      * update nest type to nest type or primitive type eg: RecordType -> MapType, MapType -> LongType.
      *
      * @param colName col name to be changed. if we want to change col from a nested filed, the
      *     fullName should be specified
      * @param newType .
    ...
    public void updateColumnType(String colName, Type newType) {
        Pair<InternalSchema, HoodieTableMetaClient> pair = getInternalSchemaAndMetaClient();
        InternalSchema newSchema =
                new InternalSchemaChangeApplier(pair.getLeft()).applyColumnTypeChange(colName, newType);
        commitTableChange(newSchema, pair.getRight());
    }
    ...
    public void updateColumnComment(String colName, String doc) {
        Pair<InternalSchema, HoodieTableMetaClient> pair = getInternalSchemaAndMetaClient();
        InternalSchema newSchema =
                new InternalSchemaChangeApplier(pair.getLeft()).applyColumnCommentChange(colName, doc);
        commitTableChange(newSchema, pair.getRight());
    }
    ...
    /**
      ...
      *
      * @param colName column which need to be reordered. if we want to change col from a nested filed,
      *     the fullName should be specified.
      ...
      */
    public void reOrderColPosition(
            String colName,
            String referColName,
    ...
    public void commitTableChange(InternalSchema newSchema, HoodieTableMetaClient metaClient) {
        ...
        String historySchemaStr =
                schemaUtil
                        ...
                        .orElseGet(() -> SerDeHelper.inheritSchemas(getInternalSchema(schemaUtil), ""));
        Schema schema =
                AvroInternalSchemaConverter.convert(
                        newSchema, getAvroRecordQualifiedName(config.getTableName()));
        String commitActionType =
        ...
        try {
            ...
            throw new HoodieCommitException(
                    "Failed to commit " + instantTime + " unable to save inflight metadata ", io);
        }
        Map<String, String> extraMeta = new HashMap<>();
        extraMeta.put(
                SerDeHelper.LATEST_SCHEMA,
                SerDeHelper.toJson(newSchema.setSchemaId(Long.parseLong(instantTime))));
        // try to save history schemas
        FileBasedInternalSchemaStorageManager schemasManager =
                new FileBasedInternalSchemaStorageManager(metaClient);
        schemasManager.persistHistorySchemaStr(
                instantTime, SerDeHelper.inheritSchemas(newSchema, historySchemaStr));
        commitStats(
                instantTime,
                        ...
                        () -> {
                            try {
                                ...
                                throw new HoodieException(
                                        String.format(
                                                "cannot find schema for current table: %s", config.getBasePath()));
                            }
                        });
    }
}
