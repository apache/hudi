...
public abstract class HoodieWriteHandle<T, I, K, O> extends HoodieIOHandle<T, I, K, O> {
  ...
  protected class AppendLogWriteCallback implements HoodieLogFileWriteCallback {
    // we will create marker file for each log file and include all log files to commit message
    // so that all log files generated in this job will be included in commit metadata. This is important for MDT.
    // Assuming if spark task failed, the log file generated by it will not appear in WriteStatus
