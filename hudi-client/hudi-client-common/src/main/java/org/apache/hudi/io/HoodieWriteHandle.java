...
public abstract class HoodieWriteHandle<T, I, K, O> extends HoodieIOHandle<T, I, K, O> {
  ...
  public HoodieLocation makeNewPath(String partitionPath) {
    HoodieLocation path = FSUtils.getPartitionPath(config.getBasePath(), partitionPath);
    try {
    ...
    return new HoodieLocation(path.toString(),
   ...
   */
  protected HoodieLocation makeNewFilePath(String partitionPath, String fileName) {
    String relativePath = new HoodieLocation((partitionPath.isEmpty() ? "" : partitionPath + "/")
        + fileName).toString();
    return new HoodieLocation(config.getBasePath(), relativePath);
  }
