...
public abstract class HoodieWriteHandle<T, I, K, O> extends HoodieIOHandle<T, I, K, O> {
    ...
    protected StoragePath makeNewFilePath(String partitionPath, String fileName) {
        String relativePath =
                new StoragePath((partitionPath.isEmpty() ? "" : partitionPath + "/") + fileName).toString();
    ...
    /**
      ...
      *
      * <p>- Whether it belongs to the same partitionPath as existing records - Whether the current
      * file written bytes lt max file size
      */
    ...
    protected HoodieLogFormat.Writer createLogWriter(String deltaCommitTime, String fileSuffix) {
        try {
            ...
            throw new HoodieException(
                    "Creating logger writer with fileId: "
                            + fileId
                            + ", "
                            + "delta commit time: "
                            + deltaCommitTime
    ...
    protected static Option<IndexedRecord> toAvroRecord(
            HoodieRecord record, Schema writerSchema, TypedProperties props) {
        try {
