...
public class LegacyArchivedMetaEntryReader {
  ...
  private ClosableIterator<ActiveAction> loadInstants(HoodieArchivedTimeline.TimeRangeFilter filter) {
    try {
      // List all files
      List<StoragePathInfo> fsStatuses = metaClient.getHoodieStorage().globEntries(
  ...
  private ClosableIterator<HoodieRecord<IndexedRecord>> getRecordIterator(
      List<StoragePathInfo> fsStatuses) throws IOException {
    return new ClosableIterator<HoodieRecord<IndexedRecord>>() {

      final Iterator<StoragePathInfo> fsItr = fsStatuses.iterator();
      ...
      public boolean hasNext() {
        ...
        // new reader
        while (fsItr.hasNext()) {
          StoragePathInfo fileStatus = fsItr.next();
          try {
            reader = HoodieLogFormat.newReader(
                metaClient.getHoodieStorage(),
                new HoodieLogFile(fileStatus.getPath()),
                HoodieArchivedMetaEntry.getClassSchema());
          } catch (IOException ioe) {
            throw new HoodieIOException(
                "Error initializing the reader for archived log: " + fileStatus.getPath(), ioe);
          }
          while (reader.hasNext()) {
            HoodieLogBlock block = reader.next();
            if (block instanceof HoodieAvroDataBlock) {
          ...
          if (!reader.hasNext()) {
            try {
              ...
              throw new HoodieIOException("Failed to close log reader " + fileStatus.getPath());
            }
          }
        }
        return false;
