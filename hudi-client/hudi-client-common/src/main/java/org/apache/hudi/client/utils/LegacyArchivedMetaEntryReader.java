...
public class LegacyArchivedMetaEntryReader {
    ...
    private Pair<HoodieInstant, Option<byte[]>> readInstant(GenericRecord record) {
        ...
        HoodieInstant instant =
                new HoodieInstant(
                        HoodieInstant.State.valueOf(record.get(ACTION_STATE).toString()),
                        action,
                        instantTime,
                        stateTransitionTime);
    ...
    /**
      * This is method to read selected instants. Do NOT use this directly use one of the helper
      * methods above If loadInstantDetails is set to true, this would also update 'readCommits' map
      * with commit details If filter is specified, only the filtered instants are loaded If
      * commitsFilter is specified, only the filtered records are loaded.
      */
    private ClosableIterator<ActiveAction> loadInstants(
        ...
        try {
            ...
            List<StoragePathInfo> pathInfoList =
                    metaClient
                            .getStorage()
                            .globEntries(new StoragePath(metaClient.getArchivePath(), ".commits_.archive*"));
            ...
            return new ClosableIterator<ActiveAction>() {
                ...
                public boolean hasNext() {
                    ...
                    while (itr.hasNext()) {
                        ...
                        Pair<HoodieInstant, Option<byte[]>> instantAndDetail =
                                readInstant((GenericRecord) record.getData());
