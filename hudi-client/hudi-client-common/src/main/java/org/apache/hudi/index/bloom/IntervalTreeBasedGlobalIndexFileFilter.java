...
/**
  * Interval Tree based index look up for Global Index. Builds an {@link KeyRangeLookupTree} for all
  * index files (across all partitions) and uses it to search for matching index files for any given
  * recordKey that needs to be looked up.
  */
class IntervalTreeBasedGlobalIndexFileFilter implements IndexFileFilter {
    ...
    IntervalTreeBasedGlobalIndexFileFilter(
        ...
        partitionToFileIndexInfo.forEach(
                (partition, bloomIndexFileInfoList) ->
                        bloomIndexFileInfoList.forEach(
                                file -> {
                                    fileIdToPartitionPathMap.put(file.getFileId(), partition);
                                    allIndexFiles.add(file);
                                }));

        // Note that the interval tree implementation doesn't have auto-balancing to ensure logN
        // search time.
        // So, we are shuffling the input here hoping the tree will not have any skewness. If not,
        // the tree could be skewed
        // which could result in N search time instead of NlogN.
        Collections.shuffle(allIndexFiles);
        allIndexFiles.forEach(
                indexFile -> {
                    if (indexFile.hasKeyRanges()) {
                        indexLookUpTree.insert(
                                new KeyRangeNode(
                                        indexFile.getMinRecordKey(),
                                        indexFile.getMaxRecordKey(),
                                        indexFile.getFileId()));
                    } else {
    ...
    @Override
    public Set<Pair<String, String>> getMatchingFilesAndPartition(
            String partitionPath, String recordKey) {
