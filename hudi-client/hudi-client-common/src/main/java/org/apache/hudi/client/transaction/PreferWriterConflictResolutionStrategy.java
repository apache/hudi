...
/**
  * This class extends the base implementation of conflict resolution strategy. It gives preference
  * to non-blocking ingestion over table services in case of conflicts.
  */
public class PreferWriterConflictResolutionStrategy
    ...
    /**
      * For tableservices like replacecommit and compaction commits this method also returns ingestion
      * inflight commits.
      */
    @Override
    ...
    private Stream<HoodieInstant> getCandidateInstantsForNonTableServicesCommits(
        ...
        List<HoodieInstant> completedCommitsInstants =
                activeTimeline
                        .getTimelineOfActions(
                                CollectionUtils.createSet(
                                        COMMIT_ACTION, REPLACE_COMMIT_ACTION, COMPACTION_ACTION, DELTA_COMMIT_ACTION))
        ...
        LOG.info(
                String.format(
                        "Instants that may have conflict with %s are %s",
                        currentInstant, completedCommitsInstants));
        return completedCommitsInstants.stream();
    }

    /**
      * To find which instants are conflicting, we apply the following logic Get both completed
      * instants and ingestion inflight commits that have happened since the last successful write. We
      * need to check for write conflicts since they may have mutated the same files that are being
      * newly created by the current write.
      */
    private Stream<HoodieInstant> getCandidateInstantsForTableServicesCommits(
        ...
        LOG.info(
                String.format(
                        "Instants that may have conflict with %s are %s", currentInstant, instantsToConsider));
