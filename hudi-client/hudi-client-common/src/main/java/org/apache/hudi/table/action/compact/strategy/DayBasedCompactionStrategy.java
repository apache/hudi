...
public class DayBasedCompactionStrategy extends CompactionStrategy {
    ...
    protected static Comparator<String> comparator =
            (String leftPartition, String rightPartition) -> {
                try {
    ...
    public List<String> filterPartitionPaths(
        ...
        return allPartitionPaths.stream()
                ...
                .subList(
                        0,
                        Math.min(
                                allPartitionPaths.size(), writeConfig.getTargetPartitionsPerDayBasedCompaction()));
    }

    /**
      * If is Hive style partition path, convert it to regular partition path. e.g.
      * year=2019/month=11/day=24 => 2019/11/24
      */
