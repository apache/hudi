...
public abstract class UniformBootstrapModeSelector extends BootstrapModeSelector {
    ...
    public UniformBootstrapModeSelector(
            HoodieWriteConfig bootstrapConfig, BootstrapMode bootstrapMode) {
    ...
    public Map<BootstrapMode, List<String>> select(
        ...
        return partitions.stream()
                .map(p -> Pair.of(bootstrapMode, p))
                .collect(
                        Collectors.groupingBy(
                                Pair::getKey, Collectors.mapping(x -> x.getValue().getKey(), Collectors.toList())));
    }
}
