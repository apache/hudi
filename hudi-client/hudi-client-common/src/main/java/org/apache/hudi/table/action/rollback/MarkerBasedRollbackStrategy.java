...
public class MarkerBasedRollbackStrategy<T, I, K, O>
    ...
    public MarkerBasedRollbackStrategy(
            HoodieTable<?, ?, ?, ?> table,
    ...
    public List<HoodieRollbackRequest> getRollbackRequests(HoodieInstant instantToRollback) {
        try {
            List<String> markerPaths =
                    MarkerBasedRollbackUtils.getAllMarkerPaths(
            ...
            return context.map(
                    ...
                    markerFilePath -> {
                        ...
                        String partitionPath =
                                HadoopFSUtils.getRelativePartitionPath(new Path(basePath), filePath.getParent());
                        ...
                        switch (type) {
                            ...
                            default:
                                throw new HoodieRollbackException(
                                        "Unknown marker type, during rollback of " + instantToRollback);
                        }
                    },
                    parallelism);
        } catch (Exception e) {
            throw new HoodieRollbackException(
                    "Error rolling back using marker files written for " + instantToRollback, e);
        }
    }
}
