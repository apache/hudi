...
public abstract class HoodieBackedTableMetadataWriter implements HoodieTableMetadataWriter {
  ...
  protected <T extends SpecificRecordBase> HoodieBackedTableMetadataWriter(Configuration hadoopConf,
    ...
    if (dataMetaClient.getTableConfig().isMetadataTableEnabled() || writeConfig.isMetadataTableEnabled()) {
      ...
      this.metadataWriteConfig = HoodieTableMetadataUtil.createMetadataWriteConfig(writeConfig, tableName);
      enabled = true;
  ...
  private void enablePartitions() {
    ...
    if (metadataConfig.isRecordIndexEnabled()) {
      enablePartition(MetadataPartitionType.RECORD_INDEX, metadataConfig, metaClient, fsView, isBootstrapCompleted);
    }
  }
  ...
  protected <T extends SpecificRecordBase> void initializeIfNeeded(HoodieTableMetaClient dataMetaClient,
    ...
    if (!dataWriteConfig.isMetadataAsyncIndex()) {
      ...
      initializeEnabledFileGroups(metadataMetaClient, metadataWriteConfig, createInstantTime, partitionsToInit);
  ...
  private boolean initializeFromFilesystem(HoodieTableMetaClient dataMetaClient,
    ...
    initializeEnabledFileGroups(metadataMetaClient, metadataWriteConfig, createInstantTime, enabledPartitionTypes);
  ...
  private HoodieTableMetaClient initializeMetaClient(boolean populateMetaFields) throws IOException {
    return HoodieTableMetaClient.withPropertyBuilder()
        ...
        .setRecordKeyFields(HoodieMetadataCommonUtils.RECORD_KEY_FIELD_NAME)
   ...
   * @throws IOException
   */
  private void initializeEnabledFileGroups(HoodieTableMetaClient metadataMetaClient, HoodieWriteConfig metadataWriteConfig, String createInstantTime,
                                           List<MetadataPartitionType> partitionTypes) throws IOException {
    for (MetadataPartitionType enabledPartitionType : partitionTypes) {
      if (enabledPartitionType != MetadataPartitionType.RECORD_INDEX) { // record index file group initialization happens along w/ record
        // generation for first time
        HoodieTableMetadataUtil.initializeFileGroups(metadataMetaClient, metadataWriteConfig, enabledPartitionType, createInstantTime,
   ...
   */
  private void initialCommit(String createInstantTime, List<MetadataPartitionType> partitionTypes) throws IOException {
    ...
    Map<String, Map<String, Long>> partitionToFilesMap = partitionInfoList.stream()
        .map(p -> {
          String partitionName = HoodieMetadataCommonUtils.getPartitionIdentifier(p.getRelativePath());
    ...
    if (partitionTypes.contains(MetadataPartitionType.RECORD_INDEX) && totalDataFilesCount > 0) {
  ...
  private HoodieData<HoodieRecord> initializeFilesPartitionRecordsFromFiles(String createInstantTime, List<DirectoryInfo> partitionInfoList, HoodieRecord allPartitionRecord) {
    ...
    HoodieData<HoodieRecord> fileListRecords = engineContext.parallelize(partitionInfoList, partitionInfoList.size()).map(partitionInfo -> {
      ...
      return HoodieMetadataPayload.createPartitionFilesRecord(
          HoodieMetadataCommonUtils.getPartitionIdentifier(partitionInfo.getRelativePath()), Option.of(validFileNameToSizeMap), Option.empty());
    });
