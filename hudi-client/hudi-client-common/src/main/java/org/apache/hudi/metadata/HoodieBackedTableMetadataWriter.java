...
/**
  * Writer implementation backed by an internal hudi table. Partition and file listing are saved
  * within an internal MOR table called Metadata Table. This table is created by listing files and
  * partitions (first time) and kept in sync using the instants on the main dataset.
  *
...
public abstract class HoodieBackedTableMetadataWriter<I> implements HoodieTableMetadataWriter {
    ...
    // Record index has a fixed size schema. This has been calculated based on experiments with
    // default settings
    ...
    /**
      ...
      */
    protected HoodieBackedTableMetadataWriter(
        ...
        ValidationUtils.checkArgument(
                !initialized || this.metadata != null,
                "MDT Reader should have been opened post initialization");
    }
    ...
    protected boolean initializeIfNeeded(
            HoodieTableMetaClient dataMetaClient, Option<String> inflightInstantTimestamp)
            throws IOException {
        HoodieTimer timer = HoodieTimer.start();
        List<MetadataPartitionType> partitionsToInit =
                new ArrayList<>(MetadataPartitionType.values().length);
        ...
        try {
            ...
            // NOTE: It needs to be guarded by async index config because if that is enabled then
            // initialization happens through the index scheduler.
            if (!dataWriteConfig.isMetadataAsyncIndex()) {
                ...
                LOG.info(
                        "Async metadata indexing disabled and following partitions already initialized: {}",
                        completedPartitions);
            ...
            if (partitionsToInit.isEmpty()) {
                // No partitions left to initialize, since all the metadata enabled partitions are
                // either initialized before
                // or current in the process of initialization.
                initMetadataReader();
                return true;
            }

            // If there is no commit on the dataset yet, use the SOLO_COMMIT_TIMESTAMP as the
            // instant time for initial commit
            ...
            if (!initializeFromFilesystem(
                    initializationTime, partitionsToInit, inflightInstantTimestamp)) {
        ...
        if (exists) {
            try {
                ...
                if (DEFAULT_METADATA_POPULATE_META_FIELDS
                    ...
                    LOG.info(
                            "Re-initiating metadata table properties since populate meta fields have changed");
    ...
    /**
      ...
      *
      * <p>Rollback of the first commit would look like un-synced instants in the metadata table.
      * Action metadata is needed to verify the instant time and avoid erroneous initializing.
      *
      * <p>TODO: Revisit this logic and validate that filtering for all commits timeline is the right
      * thing to do
      *
    ...
    private boolean initializeFromFilesystem(
        ...
        if (!filesPartitionAvailable) {
            ...
            // Check and then open the metadata table reader so FILES partition can be read during
            // initialization of other partitions
            initMetadataReader();
        ...
        partitionsToInit.removeIf(
                metadataPartition ->
                        dataMetaClient.getTableConfig().isMetadataPartitionAvailable((metadataPartition)));

        // Get a complete list of files and partitions from the file system or from already
        // initialized FILES partition of MDT
        List<DirectoryInfo> partitionInfoList;
        if (filesPartitionAvailable) {
            partitionInfoList = listAllPartitionsFromMDT(initializationTime, pendingDataInstants);
        } else {
            // if auto initialization is enabled, then we need to list all partitions from the file
            // system
                        ...
                        .map(
                                p -> {
                                    String partitionName =
                                            HoodieTableMetadataUtil.getPartitionIdentifierForFilesPartition(
                                                    p.getRelativePath());
        ...
        for (MetadataPartitionType partitionType : partitionsToInit) {
            // Find the commit timestamp to use for this partition. Each initialization should use
            // its own unique commit time.
            ...
            LOG.info(
                    "Initializing MDT partition {} at instant {}", partitionTypeName, commitTimeForPartition);
                ...
                switch (partitionType) {
                    ...
                    case FUNCTIONAL_INDEX:
                        ...
                        ValidationUtils.checkState(
                                functionalIndexPartitionsToInit.size() == 1,
                                "Only one functional index at a time is supported for now");
                        fileGroupCountAndRecordsPair =
                                initializeFunctionalIndexPartition(
                                        functionalIndexPartitionsToInit.iterator().next());
                        break;
                    case PARTITION_STATS:
                        if (dataWriteConfig.getColumnsEnabledForColumnStatsIndex().isEmpty()) {
                            LOG.warn(
                                    "Skipping partition stats index initialization as target columns are not set");
                            continue;
                        }
                    ...
                    case SECONDARY_INDEX:
                        ...
                        if (secondaryIndexPartitionsToInit.size() != 1) {
                            LOG.warn(
                                    "Skipping secondary index initialization as only one secondary index bootstrap at a time is supported for now. Provided: {}",
                    ...
                    default:
                        throw new HoodieMetadataException(
                                String.format("Unsupported MDT partition type: %s", partitionType));
                }
            } catch (Exception e) {
                String metricKey =
                        partitionType.getPartitionPath() + "_" + HoodieMetadataMetrics.BOOTSTRAP_ERR_STR;
                metrics.ifPresent(m -> m.setMetric(metricKey, 1));
                String errMsg =
                        String.format(
                                "Bootstrap on %s partition failed for %s",
            ...
            if (LOG.isInfoEnabled()) {
                LOG.info(
                        "Initializing {} index with {} mappings and {} file groups.",
                        partitionTypeName,
            ...
            ValidationUtils.checkArgument(
                    fileGroupCount > 0,
                    "FileGroup count for MDT partition " + partitionTypeName + " should be > 0");
            ...
            String partitionPath =
                    (partitionType == FUNCTIONAL_INDEX || partitionType == SECONDARY_INDEX)
                            ? dataWriteConfig.getIndexingConfig().getIndexName()
                            : partitionType.getPartitionPath();

            dataMetaClient
                    .getTableConfig()
                    .setMetadataPartitionState(dataMetaClient, partitionPath, true);
            // initialize the metadata reader again so the MDT partition can be read after
            // initialization
            initMetadataReader();
            long totalInitTime = partitionInitTimer.endTimer();
            LOG.info(
                    "Initializing {} index in metadata table took {} in ms",
                    partitionTypeName,
                    totalInitTime);
        }
    ...
    /**
      ...
      *
      * <p>Since commits are immutable, we should use unique timestamps to initialize each partition.
      * For this, we will add a suffix to the given initializationTime until we find a unique
      * timestamp.
      *
    ...
    private String generateUniqueCommitInstantTime(String initializationTime) {
        ...
        // This function would be called multiple times in a single application if multiple indexes
        // are being
    ...
    private Pair<Integer, HoodieData<HoodieRecord>> initializePartitionStatsIndex(
        ...
        HoodieData<HoodieRecord> records =
                HoodieTableMetadataUtil.convertFilesToPartitionStatsRecords(
                        engineContext, partitionInfoList, dataWriteConfig.getMetadataConfig(), dataMetaClient);
        final int fileGroupCount =
                dataWriteConfig.getMetadataConfig().getPartitionStatsIndexFileGroupCount();
        return Pair.of(fileGroupCount, records);
    }

    private Pair<Integer, HoodieData<HoodieRecord>> initializeColumnStatsPartition(
            Map<String, Map<String, Long>> partitionToFilesMap) {
        HoodieData<HoodieRecord> records =
    ...
    private Pair<Integer, HoodieData<HoodieRecord>> initializeBloomFiltersPartition(
            String createInstantTime, Map<String, Map<String, Long>> partitionToFilesMap) {
        HoodieData<HoodieRecord> records =
    ...
    protected abstract HoodieData<HoodieRecord> getFunctionalIndexRecords(
    ...
    public abstract HoodieData<HoodieRecord> getDeletedSecondaryRecordMapping(
            HoodieEngineContext engineContext,
            Map<String, String> recordKeySecondaryKeyMap,
            HoodieIndexDefinition indexDefinition);

    private Pair<Integer, HoodieData<HoodieRecord>> initializeFunctionalIndexPartition(
            String indexName) throws Exception {
        HoodieIndexDefinition indexDefinition = getFunctionalIndexDefinition(indexName);
        ValidationUtils.checkState(
                indexDefinition != null,
                "Functional Index definition is not present for index " + indexName);
                        ...
                        .filter(
                                indexName ->
                                        indexName.startsWith(
                                                HoodieTableMetadataUtil.PARTITION_NAME_SECONDARY_INDEX_PREFIX))
    ...
    private Pair<Integer, HoodieData<HoodieRecord>> initializeSecondaryIndexPartition(
        ...
        ValidationUtils.checkState(
                indexDefinition != null,
                "Secondary Index definition is not present for index " + indexName);
        List<Pair<String, FileSlice>> partitionFileSlicePairs = getPartitionFileSlicePairs();
        ...
        HoodieData<HoodieRecord> records =
                readSecondaryKeysFromFileSlices(
                        engineContext,
                        partitionFileSlicePairs,
                        parallelism,
    ...
    private List<Pair<String, FileSlice>> getPartitionFileSlicePairs() throws IOException {
        HoodieMetadataFileSystemView fsView =
                new HoodieMetadataFileSystemView(
                        dataMetaClient, dataMetaClient.getActiveTimeline(), metadata);
    ...
    private Pair<Integer, HoodieData<HoodieRecord>> initializeRecordIndexPartition()
            throws IOException {
        final HoodieMetadataFileSystemView fsView =
                new HoodieMetadataFileSystemView(
                        dataMetaClient, dataMetaClient.getActiveTimeline(), metadata);
        ...
        if (dataMetaClient.getTableConfig().getTableType() == HoodieTableType.COPY_ON_WRITE) {
            ...
            for (String partition : partitions) {
                partitionBaseFilePairs.addAll(
                        fsView
                                .getLatestBaseFiles(partition)
                                .map(basefile -> Pair.of(partition, basefile))
                                .collect(Collectors.toList()));
            }

            LOG.info(
                    "Initializing record index from "
                            + partitionBaseFilePairs.size()
                            + " base files in "
                            + partitions.size()
                            + " partitions");
        ...
        LOG.info(
                "Initializing record index with {} mappings and {} file groups.",
                recordCount,
                fileGroupCount);
        return Pair.of(fileGroupCount, records);
    }
        ...
        return engineContext
                ...
                .flatMap(
                        partitionAndFileSlice -> {
                            ...
                            return new HoodieMergedReadHandle(
                                            dataWriteConfig,
                                            instantTime,
                                            hoodieTable,
                                            Pair.of(partition, fileSlice.getFileId()),
                                            Option.of(fileSlice))
                                    .getMergedRecords().stream()
                                            .map(
                                                    record -> {
                                                        ...
                                                        return HoodieMetadataPayload.createRecordIndexUpdate(
                                                                ...
                                                                partition,
                                                                fileId,
                                                                record1.getCurrentLocation().getInstantTime(),
                                                                0);
                                                    })
                                            .iterator();
                        });
    ...
    private Pair<Integer, HoodieData<HoodieRecord>> initializeFilesPartition(
            List<DirectoryInfo> partitionInfoList) {
        ...
        LOG.info(
                "Committing total {} partitions and {} files to metadata",
                partitions.size(),
                totalDataFilesCount);
        ...
        engineContext.setJobStatus(
                this.getClass().getSimpleName(), "Creating records for metadata FILES partition");
        HoodieData<HoodieRecord> fileListRecords =
                engineContext
                        .parallelize(partitionInfoList, partitionInfoList.size())
                        .map(
                                partitionInfo -> {
    ...
    /**
      ...
      */
    private List<DirectoryInfo> listAllPartitionsFromFilesystem(
            String initializationTime, Set<String> pendingDataInstants) {
        ...
        while (!pathsToList.isEmpty()) {
            ...
            engineContext.setJobStatus(
                    this.getClass().getSimpleName(),
                    "Listing " + numDirsToList + " partitions from filesystem");
            ...
            // If the listing reveals a directory, add it to queue. If the listing reveals a hoodie
            // partition, add it to
            // the results.
            for (DirectoryInfo dirInfo : processedDirectories) {
                if (!dirFilterRegex.isEmpty()) {
                    ...
                    if (!relativePath.isEmpty() && relativePath.matches(dirFilterRegex)) {
                        LOG.info(
                                "Ignoring directory {} which matches the filter regex {}",
                                relativePath,
                                dirFilterRegex);
                        continue;
                    }
    ...
    /**
      ...
      * @return List consisting of {@code DirectoryInfo} for each partition found.
      */
    private List<DirectoryInfo> listAllPartitionsFromMDT(
        ...
        List<String> allPartitionPaths =
                metadata.getAllPartitionPaths().stream()
                        .map(
                                partitionPath ->
                                        dataWriteConfig.getBasePath() + StoragePath.SEPARATOR_CHAR + partitionPath)
                        .collect(Collectors.toList());
        ...
        for (Map.Entry<String, List<StoragePathInfo>> entry : partitionFileMap.entrySet()) {
            dirinfoList.add(
                    new DirectoryInfo(
                            entry.getKey(), entry.getValue(), initializationTime, pendingDataInstants));
        }
        return dirinfoList;
    }
    ...
    /**
      ...
      *
      * <p>All FileGroups for a given metadata partition has a fixed prefix as per the {@link
      * MetadataPartitionType#getFileIdPrefix()}. Each file group is suffixed with 4 digits with
      * increments of 1 starting with 0000.
      *
      * <p>Let's say we configure 10 file groups for record level index partition, and prefix as
      * "record-index-bucket-" File groups will be named as : record-index-bucket-0000, .... -> ...,
      * record-index-bucket-0009
      */
    private void initializeFileGroups(
            HoodieTableMetaClient dataMetaClient,
            MetadataPartitionType metadataPartition,
            String instantTime,
            int fileGroupCount)
            throws IOException {
        String partitionName =
                HoodieIndexUtils.getPartitionNameFromPartitionType(
                        metadataPartition, dataMetaClient, dataWriteConfig.getIndexingConfig().getIndexName());
        // Remove all existing file groups or leftover files in the partition
        final StoragePath partitionPath =
                new StoragePath(metadataWriteConfig.getBasePath(), partitionName);
        ...
        try {
            ...
            if (existingFiles.size() > 0) {
                ...
                ValidationUtils.checkState(
                        !storage.exists(partitionPath), "Failed to delete MDT partition " + partitionName);
            }
        ...
        // all commits in data table is in sync with metadata table. So, we always start with log
        // file for any fileGroup.

        // Even though the initial commit is a bulkInsert which creates the first baseFiles
        // directly, we still
        // create a log file first. This ensures that if any fileGroups of the MDT index do not
        // receive any records
        // during initial commit, then the fileGroup would still be recognized (as a FileSlice with
        // no baseFiles but a
        // valid logFile). Since these log files being created have no content, it is safe to add
        // them here before
        // the bulkInsert.
        final String msg =
                String.format(
                        "Creating %d file groups for partition %s with base fileId %s at instant time %s",
                        fileGroupCount, partitionName, metadataPartition.getFileIdPrefix(), instantTime);
        LOG.info(msg);
        final List<String> fileGroupFileIds =
                IntStream.range(0, fileGroupCount)
        ...
        engineContext.foreach(
                ...
                fileGroupFileId -> {
                    try {
                        ...
                        final HoodieDeleteBlock block =
                                new HoodieDeleteBlock(Collections.emptyList(), false, blockHeader);

                        try (HoodieLogFormat.Writer writer =
                                HoodieLogFormat.newWriterBuilder()
                                        .onParentPath(
                                                FSUtils.constructAbsolutePath(
                                                        metadataWriteConfig.getBasePath(), partitionName))
                        ...
                        throw new HoodieException(
                                String.format(
                                        "Failed to created fileGroup %s for partition %s",
                                        fileGroupFileId, partitionName),
                                e);
                    }
                },
    ...
    public void dropMetadataPartitions(List<MetadataPartitionType> metadataPartitions)
            throws IOException {
        for (MetadataPartitionType partitionType : metadataPartitions) {
            ...
            dataMetaClient
                    .getTableConfig()
                    .setMetadataPartitionState(dataMetaClient, partitionPath, false);
            ...
            LOG.warn(
                    "Deleting pending indexing instant from the timeline for partition: {}", partitionPath);
    ...
    /**
      * Deletes any pending indexing instant, if it exists. It reads the plan from indexing.requested
      * file and deletes both requested and inflight instants, if the partition path in the plan
      * matches with the given partition path.
      */
    private static void deletePendingIndexingInstant(
            HoodieTableMetaClient metaClient, String partitionPath) {
        metaClient
                .reloadActiveTimeline()
                .filterPendingIndexTimeline()
                .getInstantsAsStream()
                .filter(instant -> REQUESTED.equals(instant.getState()))
                .forEach(
                        instant -> {
                            try {
                                HoodieIndexPlan indexPlan =
                                        deserializeIndexPlan(
                                                metaClient.getActiveTimeline().readIndexPlanAsBytes(instant).get());
                                if (indexPlan.getIndexPartitionInfos().stream()
                                        .anyMatch(
                                                indexPartitionInfo ->
                                                        indexPartitionInfo.getMetadataPartitionPath().equals(partitionPath))) {
                                    metaClient.getActiveTimeline().deleteInstantFileIfExists(instant);
                                    metaClient
                                            .getActiveTimeline()
                                            .deleteInstantFileIfExists(getIndexInflightInstant(instant.getTimestamp()));
                                }
                            } catch (IOException e) {
    ...
    /**
      * Interface to assist in converting commit metadata to List of HoodieRecords to be written to
      * metadata table. Updates of different commit metadata uses the same method to convert to
    ...
    /**
      ...
      * @param convertMetadataFunction converter function to convert the respective metadata to List of
      *     HoodieRecords to be written to metadata table.
      */
    private void processAndCommit(
            String instantTime, ConvertMetadataFunction convertMetadataFunction) {
        Set<String> partitionsToUpdate = getMetadataPartitionsToUpdate();

        if (initialized && metadata != null) {
            // convert metadata and filter only the entries whose partition path are in
            // partitionsToUpdate
            Map<MetadataPartitionType, HoodieData<HoodieRecord>> partitionRecordsMap =
                    convertMetadataFunction.convertMetadata().entrySet().stream()
                            .filter(entry -> partitionsToUpdate.contains(entry.getKey().getPartitionPath()))
    ...
    private Set<String> getMetadataPartitionsToUpdate() {
        ...
        // add inflight indexes as well because the file groups have already been initialized, so
        // writers can log updates
        // NOTE: Async HoodieIndexer can move some partition to inflight. While that partition is
        // still being built,
        //       the regular ingestion writers should not be blocked. They can go ahead and log
        // updates to the metadata partition.
        //       Instead of depending on enabledPartitionTypes, the table config becomes the source
        // of truth for which partitions to update.
        ...
        LOG.debug(
                "There are no partitions to update according to table config. Falling back to enabled partition types in the write config.");
        return getEnabledPartitionTypes().stream()
                .map(MetadataPartitionType::getPartitionPath)
                .collect(Collectors.toSet());
    }

    public void buildMetadataPartitions(
            HoodieEngineContext engineContext,
        ...
        indexPartitionInfos.forEach(
                indexPartitionInfo -> {
                    String relativePartitionPath = indexPartitionInfo.getMetadataPartitionPath();
                    LOG.info(
                            "Creating a new metadata index for partition '{}' under path {} upto instant {}",
                    ...
                    if (!enabledPartitionTypes.contains(partitionType)) {
                        throw new HoodieIndexException(
                                String.format("Indexing for metadata partition: %s is not enabled", partitionType));
                    }
    ...
    /**
      ...
      */
    @Override
    public void updateFromWriteStatuses(
        ...
        processAndCommit(
                ...
                () -> {
                    ...
                    // Updates for record index are created by parsing the WriteStatus which is a
                    // hudi-client object. Hence, we cannot yet move this code
    ...
    private void updateFunctionalIndexIfPresent(
            HoodieCommitMetadata commitMetadata,
            String instantTime,
            Map<MetadataPartitionType, HoodieData<HoodieRecord>> partitionToRecordMap) {
        dataMetaClient.getTableConfig().getMetadataPartitions().stream()
                .filter(
                        partition ->
                                partition.startsWith(
                                        HoodieTableMetadataUtil.PARTITION_NAME_FUNCTIONAL_INDEX_PREFIX))
                .forEach(
                        partition -> {
                            HoodieData<HoodieRecord> functionalIndexRecords;
                            try {
                                functionalIndexRecords =
                                        getFunctionalIndexUpdates(commitMetadata, partition, instantTime);
                            } catch (Exception e) {
                                throw new HoodieMetadataException(
                                        String.format(
                                                "Failed to get functional index updates for partition %s", partition),
                                        e);
                            }
    ...
    /**
      * Loads the file slices touched by the commit due to given instant time and returns the records
      ...
      */
    private HoodieData<HoodieRecord> getFunctionalIndexUpdates(
                        ...
                        (dataPartition, value) -> {
                            List<FileSlice> fileSlices =
                                    getPartitionLatestFileSlicesIncludingInflight(
                                            dataMetaClient, Option.empty(), dataPartition);
                            fileSlices.forEach(
                                    fileSlice -> {
                                        // Filter log files for the instant time and add to this
                                        // partition fileSlice pairs
                                        List<HoodieLogFile> logFilesForInstant =
                                                fileSlice
                                                        .getLogFiles()
                                                        .filter(logFile -> logFile.getDeltaCommitTime().equals(instantTime))
                                                        .collect(Collectors.toList());
                                        partitionFileSlicePairs.add(
                                                Pair.of(
                                                        dataPartition,
                                                        new FileSlice(
        ...
        int parallelism =
                Math.min(
                        ...
                        dataWriteConfig.getMetadataConfig().getFunctionalIndexParallelism());
        Schema readerSchema = getProjectedSchemaForFunctionalIndex(indexDefinition, dataMetaClient);
        return getFunctionalIndexRecords(
                partitionFileSlicePairs,
                indexDefinition,
                dataMetaClient,
                parallelism,
    ...
    private void updateSecondaryIndexIfPresent(
            HoodieCommitMetadata commitMetadata,
            Map<MetadataPartitionType, HoodieData<HoodieRecord>> partitionToRecordMap,
            HoodieData<WriteStatus> writeStatus) {
        dataMetaClient.getTableConfig().getMetadataPartitions().stream()
                        ...
                        partition -> {
                            ...
                            try {
                                secondaryIndexRecords =
                                        getSecondaryIndexUpdates(commitMetadata, partition, writeStatus);
                            } catch (Exception e) {
                                throw new HoodieMetadataException(
                                        "Failed to get secondary index updates for partition " + partition, e);
                            }
                            partitionToRecordMap.put(SECONDARY_INDEX, secondaryIndexRecords);
                        });
    }

    private HoodieData<HoodieRecord> getSecondaryIndexUpdates(
            HoodieCommitMetadata commitMetadata,
        ...
        // Build a list of keys that need to be removed. A 'delete' record will be emitted into the
        // respective FileGroup of
        // the secondary index partition for each of these keys. For a commit which is
        // deleting/updating a lot of records, this
                ...
                .forEach(
                        status -> {
                            status
                                    ...
                                    .forEach(
                                            recordDelegate -> {
                                                // Consider those keys which were either updated or
        ...
        // This could be an expensive operation for a large commit (updating/deleting millions of
        // rows)
        Map<String, String> recordKeySecondaryKeyMap = metadata.getSecondaryKeys(keysToRemove);
        HoodieData<HoodieRecord> deletedRecords =
                getDeletedSecondaryRecordMapping(engineContext, recordKeySecondaryKeyMap, indexDefinition);

        int parallelism =
                Math.min(
                        partitionFilePairs.size(),
                        dataWriteConfig.getMetadataConfig().getSecondaryIndexParallelism());
        return deletedRecords.union(
                readSecondaryKeysFromBaseFiles(
                        engineContext,
                        partitionFilePairs,
                        parallelism,
                        this.getClass().getSimpleName(),
    ...
    /**
      ...
      * } } }
      */
    private static List<Pair<String, Pair<String, List<String>>>> getPartitionFilePairs(
            HoodieCommitMetadata commitMetadata) {
        List<Pair<String, Pair<String, List<String>>>> partitionFilePairs = new ArrayList<>();
        commitMetadata
                .getPartitionToWriteStats()
                .forEach(
                        (dataPartition, writeStats) -> {
                            writeStats.forEach(
                                    writeStat -> {
                                        if (writeStat instanceof HoodieDeltaWriteStat) {
                                            partitionFilePairs.add(
                                                    Pair.of(
                                                            dataPartition,
                                                            Pair.of(
                                                                    ((HoodieDeltaWriteStat) writeStat).getBaseFile(),
    ...
    public void update(HoodieRestoreMetadata restoreMetadata, String instantTime) {
        ...
        HoodieInstant restoreInstant =
                new HoodieInstant(REQUESTED, HoodieTimeline.RESTORE_ACTION, instantTime);
        ...
        try {
            ...
            throw new HoodieIOException(
                    String.format(
                            "Deserialization of restore plan failed whose restore instant time is %s in data table",
                            instantTime),
                    e);
        }
        ...
        // fetch the earliest commit to retain and ensure the base file prior to the time to restore
        // is present
        List<HoodieFileGroup> filesGroups =
                metadata
                        .getMetadataFileSystemView()
                        .getAllFileGroups(FILES.getPartitionPath())
                        .collect(Collectors.toList());

        boolean cannotRestore =
                filesGroups.stream()
                        .map(
                                fileGroup ->
                                        fileGroup
                                                ...
                                                .anyMatch(
                                                        instantTime1 ->
                                                                HoodieTimeline.compareTimestamps(
                                                                        instantTime1, LESSER_THAN_OR_EQUALS, restoreToInstantTime)))
                        .anyMatch(canRestore -> !canRestore);
        if (cannotRestore) {
            throw new HoodieMetadataException(
                    String.format(
                            "Can't restore to %s since there is no base file in MDT lesser than the commit to restore to. "
                                    + "Please delete metadata table and retry",
                            restoreToInstantTime));
        }

        // Restore requires the existing pipelines to be shutdown. So we can safely scan the dataset
        // to find the current
        ...
        Map<String, DirectoryInfo> dirInfoMap =
                dirInfoList.stream()
                        .collect(Collectors.toMap(DirectoryInfo::getRelativePath, Function.identity()));
        dirInfoList.clear();

        BaseHoodieWriteClient<?, I, ?, ?> writeClient = getWriteClient();
        writeClient.restoreToInstant(restoreToInstantTime, false);

        // At this point we have also reverted the cleans which have occurred after the
        // restoreToInstantTime. Hence, a sync
        ...
        try {
            ...
            fetchOutofSyncFilesRecordsFromMetadataTable(
                    dirInfoMap, partitionFilesToAdd, partitionFilesToDelete, partitionsToDelete);

            // Even if we don't have any deleted files to sync, we still create an empty commit so
            // that we can track the restore has completed.
            // We cannot create a deltaCommit at instantTime now because a future (rollback) block
            // has already been written to the logFiles.
            // We need to choose a timestamp which would be a validInstantTime for MDT. This is
            // either a commit timestamp completed on the dataset
    ...
    public void update(HoodieRollbackMetadata rollbackMetadata, String instantTime) {
        if (initialized && metadata != null) {
            ...
            HoodieInstant deltaCommitInstant =
                    new HoodieInstant(false, HoodieTimeline.DELTA_COMMIT_ACTION, commitToRollbackInstantTime);
            if (metadataMetaClient
                    .getActiveTimeline()
                    .getDeltaCommitTimeline()
                    .containsInstant(deltaCommitInstant)) {
                ...
                if (!getWriteClient().rollback(commitToRollbackInstantTime, instantTime)) {
                    throw new HoodieMetadataException(
                            String.format("Failed to rollback deltacommit at %s", commitToRollbackInstantTime));
                }
            } else {
                // if the instant is pending on MDT or not even exists the timeline, just ignore.
                LOG.info(
                        "Ignoring rollback of instant {} at {}. The commit to rollback is not found in MDT",
    ...
    private void validateRollback(String commitToRollbackInstantTime) {
        ...
        // The commit being rolled back should not be earlier than the latest compaction on the MDT
        // because the latest file slice does not change after all.
        // Compaction on MDT only occurs when all actions are completed on the dataset.
        // Hence, this case implies a rollback of completed commit which should actually be handled
        // using restore.
        if (compactionInstant.getAction().equals(COMMIT_ACTION)) {
            ...
            if (commitToRollbackInstantTime.length() == compactionInstantTime.length()
                ...
                throw new HoodieMetadataException(
                        String.format(
                                "Commit being rolled back %s is earlier than the latest compaction %s. There are %d deltacommits after this compaction: %s",
                                commitToRollbackInstantTime,
    ...
    protected void commitInternal(
        ...
        if (!metadataMetaClient.getActiveTimeline().getCommitsTimeline().containsInstant(instantTime)) {
            ...
            Option<HoodieInstant> alreadyCompletedInstant =
                    metadataMetaClient
                            ...
                            .lastInstant();
            LOG.info(
                    "{} completed commit at {} being applied to MDT.",
            ...
            if (!writeClient.rollback(instantTime)) {
                throw new HoodieMetadataException(
                        String.format("Failed to rollback deltacommit at %s from MDT", instantTime));
            }
            metadataMetaClient.reloadActiveTimeline();
        }
        ...
        if (isInitializing) {
            engineContext.setJobStatus(
                    this.getClass().getSimpleName(),
                    String.format(
                            "Bulk inserting at %s into metadata table %s",
                            instantTime, metadataWriteConfig.getTableName()));
            writeClient.bulkInsertPreppedRecords(preppedRecordInputs, instantTime, bulkInsertPartitioner);
        } else {
            engineContext.setJobStatus(
                    this.getClass().getSimpleName(),
                    String.format(
                            "Upserting at %s into metadata table %s",
                            instantTime, metadataWriteConfig.getTableName()));
    ...
    /**
      * Allows the implementation to perform any pre-commit operations like transitioning a commit to
      * inflight if required.
      *
    ...
    /**
      * Commit the {@code HoodieRecord}s to Metadata Table as a new delta-commit using bulk commit (if
      * supported).
      *
      * <p>This is used to optimize the initial commit to the MDT partition which may contains a large
      * number of records and hence is more suited to bulkInsert for write performance.
      *
      * @param instantTime - Action instant time for this commit
    ...
    /**
      * Tag each record with the location in the given partition. The record is tagged with respective
      * file slice's location based on its record key.
      */
    protected HoodieData<HoodieRecord> prepRecords(
            Map<MetadataPartitionType, HoodieData<HoodieRecord>> partitionRecordsMap) {
        // The result set
        ...
        try (HoodieTableFileSystemView fsView =
            ...
            for (Map.Entry<MetadataPartitionType, HoodieData<HoodieRecord>> entry :
                ...
                final String partitionName =
                        HoodieIndexUtils.getPartitionNameFromPartitionType(
                                entry.getKey(), dataMetaClient, dataWriteConfig.getIndexingConfig().getIndexName());
                HoodieData<HoodieRecord> records = entry.getValue();

                List<FileSlice> fileSlices =
                        HoodieTableMetadataUtil.getPartitionLatestFileSlices(
                                metadataMetaClient, Option.ofNullable(fsView), partitionName);
                ...
                ValidationUtils.checkArgument(
                        fileGroupCount > 0,
                        String.format("FileGroup count for MDT partition %s should be >0", partitionName));
                                    ...
                                    FileSlice slice =
                                            finalFileSlices.get(
                                                    HoodieTableMetadataUtil.mapRecordKeyToFileGroupIndex(
                                                            r.getRecordKey(), fileGroupCount));
                                    r.unseal();
                                    r.setCurrentLocation(
                                            new HoodieRecordLocation(slice.getBaseInstantTime(), slice.getFileId()));
                                    r.seal();
                                    return r;
                                });
    ...
    /**
      ...
      *
      * <p>Don't perform optimization if there are inflight operations on the dataset. This is for two
      * reasons: - The compaction will contain the correct data as all failed operations have been
      * rolled back. - Clean/compaction etc. will have the highest timestamp on the MDT and we won't be
      * adding new operations with smaller timestamps to metadata table (makes for easier debugging)
      *
      * <p>This adds the limitations that long-running async operations (clustering, etc.) may cause
      * delay in such MDT optimizations. We will relax this after MDT code has been hardened.
      */
    @Override
    public void performTableServices(Option<String> inFlightInstantTimestamp) {
        ...
        try {
            ...
            if (validateCompactionScheduling()) {
                ...
                LOG.info(
                        "Latest deltacommit time found is {}, running compaction operations.",
            ...
            metrics.ifPresent(
                    m -> m.setMetric(HoodieMetadataMetrics.TABLE_SERVICE_EXECUTION_DURATION, timeSpent));
            if (allTableServicesExecutedSuccessfullyOrSkipped) {
                metrics.ifPresent(
                        m -> m.setMetric(HoodieMetadataMetrics.TABLE_SERVICE_EXECUTION_STATUS, 1));
            } else {
                metrics.ifPresent(
                        m -> m.setMetric(HoodieMetadataMetrics.TABLE_SERVICE_EXECUTION_STATUS, -1));
            }
    ...
    private void runPendingTableServicesOperations(BaseHoodieWriteClient writeClient) {
        // finish off any pending log compaction or compactions operations if any from previous
        // attempt.
    ...
    /**
      ...
      *
      * <p>Cases to be handled: 1. We cannot perform compaction if there are previous inflight
      * operations on the dataset. This is because a compacted metadata base file at time Tx should
      * represent all the actions on the dataset till time Tx.
      *
      * <p>2. In multi-writer scenario, a parallel operation with a greater instantTime may have
      * completed creating a deltacommit.
      */
    protected void compactIfNecessary(BaseHoodieWriteClient writeClient) {
        // IMPORTANT: Trigger compaction with max instant time that is smaller than(or equals) the
        // earliest pending instant from DT.
        // The compaction planner will manage to filter out the log files that finished with greater
        // completion time.
        ...
        final String compactionInstantTime =
                dataMetaClient
                        .reloadActiveTimeline()
                        // The filtering strategy is kept in line with the rollback premise, if an
                        // instant is pending on DT but completed on MDT,
                        // generates a compaction time smaller than it so that the instant could
                        // then been rolled back.
                        .filterInflightsAndRequested()
                        .filter(instant -> metadataCompletedTimeline.containsInstant(instant.getTimestamp()))
                        .firstInstant()
                        // minus the pending instant time by 1 millisecond to avoid conflicts on the
                        // MDT.
                        .map(
                                instant ->
        ...
        // let's say we trigger compaction after C5 in MDT and so compaction completes with C4001.
        // but C5 crashed before completing in MDT.
        // and again w/ C6, we will re-attempt compaction at which point latest delta commit is C4
        // in MDT.
        // and so we try compaction w/ instant C4001. So, we can avoid compaction if we already have
        // compaction w/ same instant time.
        if (metadataMetaClient
                .getActiveTimeline()
                .filterCompletedInstants()
                .containsInstant(compactionInstantTime)) {
            LOG.info(
                    "Compaction with same {} time is already present in the timeline.",
                    compactionInstantTime);
            ...
            if (metadataMetaClient
                    .getActiveTimeline()
                    .filterCompletedInstants()
                    .containsInstant(logCompactionInstantTime)) {
                LOG.info(
                        "Log compaction with same {} time is already present in the timeline.",
                        logCompactionInstantTime);
            } else if (writeClient.scheduleLogCompactionAtInstant(
                    logCompactionInstantTime, Option.empty())) {
    ...
    protected void cleanIfNecessary(BaseHoodieWriteClient writeClient) {
        ...
        if (lastCompletedCompactionInstant.isPresent()
                && metadataMetaClient
                                .getActiveTimeline()
                                .filterCompletedInstants()
                                .findInstantsAfter(lastCompletedCompactionInstant.get().getTimestamp())
                                .countInstants()
                        < 3) {
            // do not clean the log files immediately after compaction to give some buffer time for
            // metadata table reader,
            // because there is case that the reader has prepared for the log file readers already
            // before the compaction completes
            // while before/during the reading of the log files, the cleaning triggers and delete
            // the reading files,
            // then a FileNotFoundException(for LogFormatReader) or NPE(for HFileReader) would
            // throw.

            // 3 is a value that I think is enough for metadata table reader.
            return;
        }
        // Trigger cleaning with suffixes based on the same instant time. This ensures that any
        // future
        // delta commits synced over will not have an instant time lesser than the last completed
        // instant on the
        // metadata table.
    ...
    /**
      * Validates the timeline for both main and metadata tables to ensure compaction on MDT can be
      * scheduled.
      */
    protected boolean validateCompactionScheduling() {
        // Under the log compaction scope, the sequence of the log-compaction and compaction needs
        // to be ensured because metadata items such as RLI
        // only has proc-time ordering semantics. For "ensured", it means the completion sequence of
        // the log-compaction/compaction is the same as the start sequence.
        if (metadataWriteConfig.isLogCompactionEnabled()) {
            ...
            if (pendingLogCompactionInstant.isPresent() || pendingCompactionInstant.isPresent()) {
                LOG.warn(
                        "Not scheduling compaction or logCompaction, since a pending compaction instant {} or logCompaction {} instant is present",
                        pendingCompactionInstant,
                        pendingLogCompactionInstant);
                return false;
            }
        }
        return true;
    ...
    private void fetchOutofSyncFilesRecordsFromMetadataTable(
            Map<String, DirectoryInfo> dirInfoMap,
            ...
            if (!dirInfoMap.containsKey(partition)) {
                ...
                if (metadataFiles != null && metadataFiles.size() > 0) {
                    partitionFilesToDelete.put(
                            partitionId,
                            metadataFiles.stream().map(f -> f.getPath().getName()).collect(Collectors.toList()));
                }
            } else {
                ...
                List<String> mdtFiles =
                        metadataFiles.stream()
                                .map(mdtFile -> mdtFile.getPath().getName())
                                .collect(Collectors.toList());
                List<String> filesDeleted =
                        metadataFiles.stream()
                                .map(f -> f.getPath().getName())
                                .filter(n -> !fsFiles.containsKey(n))
                                .collect(Collectors.toList());
                Map<String, Long> filesToAdd = new HashMap<>();
                // new files could be added to DT due to restore that just happened which may not be
                // tracked in RestoreMetadata.
                dirInfoMap
                        .get(partition)
                        .getFileNameToSizeMap()
                        .forEach(
                                (k, v) -> {
    ...
    /**
      * Return records that represent upserts to the record index due to write operation on the
      * dataset.
      *
                            ...
                            if (newLocation.isPresent()) {
                                if (recordDelegate.getCurrentLocation().isPresent()) {
                                    ...
                                    if (!recordDelegate
                                            ...
                                            .get()
                                            .getFileId()
                                            .equals(newLocation.get().getFileId())) {
                                        final String msg =
                                                String.format(
                                                        "Detected update in location of record with key %s from %s to %s. The fileID should not change.",
                                                        recordDelegate,
                                                        recordDelegate.getCurrentLocation().get(),
                                                        newLocation.get());
                                        LOG.error(msg);
    ...
    private HoodieData<HoodieRecord> getRecordIndexReplacedRecords(
        ...
        List<Pair<String, HoodieBaseFile>> partitionBaseFilePairs =
                replaceCommitMetadata.getPartitionToReplaceFileIds().keySet().stream()
                        .flatMap(
                                partition -> fsView.getLatestBaseFiles(partition).map(f -> Pair.of(partition, f)))
                        .collect(Collectors.toList());
    ...
    private HoodieData<HoodieRecord> getRecordIndexAdditionalUpserts(
            HoodieData<HoodieRecord> updatesFromWriteStatuses, HoodieCommitMetadata commitMetadata) {
        ...
        if (operationType == WriteOperationType.INSERT_OVERWRITE) {
            ...
            // return partition-level unmatched records (with newLocation being null) to be deleted
            // from RLI
            ...
            // load existing records from replaced filegroups and left anti join overwriting records
            // return globally unmatched records (with newLocation being null) to be deleted from
            // RLI
            return getRecordIndexReplacedRecords((HoodieReplaceCommitMetadata) commitMetadata)
