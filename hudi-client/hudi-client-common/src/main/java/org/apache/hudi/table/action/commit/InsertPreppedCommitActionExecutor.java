...
public class InsertPreppedCommitActionExecutor<T extends HoodieRecordPayload<T>>
  ...
  public InsertPreppedCommitActionExecutor(
      HoodieEngineContext context, HoodieWriteConfig config,
      HoodieTable table, String instantTime,
      HoodieData<HoodieRecord<T>> preppedRecords, BaseCommitHelper<T> commitHelper) {
    super(context, config, table, instantTime, WriteOperationType.INSERT_PREPPED, commitHelper);
    this.preppedRecords = preppedRecords;
  }

  @Override
  public HoodieWriteMetadata<HoodieData<WriteStatus>> execute() {
    return commitHelper.execute(preppedRecords);
  }
}
