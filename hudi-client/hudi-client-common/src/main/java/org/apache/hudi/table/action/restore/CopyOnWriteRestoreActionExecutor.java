...
public class CopyOnWriteRestoreActionExecutor<T, I, K, O>
    ...
    public CopyOnWriteRestoreActionExecutor(
    ...
    protected HoodieRollbackMetadata rollbackInstant(HoodieInstant instantToRollback) {
        if (!instantToRollback.getAction().equals(HoodieTimeline.COMMIT_ACTION)
            ...
            throw new HoodieRollbackException(
                    "Unsupported action in rollback instant:" + instantToRollback);
        }
        ...
        table.getMetaClient().reloadActiveTimeline();
        CopyOnWriteRollbackActionExecutor rollbackActionExecutor =
                new CopyOnWriteRollbackActionExecutor(
                        context, config, table, newInstantTime, instantToRollback, true, true, false);
