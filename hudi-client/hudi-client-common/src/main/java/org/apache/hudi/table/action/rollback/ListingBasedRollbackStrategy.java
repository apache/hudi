...
public class ListingBasedRollbackStrategy
    ...
    public ListingBasedRollbackStrategy(
            HoodieTable<?, ?, ?, ?> table,
        ...
        try {
            ...
            List<String> partitionPaths =
                    FSUtils.getAllPartitionPaths(
                            context, table.getStorage(), table.getMetaClient().getBasePath(), false);
            int numPartitions =
                    Math.max(Math.min(partitionPaths.size(), config.getRollbackParallelism()), 1);

            context.setJobStatus(
                    this.getClass().getSimpleName(),
                    "Creating Listing Rollback Plan: " + config.getTableName());
            ...
            if (commitMetadataOptional.isPresent()) {
                isCompaction.set(
                        commitMetadataOptional.get().getOperationType() == WriteOperationType.COMPACT);
            }
                    ...
                    partitionPath -> {
                        ...
                        if (HoodieTableType.COPY_ON_WRITE == tableType) {
                            hoodieRollbackRequests.addAll(
                                    getHoodieRollbackRequests(partitionPath, filesToDelete.get()));
                        } else if (HoodieTableType.MERGE_ON_READ == tableType) {
                            table.getMetaClient().reloadActiveTimeline();
                            String action = instantToRollback.getAction();
                            if (isCompaction.get()) { // compaction's action in hoodie instant will be
                                // "commit". So, we might need to override.
                            ...
                            switch (action) {
                                ...
                                case HoodieTimeline.COMPACTION_ACTION:
                                    ...
                                    // compaction), we might have to delete/rollback only the base
                                    // file that could have
                                ...
                                default:
                                    throw new HoodieRollbackException(
                                            "Unknown listing type, during rollback of " + instantToRollback);
                            }
                        } else {
                            throw new HoodieRollbackException(
                                    String.format(
                                            "Unsupported table type: %s, during listing rollback of %s",
                                            tableType, instantToRollback));
                        }
            ...
            throw new HoodieRollbackException(
                    "Generating rollback requests failed for " + instantToRollback.getTimestamp(), e);
        }
    }
    ...
    private FileStatus[] listAllFilesSinceCommit(
            ...
            throws IOException {
        LOG.info(
                "Collecting files to be cleaned/rolledback up for path "
                        + partitionPath
                        + " and commit "
                        + commit);
                ...
                (path) -> {
                    if (path.toString().contains(baseFileExtension)) {
                        ...
                        return HoodieTimeline.compareTimestamps(
                                commit, HoodieTimeline.LESSER_THAN_OR_EQUALS, fileCommitTime);
        ...
        return ((FileSystem) metaClient.getStorage().getFileSystem())
                .listStatus(
                        HadoopFSUtils.constructAbsolutePathInHadoopPath(config.getBasePath(), partitionPath),
                        filter);
    }

    @NotNull
    private List<HoodieRollbackRequest> getHoodieRollbackRequests(
            String partitionPath, FileStatus[] filesToDeletedStatus) {
    ...
    private FileStatus[] listBaseFilesToBeDeleted(
            String commit, String basefileExtension, String partitionPath, FileSystem fs)
            throws IOException {
        LOG.info(
                "Collecting files to be cleaned/rolledback up for path "
                        + partitionPath
    ...
    private FileStatus[] fetchFilesFromInstant(
            ...
            throws IOException {
        // go w/ commit metadata only for COW table. for MOR, we need to get associated log files
        // when commit corresponding to base file is rolledback.
        if (isCommitMetadataCompleted && tableType == HoodieTableType.COPY_ON_WRITE) {
            return fetchFilesFromCommitMetadata(
                    instantToRollback,
                    partitionPath,
                    basePath,
            ...
            return fetchFilesFromListFiles(
                    instantToRollback, partitionPath, basePath, baseFileExtension, fs);
        }
    }

    private FileStatus[] fetchFilesFromCommitMetadata(
            HoodieInstant instantToRollback,
            String partitionPath,
            String basePath,
                ...
                Arrays.stream(filePaths)
                        .filter(
                                entry -> {
                                    ...
                                    // if any Exception is thrown, do not ignore. let's try to add
                                    // the file of interest to be deleted. we can't miss any files
                                    // to be rolled back.
                                    return true;
                                })
                        .toArray(Path[]::new),
    ...
    /**
      * returns matching base files and log files if any for the instant time of the commit to be
      * rolled back.
      *
      ...
      * @throws IOException
      */
    private FileStatus[] fetchFilesFromListFiles(
            HoodieInstant instantToRollback,
            String partitionPath,
            String basePath,
            String baseFileExtension,
    ...
    private Boolean checkCommitMetadataCompleted(
            HoodieInstant instantToRollback, Option<HoodieCommitMetadata> commitMetadataOptional) {
    ...
    private static Path[] getFilesFromCommitMetadata(
            String basePath, HoodieCommitMetadata commitMetadata, String partitionPath) {
    ...
    @NotNull
    private static SerializablePathFilter getSerializablePathFilter(
            String basefileExtension, String commit) {
        return (path) -> {
