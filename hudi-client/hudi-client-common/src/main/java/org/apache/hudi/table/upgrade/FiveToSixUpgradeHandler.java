...
/**
  * Upgrade handle to assist in upgrading hoodie table from version 5 to 6. Since we do not
  * write/read compaction plan from .aux folder anyone, the upgrade handler will delete compaction
  * files from .aux folder.
  */
                ...
                .forEach(
                        deleteInstant -> {
                            LOG.info(
                                    "Deleting instant "
                                            + deleteInstant
                                            + " in auxiliary meta path "
                                            + metaClient.getMetaAuxiliaryPath());
                            StoragePath metaFile =
                                    new StoragePath(metaClient.getMetaAuxiliaryPath(), deleteInstant.getFileName());
                            try {
