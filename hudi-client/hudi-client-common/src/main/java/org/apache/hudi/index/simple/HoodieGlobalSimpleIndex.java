...
/**
  * A global simple index which reads interested fields(record key and partition path) from base
  * files and joins with incoming records to find the tagged location.
  */
public class HoodieGlobalSimpleIndex extends HoodieSimpleIndex {
    public HoodieGlobalSimpleIndex(
            HoodieWriteConfig config, Option<BaseKeyGenerator> keyGeneratorOpt) {
    ...
    protected <R> HoodieData<HoodieRecord<R>> tagLocationInternal(
        ...
        int fetchParallelism =
                configuredSimpleIndexParallelism > 0
                        ...
                        : inputRecords.deduceNumPartitions();
