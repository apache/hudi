...
public abstract class BaseRestoreActionExecutor<T, I, K, O>
    ...
    public BaseRestoreActionExecutor(
    ...
    public HoodieRestoreMetadata execute() {
        ...
        try {
            ...
            ValidationUtils.checkArgument(
                    restoreInstant.get().getState().equals(HoodieInstant.State.REQUESTED)
            ...
            instantsToRollback.forEach(
                    instant -> {
                        instantToMetadata.put(
                                instant.getTimestamp(), Collections.singletonList(rollbackInstant(instant)));
    ...
    private List<HoodieInstant> getInstantsToRollback(HoodieInstant restoreInstant)
            throws IOException {
        ...
        for (HoodieInstantInfo instantInfo : restorePlan.getInstantsToRollback()) {
            // If restore crashed midway, there are chances that some commits are already rolled
            // back,
            // but some are not. so, we can ignore those commits which are fully rolledback in
                            ...
                            .filter(
                                    instant ->
                                            instant.getTimestamp().equals(instantInfo.getCommitTime())
                                                    && instant.getAction().equals(instantInfo.getAction()))
                            .firstInstant();
    ...
    private HoodieRestoreMetadata finishRestore(
            Map<String, List<HoodieRollbackMetadata>> instantToMetadata,
        ...
        HoodieInstant restoreInflightInstant =
                new HoodieInstant(true, HoodieTimeline.RESTORE_ACTION, instantTime);
        ...
        // if not, rollbacks will be considered not completed and might hinder metadata table
        // compaction.
        List<HoodieInstant> instantsToRollback =
                table
                        ...
                        .filter(
                                instant ->
                                        HoodieActiveTimeline.GREATER_THAN.test(
                                                instant.getTimestamp(), savepointToRestoreTimestamp))
        ...
        LOG.info(
                "Commits "
                        + instantsRolledBack
                        + " rollback is complete. Restored table to "
                        + savepointToRestoreTimestamp);
        return restoreMetadata;
    }

    /**
      * Update metadata table if available. Any update to metadata table happens within data table
      * lock.
      *
      * @param restoreMetadata instance of {@link HoodieRestoreMetadata} to be applied to metadata.
      */
    private void writeToMetadata(
            HoodieRestoreMetadata restoreMetadata, HoodieInstant restoreInflightInstant) {
        try {
