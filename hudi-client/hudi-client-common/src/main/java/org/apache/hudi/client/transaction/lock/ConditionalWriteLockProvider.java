...
public class ConditionalWriteLockProvider implements LockProvider<ConditionalWriteLockFile> {
  ...
  private static final Map<String, String> SUPPORTED_LOCK_SERVICE_IMPLEMENTATIONS = new HashMap<>();
  static {
    SUPPORTED_LOCK_SERVICE_IMPLEMENTATIONS.put(StorageSchemes.GCS.getScheme(), GCS_LOCK_SERVICE_CLASS_NAME);
    SUPPORTED_LOCK_SERVICE_IMPLEMENTATIONS.put(StorageSchemes.S3A.getScheme(), S3_LOCK_SERVICE_CLASS_NAME);
    SUPPORTED_LOCK_SERVICE_IMPLEMENTATIONS.put(StorageSchemes.S3.getScheme(), S3_LOCK_SERVICE_CLASS_NAME);
  }
  ...
  private static @NotNull String getLockServiceClassName(String scheme) {
    if (SUPPORTED_LOCK_SERVICE_IMPLEMENTATIONS.containsKey(scheme) && StorageSchemes.isConditionalWritesSupported(scheme)) {
