...
/**
  * A SingleFileHandleCreateFactory is used to write all data in the spark partition into a single
  * data file.
  *
  * <p>Please use this with caution. This can end up creating very large files if not used correctly.
  */
public class SingleFileHandleCreateFactory<T, I, K, O> extends CreateHandleFactory<T, I, K, O>
    ...
    public HoodieWriteHandle<T, I, K, O> create(
        ...
        if (isHandleCreated.compareAndSet(false, true)) {
            return new HoodieUnboundedCreateHandle(
                    hoodieConfig,
                    commitTime,
                    hoodieTable,
                    partitionPath,
