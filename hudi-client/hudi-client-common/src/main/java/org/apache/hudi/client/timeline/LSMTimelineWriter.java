...
public class LSMTimelineWriter {
    ...
    public static LSMTimelineWriter getInstance(
    ...
    public void write(
        ...
        ValidationUtils.checkArgument(
                !activeActions.isEmpty(), "The instant actions to write should not be empty");
        StoragePath filePath =
                new StoragePath(
                        metaClient.getArchivePath(),
                        newFileName(
                                activeActions.get(0).getInstantTime(),
                                activeActions.get(activeActions.size() - 1).getInstantTime(),
                                FILE_LAYER_ZERO));
        try (HoodieFileWriter writer = openWriter(filePath)) {
            ...
            for (ActiveAction activeAction : activeActions) {
                try {
                    ...
                    writer.write(
                            metaEntry.getInstantTime(), new HoodieAvroIndexedRecord(metaEntry), wrapperSchema);
                } catch (Exception e) {
    ...
    /**
      ...
      *
      * <ol>
      *   <li>read the latest manifest version file;
      *   <li>read the latest manifest file for valid files;
      *   <li>add this new file to the existing file list from step2.
      * </ol>
      *
    ...
    /**
      ...
      *   <li>read the latest manifest version file;
      *   <li>read the latest manifest file for valid files;
      *   <li>remove files to the existing file list from step2;
      *   <li>add this new file to the existing file list from step2.
      * </ol>
      *
    ...
    private void createManifestFile(HoodieLSMTimelineManifest manifest, int currentVersion)
            throws IOException {
    ...
    /**
      ...
      * The 'level' represents the number of the level where the file is located, currently we have no
      * limit for the number of layers.
      *
      * <p>These parquet files composite as an LSM tree layout, one parquet file contains instant
      * metadata entries with consecutive timestamp. Different parquet files may have overlapping with
      * the instant time ranges.
      *
      * <pre>
      ...
      * </pre>
      *
      * <p>Compaction and cleaning: once the files number exceed a threshold(now constant 10) N, the
      * oldest N files are then replaced with a compacted file in the next layer. A cleaning action is
      * triggered right after the compaction.
      *
    ...
    private Option<String> doCompact(HoodieLSMTimelineManifest manifest, int layer)
            throws IOException {
        // 1. list all the files that belong to current layer
        List<HoodieLSMTimelineManifest.LSMFileEntry> files =
                manifest.getFiles().stream()
                        .filter(file -> LSMTimeline.isFileFromLayer(file.getFileName(), layer))
    ...
    public void compactFiles(List<String> candidateFiles, String compactedFileName) {
        ...
        try (HoodieFileWriter writer =
                openWriter(new StoragePath(metaClient.getArchivePath(), compactedFileName))) {
                ...
                try (HoodieAvroParquetReader reader =
                        (HoodieAvroParquetReader)
                                HoodieIOFactory.getIOFactory(metaClient.getStorage())
                                        ...
                                        .getFileReader(
                                                config, new StoragePath(metaClient.getArchivePath(), fileName))) {
                    // Read the meta entry
                    try (ClosableIterator<IndexedRecord> iterator =
                            reader.getIndexedRecordIterator(
                        ...
                        while (iterator.hasNext()) {
                            ...
                            writer.write(
                                    ...
                                    new HoodieAvroIndexedRecord(record),
                                    HoodieLSMTimelineInstant.getClassSchema());
                        }
                    }
                }
    ...
    public void clean(HoodieEngineContext context, int compactedVersions) throws IOException {
        ...
        if (allSnapshotVersions.size() > numVersionsToKeep) {
            ...
            Set<String> filesToKeep =
                    versionsToKeep.stream()
                            .flatMap(
                                    version ->
                                            LSMTimeline.latestSnapshotManifest(metaClient, version).getFileNames()
                                                    .stream())
                            .collect(Collectors.toSet());
            // delete the manifest file first
            List<String> manifestFilesToClean = new ArrayList<>();
            LSMTimeline.listAllManifestFiles(metaClient)
                    .forEach(
                            fileStatus -> {
            ...
            HadoopFSUtils.deleteFilesParallelize(
                    metaClient, manifestFilesToClean, context, config.getArchiveDeleteParallelism(), false);
            // delete the data files
            List<String> dataFilesToClean =
                    LSMTimeline.listAllMetaFiles(metaClient).stream()
                            .filter(fileStatus -> !filesToKeep.contains(fileStatus.getPath().getName()))
                            .map(fileStatus -> fileStatus.getPath().toString())
                            .collect(Collectors.toList());
            HadoopFSUtils.deleteFilesParallelize(
                    metaClient, dataFilesToClean, context, config.getArchiveDeleteParallelism(), false);
        }
    }
    ...
    private HoodieLSMTimelineManifest.LSMFileEntry getFileEntry(String fileName) throws IOException {
        long fileLen =
                metaClient
                        .getStorage()
                        .getPathInfo(new StoragePath(metaClient.getArchivePath(), fileName))
                        .getLength();
        return HoodieLSMTimelineManifest.LSMFileEntry.getInstance(fileName, fileLen);
    }

    /**
      * Returns at most {@code filesBatch} number of source files restricted by the gross file size by
      * 1GB.
      */
    private List<String> getCandidateFiles(
            List<HoodieLSMTimelineManifest.LSMFileEntry> files, int filesBatch) throws IOException {
        ...
        for (int i = 0; i < filesBatch; i++) {
            ...
            // we may also need to consider a single file that is very close to the threshold in
            // size,
    ...
    private static String newFileName(String minInstant, String maxInstant, int layer) {
        return String.format(
                "%s_%s_%d%s", minInstant, maxInstant, layer, HoodieFileFormat.PARQUET.getFileExtension());
    }
    ...
    public static String compactedFileName(List<String> files) {
        String minInstant =
                files.stream().map(LSMTimeline::getMinInstantTime).min(Comparator.naturalOrder()).get();
        String maxInstant =
                files.stream().map(LSMTimeline::getMaxInstantTime).max(Comparator.naturalOrder()).get();
    ...
    private HoodieFileWriter openWriter(StoragePath filePath) {
        try {
            return HoodieFileWriterFactory.getFileWriter(
                    "",
                    filePath,
                    metaClient.getStorage(),
                    getOrCreateWriterConfig(),
                    HoodieLSMTimelineInstant.getClassSchema(),
                    taskContextSupplier,
