...
public class HoodieIndexUtils {
    ...
    public static List<HoodieBaseFile> getLatestBaseFilesForPartition(
            String partition, HoodieTable hoodieTable) {
    ...
    public static List<FileSlice> getLatestFileSlicesForPartition(
            final String partition, final HoodieTable hoodieTable) {
        Option<HoodieInstant> latestCommitTime =
                hoodieTable.getMetaClient().getCommitsTimeline().filterCompletedInstants().lastInstant();
        if (latestCommitTime.isPresent()) {
            return hoodieTable
    ...
    /**
      ...
      */
    public static List<Pair<String, HoodieBaseFile>> getLatestBaseFilesForAllPartitions(
            final List<String> partitions,
            final HoodieEngineContext context,
            final HoodieTable hoodieTable) {
        context.setJobStatus(
                HoodieIndexUtils.class.getSimpleName(),
                "Load latest base files from all partitions: " + hoodieTable.getConfig().getTableName());
    ...
    /**
      ...
      */
    public static <R> HoodieRecord<R> tagAsNewRecordIfNeeded(
            HoodieRecord<R> record, Option<HoodieRecordLocation> location) {
        if (location.isPresent()) {
            // When you have a record in multiple files in the same partition, then <row key,
            // record> collection
    ...
    public static <R> HoodieRecord<R> tagRecord(
            HoodieRecord<R> record, HoodieRecordLocation location) {
        record.unseal();
    ...
    /**
      ...
      */
    public static List<Pair<String, Long>> filterKeysFromFile(
            StoragePath filePath, List<String> candidateRecordKeys, HoodieStorage storage)
        ...
        try (HoodieFileReader fileReader =
            ...
            if (!candidateRecordKeys.isEmpty()) {
                ...
                LOG.info(
                        String.format(
                                "Checked keys against file %s, in %d ms. #candidates (%d) #found (%d)",
                                filePath, timer.endTimer(), candidateRecordKeys.size(), foundRecordKeys.size()));
    ...
    /**
      ...
      *
      * <p>The commit timestamp is considered to be valid if: 1. the commit timestamp is present in the
      * timeline, or 2. the commit timestamp is less than the first commit timestamp in the timeline
      *
    ...
    /**
      ...
      *
      * <p>This will perform merged read for MOR table, in case a FileGroup contains log files.
      *
    ...
    private static <R> HoodieData<HoodieRecord<R>> getExistingRecords(
            HoodieData<Pair<String, String>> partitionLocations,
            HoodieWriteConfig config,
            HoodieTable hoodieTable) {
        final Option<String> instantTime =
                hoodieTable
                        .getMetaClient()
                        .getCommitsTimeline()
        ...
        return partitionLocations.flatMap(
                p ->
    ...
    /**
      * Merge the incoming record with the matching existing record loaded via {@link
      * HoodieMergedReadHandle}. The existing record is the latest version in the table.
      */
    private static <R> Option<HoodieRecord<R>> mergeIncomingWithExistingRecord(
        ...
        Schema writeSchemaWithMetaFields =
                HoodieAvroUtils.addMetadataFields(writeSchema, config.allowOperationMetadataField());
        // prepend the hoodie meta fields as the incoming record does not have them
        HoodieRecord incomingPrepended =
                incoming.prependMetaFields(
                        writeSchema,
                        writeSchemaWithMetaFields,
                        new MetadataValues()
                                .setRecordKey(incoming.getRecordKey())
                                .setPartitionPath(incoming.getPartitionPath()),
                        config.getProps());
        // after prepend the meta fields, convert the record back to the original payload
        HoodieRecord incomingWithMetaFields =
                incomingPrepended.wrapIntoHoodieRecordPayloadWithParams(
                        writeSchema,
                        config.getProps(),
                        Option.empty(),
    ...
    public static <R> HoodieData<HoodieRecord<R>> mergeForPartitionUpdatesIfNeeded(
        ...
        HoodieData<HoodieRecord<R>> taggedNewRecords =
                incomingRecordsAndLocations.filter(p -> !p.getRight().isPresent()).map(Pair::getLeft);
        ...
        HoodieData<HoodieRecord<R>> untaggedUpdatingRecords =
                incomingRecordsAndLocations
                        ...
                        .map(Pair::getLeft)
                        .distinctWithKey(
                                HoodieRecord::getRecordKey, config.getGlobalIndexReconcileParallelism());
        // the tagging partitions and locations
        // NOTE: The incoming records may only differ in record position, however, for the purpose
        // of
        ...
        HoodieData<Pair<String, String>> globalLocations =
                incomingRecordsAndLocations
                        .filter(p -> p.getRight().isPresent())
                        .map(
                                p -> Pair.of(p.getRight().get().getPartitionPath(), p.getRight().get().getFileId()))
                        ...
                        .flatMap(
                                entry -> {
                                    ...
                                    if (!existingOpt.isPresent()) {
                                        // existing record not found (e.g., due to delete log not
                                        // merged to base file): tag as a new record
                                    ...
                                    if (incoming.isDelete(writeSchema, config.getProps())) {
                                        ...
                                        return Collections.singletonList(
                                                        tagRecord(
                                                                incoming.newInstance(existing.getKey()),
                                                                existing.getCurrentLocation()))
                                                .iterator();
                                    }

                                    Option<HoodieRecord<R>> mergedOpt =
                                            mergeIncomingWithExistingRecord(
                                                    incoming, existing, writeSchema, config, recordMerger);
                                    if (!mergedOpt.isPresent()) {
                                        // merge resulted in delete: force tag the incoming to the
                                        // old partition
                                        return Collections.singletonList(
                                                        tagRecord(
                                                                incoming.newInstance(existing.getKey()),
                                    ...
                                    if (Objects.equals(merged.getPartitionPath(), existing.getPartitionPath())) {
                                        // merged record has the same partition: route the merged
                                        // result to the current location as an update
                                        return Collections.singletonList(
                                                        tagRecord(merged, existing.getCurrentLocation()))
                                                .iterator();
                                    } else {
                                        // merged record has a different partition: issue a delete
                                        // to the old partition and insert the merged record to the
                                        // new partition
                                        HoodieRecord<R> deleteRecord = createDeleteRecord(config, existing.getKey());
                                        deleteRecord.setIgnoreIndexUpdate(true);
                                        return Arrays.asList(
                                                        tagRecord(deleteRecord, existing.getCurrentLocation()), merged)
                                                .iterator();
                                    }
                                });
        ...
        HoodieData<Pair<HoodieRecord<R>, Option<HoodieRecordGlobalLocation>>>
                incomingRecordsAndLocations =
                        keyAndIncomingRecords
                                .leftOuterJoin(keyAndExistingLocations)
                                .values()
                                .map(
                                        v -> {
                                            ...
                                            if (currentLocOpt.isPresent()) {
                                                ...
                                                boolean shouldDoMergedLookUpThenTag =
                                                        mayContainDuplicateLookup
                                                                || !Objects.equals(
                                                                        incomingRecord.getPartitionPath(),
                                                                        currentLoc.getPartitionPath());
                                                if (shouldUpdatePartitionPath && shouldDoMergedLookUpThenTag) {
                                                    // the pair's right side is a non-empty Option,
                                                    // which indicates that a merged lookup will be
                                                    // performed
                                                    // at a later stage.
                                                    return Pair.of(incomingRecord, currentLocOpt);
                                                } else {
                                                    // - When update partition path is set to false,
                                                    //   the incoming record will be tagged to the
                                                    // existing record's partition regardless of
                                                    // being equal or not.
                                                    // - When update partition path is set to true,
                                                    //   the incoming record will be tagged to the
                                                    // existing record's partition
                                                    //   when partition is not updated and the
                                                    // look-up won't have duplicates (e.g. COW, or
                                                    // using RLI).
                                                    return Pair.of(
                                                            createNewTaggedHoodieRecord(
                                                                    incomingRecord, currentLoc, merger.getRecordType()),
                                                            Option.empty());
                                                }
                                            } else {
    ...
    public static <R> HoodieRecord<R> createNewTaggedHoodieRecord(
            HoodieRecord<R> oldRecord, HoodieRecordGlobalLocation location, HoodieRecordType recordType) {
        switch (recordType) {
            case AVRO:
                HoodieKey recordKey = new HoodieKey(oldRecord.getRecordKey(), location.getPartitionPath());
                return tagRecord(
                        new HoodieAvroRecord(recordKey, (HoodieRecordPayload) oldRecord.getData()), location);
            case SPARK:
    ...
    /**
      * Get the partition name from the metadata partition type. NOTE: For certain types of metadata
      * partition, such as functional index and secondary index, partition path defined enum is just
      * the prefix to denote the type of metadata partition. The actual partition name is contained in
      * the index definition.
      */
    public static String getPartitionNameFromPartitionType(
            MetadataPartitionType partitionType, HoodieTableMetaClient metaClient, String indexName) {
