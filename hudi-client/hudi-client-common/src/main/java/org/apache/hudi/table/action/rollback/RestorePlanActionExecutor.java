...
public class RestorePlanActionExecutor<T, I, K, O>
    ...
    public RestorePlanActionExecutor(
        ...
        try {
            ...
            List<HoodieInstant> pendingClusteringInstantsToRollback =
                    table
                            ...
                            // filter only clustering related replacecommits (Not insert_overwrite
                            // related commits)
                            .filter(
                                    instant ->
            ...
            // Combine both lists - first rollback pending clustering and then rollback all other
            // commits
            List<HoodieInstantInfo> instantsToRollback =
                    Stream.concat(
                                    pendingClusteringInstantsToRollback.stream(), commitInstantsToRollback.stream())
                            .map(entry -> new HoodieInstantInfo(entry.getTimestamp(), entry.getAction()))
                            .collect(Collectors.toList());

            HoodieRestorePlan restorePlan =
                    new HoodieRestorePlan(
                            instantsToRollback, LATEST_RESTORE_PLAN_VERSION, savepointToRestoreTimestamp);
            table
                    .getActiveTimeline()
                    .saveToRestoreRequested(
                            restoreInstant, TimelineMetadataUtils.serializeRestorePlan(restorePlan));
