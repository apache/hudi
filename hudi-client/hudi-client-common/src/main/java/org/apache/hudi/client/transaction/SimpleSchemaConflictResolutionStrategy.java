...
public class SimpleSchemaConflictResolutionStrategy implements SchemaConflictResolutionStrategy {
  ...
  public Option<Schema> resolveConcurrentSchemaEvolution(
    ...
    HoodieInstant lastCompletedInstantsAtTxnStart = lastCompletedTxnOwnerInstant.isPresent()
        ...
        lastCompletedTxnOwnerInstant.get().getTimestamp()).lastInstant().orElseGet(() -> null)
        : null;
  ...
  @NotNull TableSchemaResolver getSchemaResolver(HoodieTable table) {
    return new TableSchemaResolver(table.getMetaClient());
  }

  private static Schema getTableSchemaAtInstant(TableSchemaResolver schemaResolver, @Nonnull HoodieInstant instant) {
    Schema schema;
    try {
      schema = schemaResolver.getTableAvroSchema(instant, false);
    } catch (Exception e) {
