...
/**
  ...
  * over-ridden implementations of this abstract class can capture the relevant metrics to order and
  * filter the final list of compaction operation to run in a single compaction. Implementation of
  * CompactionStrategy cannot hold any state. Difference instantiations can be passed in every time
  */
public abstract class CompactionStrategy implements Serializable {
    ...
    /**
      * Callback hook when a HoodieCompactionOperation is created. Individual strategies can capture
      * the metrics they need to decide on the priority.
      *
    ...
    public Map<String, Double> captureMetrics(HoodieWriteConfig writeConfig, FileSlice slice) {
        ...
        FileSliceMetricUtils.addFileSliceCommonMetrics(
                Collections.singletonList(slice), metrics, defaultMaxParquetFileSize);
        return metrics;
    }

    /**
      * Generate Compaction plan. Allows clients to order and filter the list of compactions to be set.
      * The default implementation takes care of setting compactor Id from configuration allowing
      * subclasses to only worry about ordering and filtering compaction operations
      *
      * @param writeConfig Hoodie Write Config
      * @param operations Compaction Operations to be ordered and filtered
      * @param pendingCompactionPlans Pending Compaction Plans for strategy to schedule next compaction
      *     plan
    ...
    /**
      ...
      * @return
      */
    public List<String> filterPartitionPaths(
            HoodieWriteConfig writeConfig, List<String> allPartitionPaths) {
