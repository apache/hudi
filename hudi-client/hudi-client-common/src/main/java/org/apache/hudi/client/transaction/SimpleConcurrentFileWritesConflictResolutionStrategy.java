...
/**
  * This class is a basic implementation of a conflict resolution strategy for concurrent writes
...
public class SimpleConcurrentFileWritesConflictResolutionStrategy
    ...
    public Stream<HoodieInstant> getCandidateInstants(
        ...
        // 1. Get completed instants timeline only for commits that have happened since the last
        // successful write.
        // 2. Get any scheduled or completed compaction or clustering operations that have started
        // and/or finished
        // after the current instant. We need to check for write conflicts since they may have
        // mutated the same files
        ...
        Stream<HoodieInstant> compactionAndClusteringPendingTimeline =
                activeTimeline
                        .getTimelineOfActions(
                                CollectionUtils.createSet(REPLACE_COMMIT_ACTION, COMPACTION_ACTION))
    ...
    public boolean hasConflict(
        ...
        if (!intersection.isEmpty()) {
            LOG.info(
                    "Found conflicting writes between first operation = "
                            + thisOperation
                            + ", second operation = "
                            + otherOperation
                            + " , intersecting file ids "
                            + intersection);
            return true;
        }
        return false;
    ...
    public Option<HoodieCommitMetadata> resolveConflict(
            HoodieTable table, ConcurrentOperation thisOperation, ConcurrentOperation otherOperation) {
        // A completed COMPACTION action eventually shows up as a COMMIT action on the timeline.
        // We need to ensure we handle this during conflict resolution and not treat the commit from
        // a
        // compaction operation as a regular commit. Regular commits & deltacommits are candidates
        // for conflict.
        // Since the REPLACE action with CLUSTER operation does not support concurrent updates, we
        // have
        ...
        if (otherOperation.getOperationType() == WriteOperationType.COMPACT) {
            ...
            // Since log compaction is a rewrite operation, it can be committed along with other
            // delta commits.
            // The ordering of the commits is taken care by AbstractHoodieLogRecordReader scan
            // method.
            // Conflict arises only if the log compaction commit has a lesser timestamp compared to
            // compaction commit.
            return thisOperation.getCommitMetadataOption();
        }
