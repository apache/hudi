...
public class HoodieMergeHelper<T> extends BaseMergeHelper {
    ...
    public void runMerge(HoodieTable<?, ?, ?, ?> table, HoodieMergeHandle<?, ?, ?, ?> mergeHandle)
        ...
        HoodieFileReader baseFileReader =
                HoodieIOFactory.getIOFactory(
                                table
                                        .getStorage()
                                        .newInstance(
                                                mergeHandle.getOldFilePath(), table.getStorageConf().newInstance()))
        ...
        Option<Function<HoodieRecord, HoodieRecord>> schemaEvolutionTransformerOpt =
                composeSchemaEvolutionTransformer(
                        readerSchema, writerSchema, baseFile, writeConfig, table.getMetaClient());
        ...
        try {
            ...
            if (baseFile.getBootstrapBaseFile().isPresent()) {
                ...
                HoodieStorage storage =
                        table.getStorage().newInstance(bootstrapFilePath, table.getStorageConf().newInstance());
                bootstrapFileReader =
                        HoodieIOFactory.getIOFactory(storage)
                                .getReaderFactory(recordType)
                                .newBootstrapFileReader(
                                        baseFileReader,
                                        HoodieIOFactory.getIOFactory(storage)
                                                .getReaderFactory(recordType)
                ...
                recordIterator =
                        (ClosableIterator<HoodieRecord>) baseFileReader.getRecordIterator(recordSchema);
            }
                    ...
                    ExecutorFactory.create(
                            ...
                            record -> {
                                ...
                                if (schemaEvolutionTransformerOpt.isPresent()) {
                                    ...
                                    newRecord =
                                            record.rewriteRecordWithNewSchema(
                                                    recordSchema, writeConfig.getProps(), writerSchema);
                                } else {
                                    newRecord = record;
                                }
    ...
    private Option<Function<HoodieRecord, HoodieRecord>> composeSchemaEvolutionTransformer(
            Schema recordSchema,
            Schema writerSchema,
        ...
        if (querySchemaOpt.isPresent() && !baseFile.getBootstrapBaseFile().isPresent()) {
            ...
            if (fileSchema.isEmptySchema()
                ...
                try {
                    ...
                    throw new HoodieException(
                            String.format(
                                    "Failed to get InternalSchema for given versionId: %s", commitInstantTime),
                            e);
                }
            }
            ...
            List<String> sameCols =
                    colNamesFromWriteSchema.stream()
                            .filter(
                                    f -> {
                                        ...
                                        return colNamesFromQuerySchema.contains(f)
                                                ...
                                                && Objects.equals(
                                                        writeInternalSchema.findType(writerSchemaFieldId),
                                                        querySchema.findType(querySchemaFieldId));
                                    })
                            .collect(Collectors.toList());
            InternalSchema mergedSchema =
                    new InternalSchemaMerger(writeInternalSchema, querySchema, true, false, false)
                            .mergeSchema();
                ...
                return Option.of(
                        record -> {
                            return record.rewriteRecordWithNewSchema(
                                    recordSchema, writeConfig.getProps(), newWriterSchema, renameCols);
                        });
            } else {
                return Option.empty();
            }
