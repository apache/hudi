...
public class HoodieMergeHelper<T extends HoodieRecordPayload> extends
  ...
  public void runMerge(HoodieTable<T, HoodieData<HoodieRecord<T>>, HoodieData<HoodieKey>, HoodieData<WriteStatus>> table,
    ...
    try {
      ...
      Function<GenericRecord, GenericRecord> transformer;
      if (!externalSchemaTransformation) {
        transformer = Function.identity();
      } else {
        transformer = record -> transformRecordBasedOnNewSchema(gReader, gWriter, encoderCache,
            decoderCache, record);
      }
      ...
      while (readerIterator.hasNext()) {
        ...
        GenericRecord transformed = transformer.apply(record);

        mergeHandle.write(transformed);
      }
    } catch (Exception e) {
