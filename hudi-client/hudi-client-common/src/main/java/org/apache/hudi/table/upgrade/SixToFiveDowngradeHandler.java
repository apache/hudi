...
/**
  ...
  * compatibility, we need recreate the compaction requested file to .aux folder. Since version 6
  * includes a new schema field for metadata table(MDT), the MDT needs to be deleted during downgrade
  * to avoid column drop error. Also log block version was upgraded in version 6, therefore full
  * compaction needs to be completed during downgrade to avoid both read and future compaction
  * failures.
  */
public class SixToFiveDowngradeHandler implements DowngradeHandler {
    ...
    public Map<ConfigProperty, String> downgrade(
        ...
        // Since version 6 includes a new schema field for metadata table(MDT), the MDT needs to be
        // deleted during downgrade to avoid column drop error.
        HoodieTableMetadataUtil.deleteMetadataTable(config.getBasePath(), context);
        // The log block version has been upgraded in version six so compaction is required for
        // downgrade.
        ...
        try {
            if (table.getMetaClient().getTableType() == HoodieTableType.MERGE_ON_READ) {
                ...
                HoodieInstantTimeGenerator.setCommitTimeZone(
                        table.getMetaClient().getTableConfig().getTimelineTimezone());
