...
/**
  * Marker operations of using timeline server as a proxy to create and delete markers. Each data
  * file has a corresponding marker entry, which is stored in a limited number of underlying files
  * maintained by the timeline server (each file contains multiple marker entries).
  */
public class TimelineServerBasedWriteMarkers extends WriteMarkers {
    ...
    private static final TypeReference<Set<String>> SET_TYPE_REFERENCE =
    ...
    TimelineServerBasedWriteMarkers(
            ...
            String instantTime,
            String timelineServerHost,
            int timelineServerPort,
            int timeoutSecs) {
        super(basePath, markerFolderPath, instantTime);
        this.httpRequestClient =
                new HttpRequestClient(timelineServerHost, timelineServerPort, timeoutSecs, 0);
    }
    ...
    public boolean deleteMarkerDir(HoodieEngineContext context, int parallelism) {
        ...
        try {
            ...
            throw new HoodieRemoteException(
                    "Failed to delete marker directory " + markerDirPath.toString(), e);
        }
    }
    ...
    public boolean doesMarkerDirExist() {
        ...
        try {
            ...
            throw new HoodieRemoteException(
                    "Failed to check marker directory " + markerDirPath.toString(), e);
        }
    }

    @Override
    public Set<String> createdAndMergedDataPaths(HoodieEngineContext context, int parallelism)
            throws IOException {
        Map<String, String> paramsMap =
                Collections.singletonMap(MARKER_DIR_PATH_PARAM, markerDirPath.toString());
        try {
            ...
            throw new HoodieRemoteException(
                    "Failed to get CREATE and MERGE data file paths in " + markerDirPath.toString(), e);
        }
    }
    ...
    public Set<String> allMarkerFilePaths() {
        ...
        try {
            ...
            throw new HoodieRemoteException(
                    "Failed to get all markers in " + markerDirPath.toString(), e);
        }
    }
    ...
    protected Option<StoragePath> create(
        ...
        if (success) {
            return Option.of(
                    new StoragePath(
                            FSUtils.constructAbsolutePath(markerDirPath, partitionPath), markerFileName));
        } else {
            return Option.empty();
        }
    }
    ...
    public Option<StoragePath> createWithEarlyConflictDetection(
        ...
        LOG.info(
                "[timeline-server-based] Created marker file with early conflict detection "
                        + partitionPath
                        + "/"
                        + markerFileName
                        + " in "
    ...
    /**
      ...
      */
    private boolean executeCreateMarkerRequest(
            Map<String, String> paramsMap, String partitionPath, String markerFileName) {
        boolean success;
        try {
            success =
                    httpRequestClient.executeRequest(
            ...
            throw new HoodieRemoteException(
                    "Failed to create marker file " + partitionPath + "/" + markerFileName, e);
        }
        return success;
    }
