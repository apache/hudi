...
@ConfigClassProperty(
        name = "Common Lock Configurations",
...
public class HoodieLockConfig extends HoodieConfig {
    ...
    public static final ConfigProperty<String> LOCK_ACQUIRE_RETRY_WAIT_TIME_IN_MILLIS =
            ConfigProperty.key(LOCK_ACQUIRE_RETRY_WAIT_TIME_IN_MILLIS_PROP_KEY)
                    .defaultValue(DEFAULT_LOCK_ACQUIRE_RETRY_WAIT_TIME_IN_MILLIS)
                    .markAdvanced()
                    .sinceVersion("0.8.0")
                    .withDocumentation(
                            "Initial amount of time to wait between retries to acquire locks, "
    ...
    public static final ConfigProperty<String> LOCK_ACQUIRE_RETRY_MAX_WAIT_TIME_IN_MILLIS =
            ConfigProperty.key(LOCK_ACQUIRE_RETRY_MAX_WAIT_TIME_IN_MILLIS_PROP_KEY)
                    .defaultValue(String.valueOf(16000L))
                    .markAdvanced()
                    .sinceVersion("0.8.0")
                    .withDocumentation(
                            "Maximum amount of time to wait between retries by lock provider client. This bounds"
    ...
    public static final ConfigProperty<String> LOCK_ACQUIRE_CLIENT_RETRY_WAIT_TIME_IN_MILLIS =
            ConfigProperty.key(LOCK_ACQUIRE_CLIENT_RETRY_WAIT_TIME_IN_MILLIS_PROP_KEY)
                    ...
                    .sinceVersion("0.8.0")
                    .withDocumentation(
                            "Amount of time to wait between retries on the lock provider by the lock manager");

    public static final ConfigProperty<String> LOCK_ACQUIRE_NUM_RETRIES =
            ConfigProperty.key(LOCK_ACQUIRE_NUM_RETRIES_PROP_KEY)
                    .defaultValue(DEFAULT_LOCK_ACQUIRE_NUM_RETRIES)
                    .markAdvanced()
                    .sinceVersion("0.8.0")
                    .withDocumentation(
                            "Maximum number of times to retry lock acquire, at each lock provider");

    public static final ConfigProperty<String> LOCK_ACQUIRE_CLIENT_NUM_RETRIES =
            ConfigProperty.key(LOCK_ACQUIRE_CLIENT_NUM_RETRIES_PROP_KEY)
                    .defaultValue(String.valueOf(50))
                    .markAdvanced()
                    .sinceVersion("0.8.0")
                    .withDocumentation(
                            "Maximum number of times to retry to acquire lock additionally from the lock manager.");

    public static final ConfigProperty<Integer> LOCK_ACQUIRE_WAIT_TIMEOUT_MS =
            ConfigProperty.key(LOCK_ACQUIRE_WAIT_TIMEOUT_MS_PROP_KEY)
                    .defaultValue(DEFAULT_LOCK_ACQUIRE_WAIT_TIMEOUT_MS)
                    .markAdvanced()
                    .sinceVersion("0.8.0")
                    .withDocumentation(
                            "Timeout in ms, to wait on an individual lock acquire() call, at the lock provider.");

    public static final ConfigProperty<Integer> LOCK_HEARTBEAT_INTERVAL_MS =
            ConfigProperty.key(LOCK_HEARTBEAT_INTERVAL_MS_KEY)
                    .defaultValue(DEFAULT_LOCK_HEARTBEAT_INTERVAL_MS)
                    .sinceVersion("0.15.0")
                    .withDocumentation(
                            "Heartbeat interval in ms, to send a heartbeat to indicate that hive client holding locks.");

    public static final ConfigProperty<String> FILESYSTEM_LOCK_PATH =
            ConfigProperty.key(FILESYSTEM_LOCK_PATH_PROP_KEY)
                    .noDefaultValue()
                    .markAdvanced()
                    .sinceVersion("0.8.0")
                    .withDocumentation(
                            "For DFS based lock providers, path to store the locks under. use Table's meta path as default");

    public static final ConfigProperty<Integer> FILESYSTEM_LOCK_EXPIRE =
            ConfigProperty.key(FILESYSTEM_LOCK_EXPIRE_PROP_KEY)
                    .defaultValue(0)
                    .markAdvanced()
                    .sinceVersion("0.12.0")
                    .withDocumentation(
                            "For DFS based lock providers, expire time in minutes, must be a non-negative number, default means no expire");

    public static final ConfigProperty<String> HIVE_DATABASE_NAME =
            ConfigProperty.key(HIVE_DATABASE_NAME_PROP_KEY)
                    .noDefaultValue()
                    .markAdvanced()
                    .sinceVersion("0.8.0")
                    .withDocumentation(
                            "For Hive based lock provider, the Hive database to acquire lock against");

    public static final ConfigProperty<String> HIVE_TABLE_NAME =
            ConfigProperty.key(HIVE_TABLE_NAME_PROP_KEY)
                    .noDefaultValue()
                    .markAdvanced()
                    .sinceVersion("0.8.0")
                    .withDocumentation(
                            "For Hive based lock provider, the Hive table to acquire lock against");
    ...
    public static final ConfigProperty<String> HIVE_METASTORE_URI =
            ConfigProperty.key(HIVE_METASTORE_URI_PROP_KEY)
                    ...
                    .sinceVersion("0.8.0")
                    .withDocumentation(
                            "For Hive based lock provider, the Hive metastore URI to acquire locks against.");

    public static final ConfigProperty<String> ZK_BASE_PATH =
            ConfigProperty.key(ZK_BASE_PATH_PROP_KEY)
                    .noDefaultValue()
                    .markAdvanced()
                    .sinceVersion("0.8.0")
                    .withDocumentation(
    ...
    public static final ConfigProperty<Integer> ZK_SESSION_TIMEOUT_MS =
            ConfigProperty.key(ZK_SESSION_TIMEOUT_MS_PROP_KEY)
                    .defaultValue(DEFAULT_ZK_SESSION_TIMEOUT_MS)
                    .markAdvanced()
                    .sinceVersion("0.8.0")
                    .withDocumentation(
                            "Timeout in ms, to wait after losing connection to ZooKeeper, before the session is expired");

    public static final ConfigProperty<Integer> ZK_CONNECTION_TIMEOUT_MS =
            ConfigProperty.key(ZK_CONNECTION_TIMEOUT_MS_PROP_KEY)
    ...
    public static final ConfigProperty<String> ZK_CONNECT_URL =
            ConfigProperty.key(ZK_CONNECT_URL_PROP_KEY)
                    .noDefaultValue()
                    .markAdvanced()
                    .sinceVersion("0.8.0")
    ...
    public static final ConfigProperty<String> ZK_PORT =
            ConfigProperty.key(ZK_PORT_PROP_KEY)
                    .noDefaultValue()
                    .markAdvanced()
                    .sinceVersion("0.8.0")
    ...
    public static final ConfigProperty<String> ZK_LOCK_KEY =
            ConfigProperty.key(ZK_LOCK_KEY_PROP_KEY)
                    .noDefaultValue()
                    .withInferFunction(
                            p -> Option.ofNullable(p.getStringOrDefault(HoodieWriteConfig.TBL_NAME, null)))
                    .markAdvanced()
                    .sinceVersion("0.8.0")
                    .withDocumentation(
                            "Key name under base_path at which to create a ZNode and acquire lock. "
    ...
    public static final ConfigProperty<String> LOCK_PROVIDER_CLASS_NAME =
            ConfigProperty.key(LOCK_PREFIX + "provider")
                    ...
                    .sinceVersion("0.8.0")
                    .withDocumentation(
                            "Lock provider class name, user can provide their own implementation of LockProvider "
    ...
    public static final ConfigProperty<String> WRITE_CONFLICT_RESOLUTION_STRATEGY_CLASS_NAME =
            ConfigProperty.key(LOCK_PREFIX + "conflict.resolution.strategy")
                    .defaultValue(SimpleConcurrentFileWritesConflictResolutionStrategy.class.getName())
                    .withInferFunction(
                            hoodieConfig -> {
                                if (HoodieIndex.IndexType.BUCKET
                                        .name()
                                        .equalsIgnoreCase(
                                                hoodieConfig.getStringOrDefault(HoodieIndexConfig.INDEX_TYPE, null))) {
                                    return Option.of(
                                            BucketIndexConcurrentFileWritesConflictResolutionStrategy.class.getName());
                                } else {
                                    return Option.of(
    ...
    @Deprecated public static final String LOCK_PROVIDER_CLASS_PROP = LOCK_PROVIDER_CLASS_NAME.key();
    /** @deprecated Use {@link #LOCK_PROVIDER_CLASS_NAME} and its methods instead */
    @Deprecated
    ...
    public static class Builder {
        ...
        public HoodieLockConfig.Builder withRetryWaitTimeInMillis(Long retryWaitTimeInMillis) {
            lockConfig.setValue(
                    LOCK_ACQUIRE_RETRY_WAIT_TIME_IN_MILLIS, String.valueOf(retryWaitTimeInMillis));
            return this;
        }

        public HoodieLockConfig.Builder withRetryMaxWaitTimeInMillis(Long retryMaxWaitTimeInMillis) {
            lockConfig.setValue(
                    LOCK_ACQUIRE_RETRY_MAX_WAIT_TIME_IN_MILLIS, String.valueOf(retryMaxWaitTimeInMillis));
            return this;
        }
        ...
        public HoodieLockConfig.Builder withClientRetryWaitTimeInMillis(
                Long clientRetryWaitTimeInMillis) {
            lockConfig.setValue(
        ...
        public HoodieLockConfig.Builder withConflictResolutionStrategy(
