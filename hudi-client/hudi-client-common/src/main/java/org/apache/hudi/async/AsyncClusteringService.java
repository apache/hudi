...
/**
  * Async clustering service that runs in a separate thread. Currently, only one clustering thread is
...
public abstract class AsyncClusteringService extends HoodieAsyncTableService {
    ...
    public AsyncClusteringService(
            HoodieEngineContext context, BaseHoodieWriteClient writeClient, boolean runInDaemonMode) {
    ...
    protected Pair<CompletableFuture, ExecutorService> startService() {
        ExecutorService executor =
                Executors.newFixedThreadPool(
                        maxConcurrentClustering,
                        new CustomizedThreadFactory("async_clustering_thread", isRunInDaemonMode()));
        return Pair.of(
                CompletableFuture.allOf(
                        IntStream.range(0, maxConcurrentClustering)
                                .mapToObj(
                                        i ->
                                                CompletableFuture.supplyAsync(
                                                        () -> {
                                                            try {
                                                                // Set Compactor Pool Name for
                                                                // allowing users to prioritize
                                                                // compaction
                                                                LOG.info(
                                                                        "Setting pool name for clustering to " + CLUSTERING_POOL_NAME);
                                                                context.setProperty(
                                                                        EngineProperty.CLUSTERING_POOL_NAME, CLUSTERING_POOL_NAME);
                                                                ...
                                                                hasError = true;
                                                                LOG.warn(
                                                                        "Clustering executor got interrupted exception! Stopping", ie);
                                                            } catch (IOException e) {
                                                                hasError = true;
