...
public class CommitMetadataResolver {

  protected static HoodieCommitMetadata prepareHoodieCommitMetadata(HoodieEngineContext context, HoodieTable table, String instantTime, WriteOperationType operationType,
                                                                    HoodieWriteConfig config, List<HoodieWriteStat> stats, Option<Map<String, String>> extraMetadata,
    ...
    try {
      ...
      return reconcileMetadataForMissingFiles(table, commitActionType, instantTime, commitMetadata,
          config, context, table.getMetaClient().getStorageConf(), BaseHoodieWriteClient.class.getName());
  ...
  public static HoodieCommitMetadata reconcileMetadataForMissingFiles(HoodieTable table, String commitActionType, String instantTime,
                                                                      ...
                                                                      String classNameForContext) throws HoodieIOException {
    if (table.getMetaClient().getTableConfig().getTableVersion().greaterThanOrEquals(HoodieTableVersion.EIGHT)) {
      // reconciliation not required for table version 8 and above.
      return commitMetadata;
    } else {
      ...
      try {
        ...
        throw new HoodieIOException("Failed to get appended log file paths while reconciling metadata for missing files ", e);
      }
    }
  }
