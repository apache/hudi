...
public class ConsistentHashingUpdateStrategyUtils {
    ...
    /**
      * Construct identifier for the given partitions that are under concurrent resizing (i.e.,
      * clustering).
      *
      * @return map from partition to pair<instant, identifier>, where instant is the clustering
      *     instant.
      */
    public static Map<String, Pair<String, ConsistentBucketIdentifier>>
        ...
        for (Pair<HoodieInstant, HoodieClusteringPlan> pair : instantPlanPairs) {
            ...
            extractHashingMetadataFromClusteringPlan(
                    instant, plan, table, partitions, partitionToHashingMeta, partitionToInstant);
        }
        return partitionToHashingMeta.entrySet().stream()
                .collect(
                        Collectors.toMap(
                                ...
                                e ->
                                        Pair.of(
                                                partitionToInstant.get(e.getKey()),
                                                new ConsistentBucketIdentifier(e.getValue()))));
    }

    private static void extractHashingMetadataFromClusteringPlan(
            String instant,
            HoodieClusteringPlan plan,
            HoodieTable table,
            final Set<String> recordPartitions,
            Map<String, HoodieConsistentHashingMetadata> partitionToHashingMeta,
            Map<String, String> partitionToInstant) {
        for (HoodieClusteringGroup group : plan.getInputGroups()) {
            ...
            ValidationUtils.checkState(
                    p != null, "Clustering plan does not has partition info, plan: " + plan);
            ...
            ValidationUtils.checkState(
                    preInstant == null || preInstant.equals(instant),
                    "Find a partition: " + p + " with two clustering instants");
            if (!partitionToHashingMeta.containsKey(p)) {
                ...
                ValidationUtils.checkState(
                        metadataOption.isPresent(),
                        "Failed to load consistent hashing metadata for partition: " + p);
            ...
            try {
                String nodeJson =
                        group
                                .getExtraMetadata()
                                .get(BaseConsistentHashingBucketClusteringPlanStrategy.METADATA_CHILD_NODE_KEY);
                ...
                throw new HoodieException(
                        "Failed to parse child nodes in clustering plan, partition: "
                                + p
                                + ", cluster group: "
                                + group,
                        e);
