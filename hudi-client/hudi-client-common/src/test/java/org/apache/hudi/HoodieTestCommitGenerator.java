...
public class HoodieTestCommitGenerator {
    ...
    public static void setupTimelineInFS(
            ...
            Map<String, List<Pair<String, String>>> baseFileInfo,
            Map<String, List<Pair<String, String>>> logFileInfo,
            Map<String, Map<String, List<Pair<String, String>>>> instantInfoMap)
            throws IOException {
        ...
        for (String instantTime : baseFileInfo.keySet()) {
            ...
            baseFileInfo
                    .getOrDefault(instantTime, new ArrayList<>())
                    .forEach(
                            e -> {
                                List<Pair<String, String>> fileInfoList =
                                        partitionPathToFileIdAndNameMap.computeIfAbsent(
    ...
    public static String getBaseFilename(String instantTime, String fileId) {
        return FSUtils.makeBaseFileName(
                instantTime,
                BASE_FILE_WRITE_TOKEN,
                fileId,
    ...
    public static void createCommitAndDataFiles(
        ...
        for (String partitionPath : partitionPathToFileIdAndNameMap.keySet()) {
            ...
            partitionPathToFileIdAndNameMap
                    .get(partitionPath)
                    .forEach(
                            fileInfo -> {
                                ...
                                try {
                                    ...
                                    LOG.error(
                                            String.format(
                                                    "Failed to create data file: %s/%s/%s",
    ...
    public static HoodieCommitMetadata generateCommitMetadata(
        ...
        partitionPathToFileIdAndNameMap.forEach(
                (partitionPath, fileInfoList) ->
    ...
    public static void createCommitFileWithMetadata(
            String basePath, Configuration configuration, String filename, byte[] content)
            throws IOException {
        Path commitFilePath =
                new Path(basePath + "/" + HoodieTableMetaClient.METAFOLDER_NAME + "/" + filename);
        try (OutputStream os =
                HadoopFSUtils.getFs(basePath, configuration).create(commitFilePath, true)) {
            os.write(content);
        }
    }

    public static void createDataFile(
            String basePath, Configuration configuration, String partitionPath, String filename)
            throws IOException {
    ...
    public static void createPartitionMetaFile(String basePath, String partitionPath)
            throws IOException {
