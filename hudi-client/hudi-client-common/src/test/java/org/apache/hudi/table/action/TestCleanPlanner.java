...
public class TestCleanPlanner {
    ...
    void testGetDeletePaths(
            HoodieWriteConfig config,
            String earliestInstant,
            List<HoodieFileGroup> allFileGroups,
        ...
        if (!savepoints.isEmpty()) {
            ...
            for (Pair<String, Option<byte[]>> savepoint : savepoints) {
                HoodieInstant instant =
                        new HoodieInstant(false, HoodieTimeline.SAVEPOINT_ACTION, savepoint.getLeft());
        ...
        if (config.getCleanerPolicy() == HoodieCleaningPolicy.KEEP_LATEST_FILE_VERSIONS) {
            when(mockFsView.getAllReplacedFileGroups(partitionPath))
                    .thenReturn(replacedFileGroups.stream());
        } else {
            when(mockFsView.getReplacedFileGroupsBefore(earliestInstant, partitionPath))
                    .thenReturn(replacedFileGroups.stream());
        }
        ...
        HoodieInstant earliestCommitToRetain =
                new HoodieInstant(HoodieInstant.State.COMPLETED, "COMMIT", earliestInstant);
    ...
    void testPartitionsForIncrCleaning(
            boolean isPartitioned,
            HoodieWriteConfig config,
            String earliestInstant,
            String lastCompletedTimeInLastClean,
            String lastCleanInstant,
            String earliestInstantsInLastClean,
            List<String> partitionsInLastClean,
            Map<String, List<String>> savepointsTrackedInLastClean,
            Map<String, List<String>> activeInstantsPartitions,
        ...
        if (!savepoints.isEmpty()) {
            for (Map.Entry<String, List<String>> entry : savepoints.entrySet()) {
                Pair<HoodieSavepointMetadata, Option<byte[]>> savepointMetadataOptionPair =
                        getSavepointMetadata(entry.getValue());
                HoodieInstant instant =
                        new HoodieInstant(false, HoodieTimeline.SAVEPOINT_ACTION, entry.getKey());
                when(activeTimeline.getInstantDetails(instant))
                        .thenReturn(savepointMetadataOptionPair.getRight());
            }
        }

        // prepare last Clean Metadata
        Pair<HoodieCleanMetadata, Option<byte[]>> cleanMetadataOptionPair =
                getCleanCommitMetadata(
                        partitionsInLastClean,
                        lastCleanInstant,
                        earliestInstantsInLastClean,
                        lastCompletedTimeInLastClean,
        ...
        when(hoodieTableMetadata.getAllPartitionPaths())
                .thenReturn(
                        isPartitioned
    ...
    static Stream<Arguments> keepLatestVersionsArgs() {
        ...
        HoodieFileGroup fileGroup =
                buildFileGroup(Arrays.asList(instant4, instant3, instant2, instant1));
        String instant3Path =
                fileGroup
                        .getAllBaseFiles()
                        .filter(baseFile -> baseFile.getCommitTime().equals(instant3))
                        .findFirst()
                        .get()
                        .getPath();
        ...
        String instant4Path =
                fileGroup
                        .getAllBaseFiles()
                        .filter(baseFile -> baseFile.getCommitTime().equals(instant4))
                        .findFirst()
                        .get()
                        .getPath();
        ...
        arguments.add(
                Arguments.of(
                        ...
                        Pair.of(
                                false,
                                Arrays.asList(
                                        expectedCleanFileInfoForInstant3, expectedCleanFileInfoForInstant4))));
        // Four file slices in group but instant4 is part of savepiont: only instant 3's files
        // should be cleaned
        List<Pair<String, Option<byte[]>>> savepoints =
                Collections.singletonList(
                        Pair.of(
                                instant4,
                                getSavepointBytes("partition1", Collections.singletonList(instant4Path))));
        arguments.add(
                Arguments.of(
                        keepLatestVersionsConfig,
                        instant1,
        ...
        List<Pair<String, Option<byte[]>>> replacedFileGroupSavepoint =
                Collections.singletonList(
                        Pair.of(
                                instant4,
    ...
    static Stream<Arguments> keepLatestByHoursOrCommitsArgs() {
        ...
        List<Arguments> arguments = new ArrayList<>();
        // Only one file slice in the group: should still be kept even with commit earlier than
        // "earliestInstant"
        arguments.addAll(
                buildArgumentsForCleanByHoursAndCommitsCases(
                        earliestInstant,
                        Collections.singletonList(
                                buildFileGroup(Collections.singletonList(earliestInstantMinusOneMonth))),
                        Collections.emptyList(),
                        Collections.emptyList(),
                        Pair.of(false, Collections.emptyList())));
        // File group with two slices, both are before the earliestInstant. Only the latest slice
        // should be kept.
        HoodieFileGroup fileGroupsBeforeInstant =
                buildFileGroup(Arrays.asList(earliestInstantMinusOneMonth, earliestInstantMinusOneWeek));
        CleanFileInfo expectedCleanFileInfoForFirstFile =
                new CleanFileInfo(
                        fileGroupsBeforeInstant
                                .getAllBaseFiles()
                                .filter(baseFile -> baseFile.getCommitTime().equals(earliestInstantMinusOneMonth))
                                .findFirst()
                                .get()
                                .getPath(),
                        false);
        ...
        // File group with two slices, one is after the earliestInstant and the other is before the
        // earliestInstant.
        // We should keep both since base files are required for queries evaluating the table at
        // time NOW - 24hrs (24hrs is configured for test)
        arguments.addAll(
                buildArgumentsForCleanByHoursAndCommitsCases(
                        earliestInstant,
                        Collections.singletonList(
                                buildFileGroup(
                                        Arrays.asList(earliestInstantMinusOneMonth, earliestInstantPlusTwoDays))),
                        Collections.emptyList(),
                        Collections.emptyList(),
                        Pair.of(false, Collections.emptyList())));
        // File group with three slices, one is after the earliestInstant and the other two are
        // before the earliestInstant.
        // Oldest slice will be removed since it is not required for queries evaluating the table at
        // time NOW - 24hrs
        String oldestFileInstant = earliestInstantMinusOneMonth;
        HoodieFileGroup fileGroup =
                buildFileGroup(
                        Arrays.asList(
                                oldestFileInstant, earliestInstantMinusThreeDays, earliestInstantPlusTwoDays));
        String oldestFilePath =
                fileGroup
                        .getAllBaseFiles()
                        .filter(baseFile -> baseFile.getCommitTime().equals(oldestFileInstant))
                        .findFirst()
                        .get()
                        .getPath();
        ...
        // File group with three slices, one is after the earliestInstant and the other two are
        // before the earliestInstant. Oldest slice is also in savepoint so should not be removed.
        List<Pair<String, Option<byte[]>>> savepoints =
                Collections.singletonList(
                        Pair.of(
                                oldestFileInstant,
        ...
        HoodieFileGroup replacedFileGroup =
                buildFileGroup(Collections.singletonList(earliestInstantMinusOneMonth));
        String replacedFilePath = replacedFileGroup.getAllBaseFiles().findFirst().get().getPath();
        CleanFileInfo expectedReplaceCleanFileInfo = new CleanFileInfo(replacedFilePath, false);
        arguments.addAll(
                buildArgumentsForCleanByHoursAndCommitsCases(
                        ...
                        Collections.singletonList(replacedFileGroup),
                        Pair.of(false, Collections.singletonList(expectedReplaceCleanFileInfo))));
        // File group is replaced before the earliestInstant but referenced in a savepoint. Should
        // be retained.
        List<Pair<String, Option<byte[]>>> savepointsForReplacedGroup =
                Collections.singletonList(
                        Pair.of(
                                oldestFileInstant,
                                getSavepointBytes("partition1", Collections.singletonList(replacedFilePath))));
        ...
        HoodieWriteConfig writeConfigWithLargerRetention =
                HoodieWriteConfig.newBuilder()
                        .withPath("/tmp")
                        .withCleanConfig(
                                HoodieCleanConfig.newBuilder()
    ...
    static Stream<Arguments> keepLatestByHoursOrCommitsArgsIncrCleanPartitions() {
        ...
        List<String> threePartitionsInActiveTimeline =
                Arrays.asList(PARTITION1, PARTITION2, PARTITION3);
        Map<String, List<String>> activeInstantsPartitionsMap3 = new HashMap<>();
        activeInstantsPartitionsMap3.put(
                earliestInstantMinusThreeDays, threePartitionsInActiveTimeline);
        ...
        // no savepoints tracked in last clean and no additional savepoints. all partitions in
        // uncleaned instants should be expected
        arguments.addAll(
                buildArgumentsForCleanByHoursAndCommitsIncrCleanPartitionsCases(
                        true,
                        earliestInstant,
                        lastCompletedInLastClean,
                        lastCleanInstant,
        ...
        // a new savepoint is added after last clean. but rest of uncleaned touches all partitions,
        // and so all partitions are expected
        arguments.addAll(
                buildArgumentsForCleanByHoursAndCommitsIncrCleanPartitionsCases(
                        true,
                        earliestInstant,
        ...
        // previous clean tracks a savepoint which exists in timeline still. only 2 partitions are
        // touched by uncleaned instants. only 2 partitions are expected
        arguments.addAll(
                buildArgumentsForCleanByHoursAndCommitsIncrCleanPartitionsCases(
                        true,
                        earliestInstant,
        ...
        // savepoint tracked in previous clean was removed(touching partition1). latest uncleaned
        // touched 2 other partitions. But when savepoint is removed, entire list of partitions are
        // expected.
        arguments.addAll(
                buildArgumentsForCleanByHoursAndCommitsIncrCleanPartitionsCases(
                        true,
        ...
        // previous savepoint still exists and touches partition1. uncleaned touches only partition2
        // and partition3. expected partition2 and partition3.
        arguments.addAll(
                buildArgumentsForCleanByHoursAndCommitsIncrCleanPartitionsCases(
                        true,
                        earliestInstant,
        ...
        // a new savepoint was added compared to previous clean. all 2 partitions are expected since
        // uncleaned commits touched just 2 partitions.
        ...
        // 2 savepoints were tracked in previous clean. one of them is removed in latest. When
        // savepoint is removed, entire list of partitions are expected.
        ...
        // 2 savepoints were tracked in previous clean. one of them is removed in latest. But a
        // partition part of removed savepoint is already touched by uncleaned commits.
        ...
        arguments.addAll(
                buildArgumentsForCleanByHoursAndCommitsIncrCleanPartitionsCases(
                        ...
                        Collections.singletonMap(
                                savepoint2, Collections.singletonList(StringUtils.EMPTY_STRING)),
                        activeInstantsUnPartitionsMap,
                        Collections.emptyMap(),
                        unPartitionsInActiveTimeline,
                        false));

        // savepoint tracked in previous clean was removed(touching partition1). active instants
        // does not have the instant corresponding to the savepoint.
        // latest uncleaned touched 2 other partitions. But when savepoint is removed, entire list
        // of partitions are expected.
    ...
    private static List<Arguments> buildArgumentsForCleanByHoursAndCommitsCases(
            String earliestInstant,
            List<HoodieFileGroup> allFileGroups,
            List<Pair<String, Option<byte[]>>> savepoints,
            List<HoodieFileGroup> replacedFileGroups,
    ...
    private static List<Arguments> buildArgumentsForCleanByHoursAndCommitsIncrCleanPartitionsCases(
        ...
        return Arrays.asList(
                Arguments.of(
                        ...
                        lastKnownCleanInstantTime,
                        earliestInstantInLastClean,
                        partitionsInLastClean,
                        savepointsTrackedInLastClean,
                        activeInstantsToPartitionsMap,
                        savepoints,
                        expectedPartitions,
                        areCommitsForSavepointsRemoved),
                Arguments.of(
    ...
    private static HoodieFileGroup buildFileGroup(
            List<String> baseFileCommitTimes, String partition) {
        ...
        for (String baseFileCommitTime : baseFileCommitTimes) {
            ...
            HoodieBaseFile baseFile =
                    new HoodieBaseFile(
                            String.format("file:///tmp/base/%s_1-0-1_%s.parquet", fileGroup, baseFileCommitTime));
    ...
    private static Pair<HoodieCleanMetadata, Option<byte[]>> getCleanCommitMetadata(
            List<String> partitions,
            String instantTime,
            String earliestCommitToRetain,
            String lastCompletedTime,
            Set<String> savepointsToTrack) {
        try {
            ...
            partitions.forEach(
                    partition ->
                            partitionMetadata.put(
                                    partition,
                                    new HoodieCleanPartitionMetadata(
                                            partition,
            ...
            if (!savepointsToTrack.isEmpty()) {
                extraMetadata.put(
                        SAVEPOINTED_TIMESTAMPS, savepointsToTrack.stream().collect(Collectors.joining(",")));
            }
    ...
    private static Pair<HoodieSavepointMetadata, Option<byte[]>> getSavepointMetadata(
            List<String> partitions) {
        try {
            Map<String, HoodieSavepointPartitionMetadata> partitionMetadata = new HashMap<>();
            partitions.forEach(
                    partition ->
                            partitionMetadata.put(
                                    partition,
                                    new HoodieSavepointPartitionMetadata(partition, Collections.emptyList())));
            HoodieSavepointMetadata savepointMetadata =
                    new HoodieSavepointMetadata("user", 1L, "comments", partitionMetadata, 1);
            return Pair.of(
                    savepointMetadata, TimelineMetadataUtils.serializeSavepointMetadata(savepointMetadata));
        } catch (IOException ex) {
            throw new UncheckedIOException(ex);
        }
    }

    private static void mockLastCleanCommit(
        ...
        HoodieInstant latestCleanInstant =
                new HoodieInstant(HoodieInstant.State.COMPLETED, HoodieTimeline.CLEAN_ACTION, timestamp);
    ...
    private static void mockFewActiveInstants(
            HoodieTable hoodieTable,
        ...
        instantstoProcess.forEach(
                (k, v) -> {
                    HoodieInstant hoodieInstant =
                            new HoodieInstant(HoodieInstant.State.COMPLETED, HoodieTimeline.COMMIT_ACTION, k);
                    ...
                    try {
                        when(hoodieTable.getActiveTimeline().getInstantDetails(hoodieInstant))
                                .thenReturn(TimelineMetadataUtils.serializeCommitMetadata(commitMetadata));
                    } catch (IOException e) {
