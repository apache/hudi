...
public class TestPreferWriterConflictResolutionStrategy extends HoodieCommonTestHarness {
    ...
    public void testConcurrentWritesWithInterleavingScheduledCompaction() throws Exception {
        ...
        Option<HoodieInstant> currentInstant =
                Option.of(
                        new HoodieInstant(
                                HoodieInstant.State.INFLIGHT, HoodieTimeline.COMMIT_ACTION, currentWriterInstant));
    ...
    public void testConcurrentWritesWithInterleavingSuccessfulCompaction() throws Exception {
        ...
        // Sleep thread for at least 1sec for consecutive commits that way they do not have two
        // commits modified times falls on the same millisecond.
        Thread.sleep(1000);
        ...
        ConcurrentOperation thatCommitOperation =
                new ConcurrentOperation(candidateInstants.get(0), metaClient);
        ConcurrentOperation thisCommitOperation =
                new ConcurrentOperation(currentInstant.get(), currentMetadata);
    ...
    public void testConcurrentWritesWithInterleavingCompaction() throws Exception {
        ...
        Option<HoodieInstant> currentInstant =
                Option.of(
                        new HoodieInstant(
                                HoodieInstant.State.INFLIGHT, HoodieTimeline.COMPACTION_ACTION, newInstantTime));
        PreferWriterConflictResolutionStrategy strategy = new PreferWriterConflictResolutionStrategy();
    ...
    /**
      * This method is verifying if a conflict exists for already commit compaction commit with current
      * running ingestion commit.
      */
    @Test
    public void testConcurrentWriteAndCompactionScheduledEarlier() throws Exception {
        ...
        List<HoodieInstant> candidateInstants =
                strategy
                        .getCandidateInstants(metaClient, currentInstant.get(), lastSuccessfulInstant)
                        .collect(Collectors.toList());
        // writer 1 should not conflict with an earlier scheduled compaction 1 with the same file
        // ids
        Assertions.assertEquals(0, candidateInstants.size());
    }
    ...
    public void testConcurrentWritesWithInterleavingScheduledCluster() throws Exception {
        ...
        List<HoodieInstant> candidateInstants =
                strategy
                        .getCandidateInstants(metaClient, currentInstant.get(), lastSuccessfulInstant)
                        .collect(Collectors.toList());
        // Since we give preference to ingestion over clustering, there won't be a conflict with
        // replacecommit.
        Assertions.assertEquals(0, candidateInstants.size());
    }

    /**
      * This method confirms ingestion commit failing due to already present replacecommit. Here the
      * replacecommit is allowed to commit. Ideally replacecommit cannot be committed when there is an
      * ingestion inflight. The following case can occur, during transition phase of ingestion commit
      * from Requested to Inflight, during that time replacecommit can be completed.
      */
    @Test
