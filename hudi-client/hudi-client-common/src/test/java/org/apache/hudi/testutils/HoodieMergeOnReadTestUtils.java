...
public class HoodieMergeOnReadTestUtils {
    ...
    public static List<RecordReader> getRecordReadersUsingInputFormat(
            Configuration conf,
            List<String> inputPaths,
            String basePath,
            JobConf jobConf,
            boolean realtime,
            boolean populateMetaField) {
        Schema schema = new Schema.Parser().parse(HoodieTestDataGenerator.TRIP_EXAMPLE_SCHEMA);
        return getRecordReadersUsingInputFormat(
                conf,
                inputPaths,
                basePath,
                jobConf,
    ...
    public static List<RecordReader> getRecordReadersUsingInputFormat(
            ...
            boolean realtime,
            Schema rawSchema,
            String rawHiveColumnTypes,
            boolean projectCols,
            List<String> projectedColumns,
            boolean populateMetaFields) {
        HoodieTableMetaClient metaClient =
                HoodieTestUtils.createMetaClient(HadoopFSUtils.getStorageConf(conf), basePath);
        FileInputFormat inputFormat =
                HoodieInputFormatUtils.getInputFormat(
                        metaClient.getTableConfig().getBaseFileFormat(), realtime, jobConf);
        Schema schema;
    ...
    public static List<GenericRecord> getRecordsUsingInputFormat(
            StorageConfiguration<Configuration> conf, List<String> inputPaths, String basePath) {
        ...
        if (projectCols) {
            fields =
                    schema.getFields().stream()
                            .filter(f -> projectedColumns.contains(f.name()))
                ...
                while (recordReader.next(key, writable)) {
                    ...
                    schema.getFields().stream()
                            ...
                            .map(
                                    f ->
                                            Pair.of(
                                                    projectedSchema.getFields().stream()
                                                            .filter(p -> f.name().equals(p.name()))
                                                            .findFirst()
                                                            .get(),
                                                    f))
                            .forEach(
                                    fieldsPair ->
                                            newRecord.set(fieldsPair.getKey(), values[fieldsPair.getValue().pos()]));
    ...
    private static void setPropsForInputFormat(
        ...
        setPropsForInputFormat(
                inputFormat, jobConf, schema, hiveColumnTypes, projectCols, projectedCols, true);
    }
