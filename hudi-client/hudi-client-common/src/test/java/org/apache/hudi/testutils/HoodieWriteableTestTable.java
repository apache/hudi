...
public class HoodieWriteableTestTable extends HoodieMetadataTestTable {
    ...
    protected HoodieWriteableTestTable(
            String basePath,
    ...
    public StoragePath withInserts(
            String partition,
            String fileId,
            List<HoodieRecord> records,
            TaskContextSupplier contextSupplier)
            throws Exception {
        ...
        if (HoodieTableConfig.BASE_FILE_FORMAT.defaultValue().equals(HoodieFileFormat.PARQUET)) {
            HoodieAvroWriteSupport writeSupport =
                    new HoodieAvroWriteSupport(
                            ...
                            Option.of(filter),
                            new Properties());
            HoodieParquetConfig<HoodieAvroWriteSupport> config =
            ...
            try (HoodieAvroParquetWriter writer =
                ...
                for (HoodieRecord record : records) {
                    GenericRecord avroRecord =
                            (GenericRecord) ((HoodieRecordPayload) record.getData()).getInsertValue(schema).get();
                    if (populateMetaFields) {
                        HoodieAvroUtils.addCommitMetadataToRecord(
                                avroRecord, currentInstantTime, String.valueOf(seqId++));
                        HoodieAvroUtils.addHoodieKeyToRecord(
                                avroRecord, record.getRecordKey(), record.getPartitionPath(), fileName);
            ...
            try (HoodieAvroOrcWriter writer =
                    new HoodieAvroOrcWriter(
                            currentInstantTime,
                            new StoragePath(Paths.get(basePath, partition, fileName).toString()),
                            config,
                            schema,
    ...
    public Map<String, List<HoodieLogFile>> withLogAppends(
        ...
        partitionToLogfilesMap
                .computeIfAbsent(appendedLogFile.getKey(), k -> new ArrayList<>())
                .add(appendedLogFile.getValue());
        return partitionToLogfilesMap;
    }

    private Pair<String, HoodieLogFile> appendRecordsToLogFile(
            String partitionPath, String fileId, List<HoodieRecord> records) throws Exception {
                                    ...
                                    .map(
                                            r -> {
                                                try {
                                                    GenericRecord val =
                                                            (GenericRecord)
                                                                    ((HoodieRecordPayload) r.getData()).getInsertValue(schema).get();
                                                    HoodieAvroUtils.addHoodieKeyToRecord(
                                                            val, r.getRecordKey(), r.getPartitionPath(), "");
