...
public class TestBucketIndexConcurrentFileWritesConflictResolutionStrategy
    ...
    public void testNoConcurrentWrites() throws Exception {
        ...
        Option<HoodieInstant> currentInstant =
                Option.of(new HoodieInstant(State.INFLIGHT, HoodieTimeline.COMMIT_ACTION, newInstantTime));
        ...
        Stream<HoodieInstant> candidateInstants =
                strategy.getCandidateInstants(metaClient, currentInstant.get(), lastSuccessfulInstant);
    ...
    public void testConcurrentWrites() throws Exception {
        ...
        createInflightCommit(
                HoodieTestTable.makeNewCommitTime(), HoodieTestDataGenerator.DEFAULT_FIRST_PARTITION_PATH);
        // writer 2
        createInflightCommit(
                HoodieTestTable.makeNewCommitTime(), HoodieTestDataGenerator.DEFAULT_FIRST_PARTITION_PATH);
        Option<HoodieInstant> lastSuccessfulInstant =
    ...
    public void testConcurrentWritesWithInterleavingSuccessfulCommit() throws Exception {
        ...
        createInflightCommit(
                currentWriterInstant, HoodieTestDataGenerator.DEFAULT_FIRST_PARTITION_PATH);
        ...
        Option<HoodieInstant> currentInstant =
                Option.of(
                        new HoodieInstant(State.INFLIGHT, HoodieTimeline.COMMIT_ACTION, currentWriterInstant));
        SimpleConcurrentFileWritesConflictResolutionStrategy strategy =
                new BucketIndexConcurrentFileWritesConflictResolutionStrategy();
    ...
    public void testConcurrentWritesWithDifferentPartition() throws Exception {
        ...
        String currentWriterInstant = metaClient.createNewInstantTime();
        createInflightCommit(
                currentWriterInstant, HoodieTestDataGenerator.DEFAULT_SECOND_PARTITION_PATH);
        // writer 2 starts and finishes
        String newInstantTime = metaClient.createNewInstantTime();
        createCommit(newInstantTime);
    ...
    private HoodieCommitMetadata createCommitMetadata(
            String instantTime, String writeFileName, String partition) {
        HoodieCommitMetadata commitMetadata = new HoodieCommitMetadata();
        commitMetadata.addMetadata("test", "test");
        HoodieWriteStat writeStat = new HoodieWriteStat();
