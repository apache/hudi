...
public class TestTransactionManager extends HoodieCommonTestHarness {
    ...
    private void init(TestInfo testInfo) throws IOException {
        ...
        this.transactionManager =
                new TransactionManager(this.writeConfig, this.metaClient.getStorage());
    }
        ...
        return HoodieWriteConfig.newBuilder()
                        ...
                        HoodieLockConfig.newBuilder()
                                .withLockProvider(
                                        useLockProviderWithRuntimeError
                                                ? InProcessLockProviderWithRuntimeError.class
                ...
                .withMetricsConfig(
                        HoodieMetricsConfig.newBuilder()
                                .withReporterType(MetricsReporterType.INMEMORY.toString())
    ...
    public void testTransactionsWithInstantTime() {
        ...
        Assertions.assertTrue(
                transactionManager.getLastCompletedTransactionOwner() == lastCompletedInstant);
        transactionManager.endTransaction(newTxnOwnerInstant);
        Assertions.assertFalse(transactionManager.getCurrentTransactionOwner().isPresent());
        Assertions.assertFalse(transactionManager.getLastCompletedTransactionOwner().isPresent());
        ...
        // Owner reset would not happen as the end txn was invoked with an incorrect current txn
        // owner
        Assertions.assertTrue(transactionManager.getCurrentTransactionOwner() == newTxnOwnerInstant);
        Assertions.assertTrue(
                transactionManager.getLastCompletedTransactionOwner() == lastCompletedInstant);
    ...
    private Option<HoodieInstant> getInstant(String timestamp) {
        return Option.of(
                new HoodieInstant(HoodieInstant.State.REQUESTED, HoodieTimeline.COMMIT_ACTION, timestamp));
    }
}
