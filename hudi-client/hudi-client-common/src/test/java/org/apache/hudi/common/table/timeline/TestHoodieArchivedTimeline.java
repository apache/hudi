...
public class TestHoodieArchivedTimeline extends HoodieCommonTestHarness {
    ...
    public void testLoadingInstantsIncrementally() throws Exception {
        ...
        assertThat(
                archivedTimeline.firstInstant().map(HoodieInstant::getTimestamp).orElse(""),
                is("10000043"));
        assertThat(
                archivedTimeline.lastInstant().map(HoodieInstant::getTimestamp).orElse(""), is("10000050"));
        // load incrementally
    ...
    private void writeArchivedTimeline(int batchSize, long startTs) throws Exception {
        ...
        HoodieWriteConfig writeConfig =
                HoodieWriteConfig.newBuilder()
                        .withPath(this.metaClient.getBasePath())
                        .withIndexConfig(
        ...
        LSMTimelineWriter writer =
                LSMTimelineWriter.getInstance(writeConfig, new LocalTaskContextSupplier(), metaClient);
