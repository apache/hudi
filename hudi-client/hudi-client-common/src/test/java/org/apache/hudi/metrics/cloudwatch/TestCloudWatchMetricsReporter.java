...
public class TestCloudWatchMetricsReporter {
    ...
    @Mock private HoodieMetricsConfig metricsConfig;

    @Mock private MetricRegistry registry;

    @Mock private CloudWatchReporter reporter;

    @Test
    public void testReporter() {
        when(metricsConfig.getCloudWatchReportPeriodSeconds()).thenReturn(30);
        CloudWatchMetricsReporter metricsReporter =
                new CloudWatchMetricsReporter(metricsConfig, registry, reporter);
    ...
    public void testReporterViaReporterFactory() {
        try {
            ...
            // This test verifies that reflection is working well and is able to invoke the
            // CloudWatchMetricsReporter constructor
            MetricsReporterFactory.createReporter(metricsConfig, registry).get();
        } catch (Exception e) {
            assertTrue(e.getCause() instanceof InvocationTargetException);
            assertTrue(
                    Arrays.stream(
                                    ((InvocationTargetException) e.getCause()).getTargetException().getStackTrace())
                            .anyMatch(
                                    ste ->
                                            ste.toString()
                                                    .contains(
                                                            "org.apache.hudi.aws.cloudwatch.CloudWatchReporter.getAmazonCloudWatchClient")));
        }
    }
}
