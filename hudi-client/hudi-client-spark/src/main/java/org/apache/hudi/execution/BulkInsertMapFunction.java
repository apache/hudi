...
public class BulkInsertMapFunction<T extends HoodieRecordPayload>
  ...
  private BaseHoodieTable hoodieTable;
  private List<String> fileIDPrefixes;

  public BulkInsertMapFunction(String instantTime, HoodieWriteConfig config, BaseHoodieTable hoodieTable,
  ...
  public Iterator<List<WriteStatus>> call(Integer partition, Iterator<HoodieRecord<T>> sortedRecordItr) {
    return new LazyInsertIterable<T>(sortedRecordItr, config, instantTime, hoodieTable,
        fileIDPrefixes.get(partition), (SparkTaskContextSupplier) hoodieTable.getTaskContextSupplier());
  }
