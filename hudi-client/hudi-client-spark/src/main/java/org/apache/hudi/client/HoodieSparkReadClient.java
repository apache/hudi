 ...
 */
public class HoodieSparkReadClient<T extends HoodieRecordPayload> implements Serializable {
  ...
  private BaseHoodieTable<T, JavaRDD<HoodieRecord<T>>, JavaRDD<HoodieKey>, JavaRDD<WriteStatus>, JavaPairRDD<HoodieKey, Option<Pair<String, String>>>> hoodieTable;
  private transient HoodieSparkEngineContext context;
  private transient Option<SQLContext> sqlContextOpt;
  private final transient Configuration hadoopConf;

  public HoodieSparkReadClient(HoodieSparkEngineContext context, HoodieWriteConfig clientConfig) {
    ...
    this.hoodieTable = HoodieSparkTable.create(metaClient, clientConfig, hadoopConf);
    this.index = hoodieTable.getIndex();
    this.sqlContextOpt = context.getSqlContext() == null ? Option.empty() : Option.of(context.getSqlContext());
  }
