    ...
    private HoodieJavaWriteClient<RawTripTestPayload> getWriteClient() {
        HoodieWriteConfig config =
                HoodieWriteConfig.newBuilder()
                        ...
                        .withProps(
                                Collections.singletonMap(HoodieWriteConfig.TBL_NAME.key(), "hoodie_test_table"))
                        .build();
        return new HoodieJavaWriteClient<>(context, config);
    }

    private void prepareTable(HoodieJavaWriteClient<RawTripTestPayload> writeClient)
            throws IOException {
        String commitTime = "1";
        ...
        HoodieAvroRecord<RawTripTestPayload> record =
