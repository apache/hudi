...
public class JavaBulkInsertHelper<T, R>
    ...
    public HoodieWriteMetadata<List<WriteStatus>> bulkInsert(
            final List<HoodieRecord<T>> inputRecords,
            final String instantTime,
            final HoodieTable<T, List<HoodieRecord<T>>, List<HoodieKey>, List<WriteStatus>> table,
            final HoodieWriteConfig config,
            final BaseCommitActionExecutor<
                            T, List<HoodieRecord<T>>, List<HoodieKey>, List<WriteStatus>, R>
                    executor,
    ...
    public List<WriteStatus> bulkInsert(
        ...
        if (performDedupe) {
            dedupedRecords =
                    (List<HoodieRecord<T>>)
                            JavaWriteHelper.newInstance()
                                    .combineOnCondition(
                                            config.shouldCombineBeforeInsert(), inputRecords, targetParallelism, table);
        }
        ...
        if (partitioner instanceof JavaHoodieMetadataBulkInsertPartitioner) {
            ...
            FileIdPrefixProvider fileIdPrefixProvider =
        ...
        new JavaLazyInsertIterable<>(
                        repartitionedRecords.iterator(),
                        true,
                        config,
                        instantTime,
                        table,
                        ...
                        // Always get the first WriteHandleFactory, as there is only a single data
                        // partition for hudi java engine.
                        (WriteHandleFactory) partitioner.getWriteHandleFactory(0).orElse(writeHandleFactory))
