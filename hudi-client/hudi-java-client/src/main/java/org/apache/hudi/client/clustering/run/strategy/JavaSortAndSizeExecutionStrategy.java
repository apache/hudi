...
/**
  * Clustering Strategy based on following. 1) Java execution engine. 2) Uses bulk_insert to write
  * data into new files.
  */
public class JavaSortAndSizeExecutionStrategy<T> extends JavaExecutionStrategy<T> {
    private static final Logger LOG = LoggerFactory.getLogger(JavaSortAndSizeExecutionStrategy.class);

    public JavaSortAndSizeExecutionStrategy(
        ...
        return (List<WriteStatus>)
                JavaBulkInsertHelper.newInstance()
                        .bulkInsert(
                                ...
                                newConfig,
                                false,
                                getPartitioner(strategyParams, schema),
                                true,
                                numOutputGroups,
