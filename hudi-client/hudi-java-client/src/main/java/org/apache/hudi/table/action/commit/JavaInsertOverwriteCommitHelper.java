...
public class JavaInsertOverwriteCommitHelper<T extends HoodieRecordPayload<T>> extends JavaCommitHelper<T> {
  public JavaInsertOverwriteCommitHelper(
  ...
  public Map<String, List<String>> getPartitionToReplacedFileIds(
      HoodieWriteMetadata<HoodieData<WriteStatus>> writeResult) {
    return context.mapToPair(
        getList(writeResult.getWriteStatuses()).stream().map(
            status -> status.getStat().getPartitionPath()).distinct().collect(Collectors.toList()),
        partitionPath ->
