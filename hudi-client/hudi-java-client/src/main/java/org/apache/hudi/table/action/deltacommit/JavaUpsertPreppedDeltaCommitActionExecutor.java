...
public class JavaUpsertPreppedDeltaCommitActionExecutor<T>
    ...
    private static final Logger LOG =
            LoggerFactory.getLogger(JavaUpsertPreppedDeltaCommitActionExecutor.class);

    private final List<HoodieRecord<T>> preppedInputRecords;

    public JavaUpsertPreppedDeltaCommitActionExecutor(
    ...
    public HoodieWriteMetadata<List<WriteStatus>> execute() {
        ...
        LOG.info(
                String.format(
                        "Total update fileIDs %s, total inserts %s for commit %s",
            ...
            if (insertedRecords.size() > 0) {
                HoodieWriteMetadata<List<WriteStatus>> insertResult =
                        JavaBulkInsertHelper.newInstance()
                                .bulkInsert(
                                        insertedRecords, instantTime, table, config, this, false, Option.empty());
