...
public class JavaUpsertPreppedDeltaCommitActionExecutor<T> extends BaseJavaDeltaCommitActionExecutor<T> {
  ...
  public JavaUpsertPreppedDeltaCommitActionExecutor(HoodieJavaEngineContext context,
  ...
  public HoodieWriteMetadata<List<WriteStatus>> execute() {
    ...
    try {
      ...
      if (insertedRecords.size() > 0) {
        HoodieWriteMetadata<List<WriteStatus>> insertResult = JavaBulkInsertHelper.<T, HoodieWriteMetadata<List<WriteStatus>>>newInstance()
