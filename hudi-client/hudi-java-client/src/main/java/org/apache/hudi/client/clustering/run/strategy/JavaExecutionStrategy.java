...
public abstract class JavaExecutionStrategy<T>
    ...
    /**
      ...
      */
    protected BulkInsertPartitioner<List<HoodieRecord<T>>> getPartitioner(
    ...
    private List<WriteStatus> runClusteringForGroup(
        ...
        Schema readerSchema =
                HoodieAvroUtils.addMetadataFields(new Schema.Parser().parse(getWriteConfig().getSchema()));
        ...
        return performClusteringWithRecordList(
                inputRecords,
    ...
    /**
      * Get a list of all records for the group. This includes all records from file slice (Apply
      * updates from log files, if any).
      */
    private List<HoodieRecord<T>> readRecordsForGroup(
            HoodieClusteringGroup clusteringGroup, String instantTime) {
    ...
    private List<HoodieRecord<T>> readRecordsForGroupWithLogs(
        ...
        clusteringOps.forEach(
                clusteringOp -> {
                    long maxMemoryPerCompaction =
                            IOUtils.getMaxMemoryPerCompaction(new JavaTaskContextSupplier(), config);
                                ...
                                StringUtils.isNullOrEmpty(clusteringOp.getDataFilePath())
                                        ...
                                        : Option.of(
                                                HoodieIOFactory.getIOFactory(table.getStorage())
                                                        ...
                                                        .getFileReader(
                                                                config, new StoragePath(clusteringOp.getDataFilePath())));
    ...
    private List<HoodieRecord<T>> readRecordsForGroupBaseFiles(
            List<ClusteringOperation> clusteringOps) {
        List<HoodieRecord<T>> records = new ArrayList<>();
        clusteringOps.forEach(
                clusteringOp -> {
                    try (HoodieFileReader baseFileReader =
                            HoodieIOFactory.getIOFactory(getHoodieTable().getStorage())
                                    .getReaderFactory(recordType)
                                    .getFileReader(
                                            getHoodieTable().getConfig(),
                        ...
                        // engine-specific
                        //       payload pointing into a shared, mutable (underlying) buffer we get
                        // a clean copy of
                                        ...
                                        records.add(
                                                record
                                                        ...
                                                        .wrapIntoHoodieRecordPayloadWithKeyGen(
                                                                readerSchema, new Properties(), Option.empty())));
                    } catch (IOException e) {
                        throw new HoodieClusteringException(
                                "Error reading input data for "
                                        + clusteringOp.getDataFilePath()
