...
public class JavaUpsertPreppedDeltaCommitHelper<T extends HoodieRecordPayload<T>> extends JavaCommitHelper<T> {

  private static final Logger LOG = LogManager.getLogger(JavaUpsertPreppedDeltaCommitHelper.class);

  public JavaUpsertPreppedDeltaCommitHelper(
      HoodieEngineContext context, HoodieWriteConfig config, HoodieTable table, String instantTime) {
    super(context, config, table, instantTime, WriteOperationType.UPSERT_PREPPED);
  }

  @Override
  public HoodieWriteMetadata<HoodieData<WriteStatus>> execute(
      HoodieData<HoodieRecord<T>> preppedInputRecords) {
    ...
    for (HoodieRecord<T> record : getList(preppedInputRecords)) {
    ...
    try {
      recordsByFileId.forEach((k, v) -> {
        HoodieAppendHandle<?, ?, ?, ?> appendHandle = new HoodieAppendHandle(config, instantTime, table,
            k.getRight(), k.getLeft(), v.iterator(), context.getTaskContextSupplier());
      ...
      if (insertedRecords.size() > 0) {
        HoodieWriteMetadata<List<WriteStatus>> insertResult = JavaBulkInsertHelper.newInstance()
            .bulkInsert(HoodieListData.of(insertedRecords), instantTime, table, config, false,
                Option.empty(), this);
