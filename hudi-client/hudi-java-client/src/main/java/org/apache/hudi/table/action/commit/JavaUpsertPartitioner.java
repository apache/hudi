...
public class JavaUpsertPartitioner<T> implements Partitioner {
    ...
    /**
      * Stat for the input and output workload. Describe the workload before and after being assigned
      * buckets.
      */
    ...
    private Map<String, List<SmallFile>> getSmallFilesForPartitions(
        ...
        if (partitionPaths != null && partitionPaths.size() > 0) {
            ...
            partitionSmallFilesMap =
                    context.mapToPair(
                            partitionPaths,
                            partitionPath -> new ImmutablePair<>(partitionPath, getSmallFiles(partitionPath)),
                            0);
        }
        return partitionSmallFilesMap;
    }
    ...
    public int getPartition(Object key) {
        ...
        if (keyLocation.getRight().isPresent()) {
            ...
            final long totalInserts =
                    Math.max(1, workloadProfile.getWorkloadStat(partitionPath).getNumInserts());
            final long hashOfKey =
                    NumericUtils.getMessageDigestHash("MD5", keyLocation.getLeft().getRecordKey());
    ...
    /**
      ...
      */
    protected static long averageBytesPerRecord(
            HoodieTimeline commitTimeline, HoodieWriteConfig hoodieWriteConfig) {
        ...
        try {
            if (!commitTimeline.empty()) {
                // Go over the reverse ordered commits to get a more recent estimate of average
                // record size.
    ...
    public List<String> getSmallFileIds() {
        return smallFiles.stream()
                .map(smallFile -> smallFile.location.getFileId())
                .collect(Collectors.toList());
    }
}
