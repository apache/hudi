...
/**
  * Implementation of the {@link SinkTask} interface provided by Kafka Connect. Implements methods to
  * receive the Kafka records from the assigned partitions and commit the Kafka offsets. Also,
  * handles re-assignments of partitions.
  */
public class HoodieSinkTask extends SinkTask {
    ...
    public void start(Map<String, String> props) {
        ...
        LOG.info(
                String.format(
                        "Starting Hudi Sink Task for %s connector %s with id %s with assignments %s",
        ...
        try {
            ...
            controlKafkaClient =
                    KafkaConnectControlAgent.createKafkaControlManager(
                            connectConfigs.getBootstrapServers(), connectConfigs.getControlTopicName());
        ...
        for (TopicPartition partition : context.assignment()) {
            if (transactionParticipants.get(partition) == null) {
                throw new RetriableException(
                        "TransactionParticipant should be created for each assigned partition, "
                                + "but has not been created for the topic/partition: "
                                + partition.topic()
                                + ":"
                                + partition.partition());
            }
            try {
                ...
                throw new RetriableException(
                        "Intermittent write errors for Hudi "
                                + " for the topic/partition: "
                                + partition.topic()
                                + ":"
                                + partition.partition()
                                + " , ensuring kafka connect will retry ",
                        exception);
            }
        }
    }
    ...
    public Map<TopicPartition, OffsetAndMetadata> preCommit(
        ...
        // Although the connector manages offsets via commit files in Hudi, we still want to have
        // Connect
        // commit the consumer offsets for records this task has consumed from its topic partitions
        // and
        // committed to Hudi.
    ...
    public void close(Collection<TopicPartition> partitions) {
        ...
        // some effort since we'll have to reprocess those messages. It may be possible to hold on
        // to
        // the TopicPartitionWriter and continue to use the temp file, but this can get
        // significantly
    ...
    private void bootstrap(Collection<TopicPartition> partitions) {
        LOG.info(
                String.format(
                        "Bootstrap task for connector %s with id %s with assignments %s part %s",
                        connectorName, taskId, context.assignment(), partitions));
        for (TopicPartition partition : partitions) {
            try {
                // If the partition is 0, instantiate the Leader
                if (partition.partition() == ConnectTransactionCoordinator.COORDINATOR_KAFKA_PARTITION) {
                    ConnectTransactionCoordinator coordinator =
                            new ConnectTransactionCoordinator(connectConfigs, partition, controlKafkaClient);
                    coordinator.start();
                    transactionCoordinators.put(partition, coordinator);
                }
                ConnectTransactionParticipant worker =
                ...
                LOG.error(
                        String.format(
                                "Fatal error initializing task %s for partition %s", taskId, partition.partition()),
                        exception);
            }
        }
    }
    ...
    private void cleanup() {
        ...
        transactionCoordinators.forEach(
                (topic, transactionCoordinator) -> transactionCoordinator.stop());
