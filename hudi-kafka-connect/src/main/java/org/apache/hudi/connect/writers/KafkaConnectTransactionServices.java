...
/**
  * Implementation of Transaction service APIs used by {@link TransactionCoordinator} using {@link
...
public class KafkaConnectTransactionServices implements ConnectTransactionServices {
    ...
    public KafkaConnectTransactionServices(KafkaConnectConfigs connectConfigs)
        ...
        try {
            ...
            String partitionColumns =
                    KafkaConnectUtils.getPartitionColumns(
                            keyGenerator, new TypedProperties(connectConfigs.getProps()));

            LOG.info(
                    String.format(
                            "Setting record key %s and partition fields %s for table %s",
    ...
    public boolean endCommit(
            String commitTime, List<WriteStatus> writeStatuses, Map<String, String> extraMetadata) {
        ...
        if (success) {
            ...
            if (writeConfig.isAsyncClusteringEnabled()) {
                javaClient
                        .scheduleClustering(Option.empty())
                        .ifPresent(instantTs -> LOG.info("Scheduled clustering at instant time:" + instantTs));
            }
            if (isAsyncCompactionEnabled()) {
                javaClient
                        .scheduleCompaction(Option.empty())
                        .ifPresent(instantTs -> LOG.info("Scheduled compaction at instant time:" + instantTs));
            }
            syncMeta();
        }
        return success;
    ...
    public Map<String, String> fetchLatestExtraCommitMetadata() {
        ...
        throw new HoodieException(
                "Fatal error retrieving Hoodie Extra Metadata since Table Meta Client is absent");
    }
    ...
    private void syncMeta() {
        if (connectConfigs.isMetaSyncEnabled()) {
            Set<String> syncClientToolClasses =
                    new HashSet<>(Arrays.asList(connectConfigs.getMetaSyncClasses().split(",")));
            ...
            for (String impl : syncClientToolClasses) {
                ...
                String baseFileFormat =
                        connectConfigs.getStringOrDefault(HoodieSyncConfig.META_SYNC_BASE_FILE_FORMAT);
