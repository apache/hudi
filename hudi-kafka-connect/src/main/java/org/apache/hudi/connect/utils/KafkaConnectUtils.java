...
public class KafkaConnectUtils {
    ...
    static {
        ...
        try {
            ...
            LOG.error(
                    "An error occurred while getting the default Hadoop configuration. "
                            + "Please use hadoop.conf.dir or hadoop.home to configure Hadoop environment variables",
                    e);
        }
    }
    ...
    public static int getLatestNumPartitions(String bootstrapServers, String topicName) {
        ...
        try {
            ...
            LOG.info(
                    String.format(
                            "Latest number of partitions for topic %s is %s", topicName, numPartitions));
            return numPartitions;
        } catch (Exception exception) {
            throw new HoodieException(
                    "Fatal error fetching the latest partition of kafka topic name" + topicName, exception);
        }
    }
        ...
        if (!StringUtils.isNullOrEmpty(connectConfigs.getHadoopConfDir())
            ...
            try {
                List<Path> configFiles =
                        getHadoopConfigFiles(
                                connectConfigs.getHadoopConfDir(), connectConfigs.getHadoopConfHome());
                configFiles.forEach(
                        f -> hadoopConf.addResource(new org.apache.hadoop.fs.Path(f.toAbsolutePath().toUri())));
            } catch (Exception e) {
                throw new HoodieException("Failed to read hadoop configuration!", e);
            }
        } else {
            DEFAULT_HADOOP_CONF_FILES.forEach(
                    f -> hadoopConf.addResource(new org.apache.hadoop.fs.Path(f.toAbsolutePath().toUri())));
        }

        connectConfigs.getProps().keySet().stream()
                .filter(
                        prop -> {
                ...
                .forEach(
                        prop -> {
                            hadoopConf.set(
                                    prop.toString(), connectConfigs.getProps().get(prop.toString()).toString());
                        });
    ...
    /**
      * Extract partition columns directly if an instance of class {@link BaseKeyGenerator}, else
      * extract partition columns from the properties.
      *
      * @param keyGenerator key generator Instance of the keygenerator.
      ...
      */
    public static String getPartitionColumns(
            KeyGenerator keyGenerator, TypedProperties typedProperties) {
        if (keyGenerator instanceof CustomAvroKeyGenerator) {
            return ((BaseKeyGenerator) keyGenerator)
                            ...
                            .map(
                                    pathField ->
                                            Arrays.stream(pathField.split(CustomAvroKeyGenerator.SPLIT_REGEX))
                                                    ...
                                                    .orElseGet(
                                                            () ->
                                                                    "Illegal partition path field format: '$pathField' for ${c.getClass.getSimpleName}"))
    ...
    /**
      ...
      * @return An Optional {@link HoodieCommitMetadata} containing the meta data from the latest
      *     commit file.
      */
    public static Option<HoodieCommitMetadata> getCommitMetadataForLatestInstant(
        ...
        HoodieTimeline timeline =
                metaClient
                        ...
                        .filter(
                                instant ->
                                        (metaClient.getTableType() == HoodieTableType.COPY_ON_WRITE
                                                        && instant.getAction().equals(HoodieActiveTimeline.COMMIT_ACTION))
                                                || (metaClient.getTableType() == HoodieTableType.MERGE_ON_READ
                                                        && instant
                                                                .getAction()
                                                                .equals(HoodieActiveTimeline.DELTA_COMMIT_ACTION)));
    ...
    /**
      ...
      */
    public static ControlMessage.ConnectWriteStatus buildWriteStatuses(
            List<WriteStatus> writeStatuses) throws IOException {
        return ControlMessage.ConnectWriteStatus.newBuilder()
                .setSerializedWriteStatus(ByteString.copyFrom(SerializationUtils.serialize(writeStatuses)))
                .build();
    }
    ...
    /**
      ...
      *
      * @param participantInfo The {@link ControlMessage.ParticipantInfo} that contains the underlying
      *     {@link WriteStatus} sent by the participants.
    ...
    public static List<WriteStatus> getWriteStatuses(ControlMessage.ParticipantInfo participantInfo) {
        ...
        return SerializationUtils.deserialize(
                connectWriteStatus.getSerializedWriteStatus().toByteArray());
    }
}
