...
/**
  * Base Hudi Writer that manages reading the raw Kafka records and converting them to {@link
  * HoodieRecord}s that can be written to Hudi by the derived implementations of this class.
  */
        ...
        switch (connectConfigs.getKafkaValueConverter()) {
            ...
            default:
                throw new IOException(
                        "Unsupported Kafka Format type (" + connectConfigs.getKafkaValueConverter() + ")");
        }

        // Tag records with a file ID based on kafka partition and hudi partition.
        HoodieRecord<?> hoodieRecord =
                new HoodieAvroRecord<>(
                        keyGenerator.getKey(avroRecord.get()), new HoodieAvroPayload(avroRecord));
        String fileId =
                KafkaConnectUtils.hashDigest(
