...
/**
  ...
  * based on events from the {@link TransactionCoordinator} and manages the Hudi Writes for a
...
public class ConnectTransactionParticipant implements TransactionParticipant {
    ...
    public ConnectTransactionParticipant(
            KafkaConnectConfigs configs,
    ...
    public ConnectTransactionParticipant(
            ...
            KafkaControlAgent kafkaControlAgent,
            SinkTaskContext context,
            ConnectWriterProvider<WriteStatus> writerProvider)
            throws HoodieException {
            ...
            switch (message.getType()) {
                ...
                default:
                    throw new IllegalStateException(
                            "HudiTransactionParticipant received incorrect state " + message.getType().name());
            }
        }
    ...
    private void handleStartCommit(ControlMessage message) {
        ...
        // but it failed globally since we received another START_COMMIT instead of an END_COMMIT or
        // ACK_COMMIT,
        ...
        LOG.info(
                "Started a new transaction after receiving START_COMMIT for commit " + currentCommitTime);
        try {
            ongoingTransactionInfo =
                    new TransactionInfo<>(currentCommitTime, writerProvider.getWriter(currentCommitTime));
    ...
    private void handleEndCommit(ControlMessage message) {
        if (ongoingTransactionInfo == null) {
            LOG.warn(
                    String.format(
                            "END_COMMIT %s is received while we were NOT in active transaction",
                            message.getCommitTime()));
            return;
        } else if (!ongoingTransactionInfo.getCommitTime().equals(message.getCommitTime())) {
            LOG.error(
                    String.format(
                            "Fatal error received END_COMMIT with commit time %s while local transaction commit time %s",
                            message.getCommitTime(), ongoingTransactionInfo.getCommitTime()));
            // Recovery: A new END_COMMIT from leader caused interruption to an existing
            // transaction,
        ...
        try {
            ...
            ControlMessage writeStatusEvent =
            ...
            LOG.error(
                    String.format(
                            "Error writing records and ending commit %s for partition %s",
                            message.getCommitTime(), partition.partition()),
                    exception);
            ...
            while (!buffer.isEmpty()) {
                try {
                    ...
                    if (record != null
                        ...
                        LOG.warn(
                                String.format(
                                        "Received a kafka record with offset %s above the next expected kafka offset %s for partition %s, "
                        ...
                        LOG.warn(
                                String.format(
                                        "Received a kafka record with offset %s below the next expected kafka offset %s for partition %s, "
                    ...
                    LOG.warn(
                            String.format(
                                    "Error received while writing records for transaction %s in partition %s",
    ...
    private void syncKafkaOffsetWithLeader(ControlMessage message) {
        if (message
            ...
            Long coordinatorCommittedKafkaOffset =
                    message.getCoordinatorInfo().getGlobalKafkaCommitOffsetsMap().get(partition.partition());
            ...
            if (coordinatorCommittedKafkaOffset != null && coordinatorCommittedKafkaOffset >= 0) {
                ...
                if (coordinatorCommittedKafkaOffset != committedKafkaOffset) {
                    LOG.warn(
                            String.format(
                                    "The coordinator offset for kafka partition %s is %d while the locally committed offset is %d, "
                                            + "hence resetting the local committed offset to the coordinator provided one to ensure consistency",
                                    partition, coordinatorCommittedKafkaOffset, committedKafkaOffset));
                }
            ...
            LOG.warn(
                    String.format(
                            "The coordinator offset for kafka partition %s is not present while the locally committed offset is %d, "
                                    + "hence resetting the local committed offset to 0 to avoid data loss",
                            partition, committedKafkaOffset));
        }
        // If the coordinator does not have a committed offset for this partition, reset to zero
        // offset.
