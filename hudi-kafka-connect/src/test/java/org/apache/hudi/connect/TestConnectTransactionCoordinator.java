...
public class TestConnectTransactionCoordinator {
    ...
    public void testSingleCommitScenario(MockParticipant.TestScenarios scenario)
        ...
        TransactionCoordinator coordinator =
                new ConnectTransactionCoordinator(
                        configs,
    ...
    /**
      * A mock Transaction Participant, that exercises all the test scenarios for the coordinator as
      * mentioned in {@link TestScenarios}.
      */
    private static class MockParticipant implements TransactionParticipant {
        ...
        public MockParticipant(
                MockKafkaControlAgent kafkaControlAgent,
        ...
        private void testScenarios(ControlMessage message) {
            ...
            switch (message.getType()) {
                ...
                case END_COMMIT:
                    assertEquals(
                            kafkaOffsetsCommitted, message.getCoordinatorInfo().getGlobalKafkaCommitOffsets());
                    ...
                    switch (testScenario) {
                        case ALL_CONNECT_TASKS_SUCCESS:
                            ...
                            // This commit round should succeed, and the kafka offsets getting
                            // committed
                        ...
                        case SUBSET_WRITE_STATUS_FAILED_BUT_IGNORED:
                            ...
                            numPartitionsThatReportWriteStatus = TOTAL_KAFKA_PARTITIONS;
                            // Despite error records, this commit round should succeed, and the
                            // kafka offsets getting committed
                            kafkaOffsetsCommitted.putAll(kafkaOffsets);
                            expectedMsgType = ControlMessage.EventType.ACK_COMMIT;
                            break;
                        case SUBSET_WRITE_STATUS_FAILED:
                            composeControlEvent(
                                    message.getCommitTime(), true, false, kafkaOffsets, controlEvents);
                            numPartitionsThatReportWriteStatus = TOTAL_KAFKA_PARTITIONS;
                            // This commit round should fail, and a new commit round should start
                            // without kafka offsets getting committed
                            expectedMsgType = ControlMessage.EventType.START_COMMIT;
                            break;
        ...
        private static ControlMessage composeWriteStatusResponse(
                String commitTime,
        ...
        for (int i = 0; i < 1000; i++) {
            if (i % 10 == 0) {
                status.markFailure(
                        ...
                        new Throwable("Error writing record on disk"),
                        Option.empty());
            } else {
                status.markSuccess(mock(HoodieRecord.class), Option.empty());
            }
