...
public class TestAbstractConnectWriter {
                        ...
                        ((List<String>) inputRecords)
                                .stream()
                                        .map(
                                                s -> {
                                                    try {
                                                        return HoodieAvroUtils.rewriteRecord(
                                                                (GenericRecord)
                                                                        reader.read(null, DecoderFactory.get().jsonDecoder(schema, s)),
                                                                schema);
                                        ...
                                        .map(
                                                p ->
                                                        convertToHoodieRecords(
                                                                p, p.get(RECORD_KEY_INDEX).toString(), "000/00/00"))
            ...
            case AVRO:
                ...
                expectedRecords =
                        inputRecords.stream()
                                ...
                                .map(
                                        p -> convertToHoodieRecords(p, p.get(RECORD_KEY_INDEX).toString(), "000/00/00"))
                                .collect(Collectors.toList());
                break;
            default:
                ...
                KafkaConnectConfigs.newBuilder()
                        .withProperties(
                                Collections.singletonMap(
                                        KafkaConnectConfigs.KAFKA_VALUE_CONVERTER, formatConverter))
                        .build();
        AbstractHudiConnectWriterTestWrapper writer =
                new AbstractHudiConnectWriterTestWrapper(configs, keyGenerator, schemaProvider);
    ...
    private static void validateRecords(
            List<HoodieRecord> actualRecords, List<HoodieRecord> expectedRecords) {
    ...
    private static class AbstractHudiConnectWriterTestWrapper extends AbstractConnectWriter {
        ...
        public AbstractHudiConnectWriterTestWrapper(
