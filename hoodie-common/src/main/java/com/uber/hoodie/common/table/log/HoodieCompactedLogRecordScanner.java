 ...
 * log file. NOTE:  If readBlockLazily is turned on, does not merge, instead keeps reading log
 * blocks and merges everything at once This is an optimization to avoid seek() back and forth to
 * read new block (forward seek()) and lazily read content of seen block (reverse and forward
 * seek()) during merge |            | Read Block 1 Metadata |            | Read Block 1 Data | |
 *         | Read Block 2 Metadata |            | Read Block 2 Data | | I/O Pass 1 |
 * ..................... | I/O Pass 2 | ................. | |            | Read Block N Metadata |
 * | Read Block N Data |
 *
...
public class HoodieCompactedLogRecordScanner implements
  ...
  public HoodieCompactedLogRecordScanner(FileSystem fs, String basePath, List<String> logFilePaths,
    ...
    try {
      // Store merged records for all versions for this log file, set the in-memory footprint to
      // maxInMemoryMapSize
   ...
   * with the application specific payload if the same key was found before. Sufficient to just
   * merge the log records since the base data is merged on previous compaction. Finally, merge this
   * log block with the accumulated records
   */
