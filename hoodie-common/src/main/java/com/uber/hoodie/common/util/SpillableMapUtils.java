...
public class SpillableMapUtils {
  ...
  public static <R> R readFromDisk(RandomAccessFile file, Schema schema, String payloadClazz,
    ...
    return (R) convertToHoodieRecordPayload(
        HoodieAvroUtils.bytesToAvro(fileEntry.getValue(), schema),
        payloadClazz);
  }
  ...
  private static DiskBasedMap.FileEntry readInternal(RandomAccessFile file, long valuePosition,
    ...
    if (!(crc == crcOfReadValue)) {
      throw new HoodieCorruptedDataException(
          "checksum of payload written to external disk does not match, "
  ...
  /**
   * Write Value and other metadata necessary to disk. Each entry has the following sequence of
   * data
   *
   * |crc|timestamp|sizeOfKey|SizeOfValue|key|value|
   */
  ...
  public static <R> int computePayloadSize(R value, Schema schema) throws IOException {
    ...
    byte[] val = HoodieAvroUtils
        .avroToBytes((GenericRecord) payload.getData().getInsertValue(schema).get());
    return val.length;
  }
