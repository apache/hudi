...
public class HoodieTableFileSystemViewTest {
  ...
  public void init() throws IOException {
    ...
    fsView = getFileSystemView(metaClient.getActiveTimeline().getCommitTimeline().filterCompletedInstants());
  ...
  protected HoodieTableFileSystemView getFileSystemView(HoodieTimeline timeline, FileStatus[] statuses) {
    return new HoodieTableFileSystemView(metaClient, timeline, statuses);
  }

  protected void refreshFsView(FileStatus[] statuses) {
   ...
   * @param includeIncludeInvalidAndInflight Whether view includes inflight and invalid file-groups.
   * @throws Exception
   */
  protected void testViewForFileSlicesWithAsyncCompaction(boolean skipCreatingDataFile,
      ...
      boolean includeIncludeInvalidAndInflight) throws Exception {
    String partitionPath = "2016/05/01";
    new File(basePath + "/" + partitionPath).mkdirs();
    String fileId = UUID.randomUUID().toString();
    ...
    refreshFsView(null);
    List<FileSlice> slices = fsView.getLatestFileSlices(partitionPath).collect(Collectors.toList());
    ...
    if (includeIncludeInvalidAndInflight) {
      ...
      assertEquals("Expect both inflight and orphan file-slice to be included",
          includeIncludeInvalidAndInflight ? 5 : 1, fileSliceList.size());
