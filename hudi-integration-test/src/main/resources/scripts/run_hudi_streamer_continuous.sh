...
for ((i = 0; ; i++)); do
  ...
  $SPARK_HOME/bin/spark-submit \
      ...
      --op $OPERATION_TYPE \
      --continuous >> $TEST_LOG 2>&1 &
  pid=$!

  kill_streamer() {
    echo "Caught Ctrl+C. Terminating Hudi Streamer with PID $pid..."
    kill "$pid"  # Kill the process
    wait "$pid"  # Wait for the process to fully terminate
    echo "Process terminated."
    exit 0
  }

  trap kill_streamer SIGINT

  # kill continuously running Hudi streamer after time limit
  while true; do
    ...
    if ps -p "$pid" > /dev/null 2>&1; then
      ...
      if (( elapsed_time >= SECONDS_TO_WAIT )); then
        echo "The specified time of $MINUTES minutes has passed. Killing the Hudi streamer."
        kill "$pid"
        wait "$pid"
        break
      fi
    else
      echo "Hudi Streamer with PID $PID has stopped."
      break
    fi
  done
