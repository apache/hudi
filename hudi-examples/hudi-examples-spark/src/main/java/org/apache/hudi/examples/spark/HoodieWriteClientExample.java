...
/**
  ...
  *
  * <p>To run this example, you should
  *
  * <pre>
  ...
  * </pre>
  *
  * Usage: HoodieWriteClientExample <tablePath> <tableName> <tablePath> and <tableName> describe root
  * path of hudi and table name for example, `HoodieWriteClientExample
  * file:///tmp/hoodie/sample-table hoodie_rt`
  */
        ...
        try (JavaSparkContext jsc = new JavaSparkContext(sparkConf)) {
            ...
            HoodieWriteConfig cfg =
                    HoodieWriteConfig.newBuilder()
                            ...
                            .withArchivalConfig(
                                    HoodieArchivalConfig.newBuilder().archiveCommitsWith(20, 30).build())
                            .build();
            try (SparkRDDWriteClient<HoodieAvroPayload> client =
                ...
                List<String> partitionList =
                        toBeDeleted.stream()
                                .map(s -> s.getPartitionPath())
                                .distinct()
                                .collect(Collectors.toList());
                List<String> deleteList =
                        recordsSoFar.stream()
                                .filter(f -> !partitionList.contains(f.getPartitionPath()))
                                .map(m -> m.getKey().getPartitionPath())
                                .distinct()
                                .collect(Collectors.toList());
                ...
                if (HoodieTableType.valueOf(tableType) == HoodieTableType.MERGE_ON_READ) {
                    ...
                    client.commitCompaction(
                            instant.get(), compactionMetadata.getCommitMetadata().get(), Option.empty());
                }
            }
        }
    }
