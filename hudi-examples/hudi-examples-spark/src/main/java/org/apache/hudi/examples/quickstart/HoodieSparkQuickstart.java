...
public final class HoodieSparkQuickstart {
    ...
    /** Visible for testing */
    public static void runQuickstart(
            JavaSparkContext jsc, SparkSession spark, String tableName, String tablePath) {
        final HoodieExampleDataGenerator<HoodieAvroPayload> dataGen =
        ...
        Dataset<Row> withoutThirdPartitionDf =
                snapshotBeforeOverwrite.filter(
                        "partitionpath != '" + HoodieExampleDataGenerator.DEFAULT_THIRD_PARTITION_PATH + "'");
        ...
        assert snapshotAfterDeleteByPartition
                        .intersect(
                                snapshotBeforeDeleteByPartition.filter(
                                        "partitionpath == '"
        ...
        assert snapshotAfterDeleteByPartition.count()
                == snapshotBeforeDeleteByPartition
                        .filter(
                                "partitionpath != '"
                                        + HoodieExampleDataGenerator.DEFAULT_FIRST_PARTITION_PATH
                                        + "'")
    ...
    /**
      * Generate some new trips, load them into a DataFrame and write the DataFrame into the Hudi
      * dataset as below.
      */
    public static Dataset<Row> insertData(
            SparkSession spark,
    ...
    /**
      * Generate new records, load them into a {@link Dataset} and insert-overwrite it into the Hudi
      * dataset
      */
    public static Dataset<Row> insertOverwriteData(
            SparkSession spark,
            JavaSparkContext jsc,
            String tablePath,
            String tableName,
        ...
        String commitTime = Long.toString(System.currentTimeMillis());
        List<String> inserts =
                dataGen.convertToStringList(
                        dataGen.generateInsertsOnPartition(
                                commitTime, 20, HoodieExampleDataGenerator.DEFAULT_THIRD_PARTITION_PATH));
        Dataset<Row> df = spark.read().json(jsc.parallelize(inserts, 1));
    ...
    public static void queryData(
        ...
        //
        // +-------------------+--------------------+----------------------+-------------------+--------------------+------------------+
        //  |     20191231181501|31cafb9f-0196-4b1...|
        // 2020/01/02|rider-1577787297889|driver-1577787297889| 98.88075495133515|
        //  ...
    }

    /**
      * This is similar to inserting new data. Generate updates to existing trips using the data
      * generator, load into a DataFrame and write DataFrame into the hudi dataset.
      */
    public static Dataset<Row> updateData(
            SparkSession spark,
            JavaSparkContext jsc,
            String tablePath,
            String tableName,
    ...
    public static Dataset<Row> delete(SparkSession spark, String tablePath, String tableName) {
        ...
        Dataset<Row> roViewDF = spark.read().format("hudi").load(tablePath + "/*/*/*/*");
        roViewDF.createOrReplaceTempView("hudi_ro_table");
        Dataset<Row> toBeDeletedDf =
                spark.sql(
                        "SELECT begin_lat, begin_lon, driver, end_lat, end_lon, fare, partitionpath, rider, ts, uuid FROM hudi_ro_table limit 2");
    ...
    /**
      * Hudi also provides capability to obtain a stream of records that changed since given commit
      * timestamp. This can be achieved using Hudi’s incremental view and providing a begin time from
      * which changes need to be streamed. We do not need to specify endTime, if we want all changes
      * after the given commit (as is the common case).
      */
    public static void incrementalQuery(SparkSession spark, String tablePath, String tableName) {
        List<String> commits =
                spark
                        .sql(
                                "select distinct(_hoodie_commit_time) as commitTime from hudi_ro_table order by commitTime")
                        .toJavaRDD()
        ...
        spark
                .sql(
                        "select `_hoodie_commit_time`, fare, begin_lon, begin_lat, ts from hudi_incr_table where fare > 20.0")
                .show();
    }

    /**
      * Lets look at how to query data as of a specific time. The specific time can be represented by
      * pointing endTime to a specific commit time and beginTime to “000” (denoting earliest possible
      * commit time).
      */
