...
/**
  * This Flink program serves as a demonstration of inserting, updating, and deleting records in a
  * Hudi table using the DataStream API. The program inserts four messages for ten batches. Two of
  * the messages generate a random UUID, acting as new insert records, while two records reuse the
  * same record keys, resulting in an update for those two records in each batch. In the first batch,
  * four new records are inserted into a newly created Hudi table. Subsequently, after each batch,
  * two new records are inserted, leading to an increment in the count by two with each batch. In the
  * 11th batch, we illustrate the delete operation by publishing a record with the delete row kind.
  * As a result, we observe the deletion of the third ID after this batch.
  *
  * <p>After this code finishes you should see total 21 records in hudi table.
  */
public class HudiDataStreamWriter {

    public static DataType ROW_DATA_TYPE =
            DataTypes.ROW(
    ...
    /**
      * Main Entry point takes two parameters. It can be run with Flink cli. Sample Command - bin/flink
      * run -c com.hudi.flink.quickstart.HudiDataStreamWriter
      * ${HUDI_FLINK_QUICKSTART_REPO}/target/hudi-examples-0.1.jar hudi_table "file:///tmp/hudi_table"
      *
      * @param args
      * @throws Exception
      */
    public static void main(String[] args) throws Exception {
        ...
        HoodiePipeline.Builder builder =
                createHudiPipeline("hudi_table", createHudiOptions(targetS3Path));
    ...
    /**
      ...
      */
    private static HoodiePipeline.Builder createHudiPipeline(
            String targetTable, Map<String, String> options) {
    ...
    static class SampleDataSource implements SourceFunction<RowData> {
        ...
        public void run(SourceContext<RowData> ctx) throws Exception {
            ...
            while (isRunning) {
                ...
                if (batchNum < 11) {
                    // For first 10 batches, inserting 4 records. 2 with random id (INSERTS) and 2
                    // with hardcoded UUID(UPDATE)
