...
public class HoodieExampleDataGenerator<T extends HoodieRecordPayload<T>> {
    ...
    public T generateRandomValue(HoodieKey key, String commitTime) {
        GenericRecord rec =
                generateGenericRecord(key.getRecordKey(), "rider-" + commitTime, "driver-" + commitTime, 0);
    ...
    /**
      * Generates new inserts, across a single partition path. It also updates the list of existing
      * keys.
      */
        ...
        try {
            String str =
                    HoodieAvroUtils.bytesToAvro(
                                    ((HoodieAvroPayload) record.getData()).getRecordBytes(), avroSchema)
                            .toString();
            str = "{" + str.substring(str.indexOf("\"ts\":"));
            return Option.of(
                    str.replaceAll("}", ", \"partitionpath\": \"" + record.getPartitionPath() + "\"}"));
        } catch (IOException e) {
            return Option.empty();
        }
    }
