...
public final class HoodieSparkQuickstartSlim {
  ...
  public static void runQuickstart(JavaSparkContext jsc, SparkSession spark, String tableName, String tablePath) {
    final HoodieExampleDataGeneratorSlim<HoodieAvroPayload> dataGen = new HoodieExampleDataGeneratorSlim<>();
    ...
    assertTrue(insertDf.except(spark.sql(snapshotQuery)).count() == 0);
    ...
    assertTrue(snapshotAfterDelete.intersect(deleteDf).count() == 0);
    assertTrue(snapshotBeforeDelete.except(deleteDf).except(snapshotAfterDelete).count() == 0);
    ...
    queryData(spark, jsc, tablePath, tableName, dataGen);
    Dataset<Row> withoutThirdPartitionDf = snapshotBeforeOverwrite.filter("partitionpath != '" + HoodieExampleDataGeneratorSlim.DEFAULT_THIRD_PARTITION_PATH + "'");
    ...
    assertTrue(snapshotAfterOverwrite.except(expectedDf).count() == 0);
    ...
    assertTrue(snapshotAfterDeleteByPartition.intersect(snapshotBeforeDeleteByPartition.filter("partitionpath == '" + HoodieExampleDataGeneratorSlim.DEFAULT_FIRST_PARTITION_PATH + "'")).count() == 0);
    assertTrue(snapshotAfterDeleteByPartition.count() == snapshotBeforeDeleteByPartition.filter("partitionpath != '" + HoodieExampleDataGeneratorSlim.DEFAULT_FIRST_PARTITION_PATH + "'").count());
  }
  ...
  public static Dataset<Row> insertData(SparkSession spark, JavaSparkContext jsc, String tablePath, String tableName,
                                        HoodieExampleDataGeneratorSlim<HoodieAvroPayload> dataGen) {
  ...
  public static Dataset<Row> insertOverwriteData(SparkSession spark, JavaSparkContext jsc, String tablePath, String tableName,
                                                 HoodieExampleDataGeneratorSlim<HoodieAvroPayload> dataGen) {
    String commitTime = Long.toString(System.currentTimeMillis());
    List<String> inserts = dataGen.convertToStringList(dataGen.generateInsertsOnPartition(commitTime, 20, HoodieExampleDataGeneratorSlim.DEFAULT_THIRD_PARTITION_PATH));
    Dataset<Row> df = spark.read().json(jsc.parallelize(inserts, 1));
  ...
  public static void deleteByPartition(SparkSession spark, String tablePath, String tableName) {
    ...
    df.write().format("org.apache.hudi")
        ...
        .option("hoodie.datasource.write.partitions.to.delete", HoodieExampleDataGeneratorSlim.DEFAULT_FIRST_PARTITION_PATH)
        .mode(Append)
        .save(tablePath);
  }
  ...
  public  static void assertTrue(Boolean b) throws HoodieException {
    if (!b) {
      throw new HoodieException("Exception in HoodieSparkQuickstartSlim");
    }
  }
}
